...
# Editable-install the admin package so evolve_admin imports resolve:
cd packages/admin && pip install -e . && cd ../..
```
...
|------|----------------|-------------|
| **Public-launch scrub guard** | **Reserved real names** (bot/person names) in tracked files. Use placeholders — see **[docs/PLACEHOLDER_NAMING.md](docs/PLACEHOLDER_NAMING.md)**. This *will* catch you if you paste a real bot name. | `cd packages/admin && python3 -m pytest tests/test_public_launch_scrub.py` |
...
- **`docs/spec-*.md`** — design specs for in-flight subsystems (the RSI architecture, the Effectiveness Layer, the signal store, model rungs/roles).
- **[docs/roadmap-80-to-100-2026-06-09.md](docs/roadmap-80-to-100-2026-06-09.md)** — current hardening roadmap and what's left.
