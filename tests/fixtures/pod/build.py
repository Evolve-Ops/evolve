...
# ── OpenClaw's cron store ───────────────────────────────────────────────────
#
# A real operator's schedules live here, not in the user crontab.  Both
# backends are represented across the three bots because both exist in the
# wild: OpenClaw < 2026.7 keeps jobs in ``cron/jobs.json``; ≥ 2026.7 imports
# that file once into SQLite (``state/openclaw.sqlite``, table ``cron_jobs``)
# and renames the seed ``jobs.json.migrated``.  Payload kinds are OpenClaw's
# own — ``command`` (argv), ``agentTurn`` (message), ``systemEvent`` (text);
# anything else the gateway quarantines silently.
...
def _oc_cron_job(
    ...
    *,
    ...
    """One OpenClaw cron job, in the gateway's own wire shape."""
    return {
        "id": job_id,
        "name": name,
        "enabled": enabled,
...
def _oc_shell_payload(script: Path, *args: str) -> dict:
    interpreter = "python3" if script.suffix == ".py" else "bash"
    return {"kind": "command", "argv": [interpreter, str(script), *args]}


#: The columns our readers touch, named as the live 2026.7 table names them
#: (verified against a running pod).  The real table has ~50 columns, all of
#: them derived from ``job_json`` — which is the one this fixture must get
#: right, because it is the column every reader actually parses.
_CRON_JOBS_DDL = """
...
def _write_oc_cron_json(home: Path, jobs: list[dict]) -> Path:
    """Pre-2026.7 pod: the jobs live in ``cron/jobs.json``."""
    return _wj(home / ".openclaw" / "cron" / "jobs.json", {"jobs": jobs})


def _write_oc_cron_sqlite(home: Path, jobs: list[dict]) -> Path:
    """≥2026.7 pod: the gateway ingested the seed, so the jobs live in SQLite
    and ``cron/jobs.json`` is left behind renamed ``jobs.json.migrated``."""
    seed = home / ".openclaw" / "cron" / "jobs.json.migrated"
    _wj(seed, {"jobs": jobs})
    ...
    try:
        ...
        for order, job in enumerate(jobs):
            ...
            conn.execute(
                "INSERT INTO cron_jobs (store_key, job_id, name, enabled, "
                "created_at_ms, schedule_kind, schedule_expr, schedule_tz, "
...
def _build_personal_bot(home: Path, today: date) -> None:
    ...
    # This bot is on a pre-2026.7 OpenClaw, so the store is still the file.
    _write_oc_cron_json(
        home,
        [
            _oc_cron_job(
                "8f1c1d20-3a44-4b21-9a11-2b5c0d7e1101",
                "morning-brief",
                "45 6 * * *",
                _oc_shell_payload(ws / "morning-brief" / "brief.py"),
            ),
            _oc_cron_job(
                "8f1c1d20-3a44-4b21-9a11-2b5c0d7e1102",
                "evening-meds",
                "0 20 * * *",
                _oc_shell_payload(ws / "meds" / "reminder.py"),
            ),
            _oc_cron_job(
                "8f1c1d20-3a44-4b21-9a11-2b5c0d7e1103",
                "journal-prompt",
                "30 21 * * *",
                {
                    "kind": "agentTurn",
                    "message": (
                        "Run python3 "
                        f"{ws / 'journal' / 'prompt.py'} and send me tonight's "
                        "journal prompt."
                    ),
                },
            ),
            # Switched off months ago. A job that cannot fire is evidence of
            # nothing, and the scanner must not count it.
            _oc_cron_job(
                "8f1c1d20-3a44-4b21-9a11-2b5c0d7e1104",
                "meds-weekly-recap",
                "0 18 * * 0",
                _oc_shell_payload(ws / "meds" / "reminder.py", "--recap"),
                enabled=False,
            ),
        ],
    )
...
def _build_team_bot(home: Path, today: date) -> None:
    ...
    _write_oc_cron_json(
        ...
        [
            _oc_cron_job(
                "b2e4a7c8-51f0-42d6-8f2a-7c9d3e5a2201",
                "standup-digest",
                "45 9 * * 1-5",
                _oc_shell_payload(ws / "standup" / "digest.py"),
            ),
            _oc_cron_job(
                "b2e4a7c8-51f0-42d6-8f2a-7c9d3e5a2202",
                "oncall-roll",
                "0 9 * * 1",
                {
                    "kind": "agentTurn",
                    "message": (
                        f"Run python3 {ws / 'oncall' / 'rota.py'} and post who "
                        "is on call this week."
                    ),
                },
            ),
            # Platform machinery, scheduled the same way. Discovery has to
            # keep filtering it out no matter which store it comes from.
            _oc_cron_job(
                "b2e4a7c8-51f0-42d6-8f2a-7c9d3e5a2203",
                "liveness-ping",
                "*/5 * * * *",
                _oc_shell_payload(ws / "bin" / "sentry_ping.sh"),
            ),
        ],
    )
...
def _build_ops_bot(home: Path, today: date) -> None:
    ...
    # This bot is on OpenClaw ≥2026.7: the gateway imported the seed and the
    # jobs now live in state/openclaw.sqlite. cron/jobs.json is not there to
    # read — it is jobs.json.migrated.
    _write_oc_cron_sqlite(
        home,
        [
            _oc_cron_job(
                "d7a0b912-6c3e-4d18-b0e5-1f8a4c6d3301",
                "release-notes",
                "0 16 * * 5",
                _oc_shell_payload(ws / "release-notes" / "collect.py"),
            ),
            _oc_cron_job(
                "d7a0b912-6c3e-4d18-b0e5-1f8a4c6d3302",
                "repo-watch",
                "*/30 * * * *",
                {
                    "kind": "agentTurn",
                    "message": (
                        f"Run python3 {ws / 'repo-watch' / 'watch.py'} and tell "
                        "me about any tracked repo whose default branch is red."
                    ),
                },
            ),
            _oc_cron_job(
                "d7a0b912-6c3e-4d18-b0e5-1f8a4c6d3303",
                "gateway-watch",
                "*/10 * * * *",
                {
                    "kind": "systemEvent",
                    "text": "Probe the openclaw gateway and restart it if the "
                            "health endpoint does not answer.",
                },
            ),
        ],
    )
