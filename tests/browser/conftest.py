...
@pytest.fixture(scope="session")
def shared_dir(_admin_server_state: _AdminServerState) -> Path:
    """The pod-wide ``sharedDir`` the running server reads.

    Session-scoped like the server itself. Tests that need on-disk pod state
    to render against (the Pod Intelligence weeks, for one) write it here;
    the routes read from disk on every request, so a write plus a page load
    is enough — no restart.
    """
    return Path(
        json.loads(_admin_server_state.network_path.read_text())["sharedDir"]
    )
