...
**curated snapshot** of it: a fresh, scrub-gated copy of the tree published as
a **single squashed commit** that replaces the previous snapshot. There is **no
persistent public history** — the public repo always contains exactly one
commit, and each new snapshot force-replaces it by design.

The snapshot is **refreshed automatically** whenever the private `main` branch
advances in a way that changes the public tree, so it usually tracks upstream
development within minutes rather than waiting for a release. Every refresh
runs the same gates: a path denylist, a reserved-token scrub of the output
tree, a second scrub of the built artifact, and a post-publish verification of
the pushed result. A snapshot that fails any of them is not published.
...
- **External PRs cannot be merged directly.** A PR opened here targets a
  snapshot that will be replaced wholesale at the next refresh, so GitHub's
...
- **Don't build on top of public history.** Because each refresh replaces the
  single public commit — and refreshes are automatic, so they can land several
  times a day — long-lived forks will not fast-forward. Rebase your fork onto
  each new snapshot instead.
