...
def main(argv: list[str] | None = None) -> int:
    ...
    from evolve_admin.board_worker import _self_check_rung_gate, run_loop

    try:
        _self_check_rung_gate()
    except AssertionError as exc:
        # hold-fix-4279-approval-rung-and-busy-expiry: refuse to start rather
        # than install a daemon that could act on the world without the
        # approval it promises.
        logger.error("board-worker: refusing to start — %s", exc)
        return 2
