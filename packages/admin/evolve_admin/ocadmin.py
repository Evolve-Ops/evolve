...
def oc_preflight_cmd(
    ...
    Two rows are red and block an upgrade: model refs `doctor` would rewrite
    (Evolve owns model routing; a rewrite changes which model answers), and an
    agent workspace outside its bot's home (fatal from 2026.9 on).
