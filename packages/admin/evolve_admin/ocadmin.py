...
def _emit_runtime_notes_review_signal(
    ...
    and may be stale once upstream OC ships a fix. The current version's
    reminder is operator-acked (no auto-resolve; TTL-exempt in
    signals.stale_ttl), but a NEW version's reminder supersedes any
    still-active reminder for an older version — walking the file once
    against the newest version covers everything the older reminders
    asked for. No-op when old == new.
    """
    ...
    try:
        ...
        # The newest reminder supersedes any still-active reminder for an
        # older version (signature is per-new_version, so old upgrades
        # otherwise leave immortal operator-acked signals behind — the
        # mini had reminders for two versions firing at once, 2026-08-30
        # alerts-noise review Track 1.4).
        store.sweep_resolve(
            shared_dir,
            producer=RUNTIME_NOTES_REMINDER_PRODUCER,
            kept_signatures={signature},
            types={RUNTIME_NOTES_REMINDER_TYPE},
            reason=(
                f"superseded: RUNTIME_NOTES review reminder for {new_version} "
                "replaces this older version's reminder"
            ),
        )
    except Exception as e:
        console.print(
...
# ── Versioned runtime store (chip: oc-runtime-versioned-per-bot) ─────────────


def _oc_rt():
    from . import oc_runtime

    return oc_runtime


def _load_registry_strict(ctx: click.Context) -> tuple[Path, dict | None, str | None]:
    """``network.json`` exactly as on disk, or ``(path, None, why)``.

    Never ``load_network``'s default and never ``or {}``: an absent or
    unreadable registry is "cannot tell who runs what", and the verbs below
    decide deletions and restarts from it.
    """
    network_path: Path = ctx.obj.get("network_path", DEFAULT_NETWORK_CONFIG)
    try:
        return network_path, json.loads(Path(network_path).read_text()), None
    except (OSError, ValueError) as exc:
        return network_path, None, f"cannot read {network_path}: {exc}"


def _gateway_health(port: int) -> tuple[bool, str]:
    from .wizard_verify import _probe_gateway

    return _probe_gateway(port)


@menu_group.command("install")
@click.argument("version")
@click.pass_context
def oc_install(ctx: click.Context, version: str) -> None:
    """Install one exact OpenClaw VERSION into Evolve's own prefix.

    Never linked into /opt/homebrew/bin and never touches another version —
    installing beside is the whole point. Idempotent.
    """
    rt = _oc_rt()
    try:
        dest = rt.install_version(version)
    except rt.OcRuntimeError as exc:
        console.print(f"[red]❌ {exc}[/]")
        sys.exit(1)
    _ok, detail = rt.verify_version(version)
    console.print(f"[green]✅ OpenClaw {version} installed at {dest} — runs: {detail}[/]")
    console.print(f"   Pin a bot to it with: [bold]evolve-admin oc pin <bot> {version}[/]")


@menu_group.command("list")
@click.option("--json", "as_json", is_flag=True, default=False)
@click.pass_context
def oc_list(ctx: click.Context, as_json: bool) -> None:
    """Installed OpenClaw versions, who pins each, and which are removable."""
    rt = _oc_rt()
    _path, network, why = _load_registry_strict(ctx)
    rows = rt.describe_store(network)
    unpinned = rt.unpinned_bots(network)
    unknown = rt.pin_unknowns(network) if why is None else [why]
    if as_json:
        print(json.dumps({"versions": rows, "unpinned_bots": unpinned,
                          "unknown": unknown}, indent=2))
        return
    if not rows:
        console.print(
            f"  No versions in {rt.store_root()} yet. "
            f"Run [bold]evolve-admin oc adopt[/] to register the installed one."
        )
    for row in rows:
        holders = ", ".join(row["pinned_by"]) or "—"
        flag = {True: "", False: "  (in use)", None: "  (cannot tell who runs it)"}[row["removable"]]
        console.print(f"  [bold]{row['version']}[/]  pinned by: {holders}{flag}")
    if unpinned:
        console.print(
            f"\n  [yellow]{len(unpinned)} bot(s) are not pinned[/] "
            f"({', '.join(unpinned)}) — they run whatever the global binary "
            f"resolves to, and have no rollback."
        )
    for reason in unknown:
        console.print(f"  [red]unknown pin — {reason}[/]")


@menu_group.command("remove")
@click.argument("version")
@click.pass_context
def oc_remove(ctx: click.Context, version: str) -> None:
    """Delete an installed VERSION. Refuses unless it can show no bot runs it."""
    rt = _oc_rt()
    _path, network, why = _load_registry_strict(ctx)
    try:
        if network is None:
            raise rt.OcRuntimeError(
                f"refusing to remove OpenClaw {version}: {why} — cannot tell "
                f"which bots run it"
            )
        dest = rt.remove_version(version, network)
    except rt.OcRuntimeError as exc:
        console.print(f"[red]❌ {exc}[/]")
        sys.exit(1)
...
@menu_group.command("pin")
@click.argument("bot_id")
@click.argument("version")
@click.option("--dry-run", is_flag=True, default=False,
              help="Print the ordered plan without performing it.")
@click.option("--override", is_flag=True, default=False,
              help="Pin a version Evolve has not validated. Recorded in the audit log.")
@click.pass_context
def oc_pin(ctx: click.Context, bot_id: str, version: str, dry_run: bool,
           override: bool) -> None:
    """Point BOT_ID at an installed VERSION and restart that one gateway.

    This is also the rollback: `oc pin <bot> <previous>` runs the identical
    steps backwards, which is what makes it trustworthy on a bad day. Runs
    exactly the steps it prints, in that order, and says "restarted on <v>"
    only when a probe saw the gateway healthy and reporting <v>.
    """
    from .config import save_network
    from .deploy import get_bot_port, get_bot_user, install_bot_gateway_plist

    rt = _oc_rt()
    network_path, network, why = _load_registry_strict(ctx)
    try:
        if network is None:
            raise rt.OcRuntimeError(f"refusing to pin {bot_id}: {why}")
        plan = rt.plan_pin(bot_id, version, network, override=override)
    except rt.OcRuntimeError as exc:
        console.print(f"[red]❌ {exc}[/]")
        sys.exit(1)

    verb = "Rolling back" if plan.is_rollback else "Pinning"
    console.print(
        f"\n  {verb} [bold]{bot_id}[/]: "
        f"{plan.previous or 'unpinned'} → [bold]{version}[/]  ({plan.validation}"
        f"{', OVERRIDE' if plan.override else ''})"
    )
    for i, step in enumerate(plan.steps, 1):
        console.print(f"    {i}. {step.kind}: {step.detail}")
    if dry_run:
        console.print("\n  [yellow]--dry-run: nothing was changed.[/]")
        return

    updated = rt.set_pin(bot_id, version, network)
    port, user = get_bot_port(bot_id, updated), get_bot_user(bot_id, updated)
    if not port:
        console.print(f"[red]❌ no gateway port for {bot_id}; nothing was changed[/]")
        sys.exit(1)
    try:
        rt.record_pin(plan, actor=os.environ.get("SUDO_USER") or os.environ.get("USER") or "?")
    except (rt.OcRuntimeError, OSError) as exc:
        console.print(f"[red]❌ cannot write the pin audit line ({exc}); nothing was changed[/]")
        sys.exit(1)

    def _execute(kind: str) -> tuple[bool, str]:
        """One plan step. The loop below runs exactly ``plan.steps`` through
        this, so the printed plan and the executed one cannot drift apart."""
        if kind == "registry":
            save_network(updated, network_path)
            return True, f"{bot_id} pinned to {version}"
        if kind == "restart":
            return install_bot_gateway_plist(bot_id, port, user=user, network=updated)
        if kind == "probe":
            return rt.probe_gateway(version, cli=rt.bot_openclaw_bin(bot_id, updated) or "",
                                    user=user, health=lambda: _gateway_health(port))
        return False, f"no executor for plan step {kind!r}"

    for i, step in enumerate(plan.steps, 1):
        ok, detail = _execute(step.kind)
        if ok:
            console.print(f"  ✅ {i}. {step.kind}: {detail}")
            continue
        if step.kind == "probe":
            console.print(f"[red]❌ {bot_id}: restart outcome unknown — {detail}[/]")
        else:
            console.print(f"[red]❌ {step.kind} failed: {detail}[/]")
            console.print(
                "   The registry may name the new version before the gateway "
                "runs it. Re-run this command once the cause is fixed."
            )
        sys.exit(1)
    console.print(f"  [green]✅ {bot_id} restarted on {version}[/]")
    console.print(
        "  Channel packages are not touched by a pin — across a release line, "
        "check this bot's channels before pinning the next (runbook §8)."
    )


@menu_group.command("adopt")
@click.option("--dry-run", is_flag=True, default=False,
              help="Print the plan without copying, pinning or restarting.")
@click.pass_context
def oc_adopt(ctx: click.Context, dry_run: bool) -> None:
    """Copy the currently-installed OpenClaw into the store as its first
    version, then print the per-bot pins to run.

    The global install is COPIED, never moved: Homebrew keeps working exactly
    as it does now. An adopt that moved it would be the same irreversible
    fleet-wide switch this whole design exists to abolish.
    """
    rt = _oc_rt()
    _path, network, why = _load_registry_strict(ctx)
    installed = _installed_version()
    try:
        if network is None:
            raise rt.OcRuntimeError(f"refusing to adopt: {why}")
        source = rt.global_prefix(_get_profile().name)
        plan = rt.plan_adopt(network, installed, source_prefix=source)
        if plan.copy_needed and plan.source is None:
            raise rt.OcRuntimeError("no global OpenClaw install found to copy")
    except rt.OcRuntimeError as exc:
        console.print(
            f"[red]❌ {exc}[/]" if why or rt.valid_version(installed) else
            f"[red]❌ Cannot determine the installed OpenClaw version ({installed}) — "
            f"refusing to adopt a version Evolve cannot name.[/]"
        )
        sys.exit(1)

    console.print(f"\n  Adopting OpenClaw [bold]{plan.version}[/]")
    console.print(
        f"    copy into the store: from {plan.source}" if plan.copy_needed
        else "    copy into the store: already present"
    )
    console.print(f"    pin + restart, one at a time: {', '.join(plan.restart_order) or '—'}")
    if plan.already_pinned:
        console.print(f"    already pinned, untouched: {', '.join(plan.already_pinned)}")
    if dry_run:
        console.print("\n  [yellow]--dry-run: nothing was changed.[/]")
        return
    if plan.copy_needed and plan.source is not None:  # a None source was refused above
        try:
            dest = rt.adopt_copy(plan.source, plan.version)
        except rt.OcRuntimeError as exc:
            console.print(f"[red]❌ copy failed, nothing kept: {exc}[/]")
            sys.exit(1)
        console.print(f"  [green]✅ copied into the store and verified: {dest}[/]")
    validated, detail = rt.validation_verdict(plan.version)
    flag = "" if validated else " --override"
    if not validated:
        console.print(
            f"\n  OpenClaw {plan.version} is {detail}, so each pin needs "
            f"--override — the pin audit log records it."
        )
    console.print(
        "\n  [yellow]Run the per-bot pins yourself, one at a time, watching each:[/]"
    )
    for bot in plan.restart_order:
        console.print(f"    sudo evolve-admin oc pin {bot} {plan.version}{flag}")
    console.print(
        "\n  Adopt deliberately does not restart nine gateways for you. "
        "One bot at a time, with a human watching, is the entire lesson of "
        "2026-09-07."
    )


@menu_group.command("preflight")
@click.option("--target", required=True,
              help="Target OpenClaw version to preflight against (e.g. 2026.9.4).")
@click.option("--json", "as_json", is_flag=True, default=False,
              help="Emit the report JSON instead of the table.")
@click.option("--no-fetch", is_flag=True, default=False,
              help="Skip fetching the target; run only the config-only rows.")
@click.pass_context
def oc_preflight_cmd(
    ctx: click.Context, target: str, as_json: bool, no_fetch: bool,
) -> None:
    """Per-bot preflight against a target OpenClaw version. Read-only.

    Fetches the target to a scratch prefix — it is NOT installed and nothing is
    linked — then runs that version's own `config validate` and `doctor --json`
    against each bot's config, as that bot, as a dry run. `doctor --fix` is
    never invoked.

    Blocks on: model refs `doctor` would rewrite (Evolve owns model routing;
    a rewrite changes which model answers), an agent workspace outside its
    bot's home (fatal from 2026.9 on), or a row this could not fully check
    (failed fetch, unreadable config, an unrecognized `doctor` schema) — the
    last fails CLOSED rather than reading "could not look" as clean (D-CS7).

    Exit codes: 0 = nothing blocking, 1 = blocking rows, 2 = error.
    """
    import getpass
    import json as _json

    from . import oc_preflight as _pf
    ...
    network = _load_network_safe(network_path)
    if not network:
        console.print("[red]❌ Could not read the network config[/]")
        sys.exit(2)

    if not as_json and not no_fetch:
        console.print(f"  Fetching OpenClaw {target} to a scratch prefix…")
    try:
        report = _pf.preflight(network, target, fetch=not no_fetch)
    except Exception as exc:  # noqa: BLE001 — a preflight crash blocks nothing
        console.print(f"[red]❌ Preflight failed: {exc}[/]")
        sys.exit(2)

    # Audit record (item 4): every run is persisted, CLI or web, so "who ran
    # this against what pair, and when" is answerable after the fact.
    shared_dir = Path(network.get("sharedDir") or DEFAULT_SHARED_DIR)
    try:
        _pfs.persist_report(
            report, actor=f"cli:{getpass.getuser()}",
            installed_version=_uv.installed_package_version(),
            shared_dir=shared_dir,
        )
    except Exception as exc:  # noqa: BLE001 — the audit write must never block the report
        console.print(f"[yellow]⚠ could not persist the audit record: {exc}[/]")

    if as_json:
        print(_json.dumps(report.to_json(), indent=2))
    else:
        console.print(_pf.render_table(report))
