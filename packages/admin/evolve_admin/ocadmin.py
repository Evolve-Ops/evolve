...
def oc_upgrade(
    ...
    console.print("  Checking npm registry…", end="")
    latest = _latest_version()
    console.print(f"\r  Latest:     {latest}      ")

    if latest.startswith("(error"):
        console.print(f"  [red]❌ Cannot reach npm registry: {latest}[/]")
        sys.exit(1)

    if installed == latest and not force:
        console.print(f"  [green]✅ Already on latest ({installed}). Nothing to do.[/]")
    ...
    try:
        ...
        console.print(f"  [red]❌ preflight raised: {e}[/]")
        if not force:
            console.print(
            ...
            sys.exit(1)
        console.print("  [yellow]⚠️  --force: proceeding despite preflight error[/]")
        report = None
    ...
    dance_plan = _compute_neutralize_plan(report) if neutralize_externalized else {}
    phantom_cleanup = _compute_phantom_cleanup_plan(report) if neutralize_externalized else {}

    if report is not None and not report.ok:
        blockers = [r for r in report.requirements if r.blocking]
        console.print(f"  [red]❌ preflight blockers: {len(blockers)}[/]")
        for i, r in enumerate(blockers, 1):
            console.print(f"    {i}. [{r.source_gate}] {r.summary}")
            console.print(f"       [yellow]→[/] {r.remediation}")
        ...
        if only_externalized_blockers:
            ...
            console.print("  [yellow]⚠️  --force: proceeding despite blockers[/]")
    elif report is not None:
        console.print("  [green]✅ preflight: all gates passed[/]")

    if dance_plan or phantom_cleanup:
        _print_dance_preview(dance_plan, phantom_cleanup, network)
        if dry_run:
    ...
    if phantom_cleanup:
        _run_phantom_cleanup_phase(phantom_cleanup, network)
    ...
    if dance_plan:
        if not _run_neutralize_phase(dance_plan, network):
            console.print("  [red]❌ dance phase 1 (neutralize) failed; aborting upgrade.[/]")
            sys.exit(1)
    ...
    console.print(f"  Running: {' '.join(cmd[1:])}")
    ...
    if new_ver != latest:
        ...
        console.print(
            f"  [red]❌ npm reported success but {OPENCLAW_PACKAGE_JSON} still reads {new_ver} "
            f"(expected {latest}).[/]"
        )
        if r.stdout.strip():
            console.print(f"\n  [yellow]npm stdout:[/]\n{r.stdout[:800]}")
        ...
        console.print(
            ...
            f"  [yellow]Expected:   [/] {OPENCLAW_PACKAGE_JSON.parent}"
        )
        console.print(
            f"\n  [yellow]→[/] Likely cause: a stale install at "
            f"{OPENCLAW_PACKAGE_JSON.parent} is shadowing the new one, or your\n"
            f"     npm prefix has drifted from {OPENCLAW_NPM_PREFIX}. Fix:\n"
            f"       1. Inspect the path: ls -la {OPENCLAW_PACKAGE_JSON.parent}\n"
            f"          If it's a regular directory (not a symlink), it's a leftover\n"
            f"          from a previous brew node version.\n"
            f"       2. Remove it: sudo rm -rf {OPENCLAW_PACKAGE_JSON.parent}\n"
            f"       3. Re-run: sudo evolve-admin menu upgrade"
        )
        sys.exit(1)
    console.print(f"  [green]✅ Installed: {new_ver}[/]")
    ...
    if dance_plan:
        _run_install_phase(dance_plan, network)
        # PHASE 3: restore the original configs from the .preupgrade snapshots.
        _run_restore_phase(dance_plan, network)
    ...
    for bot_id in _bot_ids(network):
        try:
            ...
            console.print(f"  [yellow]⚠️  {bot_id}: CLI device scope check failed: {e}[/]")
            continue
        if outcome.changed:
            repaired_bots.append(bot_id)
            console.print(f"  [yellow]repaired[/] {bot_id}: {outcome.detail}")
        elif not outcome.ok:
            failed_bots.append(bot_id)
            console.print(f"  [yellow]⚠️  {bot_id}: {outcome.detail}[/]")
    if failed_bots:
        console.print(
        ...
        console.print("  [green]✅ CLI device scopes intact[/]")
    ...
    try:
        _verify_send_surface_post_upgrade(installed, new_ver, network)
    ...
    try:
        _verify_oc_dependencies_post_upgrade(installed, new_ver, network)
    except Exception as e:  # noqa: BLE001 — verification must not wedge the upgrade tail
        console.print(
    ...
    _emit_runtime_notes_review_signal(installed, new_ver)
...
# silently for 8 days (docs/decision-add-bot-m4-u1-proof-2026-06-11.md
# §"The 07:00 window"). The delivery convention is now `openclaw message
# send` (docs/spec-gallery-delivery-convention-2026-06-11.md) — upstream
...
def oc_safe_upgrade(
    ...
    Runs the gates from docs/spec-safe-upgrade-2026-05-02.md against
