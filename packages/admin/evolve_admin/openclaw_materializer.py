...
Phase 3 of ``internal/spec-openclaw-json-derived-artifact-2026-05-24.md``.
...
# spawns (session_surface.py). It is derived from the
...
@dataclass
class OverrideClobber:
    """A recorded override that overwrote a DIFFERENT live value.

    Auto-promotion (Pass 2) only fires for fields with NO recorded
    override — once a field has one, every later hand-edit to the live
    openclaw.json is overwritten by the override on the next deploy.
    That is by design (the override file is the source of truth), but it
    must never be silent: the 2026-07-31 incident left a bot pinned at
    ``tier=off`` because an A/B-test value had been auto-promoted and
    every subsequent hand-set tier (including the restore) was clobbered
    back with zero operator-visible signal.
    """
    ...
    def message(self) -> str:
        return (
            f"override openclaw.plugins.evolve.{self.field}="
            f"{self.override_value!r} OVERRODE hand-edited value "
            f"{self.live_value!r} — update the override (config sandbox / "
            f"Customizations page) or remove it to change this field"
        )
...
class MaterializeResult:
    ...
    Attributes:
        ...
        override_clobbered: the subset of applied overrides whose value
            REPLACED a different value present in ``current_block`` — a
            hand-edit the override just overwrote. Callers must surface
            these loudly (deploy prints them to the console); a WARN is
            also logged here. Sorted by field name.
    ...
    override_clobbered: list[OverrideClobber] = field(default_factory=list)
...
def materialize_evolve_plugin_config(
    ...
    *,
    ...
    override_clobbered: list[OverrideClobber] = []

    def _note_clobber(field_name: str, ov: OverrideEntry) -> None:
        # An applied override that differs from the value sitting in the
        # live block just overwrote a hand-edit. Pass 2 deliberately skips
        # drift detection for fields with a recorded override, so this is
        # the ONLY place the divergence is visible — be loud (the
        # 2026-07-31 tier=off incident: silent clobber on every deploy).
        if field_name not in current_block:
            return
        if current_block[field_name] == ov.value:
            return
        clobber = OverrideClobber(
            field=field_name,
            override_value=ov.value,
            live_value=current_block[field_name],
        )
        override_clobbered.append(clobber)
        _log.warning("materializer: %s: %s", bot_id, clobber.message)
    ...
    return MaterializeResult(
        ...
        override_clobbered=sorted(override_clobbered, key=lambda c: c.field),
...
def _apply_cache_tier_dwell(
    decision: Any, bot_id: str, shared_dir: Path | None, cache_shape: Any,
) -> tuple[str | None, str]:
    """Hold a freshly-resolved tier against the one already deployed.

    ``auto`` re-resolves on every materialize. Without dwell a bot parked at
    the break-even flips ``long`` → ``short`` → ``long`` across three deploys
    as a weekend rolls in and out of the seven-day window, and every flip
    invalidates the live cache and re-primes it at full price — the cost the
    tier exists to avoid, paid for a decision no operator made.

    So a NEW tier is only written when it either clears the break-even by
    :data:`cache_shape.AUTO_FLIP_MARGIN`, or the standing decision is at least
    :data:`cache_shape.AUTO_FLIP_MIN_DWELL_DAYS` old. Otherwise the deployed
    tier stands and the reason says so.

    Soft-fails to the raw decision: the dwell record is bookkeeping, and a
    bookkeeping failure must not change which tier a deploy writes.
    """
    resolved = decision.retention
    if shared_dir is None or not decision.decided:
        # A decline records nothing — "auto could not size this" is not a
        # decision to dwell on, and writing it would start a 7-day clock on
        # a tier that was never deployed.
        return resolved, decision.reason
    try:
        from better_engine_config import load as _load_be_config  # type: ignore
        from better_engine_config import save as _save_be_config  # type: ignore

        cfg = _load_be_config(shared_dir)
        previous, previous_at = cfg.per_bot_cache_retention_decision(bot_id)
    except Exception as e:  # noqa: BLE001
        _log.warning(
            "policy_slice: reading the standing cache-tier decision for %s "
            "failed (%s: %s); applying %r without dwell this pass",
            bot_id, type(e).__name__, e, resolved,
        )
        return resolved, decision.reason
    ...
    if previous is not None:
        age_days = _decision_age_days(previous_at, now)
        cleared = cache_shape.clears_flip_margin(resolved, decision.stats)
        stale = age_days is None or age_days >= cache_shape.AUTO_FLIP_MIN_DWELL_DAYS
        if not (cleared or stale):
            held = (
                f"auto held at {cache_shape.RETENTION_LABEL[previous]}: this pass "
                f"measured {cache_shape.RETENTION_LABEL[resolved]} but the re-write "
                f"factor ({decision.stats.rewrite_factor:.2f}x) is within "
                f"{cache_shape.AUTO_FLIP_MARGIN * 100:.0f}% of the break-even "
                f"(~{cache_shape.LONG_RETENTION_BREAKEVEN:.2f}x) and the standing "
                f"decision is only {age_days:.1f}d old. Flipping the tier costs a "
                f"full prefix rewrite, so the deployed tier stands."
            )
            _log.info("policy_slice: cache tier for %s — %s", bot_id, held)
            return previous, held
        # An actual flip. WARNING, not info: each one re-primes the cache at
        # full price, and an operator who notices the bill should be able to
        # find the line that did it without turning up the log level.
        _log.warning(
            "policy_slice: cache tier for %s FLIPPED %s -> %s (%s) — %s",
            bot_id,
            cache_shape.RETENTION_LABEL[previous],
            cache_shape.RETENTION_LABEL[resolved],
            "cleared the break-even margin" if cleared
            else f"standing decision {age_days:.1f}d old" if age_days is not None
            else "no timestamp on the standing decision",
            decision.reason,
        )

    try:
        cfg.set_per_bot_cache_retention_decision(
            bot_id, resolved, now.isoformat().replace("+00:00", "Z"),
        )
        _save_be_config(cfg, shared_dir)
    except Exception as e:  # noqa: BLE001
        _log.warning(
            "policy_slice: recording the cache-tier decision for %s failed "
            "(%s: %s); the tier is applied but dwell restarts next pass",
            bot_id, type(e).__name__, e,
        )
    return resolved, decision.reason


def _decision_age_days(stamp: str | None, now: datetime) -> float | None:
    """Age of an ISO-8601 decision stamp in days; ``None`` when unreadable.

    Unreadable reads as "old enough to overturn" at the call site — a record
    we cannot date must not be able to pin a tier forever.
    """
...
def resolve_cache_retention(
    configured: str | None, bot_id: str, shared_dir: Path | None = None,
) -> tuple[str | None, str]:
    """``(value to write, one-line reason)`` for this bot's cache tier.

    ``"short"`` / ``"long"`` pass through untouched — an explicit operator
    choice is not re-argued against the data. ``"auto"`` is measured from the
    bot's own inter-turn gaps (``cache_shape``) and resolved to whichever tier
    the break-even buys, then held against the tier already deployed by
    :func:`_apply_cache_tier_dwell`; ``None`` (no override) stays ``None``.

    ``shared_dir`` is where the standing decision is recorded (beside the knob
    in better-engine-config). Omit it and the resolution is applied without
    dwell — the pre-dwell behaviour, kept for callers that only want the
    measurement.

    Soft-fails to ``(None, ...)``: a cache tier is not worth failing a deploy
    over, and ``None`` is the pod's existing behavior rather than a guess.
    The reason string is logged so a deploy that moved a tier says why in the
    place an operator looks when they notice it moved.
    """
    ...
    if configured is None:
        return None, ""
    try:
        import cache_shape  # type: ignore[import]
    except Exception as e:  # noqa: BLE001 — analyzer path unavailable
        _log.warning(
            "policy_slice: cache_shape unavailable (%s: %s); cache tier "
            "%r left unresolved for %s",
            type(e).__name__, e, configured, bot_id,
        )
        return None, ""
    try:
        # No ``days``: cache_shape.AUTO_TIER_WINDOW_DAYS is the one home for
        # the window, so the deploy and the Cost page read the same stretch.
        stats = cache_shape.measure_gap_stats(bot_id)
        decision = cache_shape.resolve_retention(configured, stats)
    except Exception as e:  # noqa: BLE001
        _log.warning(
            "policy_slice: resolving cache tier %r for %s raised (%s: %s); "
            "leaving the tier unset this pass",
            configured, bot_id, type(e).__name__, e,
        )
        return None, ""
    return _apply_cache_tier_dwell(decision, bot_id, shared_dir, cache_shape)
...
def materialize_openclaw_policy_slice(
    ...
    #
    # ``"auto"`` is resolved HERE rather than stored resolved, so the tier
    # tracks the bot's traffic instead of freezing whatever it looked like on
    # the day an operator clicked. ``resolve_cache_retention`` returns None
    # when auto declined (too little history to size the break-even), and a
    # None skips the fan-out exactly as an unset knob does — an undecidable
    # bot keeps OpenClaw's own default rather than being moved onto a premium
    # nothing has shown it earns.
    effective_retention, retention_reason = resolve_cache_retention(
        be_cache_retention, bot_id, shared_dir,
    )
    if retention_reason:
        _log.info(
            "policy_slice: cache tier for %s — %s", bot_id, retention_reason,
        )
    if effective_retention is not None:
        affected = _apply_models_cache_retention(cfg, effective_retention)
        if affected:
