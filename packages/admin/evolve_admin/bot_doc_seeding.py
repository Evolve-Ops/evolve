...
# Docs whose live content is authored by the BOT (or operator) at runtime,
# where ANY divergence from the rendered template is content to preserve.
# MEMORY.md entries start small — a bot's first few durable facts sit far
# under the 1500-byte structural floor, so the floor-based guard re-templated
# them on every deploy (second finding of the 2026-06-24 member-bot MEMORY.md
# incident). The floor stays for the identity docs, where sub-floor content
# is exactly the truncation-damage signature the floor exists to catch.
PRESERVE_ANY_DIFF_DOCS: frozenset[str] = frozenset({"MEMORY.md"})


def read_existing_doc(
    dst: Path, *, run_cat: "Callable[[list[str]], Any] | None" = None,
) -> "tuple[str | None, bool]":
    """Read a bot-owned workspace doc for the operator-edit guard.

    Returns ``(text, unreadable)``:

      * ``(str, False)``  — read succeeded; ``text`` is what's on disk.
      * ``(None, False)`` — genuinely ABSENT: safe for the caller to seed.
      * ``(None, True)``  — UNREADABLE: the read failed in a way that does NOT
        prove absence (sudo denial, timeout, EACCES with no working fallback).
        The caller MUST skip the write. Treating unreadable as absent is how
        a member bot's live 12KB MEMORY.md became the template on 2026-06-24:
        the evolve-user deploy had a sudoers ``cp`` grant for the doc but no
        ``cat`` grant, so the guard's probe failed, was read as "nothing on
        disk", and the template landed over live memory.

    Read order follows the CLAUDE.md file-access pattern: direct read first
    (the evolve read ACL from ``set_evolve_read_acl`` makes bot ``.openclaw/``
    files directly readable; root/operator CLI runs read directly anyway),
    then ``sudo /bin/cat`` as the pre-ACL fallback. A clean ``ENOENT`` from
    the direct read is trusted as absent — an unsearchable parent surfaces as
    ``EACCES``, not ``ENOENT``, so it can't mask a hidden file. Content that
    is present but not valid UTF-8 is unreadable, never absent: a file
    truncated mid-multibyte character is damaged, and damage must not be
    re-seeded over. The decode is pinned to UTF-8 rather than left to
    ``locale.getpreferredencoding()`` so that sentence stays true on every
    pod — see the ``encoding=`` note below. On the sudo path, only cat's own
    "No such file" proves absence; any other failure (e.g. "a password is
    required" from sudoers drift) is unreadable.

    ``run_cat`` injects the subprocess runner for tests; production uses
    ``subprocess.run`` with a 10s timeout.
    """
    try:
        # encoding= is PINNED, never the locale default. Bare read_text() decodes
        # with locale.getpreferredencoding(), so the UnicodeDecodeError arm below
        # would mean "undecodable in whatever locale this process inherited"
        # rather than "damaged", and the two are not the same file. Under a
        # non-UTF-8 locale (LC_ALL=C with PEP 538 coercion off, or any host with
        # no UTF-8 locale available) that resolves to US-ASCII, and a PERFECTLY
        # HEALTHY MEMORY.md containing one em dash decodes as a hard error — so
        # the guard would answer UNREADABLE and skip seeding that doc forever,
        # silently, since a guarded skip is deliberately not a deploy failure.
        # These are bot workspace docs: UTF-8 is the format, not a preference.
        return dst.read_text(encoding="utf-8"), False
    except FileNotFoundError:
        return None, False
    except UnicodeDecodeError:
        # The bytes ARE there and readable — they just aren't valid UTF-8, which
        # is a damage signature (truncation mid-multibyte character), not
        # absence. Answer UNREADABLE directly rather than falling through to the
        # sudo probe: the probe would `cat` the same bytes and fail the same
        # decode, so it can only ever return the same answer one privileged
        # subprocess later. Skipping the write is the whole point — re-seeding
        # here would overwrite damaged memory with the template, which is the
        # 2026-06-24 clobber wearing a different hat.
        return None, True
    except OSError:
        # Unreadable directly (pre-ACL bot / clamped mask) → sudo probe.
        return _sudo_probe_doc(dst, run_cat)


def _sudo_probe_doc(
    dst: Path, run_cat: "Callable[[list[str]], Any] | None",
) -> "tuple[str | None, bool]":
    """The ``sudo /bin/cat`` fallback half of :func:`read_existing_doc`."""
    try:
        if run_cat is not None:
            proc = run_cat(["sudo", "/bin/cat", str(dst)])
        else:
            proc = subprocess.run(
                ["sudo", "/bin/cat", str(dst)],
                capture_output=True, text=True, timeout=10,
            )
    except Exception:  # noqa: BLE001 — timeout, missing sudo, anything: unproven
        return None, True
    if proc.returncode == 0:
        return proc.stdout, False
    if "No such file" in (proc.stderr or ""):
        return None, False  # root's cat confirms the file is absent
    return None, True
...
def should_skip_operator_edited(
    ...
    ``existing_text is None`` means the caller PROVED the file absent (see
    :func:`read_existing_doc`) — an unreadable file must never reach this
    predicate as ``None``; the caller skips the write instead.

    :data:`PRESERVE_ANY_DIFF_DOCS` (MEMORY.md): bot-authored at runtime, so
    ANY non-blank divergence from the render is preserved — no size floor. A
    blank/whitespace-only file is not memory and gets re-templated.
    ...
    if existing_text is None:
        return False  # proven absent → write
    if role == "primary" and fname in PRIMARY_VERBATIM_DOCS:
        return False  # Evolve-owned source of truth → always (re)write
    if fname in PRESERVE_ANY_DIFF_DOCS:
        return bool(existing_text.strip()) and existing_text != rendered
