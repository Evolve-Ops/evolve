...
# (see internal/incidents/README.md). NOT `{shared_dir}/incidents/` either —
...
# the source repo's `internal/incidents/` via PR.
...
class PullResult:
    ...
    # Outcome of the post-pull sweep re-exec (fresh process). Empty when
    # the tick swept inline — i.e. when the loaded deploy code was already
    # the checkout, which is every no-op tick.
    lagging_sweep_reexec: str = ""
    lagging_sweep_reexec_error: str = ""
    retired_jobs_removed: list[str] = field(default_factory=list)
    retired_job_errors: dict[str, str] = field(default_factory=dict)
    # Why the lagging-bot sweep did not run this tick. Rendered by
    # format_for_log — the reason used to go only to `steps`, which that
    # renderer never reads, so a permanently-deferred sweep left no trace
    # in repo-puller.log at all (the 2026-08-18 #3705 investigation).
    redeploy_sweep_skipped: str = ""
    ...
    # Gateway labels found on disk whose bot is NOT a current pod member
    # (stale residue from a renamed/removed bot) — reconciled out of the
    # restart set so a plugin rebuild can't re-arm a dead gateway. The
    # #3167 orphan-sweeper REMOVES these units; this just declines to
    # RESTART them in the window before a sweep. See _reconcile_gateways_to_roster.
    bot_gateways_skipped_nonmember: list[str] = field(default_factory=list)
    ...
    # internal/decision-add-bot-m4-u1-proof-2026-06-11.md (#2792).
    ...
    # land via re-staging, not the puller. See internal/runbook-vps-pod-provision.md.
...
# 2026-06-08 when the app-test surface was killed.) app_discovery.py owns the
# app-discovery-sweep JobSpec for the same reason — a PR that changes only its
# schedule or argv would otherwise leave the old unit on disk until something
# else touched deploy.py.
_INFRA_INSTALL_PATHS: tuple[str, ...] = (
    ...
    "packages/admin/evolve_admin/app_discovery.py",
)
...
def _find_lagging_bots(
    ...
    *,
    ...
    install.json stamp is NOT the currently-deployed code.

    The decision is delegated to :func:`bot_version_sync.classify_sync`, the
    same identity comparison the Versions UI and ``evolve-admin status`` use —
    a sha/commit-count check rather than a ``!=`` on the display string. The
    display string is ``YYYY.MMDD.<PR#>``, which that module's docstring is
    explicit about never comparing: a direct push carries no PR number and
    renders ``YYYY.MMDD.0``, so two different same-day pushes compare EQUAL
    and the old predicate silently skipped a redeploy the fleet needed.

    "Not synced" — not the narrower "behind" — is what triggers a redeploy.
    The sweep's contract is *parity with the deploy checkout*, in both
    directions: a bot sitting on a NEWER commit than the checkout is drift
    too, and redeploying it back down is how ``release rollback`` returns the
    fleet to stable (it force-moves the checkout backward, then runs this
    sweep via ``run_hooks_only``). The one sanctioned ahead case — the release
    canary mid-soak — is carved out by name in :func:`_redeploy_lagging_bots`,
    not by relation.

    ``current_sha`` / ``current_count`` are optional so a caller with no git
    identity degrades to exactly the old version-string equality (that is
    classify_sync's documented legacy branch).
    ...
    for bot_id, entry in bot_versions.items():
        entry = entry or {}
        deployed = entry.get("version") or ""
        synced, _relation = _vsync.classify_sync(
            deployed or None,
            entry.get("sha"),
            entry.get("commit_count"),
            current_sha,
            current_count,
            current_version,
        )
...
def _loaded_deploy_code_is_current(repo: Path) -> tuple[bool, str]:
    """Is the ``deploy`` module THIS process imported the same commit as the
    checkout on disk right now?

    ``deploy.EVOLVE_VERSION`` / ``EVOLVE_COMMIT_SHA`` are module globals
    computed once at import, and ``cli.py`` imports ``deploy`` at module load —
    i.e. BEFORE the tick's ``git pull`` runs. So on any tick that advances
    HEAD, the in-process identity is the PRE-pull commit. The sweep then
    stamps every redeployed bot at that pre-pull commit, which marks the whole
    fleet "current" at a version that is already superseded — and the next
    tick, whose fresh process computes the post-pull version, finds all of them
    lagging again. The redeploy can never mark a bot current for the commit it
    just pulled.

    (``run_hooks_only`` already dodges this by being invoked in a FRESH
    subprocess after the release manager moves the checkout — see its
    docstring, spec §2.4 review #14. ``pull()`` has no such escape, so it
    needs this guard.)

    Returns ``(is_current, reason)``. When the on-disk identity can't be
    resolved (no git, not a checkout — the case unit tests hit by passing a
    bare tmp_path) we cannot PROVE staleness, so this fails OPEN: ``(True,
    "")``, and the sweep behaves exactly as it did before the guard existed.
    """
    from evolve_admin import deploy as _deploy  # local import to avoid cycle at module load

    on_disk_sha, _ = _vsync.compute_commit_identity(repo)
    if not on_disk_sha or not _deploy.EVOLVE_COMMIT_SHA:
        return True, ""
    if on_disk_sha == _deploy.EVOLVE_COMMIT_SHA:
        return True, ""
    return False, (
        f"loaded deploy code is {_deploy.EVOLVE_COMMIT_SHA[:8]} but the "
        f"checkout is now {on_disk_sha[:8]}"
    )
...
def _redeploy_lagging_bots(
    ...
    *,
    ...
    # Compare against the identity of the code that will actually run the
    # deploy — the module globals, NOT a fresh read of the checkout. The two
    # can differ (see _loaded_deploy_code_is_current), and the caller guards
    # on that; using the globals here keeps the comparand identical to what
    # record_bot_deploy() will stamp, which is the whole convergence contract:
    # a bot the sweep just deployed must compare equal on the next sweep.
    current_version = _deploy.EVOLVE_VERSION

    succeeded: list[str] = []
    errors: dict[str, str] = {}

    lagging = _find_lagging_bots(
        shared_dir, current_version,
        current_sha=_deploy.EVOLVE_COMMIT_SHA,
        current_count=_deploy.EVOLVE_COMMIT_COUNT,
    )
...
def _roster_member_ids(network: dict) -> set[str]:
    """The authoritative set of bot ids that may own a gateway:
    ``network["members"]`` ∪ the resolved primary, falling back to
    ``bots.keys()`` on a legacy pod with no ``members`` array.

    This is a local mirror of ``deploy._is_provisionable_bot``'s roster shape
    (PR #3279 / EVO-GATEWAY-RESIDUE-RERENDER), used only as a fallback on
    builds where that single-source predicate hasn't landed yet. Once it has,
    ``_reconcile_gateways_to_roster`` delegates straight to it and this is
    bypassed — keep the two shapes identical.
    """
    ...
    # Union the resolved primary — always a bot, always a member, even on a
    # pod whose ``members`` array predates the explicit-primary field.
    try:
        from primary_bot import primary_bot_id as _pbid  # type: ignore
    except ImportError:
        # Build without `primary_bot`. Not a degradation: `members` (or, on a
        # legacy pod, `bots.keys()`) already contains the primary — this union
        # is belt-and-braces for a pod whose members array predates the
        # explicit-primary field.
        _pbid = None
    if _pbid is not None:
        try:
            primary = _pbid(network)
        except Exception:  # noqa: BLE001 — malformed network: roster stands without the union
            primary = None
...
def _reconcile_gateways_to_roster(
    labels: list[str],
    network: "dict | None" = None,
) -> tuple[list[str], list[str]]:
    """Split disk-discovered gateway labels into (kept, skipped) by pod
    membership.

    ``_discover_bot_gateways`` globs every ``ai.openclaw.*-gateway.plist`` on
    disk with no roster check, so a stale plist left by a renamed/removed bot
    is discovered and would be kickstarted on every plugin rebuild —
    re-arming a dead gateway. This is the macOS sibling of
    EVO-GATEWAY-RESIDUE-RERENDER (PR #3279, which gated the Linux
    CLI/provisioning path). Here we reconcile the discovered labels against the
    authoritative roster (``network["members"]`` ∪ resolved primary) and drop
    non-members so only roster members get restarted.

    Complementary to the #3167 orphan-sweeper, which REMOVES stale units: this
    only declines to RESTART them in the window before a sweep runs.

    Membership is delegated to ``deploy._is_provisionable_bot`` — the single
    roster predicate from PR #3279 — when present, falling back to
    ``_roster_member_ids`` (same shape) on builds where it hasn't landed.

    Bias on uncertainty differs from the provisioning gate: if the roster
    can't be determined (no readable ``network.json``), reconciliation is
    SKIPPED and every discovered label is kept. Over-restarting a real member
    is harmless (worst case a single failed kickstart of an already-gone unit,
    handled gracefully), whereas filtering blind could starve a real member of
    its post-rebuild restart. ``deploy._is_provisionable_bot`` fail-CLOSES
    instead because materializing a phantom gateway unit is worse than
    skipping one — the opposite trade-off, deliberately.
    """
    labels = list(labels)
    if not labels:
        return labels, []
    ...
    _deploy = None
    try:
        from . import deploy as _deploy  # local import to avoid cycle at module load
    except Exception:
        _deploy = None

    if network is None and _deploy is not None:
        try:
            network = _deploy.load_network(_deploy._CANONICAL_NETWORK_JSON)
        except Exception:
            network = None
    if not isinstance(network, dict):
        # Unknown roster → reconcile nothing; keep every discovered label.
        return labels, []

    pred = getattr(_deploy, "_is_provisionable_bot", None) if _deploy else None
    if pred is not None:
        def _is_member(bot: str) -> bool:
            try:
                return bool(pred(bot, network))
            except Exception:
                return True  # uncertain → keep (over-restart is harmless here)
    else:
        roster = _roster_member_ids(network)
        def _is_member(bot: str) -> bool:
            return bot in roster

    kept: list[str] = []
    skipped: list[str] = []
    for label in labels:
        (kept if _is_member(_short_bot_name(label)) else skipped).append(label)
    return kept, skipped
...
def pull(
    ...
    *,
    ...
    gateway_roster_fn: "Callable[[], dict | None] | None" = None,
) -> PullResult:
    ...
    if rc != 0:
        ...
        # internal/runbook-vps-pod-provision.md.
        if not (repo / ".git").exists():
            result.success = True
    ...
    _run_post_advance_hooks(
        ...
        gateway_roster_fn=gateway_roster_fn,
    )
    ...
    if _paths_touch_plugin(paths):
        ...
        if result.plugin_rebuilt:
            if not _auto_restart_enabled():
                ...
                # Reconcile the disk-discovered labels against the pod roster
                # before restarting: a stale ai.openclaw.<non-member>-gateway
                # plist (renamed/removed bot) would otherwise be kickstarted
                # every plugin rebuild, re-arming a dead gateway. macOS sibling
                # of EVO-GATEWAY-RESIDUE-RERENDER (PR #3279); the #3167
                # orphan-sweeper reaps such units, this declines to restart them
                # in the window before a sweep.
                if gateways:
                    roster_net = gateway_roster_fn() if gateway_roster_fn else None
                    gateways, skipped = _reconcile_gateways_to_roster(
                        gateways, network=roster_net
                    )
                    if skipped:
                        result.bot_gateways_skipped_nonmember = skipped
                        result.steps.append(
                            f"skipping {len(skipped)} non-member gateway(s) "
                            f"(stale residue, not in network['members']; "
                            f"orphan-sweeper #3167 reaps them): "
                            f"{', '.join(_short_bot_name(l) for l in skipped)}"
                        )
                if gateways:
...
def _run_gallery_reseed_hook(
    ...
    re-reads the repo package. (identity: see
    ``applications.app_identity.resolve_app_id`` — the only ``spec_id`` in
    this module is this prose describing the gallery tier path, which build
    brief §5 keeps as semantically distinct. No identity is read here.) Without this sweep a freshly-forged bot keeps
...
# The sweep-only subcommand + flags, shared by every entrypoint candidate in
# :func:`_sweep_reexec_argv` so the three can never drift. Registered from
# ``redeploy_sweep_cli`` rather than declared inline in cli.py, which is a
# no-growth-capped hot-hazard file.
SWEEP_ONLY_COMMAND = "repo-redeploy-sweep"
_SWEEP_ONLY_ARGS = (SWEEP_ONLY_COMMAND, "--quiet")


def _sweep_reexec_argv(repo: Path) -> tuple[list[str], str]:
    """argv that runs ONLY the lagging-bot sweep in a fresh process.

    Returns ``(argv, why_not)`` — an empty argv with a reason when no usable
    entrypoint can be resolved, so the caller degrades to the old skip rather
    than failing the tick.

    Resolution order, most-certain first:

    1. The ``evolve-admin`` console script **beside the running interpreter**.
       ``sys.executable`` is by definition the venv this code was imported
       from, so its sibling cannot be a different install — unlike
       ``release_manager.DEFAULT_EVOLVE_ADMIN``, which is a hardcoded
       ``/Users/Shared/...`` literal and simply wrong on a Linux pod.
    2. The platform profile's ``venv_evolve_admin`` (``{venv_dir}/bin/evolve-admin``),
       for the case where the caller is running from a system interpreter.
    3. ``sys.executable -m evolve_admin.cli`` — cli.py has a ``__main__``
       guard, so this works even with no console script on PATH.
    """
    import sys as _sys

    cand = Path(_sys.executable).parent / "evolve-admin"
    if cand.exists():
        return [str(cand), *_SWEEP_ONLY_ARGS, "--repo", str(repo)], ""
    prof_bin = Path(_PROFILE.venv_evolve_admin)
    if prof_bin.exists():
        return [str(prof_bin), *_SWEEP_ONLY_ARGS, "--repo", str(repo)], ""
    if _sys.executable:
        return [_sys.executable, "-m", "evolve_admin.cli",
                *_SWEEP_ONLY_ARGS, "--repo", str(repo)], ""
    return [], "no evolve-admin entrypoint could be resolved"


def _reexec_lagging_sweep(result: PullResult, repo: Path, why_stale: str) -> bool:
    """Run the lagging-bot sweep in a FRESH process. Returns whether it ran.

    This is what stops a merge burst from starving the sweep. The in-process
    ``deploy`` module on a HEAD-advancing tick is the pre-pull commit, so
    sweeping inline would stamp every redeployed bot at a superseded version
    — which is why ``_loaded_deploy_code_is_current`` skips. But skipping
    only converges on a NO-OP tick, and on 2026-08-18 six consecutive ticks
    advanced with no no-op tick between them (#3705 → #3721). A fresh process
    imports the post-pull ``deploy``, so it can sweep with a truthful identity
    on the SAME tick that pulled.

    Same trick ``run_hooks_only`` already uses for the release manager's
    promote/rollback path (spec-state-store-and-deploy-resilience §2.4) —
    this just applies it to the one caller that was left skipping instead.

    Deliberately NOT detached (no ``start_new_session``): the inline sweep it
    replaces dies with the tick, and orphaning a half-finished fleet redeploy
    when the puller is killed would be a new failure mode, not a fixed one.
    The child takes the pod deploy lock itself, exactly as the inline path
    does. ``cwd=repo`` avoids the documented sudo-evolve-cwd gotcha (the
    caller's cwd may be an admin home the evolve user cannot traverse, which
    kills the re-exec'd interpreter before line 1).
    """
    argv, why_not = _sweep_reexec_argv(repo)
    if not argv:
        result.lagging_sweep_reexec_error = (
            f"{why_stale}; could not re-exec ({why_not}) — falling back to the "
            f"next tick"
        )
        return False
    try:
        # No timeout on the inline sweep this replaces, so any bound is an
        # improvement; 1800s matches the release manager's hook-suite bound
        # and is generous for a full-fleet redeploy (9 bots measured ~5min).
        proc = subprocess.run(
            argv, capture_output=True, text=True, timeout=1800, cwd=str(repo),
        )
    except subprocess.TimeoutExpired:
        result.lagging_sweep_reexec_error = "fresh-process redeploy sweep timed out (1800s)"
        return False
    except Exception as e:  # noqa: BLE001 — never break the pull on a spawn failure
        result.lagging_sweep_reexec_error = (
            f"fresh-process redeploy sweep could not start: {type(e).__name__}: {e}"
        )
        return False
    out = (proc.stdout or "").strip()
    if proc.returncode != 0:
        result.lagging_sweep_reexec_error = (
            f"fresh-process redeploy sweep exited {proc.returncode}: "
            f"{(out or (proc.stderr or '').strip())[-600:]}"
        )
        return False
    # The child's own format_for_log output — the real "redeployed N lagging
    # bot(s)" line, not a paraphrase. Empty when nothing lagged.
    result.lagging_sweep_reexec = out[-2000:] if out else "nothing lagging"
    return True


def run_redeploy_sweep_only(
    repo: Path, shared_dir: Path = DEFAULT_SHARED_DIR,
) -> PullResult:
    """The lagging-bot sweep, and nothing else — the child half of
    :func:`_reexec_lagging_sweep`, wired to ``repo-pull --redeploy-sweep-only``.

    Performs no pull and runs no hooks: the parent tick already pulled, and
    this process exists only so ``deploy.EVOLVE_VERSION`` is read from the
    post-pull checkout. ``allow_reexec=False`` is the recursion stop — a child
    that somehow finds ITSELF stale (the checkout moved again under it, e.g. a
    concurrent release move) skips the old way rather than spawning a
    grandchild.
    """
    result = PullResult(success=True)
    _run_lagging_bot_redeploy_sweep(result, repo, shared_dir, allow_reexec=False)
    return result
...
def _run_lagging_bot_redeploy_sweep(
    ...
    *, allow_reexec: bool = True,
    ...
    # Only sweep when the deploy code this process loaded IS the checkout on
    # disk. Otherwise every stamp we write names a superseded commit and the
    # next tick redeploys the whole fleet again (see the helper's docstring).
    # Skipping costs at most one tick: the next process imports the post-pull
    # deploy module and sweeps with a truthful identity.
    _fresh, _why = _loaded_deploy_code_is_current(repo)
    if not _fresh:
        # Stamping from the pre-pull module would mark the fleet current at a
        # superseded commit, so this process must not sweep. Hand it to a
        # fresh one that imported the post-pull code instead of deferring to
        # a no-op tick that a merge burst may never deliver (#3705 → #3721).
        if allow_reexec and _reexec_lagging_sweep(result, repo, _why):
            return
        _msg = (
            f"{_why} — the next tick sweeps with the post-pull version "
            f"(stamping the pre-pull one would guarantee a repeat full-fleet "
            f"redeploy)"
        )
        result.steps.append(f"skipped redeploy sweep: {_msg}")
        result.redeploy_sweep_skipped = _msg
        return
    ...
    with _dres.deploy_lock(shared_dir) as _lk:
        if _lk is None:
            ...
            _msg = (
                "pod deploy lock held (manual upgrade in progress) — will "
                "retry next tick"
            )
            result.steps.append(f"skipped redeploy sweep: {_msg}")
            result.redeploy_sweep_skipped = _msg
...
def _run_retired_job_teardown_sweep(result: PullResult, shared_dir: Path) -> None:
    """Tear down per-bot jobs Evolve has retired. Runs on EVERY tick.

    Deliberately outside both of the lagging-bot sweep's gates:

    * **Not behind ``_loaded_deploy_code_is_current``.** That guard exists to
      stop a pre-pull module from writing version STAMPS; this sweep writes
      none, so staleness costs it nothing but one tick of latency (the tick
      that pulls a retirement is still running code that does not know the
      label is retired; the next tick's fresh process does). Putting it behind
      the guard is what made teardown wait for a no-op tick — and on
      2026-08-18 no no-op tick came for six consecutive ticks, which is how
      #3705's dead units survived on both pods until a manual ``deploy --all``.
    * **Not behind the pod deploy lock.** The lock (C1, deploy-resilience
      2026-06-24) guards the redeploy sweep's recursive perm passes against a
      concurrent manual upgrade. Removing a label that nothing installs any
      more cannot race an installer — no code path writes these units — so
      taking the lock would only reintroduce a way for teardown to be starved.

    Best-effort: :func:`retired_jobs.sweep_pod` never raises, and the extra
    guard here keeps an import failure from breaking a pull.
    """
    try:
        from .retired_jobs import sweep_pod
        removed, errors = sweep_pod(shared_dir)
    except Exception as e:  # noqa: BLE001 — never break the pull on teardown
        result.retired_job_errors = {"__sweep__": f"{type(e).__name__}: {e}"}
        return
    if removed:
        result.retired_jobs_removed = removed
        result.steps.append(
            f"removed {len(removed)} retired per-bot job(s): "
            f"{', '.join(sorted(removed))}"
        )
    if errors:
        result.retired_job_errors = errors
        for label, msg in errors.items():
            result.steps.append(f"FAIL remove retired job {label}: {msg}")
...
def render_plist(
    ...
    - `UserName=evolve` — the puller needs git write access to
      /Users/Shared/evolve-repo. Repo ownership should be
      evolve:<profile.ops_group> after a normal install (see
      ``deploy.ensure_pod_perms``); if it isn't, this daemon will
      surface the perms problem cleanly via the err log rather than
      failing silently.
    - `umask 002` — files and dirs created by git operations (fetch
      ...
      of the repo's group. With 002, new dirs land at 775 and both
      the daemon and the operator can write. Pairs with the
      `core.sharedRepository=group` config set up by `install_launchd`.
      NB this umask is why the checkout's GROUP has to be the operator
      group and not the all-bots one: every new object git unpacks is
      born group-writable, so the group is the access decision
      (2026-09-23).
...
def format_for_log(result: PullResult, quiet: bool = False) -> str:
    ...
    "Suppresses no-op pulls" means the *bare* no-op tick — one that did
    nothing but check. A no-op tick that TOOK ACTION still prints. This used
    to be an early return before any of the action lines below were appended,
    so every step a no-op tick took went unlogged: the lagging-bot redeploy,
    the openclaw-invalid warning, sudoers drift, gateway kicks. That blind
    spot is why the redeploy sweep looked far rarer in the log than it is —
    the sweep converges on the tick AFTER an advance, which is by definition
    a no-op tick, so the majority of its full-fleet redeploys were invisible.
    """
    if not result.success:
        return f"[repo-puller] ERROR: {result.error}"
    lines: list[str]
    noop = result.head_before == result.head_after
    if noop:
        # `run_tick_maintenance` builds a PullResult with no heads at all (it
        # performs no pull), so there is no sha to report — emit only whatever
        # action lines follow.
        lines = (
            [f"[repo-puller] up to date at {result.head_before[:8]}"]
            if result.head_before else []
        )
    else:
        lines = [
    ...
    if result.retired_jobs_removed:
        lines.append(
            f"[repo-puller] removed {len(result.retired_jobs_removed)} retired "
            f"per-bot job(s): " + ", ".join(sorted(result.retired_jobs_removed))
        )
    for label, info in sorted(result.retired_job_errors.items()):
        lines.append(
            f"[repo-puller] WARN remove retired job {label} failed: {info}"
        )
    ...
    if result.lagging_sweep_reexec:
        lines.append(
            "[repo-puller] post-pull redeploy sweep ran in a fresh process "
            "(this tick advanced HEAD, so the loaded deploy code was pre-pull):"
        )
        lines.extend(f"  {ln}" for ln in result.lagging_sweep_reexec.splitlines())
    if result.lagging_sweep_reexec_error:
        lines.append(
            f"[repo-puller] WARN post-pull redeploy sweep: "
            f"{result.lagging_sweep_reexec_error}"
        )
    if result.redeploy_sweep_skipped:
        # This line is the whole reason the 2026-08-18 #3705 stall needed a
        # manual investigation: the skip reason went only to `result.steps`,
        # which this renderer does not read, so a sweep that deferred on every
        # single tick was indistinguishable in the log from a sweep that ran
        # and found nothing lagging.
        lines.append(
            f"[repo-puller] redeploy sweep skipped: {result.redeploy_sweep_skipped}"
        )
    ...
    # Nothing happened AND nothing moved — that is the tick quiet mode exists
    # to swallow. Any action line above keeps the whole block, header included
    # (the header carries the sha the action was taken at).
    header_len = 1 if (noop and result.head_before) else 0
    if quiet and noop and len(lines) == header_len:
        return ""
    return "\n".join(lines)
...
# anyone noticed (historical records: internal/incidents/ in the source
...
# would be overwritten by merge". See internal/incidents/README.md.
