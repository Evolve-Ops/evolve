...
def _sudoers_drift_signal_firing(shared_dir: Path) -> bool:
    ...
    """
    try:
        idx = Path(shared_dir) / "signals" / "signature_index.json"
        data = json.loads(idx.read_text())
        return any("sudoers_refresh_failed" in str(k) for k in data)
    except Exception:  # noqa: BLE001 — missing/unreadable index → treat as not firing
        return False
...
def _render_unstick_recipe(
    *,
    ...
    Two shapes, picked by whether upstream has commits touching any of
    the blocking paths:
    ...
    *,
    ...
    try:
        store.observe(
            ...
            flavor="maintenance",
            scope="pod",
            title="sudoers grants dormant — run refresh-sudoers (as root)",
            body=(
                "The installed /etc/sudoers.d/evolve lags the rendered template: "
                "grants merged in recent PRs are NOT live. The evolve service "
                "user is deliberately barred from installing its own sudoers "
                "(Option B, #2759), so an operator must run "
                "`sudo evolve-admin refresh-sudoers` as root. This auto-resolves "
                "on the next pull once the install matches. "
                f"Detail: {(error or '')[:400]}"
            ),
            details=details,
        )
    except Exception:
