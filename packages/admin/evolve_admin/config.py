...
from evolve_config import (
    ...
    EVOLVE_VERSION as _NETWORK_SCHEMA_VERSION,
)
...
# The port ``evolve-admin serve`` binds when nothing overrides it. ONE
# definition: ``cli.serve``'s ``--port`` default and the derived fallback in
# ``resolve_admin_base_url`` both read it, so "the admin UI's port" cannot be
# two different numbers depending on which string you happen to be reading.
DEFAULT_ADMIN_PORT: int = 5050
...
def _default_network() -> dict[str, Any]:
    return {
        # Stamp the schema version the dict is already in. Without it every
        # network.json this function scaffolds reads as schema 0.0.0 to
        # evolve_config._migrate_network(), which used to re-migrate (and
        # re-WRITE) it on every load_config() call. The migration is now
        # in-memory only, so this stamp is what keeps a fresh pod's file
        # honest rather than what stops a write loop.
        "evolveVersion": _NETWORK_SCHEMA_VERSION,
        ...
        # See internal/design-multi-pod-2026-06-11.md §3.1.
        "peers": [],
        "members": [],
        ...
        "thresholds": {
            ...
            # checkpoint (default) | alert-only | downgrade-tier | pause-crons | suspend-bot
            # "checkpoint": stop background work and hold the next conversation
            # turn behind a fixed reply offering continue/stop (D-CC1..4).
            # Was "downgrade-tier" until 2026-09-04 — see
        ...
        # means "inherit this default". See internal/spec-app-testing-2026-05-07.md.
        ...
        "app_testing": {
            ...
            # See internal/spec-behavioral-runs-2026-05-07.md §5.
        ...
        # by these settings). See internal/spec-app-audit-2026-05-16.md §6.
...
def is_planned_bot_block(cfg: Any) -> bool:
    ...
    confirmed plan, spec internal/spec-add-bot-wizard-build-delta-2026-06-10.md
...
def pod_bot_ids(network: "dict[str, Any]") -> list[str]:
    """All existing bot ids of this pod: the resolved primary (first),
    then ``network.members``.

    ``primary`` and ``members`` are sibling keys — the primary is recorded
    in its own field and on current-schema pods never appears in
    ``members``. Any "does this bot exist in the pod?" check must consult
    both; a members-only scan can never match the primary. (Legacy pods
    listed the primary literally in ``members``, which is why members-only
    checks used to pass there.)
    """
    members = [m for m in (network.get("members") or []) if isinstance(m, str) and m]
    try:
        from primary_bot import primary_bot_id  # type: ignore
        pid = primary_bot_id(network)
    except Exception:
        pid = network.get("primary")
    if isinstance(pid, str) and pid and pid not in members:
        return [pid, *members]
    return members


def is_pod_bot(bot_id: str, network: "dict[str, Any]") -> bool:
    """True when ``bot_id`` is an existing bot of this pod — a member or
    the primary. Use this (never a bare ``in network["members"]``) for
    existence gates; see :func:`pod_bot_ids` for why."""
    return bool(bot_id) and isinstance(bot_id, str) and bot_id in pod_bot_ids(network)
...
# infrastructure. See internal/spec-evo-account-separation-2026-05-25.md.
...
def resolve_admin_base_url(network: "dict[str, Any] | None" = None) -> str:
    ...
    Resolution order (mirrors ``resolve_pod_context`` style):
      ...
      3. Derived default: ``http://<hostname>:<DEFAULT_ADMIN_PORT>`` where
         ``<hostname>`` is the short form of ``socket.gethostname()``
         (matches what the wizard prints for the laptop-side SSH tunnel
         one-liner), or ``localhost`` if hostname lookup fails.
    ...
    try:
        ...
        hostname = "localhost"
    return f"http://{hostname}:{DEFAULT_ADMIN_PORT}"


def resolve_admin_port(network: "dict[str, Any] | None" = None) -> int:
    """The TCP port the admin daemon serves on, resolved from OUTSIDE it.

    The daemon knows its own port — it is ``serve``'s ``--port``. A CLI
    process does not, and every operator-facing string that names the port
    is printed by one. This is how such a caller learns it, in order of how
    close each source sits to the thing that actually binds:

      1. the ``--port`` argument of the installed launchd job (the system
         LaunchDaemon first, then the per-user LaunchAgent) — the artifact
         that decides what ``serve`` is invoked with, and the only place an
         ``evolve-admin service install --port N`` override is recorded;
      2. the port in :func:`resolve_admin_base_url` — what the wizard wrote
         and what every other operator-facing admin URL already uses;
      3. :data:`DEFAULT_ADMIN_PORT`, the same constant ``serve`` defaults to.

    Never raises: an unreadable or absent plist is the ordinary state on a
    laptop clone, not an error.
    """
    port = _admin_port_from_launchd()
    if port is not None:
        return port
    try:
        from urllib.parse import urlsplit
        configured = urlsplit(resolve_admin_base_url(network)).port
    except ValueError:  # malformed authority — fall through to the default
        configured = None
    return configured or DEFAULT_ADMIN_PORT


def _admin_port_from_launchd() -> "int | None":
    """``--port`` from the installed admin-UI launchd job, or None.

    Both install shapes are checked, system first: the pod's
    ``ai.evolve.evolve.admin-ui`` LaunchDaemon and the laptop's
    ``com.evolve.admin`` LaunchAgent. Under ``sudo`` the LaunchAgent path
    resolves against root's home and simply misses — which is correct, since
    the pod's daemon is the system one.
    """
    try:
        import plistlib

        from .service import PLIST_PATH, SYSTEM_PLIST_PATH
    except Exception:  # noqa: BLE001 — a missing optional import is a miss
        return None
    for path in (SYSTEM_PLIST_PATH, PLIST_PATH):
        try:
            with open(path, "rb") as fh:
                args = plistlib.load(fh).get("ProgramArguments") or []
        except Exception:  # noqa: BLE001 — absent/unreadable/malformed: a miss
            continue
...
def validate_peers(raw: Any) -> "tuple[list[str], list[dict[str, str]]]":
    ...
    internal/design-multi-pod-2026-06-11.md §3.1(a).
...
def resolve_pod_host(network: "dict[str, Any] | None" = None) -> tuple[str, str]:
    ...
    See ``internal/spec-pwa-phase0-https-2026-05-18.md`` §3.3.
    """
    from urllib.parse import urlsplit
