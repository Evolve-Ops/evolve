...
Spec: internal/spec-circuit-breakers-2026-05-21.md §3.5, §5.2.
...
# Default channel priority for user-channel OOO posting. Projection of the
# channel registry's ``notify_priority`` column (invariant 7 — no channel-id
# literals outside the registry). This used to be a hand-maintained tuple that
# happened to be byte-identical to the alerts dispatcher's
# _DEFAULT_CHANNEL_PRIORITY; sharing the column makes that structural rather
# than coincidental. The *resolution* paths stay separate — we resolve
# recipients per-bot, the dispatcher resolves operator-only.
_USER_CHANNEL_PRIORITY: tuple[str, ...] = _channel_registry.by_notify_priority()
...
class NotifyResult:
    ...
    # Tri-state delivery verdict (docs/principle-tri-state-status.md):
    #   "delivered"     — the dispatcher reported the post succeeded
    #   "not_delivered" — we know it did not go out (no recipient, refused)
    #   "cannot_tell"   — the third state: we could not establish either way
    #                     (dry run, dispatcher unavailable, dispatch raised)
    # A cost trip that MAY not have reached anyone is not enforcement, so
    # the unknown gets its own value rather than collapsing into a false
    # "sent" (the 21:18 trip on 2026-09-03 had no delivery record at all).
    delivery: str = "cannot_tell"
...
def _resolve_user_recipient(
    ...
    through :func:`external_ids.resolve_notify_target` — the same resolver
    ``alerts.dispatcher.resolve_recipient`` uses for the operator, but
    indexed by bot_id rather than always-the-primary. Falls back to None
    when:
    ...
    external_ids) wins over the default priority order — operator decision
    D2, ``primary_channel`` is the notify PREFERENCE and stays singular.

    Pre-M1-B2 this indexed the raw dict and wrapped the result in ``str()``,
    which against the documented ``{channel: [id, ...]}`` shape emitted the
    literal ``"['123']"`` as a chat id — a silent no-delivery on the exact
    path that tells a user their bot has been halted.
    """
    ...
    return _external_ids.resolve_notify_target(
        ...
        priority=_USER_CHANNEL_PRIORITY,
    )
...
def _render_cost_trip_message(
    *,
    ...
    Distinct from the L2 ``_render_trip_message`` — the L1 trip leaves the
    gateway up. Spec §3.6 of the alerts/breakers UX work.

    **Every enforcement action this trip performed is named here.** The
    2026-09-03 incident had three actions fire and only two disclosed: the
    third pinned every remaining turn to the cheapest model, behind a
    mislabeled "selected model unavailable" banner, while the copy
    reassured "You can still talk to me". That sentence is gone; a trip
    now says what it did and what the reader can do about it.

    Under the default ``checkpoint`` action the third line is the
    checkpoint itself — the next conversation turn is held, and the reader
    is offered the two choices (D-CC2/D-CC3). Under an explicit
    ``downgrade-tier`` the third line discloses the downgrade instead of
    hiding it. Any other action names only what it actually did.
    """
    if cap_usd is not None and spend_usd is not None:
        head = (
            f"💰 I've hit today's spending cap — ${spend_usd:.2f} of "
            f"${cap_usd:.2f}."
        )
    else:
        head = "💰 I've hit today's spending cap."

    bits = [head, "My heartbeat is off and my scheduled jobs are paused."]

    if action == "checkpoint":
        bits.append(
            "I'm also holding our next conversation here rather than "
            "answering on a cheaper model."
        )
        if increment_usd:
            bits.append(
                f'Reply "continue" to add ${increment_usd:.2f} to today\'s '
                f'cap and carry on, or "stop" to leave me paused until '
                f"tomorrow."
            )
        else:
            bits.append(
                'Reply "continue" to raise today\'s cap and carry on, or '
                '"stop" to leave me paused until tomorrow.'
            )
        bits.append("Only my owner can say continue.")
    elif action == "downgrade-tier":
        bits.append(
            "I'm also answering on my cheapest model for the rest of the "
            "day, so my replies will be less careful than usual — that is "
            "this pod's configured cap action, not a provider outage."
        )
    elif action:
        bits.append(f"Cap action: {action}.")
    ...
    *,
    ...
    if recipient is None:
        return NotifyResult(
            ...
            # Known-negative, not unknown: there is nowhere to send.
    ...
    try:
        # dispatch_message, not the raw _dispatch_via_openclaw: it tries
        # direct Telegram HTTP first, which never execs the openclaw CLI and
        # so is immune to the CLI's own startup config-validation refusal
        # (breaker-notify-survives-openclaw-config-validation). See
        # dispatcher._dispatch_prefer_telegram_http's docstring.
        from .alerts.dispatcher import dispatch_message
    ...
    try:
        ...
        return NotifyResult(
            ...
            # The post may or may not have left the gateway before the
            # exception — genuinely unknown, so say so.
            delivery="cannot_tell",
        )
    ...
    return NotifyResult(
        ...
        delivery="delivered" if sent else "not_delivered",
    )


def _record_notify_delivery(
    *,
    shared_dir: Path | None,
    breaker_type: str,
    notifications: list[NotifyResult],
) -> None:
    """Append one delivery-ledger row per notification to the breaker log.

    "A cap trip that may not have reached anyone is not enforcement"
    (decision §Consequences). The 2026-09-03 21:18 trip message left no
    delivery record at all, so nobody could say afterwards whether the
    user had been told. Every attempt now lands a row in the breaker
    audit log carrying the tri-state ``delivery`` verdict alongside the
    resolved channel — including the ``cannot_tell`` rows, which are the
    ones worth having.

    Best-effort by construction: this is forensics about a notification
    that was itself best-effort, and it must never affect whether the
    enforcement actions are reported as ok.
    """
    if shared_dir is None or not notifications:
        return
    try:
        from breakers import store as _bstore  # noqa: WPS433
    except Exception as exc:  # noqa: BLE001
        log.warning("breakers.notify: delivery ledger unavailable: %s", exc)
        return
    for n in notifications:
        try:
            _bstore._append_audit(Path(shared_dir), {
                "timestamp": _now_iso(),
                "action": f"notify_{n.action}",
                "scope": n.bot_id,
                "type": breaker_type,
                "delivery": n.delivery,
                "channel": n.channel,
                "attempted": n.attempted,
                "detail": n.error or n.skip_reason,
            })
        except Exception as exc:  # noqa: BLE001
            log.warning(
                "breakers.notify: delivery ledger write failed for %s: %s",
                n.bot_id, exc,
            )
    _observe_notify_config_signal(shared_dir, notifications)


# Substring the openclaw CLI prints (see dispatcher._CONFIG_INVALID_HEADING)
# when it refuses to start because ITS OWN config fails schema validation —
# as opposed to a network/token/timeout failure. Matched case-insensitively
# against a NotifyResult's ``error`` text.
_NOTIFY_CONFIG_INVALID_SUBSTRING = "openclaw config is invalid"


def _is_config_validation_refusal(error: str) -> bool:
    """True when ``error`` is the openclaw CLI refusing to even start
    because the config it resolved fails schema validation — the failure
    mode that left every breaker notice ``not_delivered`` from 2026-09-08
    to 2026-09-20 (breaker-notify-survives-openclaw-config-validation).
    """
    return bool(error) and _NOTIFY_CONFIG_INVALID_SUBSTRING in error.lower()


def _observe_notify_config_signal(
    shared_dir: Path, notifications: list[NotifyResult],
) -> None:
    """A notify path that cannot start is itself a firing condition, not
    just a log line (item 4). Dedup pod-wide — the cause is always the
    SAME file (the ``evolve`` service account's own openclaw.json; see
    ``dispatcher.check_openclaw_cli_can_start``), shared by every bot's
    notify attempt, so a per-bot signature would just mint N copies of the
    same finding. Auto-resolves the moment any notice delivers again —
    never on a mere retry, since a config that still fails schema
    validation will keep producing the same refusal on the next trip.

    Lazy-imports the signals package the same way
    ``openclaw_config_validator`` does: the notify path must keep working
    even if the signals package is broken or the analyzer isn't installed
    in this context.
    """
    try:
        ...
        log.warning("breakers.notify: signals package unavailable: %s", exc)
        return

    refused = [n for n in notifications if _is_config_validation_refusal(n.error)]
    delivered = any(n.delivery == "delivered" for n in notifications)
    signature = schema.make_signature(
        "breakers_enforce", "notify_config_invalid", "pod",
    )
    try:
        if refused:
            detail = refused[0].error
            body = (
                "The breaker's push to the bot's primary user (and the "
                "operator's own alerts, which share the same path) could "
                "not be sent: the openclaw CLI refused to start because "
                "its own config fails schema validation.\n\n"
                f"Detail: {detail}\n\n"
                "Fix: run `openclaw doctor --fix` against the affected "
                "account's own openclaw.json (Maintenance → Health "
                "shows which one — see \"breaker notify path can start\"), "
                "then check Health again. This Signal auto-resolves the "
                "next time a breaker notice delivers."
            )
            store.observe(
                shared_dir,
                signature=signature,
                producer="breakers_enforce",
                type="notify_config_invalid",
                flavor="maintenance",
                severity="alert",
                scope="pod",
                title="Breaker notices cannot be delivered: OpenClaw rejects its config",
                body=body,
                details={
                    "affected_bots": sorted({n.bot_id for n in refused}),
                    "detail": detail,
                },
            )
        elif delivered:
            store.sweep_resolve(
                ...
                producer="breakers_enforce",
                kept_signatures=set(),
                types={"notify_config_invalid"},
                reason="auto-resolve: a breaker notice delivered again",
            )
    except Exception as exc:  # noqa: BLE001
        log.warning("breakers.notify: signal observe/resolve failed: %s", exc)
...
def _scope_resolves(scope: str, network: dict) -> bool:
    """True if ``scope`` still names something enforcement can act on.

    Asks the same function that rejects it, so the two can't drift.
    """
    try:
        _resolve_target_bots(scope, network)
    except ValueError:
        return False
    return True
...
def _enforce_full(
    *,
    ...
    ``shared_dir`` is optional (kept ``None``-safe for callers/tests that
    omit it, mirroring ``enforce_trip``/``enforce_reset``'s own contract)
    but every production caller supplies it — without it, notify delivery
    for this breaker type is never written to the delivery ledger nor
    observed as a Signal (``_record_notify_delivery``), which is the gap
    that let every L2 notify failure go unrecorded before
    ...
    if action == "trip":
        ...
        _record_notify_delivery(
            shared_dir=shared_dir, breaker_type="full",
...
def _clear_spend_cap_flag_one_bot(
    *,
    bot_id: str,
    shared_dir: Path,
    dry_run: bool,
) -> CostEnforcePerBot:
    """Clear today's spend-cap enforcement flag as part of an L1 reset.

    The flag (``{shared_dir}/spend-caps/<bot>-<YYYY-MM-DD>.json``) is a
    SEPARATE store from the breaker file — it is what the plugin's
    ``ModelRouter.isSpendCapActive`` reads to force the fast-rung
    downgrade. Live incident 2026-07-31: ``breaker reset <bot> cost``
    deleted the breaker file but left the flag at ``cleared: false``,
    so routing stayed pinned to ``fast`` until
    ``spend_caps.clear_enforcement`` was run by hand. One operator
    reset must clear BOTH stores (#3498 single-resolver precedent);
    the clear goes through ``spend_caps.clear_enforcement`` — never a
    bespoke rewrite of the flag file.
    """
    try:
        import spend_caps  # evolve-analyzer package (admin depends on it)
    except ImportError as exc:
        return CostEnforcePerBot(
            bot_id=bot_id, action="reset", ok=False,
            detail=f"spend_caps import failed; enforcement flag NOT cleared: {exc}",
        )
    flag = spend_caps.get_active_enforcement(shared_dir, bot_id)
    if flag is None:
        return CostEnforcePerBot(
            bot_id=bot_id, action="reset", ok=True, no_op=True,
            no_op_reason="no active spend-cap enforcement flag",
        )
    if dry_run:
        return CostEnforcePerBot(
            bot_id=bot_id, action="reset", ok=True,
            detail=(
                "dry-run: would clear spend-cap enforcement flag "
                f"(action={flag.get('action')!r})"
            ),
        )
    if not spend_caps.clear_enforcement(shared_dir, bot_id):
        return CostEnforcePerBot(
            bot_id=bot_id, action="reset", ok=False,
            detail="spend-cap flag active but clear_enforcement failed",
        )
    return CostEnforcePerBot(
        bot_id=bot_id, action="reset", ok=True,
        detail=(
            "spend-cap enforcement flag cleared "
            f"(was action={flag.get('action')!r})"
        ),
    )
...
def _enforce_cost(
    *,
    ...
    actions land, post a brief message to each affected bot's primary
    user channel naming every action this trip performed. Best-effort:
    failures are recorded but never block the enforcement actions — and
    each attempt's tri-state delivery verdict is ledgered.

    ``cap_action`` / ``cap_usd`` / ``spend_usd`` / ``increment_usd`` are
    the structured inputs to that message; they come from the cap-driven
    trip path. Omitted by the manual CLI/API callers, which get the
    generic wording.
    """
    import time
    t0 = time.monotonic()
    ...
    for bot_id, _user in targets:
        if action == "trip":
            ...
            per_bot_cost.append(res)
            # Step 3: clear today's spend-cap enforcement flag so the
            # plugin's ModelRouter stops forcing the fast rung. The flag
            # lives in a different store than the breaker file and does
            # NOT expire on its own until midnight.
            res_cap = _clear_spend_cap_flag_one_bot(
                bot_id=bot_id, shared_dir=shared_dir, dry_run=dry_run,
            )
            per_bot_cost.append(res_cap)
        else:
            raise ValueError(
    ...
    if action == "trip":
        msg = _render_cost_trip_message(
            ...
            action=cap_action, cap_usd=cap_usd, spend_usd=spend_usd,
            increment_usd=increment_usd,
        )
        notifications = _notify_user_channels(
            bots=targets, action="trip", network=network,
            message=msg, dry_run=dry_run,
    ...
    # Assert delivery, or record that we cannot tell. Written for BOTH
    # directions: "did the user ever hear that I came back?" is the same
    # question as "did they hear that I stopped?".
    _record_notify_delivery(
        shared_dir=shared_dir, breaker_type="cost",
        notifications=notifications,
    )
...
def enforce_trip(
    *,
    ...
    for L2 trips; L1 trips echo them too, alongside the structured
    ``cap_action`` / ``cap_usd`` / ``spend_usd`` / ``increment_usd`` the
    cap-driven path supplies so the trip copy can name every action it
    performed and the two choices the reader has.
    ...
    if breaker_type == "cost":
        ...
        result, cost_per_bot = _enforce_cost(
            ...
            home_override=home_override,
            reason=reason, expires_at_iso=expires_at_iso,
            cap_action=cap_action, cap_usd=cap_usd, spend_usd=spend_usd,
            increment_usd=increment_usd,
        )
...
def enforce_reset(
    *,
    ...
    the stash on each affected bot, kickstart the gateway, and clear
    today's spend-cap enforcement flag (the ModelRouter fast-rung
    downgrade input — see ``_clear_spend_cap_flag_one_bot``). No-op
    when no stash exists and no flag is active.
...
# ─────────────────────────────────────────────────────────────────────────────
# Reset + enforcement, in the one order that stays retryable
# ─────────────────────────────────────────────────────────────────────────────


@dataclass
class ResetOutcome:
    """Result of :func:`reset_and_enforce`.

    ``prior`` is the ``breakers.store.BreakerRecord`` that was cleared
    (``None`` when nothing was tripped); ``enforce`` is the
    :class:`EnforceResult` from the bring-up (``None`` when nothing was
    tripped, because there is nothing to bring back up).
    """

    was_tripped: bool
    prior: Any | None = None
    enforce: EnforceResult | None = None
    #: Set when the breaker was cleared WITHOUT bring-up because the
    #: scope no longer resolves (bot gone from network.json). ``enforce``
    #: is None in that case and the caller should report partial success.
    enforce_error: str | None = None


def record_reactivation_acceptance(
    *, shared_dir: Path, scope: str, breaker_type: str,
) -> None:
    """Write the "count from now" marker for a reactivated bot's cost breaker.

    Scope and type are both gates:

    * **cost breakers only** (``cost`` / ``cost_l2``). A ``full`` halt is not
      a spend judgement; accepting a spend window on its reset would silently
      raise the cost ceiling for a reason that had nothing to do with cost.
    * **per-bot scopes only.** A pod-wide trip has no per-bot spend total to
      accept, and inventing one per member would accept spend on bots the
      operator never looked at.

    Best-effort by construction: every failure path leaves NO marker, and no
    marker means the pre-existing behaviour (measure the raw window). The
    failure direction is therefore "might re-trip", never "silently stops
    enforcing" — an unreadable spend figure must not buy an open-ended pass.
    Called from the operator-driven reset paths only — ``reset_and_enforce``
    (web API, evo tool, admin-bot route) and the ``evolve-admin breaker
    reset`` CLI. The TTL reaper in ``heal._reap_expired_cost_breakers`` calls
    ``enforce_reset`` directly and so writes no marker: a breaker that timed
    out on its own is not an operator accepting anything.
    """
    if breaker_type not in ("cost", "cost_l2") or scope == "pod":
        return
    try:
        ...
        return
    try:
        spend = _spend_caps.get_today_spend(shared_dir, scope)
    except Exception as exc:  # noqa: BLE001
        log.warning(
            "breaker reset %s/%s: today's spend unreadable (%s) — no "
            "acceptance marker written; the next tick measures the raw "
            "window and may re-trip", scope, breaker_type, exc,
        )
        return
    if spend is None:
        log.warning(
            "breaker reset %s/%s: live spend discovery failed — no "
            "acceptance marker written (the next tick measures the raw "
            "window and may re-trip)", scope, breaker_type,
        )
        return
    try:
        marker = _spend_caps.write_accepted_through(
            shared_dir, scope, spend_usd=spend,
        )
    except Exception as exc:  # noqa: BLE001
        log.warning(
            "breaker reset %s/%s: acceptance marker write failed (%s)",
            scope, breaker_type, exc,
        )
        return
    log.info(
        "breaker reset %s/%s: accepted $%.2f of today's spend at %s — "
        "further evaluations measure from there",
        scope, breaker_type, marker.accepted_usd, marker.accepted_at,
    )


def reset_and_enforce(
    *,
    shared_dir: Path,
    scope: str,
    breaker_type: str,
    ...
    """Clear a breaker and bring the bot back up, in a retryable order.

    A breaker reset has two halves that must not diverge:

      1. **bring-up** — restore the heartbeat + exec-approval stashes,
         kickstart the gateway, clear today's spend-cap enforcement flag
         (L1), or ``launchctl bootstrap`` the gateway (L2);
      2. **state clear** — delete the breaker file.

    Doing (2) first is how the 2026-09-04 reset incident produced a bot
    that no monitor flagged: the file was gone (so the UI hid the
    Reactivate button and the runner reported no trip) while the bot was
    still heartbeat-less and pinned to the fast rung, and a retry
    short-circuited on ``prior is None`` without ever reaching (1).

    So enforcement runs FIRST. If it raises, the breaker file is left
    exactly as it was — the operator still sees an active breaker and
    can retry out of it. This is the same fail-safe direction
    ``heal._reap_expired_cost_breakers`` already takes for the TTL reap.

    A non-raising ``ok=False`` enforcement (e.g. a launchctl rc the
    operator has to chase) still clears the state, preserving the
    established partial-success contract: the caller reports HTTP 207
    with both the reset record and the enforce detail.

    ``prior is None`` (nothing tripped) skips enforcement entirely — it
    would post a "background work is back" message to the bot's user
    channel for a reset that cleared nothing.

    **One carve-out to "a raise keeps the file".** ``enforce_reset``
    raises ValueError for a scope it cannot resolve, and for a bot that
    has left network.json that is permanent, not transient: retrying can
    never succeed, so keeping the file would make the breaker
    un-resettable forever — and heal's TTL reaper cannot help, because it
    only reaps EXPIRED trips and an indefinite one never expires. That
    residue was seen live on 2026-07-31 (bot "ledger"). So when the scope
    is genuinely gone we clear anyway and report the failure through
    ``enforce_error``, reaching the same terminal state
    ``heal._clear_defunct_breaker_residue`` does. A ValueError for a
    scope that DOES still resolve is left to propagate — there the
    breaker must stay put and stay retryable.
    """
    from breakers import store as _bstore  # type: ignore[import]

    prior = _bstore.read_trip(shared_dir, scope, breaker_type)
    if prior is None:
        # store.reset validates scope/type and returns None without
        # touching disk — call it so a bad scope/type still raises
        # ValueError (the caller's 400) instead of reporting a silent
        # "nothing was tripped".
        _bstore.reset(
            shared_dir=shared_dir, scope=scope, breaker_type=breaker_type,
            initiated_by=initiated_by, reason=reason,
        )
        return ResetOutcome(was_tripped=False)

    # 1) Bring-up first. A raise here leaves the breaker file in place so
    #    the operator can retry — except for a scope that is gone for
    #    good, where retrying can never work (see the docstring).
    enforce_error: str | None = None
    try:
        enforce_result: EnforceResult | None = enforce_reset(
            scope=scope, breaker_type=breaker_type, network=network,
            shared_dir=shared_dir, dry_run=dry_run, home_override=home_override,
        )
    except ValueError as exc:
        if _scope_resolves(scope, network):
            # Not a decommissioned bot — keep the breaker, stay retryable.
            raise
        enforce_error = str(exc)
        enforce_result = None

    # 2) State clear. Best-effort audit inside; see store.reset's
    #    commit-point contract.
    if not dry_run:
        cleared = _bstore.reset(
            shared_dir=shared_dir, scope=scope, breaker_type=breaker_type,
            initiated_by=initiated_by, reason=reason,
        )
        prior = cleared or prior
        # 3) Accept the window. Bringing a bot back on purpose says "the
        #    spend that tripped this is OK, or is over" — so the next
        #    evaluation must measure from HERE, not from midnight. Without
        #    the marker the very next tick re-reads the same over-cap day
        #    and re-trips, which is what the operator hit three times on
        #    2026-09-07. Deliberately AFTER the clear: the marker is a
        #    consequence of a reset that actually happened.
        record_reactivation_acceptance(
            shared_dir=shared_dir, scope=scope, breaker_type=breaker_type,
        )

    return ResetOutcome(
        was_tripped=True, prior=prior, enforce=enforce_result,
