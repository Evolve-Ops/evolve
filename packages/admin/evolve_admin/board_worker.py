...
def _act_needs_approval(
    ...
    return _RUNG_ORDER[granted] < _RUNG_ORDER[RUNG_ACT_WITH_APPROVAL]
...
def _dispatch_one(
    ...
    *, model_client: ModelClient,
    ...
    if kind == "assigned":
        ...
        board_store.set_delegation_progress(
            shared_dir, bot_id, card_id, state="accepted", actor=bot_id)
        _record_learning_event(shared_dir, bot_id, card, "accepted", actor=bot_id)
...
def _drain_queued(
    ...
    *, model_client: ModelClient, results: list[dict[str, Any]],
) -> None:
    ...
    session" — this is what un-queues it.
    """
    ...
    for qp in sorted(d.glob("*.jsonl")):
        card_id = qp.stem
        if _is_busy(shared_dir, bot_id, card_id):
            continue
...
def poll_once(
    ...
    for bot_id in sorted((network.get("bots") or {}).keys()):
        _drain_queued(shared_dir, network, bot_id, model_client=model_client, results=results)
        events_dir = board_store.board_dir(Path(shared_dir), bot_id) / "events"
        if not events_dir.is_dir():
            continue
