...
`internal/spec-add-bot-wizard-2026-05-28.md`). It packages every step
...
def _dscl_delete_user(user: str, *, remove_home: bool = True) -> None:
    ...
    # crash. See internal/spec-evo-account-separation-2026-05-25.md.
...
def _default_catalog_for_provider(
    ...
    # The registry's tier0 slot is no longer seeded — the judge role it fed
    # was collapsed (design-judge-role-collapse-2026-08-21 §5.4).
...
def seed_model_config_if_empty(
    ...
    *,
    ...
    # The former tier0 (judge) seed is gone with the judge role (design-
    # judge-role-collapse-2026-08-21 §5.4): cross-vendor checking is now
    # derived from the standard chain at the call site
    # (primary_bot.resolve_cross_vendor) — a second credentialed provider's
    # models already land on the tier chains as fallback entries, which is
    # exactly where the derivation looks. No dedicated judge slot to seed.

    # Write catalog + tiers in one shot. oc_full_config_set accepts
    # any subset of {catalog, tiers, rungs, roles, routing, fallbackMode,
    # tierCascade, cascade}.
    #
    ...
    # Seed the rungs/roles shape, NOT the legacy tierN shape
    # (spec-model-rungs-and-roles-2026-06-09; #3566).
    #
    # `_default_catalog_for_provider` still speaks tier1/tier2/tier3 internally
    # — that is where the role-floor logic lives, and rewriting it would be a
    # much larger change for no behavioural gain. So we
    # keep building the legacy dict and run it through the SAME pure transform
    # the migrator uses before writing. The seed therefore lands exactly what
    # `evolve-admin migrate-model-roles --apply` would produce for this bot,
    # which is what makes the conversion provably behaviour-preserving.
    #
    # Why this matters: before #3566 the seed wrote a legacy `tiers` file, and
    # oc_model's writer is deliberately shape-preserving (it keeps a legacy
    # file legacy on a partial `tiers` write), so a bot seeded here stayed on
    # the deprecated shape FOREVER unless an operator hand-ran the migrator.
    # Every bot provisioned after the June migration was born un-migrated and
    # kept firing the ModelRouter DEPRECATION line. The legacy-shape fallback
    # in ModelRouter.ts cannot be removed while a writer still mints the shape
    # it exists to absorb.
    #
    # KNOWN BEHAVIOUR CHANGE (intentional): `primary_bot.bot_has_custom_tiers`
    # decides Use-pod-defaults vs Custom purely on `rungs` presence, so a
    # freshly seeded bot now reads as Custom where it used to read as
    # "follows the pod". Routing is UNCHANGED — the plugin already synthesized
    # per-bot rungs from the legacy `tiers` block, so the bot was always
    # running its own seeded allocations; only the admin-side label was wrong.
    # This makes the label agree with what the router actually does.
    #
    # But the flip DOES change auto-upgrade governance, and that has to be
    # compensated explicitly — see the autoUpgrade seed below.
    from .migrate_model_roles import migrate_evolve_tiers

    _seed_shape, _ = migrate_evolve_tiers({"tiers": tiers})
    # `_default_catalog_for_provider` guarantees at least one ladder tier with
    # models (the `not primary or not catalog` early-return above fires
    # otherwise), and the transform emits a rung for every non-empty tier, so
    # `rungs` is always populated here. Assert rather than branch: a silent
    # fallback to the legacy `tiers` key would reintroduce exactly the shape
    # this function was fixed to stop minting.
    if not _seed_shape.get("rungs"):  # pragma: no cover — unreachable
        return {
            "ok": False, "seeded": False,
            "reason": (
                f"tier→rung transform produced no rungs for {bot_id} from "
                f"provider {provider!r} — refusing to seed the deprecated "
                f"legacy tier shape (#3566)"
            ),
            "primary": primary, "catalog_count": len(catalog), "provider": provider,
        }
    updates: dict = {
        "catalog": catalog_ids,
        "rungs": _seed_shape["rungs"],
        "roles": _seed_shape.get("roles") or {},
    }

    # Lifecycle rule 1 (spec-model-auto-upgrade §Scope), mirrored from the
    # "Customize this bot" route in routes_admin_config.py: a bot that carries
    # its own rungs is Custom, and `model_auto_upgrade.bot_policy` does NOT
    # inherit the pod's `enabled` for a Custom bot — a Custom bot that has
    # never set the key resolves to the CODE default (false). So writing rungs
    # without also seeding the pod's auto-upgrade block would silently switch a
    # freshly provisioned bot from "rides the latest version, like the pod" to
    # "auto-upgrade off", which is precisely what that lifecycle rule exists to
    # prevent. Pre-#3566 the legacy seed dodged this only by accident: it wrote
    # no `rungs`, so the bot read as Use-pod-defaults even though the router was
    # already using its per-bot allocations.
    #
    # No pod block → seed nothing (the bot resolves to the code default, same
    # as the pod does).
    _pod_auto_upgrade = (network.get("models") or {}).get("autoUpgrade")
    if isinstance(_pod_auto_upgrade, dict) and _pod_auto_upgrade:
        updates["autoUpgrade"] = dict(_pod_auto_upgrade)
    ...
    #
    # The seed also writes evolve-tiers.json (tiers + cascade, and the pod
    # auto-upgrade carry the seam injects on a rungs write), so the
    # ``tiers:<key>`` names the writer reported ride along — otherwise the
    # openclaw.json half is explained and the tiers half alarms forever
    # (spec-delta-digest-audit-noise D3).
    ...
    }, oc_keys=_tier_write_oc_keys(write_result, {"agents"}))
