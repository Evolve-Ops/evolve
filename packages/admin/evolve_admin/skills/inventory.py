...
_PLUGIN_DISPLAY: dict[str, dict[str, str]] = {
    ...
    # internal/spec-zoom-skill-2026-06-06.md.
...
# the class of mismatch #3219 had to fix. This table is the OPENCLAW.JSON
# half of that set: providers whose key path sits inside the plugin entry's
# own `config` block. The LLM half is verified a different way — their keys
# live in OC's per-agent runtime store, and `_runtime_keyed_providers` below
# reads it — so INLINE_KEY_PROVIDERS is now a SUPERSET of this table's keys,
# not equal to it.
#
# Keep this table narrow. Only add a provider when the key path is stable and
...
def _runtime_keyed_providers(home: "Path", oc: dict[str, Any]) -> "frozenset[str] | None":
    """LLM providers with a key in OC's PER-AGENT runtime store, or None.

    ``None`` means "could not look" — an unimportable web package (this
    module is used by CLI paths that never touch Flask) or a scan that
    raised. Callers treat None as "don't cry wolf" and fall back to the
    enabled-only rule, which is the pre-2026-09 behaviour.

    An empty frozenset is the OPPOSITE answer: we looked and found nothing.
    """
    try:
        from ..web.oc_agent_keys_io import scan_oc_dir_runtime_llm_keys
        located = scan_oc_dir_runtime_llm_keys(home / ".openclaw", oc)
    except Exception:  # noqa: BLE001 — never fail inventory on detection
        return None
    return frozenset(hit.provider for hit in located)


def _inline_key_present(
    name: str,
    cfg: dict[str, Any],
    oc: dict[str, Any],
    runtime_keyed: "frozenset[str] | None" = None,
) -> bool:
    """True if an ``INLINE_KEY_PROVIDERS`` plugin has its credential set.
    ...
    LLM providers delegate to *runtime_keyed*, the set the Credentials tab's
    ``OcAgentCatalogKeyProbe`` builds from the same scan — so "enabled and
    keyed" means the same thing on both pages. ``None`` (we could not look)
    returns True: the enabled-only rule, unchanged.
    """
    if name == "brave":
        try:
    ...
    if _is_runtime_store_provider(name):
        if runtime_keyed is None:
            return True  # could not look → don't cry wolf
        return name in runtime_keyed
...
def _is_runtime_store_provider(name: str) -> bool:
    try:
        from ..web.credentials_oc import RUNTIME_STORE_KEY_PROVIDERS
        return name in RUNTIME_STORE_KEY_PROVIDERS
    except Exception:  # noqa: BLE001
        return False
...
# internal/design/paste-token-skills-future-2026-05-30.md and
...
# (internal/openclaw-coverage-audit-2026-06-04.md, PR #2123) caught the inverse-D
...
def _resolve_plugin_status(
    ...
    - Exception 2: providers in ``INLINE_KEY_PROVIDERS`` are ones Evolve can
      POSITIVELY VERIFY, so the false-negative risk the ``enabled``-only rule
      protects against doesn't apply — the key is right there to check.
      Enabled + no key → "missing_config".
      ...
      The LLM providers joined that set on 2026-09-04. The docstring above
      still says their keys live "in env, auth.profiles, or channels.*" —
      that was a guess, and a scan of the live pod refuted it: they live in
      OC's per-agent runtime store, which *runtime_keyed* reads. Passing
      ``None`` for it restores the old enabled-only answer exactly, which is
      what every caller that cannot reach the store gets.
    """
    ...
    if enabled and name.lower() in _inline_key_providers():
        if not _inline_key_present(name.lower(), cfg, oc, runtime_keyed):
...
def get_bot_skills(
    ...
    # One scan of the per-agent runtime store serves every LLM plugin row
    # below — the Credentials tab runs the same scan through its probe, so
    # "enabled + keyed" means the same thing on both pages.
    runtime_keyed = _runtime_keyed_providers(home, oc)
    ...
    for name, cfg in entries_block.items():
        ...
        status = _resolve_plugin_status(name, cfg, oc, runtime_keyed)
...
_POD_WIDE_LOCAL_SKILLS: tuple[tuple[str, str, str], ...] = (
    ...
    # see internal/skills-deep-audit-2026-05-30.md). The probe-as-only-consumer
