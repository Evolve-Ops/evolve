...
def _assert_no_pod_infra_collision(bot_id: str, labels: "set[str] | list[str]") -> None:
    ...
    if bad:
        raise ValueError(
            ...
            "lifecycle. See internal/spec-evo-account-separation-2026-05-25.md."
        )
...
def _credential_section(bot_id: str, network: dict[str, Any]) -> list[str]:
    """Markdown lines listing every credential still live in the bot's home.

    Delegates to ``bot_credential_inventory`` — the SAME module
    ``orphaned_bot_account_monitor`` uses, deliberately. If the closure summary
    kept its own list, the two would drift, and the failure mode of that drift
    is a summary that promises "nothing to revoke" while the monitor keeps
    reporting live tokens (or vice versa). One inventory, two readers.

    Never raises: a retirement must not fail because a credential probe hit
    EACCES. An unreadable location is reported as unreadable — which is the
    honest answer and the one that keeps the operator looking — rather than
    being dropped into an implicit "clean".
    """
    try:
        import bot_credential_inventory as _cred  # type: ignore  # analyzer pkg
    except ImportError as exc:  # analyzer package not importable (early boot)
        return [
            f"_Credential inventory unavailable ({exc}). Check "
            f"`{bot_home(bot_id, network)}/.openclaw/openclaw.json` and "
            f"`~/.ssh` by hand before considering this bot fully closed._"
        ]
    try:
        inv = _cred.collect(bot_home(bot_id, network))
    except Exception as exc:  # noqa: BLE001 — see docstring: never fail retire
        return [
            f"_Credential inventory failed ({type(exc).__name__}: {exc}). "
            "Check the bot's openclaw.json and ~/.ssh by hand before "
            "considering this bot fully closed._"
        ]
    return _cred.as_markdown(inv).splitlines()
...
def generate_closure_summary(
    ...
    # High in the document on purpose: this is the only section that asks the
    # operator to DO something, and it has a deadline in a way the rest of the
    # summary does not. Retiring a bot stops its services and takes it off the
    # roster, but it cannot revoke a Telegram bot token (a BotFather action) or
    # un-authorize an SSH key — those stay valid at their source indefinitely.
    # Before this section existed, nothing told the operator they were still
    # live: the 2026-08-02 finding was a bot retired cleanly on 2026-06-15 whose
    # 46-char Telegram token and 48-char gateway token were still on disk seven
    # weeks later, with no record anywhere that they needed revoking.
    lines.append("## Credentials still live — revoke these")
    lines.append("")
    lines.append(
        "Retirement preserves this bot's account and home, so every credential "
        "below is still valid wherever it was issued. Evolve cannot revoke "
        "them for you — a channel token is revoked at the platform."
    )
    lines.append("")
    lines.extend(_credential_section(bot_id, network))
    lines.append("")
    ...
    lines.append("")
    lines.append(
        f"- The host user account `{cfg['macos_user']}` was **not** deleted; "
        f"files are preserved in `{cfg['home_dir']}/`. That includes the "
        "credentials listed above — until they are revoked at their source, "
        "the account is inert but its tokens are not. "
        f"`sudo evolve-admin delete-bot {bot_id}` removes the account and "
        "home irreversibly once you no longer need either."
    )
    lines.append(
