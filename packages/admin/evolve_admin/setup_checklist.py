...
def _detect_ai_optim_tiers(
    ...
    so without it the bot can't take a turn at all. Other tiers (tier1
    power, tier3 grunt) have sensible defaults and aren't required
    here. Adding a second provider is a recommendation (it buys
    failover and cross-vendor checking — see the secondary_llm item),
    not a gate.
...
def _detect_secondary_llm(
    ...
    Why it's on the checklist: a second provider buys you failover when the
    primary is rate-limited or down, and a cross-vendor option for
    self-checking (the ``resolve_cross_vendor`` derivation — a second
    provider's models become fallback entries on the tier chains).
...
def _detect_gallery_app(
    ...
    field-PRESENCE probe, not an identity read. ``is_gallery_app()`` asks
    "did this come from the gallery?", and the answer is carried by ``pkg_id``
    being set at all. A resolver returns a string and would answer a different
    question: every manifest resolves to SOME id, so a resolved check would
    report every bot as having a gallery app and the checklist item would be
