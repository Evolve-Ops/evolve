...
def _check_oc_config_valid_under_installed(
    ...
    the second is not reported at all, because a validator that cannot run is
    news about the validator.
    """
    cat = "oc_runtime"
    try:
        from .openclaw_config_validator import validate_bot_openclaw_json
    except Exception:  # noqa: BLE001 — no validator, no check
        return
    for bot_id in members:
        try:
            ...
            continue
        if val_err:
            continue  # could not ask — not the same as invalid
        if valid:
            report.checks.append(CheckResult(
...
def _check_oc_channel_packages(
    ...
    if not installed:
        return  # cannot know the line — say nothing rather than guess
    for bot_id in members:
        try:
