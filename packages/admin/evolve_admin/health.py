...
def _repo_puller_verdict(
    log_path: Path,
    stale_after_seconds: int = 90 * 60,
) -> Verdict:
    """The pure, fixture-testable core of ``_check_repo_puller_freshness``
    (D-CS7). Names the log file + its mtime as the subject examined for
    every branch except the two where nothing could be read at all — those,
    and only those, are ``unknown``. ``ok``/``broken`` here are a narrower
    axis than the CheckResult WARN/FAIL below: several ``broken`` branches
    (stale, empty) still surface as WARN in the operator UI, because they
    are read here as "we know what we looked at and it isn't right", not
    as urgency.
    """
    ...
    try:
        ...
        return Verdict.unknown(f"could not stat {log_path}: {e}", evidence={"stat_error": True})

    subject = f"{log_path}@{int(mtime)}"
    age = _time.time() - mtime
    if age > stale_after_seconds:
        return Verdict.broken(
            subject, "log not written recently — likely unloaded",
            evidence={"stale": True, "age_seconds": age},
        )
    ...
    try:
        ...
        return Verdict.unknown(
            f"could not read {log_path}: {e}", subject=subject, evidence={"read_error": True},
        )

    lines = [l for l in tail.splitlines() if l.strip()]
    if not lines:
        return Verdict.broken(
            subject, "log is empty — daemon hasn't written anything",
            evidence={"empty": True, "age_seconds": age},
        )
    ...
    if last_error and not has_recent_success:
        return Verdict.broken(
            subject, "no successful pull in recent log",
            evidence={"wedged": True, "last_error": last_error, "age_seconds": age},
        )

    return Verdict.ok(subject, evidence={"age_seconds": age})
...
def _check_repo_puller_freshness(
    ...
    convention. Evidence-reading lives in ``_repo_puller_verdict`` (D-CS7);
    this function only translates its Verdict into the CheckResult shape.
    """
    cat = "repo_puller"
    ...
    v = _repo_puller_verdict(log_path, stale_after_seconds)

    if v.is_unknown:
        if v.evidence.get("missing"):
            report.checks.append(CheckResult(
            ...
            report.checks.append(CheckResult(WARN, cat, "repo-puller:log", v.reason or "unknown"))
        return

    if v.is_broken and v.evidence.get("stale"):
    ...
    if v.is_broken and v.evidence.get("empty"):
        report.checks.append(CheckResult(
    ...
    if v.is_broken and v.evidence.get("wedged"):
        last_error = v.evidence["last_error"]
        try:
    ...
    age = v.evidence.get("age_seconds", 0)
    report.checks.append(CheckResult(
...
def _check_repo_ownership(
    ...
    triggers it. Fix: chown the deploy checkout to
    ``evolve:<profile.ops_group>`` — the OPERATOR group, never the
    all-bots group, because the venv is editable-installed from this
    checkout (2026-09-23; see ``deploy.ensure_pod_perms``) — (the
    ...
    if bad:
        ...
        report.checks.append(CheckResult(
            ...
            fix_cmd=(f"sudo {get_profile().chown} -R "
                     f"evolve:{get_profile().ops_group} {repo_root}"),
            fix_args={"action": "chown", "path": str(repo_root),
                      "owner": f"evolve:{get_profile().ops_group}",
                      "recursive": True},
        ))
        return
...
# ── OpenClaw runtime coherence (chip: oc-upgrade-is-a-guarded-change item 5) ──
#
# After the 2026-09-07 upgrade every one of these was false on all nine bots
# and nothing said so: configs the new binary rejected, per-bot channel
# packages still on the old release line, and a runtime nobody had validated
# Evolve against. Each is cheap, each is a fact the pod already knows, and each
# would have turned a day of recovery into a line on the Dashboard.


def _check_oc_config_valid_under_installed(
    report: HealthReport, members: list[str], network: dict | None = None,
) -> None:
    """Does each bot's config still validate under the OC that is installed?

    WARN, not FAIL: an invalid config is usually survivable (OC ignores the
    keys it does not know) right up until the moment it is not, and a FAIL here
    on a pod that is running fine trains people to ignore the surface. The
    distinction that matters is "we know it is invalid" vs "we could not ask" —
    D-CS7: the second is still WARN, never silence, because a validator that
    cannot run is itself news the operator needs, not an absence.
    """
    cat = "oc_runtime"
    try:
        from .openclaw_config_validator import validate_bot_openclaw_json
    except Exception as exc:  # noqa: BLE001 — no validator, still a WARN, never nothing
        report.checks.append(CheckResult(
            WARN, cat, "config_valid_under_oc",
            f"Could not check config validity under the installed OpenClaw: {exc}",
        ))
        return
    for bot_id in members:
        try:
            valid, issues, val_err = validate_bot_openclaw_json(bot_id, network or {})
        except Exception as exc:  # noqa: BLE001
            report.checks.append(CheckResult(
                WARN, cat, f"{bot_id}:config_valid_under_oc",
                f"Could not check config validity: {exc}",
            ))
            continue
        if val_err:
            report.checks.append(CheckResult(
                WARN, cat, f"{bot_id}:config_valid_under_oc",
                f"Could not check config validity: {val_err}",
            ))
            continue
        if valid:
            report.checks.append(CheckResult(
                PASS, cat, f"{bot_id}:config_valid_under_oc",
                "Config validates under the installed OpenClaw",
            ))
            continue
        named = ", ".join(str(i) for i in (issues or [])[:4]) or "see `openclaw config validate`"
        report.checks.append(CheckResult(
            WARN, cat, f"{bot_id}:config_valid_under_oc",
            f"Config is invalid under the installed OpenClaw: {named}",
            fix_cmd=(
                "Run the upgrade preflight (Maintenance → OpenClaw runtime) to see "
                "what would change, then `sudo evolve-admin deploy --bot "
                f"{bot_id}`. Do not run `openclaw doctor --fix` unattended — it "
                "rewrites model refs."
            ),
        ))


# ── Breaker notify path (chip: breaker-notify-survives-openclaw-config- ────
# validation, item 5) ─────────────────────────────────────────────────────
#
# Distinct from _check_oc_config_valid_under_installed above: that check
# validates each BOT's own openclaw.json, run AS the bot (sudo -H -u <bot>)
# — the right shape for "will this bot's gateway start". This check instead
# asks "can the notify path itself start" by running the no-send validator
# EXACTLY the way the real send call resolves config: no sudo, no HOME
# override — which for every caller in this codebase means it validates
# the ``evolve`` service account's own openclaw.json (the file that failed
# silently for two weeks, 2026-09-08 to 2026-09-20).


def _check_breaker_notify_path(report: HealthReport) -> None:
    """FAIL when the openclaw CLI itself cannot start — the failure mode
    that left every breaker notice ``not_delivered`` and nobody the wiser.

    WARN (not FAIL) for "unknown": a validator that cannot run at all is
    its own news, distinct from "we ran it and it's broken" (D-CS7), but
    it does not carry the same "your spend safety net is silently mute"
    urgency a confirmed refusal does.
    """
    cat = "oc_runtime"
    try:
        from .alerts.dispatcher import check_openclaw_cli_can_start
    except Exception as exc:  # noqa: BLE001
        report.checks.append(CheckResult(
            WARN, cat, "breaker_notify_path_can_start",
            f"Could not check the breaker notify path: {exc}",
        ))
        return
    try:
        state, detail = check_openclaw_cli_can_start()
    except Exception as exc:  # noqa: BLE001
        report.checks.append(CheckResult(
            WARN, cat, "breaker_notify_path_can_start",
            f"Could not check the breaker notify path: {exc}",
    ...
    if state == "ok":
        report.checks.append(CheckResult(
            PASS, cat, "breaker_notify_path_can_start",
            "Breaker notices can reach the openclaw CLI (its config validates)",
        ))
    elif state == "broken":
        report.checks.append(CheckResult(
            FAIL, cat, "breaker_notify_path_can_start",
            f"Breaker notices cannot be delivered: the openclaw CLI refuses "
            f"to start — {detail}",
            fix_cmd=(
                "Run `openclaw doctor --fix` against the affected account's "
                "own openclaw.json, then re-run health."
            ),
        ))
    else:
        report.checks.append(CheckResult(
            WARN, cat, "breaker_notify_path_can_start",
            f"Could not check the breaker notify path: {detail}",
        ))


def _check_oc_channel_packages(
    report: HealthReport, members: list[str], network: dict | None = None,
) -> None:
    """Do per-bot ``@openclaw/*`` packages match the installed runtime line?

    The 2026-09-07 symptom was ``Package subpath
    './plugin-sdk/channel-streaming' is not defined`` — a channel dead until
    reinstalled, with nothing on any surface saying why. Compared on the
    release line, which is the granularity the compatibility contract's own
    ``channel-plugin-versions-match`` check uses.
    """
    cat = "oc_runtime"
    try:
        from .upstream_version import installed_package_version
        from .oc_preflight import channel_packages_stale
    except Exception as exc:  # noqa: BLE001 — WARN, never nothing (D-CS7)
        report.checks.append(CheckResult(
            WARN, cat, "channel_packages",
            f"Could not check channel package versions: {exc}",
        ))
        return
    installed = installed_package_version()
    if not installed:
        # Cannot know the line to compare against — still a WARN naming
        # that fact, not silence (D-CS7): "we could not check" is itself
        # something the operator needs to know, not the same as "clean".
        report.checks.append(CheckResult(
            WARN, cat, "channel_packages",
            "Could not determine the installed OpenClaw version, so per-bot "
            "channel packages could not be checked against it",
        ))
        return
    for bot_id in members:
        try:
            stale = channel_packages_stale(_bot_home(bot_id), installed)
        except Exception as exc:  # noqa: BLE001
            report.checks.append(CheckResult(
                WARN, cat, f"{bot_id}:channel_packages",
                f"Could not check channel packages: {exc}",
            ))
            continue
        if not stale:
            report.checks.append(CheckResult(
                PASS, cat, f"{bot_id}:channel_packages",
                f"Channel packages match the installed runtime ({installed})",
            ))
            continue
        report.checks.append(CheckResult(
            WARN, cat, f"{bot_id}:channel_packages",
            (
                f"Channel package(s) on a different release line than the "
                f"installed OpenClaw {installed}: {', '.join(stale)} — the "
                f"channel may fail to load until reinstalled"
            ),
            fix_cmd=f"sudo evolve-admin deploy --bot {bot_id}",
        ))


def _check_oc_version_coherence(report: HealthReport) -> None:
    """Is the installed runtime one Evolve has actually been validated against?

    Reads the compatibility manifest (``packages/plugin/compat.json``) and the
    recorded contract runs. An unvalidated runtime is a WARN, never a FAIL: the
    pod may be running it perfectly well, and the honest statement is "nobody
    checked", which is what the check says. Absence of evidence is never
    validation — the same fail-safe ``oc_compat.validation_state`` applies.
    """
    cat = "oc_runtime"
    try:
        from .upstream_version import installed_package_version
        ...
        report.checks.append(CheckResult(
            WARN, cat, "oc_version_validated",
            f"Could not check OpenClaw compatibility-contract status: {exc}",
        ))
        return
    installed = installed_package_version()
    if not installed:
        report.checks.append(CheckResult(
            WARN, cat, "oc_version_known",
            "Cannot determine which OpenClaw version is installed",
        ))
        return
    try:
        state = oc_compat.validation_state(installed)
    except Exception as exc:  # noqa: BLE001
        report.checks.append(CheckResult(
            WARN, cat, "oc_version_validated",
            f"Could not check whether OpenClaw {installed} is validated: {exc}",
        ))
        return
    name = getattr(state, "state", None) or str(state)
    if name == "tested":
        report.checks.append(CheckResult(
            PASS, cat, "oc_version_validated",
            f"OpenClaw {installed} passed Evolve's compatibility contract",
        ))
        return
    if name == "failed":
        report.checks.append(CheckResult(
            WARN, cat, "oc_version_validated",
            (
                f"OpenClaw {installed} is installed and FAILED Evolve's "
                f"compatibility contract — see the Upgrade Safety Report for "
                f"which checks"
            ),
            fix_cmd="Maintenance → OpenClaw runtime → view report",
        ))
        return
    report.checks.append(CheckResult(
        WARN, cat, "oc_version_validated",
        (
            f"OpenClaw {installed} has not been validated against this Evolve "
            f"release — no contract run is recorded. Absence of a run is not a pass."
        ),
        fix_cmd="Maintenance → OpenClaw runtime → Re-check",
    ))


def _check_oc_preflight_gate_freshness(
    report: HealthReport, *, shared_dir: Path, network: dict | None = None,
) -> None:
    """Does the per-bot OC-upgrade preflight the Update control gates on
    still describe the pod (hold-fix-4392)?

    WARN only, never FAIL: nobody has necessarily tried to upgrade, so a
    missing or stale preflight is not itself a fault. It is the operator's
    early warning that clicking Update right now would just bounce off a
    fresh 409, surfaced here instead of only at that moment.
    """
    cat = "oc_runtime"
    try:
        from . import oc_preflight_store as _pfs
        from .oc_preflight import bot_set_hash as _bsh
        from .upstream_version import installed_package_version
    except Exception as exc:  # noqa: BLE001 — WARN, never nothing (D-CS7)
        report.checks.append(CheckResult(
            WARN, cat, "oc_preflight_gate_fresh",
            f"Could not check the OpenClaw preflight gate: {exc}",
        ))
        return
    installed = installed_package_version()
    if not installed:
        report.checks.append(CheckResult(
            WARN, cat, "oc_preflight_gate_fresh",
            "Cannot determine which OpenClaw version is installed, so the "
            "preflight gate's freshness could not be checked",
        ))
        return
    try:
        data = _pfs.latest_cache_for_installed(installed, shared_dir=shared_dir)
    except Exception as exc:  # noqa: BLE001
        report.checks.append(CheckResult(
            WARN, cat, "oc_preflight_gate_fresh",
            f"Could not check the OpenClaw preflight gate: {exc}",
        ...
        return
    # `data is None` handled BEFORE the freshness call, not via the
    # "missing" reason string — pyright can't infer from `reason is None`
    # that `data` was non-None too, and this narrows it for the PASS branch
    # below without an assert.
    if data is None:
        report.checks.append(CheckResult(
            WARN, cat, "oc_preflight_gate_fresh",
            f"No per-bot OpenClaw preflight has been run for {installed} yet "
            "— the Update control will refuse to apply until one runs clean",
            fix_cmd="Maintenance → OpenClaw runtime → Run upgrade now → Run OpenClaw preflight",
        ))
        return
    try:
        reason = _pfs.pair_freshness_reason(data, bot_set_hash=_bsh(network or {}))
    except Exception as exc:  # noqa: BLE001
        report.checks.append(CheckResult(
            WARN, cat, "oc_preflight_gate_fresh",
            f"Could not check the OpenClaw preflight gate: {exc}",
    ...
    if reason is None:
        report.checks.append(CheckResult(
            PASS, cat, "oc_preflight_gate_fresh",
            f"OpenClaw upgrade preflight for {installed} is fresh "
            f"(checked {data.get('checked_at')})",
        ))
        return
    note = (
        "a bot was added or removed since it ran" if reason == "bot_set_changed"
        else "it is past its freshness window"
    )
    report.checks.append(CheckResult(
        WARN, cat, "oc_preflight_gate_fresh",
        f"The OpenClaw upgrade preflight for {installed} is stale ({note}) "
        f"— re-run it before relying on the Update control",
        fix_cmd="Maintenance → OpenClaw runtime → Run upgrade now → Re-check",
    ))
...
# Lookback window for the checkpoint-hold-evidence control below. A cleared
# trip's cost.json is gone by the time this runs, so the audit log (which
# retains 7 days, per breakers.store.read_audit_log's default) is the only
# remaining record of it.
_CHECKPOINT_HOLD_LOOKBACK_DAYS: int = 7


def _parse_iso_or_none(s: Any) -> datetime | None:
    """Best-effort ISO-8601 parse, tolerating a trailing ``Z``. None on
    anything unparseable — mirrors breakers.store's own ``_parse_iso``."""
...
def _check_cost_checkpoint_hold_evidence(
    report: HealthReport, shared_dir: Path, *,
    now: datetime | None = None,
    read_turns_fn=None,
) -> None:
    """Health control: "cap trip held the user channel" (D-CS13;
    internal/decision-cost-single-turn-2026-09-18.md — the 2026-09-17
    incident where a trip's ``checkpoint`` read ``pending`` while $4.15 was
    spent in the next seven minutes with no record of whether the hold fired).

    For every per-bot cost trip that armed a checkpoint in the last
    :data:`_CHECKPOINT_HOLD_LOOKBACK_DAYS` days (from the audit log, not
    ``list_active`` — a cleared trip's ``cost.json`` is long gone by now):

      - no interactive (user-source, cost > 0) turn in the trip's window →
        PASS "not exercised".
      - one landed and ``read_earliest_hold`` finds a marker → PASS "ok".
      - one landed and no marker exists → WARN "unknown". Never FAIL: an
        absent marker could mean the plugin never held the turn or held it
        but couldn't write the marker — this control cannot tell which, and
        a confident verdict it cannot back up is the failure D-CS7 closes.

    Best-effort: an import or read failure logs and returns with no checks
    appended, rather than failing the rest of the health scan.
    """
    cat = "cost_checkpoint_hold"
    try:
        from breakers import store as _bstore  # noqa: WPS433
        from breakers.classify import classify_turn  # noqa: WPS433
    except Exception as exc:  # pragma: no cover - defensive import guard
        log.debug("checkpoint-hold check skipped (import failed): %s", exc)
        return
    if read_turns_fn is None:
        try:
            from breakers.backtest import read_turns as read_turns_fn  # noqa: WPS433
        except Exception as exc:  # pragma: no cover - defensive import guard
            log.debug("checkpoint-hold check skipped (read_turns import failed): %s", exc)
            return

    now = now or datetime.now(timezone.utc)
    since = now - timedelta(days=_CHECKPOINT_HOLD_LOOKBACK_DAYS)

    try:
        audit = _bstore.read_audit_log(
            shared_dir, days=_CHECKPOINT_HOLD_LOOKBACK_DAYS, now=now,
        )
    except Exception as exc:  # noqa: BLE001
        log.debug("checkpoint-hold check: read_audit_log failed: %s", exc)
        return

    # Every (bot, trip_id) that armed a checkpoint, plus the nearest later
    # "reset" for that trip_id (its clear time). A trip with no reset yet
    # is still live (or was reaped without ever logging one); fall back to
    # its own expires_at, then to `now`.
    ...
    for entry in audit:
        ...
        if action == "reset":
            ...
            if isinstance(ts, str) and (trip_id not in resets or ts < resets[trip_id]):
                resets[trip_id] = ts
            continue
        if action not in ("trip", "retrip"):
            continue
        scope = entry.get("scope")
        if (
            not isinstance(scope, str) or scope == _bstore.POD_SCOPE
            or entry.get("type") != "cost" or entry.get("checkpoint") is None
        ):
            continue  # pod-wide, non-cost, or no checkpoint armed — no hold to evidence
        started = entry.get("timestamp")
        if isinstance(started, str):
            trips[(scope, trip_id)] = {
                "started": started, "expires_at": entry.get("expires_at"),
            }

    for (bot_id, trip_id), info in trips.items():
        started = _parse_iso_or_none(info["started"])
        if started is None or started < since:
            continue
        ended = (
            _parse_iso_or_none(resets.get(trip_id))
            or _parse_iso_or_none(info["expires_at"])
            or now
        )
        try:
            turns = read_turns_fn(shared_dir, bot_id, since=started, until=ended)
        except Exception as exc:  # noqa: BLE001
            log.debug("checkpoint-hold check: read_turns failed for %s: %s", bot_id, exc)
            continue

        interactive = [
            t for t in turns
            if classify_turn(t).bucket == "human" and float(t.get("cost") or 0) > 0
        ]
        name = f"{bot_id}:{trip_id[:8]}"
        if not interactive:
            report.checks.append(CheckResult(
                PASS, cat, name,
                f"{bot_id} trip {trip_id[:8]}: ok (not exercised) — no "
                f"interactive spend followed this trip.",
            ))
            continue

        hold = _bstore.read_earliest_hold(shared_dir, bot_id, trip_id)
        if hold is not None:
            report.checks.append(CheckResult(
                PASS, cat, name,
                f"{bot_id} trip {trip_id[:8]}: ok — interactive hold "
                f"recorded at {hold[0]}.",
            ))
        else:
            report.checks.append(CheckResult(
                WARN, cat, name,
                f"{bot_id} trip {trip_id[:8]}: an interactive turn spent "
                f"money after this trip and no hold marker was recorded — "
                f"whether the plugin held the turn is unknown.",
            ))


def _check_in_turn_checkpoint(
    report: HealthReport, shared_dir: Path, members: list[str], *,
    now: datetime | None = None,
) -> None:
    """Health control: "in-turn checkpoint armed" (D-CS12). From the plugin's
    status file: absent → WARN unknown; not registered → WARN; multi-call runs
    recorded spend but none was evaluated → WARN unknown (armed, not live);
    calls priced $0 → WARN (invisible to it); else PASS with the threshold,
    the price source and this week's trips. Never FAIL (D-CS7).
    """
    cat = "in_turn_checkpoint"
    try:
        from breakers import in_turn  # noqa: WPS433
    except Exception as exc:  # pragma: no cover - defensive import guard
        log.debug("in-turn checkpoint check skipped (import failed): %s", exc)
        return
    for bot_id in members:
        st = in_turn.read_status(shared_dir, bot_id)
        if st is None:
            report.checks.append(CheckResult(
                WARN, cat, bot_id,
                f"{bot_id}: unknown — the plugin has not reported the in-turn "
                f"checkpoint (no status file); whether it is armed is unknown.",
            ))
            continue
        usd = float(st.get("threshold_usd") or 0)
        source = st.get("price_source") or "unknown"
        if st.get("armed") is not True:
            status, detail = WARN, "NOT armed — before_tool_call did not register on this gateway."
        elif int(st.get("multi_call_runs") or 0) > 0 and int(st.get("evaluated_runs") or 0) == 0:
            status, detail = WARN, (
                "unknown — armed, and runs with tool calls recorded spend, but the "
                "checkpoint never evaluated one (armed but not live)."
            )
        elif int(st.get("zero_cost_calls") or 0) > 0:
            status, detail = WARN, (
                f"{st.get('zero_cost_calls')} model calls were priced $0 (no {source} row) — "
                "their spend is invisible to the checkpoint."
            )
        else:
            trips = len(in_turn.read_trips(shared_dir, bot_id, days=7, now=now))
            status, detail = PASS, (
                f"armed; in-turn checkpoints: {trips} this week; "
                f"{int(st.get('evaluated_runs') or 0)} runs evaluated since start."
            )
        report.checks.append(CheckResult(
            status, cat, bot_id,
            f"{bot_id}: in-turn checkpoint ${usd:.2f} of recorded spend "
            f"({source}-priced) — {detail}",
        ))
...
def run_health_check(
    ...
    # Runtime coherence — is the installed OpenClaw one these configs and
    # packages actually match, and one Evolve was validated against? Post-phase
    # only: mid-deploy the packages are legitimately in flux.
    if phase == "post":
        _check_oc_config_valid_under_installed(report, oc_members, network=network)
        _check_oc_channel_packages(report, oc_members, network=network)
        _check_oc_version_coherence(report)
        _check_oc_preflight_gate_freshness(report, shared_dir=shared_dir, network=network)
        _check_breaker_notify_path(report)
    ...
    # 8c. Cost-checkpoint interactive-hold evidence (D-CS13) — every
    # checkpoint trip in the last 7 days that saw interactive spend should
    # have left a hold marker; an absent one reports "unknown", never a
    # silent pass.
    _check_cost_checkpoint_hold_evidence(report, shared_dir)

    # 8d. In-turn cost checkpoint armed and live (D-CS12).
    _check_in_turn_checkpoint(report, shared_dir, members)
    ...
    # (internal/spec-alerts-signal-store-2026-05-07.md). Best-effort.
    try:
...
def run_pod_health_signals_only(
    ...
    *,
    ...
    (internal/spec-alert-notifier-2026-05-09.md) — gives the alert notifier
...
def _write_manifest(
    ...
    Format: see internal/spec-health-checker.md §Manifest schema.
    """
    from datetime import datetime, timezone
