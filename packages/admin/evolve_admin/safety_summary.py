...
# Projection of the channel registry's ``prose_label`` column — the
# mid-sentence form ("…messages you on Telegram", "…via email", "…to a
# configured webhook"), which is why ``email`` is lowercase and ``webhook``
# carries an article.
#
# WIDENING (deliberate, M1-B1): this table used to omit ``imessage``, so the
# summary rendered the .title() fallback "Imessage". The registry adds it as
# "iMessage". Label-only, strictly more correct, no set-membership logic reads
# this dict.
_CHANNEL_LABEL: dict[str, str] = _channel_registry.labels_where(
    lambda c: True, prose=True,
)
...
class BotSafetySummary:
    ...
    # internal/spec-app-derived-permissions-2026-05-24.md §7) carries the
...
def _file_access_phrases(oc: dict) -> tuple[list[str], list[str]]:
    ...
    (unrestricted) — verified in three places:
      ...
      * internal/spec-permission-posture-2026-05-10.md — only ``team_bot_c``
