...
def attach_tier_severity(
    ...
    """Classify each bot's roles (hard_break / dormant / ok) and surface the
    severity split (spec §Addendum 10 §C). Mutates ``summary`` in place:
      ...
      1. attaches ``summary['hard_break_tiers']`` — every hard-broken role
         ...
         catches the in-catalog hard break that no drift finding flags.
      2. tags each credential-gap drift finding (``provider_credentialed`` is
    ...
    (The former ``advisory_tiers`` side-car — the same-vendor judge amber
    nudge — died with the judge role; design-judge-role-collapse-2026-08-21
    §5.4. The second-provider recommendation lives on in the AI-Optimization
    provider-diversity copy, reframed as failover + cross-vendor checking.)
