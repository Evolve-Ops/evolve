...
Designed per internal/spec-manifest-v7-2026-05-20.md §9.1 (within-pod sharing).
...
def distill_to_spec(manifest: dict, bot_id: str, shared_dir: Path) -> tuple[dict, str, str]:
    ...
    if manifest.get("manifest_shape") == "v7-arc":
        ...
        # identity: NOT resolve_app_id — this is the SPEC's id, read from
        # the instance's ``provenance`` sub-object, and it
        # is what the gallery tier path below is keyed by
        # (``gallery/local/<spec_id>/<spec_version>.json``). ``resolve_app_id``
        # reads TOP-LEVEL fields only and, for a v7-arc instance, returns the
        # ``instance_id`` — a different app-layer object (AL-1.4 §6 delta 6:
        # collapsing instance identity onto the spec's is explicit 1.4c/AL-1.5
        # work). Substituting it here would look up a spec that does not exist.
