...
def _is_google_eligible(bot_id: str, network: dict) -> bool:
    """Eligibility gate, registry-first (D-CN8): a bot's ``connections.json``
    row decides when it can (see ``is_configured_via_registry``). A bot with
    NO row, or only an empty scope-shaped migration row, falls back to the
    pre-registry check unchanged — a migrated row may only narrow what the
    legacy path serves, never disable a bot it serves today.
    """
    registry_verdict = connections_store.is_configured_via_registry(
        bot_id, "google", path=connections_store.connections_path(network),
    )
    if registry_verdict is not None:
        return registry_verdict
    return google_service.is_google_configured(bot_id, network)
...
def register_google_bot_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_google_bot_call() -> ResponseReturnValue:
        ...
        # tools for this bot (spec §8 decision 3). Registry-first, per
        # D-CN8 — see _is_google_eligible.
        if not _is_google_eligible(bot_id, network):
