...
Endpoints (under ``/api/admin/bots/<bot_id>/pairing``):
  ...
  POST /commit                     finish pairing. Body:
                                   ...
                                   Gated on ``bot.roster.mutate`` — see
                                   the gate comment in the handler.
...
def register_routes(app: Flask, network_path: Path) -> None:
    ...
    def pairing_commit(bot_id: str) -> Any:  # type: ignore[no-redef]
        ...
        if not bot_users._bot_exists(net, bot_id):
            return jsonify({"error": f"unknown bot: {bot_id}"}), 404
        # Capability gate — the same floor the overlay/block/ignore roster
        # mutations already enforce in routes_bot_users, and the floor #3642
        # proposes for the DM approve/revoke/reject routes (still OPEN at time
        # of writing, so `_handle_action` is NOT yet gated on main — this route
        # does not depend on it). CBR-1.2 sibling class: `role` is an
        # unauthenticated body field that selects between a pod-WIDE admin
        # promotion, a per-bot ownership change, and a plain admission — all
        # three reached through this one handler, so one route-level gate
        # covers all three branches. Placed after the 404 so bot-existence
        # still answers first (parity with routes_bot_users).
        ok, deny_reason = bot_users._check_capability(
            net, bot_id, "bot.roster.mutate")
        if not ok:
...
def _promote_to_pod_admin(
    ...
    # Always {channel: [id, ...]}; normalizes any sibling channel still in
    # the legacy scalar shape as a side effect (M1-B2).
...
def _set_primary_user(
    ...
    # Replaces this channel's ids with the one just paired (the operator is
    # explicitly re-pairing); other channels are left alone. List shape
    # always, whatever was there before (M1-B2).
    _external_ids.set_channel_ids(primary, channel, [ext_id])
    if name:
        primary["name"] = name
