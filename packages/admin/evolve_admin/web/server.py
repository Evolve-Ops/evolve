...
def _list_manifests_as_bot(bot_id: str, user: str | None = None) -> list[str]:
    ...
    # sudo find fallback for the rare case where direct access fails.
    try:
        result = subprocess.run(
            ["sudo", "find", str(workspace_dir), "-maxdepth", "1",
             "-name", "*.json", "!", "-name", ".*", "!", "-name", "*_history*"],
            capture_output=True, text=True, timeout=10, cwd=get_profile().scratch_dir,
        )
...
def _find_manifest_by_id_field(bot_id: str, app_id: str, user: str | None = None) -> dict | None:
    ...
    for mf_path in _list_manifests_as_bot(bot_id, user=u):
        ...
        if not isinstance(data, dict):
            continue
        if data.get("id") == app_id or data.get("instance_id") == app_id:
            return data
    return None


def _read_bot_api_key(os_user: str) -> str:
    """Read a bot's Anthropic API key from its OpenClaw auth store.

    Delegates the SOURCE read to ``oc_store`` — the single home that knows the
    post-2026.6 layouts (per-agent sqlite store, then legacy auth-profiles.json,
    then the transitional .sqlite-import bak). The hardcoded ``anthropic:api``
    lookup this once held silently missed evo's auth-profiles.json (which uses
    ``anthropic:api_key``); the later direct-JSON read silently missed EVERY
    bot once OC moved the JSON into sqlite. Returns "" if no key resolves.

    ``os_user`` is already the resolved OS ACCOUNT name, so pass it as ``user``
    (and leave ``bot_id`` None — a bot_id and an account name are not always the
    same string, and the account is authoritative here).
    """
    from ..oc_store import read_anthropic_key
    return read_anthropic_key(None, user=os_user)
...
def create_app(network_path: Path = DEFAULT_NETWORK_CONFIG) -> Flask:
    ...
    def _enforce_device_auth():
        ...
        # A unix-socket request authenticated by a TRUSTED kernel peer-uid is
        # the socket's own auth (same check as require_trusted_peer) — exempt
        # it from the device-cookie gate so evo's tool runtime reaches the
        # daemon without a browser pairing. This is the enabler for
        # auth-on-by-default (roadmap 2.6). A socket request from an
        # *untrusted* uid is NOT exempt — it falls through to the cookie gate
        # and 401s, so the exemption never widens access beyond evo/evolve.
        if (request.environ.get("REMOTE_TRANSPORT") or "tcp") == "unix-socket":
            try:
                ...
                if peer_uid >= 0 and peer_uid in _pa._resolve_uids(
                    _pa.DEFAULT_TRUSTED_USERS
                ):
                    return None
...
<script>
 ...
 document.getElementById('code').addEventListener('keydown',function(e){if(e.key==='Enter')go();});
    ...
    def api_lifecycle_retire() -> Response:
        ...
        Archive directory is preserved under ``{shared_dir}/retired/`` so the
    ...
    def api_config() -> Response:
        ...
        if action == "alerts":
            ...
            # Classifiers card. Two sub-dicts (tier, judge). Merge into
            # existing so future sub-keys aren't clobbered.
            ...
            judge_in = cls_in.get("judge") if isinstance(cls_in.get("judge"), dict) else None
            if judge_in:
                merged = dict(existing.get("judge") or {})
                for k in ("tier",):
                    if k in judge_in and judge_in[k]:
                        merged[k] = judge_in[k]
                existing["judge"] = merged
            if existing:
                network["classifiers"] = existing
        elif action == "security":
            # Security card. Allowlist + merge so botId / rulesFile / any
            # future fields not surfaced by this UI survive the save.
            sec_in = body.get("security", {}) if isinstance(body.get("security"), dict) else {}
            existing = dict(network.get("security") or {})
            mode = (sec_in.get("mode") or "").strip().lower()
            if mode in ("primary", "dedicated"):
                existing["mode"] = mode
            if "requireForge" in sec_in:
                existing["requireForge"] = bool(sec_in["requireForge"])
            risks_in = sec_in.get("autoRejectRisk")
            if isinstance(risks_in, list):
                valid = {"low", "medium", "high", "critical"}
                existing["autoRejectRisk"] = [r for r in risks_in if r in valid]
            network["security"] = existing
        elif action == "pod":
            # Pod Identity card. Allowlist three scalar fields; null /
            # empty clears them (config._apply_pod_defaults then backfills
            # the passphrase defaults — "charles" / "darwin"). Leaves
            # pod.admins.* alone (separate action below).
            ...
            # Pod Admins list edit. Op-based (add | remove) so concurrent
            # edits to different channels don't clobber each other — the
            # UI sends only the one mutation it intends.
            ...
            ext = dict(admins.get("external_ids") or {})
            lst = list(ext.get(channel) or [])
            if op == "add":
                if ext_id not in lst:
                    lst.append(ext_id)
            else:  # remove
                lst = [x for x in lst if x != ext_id]
            ...
            if lst:
                ...
                ext.pop(channel, None)
            admins["external_ids"] = ext
            pod["admins"] = admins
            network["pod"] = pod
        else:
        ...
        def _monitor() -> None:
            ...
            try:
                ...
                if scanner_py.exists():
                    ...
                    # sudo strips the environment, so read the bot's API key here and
                    # inject it via SETENV (requires the SETENV tag in setup_wizard.py §7a).
                    from ..config import scanner_python as _scanner_python
                    cmd = ["sudo", "-u", scan_user]
                    _api_key = _read_bot_api_key(scan_user)
                    if _api_key:
                        cmd.append(f"ANTHROPIC_API_KEY={_api_key}")
                    ...
                    if quick:
                        cmd.append("--no-llm")
                    _log_cmd = [
                        c if not c.startswith("ANTHROPIC_API_KEY=") else "ANTHROPIC_API_KEY=***"
                        for c in cmd
                    ]
    ...
    def delete_application(bot_id: str, app_id: str) -> Response:
        ...
        # 2a. Strip pkg_id from sidecar markers (best-effort).
        if ws is not None:
            apply_manifest_marker_cleanup(
                application_id=app_id,
                bot_id=bot_id,
        ...
        # 2c. If any file delete failed, refuse to delete the manifest —
        # the operator can re-run uninstall to clean up the rest. Without
        # this guard we'd lose the checklist that lets them resume.
        if failed_deletes:
            return jsonify({
                **plan,
        ...
        # 2d. Finalize: archive + delete the manifest JSON, rebuild index.
        finalized = finalize_manifest_deletion(
            application_id=app_id,
            bot_id=bot_id,
            shared_dir=shared_dir,
        )
    ...
    def get_application_dependents(bot_id: str, app_id: str) -> "Response":
        ...
        shared = _shared()
        manifest = load_manifest(app_id, bot_id, shared)
        pkg_id = manifest.pkg_id if manifest else None
        if not pkg_id:
            return jsonify({
                "dependents": [],
    ...
    def api_file_index(bot_id: str) -> Response:
        ...
        network = load_network(network_path)
        shared_dir = Path(network.get("sharedDir", "/Users/Shared/evolve"))
        pkg_id_filter = request.args.get("pkg_id", "").strip()
    ...
    def api_reflect_reconcile(bot_id: str) -> Response | tuple[Response, int]:
        ...
        Resolution shares ONE authority with the classifier: Reflect (a thin
        reader of the recon ledger) already resolved each ``attach_candidate``
        marker — lineage-aware — to its owning app's current spec_id, and this
        action attaches the file to that app's manifest (resolved to its on-disk
        Path through the SAME ``build_spec_index``, so a ``discovered`` app with
        no materialized Instance — ``instance_id is None`` — is attachable too).
