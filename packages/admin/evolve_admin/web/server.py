...
def _write_manifest_as_bot(bot_id: str, app_id: str, data: dict, user: str | None = None) -> bool:
    ...
    Ownership is deliberately NOT restored to the bot afterwards, unlike
    ``install_helpers._restore_bot_ownership`` on workspace markdown: the
    rename leaves ``<app>.json`` owned ``evolve:staff`` (0644 + the
    inherited evolve ACE), readable but not writable by the bot. That is
    safe because no bot-side writer opens a manifest in place — they all
    rename over it, which needs write on the bot-owned DIRECTORY, not on
    the file. The full writer inventory, and what would invalidate this,
    live on ``applications.manifest._write_manifest_bytes``.
    """
...
def create_app(network_path: Path = DEFAULT_NETWORK_CONFIG) -> Flask:
    ...
    # internal/spec-user-profile-2026-05-07.md §D3) — siloed in each bot's
    ...
    from .routes_host_health import _register_host_health_routes; _register_host_health_routes(app, network_path)  # noqa: E702
    from .routes_pod_health import _register_pod_health_routes as _reg_pod_health
    _reg_pod_health(app, network_path)
    # _register_reliability_routes removed 2026-06-08 (dead app-test telemetry + Overview/Apps consumers; internal/decision-app-tests-2026-06-08.md).
    ...
    from .routes_content_scan import register_content_scan_routes; register_content_scan_routes(app, network_path)  # noqa: E702
    from .routes_report import register_report_routes; register_report_routes(app, network_path)  # noqa: E702
    from .routes_cost_measures import register_cost_measures_routes; register_cost_measures_routes(app, network_path)  # noqa: E702
    _register_help_routes(app, network_path)
    from .routes_help_docs import register_help_docs_routes; register_help_docs_routes(app, network_path)  # noqa: E702
    from .routes_better import register_better_routes; register_better_routes(app, network_path)  # noqa: E702
    # Board user surface (PA effort, D-MB2): its own per-user token auth — see routes_board.
    from .routes_board import is_board_path as _is_board_path, register_board_routes; register_board_routes(app, network_path)  # noqa: E702
    _register_arbiter_routes(app, network_path)
    from .routes_signals import register_signals_routes; register_signals_routes(app, network_path)  # noqa: E702
    ...
    from .routes_apps import register_apps_routes; register_apps_routes(app, network_path)  # noqa: E702
    # The pod-wide Roster card (D-AP5, M1 read-only). No write route exists.
    from .routes_users import register_users_roster_routes; register_users_roster_routes(app, network_path)  # noqa: E702
    # The WRITES beside those reads, each its own module (routes_apps is read-only by contract).
    from .routes_app_snapshot import register_app_snapshot_routes; register_app_snapshot_routes(app, network_path)  # noqa: E702
    from .routes_app_install import register_app_install_routes; register_app_install_routes(app, network_path)  # noqa: E702
    # Pod Intelligence (D-T5/T9): three dossier reads + the page's own-arrangement write.
    from .routes_dossier import register_dossier_routes; register_dossier_routes(app, network_path)  # noqa: E702
    ...
    # internal/cascade-validation-on-the-mini.md.
    ...
    # Spec: internal/spec-openclaw-json-derived-artifact-2026-05-24.md §4.
    ...
    # unsubscribe / list / test endpoints. Spec: internal/spec-pwa-2026-05-18.md §6.
    ...
    # (links only, no tokens). Spec: internal/design-multi-pod-2026-06-11.md §3.1.
    ...
    # internal/spec-pwa-2026-05-18.md §7. The WebSocket gates on
    ...
    # Spec: internal/spec-users-meta-2026-06-15.md §M1 (B2b's "the B4 seam").
    ...
    # Add-a-Bot wizard backend (PR β; spec internal/spec-add-bot-wizard-2026-05-28.md)
    from .wizard_routes import register_wizard_routes
    register_wizard_routes(app, network_path)

    # Path-C Google integration wizard (PR ε; spec internal/spec-google-integration-paths-2026-05-30.md §7)
    from .wizard_google_routes import register_google_wizard_routes
    register_google_wizard_routes(app, network_path)

    # Path-A Personal-Gmail wizard backend (Phase A.2; spec internal/spec-google-path-a-2026-06-01.md)
    ...
    # the path-C wizard). Spec: internal/spec-correspondence-persona-2026-05-30.md
    # §3; sibling work in internal/spec-multi-user-alias-2026-06-01.md.
    ...
    # internal/spec-files-pack-hybrid-2026-06-03.md §8.
    ...
    # Spec: internal/spec-scanned-export-2026-06-02.md §3.5 (Stage 0e).
    ...
    # internal/spec-pwa-2026-05-18.md §5.4.
    ...
    # Spec: internal/spec-primary-bot-interface-2026-05-14.md §5.
    from .primary_state_routes import register_primary_state_routes
    register_primary_state_routes(app, network_path)

    # Bot-facing, peer-uid-bound route families: curated Google tools, the user directory, the board's bot half (/api/board-bot/*, D-MB4), the backup-repo visibility VERDICT (/api/backup-bot/visibility — the bot asks, the daemon that owns the vault answers private/public/unknown, and the PAT never leaves this process), and the users.whoami/users.list read verbs (/api/users-bot/*).
    from .google_bot_routes import register_google_bot_routes; from .directory_bot_routes import register_directory_bot_routes; from .board_bot_routes import register_board_bot_routes; from .backup_bot_routes import register_backup_bot_routes; from .users_bot_routes import register_users_bot_routes  # noqa: E702
    register_google_bot_routes(app, network_path); register_directory_bot_routes(app, network_path)  # noqa: E702
    register_board_bot_routes(app, network_path); register_backup_bot_routes(app, network_path)  # noqa: E702
    register_users_bot_routes(app, network_path)

    # Bot-facing app capability index — Tier-2 expand_app(app_id) lookup.
    # Spec: internal/spec-app-invocation-just-works-2026-06-29.md §2.1. Also /api/connections/<bot> (D-CN2, an ordinary admin route, not peer-uid bound).
    from .applications_bot_routes import register_applications_bot_routes; from .connections_routes import register_connections_routes  # noqa: E702
    register_applications_bot_routes(app, network_path); register_connections_routes(app, network_path)  # noqa: E702
    ...
    try:
        ...
        _log.warning("github.pat keystore migration failed (will retry next start)", exc_info=True)
    from ..connections_migration import run_startup_migration; run_startup_migration(network_path)  # D-CN8  # noqa: E702
    ...
    def _auth_exempt(path: str) -> bool:
        # Board paths are exempt ONLY because routes_board enforces its own
        # per-user token AND its own CSRF posture, fail closed.
        return _is_board_path(path) or path in _AUTH_EXEMPT_PATHS or path.startswith("/static/")
    ...
    def _enforce_csrf():
        ...
        if request.method.upper() in _csrf.SAFE_METHODS:
            return None
        if _is_board_path(request.path):
            return None  # routes_board owns the board's CSRF posture
        if (request.environ.get("REMOTE_TRANSPORT") or "tcp") == "unix-socket":
            return None
        authed = (
...
<script>
 ...
 document.getElementById('code').addEventListener('keydown',function(e){if(e.key==='Enter')go();});
    ...
    # Spec: internal/spec-pwa-2026-05-18.md §3 + §5.1-5.3. The manifest is
    ...
    def api_status() -> Response:
        ...
        # expires_at, initiated_by, reason, accepted_usd, accepted_at,
        # accepted_at_label}]`; the response gets a top-level
        # `pod_breakers` list. Tile rendering uses these to show the
        # breaker pill (and the acceptance line) without an extra
        # round-trip. The projection lives in spend_caps.breaker_ui_entry
        # because it overlays the reactivation acceptance marker.
        try:
            from breakers import store as _bstore
            from spend_caps import breaker_ui_entry as _breaker_ui_entry
            _active = _bstore.list_active(shared_dir)
            _per_bot: dict[str, list[dict]] = {}
            _pod: list[dict] = []
            for r in _active:
                entry = _breaker_ui_entry(r, shared_dir)
                if r.bot_id == "pod":
                    _pod.append(entry)
                else:
    ...
    def api_setup_status() -> Response:
        ...
        network.json with no usable primary. See spec internal/spec-add-bot-
    ...
    # Spec: internal/spec-effectiveness-layer-2026-06-09.md §4.
    ...
    def delete_application(bot_id: str, app_id: str) -> Response:
        ...
        if ws is not None:
            actually_deleted, failed_deletes, manual_deletes = unlink_confirmed_files(
                ws, get_bot_user(bot_id, network), confirmed_deletes, plan,
            )
    ...
    def api_reflect_apply_fix(bot_id: str) -> Response:
        ...
        bot_user = get_bot_user(bot_id, network)  # ACCOUNT name, never the bot_id
        ...
        try:
            workspace = (resolve_bot_paths(bot_id, user=bot_user).get("workspace") or
                         str(_bot_home(bot_id, network) / ".openclaw" / "workspace"))
        ...
        try:
            ...
            ok, detail = embed_marker_privileged(
                target, new_text=new_text, bot_user=bot_user,
            )
            if ok:
                return jsonify({
                    "ok": True,
            ...
            manual_cli = (
                f"cd /tmp && sudo -u {bot_user} /Users/Shared/evolve-venv/bin/python3 -c \""
...
_GOOGLE_SCOPE_REGISTRY: dict[str, dict] = {
    ...
    # internal/spec-google-workspace-suite-2026-06-04.md §2.1). The Read skill
...
def _register_breaker_routes(app: Flask, network_path: Path) -> None:
    ...
    Spec: internal/spec-circuit-breakers-2026-05-21.md §3.1. Thin Flask layer
    ...
    def api_breakers_trip() -> Response:
        ...
        try:
            enforce_result = _enforce.enforce_trip(
                ...
                shared_dir=_shared_dir(), reason=record.reason, expires_at_iso=record.expires_at,
            )
        except ValueError as exc:
            return jsonify({
                "ok": False,
    ...
    def api_breakers_reset() -> Response:
        ...
        # Bring-up runs BEFORE the state clear so a failure leaves the
        # breaker tripped and the operator able to retry — see
        # breakers_enforce.reset_and_enforce (2026-09-04 reset incident).
        try:
            outcome = _enforce.reset_and_enforce(
                shared_dir=_shared_dir(),
                scope=scope, breaker_type=breaker_type, network=_net(),
        ...
        prior, enforce_result = outcome.prior, outcome.enforce
        if prior is None:
            return jsonify({
                "ok": True,
                "was_tripped": False,
        ...
        if enforce_result is None:
            # Scope gone from network.json — cleared, but bring-up was
            # impossible. Same 207 + reset-record shape as a failed
            # enforce, so the SPA still reads it as partial success.
            return jsonify({
                "ok": False, "was_tripped": True, "enforce": None,
                "error": f"state cleared, enforce failed: {outcome.enforce_error}",
                "reset": prior.to_json(),
            }), 207
...
def _register_intent_routes(app: Flask, network_path: Path) -> None:
    """Register /api/intents/* — Phase 4 of internal/spec-config-intent-system-2026-05-21.md.
...
def _register_plugins_admin_routes(app: Flask, network_path: Path) -> None:
    ...
    Spec: internal/spec-plugin-inventory-2026-05-10.md §7.
    """
    def _all_bots() -> list[str]:
        return list(load_network(network_path).get("bots", {}).keys())
    ...
    def api_plugins_admin_propose_bulk_allow_deny() -> Response:
        ...
        (internal/spec-plugin-posture-rework-2026-06-06.md §2.3) so there's
...
def _register_hooks_admin_routes(app: Flask, network_path: Path) -> None:
    ...
    Spec: internal/spec-hook-governance-2026-05-10.md §7.
    """
    def _all_bots() -> list[str]:
        return list(load_network(network_path).get("bots", {}).keys())
