...
def create_app(network_path: Path = DEFAULT_NETWORK_CONFIG) -> Flask:
    ...
    # Bot-facing, peer-uid-bound route families: curated Google tools, the user directory, the board's bot half (/api/board-bot/*, D-MB4), and the backup-repo visibility VERDICT (/api/backup-bot/visibility — the bot asks, the daemon that owns the vault answers private/public/unknown, and the PAT never leaves this process).
    from .google_bot_routes import register_google_bot_routes; from .directory_bot_routes import register_directory_bot_routes; from .board_bot_routes import register_board_bot_routes; from .backup_bot_routes import register_backup_bot_routes  # noqa: E702
