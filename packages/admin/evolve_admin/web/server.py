 ...
 document.getElementById('code').addEventListener('keydown',function(e){if(e.key==='Enter')go();});
    ...
    def api_config() -> Response:
        ...
        if action == "alerts":
            ...
            # Tolerant read (a hand-edited pod block may hold a bare
            # string), list shape written back (M1-B2).
            ext = _external_ids.read_external_ids(admins)
            lst = list(ext.get(channel.strip().lower()) or [])
            if op == "add":
            ...
            if lst:
                ext[channel.strip().lower()] = lst
            else:
                ext.pop(channel.strip().lower(), None)
            _external_ids.write_external_ids(admins, ext)
            pod["admins"] = admins
            network["pod"] = pod
        else:
