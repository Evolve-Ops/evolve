    ...
    def api_applications_sync() -> Response | tuple[Response, int]:
        ...
        Returns the unified result (see applications/sync.py::_result):
            200 {bot_id, path: "cheap"|"escalated", reason, discovered_count,
                 ...
                 warnings: [...],
                 llm_phase: "not_run"|"ran"|"skipped_degraded"|"unknown",
                 llm_degraded, llm_degraded_reason,
                 llm_degraded_note, llm_degraded_remedy}
            400 {error} — bot missing/unknown

        ``llm_phase`` is ALPHA-2 (audit finding B2a): a pod with no resolvable
        provider key used to be told "ran full scan; discovered 0 app(s)" while
        the pass that recognises apps had been skipped entirely. The fields are
        passed straight through — ``sync_bot`` reads them off the scan's own
        ``.scan-status.json`` and this route adds no interpretation of its own.
        """
    ...
    def api_applications_sync_pod() -> Response:
        ...
        Returns:
            200 {ok, total_discovered, total_findings, aggregate_counts,
                 escalated_bots, degraded_bots, unverified_bots,
                 llm_degraded_reasons,
                 bots: [{bot_id, path, reason, discovered_count,
                 uncovered_files, drift_findings, drift_counts, warnings,
                 llm_phase, llm_degraded, llm_degraded_reason,
                 llm_degraded_note, llm_degraded_remedy}
                 | {bot_id, error}]}

        ``degraded_bots`` / ``unverified_bots`` / ``llm_degraded_reasons`` are
        the pod-wide rollup of
        what the scans actually did (ALPHA-2, audit B2a). Reasons are GROUPED,
        so a pod where five bots share one missing provider key produces one
        line to render rather than five — and each carries the plain-language
        note and remedy, because the surface must never render a reason key.
        """
        from ..applications.sync import LLM_PHASE_UNKNOWN, sync_bot
        from evolve_config import CANONICAL_SHARED_DIR
        from .server import _scan_status
        ...
        # reason key → {reason, note, remedy, bots[]} — grouped, not per-bot.
        degraded_reasons: dict[str, dict] = {}
        degraded_bots: list[str] = []
        # Escalated bots whose scan left no readable record of what it did.
        # Rolled up BESIDE the degraded list rather than folded into it: "it
        # skipped its model phase" and "we cannot say what it did" are
        # different claims, and a surface that shows only the first will report
        # the second as success (docs/principle-tri-state-status.md).
        unverified_bots: list[str] = []
        total_discovered = 0
        total_findings = 0
        escalated_bots = 0
        ...
        for bot_id in bots:
            ...
            if result["path"] == "escalated":
                escalated_bots += 1
            if result.get("llm_phase") == LLM_PHASE_UNKNOWN:
                unverified_bots.append(bot_id)
            if result.get("llm_degraded"):
                degraded_bots.append(bot_id)
                key = result.get("llm_degraded_reason") or "unknown"
                slot = degraded_reasons.setdefault(key, {
                    "reason": key,
                    "note": result.get("llm_degraded_note"),
                    "remedy": result.get("llm_degraded_remedy"),
                    "bots": [],
                })
                slot["bots"].append(bot_id)
        ...
        return jsonify({
            ...
            "degraded_bots": sorted(degraded_bots),
            "unverified_bots": sorted(unverified_bots),
            "llm_degraded_reasons": [degraded_reasons[k] for k in sorted(degraded_reasons)],
