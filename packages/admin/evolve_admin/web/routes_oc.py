...
def register_oc_routes(app: Flask, network_path: Path) -> None:
    ...
    #
    # Scope: the seven producers wired into the admin UI as live observers —
    # integration_probe, host_health, error_reporter, pod_health,
    # cron_exit_monitor, exec_failure_monitor, tool_profile_monitor. Scheduled
    ...
    def api_signals_refresh() -> Response:
        ...
        pod_health, cron_exit_monitor, exec_failure_monitor and
        tool_profile_monitor in parallel
        with a 10-second wall-clock budget. Returns per-monitor outcomes
        so the UI can surface partial success. Best-effort — never 5xxs
        ...
        Response shape:
          {
            ...
            "monitors": [
              ...
              {"name": "exec_failure_monitor", "ok": True,  "ms": 8},
              {"name": "tool_profile_monitor", "ok": True,  "ms": 6},
            ],
          }
        """
        import time as _time
        ...
        def _run_exec_failure_monitor() -> None:
            from ..exec_failure_monitor import emit_exec_failure_signals
            # Scheduled home is the hourly audit-scheduler tick; the
            # Refresh button re-scans so a quieted failure shape archives
            # immediately. Pure local ledger reads — cheap.
            emit_exec_failure_signals(_shared_dir())

        def _run_tool_profile_monitor() -> None:
            from ..tool_profile_monitor import emit_tool_profile_signals
            # Scheduled home is the hourly audit-scheduler tick; the Refresh
            # button re-scans so a widened tool profile clears its refusal
            # Signals immediately. Pure local ledger reads — cheap.
            emit_tool_profile_signals(_shared_dir())
        ...
        monitors = [
            ...
            ("exec_failure_monitor", _run_exec_failure_monitor),
            ("tool_profile_monitor", _run_tool_profile_monitor),
        ]
