...
def _trusted_uids() -> "set[int] | None":
    ...
    ``models user-tier-control`` (via
    ``user_tier_override._mirror_to_shared_dir``), which run either as
    the admin-server user (this process) or as root under
    ...
    Note this file is now the MIRROR, not the authority: those commands
    write the bot's ``~/.openclaw/evolve-tiers.json`` first and copy the
    result here, because that is the file the plugin routes on. This
    reader still reads the mirror — repointing it at the bot-owned
    canonical file would need the trust gate above re-derived, since
    "owner is not an operator uid" is that file's normal state.
