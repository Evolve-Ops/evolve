...
def _read_user_tier_override(shared_dir: Path, bot_id: str) -> dict:
    ...
    if isinstance(cap, (int, float)) and 0 <= cap <= 100:
...
def register_home_chat_routes(
    ...
    def api_home_chat():
        ...
        Phase 4.1 of docs/spec-evo-oc-native-2026-05-19.md: the chat is
        ...
        # docs/spec-user-tier-control-2026-05-26.md. Per-turn, not
        ...
        # docs/diagnosis-evo-exec-approval-leak-2026-05-21.md: closes the
        ...
        if _is_leading_slash_command(message):
            return jsonify({
                ...
                # docs/spec-user-tier-control-2026-05-26.md §UI.
                "tier": tier_pref,
        ...
        # See docs/spec-user-tier-control-2026-05-26.md "tier1 daily-cap
        ...
        # inline (forcing per-bot fan-out into ``pod_state.audit`` etc.)
    ...
    def _proxy_result_to_json(
        ...
        *,
        ...
        Tier echo (docs/spec-user-tier-control-2026-05-26.md):
    ...
    def api_diagnostic_chat():
        """Outage-only fallback chat — talks to a Haiku diagnostic LLM.

        Used by the Chat UI when evo's gateway is down (the proxy returns
        ``error="gateway_down"``). Routes through the same pod-wide
        Anthropic credential the home-page narrative already uses; the
        ...
        ``source`` mirrors the narrative endpoint values:
          ...
          "no_llm"        — no Anthropic key configured for the pod
        ...
        model = chat_cfg.get("model") or _hc.DEFAULT_MODEL

        api_key = _resolve_anthropic_key(network)
        if not api_key:
            return jsonify({
                "reply": (
                    "Diagnostic LLM unavailable — no Anthropic API key is "
                    "configured for the pod. Bring up evo or fix the key in "
                    "network.json to restore chat."
                ),
                "source": "no_llm",
        ...
        try:
            ...
            return jsonify({
                "reply": (
                    ...
                    "and Anthropic API health. The pod is still observable "
    ...
    def api_home_narrative():
        ...
        chat_cfg = network.get("home_chat") or {}
        daily_cap = int(chat_cfg.get("daily_cap") or _hc.DEFAULT_DAILY_CALL_CAP)
        model = chat_cfg.get("narrative_model") or chat_cfg.get("model") or _hc.DEFAULT_MODEL
        ttl_seconds = int(
...
def _reshape_security(
    ...
    strings — evo then has to fan out into ``pod_state.audit(bot_id=…)``
...
def _resolve_anthropic_key(network: dict) -> str | None:
    """Pull the primary bot's Anthropic key via the analyzer-side
    reader. Returns None when not configured — caller surfaces that
    ...
    try:
        ...
        key = read_primary_bot_anthropic_key(network)
        return key or None
    except Exception as exc:  # noqa: BLE001
        log.warning("home_chat key resolution failed: %s", exc)
        return None
