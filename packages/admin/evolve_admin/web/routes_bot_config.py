...
def register_bot_config_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_models_tier_resolution() -> Response:
        ...
        # (spec §Addendum 2.3). Resolves fast/standard/power/max/judge through
    ...
    @app.post("/api/bot/routing")
    def api_bot_routing_save():
        """Persist tier-routing changes to network.json models.routing."""
        body = request.get_json() or {}
        net = load_network(network_path)
        routing = net.setdefault("models", {}).setdefault("routing", {})
        allowed = {"enabled", "productiveTier", "maintenanceTier", "backgroundTier", "ambiguousTier"}
        for key in allowed:
            if key in body:
                routing[key] = body[key]
        save_network(net, network_path)
        _audit_log_entry("routing.update", "network", {k: body[k] for k in allowed if k in body})
