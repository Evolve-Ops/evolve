...
See internal/design/integration-discovery.md for the architecture.
"""
...
class ProbeHelpers:
    ...
    """
    scan_agent_llm_keys: Callable[[str], list] | None = None
    """every LLM key found in the bot's PER-AGENT runtime store, as
    ``oc_agent_keys.LocatedRuntimeKey`` records (all providers, all agents).
    One scan serves every provider's ``OcAgentCatalogKeyProbe``, so the
    implementation memoises per request. Values stay in-process; only the
    masked form reaches the wire.
    """
...
def build_probes(
    ...
    *,
    ...
    # ── Storage shape S8: OC's PER-AGENT runtime store ───────────────────
    # NOT feature-flagged, unlike the Phase 2/2.5 probes. Those widened
    # discovery for shapes the dashboard already half-covered; this one is
    # the only surface that sees the file the running agent reads its LLM
    # key from, and until it shipped the Credentials tab rendered NO LLM
    # Providers section at all on a bot billing real money (operator
    # finding 2026-09-04). A flag here would default that page to lying.
    # Imported here, not at module top: `oc_agent_catalog` imports the core
    # types back out of this module, so a top-level import is a cycle.
    from .oc_agent_catalog import OcAgentCatalogKeyProbe
    from ..oc_agent_keys import KNOWN_LLM_PROVIDERS
    for prov in KNOWN_LLM_PROVIDERS:
        registry.setdefault(prov, []).append(
            OcAgentCatalogKeyProbe(provider=prov),
        )
