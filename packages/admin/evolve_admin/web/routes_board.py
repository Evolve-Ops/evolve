"""routes_board.py — the Board's user-facing surface (slices 1+2, F8-reviewed).
    ...
    POST /api/board/<bot_id>/cards/<id>/move   {to_lane, reason?} — tap/drag
...
from ..board_store import (
    LANES, OWNERS, assign_card, create_card, load_board, move_card, split_card,
    token_store_readable, validate_bot_id, visible_cards,
)
...
def register_board_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_board_read(bot_id: str) -> RouteResult:
        ...
        board = dict(board)
        board["cards"] = visible_cards(board, include_archived=include_archived)
    ...
    def api_board_move(bot_id: str, card_id: str) -> RouteResult:
        ...
        try:
            with _WRITE_LOCK:
                # ``reason`` is D-BI2's optional drop tap-reason; the store
                # validates it against the fixed set and refuses one on any
                # destination but ``dropped``.
