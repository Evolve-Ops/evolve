...
def _render_week(
    ...
    *,
    ...
    group_of = _person_grouper(network or {})
    ...
    return {
        ...
        "modules": [_module_view(payload, m, priors, name_for, group_of)
...
def _module_view(
    ...
    detail = dict(module.get("detail") or {})
    if module_id == "users_activity" and isinstance(values, dict):
        # One row per person: identities the roster maps to one human merge
        # here, and the detail pane lists which ones did.
        merged = _merge_people(values.get("top"), group_of)
        values = {**values, "top": merged}
        joined = {str(r["person_id"]): r["identities"] for r in merged
                  if isinstance(r, dict) and len(r.get("identities") or []) > 1}
        if joined:
            detail["identities_merged_into_one_person"] = joined
    return {
        "module_id": module_id,
...
def _person_grouper(network: dict[str, Any]):
    """``person_id -> person key``, or ``None`` when nothing can group.

    ONE mapping, not a second one: ``roster_identity.rows_holding`` — the
    exact-match lookup ``users_roster`` groups the Users page by. Only a row
    that models ONE human merges identities; the pod-admin bag is several
    people and never does (``PersonRef.is_person``), so an id held by both
    the bag and a person's row groups under the person. Read-only.
    """
    try:
        from ..roster_identity import rows_holding
        from .routes_analytics import _split_requester_key
    except Exception:  # pragma: no cover - defensive
        return None

    def group_of(person_id: str) -> str | None:
        platform, user_id = _split_requester_key(person_id)
        try:
            refs = rows_holding(network, platform, user_id)
        except Exception:  # noqa: BLE001 — grouping must never 500 a page
            return None
        return next((r.key for r in refs if r.is_person), None)

    return group_of


def _merge_people(top: Any, group_of) -> list[Any]:
    """The users bar rows, one per person, requests summed.

    An identity the roster maps to no person stays its own row — its label
    then names the identity kind (``_fallback_user_label``). Counts are only
    added, never recomputed: the sum of the rows is unchanged.
    """
    ...
    for entry in top:
        if not isinstance(entry, dict) or not isinstance(entry.get("person_id"), str):
            continue
        pid = entry["person_id"]
        key = (group_of(pid) if group_of is not None else None) or pid
        row = merged.get(key)
        if row is None:
            merged[key] = {**entry, "bots": list(entry.get("bots") or []),
                           "identities": [pid]}
            continue
        row["requests"] = int(row.get("requests") or 0) + int(entry.get("requests") or 0)
        row["bots"] = sorted(set(row["bots"]) | set(entry.get("bots") or []))
        row["identities"].append(pid)
    return sorted(merged.values(), key=lambda r: -int(r.get("requests") or 0))
