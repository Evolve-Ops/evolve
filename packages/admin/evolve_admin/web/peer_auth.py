...
#   /api/board-bot/ — the bot's board verbs         (board_bot_routes)
#   /api/users-bot/ — the users.whoami/users.list read verbs (users_bot_routes)
# The trailing slash makes the ``startswith`` boundary-safe (``/api/googlex`` /
# ``/api/google-private`` don't match). ``/api/board-bot/`` is deliberately a
# DIFFERENT namespace from the phone's ``/api/board/``: the latter carries its
# own per-user token auth and is exempted by ``routes_board.is_board_path``, so
# a bot route under it would inherit the wrong gate (and be routable on the
# tailnet listener, which serves board paths only). ``/api/users-bot/`` is
# likewise its own namespace, distinct from the admin-UI-only ``/api/users/``
# roster read (routes_users.py) — a prefix exemption on the latter would hand
# any bot peer the whole pod's roster with no audience check.
_PEER_BOT_ROUTE_PREFIXES: tuple[str, ...] = (
    "/api/google/", "/api/directory/", "/api/board-bot/", "/api/users-bot/",
)
...
#   /api/cost-checkpoint/answer — the D-CC3 owner-only "continue"/"stop"
# answer to a tripped cost cap (cost_checkpoint_bot_routes). Exact, not a
# prefix: the route binds the acting bot from the peer uid and resolves the
# ANSWERING person's role server-side, so it enforces its own gating, but no
# other /api/cost-checkpoint/ path should inherit the exemption.
#   /api/backup-bot/visibility — the backup daemon's private/public verdict
# (backup_bot_routes). Exact rather than a prefix so the exemption covers the
# one read the bot-user daemon needs and nothing a future /api/backup-bot/*
# might add. The route binds both the calling bot AND the repo it checks from
# the peer uid, and returns a verdict only — never the pod PAT.
