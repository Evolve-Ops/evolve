...
AL-1.8a (internal/build-AL-1.8a-apps-shell.md §2). Thin by contract:
  ...
  GET /api/apps                     — the pod's defined apps, one row per app
                                      (``?grouped=0`` → one row per stored id)
...
def register_apps_routes(app: Flask, network_path: Path) -> None:
    ...
    def _grouped_arg() -> bool:
        """Whether to apply ALPHA-3a's same-app claim on this read.

        Default ON: two rows each claiming "on 1 bot" for one app is the
        audit's B3 blocker, so the grouped answer is the one an operator gets
        without asking. ``?grouped=0`` (or ``false`` / ``no``) withdraws it.
        Anything else — including a typo — reads as ON, because failing
        toward the *ungrouped* answer would silently restore the blocker.
        """
        raw = (request.args.get("grouped") or "").strip().lower()
    ...
    def _load_outcomes(shared: Path, app_id: str, bots: "list[str]") -> dict:
        """The outcome view (design §3.2) for one app, across ``bots``.

        ``{"measured": False}`` on any failure — an app that is not a
        Tracker application (most of them) reads this way too, and the two
        must not be told apart by this function raising vs. returning.
        """
        try:
            from ..app_outcomes import get_app_outcomes
        except Exception:
            return {"measured": False}
        try:
            return get_app_outcomes(shared, app_id, bots)
        except Exception:
            return {"measured": False}
    ...
    def _pack_context(bot_id: str, raw: dict) -> "dict | None":
        ...
        THE ``pkg_id`` FALLBACK IS NOT AN IDENTITY CONFLATION (AL-3.2). An app
        installed from its own AL-3.1 pack has no ``pkg_id`` at all — there is
        no gallery package behind it — and the resolver refuses an empty slot
        for every placeholder, including ones no file uses. So the app's id
        fills the slot, and that is only safe because it can never be consumed:
        ``app_install._pkg_id_for`` REFUSES to install a pack whose entries
        declare ``{pkg_id}`` without a real package key, so any pack that got
        installed either carries one or has no entry that could substitute it.
        The two names stay distinct in the vocabulary; what is shared here is a
        value nothing reads.
        """
        try:
            ...
            return resolve_install_context(
                ...
                pkg_id=str(raw.get("pkg_id") or "") or (resolve_app_id(raw) or ""),
    ...
    def _signals_for_apps(shared: Path, app_ids: "list[str]") -> list[dict[str, Any]]:
        """Active Signals that NAME any of these app ids (design §4's third region).
        ...
        A LIST of ids, not one, because of ALPHA-3a: a grouped detail speaks
        for every member id, and a Signal about the same app under the OTHER
        bot's id would otherwise vanish from the one page that claims to
        cover both. The store is still walked once.
        """
        wanted = {a for a in app_ids if a}
        try:
            import importlib
        ...
        try:
            for sig in store.iter_active(shared):
                ...
                if not isinstance(details, dict):
                    continue
                if (details.get("app_id") or "") not in wanted:
                    continue
                out.append({
                    "id": sig.id,
                    "type": sig.type,
            ...
            log.warning("routes_apps: signal lookup failed for %s: %s",
                        sorted(wanted), exc)
            return []
    ...
    def api_apps_list() -> ResponseReturnValue:
        """The pod's defined apps, one row per app.
        ...
        Optional ``?grouped=0`` withdraws ALPHA-3a's same-app claim and
        returns one row per stored ``app_id``. That is what the surface's
        "Show separately" asks for, and it is why the claim is safe to make
        by default: it is reversible in one query parameter and it changes
        nothing on disk.
        """
        try:
            shared = _shared()
    ...
    def api_apps_detail(app_id: str) -> ResponseReturnValue:
        ...
        ``?grouped=0`` withdraws ALPHA-3a's claim here too, so a split row in
        the list opens a split detail. The two surfaces must agree: B3's
        evidence was a DETAIL page denying a bot that had the app.
        """
        try:
            shared = _shared()
            ...
            if detail is None:
                return jsonify({"ok": False, "error": "app not found",
                                "app_id": app_id}), 404
            # Signals are looked up AFTER the row, because which ids this
            # page speaks for is the row's answer (ALPHA-3a), not the
            # request's.
            detail["signals"] = _signals_for_apps(
                shared, detail.get("grouped_app_ids") or [app_id],
            )
            detail["ok"] = True
