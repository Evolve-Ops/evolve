...
def register_admin_config_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_admin_config_set_user_tier_override(bot_id: str) -> Response:
        ...
        drift. It used to declare a bare ``{"tiers"}`` — a name heal never
        emits (it namespaces per key, ``tiers:userTierOverride``), so the
        credit never matched. Same pattern as the cascade endpoint above.
        """
        rej = _reject_unknown_bot(bot_id)
        if rej is not None:
            return rej
