...
def register_admin_config_routes(app: Flask, network_path: Path) -> None:
    ...
    def _reject_unknown_bot(bot_id: str):
    ...
    def api_admin_get_models(bot_id: str) -> Response:
    ...
    def api_admin_set_models(bot_id: str) -> Response:
    ...
    def api_admin_config_set_tiers(bot_id: str) -> "Response | tuple[Response, int]":
        ...
        if not result:
            # Rung-collision (standard+judge → sonnet-class etc.): the writer
            # refuses an impossible fold rather than silently last-writer-wins —
            # operator-fixable input → 409. See tier_write_guard.
    ...
    # "Easy setup" → asks for a ranked provider-order preference → server-side
    # reorders every role's DEFAULT cluster by that preference and writes it
    # through the SAME safe paths the manual editors use (pod → _patch_network_json
    # at models.* granularity so embedding survives; bot → the tier-mode "custom"
    # materialize seam seeded with the wizard result). The compute is the pure
    # primary_bot.compute_easy_setup_catalog — no provider/model literals here;
    # the only literal home stays DEFAULT_MODEL_CATALOG.
    #
    # ``preview: true`` computes + validates WITHOUT writing, so the modal can
    # show the resulting per-tier order before the operator confirms the
    # overwrite.
    ...
    def _easy_setup_compute(scope: str, provider_order: list) -> "tuple[dict, str | None]":
        ...
        net = load_network(network_path)
        cred = _pod_credentialed_providers(net) if scope == "pod" else scope_credentialed_to_bot(scope, _bot_providers_with_keys, lambda: _pod_credentialed_providers(net))
    ...
    def api_models_easy_setup() -> "Response | tuple[Response, int]":
        ...
        - ``preview: true`` — compute + validate only; returns the resulting
          ``{rungs, roles, roleCaps}`` so the modal can show the per-tier order
          before the operator confirms. No write.
        - otherwise — compute, validate, then write. ``scope == "pod"`` splices
          ...
          result wholesale through the tiers safe-write seam.
        ...
        if preview:
            return jsonify({"ok": True, "scope": scope, "preview": True, "catalog": catalog})
    ...
    def api_admin_config_set_routing(bot_id: str) -> Response:
        """Replace agents.defaults.model.routing for a bot."""
        rej = _reject_unknown_bot(bot_id)
        if rej is not None:
            return rej
        body = request.get_json() or {}
        ...
        if not result:
            return jsonify({"error": "write failed — check server logs"}), 500
        _module._audit_log_entry("config.routing.set", bot_id, {"routing": routing}, oc_keys={"agents"})
    ...
    def api_models_validate_model() -> Response:
    ...
    def api_models_update_tier() -> Response:
        ...
        from .model_tier_apply import model_landed, stage_tier_model, tier_models
    ...
    def api_models_update_tier_bulk() -> Response:
        ...
        for bot_id in bot_order:
            ...
            from .model_tier_apply import model_landed, stage_tier_model
