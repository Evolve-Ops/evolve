...
strategy memo ``internal/design-routes-admin-decomposition-2026-06-12.md`` (Option
...
from ..power_cap import reported_power_cap
from ..user_tier_override import UserTierOverrideWriteError, apply_user_tier_override
...
def register_admin_config_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_admin_config_set_tiers(bot_id: str) -> "Response | tuple[Response, int]":
        ...
        if not result:
            # Rung-collision (two roles → one rung, different models — a
            # hand-authored roles map can collide any two roles onto one
            # rung): the writer refuses an impossible fold rather than
    ...
    def api_admin_config_set_tier_mode(bot_id: str) -> "Response | tuple[Response, int]":
        ...
        **Strict all-or-nothing**: a bot is either Custom (its own full
        ...
        _module._audit_log_entry(
            ...
            {"mode": mode}, oc_keys=_tier_oc_keys(result, {"agents"}),
        )
    ...
    #   PUT  — validate (non-empty ladder roles) + write the pod
    ...
    def api_admin_config_pod_models_set() -> "Response | tuple[Response, int]":
        ...
        Body (one of):
          ...
          - ``{"rungs": [...], "roles": {...}, "roleCaps"?: {...}}`` —
            materialize/replace the pod default. Validated (non-empty ladder
            roles) BEFORE write — an invalid candidate is rejected, never
            persisted.
    ...
    def api_admin_config_set_fallback(bot_id: str) -> Response:
        ...
        if not result:
            return jsonify({"error": "write failed — check server logs"}), 500
        _module._audit_log_entry("config.fallback.set", bot_id, updates,
                                 oc_keys=_tier_oc_keys(result, {"agents"}))
        return jsonify({
            "ok": True, "bot": bot_id,
    ...
    def api_admin_config_set_cascade(bot_id: str) -> Response:
        ...
        _module._audit_log_entry(
            ...
            {"enabled": enabled}, oc_keys=_tier_oc_keys(result, {"agents"}),
        )
        return jsonify({
            "ok": True, "bot": bot_id,
    ...
    def api_admin_config_set_user_tier_override(bot_id: str) -> Response:
        ...
        Goes through ``user_tier_override.apply_user_tier_override`` — the ONE
        door, shared with ``evolve-admin models cap`` / ``user-tier-control``
        (that module's docstring carries the full why). Partial-merge into
        ``~/.openclaw/evolve-tiers.json::userTierOverride``, mirrored to
        ``{sharedDir}/{bot}/tiers.json``, and a ``dailyCap`` also lands
        ``roleCaps.power.maxPerDayPerBot`` in the same write. Writing the
        legacy key alone — what this endpoint did until 2026-09-10 — reached
        routing on no pod; writing no mirror left the admin chat Power gate
        blind to a UI-set ``{enabled: false, dailyCap: 0}`` opt-out. Both were
        the CLI's fixed bugs on the other surface.

        The response's ``effectivePowerCap`` is what the GATEWAY will enforce,
        resolved from the post-write config — not the number the caller sent,
        which is what both surfaces echoed while routing used a different one.
        ``null`` means the resolver could not run, never "no cap".

        The plugin's ``ModelRouter._resolveOperatorDefaultTier`` reads
        ``defaultTier`` on every turn and routes user-turn / ambiguous sessions
        to the chosen tier before falling through to OC's primary. Slots in the
        precedence ladder below the user override (chip + ``session_set_tier``)
        and below the cascade verdict — both still win when set.

        Audited as ``config.user_tier_override.set`` with the ``tiers:<key>``
        names the write actually landed (``tier_write_oc_keys``) so heal.py's
        drift detector credits this endpoint as the writer, not as foreign
        drift — a bare ``{"tiers"}`` is a name heal never emits (it namespaces
        per key), so that older declaration never matched. Declaring it HERE
        rather than in the shared writer is why ``audit_action=None`` below.
        """
        rej = _reject_unknown_bot(bot_id)
        if rej is not None:
            return rej
        ...
        try:
            # audit_action=None: recorded below under THIS surface's action,
            # so the operator log keeps a page click apart from a CLI run.
            result = apply_user_tier_override(
                _net, network_path, bot_id, body, audit_action=None)
        except UserTierOverrideWriteError as e:
            return jsonify({
                "error": f"user-tier-override write failed for {bot_id}: {e}",
            }), 500
        _module._audit_log_entry(
            "config.user_tier_override.set", bot_id,
            body, oc_keys=_tier_oc_keys(result, {"agents"}),
        )
        return jsonify({
            "ok": True, "bot": bot_id,
            "userTierOverride": result.get("userTierOverride") or {},
            "effectivePowerCap": reported_power_cap(_net, result),
        })
    ...
    def api_models_freshness_status() -> Response:
        ...
        from .model_freshness_drift import attach_behavior_pins
        ...
        if summary:
            ...
            attach_behavior_pins(summary, network_path)
    ...
    # The credential-scoped view of the listings cache lives in the
    # model_listings_filter helper (this file is a frozen no-growth hot
    # hazard); the credential readers are closures, so they are passed in.
    from .model_listings_filter import filter_listings_to_credentialed

    def _filter_listings_to_credentialed(
        cache: dict, net: dict, bot_id: str = "",
    ) -> dict:
        return filter_listings_to_credentialed(
            cache, net, bot_id,
            bot_providers_with_keys=_bot_providers_with_keys,
            pod_credentialed=lambda: _pod_credentialed_providers(net),
        )
    ...
    def api_models_listings() -> Response:
        ...
        An optional ``?bot_id`` scopes the candidate set to that bot's own
        credentials (fail-open to the pod union), so the picker on a bot tab
        does not offer models the bot has no key for. Omitting it preserves the
        pod-wide set the pod default editor needs.
        ...
        # Fourth request-derived bot_id (#3566 audit C-2): it flows to
        # ``oc_keys_get`` (no roster guard) → ``sudo -H -u <bot_id>``. Same
        # contract as validate-model: DENY the unknown id, then FALL OPEN to
        # the pod union — drop the scope rather than 404.
        bot_id = (request.args.get("bot_id") or "").strip()
        if bot_id and _reject_unknown_bot(bot_id) is not None:
            bot_id = ""
        return jsonify(_filter_listings_to_credentialed(cache, net, bot_id))
    ...
    def api_models_listings_refresh() -> Response:
        ...
        Enumeration stays pod-wide (the cache is shared); an optional body
        ``bot_id`` narrows only the RESPONSE, so a refresh from a bot tab
        returns the same scoped view that GET does.
        """
        import model_discovery as _model_discovery

        body = request.get_json(silent=True) or {}
        # Fifth request-derived bot_id (#3566 audit C-2), same DENY-then-fall-
        # open as validate-model; isinstance = a non-string body falls open
        # instead of 500ing on .strip().
        ...
        out = _filter_listings_to_credentialed(cache, net, bot_id)
        out["ok"] = True
        return jsonify(out)
    ...
    # The two tier-write endpoints (single Apply + Apply All) live in
    # model_tier_update_routes — moved there whole (this file is no-growth
    # capped) when the behavior-pin gate landed: a model a rollback rejected
    # for behavior is refused re-apply without an explicit override (the
    # 2026-08-21 recurrence). The roster guard is passed in so there stays
    # exactly one implementation of it.
    from .model_tier_update_routes import register_model_tier_update_routes
    register_model_tier_update_routes(
        app, network_path, reject_unknown_bot=_reject_unknown_bot,
    )
