...
**Bootstrap-safe, self-healing.** When no admin-auth key exists (fresh pod), auth
is NOT enforced — identical to today's behaviour, so nobody is locked out. The key
is created the first time the operator runs ``evolve-admin pair``; from then on the
server requires a paired device cookie. Deleting the key disables enforcement (the
documented escape hatch). Same shape as the proposal-signing self-heal in
``evolve_config``.
