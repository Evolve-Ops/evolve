...
def _register_trust_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_heartbeat_check() -> Response:
        ...
        #
        # Live turn JSONL, via the same reader the Cost Measures cap-warning
        # chip uses. This read `metrics/<today>/<bot>.json` until it was found
        # to be structurally near-blind: measure.py writes that file at 01:00
        # pod-local for the day it names and never regenerates it, so on
        # 2026-09-03 it held $0.95 of the pod's real $55.52 — 1.7%. An
        # 80%-of-threshold heartbeat reason built on it could not fire.
        threshold = float(alerts_cfg.get("spendThresholdUSD", 0))
        try:
            from spend_caps import get_today_spend as _today_spend
        except ImportError:
            _today_spend = None  # type: ignore[assignment]
        if threshold > 0 and _today_spend is not None:
            for bot_id in bots_cfg:
                try:
                    # No explicit day: the reader resolves the pod-local
                    # day itself, which is the calendar the turns are bucketed
                    # on. Passing this endpoint's date.today() would differ
                    # on a pod with a network.json TZ override, selecting an
                    # empty bucket.
                    spend = _today_spend(shared, bot_id)
                except Exception:
                    continue
                # None = discovery failed. Silence is signal on this endpoint,
                # so an unreadable bot must not manufacture a reason — but it
                # must not manufacture a reassuring $0.00 either; it is simply
                # not a heartbeat reason.
                if spend is None:
                    continue
                pct = spend / threshold
                if pct >= 0.80:
