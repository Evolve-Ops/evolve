...
def suggest_bot_id_via_llm(description: str, api_key: str) -> str:
    """LLM-assisted name from the description.

    Tier-3 (Haiku) call — ~$0.001 per invocation per the Q1 resolution
    in the wizard spec. Falls back to the heuristic on any failure.
...
def _suggest_bot_id_with_optional_llm(description: str) -> str:
    """Try LLM-assisted suggestion using the `evolve` user's key, else heuristic.

    The wizard runs as the `evolve` daemon user — same one that the
    Anthropic admin uses for tier-3 calls elsewhere. Reading the
    key lazily on each preview is fine; previews are infrequent
    operator actions, not hot-path.

    Always returns a valid bot_id (never raises).
    """
    if not description.strip():
        return suggest_bot_id_heuristic(description)
    # Try evo's key first (the primary bot is the de-facto source of
    # admin-side LLM credentials in this codebase), fall back to
    # evolve, then heuristic.
    api_key = ""
    try:
        from .server import _read_bot_api_key
        for user_guess in ("evo", "evolve"):
            api_key = _read_bot_api_key(user_guess)
            if api_key:
                break
    except Exception:
        api_key = ""
    if api_key:
        return suggest_bot_id_via_llm(description, api_key)
    return suggest_bot_id_heuristic(description)
