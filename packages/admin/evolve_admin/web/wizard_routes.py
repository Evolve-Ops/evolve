...
`internal/spec-add-bot-wizard-2026-05-28.md` §6 + §7.
...
from ..config import is_pod_bot, load_network
...
def register_wizard_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_credentials_borrow() -> Response:
        ...
        network = load_network(network_path)
        # is_pod_bot, not a bare members scan: the primary lives in the
        # sibling `primary` key and is the usual from_bot (it holds the keys).
        if not is_pod_bot(from_bot, network):
            return jsonify({"error": f"from_bot {from_bot!r} not a pod member"}), 400
        if not is_pod_bot(to_bot, network):
    ...
    def api_wizard_seed_model_config() -> Response:
        ...
        if not bot_id:
            return jsonify({"error": "bot_id required"}), 400
        network = load_network(network_path)
        if not is_pod_bot(bot_id, network):
