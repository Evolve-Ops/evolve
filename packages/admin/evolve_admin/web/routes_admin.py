...
from .credentials_api_key_rows import build_api_key_row, runtime_only_rows
from .credentials_oc import brave_key_from_oc_config
from . import credentials_runtime_evidence as _cred_evidence
from . import oc_agent_keys_io as _oc_agent_keys_io
from .oc_agent_keys import RUNTIME_STORAGE_ID
from .llm_key_rotate import rotate_oc_agent_store, undo_oc_agent_store
...
from .probes.oc_agent_catalog import OcAgentCatalogKeyProbe
from .probes import (
...
# to these. See internal/design-routes-admin-decomposition-2026-06-12.md §3.
...
def register_admin_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_admin_get_keys(bot_id: str) -> Response:
        ...
        # OC's per-agent runtime store, scanned ONCE per request and shared
        # by all eleven OcAgentCatalogKeyProbe instances (oc_agent_keys.py).
        _rt_memo: dict = {}

        def _scan_runtime_llm_keys(_bot: str, errors_out=None) -> list:
            if "hits" not in _rt_memo:
                _rt_memo["hits"] = _oc_agent_keys_io.scan_bot_runtime_llm_keys(
                    bot_id, network_path=network_path, errors_out=errors_out)
            return _rt_memo["hits"]
        ...
        probe_helpers = ProbeHelpers(
            ...
            scan_agent_llm_keys=_scan_runtime_llm_keys,
        )
        ...
        for prov, ptype, type_label, cred_class, category in _KEY_REGISTRY:
            ...
            # Feeds visibility rule (d) — see credentials_visibility.
            _entry = oc_cfg.get("plugins", {}).get("entries", {}).get(prov) or {}
            base = {
                "provider": prov,
                ...
                "order": None,
                "order_total": None,
                "plugin_enabled": _entry.get("enabled") is True,
            }
            ...
            if cred_class == "oauth":
                ...
                # api_key / identifier: single value. Two probes feed the row
                # builder; the cascade + brave special-case live in
                # credentials_api_key_rows (routes_admin.py is line-capped).
                # The wizard predicate pins key_type because anthropic has TWO
                # probes (token + api_key) and we want the one for THIS row.
                wizard_matches = _run_matching(
                    probes_for_prov,
                    predicate=lambda p: (
                ...
                # Runtime-store keys are API keys, so they attach to the
                # api_key row only — never to anthropic's MAX-token row.
                runtime_matches = _run_matching(
                    probes_for_prov,
                    predicate=lambda p: (
                        isinstance(p, OcAgentCatalogKeyProbe)
                        and ptype == "api_key"
                    ),
                    provider=prov,
                ) if ptype == "api_key" else []
                result_keys.append(build_api_key_row(
                    base=base, provider=prov, key_type=ptype, oc_cfg=oc_cfg,
                    wizard_match=wizard_matches[0] if wizard_matches else None,
                    runtime_match=runtime_matches[0] if runtime_matches else None,
                    bot_id=bot_id, network_path=network_path,
                    canonical_profile_id=_canonical_profile_id,
                    mask_key=_mask_key,
                    actions_for_winner=_actions_for_winner,
                ))
        ...
        # Last row producer by design — see runtime_only_rows' docstring.
        result_keys.extend(runtime_only_rows(_scan_runtime_llm_keys, bot_id, seen_providers))
        ...
        # The 4th arg feeds clause (d′) and is LAZY — only called when a row
        # would otherwise be hidden.
        finalize_credential_rows(
            result_keys, pod_invariants, _provider_warnings,
            lambda: _cred_evidence.providers_with_recent_turns(
    ...
    def api_admin_add_key(bot_id: str, provider: str) -> Response:
        """Add an API key to auth-profiles.json + mirror to openclaw.json. Mirroring is load-bearing: `_RUNTIME_MIRROR_PATH` providers are read from openclaw.json at runtime (brave → plugins.entries.brave.config.webSearch.apiKey), so an auth-profiles-only write reported ACTIVE while search stayed broken. Rotate always mirrored; add did not. See test_add_key_mirrors_to_openclaw."""
        ...
        mirror_ok, mirror_err = _mirror_to_openclaw(bot_id, provider, key_type, key_value)  # same registry lookup rotate uses (no-op True when unmirrored); `key_type` doubles as field_key. requires_restart: mirrored values go live only on gateway restart.
        _module._audit_log_entry("keys.add", bot_id, {"provider": provider, "type": key_type, "profile_id": profile_id, "mirrored": mirror_ok}, oc_keys=_oc_keys_for_storage("auth_profiles", provider, mirrored=mirror_ok))
        return jsonify({"ok": True, "profile_id": profile_id, "mirrored": mirror_ok, "mirror_error": mirror_err, "requires_restart": mirror_ok and provider in _RUNTIME_MIRROR_PATH})
    ...
    def api_admin_rotate_key(bot_id: str, provider: str) -> "Response | tuple[Response, int]":
        ...
        if storage == RUNTIME_STORAGE_ID:
            return rotate_oc_agent_store(
                bot_id, provider, key_value,
                network_path=network_path, audit=_module._audit_log_entry,
            )
    ...
    @app.post("/api/admin/keys/<bot_id>/<provider>/undo-rotation")
    def api_admin_undo_llm_rotation(bot_id: str, provider: str) -> "Response | tuple[Response, int]":
        """Put back the key the last UI rotation replaced (one rotation deep).

        Covers the bad-paste case the rotate flow's verify step surfaces:
        the new key is live and 401ing, and the operator needs the previous
        one back in every agent's file without hunting for it.
        """
        return undo_oc_agent_store(
            bot_id, provider,
            network_path=network_path, audit=_module._audit_log_entry,
        )
    ...
    def api_admin_onboard_google_callback() -> Response:
        ...
        # restart. P1 from internal/design/skills-install-roadmap-2026-05-30.md.
    ...
    # + OS-ACL mode toggle). See internal/design/skills-install-roadmap-2026-05-30.md
    ...
    #   * internal/spec-google-workspace-suite-2026-06-04.md
    #   * internal/vetting-workspace-mcp-2026-06-04.md
    ...
    # preflight_check). See:
    #   * internal/spec-google-workspace-suite-2026-06-04.md §10.2
    ...
    def api_skills_catalog_list() -> Response:
        ...
        # (see internal/openclaw-coverage-audit-2026-06-04.md). Wiring now
        ...
        # still off the list — see internal/design/paste-token-skills-future-2026-05-30.md.
        ...
        # Obsidian — see internal/design/skills-install-roadmap-2026-05-30.md.
        ...
        # the deep skills audit; see internal/skills-deep-audit-2026-05-30.md).
    ...
    def api_admin_restart_gateway(bot_id: str) -> "Response | tuple[Response, int]":
        """Restart bot's openclaw gateway through the Scheduler seam.
        Uses oc_gateway_restart() — launchctl on macOS, systemctl on Linux.
        """
        body = request.get_json() or {}
    ...
    def api_admin_stop_gateway(bot_id: str) -> "Response | tuple[Response, int]":
        """Stop bot's openclaw gateway — does NOT restart.

        launchd: bootout (transient). systemd: ``disable --now`` (persistent).
        Either way the on-disk artifact survives, so /restart brings it back.
        Useful for stopping a crash-looping gateway without triggering a
        service-manager respawn.
        ...
        if not body.get("confirm"):
            return jsonify({"error": "confirm required"}), 400
        # Platform handling lives in oc_cli beside oc_gateway_restart — this
        # route used to inline a launchd-only bootout loop, which meant a hard
        # 500 on every Linux pod (the fail-fast launchd accessor RAISES there).
        from runtime.agent_runtime import get_runtime
        result = get_runtime().gateway_stop(bot_id)
        if not result.get("ok"):
            return jsonify(result), 500
        _module._audit_log_entry("gateway.stop", bot_id, {
            "service": result.get("service", ""), "domain": result.get("domain", ""),
        })
        return jsonify(result)
