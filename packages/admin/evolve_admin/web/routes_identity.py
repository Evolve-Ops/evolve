...
def register_routes(app: Flask, network_path: Path) -> None:
    ...
    def identity_overview() -> Any:  # type: ignore[no-redef]
        ...
        admin_external = admins.get("external_ids", {}) or {}
        has_admin = any(
            isinstance(v, list) and v for v in admin_external.values()
        )
        ...
        for bot_id, cfg in bots.items():
            ...
            primary_external = primary.get("external_ids") or {}
            has_primary = (
                isinstance(primary_external, dict)
                and any(primary_external.values())
            )
            missing: list[str] = []
