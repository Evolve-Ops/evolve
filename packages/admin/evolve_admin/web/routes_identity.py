...
def _check_pod_admin(net: dict, action: str) -> "tuple[bool, str | None]":
    """Pod-scoped analogue of ``routes_bot_users._check_capability``.

    ``_check_capability`` resolves a role **on a bot** — it needs a bot_id for
    the overlay load and for ``roster_overlay.resolve_role``. ``claim-admin``
    and ``revoke-admin`` write ``pod.admins.*``, which is pod-WIDE: there is no
    bot to scope to, and naming an arbitrary one would be a fiction.

    So this reuses the settled *transport* semantics verbatim (absent header ⇒
    trusted only for the authenticated admin UI; malformed ⇒ denied on every
    transport) and substitutes the one role rule that is meaningful pod-wide:
    **only an existing pod admin may mint or remove a pod admin.**

    That is a restriction of existing semantics, not a new tier.
    ``roster_overlay.resolve_role`` already returns ``"admin"`` for exactly
    this predicate — ``stable_id in _pod_admin_ids_for(net, platform)``,
    bot-independent — and ``"admin"`` maps to ``["*"]`` in
    ``capabilities.DEFAULT_ROLE_CAPABILITIES``. Any caller this helper admits
    would already resolve to ``admin`` (hence every capability) on every bot.
    ...
    There is no single overlay at pod scope, so this checks every bot's
    overlay and denies an identity blocked on ANY of them. Without it the
    pod-scoped gate would be strictly LOOSER than the per-bot one, and the
    operator's block would be trivially escapable: a blocked pod admin,
    refused by ``claim-primary``, could still call ``claim-admin`` to mint a
    second, unblocked pod-admin id and walk straight back in on every bot.

    Deliberately NOT a new ``pod.admins.mutate`` built-in: adding a key to
    ``BUILTIN_CAPABILITIES`` widens the ``"*"`` expansion in
    ``resolve_role_capabilities``, silently granting the new permission to any
    role an operator has bound to ``"*"`` in a per-bot ``role_bindings``
    overlay. That would be a capability-semantics change; this is containment.

    Bootstrapping a fresh pod is unaffected — neither first-admin path goes
    through this route. ``evo.dispatch`` (the DM ``evo claim <passphrase>``
    flow) and ``evo.wizard.engine`` (the setup wizard challenge step) both call
    ``evo.identity.claim_admin`` in-process, gated on the pod admin
    passphrase. The admin UI reaches the route but sends no header, so it is
    trusted by the same rule ``_check_capability`` applies.
    """
    ...
    # Sticky deny FIRST — same precedence resolve_role uses (blocked beats
    # the pod-admin claim). Fails CLOSED: an overlay that will not load is
    # treated as no block rather than crashing the route, but a readable
    # overlay carrying a block is decisive.
    try:
