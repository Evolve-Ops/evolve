...
#: Memo for :func:`_cache_retention_resolution`, keyed by
#: ``(bot_id, configured, UTC hour)``. Resolving ``auto`` parses a week of
#: turn JSONL three times over; without this, a GET of the Cost page and the
#: PATCH that saves it each pay for that, and a busy bot's settings read
#: blocks on tens of thousands of rows before the page renders. An hourly key
#: keeps the answer fresh enough for a knob that only changes at deploy time
#: while collapsing a page-load-and-save into one measurement.
#: Clear it (``.clear()``) to force the next read to re-measure.
_RESOLUTION_MEMO: dict[tuple[str, str | None, str], dict | None] = {}


def _cache_retention_resolution(bot_id: str, configured: str | None) -> dict | None:
    """What ``configured`` resolves to for ``bot_id``, and the reason.

    ``None`` when the resolver is unavailable — the client renders the raw
    setting alone rather than an invented resolution. Never raises: this is a
    decoration on a settings read, and a settings page that 500s because a
    turns file was unreadable is worse than one that shows a little less.

    Memoized per ``(bot_id, configured, UTC hour)`` — see
    :data:`_RESOLUTION_MEMO`. The failure result is memoized too: a resolver
    that just blew up will blow up again this hour, and re-parsing a week of
    JSONL to re-learn that is the cost this memo exists to avoid.
    """
    from datetime import datetime, timezone

    key = (bot_id, configured, datetime.now(timezone.utc).strftime("%Y-%m-%dT%H"))
    if key in _RESOLUTION_MEMO:
        return _RESOLUTION_MEMO[key]
    try:
        cs = _import_analyzer("cache_shape")
        # No ``days``: cache_shape.AUTO_TIER_WINDOW_DAYS is the one home for
        # the window, so the page and the deploy answer the same question over
        # the same stretch of traffic. Two copies of that number is the "UI
        # reader and enforcement reader consult different things" defect the
        # context-observability spec names.
        stats = cs.measure_gap_stats(bot_id) if configured == "auto" else None
        resolved = cs.resolve_retention(configured, stats).as_dict()
    except Exception as exc:  # noqa: BLE001
        _log.warning("cache tier resolution for %s failed: %s", bot_id, exc)
        resolved = None
    # Bounded: one entry per (bot, value, hour). Trim the previous hours
    # rather than growing for the life of the daemon.
    if len(_RESOLUTION_MEMO) > 256:
        _RESOLUTION_MEMO.clear()
    _RESOLUTION_MEMO[key] = resolved
    return resolved


_THRASH_MEMO: dict[tuple[str, str], dict | None] = {}


def _cache_thrash_summary(bot_id: str) -> dict | None:
    """Cache-THRASH turns over the auto-tier window, by magnitude (tokens).

    Named apart from the prefix re-warm (``cache_shape.is_cache_thrash`` vs
    ``is_prefix_rewarm``): a turn that re-caches its own tail on every call
    reads as a cache HIT under the re-warm definition
    (internal/incident-post-mortem-2026-09-17-single-turn-cache-thrash.md §5).
    ``None`` = could not measure; the client says so rather than showing 0.
    Memoized per ``(bot_id, UTC hour)`` for the same reason as
    :data:`_RESOLUTION_MEMO`.
    """
    from datetime import datetime, timezone

    key = (bot_id, datetime.now(timezone.utc).strftime("%Y-%m-%dT%H"))
    if key in _THRASH_MEMO:
        return _THRASH_MEMO[key]
    try:
        cs = _import_analyzer("cache_shape")
        ct = cs.measure_bot(bot_id).cross_turn
        out = None if ct is None else {
            k: v for k, v in ct.as_dict().items() if k.startswith("cache_thrash_")
        }
        if out is not None:
            out["window_days"] = cs.AUTO_TIER_WINDOW_DAYS
    except Exception as exc:  # noqa: BLE001
        _log.warning("cache thrash summary for %s failed: %s", bot_id, exc)
        out = None
    if len(_THRASH_MEMO) > 256:
        _THRASH_MEMO.clear()
    _THRASH_MEMO[key] = out
    return out
...
def register_cost_measures_routes(app: Flask, network_path: Path) -> None:
    ...
    Also registers the bot-facing ``POST /api/cost-checkpoint/answer``
    (D-CC1..4): the plugin's ``before_agent_run`` gate calls it to record
    an owner's "continue" / "stop" answer to a tripped cap. It lives with
    the rest of the spending-cap surface rather than in ``server.py``'s
    registration block, which is under a no-growth cap.
    """
    from .cost_checkpoint_bot_routes import register_cost_checkpoint_bot_routes
    register_cost_checkpoint_bot_routes(app, network_path)
    ...
    def api_bot_openclaw_cost_get():
        ...
        # What that setting RESOLVES to, and why. "auto" is the only value
        # whose stored form does not say what will be deployed, and a knob
        # whose effect the operator cannot read is a knob they cannot trust —
        # so the resolution ships beside the setting rather than only in the
        # deploy log.
        settings["cache_retention_resolved"] = _cache_retention_resolution(
            bot_id, settings.get("cache_retention"),
        )
    ...
    def api_bot_openclaw_cost_patch():
        ...
        new_settings["cache_retention_resolved"] = _cache_retention_resolution(
            bot_id, new_settings.get("cache_retention"),
        )
        return jsonify({"ok": True, "bot_id": bot_id, "settings": new_settings})

    @app.get("/api/bot/cache-thrash")
    def api_bot_cache_thrash():
        """Cache-thrash turns for ``?bot=``, by tokens. Its own endpoint so
        the settings read keeps its one-measurement-per-hour contract and
        never reads turns for a pinned tier. ``thrash: null`` = unmeasured."""
        bot_id = request.args.get("bot", "")
        if not bot_id:
            return jsonify({"error": "bot required"}), 400
        return jsonify({"bot_id": bot_id, "thrash": _cache_thrash_summary(bot_id)})
    ...
    def api_spend_caps_get():
        ...
        Response shape:
          ...
          in_turn_checkpoints          — {bot: count} of runs the in-turn
                                         checkpoint (D-CS12) stopped in 7d.
        """
        try:
        ...
        try:
            in_turn = _import_analyzer("breakers.in_turn").count_by_bot(shared, members)
        except Exception:  # noqa: BLE001 — a count, never the page
            in_turn = {}
        return jsonify({
            "in_turn_checkpoints": in_turn,
            "config": config,
    ...
    def api_spend_caps_patch():
        ...
        try:
            ...
            if action == "downgrade-tier" and isinstance(hard, (int, float)):
                ...
                # Picking the checkpoint must actually RETIRE a downgrade
                # rung that a previous ``downgrade-tier`` mirror wrote at
                # the L1 number — otherwise the bot still gets silently
                # downgraded on the way to its own checkpoint, which is
                # the exact behaviour D-CC2 removed. Only the mirrored rung
                # (one set to the L1 cap) is dropped; a rung the operator
                # deliberately set to a DIFFERENT, lower threshold is a
                # separate control and is left alone.
                if pod_budget.get("tier_downgrade_usd") == hard:
                    pod_budget.pop("tier_downgrade_usd", None)
