...
POST /api/arbiter/proposals/<id>/reject
...
def register_arbiter_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_arbiter_proposals() -> Response:
        ...
        try:
            ...
            # rest to the Alerts page — see docs/spec-recommendations-
            ...
            for p in filtered:
                ...
                # Effectiveness-Layer triage (docs/spec-effectiveness-layer-
                ...
                # Spec: docs/spec-fit-reviewer-2026-06-12.md §5.
            ...
            def _sort_key(v):
                ...
                # depends on this ordering. Spec: docs/spec-fit-reviewer-
    ...
    def api_arbiter_proposal_act(proposal_id: str) -> Response:
        try:
            ...
            with store.locked(shared_dir):
                ...
                sm.transition(
                    proposal, "approved_human", actor="user", reason="user clicked Act"
                )
    ...
    def api_arbiter_proposal_refine(proposal_id: str) -> Response:
        ...
        try:
            ...
            if not api_key:
                return (
                    jsonify(
                        {
                            "error": (
                                f"no Anthropic API key found for bot "
                                f"{proposal.bot_id!r}. The bot's "
                                f"auth-profiles.json must contain an "
                                f"anthropic api_key entry for refine to "
                                f"work."
                            )
                        }
                    ),
            ...
            try:
                llm_call = make_anthropic_caller(
                    api_key, bot_id=proposal.bot_id,
                )
                ...
                return (
                    jsonify(
                        {
                            "error": (
                                f"anthropic SDK not available: {exc}. "
                                f"Install it on the admin host."
                            )
                        }
                    ),
                    500,
            ...
            apply_refinement(
                proposal, result, feedback=feedback, actor="user"
            )
            store.write_proposal(proposal, shared_dir, subdir=subdir)
    ...
    def api_arbiter_proposal_dismiss(proposal_id: str) -> Response:
        ...
        # bad. Spec: docs/spec-alerts-signal-store-2026-05-07.md §9.
        ...
        resp = _transition_endpoint(proposal_id, to_status="dismissed")
        ...
        # docs/spec-proposal-drafting-protocol-2026-06-04.md
    ...
    def api_arbiter_rate_limit_state() -> Response:
        ...
        try:
            ...
            def _matches_surface(p) -> bool:
                ...
                # for routing — see docs/spec-rsi-proposal-eligibility-
    ...
    def api_arbiter_profile_get(bot_id: str) -> Response:
        ...
        docs/spec-user-profile-2026-05-07.md §D6.
    ...
    def api_arbiter_bot_setup_post(bot_id: str) -> Response:
        ...
        where each is set). Spec: docs/spec-cost-caps-2026-06-05.md.
        """
        try:
    ...
    # Phase 5 of the cost-cap normalization (spec: docs/spec-cost-caps-2026-06-05.md).
    ...
    def api_arbiter_health_watchdog_events() -> Response:
        ...
        (docs/spec-alerts-signal-store-2026-05-07.md) added a unified
