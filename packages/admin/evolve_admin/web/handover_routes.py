...
def _render_success(rec: dict, network: dict) -> dict:
    ...
    # Channel hint — quietly mention messaging if we know it; otherwise generic.
    channel = _primary_channel_hint(network, bot_id)
    channel_line = (
        f"<p class=\"subtle\">Say hi to {_esc(display)} on <strong>{_esc(channel)}</strong> "
        "— it's already there waiting.</p>"
        if channel
        else f"<p class=\"subtle\">{_esc(display)} is ready. Say hi in your usual chat app.</p>"
    )
...
def _primary_channel_hint(network: dict, bot_id: str) -> str:
    """Return a friendly channel name if we can infer one from network.json.

    Looks at bot.channel / bot.transport / bot.messaging — falls back to
    empty string when uncertain. Conservative: don't tell the user
    "find me on Slack" if we're not sure.
    """
    bot = (network.get("bots") or {}).get(bot_id) or {}
    for key in ("channel", "transport", "messaging"):
        v = bot.get(key)
