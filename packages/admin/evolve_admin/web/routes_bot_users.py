...
Endpoints (all under ``/api/admin/bots/<bot_id>/users``):
  ...
  GET  /                  → ``{bot_id, by_channel: {<ch>: {supported, approved[], group_access[], nested_group_access[], pending[], ...}}}``
  ...
  POST /group-allowlist/revoke  → body ``{channel, id, path?}``
                                                          → re-renders by_channel
...
DM store, so the two lists stay strictly separate. A revoke may also carry a
``path`` naming a NESTED member list (Discord's ``guilds.<gid>.users`` /
``guilds.<gid>.channels.<cid>.users``, surfaced by the GET's
``nested_group_access[]``) — B3a, 2026-09-22. Nested lists are revoke-only, and
the path is resolved against the lists the live config actually contains, never
traversed as given.
...
def _handle_group_action(network_path: Path, bot_id: str, *, add: bool):
    ...
    B3a (2026-09-22): a revoke may carry ``path`` — the nested member list the
    id actually lives in (Discord's ``guilds.<gid>.users``), as the GET
    rendered it. Approve never takes one: nested lists are revoke-only.
    """
    body = request.get_json(silent=True) or {}
    channel = (body.get("channel") or "").strip()
    ext_id = (body.get("id") or "").strip()
    ...
    if not ext_id:
        return jsonify({"error": "missing 'id'"}), 400
    # B3a — an optional nested target (``["guilds", "<gid>", "users"]``),
    # echoed back from the group the GET rendered. Shape-checked here; WHICH
    # list it may name is decided against the live config in
    # ``_resolve_nested_list``, never by traversing these strings.
    try:
        path = _parse_nested_path(body.get("path"))
    except _PairingError as e:
        return jsonify({"error": str(e)}), 400
    if path is not None and add:
        return jsonify({"error": (
            "nested member lists are revoke-only — add to the channel's "
            "top-level group allowlist instead")}), 400
    net = load_network(network_path)
    if not _bot_exists(net, bot_id):
        return jsonify({"error": f"unknown bot: {bot_id}"}), 404
    ...
    if not ok:
        return jsonify({"error": "forbidden", "detail": reason}), 403
    try:
        warning = _group_allowlist_write(
            net, bot_id, channel, ext_id, add=add, path=path)
    except _PairingError as e:
        return jsonify({"error": str(e)}), 400
...
def _parse_nested_path(raw: Any) -> "list[str] | None":
    """Validate a request's nested-list ``path``; ``None`` when absent.

    Shape only — a list of 2..``IDENTITY_WALK_MAX_DEPTH`` non-blank strings
    whose last segment is an identity-list key. A one-segment path would be a
    TOP-level list, which has its own (effective-key) targeting and must not be
    reachable this way; a last segment outside ``IDENTITY_LIST_KEYS`` is not a
    list of people at all (``roles``, ``tools``, a token field). Whether the
    path exists is a separate question, answered against the live config.
    """
    if raw is None:
        return None
    if not isinstance(raw, list) or not (2 <= len(raw) <= rr.IDENTITY_WALK_MAX_DEPTH):
        raise _PairingError(
            "invalid 'path': expected the nested member list's path as it "
            "appeared in the users listing")
    segs: list[str] = []
    for seg in raw:
        if not isinstance(seg, str) or not seg.strip():
            raise _PairingError("invalid 'path': segments must be non-empty strings")
        segs.append(seg)
    if segs[-1] not in rr.IDENTITY_LIST_KEYS:
        raise _PairingError(
            f"invalid 'path': {segs[-1]!r} is not a member list "
            f"({', '.join(sorted(rr.IDENTITY_LIST_KEYS))})")
    return segs
...
def _read_per_channel(network: dict, bot_id: str) -> dict[str, dict]:
    ...
    B3a (2026-09-22) adds ``nested_group_access[]`` — the per-container member
    lists that live BELOW the channel block (Discord's
    ``guilds.<gid>.users`` / ``guilds.<gid>.channels.<cid>.users``), which the
    top-level reader cannot see (see ``rr.read_nested_group_allowlists``).
    """
    out: dict[str, dict] = {}
    ...
    try:
        ...
        group_gated = rr.group_allowlist_gated_channels(
            network, bot_id, oc_reader=_oc_reader)
        # Nested per-container member lists (Discord's
        # ``guilds.<gid>.users`` / ``guilds.<gid>.channels.<cid>.users``).
        # The top-level readers above cannot see them, so before this an
        # identity that existed ONLY in a guild list rendered as a
        # gated-but-EMPTY channel — the operator was told by the coherence
        # Signal to "look at the access lists" on a page that had no row for
        # them, and hand-editing openclaw.json was the only remediation.
        nested_group = rr.read_nested_group_allowlists(
            network, bot_id, oc_reader=_oc_reader)
    except Exception as exc:  # noqa: BLE001
    ...
    for ch in KNOWN_PROVIDERS:
        ...
        for gid in (group_allowlists.get(ch) or []):
            ...
            grec = rr.build_record(
                ...
                overlay=overlay, activity=activity, shared_dir=shared_dir,
                access_sources=(rr.ACCESS_GROUP_ALLOWLIST,))
            gentry = _record_to_approved_entry(grec)
            gentry["access_source"] = rr.ACCESS_GROUP_ALLOWLIST
            _attach_directory(gentry, network, bot_id, ch, gid, grec,
                              overlay=overlay, directory=directory,
                              shared_dir=shared_dir)
            group_access.append(gentry)
        # Nested member lists, grouped by the container that holds them so the
        # provenance ("which guild is this id in") is stated once per list
        # rather than repeated on every row. Members resolve through the SAME
        # canonical join as ``approved``/``group_access``; each group carries
        # the exact ``config_path`` the operator would edit and the ``path``
        # the nested revoke writes to.
        nested_group_access: list[dict] = []
        for lst in nested_group.get(ch) or []:
            members: list[dict] = []
            for nid in lst.ids:
                nrec = rr.build_record(
                    network, bot_id, ch, nid,
                    overlay=overlay, activity=activity, shared_dir=shared_dir)
                nentry = _record_to_approved_entry(nrec)
                nentry["access_source"] = "group_allowlist_nested"
                _attach_directory(nentry, network, bot_id, ch, nid, nrec,
                                  overlay=overlay, directory=directory,
                                  shared_dir=shared_dir)
                members.append(nentry)
            if not members:
                continue
            nested_group_access.append({
                "path": [str(seg) for seg in lst.path],
                "config_path": lst.config_path(ch),
                "container_label": lst.container_label,
                "key": lst.key,
                "members": members,
            })
        ...
        # Nested members are admitted on the same gate (the channel is
        # ``groupPolicy: allowlist`` or they would not be surfaced), so they
        # must not read as "not admitted" either.
        group_access_ids |= {
            m["id"] for grp in nested_group_access for m in grp["members"]
        }
        ...
        out[ch] = {
            ...
            # Nested per-container member lists (guild / guild-channel), each
            # with the config path it lives at. Empty for every channel with
            # no nested lists — i.e. everything but Discord today.
            "nested_group_access": nested_group_access,
            "pending": pending,
...
def _enrich_unknown_names(network: dict, by_channel: dict[str, dict],
    ...
    for ch, ch_data in by_channel.items():
        ...
        for list_key in ("approved", "group_access"):
            entries.extend(ch_data.get(list_key) or [])
        # Nested guild/guild-channel members sit one level down, grouped by
        # the list they live in — same enrichment need, same treatment.
        for group in (ch_data.get("nested_group_access") or []):
            entries.extend(group.get("members") or [])
...
def _resolve_nested_list(
    block: dict, path: "list[str]",
) -> "rr.IdentityList":
    """The nested identity list ``path`` names inside a channel block.

    The client sends a path of plain strings (exactly what the GET emitted on
    the group it is revoking from). This never traverses that string: it
    enumerates the block's real identity lists with
    ``rr.walk_identity_lists`` and looks the request up among THEM, then uses
    the enumerated (server-side, correctly typed) path to address the list. So
    a request can only ever reach a list the read side already surfaced — never
    an arbitrary key, never a token field, and never a key that does not exist
    yet. Raises ``_PairingError`` (→ 400) when there is no such list.
    """
    by_str = {
        tuple(str(seg) for seg in lst.path): lst
        for lst in rr.walk_identity_lists(block)
        if lst.is_nested
    }
    found = by_str.get(tuple(path))
    if found is None:
        raise _PairingError(
            f"no nested member list at {'.'.join(path)!r} in this bot's "
            f"openclaw.json — reload the Users page (the config may have "
            f"changed since it was rendered)")
    return found
...
def _apply_group_allowlist_change(
    ...
    ``path`` (revoke only) targets a NESTED member list instead —
    ``("guilds", "<gid>", "users")`` and friends — resolved through
    ``_resolve_nested_list`` so only a list the GET actually surfaced can be
    written. The returned ``target_key`` is then the full relative path, which
    is what lands in the write's audit ``reason``.
    ...
    if path is not None:
        # Nested list: the container holding it is the curated artifact, so we
        # address it exactly (never re-derive a key) and only ever REMOVE.
        # Removal is strictly narrowing, which is why the nested surface is
        # revoke-only: granting a new member inside someone's guild list is a
        # config authoring decision, and the top-level add box already covers
        # "let this person into the channel".
        if add:
            raise _PairingError(
                "nested member lists are revoke-only — add to the channel's "
                "top-level group allowlist instead")
        lst = _resolve_nested_list(block, path)
        node: Any = block
        for seg in lst.path[:-1]:
            node = node[seg]
        key = str(lst.path[-1])
        ...
        node[key] = [x for x in current if x != ext_id]
        return cfg, lst.relative_path
...
def _group_allowlist_write(
    ...
    result = _apply_group_allowlist_change(
        current, channel, ext_id, add=add, path=path)
    if result is None:
