...
def _parse_requester_identity() -> "tuple[str, str, str | None] | None":
    ...
    Returns ``None`` when the header is absent — caller falls back to
    UI-trusted behavior (admin UI is the implicit auth layer per
    feedback_ui_authorization_presumed memory).
    """
...
def _check_capability(net: dict, bot_id: str,
    ...
    absent, returns ``(True, None)`` — the UI is the trusted client and
    capability gating doesn't apply (the UI surfaces are already
    UI-only-accessible by virtue of the admin's web auth).
