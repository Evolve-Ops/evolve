...
def _pod_admin_ids_for(network: dict, channel: str) -> set[str]:
    ...
    return {str(x) for x in (ext.get(channel) or []) if x}


def _primary_id_for(network: dict, bot_id: str, channel: str) -> str | None:
...
def _parse_requester_identity() -> "tuple[str, str, str | None] | None":
    ...
    Returns ``None`` when the header is absent. A header-less request is
    trusted **only** when it positively arrived via the authenticated
    admin-UI HTTP transport (``_is_authenticated_ui_request``); a
    header-less caller on the admin unix socket (a bot gateway) is
    DENIED — the header is self-asserted, so its absence is not proof of
    the UI (fail-CLOSED backstop, audit G-N3). See ``_check_capability``.
    """
