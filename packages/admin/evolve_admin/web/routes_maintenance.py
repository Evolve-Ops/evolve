...
/api/oc/compat/override         record an operator override for an unvalidated version
...
def _register_maintenance_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_oc_version() -> Response:
        ...
        # Has Evolve been validated against the version we would offer?
        # "npm has something newer" and "Evolve works on it" were the same
        # question to this card until 2026-09-07 — they are two now
        # (internal/design-oc-upgrade-safety-2026-09-08.md §2 row 3).
        from .. import oc_compat as _cc
        validation = _cc.validation_state(latest_version, shared_dir=shared_dir).to_json()
        validation["confirmation_phrase"] = (
            _cc.override_confirmation_phrase(latest_version, validation["failing"])
    ...
    @app.post("/api/oc/compat/override")
    def api_oc_compat_override() -> "Response | tuple[Response, int]":
        """Record an operator override for a version Evolve has not validated.

        It is their pod, so the override exists — but the confirmation the
        operator types must NAME the failing contract checks, so the
        sentence is itself the disclosure, and every override is appended to
        an on-disk log with who and when. A mismatched confirmation is a
        400 quoting the exact phrase; nothing is written.
        """
        from .. import oc_compat as _cc
        ...
        ok, err = _cc.record_override(
            version, confirmation=confirmation, shared_dir=shared_dir,
        )
        if not ok:
            return jsonify({"error": err}), 400
        _log.warning("oc-compat: operator override recorded for OpenClaw %s", version)
        return jsonify({
            "ok": True,
            "validation": _cc.validation_state(version, shared_dir=shared_dir).to_json(),
        })
    ...
    # ── Per-bot OC preflight (hold-fix-4277-preflight-fails-closed-and-is-
    #    wired-to-the-upgrade, items 2/4) ────────────────────────────────────
    #
    # Distinct from the safe-upgrade gates above: this is oc_preflight.py's
    # per-bot table (model-ref rewrites, workspace-outside-home, …), cached
    # per exact from → to pair so it runs once per pair, not on every page
    # load. api_oc_upgrade_apply below is the server-side half of the gate
    # this exists for — a UI-disabled button alone is bypassable.

    @app.post("/api/oc/preflight/check")
    def api_oc_preflight_check() -> "Response | tuple[Response, int]":
        """Spawn a background per-bot preflight and return its report_id.

        Body: ``{"target": "<version>"}`` — the exact candidate version, not
        "latest"; the cache this writes to is keyed on the literal pair.
        """
        ...
        network = load_network(network_path)
        shared_dir = Path(network.get("sharedDir") or DEFAULT_SHARED_DIR)
        installed = _uv.installed_package_version()
        rid, status = _pfs.start_background_check(
            network, target, actor="web:operator",
            installed_version=installed, shared_dir=shared_dir,
        )
        return jsonify({
    ...
    @app.get("/api/oc/preflight/report/pair")
    def api_oc_preflight_report_pair() -> "Response | tuple[Response, int]":
        """The cached report for the current installed → ``target`` pair.

        404 means this exact pair has never been checked — not that it is
        clean. Read by the apply modal on open, never on a bare page load,
        so re-opening the same pair's plan doesn't re-trigger a fetch.

        A report that exists but is too old or predates a bot-set change
        (hold-fix-4392) is still returned — 200, with ``stale``/
        ``stale_reason`` set — so the modal can show "checked 3d ago" and a
        re-run affordance instead of conflating "stale" with "never run".
        """
        from .. import oc_preflight_store as _pfs
        from .. import upstream_version as _uv
        from ..oc_preflight import bot_set_hash as _bot_set_hash
        ...
        shared_dir = Path(network.get("sharedDir") or DEFAULT_SHARED_DIR)
        installed = _uv.installed_package_version()
        data = _pfs.load_cached_for_pair(installed, target, shared_dir=shared_dir)
        if data is None:
            return jsonify({"error": "no cached preflight for this pair"}), 404
        reason = _pfs.pair_freshness_reason(data, bot_set_hash=_bot_set_hash(network))
        return jsonify({**data, "stale": reason is not None, "stale_reason": reason})

    @app.get("/api/oc/preflight/report/<report_id>")
    def api_oc_preflight_report(report_id: str) -> "Response | tuple[Response, int]":
        """A specific preflight report by id — the apply modal polls this."""
        from .. import oc_preflight_store as _pfs

        network = load_network(network_path)
        shared_dir = Path(network.get("sharedDir") or DEFAULT_SHARED_DIR)
        data = _pfs.load_report(report_id, shared_dir=shared_dir)
        if data is None:
            if _pfs.inflight_report_id() == report_id:
                return jsonify({"report_id": report_id, "status": "running"})
            return jsonify({"error": "no such report", "report_id": report_id}), 404
        return jsonify(data)
    ...
    def api_oc_upgrade_apply() -> "Response | tuple[Response, int]":
        ...
        # A report written BEFORE the compat-contract gate existed can be
        # `ok: true` without ever having asked whether Evolve is validated
        # against the target — and the staleness check above compares
        # versions, not gates, so it would let that report through. Re-derive
        # the answer here rather than trusting the report's age.
        from .. import oc_compat as _cc
        _target = (report.get("candidate") or {}).get("resolved_version")
        _val = _cc.validation_state(_target, shared_dir=shared_dir)
        if _val.state != _cc.STATE_TESTED and not _val.overridden:
            failing = ", ".join(_val.failing)
            return jsonify({
                "error": (
                    f"Evolve has not been validated against OpenClaw {_target}"
                    + (f" (failing: {failing})" if failing else "")
                    + " — it won't upgrade your pod onto it. Use \u201cUpgrade "
                      "anyway\u2026\u201d on the Update card if you want to go "
                      "regardless."
                ),
            }), 409

        # Per-bot OC preflight gate (item 2 — the reason this endpoint's
        # caller, the Update control, is meant to be disabled until this
        # passes). Enforced HERE, not only in the UI: a disabled button is a
        # suggestion, this 409 is the actual gate. Keyed on the exact pair,
        # same as the compat-contract check just above.
        #
        # hold-fix-4392: a passing run has to still describe the pod, not
        # just have happened once — `pair_freshness_reason` re-derives that
        # from the report's own age and bot-set hash, never trusted from the
        # caller (spec-shape identical to `_report_stale_reason` above).
        from .. import oc_preflight_store as _pfs
        from ..oc_preflight import bot_set_hash as _bot_set_hash
        _pf_installed = (report.get("current") or {}).get("installed_version")
        _pf_target = (report.get("candidate") or {}).get("resolved_version")
        _pf_cached = (
            _pfs.load_cached_for_pair(_pf_installed, _pf_target, shared_dir=shared_dir)
            if _pf_target else None
        )
        if _pf_cached is None:
            return jsonify({
                "error": (
                    f"No per-bot OpenClaw preflight has been run for "
                    f"{_pf_installed or '?'} → {_pf_target or '?'} — run it "
                    f"from the Update card before upgrading."
                ),
            }), 409
        _pf_reason = _pfs.pair_freshness_reason(
            _pf_cached, bot_set_hash=_bot_set_hash(network),
        )
        if _pf_reason == "bot_set_changed":
            return jsonify({
                "error": (
                    "A bot was added or removed since the per-bot OpenClaw "
                    f"preflight for {_pf_installed} → {_pf_target} ran — "
                    "re-run it from the Update card before upgrading."
                ),
            }), 409
        if _pf_reason == "stale":
            return jsonify({
                "error": (
                    f"The per-bot OpenClaw preflight is "
                    f"{_pfs.describe_age(_pf_cached)} old — re-run it from "
                    "the Update card before upgrading."
                ),
            }), 409
        if _pf_cached.get("blocking"):
            return jsonify({
                "error": (
                    "The per-bot OpenClaw preflight found blocking issues: "
                    f"{_pf_cached.get('summary') or 'see the report'}. "
                    "Resolve them and re-run the preflight."
                ),
            }), 409
