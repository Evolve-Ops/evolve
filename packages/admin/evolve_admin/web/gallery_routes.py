...
def register_gallery_routes(app: Flask, network_path: Path, shared_dir: Path) -> None:
    ...
    def api_gallery_preflight(pkg_id: str) -> Response:
        ...
        Returns a structured preflight result with two sections:
          ...
          requirements     — integrations, secrets, system tools, python packages
    ...
    def api_gallery_install(pkg_id: str) -> Response:
        ...
        try:
                ...
                if not force:
                    ...
                    if not prereq["satisfied"]:
                        ...
                        try:
                            ...
                            _oauth.start_oauth_wait(job.job_id, prereq["missing"], shared_dir)
            ...
            # Return 202 when at least one bot is awaiting OAuth — the client
            # should poll /api/forge/jobs/<job_id> and show the action_url.
            status_code = 202 if any_awaiting_oauth and not errors else 200
    ...
    def api_forge_job_retry(job_id: str) -> Response:
        ...
        try:
            ...
            try:
                ...
                # primary defence against the sha256-mismatch bug.
