...
def register_gallery_routes(app: Flask, network_path: Path, shared_dir: Path) -> None:
    ...
    def api_gallery_import() -> Response:
        ...
        try:
            ...
            # identity: NOT resolve_app_id — gallery ``app_id`` is the
            # script name, see brief §8 (D1). Echoes back the gallery package
            # key that ``import_package`` just validated (``p-<8hex>``) and
            # filed the package under; a wire contract with the uploader. An
            # uploaded package may carry its own ``app_id``, which the resolver
            # returns ahead of ``pkg_id`` — echoing a key the package was not
            # filed under. PR #3681 narrows that to conforming values only.
        ...
        try:
            ...
            def _pkgs_to_install(bot_id: str) -> list[dict]:
                ...
                return [
                    ...
                    # ``installed_state`` is keyed by the gallery package id,
                    # and these closure rows are gallery records whose own
                    # ``app_id`` holds the script name. Same rule as
                    # gallery_verify/deps.py; resolving here misses every
                    # row until PR #3681 lands.
                ...
                for bot_id in bot_ids:
                    ...
                    if len(chain_pkgs) > 1:
                        ...
                        for link_pkg in chain_pkgs:
                            # identity: NOT resolve_app_id (§8 D1) — gallery
                            # package key; it indexes the per-package cost
                            # projection the UI joins back onto the chain rows.
    ...
    def api_gallery_consistency() -> Response:
        ...
        try:
            ...
            # identity: NOT resolve_app_id — this endpoint compares
            # INSTALLED MANIFESTS against GALLERY PACKAGES,
            # so both ids below are load-bearing and distinct:
            #   * the manifest's package id is the gallery binding — its
            #     absence is exactly what marks a manifest as not
            #     gallery-sourced (skip), and it is what
            #     ``get_removed_status``/``get_update_status`` key on.
            #   * ``m.id`` is the manifest filename stem, which is what the
            #     ``app_id`` response field means to every UI caller and to
            #     ``load_manifest(app_id, bot_id, …)``. For a gallery install
            #     ``resolve_app_id`` returns the ``pkg_id`` instead
            #     (AL-1.4 §6 delta 12), so it is NOT a substitute here.
            # Both are ``ApplicationManifest`` dataclass fields; the resolver
            # takes a raw manifest dict.
            for m in manifests:
                if not m.pkg_id:
                    continue
                ...
                if removed_info:
                    issues.append({
                        "app_id": m.id,       # identity: NOT resolve_app_id
                        "pkg_id": m.pkg_id,   # identity: NOT resolve_app_id
    ...
    def api_applications_forge(bot_id: str, app_id: str) -> Response:
        ...
        try:
            ...
            job = create_improvement_job(
                # identity: NOT resolve_app_id — the job carries BOTH ids
                # on purpose: ``pkg_id`` is the gallery
                # binding (falling back to the route's app_id for a
                # non-gallery app, so the job always has a package key), and
                # ``app_id`` is the route path segment / manifest filename
                # stem. Collapsing them is AL-1.4c work, not 1.4b's.
