...
Phase E.3 (internal/spec-evo-account-separation-2026-05-25.md) routes evo's
...
def register_admin_bot_routes(app: Flask, network_path: Path) -> None:
    ...
    def admin_breakers_reset():
        ...
        try:
            # Imported for the availability check only — reset_and_enforce
            # lazily imports breakers.store itself, but a missing analyzer
            # package should read as "backend unavailable" here rather than
            # as a 500 from inside the helper.
            from breakers import store as _bstore  # noqa: F401
            from evolve_admin import breakers_enforce as _enforce
            from evolve_admin.config import load_network
        except ImportError as exc:
        ...
        enforce_result = outcome.enforce
        if not outcome.was_tripped:
            return jsonify({
                "ok": True,
                "scope": scope,
                "breaker_type": breaker_type,
        ...
        if enforce_result is None:
            # Scope gone from network.json — cleared, but not brought up.
            return jsonify({
                "ok": False,
