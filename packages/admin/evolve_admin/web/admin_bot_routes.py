...
Endpoints registered here:
  ...
  * C2 — ``GET /api/admin/bot/<bot_id>/backup-status`` (Phase E.3.2)
    Replaces evo's in-process ``pod_state.backup_status`` tool's
...
C3 is a re-plumb only — no new endpoint, just point ``pod_state.bots``
...
Additional endpoints (post-Phase-E):
  ...
  * ``PUT /api/admin/bot/<bot_id>/auto-memory`` — toggles OC's
    ...
    ``action.bot.set_auto_memory`` evo tool. Writes via
...
def register_admin_bot_routes(app: Flask, network_path: Path) -> None:
    ...
    def admin_bot_backup_status(bot_id: str):
        ...
        Same shape evo's ``pod_state.backup_status`` returned when it
    ...
    def admin_bot_set_auto_memory(bot_id: str):
        ...
        write_reason = (
            f"action.bot.set_auto_memory(enabled={enabled})"
        ...
        try:
            ...
            set_intent(
                ...
                set_by="action.bot.set_auto_memory",
                set_by_detail=(
