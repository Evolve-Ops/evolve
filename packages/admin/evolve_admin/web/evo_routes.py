...
Spec: internal/spec-evo-wizard-2026-05-05.md (dispatch + identity + guide +
wizard) and internal/spec-primary-bot-interface-2026-05-14.md §4 (help
retrieval) + §6 (intake).
...
from . import routes_bot_users as _bot_users
...
def register_evo_routes(app: Flask, network_path: Path) -> None:
    ...
    def api_evo_identity_get(bot_id: str) -> Response:
        ...
        # Normalized on the way out: the API always speaks the list shape
        # ({channel: [id, ...]}) regardless of what is on disk (M1-B2).
        external_ids = _external_ids.read_external_ids(primary_block)
    ...
    def api_evo_identity_claim(bot_id: str) -> Response:
        ...
        network = load_network(network_path)
        # Capability gate — same floor as the admin-side claim-primary and the
        # roster mutations in routes_bot_users. This route writes
        # `bots.<id>.primary_user`; it had no capability check at all, so the
        # trusted evo peer uid on the admin unix socket (exempted from the
        # device gate by peer_auth.device_gate_trusted_peer on every pod) and
        # any caller on an auth-disabled pod reached the write header-free.
        ok, deny_reason = _bot_users._check_capability(
            network, bot_id, "bot.roster.mutate")
        if not ok:
            return jsonify({"error": "forbidden",
                            "detail": deny_reason}), 403

        # Capture the prior value so the audit record reflects what was
        # actually there before mutation (None on first claim). Read via
        # the tolerant reader — indexing the raw dict wrote the literal
        # "['12345']" into the audit trail whenever the entry was already
        # in list shape, and made the idempotent re-claim look like a
        # change (M1-B2).
    ...
    def api_evo_identity_reassign(bot_id: str) -> Response:
        ...
        network = load_network(network_path)
        # Capability gate — reassign transfers ownership of the bot. The
        # `from` check proves the caller knows the CURRENT primary; it is a
        # correctness guard against blind overwrites, not an authorization
        # check (the current primary is readable from GET /api/evo/identity/
        # <bot_id>, which is unauthenticated on the same transports).
        ok, deny_reason = _bot_users._check_capability(
            network, bot_id, "bot.roster.mutate")
        if not ok:
            return jsonify({"error": "forbidden",
    ...
    def api_evo_intake_create() -> Response:
        ...
        intake = _intake_env.Intake(
            id=_intake_store.new_intake_id(shared_dir),
        ...
        try:
            _intake_store.create_intake(intake, shared_dir)
        except (PermissionError, OSError) as e:
