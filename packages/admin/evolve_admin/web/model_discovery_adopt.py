...
Spec: docs/spec-model-rungs-and-roles-2026-06-09.md §Addendum 12 (2026-06-15).
...
_VALID_ROLES = {"none", "fast", "standard", "power", "judge", "max"}
...
# The role picker always offers the four general rungs; Judge is a constrained
# role that only some pods define, so it's appended only when the catalog has it.
_BASE_PICKER_ROLES = ["fast", "standard", "power", "max"]


def picker_roles(network: dict) -> list[str]:
    """The role buttons the adopt card's segmented picker shows. Always the four
    general rungs (Fast/Standard/Power/Max); Judge only when the pod (or, if the
    pod hasn't customized roles, the shipped default catalog) defines a judge
    rung. Provider-neutral — reads the catalog ``roles`` map, no model literals."""
    pod_roles = ((network or {}).get("models") or {}).get("roles") or {}
    if not pod_roles:
        try:
            from primary_bot import default_model_catalog  # type: ignore

            pod_roles = (default_model_catalog() or {}).get("roles") or {}
        except Exception:  # pragma: no cover — catalog import is best-effort
            pod_roles = {}
    roles = list(_BASE_PICKER_ROLES)
    if "judge" in pod_roles and "judge" not in roles:
        roles.append("judge")
    return roles
...
_ROLE_PROMINENCE = ["max", "power", "standard", "fast", "judge"]
...
def list_version_upgrades(shared_dir: Path, network: dict) -> list[dict]:
    ...
    rung_slug, roles, role_label}``. Only upgrades with a real ``rung_slug``
    are surfaced — an unlocated model has no apply target."""
    rows: list[dict] = []
    for u in _compute_upgrades(shared_dir, network):
        if not u.rung_slug:
            # No rung to extend → not one-click appliable; skip (don't strand a
            # row whose Update button can't act).
            continue
