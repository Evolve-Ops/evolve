...
text, this module falls back to a fast-tier LLM call so the operator gets a
real conversational reply instead of the canned "I didn't recognize
that" message.

Key decisions, anchored to the prior conversation:

* **Primary bot's credentials** — calls go out with evo's own
  credentials, resolved provider-agnostically via ``infra_llm``
  (#3466), honoring the "per-bot LLM inference, never centralized"
  principle in the operator's memory. The admin server hosts the
  route but the inference is billed against the bot.

* **Fast tier, not the workhorse tier** — the conversation is mostly
  summarizing pod state + suggesting evo subcommands; the cheap tier
  handles that well at ~5-10x lower cost. The cost discussion in the
  design conversation established "very cheap status, tokens for
  guidance + fixing" as the priority.
...
# Model + credential resolution live in the route layer via the
# provider-agnostic ``infra_llm`` resolver (#3466); the transport below
# reconstructs the target from the provider-qualified ``model`` + key.
# Operator model override path: network.json::home_chat.model
# (provider-qualified, e.g. "anthropic/claude-haiku-4-5").
...
# Fast-tier pricing snapshot (per 1M tokens, USD; Haiku-class rates).
# Lives here so the cost-meter math is auditable. The meter is
# informational either way (operator can verify against the Admin API
# ingest under Maintenance) — token counts are estimated (~4 chars per
# token) since infra_llm returns text only.
...
# ─────────────────────────────────────────────────────────────────────────────
# LLM call (provider-agnostic via infra_llm; stubbable for tests)
# ─────────────────────────────────────────────────────────────────────────────


# Transport signature: (system_prompt, messages, api_key, model, max_tokens)
#   -> {"text": str, "input_tokens": int, "output_tokens": int}
# ``model`` is provider-qualified (the provider rides in the prefix), so
# the (api_key, model) pair round-trips a full infra_llm target.
...
def _default_transport(
    ...
    if "/" not in model:
        raise ValueError(
            f"model {model!r} is not provider-qualified — resolve the "
            "target via infra_llm first (route layer)"
        )
    ...
    # infra_llm returns text only — estimate token counts (~4 chars per
    # token) for the informational cost meter.
...
# every page reload would burn an LLM call. Cache key is the SHA1 of
...
# the fallback channel: a small, tool-less fast-tier LLM call routed through
# the same pod-wide LLM credential the briefing narrative already uses.
#
...
def call_llm(
    *,
    ...
    """Make one LLM completion call (via infra_llm). Returns:
...
def _sanitize_history(history: list[dict], *, max_turns: int) -> list[dict]:
    ...
    ``{role: "user"|"assistant", content}`` messages shape.
