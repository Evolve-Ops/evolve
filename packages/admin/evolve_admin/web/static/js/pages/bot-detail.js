...
// Scaffold-only bot predicate (EVO-SEP-S4): a residual `bots.<id>` entry that
// carries NO `role`, NO `user`, and NO `port` — only a `setup_checklist`
// scaffold. On stale / pre-#3053 pods a phantom `bots.evolve` of this shape
// survives in network.json even after the code correctly resolves the primary
// to `evo`; it must never render as a bot tab, tile, or card. Mirrors the
// server-side S2/S2b suppression (status.py keys the primary under the real id
// and never mints a literal `bots.evolve`) so the card and the per-page tab
// iterators share one definition of "scaffold-only" rather than drifting.
// The on-disk residue itself is removed by a clean re-provision, not a live
// migrate — see internal/spec-evo-account-separation-2026-05-25.md (EVO-SEP-S4).
function isScaffoldOnlyBot(info) {
  if (!info || typeof info !== 'object') return false;
  return !info.role && !info.user && !info.port;
}

// True when `info` has the shape of a network.json `bots.<id>` CONFIG record
// — the only shape isScaffoldOnlyBot() can read. Status, posture and
// synthetic records (e.g. /api/gateway/status's `{gateway_running, ts,
// gateway_pid, source}`) carry none of `role` / `user` / `port`; in THOSE
// payloads their absence means "this response doesn't carry config", not
// "this bot is a phantom". Judging one with isScaffoldOnlyBot() drops every
// bot in the payload.
function isBotConfigShape(info) {
  if (!info || typeof info !== 'object') return false;
  return ('role' in info) || ('user' in info) || ('port' in info);
}
...
// the visible order matches across pages. Pass ANY `{botId: <record>}` shape —
// _statusData.bots, _networkData.bots, /api/gateway/status, the posture
// inventories, a synthesized stub map: the values are only used for ordering
// hints, never as evidence that a bot doesn't exist.
//
// Scaffold-only entries (isScaffoldOnlyBot) are filtered out here so every
// consumer — Dashboard, Plugins, AI-Optimization, and the rest — skips the
// phantom without each re-deriving the predicate. The filter is resolved
// against the bot's CONFIG (`_networkData.bots[id]`) whenever network.json is
// loaded, because "scaffold-only" is a statement about network.json, not
// about whatever payload the caller happens to hold. An id that network.json
// doesn't carry at all is not a network.json phantom — it is kept. Only when
// `_networkData.bots` is unavailable (boot fetch failed) do we fall back to
// judging the passed entries, and then ONLY if the passed map is itself
// config-shaped — otherwise every id is kept.
//
// Regression this pins (2026-08-31, evolve-stable-616): Maintenance › Status
// and Gateway Logs rendered "No bots configured." on a 9/9-online pod because
// the status-shaped /api/gateway/status payload was fed through the config
// predicate and every bot read as a phantom. Same family as the
// AI-Optimization bot-tabs regression.
//
...
function orderedBotIds(botsObj) {
  if (!botsObj) return [];
  const allIds = Object.keys(botsObj);
  const cfgBots = (_networkData && _networkData.bots) || null;
  const haveCfg = !!cfgBots && Object.keys(cfgBots).length > 0;
  // One config-shaped entry is enough to call the whole map config-shaped: a
  // phantom is defined by the ABSENCE of role/user/port, so it can only be
  // recognised inside a map whose other entries carry them.
  const callerIsConfig = allIds.some(id => isBotConfigShape(botsObj[id]));
  const ids = allIds.filter(id => {
    if (haveCfg) {
      const cfg = cfgBots[id];
      return cfg ? !isScaffoldOnlyBot(cfg) : true;
    }
    return callerIsConfig ? !isScaffoldOnlyBot(botsObj[id]) : true;
  });
  ...
  if (!primaryId || !ids.includes(primaryId)) {
    primaryId = null;
    for (const id of ids) {
      // Prefer the config record — a status/posture payload carries no role.
      const b = (haveCfg && cfgBots[id]) || botsObj[id];
