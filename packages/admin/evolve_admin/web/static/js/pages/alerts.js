...
//
// Spec: docs/spec-alerts-signal-store-2026-05-07.md
//
...
function _alGroupKey(sig) {
  ...
  // expandable row. Spec: docs/spec-recommendations-rework-2026-06-02.md.
...
function _alSignalRow(sig) {
  ...
  // three actions are also what evo's action.signal.* tool registry
  // calls; two surfaces (UI buttons + tools) routed to identical
...
async function _alLoadLane(flavor) {
  ...
  try {
    ...
    if (apiFlavor === 'maintenance') {
      ...
      // — includes id (so evo can call action.signal.snooze /
      // action.signal.dismiss without re-fetching), severity, scope,
...
// See docs/spec-alerts-count-normalization-2026-06-06.md.
