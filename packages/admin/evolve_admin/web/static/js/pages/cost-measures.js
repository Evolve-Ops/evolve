...
function _cmRenderTile(t) {
  ...
  // Housekeeping lever (D-CS5: on by default, per-bot opt-out in
  // network.json bots.<id>.cost.levers.housekeeping_cheap_rung). Painted
  // from the tile payload itself — no fetch, so it never shows a placeholder.
  // Absent (older server) renders nothing rather than a guess.
  const hkOn = t.levers ? t.levers.housekeeping_cheap_rung : undefined;
  const hkLeverHtml = (hkOn === undefined) ? '' : `<span class="cm-lever-chip"
          title="${hkOn
            ? 'Compaction and memory notes run on the cheap model, tool-less, thinking off — never changes which model answers'
            : 'Opted out: compaction and memory notes run on the conversation model (network.json cost.levers.housekeeping_cheap_rung: false)'}"
          style="display:inline-block;padding:1px 7px;border-radius:10px;background:var(--bg3);color:${hkOn ? 'var(--green)' : 'var(--text3)'};border:1px solid var(--border)">housekeeping: ${hkOn ? 'cheap' : 'off'}</span>`;
...
function _cmPaintLevers(botId, setup) {
  ...
  // spec (internal/spec-cost-caps-2026-06-05.md §"Resolution"):
...
// internal/spec-cost-caps-2026-06-05.md §"UI".
...
async function _cmLoadChips() {
  ...
  try {
    ...
    // in_turn_checkpoints — runs the in-turn checkpoint (D-CS12) stopped.
    const itc = d?.in_turn_checkpoints || {};
    const itcBots = Object.keys(itc);
    if (itcBots.length > 0) {
      const n = Object.values(itc).reduce((a, b) => a + b, 0);
      chips.push({
        severity: 'warn',
        label: `in-turn checkpoints: ${n} this week`,
        detail: `a turn was stopped at its per-turn spend on ${itcBots.slice(0, 3).join(', ')}${itcBots.length > 3 ? '…' : ''}`,
        scroll: '#cm-bot-caps-card',
      });
    }
...
const _CM_FIELDS = [
  ...
  { group: 'Compaction',
    // Replaces the retired "Reserve floor" (compaction.reserveTokensFloor —
    // OpenClaw 2026.9.2 rejects it and fixes the reserve at 20k tokens).
    // Values are bytes of active transcript; keys match what deploy and the
    // built-in profiles write (compaction_housekeeping.py, cost_profiles.py).
    key: 'compaction.maxActiveTranscriptBytes', label: 'Compaction budget',
    desc: 'Summarize older history once the conversation transcript passes this size, instead of waiting until the model\'s context window is nearly full (on 1M-token models that can be ~980k tokens, re-read on every message). Smaller = cheaper messages and earlier summaries; larger = more verbatim history kept. The bot writes its memory note just before each summary, so nothing durable is lost.',
    type: 'enum',
    options: { '': 'None (window − 20k)', 262144: '256 KB (~85k tokens)', 409600: '400 KB (~130k tokens)', 1048576: '1 MB (~330k tokens)' } },
  { group: 'Compaction',
  ...
  { group: 'Optimization',
    ...
    desc: "Anthropic prompt-cache TTL applied to every Anthropic model, and the only setting that changes how long a cached prefix lives. short = 5-min (writes cost 1.25x base input); long = 1-hour (writes cost 2x). Long only pays off when the prefix would otherwise be re-written more than ~1.6 times an hour — it eliminates invalidations on multi-minute conversational gaps, but a bot whose turns are hours apart breaks a 1-hour cache just as often and simply pays the higher write. \"auto\" runs exactly that arithmetic on this bot's own inter-turn gaps over the last 7 days and picks each deploy; when there is too little history to size it, auto declines and leaves the setting unwritten rather than guessing.",
    type: 'enum',
    // 'auto' is not an OpenClaw value — it is stored in BE config and
    // resolved to short/long at deploy time (cache_shape.resolve_retention).
    // The row's description carries what it currently resolves to, because a
    // knob whose effect the operator can't read is a knob they can't trust.
    options: { '': '— (OC default: short)', 'short': 'short (5-min)', 'long': 'long (1-hour)', 'auto': 'auto (measured)' } },
];
...
let _cmCacheThrash = null;            // /api/bot/cache-thrash payload, or null = unmeasured
...
async function _cmLoadSettings() {
  ...
  try {
    ...
    const [s, p, th] = await Promise.all([
      api('GET', `/api/bot/openclaw-cost?bot=${encodeURIComponent(_cmBot)}`),
      api('GET', '/api/cost-profiles'),
      // Decoration only: a failed thrash read renders "not measured".
      api('GET', `/api/bot/cache-thrash?bot=${encodeURIComponent(_cmBot)}`).catch(() => null),
    ]);
    _cmCurrentSettings = s.settings || {};
    _cmCacheThrash = th && th.thrash;
...
// What a setting whose stored value doesn't state its effect currently
// resolves to. Only 'cache_retention: auto' has that shape today: the server
// merges `cache_retention_resolved` into the settings payload, and without
// this the operator would have to read a deploy log to find out which cache
// tier "auto" chose for them.
function _cmResolutionNote(field) {
  if (field.key !== 'cache_retention') return '';
  const r = _cmCurrentSettings && _cmCurrentSettings.cache_retention_resolved;
  const thrash = _cmThrashNote();
  if (!r || !r.reason) return thrash;
  if ((_cmCurrentSettings.cache_retention || '') !== 'auto') return thrash;
  return `Right now — ${r.reason} ${thrash}`;
}

// Cache THRASH (a turn wrote more cache than it read) — deliberately not
// called a re-warm, and rendered by TOKENS: an 8M-token turn and a 150k one
// are one turn each. No cache tier fixes it; it is a loop inside one turn.
function _cmThrashNote() {
  const t = _cmCacheThrash;
  if (!t) return 'Cache thrash: not measured.';
  const n = t.cache_thrash_turns || 0;
  if (!n) return `Cache thrash (last ${t.window_days}d): none.`;
  return `Cache thrash (last ${t.window_days}d, turns that wrote more cache than they read): ` +
    `${n} turn(s), ${Number(t.cache_thrash_write_tokens).toLocaleString()} tokens written, ` +
    `largest ${Number(t.cache_thrash_max_write_tokens).toLocaleString()} — a cache tier cannot fix this; it is a loop inside a turn.`;
}
...
function _cmRenderMatrix() {
  ...
  const rows = _CM_FIELDS.map(field => {
    ...
    const resolvedNote = _cmResolutionNote(field);
    const descHtml = isExpanded
      ? `<div class="cm-setting-desc">${escHtml(field.desc)}${resolvedNote ? ' ' + escHtml(resolvedNote) : ''}</div>`
      : '';
