...
function _renderCostBotTiles() {
  ...
  // Audit B6: turns Evolve could not price in this window. A spend figure with
  // a non-zero count beside it is a FLOOR — the tile must say so rather than
  // present a confident total (the $0.00-on-232-real-turns failure).
  let totUnpriced = 0;
  for (const [, b] of costBots) {
    const t = b.tile;
  ...
  const renderTile = (id, b) => {
    ...
    const unpriced = sessionsActive
      ? 0
      : (isAll ? totUnpriced : (c ? (c[`unpriced_${win}`] || 0) : 0));
    const unpricedChip = unpriced > 0
      ? `<div><span class="cost-bot-tile-chip">can't price ${unpriced} turn${unpriced === 1 ? '' : 's'}</span></div>`
      : '';
    ...
    // Name the providers in the tooltip so the operator knows WHICH turns are
    // missing a price, not just how many.
    const unpricedProviders = (c && Array.isArray(c.unpriced_providers))
      ? c.unpriced_providers : [];
    const unpricedSuffix = unpriced > 0
      ? ` — can't price ${unpriced} turn${unpriced === 1 ? '' : 's'}${unpricedProviders.length ? ` (${unpricedProviders.join(', ')})` : ''}; the figure shown is a floor`
      : '';
    const titleAttr = sessionsActive
      ? `${label} — ${win} sessions${asOfSuffix}`
      : `${label} — ${win} spend${liveSuffix}${asOfSuffix}${unpricedSuffix}`;
