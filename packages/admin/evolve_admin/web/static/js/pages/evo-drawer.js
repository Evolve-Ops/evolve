...
const _EVO_CONTEXT_PACKS = {
  ...
  'feedback': () => ({
    ...
    tool_pointers: [
      ...
      { tool: 'pod_state.signals.firing', for: 'see what is currently firing while you investigate' },
  ...
  'integrations-keys': () => {
    ...
    return {
      ...
      tool_pointers: [
        ...
        { tool: 'action.plugin.enable(bot_id=..., plugin_name=...)',
        ...
        { tool: 'action.plugin.disable(bot_id=..., plugin_name=...)',
  ...
  // pod_state.signals.firing tool with no notion of what's on the
  ...
  'reports': () => {
    ...
    return {
      ...
      // Each item carries id so evo can call action.signal.snooze /
      // action.signal.dismiss directly when the operator says "the
      ...
      tool_pointers: [
        { tool: 'pod_state.signals.firing',
          for: 'the FULL firing-alerts list when the visible top-12 isn\'t enough. Filter by producer / bot_id / scope when the operator names one. Each Signal has the same id evo can pass to action.signal.snooze / action.signal.dismiss.' },
  ...
  'self-improvement': () => {
    ...
    return {
      ...
      tool_pointers: [
        ...
        { tool: 'pod_state.proposals.in_process',
  ...
  // Errors as having tool support via pod_state.errors but no page
  ...
  // will (and historically did) fall back to pod_state.signals.firing
  ...
  'maintenance': () => {
    ...
    if (subtab === 'status') {
      ...
      const items = entries.map(([botId, s]) => {
        ...
        // the pod_state.errors tool (pointer below). Each line carries
  ...
  'settings': () => {
    ...
    summary.tool_pointers = [
      ...
      { tool: 'pod_state.bots(bot_id=...)',
        for: 'runtime tile data (chips, status, version, recent activity) for the same bot — pairs with config.bot when the operator asks "why is X showing Y?"' },
      { tool: 'config.network',
