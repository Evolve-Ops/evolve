...
// apps-shell.js.
//
// ONE APP ON N BOTS (ALPHA-3a). The same app discovered separately on two
// bots has two stored ids, so it used to be two rows each saying "on 1 bot"
// — the opposite of what this page's own header promises. The route now
// makes a reversible CLAIM that such ids are one app and sends one row for
// them, carrying `grouped: true` and a `members` array of exactly the rows
// it stands for. This module renders the claim OUT LOUD ("these look like
// the same app") and gives every grouped row a Show-separately toggle that
// swaps in its members — no re-fetch, because `members` is already here.
// Splitting is per-row and lives in memory only: nothing is stored, and a
// reload comes back grouped.
// ════════════════════════════════════════════════════════════════════════
...
// Lead app ids the operator has chosen to see split. Session-only by
// design: the grouping is a claim about what the pod looks like, not a
// setting, so it must not persist into a pod that has since changed.
let _appsSplitRows = new Set();
...
// A grouped row the operator has split becomes its member rows, in place.
// Everything downstream (filters, render) then treats them as ordinary
// rows, so there is one row renderer and not two.
function _appsExpandSplits(rows) {
  const out = [];
  for (const a of rows) {
    if (a.grouped && _appsSplitRows.has(a.app_id) && (a.members || []).length) {
      for (const m of a.members) out.push(Object.assign({}, m, { _splitFrom: a.app_id }));
    } else {
      out.push(a);
    }
  }
...
function _appsRowsAfterFilters() {
  const rows = _appsExpandSplits((_appsListData && _appsListData.apps) || []);
...
function appsRenderList() {
  ...
  const total = _appsExpandSplits(_appsListData.apps || []).length;

  if (!total) {
    host.innerHTML = _appsEmpty(
      'No defined apps on this pod yet.',
      'Apps arrive three ways: Evolve scans each bot — when it arrives, then weekly — and you promote what it finds (see Discovered), you install one from the Gallery, or you author one with + New app.',
    );
    return;
  }
  if (!rows.length) {
...
// Show separately / Group again. The claim is stated in words on the row
// itself rather than hidden in a tooltip, because a row that silently
// merged two of an operator's apps would be a worse lie than the two rows
// it replaced. No field names, no similarity vocabulary (design §7).
function appsToggleSplit(leadId) {
  if (_appsSplitRows.has(leadId)) _appsSplitRows.delete(leadId);
  else _appsSplitRows.add(leadId);
  appsRenderList();
}

function _appsGroupNote(a) {
  if (a._splitFrom) {
    return `<div style="font-size:0.72rem;color:var(--text3);margin-top:3px">
      Shown on its own.
      <span class="link" role="button" tabindex="0"
        onclick="event.stopPropagation();appsToggleSplit('${escHtml(a._splitFrom)}')"
        onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();event.stopPropagation();appsToggleSplit('${escHtml(a._splitFrom)}')}"
        >Show together again</span>
    </div>`;
  }
  if (!a.grouped) return '';
  return `<div style="font-size:0.72rem;color:var(--text3);margin-top:3px">
    Each bot found this app for itself — they look like the same app, so they are shown as one.
    <span class="link" role="button" tabindex="0"
      onclick="event.stopPropagation();appsToggleSplit('${escHtml(a.app_id)}')"
      onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();event.stopPropagation();appsToggleSplit('${escHtml(a.app_id)}')}"
      >Show separately</span>
  </div>`;
}
...
function _appsListRow(a) {
  ...
  const kindChip = a.app_kind && a.app_kind !== 'application'
    ...
    : '';
  // A row the operator has split opens a split detail: the two surfaces
  // must agree, or "Show separately" would hand back the merged view one
  // click later.
  const open = `appsShowDetail('${escHtml(a.app_id)}',${a._splitFrom ? 'false' : 'true'})`;
  return `
...
window.appsToggleSplit = appsToggleSplit;
