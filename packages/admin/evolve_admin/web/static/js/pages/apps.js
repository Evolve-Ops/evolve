...
// internal/build-AL-1.4-app-id-canonical.md §3). This page reads API response
// payloads, not raw manifests, and the API already carries the id columns
// separately. Every match in this file is one of three semantically-required
// kinds, none of which a resolver can answer:
//
//   * `m.pkg_id` — the gallery package. Rendered as its own "Stable package
//     ID" chip beside the app id, passed as the file-index query param, and
//     compared against a file marker's `owned_by` to decide owned / shared /
//     external. `owned_by` is an ATTRIBUTION NAMESPACE: the question is
//     "which package claims this file?", and answering with a resolved id
//     would mark every externally-owned file as owned.
//   * `provenance.spec_id` — the Instance's BINDING to a Spec, used to build
//     the gallery tier path and to match an orphan file's marker to the one
//     Instance that claims it. It can legitimately be null while the
//     Instance has a perfectly good identity — which is exactly the state
//     the orphan-attach flow exists to repair.
//   * `f.spec_id` / `f.instance_id` — file-index marker columns handed back
//     by the API verbatim, and the fields the stamp/attach actions write.
//
...
// AL-1.8a: repaint after an app-lifecycle action. The per-bot grid this
// used to fill is retired (its subtab and DOM left with the pod-first
// shell), so the shim keeps the NAME's meaning and changes its target:
// repaint whichever pod-first surface is on screen. Every `await
// loadCapabilities()` below is one of those "I just changed something,
// show it" calls and keeps working unchanged — rewriting ten call sites
// to say the same thing would have been churn, not clarity. The legacy
// body still runs on a page that HAS the old grid, which is the case
// only until 1.8c deletes the retired renderers outright.
async function loadCapabilities() {
  if (!document.getElementById('cap-grid')) {
    if (typeof appsShowDetail === 'function' && _appsDetailId) {
      await appsShowDetail(_appsDetailId);
    } else if (typeof appsLoadList === 'function') {
      await appsLoadList();
    }
    if (typeof appsLoadDiscovered === 'function'
        && document.querySelector('#apps-discovered.active')) {
  ...
  // scheduled-action extraction. Spec: internal/spec-audit-extensions-2026-05-17.md §3.5.
...
// The inline "New Application" form was retired by AL-1.8a (operator
// decision D-U4): there is ONE way to author an app — the wizard, behind
// "+ New app" — rather than a four-field form that minted a manifest the
// forge had never seen and no test ever ran. showNewCapabilityForm /
// hideNewCapabilityForm / updateCapId / saveCapability went with the form
// and its DOM.

// App-test rollup removed 2026-06-08 — surface killed per
// internal/decision-app-tests-2026-06-08.md. Audit + coherence already cover
...
// ── Promotion readiness (AL-1.6b; design §7.1, brief §7.2) ─────────────────
// Ranks a DISCOVERED app as a promotion candidate. Surfaces the number and
// what drives it — NOT the four-column Apps page rebuild, which is AL-1.8.
//
// Two choices here that the number is misleading without:
//
//  * A null score renders as "not enough signal", never as 0. "We cannot tell"
//    and "we looked and it is bad" are different statements, and a tile that
//    renders them alike is worse than one that renders neither.
//  * The "scored on N of 3 signals" note is NOT decoration. Two of design
//    §7.1's three components have no producer on the pod today (recurrence
//    reads the v13 migration default; identity stability has no carrier at
//    all), so a 100 here means "this manifest declares files" and nothing
//    more. Dropping the note would let the number read as a three-part
//    judgement it is not.
//
// Reuses the existing .score-breakdown / .score-bar-* primitives, so there is
// no new CSS and both themes come free from the token vars.
function _readinessBandColor(band) {
  if (band === 'ready') return 'var(--green)';
  if (band === 'emerging') return 'var(--yellow)';
  return 'var(--text3)';
}

function _readinessChip(c) {
  const r = c.readiness;
  if (!r) return '';
  // DEFENSIVE, and currently unreachable: the evidence dimension is always
  // measurable (an empty manifest is the scanner having looked and found
  // nothing), so the scorer cannot return a null score with today's dimension
  // set. Kept for a future set in which evidence itself can be unmeasurable.
  if (r.score === null || r.score === undefined) {
    const tip = "The scanner hasn't gathered enough about this app to rank it as a promotion candidate yet.";
    return `<span class="badge badge-inline" style="font-size:0.65rem;background:var(--bg4);color:var(--text3)" title="${escHtml(tip)}">readiness —</span>`;
  }
  const tip = r.eligible_to_offer
    ? `Ready to offer for promotion. Scored ${r.score} of 100 on ${r.dimensions_measured} of ${r.dimensions_total} signals.`
    : `Scored ${r.score} of 100, but on only ${r.dimensions_measured} of ${r.dimensions_total} signals — too little to offer promotion on yet.`;
  return `<span class="badge badge-inline" style="font-size:0.65rem;background:var(--bg4);color:${_readinessBandColor(r.band)}" title="${escHtml(tip)}">readiness ${r.score}</span>`;
}

function _readinessBlock(c) {
  const r = c.readiness;
  if (!r || !Array.isArray(r.dimensions) || !r.dimensions.length) return '';
  const rows = r.dimensions.map(d => {
    const measured = d.measured && d.score !== null && d.score !== undefined;
    const pct = measured ? Math.round(d.score * 100) : 0;
    // Same cuts as the badge band, carried in the payload so a tuned
    // OFFER_THRESHOLD moves both together rather than only the badge.
    const ready = r.offer_threshold != null ? r.offer_threshold : 60;
    const emerging = r.emerging_threshold != null ? r.emerging_threshold : 35;
    const fillClass = !measured ? ''
      : (pct >= ready ? ' score-bar-green' : (pct >= emerging ? ' score-bar-yellow' : ' score-bar-red'));
    const tip = measured
      ? `${d.label}: ${d.detail}`
      : `${d.label}: not measured — ${d.detail}. It does not drag the score down; it is left out of it.`;
    // The fill must be block-level: its width is set inline, and an inline
    // element ignores width — it renders the empty track for every value,
    // including 100. Caught by hand in the both-themes pass; there is no CI
    // gate that would have. Both halves of the fix are here on purpose: <div>
    // markup below, and `display:block` on the class itself in base.css.
    return `<div class="score-row">
        <span class="score-row-label">${escHtml(d.label)}</span>
        <div class="score-bar-wrap" title="${escHtml(tip)}"><div class="score-bar-fill${fillClass}" style="width:${measured ? pct : 0}%"></div></div>
        <span class="score-row-pts">${measured ? pct : '—'}</span>
      </div>`;
  }).join('');
  const unmeasured = r.dimensions.filter(d => !d.measured).length;
  const caveat = unmeasured
    ? `Scored on ${r.dimensions_measured} of ${r.dimensions_total} signals — the rest aren't recorded yet, so they're left out rather than counted as zero.`
    : `Scored on all ${r.dimensions_total} signals.`;
  const headline = (r.score === null || r.score === undefined)
    ? 'Promotion readiness: not enough signal yet'
    : `Promotion readiness: ${r.score}`;
  return `<div class="score-breakdown">
      <div style="font-size:0.72rem;color:var(--text2);margin-bottom:5px">${escHtml(headline)}</div>
      ${rows}
      <div style="font-size:0.65rem;color:var(--text3);margin-top:4px">${escHtml(caveat)}</div>
    </div>`;
}
...
async function _afterDefinitionChange(botId, stem) {
  ...
  // Discovered (AL-1.8a/b) is where a promote is usually clicked from now.
  // Promoting removes the draft from that list, so the list has to be re-read
  // and its drawer closed — otherwise the operator is left looking at a
  // draft that no longer exists, with its actions still offered.
  if (typeof appsLoadDiscovered === 'function'
      && document.getElementById('apps-discovered')?.classList.contains('active')) {
    if (typeof appsCloseDiscoveredDraft === 'function') appsCloseDiscoveredDraft();
    await appsLoadDiscovered();
  }
}
...
// PRIMARY usage line (AL-1.3) — real per-app turns + cost from the
// attribution rollup ({shared}/{bot}/usage-by-app.json). This is the first
// per-app number on this card that measures what the app actually SERVED,
// rather than whether its files were touched; the footprint line below is
// now the fallback, and stays labelled as a maintenance signal.
//
// Three honesty rules (internal/design-app-attribution-2026-08-15.md §3):
//   * turns/cost shown are the DETERMINISTIC total (scheduled + explicit).
//     Inferred turns get their own "~n inferred" suffix and are never
//     added in.
//   * null (rollup absent) renders as "not measured yet", never "$0.00".
//   * zero attributed turns under a high pod-wide unattributed share is
//     reported as "not attributed yet", not as "unused" — the tooltip
//     carries the pod's coverage number so the operator can tell which.
function _appUsageLine(c) {
  const turns = c.usage_turns_7d;
  const cost  = c.usage_cost_7d;
  const inferred = c.usage_inferred_turns_7d || 0;
  const cov = c.usage_attribution_coverage;
  const muted = 'color:var(--text3)';

  if (turns == null) {
    return `<div style="font-size:0.72rem;${muted};margin-bottom:4px" title="The daily per-app rollup (usage-by-app.json) has not run for this bot yet. This is 'not measured', not 'not used'.">Per-app usage not measured yet</div>`;
  }

  const share = cov && cov.unattributed_turns_share != null
    ? cov.unattributed_turns_share : null;
  const shareNote = share != null
    ? ` ${(share * 100).toFixed(0)}% of this bot's turns in the window carried no app id at all, so absence of turns here is not proof the app went unused.`
    : '';

  if (!turns) {
    const label = inferred
      ? `No attributed turns in 7d · ~${inferred} inferred`
      : 'No attributed turns in 7d';
    return `<div style="font-size:0.72rem;${muted};margin-bottom:4px" title="No turn in the last 7 days carried this app's id deterministically (scheduled or explicit capture).${shareNote}">${label}</div>`;
  }

  const tWord = turns === 1 ? 'turn' : 'turns';
  const costStr = cost != null ? `$${Number(cost).toFixed(2)}` : '—';
  const infSuffix = inferred
    ? ` <span style="${muted}">· ~${inferred} inferred</span>`
    : '';
  const seen = c.usage_last_seen_ts ? ` · last served ${ago(c.usage_last_seen_ts)}` : '';
  const tip = `Turns this app actually served in the last 7 days, attributed deterministically (scheduled + explicit capture)${inferred ? `. ${inferred} further turn(s) were only INFERRED by a classifier and are shown separately, never added in` : ''}.${shareNote}`;
  return `<div style="font-size:0.72rem;color:var(--text2);margin-bottom:4px" title="${escHtml(tip)}">${turns} ${tWord} · ${costStr} in 7d${seen}${infSuffix}</div>`;
}
...
function renderCapabilities() {
  ...
  const el = document.getElementById('cap-grid');
  if (!el) return;   // AL-1.8a: the Installed grid is retired; renderers go in 1.8c.
  if (_capData.length === 0) {
    el.innerHTML = `<div class="empty-state-card">
      ...
      <span style="color:var(--text3);font-size:0.78rem;margin:0 8px">or</span>
      <button class="btn btn-ghost" onclick="openCreateWizard()">+ Describe a new app</button>
    </div>`;
  ...
  el.innerHTML = countNote + apps.map(c => {
    ...
    const appUsageLine  = _appUsageLine(c);
    ...
    // ── Promotion readiness (AL-1.6b) ────────────────────────────────────────
    // Both render nothing at all when `readiness` is absent — which is the
    // case for every defined app (it is already vouched, so there is nothing
    // to rank) and for any manifest the scorer declined.
    const readinessPill = _readinessChip(c);
    const readinessBlock = _readinessBlock(c);
...
function _renderManifestModal() {
  ...
  // The app-test surface was killed per internal/decision-app-tests-2026-06-08.md.
  ...
  const auditConfigSection = `<div style="${secStyle}">
    ${secHead('App Audit (Tier 3)', 'Semantic audit — does the code still do what the manifest claims? See internal/spec-app-audit-2026-05-16.md.')}
...
function _renderCoherenceSection(m, secStyle, secHead, esc) {
  ...
  if (!hasAny) {
    return `<div id="m-coherence-section" style="${secStyle}">
      ${secHead('Coherence + Drift', 'Does the manifest match what the bot can actually do? See internal/spec-app-coherence-and-reconciliation-2026-06-05.md.')}
...
function _renderSyncResult(botId, data) {
  ...
  // ALPHA-2 (audit B2a): "scan complete" is a claim. A scan that ran with no
  // model behind it did not complete — its Phase 2 never ran, so its zero
  // means nothing — and a scan whose own record could not be read cannot be
  // claimed to have completed either. Both come from `llm_phase`, which the
  // producer keeps four-valued precisely so this branch has three outcomes.
  const degraded = data.llm_degraded === true;
  const unverified = data.llm_phase === 'unknown';
  const headline = degraded
    ? `Structural scan only — the part that finds apps did not run`
    : (unverified
      ? `Scan finished, but Evolve could not confirm what it did`
      : (path === 'escalated'
        ? `Found ${discovered} new app${discovered === 1 ? '' : 's'} — scan complete`
        : `Up to date — ${driftTotal} hygiene issue${driftTotal === 1 ? '' : 's'}`));
  const uncertain = degraded || unverified;
  toast(`${uncertain ? '⚠ ' : '✓ '}${headline}`, uncertain ? 'warn' : 'ok');

  const reasonLine = data.reason
    ? `<div style="font-size:0.7rem;color:var(--text3);margin-top:2px">${escHtml(data.reason)}</div>` : '';
  // The plain-language half of the same fact, with its remedy. Both are
  // server-supplied (applications/scan_provenance.py owns the copy) so this
  // modal and the Discovered banner cannot drift into two explanations.
  const degradedLine = (degraded && data.llm_degraded_note)
    ? `<div class="alert alert-warn" style="align-items:flex-start;margin-top:8px">
        <div>${escHtml(data.llm_degraded_note)}${
          data.llm_degraded_remedy ? ' ' + escHtml(data.llm_degraded_remedy) : ''}</div>
      </div>`
    : (unverified
      ? `<div class="alert alert-warn" style="align-items:flex-start;margin-top:8px">
          <div>The scan left no readable record of what it did, so Evolve cannot say whether it looked properly. Re-run Sync; if it keeps happening, re-deploy the bot to repair its file permissions.</div>
        </div>`
      : '');
...
async function runSyncPod() {
  ...
  // ALPHA-2 (audit B2a). A scan that ran WITHOUT a model finds nothing by
  // construction, so on such a pod "0 new apps" is a consequence of a skipped
  // phase, not a finding — and "N full scans" is not what happened.
  //
  // THREE outcomes, not two. The first cut branched on `llm_degraded`, which
  // left `llm_phase: "unknown"` — a scan whose own record could not be read —
  // rendering the green ✓ tick, restoring the exact bug for the case the
  // producer went out of its way to keep distinct. An independent review
  // caught it. Anything that is not a confirmed `ran` is excluded from the
  // full-scan count and from the success check.
  const degradedCount = (data.degraded_bots || []).length;
  const unverifiedCount = (data.unverified_bots || []).length;
  const degradedReasons = data.llm_degraded_reasons || [];
  const fullScans = Math.max(0, escalated - degradedCount - unverifiedCount);
  const scanBits = [];
  if (fullScans > 0) scanBits.push(`${fullScans} full scan${fullScans === 1 ? '' : 's'}`);
  if (degradedCount > 0) scanBits.push(`${degradedCount} structural-only scan${degradedCount === 1 ? '' : 's'}`);
  if (unverifiedCount > 0) scanBits.push(`${unverifiedCount} scan${unverifiedCount === 1 ? '' : 's'} Evolve could not confirm`);
  if (!scanBits.length) scanBits.push('no full scans needed');

  const unverifiedNames = (data.unverified_bots || []).map(b => botLabel(b));
  const degradedHtml = (degradedReasons.length || unverifiedNames.length)
    ? `<div class="alert alert-warn" style="align-items:flex-start;margin-bottom:14px">
        <div>
          <div style="font-weight:600;margin-bottom:4px">Evolve cannot vouch for this whole sync.</div>
          ${degradedReasons.map(r => {
            const names = (r.bots || []).map(b => botLabel(b));
            const who = names.length === 1 ? `On ${names[0]}: ` : `On ${names.length} bots (${names.join(', ')}): `;
            return `<div>${escHtml(who)}${escHtml(r.note || '')}${r.remedy ? ' ' + escHtml(r.remedy) : ''}</div>`;
          }).join('')}
          ${unverifiedNames.length
            ? `<div>${escHtml(unverifiedNames.length === 1
                ? `On ${unverifiedNames[0]}: `
                : `On ${unverifiedNames.length} bots (${unverifiedNames.join(', ')}): `)}the scan finished but left no readable record of what it did, so Evolve cannot say whether it looked properly. Re-run Sync; if it keeps happening, re-deploy the bot to repair its file permissions.</div>`
            : ''}
        </div>
      </div>`
    : '';
  ...
  const summaryHtml = `<div style="background:var(--bg2);border:1px solid var(--border);border-radius:6px;padding:10px 14px;margin-bottom:14px;display:flex;flex-direction:column;gap:4px">
    <div style="font-size:0.72rem;color:var(--text2);margin-bottom:2px">
      ...
      ${escHtml(scanBits.join(' · '))} ·
      <strong>${totalDrift}</strong> drift finding${totalDrift === 1 ? '' : 's'}
    </div>
    ${_reflectSummaryItems(aggCounts)}
  </div>`;

  // The green "up to date" is an assertion about the pod, and a pod whose scan
  // skipped its model phase — or whose scan left no record of what it did —
  // has not earned it: zero apps there is a consequence, not a finding.
  if (totalDrift === 0 && totalDiscovered === 0 && !bots.some(b => b.error)
      && degradedCount === 0 && unverifiedCount === 0) {
    body.innerHTML = summaryHtml +
  ...
  const botSections = bots.map(b => {
    ...
    // A bot whose model phase was skipped, or whose scan left no record, did
    // not produce a clean "0 new apps" — say which of the three happened.
    const uncertain = b.llm_degraded || b.llm_phase === 'unknown';
    const note = b.llm_degraded
      ? 'structural scan only'
      : (b.llm_phase === 'unknown'
        ? 'ran, but Evolve could not confirm what it did'
        : (b.path === 'escalated'
          ? `${discovered} new app${discovered === 1 ? '' : 's'}`
          : 'up to date'));
    if (findings.length === 0) {
      const mark = uncertain
        ? `<span style="color:var(--yellow);margin-left:6px">${escHtml(note)}</span>`
  ...
  body.innerHTML = summaryHtml + degradedHtml + botSections;
