...
async function _uwizExecute() {
  ...
  try {
    if (scope === 'archive') {
      ...
      steps.push(`Removing ${cands.length} script file(s)…`);
      _uwizProgress(body, steps);
      const delR = await api('DELETE', `/api/applications/${botId}/${appId}`, { delete_files: cands, commit: true });
      if (delR?.error) throw new Error(delR.error);

      // Override status to dormant (archive set it to hidden)
      steps.push('Marking as dormant…');
      _uwizProgress(body, steps);
      await api('PATCH', `/api/applications/${botId}/${appId}`, { status: 'dormant' });
      ...
      steps.push(`Deleting ${allToDelete.length} file(s)…`);
      _uwizProgress(body, steps);
      ...
      steps.push(`✓ ${nDel} file(s) removed, manifest deleted`);
    }
...
async function stripScrubMarker(botId, path) {
  ...
  try {
    ...
    if (r.status === 403 && data.manual_cli) {
      ...
      toast(`Direct write blocked. Copy this CLI and run via SSH:\n\n${data.manual_cli}\n\n(Copied to clipboard if your browser allows it.)`, 'err');
      return;
    }
...
async function stripAllScrubMarkers(botId) {
  ...
  try {
    ...
    if (blocked) {
      const cli = (data.blocked[0] || {}).manual_cli || '';
      if (cli) { try { await navigator.clipboard.writeText(cli); } catch (_e) { /* clipboard may be blocked */ } }
      toast(`✓ Stripped ${data.stripped_count}; ${blocked} need bot-user privileges (first SSH command copied to clipboard).`, 'warn');
    } else {
...
async function applyReflectFix(botId, findingIdx) {
  ...
  try {
    ...
    if (r.status === 403 && data.manual_cli) {
      ...
      const msg = `Direct write blocked. Copy this CLI command and run via SSH:\n\n${data.manual_cli}`;
