...
function renderOverview() {
  ...
  // stale. See internal/spec-release-tiers-2026-05-16.md + spec-deploy-meta §D-8.
  renderUpdatesDrawer();
...
function renderPodNode(id, b) {
  ...
  }
  {
    ...
    if (_anyBreaker) {
      ...
      // Acceptance line — what "Reactivate" bought. Reactivating a bot
      // records the spend total at that instant and the cap is measured
      // from there; without saying so, a re-trip reading "$5.20 ≥ $5.00"
      // looks like a lie next to a Cost page showing $32.69 for the day.
      const _accepted = _allTrips.find(t => t.accepted_usd != null);
      if (_accepted) {
        const _amt = Number(_accepted.accepted_usd).toFixed(2);
        const _when = _accepted.accepted_at_label || 'reactivation';
        breakerCountdownHtml += `<div style="margin-top:2px;font-size:0.72rem;color:var(--text3)">accepted $${escHtml(_amt)} at ${escHtml(_when)}; counting from then</div>`;
      }
    }
...
// maintenance) per internal/spec-release-tiers-2026-05-16.md. Returns an HTML string
...
// state→representation contract (internal/spec-deploy-meta-2026-06-14.md §D-8 +
