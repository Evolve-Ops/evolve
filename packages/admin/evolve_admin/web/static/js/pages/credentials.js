...
async function ikRenderKeys(botId) {
  ...
  for (const k of keys) {
    ...
    if (typeof k.should_list === 'boolean') {
      ...
      // optional integration — but ONLY for providers whose key actually
      // lives in openclaw.json (INLINE_KEY_PROVIDERS server-side). LLM
      // providers run off .env / auth-profiles the probe can't see, so an
      // unrestricted rule would flag a working Gemini as "Setup required".
      // This is the legacy fallback path; modern backends send should_list.
      const inlineKeyProvider = k.provider === 'brave';
