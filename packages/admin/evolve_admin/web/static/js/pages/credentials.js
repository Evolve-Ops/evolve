...
async function ikRenderKeys(botId) {
  ...
  for (const k of keys) {
    ...
    if (typeof k.should_list === 'boolean') {
      ...
      // Mirrors rule (d) in credentials_visibility.annotate_should_list: an
      // enabled plugin with no credential is a live defect, not an untouched
      // optional integration — for every provider Evolve can positively
      // verify. Since 2026-09-04 that includes the LLM providers, whose keys
      // live in OC's per-agent runtime store (the earlier comment here
      // guessed ".env / auth-profiles"; a live scan refuted it).
      // This is the legacy fallback path; modern backends send should_list.
      const inlineKeyProvider = k.provider === 'brave' || k.category === 'llm';
      show = !(k.status === 'missing' && !isPodInvariant && !hasWarnings
               && !(k.plugin_enabled && inlineKeyProvider));
    }
    if (!show) continue;
  ...
  const _keyRow = (k) => {
    ...
    if (cc === 'integration_token') {
      ...
      // Scope note: this row is the credential embedded in the WORKSPACE
      // BACKUP remote (.git/config), discovered by IntegrationTokenProbe.
      // Without the label, a "GitHub · Personal access token · ✅ Active"
      // row reads as the bot's GitHub identity — the 2026-07-28 incident:
      // an operator rotated here expecting to restore the bot's repo
      // access (gh CLI / general code access), which this row does not
      // touch.
      let scopeNote = '';
      if (k.provider === 'github') {
        const scopeLabel = k.auth_type === 'ssh'
          ? 'Backup remote only — SSH deploy key for this workspace’s backup push.'
          : 'Backup remote only — token embedded in this workspace’s <code>.git/config</code>. Rotating here does not affect gh CLI or MCP access.';
        scopeNote = `<div style="font-size:0.72rem;color:var(--text3);margin-top:2px">${scopeLabel}
          <a href="#" style="color:var(--accent);text-decoration:none"
             onclick="document.getElementById('ik-github-row')?.scrollIntoView({behavior:'smooth'});return false">General code access ↓</a></div>`;
      }
      return `<tr ${dp}>
        <td><strong>${display}</strong></td>
        <td style="font-size:0.8rem;color:var(--text2)">${typeLabel}${scopeNote}</td>
        <td>${maskedCell}</td>
    ...
    // Tri-state, never "missing": the provider is billing (turns in the last
    // 30 days) but no probe located its key. Saying "missing" would be a
    // false statement about a working credential, and would offer a Set-up
    // button that writes where the runtime is not reading. See clause (d′)
    // in credentials_visibility.
    const unlocated = k.status === 'active_unlocated';
    const isPodInvariant = podInvariants.includes(k.provider);
    const maskedCell = (missing || optedOut || unlocated)
    ...
    const _errBadge = _provAlerts.length
      ...
      : '';
    // Where the runtime reads this key from, and which agents carry a copy.
    // "main · email-reader" is what tells the operator that ONE key is
    // mirrored to N agents and that Rotate covered all of them — without it
    // a rotation on a two-agent bot looks like it touched one file.
    let runtimeChips = '';
    if (k.storage === 'oc_agent_store') {
      const chip = (txt, title) =>
        `<span title="${escHtml(title)}" style="display:inline-block;padding:1px 6px;background:var(--bg2);border:1px solid var(--border);border-radius:10px;font-size:0.7rem;margin-right:4px;color:var(--text2)">${txt}</span>`;
      const agents = Array.isArray(k.agents) ? k.agents : [];
      const agentChips = agents.length
        ? chip('🤖 ' + agents.map(escHtml).join(' · '),
               'Agents whose runtime store carries this key — Rotate writes every one')
        : '';
      const locs = Array.isArray(k.storage_locations) ? k.storage_locations : [];
      const where = locs.length
        ? chip('📍 stored in <code>' + escHtml(locs[0]) + '</code>' +
               (locs.length > 1 ? ' +' + (locs.length - 1) : ''),
               locs.map(l => l).join('\n'))
        : '';
      runtimeChips = `<div style="margin-top:4px">${agentChips}${where}</div>`;
    }
    const verifyNote = k.last_verified
      ? `<div style="font-size:0.7rem;color:var(--text3);margin-top:2px">verified ${escHtml(k.last_verified)}</div>`
      : '';
    let statusBadge;
    if (optedOut) {
      ...
      statusBadge = `<span style="color:var(--orange)">◐ Active — key not located</span>`
        + `<div style="font-size:0.7rem;color:var(--text3);margin-top:2px">${escHtml(k.unlocated_reason || '')}</div>`;
    } else if (missing) {
    ...
    statusBadge = `${statusBadge}${warningChip}${runtimeChips}${verifyNote}`;
    ...
    if (optedOut) {
      ...
      // No affordance we could offer would write where the runtime reads.
      actions = '<span style="color:var(--text3);font-size:0.75rem">—</span>';
    } else if (missing) {
      // An ENABLED plugin with no key needs the guided flow, not "+ Add".
      // openAddKeyModal writes auth-profiles.json only; providers whose
      // runtime credential lives in openclaw.json (brave reads
      // plugins.entries.brave.config.webSearch.apiKey) would report success
      // and still not work. openOnboardModal scaffolds the plugin entry,
      // writes the runtime path, mirrors to auth-profiles, and prompts the
      // gateway restart the key actually requires.
      actions = (isPodInvariant || (k.plugin_enabled && k.provider === 'brave'))
      ...
      const downBtn = (k.order != null && k.order_total != null && k.order < k.order_total)
        ...
        : '';
      const runtimeStore = k.storage === 'oc_agent_store';
      // The runtime store keeps its undo in the pod keystore, not in the
      // profile, so it gets its own endpoint. Both buttons say the same
      // thing to the operator: put the previous key back.
      const rollbackBtn = runtimeStore
        ? (k.has_prev
            ? `<button class="btn btn-ghost btn-sm" style="margin-right:4px" title="Put the previous key back in every agent's file${k.prev_rotated_at ? ' (replaced ' + escHtml(k.prev_rotated_at) + ')' : ''}"
                onclick="submitUndoRuntimeRotation(${q(botId)},${q(k.provider)})">↩ Undo</button>`
            : '')
        : (k.has_prev
            ...
            : '');
      // A runtime-store row must NOT offer Remove: deleting the key the
      // running agent reads takes the bot offline mid-conversation, and the
      // remove path writes auth-profiles, which is not where it lives.
      const removeBtn = runtimeStore
        ? ''
        : `<button class="btn btn-ghost btn-sm" style="color:var(--red)"
          onclick="keysRemove(${q(botId)},${q(k.provider)},${q(k.profile_id)})">✕</button>`;
      // `rotatable === false` is the server saying "listed, not rotatable" —
      // today, a provider found in the runtime store that Evolve has no
      // registry entry (and so no verify call) for. Listing it honours the
      // operator's rule that any loaded key is viewable; withholding Rotate
      // honours decision B, since an unverifiable write lands on the key the
      // gateway is live on.
      const rotateBtn = (k.rotatable === false)
        ? ''
        : `<button class="btn btn-ghost btn-sm" style="margin-right:4px"
          onclick="openRotateKeyModal(${q(botId)},${q(k.profile_id)},${q(k.provider)},${q(k.provider + ' ' + k.type_label)},null,null,null,${q(runtimeStore ? 'oc_agent_store' : '')})">↺ Rotate</button>`;
      actions = (upBtn + downBtn + rollbackBtn + rotateBtn + removeBtn)
...
function openRotateKeyModal(botId, profileId, provider, label, endpointOverride, fieldKey, noticeKind, storage) {
  _rotateKeyState = {
    ...
    // "auth_profiles" (default), "openclaw_channels" (telegram/slack bots
    // configured outside the wizard), "dotenv", or "oc_agent_store" (LLM
    // providers, whose key lives in OC's per-agent runtime store). The row's
    // probe winner decides; the modal only echoes it back so the write lands
    // where the runtime reads.
...
async function submitRotateKey() {
  ...
  resultEl.innerHTML =
    `<div class="alert alert-ok">Token rotated.${gitNote}${mirrorNote}</div>`
    + _renderRotateOutcome(r);
  // Hold the modal open long enough to READ a failed verification — a bad
  // paste is exactly the case the operator needs to see before the modal
  // disappears. A clean rotation closes at the usual speed.
  const verifyFailed = r.verify && r.verify.ok === false && r.verify.skipped !== true;
  const restartFailed = r.restart && r.restart.ok === false;
  setTimeout(closeRotateKeyModal, (verifyFailed || restartFailed) ? 8000 : 1400);
  loadKeys();
...
// Per-agent write list, gateway bounce and provider verification — the three
// facts that decide whether the next turn bills on the new key.
function _renderRotateOutcome(r) {
  let html = '';
  if (Array.isArray(r.agents) && r.agents.length) {
    html += `<div style="font-size:0.72rem;color:var(--text2);margin-top:6px">Written to ${r.agents.length} agent${r.agents.length === 1 ? '' : 's'}: <code>${r.agents.map(escHtml).join(', ')}</code></div>`;
  }
  if (r.restart && r.restart.ok === false) {
    html += `<div class="alert alert-warn" style="margin-top:6px">${escHtml(r.restart.detail || 'gateway restart failed')} — use Restart on the bot page.</div>`;
  }
  if (r.verify) {
    if (r.verify.ok) {
      html += `<div style="font-size:0.72rem;color:var(--green);margin-top:4px">✅ ${escHtml(r.verify.detail || 'verified')}</div>`;
    } else if (r.verify.skipped) {
      html += `<div style="font-size:0.72rem;color:var(--text3);margin-top:4px">${escHtml(r.verify.detail || 'not verified')}</div>`;
    } else {
      html += `<div class="alert alert-error" style="margin-top:6px">${escHtml(r.verify.detail || 'verification failed')}</div>`;
    }
  }
  if (r.can_undo === false) {
    html += `<div style="font-size:0.72rem;color:var(--orange);margin-top:4px">The previous key could not be retained — Undo is unavailable for this rotation.</div>`;
  }
  return html;
}

// Undo the last runtime-store rotation: puts the previous key back in EVERY
// agent's file, bounces the gateway and re-verifies, same as a rotation.
async function submitUndoRuntimeRotation(botId, provider) {
  if (!await confirmModal(`Put ${escHtml(provider)}'s previous key back for ${botLabel(botId)}? This rewrites every agent's runtime store and restarts the gateway.`)) return;
  const r = await api('POST', `/api/admin/keys/${botId}/${provider}/undo-rotation`, {});
  if (r && r.ok) {
    toast(`${provider}: previous key restored.`, 'ok');
    loadKeys();
  } else {
    toast(`Undo failed: ${(r && r.error) || 'unknown error'}`, 'error');
  }
}

// Reachable only from an inline onclick="" in the rendered row, so eslint's
// no-unused-vars cannot see the reference — export it explicitly.
window.submitUndoRuntimeRotation = submitUndoRuntimeRotation;
