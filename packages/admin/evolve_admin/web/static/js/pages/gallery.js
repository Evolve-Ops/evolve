...
// runbook flow — see docs/runbook-snapshot-install-to-gallery-2026-06-03.md).
...
function _renderGalleryDetailsBody(pkg, full) {
  ...
  if (allReqs.length) {
    const TYPE_LABELS = {integration:'Integration', secret:'Secret', system:'System tool', python_package:'Python package'};
    const rows = allReqs.map(r => {
      const label = r.id || r.name || r.key || r.pip_name || r.import || '';
