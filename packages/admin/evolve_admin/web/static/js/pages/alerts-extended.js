...
// /api/alerts/dispatcher-log endpoint excludes them, so this rendering
// path only ever shows successful + deferred dispatches.
//
  ...
  _alLoadActivity_inflight = (async () => {
    try {
      ...
      const [data, subsData] = await Promise.all([
        api('GET', `/api/alerts/dispatcher-log?limit=200&suppressed=${includeSuppressed}`),
        api('GET', '/api/alerts/subscriptions').catch(() => null),
      ]);
      _alCatalogMetaByKey = _alBuildCatalogMeta(subsData);
      const records = (data && data.records) || [];
      if (!records.length) {
        body.innerHTML = '<div class="empty">No dispatcher activity yet. Subscription deliveries will appear here as they happen.</div>';
        return;
      }
...
function _alActivityResultPill(result) {
  ...
  if (v === 'deferred')           return '<span class="badge badge-sm badge-neutral">DEFERRED</span>';
