...
async function _idLazyResolveNames(data) {
  ...
  for (const [, info] of Object.entries(data.bots || {})) {
    const pe = info?.primary_user?.external_ids || {};
    for (const [channel, id] of Object.entries(pe)) {
      if (!id) continue;
      const key = channel + ':' + id;
      if (!seen.has(key)) { seen.add(key); todo.push([channel, id]); }
    }
...
function _idRenderSetupNudge(el, data) {
  const pod = data.pod_admins || {};
  const external = pod.external_ids || {};
  const hasAdmin = Object.values(external).some(
    v => Array.isArray(v) && v.length > 0
  );
  const bots = data.bots || {};
  const missingPrimaryBots = Object.entries(bots)
    .filter(([, info]) => info.multi_user && !info.primary_user?.external_ids
      || (info.multi_user && info.primary_user
          && !Object.values(info.primary_user.external_ids || {}).some(v => v)))
    .map(([id]) => id);
  const parts = [];
  if (!hasAdmin) {
...
function _idRenderPodAdmins(el, admins) {
  const external = admins.external_ids || {};
  const channels = Object.entries(external).filter(([, ids]) => Array.isArray(ids) && ids.length);
...
function _idRenderBotCard(botId, info, cache) {
  ...
  const externalIds = primary.external_ids || {};
  const hasAny = Object.values(externalIds).some(v => v);
...
function _usersRenderAfterIdentity(data) {
  ...
  const bots = Object.keys(data.bots || {});
...
function _usersRenderBotTiles(data) {
  ...
  // for a flat alpha list after the primary 2026-05-30.
  const entries = Object.entries(bots).sort(([a, ai], [b, bi]) => {
