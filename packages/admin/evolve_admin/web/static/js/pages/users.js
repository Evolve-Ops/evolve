...
async function loadIdentity() {
  ...
  // container is missing. See internal/spec-per-bot-users-management-2026-05-29.md.
...
function _idRenderAliasCard(botId, info) {
  ...
  const multiUserHint = multiUser
    ? `<div style="background:var(--bg2);padding:6px 8px;border-radius:4px;margin-bottom:8px;font-size:0.72rem;color:var(--text2);border-left:3px solid var(--accent)">
         This is a multi-user bot — the alias below applies to every outbound mail today. Per-user rotation (sending as the initiating user's name) is specced at <code>internal/spec-multi-user-alias-2026-06-01.md</code> and tracked as deliverable C.
       </div>`
    : '';
...
function _usersRenderActivePane() {
  ...
  else _usersRosterLoad();
}

// ═══════════════════════════════════════════════════════════════════════════
//  Roster card — every person this pod serves, read-only (D-AP5, M1)
//
// Reads GET /api/users/roster ONCE per page visit (cached in
// _usersRosterData); the bot/app filter selects then narrow the SAME cached
// list client-side, so switching filters never re-fetches. No write path —
// the reader (evolve_admin.users_roster) exposes none.
// ═══════════════════════════════════════════════════════════════════════════

let _usersRosterData = null;

async function _usersRosterLoad() {
  const container = document.getElementById('users-roster-list');
  if (!container) return;
  if (_usersRosterData) {
    _usersRosterRender();
    return;
  ...
  try {
    const resp = await api('GET', '/api/users/roster');
    _usersRosterData = (resp && resp.users) || [];
  } catch (_e) {
    container.innerHTML = '<div class="subtle">Could not load the roster.</div>';
    return;
  }
  _usersRosterPopulateFilters();
  _usersRosterRender();
}

function _usersRosterPopulateFilters() {
  const botSel = document.getElementById('users-roster-bot-filter');
  const appSel = document.getElementById('users-roster-app-filter');
  if (!botSel || !appSel || !_usersRosterData) return;
  const bots = new Set();
  const apps = new Map();
  for (const u of _usersRosterData) {
    for (const b of (u.bots || [])) bots.add(b.bot_id);
    for (const a of (u.apps || [])) apps.set(a.app_id, a.name);
  }
  const curBot = botSel.value;
  const curApp = appSel.value;
  botSel.innerHTML = '<option value="">All bots</option>' +
    [...bots].sort().map(b =>
      `<option value="${escHtml(b)}">${escHtml(botLabel(b))}</option>`).join('');
  appSel.innerHTML = '<option value="">All apps</option>' +
    [...apps.entries()].sort((a, b) => a[1].localeCompare(b[1])).map(([id, name]) =>
      `<option value="${escHtml(id)}">${escHtml(name)}</option>`).join('');
  botSel.value = curBot;
  appSel.value = curApp;
}

function _usersRosterRender() {
  const container = document.getElementById('users-roster-list');
  if (!container || !_usersRosterData) return;
  const botFilter = (document.getElementById('users-roster-bot-filter') || {}).value || '';
  const appFilter = (document.getElementById('users-roster-app-filter') || {}).value || '';
  const rows = _usersRosterData.filter(u => {
    if (botFilter && !(u.bots || []).some(b => b.bot_id === botFilter)) return false;
    if (appFilter && !(u.apps || []).some(a => a.app_id === appFilter)) return false;
    return true;
  });
  if (rows.length === 0) {
    container.innerHTML = '<div class="subtle" style="padding:8px 0">No one matches this filter.</div>';
    return;
  }
  container.innerHTML = rows.map(_usersRosterRow).join('');
}

function _usersRosterSafeId(key) {
  return String(key).replace(/[^a-zA-Z0-9_-]/g, '_');
}

// Placeholder-safe display: the record carries the id; this is where the
// roster's own display field renders — a person with no resolved name shows
// their first channel id rather than a blank row (brief item 1).
function _usersRosterDisplayName(u) {
  if (u.display_name) return u.display_name;
  const first = (u.channels || [])[0];
  return first ? `${first.platform}:${first.stable_id}` : u.person_key;
}

function _usersRosterAccessBadge(access) {
  if (access === 'unknown') {
    return ' <span class="subtle" title="No named-audience list is stored for this app yet — the roster is silent, not denying.">(unknown)</span>';
  }
  return '';
}

function _usersRosterRow(u) {
  const name = _usersRosterDisplayName(u);
  const bots = (u.bots || [])
    .map(b => `${escHtml(botLabel(b.bot_id))} · ${escHtml(b.role)}`)
    .join(', ');
  const channels = (u.channels || [])
    .map(c => `${escHtml(c.platform)}:${escHtml(c.stable_id)}`)
    .join(', ') || '<span class="subtle">none</span>';
  const apps = (u.apps || []).length
    ? (u.apps || []).map(a =>
        `${escHtml(a.name)}${_usersRosterAccessBadge(a.access)}`).join(', ')
    : '<span class="subtle">none</span>';
  const detailId = `users-roster-detail-${_usersRosterSafeId(u.person_key)}`;
  return `
    <div class="users-roster-row" style="padding:8px 0;border-bottom:1px solid var(--border)">
      <div style="display:flex;align-items:center;gap:8px;cursor:pointer" onclick="_usersRosterToggle('${detailId}')">
        <span class="expand-icon" id="${detailId}-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg></span>
        <strong>${escHtml(name)}</strong>
        <span class="subtle" style="font-size:0.78rem">${bots}</span>
      </div>
      <div id="${detailId}" style="display:none;margin:6px 0 0 22px;font-size:0.82rem;color:var(--text2)">
        <div><strong>Channels:</strong> ${channels}</div>
        <div><strong>Apps:</strong> ${apps}</div>
        <div><strong>Linked accounts:</strong> ${escHtml(u.linked_accounts?.status || 'unknown')}</div>
      </div>
    </div>
  `;
}

function _usersRosterToggle(id) {
...
// internal/spec-users-meta-2026-06-15.md §M1, invariants 1/2/6/7). A Discord
...
const _usersCoherenceSigsByBot = {};  // bot_id → last roster_coherence Signals
...
async function _usersLoadCoherenceGaps(botId) {
  ...
  _usersCoherenceSigsByBot[botId] = sigs;
  if (!sigs.length) { el.innerHTML = ''; return; }
  _usersRenderCoherenceGaps(el, botId, sigs);
}

// Re-render the gap panel from the cached Signals once the by-channel data has
// landed. The two fetches are parallel (person-link + by-channel), so on the
// first paint the admitted-id filter above may have had nothing to filter
// against; this second pass is what makes the filter reliable rather than
// race-dependent. No-op when this bot has no coherence Signals.
function _usersRerenderCoherenceGaps(botId) {
  const sigs = _usersCoherenceSigsByBot[botId];
...
// Ids this page ALREADY shows as admitted on ``channel`` — DM-approved or
// group-authorized. A Signal is a snapshot of the last hourly tick, so between
// an admit and that tick it can still name someone the page lists by name one
// card below. Whatever the Signal's age, the page must not contradict itself:
// these are filtered out of the panel rather than rendered as intruders.
// (The producer-side fix is roster_resolver.known_identities — both admission
// gates feed one roster — so this is belt-and-braces for the stale window, not
// the fix.)
function _usersAdmittedIdsForChannel(botId, channel) {
  const byCh = ((_usersChannelDataByBot[botId] || {}).by_channel || {})[channel] || {};
  const ids = new Set();
  for (const key of ['approved', 'group_access']) {
    for (const u of (byCh[key] || [])) {
      if (u && u.id) ids.add(String(u.id));
    }
  }
  // B3b's per-container lists are rendered with a Revoke button too, so their
  // members are admitted for this purpose exactly like the top-level ones.
  for (const grp of (byCh.nested_group_access || [])) {
...
function _usersRenderCoherenceGaps(el, botId, sigs) {
  ...
  for (const sig of sigs) {
    ...
    const admitted = _usersAdmittedIdsForChannel(botId, channel);
    const ids = (Array.isArray(details.unknown_identities) ? details.unknown_identities : [])
      .filter(id => !admitted.has(String(id)));
    for (const id of ids) {
      let action;
      if (!state.row_exists) {
...
window._usersRosterRender = _usersRosterRender;
window._usersRosterToggle = _usersRosterToggle;
...
async function _usersFetchAndRenderByChannel(botId) {
  ...
  try {
    ...
    // The admitted ids are now known — repaint the "Unknown to Evolve" panel so
    // it cannot name someone this page just listed as admitted.
    _usersRerenderCoherenceGaps(botId);
    let pending = 0;
...
function _usersRenderChannelBlock(botId, channel, chData) {
  ...
  // B3a (2026-09-22) — nested per-container member lists (Discord's
  // guilds.<gid>.users / guilds.<gid>.channels.<cid>.users). The top-level
  // allowlist reader cannot see these, so a bot whose only group member lived
  // in a guild list rendered as gated-but-EMPTY: the coherence Signal told the
  // operator to come here and there was no row to act on. Each group carries
  // its config path (provenance) and the path a revoke writes to.
  const nestedGroups = chData.nested_group_access || [];
  ...
  if (!chData.supported && !approved.length && !groupAccess.length
      && !groupGated && !nestedGroups.length && !pending.length
      && !seenRecently.length) {
    return '';
  }
  ...
  const nestedSection = _usersRenderNestedGroupSection(botId, channel, nestedGroups);
  const groupSection = (groupAccess.length || groupGated || nestedGroups.length)
    ? `
...
// B3a — nested member lists. Discord keeps per-guild (and per-guild-channel)
// allowlists BELOW the channel block: channels.discord.guilds.<gid>.users.
// OpenClaw admits those senders, but the top-level reader the section above
// uses cannot see them, so they were invisible here — the operator got a
// coherence Signal saying "look at the access lists" and found an empty list.
// Each list renders as its own sub-block labelled with the container it came
// from, with the exact config path on hover. Revoke-only: adding a member to
// somebody's guild list is a config authoring decision, and the add box above
// already covers "let this person into the channel".
function _usersRenderNestedGroupSection(botId, channel, groups) {
  if (!groups || !groups.length) return '';
  const blocks = groups.map((g, i) => {
    const members = g.members || [];
    const rows = members
      .map(u => _usersRenderNestedGroupRow(botId, channel, i, u, g))
      .join('');
    const label = g.container_label || g.config_path || 'nested list';
    return `
      <div style="margin-top:8px;padding-top:6px;border-top:1px solid var(--border)">
        <div style="font-size:0.72rem;color:var(--text2)"
             title="Lives at ${escHtml(g.config_path || '')} in this bot's openclaw.json — a per-container allowlist OpenClaw enforces in addition to the channel list above.">${escHtml(label)} · <code style="font-size:0.72rem">${escHtml(g.key || 'users')}</code> (${members.length})</div>
        <div>${rows}</div>
      </div>
    `;
  }).join('');
  return `
    <div style="margin-top:10px">
      <div style="font-size:0.74rem;color:var(--text2);text-transform:uppercase;letter-spacing:0.06em"
           title="Per-guild and per-channel member lists nested inside this channel's config block. Surfaced separately because they are enforced per container, not channel-wide.">Channel access · in-config member lists</div>
      <div style="font-size:0.72rem;color:var(--text3);margin:3px 0 5px">These ids are allowlisted inside a specific guild or channel in the bot's openclaw.json. Revoking removes the id from that list only — it changes no other list and no DM access.</div>
      ${blocks}
    </div>
  `;
}

// One nested-member row. Same columns as _usersRenderGroupRow so everything in
// the channel card aligns; the provenance badge names the container instead of
// a bare "config", and Revoke carries the group's index so the click reads the
// exact path back out of the cached GET response rather than round-tripping a
// config-derived string through an onclick attribute.
function _usersRenderNestedGroupRow(botId, channel, groupIndex, u, group) {
  const name = u.display_name
    ? escHtml(u.display_name)
    : '<span style="color:var(--text2)">[unknown]</span>';
  const handle = (u.username && u.username !== u.display_name)
  ...
  if (u.last_seen) {
    const rel = _usersRelativeTime(u.last_seen);
    lastSeenCell = `<span class="users-row-lastseen" title="Last seen: ${escHtml(u.last_seen)}">${escHtml(rel)}</span>`;
  } else {
    lastSeenCell = `<span class="users-row-lastseen" style="color:var(--text3)" title="No turns recorded in the last 30 days">—</span>`;
  }
  const containerLabel = group.container_label || 'nested';
  // Badge stays short — the KIND of container ("guild", "channel"), with the
  // full path on hover. A whole snowflake id in the actions column would push
  // the row layout out of alignment with the lists above it.
  const badgeKind = containerLabel.split('›').pop().trim().split(' ')[0] || 'config';
  return `
    <div class="users-row">
      <span class="users-row-name">${name}${handle}</span>
      <code class="users-row-id">${escHtml(u.id)}</code>
      <span class="users-row-surfaces" title="Allowlisted inside ${escHtml(containerLabel)} via ${escHtml(group.config_path || '')}">GROUP</span>
      ${lastSeenCell}
      ${emailCell}
      <span class="users-role-locked" style="font-style:italic" title="Role resolved from the roster. This list is config-managed, so roles are edited on the DM-approved list.">${escHtml(u.role || 'participant')}</span>
      <span class="users-row-actions">
        <span class="badge badge-neutral" style="font-size:0.62rem" title="Source: ${escHtml(group.config_path || '')} (config). Enforced for this container only.">${escHtml(badgeKind)}</span>
        <button class="btn btn-sm btn-warning" onclick="_usersGroupRevokeNested('${escHtml(botId)}','${escHtml(channel)}',${groupIndex},'${escHtml(u.id)}')" title="Remove this id from ${escHtml(group.config_path || 'the nested member list')}. Changes no other list and no DM access. Restarts the bot gateway to apply.">Revoke</button>
      </span>
    </div>
  `;
}
...
function _usersTierPrefDisplayName(botId, pref) {
  ...
  // B3a — nested guild/channel member lists carry the same resolved entries,
  // one level down, so the name join reaches them too.
  for (const g of (chData.nested_group_access || [])) lists.push(g.members);
...
// B3a — revoke from a NESTED member list. The path is read back out of the
// cached GET response (_usersChannelDataByBot) by index, so the config-derived
// segments never travel through an onclick attribute; the server then resolves
// that path against the lists the live config actually has before writing.
async function _usersGroupRevokeNested(botId, channel, groupIndex, id) {
  const cached = (_usersChannelDataByBot[botId] || {}).by_channel || {};
  const group = ((cached[channel] || {}).nested_group_access || [])[groupIndex];
  if (!group || !group.path) {
    toast('This list changed since the page loaded — reloading.', 'err');
    _usersFetchAndRenderByChannel(botId);
    return;
  }
  const where = group.config_path || group.container_label || 'the nested member list';
  if (!await confirmModal({body: `Remove ${id} from ${where}?\n\n` +
               `They lose access inside ${group.container_label || 'that container'} only. ` +
               `No other allowlist and no DM access changes. The bot gateway restarts to apply.`, danger: true})) {
    return;
  }
  try {
    const resp = await api('POST',
      `/api/admin/bots/${encodeURIComponent(botId)}/users/group-allowlist/revoke`,
      { channel, id, path: group.path });
    if (resp && resp.error) { toast(`Revoke failed: ${resp.error}`, 'err'); return; }
    if (resp && resp.gateway_restart_warning) { toast(resp.gateway_restart_warning, 'ok'); }
    _usersFetchAndRenderByChannel(botId);
  } catch (e) {
    toast(`Revoke failed: ${e}`, 'err');
  }
}
