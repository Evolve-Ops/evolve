...
function renderBreakerModalActive() {
  ...
  box.innerHTML = `
    ...
    ${trips.map(t => {
      ...
      // leaving the modal.
      // Acceptance line — see the same block in overview.js. The panel is
      // where an operator goes to ask "why did it trip again?", so the
      // answer ("it did not; it is counting from where you left off")
      // belongs beside the reason.
      const acceptedHtml = t.accepted_usd != null
        ? `<div style="font-size:0.72rem;color:var(--text3);margin-top:2px">accepted $${escHtml(Number(t.accepted_usd).toFixed(2))} at ${escHtml(t.accepted_at_label || 'reactivation')}; counting from then</div>`
        : '';
