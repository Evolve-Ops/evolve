...
//
// internal/build-AL-1.8a-apps-shell.md · internal/design-apps-surface-2026-08-16.md
//
...
// "Install to…" — live since AL-3.2, and ON only where an install can really
// happen. The three states come from the detail read (`install{}`), which
// loads and VERIFIES the app's packaged copy rather than probing for one: an
// enabled button over a packaged copy that has drifted would move the failure
// from before the click to after it.
//
//   pack             a verified packaged copy exists — install now.
//   snapshot_needed  no packaged copy yet, but a bot has the app with its
//                    files, so one is taken first. Offered, and the preview
//                    says so before anything is written.
//   unavailable      nothing to install. The button stays OFF and says why —
//                    an app that is standing instructions rather than files
//                    has nothing for an install to place, and "Install" would
//                    be a promise this path cannot keep
//                    (principle-alerts-explain-and-remediate).
function _appsInstallToButton(label, install) {
  const text = escHtml(label || 'Install to…');
  const state = (install && install.state) || 'unavailable';
  if (state === 'unavailable') {
    // `reason` is the plain-language half of the route's answer; `detail`
    // carries the engine's error class and stays off screen (design §7 — no
    // field name from the JSON reaches an operator).
    const why = (install && install.reason)
      || 'this pod could not work out how to put this app on another bot';
    return `<button class="btn btn-ghost btn-sm" disabled
      title="This app can't be installed on another bot: ${escHtml(why)}">${text}</button>`;
  }
  const hint = state === 'snapshot_needed'
    ? 'No packaged copy of this app exists yet. One is taken from the bot that has it, then installed onto the bot you pick — same files, with the new bot\'s own name and folders substituted in. You see the file list before anything is written.'
    : 'Puts this app on another bot from its packaged copy: the same files, with the new bot\'s own name and folders substituted in. You see the file list before anything is written.';
  return `<button class="btn btn-ghost btn-sm" onclick="appsOpenInstall()"
    title="${escHtml(hint)}">${text}</button>`;
}
