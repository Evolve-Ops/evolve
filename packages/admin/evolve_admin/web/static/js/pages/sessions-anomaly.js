...
//   _monTtlApply(botId, targetTtl)   — accept-and-apply
//   _monTtlMultiTable(recs)          — per-bot recommendations table
...
function _monTtlActionCard(rec) {
  ...
  const current = escHtml(rec.current_ttl || '—');
  const target = escHtml(rec.target_ttl || '');
  ...
  // Headline: ACTION + bot + from → to. Lead with the verb in verdict color.
  const headline = `<span style="color:${v.fg};font-weight:700">${v.verb} ${botName} TTL:</span>
  ...
  const applyBtn = `<button class="btn btn-sm" style="padding:3px 14px;font-size:0.78rem;background:${v.fg};border-color:${v.fg};color:#1a1a1a;font-weight:600"
       onclick="_monTtlApply('${bot}','${target}')"
       title="Open Cost Optimization with ${botName} pre-selected and the TTL field highlighted">Apply on Cost Optimization →</button>`;
...
function _monTtlFooterLine(recs) {
  ...
  const items = recs.map(r => {
    ...
    return `<span style="color:var(--text3)"><span style="color:${v.fg}">●</span>
      ${escHtml(botLabel(r.bot_id))} <span style="opacity:0.7">— ${escHtml(r.current_ttl || '—')} ${tag}</span></span>`;
...
function _monTtlApply(botId, targetTtl) {
  // Deeplink to Cost Measures, pre-select the bot, and highlight the TTL
  // field with a brief glow so the operator can see exactly where to set it.
  // Doesn't auto-apply — the operator confirms the change in-place. That
  // matches the conservative "operator-mediated" stance from Slice 2.
  ...
  const tryHighlight = (attemptsLeft) => {
    ...
    if (typeof _cmSelectBot === 'function' && document.getElementById('cm-bot-tabs')) {
      _cmSelectBot(botId);
      // _cmSelectBot triggers loadCostMeasures(); wait again for the TTL
      // field to be rendered, then highlight it.
      const tryFocusTtl = (n) => {
        if (n <= 0) return;
        const ttl = document.getElementById('cm-prune-ttl');
        if (!ttl) { setTimeout(() => tryFocusTtl(n - 1), 100); return; }
        ttl.scrollIntoView({ behavior: 'smooth', block: 'center' });
        // Brief amber glow so the field is obvious. Removed after 4s.
        const orig = ttl.style.boxShadow;
        ttl.style.boxShadow = '0 0 0 3px rgba(251,146,60,0.6)';
        ttl.style.transition = 'box-shadow 0.3s ease';
        // If the operator hasn't already picked a value, suggest the target.
        const hasMatchingOption = Array.from(ttl.options || []).some(o => o.value === targetTtl);
        if (hasMatchingOption && ttl.value !== targetTtl) {
          ttl.value = targetTtl;
          ttl.dispatchEvent(new Event('change'));
        }
        ttl.focus();
        setTimeout(() => { ttl.style.boxShadow = orig || ''; }, 4000);
      };
      setTimeout(() => tryFocusTtl(20), 100);
      return;
    }
...
// Unused after Slice 10 rewrite (kept temporarily so any external caller
// doesn't crash; safe to remove after one release). The multi-table view
// was replaced with action-card stack + non-action footer line.
function _monTtlMultiTable(recs) {
  const rows = recs.map(r => {
    const v = _MON_TTL_VERDICT_COLORS[r.verdict] || _MON_TTL_VERDICT_COLORS.hold;
    const current = escHtml(r.current_ttl || '—');
    const arrow = r.target_ttl
      ? `<span style="color:${v.fg}">→ ${escHtml(r.target_ttl)}</span>`
      : '<span style="color:var(--text3)">—</span>';
    return `<tr>
      <td style="padding:6px 10px">${escHtml(botLabel(r.bot_id))}</td>
      <td style="padding:6px 10px"><span style="background:${v.bg};color:${v.fg};padding:2px 8px;border-radius:8px;font-size:0.72rem;font-weight:500">${escHtml(v.verb || '')}</span></td>
      <td style="padding:6px 10px;font-family:var(--mono,monospace);font-size:0.78rem">${current}</td>
      <td style="padding:6px 10px;font-family:var(--mono,monospace);font-size:0.78rem">${arrow}</td>
      <td style="padding:6px 10px;color:var(--text3);font-size:0.75rem">${escHtml(r.confidence || '')}</td>
    </tr>`;
  }).join('');
  return `<div class="card">
    <div style="overflow-x:auto">
      <table style="width:100%;font-size:0.85rem;border-collapse:collapse">
        <thead><tr style="border-bottom:1px solid var(--border);color:var(--text3);font-size:0.72rem;text-align:left">
          <th style="padding:6px 10px;font-weight:500">Bot</th>
          <th style="padding:6px 10px;font-weight:500">Verdict</th>
          <th style="padding:6px 10px;font-weight:500">Current</th>
          <th style="padding:6px 10px;font-weight:500">Target</th>
          <th style="padding:6px 10px;font-weight:500">Confidence</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
