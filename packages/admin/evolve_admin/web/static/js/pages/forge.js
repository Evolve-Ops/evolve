...
function renderForgeJobs() {
  ...
  tbody.innerHTML = visible.map(j => {
    ...
    const badgeTitle = (j.status === 'rejected' && j.reject_reason)
      ...
      : '';
    const statusHtml = `<span class="badge ${cls}"${badgeTitle}>${spinnerHtml}${escHtml(j.status)}</span>`;
    ...
    if (j.status === 'queued') {
      ...
      actionsHtml = `<span style="font-size:0.78rem;color:var(--green)">✓ Complete</span>`;
