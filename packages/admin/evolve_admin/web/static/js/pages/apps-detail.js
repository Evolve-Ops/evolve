...
//               cost 7d with its grade split · actions, including
//               "Update to vN" where the app's written spec is ahead of
//               what that bot has. Plus the bots that do NOT have it,
//               behind "Install to…" (AL-3.2 — live, and enabled only
//               where an install can actually happen).
...
// Whether this view is showing the same-app claim (ALPHA-3a) or the stored
// ids on their own. Tracked here so re-opening the view after a subtab
// round trip keeps whichever one the operator chose.
let _appsDetailGrouped = true;

async function appsShowDetail(appId, grouped) {
  _appsDetailId = appId;
  if (grouped != null) _appsDetailGrouped = !!grouped;
...
function _appsRenderDetail() {
  ...
  host.innerHTML = `
    ...
    <h3 style="margin:22px 0 8px;font-size:0.95rem;font-weight:700">Outcomes</h3>
    ${_appsOutcomesBlock(a)}
...
// ── The same-app claim, said out loud (ALPHA-3a / D-I) ────────────────────
//
// Each bot discovered this app for itself, so the pod stores a separate
// record per bot. This page treats records that look like the same app as
// one — and says so, with a way out, because a merge the operator cannot
// see or undo would be a worse answer than the duplicate rows it replaces.
function appsToggleDetailGrouping() {
  if (_appsDetailId) appsShowDetail(_appsDetailId, !_appsDetailGrouped);
}

function _appsGroupToggleLink(label) {
  return `<span class="link" role="button" tabindex="0"
    onclick="appsToggleDetailGrouping()"
    onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();appsToggleDetailGrouping()}"
    >${escHtml(label)}</span>`;
}

function _appsDetailGroupNote(a) {
  if (!_appsDetailGrouped) {
    return `<div style="font-size:0.75rem;color:var(--text3);margin-bottom:8px">
      Showing only the copy of this app kept for these bots.
      ${_appsGroupToggleLink('Show every copy together')}
    </div>`;
  }
  if (!a.grouped) return '';
  const n = (a.grouped_app_ids || []).length;
  return `<div style="font-size:0.75rem;color:var(--text3);margin-bottom:8px">
    Each bot found this app for itself, so the pod keeps ${n} separate records for it.
    They look like the same app, so they are shown together here.
    ${_appsGroupToggleLink('Show them separately')}
  </div>`;
}
...
// "Update to vN" — shown only where the app's WRITTEN spec is ahead of what
// this bot has. Two guards, both load-bearing:
//   * `spec_source === 'vnext'` — without a written spec the app's version is
//     read back off the bots themselves, so "ahead of the bot" would be
//     comparing a number to itself.
//   * a strict `>` — equal is not behind, and a bot ahead of the pod's spec is
//     a state this button must not offer to "fix" by moving it backwards.
// The label carries the version so the operator knows what they are moving TO
// before the dialog opens, and the tooltip says the update is a merge — local
// changes are surfaced, never flattened silently.
function _appsUpdateButton(a, b) {
  if (a.spec_source !== 'vnext') return '';
  const want = Number(a.spec_version), have = Number(b.spec_version);
  if (!Number.isFinite(want) || !Number.isFinite(have) || want <= have) return '';
  return `<button class="btn btn-ghost btn-sm"
    onclick="appsOpenUpdate('${escHtml(b.bot_id)}')"
    title="This bot is on an older version of the app than the one written down for it. Updating brings its files up to date — and if anything was changed on this bot, you are shown exactly what before it is touched.">Update to ${escHtml(_appsSpecVersionLabel(a.spec_version))}</button> `;
}
...
function _appsBotsWithout(a) {
  ...
  return `
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;font-size:0.78rem;color:var(--text2)">
      ...
      ${_appsInstallToButton('Install to…', a.install)}
    </div>`;
}
...
// ── Outcomes (design-application-platform-2026-09-22.md §3.2, D-AP6) ──────
//
// Did the app do the job, not just what it cost. Reads {measured, total,
// backlog} from GET /api/apps/<app_id>, computed by app_outcomes.py over
// the Tracker (Board store) — the same rollup evo's pod_state.app_outcomes
// tool reads. `measured: false` covers two honestly-identical cases: this
// app has never been on the Tracker, or nothing has touched its Tracker
// yet — the page does not try to tell them apart because the data can't.

// Minimal inline sparkline — same technique as reports.js's own
// _raSparklineSvg (a 2px polyline, no axes/legend: a single series needs
// neither), kept local to this module since that one isn't exported.
function _appsOutcomeSparkline(values) {
  if (!values || values.length < 2 || !values.some(v => v > 0)) return '';
  const w = 90, h = 20, pad = 2;
  const min = Math.min(...values), max = Math.max(...values);
  const range = max - min || 1;
  const xStep = (w - 2 * pad) / (values.length - 1);
  const pts = values.map((v, i) => [
    pad + i * xStep, h - pad - ((v - min) / range) * (h - 2 * pad),
  ]);
...
// The last 30 days' counts for one or more metrics, oldest first, summed —
// `daily` is keyed by ISO date with no ordering guarantee from JSON.
function _appsOutcomeDailySeries(daily, metricKeys) {
  const days = Object.keys(daily || {}).sort();
  return days.map(day => metricKeys.reduce((sum, m) => sum + (daily[day]?.[m] || 0), 0));
}

// One metric's count, with its source card ids as a tooltip — "a number on
// the page can be opened" (the brief's own words): there is no card-detail
// deep link yet, so the ids themselves are the opening, in the tooltip.
function _appsOutcomeCount(metric) {
  const m = metric || { count: 0, card_ids: [] };
  if (!m.count) return `<span style="color:var(--text3)">0</span>`;
  const ids = (m.card_ids || []).join(', ') + (m.truncated ? ', …' : '');
  return `<span title="Cards: ${escHtml(ids)}">${m.count}</span>`;
}

function _appsOutcomeRow(label, help, valueHtml, sparklineValues) {
  const spark = _appsOutcomeSparkline(sparklineValues);
  return `
    <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:9px 0;border-top:1px solid var(--border)">
      <div style="font-size:0.8rem;color:var(--text2)" title="${escHtml(help)}">${escHtml(label)}</div>
      <div style="display:flex;align-items:center;gap:10px;font-size:0.9rem;font-weight:600">${valueHtml}${
        spark ? `<span title="Last 30 days">${spark}</span>` : ''}</div>
    </div>`;
}

function _appsOutcomesBlock(a) {
  const o = a.outcomes;
  if (!o || o.measured !== true) {
    return _appsEmpty(
      'No outcome data for this app.',
      'Outcomes come from the Tracker (the Board’s card store): cards moved or ' +
      'resolved by the bot, jobs finished without you, reminders, proposals. ' +
      'Either this app is not one of the Tracker’s applications, or nothing ' +
      'on its Tracker has happened yet — this pod cannot tell those apart.',
    );
  }
  const t = o.total || {};
  const daily = o.daily || {};
  const moved = (t.moved_by_bot && t.moved_by_bot.count) || 0;
  const resolved = (t.resolved_by_bot && t.resolved_by_bot.count) || 0;
  const rows = [
    _appsOutcomeRow(
      'Cards moved or resolved by the bot',
      'The first Milestone-1 measure: every lane change or resolution the bot made on its own, this week.',
      `<span>${moved + resolved}</span>`,
      _appsOutcomeDailySeries(daily, ['moved_by_bot', 'resolved_by_bot']),
    ),
    _appsOutcomeRow(
      'Bot jobs finished without you',
      'The second Milestone-1 measure: a card handed to the bot that reached done or blocked with no action from you in between.',
      _appsOutcomeCount(t.jobs_finished_without_operator),
      _appsOutcomeDailySeries(daily, ['jobs_finished_without_operator']),
    ),
    _appsOutcomeRow(
      'Reminders acknowledged',
      'A reminder the bot sent, followed by something you did on that card.',
      _appsOutcomeCount(t.reminders_acknowledged),
    ),
    _appsOutcomeRow(
      'Reminders re-asked',
      'A reminder that followed another reminder with nothing from you in between.',
      _appsOutcomeCount(t.reminders_reasked),
    ),
    _appsOutcomeRow(
      'Proposals accepted',
      'A card the bot proposed from conversation, that you kept.',
      _appsOutcomeCount(t.proposals_accepted),
    ),
    _appsOutcomeRow(
      'Proposals dismissed',
      'A card the bot proposed from conversation, that you dropped.',
      _appsOutcomeCount(t.proposals_dismissed),
    ),
  ];
  const backlog = o.backlog || {};
  const backlogAge = backlog.backlog_age_p50_days == null
    ? '<span style="color:var(--text3)">—</span>'
    : `${backlog.backlog_age_p50_days}d (max ${backlog.backlog_age_max_days}d)`;
  return `
    <div class="card" style="padding:4px 16px 4px">${rows.join('')}</div>
    <div style="font-size:0.75rem;color:var(--text3);margin-top:10px">
      Backlog: ${backlog.open_cards || 0} open ·
      median age ${backlogAge} ·
      ${backlog.blocked_over_3_days || 0} blocked over 3 days.
    </div>`;
}

window.appsShowDetail = appsShowDetail;
window.appsCloseDetail = appsCloseDetail;
window.appsToggleDetailGrouping = appsToggleDetailGrouping;
