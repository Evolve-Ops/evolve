...
// The cache above is per-SURFACE, not global: the server scopes the candidate
// set to the viewing bot's own credentials, so a POD-editor payload must not be
// reused on a bot tab (or one bot's on another's). `_aiListingsScope` records
// which scope `_aiListings` holds ('' = POD) and forces a reload on a change.
let _aiListingsScope = null;
...
//   opts.title    — optional hover text REPLACING the default provider label.
//                   The chip's own title shadows any title on a wrapping
//                   element (hovering the chip is hovering the chip), so a row
//                   that has more to disclose than "Anthropic" — the
//                   consolidated "family · latest" row and the pinned versions
//                   behind it — has to say it here or it is unreachable.
function _aiModelChip(short, provider, opts) {
  ...
  const title = o.title || (_aiProviderLabel(provider) || 'provider unknown');
  return `<span class="ai-model-provider-chip ${cls}"
    title="${escHtml(title)}">${dot}${escHtml(short)}${costPill}${x}</span>`;
}
...
// ── Auto-upgrade toggle + consolidated family rows ──────────────────────────
// (spec-model-auto-upgrade-2026-07-30 §Scope.) The "Automatically use the
// latest version of each model" toggle is a TOP-LEVEL control on the tier
// cards — the pod-defaults card and each Custom bot's tier card — not a
// setting buried in the easy-setup modal (the modal only reflects the current
// posture). When the scope's policy is ON, the tier editors consolidate the
// pinned versions of one model line into a single "family · latest" row, the
// same version-less rendering the easy-setup preview uses.
//
// State per editor scope, from GET /api/models/auto-upgrade. null = unknown
// (not loaded / fetch failed): the editor hides the toggle and keeps plain
// pinned-id rows — never guess a policy. `families` is the server-computed
// qualified-id → family-stem map (model_discovery._family_of; no client-side
// version parsing beyond the bare-id display fallback below).
let _aiPodAuto = null;   // {enabled, custom, families, governed, excluded}
let _aiBotAuto = null;   // same shape for the currently-selected bot tab
// (The fetch/write pair — _aiLoadAutoUpgrade / _aiToggleAutoUpgrade — lives
// AFTER the listings helpers: everything from _aiTierReorder to
// _aiLoadListings must stay pending-state-only, no api() calls — the
// reorder-is-pending-state test pins that region.)

// Editor-scope accessor: 'pod' → the pod card's state, anything else (the
// bot editor passes 'bot') → the selected bot tab's state.
function _aiScopeAuto(scope) { return scope === 'pod' ? _aiPodAuto : _aiBotAuto; }
function _aiScopeAutoOn(scope) { const s = _aiScopeAuto(scope); return !!(s && s.enabled); }

// Family stem for a model id: server map first, bare-id fallback (same
// convention as the easy-setup preview's famOf — a missing entry only costs
// grouping precision, never an error).
function _aiFamilyOf(m, families) {
  return (families && families[m]) || String(m).slice(String(m).indexOf('/') + 1);
}

// Group a tier's fallback chain by (provider, family), preserving first-
// occurrence order across groups and insertion order within one. Auto mode
// renders ONE row per group; group ops re-flatten with flatMap so the saved
// config keeps every concrete pinned id (display consolidates, the write
// stays version-pinned — the upgrade engine owns riding the ids forward).
function _aiFamilyGroups(models, families) {
  const groups = []; const byKey = {};
  for (const m of models) {
    const key = _aiProviderForModel(m) + '/' + _aiFamilyOf(m, families);
    if (!byKey[key]) {
      byKey[key] = { fam: _aiFamilyOf(m, families), provider: _aiProviderForModel(m), models: [] };
...
// ── Saying which pin a consolidated row stands on ───────────────────────────
// Operator report 2026-08-24: with auto-latest ON the Standard tier read
// "claude-sonnet · latest" while the picker greyed "claude-sonnet-4-6 (already
// added)". Nothing was wrong — the saved chain concretely contains 4-6 and the
// display consolidates it — but nothing on screen SAID so, and the two
// statements read as a contradiction. The row now names the version it stands
// on today; the picker (below) names where the greyed id lives.

// Bare id with the provider prefix clipped ("anthropic/claude-sonnet-4-6" →
// "claude-sonnet-4-6"). Same convention as _aiFamilyOf's fallback.
function _aiBareId(m) {
  const s = String(m || '');
  return s.slice(s.indexOf('/') + 1);
}

// The concrete version a consolidated line stands on, as a short fragment:
// "claude-sonnet-4-6" under family "claude-sonnet" → "4-6". When the family
// stem is not a literal prefix of the id, the whole bare id is returned —
// families are not always a prefix (model_discovery._family_of maps
// "gemini-2.5-flash" to "gemini-flash", version mid-string), and showing the
// full id is honest where clipping would have to guess.
function _aiPinVersionLabel(model, fam) {
  const bare = _aiBareId(model);
  const stem = String(fam || '');
  return (stem && bare.slice(0, stem.length + 1) === stem + '-' && bare.length > stem.length + 1)
    ? bare.slice(stem.length + 1)
    : bare;
}

// The consolidated row's chip label. `models` is the group's pinned chain,
// head first — the resolver routes to the first credentialed member, so the
// head is the version this line actually stands on right now.
function _aiFamilyPinLabel(fam, models) {
  const head = (models && models[0]) || '';
  return head ? `${fam} · latest (now ${_aiPinVersionLabel(head, fam)})` : `${fam} · latest`;
}

// The consolidated row's hover text: provider, the standing version spelled
// out in full, and the older pins kept behind it as availability fallback.
function _aiFamilyPinTitle(provider, fam, models) {
  const prov = _aiProviderLabel(provider) || 'provider unknown';
  const list = (models || []).map(_aiBareId).filter(Boolean);
  if (!list.length) return `${prov} — ${fam}, no version pinned yet.`;
  const head = `${prov} — automatically rides the newest ${fam} version. `
    + `Right now this line is ${list[0]}.`;
  return list.length > 1
    ? `${head} Kept behind it as fallback: ${list.slice(1).join(', ')}.`
    : head;
}

// One consolidated tier row: a single "family · latest (now <version>)" chip
// standing for every pinned version of that model line in the chain. ↑/↓ move
// the whole group; × removes every member. NOT draggable — the DnD handlers
// splice flat model indices, which don't line up with group rows (↑/↓ are the
// primary, webview-safe reorder path anyway; DnD stays a pinned-row
// enhancement).
function _aiTierFamilyRow(scope, key, group, gi, total) {
  const es = escHtml(scope);
  const ek = escHtml(key);
  const label = _aiFamilyPinLabel(group.fam, group.models);
  const covered = _aiFamilyPinTitle(group.provider, group.fam, group.models);
  const upDis = gi <= 0 ? ' disabled' : '';
  const downDis = gi >= total - 1 ? ' disabled' : '';
  const moveBtns =
    `<span class="ai-tier-move">` +
      `<button type="button" class="ai-tier-move-btn"${upDis}` +
        ` aria-label="Move ${escHtml(label)} up in the fallback chain" title="Move up"` +
        ` onclick="_aiTierFamilyReorder('${es}','${ek}',${gi},${gi - 1})">↑</button>` +
      `<button type="button" class="ai-tier-move-btn"${downDis}` +
        ` aria-label="Move ${escHtml(label)} down in the fallback chain" title="Move down"` +
        ` onclick="_aiTierFamilyReorder('${es}','${ek}',${gi},${gi + 1})">↓</button>` +
    `</span>`;
  return `<div class="ai-tier-model-row">
      ${moveBtns}
      <span style="color:var(--text3);min-width:18px;text-align:right">${gi + 1}.</span>
      <span style="flex:1" title="${escHtml(covered)}">${_aiModelChip(label, group.provider, { title: covered, onRemove: `_aiTierFamilyRemove('${es}','${ek}',${gi})` })}</span>
    </div>`;
}

// Group-aware reorder/remove: rebuild the flat pending chain from the
// mutated group list, then ride the scope's existing dirty+render path.
function _aiFamilyGroupsForStore(scope, key) {
  const store = scope === 'pod' ? _aiPodDefaultsPending : _aiPendingTiers;
  const state = _aiScopeAuto(scope);
  const models = store[key]?.models;
  if (!models || !state) return null;
  return { store, groups: _aiFamilyGroups(models, state.families) };
}

function _aiTierFamilyCommit(scope, key, groups) {
  const store = scope === 'pod' ? _aiPodDefaultsPending : _aiPendingTiers;
  store[key].models = groups.flatMap(g => g.models);
  if (scope === 'pod') {
    _aiPodDefaultsDirty = true;
    _aiRenderPodDefaults();
  } else {
...
function _aiTierFamilyReorder(scope, key, from, to) {
  const g = _aiFamilyGroupsForStore(scope, key);
  if (!g || from < 0 || from >= g.groups.length || to < 0 || to >= g.groups.length) return;
  const [moved] = g.groups.splice(from, 1);
  g.groups.splice(to, 0, moved);
  _aiTierFamilyCommit(scope, key, g.groups);
}

function _aiTierFamilyRemove(scope, key, gi) {
  const g = _aiFamilyGroupsForStore(scope, key);
  if (!g || gi < 0 || gi >= g.groups.length) return;
  g.groups.splice(gi, 1);
  _aiTierFamilyCommit(scope, key, g.groups);
}
// Inline-onclick handlers — window-export for lint visibility (classic-script
// declarations are already global; this is the explicit contract).
window._aiTierFamilyReorder = _aiTierFamilyReorder;
window._aiTierFamilyRemove = _aiTierFamilyRemove;

// Picker note for an already-picked model: WHERE it lives, not the bare
// "(already added)". Under auto-latest the tier row is consolidated to
// "<family> · latest", so an operator hunting for a specific version sees that
// exact id greyed out with no row bearing its name — the 2026-08-24 report.
// Naming the tier AND the row it is folded into makes the two statements agree.
// Returns the `alreadyNote` callback _aiModelPickerRow takes; `scope` is the
// editor scope ('pod' | 'bot'), `tierLabel` the human role name ("Standard").
function _aiAlreadyNoteFor(scope, tierLabel) {
  return (id) => {
    const state = _aiScopeAuto(scope);
    if (state && state.enabled) {
      return `in ${tierLabel} as ${_aiFamilyOf(id, state.families)} · latest`;
    }
    return `already in ${tierLabel}`;
  };
}

// The toggle row itself. `apiScope` is what the PUT sends ('pod' | bot id);
// `owned` false renders it disabled (a Use-defaults bot follows the pod
// toggle — spec §Scope). state == null (fetch failed) → render nothing
// rather than a checkbox that lies.
function _aiAutoUpgradeToggleRow(apiScope, state, owned) {
  if (!state) return '';
  const pod = apiScope === 'pod';
  const governLine = pod && (state.governed.length || state.excluded.length)
    ? `<div style="font-size:0.76rem;color:var(--text3);margin-top:4px;padding-left:22px">
        Governs the ${state.governed.length} bot${state.governed.length === 1 ? '' : 's'} on pod defaults${state.excluded.length
          ? `; Custom bots keep their own setting: ${escHtml(state.excluded.map(b => (typeof botLabel === 'function' ? botLabel(b) : b)).join(', '))}`
          : ''}.
      </div>` : '';
  const followsLine = owned ? '' : `<div style="font-size:0.76rem;color:var(--text3);margin-top:4px;padding-left:22px">
        This bot follows the pod default — change it on the POD tab, or switch the bot to Custom to give it its own setting.
      </div>`;
  const consolidatedLine = state.enabled
    ? `<div style="font-size:0.76rem;color:var(--text3);margin-top:4px;padding-left:22px">
        Tiers below show one entry per model line — “family · latest (now …)”, naming the version that line stands on today — instead of one row per pinned version.
      </div>` : '';
  return `
    <div style="margin-bottom:16px;padding:10px 14px;background:var(--bg2);border:1px solid var(--border);border-radius:8px">
      <label style="font-size:0.85rem;display:flex;align-items:center;gap:6px;cursor:${owned ? 'pointer' : 'default'}">
        <input type="checkbox" id="ai-auto-upgrade-${pod ? 'pod' : 'bot'}" onchange="_aiToggleAutoUpgrade('${escHtml(apiScope)}',this.checked)"
          ${state.enabled ? 'checked' : ''} ${owned ? '' : 'disabled'}>
        Automatically use the latest version of each model
      </label>
      <div style="font-size:0.76rem;color:var(--text3);margin-top:4px;padding-left:22px">
        Each entry rides its model line's newest same-family GA version — never a costlier one, never a different line. Upgrades wait out a visible veto window on this page before landing.
      </div>
      ${consolidatedLine}
      ${followsLine}
      ${governLine}
...
async function _aiLoadListings(force) {
  ...
  const scope = _aiBot || '';
  if (_aiListings && !force && _aiListingsScope === scope) return _aiListings;
  _aiListingsLoading = true;
  try {
    const q = scope ? '?bot_id=' + encodeURIComponent(scope) : '';
    const r = await api('GET', '/api/models/listings' + q).catch(() => null);
...
async function _aiRefreshListings(rerender) {
  const scope = _aiBot || '';
  const body = scope ? { bot_id: scope } : {};
  const r = await api('POST', '/api/models/listings/refresh', body).catch(() => null);
...
// Auto-upgrade fetch/write pair — the network half of the toggle block above
// (kept OUT of the _aiTierReorder…_aiLoadListings region, which is pinned as
// pending-state-only). GET seeds the per-scope state; PUT flips `enabled`
// through the scope's safe write path and re-renders the owning editor.
async function _aiLoadAutoUpgrade(scope) {
  const r = await api('GET', '/api/models/auto-upgrade?scope=' + encodeURIComponent(scope)).catch(() => null);
  if (!r || r.error) return null;
  return {
    enabled: !!(r.auto_upgrade && r.auto_upgrade.enabled),
    custom: scope === 'pod' ? true : !!r.custom_tiers,
    families: (r.families && typeof r.families === 'object') ? r.families : {},
    governed: Array.isArray(r.auto_upgrade_governed) ? r.auto_upgrade_governed : [],
    excluded: Array.isArray(r.auto_upgrade_excluded) ? r.auto_upgrade_excluded : [],
  };
}

async function _aiToggleAutoUpgrade(apiScope, checked) {
  const state = apiScope === 'pod' ? _aiPodAuto : _aiBotAuto;
  const rerender = () => {
    if (apiScope === 'pod') _aiRenderPodDefaults();
    else _aiRenderTiers(_aiBot, _aiBotConfig);
  };
  const r = await api('PUT', '/api/models/auto-upgrade', { scope: apiScope, enabled: !!checked })
    .catch(() => null);
  if (!r || r.error) {
    toast('Could not update auto-upgrade: ' + ((r && r.error) || 'write failed'), 'err');
    rerender();   // snap the checkbox back to the stored state
    return;
  }
  if (state) state.enabled = !!checked;
  toast(checked ? 'Automatic latest-version updates on' : 'Automatic latest-version updates off', 'ok');
  rerender();
}
window._aiToggleAutoUpgrade = _aiToggleAutoUpgrade;
...
//   alreadyNote(id)         — OPTIONAL callback returning the parenthetical for
//                             an already-picked id, replacing the bare "already
//                             added". The tier editors pass _aiAlreadyNoteFor
//                             so the note names the tier and — under
//                             auto-latest — the consolidated row the id is
//                             folded into. Absent → "already added".
...
function _aiModelPickerRow(opts) {
  ...
  const alreadyNote = typeof opts.alreadyNote === 'function' ? opts.alreadyNote : null;
  ...
  const groups = providerOrder.map(prov => {
    ...
    const opts2 = models.map(m => {
      ...
      // and disable selection (re-adding would be a no-op / dup). `alreadyNote`
      // says WHERE it is present — under auto-latest "already added" alone is
      // what read as a contradiction against the consolidated row.
      if (isSug && isAdded) label = `★ ${label} (${alreadyNote ? alreadyNote(id) : 'already added'})`;
...
function _aiRoutingText(routing, tiers) {
  ...
  const maintTier = _AI_ROUTE_ROLE_TO_TIER[routing?.maintenanceRole] || routing?.maintenanceTier || 'tier3';
  const bgTier = _AI_ROUTE_ROLE_TO_TIER[routing?.backgroundRole] || routing?.backgroundTier || 'tier3';
...
let _aiLastUpgrades = [];
// ── Auto-update preview (spec-model-auto-upgrade-2026-07-30, Phase 1) ─────────
// READ-ONLY. The eligibility engine ships dark: this column says what automatic
// updates WOULD do — which rows would apply on their own, which are held back
// and why, which providers couldn't be checked at all, and which bots the pod
// setting does not actually govern. Nothing here is clickable and nothing here
// applies; the operator watches it be right for a week before Phase 3 gives it
// hands. Source: /api/models/discoveries `auto_upgrade` + each row's own
// `auto_upgrade` verdict.
let _aiLastAutoUpgrade = null;
// Role buttons the segmented picker offers (server-sourced from the pod
// catalog): Fast/Standard/Power/Max.
...
// (Fast/Standard/Power/Max) via TIER_DISPLAY — never a rung slug. The
...
function _aiRenderUpgradeRow(u, i) {
  ...
  return `
    <div class="card" style="margin-bottom:6px;padding:8px 10px;background:var(--bg2);border:1px solid var(--border)">
      ...
      ${_aiRenderAutoUpgradeVerdict(u.auto_upgrade)}
    </div>`;
}

// The read-only auto-update verdict for one upgrade row: would it apply on its
// own, and if not, WHY not — in one sentence the operator can act on. A held
// row's reason is the deliverable here, not decoration: the whole feature exists
// because 57 recommendations were emitted and 0 applied with nobody able to see
// why, and that invisibility must not reappear one layer down.
function _aiRenderAutoUpgradeVerdict(a) {
  if (!a) return '';
  const cls = a.verdict === 'would-apply' ? 'badge-ok'
    : (a.verdict === 'eligible' ? 'badge-neutral' : 'badge-warn');
  const label = a.verdict === 'would-apply' ? 'Would update automatically'
    : (a.verdict === 'eligible' ? 'Eligible — automatic updates off' : 'Held back');
  return `
    <div style="display:flex;gap:6px;align-items:baseline;flex-wrap:wrap;margin-top:6px">
      <span class="badge badge-sm ${cls}">${escHtml(label)}</span>
      <span style="font-size:0.72rem;color:var(--text3);flex:1;min-width:200px">${escHtml(a.summary || '')}</span>
    </div>`;
}

// The card-level auto-update preview block: what the setting governs today, the
// providers that could NOT be checked this run (never "you're up to date"), and
// any held item that has no Update row of its own. Rendered even when there is
// nothing to update — an unchecked provider is exactly the case a silent card
// would misreport.
function _aiRenderAutoUpgradePreview(auto, hasRows) {
  if (!auto) return '';
  const gov = auto.governance || {};
  const skipped = Array.isArray(auto.skippedProviders) ? auto.skippedProviders : [];
  const heldOnly = Array.isArray(auto.heldWithoutRow) ? auto.heldWithoutRow : [];
  // The governance note is never empty, so gate on having something to SAY: a
  // pod with nothing stale and nothing unchecked must not carry a permanent
  // paragraph on the card (a card nobody reads is the failure this feature
  // exists to fix).
  if (!hasRows && !skipped.length && !heldOnly.length) return '';
  const skippedRows = skipped.map(p => `
    <div style="margin-top:4px"><span class="badge badge-sm badge-warn">Not checked</span>
      <span style="margin-left:6px">${escHtml(p.message || '')}</span></div>`).join('');
  const heldRows = heldOnly.map(d => `
    <div style="margin-top:4px"><span class="badge badge-sm badge-warn">Held back</span>
      <span style="margin-left:6px">${escHtml(d.latest_model || '')} — ${escHtml(d.summary || '')}</span></div>`).join('');
  return `
    <div style="font-size:0.72rem;color:var(--text3);margin-bottom:8px;padding:8px 10px;background:var(--bg2);border:1px solid var(--border);border-radius:6px">
      <div><strong style="color:var(--text2)">Automatic updates — preview only.</strong> Nothing on this card applies on its own. This shows what would happen if you turned automatic updates on.</div>
      ${gov.note ? `<div style="margin-top:4px">${escHtml(gov.note)}</div>` : ''}
      ${skippedRows}
      ${heldRows}
    </div>`;
}
...
function _aiRenderUpgrades(upgrades, auto) {
  const list = Array.isArray(upgrades) ? upgrades : [];
  const preview = _aiRenderAutoUpgradePreview(auto, list.length > 0);
  // Nothing to update, but a provider may still have gone unchecked — say so
  // rather than letting silence read as "everything is current".
...
function _aiFreshnessRender(summary) {
  ...
  const discoveryBlock =
    _aiRenderUpgrades(_aiLastUpgrades, _aiLastAutoUpgrade)
  ...
  let credStoreBlock = _aiRenderCredStoreUnreadable(
    summary && summary.credential_store_unreadable
  );

  // Pin state unknown (behavior-pin ledger present but unreadable): the
  // server refuses every apply until it's repaired, so say why up front
  // rather than letting each Update click surface a bare 409.
  if (summary && summary.behavior_pin_error) {
    credStoreBlock += `<div style="padding:8px 10px;margin-bottom:10px;border:1px solid var(--yellow);border-radius:6px;font-size:0.78rem;color:var(--yellow)">
      📌 ${escHtml(summary.behavior_pin_error)} — model updates are held until the pin ledger is repaired (each row still offers an explicit Override).
    </div>`;
  }
  ...
  // credentialed model, so they WON'T ROUTE. It also catches an in-catalog
  // break that carries no drift finding.
  ...
  if (advisories.length === 0 && driftFindings.length === 0 && diversity.length === 0 && redundantBots.length === 0 && hardBreakTiers.length === 0) {
  ...
  // Stash row identities on window (the file's index-keyed onclick
  // convention) — a behavior_pin's reason text is free prose and must not be
  // JSON-embedded inside an onclick attribute.
  window._aiFreshnessAdvisories = advisories;
  const rows = advisories.map((a, i) => {
    ...
    const released = a.recommended_released
      ...
      : '';
    // A behavior-pinned recommendation was rolled back for behavior on this
    // exact rung — the server refuses a plain apply, so the row offers an
    // explicit Override instead and is excluded from Apply All.
    const pin = a.behavior_pin;
    const pinNote = pin
      ? `<div style="font-size:0.7rem;color:var(--yellow)" title="${escHtml(pin.reason || '')}">📌 rolled back for behavior ${escHtml(String(pin.ts || '').slice(0, 10))} — held back</div>`
      : '';
    const updateBtn = `<button class="btn btn-secondary btn-sm"
        onclick="_aiUpdateRecommendedModel(window._aiFreshnessAdvisories[${i}])"
        style="font-size:0.72rem;padding:3px 9px">${pin ? 'Override…' : 'Update'}</button>`;
    return `<tr>
    ...
    return `
      <div style="display:flex;flex-direction:column;gap:4px;padding:8px 0;border-bottom:1px solid var(--border)">
        <div style="font-size:0.82rem">
          ...
          <span style="color:var(--text2)"> has only ${escHtml(current)}. Consider adding ${escHtml(suggestedText)} for failover and cross-vendor checking.</span>
        </div>
  ...
  return `
    <div style="background:rgba(126,184,247,0.06);border:1px solid rgba(126,184,247,0.25);border-radius:6px;padding:8px 12px 4px 12px;margin-bottom:14px">
      ...
      <div style="font-size:0.74rem;color:var(--text2);margin-bottom:4px">
        A second LLM provider buys you failover when the primary is rate-limited or down, and a cross-vendor option for independent self-checking.
      </div>
      ${blocks}
    </div>`;
}
...
// catches an in-catalog break that has no drift finding. Each record carries
// the providers it needs (`needed_providers`,
...
async function _aiRemoveFromTier(botId, tier, modelId) {
  ...
  // Under the rungs/roles shape legacy tiers can collapse onto one rung
  // (a hand-authored roles map can point two roles at one rung);
  // resending an unchanged sibling lets the server-side fold clobber this edit
  // last-writer-wins (the write "succeeds" but the model never moves). One tier
...
async function _aiLoadFreshnessStatus() {
  ...
  _aiLastAutoUpgrade = (disc && disc.auto_upgrade) ? disc.auto_upgrade : null;
...
function _aiSyncApplyAllButton(summary) {
  ...
  if (!btn) return;
  // Behavior-pinned advisories never ride Apply All (the server would refuse
  // them row-by-row); they only apply via the per-row Override confirm.
  const advisories = ((summary && summary.advisories) || [])
    .filter(a => !a.behavior_pin);
  if (advisories.length === 0) {
...
function _aiSyncFreshnessSummaryCount(summary) {
  ...
  // add only the pure-resolution ones (no offending tier entry) so an
  // in-catalog-only break still surfaces without double-counting the rest.
...
async function _aiApplyAllFreshness() {
  const summary = _aiLastFreshnessSummary;
  const all = (summary && summary.advisories) || [];
  // Pinned rows are excluded, never overridden in bulk — reintroducing a
  // behavior-rejected model is a deliberate per-row decision.
  const advisories = all.filter(a => !a.behavior_pin);
  const pinnedCount = all.length - advisories.length;
  ...
  const pinnedNote = pinnedCount > 0
    ? `\n\n${pinnedCount} pinned update${pinnedCount === 1 ? ' is' : 's are'} excluded — ${pinnedCount === 1 ? 'its model was' : 'their models were'} rolled back for behavior. Use each row's Override button to re-apply deliberately.`
    : '';
  const msg = `Apply all ${advisories.length} freshness updates across ${Object.keys(byBot).length} bot${Object.keys(byBot).length === 1 ? '' : 's'}?\n\n${botSummary}\n\nThis rewrites each bot's tier definitions and adds any new models to their catalog. Restart the gateway after for changes to take effect on new sessions.${pinnedNote}`;
  if (!await confirmModal(msg)) return;
...
async function _aiUpdateRecommendedModel(advisory) {
  ...
  const pin = advisory.behavior_pin;
  const tierName = TIER_DISPLAY[advisory.tier] || advisory.tier;
  // A pinned advisory gets the danger confirm and sends the explicit
  // override — the server 409s a plain apply of a behavior-rejected model.
  const msg = pin
    ? { danger: true, body: `Override the behavior pin on ${advisory.bot_id} ${tierName}?\n\n`
        + `${advisory.recommended_model} was rolled back on ${String(pin.ts || '').slice(0, 10)}`
        + ` (${pin.reason || 'behavior-rejected'}). Re-applying it lifts the pin and`
        + ` restores the model that was rolled back. The post-swap behavior watch`
        + ` will judge this rung again, sooner than usual.` }
    : `Update ${advisory.bot_id} ${tierName} `
...
function _aiTierRoleHeader(roleId, rv) {
  const meter = _aiRankMeter(rv.rank, rv.rungCount);
...
//
// Neither fires when credentials are unknown (inertProviders == [],
// available stays true via fail-open), so existing presentation readers are
// safe.
...
// Renders all four resolved clusters (from config.roles, the
...
function _aiRenderTiersUseDefaults(botId, config, el) {
  ...
  const renderRole = (roleId) => {
    ...
    // Auto mode (inherited from the pod for a Use-defaults bot) consolidates
    // the read-only list the same way the editors do: one "family · latest"
    // line per model line, pinned ids in the tooltip.
    const modelRows = resolved.length
      ? (_aiScopeAutoOn('bot')
          ? _aiFamilyGroups(resolved, _aiBotAuto.families).map(g =>
              `<div style="font-family:monospace;color:var(--text2);font-size:0.82rem;padding:2px 0" title="${escHtml(_aiFamilyPinTitle(g.provider, g.fam, g.models))}">${escHtml(_aiFamilyPinLabel(g.fam, g.models))}</div>`).join('')
  ...
  el.innerHTML = `
    ...
    ${_aiAutoUpgradeToggleRow(botId, _aiBotAuto, false)}
    ${ladderSections}
...
function _aiRenderTiersCustom(botId, config, el) {
  ...
  const renderRole = (roleId) => {
    ...
    // Auto mode consolidates the chain to one row per model line ("family ·
    // latest") — display-only grouping over the same pending buffer; the save
    // payload keeps the concrete pinned ids (see the pod editor twin).
    const modelRows = (_aiScopeAutoOn('bot')
      ? _aiFamilyGroups(models, _aiBotAuto.families).map((g, gi, arr) =>
          _aiTierFamilyRow('bot', tid, g, gi, arr.length)).join('')
      : models.map((m, i) =>
          _aiTierModelRow('bot', tid, m, i,
            `(idx=>_aiTierRemoveModel('${escHtml(tid)}',idx))`, models.length)).join('')
      ) || `<div style="font-size:0.8rem;color:var(--text3);padding:3px 0;font-style:italic">(empty — add models below)</div>`;
    ...
    const addRow = _aiModelPickerRow({
      ...
      already: models,
      alreadyNote: _aiAlreadyNoteFor('bot', _AI_ROLE_LABELS[roleId] || roleId),
  ...
  el.innerHTML = `
    ...
    ${_aiAutoUpgradeToggleRow(botId, _aiBotAuto, true)}
    ${ladderSections}
...
// ── Routing card (rework 2026-08) ────────────────────────────────────────────
// Organized by WHO STARTED THE SESSION — the objective axis — instead of the
// old Productive/Maintenance/Background/Ambiguous mix (which conflated the
// trigger kind with the content classifier's guess):
//   • Conversations — humans messaging the bot. The router abstains here by
//     default (bot default model = the Standard rung head by deploy
//     convention); the picker is the long-promised operator default
//     (userTierOverride.defaultTier, spec-user-tier-control Phase A) that
//     replaces the old misleading "Standard fixed" badge.
//   • Scheduled — cron + heartbeats (trigger-anchored `background`).
//   • Internal work — Evolve's own scaffolding subagents (trigger-anchored
//     `maintenance`: summarizer, classifiers, judges).
// The CONTENT-classifier maintenance downgrade for human chats is a separate
// opt-in checkbox (routing.classifierDowngrade, default off — a keyword
// misread sends real work to the cheap model). The old Ambiguous row was a
// router no-op (ambiguousTier is parsed but never consulted) and is gone;
// any stored value is passed through on save untouched.
const _AI_ROUTE_ROLE_TO_TIER = { fast: 'tier3', standard: 'tier2', power: 'tier1' };
// Saves emit ROLE keys only (#3662): the plugin runtime refuses a persisted
// routing.*Tier key outright, so the card must never PUT one. Reads stay
// tolerant of both shapes (Role wins, mirroring the plugin's `Role ?? Tier`).
const _AI_ROUTE_TIER_TO_ROLE = { tier3: 'fast', tier2: 'standard', tier1: 'power' };
...
function _aiRenderRouting(botId, config) {
  ...
  const enabled = routing.enabled !== false;
  const maintenanceTier = _AI_ROUTE_ROLE_TO_TIER[routing.maintenanceRole] || routing.maintenanceTier || 'tier3';
  const backgroundTier = _AI_ROUTE_ROLE_TO_TIER[routing.backgroundRole] || routing.backgroundTier || 'tier3';
  const classifierDowngrade = routing.classifierDowngrade === true;
  const confidenceThreshold = routing.confidenceThreshold != null ? routing.confidenceThreshold : 0.65;
  // Conversations default — role-shaped, stored in userTierOverride
  // ("auto" = router abstains, bot default model wins).
  const uto = config.userTierOverride || {};
  const rawDefault = uto.defaultRole ?? uto.defaultTier;
  const defaultRole = (typeof rawDefault === 'string' && rawDefault) ? rawDefault : 'auto';
  ...
  const tierOpts = () => tierIds.map(tid =>
    `<option value="${escHtml(tid)}">${escHtml(TIER_DISPLAY[tid] || tid)}</option>`).join('');
  const roleOpts = () => `<option value="auto">Auto — bot default</option>` +
    ['fast', 'standard', 'power'].map(r =>
      `<option value="${r}">${escHtml(TIER_DISPLAY[r] || r)}</option>`).join('');

  const sessionCell = (name, caption) => `
    <td style="padding:7px 6px;vertical-align:top">
      <strong>${name}</strong>
      <div style="font-size:0.72rem;color:var(--text3);margin-top:1px">${caption}</div>
    </td>`;
  ...
  el.innerHTML = `
    ...
    <table style="width:100%;margin-bottom:12px">
      ...
      <tbody>
        <tr>
          ${sessionCell('Conversations', 'humans messaging this bot')}
          <td style="padding:7px 6px;vertical-align:top">
            <select id="ai-route-defaultRole" class="form-select input-w-md" onchange="_aiRoutingDirty()"
              style="padding:3px 6px;font-size:0.78rem">
              ${roleOpts()}
            </select>
          </td>
          <td id="ai-route-conv-model" style="font-family:monospace;font-size:0.78rem;color:var(--teal);padding:7px 6px;vertical-align:top">—</td>
        </tr>
        <tr>
          ${sessionCell('Scheduled', 'cron jobs + heartbeats')}
          <td style="padding:7px 6px;vertical-align:top">
          ...
          <td id="ai-route-bg-model" style="font-family:monospace;font-size:0.78rem;color:var(--teal);padding:7px 6px;vertical-align:top">—</td>
        </tr>
        <tr>
          ${sessionCell('Internal work', "Evolve's own summaries, classification, and upkeep subagents")}
          <td style="padding:7px 6px;vertical-align:top">
          ...
          <td id="ai-route-maint-model" style="font-family:monospace;font-size:0.78rem;color:var(--teal);padding:7px 6px;vertical-align:top">—</td>
        </tr>
      </tbody>
    </table>
    <div style="margin-bottom:14px;padding:10px 12px;background:var(--bg2);border:1px solid var(--border);border-radius:6px">
      <label style="font-size:0.85rem;display:flex;align-items:center;gap:6px;cursor:pointer">
        <input type="checkbox" id="ai-route-classifier-downgrade" onchange="_aiRoutingDirty()"${classifierDowngrade ? ' checked' : ''}>
        Downgrade upkeep conversations
      </label>
      <div style="font-size:0.76rem;color:var(--text3);margin-top:4px;padding-left:22px">
        When a conversation with a human looks like system upkeep (config fixes, debugging, restarts), route it to the Internal work tier. Off by default — a misread here sends real work to the cheap model. Scheduled and internal sessions route by their trigger and don't need this.
      </div>
      <div style="margin-top:8px;padding-left:22px">
        ...
        <div style="font-size:0.72rem;color:var(--text3);margin-top:4px">How confident the classifier must be before its label sticks.</div>
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:10px">
  ...
  const roleSel = document.getElementById('ai-route-defaultRole');
  ...
  if (roleSel) roleSel.value = _AI_ROUTE_ROLE_TO_TIER[defaultRole] ? defaultRole : 'auto';
...
function _aiRoutingUpdateModels() {
  ...
  const convRole = document.getElementById('ai-route-defaultRole')?.value || 'auto';
  // "auto" = the router abstains and OC uses the bot default model, which
  // deploy seeds from the Standard rung head — show that model.
  const convTier = _AI_ROUTE_ROLE_TO_TIER[convRole] || 'tier2';
...
async function _aiSaveRouting() {
  ...
  if (statusEl) statusEl.innerHTML = '<span style="color:var(--text2)">Saving…</span>';
  if (saveBtn) saveBtn.disabled = true;
  // ROLE keys only in the PUT body (#3662): the dropdowns hold tierN values
  // (their display currency), translated on the way out. A *Tier key must
  // never be sent — the runtime refuses a routing block that carries one.
  const _priorRouting = _aiBotConfig.routing || {};
  const routing = {
    enabled: document.getElementById('ai-route-enabled')?.checked !== false,
    maintenanceRole: _AI_ROUTE_TIER_TO_ROLE[document.getElementById('ai-route-maintenanceTier')?.value] || 'fast',
    backgroundRole: _AI_ROUTE_TIER_TO_ROLE[document.getElementById('ai-route-backgroundTier')?.value] || 'fast',
    // The Ambiguous row is gone from the card (router no-op) — pass any
    // stored value through (translated if it was still tier-shaped) so the
    // wholesale write can't clobber it.
    ambiguousRole: _priorRouting.ambiguousRole ?? (_AI_ROUTE_TIER_TO_ROLE[_priorRouting.ambiguousTier] ?? null),
    confidenceThreshold: parseFloat(document.getElementById('ai-route-confidence')?.value || '0.65'),
    classifierDowngrade: document.getElementById('ai-route-classifier-downgrade')?.checked === true,
  };
  ...
  // Conversations default — role-shaped, lives in userTierOverride (its own
  // endpoint, partial-merge). Only written when it actually changed, so a
  // routine routing save doesn't churn evolve-tiers.json / the audit log.
  const uto = _aiBotConfig.userTierOverride || {};
  const priorRaw = uto.defaultRole ?? uto.defaultTier;
  const prior = (typeof priorRaw === 'string' && priorRaw) ? priorRaw : 'auto';
  const chosen = document.getElementById('ai-route-defaultRole')?.value || 'auto';
  if (chosen !== prior) {
    const r2 = await api('PUT', '/api/admin/config/' + _aiBot + '/user-tier-override',
      { defaultTier: chosen });
    if (r2.error) {
      if (statusEl) statusEl.innerHTML = `<span style="color:var(--danger)">✗ ${escHtml(r2.error)}</span>`;
      if (saveBtn) saveBtn.disabled = false;
      return;
    }
    _aiBotConfig.userTierOverride = (r2.userTierOverride && typeof r2.userTierOverride === 'object')
      ? r2.userTierOverride : { ...uto, defaultTier: chosen };
  }
...
function _aiRenderFallback(botId, config) {
  ...
  // Cascade order editor (a stale legacy tier0 entry is excluded — the judge
  // slot is gone and never participated in fallback)
...
async function _aiRenderBotData(botName) {
  ...
  // The bot's auto-upgrade posture (toggle + family map for consolidated
  // rows) rides the same await — a failure degrades to pinned-id rows.
  const [, botAuto] = await Promise.all([
    _aiLoadListings(),
    _aiLoadAutoUpgrade(botName),
  ]);
  _aiBotAuto = botAuto;
...
// Guarantee this page has the pod's network.json before it renders anything
// derived from it — the bot tab bar and the routing-flow line both read the
// `_networkData` global, and this page is not one of that global's writers
// (settings.js, pod-config.js and self-improvement.js are).
//
// Boot normally fills it (load() → loadNetwork()), so on a healthy pod the
// bare read worked. But it is a free-ride: any boot where that ONE fetch
// fails — transient 5xx, daemon bouncing mid-boot, a dropped request — leaves
// `_networkData` holding api()'s error object, and every read below then
// degrades silently. The visible damage is total: `orderedBotIds({})` is
// empty, so the tab bar collapses to POD alone and the operator loses every
// per-bot editor (custom tiers, cascade, fallback) with no retry path — the
// page never re-renders its tabs until some OTHER page happens to refill the
// global. That is the 2026-08-24 regression (evolve-stable-528), where
// visiting Settings first was the only way back to the bot tabs.
//
// So: ensure our own data. Render must not depend on visit order.
async function _aiEnsureNetworkData() {
  // Presence of `bots`, not its size: a pod with genuinely zero bots answers
  // `bots: {}` and must NOT re-fetch on every page activation, while an api()
  // error payload has no `bots` key at all and must.
  if (_networkData && _networkData.bots) return;
  const j = await api('GET', '/api/network');
  // Only adopt a response that actually carries bots — an error payload
  // must not overwrite a global some other page populated correctly.
...
async function loadAiOptimization() {
  ...
  // _aiEnsureNetworkData rides the same await (see above) — the tabs and the
  // routing line below are built synchronously from `_networkData`.
  await Promise.all([_aiLoadObservedCost(), _aiEnsureNetworkData()]);
  ...
  // Bot tabs — POD first (default landing), then per-bot. Scaffold-only
  // residue (a phantom `bots.evolve` on stale pods) is filtered by
  // orderedBotIds (EVO-SEP-S4), so no phantom tab surfaces here.
  ...
  if (tabsEl) {
    ...
    // No hardcoded "evolve" placeholder tab: post evo-account-separation the
    // primary is `evo` (provisioned through the normal flow), so a grayed
    // "evolve (not provisioned)" tab is itself a stale phantom (EVO-SEP-S4).
...
const _AI_TIER_TO_ROLE_LABEL = {
  tier1: 'Power', tier2: 'Standard', tier3: 'Fast',
};

// Filled-steps rank meter (spec §Addendum3.C.3). steps = rung count, filled =
// rank. Token vars only; accessible aria-label "rank N of M".
...
async function _aiLoadPodDefaults() {
  ...
  // The auto-upgrade toggle + family map ride a parallel fetch; a failure
  // there degrades to pinned-id rows without blanking the editor.
  const [r, auto] = await Promise.all([
    api('GET', '/api/admin/config/pod/models').catch(() => null),
    _aiLoadAutoUpgrade('pod'),
  ]);
  if (!r || r.error) {
...
const _AI_POD_ROLE_ORDER = ['fast', 'standard', 'power', 'max'];
...
function _aiRenderPodDefaults() {
  ...
  // models to the pod's credentialed providers, so when EVERY ladder role
  ...
  const renderRole = (roleId) => {
    ...
    // Auto mode consolidates the chain to one row per model line ("family ·
    // latest"); the pending buffer (and the save payload) keeps the concrete
    // pinned ids either way — display-only grouping, group-aware ops.
    const modelRows = (_aiScopeAutoOn('pod')
      ? _aiFamilyGroups(models, _aiPodAuto.families).map((g, gi, arr) =>
          _aiTierFamilyRow('pod', roleId, g, gi, arr.length)).join('')
      : models.map((m, i) =>
          _aiTierModelRow('pod', roleId, m, i,
            `(idx=>_aiPodTierRemoveModel('${escHtml(roleId)}',idx))`, models.length)).join('')
      ) || `<div style="font-size:0.8rem;color:var(--text3);padding:3px 0;font-style:italic">(empty — add models below)</div>`;
    ...
    const addRow = _aiModelPickerRow({
      suggested: suggested,
      already: models,
      alreadyNote: _aiAlreadyNoteFor('pod', label),
  ...
  // The auto-upgrade toggle sits right under it — a top-level control on this
  // card (spec-model-auto-upgrade §Scope), not an easy-setup modal detail.
  el.innerHTML = `
    ...
    ${_aiAutoUpgradeToggleRow('pod', _aiPodAuto, true)}
    ${ladderSections}
...
// sync). Emitting these instead of synthetic
// "<role>-default" ids makes a saved pod default OVERLAY the code default
// (matching ids → replace in the keyed merge) instead of accumulating (spec
// Addendum 8 §D). The server canonicalizes again as the enforcement point, so
// an older client still produces a correct catalog; this just keeps the wire
// payload clean. Rung ids/costClass are catalog data, not provider literals.
...
// rung ids + costClass. No model literals; rung ids/costClass are catalog data.
...
let _aiEasyPreviewPending = false;  // a preview POST is in flight
let _aiEasyPreviewError = null;     // last preview error message (string | null)
let _aiEasyPreviewSeq = 0;      // discards stale in-flight responses on reorder
let _aiEasyFamilies = {};       // qualified id → family stem (server-computed)
// Auto-upgrade DISPLAY mode (spec-model-auto-upgrade-2026-07-30): null until
// the first preview seeds it from the scope's CURRENT resolved policy. The
// toggle itself lives on the tier cards (spec §Scope) — in here the flag only
// picks the preview rendering (family chips vs pinned ids); the modal never
// writes the policy.
let _aiEasyAutoUpgrade = null;
...
async function _aiOpenEasySetup(scope) {
  _aiEasyScope = scope;
  _aiEasyPreview = null;
  _aiEasyPreviewPending = false;
  _aiEasyPreviewError = null;
  _aiEasyFamilies = {};
  _aiEasyAutoUpgrade = null;
  await _aiLoadListings();
  ...
  // The preview IS the confirmation surface — compute it immediately (and on
  // every reorder) so there is never a dead Apply button waiting on a manual
  // Preview click. Cheap read-only POST; no write happens until Apply.
  _aiEasyPreviewRun();
}
...
function _aiEasyMoveProvider(idx, dir) {
  ...
  _aiEasyPreviewRun();     // recompute right away (renders the pending state)
}
...
function _aiRenderEasySetupModal() {
  ...
  // Preview: the server-returned reordered per-tier model order. Computed
  // automatically on open and on every reorder — the preview is the
  // confirmation surface, so it must always be present (or visibly on its
  // way) rather than gated behind a manual Preview click.
  let previewBody;
  if (_aiEasyPreview) {
    previewBody = _aiEasyPreviewRows(_aiEasyPreview);
  } else if (_aiEasyPreviewPending) {
    previewBody = `<div style="font-size:0.78rem;color:var(--text3)">Computing preview…</div>`;
  } else if (_aiEasyPreviewError) {
    previewBody = `<div style="font-size:0.78rem;color:var(--danger)">✗ ${escHtml(_aiEasyPreviewError)}</div>`;
  } else {
    previewBody = `<div style="font-size:0.78rem;color:var(--text3)">No preview — reorder a provider to recompute.</div>`;
  }
  const previewBlock = `
    <div style="margin-top:14px;padding-top:12px;border-top:1px solid var(--border)">
      <div style="font-size:0.8rem;font-weight:600;margin-bottom:8px">Resulting tiers — applied when you confirm</div>
      ${previewBody}
    </div>`;

  // Auto-upgrade posture note (spec-model-auto-upgrade §Scope): the toggle
  // lives on the tier cards, so the modal only SAYS which display it's using.
  // When the scope's policy is on, the preview above already renders one
  // "family · latest" chip per model line.
  const autoNote = _aiEasyAutoUpgrade
    ? `<div style="font-size:0.76rem;color:var(--text3);margin-top:10px">
        Automatic latest-version updates are on for ${_aiEasyScope === 'pod' ? 'this pod' : 'this bot'} — each entry above is the model line ridden, shown as “family · latest”. The toggle lives on the tier definitions card.
      </div>` : '';
  ...
  overlay.innerHTML = `
    <div class="modal" style="max-width:560px">
      ...
      <div style="font-size:0.82rem;color:var(--text2);margin-bottom:12px">
        Rank your providers by preference. Each tier keeps Evolve's sensible default models, reordered so your preferred provider leads its fallback chain. This overwrites the current ${_aiEasyScope === 'pod' ? 'pod default' : 'tier'} config when you apply.
      </div>
...
function _aiEasyPreviewRows(catalog) {
  ...
  // Auto-upgrade display mode: version-less family chips ("claude-haiku ·
  // latest") instead of pinned ids, deduped per (provider, family) — two
  // versions of one line collapse to the single entry auto-upgrade will ride.
  // Display only; the written config keeps the concrete ids either way.
  // (Pure reads only in here — the display-only guard test bans mutator
  // tokens in this region, so no push/pop even on fresh local arrays.)
  const auto = _aiEasyAutoUpgrade === true;
  const famOf = (m) => _aiEasyFamilies[m]
    || String(m).slice(String(m).indexOf('/') + 1);
  const dedupeForDisplay = (models) => {
    if (!auto) return models;
    const seen = new Set();
    return models.filter((m) => {
      const key = `${_aiProviderForModel(m)}/${famOf(m)}`;
  ...
  const modelsForRole = (roleId) => {
    ...
    return dedupeForDisplay((rung && rung.models) || []);
  };
  // Auto mode names the version the consolidated line would stand on — `m` is
  // the head of its family after dedupeForDisplay, i.e. the pin the resolver
  // would route to. "family · latest" alone is what read as a contradiction
  // against the picker's greyed exact id (2026-08-24 report).
  const chip = (m) => auto
    ? _aiModelChip(_aiFamilyPinLabel(famOf(m), [m]), _aiProviderForModel(m),
        { title: _aiFamilyPinTitle(_aiProviderForModel(m), famOf(m), [m]) })
    : _aiModelChip(String(m).replace('anthropic/', ''), _aiProviderForModel(m));
  ...
  return _AI_POD_ROLE_ORDER.map((roleId) => {
    ...
    return `<div style="display:flex;gap:8px;padding:3px 0;font-size:0.8rem">
        ...
        <span style="flex:1">${modelText}</span>
      </div>`;
  }).join('');
}
...
async function _aiEasyPreviewRun() {
  ...
  const seq = ++_aiEasyPreviewSeq;   // a reorder mid-flight obsoletes this run
  _aiEasyPreview = null;
  _aiEasyPreviewPending = true;
  _aiEasyPreviewError = null;
  _aiRenderEasySetupModal();
  const r = await api('POST', '/api/models/easy-setup', {
    scope, provider_order: _aiEasyOrder, preview: true,
  });
  if (seq !== _aiEasyPreviewSeq || scope !== _aiEasyScope) return;  // stale / modal closed
  _aiEasyPreviewPending = false;
  if (!r || r.error) {
    _aiEasyPreview = null;
    _aiEasyPreviewError = (r && r.error) || 'preview failed';
    _aiRenderEasySetupModal();
    return;
  }
  _aiEasyPreview = r.catalog || null;
  _aiEasyFamilies = (r.families && typeof r.families === 'object') ? r.families : {};
  // Seed the display mode from the scope's CURRENT policy once (the toggle
  // itself lives on the tier cards — the modal only mirrors the posture).
  if (_aiEasyAutoUpgrade === null) {
    _aiEasyAutoUpgrade = !!(r.auto_upgrade && r.auto_upgrade.enabled);
  }
  _aiRenderEasySetupModal();
}
...
async function _aiEasyConfirm() {
  ...
  // surface — the operator ranks providers, reviews the always-live per-tier
  // preview, then clicks "Apply easy setup" (whose title states it overwrites
  ...
  // No auto_upgrade_enabled here: the modal never writes the policy — the
  // top-level toggle on the tier cards owns it.
  //
  // That is a statement about the FILE KEY, not the RESOLVED POLICY, and the
  // two came apart (#3566 audit E-1). A bot scope flips the bot to Custom, and
  // model_auto_upgrade.bot_policy does NOT give a Custom bot the pod's
  // `enabled` — so leaving the key absent used to mean "resolves to the code
  // default, false" the moment the write landed, however true the pod's toggle
  // was. The writer now carries the pod's block across that flip (lifecycle
  // rule 1), which is what makes "untouched" true of the policy too. Omitting
  // the key here is correct; it is NOT self-evidently inert.
  const payload = { scope, provider_order: order };
  const r = await api('POST', '/api/models/easy-setup', payload);
  if (!r || r.error) {
...
function _aiRenderPodEngine(el, data) {
  ...
  // ALL FOUR roles render unconditionally from the merged view
  ...
  // steps meter. No "Tier N" numerals anywhere.
...
function _aiRenderPodOverride(el, currentVal) {
  ...
  // in canonical ascending order. Numeral-free role labels (spec
  // §Addendum3.C.1); the stored value stays tierN (the engine override is a
  // tier pin, not a role).
  const opts = [
