...
async function _aiRemoveFromTier(botId, tier, modelId) {
  ...
  // (historically standard+judge → sonnet-class; judge is its own judge-class
  // now, spec §Addendum 16, but a hand-authored roles map can still collide);
...
const _AI_ROUTE_TIER_TO_ROLE = { tier3: 'fast', tier2: 'standard', tier1: 'power', tier0: 'judge' };
...
// sync). judge owns its own judge-class rung (spec §Addendum 16) so its order
// persists independently of standard. Emitting these instead of synthetic
...
// rung ids + costClass. judge maps to its OWN judge-class rung (distinct from
// standard's sonnet-class), so a reordered/curated judge list persists instead
// of folding into — and reverting to — standard's rung. Keeps the
