...
// One consolidated tier row: a single "family · latest" chip standing for
// every pinned version of that model line in the chain. ↑/↓ move the whole
// group; × removes every member. NOT draggable — the DnD handlers splice flat
// model indices, which don't line up with group rows (↑/↓ are the primary,
// webview-safe reorder path anyway; DnD stays a pinned-row enhancement).
function _aiTierFamilyRow(scope, key, group, gi, total) {
  const es = escHtml(scope);
  const ek = escHtml(key);
  const label = `${group.fam} · latest`;
  const covered = group.models.map(m => m.replace('anthropic/', '')).join(', ');
  ...
  return `<div class="ai-tier-model-row">
      ...
      <span style="flex:1" title="Automatically rides the newest same-family version. Currently pinned: ${escHtml(covered)}">${_aiModelChip(label, group.provider, { onRemove: `_aiTierFamilyRemove('${es}','${ek}',${gi})` })}</span>
    </div>`;
}
...
function _aiAutoUpgradeToggleRow(apiScope, state, owned) {
  ...
  const consolidatedLine = state.enabled
    ? `<div style="font-size:0.76rem;color:var(--text3);margin-top:4px;padding-left:22px">
        Tiers below show one entry per model line — “family · latest” — instead of every pinned version.
      </div>` : '';
  return `
    ...
    const modelRows = resolved.length
      ? (_aiScopeAutoOn('bot')
          ? _aiFamilyGroups(resolved, _aiBotAuto.families).map(g =>
              `<div style="font-family:monospace;color:var(--text2);font-size:0.82rem;padding:2px 0" title="Automatically rides the newest same-family version. Currently pinned: ${escHtml(g.models.map(m => m.replace('anthropic/', '')).join(', '))}">${escHtml(g.fam)} · latest</div>`).join('')
...
async function _aiSaveRouting() {
  ...
  const routing = {
    ...
    // stored value through untouched so the wholesale write can't clobber it.
    ambiguousTier: (_aiBotConfig.routing || {}).ambiguousTier ?? null,
...
function _aiEasyPreviewRows(catalog) {
  ...
  const chip = (m) => auto
    ? _aiModelChip(`${famOf(m)} · latest`, _aiProviderForModel(m))
