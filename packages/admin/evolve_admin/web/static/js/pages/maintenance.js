...
// Last /api/oc/version payload — the apply modal reads the bot roster from it
// to render the "which gateways restart" half of the plan without a second
// round-trip.
let _ocVersionData = null;

// Why a report can't be applied, phrased for an operator. `stale` alone is a
// dead end when the cause is an unreadable version: re-checking produces the
// same verdict forever, so the note has to distinguish "the pod moved"
// (re-check) from "we can't read a version" (nothing to re-check).
function _ocStaleNote(sc) {
  if (!sc || !sc.stale) return '';
  const REASONS = {
    installed_changed: ['stale', 'The installed OpenClaw version changed since this report — re-check to enable the upgrade.'],
    candidate_changed: ['stale', 'npm published a newer version since this report — re-check to enable the upgrade.'],
    installed_unknown: ["can't read installed version", "Evolve could not read OpenClaw's package.json, so it cannot confirm this report still describes the pod. Re-checking will not clear this."],
    candidate_unknown: ['no candidate version', 'This report never resolved a candidate version from npm — re-check once the registry is reachable.'],
  };
  const [label, tip] = REASONS[sc.stale_reason] || REASONS.installed_changed;
  return ` <span style="color:var(--yellow)" title="${escHtml(tip)}">· ${escHtml(label)}</span>`;
}

// "Evolve has been validated against this OpenClaw", phrased for the banner.
// Before 2026-09-07 the card asked one question — is there a newer version on
// npm? — and treated the answer as permission to upgrade nine bots. It asks
// two now (internal/design-oc-upgrade-safety-2026-09-08.md §2 row 3), and an
// unvalidated version gets no button: absence of a contract run is not a pass.
function _ocValidationNote(v) {
  if (!v || v.state === 'tested') return '';
  const failing = (v.failing || []);
  const why = v.state === 'failed'
    ? (failing.length
      ? `failed on: ${failing.join(', ')}`
      : 'the compatibility contract failed on this version')
    : 'contract run pending';
  const overridden = v.overridden
    ? ' · <span style="color:var(--yellow);font-weight:600">operator override recorded</span>'
    : '';
  return `<div style="margin-top:8px;font-size:0.8rem">
      <strong>${escHtml(v.version || '?')} — not yet validated by Evolve</strong>
      <span style="color:var(--text3)">(${escHtml(why)})</span>${overridden}
    </div>`;
}
...
function renderOcUpgradeBanner(d) {
  ...
  // An unvalidated version is offered only behind a recorded override.
  const validated = !validation || validation.state === 'tested' || !!validation.overridden;
  const installed = escHtml(d.installed || '?');
  const latest = escHtml(d.latest || '?');
  const running = sc && sc.running;
  // A green, current report is the ONLY state that offers an apply
  // affordance (spec §7). Red or stale is not a thing to override in the UI —
  // forcing past a failed preflight stays CLI-only, with a human at a
  // terminal. An unvalidated OpenClaw is a separate veto on top of that one.
  const canApply = !!(sc && sc.ok && !sc.stale && !running && sc.report_id && validated);
  ...
  // One primary per surface (style-guide §10.4): while "Run upgrade now" is
  // present it is the primary and the check button demotes to secondary.
  const checkBtnClass = canApply ? 'btn btn-ghost btn-sm' : 'btn btn-primary btn-sm';

  const staleNote = _ocStaleNote(sc);
  ...
  if (running) {
    ...
    pill = `<span style="color:var(--green);font-size:0.82rem;font-weight:600">✅ Safe to upgrade</span>
      ...
      · <span style="font-size:0.74rem;color:var(--text3)">${escHtml(_relativeAge(sc.checked_at))}</span>${staleNote}`;
  } else {
  ...
  // The apply row replaces what used to be a copy-to-clipboard
  // `sudo evolve-admin oc upgrade` hint. That hint was a dead end for the
  // audience it was shown to: the in-app Terminal runs as the `evolve` service
  // user, which has no login password and no NOPASSWD: ALL, so the sudo prompt
  // could never be answered. Any privileged operation the web UI offers gets a
  // server-side endpoint, never a CLI hint.
  //
  // When the row is withheld, say why in its place. A yellow "update
  // available" banner with no affordance and no explanation is the state the
  // operator can do nothing with — they can't tell whether Evolve applies the
  // upgrade for them, and there is no command to fall back to.
  let applyRow = '';
  if (canApply) {
    applyRow = `<div style="margin-top:10px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
        <button class="btn btn-primary btn-sm" onclick="openOcApply('${escHtml(sc.report_id)}')">Run upgrade now</button>
        <span style="font-size:0.74rem;color:var(--text3)">You'll see the full plan before anything changes.</span>
      </div>`;
  } else if (!validated) {
    // Withholding the button because Evolve has not been validated against
    // this OpenClaw is a different fact from a red preflight, and it has a
    // different remedy — so it gets its own sentence and its own affordance.
    applyRow = `<div style="margin-top:10px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">
        <span style="font-size:0.74rem;color:var(--text3);flex:1;min-width:240px">Evolve has not been validated against this OpenClaw release, so it won't upgrade your pod onto it. A validated release is offered here automatically.</span>
        <button class="btn btn-ghost btn-sm" onclick="openOcOverride()">Upgrade anyway…</button>
      </div>`;
  } else if (!running) {
    const why = !sc
      ? 'Run the safety check to enable a one-click upgrade.'
      : (!sc.ok
        ? 'Evolve can apply this upgrade once the failing checks are resolved — open the report for what to fix.'
        : (sc.stale_reason === 'installed_unknown'
          ? "Evolve can't confirm which OpenClaw version is installed, so it won't apply an upgrade. Open the report for the path it tried to read."
          : 'This report no longer matches the pod. Re-check to enable a one-click upgrade.'));
    applyRow = `<div style="margin-top:10px;font-size:0.74rem;color:var(--text3)">${escHtml(why)}</div>`;
  }

  // A CARD titled for what it upgrades, not a banner that says "update
  // available" (chip oc-upgrade-is-a-guarded-change item 1). On 2026-09-07 the
  // operator read an update affordance on the Maintenance page as Evolve's and
  // moved the OpenClaw runtime under nine bots: "There was no warning that it
  // wasn't safe to do." The title now names the thing, and the red caption
  // says the blast radius before the version numbers, because the blast radius
  // is what was missing — not the numbers.
  const notesUrl = `https://github.com/openclaw/openclaw/releases/tag/v${encodeURIComponent(d.latest || '')}`;
  return `<div class="card" style="margin-bottom:14px;border-color:var(--yellow)">
    <div class="card-title">OpenClaw runtime</div>
    <p style="font-size:0.78rem;color:var(--red);margin:0 0 10px"><strong>Changes the runtime under every bot.</strong> This is not the Evolve upgrade — for that, use the <strong>Upgrade Evolve Pod</strong> card below.</p>
    <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;font-size:0.85rem;color:var(--yellow)">
      <div style="flex:1;min-width:240px">⚠️ Update available — installed <strong>${installed}</strong>, target <strong>${latest}</strong>${d.latest ? ` · <a href="${notesUrl}" target="_blank" rel="noopener noreferrer" style="font-size:0.78rem">release notes</a>` : ''}</div>
      <button class="${checkBtnClass}" onclick="runSafetyCheck(false)" ${btnDisabled}>${btnLabel}</button>
    </div>
    <div style="margin-top:8px">${pill}</div>
    ${_ocValidationNote(validation)}
    ${applyRow}
  </div>`;
}

// ── Operator override for an unvalidated OpenClaw ───────────────────────
//
// It is their pod, so the escape hatch exists. What it is NOT is a
// one-click "yes": the phrase the operator types names the contract checks
// that fail, so the confirmation is itself the disclosure — "the operator
// was the safety mechanism" is the failure mode this design retires, and a
// confirmation nobody reads recreates it. The server re-derives the phrase
// from the CURRENT failing set and rejects anything else, so an override
// goes stale the moment a fresh contract run changes which checks fail.

function openOcOverride() {
  const v = (_ocVersionData || {}).validation;
  if (!v || !v.version) return;
  const failing = (v.failing || []);
  const phrase = v.confirmation_phrase || `upgrade ${v.version} without validation`;
  document.getElementById('oc-override-version').textContent = v.version;
  document.getElementById('oc-override-checks').innerHTML = failing.length
    ? `<ul style="margin:6px 0 0 18px;padding:0">${failing.map(f => `<li><code>${escHtml(f)}</code></li>`).join('')}</ul>`
    : '<div style="color:var(--text3);margin-top:6px">No contract run has been recorded for this version — nothing has been checked, in either direction.</div>';
  document.getElementById('oc-override-phrase').textContent = phrase;
  const input = document.getElementById('oc-override-input');
  input.value = '';
  document.getElementById('oc-override-error').textContent = '';
  document.getElementById('oc-override-modal').classList.add('open');
  input.focus();
}

function closeOcOverride() {
  document.getElementById('oc-override-modal').classList.remove('open');
}

async function confirmOcOverride() {
  const v = (_ocVersionData || {}).validation || {};
  const err = document.getElementById('oc-override-error');
  err.textContent = '';
  try {
    await api('POST', '/api/oc/compat/override', {
      version: v.version,
      confirm: document.getElementById('oc-override-input').value,
    });
  } catch (e) {
    err.textContent = (e && e.message) ? e.message : 'Override was refused.';
    return;
  }
  closeOcOverride();
  loadOcVersion();
...
// ── Apply the upgrade (spec-oc-upgrade-from-ui-2026-07-28.md §7) ────────
//
// Two steps by design. Step 1 opens a modal showing X → Y, the gate summary,
// and the per-bot plan — the moment the operator learns which bots briefly
// lose plugin-backed features. Step 2 (confirm inside that modal) posts to
// /api/oc/upgrade/apply and the same modal becomes a live log. That ordering
// mirrors the CLI's `click.confirm("Proceed?")` sitting AFTER the dance
// preview, and it is load-bearing: a one-click button would commit before the
// operator has seen the blast radius.
//
// Never `confirm()` — native dialogs are suppressed in the desktop shell, so
// the button would silently do nothing (style-guide §9.6 rule 5).

let _ocApply = null;   // { reportId, from, to, jobId, timer, logged, planReady, preflightOk, preflightAcked }

async function openOcApply(reportId) {
  if (!reportId) return;
  _ocApply = {
    reportId, from: null, to: null, jobId: null, timer: null, logged: 0,
    planReady: false, preflightOk: false, preflightAcked: false,
  };
  document.getElementById('oc-apply-run').style.display = 'none';
  document.getElementById('oc-apply-log').innerHTML = '';
  document.getElementById('oc-apply-title').textContent = 'Run OpenClaw upgrade';
  document.getElementById('oc-apply-meta').textContent = '';
  document.getElementById('oc-apply-cancel').textContent = 'Cancel';
  document.getElementById('oc-preflight-gate').innerHTML = '';
  const confirmBtn = document.getElementById('oc-apply-confirm');
  confirmBtn.style.display = '';
  confirmBtn.disabled = true;
  document.getElementById('oc-apply-plan').innerHTML =
    '<div class="loading"><div class="spinner"></div> Building the plan…</div>';
  const overlay = document.getElementById('oc-apply-modal');
  overlay.classList.add('open');
  // Style-guide §9.6 rule 2 — X button, overlay click, and Esc all close.
  // Closing mid-run only stops watching; the job continues server-side.
  _ocApply.onOverlay = (e) => { if (e.target === overlay) closeOcApply(); };
  _ocApply.onKey = (e) => { if (e.key === 'Escape') closeOcApply(); };
  overlay.addEventListener('click', _ocApply.onOverlay);
  document.addEventListener('keydown', _ocApply.onKey);

  const report = await api('GET', `/api/oc/safe-upgrade/report/${encodeURIComponent(reportId)}`);
  if (!report || !report.gates) {
    document.getElementById('oc-apply-plan').innerHTML =
      '<div class="empty" style="color:var(--red)">That safety report is no longer available. Run the check again.</div>';
    return;
  }
  _renderOcApplyPlan(report);
  _ocApply.planReady = true;
  await _loadOcPreflightGate();
}

// ── Per-bot OC preflight gate (item 2) ───────────────────────────────────
//
// Distinct from the plan above (the safe-upgrade gates: node/stub/
// config-refs/…) — this is oc_preflight.py's per-bot table (model-ref
// rewrites, workspace-outside-home), cached per exact from → to pair so
// re-opening this same modal doesn't re-fetch and re-run nine bots' worth
// of `doctor`. The server enforces this gate too (POST /upgrade/apply
// 409s without it) — this is the UI half, not the only half.

function _updateOcApplyConfirmEnabled() {
  const btn = document.getElementById('oc-apply-confirm');
  if (!btn) return;
  btn.disabled = !(_ocApply && _ocApply.planReady && _ocApply.preflightOk && _ocApply.preflightAcked);
}

async function _loadOcPreflightGate() {
  const el = document.getElementById('oc-preflight-gate');
  el.innerHTML = '<div class="loading"><div class="spinner"></div> Checking per-bot OpenClaw preflight…</div>';
  const cached = await api('GET', `/api/oc/preflight/report/pair?target=${encodeURIComponent(_ocApply.to)}`);
  _renderOcPreflightGate(cached && !cached.error ? cached : null);
}

function _renderOcPreflightGate(report) {
  const el = document.getElementById('oc-preflight-gate');
  if (!_ocApply) return;
  if (!report) {
    _ocApply.preflightOk = false;
    _ocApply.preflightAcked = false;
    el.innerHTML = `
      <div style="font-size:0.85rem;color:var(--text)">
        <strong>Per-bot OpenClaw preflight</strong>
        <span style="color:var(--text3)"> — not yet run for ${escHtml(_ocApply.from)} → ${escHtml(_ocApply.to)}</span>
      </div>
      <div style="font-size:0.78rem;color:var(--text2);margin:6px 0 10px">
        Fetches the target to a scratch prefix (nothing is installed) and runs its own
        <code>config validate</code>/<code>doctor</code> against every bot's config as a dry run —
        this is what catches a model-ref rewrite or an agent workspace that would not boot.
      </div>
      <button class="btn btn-primary btn-sm" onclick="_runOcPreflightCheck()">Run OpenClaw preflight</button>
    `;
    _updateOcApplyConfirmEnabled();
    return;
  }
  const blocking = !!report.blocking;
  // hold-fix-4392: a report that exists but is too old, or predates a
  // bot-set change, can't gate the button either — same as blocking, from
  // the button's point of view, but a different sentence for the operator.
  const stale = !!report.stale;
  _ocApply.preflightOk = !blocking && !stale;
  _ocApply.preflightAcked = false;
  const rows = report.rows || [];
  const redRows = rows.filter(r => r.blocking);
  const rowLines = redRows.map(r => {
    const bits = [];
    (r.model_ref_rewrites || []).forEach(mr => {
      bits.push(mr.from
        ? `model ref ${escHtml(mr.from)} → ${escHtml(mr.to)} (${escHtml(mr.path || '')})`
        : `model ref change at ${escHtml(mr.path || '')}: ${escHtml(mr.detail || '')}`);
    });
    if (r.workspace_outside_home) bits.push(`agent workspace outside home: ${escHtml(r.workspace_outside_home)}`);
    if (r.error) bits.push(`could not check: ${escHtml(r.error)}`);
    return `<li><strong>${escHtml(botLabel(r.bot_id))}</strong> — ${bits.join('; ')}</li>`;
  }).join('');
  const statusLine = blocking
    ? `<span style="color:var(--red);font-weight:600">❌ ${escHtml(report.summary || 'blocked')}</span>`
    : `<span style="color:var(--green);font-weight:600">✅ ${escHtml(report.summary || 'clean')}</span>`;
  const staleNote = stale
    ? ` <span style="color:var(--yellow)">· ${escHtml(_ocPreflightStaleLabel(report.stale_reason))} — re-run</span>`
    : '';
  const ackRow = (blocking || stale) ? '' : `
      <label style="display:flex;align-items:flex-start;gap:8px;margin-top:10px;font-size:0.82rem;color:var(--text);cursor:pointer">
        <input type="checkbox" id="oc-preflight-ack" onchange="_ocPreflightAckChanged()" style="margin-top:2px">
        <span>I have read the per-bot preflight results (from → to list above) for ${escHtml(_ocApply.from)} → ${escHtml(_ocApply.to)}.</span>
      </label>`;
  el.innerHTML = `
    <div style="font-size:0.85rem;color:var(--text);margin-bottom:6px"><strong>Per-bot OpenClaw preflight</strong></div>
    <div style="font-size:0.82rem">${statusLine}
      <span style="font-size:0.72rem;color:var(--text3)"> · ${escHtml(_relativeAge(report.checked_at))}${report.actor ? ' by ' + escHtml(report.actor) : ''}</span>${staleNote}
    </div>
    ${rowLines ? `<ul style="margin:8px 0 0 18px;padding:0;font-size:0.8rem;color:var(--text)">${rowLines}</ul>` : ''}
    <div style="margin-top:8px">
      <button class="btn btn-ghost btn-sm" onclick="_runOcPreflightCheck()">Re-check</button>
    </div>
    ${ackRow}
  `;
  _updateOcApplyConfirmEnabled();
...
// "the pod redeployed since this ran" / "a bot was added or removed since
// this ran" — the two `stale_reason` values the pair route can send.
function _ocPreflightStaleLabel(reason) {
  return reason === 'bot_set_changed'
    ? 'a bot was added or removed since this ran'
    : 'this is no longer fresh';
}

function _ocPreflightAckChanged() {
  const box = document.getElementById('oc-preflight-ack');
  if (_ocApply) _ocApply.preflightAcked = !!(box && box.checked);
  _updateOcApplyConfirmEnabled();
}

async function _runOcPreflightCheck() {
  if (!_ocApply) return;
  const el = document.getElementById('oc-preflight-gate');
  el.innerHTML = '<div class="loading"><div class="spinner"></div> Fetching OpenClaw '
    + escHtml(_ocApply.to) + ' to a scratch prefix and checking every bot…</div>';
  const r = await api('POST', '/api/oc/preflight/check', { target: _ocApply.to });
  if (!r || !r.report_id) {
    el.innerHTML = '<div class="empty" style="color:var(--red)">Could not start the preflight check.</div>';
    return;
  }
  _pollOcPreflight(r.report_id);
}

function _pollOcPreflight(reportId) {
  const timer = setInterval(async () => {
    if (!_ocApply) { clearInterval(timer); return; }
    const r = await api('GET', `/api/oc/preflight/report/${encodeURIComponent(reportId)}`);
    if (r && r.status !== 'running') {
      clearInterval(timer);
      _renderOcPreflightGate(r.error ? null : r);
    }
  }, 1500);
}

window._runOcPreflightCheck = _runOcPreflightCheck;
window._ocPreflightAckChanged = _ocPreflightAckChanged;

function _renderOcApplyPlan(report) {
  const from = (report.current || {}).installed_version || '?';
  const to = (report.candidate || {}).resolved_version || '?';
  _ocApply.from = from;
  _ocApply.to = to;
  document.getElementById('oc-apply-meta').textContent =
    `openclaw ${from} → ${to} · report ${report.report_id || '?'} · checked ${report.checked_at || '?'}`;

  const gates = report.gates || {};
  const gateKeys = Object.keys(gates);
  const failed = gateKeys.filter(k => !gates[k].ok);
  const gateHead = failed.length
    ? `<span style="color:var(--red)">${failed.length} of ${gateKeys.length} gates failed</span>`
    : `<span style="color:var(--green)">All ${gateKeys.length} gates passed</span>`;
  const gateRows = gateKeys.map(k => `
    <div style="display:flex;gap:10px;padding:4px 0;font-size:0.8rem">
      <div style="width:20px;flex-shrink:0">${gates[k].ok ? '<span style="color:var(--green)">✅</span>' : '<span style="color:var(--red)">❌</span>'}</div>
      <div style="width:170px;flex-shrink:0;font-family:monospace;color:var(--text2)">${escHtml(k.replace(/_/g, '-'))}</div>
      <div style="flex:1;color:var(--text)">${escHtml(_gateSummary(k, gates[k]))}</div>
    </div>`).join('');

  // Per-bot plan. Neutralize + phantom cleanup are distilled from the same
  // config_references gate details the CLI's dance preview reads; the restart
  // list is every pod member, because the upgrade restarts all gateways.
  const details = (gates.config_references || {}).details || {};
  const missingByBot = details.missing_by_bot || {};
  const phantomByBot = {};
  (details.bots || []).forEach(b => {
    if ((b.phantom_installs || []).length) phantomByBot[b.bot_id] = b.phantom_installs;
  });
  const restartIds = Object.keys((_ocVersionData && _ocVersionData.bots) || {}).sort();

  const planItems = [];
  Object.keys(missingByBot).sort().forEach(botId => {
    planItems.push(`<li><strong>${escHtml(botLabel(botId))}</strong> — neutralize config references to ${escHtml((missingByBot[botId] || []).join(', '))} during the upgrade, then restore</li>`);
  });
  Object.keys(phantomByBot).sort().forEach(botId => {
    planItems.push(`<li><strong>${escHtml(botLabel(botId))}</strong> — drop stale plugin install records: ${escHtml(phantomByBot[botId].join(', '))}</li>`);
  });
  planItems.push(restartIds.length
    ? `<li><strong>Restart</strong> — every gateway (${restartIds.map(b => escHtml(botLabel(b))).join(', ')}) restarts on the new version</li>`
    : '<li><strong>Restart</strong> — every gateway restarts on the new version</li>');

  document.getElementById('oc-apply-plan').innerHTML = `
    <div style="font-size:0.85rem;color:var(--text);margin-bottom:12px">
      This runs the same sequence the pod's upgrade command runs, on this host,
      as a background job you can watch here.
    </div>
    <div style="font-weight:600;margin-bottom:6px;color:var(--text)">What will happen</div>
    <ul style="padding-left:18px;margin:0 0 6px;font-size:0.82rem;color:var(--text)">
      <li>Re-run the safety gates, then install <strong>openclaw ${escHtml(to)}</strong></li>
      ${planItems.join('')}
      <li>Re-check message delivery and Evolve's OpenClaw readers afterwards</li>
    </ul>
    <div style="font-size:0.78rem;color:var(--text2);margin:0 0 14px">
      Each bot is briefly offline while its gateway restarts, so plugin-backed
      features (chat channels, search, calendars) pause for a few seconds.
      There is no automatic rollback — if the install fails, this log and the
      safety report are what you'll have to work from.
    </div>
    <details style="border-top:1px solid var(--border);padding-top:8px">
      <summary style="cursor:pointer;font-weight:600;color:var(--text);font-size:0.85rem;display:flex;align-items:center;gap:6px">
        <span class="expand-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg></span>
        Safety gates — ${gateHead}
      </summary>
      <div style="margin-top:6px">${gateRows}</div>
    </details>`;
}

async function confirmOcApply() {
  if (!_ocApply || !_ocApply.reportId) return;
  // Client-side mirror of the server's own gate (item 2) — the server 409s
  // without a clean, acknowledged preflight for this exact pair regardless,
  // this just avoids a round trip for the common case of a stale checkbox.
  if (!_ocApply.preflightOk || !_ocApply.preflightAcked) return;
  const confirmBtn = document.getElementById('oc-apply-confirm');
  confirmBtn.disabled = true;
  document.getElementById('oc-apply-title').textContent = 'Upgrading OpenClaw…';
  document.getElementById('oc-apply-plan').innerHTML = '';
  document.getElementById('oc-apply-run').style.display = '';
  document.getElementById('oc-apply-progress-fill').style.width = '0%';
  document.getElementById('oc-apply-progress-label').textContent = 'Starting…';

  const r = await api('POST', '/api/oc/upgrade/apply',
                      { reportId: _ocApply.reportId, confirm: true });
  // api() returns {error: "..."} on HTTP errors rather than throwing, so a
  // refusal that never spawns a job (canary guard, deploy lock, stale report)
  // arrives here as parsed JSON. Surface the server's own reason.
  if (!r || !r.jobId) {
    _ocApplyLog('error', (r && r.error) || 'The upgrade could not be started.');
    _finishOcApply(false);
    return;
  }
  _ocApply.jobId = r.jobId;
  confirmBtn.style.display = 'none';
  document.getElementById('oc-apply-cancel').textContent = 'Close';
  _pollOcApplyJob(r.jobId);
}

function _ocApplyLog(level, msg) {
  const log = document.getElementById('oc-apply-log');
  if (!log) return;
  const line = document.createElement('div');
  line.className = `job-log-line ${level || 'info'}`;
  line.textContent = msg;
  log.appendChild(line);
  log.scrollTop = log.scrollHeight;
}

function _pollOcApplyJob(jobId) {
  if (_ocApply && _ocApply.timer) clearInterval(_ocApply.timer);
  const timer = setInterval(async () => {
    if (!_ocApply || _ocApply.jobId !== jobId) { clearInterval(timer); return; }
    const j = await api('GET', `/api/jobs/${encodeURIComponent(jobId)}`);
    if (!j || (j.error && !j.status)) {
      clearInterval(timer);
      _ocApplyLog('warning', 'Lost track of the upgrade job. Re-run the check to see where the pod landed.');
      _finishOcApply(false);
      return;
    }
    (j.log || []).slice(_ocApply.logged).forEach(l => _ocApplyLog(l.level, l.msg));
    _ocApply.logged = (j.log || []).length;
    const p = j.progress || {};
    if (p.total > 0) {
      document.getElementById('oc-apply-progress-fill').style.width =
        `${Math.round((p.current / p.total) * 100)}%`;
      document.getElementById('oc-apply-progress-label').textContent =
        `${p.label || ''} (${p.current}/${p.total})`;
    }
    if (j.status === 'complete' || j.status === 'failed') {
      clearInterval(timer);
      _finishOcApply(j.status === 'complete', j.error);
    }
  }, 1500);
  _ocApply.timer = timer;
}

function _finishOcApply(ok, error) {
  if (_ocApply && _ocApply.timer) { clearInterval(_ocApply.timer); _ocApply.timer = null; }
  document.getElementById('oc-apply-title').textContent =
    ok ? 'Upgrade complete' : 'Upgrade did not finish';
  document.getElementById('oc-apply-progress-label').textContent =
    ok ? 'Done.' : (error || 'Stopped.');
  document.getElementById('oc-apply-confirm').style.display = 'none';
  document.getElementById('oc-apply-cancel').textContent = 'Close';
  toast(ok ? 'OpenClaw upgraded' : 'Upgrade failed — see the log', ok ? 'ok' : 'err');
}

function closeOcApply() {
  const overlay = document.getElementById('oc-apply-modal');
  if (_ocApply) {
    if (_ocApply.timer) { clearInterval(_ocApply.timer); _ocApply.timer = null; }
    if (_ocApply.onOverlay) overlay.removeEventListener('click', _ocApply.onOverlay);
    if (_ocApply.onKey) document.removeEventListener('keydown', _ocApply.onKey);
  }
  _ocApply = null;
...
// Inline onclick handlers are invisible to ESLint's no-unused-vars — export
// them so the suppressions baseline stays flat.
window.openOcApply = openOcApply;
window.confirmOcApply = confirmOcApply;
window.closeOcApply = closeOcApply;
window.openOcOverride = openOcOverride;
window.confirmOcOverride = confirmOcOverride;
window.closeOcOverride = closeOcOverride;
...
function renderSafetyReportInto(report) {
  ...
  const gateOrder = ['node_version', 'stub_install', 'config_references', 'user_launchagents', 'plist_paths', 'port_owners', 'send_surface', 'compat_contract'];
  const gateLabels = {
    ...
    compat_contract: 'compat-contract',
  };
...
function _gateSummary(k, g) {
  const d = g.details || {};
  if (k === 'compat_contract') {
    // The gate's own rows are in the requirements block below — one per
    // failing contract check, which is what makes "3 blockers" read as WHICH
    // assumptions break. This line is the headline.
    if (d.skipped) return d.skipped;
    if (d.state === 'tested') return `Evolve is validated against ${d.candidate || '?'}`;
    const failing = d.failing_checks || [];
    if (d.overridden) {
      return failing.length
        ? `operator override recorded; still failing: ${failing.join(', ')}`
        : 'operator override recorded for an unvalidated version';
    }
    if (failing.length) return `contract fails on: ${failing.join(', ')}`;
    return `no contract run recorded for ${d.candidate || '?'} — not validated`;
  }
  if (k === 'node_version') {
