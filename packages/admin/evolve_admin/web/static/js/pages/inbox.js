...
function _inboxOpenIntakeTokenModal(slot) {
  ...
  // Slot-aware copy: the same modal serves the intake slots
  // (github_intake*) AND the general-code-access slots
  // (github_general_access / github-<bot> overrides) since the
  // Plugins/POD CTAs route here. The default markup explains the inbox
  // watcher — misleading when the operator came from "Set override" on
  // General code access, so swap title + description for those slots.
  const titleEl = document.getElementById('inbox-intake-token-title');
  const descEl = document.getElementById('inbox-intake-token-desc');
  if (descEl && !descEl.dataset.intakeHtml) descEl.dataset.intakeHtml = descEl.innerHTML;
  const isGeneralAccess = /^github_general_access|^github-/.test(slot || '');
  if (titleEl) {
    titleEl.textContent = isGeneralAccess
      ? 'Set up GitHub token for general code access'
      : 'Set up GitHub token for issue tracking';
  }
  if (descEl) {
    if (isGeneralAccess) {
      descEl.innerHTML =
        'This token is what bots use for general GitHub work — the ' +
        'GitHub MCP server, <code>gh</code>-style repo browsing, issues ' +
        'and PRs. It is separate from the per-bot <em>backup remote</em> ' +
        'credential and from the issue-tracking token.<br><br>' +
        'Use a PAT with <code>repo</code> scope (classic) on the account ' +
        'the bot should act as; for org repos behind SAML SSO, authorize ' +
        'the token for the org after creating it.';
    } else if (descEl.dataset.intakeHtml) {
      descEl.innerHTML = descEl.dataset.intakeHtml;
    }
  }
  modal.style.display = 'flex';
