// ════════════════════════════════════════════════════════════════════════
// PWA Phase 4 — tunnel CONNECT helper (spec docs/spec-pwa-2026-05-18.md §8)
//
...
(() => {
  ...
  function _showOverlay() {
    ...
    // Populate the pod URL from network config / current origin. Done
    // lazily here so we don't fight network.json load order at startup.
    if ($podUrl) {
      let podUrl = '';
      try {
        if (window._networkData && window._networkData.adminBaseUrl) {
          podUrl = window._networkData.adminBaseUrl;
        }
      } catch (_) { /* ignore */ }
