...
GET  /api/analytics/usage/by-user      per-user (and per-app x user) usage rollup
...
def _split_requester_key(key: str) -> "tuple[str, str]":
    """Split a rollup requester key into ``(platform, user_id)``.

    The key is ``platform:senderId``, written verbatim by
    ``recurringRequest.ts`` and stored verbatim by
    ``analyzer/usage_by_user.py``. Split on the FIRST colon only —
    a Telegram/Slack id never carries one, but a future platform whose
    ids do must keep its id intact rather than lose the tail.

    A key with no colon has no platform half; return ``("unknown", key)``
    rather than guessing one from the id's shape. Guessing a platform
    mis-attributes across id spaces, which is exactly the rule the
    writer of these keys refuses to break.
    """
    raw = str(key or "")
    platform, sep, user_id = raw.partition(":")
    if not sep:
        return "unknown", raw
    return (platform or "unknown"), user_id


def _user_row(
    network: dict, bot_id: str, shared_dir: Path, key: str, row: dict
) -> dict:
    """Attach a cache-only display name to one per-user rollup row.

    Display-name resolution is the ROSTER's job at read time, not the
    rollup's — the analyzer stores only the stable ``platform:senderId``
    key. This is the same cache-only resolver (``network.json`` +
    ``{shared_dir}/identity_cache``, no live Slack/Telegram call) that
    enriches the Usage page's By User rows, so both surfaces name the
    same person the same way.
    """
    from ..roster_resolver import resolve_display_name

    platform, user_id = _split_requester_key(key)
    name: "str | None" = None
    ...
    if user_id:
        ...
        except Exception:  # noqa: BLE001 — a name lookup must never 500 usage
...
def register_analytics_routes(app: Flask, network_path: Path) -> None:
    ...
    # ── Analytics: usage by user (D-S2 track 3) ───────────────────
    @app.get("/api/analytics/usage/by-user")
    def api_analytics_usage_by_user():
        """Per-user usage, and the per-app x user breakdown, per bot.

        Reads ``{shared}/{bot}/usage-by-user.json`` (written daily by
        ``analyzer/usage_by_user.py``). Like its ``by-app`` sibling this
        route is a thin re-shape and deliberately re-derives nothing —
        the honesty contract lives in the file:

          * a user's per-app metrics keep the grade split (``total`` is
            scheduled + explicit; ``inferred`` rides beside it and is
            never folded in);
          * ``unattributed_user`` and its ``coverage`` share are
            returned for EVERY bot, with the three-way split of WHY
            (no session id / no summary in the window / a summary that
            states no requester). A client that renders ``users``
            without that share is misreporting coverage — most turns on
            a real pod are machine-triggered and have no requester by
            construction;
          * ``user_attribution.available: false`` means the do-not-track
            gates WITHHELD attribution, which is not the same fact as
            "no users". Render it as withheld, never as zero.

        Identities are the ``platform:senderId`` keys the rollup stores
        verbatim; display names are resolved HERE, cache-only, via the
        same roster resolver the Usage page's By User rows use.
        ...
        try:
            ubu = _import_analyzer("usage_by_user")
        except Exception as exc:  # analyzer package missing — say so
            return jsonify({"error": f"usage_by_user unavailable: {exc}"}), 500

        network = load_network(network_path)
        out: dict[str, Any] = {}
        withheld: list[str] = []
        for bot in bots:
            payload = ubu.load_usage_by_user(shared, bot)
            if not payload:
                # No rollup file yet: "not measured", never zero usage.
                out[bot] = {"measured": False, "users": [], "apps": [],
                            "unattributed_user": None, "coverage": None,
                            "evolve_overhead": None, "user_attribution": None,
                            "generated_at": None}
                continue
            attribution = payload.get("user_attribution") or {}
            users = [
                _user_row(network, bot, shared, key, {
                    "user_key": key,
                    "first_seen_ts": entry.get("first_seen_ts"),
                    "last_seen_ts": entry.get("last_seen_ts"),
                    **(entry.get(window) or {}),
                })
                for key, entry in (payload.get("users") or {}).items()
            ]
            users.sort(key=lambda u: u.get("cost_estimated") or 0.0, reverse=True)
            apps = [
                {
                    "app_id": app_id,
                    "users": sorted(
                        (
                            _user_row(network, bot, shared, key, {
                                "user_key": key,
                                "total": blocks.get("total"),
                                "inferred": blocks.get("inferred"),
                            })
                            for key, blocks in (
                                (entry.get(window) or {}).get("users") or {}
                            ).items()
                        ),
                        key=lambda u: (u.get("total") or {}).get("cost_estimated") or 0.0,
                        reverse=True,
                    ),
                    "unattributed_user": (
                        (entry.get(window) or {}).get("unattributed_user")
                    ),
                }
                for app_id, entry in (payload.get("apps") or {}).items()
            ]
            out[bot] = {
                ...
                "apps": apps,
                "unattributed_user": (
                    (payload.get("unattributed_user") or {}).get(window)
                ),
                "evolve_overhead": (
                    (payload.get("evolve_overhead") or {}).get(window)
                ),
                "coverage": (payload.get("coverage") or {}).get(window),
            }
            if attribution.get("available") is False:
                withheld.append(bot)

        return jsonify({
            "window": window,
            "bots": out,
            # Bots whose user attribution the do-not-track gates withheld.
            # Distinct from "not measured" — say so in the UI.
            "attribution_withheld_bots": withheld,
        })
