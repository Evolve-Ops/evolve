...
construction (D-MB1).
    ...
    POST /api/board-bot/cards/<id>/progress {state,note,cost_to_date?}
                                                               board.progress
    POST /api/board-bot/briefing              {text,date?}   Morning Board's
                                                               composed panel
                                                               text (D-BI6d)
...
from ..board_store import (
    ...
    save_briefing, set_delegation_progress, visible_cards,
)
...
def _card_row(card: dict[str, Any]) -> dict[str, Any]:
    ...
    if card.get("actions"):
        # A one-glyph presence flag, not the actions themselves (D-BI5 §6):
        # est_cost/gating need a live pod-state read (board_actions), which
        # this per-turn list must not pay for on every card. The bot fetches
        # detail (and the current, re-gated actions) by acting on the id.
...
def register_board_bot_routes(app: Flask, network_path: Path) -> None:
    ...
    @app.post("/api/board-bot/briefing")
    def board_bot_briefing() -> ResponseReturnValue:
        """Morning Board (D-BI6d/e): hand the daemon the SAME text the daily
        message already delivered, so the board page's collapsible panel is
        the one composition delivered twice — never recomposed here."""
        bot_id = _caller()
        if bot_id is None:
            return _forbidden()
        body, refusal = _body()
        ...
        try:
            with WRITE_LOCK:
                briefing = save_briefing(
                    _shared_dir(), bot_id, str(body.get("text") or ""),
                    date=body.get("date"))
        except ValueError as exc:
            return _bad(str(exc))
        return jsonify({"ok": True, "briefing": briefing})

    # Fail closed (app contract v1): an app-facing route registered above
    # with no contract row refuses the whole registration, naming the row.
    app_contract.require_route_rows(app, "/api/board-bot/")
