...
def _resolve_api_key() -> str | None:
    """Resolve Anthropic API key from env var or the primary bot's
...
def _resolve_spec_model(power: bool = False) -> tuple[str, str, str]:
    """Resolve the model to use for spec generation.

    Returns ``(tier, model_bare, model_full)`` where:
      • tier is "tier2" by default, "tier1" when ``power=True``
      • model_bare is the Anthropic API form (no provider prefix), e.g.
        ``"claude-sonnet-4-6"`` — what the Messages API accepts
      • model_full is the canonical Evolve form, e.g.
        ``"anthropic/claude-sonnet-4-6"`` — what the SSE 'phase' event
        and audit logs surface

    Mirrors the resolver pattern in
    :func:`evolve_admin.applications.forge_engine._resolve_tier2_anthropic`
    so a single tier-registry change moves both surfaces in lockstep.
    Raises :class:`RuntimeError` if the tier resolves to a non-Anthropic
    provider — this code path posts directly to the Anthropic Messages
    API and cannot dispatch other providers.
    """
    try:
        from models import resolve_tier, check_tier_policy  # type: ignore
    ...
    try:
        full = resolve_tier(tier, config)
    except Exception as exc:
        raise RuntimeError(f"Could not resolve {tier}: {exc}")

    if not full.startswith("anthropic/"):
        raise RuntimeError(
            f"Spec generation requires an Anthropic-provider model for "
            f"{tier}; primary bot's {tier} resolves to {full!r}. "
            f"Configure an Anthropic model at AI Optimization → {tier} "
            f"to enable the wizard."
        )

    return tier, full.split("/", 1)[1], full
...
def _build_draft_events(
    *,
    ...
    if not api_key:
        yield {
            ...
            "message": (
                ...
                "or configure auth-profiles.json on the primary bot "
    ...
    try:
        tier, model_bare, model_full = _resolve_spec_model(power=power)
    ...
    for evt in _stream_anthropic(_SPEC_SYSTEM_PROMPT, user_message, model_bare, api_key):
        if evt["type"] == "delta":
    ...
    if not raw_text.strip():
        yield {"type": "error", "message": "Anthropic returned an empty response."}
        return
