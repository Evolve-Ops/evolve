...
# identity: see applications.app_identity.resolve_app_id (AL-1.4b). The only ``pkg_id`` in this module is MINTED by
# ``new_pkg_id()`` for a newly published package and then threaded into the
# write. There is nothing to resolve: the string does not exist until that call
# returns.
