...
def _announced(sig: Any, entry: dict[str, Any]) -> bool:
    ...
    1. ``Signal.deliveries`` holds an entry with no ``suppressed_reason`` —
       a producer recorded a real dispatch against this Signal.
    ...
    announcement — same principle, one layer down. Until audit learned to
    mark a page whose send failed, the only suppressed deliveries lived on
    snoozed/dismissed Signals, which this inventory never collects (firing
    only), so the distinction was unreachable here. It is reachable now: a
    CRITICAL whose page failed end-to-end stays firing and carries the
    undelivered marker, and counting that as announced would hide exactly the
    condition this digest exists to surface.
    """
    if any(getattr(d, "suppressed_reason", None) is None
           for d in (getattr(sig, "deliveries", None) or ())):
        return True
...
class StandingEntry:
    ...
    # R-3 event-vs-posture classification, mirrored onto the Signal by
    # audit's _emit_signals_from_findings. "posture" | "event" for audit
    # criticals; "" for everything else (warns, and every non-audit
    # producer) — so an empty value means "unclassified", NOT "event".
    finding_kind: str = ""
...
class StandingSummary:
    ...
    # How many of ``total`` are R-3 "posture" findings: standing
    # configuration violations that need one fix each and do not re-page
    # while they stand. Counts only what is explicitly classified, so it is
    # a floor, not a complement — the unclassified remainder is a mix of
    # "event" criticals and every non-audit producer, and must never be
    # rendered as though it were all active.
    posture_count: int = 0
...
def _sort_key(entry: StandingEntry) -> tuple:
    ...
    Deliberately does NOT rank on ``finding_kind``. An earlier revision did,
    on the theory that a posture finding is "old by definition" so age-first
    buries active events behind chores. Both halves were wrong:

    * The premise is false for the tamper-shaped rows — identity-file hash
      mismatch, writable policy file, and `.zshrc` drift are *change
      detections*, born at age ~0, and pending-security-update is commented
      "urgent" in audit.py itself.
    * The effect was the opposite of the intent. Only audit criticals carry
      a classification, so demoting posture promoted every UNCLASSIFIED
      entry above it — and the unclassified population is operational noise
      (disk, probes, gateway restarts) while the posture population is
      security criticals. On a pod with ordinary noise it evicted
      FileVault-off, SSH-password-on and bot-holds-admin from the board
      entirely.

    Surfacing an active event is handled by ``_reserve_event_slot`` instead,
    which can only ever displace another AUDIT finding — never an
    unclassified one.
    """
    return (
...
def _reserve_event_slot(
    listable: list[StandingEntry], top: tuple[StandingEntry, ...],
) -> tuple[StandingEntry, ...]:
    """Offer one shortlist slot to an active audit ``event``.

    Deliberately NOT a guarantee, and the docstring said "guarantee" until
    both review passes pushed back. When the shortlist holds no ``posture``
    entry there is nothing this may displace, so a one-hour-old credential
    exposure can still sit in "+N more" behind three older operational
    lines. That case is unserved on purpose: displacing an unclassified
    entry would assert that an audit event outranks a gateway-down, which is
    a much larger claim than this module should make by default. The
    asymmetry is real and worth naming — the rule will claim an event
    outranks a pending security update, but not that it outranks
    disk-91%-full.

    The narrow form of the problem the sort-key change got wrong: a posture
    finding stands for months while an active event is new, so a
    strictly-age-ordered shortlist can bury a three-day-old
    credential-in-workspace behind four fifty-day configuration chores.

    The fix displaces at most ONE entry, and only ever a ``posture`` entry —
    so it cannot demote a security finding below unclassified operational
    noise, which is how the previous attempt failed. No event outside the
    shortlist, or no posture inside it, means no change at all.

    Ordering within the returned shortlist is unchanged (the caller's sort
    key), so the board still reads worst-first.

    INTERIM, pending D-A5. This is a surface-local ranking exception, and the
    contract in spec-delta-alerts-two-lane-and-home-queue-2026-09-01 is that
    "needing action" is computed in the store, once, never re-derived by a
    surface — its own falsifier names a surface-local exception as the wrong
    fix. Defensible only because D-A5 is proposed rather than ratified and this
    rule is narrow. RETIRE IT when ``needs_operator`` feeds the board's top-N:
    an entry satisfying that predicate is what deserves the slot, and this
    function's event-vs-posture proxy stops being needed. Recorded as a row in
    that spec delta so the retirement is owed, not merely intended.
    """
    if not top:
        return top
    in_top = {e.signature for e in top}
    candidate = next(
        (e for e in listable
         if e.finding_kind == "event" and e.signature not in in_top),
        None,
    )
    if candidate is None:
        return top
    # Displace the LOWEST-ranked posture entry currently shown; `top` is
    # already in sort order, so that is the LAST posture in it. Taking the
    # first would drop the worst standing finding instead of the least-bad
    # one — it decides whether FileVault-off or bot-holds-admin falls off
    # the board.
    victim = next(
        (e for e in reversed(top) if e.finding_kind == "posture"), None,
    )
    if victim is None:
        return top
    # Never let the reservation undo the sort's own priorities. `_sort_key`'s
    # first two components are severity and never-announced-first, and a
    # swap that loses on either makes the board strictly worse:
    #
    #   * an ANNOUNCED event displacing a NEVER-ANNOUNCED critical inverts
    #     this module's founding purpose (the cold-start-silenced backlog it
    #     exists to surface). This is the steady state, not an edge — R-1
    #     pages events on create, so a standing event is normally announced
    #     while the silenced posture backlog is not;
    #   * a WARN event displacing an ALERT posture critical undoes the
    #     severity ordering. No audit emit site pairs a non-critical level
    #     with "event" today, but __post_init__ permits it and collect()
    #     reads finding_kind from any producer.
    if _sort_key(candidate)[:2] > _sort_key(victim)[:2]:
        return top
    kept = [e for e in top if e.signature != victim.signature]
    ...
    *transitions* are announced on their own path.
    ...
    for sig in store.iter_signals(shared_dir, subdirs=("firing",)):
        ...
        entries.append(StandingEntry(
            ...
            finding_kind=str(
                (getattr(sig, "details", None) or {}).get("finding_kind") or ""
            ),
        ))
    ...
    top = _reserve_event_slot(listable, tuple(listable[:top_n]))
    ...
    return StandingSummary(
        ...
        posture_count=sum(1 for e in entries if e.finding_kind == "posture"),
...
def render(summary: StandingSummary) -> str:
    ...
    # R-3's payoff for the operator: separate "there is a list of things to
    # go fix" from "something is happening right now". Without it the total
    # reads as one undifferentiated emergency, which is what trained the
    # operator to ignore the channel in the first place (design §1).
    #
    # Plain English, not "posture": a chat message is a PRIMARY surface
    # under docs/principle-plex-test.md §2, so the codebase's word for this
    # stays in the code. Phrased as a floor ("N of these"), never as a
    # complement — the rest of the set is NOT necessarily active, it is
    # merely unclassified (warns, and every non-audit producer).
    #
    # State what the finding IS. Two earlier revisions each added a clause
    # that was not ours to make, and both had to come out:
    #
    #   1. "…and will not message you again while they stand" — a DELIVERY
    #      promise, false four ways at once: this very section names them
    #      daily, the pod report's Broken bucket carries them as "Audit: N
    #      CRITICAL", a snooze wake re-pages, and sweep_resolve + re-fire
    #      re-pages.
    #   2. "…rather than a response right now" — a TRIAGE claim, and a
    #      wrong one. audit.py comments the pending-security-update rows
    #      "urgent but standing" (~:1498, :2218), and `.zshrc` drift is the
    #      most persistence-flavored member of the posture set; _sort_key's
    #      docstring a few lines above says exactly that, so the module
    #      contradicted itself in one message. Over-claiming about triage is
    #      worse than over-claiming about delivery — triage is the
    #      operator's call, and this line is not entitled to make it.
    #
    # What survives is the bare fact: these are fix-once items. The operator
    # decides what that means for their morning.
    if summary.posture_count:
        n = summary.posture_count
        lines.append(f"{n} of these {'is a' if n == 1 else 'are'} "
                     f"one-time fix{'' if n == 1 else 'es'}.")
