...
Per ``internal/spec-alert-notifier-2026-05-09.md``:
...
class Severity(str, Enum):
    ...
    Recorded on the audit log (so it is queryable), and load-bearing in
    exactly one gate: :func:`_is_bypass_alert` treats ``CRITICAL`` as a
    rate-cap / storm-mode bypass. It is NOT inert metadata awaiting a
    future gating pass, and it is NOT a cadence control — see the
    ``severity`` parameter docstring on :func:`send`. The dispatcher does
    not prepend severity emojis to the message; sources own their full
    message body.
    """
...
def send(
    *,
    ...
    message:
        ...
        provided.
    severity:
        Recorded on the audit log, and consulted by
        :func:`_is_bypass_alert`: ``Severity.CRITICAL`` exempts the send
        from the global rate cap and from storm-mode digest-only
        batching, routing it under the higher ``critical_max_per_window``
        ceiling instead. It does NOT change rendering, and it does NOT
        feed the subscription frequency/cadence decision — a producer
        cannot promote a digest-defaulted event to an immediate send by
        raising ``severity``. Frequency comes from the catalog event +
        operator override; ``severity`` only decides whether an
        already-immediate send may bypass the frequency-independent rate
        breaker.
    dedup_key:
    ...
    # ── Repeat-gate inputs, hoisted ──────────────────────────────────────
    # The digest short-circuit and the flapper demotion below both consult
    # the same (identical-content, cooldown) gate the immediate path uses
    # (_consult_repeat_gates), so its two inputs must exist before either
    # branch runs. The message body is FINAL here — catalog render and the
    # subscription footer are both already applied — so this is the hash of
    # the text the operator would actually see, on every path.
    ...
    if catalog_event is not None:
        ...
        if catalog_decision is None:
            ...
            # Digest modes route to the queue and return DEFERRED. The
            # Phase G flush daemon drains the queue on its own schedule.
            #
            # This branch used to short-circuit the cooldown / identical-
            # content gates outright ("digest modes short-circuit cooldown"),
            # which made every digest-defaulted producer's cooldown dead code
            # on this path: heal's deliberate 24h config_drift cooldown wrote
            # 287 queue entries + 287 audit rows in one day on the live pod to
            # produce the same 3 digest lines _dedup_records would collapse to
            # anyway (spec-delta-digest-audit-noise-2026-08-25 D1). We now
            # consult the SAME gate the immediate path uses before enqueuing,
            # and record the enqueue in the dispatcher state so the gate has
            # something to read on the next tick — the two halves are only
            # useful together.
            if frequency in ("daily_digest", "weekly_digest"):
                gate = _consult_repeat_gates(
                    shared_dir, source=source, dedup_key=dedup_key,
                    body_hash=body_hash, effective_cooldown=effective_cooldown,
                    now=now,
                )
                if gate is not None:
                    gate_result, gate_reason = gate
                    # Suppressed, but not ignored: repoint the record already
                    # pending in the queue at THIS body so the digest renders
                    # the latest figure rather than freezing on the first one.
                    # Replaces, never appends — the queue bound D1 bought
                    # (one row per collapse key per window) is preserved.
                    _refresh_digest_queue(
                    ...
                    return _suppressed(
                        gate_result,
                        source, severity, dedup_key, message,
                        shared_dir, now, reason=gate_reason,
                        catalog_event=catalog_event,
                    )
                _enqueue_digest(
                    shared_dir, frequency, now,
                ...
                if dedup_key is not None:
                    _record_dispatch(
                        shared_dir, source, dedup_key, now, body_hash,
                        result=DispatchResult.DEFERRED,
                    )
                return outcome
    ...
    # Apply the operator's floor HERE rather than after the flapper block, so
    # ``effective_cooldown`` means one thing at every consult site below. The
    # demotion path now consults the gate too, and it must not read a laxer
    # cooldown than the immediate path would for the same subscription.
    ...
    # this) and BEFORE the recipient/rate-breaker gates — because a demoted
    # fire must roll into the digest rather than page. It is NOT above the
    # cooldown gate any more: ``_demote_flapper`` consults the same gate
    # before enqueuing (D1), so a demoted signature queues once per cooldown
    # window instead of once per producer tick.
    ...
    # genuinely-unique events). The body_hash was computed above against the
    # final message text — same text the operator will see — so any
    # rendering or footer addition is reflected in the hash.
    gate = _consult_repeat_gates(
        shared_dir, source=source, dedup_key=dedup_key,
        body_hash=body_hash, effective_cooldown=effective_cooldown, now=now,
    )
    if gate is not None:
        gate_result, gate_reason = gate
        return _suppressed(
    ...
    # the bot token isn't on disk. See ``_dispatch_prefer_telegram_http``:
    # the same preference ALSO sidesteps the openclaw CLI's own config-
    # validation startup gate (breaker-notify-survives-openclaw-config-
    # validation) since the HTTP path never execs the CLI at all.
    sent_ok: bool = False
    error: str | None = None
    pre = _dispatch_prefer_telegram_http(channel, chat_id, message)
    if pre is not None:
        sent_ok, error = pre
    else:
...
def resolve_bot_recipient(bot_id: str, network: dict[str, Any]) -> tuple[str, str] | None:
    """Return ``(channel, chat_id)`` for *bot_id*'s own user — steps 2-3 of
    :func:`resolve_recipient`, but keyed to an explicit bot rather than the
    pod's primary bot. ``resolve_recipient`` answers "who does the OPERATOR
    hear alerts on"; this answers "who does THIS bot's own owner hear from
    it on" — the distinction a per-card ``remind`` touch (D-TM3, one bot's
    board, not the pod-wide alert channel) needs.
    """
    bots = (network.get("bots") or {}) if isinstance(network, dict) else {}
    bot_cfg = bots.get(bot_id) or {}
    return _external_ids.resolve_notify_target(
        bot_cfg.get("primary_user"),
        primary_channel=bot_cfg.get("primary_channel"),
        priority=_DEFAULT_CHANNEL_PRIORITY,
    )


def send_direct_to_bot(
    bot_id: str, network: dict[str, Any], message: str,
) -> tuple[bool, str | None]:
    """Deliver *message* to *bot_id*'s own user, zero model calls.

    Not an alert (no cooldown, no dedup, no audit log — the caller's own
    record, e.g. a card's ``touches[]``, is the audit trail) — a thin,
    bot-scoped composition of the same delivery primitives
    (:func:`resolve_bot_recipient`, :func:`_dispatch_via_openclaw_resilient`)
    the operator alert path uses, generalized off the primary-bot-only
    :func:`resolve_recipient`. Returns ``(ok, error)``; per D-CS7, ``error``
    always names the bot it could not reach rather than failing silently.
    """
    target = resolve_bot_recipient(bot_id, network)
    if target is None:
        return False, f"no notify target configured for bot {bot_id!r}"
    channel, chat_id = target
    from ..config import get_bot_port
    gateway_port = get_bot_port(bot_id, network)
    return _dispatch_via_openclaw_resilient(channel, chat_id, message, gateway_port)
...
def _dispatch_prefer_telegram_http(
    channel: str, chat_id: str, message: str,
) -> tuple[bool, str | None] | None:
    """Try the direct Telegram HTTP path; ``None`` means "fall through to
    the openclaw CLI".

    Factored out of ``send()`` so other best-effort senders — notably
    ``breakers_enforce``'s user-channel OOO post — get the same preference
    without duplicating the branch. The preference exists for latency
    (``_dispatch_via_telegram_http``'s docstring), but it has a second,
    load-bearing property this codebase now depends on: it never execs the
    ``openclaw`` binary, so it is immune to the CLI's own startup config-
    validation gate. The CLI resolves config off the CALLING PROCESS's
    ``$HOME`` (see ``_dispatch_via_openclaw``'s module docstring reference
    and ``feedback_oc_cli_is_uid_coupled_not_just_config_path``), which for
    every caller in this codebase is the ``evolve`` service account — so an
    invalid ``/Users/evolve/.openclaw/openclaw.json`` blocks EVERY CLI send
    regardless of which bot's message it is. Trying the token-only HTTP
    path first means a broken evolve-account config degrades to "the CLI
    fallback is unavailable", not "no bot's messages go out" (breaker-
    notify-survives-openclaw-config-validation).

    Returns the ``(ok, error)`` result when this path is authoritative
    (channel is ``"telegram"`` and a bot token was found on disk), or
    ``None`` when the caller should fall through to the openclaw CLI
    (a non-Telegram channel, or no token on disk yet).
    """
    if channel != "telegram":
        return None
    sent_ok, error = _dispatch_via_telegram_http(chat_id, message)
    token_missing = bool(error) and error.startswith("no-telegram-token")
    if sent_ok or not token_missing:
        return sent_ok, error
    return None
...
def dispatch_message(
    ...
    """Best-effort, single-attempt send: direct Telegram HTTP first, a
    single openclaw CLI attempt otherwise. Returns ``(ok, error)``.

    Public (unlike the underscored helpers above) because callers outside
    this module use it — ``breakers_enforce._notify_one_bot`` is the first,
    for the same reason ``send()`` prefers the HTTP path (see
    ``_dispatch_prefer_telegram_http``). No retry: a caller that wants
    bounded backoff across a gateway-restart window should call
    ``_dispatch_via_openclaw_resilient`` directly, as ``send()`` does — this
    function is for callers like the emergency-halt notify path that cap
    their own per-attempt wait and must not be blocked by CLI retries.
    """
    pre = _dispatch_prefer_telegram_http(channel, chat_id, message)
    if pre is not None:
        return pre
    return _dispatch_via_openclaw(
        channel, chat_id, message, gateway_port=gateway_port,
        timeout_seconds=timeout_seconds,
    )


# ─── No-send startup check (health control) ──────────────────────────────────

# Substring OpenClaw's config-validation startup guard prints as its error
# heading, on every command it blocks (verified against the installed
# runtime's ``dist/config-guard-*.js``: ``params.runtime.error(heading(
# "OpenClaw config is invalid"))``) — the exact text the 2026-09-20 incident
# recorded in ``notify_trip``/``notify_reset``'s ``detail`` field. Matched
# case-insensitively; CLI theming may colorize but does not reword it.
_CONFIG_INVALID_HEADING = "openclaw config is invalid"


def check_openclaw_cli_can_start(timeout: int = 10) -> tuple[str, str]:
    """No-send ``openclaw config validate`` run exactly as the notify path
    runs the CLI — no ``sudo -u <bot>``, no ``HOME`` override — so it
    answers "can THIS process's openclaw CLI even start", not "is some
    bot's config valid" (that question is
    ``openclaw_config_validator.validate_bot_openclaw_json``, which
    deliberately runs AS the bot for the opposite reason — see its module
    docstring). Every caller in this codebase that never overrides
    ``HOME``/``OPENCLAW_CONFIG_PATH`` (``_dispatch_via_openclaw`` included)
    resolves against the CALLING PROCESS's own ``$HOME/.openclaw/
    openclaw.json`` — the ``evolve`` service account's own file, since the
    admin daemon and analyzer jobs run as ``evolve`` (breaker-notify-
    survives-openclaw-config-validation item 5).

    Returns ``(state, detail)``:
      "ok"      — the CLI ran and its own config validates.
      "broken"  — the CLI ran and refused to start (its config fails
                  schema validation) — the exact failure mode that left
                  every breaker notice ``not_delivered`` for two weeks.
      "unknown" — the CLI could not be run or its output could not be
                  parsed (binary missing, timeout, non-JSON) — "could not
                  check" is its own state, never silently "ok" (D-CS7).
    """
    try:
        result = subprocess.run(
            ["openclaw", "config", "validate", "--json"],
            capture_output=True, text=True, timeout=timeout,
            # Same reasoning as _dispatch_via_openclaw: cwd must be
            # traversable by whoever runs this (the evolve user), and /tmp
            # always is.
            cwd="/tmp",
        )
    except FileNotFoundError:
        return "unknown", "openclaw binary not found on PATH"
    except subprocess.TimeoutExpired:
        return "unknown", f"validator timed out after {timeout}s"
    except OSError as exc:
        return "unknown", f"validator failed to launch: {exc}"

    text = (result.stdout or "").strip()
    if not text:
        stderr = (result.stderr or "").strip()
        if _CONFIG_INVALID_HEADING in stderr.lower():
            return "broken", stderr[:500]
        return "unknown", (stderr or f"no output (exit {result.returncode})")[:500]

    try:
        payload = json.loads(text)
    except json.JSONDecodeError:
        if _CONFIG_INVALID_HEADING in text.lower():
            return "broken", text[:500]
        return "unknown", f"validator output not JSON: {text[:200]}"

    if payload.get("valid") is True:
        return "ok", "config validates"
    issues = payload.get("issues") or []
    named = "; ".join(
        f"{i.get('path', '?')}: {i.get('message', '?')}" if isinstance(i, dict) else str(i)
        for i in issues
    )
    return "broken", (named or "config invalid")[:500]
...
def _record_dispatch(
    ...
    body_hash: str,
    *,
    result: DispatchResult = DispatchResult.SENT,
) -> None:
    """Stamp the per-(source, dedup_key) cooldown identity.

    ``result`` records which lane carried the dispatch. It used to be
    hardcoded ``SENT`` because the delivery path was the only writer; the
    digest-enqueue paths now write ``DEFERRED`` here too
    (spec-delta-digest-audit-noise-2026-08-25 D1), and stamping the real
    result keeps this on-disk forensic surface honest rather than claiming
    a chat push that never happened.

    ONE map, not one per lane: ``_consult_repeat_gates`` reads it
    result-blind, so the cooldown identity is lane-INDEPENDENT. A repeat is
    a repeat whether the previous fire was pushed to chat or queued for the
    digest — which is the property that makes the D1 fix hold when an
    operator flips a subscription between immediate and digest mid-window.
    """
    state = _load_state(shared_dir)
...
def _consult_repeat_gates(
    ...
    *,
    ...
    """The identical-content + cooldown gates, as one consultable unit.

    Returns ``(result, reason)`` when this (source, dedup_key) must be
    suppressed, or ``None`` to proceed. Extracted so the digest-enqueue
    paths consult the *same* logic as the immediate-push path rather than
    short-circuiting past it (spec-delta-digest-audit-noise-2026-08-25 D1);
    a second copy would drift.

    ``dedup_key is None`` always proceeds — those callers (resolve/recovery
    messages, the rate-cap downgrade, genuinely-unique events) have no
    cooldown identity by construction and must keep flowing. So does the
    first fire of a key with no recorded entry.

    Identical-content suppression (spec-alert-no-repeat-2026-06-01.md) is
    independent of the source's cooldown_seconds: a STATE_PERSISTS or
    PER_EVENT_UNIQUE source repeating the same (dedup_key, body_hash)
    inside the 24h floor is suppressed even when cooldown_seconds is
    shorter (e.g. an operator dropped it to 60s to debug a flap).
    STATE_TRACKED sources opt out via category — signal_notifier owns
    per-Signal-id dedup elsewhere and re-announces fresh ids deliberately.
    """
    ...
    if last_entry is None:
        return None
    elapsed = (now - last_entry.ts).total_seconds()
    category = _DEFAULT_SOURCE_CATEGORY.get(source, SourceCategory.PER_EVENT_UNIQUE)
    if (
        last_entry.body_hash is not None
        and last_entry.body_hash == body_hash
        and category != SourceCategory.STATE_TRACKED
        and elapsed < _IDENTICAL_CONTENT_FLOOR_SECONDS
    ):
        return (
            DispatchResult.SUPPRESSED_IDENTICAL,
            f"identical_within_24h:{int(elapsed)}s",
        )
    if effective_cooldown > 0 and elapsed < effective_cooldown:
        return (DispatchResult.SUPPRESSED_COOLDOWN, f"cooldown:{int(elapsed)}s")
    return None
...
# Logical name of the QUEUED lane — messages enqueued to a daily/weekly
# digest and not yet flushed. Date-partitioned like the other dispatcher
# log streams (alerts/dispatcher-queued/<YYYY-MM-DD>.jsonl) and pruned by
# the same retention sweep (signals.retention._ALERTS_LOG_STREAMS).
_QUEUED_LOG = "dispatcher-queued.jsonl"
...
def _log_filename_for(outcome: DispatchOutcome) -> str:
    ...
    Four lanes:
      * ``dispatcher.jsonl`` — the message reached (or terminally failed to
        reach) the operator's chat thread. The Recent Messages list on
        Reports → Subscriptions surfaces these by default, and its label
        is a promise: *"what the dispatcher actually sent"*.
      * ``dispatcher-queued.jsonl`` — DEFERRED: enqueued to a daily/weekly
        digest, not yet delivered. A deferred message is not something the
        operator's phone received, so it is NOT in the sent lane; it is
        also not suppressed (it *will* go out on the next flush), so
        folding it into the suppressed lane would trade one wrong label
        for another. Hidden by default; surfaced under the "include
        queued" toggle. Anchor: 2026-08-25, 287 of 292 rows in the live
        pod's "actually sent" feed were heal config-drift deferrals.
    ...
    if outcome.result == DispatchResult.DEFERRED:
        return _QUEUED_LOG
    if outcome.result in (
        ...
        # Deliberately NOT moved to the queued lane with DEFERRED: these
        # rows exist precisely so the operator sees the downgrade happen.
...
def _resolve_log_path(
    ...
    Log streams (``dispatcher.jsonl``, ``dispatcher-queued.jsonl``,
    ``dispatcher-suppressed.jsonl``, ``delivery-failures.jsonl``) are
    date-partitioned under
...
def iter_log_records(
    ...
    *,
    ...
    ``"dispatcher-queued.jsonl"``, ``"dispatcher-suppressed.jsonl"``,
    ``"delivery-failures.jsonl"``) — the same value
    :func:`_log_filename_for` returns.
...
def _demote_flapper(
    ...
    *,
    ...
    flap fields so the row explains WHY the condition stopped paging, rather
    than it going silently quiet.

    Lane note: because the outcome is ``DEFERRED`` these rows land in the
    QUEUED lane (``dispatcher-queued.jsonl``), not the sent lane — a demoted
    fire has not reached the operator's phone. The standing explanation is
    not lost: ``flap_health`` renders every currently-demoted signature on the
    Reports → Dispatcher Health panel, which is always visible, and the
    per-fire rows are one "include queued" click away.

    Cooldown note (spec-delta-digest-audit-noise-2026-08-25 D1): this is the
    second digest-enqueue path, and it had the same shape as the first —
    enqueue, log, return DEFERRED, all above the cooldown gates. It consults
    the same gate and records the same state, so a demoted signature ticking
    every few minutes queues once per cooldown window rather than once per
    tick. ``body_hash`` / ``effective_cooldown`` are required (not defaulted)
    precisely so a future caller cannot re-open the bypass by omitting them.

    A suppressed repeat still REFRESHES the record already pending in the
    queue (``_refresh_digest_queue``), so a demoted signature whose body
    mutates under a stable ``dedup_key`` renders its latest text rather
    than freezing on the first — without adding a second queue row.
    """
    gate = _consult_repeat_gates(
        shared_dir, source=source, dedup_key=dedup_key,
        body_hash=body_hash, effective_cooldown=effective_cooldown, now=now,
    ...
    if gate is not None:
        ...
        _refresh_digest_queue(
            shared_dir, "daily_digest", now,
            source=source, catalog_event=catalog_event, severity=severity,
            dedup_key=dedup_key, message=message, extra=digest_meta,
        )
        return _suppressed(
            gate_result, source, severity, dedup_key, message,
            shared_dir, now, reason=gate_reason, catalog_event=catalog_event,
        )
    _enqueue_digest(
        shared_dir, "daily_digest", now,
        source=source, catalog_event=catalog_event,
...
def _enqueue_digest(
    ...
    *,
    ...
    (the ones :func:`_digest_record` builds) always win, so render-critical
    fields can't be clobbered by an errant caller.
    """
    filename = _digest_queue_filename(frequency)
    if filename is None:
        return  # caller's bug; nothing to do
    _append_log(shared_dir, filename, _digest_record(
        now, source=source, catalog_event=catalog_event, severity=severity,
        dedup_key=dedup_key, message=message, extra=extra,
    ))


def _refresh_digest_queue(
    shared_dir: Path, frequency: str, now: datetime,
    *,
    source: str,
    catalog_event: str | None,
    ...
    """Point the pending digest record at THIS tick's body instead of enqueuing.

    Called on the two digest paths when ``_consult_repeat_gates`` suppresses a
    repeat. D1 made that suppression skip the queue entirely, which is right
    for the noise (no second queue row, no second audit row) but wrong for
    content: a producer whose body mutates under a stable ``dedup_key`` —
    ``cost.daily_threshold``, keyed ``spend_alert/threshold/<bot>/<date>`` for
    a whole day with a changing ``${amount:.2f}`` on its first line, the only
    line ``digest_dispatcher._short_line`` keeps — left the digest frozen on
    the FIRST body. Ticks of $5.02 → $12.40 → $41.85 rendered $5.02.

    So the repeat still updates the queue, it just does not GROW it: the
    pending record is replaced in place, keeping the bound at one row per
    ``(catalog_event, collapse_key)`` per cooldown window. The suppression
    itself is unchanged — the caller still returns the gate's result and
    still writes no audit row.

    Best-effort by construction. A failure here means the digest carries a
    slightly stale figure; raising would mean a producer tick blows up on a
    queue-maintenance detail. ``refresh_pending_record`` already swallows the
    lost-claim races internally and never rewrites a queue it did not claim.

    The import is function-local because ``digest_dispatcher`` imports THIS
    module at module scope; a top-level import here would be circular.
    """
    if dedup_key is None:
        # No collapse identity of the caller's choosing. The auto: fallback
        # keys on the body hash, so a CHANGED body would never match the
        # pending record anyway — and these callers (resolve/recovery, the
        # rate-cap downgrade) are never gated in the first place.
        return False
    from . import digest_dispatcher as _digest
    queue = _digest_queue_filename(frequency)
    if queue is None:
        return False
    try:
        return _digest.refresh_pending_record(
            shared_dir, frequency.removesuffix("_digest"),
            record=_digest_record(
                now, source=source, catalog_event=catalog_event,
                severity=severity, dedup_key=dedup_key, message=message,
                extra=extra,
            ),
        )
    except (OSError, ValueError):
        # ValueError covers the UnicodeDecodeError a non-UTF-8 byte in the
        # queue file raises out of ``_read_jsonl``. Before this path existed
        # only ``flush`` read that file, so a corrupt byte cost one flush;
        # it must not now propagate out of ``send`` and take down a producer
        # tick over a queue-maintenance detail. Deliberately caught HERE and
        # not papered over inside ``_read_jsonl`` with ``errors="replace"`` —
        # #3797 settled that an undecodable byte is a reportable finding, and
        # silently substituting characters in the flush path would undo that.
        return False


def _digest_queue_filename(frequency: str) -> str | None:
    """``digest-pending/<daily|weekly>.jsonl`` for a digest frequency."""
    if frequency == "daily_digest":
        return "digest-pending/daily.jsonl"
    if frequency == "weekly_digest":
        return "digest-pending/weekly.jsonl"
    return None
...
def _digest_record(
    ...
    *,
    ...
    """Build one digest queue record.

    Split out of :func:`_enqueue_digest` because a second caller needs the
    IDENTICAL shape: ``digest_dispatcher.refresh_pending_record`` replaces a
    pending record with the one a fresh enqueue would have written, so that
    a within-cooldown repeat whose body changed still renders the latest
    body (spec-delta-digest-audit-noise-2026-08-25 D1 follow-up). Two
    builders would let the refreshed record drift from the enqueued one —
    a different ``digest_collapse_key`` in particular would silently stop
    collapsing onto the record it was meant to replace.
    """
    record = {
        "ts": _iso(now),
        "source": source,
