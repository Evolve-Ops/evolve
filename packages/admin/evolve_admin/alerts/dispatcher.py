...
_DEFAULT_SOURCE_ENABLED: dict[str, bool] = {
    ...
    "review": True,            # Phase C: proposal rejected / reviewed / quarantined
...
_DEFAULT_SOURCE_CATEGORY: dict[str, SourceCategory] = {
    ...
    # Per-proposal dedup_keys cycle per proposal.
    "review": SourceCategory.PER_EVENT_UNIQUE,
...
_DEFAULT_CHANNEL_PRIORITY = ("telegram", "signal", "whatsapp", "slack", "discord")
...
def resolve_recipient(network: dict[str, Any]) -> tuple[str, str] | None:
    ...
    Resolution order (per spec § "Channel layer"):
      ...
      3. First channel present in ``<primary>.primary_user.external_ids``
         in priority order (telegram → signal → whatsapp → slack → discord).
    ...
    explicit = primary_cfg.get("primary_channel")
    primary_user = primary_cfg.get("primary_user") or {}
    ...
    for ch in _DEFAULT_CHANNEL_PRIORITY:
