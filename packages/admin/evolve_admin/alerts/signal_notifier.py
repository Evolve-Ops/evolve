...
Phase 4 of ``docs/spec-alert-notifier-2026-05-09.md``. Default-off in v1
(opt-in via ``alerts.signal_notifier.enabled``); flip to default-on as
Phase 7 once the burn-in is clean.
...
def _render_fire(sig: Any) -> str:
    ...
    if body:
        ...
        if body_norm and body_norm.lower() != _clean_title(sig.title or "", sig.bot_id).strip().lower():
            if len(body_norm) > _BODY_EXCERPT_CHARS:
                body_norm = body_norm[:_BODY_EXCERPT_CHARS] + "…"
...
def _attempt_delivery_recovery(
    ...
    if outcome.result in _HANDLED_DISPATCH_RESULTS:
        _sync_firing_as_known(store, shared_dir, state, denylist, now)
        state.pop("delivery_down", None)
...
def run_once(
    ...
    **Cold-start protection** — when ``notifier-state.json`` is absent
    ...
    backlog to the operator's chat in one tick. Operators see the
    backlog on the Alerts page already — signal_notifier is for
