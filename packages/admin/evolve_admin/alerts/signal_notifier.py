...
Phase 4 of ``internal/spec-alert-notifier-2026-05-09.md``. **Enabled by
...
def _catalog_event_for_signal(sig: Any) -> str:
    ...
    **Totality invariant (subscription-completeness keystone, spec-
    ...
    if producer == "security_warden":
        return "security.audit_finding"
    if producer == "orphaned_bot_account":
        # A decommissioned bot's account still holding live credentials is an
        # audit finding in every sense the operator cares about, and mapping it
        # here (rather than letting it fall through to meta.unclassified) gives
        # it the security_findings subscription handle. Digest-frequency is
        # correct: the condition is created by one operator action and then
        # persists indefinitely — it cannot flap, and nothing degrades while it
        # waits for the next digest.
        return "security.audit_finding"
    if producer == "plugin_install_provenance":
        # An `openclaw plugins install` refused because the package is not in
        # Evolve's provenance table. Supply-chain-shaped, so it belongs under
        # the security-findings handle — and it MUST have one: for the
        # programmatic caller (add_channel_to_bot) this Signal is the operator's
        # only view of the refusal (internal/design-plugin-install-provenance-gate-
        # 2026-08-11.md §5). meta.unclassified would hand it a mute labelled
        # "Uncategorized alert".
        return "security.audit_finding"
    if producer == "install_integrity_monitor":
        # The plugin install-tree findings are content-integrity on code that
        # runs inside every gateway — audit-shaped, and they must not land in
        # meta.unclassified, which would label a tampering finding
        # "Uncategorized alert". NOTE for anyone reading the `alert` severity
        # on plugin_digest_mismatch and expecting a page: security.audit_finding
        # is NOT in catalog.MUST_PAGE_ALLOWLIST, so it batches into the
        # security-findings digest like every other audit finding. That is
        # deliberate — detection here is daily, so a page would buy less than
        # a day, and the allowlist is curated for conditions where deferral
        # does real harm. Making it page means a bespoke catalog event plus an
        # allowlist slot, which is an operator's call, not a default.
        if sig_type.startswith("plugin_"):
            return "security.audit_finding"
        # Deliberately NOT mapping this producer's per-bot gauntlet findings
        # (ownership_drift, gateway_down, channel_handshake_failed, …). They
        # have always fallen through to meta.unclassified; both it and the
        # umbrella they would land under are DAILY_DIGEST, so remapping them
        # would not change any cadence — only move an operator's existing mute
        # from one handle to another. Worth doing, but as its own change, not
        # as a side effect of adding the plugin findings.
    ...
    # board_listener: inbound connections to the board's tailnet socket are
    # dying before the first byte. Dedicated IMMEDIATE event for the same
    # reason as sudoers/credential-store below — the digest-default
    # system.watchdog_event bucket would bury a deterministic, one-command
    # repair, and this producer only ever speaks when the mobile surface is
    # already fully dark.
    if producer == "board_listener":
        return "system.board_listener_unreachable"
