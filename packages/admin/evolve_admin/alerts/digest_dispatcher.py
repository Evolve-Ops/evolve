...
def collapse_identity(rec: dict[str, Any]) -> tuple[str, str] | None:
    """The ``(catalog_event, collapse_key)`` a queue record collapses onto.

    ``None`` when the record carries no collapse key at all — a legacy
    pre-``digest_collapse_key`` record with no ``dedup_key`` either, which
    :func:`_dedup_records` passes through uncollapsed.

    Factored out because two callers must agree on it exactly:
    :func:`_dedup_records` (flush-time collapse) and
    :func:`refresh_pending_record` (which rewrites the pending record that
    collapse would have picked). A second copy of the key construction is
    how the refresh would silently start patching a different record than
    the one the digest renders.
    """
    ck = rec.get("digest_collapse_key") or rec.get("dedup_key")
    if not ck:
        return None
    return (rec.get("catalog_event") or "", ck)


def _render_equivalent(a: dict[str, Any], b: dict[str, Any]) -> bool:
    """Do two queue records carry the same content, ignoring ``ts``?

    ``ts`` is excluded because a refresh never writes it: nothing downstream
    RENDERS it (``_render_digest`` never reads it, ``_cancel_transient_pairs``
    pairs on ``coalesce_key``/``kind``/``lifetime_seconds``, and
    ``_rollup_recovered_flaps`` only *copies* it), and its one real consumer
    — ``_pending_stats``, the staleness clock behind ``digest_health``'s
    ``stuck`` flag — needs the ORIGINAL, so :func:`refresh_pending_record`
    carries the replaced record's ``ts`` forward. Two records differing only
    in ``ts`` are therefore genuinely the same record for every purpose this
    function serves.

    That is also what keeps the constant-body producer this whole spec-delta
    exists to quiet (heal, 288 ticks/day) off the claim path entirely: every
    one of its repeats is ts-only, so :func:`refresh_pending_record` returns
    before claiming the queue.
    """
...
# Infix for the transient file ``refresh_pending_record`` claims the active
# queue into. Distinct from ``.flushing-`` so the two claim mechanisms can
# never target the same path, and matched by ``_stranded_orphans`` so a
# refresh that dies mid-claim has its records re-adopted by a later flush
# instead of stranded.
_REFRESH_CLAIM_INFIX = ".refreshing-"

# Absolute cap on how long a refresh claim may defer adoption, measured from
# the OLDEST evidence of the inode rather than the newest. The normal guard
# ages off `max(ctime, mtime)`, and ctime is bumpable by any metadata write —
# so a claim touched on a sub-30-minute cadence would look perpetually fresh
# and its records would never be delivered. That is the one shape in this
# design that would be LOSS rather than at-most-once-extra, and unlike the
# `.flushing-*` branch (whose staleness comes from the filename, which nothing
# can bump) it would otherwise be unbounded. `min` cannot be pushed forward by
# a metadata touch, so this bounds the strand at a day — far past any real
# claim, which lives for microseconds.
_REFRESH_CLAIM_HARD_ADOPT_SECONDS = 86_400


def _stranded_orphans(
    digest_dir: Path, frequency: str, now: datetime,
) -> list[Path]:
    """Claim files old enough to be genuine crash leftovers.

    Two shapes: ``.flushing-*`` (a ``flush`` rotation) and ``.refreshing-*``
    (a :func:`refresh_pending_record` claim). Both hold records that were
    taken out of the active queue and never put back, so adopting them into
    the next flush is what turns a crashed claim into at-most-once-extra
    delivery rather than lost events.

    For ``.flushing-*`` the embedded ISO timestamp is the rotation time. Files whose stamp is
    ...
    for p in sorted(digest_dir.glob(f"{frequency}.flushing-*")):
        ...
        if stale:
            out.append(p)
    # Refresh claims (``refresh_pending_record``) strand the same way, and are
    # recovered on the same terms. Their names carry no timestamp on purpose —
    # ``mkstemp`` uniqueness beats a clock here, because two refreshes inside
    # the same second must not collide with each other OR with a flush's
    # seconds-precision rotation name (an ``os.replace`` onto a live
    # ``.flushing-<iso>`` would destroy that flush's whole queue). Staleness
    # therefore comes from the inode stamps, which are name-independent.
    for p in sorted(digest_dir.glob(f"{frequency}{_REFRESH_CLAIM_INFIX}*")):
        try:
            st = p.stat()
        except OSError:
            continue
        # Age off ``max(ctime, mtime)``, and ctime is the load-bearing half.
        # A claim is made with ``os.replace``, which moves the ACTIVE QUEUE'S
        # inode onto this name — ``rename(2)`` carries the source's mtime
        # (its last APPEND, possibly hours ago) but resets ctime to the
        # rename, atomically. Reading mtime alone would therefore mint claims
        # that are born already "stale" on any queue not written in the last
        # half hour, and a flush globbing in the window before the refresh
        # requeues would adopt a claim that is very much in flight and
        # re-deliver its records.
        #
        # ``max`` rather than ctime alone so that an unrelated metadata touch
        # (or a filesystem where rename does not reset ctime) can only ever
        # make a claim look FRESHER. That errs toward "leave it alone", whose
        # cost is one more hourly tick before a genuinely stranded claim is
        # adopted; the opposite error double-delivers. The ``min`` arm is the
        # backstop that keeps "looks fresher" from meaning "never adopted" —
        # see ``_REFRESH_CLAIM_HARD_ADOPT_SECONDS``.
        stamps = (st.st_ctime, st.st_mtime)
        if (
            now.timestamp() - max(stamps) >= _ORPHAN_ADOPT_AFTER_SECONDS
            or now.timestamp() - min(stamps) >= _REFRESH_CLAIM_HARD_ADOPT_SECONDS
        ):
            out.append(p)
    return out
...
def refresh_pending_record(
    shared_dir: Path, frequency: str, *, record: dict[str, Any],
) -> bool:
    """Rewrite the pending queue record ``record`` would collapse onto.

    Returns ``True`` when a pending record was actually replaced.

    **Why this exists.** ``dispatcher.send``'s digest branch consults the
    cooldown / identical-content gate before enqueuing (spec-delta-digest-
    audit-noise-2026-08-25 D1), so a within-cooldown repeat never reaches the
    queue. For a producer whose body MUTATES under a stable ``dedup_key`` —
    ``cost.daily_threshold``, whose key is ``spend_alert/threshold/<bot>/<date>``
    for a whole day while its first line carries a changing ``${amount:.2f}``
    — the most-recent record therefore stopped existing, and the digest froze
    on the FIRST body. An understated cost figure is exactly the kind of
    number an operator acts on. This restores "the queue holds the latest
    body" without restoring "the queue grows one row per producer tick":
    the pending record is REPLACED, so the bound stays one row per
    ``(catalog_event, collapse_key)`` per window.

    **The race, and how it is defused.** The queue is append-only by design
    and ``flush`` rotates it out from under any reader with ``os.replace``.
    A naive read-modify-write would (a) resurrect records a concurrent flush
    had already claimed, and (b) silently drop a record another producer
    appended between the read and the rename. So this does NOT rewrite the
    file. It uses ``flush``'s own no-loss discipline:

      1. **Claim** — ``os.replace(active, staging)``. Atomic: exactly one of
         {this refresh, a concurrent flush, another refresh} gets the inode.
         Losing the claim means the records left with a flush and there is
         nothing to refresh — return ``False``, never rewrite.
         Concurrent producers keep appending; their ``O_APPEND`` opens
         re-create ``active`` and land there, untouched by us.
      2. **Patch** — replace the collapse group with ``record`` at the
         group's first position (exactly the record ``_dedup_records`` would
         have produced) and drop the group's stragglers.
      3. **Requeue** — append the patched set back onto ``active`` with the
         same append-mode writer ``flush`` requeues with.
      4. **Consume** — unlink the staging file.

    A crash anywhere in 1–3 leaves the staging file behind holding every
    claimed record; ``_stranded_orphans`` adopts it into a later flush, so
    the failure mode is at-most-once-extra delivery, never loss.

    Cheap pre-check first: the queue is read WITHOUT claiming it, and a
    no-op refresh (no matching record, or one already carrying this content
    per :func:`_render_equivalent`) returns before step 1. The constant-body
    producer never rotates the queue at all.
    """
    if frequency not in ("daily", "weekly"):
        raise ValueError(f"frequency must be 'daily' or 'weekly', got {frequency!r}")
    key = collapse_identity(record)
    if key is None:
        return False      # nothing to collapse onto → nothing to refresh
    active = _active_queue_path(shared_dir, frequency)

    pending = [r for r in _read_jsonl(active) if collapse_identity(r) == key]
    if not pending:
        return False      # never queued (or already flushed) — leave it alone
    if len(pending) == 1 and _render_equivalent(pending[0], record):
        return False      # ts-only delta — not worth claiming the queue

    digest_dir = _digest_dir(shared_dir)
    try:
        digest_dir.mkdir(parents=True, exist_ok=True)
        fd, staging_name = tempfile.mkstemp(
            dir=digest_dir, prefix=f"{frequency}{_REFRESH_CLAIM_INFIX}",
        )
        os.close(fd)
    except OSError:
        return False      # cannot stage → leave the queue exactly as it is
    staging = Path(staging_name)
    try:
        os.replace(active, staging)
    except OSError:
        # FileNotFoundError: a flush (or another refresh) claimed the queue
        # between our read and here. Any other OSError: we did not take the
        # inode either. Both mean "not ours to rewrite".
        _unlink_quiet(staging)
        return False

    claimed = _read_jsonl(staging)
    patched: list[dict[str, Any]] = []
    replaced = False
    for rec in claimed:
        if collapse_identity(rec) != key:
            patched.append(rec)
            continue
        if not replaced:
            # Position matters: _dedup_records keeps the group's FIRST slot
            # and the LAST content, so writing `record` here reproduces the
            # record flush would have rendered. The group's stragglers are
            # dropped — flush would have collapsed them anyway.
            #
            # ``ts`` is the ONE field carried over from the record being
            # replaced rather than taken from the fresh one. It is not a
            # render field — it is the staleness clock: ``_pending_stats``
            # takes the OLDEST pending ``ts`` and ``digest_health`` trips
            # ``stuck`` off its age. Re-stamping it on every refresh would
            # mean a queue whose flush daemon is dead can never age, so a
            # producer ticking every 5 min would report a stuck pipeline as
            # healthy indefinitely. Keeping the original ts leaves that
            # detector reading exactly what it read before this change.
            patched.append({**record, "ts": rec.get("ts") or record.get("ts")})
            replaced = True
    if not replaced:
        # The group vanished between the pre-check read and the claim (a
        # flush took it and a producer re-created `active` in the gap). Put
        # what we claimed back verbatim; do not invent a record.
        patched = claimed

    _requeue_records(active, patched)
    _unlink_quiet(staging)
    return replaced
