...
--------------------
Tracks, per ``(source, coalesce_key)``, the number of *flap events* — fires
AND standalone clears that reached the dispatch chokepoint, i.e. pushes the
operator would otherwise receive — inside a rolling 24h window. When a
signature has flapped ``≥ count`` times (default 4 events = two full
fire+clear oscillations), it is **auto-demoted**:
...
6h) AND the rolling window has drained below the threshold, at which point the
demotion **self-lifts** and the next fire pages normally again. The dispatcher
is the one call site; routing/logging live there, the window + episode *state*
live here — exactly the ``rate_breaker.py`` split.

Why the lift keeps the window (V-1, 2026-09-01)
-----------------------------------------------
The original D3 lift required only the 6h stability gap and WIPED the event
window on lift ("fresh accounting"). That combination was the demotion hole
the live evo-vps ``error_reporter`` ``secret_config_perms`` flapper drove
through for two weeks: it fires in clusters (1–4 fire/clear episodes per day,
45min lifetimes) whose natural gaps — overnight, mid-day — routinely exceed
6h. Each demotion self-lifted at the next cluster's first fire, erased the
accounting, and the whole K-count had to be re-earned with K−1 fresh pages:
6–8 operator pushes/day forever, ``demote_count`` ticking up while almost
nothing was ever suppressed (dispatcher JSONL 2026-08-18 → 09-01: one
deferred fire+clear pair per demotion, everything else ``sent``). Counting
only fires had the same edge-hugging failure: 3-episode days never reached
K=4 inside a strict 24h window. Now every push counts, and a lift requires
the window itself to have drained — a quiet afternoon no longer amnesties a
chronic daily flapper, while a genuinely settled condition (quiet past the
24h window) still lifts and pages on its next fire.
...
# Flap events (fires + standalone clears reaching the chokepoint) within the
# window that trip demotion. K=4 = two full fire+clear oscillations: the
# evo-vps motivating cases flap 1–4 episodes/day (2–8 pushes), so 4 catches
# even a 2-episode day within hours while leaving a single fire→clear→fire
# blip (3 events) paging normally.
...
# fire — AND the rolling window must have drained below the threshold — before
# the demotion self-lifts. 6h ≈ 3× the ~2h motivating flap cadence. The window
# condition is the load-bearing half for cluster-shaped flappers (see the V-1
# note in the module docstring); the cooldown remains as the fast-release path
# for a signature whose window is nearly empty.
...
def evaluate(
    ...
    *,
    ...
    Records the event into the per-signature rolling window (atomic write).
    ...
    ``kind`` is ``"fire"`` or ``"resolve"``. BOTH count toward the flap
    threshold — each is one operator push, and a fire+clear pair is one full
    oscillation — but only fires reset the stability clock the self-lift
    cooldown measures.
...
def _evaluate_locked(
    ...
    # Pre-V-1 state kept the list under "fires" (fires only); carry it
    # forward as the event window — counts start conservatively low.
    events = _prune(entry.get("events", entry.get("fires", [])), window_start)

    demoted = bool(entry.get("demoted"))
    prev_fire_at = _parse_iso(entry.get("last_fire_at"))
    last_fire_at = prev_fire_at
    demote_count = int(entry.get("demote_count") or 0)

    # ── Record this event ───────────────────────────────────────────────────
    # Fires and clears both count: each is one push the operator would
    # otherwise receive, so a fire+clear episode is two flap events. Only a
    # fire resets the stability clock — a trailing clear is the condition
    # settling, not new activity.
    events.append(_iso(now))
    events = events[-_MAX_RETAINED_TIMESTAMPS:]
    if kind == "fire":
        last_fire_at = now
    flap_count = len(events)

    # ── Self-lift (hysteresis): stable past the cooldown AND the window has
    # drained below the threshold → release. The stability gap alone is NOT
    # enough: a cluster-cadence flapper's natural gaps exceed the cooldown,
    # and lifting on the gap (as D3 originally did, wiping the window too)
    # meant every demotion was amnestied by the next quiet spell — the V-1
    # hole. The window is the arbiter of "settled"; the cooldown only gates
    # how soon after the last fire we are willing to re-page.
    lifted = False
    if demoted and prev_fire_at is not None and flap_count < cfg.flap_count:
        stable_for = (now - prev_fire_at).total_seconds()
    ...
    if demoted:
        action = ACTION_DEMOTE_FIRE if kind == "fire" else ACTION_SUPPRESS_CLEAR
        ...
        if lifted:
            ...
            reason = "below_threshold" if kind == "fire" else "not_demoted"

    # ── Persist ─────────────────────────────────────────────────────────────
    entry["events"] = events
    entry.pop("fires", None)  # legacy key, migrated above
    ...
    # Housekeeping: drop sibling entries that carry no signal anymore — not
    # demoted AND nothing left in their window — so the file doesn't grow
    # unbounded. (This entry itself always has the event just appended;
    # demoted entries are kept for the lazy lift on their next event.)
    sigs = state.get("signatures", {})
    for other_key in list(sigs):
        if other_key == key:
            continue
        other = sigs[other_key]
        if not isinstance(other, dict):
            sigs.pop(other_key, None)
            continue
        if other.get("demoted"):
            continue
        if not _prune(other.get("events", other.get("fires", [])), window_start):
            sigs.pop(other_key, None)
    ...
    *,
    ...
    for key, entry in (state.get("signatures") or {}).items():
        ...
        demoted.append({
            ...
            "flap_count": len(_prune(
                entry.get("events", entry.get("fires", [])), window_start,
            )),
