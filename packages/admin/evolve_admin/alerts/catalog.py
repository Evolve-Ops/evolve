...
CATALOG: tuple[CatalogEvent, ...] = (
    ...
    CatalogEvent(
        key="cost.soft_cap_warning",
        subscription="cost_warnings",
        category=Category.COST,
        label="Daily spend at 80% of the cap",
        description=(
            "A bot's daily spend crossed 80% of its hard cap. No "
            "enforcement — this is the heads-up that lets the operator "
            "raise the cap, stop a runaway, or simply expect the "
            "checkpoint, instead of meeting the cap for the first time "
            "as a held conversation. Fires at most once per bot per day, "
            "and never once the cap has actually tripped (the trip event "
            "owns that bot-day). Operator decision D-CC4, "
            "internal/decision-cost-cap-checkpoint-2026-09-04.md; the "
            "principle blesses soft warnings explicitly — 100% means "
            "stop, not warn harder."
        ),
        default_enabled=True,
        default_frequency=Frequency.DAILY_DIGEST,
        allowed_frequencies=_F_IMMEDIATE_OR_DAILY_DIGEST,
        ...
        producer_source="spend_alert",
        body_template=(
            "💰 {bot_id} is at {pct}% of today's cap\n"
            "Spent ${spend:.2f} of ${cap:.2f}. At the cap I pause "
            "background work and check in before the next reply."
        ),
        sample_payload={
            "bot_id": "admin_bot", "spend": 16.40, "cap": 20.00, "pct": 82,
        },
    ...
    # Our estimate has drifted from what the provider actually billed. Nothing
    # is broken and nothing was enforced differently — but every cost surface
    # (the daily cap, the 80% warning, the checkpoint message, the receipt)
    # reads the estimate, so a ratio outside 0.9-1.1 means all of them are off
    # by that factor. This is the standing check whose absence let a
    # family-substring price table overstate the power model ~3x for months
    # (internal/finding-cost-forensics-power-bot-2026-09-04.md §1). Digest by
    # default: it is a measurement-quality note the operator acts on at
    # leisure, raised at most once a week by the weekly-summary path.
    CatalogEvent(
        key="cost.estimate_drift",
        subscription="cost_warnings",
        category=Category.COST,
        label="Cost estimate doesn't match the provider bill",
        description=(
            "The last cost reconcile found Evolve's estimate more than 10% "
            "away from the figure the provider console billed. Spend caps and "
            "cost messages all run on the estimate, so they are off by the "
            "same factor until the prices behind it are corrected."
        ),
        default_enabled=True,
        default_frequency=Frequency.DAILY_DIGEST,
        allowed_frequencies=_F_IMMEDIATE_OR_DAILY_DIGEST,
        ...
        producer_source="spend_alert",
        body_template=(
            "⚠️ Cost estimate vs provider bill ({day})\n"
            "Estimated ${estimate:.2f}, billed ${console:.2f} "
            "(ratio {ratio}).\n"
            "Caps and cost messages read the estimate, so they are off by "
            "roughly that factor."
        ),
        sample_payload={
            "day": "2026-09-04",
        ...
        # No angle-bracket placeholder: the rendered action is HTML, and
        # "<export.csv>" parses as a tag (the dispatcher's renderer rejects
        # it). A concrete filename reads as an example anyway.
        action=cli_action(
    ...
    CatalogEvent(
        key="system.daemon_reloaded",
        subscription="pod_health",
        category=Category.SYSTEM,
        label="Evolve re-loaded a stopped background job",
        description=(
            "One of Evolve's own scheduled background jobs was installed but "
            "had stopped being registered with the system scheduler, so it "
            "silently wasn't running. Evolve re-registered it automatically. "
            "You're told because a job that drops out repeatedly is a bug "
            "worth chasing — the fix is not meant to hide it."
        ),
        default_enabled=True,
        # Digest, not IMMEDIATE: the repair already happened and nothing is
        # broken right now. The operator needs to KNOW (a job that keeps
        # falling out is a real bug — the 2026-09-07 doctor-pass bootout went
        # unnoticed for 7 days), not to be paged at 3am about a fixed problem.
        default_frequency=Frequency.DAILY_DIGEST,
        allowed_frequencies=_F_IMMEDIATE_OR_DAILY_DIGEST,
        severity=Severity.WARNING,
        producer_source="heal",
        body_template=(
            "🔧 Restarted {count} background job(s) that had stopped running\n"
            "{labels}\n"
            "These were installed but not registered with the scheduler, so "
            "they weren't firing. If this keeps happening, something is "
            "unloading them — worth investigating."
        ),
        sample_payload={
            "count": 2,
            "labels": "ai.openclaw.evolve.doctor-pass.team-bot-a, "
                      "ai.openclaw.evolve.doctor-pass.team-bot-b",
        },
    ),
    CatalogEvent(
        key="system.daemon_reload_failed",
        subscription="pod_health",
        category=Category.SYSTEM,
        label="A background job won't restart",
        description=(
            "Evolve tried to re-register a background job that had stopped "
            "running and the system scheduler refused, several times. The job "
            "is not running and Evolve has stopped retrying, so this needs a "
            "look — a malformed job file or a missing permission."
        ),
        default_enabled=True,
        default_frequency=Frequency.IMMEDIATE,
        allowed_frequencies=_F_IMMEDIATE_ONLY,
        ...
        producer_source="heal",
        body_template=(
            "🔴 {count} background job(s) won't start\n"
            "{labels}\n"
            "Evolve tried {attempts} times and has stopped retrying. "
            "Re-deploying the owning bot usually rewrites the job file and "
            "clears this."
        ),
        sample_payload={
            "count": 1,
        ...
        action=cli_action("sudo evolve-admin deploy --all"),
    ),
    CatalogEvent(
    ...
    CatalogEvent(
        key="system.board_listener_unreachable",
        subscription="pod_health",
        category=Category.SYSTEM,
        label="The board can't be reached from your phone",
        description=(
            "A device opened the board and the connection was cut before it "
            "could send anything. On a Mac this is the built-in firewall: it "
            "is on, and the Python that Evolve's admin service runs is not on "
            "its allowed list. Traffic from this machine to itself is exempt, "
            "so the admin pages keep working normally and nothing else looks "
            "wrong — only the phone fails."
        ),
        default_enabled=True,
        # IMMEDIATE, not digest — same reasoning as system.sudoers_refresh_failed
        # and system.credential_store_unreadable above. The condition is
        # deterministic (it cannot flap: it is a firewall rule, not a load
        # spike), the operator has one command, and until they run it the whole
        # mobile board is dark. A digest bucket would reproduce the 2026-06-10
        # shape where a firing signal sat undelivered for 24h+.
        default_frequency=Frequency.IMMEDIATE,
        allowed_frequencies=_F_IMMEDIATE_ONLY,
        severity=Severity.WARNING,
        producer_source="signal_notifier",
        body_template=(
            "⚠️ The board can't be reached from your phone\n"
            "{title} — the admin pages still work, so this only shows up on "
            "the phone. Run the command below, then restart the admin service."
        ),
        sample_payload={"title": "The board is unreachable from the phone"},
        action=cli_action("sudo evolve-admin ensure-pod-perms"),
    ),
    CatalogEvent(
