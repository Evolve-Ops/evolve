...
See ``docs/spec-alert-subscriptions-2026-05-10.md`` for design.
...
class Category(str, Enum):
    ...
    # Spec: docs/spec-subscription-completeness-2026-06-24.md.
    META = "meta"
...
# Spec: docs/spec-subscription-digest-default-2026-06-28.md.
#
...
CATALOG: tuple[CatalogEvent, ...] = (
    ...
    CatalogEvent(
        ...
        # CRITICAL security.cve_finding event, which stays IMMEDIATE.
    ...
    CatalogEvent(
        ...
        # critical, WARNING otherwise; CRITICAL is the honest ceiling for
        # the catalog metadata + any future severity-floor filtering.
        severity=Severity.CRITICAL,
    ...
    CatalogEvent(
        key="security.proposal_quarantined",
        subscription="security_findings",
        category=Category.SECURITY,
        label="Auto-proposed change blocked as unsafe",
        description="The reviewer screened a self-improvement proposal and flagged it as unsafe to apply.",
        default_enabled=True,
        default_frequency=Frequency.IMMEDIATE,
        allowed_frequencies=_F_IMMEDIATE_ONLY,
        severity=Severity.WARNING,
        producer_source="review",
        body_template=(
            "🛡️ Proposal {proposal_id} held back\n"
            "An auto-generated proposal was blocked from applying because it failed a security check.\n"
            "Reason: {reason}"
        ),
        sample_payload={
            "proposal_id": "prop_abc1234",
            "reason": "writes to bot's auth-profiles.json",
        },
        action=ui_action("Proposals → Quarantined"),
        is_safety_critical=True,
    ),
    CatalogEvent(
    ...
    # Spec: docs/spec-agent-freelance-bypass-2026-06-05.md.
    CatalogEvent(
    ...
    # killed per docs/decision-app-tests-2026-06-08.md. No producer can
    # emit this any more.
    CatalogEvent(
    ...
    # routine version bump (docs/openclaw-coverage-audit-2026-06-04.md F1/F2).
    ...
    CatalogEvent(
        key="decisions.proposal_rejected",
        subscription=None,
        # Internalized (Phase 2 chip 3): "told you the thing we decided
        # not to tell you." Drop the chat message. The internal records
        # this path feeds are untouched — the rejected proposal still
        # lands in proposals/rejected/, and the signal-tuning
        # feedback.jsonl (written by the Signal store / proposal-rejection
        # routes, NOT this dispatch) keeps working.
        notify=False,
        category=Category.DECISIONS,
        label="Self-improvement proposal rejected",
        description="The automated reviewer rejected a self-improvement proposal before showing it to you.",
        default_enabled=False,
        default_frequency=Frequency.DAILY_DIGEST,
        allowed_frequencies=_F_IMMEDIATE_OR_DAILY_DIGEST,
        severity=Severity.INFO,
        ...
        action=ui_action("Proposals → Rejected"),
    ),
    CatalogEvent(
