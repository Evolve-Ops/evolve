...
role IDs (``fast``/``standard``/``power``) introduced by
...
Mapping (spec §Legacy-shape fallback):
  ...
  tier0 → (no role) — merged into the standard chain, see below
...
Judge-role collapse (internal/design-judge-role-collapse-2026-08-21.md §6,
2026-08-31): the ``judge`` role and its ``judge-class`` rung were deleted —
cross-vendor checking is now derived from the standard chain at the call
site (``primary_bot.resolve_cross_vendor``). This migration additionally:

  - drops ``roles.judge`` (either shape — bare slug or the structured
    ``{rung, provider}`` object);
  - merges the judge rung's ``models[]`` into the STANDARD rung's chain
    (append after the existing chain, dedup, preserving the judge list's
    operator-curated order) so curated non-standard-vendor picks become
    fallback entries exactly where the cross-vendor derivation looks —
    nothing the operator chose is discarded. A judge rung still carrying
    the never-edited buggy default seed is deleted instead (nothing curated
    to preserve; the corrected standard default already covers it);
  - legacy ``tiers.tier0`` models merge into the standard chain the same way.

Idempotent, like every other transform here — a config with no judge
artifacts is returned unchanged.
...
TIER_TO_ROLE: dict[str, str] = {
    ...
    "tier2": "standard",
    "tier1": "power",
    # tier0 deliberately unmapped: the judge role is gone. Legacy tier0
    # models are merged into the standard chain by _collapse_judge_role /
    # the tier0 branches below (judge-role collapse §6).
}
...
_OLD_BUGGY_DEFAULT_CLUSTERS: dict[str, frozenset[str]] = {
    ...
    # judge shared the sonnet-class rung pre-decouple, so a never-edited buggy
    # judge seed carries the same stale set. Since the judge-role collapse this
    # entry is read only by _collapse_judge_role: a judge rung whose cluster
    # still equals this seed was never operator-edited and is DELETED rather
    # than merged into the standard chain (nothing curated to preserve).
...
def migrate_default_catalog_shape(models: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    # ``judge-class`` counts as canonical for the churn check even though the
    # judge role is gone: an orphan judge-class rung can legitimately survive
    # the collapse (see _collapse_judge_role's no-standard-rung case) and must
    # not flip needs_fix on every pass.
...
def migrate_models_block(models: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    ...
    if isinstance(tiers, dict) and not _is_already_migrated_models(models):
        ...
        # tier0 (the legacy judge slot): the judge role is gone (judge-role
        # collapse §6) — its models merge into the STANDARD rung's chain as
        # fallback entries (appended after the standard primaries, dedup,
        # preserving the tier0 list's curated order), which is where the
        # cross-vendor derivation looks. When the file defines no standard
        # rung of its own, the models are kept as an orphan ``judge-class``
        # rung instead (no role points at it) — creating a standard rung from
        # judge picks would silently change the bot's workhorse.
        ...
        if isinstance(tier0_models, list) and tier0_models:
            std_rung = next((r for r in rungs if r["id"] == "sonnet-class"), None)
            if std_rung is not None:
                for m in tier0_models:
                    if m not in std_rung["models"]:
                        std_rung["models"].append(m)
            else:
        ...
        # valid cost rank regardless of the payload's tier-key order.
    ...
    # Judge-role collapse (design-judge-role-collapse-2026-08-21 §6): drop
    # roles.judge and fold the judge rung into the standard chain. Runs BEFORE
    # canonicalization — the canonicalizer only emits rungs a ladder role
    # points at, so an un-collapsed judge rung passing through it would be
    # silently DROPPED instead of merged (discarding operator-curated models).
    collapsed_out, collapse_changed = _collapse_judge_role(out)
    if collapse_changed:
        out = collapsed_out
        changed = True
...
def _collapse_judge_role(models: dict[str, Any]) -> tuple[dict[str, Any], bool]:
    """Drop ``roles.judge`` and fold the judge rung into the standard chain.

    The judge-role collapse (design-judge-role-collapse-2026-08-21 §6, item 2).
    Pure + idempotent. On a rungs/roles-shaped block:

      - ``roles.judge`` (bare slug or structured ``{rung, provider}``) is
        removed.
      - The rung judge pointed at (or an orphan ``judge-class`` rung) is
        merged into the STANDARD rung's chain: its models are appended after
        standard's existing chain, dedup, preserving the judge list's
        operator-curated order — curated cross-vendor picks become fallback
        entries where ``resolve_cross_vendor`` looks. The judge rung is then
        deleted.
      - A judge rung whose cluster still equals the never-edited buggy default
        seed (``_OLD_BUGGY_DEFAULT_CLUSTERS['judge-class']``) is deleted
        WITHOUT merging — nothing operator-curated to preserve, and merging
        would pollute the corrected standard chain with ghost model ids.
      - When the block defines NO standard rung of its own, the judge rung is
        left in place as an orphan (only the role is dropped): seeding a
        standard rung from judge picks would silently change the workhorse,
        and the orphan stays parseable (read-compat) while never routing.
      - Judge pointing at standard's own rung (the pre-decouple shape) → only
        the role is dropped.

    Returns ``(new_models, changed)``; ``changed=False`` when no judge
    artifact exists (the idempotent second run).
    """
    if not isinstance(models, dict):
        return models, False
    raw_rungs = models.get("rungs")
    rungs: list[dict[str, Any]] = (
        [r for r in raw_rungs if isinstance(r, dict)]
        if isinstance(raw_rungs, list) else []
    )
    raw_roles = models.get("roles")
    roles: dict[str, Any] = raw_roles if isinstance(raw_roles, dict) else {}

    judge_entry = roles.get("judge")
    judge_slug: str | None = None
    if isinstance(judge_entry, str) and judge_entry:
        judge_slug = judge_entry
    elif isinstance(judge_entry, dict) and isinstance(judge_entry.get("rung"), str):
        judge_slug = judge_entry["rung"]
    if judge_slug is None and any(r.get("id") == "judge-class" for r in rungs):
        judge_slug = "judge-class"

    if judge_entry is None and judge_slug is None:
        return models, False  # no judge artifacts — nothing to do

    changed = False
    new_roles = dict(roles)
    if "judge" in new_roles:
        new_roles.pop("judge")
        changed = True

    new_rungs = [dict(r) for r in rungs]
    std_entry = new_roles.get("standard")
    std_slug = std_entry if isinstance(std_entry, str) and std_entry else "sonnet-class"
    judge_rung = next((r for r in new_rungs if r.get("id") == judge_slug), None)
    std_rung = next((r for r in new_rungs if r.get("id") == std_slug), None)

    if judge_rung is not None and judge_rung is not std_rung:
        judge_models = [
            m for m in (judge_rung.get("models") or []) if isinstance(m, str) and m
        ]
        if frozenset(judge_models) == _OLD_BUGGY_DEFAULT_CLUSTERS["judge-class"]:
            # Never-edited buggy seed — delete, don't pollute standard.
            new_rungs = [r for r in new_rungs if r is not judge_rung]
            changed = True
        elif std_rung is not None:
            std_models = [
                m for m in (std_rung.get("models") or []) if isinstance(m, str)
            ]
            for m in judge_models:
                if m not in std_models:
                    std_models.append(m)
            std_rung["models"] = std_models
            new_rungs = [r for r in new_rungs if r is not judge_rung]
            changed = True
        # else: orphan case — keep the rung, role already dropped above.
...
def _fold_legacy_tiers_into_rungs(
    ...
    for tier_key, t in tiers.items():
        ...
        if not cluster:
            continue
        # tier0 (the legacy judge slot — no role since the judge collapse):
        # MERGE into the standard rung's chain as fallback entries rather
        # than replacing any rung. No standard rung in this file → keep the
        # models as an orphan judge-class rung (parseable, never routed).
        if tier_key == "tier0":
            std_slug = _rung_slug_for("tier2") or "sonnet-class"
            target = next(
                (r for r in rungs if r.get("id") in (std_slug, "judge-class")),
                None,
            )
            if target is None:
                rungs.append({"id": "judge-class", "models": cluster, "costClass": "medium"})
                changed = True
            else:
                for m in cluster:
                    if m not in target.get("models", []):
                        target.setdefault("models", []).append(m)
                        changed = True
            continue
...
def register_cli(main) -> None:  # noqa: ANN001 — main is a click.Group
    ...
    def migrate_model_roles_cmd(ctx: click.Context, apply_changes: bool) -> None:
        ...
        Mapping: tier3→fast, tier2→standard, tier1→power;
        userTierOverride.dailyCap→roleCaps.power; allowBotInitiated(bool)→
        {power: <bool>, max: false}.

        ALSO collapses the retired judge role (design-judge-role-collapse-
        2026-08-21 §6): drops roles.judge, merges the judge rung's models into
        the standard chain as fallback entries (dedup, curated order kept;
        a never-edited buggy default judge seed is deleted instead), and folds
        legacy tier0 models the same way. Nothing the operator chose is
        discarded.
