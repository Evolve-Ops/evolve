...
PAUSE_ALL_TOOL = Tool(
    ...
    wire_description=(
        "Pause every bot in the pod: writes the pod-wide pause flag and "
        "unloads each gateway (~30s). Idempotent; reversible via "
        "action.pod.resume_all. Requires explicit confirm: true — this is a "
        "fleet-wide outage with a destructive-tier confirmation."
    ),
    input_schema={
        "type": "object",
        "properties": {
