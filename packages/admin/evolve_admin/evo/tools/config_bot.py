...
def _handler(
    ...
    try:
        ...
        # Daemon not yet serving the unix socket. Expected during the
        # Phase E migration window before the admin-ui daemon has been
        # restarted with the new binding code.
        log.debug(
            "config.bot: admin daemon unix socket unavailable (%s); "
            "falling back to direct-fs read", exc,
        )
