...
TOOL = Tool(
    ...
    wire_description=(
        "Read a bot's openclaw.json, summarized: gateway shape, "
        "agents defaults (model + fallbacks, heartbeat, compaction, "
        "context pruning), channels, plugins enabled, tools.exec/"
        "tools.web settings, and commands. Secrets are always "
        "redacted to presence/length sentinels. Use for 'what model "
        "is X running?', 'is Slack configured on Y?', 'what's Z's "
        "exec policy?', or before recommending a config change."
    ),
    input_schema={
        "type": "object",
        "properties": {
