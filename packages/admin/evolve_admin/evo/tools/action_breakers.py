...
def _reset_bot_handler(
    ...
    # Bring-up runs BEFORE the state clear so a failure leaves the breaker
    # tripped and the caller able to retry — see
    # breakers_enforce.reset_and_enforce (2026-09-04 reset incident).
    net = _network(network_path) or {}
    try:
        outcome = enforce.reset_and_enforce(
            shared_dir=shared_dir,
            scope=bot_id,
            breaker_type=breaker_type,
            network=net,
    ...
    if enforce_result is None:
        ...
        return {
            "ok": False,
            "error": f"state cleared but enforce failed: {outcome.enforce_error}",
            "scope": bot_id,
            "breaker_type": breaker_type,
            "was_tripped": True,
