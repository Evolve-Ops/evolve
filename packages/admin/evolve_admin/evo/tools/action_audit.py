...
Closes the Security page gap from internal/audit-evo-tool-coverage-2026-06-02.md:
...
RUN_TOOL = Tool(
    ...
    wire_description=(
        "Trigger an app-audit pass: scope='pod' re-runs every bot's "
        "audit, scope='bot:<id>' one bot's. Runs asynchronously (tens "
        "of seconds pod-wide) — returns immediately; re-read "
        "pod_state.audit after the sweep for fresh findings. If an "
        "audit is already running, joins it rather than queueing a "
        "duplicate."
    ),
    input_schema={
        "type": "object",
...
MUTE_TOOL = Tool(
    ...
    wire_description=(
        "Mute one audit finding so it leaves the active advisory "
        "list. finding_id is '<bot_id>::<message>' — discover the "
        "exact pair via pod_state.audit. reason is REQUIRED "
        "(non-empty free text); silent mutes are refused. Reversible "
        "via action.audit.unmute or the UI's Unmute button."
    ),
    input_schema={
        "type": "object",
...
def init_for_shared_dir(
    ...
    register(Tool(
        ...
        wire_description=RUN_TOOL.wire_description,
    ...
    register(Tool(
        ...
        wire_description=MUTE_TOOL.wire_description,
    ...
    register(Tool(
        ...
        wire_description=UNMUTE_TOOL.wire_description,
