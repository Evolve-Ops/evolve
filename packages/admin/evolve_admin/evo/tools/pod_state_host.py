...
TOOL = Tool(
    ...
    wire_description=(
        "Return host (admin server) CPU / memory / disk / load / uptime, "
        "each with a percent and an ok/warn/crit status tier. If psutil is "
        "unavailable, returns {available: false, reason} — say so rather "
        "than guessing. Use for 'is the host healthy?' / 'how full is the "
        "disk?' or to check whether a host_health signal reflects real "
        "pressure."
    ),
    input_schema={
        "type": "object",
