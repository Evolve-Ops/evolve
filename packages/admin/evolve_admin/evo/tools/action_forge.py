...
``internal/audit-evo-tool-coverage-2026-06-02.md`` for the Improve → Apps
→ Forge surface:
...
APPROVE_TOOL = Tool(
    ...
    wire_description=(
        "Approve a forge job awaiting operator sign-off — the build "
        "starts on the next dispatcher tick. The job MUST be in state "
        "'awaiting_approval'; check pod_state.forge_job first if "
        "uncertain. Starts real npm/git/llm work: cancellable "
        "mid-flight via action.forge.cancel but not easily undone "
        "post-completion."
    ),
    input_schema={
        "type": "object",
...
REJECT_TOOL = Tool(
    ...
    wire_description=(
        "Reject a forge job awaiting operator sign-off — the build "
        "does NOT run. The job must be in state 'awaiting_approval', "
        "'queued', or 'running'; completed jobs cannot be rejected. "
        "Not destructive: revisiting just means dispatching a new "
        "job. Use when the operator rejects the job or the plan needs "
        "rework."
    ),
    input_schema={
        "type": "object",
...
def init_for_shared_dir(
    ...
    register(Tool(
        ...
        wire_description=APPROVE_TOOL.wire_description,
    ...
    register(Tool(
        ...
        wire_description=REJECT_TOOL.wire_description,
