...
from mcp.types import CallToolRequestParams, CallToolResult  # noqa: E402
from mcp.types import ListToolsResult, PaginatedRequestParams  # noqa: E402
...
def build_server(context: RuntimeContext) -> Server:
    ...
    # decorators; handlers are constructor keyword arguments taking a
    # `ServerRequestContext` and the request's typed params, and returning the
    # typed Result. The bodies are unchanged — same advertised surface, same
    # JSON payload — so the wire is identical; only the envelope moved.
    async def _list_tools(
        _ctx: ServerRequestContext, _params: PaginatedRequestParams | None = None
    ) -> ListToolsResult:
        return ListToolsResult(tools=advertised_mcp_tools())

    async def _call_tool(
        _ctx: ServerRequestContext, params: CallToolRequestParams
    ) -> CallToolResult:
        payload = await dispatch_tool_call(
            params.name, dict(params.arguments or {}), context)
