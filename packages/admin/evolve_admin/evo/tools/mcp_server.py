...
def _to_mcp_tool(
    ...
    return McpTool(
        ...
        description=_sanitize_description(tool.description, canonical_names),
...
def build_server(context: RuntimeContext) -> Server:
    ...
    async def _list_tools() -> list[McpTool]:
        # Snapshot the canonical name set once so description rewrite is
        # O(N×K) instead of O(N²) when the registry grows.
        tools = all_tools()
        canonical = tuple(t.name for t in tools)
        return [_to_mcp_tool(t, canonical_names=canonical) for t in tools]

    @server.call_tool()
    async def _call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
        # OC dispatches via the SANITIZED name (what was advertised in
        # list_tools), e.g. ``pod_state__config_drift``. Reverse-translate
        # to the canonical dotted form to find the registered handler.
        # We accept the canonical form too as a defensive measure — internal
        # callers (tests, fallback paths) may pass it directly.
        ...
        if match is None:
            ...
            # Phase 2 authorization gate. Refuse before invoking the
            # handler when the caller's surface doesn't satisfy the
            # tool's ``authorization_scope``. The refusal envelope is
            # the tool result the model sees — it carries a clear
            # operator-facing message + structured fields so the model
            # can route the operator to the right escalation path
            # (Telegram or admin UI).
            denial = check_authorization(match, context.caller_identity)
            if denial is not None:
                log.info(
                    "Tool %s denied for caller surface=%s "
                    "(scope=%s)", match.name, context.caller_identity.surface,
                    match.authorization_scope,
                )
                payload = denial
            else:
                wrapped = _wrap_handler(match, context)
                payload = await wrapped(arguments or {})
