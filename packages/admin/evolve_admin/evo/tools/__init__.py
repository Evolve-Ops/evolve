...
Architecture (per ``internal/spec-evo-oc-native-2026-05-19.md``):
...
# explicitly opened. See ``internal/audit-evo-tool-coverage-2026-06-02.md``
...
class Tool:
    ...
    # B7 (spec-evolve-overhead-budget §B7): the SHORT description that rides
    # the OC/MCP wire — and therefore the primary bot's per-turn prompt, where
    # the full 88-tool manifest measured ~27k tokens. When set, this is what
    # the model sees in-prompt (selection + basic usage + any hard DO-NOT);
    # the rich ``description`` stays as the on-demand teaching surface served
    # by ``meta.tools``. Empty → ``description`` rides the wire unchanged
    # (fine for tools whose description is already budget-sized; the wire
    # budget test caps the EFFECTIVE wire form for every tool either way).
    ...
    @property
    def oc_description(self) -> str:
        """The description that rides the OC/MCP wire (B7 context diet).

        ``wire_description`` when set, else the full ``description``. Every
        prompt-facing renderer (``build_tool_manifest``, the MCP bridge's
        ``list_tools``) MUST use this — the full ``description`` is the
        on-demand teaching surface (``meta.tools``), not per-turn context.
        """
        return self.wire_description or self.description
...
def _render_for_oc(tool: Tool) -> dict:
    ...
    return {
        "name": tool.name,
        "description": tool.oc_description,
...
from . import pod_state_app_usage  # noqa: E402, F401  -- registers pod_state.app_usage (AL-1.3 per-app attribution rollup)
from . import pod_state_app_outcomes  # noqa: E402, F401  -- registers pod_state.app_outcomes (design §3.2 outcome view)
...
from . import action_scan          # noqa: E402, F401  -- registers action.scan.run (umbrella for Pattern A one-shot scan/refresh triggers — closes 8 gaps from internal/audit-evo-tool-coverage-2026-06-02.md)
from . import action_keys          # noqa: E402, F401  -- registers action.keys.{add,rotate,remove} (closes 10 Pattern B gaps from internal/audit-evo-tool-coverage-2026-06-02.md — keystore touches via admin-ui HTTP)
