...
ACCEPT_DRIFT_TOOL = Tool(
    ...
    wire_description=(
        "Accept a bot's live openclaw.json as the new backup-baseline "
        "so the config_drift signal clears — same as the 'Accept as "
        "baseline' button. Use when the operator says 'accept the "
        "config as baseline'; for 'accept all', enumerate with "
        "pod_state.config_drift and call once per drifted bot. "
        "Distinct from audit-finding Mute, which suppresses an audit "
        "advisory."
    ),
    input_schema={
        "type": "object",
