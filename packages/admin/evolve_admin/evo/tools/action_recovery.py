...
POINTS_TOOL = Tool(
    ...
    wire_description=(
        "List candidate rollback targets for a bot — daily backup "
        "commits by default (only_backup=false for the full workspace "
        "git log). Each entry's 'commit' field is what to pass as "
        "'target' to action.bot.rollback. Use when the operator asks "
        "'what can I roll back to?'. Read-only — no side effects."
    ),
    input_schema={
        "type": "object",
        "properties": {
...
ROLLBACK_TOOL = Tool(
    ...
    wire_description=(
        "Roll back a bot's openclaw.json to a prior commit (target — "
        "get it from pod_state.rollback_points). Captures a "
        "pre-rollback snapshot so it can be undone via "
        "action.bot.reverse_rollback. Restarts the gateway by default "
        "(skip_restart=true to defer). DESTRUCTIVE — always asks for "
        "confirmation regardless of authority tier."
    ),
    input_schema={
        "type": "object",
        "properties": {
