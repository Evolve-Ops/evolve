...
Phase C.1 of internal/spec-user-roster-and-roles-2026-06-07.md (Path C — evo
cross-bot admin).
...
Spec: internal/spec-user-roster-and-roles-2026-06-07.md §9 (Mutation API)
...
# Channels we surface — the same registry projection routes_bot_users.
# KNOWN_PROVIDERS uses, so tool validation rejects unknown channel names
# before they reach the daemon. Derived from ``channel_registry`` (a leaf
# module with no Flask dependency) rather than imported from the route,
# which would create the circular import evo/tools → web/routes_bot_users
# → flask → ...
KNOWN_CHANNELS: tuple[str, ...] = _channel_registry.pairing_channel_ids()
...
def _resolve_requester_identity(
    ...
    pod-admin claim exists. When it returns ``None`` the caller MUST
    refuse locally (see ``_requester_headers_or_refusal``): evo reaches
    the daemon socket-first, and the daemon's capability backstop now
    denies a header-less socket call (fail-CLOSED, audit G-N3) — so
    sending the mutation with no ``X-Requester-Identity`` header would
    just surface a confusing 403 rather than applying the change.
    """
    try:
    ...
    # Registry notify-priority order (invariant 7) rather than a local
    # channel tuple; any recorded admin identity is a valid attribution,
    # so "first one the operator is reachable on" is the right pick. The
    # reader tolerates both external_ids shapes and hands back a bare id.
    return _external_ids.resolve_notify_target(pod.get("admins"))


def _requester_headers_or_refusal(
    network_path: Path,
) -> "tuple[dict[str, str] | None, dict[str, Any] | None]":
    """Build the daemon call headers, or a local refusal envelope.

    Resolves the pod-admin identity to attribute this cross-bot roster
    mutation to. On success returns ``(headers, None)`` where ``headers``
    carries ``X-Requester-Source-Bot`` + ``X-Requester-Identity``.

    When no pod-admin external id is configured, the identity is
    UNRESOLVED. We refuse **locally** rather than sending a header-less
    request: evo calls the daemon socket-first, and the daemon's
    capability backstop denies a header-less socket call (fail-CLOSED),
    so a header-less send would only bubble up a confusing 403. This
    mirrors the plugin ``RosterTools`` resolve-or-refuse guard. Returns
    ``(None, refusal)`` in that case.
    """
    ...
    if not requester:
        return None, {
            "ok": False,
            "error": (
                "This roster change can't be attributed to an authorized "
                "admin identity: no pod admin is configured, so the change "
                "can't be verified against the target bot's capability check."
            ),
            "hint": (
                "Add a pod admin's messaging id under "
                "network.json::pod.admins.external_ids (e.g. the admin's "
                "telegram numeric user_id), or make the change from the "
                "admin UI Users page, then retry."
            ),
        }
    return {
        "X-Requester-Source-Bot": "evolve",
        "X-Requester-Identity": f"{requester[0]}:{requester[1]}",
    }, None
...
def _set_role_handler(
    *,
    ...
    headers, refusal = _requester_headers_or_refusal(network_path)
