...
ENABLE_TOOL = Tool(
    ...
    wire_description=(
        "Propose enabling an OC plugin on one bot — creates a pending "
        "EnablePluginEntry proposal the operator approves from "
        "Recommendations (or chain action.proposal.apply if authority "
        "allows). The applier edits openclaw.json and restarts the "
        "gateway itself. DO NOT walk the operator through hand-editing "
        "config or shell commands."
    ),
    input_schema={
        "type": "object",
        "properties": {
...
DISABLE_TOOL = Tool(
    ...
    wire_description=(
        "Propose disabling an OC plugin on one bot — creates a "
        "pending DisablePluginEntry proposal the operator approves "
        "from Recommendations. The applier refuses plugins in the "
        "baseline's required set, and handles the openclaw.json edit "
        "+ gateway restart itself. DO NOT generate shell snippets for "
        "the operator to run."
    ),
    input_schema={
        "type": "object",
