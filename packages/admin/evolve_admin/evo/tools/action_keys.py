...
``docs/audit-evo-tool-coverage-2026-06-02.md`` (Pattern B — "touch
    ...
    input_schema={
        ...
        "properties": {
            "bot_id": {
                "type": "string",
                "description": (
                    "Bot id — Unix username, e.g. 'team_bot_a', "
                    "'personal_bot'."
                ),
            },
            "provider": {
                "type": "string",
                "description": (
                    "Provider catalog identifier. Known set includes: "
                    "anthropic, openai, google, brave, runway, slack, "
                    "telegram, discord, google_workspace. Skill catalogs "
                    "ship more — admin server accepts any string here."
                ),
            },
            "key_value": {
                "type": "string",
                "description": (
                    "The credential to store. Sent to admin-ui via "
                    "HTTPS POST; NEVER echoed back in the response or "
                    "audit log."
                ),
            },
            "key_type": {
        ...
        "properties": {
            ...
            "provider": {
                ...
                "description": (
                    ...
                    "'anthropic', 'slack', 'telegram')."
                ),
            },
            "new_value": {
                "type": "string",
                "description": (
                    "The new credential. Sent to admin-ui via HTTP; "
                    "NEVER echoed back in the response or audit log."
                ),
            },
            "field_key": {
                "type": "string",
                "description": (
                    "REQUIRED for token_pair providers (slack/telegram) "
                    "to disambiguate which field to rotate. Slack: "
                    "'bot_token' | 'app_token' | 'user_token'. "
                    "Telegram: 'bot_token' | 'chat_id'. Optional for "
                    "api_key providers (defaults to 'api_key')."
                ),
            },
            "profile_id": {
                "type": "string",
                "description": (
                    ...
                    "this, the admin route picks the first profile "
            ...
            "storage": {
                "type": "string",
                "description": (
                    "Where the credential is read from at runtime. "
                    "'auth_profiles' (default) is the canonical "
                    "wizard-managed flow. 'openclaw_channels' writes "
                    "directly to openclaw.json#channels — used for bots "
                    "configured outside the wizard. 'dotenv' rewrites "
                    "the matching NAME=value line in workspace/.env. "
                    "Use the value the Rotate Key modal surfaces; "
                    "guessing here will silently target the wrong file."
                ),
            },
        },
