...
CONFIG_DRIFT_TOOL = Tool(
    ...
    wire_description=(
        "Return which bots have backup-baseline drift — live "
        "openclaw.json differing from the last backup commit — same "
        "data as the Security → Backups subtab. Use for 'which bots "
        "have config drift?' or 'accept all configs as baseline' "
        "(enumerate first, then action.security.accept_drift per "
        "bot). Distinct from audit findings: drift is baseline-vs-"
        "live. Pure read."
    ),
    input_schema={
        "type": "object",
