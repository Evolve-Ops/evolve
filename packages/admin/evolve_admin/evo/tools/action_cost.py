...
SET_BOT_CAP_TOOL = Tool(
    ...
    wire_description=(
        "Set a per-bot daily LLM-spend cap (USD) — real enforcement: when "
        "daily spend exceeds it, the L1 breaker auto-trips and disables the "
        "bot's heartbeat for the rest of the day. Pick a value with "
        "headroom over the bot's normal pattern. Remove with "
...
# internal/spec-cost-caps-2026-06-05.md). General-purpose setter for any of
...
SET_CAP_TOOL = Tool(
    ...
    wire_description=(
        "Set one field of a bot's cost ladder (daily_warn / weekly_warn / "
        "tier_downgrade / l1_breaker / l2_breaker / per_session_cap, plus "
        "monthly_budget and cache_retention). Validates ladder ordering and "
        "rejects inversions; value=null clears the field back to pod "
        "default. Generalizes action.cost.set_bot_cap (L1-only)."
    ),
    input_schema={
        "type": "object",
        "properties": {
...
RESET_REMEDIATION_TOOL = Tool(
    ...
    wire_description=(
        "Clear a cost-driven remediation on a bot mid-day — tier_downgrade, "
        "L1 breaker, or L2 breaker — instead of waiting for the midnight "
        "auto-revert (L2 is manual-only). L1/L2 reset also bootstraps the "
        "gateway / restores heartbeat. Use after the operator has triaged "
        "the spend cause."
    ),
    input_schema={
        "type": "object",
