        ...
        out[bot] = {
            ...
            "apps": {
                k: {
                    ...
                    **(v.get(window) or {}),
                    "outcomes": _outcomes_block(shared_dir, k, [bot], window),
                }
...
def _outcomes_block(shared_dir: Path, app_id: str, bots: list[str], window: str) -> dict:
    """Design §3.2's outcome view, alongside usage, for the same app + window.

    Best-effort: a pod without the outcome rollup (or an app_id that is not
    a Tracker application) degrades to ``measured: False`` — never a
    fabricated zero, and never a reason to drop the usage numbers this
    ...
    try:
        return get_app_outcomes(shared_dir, app_id, bots, period=window)
    except Exception:  # noqa: BLE001
        log.exception("pod_state.app_usage: outcomes read failed for %s", app_id)
        return {"measured": False}
