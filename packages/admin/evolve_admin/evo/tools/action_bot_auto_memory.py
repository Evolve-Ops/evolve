...
SET_AUTO_MEMORY_TOOL = Tool(
    ...
    wire_description=(
        "Enable or disable OpenClaw's auto-memory for one bot (flips "
        "``plugins.slots.memory``; kickstarts the gateway, ~3-5 s "
        "offline). Use when the operator says 'turn off memory on "
        "<bot>' or wants it re-enabled. DO NOT use "
        "``memorySearch.enabled`` — that only kills retrieval, not "
        "writes. Disabling stops new writes only; existing memory "
        "files stay. Idempotent."
    ),
    input_schema={
        "type": "object",
        "properties": {
