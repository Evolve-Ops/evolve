...
TOOL = Tool(
    ...
    wire_description=(
        "Read the pod's network.json, summarized: network identity "
        "(networkId, sharedDir, primary bot id), membership, per-bot "
        "role/port/user, tuning surfaces (pod_report thresholds, "
        "home_chat, alerts channel), and security_bot pointer. "
        "Secrets are redacted. Use for 'who's primary?', 'what bots "
        "exist?', or before a network-level config change."
    ),
    input_schema={
        "type": "object",
        "properties": {},
