...
(``internal/diagnosis-evo-subscription-awareness-2026-06-02.md``):
...
Safety-critical guardrail: muting a safety-critical event carries a
second confirmation requirement. Since Phase 2 safety-critical is a
**Subscription-GROUP** property, not a per-event one: the gate reads
``subscriptions_catalog.by_id(entry.subscription).is_safety_critical``
and only falls back to ``CatalogEvent.is_safety_critical`` for an
internal event with no group. Do NOT read the per-event flag as if it
were the roster — it answers a different question and disagrees with
the gate in both directions (today: 31 events gated, 8 carrying the
per-event flag, 3 of those 8 not gated).

Deliberately NOT enumerated here. An earlier version of this docstring
hardcoded a roster ("all four ``security.*`` events, ...") and went
stale three times over; worse, the roster's *shape* was wrong —
``system.*`` is the single largest gated slice, and the prose never
mentioned it. To ask the question, resolve it the way the gate does,
or just read ``is_safety_critical`` off ``pod_state.subscriptions``,
which already reports the group-resolved value.

Note the gate fires only on ``enabled=False`` (muting). A
frequency-only change never trips it — the event still fires, just at
a different cadence. The first call returns
``confirmation_required=True`` with a clear warning; the caller
must re-invoke with ``confirmed=True`` to actually flip the toggle.
Mirrors the ``confirmModal()`` guard in
``web/static/js/pages/alerts-extended.js`` (``_alMsgToggleSubscription``
/ ``_alSubsGroupToggle`` — toggle-WITH-warning, not block). Named, not
line-numbered, on purpose: the pointer this replaced read
``index.html:44972-44982``, a file that is 14,648 lines long. The
...
``internal/spec-app-derived-permissions-2026-05-24.md``) can attribute
...
TOOL = Tool(
    ...
    description=(
        ...
        "least one of enabled/frequency must be set. Muting "
        "(enabled=false) a safety-critical event without "
        "`confirmed=true` returns `confirmation_required=true` with a "
        "warning string for the operator; re-invoke with confirmed=true "
        "to apply. Which events those are is NOT inferable from the key "
        "prefix (the set spans system.*, cost.*, security.* and meta.*, "
        "and covers only some of each) — pod_state.subscriptions reports "
        "`is_safety_critical` per event; read it there rather than "
        "guessing. A frequency-only change never needs confirming. Returns "
    ...
    wire_description=(
        ...
        "the dotted catalog event key; set `enabled` and/or `frequency` "
        "— at least one. pod_state.subscriptions lists the keys and "
        "reports `is_safety_critical` — NOT inferable from the key. "
        "Muting a safety-critical event first returns "
        "confirmation_required=true; re-invoke with confirmed=true. "
        "Returns previous + applied state."
    ),
    input_schema={
        "type": "object",
        "properties": {
...
# TODO(config-intent): once internal/spec-app-derived-permissions-2026-05-24.md
