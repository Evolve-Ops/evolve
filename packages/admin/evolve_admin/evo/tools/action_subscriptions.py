        ...
        "properties": {
            "key": {
                ...
                "description": (
                    ...
                    "'security.config_drift'). The same string that "
                    "appears at the bottom of every dispatched "
                    "notification as 'subscription: <key>'. Discover "
                    "valid keys via pod_state.subscriptions."
                ),
            },
            "enabled": {
                "type": "boolean",
                "description": (
                    "Whether the operator should keep receiving this "
                    "alert. False mutes it. Leave unset to change only "
                    "frequency."
                ),
            },
            "frequency": {
                "type": "string",
                "description": (
                    "How often the alert reaches the operator. One of "
                    "'immediate' | 'daily_digest' | 'weekly_digest' | "
                    "'daily' | 'weekly' — but each event has its own "
                    "allowed set; using a disallowed value returns an "
            ...
            "confirmed": {
                "type": "boolean",
                "description": (
                    "Required to be true when muting a "
                    "safety-critical event (is_safety_critical=true "
                    "in the catalog). Mirrors the UI's 'Yes, mute it' "
                    "confirm() modal."
                ),
            },
            "intent_reason": {
                ...
                "description": (
                    ...
                    "audit log (e.g. 'operator requested via evo on "
                    "2026-06-02 — too noisy during maintenance'). "
                    "Surfaces in subscriptions-audit.jsonl. Future "
                    "config_intent system (see spec-app-derived-"
                    "permissions-2026-05-24) will use this for "
                    "attribution-aware generators."
                ),
            },
        },
        "required": ["key"],
