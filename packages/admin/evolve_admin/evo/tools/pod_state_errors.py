...
ERRORS_TOOL = Tool(
    ...
    wire_description=(
        "Recent raw error-log lines per bot (last ~10, with timestamps + "
        "freshness) from heal.py's ~5-min status snapshot. Use for 'show me "
        "the actual error message'. For curated this-matters surfaces "
        "prefer pod_state.signals.firing — that is the observation layer; "
        "this is the raw-log layer."
    ),
    input_schema={
        "type": "object",
