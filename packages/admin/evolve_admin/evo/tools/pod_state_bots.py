...
def _handler(
    ...
    internal/spec-evo-oc-native-2026-05-19.md §3.5 (operator asks evo about
...
TOOL = Tool(
    ...
    wire_description=(
        "List every bot with status (online/active/offline), role, "
        "version, port, and the same tile health chips the admin "
        "dashboard renders (scan_needed, cost_spike, "
        "security_critical, repo_puller_stale, apps_untested). Use "
        "when the operator asks about anything on a bot tile so your "
        "answer matches the screen. Filter with bot_id; also returns "
        "'primary' and 'network_id'."
    ),
    input_schema={
        "type": "object",
        "properties": {
