    ...
    input_schema={
        ...
        "properties": {
            "pins": {
                "type": "array",
                "description": (
                    "List of pins to apply. Each entry is "
                    "``{bot_id, plugin_name, version}``. A top-level "
                    "``reason`` (below) is inherited by any pin entry "
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "plugin_name": {
                            "type": "string",
                            "description": (
                                "Either the OC plugin id (the key in "
                                "``plugins.installs`` — e.g. 'brave') or "
                                "the npm package name (e.g. "
                                "'@openclaw/brave-plugin'). The tool "
                                "tries the direct-key match first, then "
                                "falls back to spec-prefix scan."
                            ),
                        },
                        "version": {
                            "type": "string",
                        ...
                        "reason": {
                            "type": "string",
                            "description": (
                                "Optional per-pin reason recorded with "
                                "the config-intent (overrides the "
                                "top-level reason for this entry)."
                            ),
                        },
                    },
            ...
            "reason": {
                "type": "string",
                "description": (
                    "Optional reason inherited by any pin that doesn't "
                    "specify its own (audit log + recorded intent)."
                ),
            },
        },
        "required": ["pins"],
