...
TOOL = Tool(
    ...
    wire_description=(
        "Diagnose the ``scan_needed`` tile chip for one bot: the "
        ".scan-status.json file's existence, mtime, days_old, parsed "
        "contents, and a chip_firing_reason explaining why the chip "
        "is on (or off). Use when the operator asks about the "
        "scan_needed chip or says scans ran but the warning persists. "
        "DO NOT propose ``find``/``ls`` shell snippets — use this; "
        "re-trigger scans via ``action.bot.rescan_apps``."
    ),
    input_schema={
        "type": "object",
        "properties": {
