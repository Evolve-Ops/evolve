...
All writes route through the admin daemon's /api/signals/<id>/*
endpoints over the unix socket, FAIL CLOSED (roadmap 7.1 C2, operator
decision 2026-08-25): when the daemon is unreachable the tools refuse
with an operator-legible error and write nothing. The signal store has
exactly one writer — the admin daemon. Local reads (validate/lookup)
still use the store API directly.
...
def _daemon_signal_transition(
    signal_id: str,
    endpoint: str,
    body: dict[str, Any],
) -> "tuple[dict[str, Any], dict[str, Any] | None]":
    """POST /api/signals/<id>/<endpoint> on the admin daemon — the ONLY
    write path (roadmap 7.1 C2, fail closed; operator decision
    2026-08-25). Returns ``(daemon_body, None)`` on HTTP 200 or
    ``({}, error_dict)`` for the caller to return as the tool result.
    There is deliberately no in-process signals-store fallback: a
    ...
    reachable, status, resp = require_daemon_call(
        "POST", f"/api/signals/{signal_id}/{endpoint}", body=body,
    )
    if not reachable:
    ...
    if status != 200:
        ...
        return {}, {
            "ok": False,
            "error": (
                f"transition failed: {detail}" if detail
...
def _snooze_handler(
    ...
    """Snooze a firing signal via the admin daemon.
    ...
    surface rather than direct UI / Telegram. The write happens
    daemon-side; this tool holds no store-write path (7.1 C2).
    """
    ...
    body, err = _daemon_signal_transition(signal_id, "snooze", {
        "until": snoozed_until,
        "actor": "evo",
    ...
    if err is not None:
        return err
    view = body.get("signal") or {}

    return {
        "ok": True,
        "signal_id": view.get("id") or signal_id,
        "to_state": "snoozed",
        ...
        "verify_via": {
            ...
            "args": {"signal_id": view.get("id") or signal_id, "state": "snoozed"},
...
def _dismiss_handler(
    ...
    """Dismiss a signal (terminal — 'don't show again') via the admin
    daemon.

    The daemon's /api/signals/<id>/dismiss endpoint owns the write AND
    the verdict-feedback side effect (it writes feedback.jsonl for each
    proposal motivated by this signal when a verdict is provided) —
    same flow as the admin UI. This tool holds no store-write path
    (7.1 C2, fail closed).
    """
    body, err = _daemon_signal_transition(signal_id, "dismiss", {
        "actor": "evo",
    ...
    # The daemon writes one feedback entry per motivated proposal (or a
    # single proposal-less entry) when the verdict is valid — mirror
    # that count for the tool's contract.
    feedback_written = 0
    if verdict in _VALID_VERDICTS:
        feedback_written = len(view.get("motivated_proposals") or [None])
    ...
    out = {
        ...
        "verify_via": {
            "tool": "pod_state.signals.history",
            "args": {"signal_id": view.get("id") or signal_id, "state": "dismissed"},
...
def _resolve_handler(
    ...
    same signature, same producer, different id. The write happens
    daemon-side; this tool holds no store-write path (7.1 C2).
    """
    body, err = _daemon_signal_transition(signal_id, "resolve", {
        "actor": "evo",
    ...
    return {
        ...
        "verify_via": {
            "tool": "pod_state.signals.history",
            "args": {"signal_id": view.get("id") or signal_id, "state": "resolved"},
