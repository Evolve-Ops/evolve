...
TOOL = Tool(
    ...
    wire_description=(
        "Most recent OC security-audit results per bot: critical/warn/"
        "info counts plus up to 10 severity-sorted findings. Filter "
        "with bot_id. Use for 'is X's audit clean?', 'which bots have "
        "critical findings?', or before recommending a fix that "
        "depends on audit state. If stale=true with count=0, no audit "
        "has run yet — say audits aren't gathered, don't report clean."
    ),
    input_schema={
        "type": "object",
        "properties": {
