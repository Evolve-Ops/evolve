...
def _handle_claim(
    ...
    *,
    ...
    if not tokens:
        ...
        recorded_primary = None
        if isinstance(primary_block, dict):
            ext = primary_block.get("external_ids") or {}
            if isinstance(ext, dict):
                recorded_primary = ext.get(channel)
        already_primary = (
            recorded_primary is not None
            and str(recorded_primary) == str(sender_external_id)
        )
