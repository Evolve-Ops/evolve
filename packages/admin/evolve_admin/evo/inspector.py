...
# Default haiku model. Overridable via env var without code changes.
_DEFAULT_HAIKU_MODEL = os.environ.get(
...
def _anthropic_haiku_fn(text: str) -> HaikuVerdict:
    """Default haiku-confirmer backed by the Anthropic Messages API.

    Returns ``"no"`` (pass-through) on any error — the inspector
    degrades gracefully when haiku is unavailable rather than blocking
    every reply.
    """
    api_key = _resolve_haiku_api_key()
    if not api_key:
        log.debug("inspector: no haiku api key available; defaulting to 'no'")
        return HaikuVerdict("no", reason="no_api_key")
    ...
    try:
        with urllib.request.urlopen(req, timeout=_HAIKU_TIMEOUT_S) as resp:
        ...
        log.exception("inspector: haiku call unexpected error")
        return HaikuVerdict("no", reason=f"unexpected: {exc}")
    return _parse_haiku_verdict(raw)


def _resolve_haiku_api_key() -> str | None:
    """Resolve the Anthropic API key for the haiku confirmer.

    Resolution order:
      1. ``ANTHROPIC_API_KEY`` env var (operator override / dev path)
      2. Primary bot's auth-profiles.json (production path — same source
         the wizard extractor uses; the admin server can read this
         because the evolve user has ACL access to .openclaw/).
    """
...
def _haiku_confirm(text: str, haiku_fn: HaikuFn | None) -> HaikuVerdict:
    ...
    fn = haiku_fn or _anthropic_haiku_fn
    try:
        verdict = fn(text)
    except Exception as exc:  # noqa: BLE001
...
_SUBSTITUTION_STUBS: dict[str, str] = {
    ...
    "no_evidence_reject": (
        ...
        "in that. You don't need to verify anything yourself."
    ),
}
