...
Spec: ``internal/spec-surface-aware-help-style-2026-05-22.md`` §7.
...
SHELL_AS_ADVICE_PATTERNS: tuple[tuple[str, str], ...] = (
    ...
    ("sudo launchctl kickstart -k gui/502/ai.evolve.team_bot_a.backup",
     ...
     "Backup now / bot_action(action=\"backup_workspace\"))"),
)
...
EVIDENCE_GROUNDED_REMEDIATION_NEGATIVES: tuple[tuple[str, str], ...] = (
    ("`pod_state(query=\"backup_status\", bot_id=atlas)` returned `{\"last_push\": "
...
PROSE_MENTION_NEGATIVES: tuple[tuple[str, str], ...] = (
    ...
    # (The original fixture named ``action.bot.update_behavior_config``,
    # a tool that never existed in the registry; B7 Phase 2 replaced it
    # with a real facade form so the fixture matches post-diet evo prose.)
    ("In Bash you'd write `sudo cp source dest` but that's not how I'd "
     "do it here — I'll use bot_action(action=\"set_auto_memory\") instead.",
...
_UI_ALTERNATIVES: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (re.compile(r"launchctl\s+kickstart\b[^\n]*?ai\.evolve\.[\w-]+\.backup\b"),
     ...
     "`bot_action(action=\"backup_workspace\", bot_id=...)`"),
    ...
    (re.compile(r"launchctl\s+kickstart"),
     "Dashboard → bot tile → **Restart Gateway** (or the registered "
     "tool `bot_action(action=\"restart\", bot_id=...)`)"),
    ...
    (re.compile(r"openclaw\s+plugins\s+install\s+--pin|plugins\.installs\.[^=]*=|--pin\s+@openclaw/"),
     "use ``bot_action(action=\"pin_plugin_version\", pins=[{bot_id, plugin_name, version}, ...])`` — "
    ...
    (re.compile(r"chmod\s+\+a.*?evolve\s+allow"),
     "Maintenance page → Repair ACLs (or the registered tool "
     "`bot_action(action=\"repair_acls\", bot_id=...)`)"),
...
#      rejected reply's topic to the specific pod_state query evo
...
# operator/model-facing suggestions in the advertised facade calling
# form (B7 Phase 2 tool diet — facades.py), not invocations — keep the
# query values in sync with facades._pod_state_members().
...
def format_followup_nudge(record: dict[str, Any]) -> str:
    ...
    read_tool = record.get("read_tool") or "the most relevant pod_state(query=...) read call"
