      ...
      "bots": {
        "team_bot_a": {
          "primary_user": {
            ...
            // {channel: [external_id, ...]} — always a LIST (M1-B2,
            // invariant 6). Legacy files may hold a bare string here;
            // evolve_admin.external_ids reads both and writes the list.
            "external_ids": {
              "slack":    ["U123ABC"],
              "telegram": ["4567890"],
              "discord":  ["987654321"]
...
def resolve_role(
    ...
    recorded = _ext_ids.ids_for(primary_block, channel)
    if not recorded or sender_external_id is None:
        return "primary"

    # Membership, not equality: one person may hold several ids on one
    # platform (invariant 6). Any of them is the primary.
    return (
        "primary"
        if _ext_ids.has_id(primary_block, channel, sender_external_id)
        else "secondary"
    )
...
def _is_pod_admin(
    ...
    if not isinstance(pod, dict):
        return False
    return _ext_ids.has_id(pod.get("admins"), channel, sender_external_id)
...
def primary_external_ids(
    ...
    """Return the normalized ``external_ids`` map for a bot's primary, or ``{}``.

    ``{channel: [external_id, ...]}`` — the list shape, whichever shape is
    on disk (M1-B2). Callers that want one id should take ``[0]`` or use
    :func:`evolve_admin.external_ids.first_id_for`.
    """
    bots = network.get("bots") or {}
    bot_cfg = bots.get(bot_id) or {}
    return _ext_ids.read_external_ids(bot_cfg.get("primary_user"))
...
def claim_admin(
    ...
    *,
    ...
    # Always emits {channel: [id, ...]} and normalizes any sibling channel
    # still in the legacy scalar shape (M1-B2).
    _ext_ids.add_external_id(admins, channel, external_id)
...
def revoke_admin(
    ...
    *,
    ...
    if not isinstance(admins, dict):
        return False
    # Tolerates both shapes; an emptied channel key is dropped rather than
    # left as [] (M1-B2). A miss still falls through to the pod_user
    # cleanup below — the operator may be clearing a half-removed admin.
    removed_any = _ext_ids.remove_external_id(admins, channel, external_id)
...
def set_bot_primary_passphrase(
    ...
    if not bot_id or not isinstance(bot_id, str):
        raise ClaimError("bot_id is required")
    _require_pod_bot(network, bot_id, action="passphrase override")
...
def _require_pod_bot(network: dict[str, Any], bot_id: str, *, action: str) -> None:
    """Raise :class:`ClaimError` unless ``bot_id`` exists in the pod.

    Existence means ``network.members`` OR the pod primary —
    ``network.primary`` is a sibling of ``members`` and never appears in
    it on current-schema pods, so a members-only check would make the
    primary permanently unclaimable (the exact bot most installs claim
    first)."""
    from ..config import is_pod_bot, pod_bot_ids

    if is_pod_bot(bot_id, network):
        return
    raise ClaimError(
        f"bot {bot_id!r} is not a pod member or the primary — {action} "
        f"aborted (pod bots = {sorted(pod_bot_ids(network))})"
    )
...
def claim_primary(
    ...
    *,
    ...
    The bot must already exist in the pod — a ``network["members"]`` entry
    or the primary (``network["primary"]`` is a sibling of ``members``, not
    an entry in it); we don't invent bots here. The ``bots[bot_id]`` block
    ...
    """
    _require_pod_bot(network, bot_id, action="claim")
    ...
    # Both shapes read, list shape written (M1-B2). Claiming a channel
    # that already holds a DIFFERENT id is still a conflict, not an
    # append: recording a second id for the same person on one channel is
    # an explicit operator surface (M1-B4), not a side effect of a claim.
    existing = _ext_ids.ids_for(primary_block, channel)
    if existing and external_id not in existing and not force:
        raise ClaimError(
            f"primary for {bot_id!r} on channel {channel!r} is already "
            f"recorded as {existing[0]!r}; pass force=True to overwrite"
        )

    if force:
        _ext_ids.set_channel_ids(primary_block, channel, [external_id])
    else:
        _ext_ids.add_external_id(primary_block, channel, external_id)
    if pod_user:
...
def reassign_primary(
    ...
    *,
    ...
    """
    _require_pod_bot(network, bot_id, action="reassign")
    ...
    current = _ext_ids.ids_for(primary_block, channel)
    if not current:
        raise ClaimError(
            f"no primary is currently recorded for {bot_id!r} on channel "
            f"{channel!r} — use claim-primary to set one"
        )
    if from_external_id not in current:
        raise ClaimError(
            f"current primary for {bot_id!r} on channel {channel!r} is "
            f"{current[0]!r}, not {from_external_id!r} — reassign aborted "
            "(check `evo show-identity` for current state)"
        )

    # Swap in place so the reassigned id keeps its position in the list —
    # position 0 is what the notify path sends to (M1-B2 / D2).
    _ext_ids.set_channel_ids(
        primary_block,
        channel,
        [to_external_id if i == from_external_id else i for i in current],
    )
    if pod_user:
        primary_block["pod_user"] = pod_user
