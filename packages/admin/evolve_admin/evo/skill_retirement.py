...
obviated_by semantics (spec §14.2):
  ...
  - Exact match — ``action.bot.set_auto_memory`` against the tool name
    (canonical registry names, which stay dotted even for tools
    advertised through the enum-dispatch facades).
...
def format_deploy_notice(candidate: RetirementCandidate) -> str:
    ...
    Format matches the spec's example shape: "local skill
    ``disable-auto-memory`` is obviated by the now-available
    ``action.bot.set_auto_memory`` tool…"
