...
def seed_channel_identity(
    ...
    *,
    ...
    # Membership through the tolerant reader (M1-B2): the pre-existing entry
    # may be a bare string or a list, and any recorded id counts as "me".
    already_owner = _ext_ids.has_id(
        (bots.get(bot_id) or {}).get("primary_user"), channel, external_id,
    )
    try:
        claim_primary(
