...
# identity: see applications.app_identity.resolve_app_id (AL-1.4b). ``pkg_id`` is the gallery catalog row's primary
# key — the sort tie-break, and the value recorded in the accepted/dismissed
# ledgers so a dismissed recommendation stays dismissed. ``c.get("pkg_id") or
# ""`` is a null-coalesce on that one column, not a resolution chain. Resolving
# a catalog row would be wrong for the #3413 reason: gallery/index.json's
# ``app_id`` column holds the app SCRIPT name, not the package key.
...
def load_candidates(
    ...
    *,
    ...
    scored = [
        # identity: see resolve_app_id — catalog primary key as the sort tie-break.
...
def classify_reply(
    ...
    if _matches_dismiss_all(text):
        return {
            "intent": "dismiss_all",
            "accepted": [],
            # identity: see resolve_app_id — the accepted/dismissed ledgers record the catalog key, so a dismissed rec stays dismissed.
