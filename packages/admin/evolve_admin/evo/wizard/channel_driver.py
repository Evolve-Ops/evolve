...
def _claim_recipient(
    *,
    ...
    if _phases.ab_group_audience(extracted):
        return False, False
    # First recorded id on this channel — the one a notify path would use
    # (M1-B2 / D2), read tolerantly across both external_ids shapes.
    admin_id = _external_ids.first_id_for(
        (network.get("pod") or {}).get("admins"), channel,
    )
    if not admin_id:
        return False, False
    try:
