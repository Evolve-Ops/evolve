...
Spec: internal/spec-add-bot-wizard-build-delta-2026-06-10.md §5 + §6: the
...
# identity: see applications.app_identity.resolve_app_id (AL-1.4b). ``pkg_id`` is the gallery package key being
# QUEUED for install — carried on QueuedApp beside ``app_id``, matched against
# the briefing-announce constant, and passed to load_gallery_package. The app
# id is what the install produces; the package key is its input.
...
def notify_outcome(
    ...
    *,
    ...
    try:
        ...
        # identity: see resolve_app_id — compares the queued package against the briefing CONSTANT; both gallery keys.
...
def queue_pack_installs(
    ...
    *,
    ...
    for app in apps:
        # identity: see resolve_app_id — the package key being queued, read off the AB_SHAPE selection.
