...
def build(
    ...
    *,
    ...
    # App-promotion offer (AL-1.7) — design §7.2. OFFER has no first-turn
    # render of its own: the pitch is delivered by the REC_PENDING flow the
    # offer arrives on (design §7.2 puts it there deliberately), and this
    # phase is entered only to close out a "never". CONFIRM echoes a rename.
    if phase_name == _phases.PHASE_APP_PROMOTE_OFFER:
        if (ctx or {}).get("variant") == "ambiguous":
            return _promote_ambiguous_block(ctx)
        return _promote_never_block(ctx)
    if phase_name == _phases.PHASE_APP_PROMOTE_CONFIRM:
        return _promote_rename_block(ctx)
...
def _gallery_recs_block(extracted: dict[str, Any], ctx: dict[str, Any]) -> str:
    ...
    cand_lines = []
    for c in candidates:
        # identity: see resolve_app_id — LLM-VISIBLE label. Candidates are
        # gallery index records whose legacy `app_id` the resolver would prefer,
        # so swapping would render `app_task_manager` where the fallback renders
        # `p-9bfa1c84`. Changing what a prompt says is not a mechanical sweep.
        # AL-1.4b, brief §8 D1.
...
def _promote_ambiguous_block(ctx: dict) -> str:
    """Ask, when a reply carries both an acceptance and a "never".

    The generic REC_PENDING clarify text does not name the ambiguity and does
    not offer the "never" as one of the choices — so a user who genuinely meant
    never would have to find the phrasing again. Naming both options is the
    whole point: this branch exists BECAUSE the answer is irreversible one way
    and cheap the other.
    """
    return (
        "Just so I don't get this wrong — I read that as a yes and a "
        "\"never ask me again\" at the same time.\n\n"
        "Say **yes** and I'll set it up. Say **never** and I'll take it off "
        "the list for good."
    )


def _promote_never_block(ctx: dict) -> str:
    """The acknowledgement for design §7.2's "never" — one of THREE sentences.

    Says what was actually done rather than a bare "ok": a user who says never
    and hears "ok" has no way to know whether they will be asked again next
    week.

    **Which of the three fires is measured, not assumed** — the summary here
    used to read "the shield is permanent and the scanner keeps it", which was
    round 4's D1 verbatim and was false. ``engine._apply_promotion_never``
    returns ``(written, durable)``; ``durable`` is
    ``coherence_actions._is_v7_arc`` on the manifest actually written, because
    ``native_write`` carries the shield across a re-mint / re-spec / legacy
    upgrade while ``scanner.py``'s legacy fallback (brief §8.3 step 7) does not.

    * durable → "the marker survives the next scan"
    * legacy  → "the marker can get cleared by a routine rescan"
    * failed  → "I couldn't get the permanent marker written"

    **And it tells the truth when the shield did NOT stick.** The engine's
    ``_apply_promotion_never`` can fail (no resolvable manifest, a failed write)
    and reports that back as ``shield_written=False``. Promising permanence we
    did not achieve is the one thing worse than the generic wrap this block
    replaced — the user would stop expecting the offer and get it anyway.
    """
    ctx = ctx or {}
    if ctx.get("shield_written") is False:
        return (
            "Understood — I won't bring that up again.\n\n"
            "One caveat, and I'd rather say it than not: I couldn't get the "
            "permanent marker written, so it's possible this comes back around. "
            "Tell me again if it does and I'll get it sorted."
        )
    if ctx.get("shield_durable") is False:
        # The legacy manifest shape. ``native_write`` carries the shield across
        # a re-mint; the legacy write path lives in ``scanner.py`` (out of scope
        # per brief §5) and does not. Telling this user the marker survives the
        # next scan would be exactly the defect round 4's D1 found — a
        # confident sentence about a layer nobody ran.
        return (
            "Understood — I won't bring that up again.\n\n"
            "I've marked it. I'll be straight with you though: this one's on an "
            "older format where the marker can get cleared by a routine rescan. "
            "If it ever comes back around, just tell me again."
        )
    return (
        "Understood — I won't bring that up again.\n\n"
        "I've marked it so it stays off the list, and the marker survives the "
        "next scan — not just for now."
    )


def _promote_rename_block(ctx: dict) -> str:
    """The CONFIRM phase's three renders (design §7.2's rename branch).

    ``rename`` echoes the proposed name back BEFORE anything is conferred,
    because on a draft with no id yet the name becomes the app's durable id and
    §7.3a's ADOPT rule then protects that id from ever changing. A mis-heard
    name is permanent in a way a mis-heard yes is not — which is the whole
    reason a rename gets a confirmation turn and a plain yes does not.

    Since ALPHA-3b the ``rename`` echo has TWO renders, because the sentence it
    used to end on stopped being universally true — see the comment on
    ``identity_mode`` below.

    ``rename_applied`` / ``rename_cancelled`` close the phase out. Both exist
    because before them the CONFIRM phase had no engine handler at all and the
    turn after the echo fell through to the primary-chain finaliser — the user
    said "yes", their session ended with a generic wrap, and the rename was
    dropped. An independent review caught that on #3734.
    """
    ctx = ctx or {}
    variant = str(ctx.get("variant") or "rename")
    name = str(ctx.get("proposed_name") or "").strip()

    if variant == "rename_applied":
        app_id = str(ctx.get("app_id") or "").strip()
        if not app_id:
            # Honest: the accept was recorded but the rename did not land.
            return (
                f"Recorded — though I couldn't get the name **{name}** saved. "
                f"I'll set it up under its current name; say the word and we can "
                f"rename it after."
            )
        return (
            f"Done — I'll set it up as **{name}**.\n\n"
            f"You'll find it in the menu once it's through."
        )

    if variant == "rename_cancelled":
        if name:
            return (
                f"No problem — I've left **{name}** aside for now. "
                f"The original offer is still open whenever you want it."
            )
        return "No problem — I've left that for now."

    if not name:
        return "What would you like me to call it?"

    # ALPHA-3b (D-I half 2). "The name becomes its permanent id" is true only
    # on the CONFERRAL branch. When the draft is an app a sibling bot already
    # has, promotion adopts that app's id and the name is a label — telling the
    # user their name is about to become a permanent identity would be asking
    # them to weigh a decision that is not theirs to make on this turn. The
    # engine resolves which branch this rename lands on and passes it here;
    # with no answer we fall through to the conferral wording, which is what
    # every draft did before ALPHA-3b.
    if str(ctx.get("identity_mode") or "").strip() == "adopted":
        source = str(ctx.get("adopted_from_bot") or "").strip()
        # ``adopted_from_bot`` can name several bots (one app, several
        # installs). Reading a comma-joined list back as "the one a, b already
        # has" is worse than not naming them, so past one it goes plural.
        if not source:
            whose = "one already set up here"
        elif "," in source:
            whose = "the one your other bots already have"
        else:
            whose = f"the one {source} already has"
        return (
            f"**{name}** it is.\n\n"
            f"Say yes and I'll set it up under that name. It's the same app as "
            f"{whose}, so it'll join that one rather than becoming a separate "
            f"app — the name is just what you'll see it called here."
        )

    return (
        f"**{name}** it is.\n\n"
        f"Say yes and I'll set it up under that name — the name becomes its "
        f"permanent id, so now is the moment to change it."
    )
...
def _format_draft_summary(draft: dict) -> str:
    ...
    if deps:
        # identity: see resolve_app_id — LLM-VISIBLE label for an
        # app_dependencies row, whose id is the depended-on gallery PACKAGE.
        # AL-1.4b, brief §8 D1.
        dep_names = [
...
# internal/reference-conversational-bot-creation-2026-05-19.md.
# ─────────────────────────────────────────────────────────────────────────────
