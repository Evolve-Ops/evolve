...
# so unit tests can run deterministically without an Anthropic key.
...
def get_intent_parser() -> IntentParserFn:
    ...
    return _anthropic_intent_parser
...
def _anthropic_intent_parser(
    ...
    *,
    ...
    api_key = _extractor._resolve_api_key()
    if not api_key:
        return IntentResult(
            action="unknown",
            confidence=0.0,
            rationale="no Anthropic API key — stage 2 disabled",
            stage="fallback",
        )
    ...
    try:
        raw = _extractor._call_anthropic(
