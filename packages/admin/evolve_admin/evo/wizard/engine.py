...
def _handle_promotion_reply(
    ...
    * **rename** → the proposed name is parked on state and the session moves to
    ...
    return TurnResult(
        system_append=_build_prompt(
            shared_dir, st, bot_id,
            extra_ctx={"variant": "rename", "proposed_name": getattr(route, "name", "")},
        ),
        wizard_session_id=user_key,
    ...
    * **anything else** → treated as backing out: nothing is applied and the
    ...
    if reply.action == "rename" and reply.name and reply.name != proposed_name:
        ...
        return TurnResult(
            system_append=_build_prompt(
                shared_dir, st, bot_id,
                extra_ctx={"variant": "rename", "proposed_name": reply.name},
            ),
            wizard_session_id=user_key,
            phase=_phases.PHASE_APP_PROMOTE_CONFIRM,
            completed=False,
...
def _apply_promotion_rename(
    ...
    target = None
    subdir = "pending"
    for candidate_subdir in ("pending", "snoozed"):
        for proposal in _astore.iter_proposals(shared_dir, subdirs=(candidate_subdir,)):
            if proposal.id == proposal_id:
                target, subdir = proposal, candidate_subdir
                break
...
# Spec: docs/spec-add-bot-wizard-build-delta-2026-06-10.md §3.2 + §9 (M2).
# ─────────────────────────────────────────────────────────────────────────────
...
# Spec: docs/spec-add-bot-wizard-build-delta-2026-06-10.md §5 + §6 + §7.
# ─────────────────────────────────────────────────────────────────────────────
