...
def set_user_pref(
    ...
    *,
    ...
    Mode is pinned to 0644 (``atomic_write``'s mkstemp default is 0600):
    the OTHER writer/reader of this file is the bot's gateway plugin
    (``ModelRouter`` reads it on every routing decision and
    ``setStandingUserTierDefault`` read-merges before its own rename), and
    an evolve-owned 0600 file is invisible to the bot user — the per-user
    pref silently never applies and the plugin's read-merge refuses to
    write. The plugin's writer pins 0644 for the mirror-image reason; the
    file holds no secrets (role names + timestamps).
    """
    if not user_key:
    ...
    if choice_n == "auto":
        ...
        _atomic_write(path, data, sort_keys=True, mode=0o644)
        return {}
