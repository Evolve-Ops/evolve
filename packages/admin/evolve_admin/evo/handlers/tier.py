...
def _user_tier_override_enabled(bot_id: str) -> bool:
    ...
    # the plugin reads.
    #
    # bot_id is the network.json LOGICAL name and is not the macOS account
    # name (team-bot-b runs on the personal-bot-user account). A literal /Users/{bot_id}
    # misses the file entirely, and the read-failure branch below then
    # returns True — silently ignoring an operator's explicit
    # ``enabled: false``. Resolve the account through bot_home().
    try:
        from ...config import bot_home
        path = bot_home(bot_id) / ".openclaw" / "evolve-tiers.json"
    except Exception:
        return True
    try:
        import json
