...
def render_tier(
    *, role: Role, bot_id: str, args: str, network: dict[str, Any],
    ...
    if choice is None:
        body = (
            f"`{args.strip().split()[0]}` isn't one I know. "
            "Pick one of: `auto`, `fast`, `standard`, `power`, `max`. "
            "Type `evo tier` for the full usage."
        )
        return speak("tier", body, role)
    ...
    * ``"fast" | "standard" | "power" | "max"`` — write the entry. The
    ...
    if choice is None:
        body = (
            f"`{args.strip().split()[0]}` isn't one I know. "
            "Pick one of: `auto`, `fast`, `standard`, `power`, `max`. "
            "Type `evo tier-default` for the full usage."
        )
