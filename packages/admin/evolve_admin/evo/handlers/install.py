...
# identity: see applications.app_identity.resolve_app_id (AL-1.4b). ``pkg_id`` is the gallery catalog key the user
# names in chat — matched by prefix against catalog rows, echoed back in the
# disambiguation list, and handed to preflight_check. The app is by construction
# NOT yet installed, so there is no manifest to resolve.
        ...
        if pkg is None:
            if candidates:
                ...
                for c in candidates[:8]:
                    # identity: see resolve_app_id — gallery catalog rows echoed back so the user can disambiguate; the app is not installed yet.
...
def _resolve_package(needle: str, shared_dir: Path):
    ...
    prefix = [p for p in all_pkgs
              ...
              # identity: see resolve_app_id — prefix match against the catalog's own key, the string the user typed.
