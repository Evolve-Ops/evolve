...
Phase E.3 (internal/spec-evo-account-separation-2026-05-25.md): evo's tools
...
EXCEPTION — the proposal/signal action tools are FAIL CLOSED (roadmap
7.1 C2): they use ``require_daemon_call`` and refuse with
``DAEMON_REQUIRED_REFUSAL`` when the daemon is unreachable. No
direct-write fallback exists for them, so the arbiter/signal stores
keep exactly one writer (the admin daemon).
"""
...
class AdminDaemonUnavailable(RuntimeError):
    ...
    during the Phase E migration window — EXCEPT the proposal/signal
    action tools, which are fail-closed (see ``require_daemon_call``).
    """


# Operator-legible refusal for the fail-closed tools (roadmap 7.1 C2,
# operator decision 2026-08-25): when the admin daemon is unreachable,
# evo's proposal/signal action tools REFUSE — no direct-write fallback
# exists, because a fallback that activates on "daemon unreachable" is a
# bypass an attacker can induce by killing the socket. The bot relays
# this string to the operator verbatim.
DAEMON_REQUIRED_REFUSAL = (
    "admin daemon unreachable — proposal/signal actions are disabled "
    "until it returns"
)


def daemon_refusal() -> "dict[str, Any]":
    """The tool-result shape every fail-closed tool returns when the
    admin daemon can't be reached. Structured (never a raise, never a
    silent no-op) so the model can relay it in prose."""
    return {
        "ok": False,
        "error": DAEMON_REQUIRED_REFUSAL,
...
class _UnixSocketHTTPConnection(http.client.HTTPConnection):
    ...
    def connect(self) -> None:
        # The socket is closed on EVERY failure path, not just the three
        # named below. Without that, a failed connect() leaves the fd
        # reachable only from the raised exception's traceback frame, so
        # it lives until the caller drops that traceback — and callers
        # that log with exc_info or stash the exception hold it longer.
        # This fires precisely when it hurts most: during an fd-exhaustion
        # storm the daemon's listener is gone, so every retry takes this
        # path (the 2026-07-21..26 EMFILE incident, #3446). A bare
        # ``except`` clause rather than ``OSError`` because settimeout()
        # and non-OSError failures would strand the fd just the same.
...
def _request_json(
    ...
    *,
    ...
    ``socket_path=None`` (the default) resolves ``DEFAULT_SOCKET_PATH``
    at CALL time, not def time — so tests can monkeypatch the module
    attribute and exercise a real connect failure against a path with
    no socket bound, instead of mocking above the transport.
    """
...
def try_daemon_call(
    ...
    *,
    ...
    """
    try:
        status, resp_body = _dispatch_daemon_call(
            method, path, body, extra_headers=extra_headers, timeout=timeout)
...
def _dispatch_daemon_call(
    ...
    *,
    ...
    """Method-preserving dispatch shared by ``try_daemon_call`` and
    ``require_daemon_call``. Raises ``AdminDaemonUnavailable`` (or
    whatever the transport raised) — the wrappers decide fallback vs
    refusal."""
...
def require_daemon_call(
    ...
    *,
    ...
    """FAIL-CLOSED daemon call for the proposal/signal action tools.

    Same return shape as ``try_daemon_call`` — (reachable, status, body)
    — but the contract on ``reachable=False`` is inverted: the caller
    must REFUSE with ``daemon_refusal()`` and perform NO write of its
    own. There is deliberately no legacy path behind this helper
    (roadmap 7.1 C2, operator decision 2026-08-25): a direct-write
    fallback would keep the store's second writer alive and stay
    inducible by killing the socket.

    (True, status, body) covers every completed HTTP round-trip,
    including 4xx/5xx — the caller maps status codes to tool errors.
    """
    try:
        status, resp_body = _dispatch_daemon_call(
            method, path, body, extra_headers=extra_headers, timeout=timeout)
        ...
        cause = getattr(exc, "__cause__", None)
        log.warning(
            "admin-daemon unavailable — REFUSING %s %s (fail closed, no "
            "fallback) via %s (errno=%s cause=%s: %s)",
            method.upper(), path, DEFAULT_SOCKET_PATH,
            getattr(cause, "errno", None),
            type(cause).__name__ if cause is not None else None,
            exc,
        )
        return False, None, None
    except Exception:  # noqa: BLE001 — a client bug must refuse, not raise into the tool
        log.warning(
            "admin-daemon call raised unexpectedly — REFUSING %s %s "
            "(fail closed, no fallback) via %s",
            method.upper(), path, DEFAULT_SOCKET_PATH, exc_info=True,
        )
        return False, None, None
