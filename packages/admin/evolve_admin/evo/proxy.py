...
def read_run_tool_calls(
    ...
    *,
    ...
    single ``run_id`` (one operator turn). Returns most-recent-first:
        ...
        [{"tool": "evo_tools.action.proposal.apply",
          "outcome": "ok" | "error",
...
def _render_summary_body(summary: Any) -> str:
    ...
    if isinstance(elided_f, int) and elided_f > 0:
        lines.append(
            ...
            "`pod_state.audit(bot_id=…)` for the full per-bot list."
        )
    ...
    if isinstance(elided_s, int) and elided_s > 0:
        lines.append(
            ...
            "`pod_state.signals.firing` for the full list."
        )
...
def send_to_evo(
    ...
    *,
    ...
    prefix_parts: list[str] = [b for b in (sc_block, pc_block) if b]
