...
def _pod_admin_ids_for(network: dict, platform: str) -> "set[str]":
    ...
    return {str(x) for x in (ext.get(platform) or []) if x}


def _primary_owner_id_for(network: dict, bot_id: str,
                          platform: str) -> "str | None":
    """Primary-owner external_id[<platform>] for the bot."""
    bots = network.get("bots") or {}
    bot = bots.get(bot_id) or {}
    primary = bot.get("primary_user") or {}
    ext = primary.get("external_ids") or {}
    val = ext.get(platform)
...
def resolve_role(overlay: dict, network: dict, bot_id: str,
    ...
    if explicit in ROLES:
        return explicit
    if stable_id == _primary_owner_id_for(network, bot_id, platform):
        return "primary_user"
    return "participant"
...
def resolve_engagement_surfaces(overlay: dict, network: dict, bot_id: str,
    ...
    if explicit:
        return explicit
    if (stable_id in _pod_admin_ids_for(network, platform)
            or stable_id == _primary_owner_id_for(network, bot_id, platform)):
        return ["group", "dm"]
