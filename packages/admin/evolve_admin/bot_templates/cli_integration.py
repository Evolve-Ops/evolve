...
def apply_embedded_app(
    ...
    *,
    ...
    #
    # identity: see resolve_app_id — nothing to sweep here (AL-1.4b).
    # ``plan.app_id`` is the BLUEPRINT's ``id``, resolved once in
    # ``app_blueprint.plan_from_blueprint``; this file never reads an
    # identity out of a manifest. The derived pkg_id is a marker value,
    # and the derivation is what makes it stable across re-renders.
    import hashlib
    if plan.app_id:
