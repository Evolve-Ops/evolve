...
def ensure_evolve_venv() -> None:
    ...
    Builds the venv with the interpreter currently running the installer (a
    fresh box has no deploy venv yet to bootstrap from) — via ``uv venv`` when
    ...
    # venv's own pip unchanged; ``--python`` pins the running interpreter (a
    # fresh box has no other). ``--clear`` makes the build idempotent on a
    ...
    if uv is not None:
        create_cmd = [
            uv, "venv", "--clear", "--seed", "--python", sys.executable, str(venv_dir)
        ]
    else:
    ...
    # console script exists. Linux-only + best-effort; no-op on macOS.
    ensure_evolve_admin_on_path()
