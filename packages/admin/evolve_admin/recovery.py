...
Spec: internal/spec-mvp-sprint-b2e-recovery.md (sprint pillar B2.e).
...
from .sudo_dest import sudo_dest_refusal  # D-2 gate: bot-owned dest of a root cp/chown
...
def _write_bot_openclaw(bot_id: str, network: dict, content: str) -> tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            # D-2: the bot owns .openclaw/ and can replace openclaw.json with a
            # link; neither `cp` nor `chown` is passed -h, so root would write
            # THROUGH it and then hand the bot the victim inode. Same shape
            # deploy.safe_write_bot_config already gates — this rollback path was
            # missed. File-shaped dest, so the absent-or-regular-file primitive
            # (sudo_dest_refusal) is the right one here, not redirect_refusal.
            if why := sudo_dest_refusal(dest):
                return False, f"refusing sudo write to {dest}: {why}"
            r = subprocess.run(
            ...
            # Re-assert between the cp and the chown: a `cp` through a link
            # leaves the LINK in place, so a plant landing inside the write
            # window is still there for the chown — and the chown is permanent.
            if why := sudo_dest_refusal(dest):
                return False, f"refusing sudo chown of {dest}: {why}"
