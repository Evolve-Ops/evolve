...
REQUIRE_CAPABILITY_BASELINE = False
"""Staged-rollout switch for the capability-widening check. **Read before flipping.**

``False`` (this release): a plugin whose declared surface has widened past
:mod:`evolve_admin.plugin_capability_baseline` verifies OK and returns a
warning. ``True``: widening is a verification failure that refuses the install.

Why it cannot ship as ``True``, and it is the same hazard
:data:`REQUIRE_TREE_DIGEST` documents, in its nastiest form. ``release
rollback``/``pin`` reverts ``packages/admin`` **without** touching
``packages/plugin``, and the puller only rebuilds the plugin when the pulled
diff touches ``packages/plugin/``. So a rolled-back pod can run a NEWER plugin
than the baseline module it just reverted to — the surface then reads as
widened, and a hard failure would refuse every plugin install on that pod with
no way to re-stamp on the way back. Rollback must stay a safe operation.

That is survivable precisely because this is the backstop, not the gate:
``tools/plugin-capability-lint`` fails the PR at CI time, where there is no
fleet to take down and a human is already reading the diff. The install-time
check exists for the pod whose deployed plugin is not the one CI saw.

**Flip criterion** — not a date: flip once ``release rollback`` also forces a
plugin rebuild (or otherwise guarantees the plugin and ``packages/admin`` move
together), at which point the newer-plugin-than-baseline state cannot arise.
Until then a widened surface on a pod is a loud warning in the deploy log,
...
def check_capability_baseline(manifest_data: dict) -> tuple[bool, str]:
    """Compare the manifest's declared capability surface to the reviewed baseline.

    Returns ``(True, "")`` when the surface matches or has narrowed,
    ``(False, <reason>)`` when it has widened. Answers the question the digests
    structurally cannot: ``treeDigest`` covers the manifest's content, so it
    catches a capability added by *tampering with the install tree* — but a
    capability added in our own source and built normally produces a perfectly
    valid stamp. See :mod:`evolve_admin.plugin_capability_surface`.

    The caller decides whether a widened surface refuses the install; see
    :data:`REQUIRE_CAPABILITY_BASELINE`.
    """
    from .plugin_capability_baseline import BASELINE
    from .plugin_capability_surface import declared_surface, diff_widening, format_diff

    widened = diff_widening(BASELINE, declared_surface(manifest_data))
    if not widened:
        return True, ""
    return False, (
        f"declared capability surface widened past the reviewed baseline "
        f"({format_diff(widened)}). Deploy passes --accept-capabilities, so "
        f"each of these is granted on every bot with no prompt. If the "
        f"widening is intended, re-freeze the baseline with "
        f"`tools/plugin-capability-lint --update-baseline` and land it; if it "
        f"is not, the deployed plugin is not the one CI reviewed — investigate "
        f"what built it"
    )
...
def verify_plugin_signature(plugin_dir: Path) -> tuple[bool, str]:
    ...
    **Callers must treat a truthy second element as output even when ok is
    ...
    - the declared capability surface widened past the reviewed baseline,
      once :data:`REQUIRE_CAPABILITY_BASELINE` is ``True`` (it warns until
      then — read that constant before flipping it)

    Two independent warnings can be live at once (legacy stamp, widened
    surface); they are joined with ``" | "`` rather than one shadowing the
    other.
    ...
    # Deliberately BEFORE the digest work and outside the legacy early-return
    # below: this check reads only the manifest, and a legacy-stamped pod is
    # exactly one that has not rebuilt in a while — the case where an
    # unreviewed surface is most likely to be sitting there unnoticed.
    cap_ok, cap_msg = check_capability_baseline(data)
    if not cap_ok and REQUIRE_CAPABILITY_BASELINE:
        return False, cap_msg
    cap_warning = "" if cap_ok else cap_msg
    ...
    if "treeDigest" in trust:
        ...
        if REQUIRE_TREE_DIGEST:
            return False, legacy
        return True, " | ".join(x for x in (legacy, cap_warning) if x)
