...
from .deploy import (
    ...
    ensure_pod_perms, _openclaw_bin,
...
from .lifecycle.cli_output import (  # presentation for detach/retire/delete
    collect_credentials_quietly,
    print_credential_followup,
    print_lifecycle_preview,
    resolve_bot_home_quietly,
)
...
def diagnose(json_output: bool, category: str | None) -> None:
    ...
    See internal/spec-etr-diagnose-tool-2026-04-26.md.
    """
...
def setup(ctx: click.Context, network_path: Path | None, fresh: bool, non_interactive: bool, bots_manifest: Path | None, platform_opt: str | None) -> None:
    ...
    Without --fresh, runs the config wizard for a pod that ALREADY has an
    Evolve service layer; with no service layer it refuses and points at --fresh.
...
def setup_evolve_user(uid: int | None) -> None:
    ...
    # ── Step 12: REMOVED 2026-08-18 ───────────────────────────────────────────
    # Generated {shared_dir}/keystore/evolve-signing.key, the symmetric HMAC
    # secret for proposal-pipeline signing. The whole sign/verify surface is
    # retired (internal/design-proposal-signing-key-2026-08-18.md) — the sole
    # verifier was structurally unreachable, and being bot-readable was what
    # kept the key world-open.
    #
    # This step also hardcoded /Users/Shared/evolve with no platform_profile,
    # so the Linux VPS pod never got a key or an enforcement marker at all and
    # every verify there bootstrapped open. That latent /Users-only-path bug is
    # closed by removal rather than fixed.
    #
    # Existing pods keep an inert evolve-signing.key and
    # .proposal-signing-enabled marker on disk. Deliberately NOT deleted from a
    # deploy path — removing live keystore state as a side effect of a deploy is
    # a privileged side effect operators should perform explicitly. See the PR
    # body for the operator cleanup step.
...
def _full_deploy(bot_id: str, network_path: Path, network: dict, dry_run: bool) -> SmokeAuditResult:
    ...
    if port:
        ...
        gateway_ok, gateway_detail = install_bot_gateway_plist(
            bot_id, port, user=bot_user, network=network,
        )
        if gateway_ok:
            ok()
        else:
...
def install_infra_jobs(ctx: click.Context, dry_run: bool, with_opik: bool) -> None:
    ...
    # Platform-keyed evolve home; a macOS-root literal broke this on Linux.
    evolve_dir = Path(get_profile().user_home_root) / "evolve"
...
# internal/spec-upstream-issue-watcher-2026-05-22.md and
...
def features_group() -> None:
    ...
    profile default. See ``internal/spec-upstream-issue-watcher-2026-05-22.md``.
    """
    pass
...
# (internal/spec-pwa-phase0-https-2026-05-18.md). Standalone CLI surfaces
...
def enable_https_cmd(ctx: click.Context) -> None:
    ...
    See internal/spec-pwa-phase0-https-2026-05-18.md.
    """
...
def disable_https_cmd(ctx: click.Context) -> None:
    ...
    See internal/spec-pwa-phase0-https-2026-05-18.md §3.6.
    """
    from . import https_setup
    network_path: Path = ctx.obj["network_path"]
    try:
...
def slack_doctor(
    ...
    internal/spec-slack-policy-2026-05-13.md (name-keyed channels under
...
def doctor_pass(
    ...
    targets = [t for t in (list(bots_cfg.keys()) if all_bots else [bot_id]) if t]
    if not targets:
        console.print("[yellow]No bots to scan.[/]")
        return
    ...
    for target in targets:
        ...
        try:
            # Binary + cwd resolve per platform, as everywhere else deploy runs a
            # bot-user subprocess — both literals this had are absent on Linux.
            r = subprocess.run(
                ["sudo", "-H", "-u", user, _openclaw_bin(), "doctor", "--fix"],
                stdin=subprocess.DEVNULL,
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                text=True,
                timeout=timeout,
                cwd=get_profile().scratch_dir,  # `sudo -H` still sets HOME
            )
...
def add_bot_cmd(
    ...
    # EVO-SEP-S5: refuse reserved `evolve`/`evo` (or a reserved --user alias) before any planning/dry-run output — they are service/assistant accounts, not member bots. deploy.add_bot backstops the real path; this also gives a clean refusal on `--dry-run`. internal/spec-evo-account-separation-2026-05-25.md
...
# Spec: internal/spec-add-bot-wizard-2026-05-28.md §5. Used both as a
...
def reconcile_catalog_cmd(
    ...
    try:  # ValueError = malformed on-disk tiers doc; see #3566 audit C-3
        result = reconcile_catalog(
            current_catalog, tiers, credentialed_providers=credentialed,
            add_recommended_for_credentialed=add_recommended, resolved_role_models=resolved_role_models)
    except ValueError as exc:
        raise SystemExit(f"✗ {bot_id}'s tier definitions are malformed, so the catalog cannot be reconciled: {exc}")
...
def ensure_pod_perms_cmd(ctx: click.Context, check_only: bool, bot_id: str | None) -> None:
    ...
    Codifies the perm layers we hand-applied during the 2026-04-25
    apply.py-zombie incident:
      ...
      2. /opt/homebrew/Cellar/python@3.14/ is readable + executable for all users
      3. /Users/Shared/evolve/proposals/ + standard subdirs are mode 1777
...
def provision_evo_account_cmd(dry_run: bool) -> None:
    ...
    Spec: internal/spec-evo-account-separation-2026-05-25.md §"Phase E.2.a".
...
def retire_bot_cmd(
    ...
    print_lifecycle_preview(console, bot_id, network_path, action="archive")

    # Resolve the home BEFORE the retire runs: retirement strips the bot from
    # network.json, after which get_bot_user falls back to the bot id — which
    # is wrong for any bot running under a different account name (team_bot_b runs
    # on `shared_account_b`), and would point the credential follow-up at a home that
    # does not exist.
    _creds = collect_credentials_quietly(
        resolve_bot_home_quietly(bot_id, network_path)
    )
    ...
    if result.success:
        if dry_run:
            ...
            print_credential_followup(console, _creds)
...
def remove_evolve_cmd(
    ...
    print_lifecycle_preview(console, bot_id, network_path, action="detach")
...
def delete_bot_cmd(
    ...
    in ``{shared_dir}/archived-bots/`` for forensics but the bot cannot
    ...
    # Resolve the home BEFORE deleting: afterwards the bot is gone from
    # network.json AND the account is gone, so nothing can find it. Also fixes
    # the confirmation text, which used to name `/Users/<bot_id>/` — a
    # hardcoded macOS root, and the wrong account for any bot running under a
    # different name (team_bot_b runs on `shared_account_b`).
    _cred_home = resolve_bot_home_quietly(bot_id, network_path)

    if not dry_run and not skip_confirm:
        console.print(
            f"[bold red]IRREVERSIBLE:[/] this will delete bot "
            f"[bold]{bot_id}[/], stop its daemons, remove it from the "
            f"network, AND delete the host user account + "
            f"[bold]{_cred_home or bot_id}/[/]. An archive remains in "
            f"{shared_dir}/archived-bots/ but the bot cannot be revived."
        )
        typed = click.prompt(
    ...
    print_lifecycle_preview(console, bot_id, network_path, action="delete")

    # Enumerate credentials while the home still EXISTS. Deleting the account
    # removes the local copies but revokes nothing — the channel token stays
    # valid at the platform — so this is the last moment the list can be
    # produced at all.
    _creds = collect_credentials_quietly(_cred_home)
    ...
    if result.success:
        if dry_run:
            ...
            print_credential_followup(console, _creds, deleted=True)
            try:
                from .mcp_service import reload_registry as _mcp_reload, status as _mcp_status
                if _mcp_status().get("running"):
                    ok, msg = _mcp_reload()
...
def repo_pull(repo: str, remote: str, branch: str, quiet: bool,
    ...
    Manual invocation: `sudo -H -u evolve evolve-admin repo-pull` (bare
    `sudo` re-execs as evolve — root has no deploy key).
    ...
    # caller (the daemon runs `repo-pull --quiet` with no --repo) didn't pin
    # one: /Users/Shared/evolve-repo on macOS, /var/lib/evolve/repo on Linux
    # — without it the daemon pulled the macOS literal on a Linux pod.
    if not repo:
        repo = str(_rp.DEFAULT_REPO)

    _rp.enforce_evolve_invocation(allow_root=setup_key)  # root has no deploy key: re-exec as evolve, never file an incident
...
# internal/spec-state-store-and-deploy-resilience-2026-06-10.md Part 2.
from .release_cli import release_group as _release_group  # noqa: E402
main.add_command(_release_group)

# `evolve-admin board token|revoke <bot_id>` (D-MB2). Same reason as release:
# cli.py is line-count capped, so the group lives in board_cli.py.
from .board_cli import board_group as _board_group  # noqa: E402
main.add_command(_board_group)

# `evolve-admin cost reconcile --console-csv <file>` — estimate vs the
# provider bill. Same reason again: the group lives in cost_cli.py.
from .cost_cli import cost_group as _cost_group  # noqa: E402
main.add_command(_cost_group)

# `evolve-admin install-pm-probe-sudoers` — the PM read-only pod probe's privileged half; same reason as release/board/cost, the command's body lives in pm_probe_sudoers.py because cli.py is line-count capped. Brief: internal/dispatch/done/pm-mini-probe-readonly-mcp.md.
from .pm_probe_sudoers import install_pm_probe_sudoers_cmd as _pm_probe_cmd; main.add_command(_pm_probe_cmd)  # noqa: E402,E702 — folded onto one line: cli.py is at its no-growth cap
...
@main.command()
@click.option("--port", default=DEFAULT_ADMIN_PORT, show_default=True,
...
def serve(ctx: click.Context, port: int, host: str, open_browser: bool) -> None:
    ...
    # internal/spec-evo-account-separation-2026-05-25.md §3.
    ...
    # Board's tailnet-only second listener (design D-MB2 / F8 review). Off
    # unless network.json sets board.tailnetListener.enabled; binds the
    # tailnet address only and routes /board/* + /api/board/* only. The
    # loopback binding below is untouched either way.
    from .web.board_listener import start_board_listener
    if start_board_listener(app, network_path, default_port=port):
        console.print("[dim]  board tailnet listener active (board paths only)[/]")
...
def gen_evo_glossary_cmd(
    ...
    internal/spec-evo-oc-native-2026-05-19.md.
...
# ── Circuit breakers: manual trip/reset + §5.2 arming toggle ─────────────────
#
# The `evolve-admin breaker` command group lives in breakers_cli.py (cli.py is
# line-count capped); register it onto the top-level group here. Spec:
# internal/spec-circuit-breakers-2026-05-21.md.
from .breakers_cli import breaker_group as _breaker_group  # noqa: E402
main.add_command(_breaker_group)
...
@main.group()
def models() -> None:
    """Manage model roles (fast/standard/power/max).
...
def models_show(ctx: click.Context, role: str) -> None:
    """Show full details for a role ID (fast/standard/power/max) or legacy tier key."""
    tier = role
    network_path: Path = ctx.obj["network_path"]
    network = load_network(network_path)
...
def models_set(ctx: click.Context, role: str, rung: str, assume_yes: bool) -> None:
    ...
    ROLE is a role ID: fast, standard, power, max.
    ...
    _VALID_ROLES = {"fast", "standard", "power", "max"}
...
def _apply_tier_override(ctx: click.Context, bot: str, apply, *args):
    """Run one ``user_tier_override`` writer, or exit 1 saying why."""
    from .user_tier_override import UserTierOverrideWriteError

    network_path: Path = ctx.obj["network_path"]
    try:
        return apply(load_network(network_path), network_path, bot, *args)
    except UserTierOverrideWriteError as e:
        console.print(f"[red]✗[/red] {bot}: userTierOverride write failed: {e}")
        sys.exit(1)
...
def models_cap(ctx: click.Context, bot: str, value: int) -> None:
    ...
    Writes ``roleCaps.power.maxPerDayPerBot`` — the key the gateway enforces
    the cap from — into ``<bot home>/.openclaw/evolve-tiers.json``, read-merged
    over any caps the bot already carries, and keeps the legacy
    ``userTierOverride.dailyCap`` in step (canonical file + the
    ``{sharedDir}/{bot}/tiers.json`` mirror) for the admin chat gate, which
    still reads that one. Range 0-100.

    Set 0 to stop Power turns on this bot: the cap is exhausted from the first
    turn, so the router degrades Power → standard. That is a DIFFERENT gate
    from ``user-tier-control off``, which disables the feature outright.

    Caps set before this key was written are stranded — ``evolve-admin models
    migrate-tier-overrides`` reports them and can move them.

    Spec: internal/spec-user-tier-control-2026-05-26.md §"Per-bot opt-out +
    adjustable daily cap"; why two keys and two files: ``power_cap`` /
    ``user_tier_override``.

    \b
    Examples:
      evolve-admin models cap evolve 20    # raise to 20/day
      evolve-admin models cap forge 0      # stop Power turns on forge
      evolve-admin models cap evolve 10    # reset to default
    """
    if not (0 <= value <= 100):
        console.print(f"[red]value must be 0-100, got {value}[/red]")
        raise click.Abort()
    from .user_tier_override import apply_power_daily_cap

    effective = _apply_tier_override(ctx, bot, apply_power_daily_cap, value)
    console.print(
        f"[green]✓[/green] {bot}: Power daily cap set to "
        f"[bold]{value}[/bold]/day"
        + (" (Power turns stop; standard still answers)" if value == 0 else "")
    )
    # What the GATEWAY will enforce, not what was typed — echoing the
    # operator's own number back is what this command did while routing used
    # a different one. A divergence is that bug returning: say it, loudly.
    if effective != value:
        console.print(
            f"[yellow]![/yellow] {bot}: routing will still use "
            f"[bold]{effective}[/bold]/day — something in this bot's config "
            f"shadows the cap just written; check roleCaps in its "
            f"evolve-tiers.json and in network.json models."
        )
...
def models_user_tier_control(ctx: click.Context, bot: str, state: str) -> None:
    """Turn the per-bot model-tier feature on or off.

    Writes ``<bot home>/.openclaw/evolve-tiers.json::userTierOverride.enabled``
    — the file the gateway plugin routes on — and mirrors it to the legacy
    ``{sharedDir}/{bot}/tiers.json``.

    ``off`` does TWO things, not one: it hides the model-tier chip on the
    admin UI chat composer, AND it disables the bot's own self-escalation to
    Power — the plugin's ``canEscalateToRole`` refuses on
    ``feature_disabled``, so ``session_set_tier`` is refused too. It is not a
    cosmetic switch; it changes routing behaviour. (The backend still accepts
    whatever the frontend sends — defense in depth for a cached client that
    has not re-rendered.)
    ...
    """
    from .user_tier_override import apply_user_tier_override

    _apply_tier_override(
        ctx, bot, apply_user_tier_override, {"enabled": state == "on"},
    )
    console.print(
        f"[green]✓[/green] {bot}: model-tier feature "
        + (
            "[bold]enabled[/bold] — chip shown, bot may self-escalate to Power"
            if state == "on" else
            "[bold]disabled[/bold] — chip hidden AND the bot can no longer "
            "self-escalate to Power"
        )
    )


# ── models migrate-tier-overrides ─────────────────────────────────────────────
# Body lives beside the migration rules it enforces, not here (same shape as
# migrate-model-roles). See tier_override_migration.
from . import tier_override_migration as _tier_override_migration  # noqa: E402

_tier_override_migration.register_cli(models)
...
def models_usage(ctx: click.Context, bot: str | None) -> None:
    ...
    from rich.table import Table
    # Usage records accept both legacy tier keys (tier1-tier3) and new
    # role IDs (fast/standard/power/max). Sum both forms so the
    # table works during and after the migration. (The judge/tier0 column
    # died with the judge role — nothing routes turns to it.)
    t = Table(show_header=True, header_style="bold blue")
    t.add_column("Bot")
    t.add_column("max")
    t.add_column("power")
...
# internal/skills-deep-audit-2026-05-30.md F1.
...
def migrate_jobs(dry_run: bool) -> None:
    ...
    apply.py used to be excluded from this migration because it ran as each
    bot's own user. That daemon was retired 2026-08-18
    (internal/design-proposal-signing-key-2026-08-18.md); `evolve-admin deploy
    <bot>` now boots out any surviving unit instead of installing one.
...
from .applications.restore_cli import register_cli as _register_restore_manifest; _register_restore_manifest(application); from .applications.app_cron_cli import register_cli as _register_repair_app_crons; _register_repair_app_crons(application); from .applications.id_migration_cli import register_cli as _register_migrate_ids; _register_migrate_ids(application); from .applications.spec_migration_cli import register_cli as _register_migrate_specs; _register_migrate_specs(application); from .applications.draft_identity_cli import register_cli as _register_draft_identity; _register_draft_identity(application); from .purge_ingested_cli import register_cli as _register_purge_ingested; _register_purge_ingested(main); from .pod_baseline_cli import register_cli as _register_pod_baseline; _register_pod_baseline(main); from .model_swap_cli import register_cli as _register_model_swaps; _register_model_swaps(models); from .redeploy_sweep_cli import register_cli as _register_redeploy_sweep; _register_redeploy_sweep(main); from .applications.app_snapshot_cli import register_cli as _register_app_snapshot; _register_app_snapshot(application); from .applications.gallery_pack_install_cli import register_cli as _register_gallery_pack_install; _register_gallery_pack_install(application)  # noqa: E402,E702 — bodies in helpers keep cli.py under its no-growth cap (restore-manifest + repair-app-crons + META:footprint _ingested-purge + META:pod-plane pod-baseline + AL-1.4a migrate-ids + AL-1.5a migrate-specs + AL-1.6a draft-identity + model-swap ledger/rollback + META:deploy repo-redeploy-sweep registrations folded onto one line)
...
def evo_claim_primary(
    ...
    # Tolerant read (M1-B2) — indexing the raw dict recorded the literal
    # "['12345']" as the audit "from" value on a list-shaped entry.
    prior = _external_ids.first_id_for(
    ...
    other_channels = [
        c for c in _external_ids.read_external_ids(primary_block)
        if c != channel_lc
    ]
    if other_channels:
        console.print(
...
def evo_show_identity(ctx: click.Context, bot_id: str | None) -> None:
    ...
    network_path: Path = ctx.obj["network_path"]
    network = load_network(network_path)
    pod_bots = pod_bot_ids(network)  # primary first, then members
    bots = network.get("bots") or {}

    if bot_id is not None:
        if bot_id not in pod_bots:
    ...
    if not pod_bots:
        ...
        return
    for b in pod_bots:
        _print_identity_block(b, (bots.get(b) or {}).get("primary_user") or {})


def _print_identity_block(bot_id: str, block: dict) -> None:
    external_ids = _external_ids.read_external_ids(block)
    ...
    for ch, ext_ids in sorted(external_ids.items()):
        # Several ids per channel is legal (invariant 6) — render them all.
        console.print(f"  {ch}: {', '.join(ext_ids)}")
    if pod_user:
        console.print(f"  pod_user: {pod_user}")
...
def evo_show_admins(ctx: click.Context) -> None:
    ...
    if not isinstance(admins, dict):
        admins = {}
    by_channel = _external_ids.read_external_ids(admins)
...
def openclaw_migrate_overrides_cmd(
    ...
    See internal/spec-openclaw-json-derived-artifact-2026-05-24.md §9.
    """
...
def migrate_evo_account_cutover(ctx: click.Context, confirm: bool) -> None:
    ...
    Spec: internal/spec-evo-account-separation-2026-05-25.md §"Phase E.2.b".
...
# Spec: internal/spec-primary-bot-interface-2026-05-14.md §4.
...
# (internal/spec-config-intent-system-2026-05-21.md). Phase 2 ships the read +
...
def export_app_cmd(
    ...
    internal/spec-scanned-export-2026-06-02.md) against the named
    ...
    already_packaged = (scanned_manifest.get("pkg_id") or "").strip()  # identity: see resolve_app_id — PRESENCE probe (is this already forge/gallery-packaged?), not an id read; a canonical app_id is present on every manifest and would make this always true
    if already_packaged or (scanned_manifest.get("build_spec") or "").strip():
        console.print(
...
def promote_app_cmd(
    ...
    Spec: internal/spec-files-pack-hybrid-2026-06-03.md §8.
    Runbook (manual equivalent): internal/runbook-snapshot-install-to-
    ...
    try:
        ...
        for entry in index_entries:
            if isinstance(entry, dict) and entry.get("pkg_id") == pkg_id:  # identity: see resolve_app_id — gallery/index.json is keyed by pkg_id; the index row is not a manifest
