...
def generate_plist(host: str = "127.0.0.1", port: int = 5050) -> str:
    ...
    spec = JobSpec(
        ...
        run_at_load=True,
        keep_alive=True,
        # launchd's 256 soft NOFILE default is too low for the threaded
        # admin server — the 2026-07-28 incident was an EMFILE storm that
        # permanently killed the unix-socket accept path. Mirror the
        # deploy-side _admin_ui_jobspec cap.
