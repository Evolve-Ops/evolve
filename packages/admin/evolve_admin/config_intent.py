...
Spec: internal/spec-config-intent-system-2026-05-21.md.
...
Timestamp semantics — ``set_at`` describes the VALUE, not the row
--------------------------------------------------------------------
``intents[].set_at`` is **the date the value sitting beside it was
recorded**, not the date the row was first created. A re-record that
moves ``value`` advances it; a re-record that only rewrites ``reason``
(or ``edit_intent_reason`` / ``confirm_queued_intent``, which never
touch ``value``) leaves it alone.

The two sibling stores carrying the same field name are close but not
identical: ``analyzer.autonomy.store`` (``current.set_at = at`` on every
``set_posture``) and ``config_sandbox.overrides`` (restamped on every
``write_override``) are **write-time** — they restamp even when the
value is unchanged. This store is **value-change**-time. The stricter
rule is deliberate: the two readers below print ``set_at`` directly
beside the value, so the date has to describe the value rather than the
last time someone touched the record.

It did not always. Until 2026-08-23 the update branch rewrote
``value``/``reason``/``set_by`` but never ``set_at``, so a row routinely
showed a current ``value`` beside a creation date that predated it by
months, and readers that print "recorded <set_at>" next to the value
printed a date that was not the value's. ``_load_sidecar`` now heals
such rows on read (see ``_heal_set_at``) — no migration is required and
none should be run against a live pod.

The row's *creation* date is still recorded, in ``audit_history[0]``
(the ``set`` event). Note that ``audit_history`` is not a complete
history of the underlying config field: callers only reach
``set_intent`` for values they choose to record, and
``deploy._record_exec_policy_intent`` deliberately skips recording when
the chosen value equals the deploy baseline. A field can therefore move
without any entry appearing here.

Known residual — ``set_at`` is value-scoped, ``set_by`` is not.
``set_by`` / ``set_by_detail`` / ``reason`` still describe the LAST
write to the record, and both operator-facing readers concatenate the
two ("recorded <set_at> by <set_by>"). The RSI appliers
(``arbiter.appliers.permissions`` / ``.agent_defaults``) call
``set_intent`` unconditionally per field, so a same-value re-record can
render a correct date beside a ``set_by`` from a later, different write.
Narrowed but not closed here: fixing it means either scoping ``set_by``
the same way or splitting the rendered string, and both are reader
...
def _load_sidecar(bot_id: str, shared_dir: Path | None) -> dict[str, Any]:
    ...
    Heals legacy ``set_at`` values in memory before returning (see
    ``_heal_set_at``), so every reader — including the pass-through
    ``GET /api/intents`` routes — sees the value's own date without a
    migration having to run first. The healed value persists on the next
    write; this function never writes.
    """
...
# ── set_at healing ───────────────────────────────────────────────────────────
#
# ``set_at`` tracks the current ``value`` (see module docstring). Rows written
# before 2026-08-23 carry the row's creation date instead, because the update
# branch never rewrote the field. Rather than migrate the live sidecars, we
# recompute the correct timestamp from ``audit_history`` on every read and let
# the next write persist it.


def _parse_iso(at: Any) -> datetime | None:
    """Tolerant ISO parse — accepts a trailing ``Z`` or an explicit offset.

    Everything this module writes comes from ``evolve_util.now_iso`` (the
    ``...Z`` form), but sidecars can be hand-edited, so parse rather than
    compare strings. Unparseable input is ``None``, which every caller
    treats as "no opinion" rather than as an error.

    A naive timestamp is read as UTC. That keeps every return value
    offset-aware, so callers can compare two of them without risking the
    ``TypeError`` Python raises on naive-vs-aware — which would escape
    ``get_intent``/``list_intents``' ``(RuntimeError, OSError)`` fail-open
    and blank the Intentional Deviations page over one hand-edited row.
    """
...
def _audit_entry_changed_value(entry: dict[str, Any]) -> bool:
    """Whether one ``audit_history`` entry (re)established the row's ``value``.

    ``set`` always did — it created the row. ``updated`` did only when the
    recorded from/to differ; ``set_intent`` also lands there for a
    reason-only re-record. Every other event (``reason_edited``,
    ``confirmed_queued``, ``audited``, ``stale_flagged``, ``revoked``)
    leaves ``value`` alone by construction, so none of them may move
    ``set_at``. Of those, only ``reason_edited`` and ``confirmed_queued``
    have a producer — ``audited`` and ``stale_flagged`` are declared in
    spec §2.1 and written by nothing in this repo; they are listed so the
    classifier's answer for them is deliberate rather than incidental.
    """
    ...
    if event != "updated":
        return False
    if "from_value" not in entry or "to_value" not in entry:
        # Cannot prove the value moved, so do not move its date. Spec §8.5
        # describes the Edit-reason path as appending an ``updated`` entry
        # carrying only ``from_reason``/``to_reason`` — the implementation
        # writes ``reason_edited`` instead, but a sidecar built to the
        # spec'd shape exists in principle, and reading one of those as a
        # value change would restamp ``set_at`` to a date months after the
        # value it describes. That is the bug this module is fixing, so the
        # ambiguous case resolves toward leaving the stored stamp alone.
        return False
    return entry["from_value"] != entry["to_value"]


def _derive_value_set_at(intent: dict[str, Any]) -> str | None:
    """Latest ``audit_history[].at`` that changed ``value``, or None."""
    history = intent.get("audit_history")
    if not isinstance(history, list):
        return None
    best: str | None = None
    best_dt: datetime | None = None
    for entry in history:
        if not isinstance(entry, dict) or not _audit_entry_changed_value(entry):
            continue
        at = entry.get("at")
        dt = _parse_iso(at)
        if dt is None:
        ...
        if best_dt is None or dt > best_dt:
            best_dt, best = dt, str(at)
    return best


def _heal_set_at(intent: dict[str, Any]) -> bool:
    """Advance a legacy row's ``set_at`` to the date its ``value`` was recorded.

    In-place; returns True when something changed. Only ever moves the
    timestamp FORWARD: a row whose history was truncated or hand-trimmed
    keeps whatever it already had rather than being back-dated to a
    surviving early entry. A row with no parseable stored ``set_at`` takes
    the derived one outright.
    """
    derived = _derive_value_set_at(intent)
    derived_dt = _parse_iso(derived)
    if derived_dt is None:
        return False
    stored_dt = _parse_iso(intent.get("set_at"))
    if stored_dt is not None and derived_dt <= stored_dt:
        return False
    intent["set_at"] = derived
    return True


def _heal_sidecar_timestamps(data: dict[str, Any]) -> None:
    """Apply ``_heal_set_at`` across both intent lists of a loaded sidecar.

    ``isinstance(seq, list)`` is load-bearing, not decoration:
    ``intents_archive`` was never iterated on any read path before this
    function existed, so a scalar there used to be harmless. Iterating one
    raises ``TypeError``, which is NOT in the ``(RuntimeError, OSError)``
    fail-open of ``get_intent`` / ``list_intents`` / ``list_all_intents``,
    and ``deploy._record_exec_policy_intent`` calls ``get_intent`` with no
    guard at all — so one corrupt archive would abort a deploy.
    """
    for key in ("intents", "intents_archive"):
        seq = data.get(key)
...
def set_intent(
    ...
    *,
    ...
    ``set_at`` moves with the value: a re-record that changes ``value``
    restamps it, a re-record that only changes ``reason`` (or re-runs
    inference on an unchanged value) does not. The row's creation date
    stays available as ``audit_history[0]["at"]``. See the module
    docstring for why the two are different dates and which one every
    current reader wants.
    ...
    if idx < 0:
        ...
        # ``set_at`` belongs to the VALUE, so it advances when the value
        # moves and holds still when a caller only rewrites the reason or
        # re-runs inference on an unchanged value. (``or not
        # intent.get("set_at")`` backstops a row whose stored timestamp is
        # missing and whose history was too thin for ``_heal_set_at``.)
        if prior_value != value or not intent.get("set_at"):
            update["set_at"] = now
...
def record_migration_intent(
    ...
    *,
    ...
    Spec: internal/spec-config-intent-system-2026-05-21.md §3 (Phase 1
...
def edit_intent_reason(
    ...
    *,
    ...
    Spec: internal/spec-config-intent-system-2026-05-21.md §8.5 — the
...
def confirm_queued_intent(
    ...
    *,
    ...
    Spec: Phase 4.1 of internal/spec-config-intent-system-2026-05-21.md
