...
def _print_qr(url: str) -> bool:
    ...
    try:
        # The console's own answer about colour picks the renderer: with
        # colour each module is a painted cell, which no font can break;
        # without it we are back to glyphs, which one already did. Both
        # halves are needed: under NO_COLOR rich leaves ``color_system`` set
        # and strips the SGR at print time instead, so painting there would
        # emit 45 rows of bare spaces — an invisible code, and no warning.
    ...
    console.print()
    if code.font_dependent:
        # The glyph renderers paint inside the font's ink box, which is
        # shorter than the terminal cell — that is what left a light stripe
        # across every row and failed the first phone test. Say so, rather
        # than letting the operator conclude the code itself is broken.
        console.print("[yellow]Drawn without colour[/] — if this does not "
                      "scan, re-run it in a colour terminal.")
    return True
