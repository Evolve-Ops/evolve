...
#: A coarse, timezone-insensitive relative-time bucket for
#: :func:`next_touch_line` (minutes / hours / days / months) — good enough
#: for "in 2h" / "overdue 3d"; not the viewer's locale wall-clock text
#: (that is D-TM's still-queued sibling brief, the "format chip"
#: ``board-sheet-renders-time-text-and-price-is-warmed`` — not landed as of
#: this chip, so nothing here depends on it).
_HOUR = 3600
_DAY = 86400
_MONTH = 86400 * 30
...
def _relative_time(seconds: float) -> str:
    ...
    s = abs(seconds)
    if s < _HOUR:
        n, unit = max(1, round(s / 60)), "min"
    elif s < _DAY:
        n, unit = max(1, round(s / _HOUR)), "h"
    elif s < _MONTH:
        n, unit = max(1, round(s / _DAY)), "d"
    else:
        n, unit = max(1, round(s / _MONTH)), "mo"
    return f"overdue {n}{unit}" if overdue else f"in {n}{unit}"


def next_touch_line(
    card: dict[str, Any], *, now: datetime | None = None,
) -> str | None:
    """D-TM's Stack-face one-liner (build item 3): "next: <action>
    <relative time>" — None when the card carries no resolved pace
    (nothing to show). Pure and deterministic given ``now``, same
    discipline as :func:`stack_order`.

    Not yet wired into ``routes_board.py``'s stack payload (that file is
    outside this chip's touched set) — ``stack.html`` renders the
    equivalent text client-side for now, the same way its own
    :func:`whenLine`-equivalent already does its own date math rather than
    asking the server. This function exists so the logic is pure, tested,
    and ready for the touch-scheduler chip (or the format chip) to call
    server-side once it's wired through.
    """
    next_touch = card.get("next_touch")
    action = card.get("touch_action")
    if not next_touch or not action:
        return None
    when = board_store._parse_ts(next_touch)
    if when is None:
        return None
    now = now or datetime.now(timezone.utc)
    label = board_store.TOUCH_ACTION_LABELS.get(action, action)
    return f"next: {label} {_relative_time((when - now).total_seconds())}"
...
__all__ = [
    ...
    "CardNotReadyForDecision", "today_stats", "bots_plate", "next_touch_line",
]
