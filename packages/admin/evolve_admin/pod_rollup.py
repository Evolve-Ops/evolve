...
def _load_live_daily_costs(
    ...
    # Audit B6: the pod's ONE per-turn cost rule. The old ImportError shim here
    # manufactured ``0.0``, which made an unreadable analyzer path look like a
    # pod that spent nothing. A failed import now leaves every turn UNPRICED.
    # Audit B6: the pod's ONE per-turn cost rule. The old ImportError shim here
    # manufactured ``0.0``, which made an unreadable analyzer path look like a
    # pod that spent nothing. When the resolver is unimportable the resolvers
    # stay ``None`` and EVERY turn is reported unpriced instead.
    resolve_cost: Callable[..., float | None] | None = None
    resolve_provider: Callable[[dict], str] | None = None
    catalog: dict | None = None
    try:
        from turn_cost import (  # type: ignore[import]
            load_pricing_catalog,
            turn_cost as _resolve_turn_cost,
            turn_provider,
        )
        resolve_cost = _resolve_turn_cost
        resolve_provider = turn_provider
        catalog = load_pricing_catalog(Path(shared_dir))
    except Exception as exc:
        _log.warning(
            "pod_rollup: turn_cost resolver unavailable (%s) — every turn for "
            "%s will be reported unpriced rather than priced at $0",
            exc, bot_id,
        )
    ...
    for i in range(days):
        ...
        for line in text.splitlines():
            ...
            d_str = ts[:10]
            resolved = (
                resolve_cost(t, catalog=catalog)
                if resolve_cost is not None else None
            )
            cost = resolved if resolved is not None else 0.0
            slot = per_date.setdefault(
                d_str,
                {"usd": 0.0, "turns": 0, "unpriced": 0, "session_ids": set()},
            )
            slot["usd"] += cost
            slot["turns"] += 1
            if resolved is None:
                slot["unpriced"] += 1
                p = resolve_provider(t) if resolve_provider is not None else "unknown"
...
def compute_spend_rollup_live(
    ...
    any_live = False
    # Audit B6: providers whose turns nothing on this pod could price.
    pod_unpriced_providers: list[str] = []
    for bot_id in bots:
        live = _load_live_daily_costs(shared_dir, bot_id, days=28, today=today)
        if live["ok"]:
            any_live = True
        live_per_bot[bot_id] = live["per_date"]
        for p in live.get("unpriced_providers") or []:
            if p not in pod_unpriced_providers:
                pod_unpriced_providers.append(p)
    ...
    pod_daily: dict[str, dict] = {
        d.isoformat(): {"usd": 0.0, "turns": 0, "unpriced": 0} for d in dates_28
    }
    ...
    for bot_id in bots:
        ...
        for d in dates_28:
            iso = d.isoformat()
            slot = per_date.get(iso) or {"usd": 0.0, "turns": 0, "unpriced": 0}
            usd = float(slot.get("usd", 0.0))
            turns = int(slot.get("turns", 0))
            unpriced = int(slot.get("unpriced", 0))
            daily_28.append({
                "date": iso, "usd": round(usd, 4), "turns": turns,
                "unpriced": unpriced,
            })
            pod_daily[iso]["usd"] += usd
            pod_daily[iso]["turns"] += turns
            pod_daily[iso]["unpriced"] += unpriced
        ...
        unpriced_7d = sum(e["unpriced"] for e in daily_7)
        unpriced_28d = sum(e["unpriced"] for e in daily_28)
        ...
        by_bot[bot_id] = {
            ...
            "usd_7d": round(usd_7d, 4),
            "usd_28d": round(usd_28d, 4),
            # Audit B6: non-zero ⇒ the usd_* figures above are FLOORS.
            "unpriced_today": daily_28[-1]["unpriced"],
    ...
    pod_daily_28 = [
        {
            ...
            "unpriced": int(pod_daily[d.isoformat()]["unpriced"]),
        }
        for d in dates_28
    ]
    ...
    return {
        ...
        "pod_unpriced_28d": sum(e["unpriced"] for e in pod_daily_28),
        "pod_unpriced_providers": sorted(pod_unpriced_providers),
        "currency": "USD",
    }
