...
Per internal/spec-app-identity-ledger-2026-06-27.md (§3 five buckets, §4.C, §5 row
...
``spec_id`` / ``instance_id`` in it. Reflect is a THIN READER: every one of
those values is copied verbatim off a ``recon_ledger.LedgerRow`` /
``SpecIndexEntry`` (``row.spec_id``, ``entry.owning_instance``) and handed
straight back out in a ``ReflectFinding`` and its ``proposed_action{}``. The
actions on the other end — ``strip_marker``, ``rewrite_marker_to_spec``,
``stamp_marker``, ``remove_from_realized_files`` — consume those exact keys
against the ``_evolve`` marker namespace and ``realized_files[]``, so
re-resolving to an app id here would hand the action a key its target does
not use. See the note at the head of ``recon_ledger.py`` for why that
namespace is not the canonical app id.
...
def _orphan_finding(bot_id: str, row) -> ReflectFinding:
    ...
    # identity: see resolve_app_id — copied verbatim off the ledger row into
    # the finding + proposed_action — see the module note.
...
def reflect(bot_id: str, shared_dir: Path = Path("/Users/Shared/evolve")) -> ReflectResult:
    ...
    """
    # identity: see resolve_app_id — the ledger row's spec binding, resolved to
    # its owning Instance through the ledger's own lineage-aware index — see
    # the module note.
...
def main(argv: Optional[list[str]] = None) -> int:
    # identity: see resolve_app_id — CLI rendering of the finding fields, all
