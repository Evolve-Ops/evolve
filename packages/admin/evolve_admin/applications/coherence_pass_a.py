...
manifest and the v7-arc Instance shape (spec internal/spec-manifest-v7-2026-05-20.md).
...
# identity: this module was SWEPT onto applications.app_identity.resolve_app_id in AL-1.4b (area 3):
# ``_declared_dependency_keys`` had two hand-rolled chains that disagreed with
# each other and both now go through the resolver. The remaining mentions below
# are that function's docstring explaining what was removed, plus the two
# declared dependency-entry SCHEMAS it reads (``{pkg_id, display_name}`` and
# the v7-arc ``{spec_id, ...}``) — field names, not identity reads.
...
# surfaces results to the operator (spec internal/spec-manifest-v7-2026-05-20.md
...
def _iter_declared_files(manifest: dict):
    ...
    Instance; spec internal/spec-manifest-v7-2026-05-20.md §4). Without reading
...
def _declared_dependency_keys(manifest: dict) -> set[str]:
    ...
    through ``app_identity.resolve_app_id`` — the one place that answers
    "which app is this?" — rather than by two hand-rolled field chains that
    disagreed with each other (``pkg_id`` here, ``spec_id or pkg_id``
    there). Both declared shapes carry exactly one of those fields
    (``_migrate_app_dependencies`` emits ``{spec_id, required, purpose}``;
    the ea-pack shape emits ``{pkg_id, display_name, required, reason}``),
    so the resolver returns the same string either chain did — and it now
    also honors a canonical ``app_id`` on a dependency entry the moment
    writers start emitting one.
    """
    keys: set[str] = set()
    for dep in manifest.get("app_dependencies") or []:
        if not isinstance(dep, dict):
            continue
        pid = resolve_app_id(dep)
        if pid:
            keys.add(pid)
