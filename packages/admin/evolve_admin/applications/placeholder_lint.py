...
# identity: see resolve_app_id — ``pkg_id``/``app_id`` here are PLACEHOLDER
# NAMES (the ``{{pkg_id}}`` / ``{{app_id}}`` tokens a files-pack may use), not
# identity reads. Their bound VALUES are set by ``workspace_sync`` /
# ``forge_engine``, which area 3 annotated; reconciling that namespace is 1.4c.
