...
Spec: internal/spec-app-identity-ledger-2026-06-27.md (§2, §3 the five buckets,
§4.C, §5 row ``apps-recon-ledger``, §6 invariants); sub-spec of
internal/spec-apps-meta-2026-06-13.md.
...
``spec_id`` / ``instance_id`` in it (audit-app-framework-2026-07-02 §2.4:
the ledger KEYS ROWS ON IDENTITY, so what those keys mean is load-bearing).
Nothing here routes through ``app_identity.resolve_app_id``, because nothing
here is asking "which app is this manifest?":

  * ``LedgerRow.spec_id`` is the resolved CURRENT ``provenance.spec_id`` a
    marked FILE ties to — the join key against the ``_evolve`` marker's
    ``spec=`` value written by ``provenance.embed_marker``. That namespace is
    the spec binding, not the app's canonical id; on a v7-arc Instance the
    two are different strings (``instance_id`` vs ``provenance.spec_id``), so
    resolving the owning manifest's app id here would key the ledger against
    a namespace no marker on disk uses and orphan every marked file.
  * ``SpecIndexEntry.owning_instance`` is ``instance_id`` specifically — the
    per-bot realization handle, deliberately ``None`` for a ``discovered``
    app with no materialized Instance. ``reflect.py`` surfaces that ``None``
    as "no owning Instance"; a resolver that always answers with a non-empty
    id would erase the distinction the discovered/defined split exists to
    make.
  * ``prior_spec_ids[]`` is a chain of RETIRED ids (see ``spec_lineage``).

``lookup_spec`` is spec-BINDING resolution (a possibly-dead marker id -> the
live Instance that superseded it), the inverse question from app-identity
resolution. See the note at the head of ``spec_lineage.py``.
"""
...
def build_reverse_spec_index(
    ...
    for inst in instances:
        ...
        index[sid] = SpecIndexEntry(
            ...
            # identity: see resolve_app_id — ``instance_id`` specifically, and
            # its ABSENCE is data: a ``discovered`` app (manifest + Spec on
            # disk, nothing materialized) carries none, and ``reflect.py`` /
            # ``manifest_hygiene`` both branch on the resulting ``None``.
...
def rows_from_dict(ledger: dict, bucket: str) -> list[LedgerRow]:
    ...
    # identity: see resolve_app_id — round-trips the persisted
    # LedgerRow.spec_id verbatim — see the module note.
