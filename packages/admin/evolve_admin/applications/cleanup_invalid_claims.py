    ...
    *,
    ...
    for bot_id in bot_ids:
        ...
        for ipath, inst in _iter_v7_instances(bot_id):
            # identity: see resolve_app_id — the v7-arc realization handle,
            # kept as the field. Its only use is labelling the removal in the
            # plan/CLI output; the WRITE targets ``ipath`` (the real on-disk
            # path this dict was read from), never ``{instance_id}.json``, so
            # this must not be re-resolved into a different namespace and
            # imply a filename that does not exist. See the ``_iter_v7_
            # instances`` docstring for that invariant.
...
def apply_removal_plan(
    ...
    *,
    ...
    for ipath_str, removals in plan.by_instance().items():
        ...
        for rf in dropped:
            # identity: see resolve_app_id — names the Instance whose claim is being
            # removed; the row is keyed on it.
...
def _print_dry_run(plan: RemovalPlan) -> None:
    # identity: see resolve_app_id — grouping the CLI dry-run output by the
    # realization label built above; not an id read.
    ...
    for bot_id in sorted(by_bot):
        ...
        for r in bot_removals:
            # identity: see resolve_app_id — grouping key: rows are grouped by the Instance
            # they belong to, which is also its filename stem. The resolver returns the
            # bound SPEC id for a hydrated instance (AL-1.4 §6 delta 6), collapsing every
            # Instance of one Spec onto a single row.
