...
-------------------------
Four read paths turn a ``shared_dir`` into evidence — three here:
``conversation_detections()`` (the scanner's), ``main()`` (the operator
CLI) and ``detect_from_shared_dir()`` — plus ``analyzer/usage_by_user.py``,
which joins these same ``recurring_request`` rows onto turn annotations to
answer "who uses which app". ALL FOUR route through
``apply_do_not_track_gates``; the analyzer one imports this module and
calls the chokepoint rather than re-deriving the two switch reads (it
gates once per distinct requester and fails CLOSED — no attribution at
all — when the gate cannot be evaluated, including when this module is
not importable in that interpreter).

None of the three here did originally: the CLI shipped
