...
Dispatch:
  sudo -iu <bot> /opt/homebrew/bin/openclaw agent \\
       --local --agent main --json --timeout <n> --message "<prompt>"

The `evolve ALL=(ALL) NOPASSWD: SETENV: /opt/homebrew/bin/openclaw` grant
already covers this — no new sudoers required.
"""
...
def _resolve_provisioning_build_model(bot_id: str) -> str | None:
    ...
    docs/finding-new-bot-activation-cost-2026-06-12.md; decision C in
    docs/roadmap-user-value-2026-06-10.md). Pin the dispatch to the pod's
...
class BuildRequest:
    ...
    # (docs/note-smart-forge-and-file-provenance-2026-06-04.md). When a
    ...
    # prior attempt don't survive (which caused sha256-mismatch failures
    # when the bot's fresh declarations didn't match leftover on-disk
    # content). build_prompt prepends the cleanup instruction when this
...
def build_prompt(
    ...
    *,
    ...
    isn't reachable from the admin user, and stale files there cause
    sha256-mismatch failures on the second attempt.
    """
    retry_paragraph = ""
    if is_retry:
        retry_paragraph = (
            ...
            "Without this step, sha256 verification fails when your fresh "
            "content doesn't match the leftover on-disk files.\n\n"
        )
    return (
...
def _dispatch_agent(
    ...
    if outbox.exists() and inbox.exists():
        try:
    ...
    # grant (`evolve ALL=(ALL) NOPASSWD: SETENV: /opt/homebrew/bin/openclaw`)
    # only matches the latter, so `-i` would prompt for a password and fail.
...
# Lifecycle order of file-writing outbox suffixes for a single forge job.
# Build (step 2) writes ``<job>.json``; refine round 1 (step 4) writes
# ``<job>-r1.json``; refine round 2 (step 6) writes ``<job>-r2.json``.
# Critique outboxes (-c1, -c2) don't appear here — they carry ``issues``,
# not ``files_written`` — so they're not part of the rewrite chain.
_LIFECYCLE_SUFFIXES: tuple[str, ...] = ("", "-r1", "-r2")


def paths_rewritten_after(
    bot_id: str,
    job_id: str,
    after_suffix: str,
) -> set[str]:
    """Return paths declared in any outbox for this job whose lifecycle
    position is strictly AFTER ``after_suffix``.

    Used by ``verify_files_on_disk`` to recognise that a build hash is no
    longer authoritative for an on-disk file once a refine round has
    rewritten it. Without this, replaying step 2 after step 4 has touched
    the same path produces a sha256 mismatch on a file the bot legitimately
    edited.

    The lifecycle chain is fixed (`build → refine r1 → refine r2`);
    critique outboxes are skipped because they don't write files.
    Unknown ``after_suffix`` is treated as the earliest position so an
    unexpected call sees the broadest skip set (fail-safe toward
    "verification skips this path").
    """
    order = {sfx: i for i, sfx in enumerate(_LIFECYCLE_SUFFIXES)}
    after_order = order.get(after_suffix, -1)
    rewritten: set[str] = set()
    for suffix in _LIFECYCLE_SUFFIXES:
        if order[suffix] <= after_order:
            continue
        ob = outbox_path(bot_id, job_id, suffix)
        if not ob.exists():
            continue
        try:
            data = json.loads(ob.read_text(encoding="utf-8"))
        except Exception:
            # Unreadable outbox isn't a verification concern — the dispatch
            # path will have its own failure mode if the file is malformed.
            continue
        for entry in data.get("files_written") or []:
...
def verify_files_on_disk(
    ...
    *,
    skip_hash_for_paths: set[str] | None = None,
) -> tuple[list[str], list[str]]:
    """Confirm each file in *files_written* exists in the bot workspace.

    If the entry includes ``sha256``, the on-disk file is hashed and
    compared. Returns ``(verified_paths, errors)``.

    ``skip_hash_for_paths``: relative paths whose on-disk content has
    legitimately diverged from this outbox's declared hash because a
    LATER outbox in the same forge job rewrote them. Existence is still
    required; only the hash check is skipped. Use
    :func:`paths_rewritten_after` to compute this set.
    """
    workspace = bot_workspace(bot_id)
    verified: list[str] = []
    errors: list[str] = []
    skip = skip_hash_for_paths or set()
    ...
    for entry in files_written:
        ...
        if expected and rel not in skip:
            try:
            ...
            if actual != expected:
                errors.append(
                    ...
                    f"(claimed {expected[:8]}, actual {actual[:8]})"
                )
                continue
