...
class _Inst:
    ...
    def spec_id(self) -> Optional[str]:
        # identity: see resolve_app_id — this is the Instance's CURRENT SPEC
        # BINDING (``provenance.spec_id``, read through ``spec_lineage``'s
        # shared ``current_spec_id``), which is what a provenance marker's
        # ``spec=`` token names and what ``build_spec_index`` keys on together
        # with ``prior_spec_ids[]``. The resolver would return the Instance's
        # own id (AL-1.4 §6 delta 6), so every marker would look orphaned and
        # this module would re-point the whole workspace.
...
def compute_plan(bot_id: str) -> list[PlanRow]:
    ...
    for wf in marked:
        ...
        # identity: see resolve_app_id — ``_Inst.spec_id`` is the Instance's
        # CURRENT SPEC BINDING (see the property), which is what a provenance
        # marker names and what the re-point below writes back. Every
        # ``.spec_id`` read in this block is that same binding.
