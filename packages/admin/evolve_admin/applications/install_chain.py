...
def _mark_blocked_after(chain: InstallChain, failed_link: ChainLink) -> None:
    for later in chain.links:
        if later.order > failed_link.order and later.state in ("pending", "blocked"):
            later.state = "blocked"
            later.blocked_by = failed_link.pkg_id
...
def _create_link_job(chain: InstallChain, link: ChainLink, shared_dir: Path):
    ...
    pkg = load_gallery_package(link.pkg_id, shared_dir)
    if pkg is None:
        raise ValueError(f"package {link.pkg_id!r} disappeared from the gallery")
...
def _advance_locked(
    ...
    for link in sorted(chain.links, key=lambda lk: lk.order):
        ...
        with _slot_lock(chain.bot_id, link.pkg_id):
...
def _process_link(
    ...
    *,
    ...
    state = installed_state(link.pkg_id, chain.bot_id, shared_dir)
    if state == "installed":
        link.state = "skipped"
        link.detail = "already installed"
        save_chain(chain, shared_dir)
    ...
    if job is None or job.status != "queued" or just_cloned:
        ...
        try:
            ...
            log.warning(
                ...
                chain.chain_id, link.pkg_id, exc,
            )
        ...
        pf = preflight_check(link.pkg_id, chain.bot_id, shared_dir)
        hard_blockers = [
            item for item in (
    ...
    try:
        ...
        log.error(
            ...
            chain.chain_id, link.pkg_id, link.display_name, exc,
        )
