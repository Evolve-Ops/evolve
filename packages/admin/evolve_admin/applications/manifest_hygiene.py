...
identity: see resolve_app_id — apart from ``_app_label`` (annotated at its
definition), every ``spec_id`` in this module is the marker/binding namespace
it inherits from ``recon_ledger`` — the already-resolved CURRENT
``provenance.spec_id`` a Reflect ``orphan_file`` finding carries — and every
``instance_id`` is a label produced by ``_app_label``. Resolution happens once,
upstream, through ``spec_lineage.build_spec_index``; this module deliberately
never re-resolves (see the "SAME build_spec_index / never a second, divergent
index" invariant in ``reconcile_orphan_markers``).
...
class ReconcileResult:
    ...
    # identity: see resolve_app_id — the per-spec rollup is keyed by the
    # resolved CURRENT spec_id each row carries — see the module note.
    bot_id: str
...
def _app_label(inst: dict, path: Path) -> str:
    ...
    identity: see resolve_app_id — NOT swept (AL-1.4b), and the ORDER is the
    reason. This chain is ``instance_id`` FIRST, deliberately: the label names
    the per-bot realization the operator is being asked about, and its
    fallback to ``id``/stem is what makes a ``discovered`` app (no
    ``instance_id``) still nameable. ``resolve_app_id`` answers a different
    question and answers it in the opposite order — ``pkg_id`` leads its
    legacy chain, so for a gallery-installed app it would return
    ``p-a3f91c8b`` where this returns the readable stem. That string is
    surfaced verbatim in the success toast and in ``AmbiguousOrphan
    .instance_ids``, and is also the caller-visible ``ResolvedOrphan
    .instance_id``.
    """
...
def reconcile_orphan_markers(
    ...
    *,
    ...
    """
    # identity: see resolve_app_id — the already-resolved spec binding from the
    # recon ledger; this function never re-resolves (module note).
...
def main(argv: Optional[list[str]] = None) -> int:
    # identity: see resolve_app_id — CLI rendering of the spec binding + the
    # _app_label realization label, both annotated at their source.
