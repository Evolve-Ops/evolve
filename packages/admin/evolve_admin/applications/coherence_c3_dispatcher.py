...
def _resolve_llm_target(network: dict[str, Any]):
    ...
    C3 is a one-shot cheap judgment call — same shape as the behavioral
    judge, so the behavioral judge's model pin is honored first (when
    its provider is credentialed on the primary bot); otherwise
    ``infra_llm`` walks the pod's credentialed providers at the fast
    tier. Returns ``None`` when no LLM provider is credentialed.
    """
    try:
    ...
    try:
        ...
        pinned = _resolver(network or {})
    except Exception:  # noqa: BLE001
        pinned = ""
    try:
        return credentialed_target(
            pinned, network=network or None
        ) or resolve_infra_llm("fast", network=network or None)
    except Exception:  # noqa: BLE001
        return None
