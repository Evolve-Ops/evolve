...
Per internal/spec-manifest-v7-2026-05-20.md §8.3. The bot's LLM calls this during a
...
def extend_application(
    ...
    # identity: see resolve_app_id — the Instance->Spec BINDING, and NESTED
    # under ``provenance`` where the resolver (top-level only) cannot see it.
    # It travels as a pair with ``spec_version`` into ``embed_marker`` below,
    # where it becomes the ``spec=`` marker namespace on the new file. A v7-arc
    # Instance's own ``resolve_app_id`` answer is its ``instance_id`` (AL-1.4
    # §6 delta 6), not its Spec — stamping that into the marker would attribute
    # the file to the instance rather than the portable spec.
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    result = extend_application(
        # identity: see resolve_app_id — ``instance_id`` is this tool's PUBLIC
        # PARAMETER name and the Instance's filename stem
        # (``manifests/<instance_id>.json``), not a manifest field being read.
