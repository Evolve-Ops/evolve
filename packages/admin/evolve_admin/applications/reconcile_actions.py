...
class AppDriftReport:
    ...
    # identity: see resolve_app_id — serializes the two distinct columns
    # annotated at the scan loop below: pkg_id is the gallery lookup key,
    # app_id the manifest key.
    ...
    def to_dict(self) -> dict:
        return {
            "bot_id":               self.bot_id,
            "app_id":               self.app_id,
            # identity: see resolve_app_id — the result row reports the catalog key
            # alongside a distinct ``app_id`` one line up, because they differ for a
            # gallery install.
            "pkg_id":               self.pkg_id,
    ...
    *,
    ...
    for bot_id in bot_ids:
        ...
        for manifest in _iter_bot_manifests(bot_id, bot_info):
            ...
            # identity: see resolve_app_id — NOT swept (AL-1.4b). Both fields
            # below are semantically required, and the report carries them as
            # two SEPARATE columns precisely because they are two different
            # things:
            #   * ``pkg_id`` is the GALLERY LOOKUP KEY — it is fed straight to
            #     ``load_gallery_package(pkg_id, shared_dir)``, and its
            #     ABSENCE is a classification of its own
            #     (``CLASS_SKIPPED_NO_PKG_ID``, "a forge-only custom app").
            #     A canonical ``app_id`` is present on every stamped manifest,
            #     so resolving here would make ``if not pkg_id`` unreachable
            #     and send every custom app into a gallery lookup that cannot
            #     succeed.
            #   * ``manifest.id`` is the report's ``app_id`` column — the
            #     manifest/URL key the operator matches with ``--app-id``
            #     (see the ``app_filter`` compare just above, which must stay
            #     the same namespace). ``resolve_app_id`` leads with
            #     ``pkg_id``, so sweeping it would make the two columns
            #     identical for every gallery app and drop the only handle on
            #     the manifest.
