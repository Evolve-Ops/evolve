...
def _recurrence_dimension(manifest: Mapping[str, Any]) -> Dimension:
    ...
    1. **Conversation evidence (D-R, 2026-08-22).** A draft carrying
       ...
       manifest. This is a real producer: ``scanner._stamp_conversation_evidence``
       writes the block on every conversation-only detection.
    ...
    ORDER, and why. Conversation evidence is checked first because for the class
    it identifies it IS the recurrence rule, and that class has no runs by
    construction (there is no file to invoke). A manifest carrying both a
    conversation block and a positive run count is a shape no producer writes
    today; if one ever does, the §7.1a rule still describes what the user
    actually does, which is what a promotion offer is about.
