...
Per internal/spec-manifest-v7-2026-05-20.md §8.1.5.
...
``spec_id`` / ``instance_id`` in it. Adopt is entirely a VERSION-LINE
operation: ``provenance.spec_id`` selects which gallery version-line to
re-pin against (``gallery/<tier>/<spec_id>/<version>.json``, built by
``load_spec_version``) and is cross-checked against both Spec files' own
``spec_id`` before any write — resolving the Instance's canonical app id
there would compare two different namespaces and fail every adopt.
``instance_id`` is the Instance FILENAME STEM
(``manifests/<instance_id>.json``, see ``_instance_path``) and the value the
``--app-id`` flag takes; it must stay the field or the read/write lands on
the wrong file. ``_IGNORED_FIELDS`` names ``spec_id`` as a literal key of
the diffed dicts, not as an identity read.
"""
...
def adopt_with_specs(
    ...
    *,
    ...
    """
    # identity: see resolve_app_id — the gallery version-line key + Instance
    # filename stem — see the module note.
