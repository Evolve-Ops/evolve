...
internal/decision-app-tests-2026-06-08.md. The fields remain on the dataclass
...
runner (see internal/spec-app-audit-2026-05-16.md):
...
scanner's new extraction pass. See internal/spec-audit-extensions-2026-05-17.md
...
manifest's claims" (Tier 3, LLM-driven). See internal/spec-app-audit-2026-05-16.md
...
from .app_identity import APP_ID_FIELD, ensure_app_id
...
# the app-substrate lifecycle (internal/spec-apps-meta-2026-06-13.md §9). One new
...
# (internal/spec-apps-meta-2026-06-13.md §9.3). One new top-level field with inert
...
# v29: provided_capabilities[] — the app-declared half of the Layer-2 per-role
# tool-loading model (internal/spec-user-roster-and-roles-2026-06-07.md §4/§8,
# internal/design-layer2-tool-loading-filter-2026-07-16.md). One new top-level list
# field with inert default []; each entry declares which MCP tools a provided
# capability requires (requires_mcp_tools) and its suggested initial role binding
# (default_role_binding). SCHEMA + VALIDATION ONLY — the enforcement gate that
# consumes it is a separate later build step; nothing branches on the counter.
# v30: app_id + draft_id — the canonical identity field (AL-1.4a;
# internal/build-AL-1.4-app-id-canonical.md §2, decision in
# internal/design-app-spec-and-discovery-2026-08-15.md §3). Vocabulary-only bump:
# app_id is populate-on-absent from the SAME legacy chain
# (pkg_id -> id -> spec_id -> instance_id) that every reader already resolves,
# so no manifest changes identity and nothing branches on the counter.
# draft_id is the scanner-born counterpart — a discovered draft never gets an
# app_id, because identity is conferred by promotion, not by discovery.
# Resolution + stamping live in applications/app_identity.py (TS twin
# packages/plugin/src/apps/appIdentity.ts).
MANIFEST_SCHEMA_VERSION = 30
...
# migration sets it to "v7-arc". See internal/spec-manifest-v7-2026-05-20.md.
...
# (internal/spec-forge-side-effects-2026-06-02.md). Five new optional sub-fields
...
#     Stamped by forge Phase 4.5 when the most recent materialize attempt
#     for this action FAILED (failure-visibility layer, audit slate S2
#     2026-07-02); ``install_failed_at`` carries the ISO timestamp of that
#     attempt. Both are cleared when a later attempt installs (or
#     idempotently skips) the action. Presence of ``install_error`` is the
#     enumerable "declared but not live" marker read by the Apps card,
#     uninstall teardown, and the live-test harness.
#
...
#
# Spec: internal/spec-heartbeat-instruction-2026-06-03.md.
#
...
# internal/spec-launchd-python-signal-2026-06-03.md.
...
#
# Spec: internal/spec-proactive-delivery-monitor-2026-06-10.md §5 (Option B
...
# ── Schema v29: provided_capabilities[] validation ───────────────────────────
#
# Spec: internal/spec-user-roster-and-roles-2026-06-07.md §4 (App-declared
# capabilities) + §8 (Manifest schema v7 dependency);
# internal/design-layer2-tool-loading-filter-2026-07-16.md (the Layer-2 build).
#
# Each entry declares one app-provided capability:
#   {
#     "name": str,                       # capability id, e.g. "app.archive.add"
#     "description": str,                # optional one-line human description
#     "requires_mcp_tools": [str, ...],  # MCP tools the capability needs loaded
#                                        # (the Layer-2 link — the enforcement
#                                        # gate unions these per resolved role)
#     "default_role_binding": <role>|null,  # suggested initial binding at install
#   }
#
# Pinned structure, open capability/tool vocabulary. Empty list = "not yet
# declared" (backward-compatible — apps that declare nothing default to the
# existing all-tools behavior; see spec §8). SCHEMA + VALIDATION ONLY: no
# gateway/tool-loading consumer branches on this yet.

PROVIDED_CAPABILITY_KEYS: frozenset[str] = frozenset({
    "name",
    "description",
    "requires_mcp_tools",
...
def _known_role_bindings() -> frozenset[str]:
    """The roster role vocabulary a ``default_role_binding`` may name.

    Single-sourced from ``roster_overlay.ROLES`` (admin / primary_user /
    participant / blocked — the same set as
    ``capabilities.DEFAULT_ROLE_CAPABILITIES``). Lazy import keeps manifest.py
    free of a module-level dependency; the hardcoded fallback matches the
    canonical tuple so validation never crashes if the import is unavailable.
    """
    try:
        from ..roster_overlay import ROLES
        return frozenset(ROLES)
    except Exception:
        return frozenset({"admin", "primary_user", "participant", "blocked"})


def validate_provided_capabilities(entries: Any) -> list[str]:
    """Return a list of shape errors for a ``provided_capabilities`` block.

    Empty list means valid. Pure (no I/O), self-contained shape validator in
    the same style as ``validate_delivery_contract`` — co-located with the
    schema so any future consumer (the Layer-2 enforcement gate, forge/gallery
    preflight) agrees on what a well-formed declaration is. Absence / empty
    list is valid ("not yet declared"); a malformed entry is reported, never
    raised.
    """
    errors: list[str] = []
    if not isinstance(entries, list):
        return [
            f"provided_capabilities must be a list, got {type(entries).__name__}"
        ]
    roles = _known_role_bindings()
    seen_names: set[str] = set()
    for idx, entry in enumerate(entries):
        where = f"provided_capabilities[{idx}]"
        if not isinstance(entry, dict):
            errors.append(f"{where} must be an object, got {type(entry).__name__}")
            continue
        for key in sorted(set(entry) - PROVIDED_CAPABILITY_KEYS):
            errors.append(
                f"{where} has unknown key {key!r}; allowed keys: "
                f"{', '.join(sorted(PROVIDED_CAPABILITY_KEYS))}."
            )
        name = entry.get("name")
        if not isinstance(name, str) or not name.strip():
            ...
            if name in seen_names:
                errors.append(f"{where}.name {name!r} is declared more than once.")
            seen_names.add(name)
        if "description" in entry and not isinstance(entry["description"], str):
            errors.append(f"{where}.description must be a string.")
        rmt = entry.get("requires_mcp_tools", [])
        if not isinstance(rmt, list) or not all(
            isinstance(t, str) and t.strip() for t in rmt
        ):
            errors.append(
                f"{where}.requires_mcp_tools must be a list of non-empty strings."
            )
        drb = entry.get("default_role_binding")
        if drb is not None and drb not in roles:
            errors.append(
                f"{where}.default_role_binding {drb!r} is not a known role; "
                f"expected one of {', '.join(sorted(roles))} or null."
            )
    return errors
...
# internal/decision-app-tests-2026-06-08.md. Manifest's `test_cadence` field is
...
# bot config." See internal/spec-app-audit-2026-05-16.md §6 for the three-layer
...
# removed 2026-06-08 — app-test surface killed per
# internal/decision-app-tests-2026-06-08.md. Coherence gate
...
# ── Per-file provenance (smart forge, internal/note-smart-forge-and-file-
...
# app-substrate lifecycle work; internal/spec-apps-meta-2026-06-13.md §9).
#
...
class ApplicationManifest:
    ...
    # internal/decision-app-tests-2026-06-08.md. Field kept so existing on-disk
    ...
    # App-test surface removed; see internal/decision-app-tests-2026-06-08.md.
    # Fields kept for backward compatibility; will be dropped in schema v11.
    ...
    # Spec: internal/spec-files-pack-hybrid-2026-06-03.md.
    #
    ...
    # Spec: internal/spec-app-coherence-and-reconciliation-2026-06-05.md §3, §4,
    ...
    # ── Schema v23: delivery_contract{} on scheduled_actions[] entries ──────
    # Spec: internal/spec-proactive-delivery-monitor-2026-06-10.md §5.
    #
    ...
    # Spec: internal/spec-manifest-v7-slicing-2026-06-10.md §4; field shapes are
    ...
    # Spec: internal/spec-apps-meta-2026-06-13.md §9. The third manifest axis,
    ...
    # ── Schema v29: provided_capabilities[] (Layer-2 per-role tool loading) ────
    # Spec: internal/spec-user-roster-and-roles-2026-06-07.md §4/§8;
    # internal/design-layer2-tool-loading-filter-2026-07-16.md. The app-declared
    # half of the Layer-2 per-role tool-loading model: each entry names a
    # capability the app provides, the MCP tools that capability requires
    # (requires_mcp_tools — the link into the gateway tool allowlist), and the
    # suggested initial role binding (default_role_binding). Entry shape +
    # validation live in validate_provided_capabilities / PROVIDED_CAPABILITY_KEYS
    # above.
    #
    # Inert default []: an app that declares nothing keeps the existing
    # all-tools behavior (spec §8 — "apps that don't declare provided_capabilities
    # default to all exposed tools"), so no legacy manifest changes meaning.
    # SCHEMA + VALIDATION ONLY — the enforcement gate that unions
    # requires_mcp_tools per resolved role is a separate later build step.
    # Readers branch on the field, never on MANIFEST_SCHEMA_VERSION.
    provided_capabilities: list[dict] = field(default_factory=list)

    # ── Schema v30: app_id / draft_id (the one identity) ──────────────────────
    # AL-1.4a. internal/build-AL-1.4-app-id-canonical.md §2;
    # internal/design-app-spec-and-discovery-2026-08-15.md §3.
    #
    # app_id is THE identity: a lowercase slug (app_identity.APP_ID_PATTERN —
    # the rule RecordApplicationTool already enforces), conferred when an app
    # becomes *defined* and immutable thereafter. It leads the resolution order
    # app_id > pkg_id > id > spec_id > instance_id in
    # app_identity.resolve_app_id (and its TS twin appIdentity.appIdOf).
    #
    # Inert in 1.4a by construction: the stamp writes the id the legacy chain
    # ALREADY resolves to, and every legacy field keeps being written, so all
    # ~96 identity readers still find exactly what they found before. 1.4b
    # sweeps those readers onto the shared resolver; 1.4c drops the fallback
    # and stops mirroring the legacy fields. Never branch on the counter.
    #
    # draft_id is the discovered-side counterpart: the scanner assigns it to a
    # draft it has not been asked to vouch for, and it is explicitly UNSTABLE —
    # the scanner may merge/rename/drop the draft freely. It never appears in
    # attribution, access or sharing, and app_identity.draft_id_of deliberately
    # refuses to fall back to it from the app-id side. A manifest carries one
    ...
    def files_partition(
        ...
        ``internal/note-smart-forge-and-file-provenance-2026-06-04.md``.
    ...
    def is_gallery_app(self) -> bool:
        ...
        # identity: see resolve_app_id — a PRESENCE probe, not an id read
        # ("did this install come from a gallery package at all?"). A canonical
        # app_id is stamped on every conforming manifest, so routing this
        # through the resolver would make every app a gallery app.
...
def hydrate_v7_arc_instance(instance: dict, shared_dir: Path) -> dict:
    ...
    """
    # identity: see resolve_app_id — every id in this function is
    # semantically required, and this function is WHY ``resolve_app_id``'s
    # docstring says "feed it the RAW manifest": hydration REWRITES the
    # identity fields (``id`` <- ``instance_id``, ``pkg_id`` <-
    # ``provenance.spec_id``), so a hydrated dict resolves to a different key
    # than the raw one on disk. ``provenance.spec_id`` + ``spec_version``
    # below are the gallery version-line coordinates
    # (``gallery/<tier>/<spec_id>/<spec_version>.json``); resolving the
    # Instance's canonical app id would name a path that does not exist and
    # return every v7-arc Instance un-hydrated.
    ...
    # v29 — provided_capabilities[] is a Spec-owned declaration (the app-declared
    # half of the Layer-2 per-role tool-loading model, spec §4/§8). Overlay it so
    # a hydrated v7-arc manifest surfaces the Spec's capability declarations.
    # Instance-local values win when present (same fill-if-missing posture).
    if spec.get("provided_capabilities") and not hydrated.get("provided_capabilities"):
        hydrated["provided_capabilities"] = spec["provided_capabilities"]
...
def get_app_dependents(pkg_id: str, bot_id: str, shared_dir: Path) -> list[dict]:
    ...
    """
    # identity: see resolve_app_id — the app_dependencies[] namespace: a
    # dependency entry names ANOTHER app by pkg_id, and the caller passes the
    # pkg_id to match. migrate_v7._migrate_app_dependencies /
    # coherence_pass_a._declared_dependency_keys are the swept pair that writes
    # and reads it.
    if not pkg_id:
        return []
    result: list[dict] = []
    for m in list_manifests(shared_dir, bot_id):
        ...
        for dep in (m.app_dependencies or []):
            # identity: see resolve_app_id — the dependency edge is keyed on ``pkg_id`` on
            # BOTH sides (the caller passes one, the dep entry declares one), and the row
            # below reports the dependent's ``app_id`` and ``pkg_id`` as separate columns
            # because they differ for a gallery install (AL-1.4 §6 delta 12).
...
def _write_manifest_bytes(path: Path, content: bytes) -> None:
    ...
    THE ONE IN-PLACE MANIFEST WRITER — and the reason nothing chowns
    manifests back to the bot.

    Step 1 is a genuine in-place ``open(..., 'wb')``: it needs write on
    the FILE. Every other writer in the pod renames over the manifest
    instead, which needs write on the manifests DIRECTORY (bot-owned,
    with an inherited evolve ACE) and not on the file:

      • ``web/server.py::_write_manifest_as_bot`` — same-dir mkstemp +
        os.replace (bumps the dir mtime the plugin's trigger cache keys
        on); leaves the file ``evolve:staff``
      • plugin ``PUT /evolve/manifests/:id`` → ``writeJSON`` temp+rename
        (packages/plugin/src/api/networkRoutes.ts)
      • ``analyzer/application_scanner.py``, run as ``sudo -u <bot>`` →
        ``evolve_util.atomic_write_json`` (same-dir mkstemp + os.replace)
      • ``analyzer/app_audit_runner.py`` (bot-user launchd, Tier 2 / Tier
        3) — the only bot-side path that mutates manifest CONTENT
        (``coherence.findings[]`` via ``pass_runner.apply_all_pure_passes``,
        ``reconciliation``, ``last_structural_verify`` / ``last_audit``) →
        ``_stamp_manifest_field``, same-dir temp + os.replace, which
        flips the file back to bot-owned

    Bot agents never author manifests directly — ``docs/system/POD_CONDUCT.md``
    routes them through ``record_application``, whose queue is drained by
    ``manifest_reflex_runner`` running as ``evolve``.

    Net effect: ownership under ``workspace/manifests/`` records only who
    renamed last and churns in BOTH directions, which is why
    ``wizard_verify``'s Check 1 exempts the subtree wholesale (and why the
    gauntlet spec, §1 "Carve-outs", says contents "may be evolve:staff").
    A chown-back would be cosmetic, and is ungranted besides: the sudoers
    set covers the manifests DIRECTORY and ``workspace/*.md``, never
    ``workspace/manifests/*.json`` (setup_wizard §11b / §12).

    What invalidates all of the above: giving THIS function a caller that
    runs as the bot user. Today every caller of ``save_manifest`` is
    admin-side (evolve) — the web routes, the CLI, the forge engine, the
    arbiter appliers, ``manifest_reflex_runner``. A bot-user caller would
    hit EACCES on any evolve-owned manifest and fall through to a sudo
    path the bot has no grant for. If that ever becomes necessary, restore
    bot ownership in ``_write_manifest_as_bot`` and add the
    ``workspace/manifests/*.json`` chown grant to
    ``setup_wizard._render_evolve_sudoers`` + ``tools/sudo-grant-lint``.
    """
    import tempfile
    import subprocess
    import os
...
#
# Spec: internal/spec-app-coherence-and-reconciliation-2026-06-05.md §4 + §13.1.
#
...
def save_manifest(manifest: ApplicationManifest, shared_dir: Path) -> Path:
    ...
    # v30 (AL-1.4a): stamp the canonical app_id before the write. Every writer
    # that composes a manifest through the dataclass lands here — forge apply
    # (_apply_forge_output), the record_application manifest reflex, the editor
    # / adopt / extend paths — so one call covers all of them. The value is the
    # id the legacy chain already resolves to, so the stamp is behavior-neutral
    # (see app_identity's module docstring); populate-on-absent, so an id
    # conferred earlier is never rewritten.
    #
    # NOT stamped inside _write_manifest_bytes, which the brief names: that
    # function is a byte-level atomic-write primitive shared with the _history
    # archive writes and with callers that hand it payloads which are not whole
    # manifests, and parsing/re-serializing bytes there would break both its
    # contract and the tests that pin it (test_manifest_write_fnf_fallback).
    payload = manifest.to_dict()
    ensure_app_id(payload, context=str(path))
    _write_manifest_bytes(path, json.dumps(payload, indent=2).encode("utf-8"))
...
def plan_manifest_deletion(
    ...
    """
    from .install_helpers import derive_scheduled_teardown

    manifest = load_manifest(application_id, bot_id, shared_dir)
    if manifest is None:
        return {"ok": False, "error": "manifest not found"}

    # identity: see resolve_app_id — the file-index/marker attribution
    # namespace (see the module note in ``file_index.py``). ``owned_by`` is
    # compared against this exact value to decide preserve/clean/delete, and
    # the same string is handed to ``remove_pkg_id_from_marker`` to match the
    # literal text in each file's ``_evolve`` marker.
    ...
    return {
        ...
        # Phase-4.5 artifacts (audit S4): launchd/systemd units, python-signal
        # wrapper files, and heartbeat/AGENTS managed sections this uninstall
        # will tear down. Enumeration is pure; execute_scheduled_teardown
        # acts on it.
        "scheduled_teardown": derive_scheduled_teardown(
            manifest.scheduled_actions, bot_id,
        ),
    }


def execute_scheduled_teardown(
    application_id: str,
    bot_id: str,
    shared_dir: Path,
    plan: dict | None = None,
    ...
    """Tear down the manifest's Phase-4.5 artifacts (uninstall step 2.0b).

    Runs AFTER :func:`unwire_event_triggers` and BEFORE any file unlink —
    a unit must stop firing before the script it invokes leaves disk, and
    a failure here must leave the manifest (the resumable checklist) plus
    all files intact.

    Three item kinds from the plan's ``scheduled_teardown``:
      - ``scheduled_unit`` → scheduler-seam ``remove`` (launchd bootout +
        rm plist / systemd ``disable --now`` + unit removal), gated to the
        ``ai.evolve.<bot_id>.*`` label namespace;
      - ``wrapper_file``   → workspace-confined unlink;
      - ``heartbeat_section`` → strip the evolve-managed markdown section.

    Idempotent — re-running after a partial failure re-removes only what
    is still live. Returns ``{ok, results}`` where ``ok`` is False iff
    any item FAILED (skips are benign and never block the uninstall).
    """
    # identity: see resolve_app_id — reads back the pkg_id
    # plan_manifest_deletion put in the plan (annotated there); it is the
    # marker namespace remove_pkg_id_from_marker matches on.
    from .install_helpers import (
        remove_heartbeat_instruction,
        remove_scheduled_units,
    )

    if plan is None or "scheduled_teardown" not in plan:
        plan = plan_manifest_deletion(application_id, bot_id, shared_dir)
    if not plan.get("ok"):
        return {"ok": False, "results": [],
                "error": plan.get("error", "plan failed")}

    items = plan.get("scheduled_teardown") or []
    # identity: see resolve_app_id — the gallery catalog key carried on the deletion
    # PLAN (a plan dict, not a manifest); it is the ``spec=`` marker namespace the
    # teardown below strips, so it must be the value the marker was stamped with.
    pkg_id = plan.get("pkg_id", "")
    results = remove_scheduled_units(
        bot_id,
        [i for i in items if i.get("kind") in ("scheduled_unit", "wrapper_file")],
        network=network,
    )
    for item in items:
        if item.get("kind") != "heartbeat_section":
            continue
        r = remove_heartbeat_instruction(
            bot_id, item.get("file", ""), item.get("section_anchor", ""),
        ...
        results.append({
            ...
            "file": item.get("file"),
            "status": ("ok" if r.get("ok") and r.get("removed")
                       else "skipped" if r.get("ok") else "failed"),
...
def apply_manifest_marker_cleanup(
    ...
    # identity: see resolve_app_id — catalog key off the plan, as above. The falsy
    # early-return is the "nothing was stamped, nothing to strip" guard, which the
    # always-resolving resolver would make unreachable.
    pkg_id = plan.get("pkg_id", "")
    if not pkg_id:
        return plan
...
def unlink_confirmed_files(
    workspace_path: Path,
    bot_user: str,
    confirmed_deletes: list[str],
    plan: dict,
) -> "tuple[list[str], list[str], list[dict]]":
    """Unlink operator-confirmed files (uninstall step 2b/step 4 of the
    :func:`plan_manifest_deletion` sequence). Only paths the plan partitioned
    (``deletion_candidates`` + ``preserved_files``) are eligible — anything
    else in ``confirmed_deletes`` is silently skipped.

    Bot-owned app files evolve cannot unlink directly — a file the bot wrote
    itself (e.g. ``scripts/*.py`` from a forge build/refine round) sits in a
    bot-owned subdir where evolve has ACL read but not write, so the plain
    ``unlink`` raises ``PermissionError`` (live fingerprint: Contacts'
    ``scripts/contacts.py``, Gate-2 gallery sweep 2026-07-11). Those fall back
    to :func:`marker_embed_helper.delete_workspace_file_privileged`, which
    re-validates containment/ownership root-side and unlinks via a held dirfd.
    ``bot_user`` is that helper's expected-bot binding and must therefore be
    the ACCOUNT name (``config.get_bot_user``), never the logical bot_id — the
    helper derives the bot from the destination's own home-directory component,
    so an id refuses every delete on a pod where the two differ.

    Returns ``(actually_deleted, failed_deletes, manual_delete_required)``.
    The first two are workspace-relative paths; a missing file counts as
    deleted (idempotent re-run). ``manual_delete_required`` is
    ``[{"path", "reason"}, ...]`` for files the privileged helper *refused*
    (root-side validation — e.g. not a ``can_app_own`` path, or a directory
    where the manifest expects a file — plus an unlink denied even to root,
    i.e. an immutability flag like macOS ``uchg``) — deterministic for the
    current on-disk state, so no re-run deletes them without operator
    intervention. Callers must NOT treat
    those as retryable failures: blocking the uninstall's finalize on them
    would wedge the resumable checklist forever (PR #3421 follow-up). Only
    ``failed_deletes`` (transient/environmental: EACCES with no working
    helper, I/O error) should keep the manifest on disk for a re-run.
    """
    from .marker_embed_helper import delete_workspace_file_privileged

    allowed = set(plan.get("deletion_candidates", []) + plan.get("preserved_files", []))
    actually_deleted: list[str] = []
    failed_deletes: list[str] = []
    manual_delete_required: list[dict] = []
    for rel in confirmed_deletes:
        if rel not in allowed:
            continue  # safety: only delete files the plan partitioned
        fpath = workspace_path / rel
        try:
            if fpath.exists():
                fpath.unlink()
            ...
            # A directory sitting where the manifest expects a file raises
            # IsADirectoryError (EISDIR) on Linux but PermissionError (EPERM)
            # on macOS — both must reach the helper, whose walk refuses a
            # non-regular leaf with exit 2 → manual_delete_required. Catching
            # only PermissionError left the Linux pod dropping it into the
            # generic handler below → failed_deletes → finalize wedged
            # forever on a condition no re-run clears.
            try:
                ok, detail, refused = delete_workspace_file_privileged(
                    fpath, bot_user=bot_user)
            except Exception:  # helper never raises by contract; belt-and-suspenders
                ok, detail, refused = False, "privileged helper raised", False
            if ok:
                actually_deleted.append(rel)
            elif refused and not fpath.exists():
                # Gone between the EACCES and the helper run (the helper
                # refuses a missing leaf with exit 2) — count as deleted,
                # not as residue the operator must hunt for.
                actually_deleted.append(rel)
            elif refused:
                manual_delete_required.append({"path": rel, "reason": detail})
            else:
                failed_deletes.append(rel)
        except Exception:
            failed_deletes.append(rel)
    return actually_deleted, failed_deletes, manual_delete_required
...
def finalize_manifest_deletion(
    ...
    bot_id: str,
    shared_dir: Path,
    manual_delete_leftovers: "list[dict] | None" = None,
) -> dict:
    ...
    ``manual_delete_leftovers`` (``[{"path", "reason"}, ...]`` from
    :func:`unlink_confirmed_files`) records files the privileged delete
    helper permanently refused — the uninstall completed around them, and
    the archived manifest copy is stamped with the list so the on-disk
    residue stays attributable after the live manifest is gone.
    """
    from .file_index import rebuild_file_index
    ...
    try:
        ...
        try:
            ...
            if manual_delete_leftovers:
                # Annotate the archived copy (never the live manifest — it is
                # about to be unlinked). Copy-then-annotate via same-dir temp +
                # rename: if this rewrite fails at any point, the plain
                # archive above still exists intact.
                import os as _os
                data = json.loads(archive_path.read_text())
                data["manual_delete_leftovers"] = manual_delete_leftovers
                tmp_archive = archive_path.with_name(archive_path.name + ".tmp")
                tmp_archive.write_text(json.dumps(data, indent=2))
                _os.replace(tmp_archive, archive_path)
        except Exception:
            pass
    ...
    # Capability-index resync (audit S4 fix 3): drop the uninstalled app
    # from INSTALLED_APPS.md + the AGENTS.md Tier-1 section now, not at
    # the next install/scan — a stale expand_app menu entry points the
    # bot at an app that no longer exists. Best-effort, same posture as
    # the lifecycle routes' guidance resync.
    guidance_resynced = False
    try:
        from .app_registry import regenerate_installed_apps_md
        guidance_resynced = regenerate_installed_apps_md(bot_id, shared_dir) is not None
    except Exception as exc:
        import logging
        logging.getLogger(__name__).warning(
            "INSTALLED_APPS.md resync after uninstall of %s/%s failed: %s",
            bot_id, application_id, exc,
        )

    return {"ok": True, "guidance_resynced": guidance_resynced}


def mark_manifest_dormant(
    application_id: str,
    bot_id: str,
    shared_dir: Path,
) -> dict:
    """Flip a manifest to ``status = dormant`` in place, keeping it as provenance.

    The keep-data uninstall scope (admin UI wizard) removes an app's
    scripts + scheduled units but leaves its data/state files on disk.
    Those files need a manifest to explain what they are, so the uninstall
    must NOT delete the manifest — it marks it ``dormant`` instead. This is
    the ``keep_manifest`` branch of ``delete_application``: the mirror of
    ``finalize_manifest_deletion`` (which deletes the manifest).

    Writes the raw JSON dict back (not via ``ApplicationManifest.to_dict``)
    so a v7-arc Instance keeps its on-disk shape — the same reason the
    PATCH route deep-merges raw dicts instead of round-tripping the
    dataclass. Only ``status`` + ``updated_at`` change.
    """
    manifest_path = applications_dir(shared_dir, bot_id) / f"{application_id}.json"
    if not manifest_path.exists():
        return {"ok": False, "error": "manifest not found"}
    ...
    try:
        ...
        return {"ok": False, "error": f"could not read manifest: {exc}"}
    ...
    try:
        _write_manifest_bytes(
            manifest_path, json.dumps(data, indent=2).encode("utf-8")
        )
    except Exception as exc:
        return {"ok": False, "error": f"could not write manifest: {exc}"}

    # The app's files stay claimed by the (now dormant) manifest; rebuild
    # the index best-effort so its status row reflects the flip.
    try:
        _rebuild_file_index_for(shared_dir)
    except Exception as exc:
        import logging
        logging.getLogger(__name__).warning(
            "file-index rebuild after dormant-mark of %s/%s failed: %s",
            bot_id, application_id, exc,
        )

    # Drop the dormant app from INSTALLED_APPS.md + the AGENTS.md Tier-1
    # section — dormant is not a visible status, so its guidance goes dark,
    # same as finalize_manifest_deletion does on delete.
    guidance_resynced = False
    try:
        from .app_registry import regenerate_installed_apps_md
        guidance_resynced = regenerate_installed_apps_md(bot_id, shared_dir) is not None
        ...
        import logging
        logging.getLogger(__name__).warning(
            "INSTALLED_APPS.md resync after dormant-mark of %s/%s failed: %s",
            bot_id, application_id, exc,
        )

    return {"ok": True, "status": "dormant", "guidance_resynced": guidance_resynced}
...
def _populate_provenance_v20(data: dict) -> bool:
    ...
    Spec: internal/spec-app-coherence-and-reconciliation-2026-06-05.md §4 +
    §13.1.
...
def _migrate_scheduled_action_entry_v17(entry: dict) -> bool:
    ...
    Spec: internal/spec-heartbeat-instruction-2026-06-03.md §2.1, §9.
...
def _normalize_manifest_shape(data: dict) -> bool:
    ...
    if shape in VALID_MANIFEST_SHAPES:
        return False
    # identity: see resolve_app_id — a SHAPE PROBE, not an id read: the
    # presence of ``instance_id`` is what distinguishes a v7-arc Instance
    # from a legacy single-file manifest. A canonical ``app_id`` is stamped
    # on both shapes, so it carries no discriminating signal here.
    inferred = (
    ...
    print(
        ...
        # identity: see resolve_app_id — a log breadcrumb naming the FILE
        # being healed, so it stays the readable stem rather than the
        # pkg_id-first canonical id an operator cannot match to a filename.
...
def migrate_manifest(path: Path) -> None:
    ...
    defaults: dict[str, Any] = {
        ...
        # app-test surface killed per internal/decision-app-tests-2026-06-08.md.
        ...
        # internal/spec-manifest-v7-2026-05-20.md.
        ...
        # before). Spec: internal/spec-backup-and-data-classification-2026-05-28.md.
        "app_files_privacy": "",
        "data_paths": [],
        ...
        # and internal/spec-forge-side-effects-2026-06-02.md §4.
        #
        ...
        # internal/spec-app-coherence-and-reconciliation-2026-06-05.md.
        #
        ...
        # internal/spec-manifest-v7-slicing-2026-06-10.md §4.3.
        "privacy": {},
        "audience_scoping": {},
        # v29: provided_capabilities[] — the app-declared half of the Layer-2
        # per-role tool-loading model. Inert default [] = "not yet declared":
        # a migrated legacy manifest doesn't silently acquire capability
        # declarations. Spec: internal/spec-user-roster-and-roles-2026-06-07.md §4/§8.
    ...
    # Spec: internal/spec-heartbeat-instruction-2026-06-03.md §9.
    ...
    # v30: backfill the canonical app_id from the legacy chain (AL-1.4a).
    # Populate-on-absent, and the value written is exactly what
    # resolve_app_id already returned for this manifest — so the stamp cannot
    # change any reader's answer. Unconditional on definition_status: this
    # manifest already EXISTS, so its id is load-bearing wherever it is already
    # written down (filename, workspace markers, cron labels, the plugin's
    # app-integrity coverage keys) regardless of whether an operator has
    # vouched for it. The discovered-drafts-have-no-app_id rule (design §3)
    # binds at the MINT boundary instead — see app_identity.stamp_identity's
    # ``mint`` flag and the scanner's two first-write sites. Runs before the
    # provenance population below so field_origins captures app_id.
    if not data.get(APP_ID_FIELD) and ensure_app_id(data, context=str(path)):
        changed = True
