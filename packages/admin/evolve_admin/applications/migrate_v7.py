...
Per internal/spec-manifest-v7-2026-05-20.md §10.
...
identity: see resolve_app_id — this is a MIGRATION module, one of the two the
AL-1.4b grep gate exempts (build-AL-1.4-app-id-canonical.md §3). It is the
code that MINTS the v7-arc namespace, so ``spec_id`` / ``instance_id`` /
``pkg_id`` appear here as the fields being CREATED, not as an identity being
read: ``_extract_spec`` writes a Spec's ``spec_id``, ``_extract_instance``
writes an Instance's ``instance_id`` (== the v13 filename stem, so URLs and
``load_manifest`` keys survive the split) and its
``provenance.spec_id``/``prior_spec_ids[]``, and the gallery path builders
key on the ``spec_id`` those writes just assigned. Resolving any of them
through ``resolve_app_id`` would ask a manifest for an id the migration has
not conferred yet. AL-1.4a already wired the stamper (``stamp_identity`` /
``canonical_app_id``) into those write sites; AL-1.4b's one sweep here is
``_migrate_app_dependencies``, which reads ANOTHER app's id (annotated at its
definition).
"""
...
# AL-1.4a identity helpers — the ONE resolver/stamper for app_id (and the
# draft_id a discovered draft gets instead). stdlib-only, imports nothing from
# this package, so no cycle.
from .app_identity import canonical_app_id, is_discovered, resolve_app_id, stamp_identity
...
# nothing re-seeded it — see internal/decision-add-bot-m4-u1-proof-2026-06-11.md
...
def _extract_spec(
    ...
    *,
    ...
    """
    # v30 / AL-1.4a: app_id IS the spec identity (design §3 — "spec_id ==
    # app_id"). Through the canonical gate, so a spec_id that is not a
    # conforming slug is reported by ``migrate-ids`` rather than silently
    # rewritten into a different id than the gallery path, workspace markers
    # and provenance.spec_id all already use — and omitted rather than written
    # empty, so "absent" keeps meaning "no identity conferred".
    spec_app_id = canonical_app_id(spec_id, context=f"spec {spec_id}")
    spec: dict[str, Any] = {
        "spec_id": spec_id,
        **({"app_id": spec_app_id} if spec_app_id else {}),
    ...
    # provided_capabilities (v29) — the app-declared half of the Layer-2
    # per-role tool-loading model (spec-user-roster-and-roles-2026-06-07 §4/§8).
    # Spec-owned declaration; passthrough verbatim so a migrated legacy manifest
    # that declared capabilities carries them onto the v7-arc Spec (both shapes
    # stay valid — slicing spec §2.3 rule 1). Absent/empty → not written.
...
def _extract_instance(
    ...
    *,
    ...
    ``mint`` (AL-1.4a) marks a FIRST write of an app that did not exist
    before — only the scanner's detection mint passes it. It is what gives a
    born-discovered draft a ``draft_id`` instead of an ``app_id`` (design §3);
    the migration and forge-conversion callers leave it False because their
    input is an app that already exists on disk.
    """
    instance: dict[str, Any] = {
        ...
        "provenance": {
            ...
            # back-compat: absent reads as []. See internal/spec-manifest-v7-2026-
            # 05-20.md §5.
            "prior_spec_ids": [],
        },
    ...
    # v30 / AL-1.4a identity stamp. ``mint=True`` is passed only by the
    # scanner's native mint (native_write.mint_scanner_detection): a
    # scanner-born DRAFT gets a ``draft_id`` and no ``app_id``, because
    # identity is conferred by promotion, not by discovery (design §3). Every
    # other caller — the v13→v7 migration, forge install conversion — is
    # re-shaping a manifest that already exists, so it gets the app_id its
    # legacy chain already resolves to.
    #
    # NB the id stamped here is the INSTANCE id, not ``provenance.spec_id``.
    # Design §3 ends with instance identity being the pair (app_id, bot_id)
    # where app_id == the spec's — but collapsing them now would change what
    # resolve_app_id/appIdOf return for every v7-arc instance on the pod (they
    # resolve ``instance_id`` today), which is exactly the behavior change
    # 1.4a must not make. The collapse rides with AL-1.5's v-next spec and
    # 1.4c, using the migration table this stage produces.
    # ``discovered`` is read off the SOURCE dict: definition_status is folded
    # onto the Instance by native_write's passthrough AFTER this returns, so
    # reading it off ``instance`` here would see nothing and treat a
    # re-discovered *defined* app as a fresh draft — stripping the app_id it
    # has already been vouched for.
    stamp_identity(
        instance,
        mint=mint,
        discovered=is_discovered(v13),
        seed=instance_id,
...
def _migrate_app_dependencies(raw: list) -> list[dict]:
    ...
    AL-1.4b: a dependency entry NAMES ANOTHER APP, so its id is resolved
    through ``app_identity.resolve_app_id`` rather than by a hand-rolled
    field chain. This is the WRITE side of the pair whose read side area 3
    already swept (``coherence_pass_a._declared_dependency_keys``); the two
    now agree by construction instead of by two copies of the same chain.
    Behavior-neutral: neither declared shape carries ``id`` or
    ``instance_id`` (the ea-pack shape is ``{pkg_id, display_name, required,
    reason}``, the already-v7 shape ``{spec_id, required, purpose}``), so the
    resolver returns exactly the string ``pkg_id or spec_id`` returned — and
    it now also honors a canonical ``app_id`` the moment writers emit one on
    a dependency entry. ``_SPEC_ID_RE`` still gates the output either way.
    """
    out: list[dict] = []
    for entry in raw:
        if isinstance(entry, str):
...
def reseed_builtin_specs(
    ...
    *,
    ...
    internal/decision-add-bot-m4-u1-proof-2026-06-11.md Resolution note + #2792.
