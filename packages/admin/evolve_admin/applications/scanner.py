...
class MissingApiKeyError(RuntimeError):
    """LLM phase requested but no Anthropic API key is reachable.

    Raised by ``llm_discover_applications`` when ``use_llm=True`` and the
    bot's auth-profiles.json contains no usable Anthropic key. Catchers
...
def _resolve_tier(tier: str = "tier3") -> str:
    """Resolve a tier reference to the currently configured model string.
    Falls back to the tier's hardcoded default on any import error.
    """
    try:
        from evolve_config import load_config  # type: ignore
        from models import resolve_tier  # type: ignore
        config = load_config()
        return resolve_tier(tier, config)
    except Exception:
        # Hardcoded fallbacks — only used if analyzer package is unavailable
        _defaults = {
            "tier1": "anthropic/claude-opus-4-6",
            "tier2": "anthropic/claude-sonnet-4-6",
            "tier3": "anthropic/claude-haiku-4-5",
        }
        return _defaults.get(tier, "anthropic/claude-haiku-4-5")


def _resolve_model_name(model: str) -> str:
    """Strip 'anthropic/' prefix if present — Anthropic API doesn't want it."""
    return model.removeprefix("anthropic/")


def _read_api_key(bot_id: str, user: str | None = None) -> str:
    """
    Read the bot's Anthropic API key. Falls back to ANTHROPIC_API_KEY env var.

    ``user`` is the actual OS username (from network.json); falls back to bot_id.

    The SOURCE read is delegated to ``oc_store``, which knows the post-2026.6
    OpenClaw layouts (per-agent sqlite store → legacy auth-profiles.json →
    transitional .sqlite-import bak) and tolerates profile-key variance
    (``anthropic:api`` vs ``anthropic:api_key``) via ``primary_bot``. Returns
    "" when no key resolves — the caller degrades to a structural scan rather
    than hard-failing (see scan_workspace_pipeline).
    """
...
def _call_anthropic(model: str, prompt: str, api_key: str, timeout: int = 60) -> str:
    """
    Make a direct Anthropic API call. Returns the text response, or "" on failure.
    Uses urllib (stdlib only) — no extra deps.
    """
    import urllib.request as _req
    import urllib.error as _uerr

    model_id = _resolve_model_name(model)
    payload = {
        "model": model_id,
        "max_tokens": 4096,
    ...
    try:
        ...
        print(f"[scanner] Anthropic API HTTP {e.code}: {body}", flush=True)
        return ""
    except Exception as e:
        print(f"[scanner] Anthropic API error: {e}", flush=True)
        return ""
...
# the injectable ``_call_anthropic`` seam so tests run fully offline.
...
def llm_discover_applications(
    ...
    """
    Phase 2 — ask tier3 LLM to identify and cluster applications from the
    rich workspace inventory. Returns DetectedApplication list (source='llm-inferred').
...
[{{"id": "...", "name": "...", "description": "...", "confidence": 0.9,
  "evidence_files": ["ops/tools/unified_task_system.py", "ops/tasks/unified_tasks/tasks.json", "ops/tools/task_runner.py"]}}]"""
    ...
    api_key = _read_api_key(inventory.bot_id, user=inventory.user)
    print(f"[scanner] API key found: {'yes' if api_key else 'NO'}", flush=True)
    print(f"[scanner] LLM call: Anthropic API model={model} prompt_chars={len(context)}", flush=True)
    if not api_key:
        ...
        raise MissingApiKeyError(
            f"No Anthropic API key reachable for bot {inventory.bot_id!r}. "
            f"Checked ANTHROPIC_API_KEY env and the bot's OpenClaw auth store "
            f"(openclaw-agent.sqlite → legacy auth-profiles.json → .sqlite-import bak). "
            f"Fix: add an api_key-typed Anthropic profile, or re-run the scan with --no-llm."
        )

    try:
        text = _call_anthropic(model, prompt, api_key, timeout=60)
...
def generate_manifest_for_app(
    ...
    *,
    ...
    Uses tier3 LLM with rich context from the inventory.
    Falls back to a stub manifest if LLM fails or times out.
...
{{
  ...
  "test_cases": [
    ...
    if api_key:
        try:
            text = _call_anthropic(model, prompt, api_key, timeout=90)
            if text:
                start = text.find("{")
                end = text.rfind("}") + 1
...
def scan_workspace(
    ...
    if use_llm:
        model = _resolve_tier("tier3")
        llm_results = llm_discover_applications(inventory, model)
...
def scan_workspace_pipeline(
    ...
    model = _resolve_tier("tier3") if use_llm else ""
    # Purpose/fit classifier (bite D1) routes through a stronger tier than the
    # tier3 discovery above (see _stamp_app_kind). api_key is resolved once
    # below (after os_user is known) and shared by the mint gate + reconcile
    # reclassification pass.
    classifier_model = _resolve_tier(_CLASSIFIER_TIER) if use_llm else ""
    classifier_api_key = ""
    # Set when an LLM scan was requested but no Anthropic key resolves — we
    ...
    if use_llm:
        # Resolve the Anthropic key ONCE here (shared by LLM discovery, the
        # mint gate, and the reclassification pass). If no key resolves —
        # e.g. a bot that authenticates to Anthropic via the gateway token and
        # never had a raw key, or the post-2026.6 OpenClaw migration that moved
        # auth-profiles.json into the per-agent sqlite store — DEGRADE to a
        # structural (--no-llm) scan instead of aborting at Phase 2 with
        # MissingApiKeyError. The structural scan still inventories the
        # workspace and stamps stub manifests; the operator gets a clear
        # "degraded — no Anthropic key" note rather than a red terminal error.
        try:
            classifier_api_key = _read_api_key(bot_id, user=os_user)
        ...
        if not classifier_api_key:
            ...
            model = ""
            classifier_model = ""
            llm_degraded_reason = "no_anthropic_key"
            _slog(
                "[scanner] No Anthropic API key reachable — degrading to a "
                "structural (--no-llm) scan (checked the sqlite auth store, "
                "legacy auth-profiles.json, and the env var)."
            )
    print(
    ...
    def _gen_and_save(app: DetectedApplication) -> str:
        ...
        try:
            manifest = generate_manifest_for_app(
                ...
                repair=repair, repair_log=_scan_log,
    ...
    if llm_degraded_reason:
        # The scan completed structurally because no Anthropic key resolved.
