...
def _stamp_conversation_evidence(manifest: dict, evidence: dict) -> None:
    """Mark a freshly stubbed manifest as conversation-only evidence.
...
def scan_workspace_pipeline(
    ...
    # Matching it is still worth doing: it is what stops a second draft being
    # minted for an app the scanner already found in the workspace.
    ...
    for d in conversation_apps:
        ...
        if match is not None:
            _slog(
                ...
                f"manifest ({match['path'].name}); not drafting a second one"
            )
            continue
