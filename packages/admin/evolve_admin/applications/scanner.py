...
Pipeline (Python-orchestrated; LLM answers questions, never drives):
  ...
  Phase 1 — Inventory    : filesystem walk + schedules (crontab + OpenClaw's own
                           cron store) + memory files. Fast, no LLM. (~5s)
...
# The pod's one reader of OpenClaw's own cron store (jobs.json, else the
# ``cron_jobs`` SQLite table). Same package, no cycle — cron_manager imports
# only ``..config``.
from .cron_manager import read_oc_cron_jobs
...
class WorkspaceInventory:
    ...
    #                     source, job_name}  — ``source`` is "crontab" | "openclaw"
...
def _is_defined(manifest: dict) -> bool:
    ...
    internal/spec-apps-meta-2026-06-13.md §9).
...
def _load_bound_spec(manifest: dict, shared_dir: Path | None) -> dict:
    ...
    # identity: see resolve_app_id — NOT swept (AL-1.4b). ``provenance.spec_id``
    # is the Instance->Spec BINDING and half of the gallery path key
    # ``gallery/<tier>/<spec_id>/<spec_version>.json``; it is nested, and
    # ``resolve_app_id`` reads top level only. The pair must stay together.
...
def collect_inventory(workspace: Path, bot_id: str) -> WorkspaceInventory:
    ...
    Reads schedules (the crontab via sudo, plus OpenClaw's own cron store),
    SOUL.md, all scripts, and markdown files.
    ...
    inv.cron_entries = [
        # ``label`` is the OpenClaw job's own name where there is one (crontab
        # lines have none) — the same key ``cron_manager`` matches an app's
        # installed crons on.
        {
            "schedule": j["schedule"],
            "script": j["script_path"],
            "label": j.get("job_name", ""),
            "file_id": "",
        }
...
def _collect_crons(bot_id: str, workspace: Path) -> list[dict]:
    """Every scheduled job the scanner can see for *bot_id*, as cron evidence.

    Two sources, each optional and each degrading to nothing:

    * the user crontab (``crontab -l`` run as the bot user), and
    * **OpenClaw's own cron store** — ``~/.openclaw/cron/jobs.json``, else the
      ``cron_jobs`` table in ``~/.openclaw/state/openclaw.sqlite``.

    The second source is the one a stranger's pod actually uses.  An operator
    who schedules work through OpenClaw has an empty crontab, so reading only
    ``crontab -l`` reported ``cron_jobs=0`` on pods carrying a dozen live
    schedules — and cron is the strongest evidence class discovery has
    (``docs/help/apps.md``; ALPHA journey audit F1,
    ``internal/audit-alpha-journey-2026-08.md`` §4.1).

    Entries from both sources carry the same evidence shape, so every
    downstream consumer (the discovery prompt, the manifest prompt, the
    infrastructure filter, ``cron_entries``) is unchanged.  A bot whose store
    is absent or unreadable contributes nothing and never fails the scan.
    """
    crons = _collect_crontab_crons(bot_id)
    seen = {c["script_path"] for c in crons}
    for job in _collect_openclaw_crons(bot_id, workspace):
        # A job scheduled in BOTH surfaces (an Evolve app cron that also has a
        # hand-written crontab line) is one habit, not two.
        if job["script_path"] in seen:
            continue
        seen.add(job["script_path"])
        crons.append(job)
    return crons


def _collect_crontab_crons(bot_id: str) -> list[dict]:
...
# A path-shaped token naming a script, as it appears inside an OpenClaw cron
# payload — ``python3 /Users/x/.openclaw/workspace/standup/digest.py --post``
# or ``bash ~/bin/nightly.sh``.  Anchored on the extension because that is the
# only thing that distinguishes a path from prose in an agent-turn message.
_OC_CRON_SCRIPT_RE = re.compile(r"[\w./~+-]+\.(?:py|sh)\b")


def _oc_cron_payload_text(payload: dict) -> str:
    """The instruction text of an OpenClaw cron payload, whatever its kind.

    ``command`` payloads carry ``argv``, ``agentTurn`` a ``message``,
    ``systemEvent`` a ``text``.  Read whichever key is present rather than
    switching on ``kind``: OpenClaw's kind vocabulary has already changed once
    (the 2026.7 migration), and a kind this function does not know about still
    has its text mined for evidence instead of being dropped.
    """
    parts: list[str] = []
    argv = payload.get("argv")
    if isinstance(argv, list):
        parts.extend(str(a) for a in argv)
    for key in ("message", "text", "command", "script"):
...
def _oc_cron_schedule_str(job: dict) -> str:
    """Render an OpenClaw schedule object as the human string cron evidence uses.

    ``{"kind":"cron","expr":"0 9 * * 1-5","tz":"America/Los_Angeles"}`` reads
    back as ``0 9 * * 1-5 (America/Los_Angeles)`` — close enough to a crontab
    line that the same downstream prompt text works for both.  A bare string
    schedule (pre-2026.7 shape) passes through.
    """
    ...
    if kind == "cron":
        ...
        tz = sched.get("tz")
        if expr and isinstance(tz, str) and tz.strip():
    ...
    if kind == "every":
        ...
        return f"every {minutes}m" if minutes else f"every {int(ms)}ms"
    if kind == "at":
        at = sched.get("at")
        return f"at {at}" if at else "at"
    return kind


def _oc_cron_script_path(token: str, bot_id: str, workspace: Path) -> Path:
    """Resolve a script token mined from a cron payload to an absolute path."""
    if token.startswith("~/"):
        return bot_home(bot_id) / token[2:]
    p = Path(token)
    return p if p.is_absolute() else workspace / token


def _collect_openclaw_crons(bot_id: str, workspace: Path) -> list[dict]:
    """Cron evidence from OpenClaw's own store, in the shared evidence shape.

    Silent skip on every failure path — an unreadable or absent store logs one
    line and contributes nothing, because "I could not see this bot's
    schedules" must never be scanned as "this bot has none".
    """
    try:
        jobs = read_oc_cron_jobs(bot_id)
    except Exception as e:  # defensive: the scan must never die on a store read
        _log.warning("scanner: OpenClaw cron store read failed for %s: %s", bot_id, e)
        print(f"[scanner] OpenClaw cron store unreadable for {bot_id}: {e}", flush=True)
        return []
    if jobs is None:
        print(
            f"[scanner] OpenClaw cron store absent/unreadable for {bot_id} "
            f"— no schedule evidence from it (crontab still read)",
            flush=True,
        )
        return []
    ...
    crons: list[dict] = []
    for job in jobs:
        # OpenClaw spells "off" as enabled=false; Evolve's pause/resume path
        # (cron_manager) spells it disabled=true. A job that cannot fire is
        # the OC-store equivalent of a commented-out crontab line.
        if job.get("enabled") is False or job.get("disabled") is True:
            continue
        payload = job.get("payload")
        text = _oc_cron_payload_text(payload) if isinstance(payload, dict) else ""
        if not text:
            for key in ("script", "command", "task"):
                value = job.get(key)
                if isinstance(value, str) and value.strip():
                    text = value.strip()
                    break
        name = str(job.get("name") or job.get("id") or "").strip()
        match = _OC_CRON_SCRIPT_RE.search(text)
        if match:
            script_path = match.group(0)
            content = _safe_read_as_bot(
                _oc_cron_script_path(script_path, bot_id, workspace), max_chars=3000
            ) or text[:3000]
        else:
            # No script behind it — an agent-turn or system-event job. The
            # instruction text IS the evidence; give it a stable synthetic
            # path so dedup and the infra-name filter both still work.
            if not name and not text:
                continue
            script_path = f"openclaw:cron/{name or 'unnamed'}"
            content = text[:3000]
        crons.append({
            "schedule": _oc_cron_schedule_str(job),
            "script_path": script_path,
            "script_content": content,
            "is_infrastructure": _is_infrastructure_script(content, script_path),
            "source": "openclaw",
            "job_name": name,
        })
    return crons
...
def _attribute_instruction_to_app(
    ...
    if not isinstance(section, dict):
        return False
    # identity: see resolve_app_id — NOT swept (AL-1.4b). ``section`` is a
    # parsed HEARTBEAT.md/AGENTS.md managed-section marker, not a manifest:
    # its ``pkg_id=`` attribute is the literal string the forge wrote into the
    # bot's markdown, so the comparison must stay in the marker's namespace.
...
class DetectedApplication:
    ...
    source: str = "detected"
    #: Design §7.1a — set only on a conversation-only detection (a recurring
    #: ask with no file, cron or memory structure behind it). Carries the
    #: recurrence arithmetic onto the manifest, and switches Phase 4 onto the
    #: deterministic stub path: there is no file here for an LLM to read, so
    #: sending one would be paying tier3 to hallucinate an implementation.
    #: The purpose/fit classifier is NOT skipped — it judges on name + prose,
    #: which is all a conversation draft has, and a "system" verdict on
    #: "openclaw heartbeat poll" (the noise class AL-1.6c measured on the pod)
    #: is exactly the discrimination that gate exists for.
    conversation_evidence: dict[str, Any] | None = None


# ── Conversation-only evidence (design §7.1 / §7.1a) ─────────────────────────

#: Manifest field the readiness scorer already reads for design §7.1's
#: lowest evidence rung (``app_readiness.EVIDENCE_TIERS``, weight 0.2).
#: AL-1.6b built that read tolerantly against a carrier nothing wrote yet;
#: this is the producer. No weight or threshold there is touched here.
CONVERSATION_ONLY_FIELD = "conversation_only"
#: Where the arithmetic that justified the draft is kept, so an operator
#: opening the draft can see WHY it exists without re-running the detector.
CONVERSATION_EVIDENCE_FIELD = "conversation_evidence"


def _stamp_conversation_evidence(
    manifest: dict, evidence: dict, *, conversation_only: bool = True,
) -> None:
    """Record §7.1a's recurrence arithmetic on a manifest.

    Two fields, both additive. ``evidence`` (the legacy mapping of
    ``{files: [...]}``) is deliberately NOT touched: ``routes_analytics``
    and ``app_readiness`` both read it as the file-evidence carrier, and
    appending a differently-shaped entry there to say "conversation" would
    be re-purposing a live field to carry a new meaning.

    ``conversation_only`` is the EVIDENCE-CLASS marker, and it is a claim about
    the whole manifest rather than about this block. ``app_readiness`` reads it
    as design §7.1's lowest evidence rung. So it is set on a freshly stubbed
    conversation draft — "no file exists yet" IS that class — and NOT when the
    block is being merged onto a manifest the scanner already backs with files
    or a verified cron. That manifest is not conversation-only, and the reason
    to withhold the marker is that it would be **false**, not that it would
    move a number.

    The arithmetic, stated accurately (independent review of PR #3805, finding
    3, correcting an earlier overstatement here). Setting the marker adds a
    second evidence rung, so ``_evidence_dimension`` pays one
    ``CORROBORATION_BONUS``. On an ``authored_code`` manifest the tier is
    already 1.0 and the bonus is clipped by ``min(1.0, …)``, so the composite
    does not move at all (81 → 81). It bites only BELOW the top rung — a
    ``scheduled_verified`` draft goes 69 → 72. So the marker is not load-bearing
    for the flagship case; it is a false statement about the manifest that would
    inflate a lower-rung one. Withhold it because it is untrue, and note that no
    test may pin this by score alone on a top-rung fixture — assert the tier
    set.

    Assignment, never append: re-stamping on a later scan REPLACES the block, so
    ``days_seen`` tracks the current window and no duplicate entry accumulates.
    """
    if conversation_only:
        manifest[CONVERSATION_ONLY_FIELD] = True
    manifest[CONVERSATION_EVIDENCE_FIELD] = dict(evidence)


def _merge_conversation_evidence_into_manifest(
    path: Path, evidence: dict, *, log=None,
) -> str:
    """Stamp conversation recurrence onto a manifest that already exists.

    ALPHA-4 (audit ``internal/audit-alpha-journey-2026-08.md`` finding B5). Phase
    3b used to ``continue`` here, discarding the recurrence it had just measured
    because the app already had a manifest. That discard is why no draft on any
    pod could reach ``eligible_to_offer``: a file/cron draft measures evidence and
    nothing else (one dimension, under ``MIN_DIMENSIONS_FOR_OFFER``), a
    conversation-only draft composites to at most 50 against a threshold of 75,
    and the one shape that clears the gate on merit — files AND recurrence — was
    the shape this line prevented from ever existing.

    Nothing here moves a threshold or a weight. It writes the number §7.1a
    already computed onto the manifest that already exists, so
    ``app_readiness._recurrence_dimension`` can measure the same draft twice: once
    on its files, once on the fact that its user keeps asking for it.

    The evidence-class marker is deliberately withheld (see
    ``_stamp_conversation_evidence``) — this manifest is not conversation-only.

    Returns ``"stamped"``, ``"unchanged"`` (the block on disk already equals this
    one, so no write is made) or ``"failed"``. Never raises: a scan must not die
    because one manifest is unreadable, and the next scan re-measures anyway.
    """
    try:
        data = json.loads(path.read_text())
        if not isinstance(data, dict):
            raise ValueError("manifest is not a JSON object")
    except Exception as exc:  # noqa: BLE001 — never block a scan on one manifest
        if log is not None:
            log(f"[scanner]   conversation evidence: cannot read {path.name}: {exc}")
        return "failed"

    fresh = dict(evidence)
    if data.get(CONVERSATION_EVIDENCE_FIELD) == fresh:
        return "unchanged"

    _stamp_conversation_evidence(data, fresh, conversation_only=False)
    try:
        _atomic_write(path, data, mode=0o644)
    except Exception as exc:  # noqa: BLE001 — same reason as the read above
        if log is not None:
            log(f"[scanner]   conversation evidence: cannot write {path.name}: {exc}")
        return "failed"
    return "stamped"


def _conversation_detections(
    shared_dir: Path,
    bot_id: str,
    log=None,
) -> list["DetectedApplication"]:
    """Adapt ``conversation_recurrence`` matches into scanner detections.

    The arithmetic, the ±2h hour cluster, the ≥5-of-10-days gate and the
    per-bot do-not-track switch all live in that module — this function is
    the adapter and nothing else, so the detector stays independently
    runnable and testable (``python3 -m
    evolve_admin.applications.conversation_recurrence --json``).

    ``min_confidence`` is deliberately NOT applied to these. That gate is
    the LLM discovery confidence; §7.1a's gate is ``min_days``, already
    applied inside the detector. Running a second, unrelated threshold over
    a days-seen ratio would silently suppress design-valid drafts on any
    pod whose operator raised ``--min-confidence``.

    Never raises: a scan must not die because ``{shared_dir}/annotations``
    is unreadable. On failure it contributes nothing, which is the same
    thing an empty annotations dir contributes.
    """
    ...
    try:
        ...
        # Read the switch here as well as inside the detector, only so the log
        # can say WHICH zero this is. "nothing cleared the threshold" printed
        # over a pod where the operator turned observation off is the kind of
        # reassuring-but-wrong line that makes an operator debug the detector
        # instead of reading their own config.
        enabled = recurring_request_signal_enabled(shared_dir, bot_id)
        found = conversation_detections(shared_dir, bot_id) if enabled else []
    except Exception as exc:  # noqa: BLE001 — never block a scan on evidence
        _slog(f"[scanner] conversation evidence unavailable: {exc}")
        return []
    # The second conversation-only signature (artifact-by-reference, part 3):
    # a document the assistant has rebuilt N times. Same evidence class, same
    # per-bot switch, same adapter below — a separate try so an unreadable
    # session tree costs this signature and not the recurring-request one.
    rebuilt: list = []
    if enabled:
        try:
            from .artifact_reemission import reemission_detections
            rebuilt = reemission_detections(shared_dir, bot_id)
        except Exception as exc:  # noqa: BLE001 — never block a scan on evidence
            _slog(f"[scanner] re-emission evidence unavailable: {exc}")
    found = list(found) + list(rebuilt)

    if not enabled:
        _slog(f"[scanner] Phase 3b: conversation evidence OFF for {bot_id} "
              f"(bots.{bot_id}.recurringRequestSignal = false)")
        return []
    if not found:
        _slog("[scanner] Phase 3b: conversation evidence — no recurring request "
              "cleared the ≥5-of-10-days threshold, and no rebuilt document "
              "cleared the ≥3-in-14-days one")
        return []

    out: list[DetectedApplication] = []
    for det in found:
        # ── The infra floor, applied BEFORE the mint (independent review of
        # this PR, finding 1). `_archive_platform_file_only_stubs` Rule 4
        # already recognises this exact shape — but it runs after Phase 4, and
        # `mint_scanner_detection` has by then published a Spec carrying a
        # durable ``p-`` id AND a lessons file, neither of which the archive
        # reclaims. `_stub_manifest` sets no ``pkg_id``, so every scan mints a
        # FRESH random ``p-``: an hourly heartbeat label clears 5-of-10 days
        # trivially, and three scans of one such habit measurably deposited
        # three orphan Specs plus three lessons files.
        #
        # Rule 4's own docstring says "the Phase-2 floor prevents any from
        # recurring" — that floor lives inside `llm_discover_applications`,
        # which conversation detections bypass by construction. This re-applies
        # it on the path that bypassed it, using the SAME two predicates, so
        # the two layers cannot drift apart. Rule 4's third condition
        # (`_asserts_recurring_behavior`) is true of every conversation draft
        # by construction, so matching the infra half here reproduces Rule 4's
        # verdict for this class exactly, one phase earlier. Rule 4 stays as
        # the backstop for anything that reaches disk another way.
        identity_text = f"{det.name} {det.evidence.get('label') or ''}"
        if _describes_pod_infra(identity_text) or _name_is_oc_system_function(det.name):
            _slog(
                f"[scanner]   conversation evidence: {det.detection_id} is "
                f"infra-shaped (realizes nothing + infra signal) — not drafted"
            )
            continue
        if det.evidence.get("detector") == "artifact_reemission":
            _slog(
                f"[scanner]   re-emission evidence: {det.detection_id} "
                f"(rebuilt x{det.evidence.get('count')} on "
                f"{det.evidence.get('days_seen')}/{det.evidence.get('window_days')}d)"
            )
        else:
            _slog(
                f"[scanner]   conversation evidence: {det.detection_id} "
                f"({det.evidence.get('days_seen')}/"
                f"{det.evidence.get('window_days')}d "
                f"~{det.evidence.get('center_hour')}:00)"
            )
        out.append(DetectedApplication(
            id=det.detection_id,
            name=det.name,
            confidence=det.confidence,
            # Empty by construction — "no file exists yet" IS the case this
            # evidence class covers. Phase 5's file stamp finds nothing to
            # attach, which is correct and is what keeps this draft on the
            # lowest evidence rung.
            evidence_files=[],
            evidence_summary=det.description,
            suggested_goals=[],
            suggested_tests=[],
            suggested_privacy=[],
            description=det.description,
            source="conversation",
            conversation_evidence=det.evidence,
        ))
    _slog(f"[scanner] Phase 3b: conversation evidence — {len(out)} candidate(s)")
    return out
...
def scan_workspace_pipeline(
    ...
    classifier_model = ""
    classifier_api_key = ""
    # Set whenever this run's Phase 2 does not have a model behind it, for
    # EITHER reason. Recorded in the terminal status so a reader can tell a
    # scan that recognised apps from one that only listed files.
    #
    #   no_llm_provider_key   — an LLM scan was requested and no provider key
    #                           resolved, so we DEGRADE to a structural pass
    #                           rather than aborting with
    #                           error_kind="missing_api_key" (set below).
    #   structural_by_request — the caller asked for --no-llm (the admin UI's
    #                           "Quick scan (evidence only, ~5s)" button). The
    #                           run is not a failure, but its result is still
    #                           a structural pass, and audit B2b establishes
    #                           that a structural pass discovers nothing by
    #                           construction. Without this, a completed quick
    #                           scan wrote a clean terminal status and the
    #                           Discovered tab told the operator everything on
    #                           the pod had been vouched for (ALPHA-2 / U5).
    llm_degraded_reason: str | None = (
        None if use_llm else "structural_by_request"
    )
    ...
    # ── Phase 3b: conversation-only evidence (design §7.1 / §7.1a) ───────────
    # "a recurring conversational pattern with no app behind it ... is a
    # discovered draft even if no file exists yet". The arithmetic lives in
    # conversation_recurrence.py (AL-1.6c); this is the one call site.
    #
    # These join `new_apps` AFTER the existing-manifest match above rather
    # than `merged` before it, for one reason: a detection that DOES match an
    # existing manifest must not enter `existing_app_hits`. That dict feeds
    # the identity/success backfill pass, and a conversation detection has no
    # identity fields to contribute — only a claim that someone keeps asking.
    # Matching it is still worth doing twice over: it is what stops a second
    # draft being minted for an app the scanner already found in the workspace,
    # AND (ALPHA-4) it is how "someone keeps asking" reaches that app's manifest
    # instead of being thrown away.
    conversation_apps = _conversation_detections(shared_dir, bot_id, log=_slog)
    # Canonical keys the LLM pass already accounted for THIS scan — whether it
    # matched them to an existing manifest or is about to create one. A
    # conversation detection that collides with one of these is the same app
    # seen a second way, not a second app.
    llm_canons = set()
    for d in merged:
        llm_canons |= {c for c in (_canon_name(d.id), _canon_name(d.name)) if c}
    # Manifests that have already taken a conversation block THIS scan. Two
    # different recurring labels can canonicalize onto one app ("revenue digest"
    # and "revenue report" both matching ``revenue-digest.json``), and the block
    # is a single field — so without this the last one iterated would silently
    # overwrite the first, and which one that is would depend on detector
    # ordering. `conversation_detections` sorts best-first (``-days_seen``,
    # ``-occurrences``), so first-wins means the STRONGEST recurrence is the one
    # the manifest keeps, and it is the same answer on every run.
    stamped_paths: set[Path] = set()
    for d in conversation_apps:
        # NOTE the empty `claimed` set, and it is load-bearing (independent
        # review of this PR, finding 2). `claimed_paths` exists so two
        # DETECTIONS cannot both bind one manifest for the backfill pass. A
        # conversation detection binds nothing — it is asking "does this app
        # already exist?" — and passing `claimed_paths` made the answer NO in
        # exactly the case that matters: when the LLM had just claimed that
        # same manifest, which is the common case for a real habit that has
        # files. Measured: the draft was minted, dedup merged the manifest a
        # few phases later, and its Spec was left orphaned in the gallery.
        match = _match_detected_to_existing(d, existing_index, set())
        if match is not None:
            # ALPHA-4 (audit finding B5): the app already exists, so do not draft
            # a second one — but MERGE the recurrence onto it rather than
            # discarding it. A draft with authored code and a real habit behind
            # it is the one shape that clears the offer gate on merit; before
            # this, the two halves could never meet on one manifest.
            #
            # `continue` still guards the double-count: a detection either stamps
            # a matched manifest or births a new draft, never both.
            mpath = match["path"]
            if mpath in stamped_paths:
                _slog(
                    f"[scanner]   conversation evidence for {d.id} matches "
                    f"{mpath.name}, which a stronger recurrence already stamped "
                    f"this scan; not stamping a second time"
                )
                continue
            outcome = _merge_conversation_evidence_into_manifest(
                mpath, d.conversation_evidence or {}, log=_slog,
            )
            if outcome != "failed":
                stamped_paths.add(mpath)
            _slog(
                f"[scanner]   conversation evidence for {d.id} already has a "
                f"manifest ({mpath.name}); not drafting a second one "
                f"(recurrence {outcome})"
            )
            continue
        if {c for c in (_canon_name(d.id), _canon_name(d.name)) if c} & llm_canons:
            # Deliberately NOT stamped, and the two cases this covers recover
            # differently — the original comment here claimed they both
            # self-heal, which is false for the second (independent review of
            # PR #3805, finding 1).
            #
            #   a) The LLM detection has no manifest yet; Phase 4 is about to
            #      mint one. Nothing on disk to stamp. This DOES self-heal: next
            #      scan the manifest exists under a name the direct match sees.
            #   b) The LLM detection bound an EXISTING manifest by a key this
            #      detection did not match — in practice pass (d), file-evidence
            #      overlap, under a different name. This does NOT self-heal.
            #      Pass (d) is structurally dead for a conversation detection
            #      (`evidence_files` is empty by construction, so the overlap is
            #      always 0), and the manifest's canonical keys never acquire
            #      this detection's canon. The miss is permanent, not one-scan.
            #
            # Case (b) is accepted anyway: reaching that manifest would mean
            # matching transitively through the LLM's file-evidence overlap, a
            # weaker identity chain than the direct match above, and a wrong
            # stamp writes recurrence onto an app nobody is asking for. Losing
            # the stamp is the safe direction. If (b) turns out to be common on
            # a real pod, the fix is a stronger direct match, not a transitive
            # one.
            _slog(
                f"[scanner]   conversation evidence for {d.id} was already "
                f"detected in the workspace this scan; not drafting a second one"
            )
            continue
        new_apps.append(d)

    total_found = len(merged) + len(conversation_apps)
    print(
    ...
    def _gen_and_save(app: DetectedApplication) -> str:
        ...
        try:
            if app.conversation_evidence is not None:
                # Conversation-only: the stub path, never the LLM. The whole
                # detection is a label and a day count — there is no file for
                # generate_manifest_for_app to read, so an LLM call here would
                # invent an implementation that does not exist.
                manifest = _stub_manifest(app, bot_id)
                _stamp_conversation_evidence(manifest, app.conversation_evidence)
            else:
            ...
            # cutover, internal/spec-manifest-v7-slicing-2026-06-10.md §5.1).
            ...
            try:
                ...
                if mint.succeeded:
                    minted = True
                    _slog(
                        # identity: see resolve_app_id — NOT swept (AL-1.4b).
                        # These are the mint RESULT's two distinct outputs (a
                        # Spec and an Instance), not a manifest identity read;
                        # collapsing them would lose half the log line.
            ...
            if not minted:
                # AL-1.4a identity stamp for the LEGACY fallback write (the
                # native mint above stamps inside _extract_instance). A
                # scanner detection is born DISCOVERED, so it gets a stable
                # ``draft_id`` and no ``app_id``: identity is conferred by
                # promotion, not by discovery (design §3), and the scanner must
                # stay free to merge/rename/drop drafts without breaking
                # anything downstream. The draft id is deterministic on the
                # stem, so a re-scan of the same detection does not churn it.
                # Runs before the provenance stamp below so field_origins
                # covers it.
                try:
                    from .app_identity import stamp_identity
                    stamp_identity(
                        manifest, mint=True,
                        seed=str(manifest.get("id") or app.id),
                        context=f"scanner mint {app.id}",
                    )
                except Exception as _exc:  # noqa: BLE001 — never block a scan
                    _slog(f"[scanner]   identity stamp failed for {app.id}: {_exc}")
                # v20: stamp observational provenance on every field of a
                # fresh-discovery LEGACY manifest. Spec: internal/spec-app-
                ...
                # ── User-decision carry-forward on the LEGACY fallback (AL-1.7,
                # brief §8.3 step 7) ────────────────────────────────────────────
                # This branch fires when the v7-arc mint FAILED, and it writes a
                # freshly generated dict without reading disk — so it dropped
                # every promotion answer the user had given: the permanent
                # "never" shield and a live snooze alike. ``native_write`` grew
                # the same carry-forward in rounds 4–5 of #3734 (proved by
                # execution, not asserted); this is the path that was left out
                # of scope then, because ``scanner.py`` belonged to a sibling
                # chip that cycle. AL-1.6c merged 2026-08-21, so it no longer
                # does, and the gap closes here rather than staying on a list.
                #
                # A mint failure is the WORST time to lose the answer: the file
                # is already in the state that needs re-discovering, and the
                # user is about to be asked a question they have answered.
                try:
                    from .native_write import (
                        DO_NOT_OFFER_BY_FIELD as _DNO_BY,
                        DO_NOT_OFFER_FIELD as _DNO,
                        SNOOZE_UNTIL_FIELD as _SNOOZE,
                        _read_any_manifest_on_disk as _read_any,
                    )
                    _prior = _read_any(caps_dir / f"{_legacy_stem}.json")
                    if _prior is not None:
                        for _decision_field in (_DNO, _DNO_BY, _SNOOZE):
                            _prior_decision = _prior.get(_decision_field)
                            if _prior_decision not in (None, "", False):
                                manifest[_decision_field] = _prior_decision
                except Exception as _exc:  # noqa: BLE001 — never block a scan
                    _slog(
                        f"[scanner]   promotion-decision carry-forward failed "
                        f"for {app.id}: {_exc}"
                    )
    ...
    # Spec: internal/spec-app-coherence-and-reconciliation-2026-06-05.md §3.1 +
    # §3.5 + §7.1. PR 3.
    #
    ...
    # Spec: internal/spec-app-coherence-and-reconciliation-2026-06-05.md §7.4.
    ...
    # Spec: internal/spec-app-coherence-and-reconciliation-2026-06-05.md §6.1.
    ...
    for mf in sorted(caps_dir.glob("*.json")):
        ...
        try:
            ...
            # AUTO_MANAGED_FIELDS — see internal/spec-app-derived-permissions-2026-05-24.md).
    ...
    # internal/spec-app-derived-permissions-2026-05-24.md, sub-spec
    # internal/spec-app-permission-review-2026-05-26.md).
    #
...
def _build_existing_manifest_index(
    ...
    **v7-arc hydration.** A thin v7-arc Instance carries ``id``/``name`` of
    ...
    for p in sorted(caps_dir.glob("*.json")):
        ...
        # identity: see resolve_app_id — NOT swept (AL-1.4b). The matcher below
        # compares a DetectedApplication's LLM-generated slug against this key,
        # so it must stay the (hydrated) legacy ``id``. ``resolve_app_id`` on a
        # HYDRATED v7-arc view returns ``provenance.spec_id`` (hydration rewrites
        # ``pkg_id``), which no detection ever produces — every v7-arc Instance
        # would stop matching and be re-minted on each scan.
        index.append({
            "path": p,
...
def _rediscovery_match(
    ...
    *,
    ...
    for entry in existing_index:
        ...
        # identity: see resolve_app_id — NOT swept (AL-1.4b). The re-discovery
        # target must be an Instance with a LIVE bound Spec; that is
        # ``provenance.spec_id`` specifically (nested), not whatever id the
...
def _merge_two_manifests(winner: dict, loser: dict) -> dict:
    ...
    if f_w or f_l:
        # identity: see resolve_app_id — NOT swept (AL-1.4b). ``files[*].
        # owned_by`` is a pkg_id-namespaced attribution tag; the rewrite below
        # compares on-disk tags against it, so both sides must be read from
        # ``pkg_id`` alone. Resolving through the canonical chain would let a
        # manifest with no ``pkg_id`` fall back to its ``id`` and rewrite
        # ``owned_by`` tags to a value no other reader recognizes.
    ...
    # identity: see resolve_app_id — NOT swept (AL-1.4b). This chain is the
    # loser's FILENAME STEM, not its app identity: the stem invariant is
    # ``id`` == stem for legacy manifests and ``instance_id`` == stem for
    # v7-arc Instances. The sibling audit breadcrumb in ``_dedup_manifests``
    # appends ``loser["stem"]`` literally, and ``merged_from`` has no other
    # consumer. Resolving canonically here would emit the SPEC id for every
    # v7-arc loser and diverge from that sibling.
...
def _dedup_manifests(caps_dir: Path, shared_dir: Path | None = None) -> int:
    ...
    for i, (path_a, raw_a, view_a) in enumerate(manifests):
        ...
        for path_b, raw_b, view_b in manifests[i + 1:]:
            ...
            # identity: see resolve_app_id — NOT swept (AL-1.4b). Same-Spec
            # is a BINDING comparison over ``provenance.spec_id`` (nested);
            # two Instances of the same Spec resolve to DIFFERENT canonical
            # ids today (each to its own ``instance_id`` — see the 1.4a
            # implementation record item 6), so the resolver cannot express
            # this test until 1.4c collapses instance identity onto the Spec.
...
def _archive_platform_file_only_stubs(
    ...
    for mf in candidates:
        ...
        # lifecycle, Bite 1; internal/spec-apps-meta-2026-06-13.md §9.2 guarantee 1).
...
def _stamp_discovered_files(
    ...
    # identity: see resolve_app_id — deliberately the LEGACY ``id``, and named
    # for what it is (AL-1.4b renamed this local off ``app_id``, which now means
    # the canonical field). This is the log label for a legacy-shape stamp pass
    # that returns early on v7-arc, and it is pinned byte-for-byte by the
    # scanner's --dry-run output.
    legacy_id = manifest_dict.get("id", "?")
    app_name = manifest_dict.get("name", "?")

    _slog(f"[stamp] {legacy_id} ({app_name})")
...
def scan_compliance(bot_id: str, shared_dir: Path) -> dict[str, Any]:
    ...
    identity: see resolve_app_id — NOT swept (AL-1.4b). Every ``app_id`` key
    emitted below is ``ApplicationManifest.id``, the LEGACY id, and stays that
    way: these dicts are rolled up into compliance Signals whose dedup
    signature and ``details.items`` are a wire contract (a changed value
    re-fires every open compliance Signal on the pod). The field NAME and the
    canonical ``app_id`` field are a namespace collision inherited from before
    AL-1.4 — reconciling them is 1.4c work with a Signal migration, not a
    mechanical reader sweep.
    """
    ...
    if workspace and exists_or_unreachable(workspace):
        ...
        # manifests (identity: see resolve_app_id — the value is ``_m.id``, the
        # same legacy-id wire contract the docstring above describes) so
        # misplaced_secret findings at a declared api_key_source
                ...
                if _pat.search(_content):
                    ...
                    if _owning_app:
                        ...
                        _issue["message"] = (
                            ...
                            f"internal/spec-apps-inherit-bot-llm-2026-06-06.md "
