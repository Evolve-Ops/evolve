...
See internal/spec-manifest-v7-2026-05-20.md §5 (Provenance) and the app-identity
ledger sub-spec of internal/spec-apps-meta-2026-06-13.md.
...
``spec_id`` in it. Nothing here is a generic "which app is this?" read, so
nothing here routes through ``app_identity.resolve_app_id``. Every occurrence
is one of two structural fields of the supersession chain:

  * ``provenance.spec_id`` — the ONE Spec version-line an Instance is bound
    to right now. It is the key gallery paths, ``spec_version`` pinning and
    ``adopt.py`` are all written against, and it is deliberately NOT the
    Instance's own identity (that is ``instance_id``, and post-hydration the
    two are different strings on the same manifest).
  * ``provenance.prior_spec_ids[]`` — a chain of RETIRED ``spec_id`` values,
    oldest → newest. A retired id resolves to no live app by construction;
    that is the whole point of keeping it. Handing this list to a resolver
    that answers "the current canonical id" would collapse a supersession
    chain into repeats of the current id and silently un-orphan nothing.

``resolve_spec`` / ``build_spec_index`` are spec-BINDING resolution (marker id
-> live Instance), a different question from app-identity resolution, and the
two must not be confused: ``resolve_spec`` takes an id that may be dead and
returns a manifest; ``resolve_app_id`` takes a manifest and returns its id.
"""
...
def current_spec_id(instance: dict) -> Optional[str]:
    ...
    # identity: see resolve_app_id — THE accessor for the live Spec binding —
    # the module's whole subject; see the head note.
    prov = instance.get("provenance")
    if not isinstance(prov, dict):
        return None
...
def record_spec_supersession(instance: dict, old_spec_id: str) -> dict:
    ...
    """
    # identity: see resolve_app_id — compares against the live binding so a
    # Spec never supersedes itself — see the head note.
...
def resolve_spec(spec_id: str, instances: Iterable[dict]) -> Optional[dict]:
    ...
    """
    # identity: see resolve_app_id — spec-BINDING resolution: matches a
    # possibly-RETIRED id against the current binding and the prior chain — the
    # inverse of resolve_app_id (head note).
    if not spec_id:
        return None
