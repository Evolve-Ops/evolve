...
def _run_bot_dispatch(
    ...
    request = bot_forge.BuildRequest(
        ...
        # the prior attempt don't cause sha256-mismatch verification
        # failures. Defaults False via getattr so older serialised jobs
    ...
    # Files rewritten by a later refine round must skip the sha256 check —
    # their on-disk content reflects the refine's hashes, not the build's.
    # This is load-bearing on recovery: `recover_orphaned_jobs` replays
    # `run_forge_job` from step 1, and `dispatch_build`'s
    # resume-from-outbox returns the original build hashes. Without this
    # skip set, every multi-critique-round install that gets recovered
    # mid-flight fails step 2 verification on files refine touched.
    skip_hash = bot_forge.paths_rewritten_after(
        job.bot_id, job.job_id, after_suffix="",
    )
    if skip_hash:
        _append_log(
            job.job_id, shared_dir,
            f"Step 2 verify: skipping sha256 check on "
            f"{len(skip_hash)} path(s) rewritten by a later refine round "
            f"(existence still required): {sorted(skip_hash)[:3]}",
        )
    verified, errors = bot_forge.verify_files_on_disk(
        job.bot_id, bot_files, skip_hash_for_paths=skip_hash,
    )
    if errors:
    ...
    # filtered list here so a stale echoed plist path can't leak into
    # ``manifest.files`` and trip the post-apply reality check.
    if manifest is not None:
        records = bot_forge.build_manifest_file_records(
            bot_id=job.bot_id,
            files_written=bot_files,
            app_id=job.app_id or "",
    ...
    for round_idx, (critique_step, refine_step) in enumerate(critique_step_pairs):
        ...
        # On recovery, refine round 1 may replay after round 2 has touched
        # the same files. Same skip-hash logic as the build verify above.
        refine_skip_hash = bot_forge.paths_rewritten_after(
            job.bot_id, job.job_id, after_suffix=f"-r{round_num}",
        )
        if refine_skip_hash:
            _append_log(
                job.job_id, shared_dir,
                f"Refine round {round_num} verify: skipping sha256 check on "
                f"{len(refine_skip_hash)} path(s) rewritten by a later "
                f"refine round: {sorted(refine_skip_hash)[:3]}",
            )
        verified, errors = bot_forge.verify_files_on_disk(
            job.bot_id, refine_bot_files, skip_hash_for_paths=refine_skip_hash,
        )
        if errors:
            err = (
        ...
        # Use the Phase-4.5-filtered list, same reason as the build path above:
        # ``manifest.files`` records workspace build outputs only.
        if manifest is not None and refine_bot_files:
            refined_paths = {
                (e.get("path") or "").lstrip("/")
                for e in refine_bot_files
            }
            kept = [
                r for r in (manifest.files or [])
                if isinstance(r, dict)
            ...
            refined_records = bot_forge.build_manifest_file_records(
                bot_id=job.bot_id,
                files_written=refine_bot_files,
                app_id=job.app_id or "",
                pkg_id=job.pkg_id or manifest.pkg_id or "",
                run_id=job.run_id or "",
                now_iso_str=_now_iso(),
        ...
        current_files = refine_bot_files or current_files
    ...
    for action in actions:
        ...
        try:
            if mechanism in ("oc_heartbeat_instruction", "oc_session_instruction"):
                ...
                # ${bot_id} / ${workspace} substitution and is the gate that
                # ``scheduled_actions_validator`` checks. Shape (b) is kept
                # only so older manifests already in the wild keep working
                # until they're re-forged.
                label = (install_cfg.get("plist_label") or "").strip()
...
def _apply_forge_output(
    ...
    try:
        ...
        _append_log(
            ...
            f"failed={sum(1 for e in install_summary if e.get('status') == 'failed')}, "
...
def approve_forge_job(
    ...
    mark_step_done(job, 10, f"Applied: pkg_version → {new_version}", shared_dir)
