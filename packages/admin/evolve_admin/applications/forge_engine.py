...
Phases 1-3 are executed by the target bot's own LLM via ``openclaw agent``
(see ``_run_bot_dispatch``). Admin-side LLM calls (Phase 2.5 critics, Phase 5
spec reconciliation) go through the provider-agnostic ``infra_llm`` client —
whichever provider the pod is credentialed for, no third-party SDK
dependency (#3466).
"""
...
import os
import re
import subprocess  # noqa: F401 — tests monkeypatch forge_engine.subprocess.run
...
BUILTIN_BUILDER_PROMPT = (
    ...
    "(§1 enforces these defaults; §3.1 of internal/spec-export-import-forge-2026-05-26.md "
...
# ── LLM key + target resolution (#3466: provider-agnostic) ────────────────────

def _resolve_llm_keys(bot_id: str | None = None) -> dict[str, str]:
    """``{provider: secret}`` for a forge job's admin-side LLM calls.

    Sources, first hit wins: the job bot's OWN auth store (per-bot cost
    attribution — the SOURCE read is delegated to ``oc_store``: per-agent
    sqlite store → legacy auth-profiles.json → transitional bak), then the
    invoking user's own ``~/.openclaw`` store (the daemon-local fallback).
    Per-provider env vars (``ANTHROPIC_API_KEY``, ``OPENAI_API_KEY``, …)
    override/extend their own provider's entry, mirroring
    ``primary_bot.read_primary_bot_llm_keys``.

    Returns ``{}`` when nothing is credentialed — callers must degrade
    gracefully (skip the LLM-dependent steps with a log line).
    """
    ...
    from ..oc_store import read_auth_store

    keys: dict[str, str] = {}
    sources: "list[tuple[str | None, Path]]" = []
    if bot_id:
        sources.append((bot_id, _bot_home(bot_id)))
    sources.append((None, Path(os.path.expanduser("~"))))
    for src_bot, home in sources:
        raw = read_auth_store(src_bot, home=home)
        if not raw:
            continue
        try:
...
def _target_for_model(model: str, keys: dict[str, str]):
    """``InfraLLMTarget`` for a provider-qualified model, or ``None`` when the
    id is bare (provider unknowable — never presumed) or its provider has no
    key (never an uncredentialed target)."""
    from infra_llm import InfraLLMTarget  # type: ignore

    if "/" not in model:
        return None
    prov = model.split("/", 1)[0]
    if prov in keys:
        return InfraLLMTarget(provider=prov, model=model, api_key=keys[prov])
    return None
...
def _call_llm(
    ...
    """
    One completion against the resolved ``infra_llm`` target (whichever
    provider the pod is credentialed for — stdlib urllib underneath, no SDK
    dependency). 120s timeout: forge prompts are big.

    Returns the assistant's text response.
    Raises on HTTP errors or parse failures (``infra_llm.InfraLLMError``).
    """
...
def _resolve_standard_target(bot_id: "str | None", keys: dict[str, str]):
    """The pod's tier2/standard target for forge admin-side calls.

    1. ``models.resolve_tier("tier2", config, bot_id=bot_id)`` — the pod's
       tier2 with per-bot ``tier_assignments`` respected — taken when its
       provider is credentialed. A tier2 pinned to an uncredentialed
       provider is walked past, NOT "corrected" back to a presumed default
       provider (#3466 — this replaces the old ``_resolve_tier2_anthropic``
       guard that DISCARDED a correctly configured non-Anthropic tier2 and
       called Claude anyway).
    2. Otherwise the derived catalog tier2 pick among the credentialed
       providers (``models.derive_default_tiers``).

    Returns ``None`` only when nothing is credentialed / derivable.
    """
    try:
        ...
        resolved = ""
    if resolved:
        target = _target_for_model(resolved, keys)
        if target is not None:
            return target
        log.info(
            "forge_engine: tier2 %r has no credentialed provider on this pod; "
            "walking the credentialed providers instead",
            resolved,
        )
    try:
        ...
        return None
    credentialed = {prov for prov in keys if prov in _KNOWN_LLM_PROVIDERS}
    if not credentialed:
        return None
    derived = derive_default_tiers(credentialed)
    for model in derived.get("tier2", {}).get("models", []):
        target = _target_for_model(model, keys)
        if target is not None:
            return target
    return None
...
def _get_targets(
    ...
    """Return ``(builder_target, critic_target)`` for a forge job — each an
    ``infra_llm.InfraLLMTarget`` or ``None``.

    Resolution priority per role, each step short-circuiting:
      1. network.json::forge.builder_model / forge.critic_model — explicit
         operator pin. A provider-QUALIFIED pin (``"openai/gpt-4o"``) binds
         fully; when its provider has no key it is ignored with a warning
         (never an uncredentialed target). A BARE pin
         (``"claude-sonnet-4-6"``, the historic form) pins the MODEL ID on
         whatever provider steps 2-3 resolve — exact legacy behavior on the
         single-provider pods that wrote such pins.
      2. The pod's tier2 (per-bot ``tier_assignments`` respected) when its
         provider is credentialed.
      3. The derived catalog tier2 pick among the credentialed providers.

    Both ``None`` when no LLM provider is credentialed — callers degrade
    (log + skip the admin-side LLM steps).
    """
    keys = _resolve_llm_keys(bot_id)
    if not keys:
        return None, None
    ...
    base = _resolve_standard_target(bot_id, keys)

    def _for_role(pin: "str | None"):
        if pin:
            if "/" in pin:
                target = _target_for_model(pin, keys)
                if target is not None:
                    return target
                log.warning(
                    "forge_engine: forge model pin %r names an uncredentialed "
                    "provider — ignoring the pin. Qualify it as "
                    "provider/model for a credentialed provider.",
                    pin,
                )
            elif base is not None:
                from dataclasses import replace

                return replace(base, model=pin)
        return base

    return _for_role(operator_builder), _for_role(operator_critic)
...
def assemble_context_package(job: ForgeJob, shared_dir: Path) -> dict:
    ...
    if manifest is None:
        ...
        manifest_dict: dict = {
            # identity: see resolve_app_id — this skeleton MIRRORS the job's two
            # distinct id fields onto the shape the prompt builder reads; the
            # ForgeJob carries ``app_id`` (manifest stem) and ``pkg_id``
            # (gallery catalog key) separately and both must survive.
            "id": job.app_id,
            "bot_id": job.bot_id,
            "pkg_id": job.pkg_id,
    ...
    if app_deps:
        ...
        # identity: see resolve_app_id — both sides of this lookup are pinned to
        # ``pkg_id`` and must stay pinned TOGETHER: the map is keyed by the
        # installed manifest's ``pkg_id`` and probed with the dependency
        # entry's. ``m.pkg_id`` doubles as the "came from a package at all"
        # guard, which the resolver (which always resolves something) would
        # erase. See ``_check_app_dependencies`` for the other reader of this
        # same ``app_dependencies[]`` list and the divergence between them.
        ...
        for dep in app_deps:
            dep_pkg_id = dep.get("pkg_id", "")  # identity: see resolve_app_id — map key, as above
...
def _context_to_user_message(ctx: dict, phase: str = "build", critique: str | None = None) -> str:
    ...
    if manifest:
        # identity: see resolve_app_id — a KEY ALLOWLIST for the prompt
        # projection, not an id read: both ``id`` and ``pkg_id`` are forwarded
        # to the forge bot verbatim and it is told to distinguish them.
        condensed = {
            k: manifest[k]
    ...
    if dep_ctx:
        dep_parts: list[str] = []
        for dep in dep_ctx:
            # identity: see resolve_app_id — ``dep_ctx`` rows are the projection
            # built above, whose ``pkg_id`` key is the gallery catalog key.
...
def _materialize_files(
    ...
    # identity: see resolve_app_id — PROVENANCE-MARKER NAMESPACE. It is written
    # into every materialized file as ``spec=<pkg_id>`` via ``embed_marker``
    # below and later matched by ``provenance``/``lineage_repoint``/the
    # ``owned_by`` audit against Spec ids. The empty-string case is also the
    # "don't stamp a marker at all" guard (``if embed_marker is not None and
    # pkg_id``), which a resolver fallback would turn always-on.
...
def _check_app_dependencies(app_dependencies: list, bot_id: str, shared_dir: Path) -> list[str]:
    ...
    for dep in app_dependencies:
        if not isinstance(dep, dict):
            ...
            continue
        # identity: see resolve_app_id — NOT applicable here, and this is the
        # only genuine two-field chain in area 4b, so the reasoning is spelled
        # out. ``dep_id`` becomes a MANIFEST FILENAME STEM
        # (``applications/<bot>/<id>.json`` — ``save_manifest`` writes
        # ``f"{manifest.id}.json"``). ``resolve_app_id`` leads with ``pkg_id``,
        # which for a gallery install is NOT the stem (AL-1.4 §6 delta 12), so
        # substituting it would look for ``p-a3f91c8b.json`` and report every
        # gallery-installed dependency as missing.
        #
        # KNOWN DIVERGENCE, recorded not fixed (AL-1.4b is behavior-neutral):
        # this reads the same ``app_dependencies[]`` list as the dependency-
        # context injection above, but on a different field — ``spec_id`` here,
        # ``pkg_id`` there. The ea-pack shape (``{pkg_id, display_name,
        # required, reason}``, and ``migrate_v7._migrate_app_dependencies``
        # treats ``pkg_id`` as the leading field of this very shape) therefore
        # falls into the "no `spec_id`" branch below rather than being checked.
        # The finding is non-blocking and log-only, so the wrong warning is the
        # whole cost; fixing it is a behavior change and belongs in its own PR.
...
# Spec: internal/spec-heartbeat-instruction-2026-06-03.md §4 (Phase 4.5
    ...
    for action in actions:
        ...
        if mech in ("oc_heartbeat_instruction", "oc_session_instruction"):
            ...
            if label:
                ...
                owned.add(f"{_bot_home(bot_id)}/Library/LaunchAgents/{label}.plist")
            ...
            if action_id:
                ...
                owned.add(rel)
                owned.add(f"{_bot_home(bot_id)}/.openclaw/workspace/{rel}")
...
def _run_bot_dispatch(
    ...
    # Spec: internal/spec-files-pack-hybrid-2026-06-03.md §7.
    ...
    request = bot_forge.BuildRequest(
        job_id=job.job_id,
        kind="build",
        pkg_id=job.pkg_id or "",  # identity: see resolve_app_id — gallery catalog key, paired with a distinct app_id
    ...
    if manifest is not None:
        records = bot_forge.build_manifest_file_records(
            ...
            app_id=job.app_id or "",
            pkg_id=job.pkg_id or manifest.pkg_id or "",  # identity: see resolve_app_id — gallery catalog key, paired with a distinct app_id
            run_id=job.run_id or "",
    ...
    try:
        _, critic_target = _get_targets(shared_dir, bot_id=job.bot_id)
...
def _notify_operator(job: ForgeJob, shared_dir: Path) -> None:
    ...
    fallback_msg = (
        ...
        # identity: see resolve_app_id — renders BOTH ids because they differ
        # for a gallery install and the operator needs each to find the app.
    ...
    try:
        ...
        outcome = _dispatch_send(
            ...
            # identity: see resolve_app_id — signal payload contract; the Alerts
            # page renders the stem and the catalog key as separate columns.
            payload={
                "app_id": job.app_id,
                "pkg_id": job.pkg_id,
                "bot_id": job.bot_id,
...
def _run_static_analysis_phase(
    ...
    *,
    ...
    Spec: internal/spec-forge-side-effects-2026-06-02.md §13 (PR 6) + §14
    ...
    # personal-bot-user account) — _bot_home resolves it via network.json + pwd.
    workspace = _bot_home(manifest.bot_id) / ".openclaw" / "workspace"
    if not workspace.exists():
        workspace = _bot_home(job.bot_id) / ".openclaw" / "workspace"
    ...
    try:
        ...
        # Only run if there are any constraint items AND an LLM provider is
        # credentialed. No-op when either is missing — the call wrapper
        # would just raise.
        if critic_target is not None:
            ...
            if items:
                def _llm(system: str, user: str) -> str:
                    return _call_llm(system, user, critic_target,
                                     max_tokens=4096)
    ...
    # (internal/spec-manifest-v7-slicing-2026-06-10.md §4.1). Advisory like
    # the rest of Phase 2.5; ``undeclared_collection`` findings are the
    # signal — code collecting user data the consent notice omits.
    try:
        if critic_target is not None and getattr(manifest, "privacy", None):
...
def _verify_community_signature_or_raise(
    ...
    # identity: see resolve_app_id — GALLERY CATALOG KEY throughout this
    # signature check and every log line in it: ``_read_gallery_package_manifest``
    # resolves ``gallery/<app-name>/<pkg_id>.json``, so only ``pkg_id`` locates
    # the package whose ``provenance`` and signature are being verified.
    ...
    if ok:
        # identity: see resolve_app_id — gallery catalog key, as at the top of
        # this function; it names the package whose signature was verified.
        _append_log(
            job.job_id, shared_dir,
...
def _maybe_install_via_files_pack(
    ...
    *,
    ...
    Spec: internal/spec-files-pack-hybrid-2026-06-03.md §7. The cost lever:
    ...
    if not files_pack_meta.get("format_version"):
        return None
    # identity: see resolve_app_id — GALLERY CATALOG KEY for the rest of this
    # function (``find_files_pack_dir`` walks ``gallery/<app-name>/files/``
    # keyed on it). The bare-falsy check is the "no package to install from"
    # guard, which a resolver fallback would make always-true and send every
    # scanner-discovered app down the files-pack path.
    if not job.pkg_id:
        return None
    ...
    try:
        ...
        context = resolve_install_context(
            ...
            # identity: see resolve_app_id — the files-pack PLACEHOLDER pair
            # (``{pkg_id}`` and ``{app_id}`` are two distinct names in
            # ``files_pack.KNOWN_PLACEHOLDERS``). ``manifest.id`` is the stem
            # fallback; the resolver would return ``pkg_id`` here and make both
            # placeholders substitute to the same string.
            pkg_id=job.pkg_id,
        ...
        # Smart-forge dispatcher (internal/note-smart-forge-and-file-
...
def _materialize_scheduled_actions(
    ...
    Spec: internal/spec-heartbeat-instruction-2026-06-03.md §4 (v17),
          internal/spec-launchd-python-signal-2026-06-03.md (v18).
        ...
        try:
            if mechanism in ("oc_heartbeat_instruction", "oc_session_instruction"):
                ...
                result = install_heartbeat_instruction(
                    ...
                    # identity: see resolve_app_id — managed-section marker key
                    # for the HEARTBEAT.md block; matched textually on teardown.
                ...
                summary.append({
                    ...
                    "error": (
                        ...
                        f"then re-forge. "
                        f"See internal/spec-heartbeat-instruction-2026-06-03.md."
                    ),
                })
                continue
                ...
                if command:
                    ...
                    result = install_launchd_command_action(
                        ...
                        # AL-1.2 Lane B: the app id this cron's turns belong
                        # to. ``manifest.id`` for the same reason the
                        # python_signal branch below uses it — it is the
                        # manifest stem the Alerts surface,
                        # ``app-cron-map.json`` and ``usage-by-app.json`` all
                        # key on; ``resolve_app_id`` would return ``pkg_id``
                        # for a gallery install and split one app across two
                        # rollup rows.
                ...
                # zero LLM cost.
                # Spec: internal/spec-launchd-python-signal-2026-06-03.md.
                label = (install_cfg.get("label") or "").strip()
                command = (install_cfg.get("command") or "").strip()
                ...
                result = install_python_signal_action(
                    ...
                    # identity: see resolve_app_id — two distinct values.
                    # ``app_id`` is baked into the generated wrapper script as
                    # ``APP_ID`` and emitted in every Signal the action raises
                    # (``details.app_id``), so it must stay the manifest stem
                    # the Alerts surface and the manifest agree on; ``pkg_id``
                    # is the separate marker key. The resolver returns
                    # ``pkg_id`` for a gallery install and would collapse them.
                    app_id=manifest.id or "",
                    pkg_id=manifest.pkg_id or "",
                    job_id=job.job_id or "",
                )
...
# (internal/spec-agent-freelance-bypass-2026-06-05.md). The structural fix is
...
def _bake_result_harness(
    ...
    Spec internal/spec-app-invocation-just-works-2026-06-29.md §2.2; OQ-2 re-resolved
...
def _apply_forge_output(
    ...
    per internal/decision-app-tests-2026-06-08.md. Coherence gate (below) is
    ...
    if final_impl and builder_target is not None:
    ...
    elif builder_target is None:
        _append_log(job.job_id, shared_dir,
                    "Phase 5: skipping spec reconciliation (no LLM provider credentialed)")
    ...
    # (spec internal/spec-agent-freelance-bypass-phase2-2026-06-06.md). No-op for
    ...
    # internal/spec-apps-inherit-bot-llm-2026-06-06.md for the full rule.
    ...
    # Spec: internal/spec-forge-side-effects-2026-06-02.md §5.
    #
...
def run_forge_job(
    ...
    # Linux ACL-clamp pre-heal (round-8 family; completes #3443/#3447): a
    # PRIOR job's OC teardown can leave ~/.openclaw's POSIX-ACL mask clamped
    # to --- so the very first bot-workspace write of THIS job (the step-1
    # manifest seed into workspace/manifests/) dies with a bare EACCES —
    # observed live on the reference VPS pod 2026-07-28 after the dispatch
    # seams were already guarded. One probe-and-heal up front clears any
    # pre-existing clamp for the whole run; mid-run clamps are handled by
    # the bot_forge dispatch seam. Best-effort: macOS is a structural no-op
    # and a heal failure must not block the job (the next workspace write
    # fails loud on its own if access is genuinely gone).
    try:
        ws = bot_forge.bot_workspace(job.bot_id)
        try:
            (ws / "manifests").exists()
        except PermissionError:
            from ..config import get_bot_user as _get_bot_user
            from ..secret_config_perms import heal_evolve_access
            try:
                _bu = _get_bot_user(job.bot_id, bot_forge._load_network_for_guard())
            except Exception:
                _bu = job.bot_id
            healed = heal_evolve_access(job.bot_id, _bu)
            _append_log(job_id, shared_dir,
                        f"Pre-run ACL clamp on {ws.parent} healed: {healed}")
    except Exception as exc:
        # Best-effort by design — never block the job on the pre-heal; the
        # next workspace write fails loud on its own if access is really gone.
        _append_log(job_id, shared_dir, f"Pre-run ACL probe skipped: {exc}")
    ...
    # internal/finding-new-bot-activation-cost-2026-06-12.md (decision B).
    if job.job_type == "install":
    ...
    # LLM-credential preflight (informational only).
    #
    # Bot-driven forge (Phases 1-4: build/critique/refine/test) doesn't need
    # admin-side LLM credentials — the bot uses its own openclaw credentials
    # via `openclaw agent --local`. Admin-side calls are only needed for the
    # Phase 2.5 critics and approval-time spec reconciliation; those resolve
    # their own targets (`_get_targets`) and degrade gracefully, so we don't
    # block the run — just leave an operator-visible note.
    if not _resolve_llm_keys(bot_id):
        _append_log(
            job_id, shared_dir,
            "Note: no LLM provider credentialed for admin-side calls; "
            "spec reconciliation + Phase 2.5 critics will be skipped. "
    ...
    try:
        ...
        # identity: see resolve_app_id — GALLERY CATALOG KEY: the seed comes from
        # ``load_gallery_package``, and the presence check distinguishes a
        # gallery install from a forge-from-scratch job.
        if manifest is None and job.job_type == "install" and job.pkg_id:
            ...
            try:
                ...
                # identity: see resolve_app_id — gallery catalog key naming the
                # package the seed came from, as at the top of this branch.
                err = (
    ...
    # internal/spec-agent-freelance-bypass-phase2-2026-06-06.md. If the
    ...
    # internal/spec-apps-inherit-bot-llm-2026-06-06.md. Refuses installs whose
    ...
    # Manifest-v7 Slice 2 (internal/spec-manifest-v7-slicing-2026-06-10.md §4.1).
    ...
    # See internal/spec-export-import-forge-2026-05-26.md §3.3.
...
def approve_forge_job(
    ...
    # Resolve the builder target for Phase 5 spec reconciliation
    builder_target, _ = _get_targets(shared_dir, bot_id=job.bot_id)
    ...
    if job.job_type == "install":
        try:
            ...
            if mint is not None and mint.succeeded:
                _append_log(
                    job_id, shared_dir,
                    # identity: see resolve_app_id — the mint RESULT's two
                    # distinct outputs (a Spec and an Instance), not a manifest
                    # identity read; collapsing them loses half the log line.
