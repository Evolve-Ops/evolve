...
def start_oauth_wait(
    ...
    ``requirements`` is the manifest's full requirements dict (the same one
    the install handler passed to ``evaluate_install_prerequisites``).  The
    sweeper re-evaluates with these full entries — the missing items alone
    have dropped ``check_path`` / ``alternatives[]`` / ``setup_doc``, so
    without them a requirement the operator satisfies via a declared
    alternative during the wait window would never be detected.
    """
    ...
    if requirements is not None:
        job.context_snapshot["oauth_requirements"] = requirements
