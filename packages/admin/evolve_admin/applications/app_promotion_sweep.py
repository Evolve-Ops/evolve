...
Brief ``internal/build-AL-1.7-promotion.md`` §4 item 1.
...
ALPHA-3b READS THE POD, ONCE, AND LATE. D-I half 2 asks whether a sibling bot
already has this app defined, so the sweep needs pod-wide facts it never needed
before (:func:`pod_defined_app_facts`). That read happens only after a candidate
has cleared the whole gate and an offer is actually being minted — the sweep's
steady state is "nothing was eligible", and paying for a pod-wide manifest walk
to reach that answer would be a cost with no offer at the end of it.
...
def plan_offer(
    ...
    * ``generators.app_promotion.observe`` returns it to ``generator_runner``,
    ...
    proposal = _promotion.build_promotion_proposal(
        ...
        taken_app_ids=taken,
        # ALPHA-3b. Read here, at the last moment, and not once per sweep:
        # this is a pass over every bot's manifests, and the sweep's steady
        # state is "nothing was eligible", which must stay cheap.
        defined_apps=pod_defined_app_facts(
            shared_dir, network=network, read_manifests=read_manifests
        ),
        now=now,
    )
    if proposal is None:
...
def pod_defined_app_facts(
    shared_dir: Path,
    *,
    network: Any,
    read_manifests: Callable[[str], Iterable[tuple[str, Mapping[str, Any]]]] | None = None,
) -> "dict[str, dict]":
    """The pod's DEFINED apps as grouping facts — ALPHA-3b's input.

    Delegates to ``pod_apps.defined_app_facts`` (the same assembly the Apps
    page groups with) over every bot ``network.json`` lists. The bot list comes
    from the network rather than from the sweep's own bot, because the question
    ALPHA-3b asks is about the POD: "does a sibling already have this app?"

    **Tolerant, and it fails toward CONFERRING.** An unreadable pod is reported
    and returns ``{}``, which sends promotion down the conferral branch it used
    before ALPHA-3b. That direction is deliberate: a missed adoption costs a
    duplicate id that ALPHA-3a's presentation grouping still collapses on the
    page and a later promotion can still converge, while a made-up adoption
    would hand one bot's app the identity of another's on the strength of a
    half-read pod.

    ``read_manifests`` defaults to the real privileged reader, so the wizard —
    which has no reader of its own to inject — can call this with a shared_dir
    and a network alone.
    """
    reader = read_manifests or _read_manifests_for_bot
    try:
        from .pod_apps import defined_app_facts

        bots = sorted((network.get("bots") or {}).keys()) if isinstance(network, Mapping) else []
        if not bots:
            return {}
        return defined_app_facts(
            bots,
            read_manifests=reader,  # type: ignore[arg-type]
            shared_dir=Path(shared_dir),
        )
    except Exception as e:  # noqa: BLE001 — reported, never swallowed
        logger.warning(
            "app_promotion_sweep: could not read the pod's defined apps (%s) — "
            "promotion will CONFER a new id rather than adopt an existing one",
            e,
        )
        return {}
