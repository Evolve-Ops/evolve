...
LEGACY_KEY_MIGRATION: dict[str, str] = {
    ...
    # Rationale: docs/gallery-merge-task-manager-2026-06-05.md.
...
#
# Spec: docs/note-smart-forge-and-file-provenance-2026-06-04.md flags
...
def find_files_pack_dir(pkg_id: str) -> Path | None:
    ...
    Spec: docs/spec-files-pack-hybrid-2026-06-03.md §4.
...
#   4. Document the removal in ``docs/gallery-removals-*.md``.
...
def preflight_check(pkg_id: str, bot_id: str, shared_dir: Path) -> dict:
    ...
    Returns::

        {
          ...
          "requirements": [
              {
                "type":     "integration" | "secret" | "system" | "python_package",
                "id":       str,
                "required": bool,
                "state":    str,
                "severity": str,
    ...
    def _installed_state(dep_pkg_id: str) -> str:
        """
        Determine the install state of a dependency on this bot.
        ...
        Returns one of:
          ...
          "scanner_only" — manifest exists but pkg_id does not match (scanner-detected)
          "not_installed"— no manifest found at all
        """
        # Look for any manifest on this bot whose pkg_id matches the dependency
        from .manifest import list_manifests as _list
        try:
            for m in _list(shared_dir, bot_id):
                if m.pkg_id != dep_pkg_id:
                    continue
                ...
                if m.status in ("active", "approved"):
                    return "installed"
                return "installing"
            # No manifest with matching pkg_id — check for scanner-only
            for m in _list(shared_dir, bot_id):
                if not m.pkg_id:
                    # Could be the same app detected by scanner — inconclusive
                    pass
        except Exception:
            pass
        return "not_installed"

    def _read_oc_json(b_id: str) -> dict:
        p = _Path(f"/Users/{b_id}/.openclaw/openclaw.json")
        try:
            if p.exists():
    ...
    def _check_integration(req: dict) -> tuple[str, str]:
        ...
        """
        state, message = _check_one_integration(req)
        if state == "satisfied":
        ...
        for alt in alternatives:
            alt_state, alt_message = _check_one_integration(alt)
    ...
    def _check_one_integration(req: dict) -> tuple[str, str]:
        ...
        if integration_id == "telegram":
            ...
            skill_file = _Path(f"/Users/{bot_id}/.openclaw/skills/telegram.json")
            file_ok = False
            try:
    ...
    def _check_path_c_google_integration(
        ...
        ``docs/spec-google-integration-paths-2026-05-30.md``. A satisfied
        check requires:
    ...
    def _check_system(req: dict) -> tuple[str, str]:
        ...
        try:
            r = _subprocess.run(argv, capture_output=True, timeout=5)
            if r.returncode == 0:
    ...
    def _read_github_token_from_git(b_id: str, current_oc: dict) -> tuple[str, str] | None:
        ...
        """
        import re as _re
        _ws = (current_oc or {}).get("agents", {}).get("defaults", {}).get("workspace") or f"/Users/{b_id}/.openclaw/workspace"
        git_config = _Path(_ws) / ".git" / "config"
        try:
    ...
    for req in reqs.get("integrations", []):
        state, message = _check_integration(req)
    ...
    # docs/spec-manifest-v7-slicing-2026-06-10.md §4.1) — structural keys
