...
def check_messaging_channel_requirement(bot_id: str, req: dict) -> tuple[str, str]:
    ...
    (docs/spec-gallery-delivery-convention-2026-06-11.md) without forking a
    ...
    try:
        ...
        route_channels = sorted(
            str(ch) for ch, target in ids.items()
            if target and str(ch).lower() in MESSAGING_INTEGRATION_IDS
        )
    except Exception:
        route_channels = []
