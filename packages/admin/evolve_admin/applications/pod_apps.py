...
def build_app_detail(
    ...
    *,
    ...
    load_usage: Callable[[Path, str], dict] | None = None,
    load_spec: Callable[[Path, str], "AppSpec | None"] | None = None,
    load_outcomes: Callable[[Path, str, "list[str]"], dict] | None = None,
    ...
    # ── Outcomes (design §3.2, D-AP6) — beside cost, not instead of it ──────
    # ``load_outcomes`` wraps ``app_outcomes.get_app_outcomes``, which reads
    # the Tracker rollup keyed by the Tracker's OWN two application ids
    # (``assistant`` / ``project-manager`` — D-TM11's vocabulary, not a
    # per-manifest field: see outcome_by_app.py's module docstring for why).
    # Most apps are not Tracker applications at all, so ``measured: False``
    # here is the ordinary answer, not a failure — the same tri-state shape
    # ``usage_measured`` already uses for cost.
    if load_outcomes is None:
        row["outcomes"] = {"measured": False}
    else:
        try:
            row["outcomes"] = load_outcomes(shared_dir, app_id, bot_list)
        except Exception as exc:  # noqa: BLE001
            log.warning("pod_apps: outcomes unavailable for %s: %s", app_id, exc)
            row["outcomes"] = {"measured": False}
    return row
