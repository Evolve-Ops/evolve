...
Design: ``internal/design-app-spec-and-discovery-2026-08-15.md`` §4 (the lifecycle),
...
Brief: ``internal/build-AL-1.7-promotion.md``.
...
**ALPHA-3b widened ADOPT from one bot's history to the pod's** (operator decision
D-I, 2026-08-24, half 2). Before it, promoting the second bot's draft of an app
the pod already had conferred a SECOND id for one app — the very condition
ALPHA-3a's presentation grouping papers over at read time. Now
:func:`adopt_or_confer_app_id` consults the pod's DEFINED apps first, and adopts
the existing id when one of them is equivalent. Three properties hold it in
place, and each was a choice:

* **The equivalence is ALPHA-3a's, imported** (``app_grouping.equivalent``), not
  restated. One definition of "the same app" for both halves of D-I.
* **Defined only.** A draft never adopts from another draft. Identity converges
  at the moment a human vouches, and a draft carries no vouch.
* **This module stays pure.** The pod's facts arrive as an argument
  (``defined_apps``); no caller-free default reads a disk. The reader is
  ``pod_apps.defined_app_facts``, and the sweep pays for it only when it is
  actually minting an offer.
...
class AppIdentityDecision:
    ...
    ``adopted_from_bot`` is set only by ALPHA-3b's cross-bot adoption: the bot
    (or bots) the adopted id was already defined on. It is what lets the offer
    the user answers say *whose* app this is joining rather than presenting a
    borrowed identity as a fresh one — empty for every other branch, including
    D-H's adoption of the manifest's own id, where there is no other bot to
    name.
    """
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "reason": self.reason,
            "adopted_from_bot": self.adopted_from_bot,
        }
...
def cross_bot_adoption(
    manifest: Mapping[str, Any],
    *,
    name: str = "",
    bot_id: str = "",
    defined_apps: Mapping[str, Mapping[str, Any]] | None = None,
) -> AppIdentityDecision | None:
    """**D-I half 2 (ALPHA-3b).** The pod's equivalent DEFINED app, or ``None``.

    Returns an ``adopted`` decision carrying an id that already exists on
    another bot in this pod, so that promoting the second bot's draft of an
    app the pod already has converges on ONE identity instead of minting a
    second one for the same thing. ``None`` means "the pod has no equivalent"
    and the caller confers as it did before ALPHA-3b — this function never
    blocks and never confers.

    ``defined_apps`` is ``{app_id: {"name", "evidence", "bots", ...}}``, the
    facts shape ``app_grouping`` works in, built for the pod's DEFINED apps by
    ``pod_apps.defined_app_facts``. **Defined only**, because a draft adopting
    from another draft would propagate an identity no human has vouched for,
    and the vouch is the whole event D-I converges on.

    THE EQUIVALENCE IS NOT RE-STATED HERE. It is
    ``app_grouping.equivalent`` — the same function, the same threshold, the
    same three clauses the Apps page groups rows with (ALPHA-3a). A second
    copy would be a second definition of "the same app", and the two halves of
    D-I would eventually disagree about an id that by then is durable.

    Two consequences of reusing it, both wanted:

    * Its **no-shared-bot** clause means a draft never adopts from a defined
      app on its OWN bot. That is a duplicate record on one bot, which is the
      deduplication work; adopting there would quietly merge two records the
      operator can still see separately.
    * Its **evidence** clause is compared on the draft's own manifest, so this
      function needs no pod, no clock and no filesystem — the module's purity
      contract survives ALPHA-3b intact.

    More than one match resolves through ``app_grouping.lead_of`` rather than
    by refusing: the matches are, by construction, apps the page already draws
    as one grouped row, and adopting that row's lead is the answer that agrees
    with what the operator is looking at. Refusing would confer a *third* id
    for the same app, which is the failure D-I exists to stop.
    """
    if not defined_apps:
        return None
    from .app_grouping import (
    ...
    draft = {
        "name": name or _manifest_name(manifest),
        "evidence": evidence_signature(evidence_paths(dict(manifest))),
        "bots": {bot_id} if bot_id else set(),
    }
    matched: dict[str, Mapping[str, Any]] = {}
    bases: set[str] = set()
    for app_id, facts in defined_apps.items():
        if not isinstance(app_id, str) or not is_canonical_app_id(app_id):
            continue
        basis = equivalent(draft, facts)
        if basis is None:
            continue
        matched[app_id] = facts if isinstance(facts, Mapping) else {}
        bases.add(basis)
    if not matched:
        return None

    adopted = lead_of(sorted(matched), {k: dict(v) for k, v in matched.items()})
    source = ", ".join(
        sorted(str(b) for b in ((matched[adopted] or {}).get("bots") or ()) if b)
    )
    basis = "name and files" if GROUP_BASIS_NAME_AND_FILES in bases else "name"
    also = (
        f" (also equivalent to {', '.join(sorted(k for k in matched if k != adopted))})"
        if len(matched) > 1
        else ""
    )
    return AppIdentityDecision(
        app_id=adopted,
        mode="adopted",
        # No prior: this draft's own legacy-chain id is a scanner handle, and
        # reporting one here would read as an id this promotion moved.
        prior_app_id="",
        adopted_from_bot=source,
        reason=(
            f"the pod already has this app defined on "
            f"{source or 'another bot'} as {adopted!r} (matched on {basis})"
            f"{also}: promotion adopts the pod's identity rather than "
            f"conferring a second one for the same app (D-I / ALPHA-3b)"
        ),
    )
...
def adopt_or_confer_app_id(
    ...
    * A draft the POD already has, defined on another bot, adopts THAT app's
      id (``mode="adopted"``, ``adopted_from_bot`` set) — D-I half 2, ALPHA-3b.
      This is checked after the true-draft test and before conferral, so it
      changes which id a conferral-bound draft gets and changes nothing about
      which manifests are conferral-bound. Inert unless the caller passes
      ``defined_apps``; see :func:`cross_bot_adoption`.
    ...
    * Neither available → ``mode="blocked"``; the caller must refuse rather than
    ...
    ``bot_id`` / ``defined_apps`` are ALPHA-3b's inputs and both default to
    empty, so every existing caller keeps exactly the behaviour it had: with no
    pod facts there is no equivalent to find, and the function is the one D-H
    shipped.
    ...
    **THE DRAFT CHECK RUNS BEFORE THE RESOLVER** (brief §6's guardrail, and the
    ...
    if draft_id_of(manifest) and not _canonical_app_id_field(manifest):
        ...
        # ALPHA-3b, and its position is the point: AFTER the true-draft check
        # (so D-H's rule about which manifests may be conferred at all is
        # untouched) and BEFORE conferral (so an app the pod already has is
        # never given a second id).
        cross = cross_bot_adoption(
            manifest, name=candidate, bot_id=bot_id, defined_apps=defined_apps
        )
...
def offer_text(
    ...
    *,
    ...
    lines = []
    rebuilt = _rebuilt_document_offer(manifest)
    if rebuilt:
        # Artifact-by-reference, part 3: a document the assistant has rebuilt
        # N times gets the operator's own sentence (brief, verbatim) instead
        # of the generic pitch — the payoff is "edit instead of rewrite", not
        # "tell you if it doesn't run". Everything after the copy (the gate,
        # the cadence cap, the Proposal shape) is unchanged.
...
def _rebuilt_document_offer(manifest: Mapping[str, Any]) -> str:
    """The re-emission offer copy, or "" when the draft is not that shape.

    Reads ``conversation_evidence`` stamped by ``scanner`` from
    ``artifact_reemission.detection_from_match``; the sentence itself is
    that module's ``offer_copy`` so the two never drift.
    """
    if not isinstance(manifest, Mapping):
        return ""
    evidence = manifest.get("conversation_evidence")
    if not isinstance(evidence, Mapping) or evidence.get("detector") != "artifact_reemission":
        return ""
    from .artifact_reemission import offer_copy
    try:
        count = int(evidence.get("count") or 0)
    except (TypeError, ValueError):
        count = 0
    return offer_copy(count) if count >= 2 else ""
...
def resolve_rename(
    ...
    **``defined_apps`` must be passed wherever the offer passed it** (ALPHA-3b).
    Re-running the decision without the pod's facts would take a draft whose
    offer adopted a sibling bot's id and confer it a brand-new slug — the
    rename would move the id after all, through the one path §7.2 built a
    confirmation turn to protect. The wizard passes the same facts on both
    hops; a caller with none is unchanged, because it had no adoption to lose.
    """
