...
Spec: internal/spec-forge-side-effects-2026-06-02.md §5 (Materialize phase),
amended by internal/spec-heartbeat-instruction-2026-06-03.md.
...
from . import app_run_shim as _app_run_shim
...
# pattern lists. Decision D3 (internal/decision-security-defaults-2026-06-10.md)
...
# internal/spec-heartbeat-instruction-2026-06-03.md §3.
...
def install_heartbeat_instruction(
    ...
    *,
    ...
    Spec: internal/spec-heartbeat-instruction-2026-06-03.md §3.
    ...
    # ``pkg=<pkg_id>`` is written into the managed-section marker and later
    # parsed back out and compared for equality by
    # ``remove_heartbeat_instruction`` (below) to decide whether teardown owns
    # the section. Both ends must agree on the same string, and the ``app_id``
    # this module also takes is a separate value carried on its own axis (baked
    # into generated wrapper scripts as ``APP_ID`` and emitted in Signal
    # ``details.app_id``) — the resolver would collapse the two.
    marker_kv = []
    if pkg_id:
...
def install_scheduled_jobspec(
    ...
    *,
    ...
    # An app's scheduled action installs an `ai.evolve.<bot>.<app>` daemon
    # between deploys, so its label has to reach the incursion detector's
    # owned-job registry now — expected_plist_labels() only picks it up on the
    # NEXT deploy, and until then the detector would page for a job Evolve
    # just installed. Best-effort by contract (see record_owned_job_labels).
...
def install_launchd_command_action(
    ...
    **App-run attribution (AL-1.2 Lane B).** When ``app_id`` is supplied the
    plist also carries the claim-minting ``openclaw`` shim: the shim directory
    is prepended to ``PATH`` and ``EVOLVE_APP_ID`` / ``EVOLVE_APP_RUN_LABEL`` /
    ``EVOLVE_APP_RUN_CLAIM_DIR`` are exported, so a bare ``openclaw agent`` in
    the app's cron script mints a session id, writes
    ``{shared}/{bot}/app-runs/<uuid>.json`` and passes ``--session-id`` — the
    AL-0.4 contract the plugin's ``before_agent_run`` consumer joins on. See
    ``app_run_shim``. Wiring the shim is best-effort: a failure there logs and
    installs the cron unchanged, because attribution is observation and must
    never cost the user their app.
    ...
    # AL-1.2 Lane B producer: put the claim-minting openclaw shim in front of
    # the PATH we just computed, so an `openclaw agent` run from this app's
    # cron script is stamped with the app it serves. Additive and best-effort
    # — logic lives in app_run_shim.py, the call here is two lines.
    if app_id:
        env_sub = _app_run_shim.app_run_env(
            env_sub,
            workspace=workspace,
            shared_dir=(shared_dir if shared_dir is not None else
                        (net.get("sharedDir") or DEFAULT_SHARED_DIR)),
            bot_id=bot_id,
            app_id=app_id,
            label=label_sub,
        )
...
def repair_app_cron_env_paths(
    ...
    *,
    ...
    """Self-heal: ensure every installed launchd app-cron plist carries the
    ``PATH`` the installer would write today — openclaw/node reachable, and the
    AL-1.2 app-run attribution shim in front of them.
    ...
    daily-digest + ledger morning-briefing/note-taker, 2026-06-22). App crons
    installed before the Lane B producer landed carry a PATH but no shim, so
    their turns stay unattributed forever. Both are the same repair: re-install
    the action through the current command installer. Idempotent in both
    directions — once a plist has a PATH *and* (for an app with an id) the shim
    directory first on it, this loop skips it, so this is a one-time heal per
    reason and then a no-op.

    Returns ``{"checked", "missing": [labels], "reason": {label: why},
    "healed": [labels], "failed": [{label, error}]}``. ``check_only=True``
    reports ``missing`` without installing.
    """
        ...
        for manifest in manifests:
            for action in (getattr(manifest, "scheduled_actions", None) or []):
                ...
                try:
                    ...
                    continue
                app_id = str(getattr(manifest, "id", "") or "")
                # Two independent reasons to re-install, both idempotent (once
                # healed the plist carries the key and this loop skips it):
                #   1. no PATH at all — the 2026-06-22 openclaw exit-127 fix
                #   2. no app-run shim on PATH — the AL-1.2 Lane B producer,
                #      which otherwise reaches only apps installed after it
                #      shipped (the fleet's existing app crons would stay
                #      unattributed forever).
                shim_dir = _app_run_shim.shim_dir_for(workspace)
                if "<key>PATH</key>" not in text:
                    reason = "no PATH (openclaw exit-127 risk)"
                elif app_id and f"<string>{shim_dir}" not in text:
                    reason = "no app-run attribution shim on PATH"
                else:
                    continue
                report["missing"].append(label_sub)
                report["reason"][label_sub] = reason
                if check_only:
                    continue
                result = install_launchd_command_action(
                    ...
                    app_id=app_id, shared_dir=shared_path,
                    network=net, bootstrap=bootstrap,
                )
                if result.get("ok"):
...
def heal_app_cron_paths_into(result, perm_check, bot_ids, shared_dir, network,
    ...
    try:
        ...
        return
    reasons = heal.get("reason") or {}
    for lbl in heal.get("healed", []):
        result.checks.append(perm_check(category=C, target=lbl, ok=True,
                             detail="re-installed launchd plist: "
                                    + reasons.get(lbl, "no PATH (openclaw exit-127 risk)")))
    healed = set(heal.get("healed", []))
    if check_only:  # report-only: the drift is named, no fix applied
        for lbl in heal.get("missing", []):
            if lbl not in healed:
                result.checks.append(perm_check(category=C, target=lbl, ok=False,
                    detail="app-cron plist has "
                           + reasons.get(lbl, "no PATH (openclaw exit-127 risk)"),
                    fix_description="re-install app-cron plist with a homebrew-aware "
                                    "PATH and the app-run attribution shim"))
...
# internal/audit-gallery-framework-2026-07-02.md §2 S4). Before this,
    ...
    *,
    ...
    if pkg_id:
        ...
        for tok in kv.split():
            ...
            if sep and k == "pkg":
                marker_pkg = v
        # identity: see resolve_app_id — the equality half of the marker
        # vocabulary above; the compared string must be the one that was
        # stamped, whatever it is.
...
# ── AL-1.2 Lane B: scheduled-turn claim writer ───────────────────────────────
#
# The AL-0.4 contract (design-app-attribution-2026-08-15 §4.1, probe-verified
# 2026-08-15 and re-verified end to end through the shipped consumer on
# 2026-08-20): a wrapper that shells out to ``openclaw agent`` for an app's
# scheduled turn mints a UUIDv4, writes ``{shared}/{bot}/app-runs/<uuid>.json``
# = ``{app_id, label, ts}``, then invokes ``openclaw agent --session-id
# <uuid>``. The plugin (apps/scheduledAttribution.ts) joins ``event.sessionId``
# against the claim in ``before_agent_run`` and grades the session
# ``scheduled`` / ``claim_file``. Claims are single-use (the plugin consumes
# them) and swept after 24h.
#
# THE LIVE PRODUCER IS ``app_run_shim`` — the claim-minting ``openclaw`` shim
# that ``install_launchd_command_action`` puts first on an app cron's PATH.
# The shim writes the claim in bash because it runs inside the app's own cron
# process, where importing this package is neither available nor desirable;
# this function stays the Python-side writer of the SAME contract (and the
# shape both are pinned to — see ``tests/test_app_run_shim.py``), for callers
# that already run in an Evolve process.
#
# The forge dispatcher (bot_forge.py) also passes --session-id and MUST NEVER
# write claims: forge turns are Evolve subagent turns and stay unattributed by
# design (§2 non-goals; see the negative test in tests/test_app_run_claims.py).

APP_RUNS_DIR_NAME = "app-runs"

_CLAIM_SESSION_ID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,199}$")


def write_app_run_claim(
    shared_dir: "Path | str",
    bot_id: str,
    session_id: str,
    app_id: str,
    ...
    """Write the claim file binding a wrapper-minted session id to an app.

    Runs as the BOT user (the wrapper's own context) — plain writes, no sudo:
    ``{shared}/{bot}/app-runs/`` is created 0755 on first use and the claim
    itself is written atomically (same-dir temp + ``os.replace``) with the
    mode pinned to 0644 so the sweep/consumer never races a partial file.

    Returns ``{ok, path, error?}``. Rejects a session id that is not
    filename-safe (the id keys a file lookup — never sanitize, refuse).
    """
    if not isinstance(session_id, str) or not _CLAIM_SESSION_ID_RE.match(session_id or ""):
        return {"ok": False, "path": None,
                "error": f"unsafe session_id for claim file: {session_id!r}"}
    if not isinstance(app_id, str) or not app_id.strip():
        return {"ok": False, "path": None, "error": "app_id is required"}
    try:
        runs_dir = Path(shared_dir) / bot_id / APP_RUNS_DIR_NAME
        runs_dir.mkdir(mode=0o755, parents=True, exist_ok=True)
        dest = runs_dir / f"{session_id}.json"
        payload = json.dumps({
            "app_id": app_id.strip(),
            "label": str(label or ""),
            "ts": _dt.datetime.now(_dt.timezone.utc).isoformat(),
        }, indent=2) + "\n"
        fd, tmp = tempfile.mkstemp(dir=str(runs_dir), prefix=".claim-", suffix=".tmp")
        try:
            with os.fdopen(fd, "w") as f:
                f.write(payload)
            # Pin 0644: mkstemp mints 0600 and os.replace would carry it onto
            # the dest (the tempfile-rename mode gotcha).
            os.chmod(tmp, 0o644)
            os.replace(tmp, dest)
        finally:
            try:
                ...
                logger.debug("claim tmp cleanup: %s", cleanup_err)  # gone after replace — normal
        return {"ok": True, "path": str(dest)}
    except Exception as exc:  # noqa: BLE001 — claim write is best-effort for callers
