...
# ── AL-1.2 Lane B: scheduled-turn claim writer (UNWIRED BY DESIGN) ───────────
#
# The AL-0.4 contract (design-app-attribution-2026-08-15 §4.1, probe-verified):
# a wrapper that shells out to ``openclaw agent`` for an app's scheduled turn
# mints a UUIDv4, writes ``{shared}/{bot}/app-runs/<uuid>.json`` =
# ``{app_id, label, ts}``, then invokes ``openclaw agent --session-id <uuid>``.
# The plugin (apps/scheduledAttribution.ts) joins ``event.sessionId`` against
# the claim in ``before_agent_run`` and grades the session ``scheduled`` /
# ``claim_file``. Claims are single-use (the plugin consumes them) and swept
# after 24h.
#
# NO live producer calls this today — the 2026-08-15 census found zero
# launchd/crontab surfaces that shell out to ``openclaw agent``. It ships as a
# tested, reusable helper so the first such wrapper (AL-2.x) doesn't reinvent
# the contract. The forge dispatcher (bot_forge.py) also passes --session-id
# and MUST NEVER write claims: forge turns are Evolve subagent turns and stay
# unattributed by design (§2 non-goals; see the negative test in
# tests/test_app_run_claims.py).
