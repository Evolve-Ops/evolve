...
TWO PACK CARRIERS, NOT ONE (AL-3.2). A gallery package's pack lives under
``gallery/<pkg_id>/files/`` and is found by ``pkg_id``; an app snapshotted by
AL-3.1 has one at ``{shared_dir}/apps/packs/<app_id>/`` and is found by
``app_id``. Both hold SOURCE digests and both can answer §9.6's
re-substitution question. Only the first existed when this module was written,
so an app installed by AL-3.2's deterministic install — which is keyed on
``app_id`` and has no ``pkg_id`` at all — would have read ``recorded`` for its
digest carrier and ``differs`` for every placeholder-bearing file: the panel
would have reported a correct, machine-verified install as unexplained drift on
every bot but the source. ``app_pack_dir`` is that second carrier, checked
first because it is the app's own and the gallery one is at best a sibling.
...
def build_package_view(
    ...
    *,
    ...
    ``app_pack_dir`` is this app's own AL-3.1 files-pack when one exists (see
    the module docstring). It is a directory the caller has already resolved —
    never a flag — so a caller that cannot reach ``{shared_dir}`` passes None
    and the panel falls back to exactly what it did before, rather than
    asserting a carrier it did not read.
    """
    ...
    entries, sha_kind, sha_kind_bot = _source_entries(
        ordered, vnext, ws_files, packs, app_pack_dir)
...
def _source_entries(
    ...
    if vnext is not None and (vnext.package.get("files") or []):
        ...
        # knowable; where it does not, "recorded" is the honest label. An
        # AL-3.1 pack counts as knowable for the same reason a gallery one
        # does, and more directly: ``app_snapshot`` writes the Spec's
        # ``package.files[].sha256`` FROM that pack's source digests, so where
        # it exists the Spec's digests are source digests by construction.
        return entries, (SHA_SOURCE if (pack_bot or app_pack_dir) else SHA_RECORDED), None
...
def _substitution_explains(
    ...
    *,
    ...
    """
    if pack_context_for is None or not (pack or app_pack_dir):
        return False
    try:
        ...
        # The app's OWN pack wins over a gallery sibling: it is keyed on the
        # app's identity, its digests are the ones the Spec mirrors, and it is
        # what AL-3.2's install actually materialized from.
        if app_pack_dir is not None:
            pack_dir = app_pack_dir
        else:
