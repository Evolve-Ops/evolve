...
FAIL-OPEN IS NOT FAIL-SILENT, THOUGH. Every claim-side failure branch prints
one line to the app cron's own stderr before falling through. It printed none
until 2026-09-03, when the sibling ``app-runs/`` permissions drift (see
``bot_shared_subdirs``) showed what that costs: the dir EXISTS but is owned by
``evolve``, so the ``mkdir -p`` guard passes and the claim WRITE is what
EACCESes — under ``2>/dev/null``. An operator reading the err log then sees
exactly what a legitimately claim-free app produces: nothing. The breadcrumb
is what makes those two states distinguishable without costing the fail-open
guarantee a thing.
...
if [ "$_evolve_sub" = "agent" ] && [ "$_evolve_pinned" = 0 ] \
   && [ -n "$_evolve_app" ] && [ -n "$_evolve_claim_dir" ]; then
    ...
    # FAIL-OPEN, NOT FAIL-SILENT. Every branch below still ends at the exec at
    # the bottom with the original argv — but each one says why first. Until
    # 2026-09-03 the failure branches printed nothing while the success branch
    # printed a line, so an operator reading an app cron's err log could not
    # tell "attribution is broken" from "this app never calls openclaw": both
    # were zero lines. That is the same absence-of-a-record that let the
    # sibling exec-failures drift live undetected (see bot_shared_subdirs).
    if [ -z "$_evolve_sid" ]; then
        echo "evolve app-run shim: no usable uuid source; $_evolve_app turn runs unattributed" >&2
    elif ! mkdir -p "$_evolve_claim_dir" 2>/dev/null; then
        echo "evolve app-run shim: claim dir $_evolve_claim_dir is not creatable; $_evolve_app turn runs unattributed" >&2
    else
        ...
        if printf '{\n  "app_id": "%s",\n  "label": "%s",\n  "ts": "%s"\n}\n' \
           ...
           && mv -f "$_evolve_tmp" "$_evolve_claim_dir/$_evolve_sid.json" 2>/dev/null; then
            ...
            # The shape the 2026-09-03 drift produced: the dir EXISTS, so the
            # mkdir guard above passes, and the write is what EACCESes. Name
            # the dir and the likely cause — an operator with this line can go
            # straight to `ls -ld`.
            echo "evolve app-run shim: claim write failed in $_evolve_claim_dir (must be writable by this bot; check owner/mode); $_evolve_app turn runs unattributed" >&2
        fi
    fi
fi
