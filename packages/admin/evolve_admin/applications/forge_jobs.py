...
class ForgeJob:
    ...
    # at the same app survive into the fresh install). The flag is
    ...
    # Phase 4.5 honesty flag (audit slate S2, 2026-07-02). True when the job
    # reached ``complete`` but one or more scheduled_actions[] failed to
    # materialize — the app's files/instructions are real, its schedule is
    # (partially) not. status stays "complete" on purpose: flipping the whole
    # install to failed would misrepresent the non-scheduled parts and break
    # the sequential starter-pack installer's completion check. The per-action
    # detail lives in context_snapshot["scheduled_actions_installed"].
    ...
    def _load_from_dir(directory: Path) -> None:
        ...
        for p in sorted(directory.iterdir()):
            if p.suffix == ".json" and p.is_file():
                try:
                    data = json.loads(p.read_text())
                    # identity: see resolve_app_id — ``data`` is a ForgeJob
                    # record, not a manifest; ``pkg_id`` is the job's own field
                    # (the gallery catalog key) and the job carries a separate
                    # ``app_id`` alongside it.
...
def clone_for_retry(
    ...
    *,
    ...
    run_number = _allocate_next_run_number(
        # identity: see resolve_app_id — the run-number sequence is keyed on
        # ``(pkg_id, bot_id)`` (see ``_allocate_next_run_number``); ``app_id`` is
        # inherited separately via ``_CLONE_INHERIT_FIELDS``.
    ...
    clone = ForgeJob(
        ...
        # prior — stale files there would still ship stale content.
        # See PR #2210 / bot_forge.build_prompt(is_retry).
        is_retry=True,
    )
