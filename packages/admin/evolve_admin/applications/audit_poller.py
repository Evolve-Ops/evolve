...
def _outbox_order_key(path: Path) -> tuple:
    """The total order an outbox drains in.

    ``(st_mtime, is_run_summary, record_ts, name)``:

    * ``st_mtime`` first — arrival time is the best evidence of write order,
      and across separate runs it is decisive.
    * ``is_run_summary`` second — a run's SUMMARY must be drained after that
      run's FINDINGS, because the summary's sweep resolves every Signal whose
      signature it does not carry. That is the invariant the old docstring
      claimed and the old key did not enforce.
    * ``record_ts`` third — the bot's own claim about when it produced the
      record, used only between peers that tie on both keys above.
    * ``name`` last, so the order is TOTAL. Two records identical on every
      other key must still have one fixed order, or this function is right on
      one run and wrong on the next.

    Every failure degrades to a position rather than an exception: an
    unstattable file sorts first (the drain loop is about to report it
    unreadable), and an unparseable one is simply not a summary. Sorting must
    never be where a broken record becomes invisible.

    ``kind`` is ``tier2_run_summary`` today and has carried other prefixes, so
    this matches on the suffix — a future ``<tier>_run_summary`` is ordered
    correctly the day it is introduced rather than the day someone notices it
    is not.
    """
    try:
    ...
    return (mtime, str(kind).endswith("run_summary"), str(ts), path.name)


def _list_outbox_files(bot_user: str) -> list[Path]:
    """Return processable outbox files (skips the _ingested archive dir).

    Ordered by `_outbox_order_key`, so a run's summary is drained after that
    run's findings.

    Until 2026-09-18 the only key was ``st_mtime``, and a bot writes a run's
    records in one burst: three files routinely land on the same filesystem
    timestamp. Python's sort is stable, so equal keys leave the order exactly
    as ``iterdir()`` produced it — whatever the filesystem felt like, and not
    the same twice. Drain a summary first and its sweep resolves the Signals
    the findings behind it are about to re-observe, so each one REOPENS instead
    of deduping and the same outbox produces different counters run to run.
    That was never a flaky test; it was an unordered input the code described
    as ordered.
    """
    ...
    try:
        ...
        try:
            ...
            pass
    files.sort(key=_outbox_order_key)
    return files
...
def _list_infra_outbox_files(shared_dir: Path) -> list[Path]:
    ...
    Same ordering, and the same reason, as `_list_outbox_files` — see
    `_outbox_order_key`. This list had the identical `st_mtime`-only key, so it
    had the identical unspecified order whenever two records shared a
    timestamp; it is fixed here rather than left as the copy that still bites.
    """
