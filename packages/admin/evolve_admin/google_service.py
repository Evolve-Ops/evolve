...
#: Total attachment budget per send. Gmail's own limit is 25MB for the whole
#: message; 20MB leaves room for the body + base64 overhead.
ATTACHMENT_TOTAL_CAP_BYTES = 20 * 1024 * 1024

#: Outbound-send guards (below the LLM, so a looping model cannot argue with
#: them). Field calibration: the 2026-08-31 PoC-bot incident was ~16 identical
#: emails in two minutes — a model on a weak-schema rung re-issuing the same
#: call while its prose claimed otherwise. The duplicate window stops that
#: storm at message #2; the rate cap (8 per 10 min) is 20x the incident's
#: legitimate need and still an order of magnitude under its storm rate.
SEND_DUPLICATE_WINDOW_SECONDS = 15 * 60
SEND_RATE_MAX = 8
SEND_RATE_WINDOW_SECONDS = 10 * 60


def _send_ledger_path(network: dict, bot_id: str) -> Path:
    shared = Path(network.get("sharedDir") or str(config.DEFAULT_SHARED_DIR))
    return shared / "google_send_ledger" / f"{bot_id}.jsonl"


def _read_recent_sends(network: dict, bot_id: str, now: float) -> list[dict]:
    """Ledger rows still inside the widest guard window. File-backed on
    purpose: a daemon-process counter resets on every restart, and a
    restart mid-storm would re-arm the storm."""
    horizon = now - max(SEND_DUPLICATE_WINDOW_SECONDS, SEND_RATE_WINDOW_SECONDS)
    try:
        lines = _send_ledger_path(network, bot_id).read_text(
    ...
    for line in lines[-200:]:
        try:
            ...
            continue
        if isinstance(row, dict) and row.get("ts", 0) >= horizon:
            rows.append(row)
    return rows


def _check_send_guards(network: dict, bot_id: str, digest: str, now: float) -> None:
    """Raise (refusing the send) on a duplicate or a rate-cap trip. The
    error text is written FOR the bot: it says what happened and what to do
    instead, because the observed failure mode is a model that retries."""
    recent = _read_recent_sends(network, bot_id, now)
    for row in recent:
        if row.get("digest") == digest and (
                now - row["ts"]) < SEND_DUPLICATE_WINDOW_SECONDS:
            raise ValueError(
                "duplicate send refused: an email with this exact recipient, "
                "subject, body, and attachments was already sent in the last "
                f"{SEND_DUPLICATE_WINDOW_SECONDS // 60} minutes. Do NOT retry "
                "the same call — it already succeeded. If something about it "
                "was wrong, change the content (e.g. actually add the "
                "attachments parameter) or ask the user.")
    in_rate_window = [r for r in recent
                     if (now - r["ts"]) < SEND_RATE_WINDOW_SECONDS]
    if len(in_rate_window) >= SEND_RATE_MAX:
        _emit_send_rate_signal(bot_id, len(in_rate_window))
        raise ValueError(
            f"send rate cap: {SEND_RATE_MAX} emails in "
            f"{SEND_RATE_WINDOW_SECONDS // 60} minutes reached for this bot. "
            "Stop sending, tell the user the cap tripped, and wait for their "
            "instruction.")


def _record_send(network: dict, bot_id: str, digest: str, now: float) -> None:
    """Append one row per SUCCESSFUL send (failures cost nothing external,
    so they never count against the caps)."""
    p = _send_ledger_path(network, bot_id)
    try:
        p.parent.mkdir(parents=True, exist_ok=True)
        with p.open("a", encoding="utf-8") as f:
            f.write(json.dumps({"ts": now, "digest": digest}) + "\n")
    except OSError:
        # A ledger write failure must not fail a send that already happened;
        # the guards simply see one row fewer.
        return


def _emit_send_rate_signal(bot_id: str, count: int) -> None:
    """Best-effort Signal on a rate trip — same defensive shape as
    ``_emit_reauth_signal`` (a signal-store hiccup never breaks the
    tool-call's own error path)."""
    try:
        from . import config as _config
        from signals.store import observe  # type: ignore[import]
        net = _config.load_network()
        ...
        observe(
            shared_dir,
            producer="gmail_integration_health",
            type="gmail_send_rate_tripped",
            severity="warn",
            scope="bot",
            bot_id=bot_id,
            signature=f"{bot_id}|gmail_send|rate_tripped",
            body=(f"Bot {bot_id!r} hit the outbound email rate cap "
                  f"({count} sends inside the window) — a possible "
                  f"retry loop; sends are being refused."),
            details={"bot_id": bot_id, "sends_in_window": count},
        )
    except Exception:
        return


#: Words in an email's own text that claim an attachment. Used only to add a
#: WARNING to the tool result when no attachment was actually provided —
#: never to block the send (plenty of honest emails say "no attachment
#: needed"); the duplicate/rate guards handle the storm case.
_ATTACHMENT_CLAIM_WORDS = ("attach", "enclos")


def _workspace_root(bot_id: str, network: dict) -> Path:
    """The ONLY directory gmail_send may read attachments from. The daemon
    runs as the evolve user, whose read ACL spans pod secrets — so an
    unconfined path parameter would let a prompt-injected bot mail out
    key material. Confinement to the calling bot's own workspace keeps the
    attachment surface exactly as wide as what the bot can already read."""
    from .config import bot_home
    return (bot_home(bot_id, network) / ".openclaw" / "workspace").resolve()


def _read_attachment(path_str: str, root: Path) -> tuple[str, bytes]:
    """Resolve + read one attachment, symlink-safe.

    ``resolve()`` follows symlinks BEFORE the containment check, so a link
    pointing outside the workspace fails the check; ``O_NOFOLLOW`` on the
    open closes the resolve→open swap race on the final component
    (the root-TOCTOU shape from the privileged-fs-ops incident).
    """
    p = Path(path_str)
    rp = (p if p.is_absolute() else root / p).resolve()
    if not rp.is_relative_to(root):
        raise ValueError(
            f"attachment must be inside the bot's workspace: {path_str}")
    flags = os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0)
    try:
        fd = os.open(str(rp), flags)
    except OSError as exc:
        raise ValueError(f"attachment unreadable: {path_str} ({exc.strerror})")
    with os.fdopen(fd, "rb") as f:
        return rp.name, f.read()
...
def gmail_send(bot_id: str, args: dict, network: dict | None = None) -> dict:
    ...
    Arguments:
        ...
        attachments: optional list of file paths INSIDE the bot's workspace
            (absolute or workspace-relative), total <= 20MB. The gmail.send
            scope already covers attachments — no re-auth involved.
    """
    ...
    if attachments:
        root = _workspace_root(bot_id, network)
        total = 0
        for path_str in attachments:
            name, data = _read_attachment(str(path_str), root)
            total += len(data)
            if total > ATTACHMENT_TOTAL_CAP_BYTES:
                raise ValueError(
                    f"attachments exceed {ATTACHMENT_TOTAL_CAP_BYTES // (1024 * 1024)}MB total")
            guessed, _enc = mimetypes.guess_type(name)
            maintype, _, subtype = (guessed or "application/octet-stream").partition("/")
            msg.add_attachment(data, maintype=maintype, subtype=subtype, filename=name)
            attached_names.append(name)

    # Guards run AFTER the message is fully built so the digest covers what
    # would actually go out — a corrected resend (same text, attachment now
    # actually present) digests differently and passes.
    digest = hashlib.sha256("\x1f".join([
        ",".join(sorted(to + cc + bcc)), subject, body,
        ",".join(attached_names),
    ]).encode("utf-8")).hexdigest()
    now = time.time()
    _check_send_guards(network, bot_id, digest, now)

    raw = base64.urlsafe_b64encode(msg.as_bytes()).decode()
    result = service.users().messages().send(
        userId="me", body={"raw": raw}
    ).execute()
    _record_send(network, bot_id, digest, now)
    ...
    if not attached_names:
        hay = f"{subject}\n{body}".lower()
        if any(w in hay for w in _ATTACHMENT_CLAIM_WORDS):
            out["warning"] = (
                "this email's text mentions an attachment, but the call "
                "provided no 'attachments' parameter — it was sent WITHOUT "
                "any attachment. To attach a file, pass attachments: "
                "[\"<path inside your workspace>\"]. Do not resend the same "
                "email unchanged.")
    return out
