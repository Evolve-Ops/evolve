...
from mcp.types import PaginatedRequestParams, TextContent, Tool
...
def build_starlette_app(
    ...
    *,
    ...
    # mcp 2.x removed the `@mcp_server.list_tools()` / `@mcp_server.call_tool()`
    # decorators; handlers are constructor keyword arguments now. The two
    # `_impl` functions below are the 1.x bodies UNCHANGED — same tools, same
    # TextContent, same logging — and the two adapters under them do nothing but
    # unwrap the request params and wrap the result. Keeping the bodies intact
    # is deliberate: this is a registration migration, and a diff that also
    # reshaped what a tool returns would be a wire change hiding inside a
    # dependency bump. The SSE transport, the `/messages` endpoint, the `/sse`
    # route and port 5051 are untouched — measured unchanged under 2.2.0.
    ...
    async def _on_list_tools(
        _ctx: ServerRequestContext, _params: PaginatedRequestParams | None = None
    ) -> ListToolsResult:
        return ListToolsResult(tools=await _list_tools_impl())

    async def _on_call_tool(
        _ctx: ServerRequestContext, params: CallToolRequestParams
    ) -> CallToolResult:
        content = await _call_tool_impl(params.name, dict(params.arguments or {}))
        # `is_error` stays False even for the unknown-tool and handler-error
        # paths, because 1.x carried those in the TEXT and every existing client
        # reads them there. Promoting them to the flag is a behaviour change
        # this PR deliberately does not make.
        return CallToolResult(content=list(content))

    mcp_server = Server(
        "evolve-pod", on_list_tools=_on_list_tools, on_call_tool=_on_call_tool,
    )
