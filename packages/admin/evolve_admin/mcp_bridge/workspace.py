...
class BotWorkspace:
    def __init__(self, bot_id: str, root: Path,
                 network: dict | None = None) -> None:
        """``network`` is the already-loaded config the caller resolved this
        bot from. Pass it: omitting it re-reads the *canonical*
        network.json, so a caller pointed at any other config (the MCP
        bridge accepts an explicit ``network_path``) would resolve the bot's
        macOS user from a different file than the one it read the bot out
        of — and every construction becomes a read of whatever pod state the
        host happens to have, which is what made the hot-reload tests
        non-hermetic.
        """
        self.bot_id = bot_id
        self.root = root
        self.bot_user = get_bot_user(
            bot_id, load_network() if network is None else network)
