...
def breaker_reset_cmd(
    ...
    if prior is not None or breaker_type == "cost":
        ...
        if prior is not None:
            # Reactivating from the CLI accepts the window exactly as the
            # Reactivate button does. Without this the two operator paths
            # would disagree: a bot brought back with `breaker reset` would
            # re-trip on the next tick against the spend that tripped it.
