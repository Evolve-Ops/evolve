...
class SecretState(str, Enum):
    """Tri-state outcome of a vault read. ``str`` mixin so the value
    serializes straight into a JSON payload / log line.

    * ``PRESENT``    — the entry exists and decrypted; the value is usable.
    * ``ABSENT``     — the entry genuinely isn't stored (ENOENT on the
                       ciphertext, with the vault dir itself readable).
    * ``UNREADABLE`` — the entry may well exist, but THIS caller cannot
                       read it: the vault dir or ``.machine-key`` denied
                       the lookup (EACCES/EPERM), or the ciphertext failed
                       to decrypt. NOT "absent."
    """
...
def _stat_state(path: Path) -> SecretState:
    """Classify a path's existence WITHOUT collapsing permission denials.

    ``Path.exists()`` returns False both when a file is missing and when a
    parent dir denies the lookup. In the vault's case the second reading is
    wrong and load-bearing: ``{shared}/keystore/vault`` is 0750 ``evolve:wheel``,
    so every bot account gets EACCES on a stat of an entry that is really there.

    Only ``ENOENT`` earns ``ABSENT``. Every other ``OSError`` — EACCES/EPERM,
    but also ELOOP, ENOTDIR, EIO — resolves to ``UNREADABLE``, because the one
    answer we must never invent is "it isn't there."

    This is the same rule as ``evo/tools/fs_tristate.py`` (the ``exists() lies
    under 0700 parents`` bug class), deliberately re-stated here rather than
    imported: ``evo.tools.__init__`` eagerly imports ~45 tool modules, one of
    which (``action_keys``) imports THIS module, so importing the primitive
...
def _file_store_get_state(
    vault_dir: Path, key_name: str
) -> tuple[SecretState, str | None]:
    """Tri-state vault read — ``(state, value)``; value non-None iff PRESENT.

    The three-way split exists because folding "I cannot read this" into
    "this was never stored" produced the 2026-08-18 backup outage: PR #3696
    clamped ``.machine-key`` to 0640 ``evolve:wheel``, every bot-user backup
    daemon started getting None here, and the caller reported it to operators
    as an unfinished setup wizard step for 23 nights.
    See ``internal/finding-backups-silently-off-since-2026-08-18.md``.
    """
    path = vault_dir / f"{key_name}.enc"
    stat_state = _stat_state(path)
    if stat_state is not SecretState.PRESENT:
        return stat_state, None
    try:
        machine_key = _get_vault_key(vault_dir)
        raw = path.read_bytes()
    except Exception:
        # Includes the PermissionError _get_vault_key propagates when
        # ``.machine-key`` is unreadable to this uid — the exact #3696 shape.
        return SecretState.UNREADABLE, None
    from cryptography.fernet import InvalidToken

    try:
        return SecretState.PRESENT, _fernet(machine_key).decrypt(raw).decode()
    except InvalidToken:
        ...
        try:
            return SecretState.PRESENT, bytes(
...
def _file_store_get(vault_dir: Path, key_name: str) -> str | None:
    """Two-state vault read — the value, or None for absent OR unreadable.

    Retained for the many callers that genuinely only need the value.
    Anything that REPORTS on the outcome must use
    :func:`_file_store_get_state` instead, so "cannot read" never renders
    as "not configured".
    """
    return _file_store_get_state(vault_dir, key_name)[1]
...
def load_github_pat_state(shared_dir: Path) -> "tuple[SecretState, str | None]":
    ...
    The three states are NOT interchangeable to a caller that reports:

    * ``ABSENT``     — the operator has genuinely not stored a PAT. Telling
                       them to run the Backup → Cloud wizard is correct.
    * ``UNREADABLE`` — a PAT IS stored; this uid cannot decrypt it. Telling
                       them to run the wizard is WRONG and wastes the
                       incident — they already did, and re-running it will
                       not grant the caller vault access. This is the state
                       every bot-user backup daemon has been in since
                       2026-08-18.
    * ``PRESENT``    — usable value.
    """
    try:
        state, value = _file_store_get_state(
            _github_pat_vault_dir(shared_dir), GITHUB_PAT_KEY
        )
    except Exception:
        # Belt-and-braces: an unexpected failure is "cannot read", never
        # "not configured" — the whole point of this function.
        return SecretState.UNREADABLE, None
    if state is not SecretState.PRESENT:
        return state, None
    cleaned = (value or "").strip()
    if not cleaned:
        # Stored but empty — treat as never configured; the wizard IS the fix.
        return SecretState.ABSENT, None
    return SecretState.PRESENT, cleaned


def load_github_pat(shared_dir: Path) -> "str | None":
    """The pod-wide GitHub PAT from the file vault, or None.

    Two-state convenience wrapper. Callers that surface the outcome to an
    operator must use :func:`load_github_pat_state` instead — None here
    cannot distinguish "never stored" from "this uid cannot read the vault",
    and reporting the second as the first is the 2026-08-18 outage.
    """
    return load_github_pat_state(shared_dir)[1]
