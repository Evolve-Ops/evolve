...
def _resume_job(job, shared_dir: Path, *, dispatch_fn=None) -> None:
    ...
    # Chain-membered jobs (audit slate S5a) re-enter their install chain
    # instead of dispatching standalone: the chain's advancer runs this job
    # (it is the first non-succeeded link, now queued) AND continues the
    # links behind it — a bare dispatch would finish the app but strand the
    # rest of the bundle. Double-run is excluded structurally: the chain
    # advancer is the only dispatcher for a chain-stamped queued job.
    chain_id = str(job.context_snapshot.get("install_chain_id") or "")
    if chain_id:
        try:
            from ..applications.install_chain import (
                advance_chain_async, load_chain,
            )

            # Only delegate if the chain still exists: advance_chain_async
            # no-ops silently on a missing/unreadable chain, which would
            # strand this now-queued job forever (the sweeper only revisits
            # awaiting_oauth). Fall back to a standalone dispatch instead.
            if load_chain(chain_id, shared_dir) is not None:
                advance_chain_async(chain_id, shared_dir)
                return
            log.warning(
                "sweeper: job %s references missing chain %s — dispatching "
                "standalone so the resumed job still runs",
                job.job_id, chain_id,
            )
        except Exception as exc:
            log.error(
                "sweeper: chain re-entry failed for job %s (chain %s): %s — "
                "falling back to standalone dispatch",
                job.job_id, chain_id, exc,
            )
