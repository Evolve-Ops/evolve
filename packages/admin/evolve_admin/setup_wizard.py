...
def provisionable_channel_ids() -> tuple[str, ...]:
    ...
    vocabulary — invariant 7, internal/spec-users-meta-2026-06-15.md §5. We select
...
def _load_existing_config(net_path: Optional[Path] = None) -> dict:
    ...
    try:
        # install.json is pod-wide state under {shared_dir}, so it follows
        # network.json's sharedDir (read just above) — same shape as the
        # keystore probe below. DEFAULT_SHARED_DIR is only the fallback for
        # when no network.json was found at all.
        install_file = Path(result.get("shared_dir") or str(DEFAULT_SHARED_DIR)) / "install.json"
...
def _resolved_shared_dir(network_path: Optional[Path] = None) -> Path:
    """Resolve ``{shared_dir}`` the way every other consumer does — via
    network.json's ``sharedDir`` key, falling back to the platform default.

    ``DEFAULT_SHARED_DIR`` is a snapshot of the *platform* default taken at
    import (``/Users/Shared/evolve`` on macOS, ``/var/lib/evolve`` on Linux).
    It is completely blind to ``sharedDir``, so anything that reads or writes
    pod state through it lands in the wrong place on a pod that sets a custom
    one — invisibly, since the state is written *somewhere*, just not where
    the pod's readers look.

    Never raises. On an unresolvable config it returns ``DEFAULT_SHARED_DIR``,
    which is exactly what the callers used before this resolver existed, so a
    best-effort caller still writes somewhere rather than dropping the write.
    """
    try:
        from evolve_config import get_shared_dir, load_config, resolve_network_path
        return Path(get_shared_dir(load_config(str(resolve_network_path(network_path)))))
    except Exception:  # noqa: BLE001 — a resolver hiccup must not fail the caller
        return Path(DEFAULT_SHARED_DIR)
...
def _log_admin_action(action: str, result: str, bot: str = "", initiated_by: str = "wizard") -> None:
    ...
    # Must follow network.json's sharedDir — writing the audit trail under the
    # platform default on a custom-sharedDir pod makes it invisible to every
    # reader of {shared_dir}/logs/admin-actions.jsonl (and, where that path
    # isn't writable, silently drops the entry via the swallow below).
    log_path = _resolved_shared_dir() / "logs" / "admin-actions.jsonl"
    try:
...
# unprivileged `evo` account. Spec: internal/spec-evo-account-separation-
...
# the unified shared-key model (internal/spec-backup-key-distribution-
...
def _evo_cutover_migrate_backup_ssh_key(
    ...
    shared-key model (internal/spec-backup-key-distribution-unification-
...
def _evo_cutover_apply_acl(network_path: Optional[Path] = None) -> tuple[bool, str | None]:
    ...
    try:
        ...
        # sharedDir-aware: on a custom-sharedDir pod the platform default
        # holds no proposals/ or signals/, so the `p.exists()` guard below
        # would skip every grant and the step would report success having
        # done nothing — the exact silent failure this ACL exists to prevent.
        shared_dir = _resolved_shared_dir(network_path)
...
def _perform_evo_cutover(
    ...
    ok, err = _evo_cutover_apply_acl(network_path)
    if not ok:
...
def _per_bot_subdir_grants(
    commands: "dict[str, str]", shared: str, *, acl: bool, macos: bool,
) -> str:
    """NOPASSWD lines for the per-bot shared-store subdirs, from the registry.

    ``bot_shared_subdirs.BOT_SHARED_SUBDIRS`` is the single source: the same
    tuple ``deploy.fix_shared_dir_permissions`` creates these directories from
    and ``ensure_pod_perms`` re-verifies. Rendering the grants off it is what
    makes the 2026-09-03 drift class structurally impossible — a subdir cannot
    reach a privileged deploy step that has no matching grant, because both
    come off one list.

    ``acl=False`` renders the §9a ``mkdir`` / ``chmod <mode>`` / ``chown``
    trio (identical on both platforms). ``acl=True`` renders the §9b read-ACE
    leg for the entries that ask for one, in the argv the Perms seam actually
    execs: ``chmod +a`` on macOS, the ``setfacl`` access+default pair on
    Linux. Split because the two sections sit in different parts of the file
    and the ACL leg is platform-branched.

    Sudoers escaping stays here, not in the registry: visudo rejects a bare
    ``:`` in an argument pattern, so the Linux entry spec is written
    ``u\:evolve\:rX``. It is pinned literally rather than wildcarded because
    these path patterns carry a wildcard tail — a bare ``-m *`` there would
    let evolve grant ANY user access to any bot's subdir.
    """
    from .bot_shared_subdirs import BOT_SHARED_SUBDIRS, LINUX_READ_ACL_ENTRY

    def line(binary: str, *args: str) -> str:
        return f"evolve ALL=(root) NOPASSWD: {binary} {' '.join(args)}\n"

    out: list[str] = []
    if not acl:
        for entry in BOT_SHARED_SUBDIRS:
            out.append(line(commands["mkdir"], "-p", f"{shared}/{entry.glob}"))
        for entry in BOT_SHARED_SUBDIRS:
            out.append(line(commands["chmod"], entry.mode_token, f"{shared}/{entry.glob}"))
        for entry in BOT_SHARED_SUBDIRS:
            out.append(line(commands["chown"], "*", f"{shared}/{entry.glob}"))
        return "".join(out)

    acl_entries = [e for e in BOT_SHARED_SUBDIRS if e.evolve_read_acl]
    if macos:
        for entry in acl_entries:
            out.append(line(commands["chmod"], "+a", "*", f"{shared}/{entry.glob}"))
        return "".join(out)
    setfacl = commands["setfacl"]
    spec = LINUX_READ_ACL_ENTRY.replace(":", "\\:")
    for entry in acl_entries:
        target = f"{shared}/{entry.glob}"
        out.append(line(setfacl, "-m", spec, target))
        out.append(line(setfacl, "-d", "-m", spec, target))
    return "".join(out)
...
def _render_evolve_sudoers() -> str | None:
    ...
    content = (
        ...
        "# OC's PER-AGENT LLM key store (web/oc_agent_keys.py, storage shape S8).\n"
        "# The files the running agent actually reads its provider API key from.\n"
        "# `cat`, not a direct read: the OC gateway re-hardens agents/<a>/agent to\n"
        "# 0700 with NO acl after every auth write, so evolve cannot traverse in.\n"
        "# Without these the Credentials tab renders no LLM Providers section at\n"
        "# all and the operator cannot rotate the key that is billing (2026-09-04).\n"
        "# One wildcard per path segment reads like a bound. It is NOT one, and\n"
        "# nothing below should be trusted as if it were: sudo applies fnmatch\n"
        "# with FNM_PATHNAME only to the COMMAND path. Command ARGUMENTS are\n"
        "# matched as ONE space-joined string with FNM_PATHNAME off, so a `*` in\n"
        "# an argument pattern spans both spaces and '/'. A single\n"
        f"# `{cat}` invocation naming `{home}/a/.openclaw/agents/x /etc/shadow\n"
        f"# {home}/b/.openclaw/agents/c/agent/models.json` satisfies the\n"
        "# models.json grant below and reads /etc/shadow as root.\n"
        "# What bounds these grants is the CALLER, not the pattern: every path is\n"
        "# built in web/oc_agent_keys.py from a sanitised agent id (_AGENT_ID_RE)\n"
        "# plus a fixed relpath, one path per invocation, never from bot- or\n"
        "# operator-supplied text.\n"
        f"evolve ALL=(root) NOPASSWD: {cat} {home}/*/.openclaw/agents/*/agent/plugins/*/catalog.json\n"
        f"evolve ALL=(root) NOPASSWD: {cat} {home}/*/.openclaw/agents/*/agent/models.json\n"
        f"evolve ALL=(root) NOPASSWD: {cat} {home}/*/.openclaw/agents/*/agent/codex-home/auth.json\n"
        "# Listing the plugins dir turns eleven speculative `cat`s per agent into\n"
        "# one `ls` plus a read of the catalogs that actually exist — and finds a\n"
        "# provider directory Evolve has never heard of, which a fixed candidate\n"
        "# list cannot. A failed listing degrades to the candidate list, never to\n"
        "# an empty answer.\n"
        f"evolve ALL=(root) NOPASSWD: {ls} {home}/*/.openclaw/agents/*/agent/plugins\n"
        "\n"
        ...
        "# Spec: internal/spec-per-bot-users-management-2026-05-29.md. The admin UI's\n"
        ...
        "# Spec: internal/spec-google-workspace-suite-2026-06-04.md §8.\n"
        ...
        "\n"
        "# ── 5d. LLM key rotation into OC's per-agent runtime store ──────────────────\n"
        "# Brief: internal/dispatch/done/llm-keys-visible-and-rotatable-from-runtime-\n"
        "# store.md. `POST /api/admin/keys/<bot>/<provider>/rotate` with\n"
        "# storage=oc_agent_store writes the new key to EVERY agent's copy — that is\n"
        "# where the runtime reads it, so writing anywhere else leaves the gateway on\n"
        "# the stale key (decision B). TWO steps, not the five auth-profiles uses:\n"
        "# /tmp staging + cp, then chmod 600. There is deliberately NO chown grant.\n"
        "# oc_agent_keys.write_targets_for_provider only yields files the scan\n"
        "# FOUND, so the destination inode always already exists, and `cp` (no -p)\n"
        "# truncates and rewrites it in place — the bot's ownership survives\n"
        "# untouched. The chmod is NOT optional: `cp` also PRESERVES an existing\n"
        "# dest's mode, so a file that ever became 0644 would stay world-readable.\n"
        "# Read the wildcards as convention, not containment (see §2): these are\n"
        "# ARGUMENT patterns, matched against the whole space-joined argv with\n"
        "# FNM_PATHNAME off, so every `*` spans words and '/' alike — including the\n"
        "# `/tmp/evolve-llmkey-*` source, which on Linux (`cp -t`) would let a\n"
        "# crafted argv redirect the write. The bound is the caller:\n"
        "# oc_agent_keys_io builds one destination per invocation from a sanitised\n"
        "# agent id and a fixed relpath.\n"
        "# The relpath shapes are BOT_SECRET_AGENT_STORE_RELGLOBS in\n"
        "# secret_config_perms — keep the two in sync.\n"
        f"evolve ALL=(root) NOPASSWD: {cp} /tmp/evolve-llmkey-* {home}/*/.openclaw/agents/*/agent/plugins/*/catalog.json\n"
        f"evolve ALL=(root) NOPASSWD: {cp} /tmp/evolve-llmkey-* {home}/*/.openclaw/agents/*/agent/models.json\n"
        f"evolve ALL=(root) NOPASSWD: {cp} /tmp/evolve-llmkey-* {home}/*/.openclaw/agents/*/agent/codex-home/auth.json\n"
        f"evolve ALL=(root) NOPASSWD: {chmod} 600 {home}/*/.openclaw/agents/*/agent/plugins/*/catalog.json\n"
        f"evolve ALL=(root) NOPASSWD: {chmod} 600 {home}/*/.openclaw/agents/*/agent/models.json\n"
        f"evolve ALL=(root) NOPASSWD: {chmod} 600 {home}/*/.openclaw/agents/*/agent/codex-home/auth.json\n"
        "\n"
        ...
        "# Spec: internal/spec-oc-upgrade-from-ui-2026-07-28.md §4. The banner's 'Run\n"
        ...
        + (
            ...
            # `list *` does NOT cover the label-less enumeration: sudo fnmatches
            # the JOINED argument string, and the literal space in "list *"
            # cannot match empty. So `launchctl list <label>` was granted while
            # bare `launchctl list` was denied — 1,118 times in 3 days on the
            # reference pod, silently, because Scheduler.list() returns [] on
            # any failure. Callers read that as "no jobs registered"
            # (gateway_supervision's orphan sweep, spend_caps' cron pause).
            ...
            # ocadmin's legacy generic-agent sweep probes the per-user GUI
            # domain via `launchctl asuser <uid> launchctl list <label>`.
            #
            # The obvious grant — `{svc} asuser * {svc} list *` — is ROOT.
            # `launchctl asuser <uid> <program> [args…]` RUNS <program>, and a
            # sudoers ARGUMENT wildcard is matched with FNM_PATHNAME off, so
            # `*` spans spaces and '/': `asuser 0 /bin/sh -c … /bin/launchctl
            # list ai.openclaw.gateway` satisfies that pattern and execs a
            # shell as root. Same illusion-of-narrowness class as the
            # `npm install -g openclaw@*` grant refused in §7c above.
            #
            # So the uid is pinned by digit CLASS (no wildcard that could
            # absorb an injected program) and the label is the fixed constant
            # the probe uses. Two lines for 3- and 4-digit uids: macOS local
            # accounts start at 501, and a pod that ever crosses 999 adds a
            # line rather than widening the pattern.
            f"evolve ALL=(root) NOPASSWD: {svc} asuser [0-9][0-9][0-9] {svc} list ai.openclaw.gateway\n"
            f"evolve ALL=(root) NOPASSWD: {svc} asuser [0-9][0-9][0-9][0-9] {svc} list ai.openclaw.gateway\n"
        ...
        + (
            ...
            "# Same re-grant for OC's per-agent LLM key store (§5d writes them).\n"
            f"evolve ALL=(root) NOPASSWD: {setfacl} -m * {home}/*/.openclaw/agents/*/agent/plugins/*/catalog.json\n"
            f"evolve ALL=(root) NOPASSWD: {setfacl} -m * {home}/*/.openclaw/agents/*/agent/models.json\n"
            f"evolve ALL=(root) NOPASSWD: {setfacl} -m * {home}/*/.openclaw/agents/*/agent/codex-home/auth.json\n"
        ...
        "# exactly the per-bot level it names. deploy_shared_dir's own\n"
        "# `chmod 1777 {shared}/*/turns` re-assertion runs the identical argv,\n"
        "# so the registry-rendered grant below serves that call site too.\n"
        "# GENERATED from bot_shared_subdirs.BOT_SHARED_SUBDIRS — the same tuple\n"
        "# deploy.fix_shared_dir_permissions creates these dirs from. Adding a\n"
        "# subdir to that registry adds its grants here; there is no second list\n"
        "# to forget (2026-09-03: exec-failures/ and app-runs/ were in deploy's\n"
        "# list and not in this one, so their chown/ACL steps had no grant and\n"
        "# failed silently on every deploy for two days).\n"
        + _per_bot_subdir_grants(c, shared, acl=False, macos=macos)
        + "\n"
        + (
            "# ── 9b. Grant evo user write ACL on proposal / signal stores ───────────────\n"
            "# Phase E.2.b of internal/spec-evo-account-separation-2026-05-25.md split evo\n"
            ...
            + _per_bot_subdir_grants(c, shared, acl=True, macos=True)
            + f"evolve ALL=(root) NOPASSWD: {chmod} -R +a * {repo}/packages\n"
            "\n"
            "# ── 9c. Delivery-monitor heal: probe + one-shot restart of app jobs ─────────\n"
            "# Spec: internal/spec-proactive-delivery-monitor-2026-06-10.md §8 (U2.2 heal).\n"
            ...
            + _per_bot_subdir_grants(c, shared, acl=True, macos=False)
        ...
        "# the /tmp-staged source can only overwrite Evolve-owned identity docs —\n"
        "# the same blast-radius limit as the long-standing AGENTS.md grant.\n"
        "#\n"
        "# Rendered from CONTENT_SCAN_WORKSPACE_DOCS — the SAME list that drives the\n"
        "# §12 cat grants — so read and write cover one set and a newly required doc\n"
        "# gets both for free. It used to enumerate four names by hand while the\n"
        "# seeder wrote eight: TOOLS.md and HEARTBEAT.md had no cp grant, so on the\n"
        "# evolve-user path their writes were denied silently. Measured on the\n"
        "# reference pod 2026-09-23: TOOLS.md absent on 7 of 8 bots, HEARTBEAT.md\n"
        "# absent on 6 and four months stale on a 7th — the only bot with both\n"
        "# current was last seeded by a root-context run, where every cp lands.\n"
        + "".join(
            f"evolve ALL=(root) NOPASSWD: {cp} /tmp/evolve-*.md "
        ...
        "# …and the READ side of the same files. The writer was granted, the reader\n"
        "# was not, so the procedure-content check fell through to a denial 75 times\n"
        "# in 3 days. Same enumerated-directory blast radius as the cp above.\n"
        f"evolve ALL=(root) NOPASSWD: {cat} {home}/*/.openclaw/workspace/procedures/*.md\n"
        ...
        "# The stale-listener cleanup on the deploy path (deploy.py, before a gateway\n"
        "# (re)bind) asks the same question with a different argv — `-ti :<port>`, PIDs\n"
        "# only — and had no grant, so it silently found nothing and the stale listener\n"
        "# survived (21 denials in 3 days on the reference pod). Read-only, like its\n"
        "# sibling above.\n"
        f"evolve ALL=(root) NOPASSWD: {lsof} -ti \\:* -sTCP\\:LISTEN\n"
        "\n"
        ...
        "# …and the single-PID form, which the stale-listener cleanup above feeds from\n"
        "# lsof. Not observed denied (the ungranted lsof failed first, so the loop never\n"
        "# ran) — granting one without the other leaves that path dead either way. No\n"
        "# broader than the process-group grant on the line above: -<pgid> already\n"
        "# signals every member of a group.\n"
        f"evolve ALL=(root) NOPASSWD: {kill} -9 *\n"
        "\n"
        ...
        "# dir doesn't exist yet. See internal/diagnosis-cron-caps-applier-sudo-wall-2026-05-20.md.\n"
        ...
        "# exec-approvals.json. See internal/spec-app-derived-permissions-2026-05-24.md.\n"
...
def _record_installed_sudoers_marker(content: str) -> None:
    ...
    try:
        ...
        state_dir = _resolved_shared_dir() / "state"
    ...
    if dst.exists():
        ...
        oc_json = {
            ...
            "tools": {
                ...
                # 2026-05-25; see internal/spec-app-derived-permissions-2026-05-24.md).
...
_LINUX_DESIGN_DOC = "internal/design-linux-port-2026-06-10.md"
...
def _preflight_repo_root_traversable() -> None:
    ...
    staging remediation. See internal/design-linux-port-2026-06-10.md §8.2.
    """
    ...
    remediation = (
        ...
        f"internal/runbook-vps-pod-provision.md and design-linux-port-2026-06-10.md §8.2."
    )
...
def _preflight_no_stale_users_tree() -> None:
    ...
    the real home root there). See internal/runbook-vps-pod-provision.md.
    """
    ...
    _info("  Full teardown set: internal/runbook-vps-pod-provision.md.")
    raise SystemExit(1)
...
# ask. They never hard-block (internal/research-platform-expansion-2026-06-10.md
# §4a).
...
def _run_dedication_ack_step(
    ...
    if get_profile().name == "macos":
        ...
        panel_body = (
            ...
            "first.\n\n"
            "[dim]Full detail: docs/threat-model.md §2 · "
            "internal/design-linux-port-2026-06-10.md §1[/]"
        )
        confirm_q = (
...
def _run_https_phase(net_path: Path, non_interactive: bool = False) -> None:
    ...
    Sub-spec: ``internal/spec-pwa-phase0-https-2026-05-18.md`` §3.4 + §5.4.
...
def _run_repo_access_step(
    ...
    *,
    ...
    if not remote.is_git_repo or not remote.web_url:
        ...
        _info("    see internal/runbook-linux-vm-pass-2026-06-11.md → "
...
def run_fresh_wizard(
    ...
    # only — never on repair/upgrade. See internal/spec-pod-bringup-settle-2026-06-23.md.
    if not is_repair:
        try:
    ...
    if primary_mode == "dedicated":
        ...
        if primary_account == "evolve":
            ...
            # (internal/runbook-evo-account-separation-cutover-2026-05-26.md);
    ...
    # (ACL/perms) normally. Idempotent. See internal/spec-pod-bringup-settle-2026-06-23.md.
    try:
