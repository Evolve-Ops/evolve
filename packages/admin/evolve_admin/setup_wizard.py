...
def load_bots_manifest(path: Path) -> list[BotSpec]:
    ...
    Optional: `user`, `role`, `multi_user`. `name` is accepted as a synonym
    for `bot_id` since BotSpec stores it under `name` internally.
...
def _select_api_keys_for_evolve(non_interactive: bool) -> dict:
    ...
    if not existing:
        ...
        console.print()
        console.print("  [bold]Enter an Anthropic API key for the Evolve bot:[/]")
        console.print("  [dim](Leave blank to configure manually later)[/]")
        ...
        api_key = _ask_secret(
            "Anthropic API key (input hidden — paste won't show)", non_interactive)
        if api_key:
            return _new_bot_auth_profiles("anthropic", "api_key", api_key)
        return {"version": 1, "profiles": {}, "lastGood": {}}
...
# Anthropic is always present; others are added only if that provider's key exists.
...
def _evolve_openclaw_config(
    ...
    # Build model catalog from whichever providers have keys
    catalog_models: dict[str, dict] = {}
    # Anthropic always present (it's the primary provider)
    for m in _PROVIDER_CATALOG_MODELS.get("anthropic", []):
        catalog_models[m] = {}
    for provider in active_providers - {"anthropic"}:
...
def _provision_evo_oc(
    ...
    if profile.name == "macos":
        ...
        # (a config change needs a restarted gateway to take effect).
        subprocess.run(["sudo", "/bin/mkdir", "-p", str(log_dir)], capture_output=True)
...
def _render_evolve_sudoers() -> str | None:
    ...
    content = (
        ...
        f"evolve ALL=(root) NOPASSWD: {chmod} 644 {home}/*/.openclaw/workspace/.git/config\n"
        "\n"
...
def _check_prerequisites(runner: "Optional[Callable[..., subprocess.CompletedProcess]]" = None) -> list[Prereq]:
    ...
    results.append(Prereq(
        ...
        "" if py_ok else "Python 3.9+ required",
        hard=True,
    ))
...
def _build_security_section(shared_dir: Path) -> dict:
    """network.json's top-level ``security`` block for a fresh pod.

    ``rulesFile`` is derived from the operator-chosen shared dir, never a
    macOS literal: under ``EVOLVE_PLATFORM=linux`` ``shared_dir`` is
    ``/var/lib/evolve``, and even on macOS the operator may have picked a
    non-default dir at Step 12. This matches the security reviewer's own
    default (``review.py``: ``shared_dir / "security_rules.json"``) so the
    written path and the consumer's fallback agree on every platform — a
    hardcoded ``/Users/Shared/...`` path here would point a Linux pod's
...
def _discover_deploy_repo(network: dict) -> _RepoRemote:
    ...
    return _RepoRemote(is_git_repo=is_git, web_url=url or "")
...
def run_fresh_wizard(
    ...
    if not non_interactive:
        ...
        while not bots:
            ...
            if bot:
                bots.append(BotSpec(name=bot["name"], port=bot["port"], role="member", multi_user=bot.get("multi_user", False)))
    ...
    network: dict = {
        ...
        "security": _build_security_section(shared_dir),
        "heal": {
    ...
    if deploy_errors:
        ...
        console.print(Panel.fit(
            ...
            + _primary_line
            + "\n\n"
            "  [green]✅[/] Evolve admin UI: [bold]evolve-admin serve --open[/]\n\n"
    ...
    console.print(Panel.fit(
        ...
        "  [green]Recommended[/] — installs a persistent tunnel that auto-reconnects:\n"
        f"    [bold]pip install --user --upgrade evolve-admin[/]\n"
        f"    [bold]evolve-admin connect --host {_hostname}[/]\n\n"
        "  [dim]Equivalent without installing evolve-admin on the laptop:[/]\n"
        f"    [bold]ssh -N -L 5050:localhost:5050 {_hostname} &[/]\n"
        f"    [dim]then open[/] [bold]http://localhost:5050[/]\n\n"
        "  [dim]Optional — handy shell alias to type in your ~/.zshrc:[/]\n"
        f"    [bold]alias pod='evolve-admin connect --host {_hostname}'[/]\n\n"
        "  [dim]Manage the tunnel later:[/] "
    ...
    console.print("    [bold]evolve-admin serve --open[/]             — start admin UI locally on this machine")
    console.print()
