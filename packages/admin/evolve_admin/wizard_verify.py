...
_OWNERSHIP_EXEMPT_RELATIVE_PREFIXES: tuple[str, ...] = (
    ...
    # Admin (evolve) is a first-class writer in these subtrees, so POSIX
    # ownership inside them is heterogeneous by design and says nothing
    # about health.
    #
    # Precise contract for workspace/manifests (the one most often
    # misread): the DIRECTORY is bot-owned, evolve holds an inherited
    # write ACE on it, and both sides write through same-dir temp+rename.
    # Whoever renamed last owns the file. The bot therefore does NOT have
    # write access to an evolve-written <app>.json — it has read (0644 +
    # the mode bits) and it can still REPLACE the file, because rename
    # needs write on the directory it owns, not on the file. Every
    # bot-side writer relies on exactly that: the plugin's PUT
    # /evolve/manifests/:id, application_scanner.py, and
    # app_audit_runner.py's _stamp_manifest_field. See the ownership note
    # in web/server.py::_write_manifest_as_bot for the full inventory and
    # for what would invalidate the carve-out.
    "workspace/evolve",
...
# Projection of the channel registry's ``verify_handler`` column — the
# channels for which a handshake check exists at all. A registry row with
# ``verify_handler=None`` (sms / email / webhook) is unverifiable and is
# correctly absent here.
#
# NOTE for readers: the executor below dispatches through an explicit
# if/elif chain over ``configured``, not through this map, so this table is
# documentation + the capability declaration rather than the dispatch itself.
# Collapsing the chain onto this map is a behaviour change and is tracked as
# M1-B1 follow-up, not done here.
_CHANNEL_HANDLERS: dict[str, str] = _channel_registry.verify_handlers()
