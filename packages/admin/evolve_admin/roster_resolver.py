...
# Channels the roster spans. Mirrors ``routes_bot_users.KNOWN_PROVIDERS`` — kept in
# sync via the guard test ``test_roster_resolver.test_channels_match_known_providers``
# rather than importing routes here (avoids pulling Flask into the join's import graph).
ROSTER_CHANNELS: tuple[str, ...] = ("telegram", "slack", "discord", "whatsapp")
...
def resolve_labels(
    ...
    if stable_id == ro._primary_owner_id_for(network, bot_id, platform):
...
def resolve_display_name(
    ...
    primary_id = ro._primary_owner_id_for(network, bot_id, channel)
    if ext_id == primary_id:
        primary_block = (
