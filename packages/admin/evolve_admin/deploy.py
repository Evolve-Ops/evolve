...
from .plugin_signature import canonical_dist_digest, stamp_manifest_in_place, verify_plugin_signature
from .runtime import JobSpec, get_perms, get_scheduler, render_launchd_plist
...
from . import bot_doc_seeding as _bot_docs, evo_socket_acl as _evo_sock_acl, deploy_resilience as _dres
...
ANALYZER_SCRIPTS = ["measure.py", "analyze.py",
                    "validate.py", "apply.py",
                    "review.py", "heal.py", "cost.py",
                    ...
                    "app_session_correlator.py", "usage_logger.py"]
ANALYZER_DATA_FILES = ["security_rules.json"]  # static files copied alongside scripts
...
_PLUGIN_CONFIG_DEFAULTS: dict[str, Any] = {
    ...
    # Layer-2 per-role tool-call enforcement gate ARMING flag.
    # False (default) = OBSERVE-ONLY: the below-LLM before_tool_call gate
    # computes and records every would-block decision but always allows the
    # call. Flip to True (or set nested `layer2.enforce: true` by hand) to arm
    # real blocking. Merging the gate code is non-arming by design.
    # Spec: docs/spec-user-roster-and-roles-2026-06-07.md §8 (Layer 2).
    "layer2Enforce": False,
}
...
def per_bot_evolve_plist_labels(bot_id: str) -> list[str]:
    ...
    """
    return [
        f"ai.openclaw.evolve.apply.{bot_id}",
        # ai.openclaw.evolve.test.{bot_id} removed 2026-06-08 — app-test
        # surface killed per docs/decision-app-tests-2026-06-08.md.
        # Listed below in _legacy_per_bot_labels_to_bootout so existing
        # plists get cleaned up on the next deploy.
...
def expected_plist_labels(network: dict, *, realized_only: bool = False) -> set[str]:
    ...
    labels.update({
        ...
        "ai.evolve.evolve.breakers-runner",  # every 10 min, observe-only: evaluate the activity-shape detector and log "what would have tripped" to {shared}/breakers/runner-log/ (5.2 calibration soak). Acts on trips ONLY when network.json::breakers.auto_trip_enabled is true (default false). Installed by _install_launchd_breakers_runner. Spec: docs/spec-circuit-breakers-2026-05-21.md §5.1 / §8 Phase 5.
        ...
        "ai.evolve.evolve.usage-logger",     # daily usage aggregator feeding the Usage tab + Anthropic cost reconciliation. Installed by _install_launchd_usage_logger("evolve"). Omitted before 2026-05-26 → flagged orphan.
...
def build_plugin() -> None:
    ...
    try:
        ...
        # ownership check started rejecting that path. node_modules/ here costs ~100MB
        # of disk per deploy; cheap insurance against a class of regressions where a
        # new dependency lands without its install path also landing.
        node_modules = PLUGIN_SRC_DIR / "node_modules"
        if dist_dir.exists():
            ...
            if node_modules.exists():
                ...
                subprocess.run(["sudo", "/bin/cp", "-R", str(node_modules),
                                str(PLUGIN_INSTALL_DIR / "node_modules")],
                               capture_output=True)
            for fname in ("package.json", "package-lock.json", "openclaw.plugin.json"):
            ...
            # Stamp the deployed manifest with the canonical digest of dist/.
            # The committed manifest stays clean (no digest) — only the deployed
            # copy at PLUGIN_INSTALL_DIR is stamped. This is the anchor for
            # verify_plugin_signature() at install time; without it, the
            # --dangerously-force-unsafe-install flag in install_oc_plugin
            # would trust the path /Users/Shared/evolve-plugin/ rather than
            # its content. See docs/spec-plugin-install-trust-2026-06-06.md.
            installed_manifest = PLUGIN_INSTALL_DIR / "openclaw.plugin.json"
            if installed_manifest.exists() and (PLUGIN_INSTALL_DIR / "dist").is_dir():
                try:
                    digest = canonical_dist_digest(PLUGIN_INSTALL_DIR / "dist")
                except Exception as e:
                    raise RuntimeError(
                        f"could not compute dist digest for plugin signature: {e}"
                    )
                # Capture the commit the build came from, best-effort. Not
                # load-bearing for verification — the digest alone is the
                # security boundary — but useful for operator forensics
                # when a mismatch shows up.
                _commit = ""
                try:
                    _r = subprocess.run(
                        ["git", "rev-parse", "HEAD"],
...
def _ensure_evolve_write_dir(rel: str, bot_user: str, perms, *,
    ...
    if not _secret_perms.exists_or_unreachable(Path(d)):
...
def ensure_plugin_config(bot_id: str, network: dict) -> None:
    ...
    from .openclaw_materializer import materialize_evolve_plugin_config
    ...
    materialize_result = materialize_evolve_plugin_config(
        ...
        defaults_registry=_PLUGIN_CONFIG_DEFAULTS,
    ...
    # 3. Required by openclaw ≥ 2026.4.x: lets the evolve plugin pass a model
    # override in subagent.run() calls (LLMTierClassifier, SessionSummarizer).
    # ``subagent`` is a SIBLING of ``config`` (under plugin_entry, not
    # inside the materialized config block) so the materializer leaves it
    # alone.
    ...
    # 7. Brave plugin entry — pod-wide required plugin per the plugin_monitor
    # baseline (packages/analyzer/plugins/bootstrap.py: required_plugins =
    # ["evolve", "brave"]). The strip-stale-entries pass above will remove
    # this entry on subsequent deploys if brave isn't loadably installed, so
    # we only add the entry when an install record actually exists for it.
    # Don't write the entry when not installed — that produces an entry-then-
    # strip oscillation visible in the audit log.
    #
    # Gap-fill: if brave is missing, run `openclaw plugins install
    # @openclaw/brave-plugin` here before the entry check. Without this, an
    # existing bot whose brave install was never run — or was wiped by an
    # upstream API-version bump that required a fresh per-bot install —
    # stays stuck because the setup wizard's brave install only runs at
    # fresh setup. This mirrors the entries-dict gap-fill pattern: every
    # deploy must self-heal toward the pod-wide invariant, not just bless
    # an operator-prepared state.
    try:
    ...
    try:
        ...
        from .safe_upgrade import _read_installs_json as _read_inst
...
def install_oc_plugin(bot_id: str, port: int | None = None, network: dict | None = None) -> None:
    ...
    # `--dangerously-force-unsafe-install` (OC 2026.6.1+) bypasses the built-in
    # dangerous-code scanner, which flags TurnObserver's spawn("python3", ...)
    # — the subagent-transport hop that lets the plugin invoke evolve's Python
    # helpers (signal store, profile storage, etc.). The scanner is designed to
    # protect operators from malicious third-party plugins; our plugin ships
    # from this repo and the child_process call is intentional and audited.
    #
    # Signed-bypass gate: refuse to pass the flag unless the deployed
    # /Users/Shared/evolve-plugin/openclaw.plugin.json carries a stamped
    # x-evolve-trust.distDigest that matches a fresh canonical hash of
    # /Users/Shared/evolve-plugin/dist/. Anchors the bypass to content, not
    # path — closes the gap PR #2293's unconditional bypass left open. See
    # docs/spec-plugin-install-trust-2026-06-06.md.
    ...
    if not ok:
        raise RuntimeError(
            ...
            f"clear it, investigate what wrote to "
            f"{PLUGIN_INSTALL_DIR}/dist/ since the last build."
        )
...
def safe_write_bot_config(
    ...
    try:
        ...
        # PermissionError on modern Python (the live VPS 0-of-N upgrade blocker, error
        # "[Errno 13] Permission denied: .../openclaw.json"). The backup + write
        # below run as root via sudo, so classify an unreachable path as present
        # and proceed; the reassert at the end re-widens evolve's traverse.
        ...
        get_perms().reassert_mask(config_path.parent)  # the `openclaw config validate` above ran as the bot user → 0700-clamped .openclaw's ACL mask; re-widen evolve's traverse (Linux; macOS no-op). Harden→reassert coupling for the gateway's own re-clamp.
        return True, ""
...
# pick up the same grant. Shape matches the evolve-side write ACL on bot
# workspace/evolve/ dirs (set_evolve_read_acl), inverted in user direction.
EVO_WRITE_ACL_PERMS = (
...
# The apply lock file path template. Pre-created so apply.py never has to
# create-with-correct-owner under a TOCTOU race against a sibling daemon.
# Platform-keyed: macOS renders /Users/Shared/evolve byte-identically; Linux
# renders /var/lib/evolve so the lock path carries no /Users leak (W10-E).
APPLY_LOCK_TEMPLATE = str(_CANONICAL_SHARED_DIR / "apply_processed-{bot}.lock")
APPLY_LOCK_MODE = 0o644
...
def _check_apply_lock(bot_id: str, bot_user: str) -> _PermCheck:
    """Check {sharedDir}/apply_processed-<bot_id>.lock exists & is owned by bot_user.

    The lock file is named by bot_id (matching apply.py's --bot-id arg) but
    must be owned by bot_user (the macOS account that runs the daemon). These
    differ when a bot has been renamed / migrated to a new OS account.
    """
    lock = Path(APPLY_LOCK_TEMPLATE.format(bot=bot_id))
    if not lock.exists():
        return _PermCheck(
            category="lock-file",
            target=str(lock),
            ok=False,
            detail="missing",
            fix_description=f"touch {lock} && chown {bot_user} {lock} && chmod 644 {lock}",
            apply=lambda: _ensure_apply_lock(bot_id, bot_user),
        )
    try:
        st = lock.stat()
        ...
        return _PermCheck(
            category="lock-file", target=str(lock), ok=False,
            detail=f"stat failed: {e}",
            fix_description=f"recreate {lock} owned by {bot_user}",
            apply=lambda: _ensure_apply_lock(bot_id, bot_user),
        )
    if cur_owner != bot_user:
        return _PermCheck(
            category="lock-file", target=str(lock), ok=False,
            detail=f"owner is {cur_owner!r} (stale); expected {bot_user!r}",
            fix_description=f"chown {bot_user} {lock}",
            apply=lambda: _ensure_apply_lock(bot_id, bot_user),
        )
    return _PermCheck(
        category="lock-file", target=str(lock), ok=True,
        detail=f"owner={cur_owner}, mode={oct(st.st_mode & 0o777)}",
    )


def _ensure_apply_lock(bot_id: str, bot_user: str) -> bool:
    """Create/repair the apply lock file with correct owner and mode. Idempotent.

    Lock is named by bot_id (apply.py uses --bot-id for the path) but owned
    by bot_user (the OS account running the daemon). Chown always runs so a
    stale file left by a previous OS account (e.g. after a bot rename) is
    fixed in place without needing to delete and recreate it.
    """
    lock = Path(APPLY_LOCK_TEMPLATE.format(bot=bot_id))
    try:
        # touch — only needed if missing; existing stale-owner file is fine to chown in place
        if not lock.exists():
            try:
                lock.touch(mode=APPLY_LOCK_MODE, exist_ok=True)
            except PermissionError:
                # Shared/evolve may not be writable as the current user.
                proc = subprocess.run(
                ...
                if proc.returncode != 0:
                    return False
        # chown — always run so a stale owner from a renamed OS account is corrected
        proc = subprocess.run(
            ["sudo", _PROFILE.chown, bot_user, str(lock)],
            capture_output=True, text=True, timeout=5,
        )
        if proc.returncode != 0:
            return False
        # chmod
        proc = subprocess.run(
            ["sudo", "/bin/chmod", oct(APPLY_LOCK_MODE)[2:], str(lock)],
            capture_output=True, text=True, timeout=5,
        )
...
def _check_apply_plist(bot_id: str, bot_user: str) -> _PermCheck:
    """Check the apply daemon plist exists with the canonical UserName.

    The plist is named by bot_id (ai.openclaw.evolve.apply.<bot_id>.plist) but
    must have UserName=bot_user. These differ when bot_id != bot_user (e.g.
    after a bot OS-account rename).

    We do NOT bounce the daemon when it's already running and the plist looks
    good — this would create a deploy footgun. We only flag the plist as
    "missing" or "wrong UserName" so a regen is unambiguous.
    """
    if _PROFILE.name == "linux":
        # systemd: the apply daemon is a .service unit, not a /Library plist on
        # disk. Probe installed-ness through the seam so this check doesn't
        # perpetually false-fire "missing" on a Linux pod (and trigger an
        # endless regen). UserName-drift detection is a macOS account-rename
        # concern; on a Linux pod bot_id == bot_user, so installed-ness suffices.
        label = f"ai.openclaw.evolve.apply.{bot_id}"
        installed = bool(get_scheduler().status(label).get("installed"))
        return _PermCheck(
            category="plist", target=f"{label}.service", ok=installed,
            detail="installed" if installed else "missing",
            fix_description=(
                "" if installed
                else f"regenerate {label}.service (apply daemon for {bot_id} as {bot_user})"
            ),
            apply=(None if installed else (lambda: _regen_apply_plist(bot_id, bot_user))),
        )
    plist = LAUNCHD_DIR / f"ai.openclaw.evolve.apply.{bot_id}.plist"
    if not plist.exists():
        return _PermCheck(
            category="plist", target=str(plist), ok=False,
            detail="missing",
            fix_description=f"regenerate {plist.name} from canonical template (apply daemon for {bot_id} as {bot_user})",
            apply=lambda: _regen_apply_plist(bot_id, bot_user),
        )
    # plists in /Library/LaunchDaemons/ are owned by root:wheel but
    # mode 644 (world-readable), so plistlib.load handles them without
    # privilege escalation. The previous sudo-PlistBuddy round-trip
    # silently fell back to UserName='' when the caller lacked the
    # sudo grant — pod_perms_drift_monitor running as the `evolve`
    # daemon hit that path and reported every plist as drifted (2026-06-06).
    cur_user = _read_plist_username(plist)
    if cur_user is None:
        return _PermCheck(
            category="plist", target=str(plist), ok=False,
            detail="plist read failed",
        )
    if cur_user != bot_user:
        return _PermCheck(
            category="plist", target=str(plist), ok=False,
            detail=f"UserName={cur_user!r} != expected {bot_user!r}",
            fix_description=f"regenerate {plist.name} (UserName must be {bot_user})",
            apply=lambda: _regen_apply_plist(bot_id, bot_user),
        )
    return _PermCheck(
        category="plist", target=str(plist), ok=True,
        detail=f"UserName={cur_user}",
    )


def _read_plist_username(plist: Path) -> str | None:
    """Read the ``UserName`` field from a launchd plist.

    Returns the string value, ``""`` if the key is absent (we treat
    "absent" identically to "empty string" — both fail the equality
    check downstream), or ``None`` on a read error.

    Uses ``plistlib.load`` directly. The system plist directory
    (/Library/LaunchDaemons/) holds files owned by root:wheel with mode
    644, so the caller doesn't need root. The prior implementation
    shelled out to ``sudo /usr/libexec/PlistBuddy`` which silently
    yielded an empty UserName string when ``sudo`` denied the grant —
    the exact condition that hit pod_perms_drift_monitor running as the
    ``evolve`` daemon user.
    """
    import plistlib
    try:
...
def _regen_apply_plist(bot_id: str, bot_user: str) -> bool:
    """Regenerate just the apply.<bot_id>.plist from the canonical template.

    Reuses _install_launchd_apply so the plist content matches what every
    other code path produces. Note: _install_launchd does its own bootout +
    bootstrap, so the daemon WILL bounce when this is called — but only when
    the plist was actually missing or wrong, never on a no-op pass.
    """
    workspace = get_bot_workspace(bot_user)
    if workspace is None:
        return False
    evolve_dir = workspace / "evolve"
    sub_result = DeployResult(bot_id=bot_id, success=True)
    _install_launchd_apply(bot_id, evolve_dir, sub_result, user=bot_user)
...
def ensure_pod_perms(
    ...
    2026-05-25 evo-account-separation post-cutover dismiss bug):
      ...
      2. apply_processed-<bot>.lock exists, owned by <bot>, mode 0644
      3. /opt/homebrew/Cellar/python@3.14 is g+rX o+rX
      4. {sharedDir}/proposals/ + standard subdirs are mode 1777
      5. {sharedDir}/{proposals,signals}/ grant the evo user a write+
         inherit ACL (only if the `evo` macOS user exists, i.e. post
         Phase E.2.a of spec-evo-account-separation-2026-05-25.md)
      6. each bot's own CLI device in ~/.openclaw/devices/paired.json
      ...
      Plus: apply daemon plist exists with correct UserName.
    ...
    result.checks.extend(_check_alerts_dir(shared_dir) + _secret_perms.check_shared_secret_modes(shared_dir) + _secret_perms.check_shared_directory_modes(shared_dir) + [_evo_gw.check_evolve_gateway_client(network)])  # + evolve-owned Google secret-key 0600 self-heal + directory/ contact-PII 0600 self-heal + admin→evo gateway client-token drift self-heal (EVO-SEP-GW-CRED) (folded: deploy.py at no-growth cap; rationale in the helper docstrings)
    ...
    for bid, bot_user in bot_pairs:
        ...
        result.checks.append(_check_apply_lock(bid, bot_user))
        result.checks.append(_check_apply_plist(bid, bot_user))
...
def _install_job_ensuring_restart(spec: JobSpec) -> tuple[bool, str]:
    """``Scheduler.install()`` + the legacy always-bounce guarantee.
    ...
    effect). ``install()`` skips the bounce when the on-disk plist is
...
def deploy_bot(
    ...
    Steps:
      ...
      6. Install apply.py launchd job as this bot's user (per-bot by design — see note)
      7. Update network config to include this bot

    Note on apply.py: intentionally runs as the bot's own user, not as evolve.
    apply.py patches the bot's own openclaw.json — no cross-user writes, no sudo needed.
    This is a deliberate safety boundary: evolve cannot silently rewrite all bot configs.
    ...
    if not dry_run:
        ...
        # ── 4e. Ensure brave plugin is installed (recommended default) ────
        # brave is no longer a pod-wide required plugin (v2 plugin baseline,
        # docs/spec-plugin-posture-rework-2026-06-06.md §2.6 — brave is one
        # search backend among several). It remains the default for new bots
        # so out-of-the-box web search works without operator setup. Gap-fill
        # here on every deploy so a bot that landed without brave still picks
        # it up. Skip if already installed; install with force=False so we
        # don't reinstall on every deploy.
        #
        # ``_full_deploy`` already ran ``ensure_plugin_config`` before
        # calling us, but it ran *before* this brave install — so the
        # ``plugins.entries.brave`` gap-fill saw "not installed yet" and
        # skipped. Re-run it here to pick up the fresh install record.
        try:
            from .safe_upgrade import _installed_plugin_ids
            _net_for_brave = load_network(network_path)
            if "brave" not in _installed_plugin_ids(bot_id, _net_for_brave):
                from .oc_neutralize import install_externalized_plugin
                ok_brave, err_brave = install_externalized_plugin(
        ...
        # ── 6. Install per-bot apply watcher ──────────────────────────────
        # Runs as the bot's macOS user (bot_user, may differ from bot_id —
        # e.g. when the bot's macOS account differs from bot_id). Without `user=bot_user`,
        # the plist gets `UserName=<bot_id>` and creates an orphan
        # /Users/<bot_id>/.openclaw/ directory; oc_cli probes from heal.py
        # then fail with token mismatch and heal kills the gateway every
        # 5 min in a self-cancelling restart cascade.
        _install_launchd_apply(bot_id, evolve_dir, result, user=bot_user)

        # ── 6b. Install per-bot test job ──────────────────────────────────
        ...
        # killed. Existing plists get booted out by _bootout_legacy_test_plists.
        _bootout_legacy_test_plists(bot_id, result)
...
def deploy_shared_dir(shared_dir: Path, dry_run: bool = False) -> DeployResult:
    ...
    if not dry_run:
        ...
        _secret_perms.tighten_shared_pii_trees(shared_dir)  # undo a+rX's g/o-read on the PII-at-rest trees: secrets/ (Google SA/DwD keys + OAuth tokens) + directory/ (per-bot contact emails)
        ...
        for multi_writer_dir in ["metrics", "annotations"]:
            ...
            try:
                ...
                _run_sudo(["chmod", "1777", str(p)], result)
        # Re-assert 1777 on per-bot turns dirs — sticky world-writable so each
        # bot user can write its own turn files without deleting others'.
        for turns in shared_dir.glob("*/turns"):
            if turns.is_dir():
                try:
                    os.chmod(turns, 0o1777)
                except PermissionError:
                    _run_sudo(["chmod", "1777", str(turns)], result)
...
def install_bot_docs(
    ...
    *,
    ...
    for src, fname, substitute in plan:
        ...
        # Primary's AGENTS.md gets the auto-generated evo glossary appended
        # (evolve_bot/glossary.yaml + network.json::evo_glossary_overrides;
        # spec §3.7 brittleness mitigation — keeps it in lock-step with source).
        if role == "primary" and fname == "AGENTS.md":
            rendered = _append_evo_glossary(rendered, result)
...
def _append_evo_glossary(agents_md_text: str, result: "DeployResult") -> str:
    """Concatenate the rendered evo glossary onto AGENTS.md text.

    Spec §3.7 brittleness mitigation. The glossary lives at
    ``packages/analyzer/evolve_bot/glossary.yaml`` (single source of
    truth); the deploy step regenerates the markdown each time with
    the pod's ``network.json::evo_glossary_overrides`` applied so
    each install gets a personalized glossary without forking the
    yaml.

    Defensive: never raises. A missing yaml or rendering failure
    falls through to "no glossary appended" + a warning in the deploy
    log — evo still loads AGENTS.md with the rules section, just
    without the glossary. The AGENTS.md pointer block explains this
    is expected behavior when the glossary is unavailable.
    """
    try:
    ...
    result.log(
        f"evo glossary appended to AGENTS.md "
    ...
    # Separator + glossary block. AGENTS.md already ends with a
    # trailing newline; we add a horizontal rule before the glossary
    # so it's visually distinct from the pointer block in AGENTS.md.
    return agents_md_text.rstrip() + "\n\n---\n\n" + md + "\n"


def _resolve_network_path_for_overrides() -> Path | None:
    """Best-effort: locate network.json so the glossary renderer can
    read pod-specific overrides. Checks the standard pod path; falls
    back to None when unreadable (deploys outside the standard layout
    are rare; they get default glossary content)."""
    candidate = Path("/Users/Shared/evolve/network.json")
...
def _merge_cron_entries(jobs_path: Path, new_entries: list[dict], result: DeployResult, owner: str = "evolve") -> None:
    ...
    for entry in new_entries:
        wire = _normalize_cron_delivery(entry)
        name = wire.get("name")
        if name not in existing_by_name:
            existing_jobs.append(wire)
            existing_by_name[name] = wire
            added += 1
        elif _is_no_delivery(wire) and not _is_no_delivery(existing_by_name[name]):
            # Heal a previously-installed file-only cron that still carries the
            # announce default — it would error "Delivering to Telegram requires
            # target <chatId>" on a chatId-less primary account (#3151 Facet B).
            # Narrowly reconcile ONLY the delivery field; never touch the
            # operator's schedule/task.
            existing_by_name[name]["delivery"] = {"mode": "none"}
            reconciled += 1
    ...
    try:
        ...
        if result.success:
            ...
            if reconciled:
                summary += f" ({reconciled} healed to no-delivery)"
            result.log(summary)
    except Exception as e:
...
def install_evolve_app(app_id: str, shared_dir: Path, dry_run: bool = False) -> "DeployResult":
    ...
    if crons:
        ...
        # No dir-chown: _merge_cron_entries writes jobs.json via the §19 file-level grants (cp + chown jobs.json), which carry the write into this root-owned dir — the dir need not be evolve-owned (mirrors permissions.writer.write_cron_jobs).
            ...
            try:
                ...
                if changed:
                    ...
                    try:
                        ...
                        if result.success:
                            # openclaw.json carries tokens → MUST be 0600 (CLAUDE.md
                            # secret-config contract). The dest always pre-exists
                            # here (we only patch what we just read), so `cp` (no -p)
                            # PRESERVES its owner — no chown needed. chmod_secret_config
                            # enforces 0600 (+ evolve read-ACL re-assert on Linux).
...
def install_evolve_infra_jobs(
    ...
    if not dry_run:
        ...
        _install_launchd_usage_logger("evolve", result)
        ...
        # Circuit-breakers detector runner — every 10 min, observe-only. Starts
        # the §5.2 calibration soak: evaluates the activity-shape detector and
        # logs "what would have tripped" to {shared}/breakers/runner-log/. Does
        # NOT arm enforcement — acts on trips only when
        # network.json::breakers.auto_trip_enabled is true (default false).
        # Spec: docs/spec-circuit-breakers-2026-05-21.md §5.1 / §8 Phase 5.
...
def _install_launchd(label: str, user: str, script_path: Path, schedule: dict,
    ...
    if via_seam:
        ...
        # _install_job_ensuring_restart keeps the always-bounce guarantee the
        # legacy cp-to-LAUNCHD_DIR path below had. That legacy path is retained
...
def _install_launchd_apply(
    bot_id: str, evolve_dir: Path, result: DeployResult,
    *, user: str | None = None,
) -> None:
    """Install the per-bot apply watcher as the bot's macOS user.

    `user` is the resolved Unix username, which may differ from `bot_id`
    (e.g. when one bot lives on a personal/shared account, bot_id != user). Defaults to `bot_id` for
    back-compat with callers that haven't been updated to pass it; that
    path is only correct when bot_id == os user, and breaks the deploy
    when they differ (creates /Users/<bot_id>/.openclaw/ orphan dirs and
    a launchd plist with the wrong UserName).

    Intentionally runs as the bot's own user, NOT as evolve. apply.py
    patches the bot's own openclaw.json — keeping this per-bot means no
    cross-user writes and no sudo needed for config changes. Moving it
    to evolve would require sudo for every config write and would let
    a single compromised process rewrite all bot configs.
    """
    plist_user = user or bot_id
    _install_launchd(
        label=f"ai.openclaw.evolve.apply.{bot_id}",
        user=plist_user,
        script_path=ANALYZER_DIR / "apply.py",
        schedule={"interval": 300},  # StartInterval, not calendar
        result=result,
        extra_args=["--bot-id", bot_id],
        run_at_load=True,
        # Spread per-bot apply firings across 4 of every 5 minutes — without
        # this, RunAtLoad fires N bots' applies within seconds and the cycle
        # stays locked thereafter (launchd does not jitter StartInterval).
        jitter_seconds=240,
    )


def _bootout_legacy_test_plists(bot_id: str, result: DeployResult) -> None:
    """Boot out the legacy ai.openclaw.evolve.test.<bot> LaunchDaemon.

    The per-bot weekly regression-test runner was killed 2026-06-08 along
    with the rest of the app-test surface (see
    docs/decision-app-tests-2026-06-08.md). The plist + bootstrap remained
    on existing pods until they redeployed; this helper takes care of
    that on the next deploy. Idempotent — safe to call on bots where the
    plist is already gone.
    """
    label = f"ai.openclaw.evolve.test.{bot_id}"
    plist_path = f"/Library/LaunchDaemons/{label}.plist"
    # rc ignored — the legacy daemon is usually not loaded anymore.
    _scheduler_launchctl("bootout", f"system/{label}")
    rm = subprocess.run(
        ["sudo", "/bin/rm", "-f", plist_path],
        capture_output=True, text=True,
    )
    if rm.returncode == 0:
        result.log(
            f"booted out legacy {label} (app-test surface removed 2026-06-08)"
        )
    else:
        result.log(
            f"[warn] could not remove legacy {plist_path}: {rm.stderr.strip()}"
        )
...
def _install_launchd_usage_logger(bot_id: str, result: DeployResult) -> None:
    """Install daily app usage aggregator (03:30, after analyze.py at 02:00).

    Reads daily metrics files written by measure.py and aggregates per-app
    usage stats into {sharedDir}/{botId}/recommendations/usage-stats.json.
    Runs for all bots listed in network.json in a single invocation.
    """
    _install_launchd(
        label=f"ai.evolve.{bot_id}.usage-logger",
        user=bot_id,
        script_path=ANALYZER_DIR / "usage_logger.py",
...
def _install_launchd_better_engine(bot_id: str, result: DeployResult) -> None:
    ...
    (WatchPaths), which is written by health.py, spend_alert.py, and review.py
    on critical events that require immediate recommendation refresh.
...
def _install_launchd_log_cap(bot_id: str, result: DeployResult) -> None:
    ...
    Caps the three flat-file logs that have no in-process rotation:
...
def _install_launchd_breakers_runner(bot_id: str, result: DeployResult) -> None:
    ...
    evaluates the activity-shape detector and appends a "what would have
    tripped" entry to ``{shared_dir}/breakers/runner-log/`` every cycle. It
    acts on trips ONLY when ``network.json::breakers.auto_trip_enabled`` is
    true (default ``false``) — so scheduling this is purely the §5.2
    observe-only calibration soak, NOT arming enforcement.
