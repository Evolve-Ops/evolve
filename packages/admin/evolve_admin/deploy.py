...
def _bootout_legacy_test_plists(bot_id: str, result: DeployResult) -> None:
    ...
    macOS-only legacy — the daemon never existed on Linux (the hardcoded
    /Library/LaunchDaemons rm burned a denial per bot per deploy on the
    VPS, 2026-07-29 census). Killed 2026-06-08 with the app-test surface
    (docs/decision-app-tests-2026-06-08.md). Idempotent.
    """
    if _PROFILE.name != "macos":
        return
