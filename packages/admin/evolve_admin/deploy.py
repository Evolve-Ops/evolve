...
_PLUGIN_CONFIG_DEFAULTS: dict[str, Any] = {
    ...
    # Merging is non-arming. Spec: internal/spec-user-roster-and-roles-2026-06-07.md §8.
    "layer2Enforce": False,
    # Context-observability Phase 0 prefix-hash ledger: dark by default; set True
    # per-bot to arm (sticky). Spec: internal/spec-context-observability-2026-07-30.md.
...
def fix_shared_dir_permissions(bot_id: str, shared_dir: Path) -> None:
    ...
    # ── Pre-create {sharedDir}/{bot_id}/app-runs/ — AL-1.2 claim files ──────
    # Written by the app-run shim running as the BOT user (inside an app's
    # cron process) and read+unlinked by the bot's own gateway plugin. The
    # bot's ACE on the parent grants add_file/delete_child but NOT
    # add_subdirectory, so on a pod where {sharedDir}/{bot_id}/ is
    # evolve-owned the shim's own mkdir would EACCES and every scheduled turn
    # would stay silently unattributed — the same failure spans/ and cascade/
    # above were pre-created to avoid. Mode 755, single writer, bot-owned; no
    # evolve ACL because no evolve-user job reads claims.
    _create_bot_subdir(shared_dir / bot_id / "app-runs", 0o755)
