...
from .app_discovery import APP_DISCOVERY_SWEEP_LABEL, first_scan_on_deploy, install_app_discovery_sweep  # ALPHA-1/B1: discovery runs on arrival + weekly, not only on a click
...
from .compaction_housekeeping import apply_for_bot as _apply_compaction_housekeeping
...
from .tier_usage_dir_perms import check_tier_usage_dir as _check_tier_usage_dir
from .brave_key import resolve_pod_brave_key
from . import bot_doc_seeding as _bot_docs, evo_socket_acl as _evo_sock_acl, deploy_resilience as _dres, tier_prefs_acl as _tier_prefs_acl, shared_bot_dir_perms as _bot_dir_perms, bot_shared_subdirs as _bot_subdirs, board_store_perms as _board_perms, firewall_allow as _fw, pm_probe_install as _pm_probe
...
ANALYZER_SCRIPTS = ["measure.py", "analyze.py",
                    ...
                    "app_session_correlator.py", "usage_logger.py", "usage_by_app.py", "usage_by_user.py"]
...
_PLUGIN_CONFIG_DEFAULTS: dict[str, Any] = {
    ...
    # Layer-2 before_tool_call enforcement ARMING flag; False = OBSERVE-ONLY (records would-blocks, allows all). True (or nested `layer2.enforce: true`) arms real blocking; merging is non-arming. Spec: internal/spec-user-roster-and-roles-2026-06-07.md §8.
    "layer2Enforce": False,
    # Context-observability Phase 0 prefix-hash ledger: dark by default; set True per-bot to arm (sticky). Spec: internal/spec-context-observability-2026-07-30.md.
    "prefixHashLedgerEnabled": False,
    # Exec-failure channel-hygiene ARMING flag; False = OBSERVE-ONLY (trailer matches ledgered, delivery unchanged). Merging is non-arming. Design: internal/design-exec-failure-hygiene-2026-08-31.md.
    "execFailureAbsorb": False, "inTurnCheckpointUsd": 5.0,  # D-CS12 in-turn cost checkpoint ($ recorded per run; floor 1.00)
}
...
def per_bot_evolve_plist_labels(bot_id: str) -> list[str]:
    ...
    return [
        ...
        # internal/design-proposal-signing-key-2026-08-18.md.
        # ai.openclaw.evolve.test.{bot_id} removed 2026-06-08 — app-test
        # surface killed per internal/decision-app-tests-2026-06-08.md.
...
def expected_plist_labels(network: dict, *, realized_only: bool = False) -> set[str]:
    ...
    # Opik observability companion — pod-wide and OPT-IN (`evolve-admin
    # install-infra-jobs --with-opik`), installed by ``install_opik_companion``
    # straight through the Scheduler seam rather than ``_install_launchd``.
    # Same presence-gated shape as the iMessage poller above, and for the same
    # two reasons: unconditionally listing it would make ``_check_launchd``
    # report a permanent "Plist not found" on the majority of pods that never
    # opted in, while omitting it entirely means the orphan-sweeper deletes the
    # daemon on the next `evolve-admin upgrade` for every pod that DID
    # (`ai.evolve.opik` matches both the macOS `ai.evolve.*.plist` glob and
    # Linux's `_is_evolve_owned_label`, and it is not feature-gated). Found
    # 2026-08-23 by the review pass on the autonomy-limits repair — the same
    # bug class, one seam over.
    if (LAUNCHD_DIR / f"{OPIK_LAUNCHD_LABEL}.plist").exists():
        labels.add(OPIK_LAUNCHD_LABEL)
    ...
    labels.update({
        ...
        APP_DISCOVERY_SWEEP_LABEL,                     # App discovery — weekly Sunday 05:15 pod-wide sync_bot sweep (ALPHA-1/B1). Installed by app_discovery.install_app_discovery_sweep.
        ...
        "ai.evolve.evolve.preflight-prior",  # daily 03:20: learn each bot's routing prior from 14d of its own turns; writes bots.<id>.preflight.bot_prior only when network.json::cascade.prior.enabled is true (fails closed). D-OH2, internal/decision-evolve-overhead-2026-09-07.md
        ...
        "ai.evolve.evolve.breakers-runner",  # every 10 min: evaluate the activity-shape detector, log decisions to {shared}/breakers/runner-log/, and act on trips unless network.json::breakers.auto_trip_enabled is false (default true — ARMED since the §5.2 arming PR; `evolve-admin breaker disarm` = observe-only). Installed by _install_launchd_breakers_runner. Spec: internal/spec-circuit-breakers-2026-05-21.md §5.1 / §8 Phase 5.
        "ai.evolve.evolve.autonomy-limits",  # every 5 min: rung-3 daily-cap evaluation + demotion reflex over the bot-side outward-action ledger. Installed by _install_launchd_autonomy_limits("evolve") from install_evolve_infra_jobs. Spec: internal/spec-autonomy-ladder-2026-06-10.md §1.3 + §3.3 (Phase B). Omitted from this set between 2026-06-11 (#2679) and 2026-08-23 — the orphan-sweeper deleted the plist on every `evolve-admin upgrade`, leaving the daemon DEAD until something re-ran install_evolve_infra_jobs — which a bot deploy does not do: only `evolve-admin install-infra-jobs`, the setup wizard, or a repo-puller pull whose diff touches _INFRA_INSTALL_PATHS (deploy.py is one of those, so in practice recovery came hours-to-days later on an active pod, not never). So the failure was a spurious orphan banner plus a recurring outage, not a tidy delete/reinstall loop. The meta-test that would have caught it at PR time was itself quarantined; see #3768.
        ...
        "ai.evolve.evolve.pairing-sweep",        # 30s sweep: auto-approves pod-admin / primary-owner / auto_admit-channel pending pairings; never auto-approves blocked. Installed by pairing.auto_approver.install_launchd. Spec: internal/spec-user-roster-and-roles-2026-06-07.md.
        ...
        "ai.evolve.evolve.growth-sweep",     # daily 03:40: reconcile app file changes the growth-log observer missed into {shared}/app-growth/_sweep/. Installed by _install_launchd_growth_sweep("evolve"). Report-only — nothing reads the store yet.
        "ai.evolve.evolve.usage-logger", "ai.evolve.evolve.usage-by-app", "ai.evolve.evolve.usage-by-user",   # the three daily app-usage sweeps, all installed by _install_launchd_usage_jobs("evolve"): 03:30 manifest-mtime footprint (usage-stats.json; the FALLBACK signal since AL-1.3) + 03:35 per-app annotation rollup (usage-by-app.json; the PRIMARY one) + 03:37 per-user x per-app rollup (usage-by-user.json; D-S2 track 3, do-not-track gated). usage-logger was omitted before 2026-05-26 → flagged orphan.
        ...
        "ai.openclaw.evolve.install_integrity_monitor",  # daily: wizard-verify gauntlet (ownership/agent/channels) → Signals when install drift detected. Spec: internal/spec-wizard-verification-gauntlet-2026-05-30.md.
        "ai.openclaw.evolve.cascade_pressure_watchdog",  # 60s: tier-cascade pressure-flag emitter. Reads cascade telemetry spans + in-process tier1 counters, writes {shared}/cascade/pressure_flags.json. CascadeController reads the flags at decision time to throttle escalation. Installed by _install_launchd_cascade_pressure_watchdog. Spec: internal/spec-tier-cascade-2026-05-26.md § pressure watchdog.
        "ai.openclaw.evolve.cascade_audit_runner",       # hourly: cascade telemetry → Signals + labels. Bridges anomaly_detector + labeler + plugin-recorded runaway/dangerous-combo flags into the pod's standard alerting layer (Signals via signals.store.observe) and the Phase 4 calibration layer (labels at {shared}/cascade/labels/<day>.jsonl). Installed by _install_launchd_cascade_audit_runner. Spec: internal/spec-tier-cascade-2026-05-26.md § audit layer.
        ...
        "ai.openclaw.evolve.gmail_integration_health",   # 30 min: per-bot Google API probe → Signals per (bot, failure_category); auto-resolves on next clean probe. Installed by _install_launchd_gmail_integration_health. Spec: internal/spec-google-integration-paths-2026-05-30.md §8 (PR δ).
        ...
        "ai.evolve.evolve.agent-bypass-audit",           # daily 04:40: walks recent session transcripts for bots with at-risk apps installed (manifests whose bot_guidance routes a chat trigger through a bot-local script). Two sibling producers over the same walk (run_agent_bypass_audit.py wrapper): agent_bypass_audit counts trigger messages that did NOT invoke the declared script (type=agent_bypass); app_script_failure_audit counts declared scripts the agent invoked but that FAILED — the "(agent) failed" exec chip (type=app_script_failure). One Signal per (bot, app); both auto-resolve when the bot returns to compliance. Installed by _install_launchd_agent_bypass_audit. Phase 1 of internal/spec-agent-freelance-bypass-2026-06-05.md.
        "ai.evolve.evolve.signal-subscriber",            # long-running (KeepAlive): watches {shared}/signals/firing/ and dispatches generators with subscribes_to: [<signal_type>, ...] on Signal arrival. Daily generator_runner sweep is the safety net. Installed by _install_launchd_signal_subscriber. Spec: internal/spec-signal-subscriber-2026-05-31.md.
        "ai.evolve.evolve.delivery-monitor",             # every 5 min: per-window delivery outcomes for scheduled user-facing apps (manifest scheduled_actions[] + optional delivery_contract{}). Tri-state Signals (app_delivery_missed / app_delivery_unmeasurable) + {shared}/delivery_monitor/ledger/<date>.jsonl. Heal (§8): one kickstart/bootstrap attempt per missed window, canary-gated to Morning Briefing v2 (HEAL_CANARY_APP_IDS) during the soak. Installed by _install_launchd_delivery_monitor. Spec: internal/spec-proactive-delivery-monitor-2026-06-10.md.
...
def record_owned_job_labels(
    labels: Iterable[str], shared_dir: Path | None = None,
) -> None:
    """Tell the incursion detector which scheduled-job labels are ours.

    ``incursion.job_inventory`` decides whether a newly appeared LaunchDaemon
    or systemd unit is Evolve's by looking its label up in
    ``{shared_dir}/security/evolve-owned-jobs.json``. It used to decide by
    string prefix, which meant an attacker's ``ai.evolve.helper`` was blessed
    and baselined for the price of one filename (review of PR #3967, finding
    1). This is the write side of that fix: every path that SUCCESSFULLY
    installs a job through the Scheduler seam records the label, and
    :func:`install_evolve_infra_jobs` re-asserts the whole
    :func:`expected_plist_labels` set at the end of a run.

    Deliberately best-effort and silent: an install must never fail, or even
    log noise, because a detector's lookup file could not be written. The cost
    of a missed write is one coverage-gap row on the audit's next pass.
    """
    try:
        from incursion import owned_jobs  # analyzer package (sys.path-installed)
        target = shared_dir
        if target is None:
            try:
                target = Path(
                    load_network(_CANONICAL_NETWORK_JSON).get("sharedDir")
                    or _CANONICAL_SHARED_DIR
                )
            except Exception:  # noqa: BLE001 — no network.json yet (fresh setup)
                target = _CANONICAL_SHARED_DIR
        owned_jobs.record(target, labels)
    except Exception as exc:  # noqa: BLE001 — never break an install over this
        _log.debug("record_owned_job_labels(%s) skipped: %s", labels, exc)
...
def build_plugin() -> None:
    ...
    if not os.access(str(PLUGIN_SRC_DIR), os.W_OK):
        try:
            ...
            pass
        subprocess.run(
            ["sudo", _PROFILE.chown, "-R", f"evolve:{_PROFILE.ops_group}", str(PLUGIN_SRC_DIR)],
    ...
    try:
        ...
        if dist_dir.exists():
            if not os.access(str(dist_dir), os.W_OK):
                ...
                chown = subprocess.run(
                    ["sudo", _PROFILE.chown, "-R", f"evolve:{_PROFILE.ops_group}",
                     str(PLUGIN_SRC_DIR)],
                    capture_output=True, text=True, timeout=30,
                )
        ...
        if dist_dir.exists():
            ...
            # see internal/spec-plugin-install-trust-2026-06-06.md §4.
            #
        ...
        # any other member of the repo's group (the human admin) still hits
        ...
        if dist_dir.exists():
            ...
            if repo_owner and repo_group:
                ...
                # Make group-writable so the repo's group (read off the
                # checkout by the chown above) can later `git pull` past
                # these files. `g+rwX` (capital X) only
        ...
        # Group = ops_group — a `staff` literal re-widened it (see above).
        if _plugin_orig_owner:
            subprocess.run(
                ["sudo", _PROFILE.chown, "-R", f"{_plugin_orig_owner}:{_PROFILE.ops_group}",
                 str(PLUGIN_SRC_DIR)],
                capture_output=True, timeout=30,
            )
...
def _infer_exec_policy(
    ...
    *,
    ...
    internal/diagnosis-evo-exec-approval-leak-2026-05-21.md. Phase E.2.b's
    ...
    injection on team_bot_a. See internal/spec-evo-account-separation-2026-05-25.md
    ...
    process." See internal/spec-app-derived-permissions-2026-05-24.md §Principle.
...
def _explain_exec_policy_inference(
    ...
    return (
        ...
        (
            ...
            f"internal/spec-app-derived-permissions-2026-05-24.md). "
...
_BALANCED_COST_DEFAULTS: dict[str, Any] = {
    ...
    "contextPruning": {"mode": "cache-ttl", "ttl": "5m"},  # no keepLastAssistants — retired by OC 2026.9.2, see oc_retired_keys.py
    "compaction": {
        "mode": "safeguard",
        "maxActiveTranscriptBytes": 400 * 1024,  # budget, not a cliff: compaction_housekeeping.py
        "memoryFlush": {"enabled": True, "softThresholdTokens": 10000, "forceFlushTranscriptBytes": 320 * 1024},
    },
...
def ensure_plugin_config(bot_id: str, network: dict) -> None:
    ...
    # unnecessary — see internal/spec-evo-account-separation-2026-05-25.md.
    #
    # Pivoted 2026-05-25 from a "deny" default to a "full" default for
    # member bots — see internal/spec-app-derived-permissions-2026-05-24.md.
    ...
    # Phase 1 of internal/spec-config-intent-system-2026-05-21.md §3:
    ...
    # ── Plugin entry ────────────────────────────────────────────────────────────
    # Phase 3 cutover (internal/spec-openclaw-json-derived-artifact-2026-05-24.md):
    ...
    try:
        ...
        pass
    changed = gap_fill_cost_settings(cfg, snapshot) or changed
    changed = _apply_compaction_housekeeping(cfg, network, bot_id) or changed  # + retired-key strip
    ...
    # internal/spec-openclaw-json-derived-artifact-2026-05-24.md §7:
    #
    ...
    # internal/spec-autonomy-ladder-2026-06-10.md §2.3 — renderer runs on
...
def install_oc_plugin(bot_id: str, port: int | None = None, network: dict | None = None) -> None:
    ...
    # open. See internal/spec-plugin-install-trust-2026-06-06.md §4.
    #
    # Deliberately NOT coupled to any OC flag or scanner state: the old
    # "withhold --dangerously-force-unsafe-install" framing was never enforcement
    # (OC now documents it a no-op; dropped 2026-08, spec §4). Does NOT generalize
    # — OC 2026.9.2's --force/--accept-capabilities are real; see install below.
    ...
    cmd = [
        ...
        "-l", str(PLUGIN_INSTALL_DIR), "--force", "--accept-capabilities",
    ]
    # Both flags are TERMINAL for a local-path install in OC 2026.9.2 and deploy
    # is headless — without them steps 5-8 die on every bot. Rationale: spec §4.2.
    try:
...
# Re-export: the inheritable evolve-read grant for bot-owned shared-dir
# subdirs now lives beside the subdir registry it belongs to.
BOT_SHARED_SUBDIR_READ_ACL_PERMS = _bot_subdirs.BOT_SHARED_SUBDIR_READ_ACL_PERMS
...
def fix_shared_dir_permissions(bot_id: str, shared_dir: Path) -> None:
    ...
    # directories — the alert dispatcher writes state as `evolve`,
    ...
    # most-recently-deployed bot broke audit-as-`evolve` alert writes
    # with PermissionError (2026-05-07), and (proposals/) caused
    ...
    try:
        ...
        subprocess.run(["sudo", _PROFILE.mkdir, "-p", str(ann_parent)],
                       capture_output=True, check=False)
        subprocess.run(["sudo", _PROFILE.chmod, "1777", str(ann_parent)],
                       capture_output=True, check=False)

    # ── Pre-create the per-bot shared-store leaves ───────────────────────────
    # One entry per leaf in bot_shared_subdirs.BOT_SHARED_SUBDIRS — the SAME
    # tuple _render_evolve_sudoers renders the NOPASSWD grants from and
    # ensure_pod_perms re-verifies. Each entry carries the incident that put it
    # there; read them there, not here. The list was duplicated (here + the
    # sudoers renderer) until 2026-09-03, when exec-failures/ and app-runs/
    # turned out to have landed in this one and not that one: their chown and
    # ACL steps had no grant, both ran check=False, and eight bots on the
    # reference pod carried an evolve-owned 0755 ledger dir the bot user could
    # not append to — silently, for two days.
    _bot_subdirs.create_bot_subdirs(
        shared_dir, bot_id, bot_user, _PROFILE.admin_group, _PROFILE.chown,
    )
...
#   - the alert dispatcher writes its state as the `evolve` user.
...
# After Phase E.2.b of internal/spec-evo-account-separation-2026-05-25.md, the evo
...
# succeed. See internal/design-proposal-signing-key-2026-08-18.md.
...
def ensure_pod_perms(
    ...
    result.checks.extend(_check_tier_usage_dir(shared_dir))  # role-cap ledger: multi-writer 1777 (every bot daemon appends its own power/max transitions; evo-owned 0755 made every non-`evolve` bot's append EACCES, so the caps seeded 0 on each restart).
    ...
    result.checks.extend(_fw.check_daemon_firewall_allow() + [_pm_probe.check_pm_probe_wrapper(), _pm_probe.check_pm_staging_dir()])  # macOS Application Firewall: the daemon's own (Homebrew) interpreter must be allowed to accept incoming connections, or every off-box request — the board on the tailnet address — dies before its first byte while loopback keeps working (2026-09-04 phone test; see firewall_allow) + the PM probe's privileged half, which no-ops until /etc/sudoers.d/pm-probe exists (folded: deploy.py at no-growth cap; rationale in check_pm_probe_wrapper / check_pm_staging_dir docstrings)
    ...
    for bid, bot_user in bot_pairs:
        ...
        # Per-bot shared-store leaves (bot_shared_subdirs): owner + evolve read
        # ACE. Self-heal backstop to the at-deploy pre-creation above — and the
        # check the 2026-09-03 exec-failures drift had no equivalent of, which
        # is why a subdir that silently failed to chown stayed wrong until an
        # operator read a gateway log by hand.
        result.checks.extend(_bot_subdirs.check_bot_shared_subdirs(shared_dir, bid, bot_user) + _board_perms.check_board_store(shared_dir, bid))  # + {sharedDir}/boards/{bid}/ is owned by the daemon that reads it: `sudo evolve-admin board token` runs as ROOT, and a root-owned 0600 token hash 401s every board request with a valid link (2026-09-04 phone test; see board_store_perms)
...
def _install_job_ensuring_restart(spec: JobSpec) -> tuple[bool, str]:
    ...
    **That bounce is for long-running daemons only.** ``restart()`` is
    ...
    if res.ok:
        record_owned_job_labels([spec.label])
...
def install_bot_gateway_plist(
    ...
    # Which OpenClaw it execs (oc_runtime); a pin that cannot resolve is REFUSED.
    from .oc_runtime import PinResolutionError, resolve_gateway_runtime

    try:
        _rt = resolve_gateway_runtime(bot_id, network, platform_name=_PROFILE.name)
    except PinResolutionError as exc:  # never rendered on the global binary
        _log.warning("install_bot_gateway_plist(%s): %s", bot_id, exc)
        return False, str(exc)
    oc_index, pin_env, pin_path_prefix = _rt.entrypoint, _rt.env, _rt.path_prefix
    ...
    spec = JobSpec(
        ...
        env={
            ...
            # A pinned bot's PATH leads with ITS version's bin dir, so the
            # `openclaw` the gateway spawns (doctor, plugins, config) is the
            # same build as the gateway itself. Without this the plist and its
            # own children run different versions — the same split the pins
            # exist to remove, and much harder to see than a wrong plist.
            "PATH": (
                (f"{pin_path_prefix}:" if pin_path_prefix else "")
                + (
...
def _restart_gateway_linux(bot_id: str, bot_user: str) -> None:
    ...
    ok, detail = install_bot_gateway_plist(bot_id, port, user=bot_user, network=network)
    if not ok:
...
def deploy_bot(
    ...
    retired 2026-08-18 (internal/design-proposal-signing-key-2026-08-18.md) —
    ...
    internal/spec-state-store-and-deploy-resilience-2026-06-10.md §2.4):
    ...
    if not dry_run:
        ...
        # to (internal/design-proposal-signing-key-2026-08-18.md). The script
        ...
        # internal/spec-app-audit-2026-05-16.md §7.
        ...
        # internal/spec-backup-key-distribution-unification-2026-06-08.md.
        try:
    ...
    # ALPHA-1/B1: Evolve's ONE arrival scan of this bot, so a freshly-adopted
    # pod's Discovered queue has content the first time it is opened. Once per
    # bot ever; weekly thereafter via the app-discovery-sweep job. Never fails
    # the deploy — see app_discovery.first_scan_on_deploy.
    if not dry_run and result.success:
        first_scan_on_deploy(bot_id, network_path, result, pod_side_effects=pod_side_effects)
...
def _heal_primary_from_tier_config(
    ...
    if write_result is None:
        ...
        return
    # Declare whatever this write LANDED in evolve-tiers.json. Normally
    # nothing — the payload is the bot's own current tiers, so the write is
    # idempotent and ``tiersKeysWritten`` is absent. But it is not always a
    # no-op: ``normalize_tiers_file_shape`` folds a stale legacy ``tiers``
    # sibling into ``rungs`` on the way through, and that fold can flip the bot
    # to Custom and carry the pod's ``autoUpgrade`` block with it. Undeclared,
    # those land as permanent unexplained ``tiers:rungs`` /
    # ``tiers:autoUpgrade`` drift on a bot nobody touched — this heal pass runs
    # on EVERY deploy (spec-delta-digest-audit-noise D3). openclaw.json's
    # ``agents`` is declared alongside, since the primary recompute rewrites it.
    from .provisioning import _record_audit
    from .web.routes_shared import tier_write_oc_keys as _tier_write_oc_keys
    _tier_keys = _tier_write_oc_keys(write_result, {"agents"})
    if _tier_keys - {"agents"}:
        _record_audit("deploy.primary_heal", bot_id, {
            "role": role,
...
def add_bot(
    ...
    *graduated new-bot default* (resolved in code), so nothing static is
    ...
    network = load_network(network_path)
    # EVO-SEP-S5: `evolve`/`evo` are reserved service/assistant accounts, not member bots — refuse to register one (alias-aware via resolved user + explicit --user). This is the registration-side complement to the removal guards in retire.py (PR #3083). See internal/spec-evo-account-separation-2026-05-25.md.
    ...
    if not dry_run:
        ...
        for subdir in ["metrics",
                        *PROPOSAL_LIFECYCLE_SUBDIRS,
                        ...
                        # app-growth: growth-log root, sticky-1777 below —
                        # two writers as different users (each bot's plugin
                        # writes <bot>/, the evolve sweep writes _sweep/<bot>/).
                        "app-growth",
        ...
        for multi_writer_dir in ["metrics", "annotations", "app-growth"]:
        ...
        # Normalize evolve-repo ownership. OWNER must be evolve: a fresh
        # clone lands owned by whoever ran `git clone`, pulls update content
        # but not dir ownership, and OC's plugin scanner silently drops any
        # plugin dir whose uid isn't evolve|root. GROUP must be ops_group,
        # NEVER bot_shared_group — `staff` is every bot's primary group and
        # the venv is editable-installed from here, so `evolve:staff` + the
        # g+rwX below handed every bot write on code root later runs (#2759,
        # one group wider). internal/decision-deploy-checkout-group-2026-09-23.md
        evolve_repo_root = Path(_get_profile().deploy_checkout_default)
        repo_group = _get_profile().ops_group
        if evolve_repo_root.exists():
            r = subprocess.run(
                ["sudo", _PROFILE.chown, "-R", f"evolve:{repo_group}", str(evolve_repo_root)],
                check=False, capture_output=True, text=True, timeout=60,
            )
            if r.returncode == 0:
                result.log(f"Normalized {evolve_repo_root} to evolve:{repo_group}")
            else:
                result.log(
                    f"[warn] chown -R evolve:{repo_group} {evolve_repo_root} failed "
                    f"(rc={r.returncode}): {(r.stderr or r.stdout).strip()}"
                )
            # Mode normalization: chown alone leaves dirs at 755, locking
            # the operator group out of objects the daemon just created.
            # `g+rwX` gets 775 dirs + 664 files in one pass; pairs with the
            # puller's `umask 002` + `core.sharedRepository=group` so both
            # coexist. Safe ONLY because of the chown above.
            r = subprocess.run(
...
def install_bot_docs(
    ...
    *,
    ...
    (differs from the template *and* ≥ the 1500-byte structural floor; MEMORY.md
    is preserved on ANY non-blank diff), and skips — never overwrites — any
    destination it cannot read (see bot_doc_seeding.read_existing_doc), so it is
    ...
    for src, fname, substitute in plan:
        ...
        # they rewrite unconditionally, so skip the read probe entirely for them.
        if role == "primary" and fname in _bot_docs.PRIMARY_VERBATIM_DOCS:
            existing_text, unreadable = None, False
        else:
            existing_text, unreadable = _bot_docs.read_existing_doc(dst)
        if unreadable:
            # Fail SAFE, not open: an unproven read must never become "nothing
            # on disk → write". That inversion is the 2026-06-24 MEMORY.md incident —
            # sudoers granted the cp but not the cat, so live memory was
            # template-clobbered. Skip and say why; seeding resumes once the
            # read path (evolve ACL / sudoers cat grant) is repaired.
            result.log(
                f"[warn] Skipped (unreadable): {dst} — cannot verify it is not "
                "operator/bot-authored; refusing to overwrite. Fix the evolve "
                "read ACL or the sudoers cat grant and redeploy."
            )
            continue
    ...
    def _present(fn: str) -> bool:
        # Unreadable counts as PRESENT: stubbing over a live onboard-written
        # doc we merely can't read is the same fail-open clobber as the
        # 2026-06-24 MEMORY.md incident, one grant-drift away.
        text, unreadable = _bot_docs.read_existing_doc(workspace_dir / fn)
        return unreadable or text is not None
...
def install_evolve_infra_jobs(
    ...
    if not dry_run:
        ...
        install_app_discovery_sweep(result)
        ...
        _install_launchd_growth_sweep("evolve", result)
        ...
        # See internal/spec-upstream-issue-watcher-2026-05-22.md.
        ...
        _install_launchd_preflight_prior("evolve", result)
        ...
        # internal/spec-wizard-verification-gauntlet-2026-05-30.md.
        ...
        # suspension, scope-list drift. Spec: internal/spec-google-integration-
        ...
        # Spec: internal/spec-proactive-delivery-monitor-2026-06-10.md.
        ...
        # watchdog at internal/spec-tier-cascade-2026-05-26.md.
        ...
        _fw.ensure_daemon_firewall_allow(result.log)  # and let the firewall know the interpreter that plist runs
        ...
        # See internal/decision-app-tests-2026-06-08.md and
        ...
        # internal/spec-user-roster-and-roles-2026-06-07.md §11.
        try:
        ...
        # Phase G of internal/spec-alert-subscriptions-2026-05-10.md. Each
    ...
    # Re-assert the incursion detector's owned-job registry from the labels
    # this installer knows it owns. The per-install writes above already cover
    # everything installed in THIS run; the whole-set pass is what backfills a
    # pod upgrading into the registry, and what recovers a registry that was
    # deleted. It is derived from expected_plist_labels() — a statement of
    # installer intent — and never from what is on disk under
    # /Library/LaunchDaemons, which would launder an intruder's job into
    # "Evolve owns this".
    if not dry_run:
        try:
            _net_for_labels = load_network(_CANONICAL_NETWORK_JSON)
            record_owned_job_labels(
                expected_plist_labels(_net_for_labels), _effective_shared_dir,
            )
            result.log("incursion: recorded Evolve-owned scheduled-job labels")
        except Exception as _e:  # noqa: BLE001 — detector state, not the install
            result.log(f"[warn] owned-job registry not recorded: {_e}")
...
def _job_spec_for(label: str, user: str, script_path: Path, schedule: dict,
                  ...
                  jitter_seconds: int = 0, *, bot_id: str | None = None) -> JobSpec:
    ...
    ``bot_id`` is the Evolve bot id, which is NOT always the macOS account the
    job runs as (``user``) — ``network.json`` maps bot ``evolve`` to account
    ``evo``, and any bot may be given an account named for the person who uses
    it. It strips the trailing ``.<bot_id>`` off a per-bot label so the log stem
    is ``evolve-<job>`` for every bot regardless of account name. Anchored, not
    a substring replace: the pre-2026-09-14 code stripped ``.<user>`` anywhere
    in the string, so ``doctor-pass.evolve`` minus ``.evo`` became
    ``doctor-passlve``. Callers with a pod-wide label pass nothing.
    ...
    job_name = label.removeprefix("ai.openclaw.evolve.").removeprefix("ai.evolve.")
    if bot_id:
        job_name = job_name.removesuffix(f".{bot_id}")
    spec = JobSpec(
        label=label,
...
def _plist_content(label: str, user: str, script_path: Path, schedule: dict,
                   extra_args: list[str] | None = None, run_at_load: bool = False,
                   jitter_seconds: int = 0, *, bot_id: str | None = None) -> str:
    ...
    return render_launchd_plist(
        _job_spec_for(label, user, script_path, schedule, extra_args,
                      run_at_load, jitter_seconds, bot_id=bot_id)
    )
...
def _install_launchd(label: str, user: str, script_path: Path, schedule: dict,
                     ...
                     via_seam: bool = True, bot_id: str | None = None) -> None:
    ...
    content = _plist_content(label, user, script_path, schedule, extra_args, run_at_load, jitter_seconds, bot_id=bot_id)
...
def _bootout_retired_per_bot_jobs(bot_id: str, result: DeployResult) -> None:
    ...
    * ``ai.openclaw.evolve.test.<bot>`` — killed 2026-06-08 with the app-test
      surface (internal/decision-app-tests-2026-06-08.md).
...
def _install_launchd_audit_runner(
    ...
    *, user: str | None = None,
    ...
    See ``internal/spec-app-audit-2026-05-16.md`` §7 for the trigger model.
    """
    plist_user = user or bot_id
    _install_launchd(
...
def _install_launchd_usage_jobs(bot_id: str, result: DeployResult) -> None:
    """Install the three daily app-usage sweeps; each covers every bot per run.
    03:30 usage-logger — manifest-mtime footprint (usage-stats.json): the
    FALLBACK signal since AL-1.3. 03:35 usage-by-app — per-app annotation
    rollup, split by attribution grade: the PRIMARY one. 03:37 usage-by-user —
    the same annotations by requester and app x requester, do-not-track gated."""
    for label, script, minute in (
        ("usage-logger", "usage_logger.py", 30), ("usage-by-app", "usage_by_app.py", 35),
        ("usage-by-user", "usage_by_user.py", 37),
    ):
        _install_launchd(
...
def _install_launchd_growth_sweep(bot_id: str, result: DeployResult) -> None:
    """Daily app growth-log backstop (03:40, evolve user; every bot per run).
    Records the app-file changes the plugin-side observer could not see as
    second-class ``attribution: "sweep"`` deltas; report-only, nothing reads
    the store yet. Rationale + on-disk contract: analyzer/app_growth_sweep.py.
    03:40 follows the two usage sweeps (03:30/03:35); no ordering dependency."""
    _install_launchd(
        label=f"ai.evolve.{bot_id}.growth-sweep", user=bot_id, result=result,
        script_path=ANALYZER_DIR / "app_growth_sweep.py",
        schedule={"Hour": 3, "Minute": 40},
        extra_args=["--network", str(_CANONICAL_NETWORK_JSON)],
    )
...
def _install_launchd_pod_health(bot_id: str, result: DeployResult) -> None:
    ...
    (internal/spec-alert-notifier-2026-05-09.md). Gives the not-yet-built
...
def _install_launchd_signal_notifier(bot_id: str, result: DeployResult) -> None:
    ...
    (internal/spec-alert-notifier-2026-05-09.md). The daemon itself is
...
def _maybe_install_launchd_inbound_issues_watcher(
    ...
    See internal/spec-issue-inbox-2026-05-21.md (Phase 4).
    """
    try:
        from install_profile import is_feature_enabled  # type: ignore
    except Exception as exc:  # noqa: BLE001
...
def _install_launchd_proposal_synthesizer(
    ...
    Spec: internal/spec-proposal-synthesizer-2026-05-10.md §5, §7.
...
def _install_launchd_retention(bot_id: str, result: DeployResult) -> None:
    ...
    Prunes data categories that have no other cleanup path:
      ...
      - alerts/{dispatcher,dispatcher-queued,dispatcher-suppressed,
         delivery-failures}/
...
def _install_launchd_preflight_prior(bot_id: str, result: DeployResult) -> None:
    """Install the nightly routing-prior learner (03:20, evolve user).

    Reads the last 14 days of every bot's own turns and writes
    ``bots.<id>.preflight.bot_prior`` + ``prior_evidence`` into
    network.json — the offline answer to the question the retired
    per-turn LLM tier classifiers used to ask in front of the user
    (D-OH2, internal/decision-evolve-overhead-2026-09-07.md).

    **Writes are opt-in and fail closed.** The plist passes ``--apply``,
    but ``preflight_prior.py`` also requires
    ``network.json::cascade.prior.enabled`` to be true. On a pod that has
    not flipped that switch the job runs, logs one line per bot, and
    writes nothing — so installing this daemon changes no routing until
    an operator says so (D-OH5: every Evolve switch fails closed).

    03:20 sits before retention (03:30) and after the 02:00 backup; the
    job only reads turns files and writes network.json, so it has no
    ordering dependency on either — distinct minutes just keep launchd
    from waking three daemons at once. run_at_load=False: first fire is
    the next 03:20.
    """
    _install_launchd(
        label=f"ai.evolve.{bot_id}.preflight-prior",
        user=bot_id,
        script_path=ANALYZER_DIR / "preflight_prior.py",
        schedule={"Hour": 3, "Minute": 20},
        result=result,
        extra_args=["--shared-dir", str(_CANONICAL_SHARED_DIR), "--apply"],
        run_at_load=False,
    )
...
def _install_launchd_openclaw_overrides_expiry(
    ...
    of internal/spec-openclaw-json-derived-artifact-2026-05-24.md.
...
def _install_launchd_breakers_audit(bot_id: str, result: DeployResult) -> None:
    ...
    internal/incident-cost-audit-2026-05-21.md, with a manual-review
...
def _install_launchd_autonomy_limits(bot_id: str, result: DeployResult) -> None:
    ...
    Spec internal/spec-autonomy-ladder-2026-06-10.md §1.3 + §3.3 (Phase B).
...
def _install_launchd_alerts_loop_monitor(result: DeployResult) -> None:
    ...
    Reads {shared_dir}/alerts/dispatcher.jsonl (successful sends — the
    deferred-to-digest rows moved to dispatcher-queued.jsonl 2026-08-25,
    which this monitor never counted) and
...
def _install_launchd_delivery_monitor(result: DeployResult) -> None:
    ...
    Spec: internal/spec-proactive-delivery-monitor-2026-06-10.md §4 (U2.1).
    """
    _install_launchd(
...
def _install_launchd_install_integrity_monitor(result: DeployResult) -> None:
    ...
    Spec: internal/spec-wizard-verification-gauntlet-2026-05-30.md (the
...
def _install_launchd_agent_bypass_audit(result: DeployResult) -> None:
    ...
    structurally. Spec at internal/spec-agent-freelance-bypass-2026-06-05.md.
