...
def _reassert_evolve_read_acl(oc_dir: "str | Path") -> bool:
    ...
    return _get_perms().grant_read_recursive(Path(oc_dir), _EVOLVE_USER)
...
def reassert_evolve_access(bot_id: str, bot_user: str) -> "tuple[bool, list[str]]":
    ...
    Two tiers, cheapest first:
      ...
      1. **Light** — ``reassert_mask`` on ``.openclaw`` (``setfacl -m m::rwX``)
         AND, recursively, on the ``workspace/`` subtree. The top-dir re-widen
         ...
         facet (3) — the 2026-06-29 evo-vps recurrence). It uses only evolve's
         existing ``setfacl`` sudoers grant — no root chmod/chown — so it is safe
