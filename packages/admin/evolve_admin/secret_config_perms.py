...
# True while ``heal_evolve_access`` is mid-flight. ``set_evolve_read_acl``
# (deploy.py) ends with an INLINE post-grant ``verify_evolve_access`` backstop,
# and the heal invokes ``set_evolve_read_acl`` as its FIRST step — before its
# own belt-and-suspenders mask re-widens run — so a failure seen by that inline
# verify is an intermediate state, not an outcome. Without this guard the heal
# logs the loud "contract NOT satisfied" ERROR mid-flight and then WINS, which
# is exactly the fire+clear alert-pair flap the error_reporter turns into
# operator pushes (evolve-vps 2026-08-31, V-2). The heal's own FINAL verify is
# the authoritative one and logs normally, so a heal that LOSES is still loud.
# A contextvar (not a module global) so concurrent admin-server threads healing
# different bots cannot suppress each other's genuine post-grant backstop.
_IN_HEAL: "contextvars.ContextVar[bool]" = contextvars.ContextVar(
    "evolve_access_heal_in_progress", default=False,
)
...
# Token-bearing files in OC's PER-AGENT runtime store — the LLM provider API
# keys the running agent reads (``web/oc_agent_keys.py``). Glob-shaped rather
# than literal because both the agent id and the plugin id vary per bot, so
# they cannot join ``BOT_SECRET_CONFIG_RELPATHS`` (whose entries are also
# walked by ``_secret_relpath_parent_dirs`` for the ACL re-grant, where a
# wildcard would be a nonsense directory name).
#
# OC creates all three 0600 today; the enforcement here exists because
# Evolve's rotate path writes them, and ``cp`` without ``-p`` PRESERVES an
# existing dest's mode — one file that ever became 0644 would stay 0644 on a
# multi-user box. A new relpath shape needs a ``chmod 600`` grant in
# ``setup_wizard._render_evolve_sudoers`` §5d as well.
#
# KNOWN COVERAGE LIMIT, stated rather than papered over: discovery is a glob,
# and on macOS the OC gateway re-hardens ``agents/<a>/agent/`` to 0700 with no
# ACL after every auth write — so ``evolve`` often cannot traverse into it and
# the glob returns nothing. That is a silent no-op, not a false pass: nothing
# was observed, so nothing is claimed. The mode is still enforced at WRITE
# time by ``chmod_secret_config`` in ``oc_agent_keys_io.write_runtime_key``,
# which goes through a root ``sudo /bin/chmod`` and is unaffected by the
# clamp. This check is the drift net for the pods (and platforms) where the
# tree IS traversable.
BOT_SECRET_AGENT_STORE_RELGLOBS: tuple[str, ...] = (
    "agents/*/agent/plugins/*/catalog.json",
    "agents/*/agent/models.json",
    "agents/*/agent/codex-home/auth.json",
)


def discovered_agent_store_relpaths(oc_dir: "str | Path") -> "tuple[str, ...]":
    """Concrete relpaths matching :data:`BOT_SECRET_AGENT_STORE_RELGLOBS`.

    Sorted for a deterministic check order. Returns an empty tuple when the
    tree is absent or untraversable — see the coverage-limit note above.
    """
    base = Path(oc_dir)
    found: set[str] = set()
    for pattern in BOT_SECRET_AGENT_STORE_RELGLOBS:
        try:
            for hit in base.glob(pattern):
                found.add(hit.relative_to(base).as_posix())
...
def verify_evolve_access(
    bot_user: str, perms: "_Perms | None" = None, *, log_error: bool = True,
) -> list[str]:
    ...
    ``log_error`` governs the loud "contract NOT satisfied" ERROR (whose text
    says "after the ACL grants" — only true when grants actually preceded the
    call). It defaults to True — an unknown caller fails toward alarming — but
    the DETECTION-side callers pass False, because on this codepath the clamp
    is *expected* transient state the very next step heals: the OC gateway
    re-hardens ``agents/main/agent`` to 0700 on every auth write, so every
    pull-deploy / hourly-monitor detection pass that lands after such a write
    used to emit the ERROR before its own heal repaired the mask seconds later
    (6-8 fire+clear alert pairs a day on evolve-vps, 2026-08-31 V-2). The
    heal-then-verify contract: detection is quiet, ``heal_evolve_access``'s
    FINAL verify logs loud, so the ERROR fires exactly when a heal LOSES —
    never silencing a real lockout (which also still rides the drift Signal).
    The log is additionally suppressed while ``_IN_HEAL`` is set (the inline
    post-grant backstop in ``set_evolve_read_acl`` runs mid-heal, before the
    heal's belt-and-suspenders mask re-widens — see ``_IN_HEAL``'s docstring).
    """
    if _get_profile().name == "macos":
        return []
    ...
    if failures and log_error and not _IN_HEAL.get():
        _log.error(
...
def heal_evolve_access(bot_id: str, bot_user: str) -> bool:
    ...
    in order:
      ...
      3. Re-verifies via ``verify_evolve_access`` (the perms-seam ``getfacl``
         EFFECTIVE check) and returns ``True`` iff no gaps remain. This final
         verify is THE loud one (heal-then-verify, V-2): it logs the
         "contract NOT satisfied" ERROR exactly when the heal LOSES, while the
         intermediate verify inside step 1 is suppressed via ``_IN_HEAL``.
    ...
    # _IN_HEAL scopes ONLY step 1: set_evolve_read_acl ends with an inline
    # verify_evolve_access backstop that here runs BEFORE the belt-and-
    # suspenders mask re-widens below, so its failure is an intermediate state
    # — logging it loud mid-heal produced the fire+clear alert flap even when
    # the heal WON (V-2). The final verify at the end of this function is
    # outside the guard and stays authoritative + loud.
    _token = _IN_HEAL.set(True)
    try:
...
def reassert_evolve_access(bot_id: str, bot_user: str) -> "tuple[bool, list[str]]":
    ...
    for parent in _secret_relpath_parent_dirs(oc_dir):
        if exists_or_unreachable(parent):
            perms.reassert_mask(parent)
    # Quiet (log_error=False): a Tier-1 miss is not an outcome — Tier 2's heal
    # runs next and ITS final verify is the loud one, so the ERROR fires only
    # when the heal loses (heal-then-verify, V-2).
    failures = verify_evolve_access(bot_user, perms, log_error=False)
    if not failures:
        return True, []
    ...
    # (its final verify already logged the loud ERROR if the heal lost, so the
    # re-read below only collects the failure strings for the caller/Signal).
    if heal_evolve_access(bot_id, bot_user):
        return True, []
    return False, verify_evolve_access(bot_user, perms, log_error=False)
...
def check_evolve_access(bot_id: str, bot_user: str) -> "_PermCheck":
    ...
    # DETECTION is quiet (log_error=False): this check runs at the top of every
    # ensure_pod_perms pass — every pull-deploy and the hourly drift monitor —
    # so it is the first reader to land after the OC gateway's routine
    # auth-write 0700 re-harden of agents/main/agent. The clamp it sees is
    # expected transient state whose apply (heal_evolve_access, below) repairs
    # it seconds later; logging ERROR here before the heal ran produced the
    # recurring fire+clear alert-pair flap on evolve-vps (2026-08-31, V-2 —
    # the log's "after the ACL grants" wording was a lie on this codepath: no
    # grants preceded it). Alarming still fails toward DOING: a heal that
    # LOSES logs the same ERROR from its final verify, ensure_pod_perms
    # records "fix did not return success", and the drift Signal still fires.
    failures = verify_evolve_access(bot_user, log_error=False)
    ok = not failures
    return _PermCheck(
...
def check_bot_secret_modes(bot_user: str) -> list:
    ...
    checks: list = []
    for rel in (
        BOT_SECRET_CONFIG_RELPATHS + discovered_agent_store_relpaths(oc_dir)
    ):
        path = oc_dir / rel
...
SHARED_SECRET_SUBDIRS: tuple[str, ...] = (
    ...
    # One-rotation undo for an LLM provider key
    # (``web/oc_agent_keys_io.stash_previous_key``). Same contract as the
    # neighbours: evolve-owned, 0600, plain-chmod repair, no sudo grant.
    "secrets/llm_key_rollback",
)
...
# ── Reader inventory ────────────────────────────────────────────────────────
#
# WHY THIS EXISTS. PR #3696 clamped ``.machine-key`` from 0644 to 0640 on
# 2026-08-18. It named the attacker it was locking out — a prompt-injected bot
# decrypting the stored PAT — and did not name the daemon it locked out
# alongside them: ``analyzer/backup.py``, which runs as the BOT user and read
# that key on every nightly run to verify its backup repo was private. Nine
# bots stopped backing up that night. Nothing fired, because the resulting
# failure was classified ``skipped`` (see
# ``internal/finding-backups-silently-off-since-2026-08-18.md``).
#
# The comment block above even records the moment the gap was measured: the
# vault's readers are listed as "the admin daemon (evolve) and evo's MCP
# tools… No bot-user reader exists." That was *observably* true on 2026-08-18 —
# because the bot-user reader had already been broken three hours earlier, by
# the very change being justified. A prose claim cannot tell "no reader" from
# "a reader I just broke."
#
# So the inventory is DATA, and a test walks it. For every protected path:
# which accounts read it, and through what. Adding a path to a mode table
# without adding it here fails ``test_keystore_reader_inventory``; so does
# claiming a ``bot`` reader on a path whose mode denies bot accounts. A future
# tightening now has to answer "who reads this?" in a form that CI can check.
#
# ``account`` vocabulary — the uid the reading process actually runs as:
#   "evolve"  the admin daemon and the LaunchDaemons it owns
#   "evo"     the primary bot's user, which holds a named ACE on keystore/
#   "bot"     ANY ordinary bot account — the class #3696 locked out; a claim
#             here is a claim about every bot, not about one lucky one
#   "root"    operator tooling under sudo
class KeystoreReader(NamedTuple):
    account: str
    via: str


# An EMPTY tuple is a positive claim ("nothing reads this any more"), and is
# accepted by the test. A MISSING key is not — that is the #3696 shape.
KEYSTORE_READERS: dict[str, tuple[KeystoreReader, ...]] = {
    f"{KEYSTORE_SUBDIR}/.machine-key": (
        KeystoreReader("evolve", "keystore._get_vault_key — admin daemon, "
                                 "wizard, backup_signal monitor"),
        KeystoreReader("evo", "keystore._get_vault_key via the named ACE on "
                              "keystore/ — evo's MCP keystore tools"),
        # NO "bot" entry, and that is now true BY DESIGN rather than by
        # accident: since the verdict endpoint landed, a bot-user backup run
        # asks web.backup_bot_routes for private/public and never touches the
        # vault. Re-adding a bot reader here would mean re-widening the key.
    ),
    f"{KEYSTORE_SUBDIR}/admin-auth.key": (
        KeystoreReader("evolve", "web.admin_auth.ensure_key — mints admin "
                                 "session cookies"),
    ),
    # Retired with its daemon (design-proposal-signing-key-2026-08-18).
    f"{KEYSTORE_SUBDIR}/evolve-signing.key": (),
    KEYSTORE_VAULT_GLOB: (
        KeystoreReader("evolve", "keystore._file_store_get — admin daemon, "
                                 "incl. the /api/backup-bot/visibility verdict "
                                 "it answers on a bot's behalf"),
        KeystoreReader("evo", "keystore._file_store_get via the named ACE"),
    ),
    f"{KEYSTORE_SUBDIR}/vault": (
        KeystoreReader("evolve", "directory traversal for the above"),
        KeystoreReader("evo", "directory traversal via the named ACE"),
    ),
}
# The five bot-readable files at the keystore root — the reason ``keystore/``
# itself must stay 0755. Registered here so the inventory covers the whole
# surface, not only the tightened half.
for _bot_file in KEYSTORE_BOT_READABLE_FILES:
    KEYSTORE_READERS[f"{KEYSTORE_SUBDIR}/{_bot_file}"] = (
        KeystoreReader("evolve", "admin daemon"),
        KeystoreReader("evo", "named ACE"),
        KeystoreReader("bot", "read directly as the bot user — see the "
                              "KEYSTORE_BOT_READABLE_FILES note above"),
    )
del _bot_file


# The predicates that WALK this inventory live in
# ``packages/admin/tests/test_keystore_reader_inventory.py``, not here: they
# are the enforcement, and this module holds the claim. Same split as
# ``KEYSTORE_BOT_READABLE_FILES`` above, which is data here and an assertion
# in ``test_ensure_pod_perms``.
