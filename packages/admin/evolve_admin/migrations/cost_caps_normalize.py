...
class MigrationResult:
    ...
    # D-CC4 default flip: an un-chosen ``downgrade-tier`` was rewritten to
    # the checkpoint action. See ``_flip_default_spend_cap_action``.
    pod_spend_cap_action_flipped: bool = False
    ...
    def total_changes(self) -> int:
        return (
            ...
            + (1 if self.pod_spend_cap_action_flipped else 0)
        )
    ...
    def summary_line(self) -> str:
        ...
        if self.pod_spend_cap_action_flipped:
            parts.append("spendCapAction downgrade-tier → checkpoint (D-CC4)")
        if self.errors:
...
def run(shared_dir: Path, network_path: Path) -> MigrationResult:
    ...
    # Captured BEFORE the Phase-8 block below mutates ``thresholds``. The
    # already-normalized retirement further down must read what the OPERATOR
    # expressed, not what this pass leaves behind: the flip writes a valid
    # ``checkpoint`` back, and the strip erases an explicit ``alert-only`` —
    # reading either afterwards would answer the wrong question.
    _raw_thresholds = network.get("thresholds")
    original_spend_cap_action = (
        _raw_thresholds.get("spendCapAction")
        if isinstance(_raw_thresholds, dict)
        else None
    )
    ...
    if isinstance(thresholds, dict):
        ...
        # ── D-CC4 default flip, BEFORE the action mapping below ──────────
        # Rewrite an un-chosen ``downgrade-tier`` to ``checkpoint`` so the
        # mapping never enshrines it as a rung, and so the stripped-key
        # restore at the bottom writes the new default back.
        if _flip_default_spend_cap_action(thresholds):
            result.pod_spend_cap_action_flipped = True
            network_changed = True
        ...
        if stripped_any:
            ...
            network_changed = True
        # The action is the one cost key with no home in the BE rungs:
        # ``checkpoint`` maps to no threshold, and the resolver's BE
        # inference would read a legitimately-separate ``tier_downgrade``
        # rung as if it were the cap action. Restore it explicitly so the
        # flip survives the strip. ``alert-only`` / ``pause-crons`` /
        # ``suspend-bot`` keep their existing behaviour (dropped here,
        # inferred or defaulted at read time) — this is not the migration
        # that re-homes them.
        if spend_cap_action == "checkpoint":
            thresholds["spendCapAction"] = "checkpoint"
            network_changed = True
        # Only delete the thresholds key when it becomes empty — the
        # wider key may carry burst / velocity-forecast fields owned by
        # other code.
        if stripped_any and not thresholds:
            del network["thresholds"]

    # ── D-CC4 on a pod the 2026-06 normalization already ran ──────────────
    # The flip above only reaches a pod whose network.json still SAYS
    # ``downgrade-tier``. On a pod normalized in 2026-06 the stock action was
    # already turned into ``pod_defaults.budget.tier_downgrade_usd = <L1 cap>``
    # and the key stripped, so nothing above touches it — and that rung is the
    # live cheap-model path: it rewrites the bot's primary model at the same
    # dollar the checkpoint fires at, so an owner who says "continue" resumes
    # on the tier-3 model behind a message that says otherwise (the exact
    # D-CC2 failure). Retire it by the same fingerprint the UI save path uses
    # (``routes_cost_measures``): a rung whose value EQUALS the L1 cap is the
    # mirror the migration minted, not a rung an operator chose.
    if _retire_stock_mirror_rung(be_config, original_spend_cap_action):
        be_changed = True
        result.pod_spend_cap_action_flipped = True
...
#: The value the installer used to scaffold into every fresh network.json.
_OLD_STOCK_SPEND_CAP_ACTION = "downgrade-tier"


def _flip_default_spend_cap_action(thresholds: dict) -> bool:
    """Rewrite an un-chosen ``downgrade-tier`` to ``checkpoint``. Returns
    True iff the value changed.

    Operator decision D-CC4. ``downgrade-tier`` was never a choice most
    pods made: ``config.py``'s scaffold wrote it into every fresh
    network.json and ``config_sandbox/schema.py`` labelled it
    "(recommended)". The 2026-09-03 incident is what that un-chosen
    default does on a real user turn — every remaining turn answered on
    the cheapest model, undisclosed, still billing.

    **What "default-by-omission" means here.** The pod-wide
    ``spendCapAction`` key has no per-bot override record, so there is no
    stored evidence distinguishing "the installer wrote this" from "an
    operator typed this" — the two are byte-identical. Given that, the
    migration treats the OLD STOCK VALUE as the default and every other
    value as a choice: ``alert-only``, ``pause-crons`` and ``suspend-bot``
    are left exactly as they are, and so is an absent key (which the
    resolver already reads as the checkpoint). An operator who genuinely
    wants the downgrade re-selects it in one edit, and the schema entry
    now says plainly what it costs them.
    """
    if thresholds.get("spendCapAction") != _OLD_STOCK_SPEND_CAP_ACTION:
        return False
    thresholds["spendCapAction"] = "checkpoint"
    return True


def _valid_spend_cap_action(value) -> bool:
    """True iff ``value`` is a spendCapAction an operator could have chosen.

    Validity is read from ``spend_caps.VALID_ACTIONS`` — the same tuple the
    resolver gates on, so "the operator expressed a choice" means exactly what
    it means at read time. When the analyzer package is not importable the
    answer is the conservative one: any non-empty string counts as a choice,
    so an unreadable environment never retires a rung.
    """
    if not isinstance(value, str) or not value.strip():
        return False
    try:
        from spend_caps import VALID_ACTIONS  # type: ignore[import]
    except Exception:  # noqa: BLE001
        return True
    return value.strip() in VALID_ACTIONS


def _retire_stock_mirror_rung(be_config: Any, spend_cap_action: Any) -> bool:
    """Pop the ``tier_downgrade_usd`` rung the 2026-06 migration mirrored.

    Returns True iff a rung was removed. Two conditions, both required:

      * ``network.json`` carries no valid ``spendCapAction`` — the operator
        never typed a choice, so the checkpoint default applies; and
      * ``pod_defaults.budget.tier_downgrade_usd`` EQUALS
        ``per_bot_daily_hard_usd`` (both set) — the fingerprint of the value
        the 2026-06 pass copied out of the stock ``downgrade-tier``, rather
        than a downgrade threshold an operator placed at a different dollar.

    A rung at any other value is a real, separate choice and is preserved.
    """
    if _valid_spend_cap_action(spend_cap_action):
        return False
    budget = be_config.pod_defaults.setdefault("budget", {})
    if not isinstance(budget, dict):
        return False
    rung = budget.get("tier_downgrade_usd")
    l1_cap = budget.get("per_bot_daily_hard_usd")
    if rung is None or l1_cap is None:
        return False
    try:
        if float(rung) != float(l1_cap):
            return False
    except (TypeError, ValueError):
        return False
    budget.pop("tier_downgrade_usd", None)
    return True
...
def _cli_entrypoint() -> int:
    ...
    print(_json.dumps({
        ...
        "pod_spend_cap_action_flipped": result.pod_spend_cap_action_flipped,
