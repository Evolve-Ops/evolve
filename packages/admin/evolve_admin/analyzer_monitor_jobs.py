...
ANALYZER_MONITOR_PLIST_LABELS: tuple[str, ...] = (
    ...
    "ai.openclaw.evolve.model_liveness_monitor",  # daily 04:40: model liveness probe (auto-upgrade Phase 2, spec-model-auto-upgrade-2026-07-30 §Liveness guard). One REAL agent turn per routed model (each bot's role-resolved models[0] targets, deduped pod-wide, capped at model_liveness.MAX_PROBES_PER_RUN) through the same gateway dispatch path user messages ride — sudo -u <bot> openclaw agent --json --model <target>, via oc_cli. Catches the #3489 class: a provider block dispatching to the wrong transport 401s like key expiry while `models list` stays green, leaving rungs silently dead fleet-wide. Fires model_dispatch_failed per dead/rerouted target; sweeps ONLY targets this run probed ok (unverified probes and unprobed targets — spot-checks, cap-dropped, broken-bot — never sweep; fail-safe). COSTS REAL TOKENS — each probe pays the bot's full system-prefix input (~10-27k tokens/turn), ~8 turns/day, billed to the probing bot; the one LLM-dispatching monitor; cadence stays daily. Read+dispatch only, never mutates config. Watched by monitor_coverage's producer-liveness layer. Installed by analyzer_monitor_jobs.install_model_liveness_monitor.
...
def install_model_liveness_monitor(result: "DeployResult") -> None:
    ...
    accounting. The spend is bounded by design: probes only the role-resolved
    ...
    09:00 digest; ``run_at_load=False`` keeps deploys from firing a probe
    burst while gateways may be mid-restart (a transient false RED).
    """
    from .deploy import ANALYZER_DIR, _CANONICAL_SHARED_DIR, _install_launchd
    _install_launchd(
