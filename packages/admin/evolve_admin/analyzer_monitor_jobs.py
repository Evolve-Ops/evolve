...
All run as the ``evolve`` user, pod-wide (no per-bot suffix). All but ``dossier_edition`` are Signal producers; that one is a weekly metric writer and lives here because it shares the same install shape. The shared
...
ANALYZER_MONITOR_PLIST_LABELS: tuple[str, ...] = (
    "ai.openclaw.evolve.capability_gap_monitor",     # daily 03:15: walks per-bot ObservationTuples, clusters by noun, emits app_suggester_gap Signals where the conversational theme has no installed app coverage AND the bot's AGENTS.md doesn't contradict. Pure Python, no LLM. Closes the producer side of the app_suggester grounding contract — without it, app_suggester is silent. Installed by analyzer_monitor_jobs.install_capability_gap_monitor. Spec: internal/spec-rsi-proposal-eligibility-2026-06-05.md §"Phase 2".
...
def install_capability_gap_monitor(result: "DeployResult") -> None:
    ...
    (``internal/spec-rsi-proposal-eligibility-2026-06-05.md``), this monitor
...
def install_engagement_amplifier_monitor(result: "DeployResult") -> None:
    ...
    ``internal/spec-rsi-proposal-eligibility-2026-06-05.md``. Companion to
...
def install_evo_path_probe_monitor(result: "DeployResult") -> None:
    ...
    Spec: internal/spec-reports-2026-06-12.md (signal-producer quality + UX).
    """
    from .deploy import ANALYZER_DIR, _CANONICAL_SHARED_DIR, _install_launchd
    _install_launchd(
...
def install_model_liveness_monitor(result: "DeployResult") -> None:
    ...
    (``internal/spec-model-auto-upgrade-2026-07-30.md`` §Liveness guard). One real
...
def install_autogen_volume_monitor(result: "DeployResult") -> None:
    ...
    (``internal/footprint-disk-output-audit-2026-06-28.md``). Walks the auto-gen
...
def install_roster_allowlist_drift_monitor(result: "DeployResult") -> None:
    ...
    R1a PR3 safety net (internal/spec-users-meta-2026-06-15.md §"Deferred to a
...
def install_roster_coherence_monitor(result: "DeployResult") -> None:
    ...
    M1-B3 (internal/spec-users-meta-2026-06-15.md §"M1 — Multi-platform messaging
...
def install_model_swap_watch(result: "DeployResult") -> None:
    ...
    (internal/design-model-swap-behavior-guard-2026-08-19.md). The admin UI's
...
def install_dossier_edition_writer(result: "DeployResult") -> None:
    """Install the weekly dossier edition writer (Mon 04:10, evolve user).

    The pod dossier's longitudinal spine is the SEQUENCE of weekly editions,
    which means the only way to have history in October is to start recording
    in August. This job is that recorder. It reads producers that already
    exist and writes one raw-metric JSON per ISO week, plus the module set
    beside it that says in plain English what that week's numbers mean; it
    emits no Signals and no Proposals. Its output is read by the Pod
    Intelligence page, which renders and never writes back.

    The synthesis rides in the SAME job rather than a second one because a
    trend is a comparison between editions: the module set for a week has to
    be written while that week's edition is the newest on disk, and splitting
    the two into separately-scheduled daemons would put a race between them.

    Cadence: Monday 04:10. Monday because ISO weeks start there, so the run
    finds the week it is about to record already complete — the edition seals
    as a final measurement rather than a provisional one. 04:10 because it
    must land AFTER the daily producers it snapshots have rolled Sunday over
    (usage-logger 03:30, usage-by-app 03:35) and clear of the 03:15-04:50
    monitor block. No RunAtLoad: the first scheduled firing is next Monday,
    and the operator seeds the spine's first edition by hand with ``--now``.
    """
    from .deploy import (
        ANALYZER_DIR,
        _CANONICAL_SHARED_DIR,
    ...
    _install_launchd(
        label="ai.openclaw.evolve.dossier_edition",
        user="evolve",
        script_path=ANALYZER_DIR / "dossier_edition.py",
        schedule={"Weekday": 1, "Hour": 4, "Minute": 10},  # Monday
        result=result,
        extra_args=["--shared-dir", str(_CANONICAL_SHARED_DIR)],
        run_at_load=False,
        jitter_seconds=120,
...
def install_analyzer_monitor_jobs(result: "DeployResult") -> None:
    """Install every pod-wide analyzer job this module owns, in order.
    ...
    All but the last are Signal-producing monitors watched by
    ``monitor_coverage``'s producer-liveness layer. ``dossier_edition`` is
    deliberately not one: it emits no Signals, it writes a metric file, and a
    stdout-mtime liveness check on a weekly writer would say nothing useful.
    """
    ...
    install_dossier_edition_writer(result)
