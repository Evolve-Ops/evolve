...
Spec: internal/spec-safe-upgrade-2026-05-02.md.
...
def _installed_version() -> str | None:
    """Installed OC version, read through the shared platform-keyed reader.

    Was a hardcoded ``/opt/homebrew/...`` read — a path that does not exist on
    a Linux pod, so every report there recorded ``installed_version: null``
    and was therefore judged stale the instant it was written, permanently
    suppressing the apply affordance. ``upstream_version`` resolves the
    package.json per platform (#3194 seam) and normalizes exactly the way the
    banner and the root helper's freshness gate do, so the recorded value and
    its comparison partners cannot drift apart.
    """
...
def _query_install_index(uri: str) -> "tuple[Any, ...] | None":
    """Run the newest-row query against one sqlite URI. None on any error."""
    try:
        conn = sqlite3.connect(uri, uri=True, timeout=2.0)
    except sqlite3.Error:
        return None
    try:
        return conn.execute(_INSTALL_INDEX_QUERY).fetchone()
    except sqlite3.Error:
        return None
    finally:
        conn.close()
...
def _read_installs_from_sqlite(path: Path) -> dict[str, Any] | None:
    ...
    WAL-AWARE FIRST, immutable as the fallback (2026-08-18). OC 2026.7 keeps
    this table in a WAL-mode DB, so a row OC just wrote lives in the ``-wal``
    sidecar until a checkpoint folds it into the main file. ``immutable=1``
    reads the main file ALONE — by design it does not consult the -wal/-shm —
    so a reader using it sees the last CHECKPOINTED snapshot and cannot tell
    a stale view from a current one.

    Measured on the pod 2026-08-18: the same DB, same query, read both ways
    minutes after an ``openclaw plugins install`` —

        immutable=1 : codex.spec == "@openclaw/codex"              (29 min stale)
        mode=ro     : codex.spec == "@openclaw/codex@2026.7.1-1"   (current)

    — which is what put ``deploy``'s unpinned-spec reconciler in a loop: it
    read "unpinned", ran a real npm install to fix it, OC wrote the pinned row
    into the WAL, the next read still said "unpinned", every deploy, forever.

    ``immutable=1`` is KEPT as the fallback because it is the only mode that
    works for a reader holding ACL read on the DB file but no write on its
    directory: a plain ``mode=ro`` connection to a WAL database may need to
    create/attach the ``-shm``, which fails with SQLITE_CANTOPEN there. So:
    try the honest read, degrade to the stale-but-possible one.

    The returned dict carries ``_wal_visible`` — True when the WAL-aware read
    won. A consumer about to make a WRITE decision (the re-pin reconciler) or
    a SAFETY decision (the upgrade gate's ``_install_state_determinable``)
    can use it to distinguish "I read the registry" from "I read a snapshot of
    the registry that may predate what OpenClaw just did".

    Returns None if the DB can't be opened either way or has no usable row."""
    # mode=ro (not immutable): consults -wal, so it sees uncheckpointed rows.
    row = _query_install_index(f"file:{path}?mode=ro")
    wal_visible = row is not None
    if row is None:
        row = _query_install_index(f"file:{path}?mode=ro&immutable=1")
    if not row:
        return None
    try:
    ...
    return {
        ...
        # False ⇒ this is the last CHECKPOINTED snapshot; OpenClaw may have
        # written newer records that are not visible here.
        "_wal_visible": wal_visible,
    }


def read_installs_for_write_decision(user: str) -> dict[str, Any] | None:
    """``_read_installs_json`` for callers that will WRITE based on the answer.

    Returns None when the registry could only be read in the degraded
    (non-WAL) mode — a snapshot that may predate what OpenClaw itself just
    wrote. Acting on that is how ``deploy``'s unpinned-spec reconciler ended
    up re-installing the same plugin on every deploy forever: it read a stale
    "unpinned", wrote a fix, OpenClaw recorded the fix in the WAL, and the
    next read still said "unpinned".

    Read-only consumers should keep using ``_read_installs_json`` — a stale
    view still beats no view when nothing is being changed on the strength
    of it.
    """
    data = _read_installs_json(user)
    if isinstance(data, dict) and data.get("_wal_visible") is False:
        return None
    return data
...
# (internal/spec-gallery-delivery-convention-2026-06-11.md §3):
#
...
# internal/decision-add-bot-m4-u1-proof-2026-06-11.md §"The 07:00 window").
...
def gate_compat_contract(
    candidate_version: str | None,
    *,
    shared_dir: Path | None = None,
) -> tuple[GateResult, list[Requirement]]:
    """Has Evolve's OpenClaw compatibility contract passed on the candidate?

    Design: internal/design-oc-upgrade-safety-2026-09-08.md §2 row 3. The
    contract (``packages/plugin/src/contract/``) turns every assumption the
    plugin makes about OpenClaw into a test that runs against a REAL
    installed candidate; :mod:`evolve_admin.oc_compat` reads the verdict.

    This gate is what makes "3 blockers" read as *which* assumptions break.
    Each failing contract check becomes its own blocking Requirement naming
    the check and the incident it exists for, so the preflight table has one
    row per broken assumption rather than one opaque "not validated" row.

    A candidate with no evidence at all is ONE blocking requirement, not
    nine: "the contract has not been run against this version" is a single
    fact, and listing nine unknowns would read as nine failures.
    """
    from . import oc_compat

    if not candidate_version:
        # No resolved candidate — the registry gate already blocks on that,
        # and a second blocker saying the same thing is noise.
        return GateResult(ok=True, details={"skipped": "no candidate version resolved"}), []

    state = oc_compat.validation_state(candidate_version, shared_dir=shared_dir)
    details: dict[str, Any] = {
        "candidate": candidate_version,
        "state": state.state,
        "tested_versions": state.tested,
        "failing_checks": state.failing,
        "checked_at": state.checked_at,
        "overridden": state.overridden,
        "manifest_error": state.manifest_error,
    }

    if state.state == oc_compat.STATE_TESTED:
        return GateResult(ok=True, details=details), []

    if state.overridden:
        # The operator typed a confirmation naming these checks. Keep the
        # rows visible and advisory so the report still says what is broken,
        # but stop blocking: it is their pod, and the override is logged.
        reqs = [
            Requirement(
                id=f"compat-contract-{check}",
                summary=f"Contract check {check} fails on OpenClaw {candidate_version} (operator override recorded)",
                remediation="The override stands until a fresh contract run changes which checks fail.",
                blocking=False,
                source_gate="compat_contract",
            )
            for check in state.failing
        ] or [
            Requirement(
                id="compat-contract-unvalidated",
                summary=f"OpenClaw {candidate_version} is not validated by Evolve (operator override recorded)",
                remediation="The override stands until a contract run is recorded for this version.",
                blocking=False,
                source_gate="compat_contract",
            )
        ]
        return GateResult(ok=True, details=details), reqs

    if state.failing:
        return GateResult(ok=False, details=details), [
            Requirement(
                id=f"compat-contract-{check}",
                summary=f"Contract check {check} fails on OpenClaw {candidate_version}",
                remediation=(
                    "This is an assumption Evolve makes about OpenClaw that the "
                    "candidate breaks. See internal/oc-compatibility.md for the "
                    "check and the incident it exists for; the fix ships in Evolve, "
                    "not in your pod."
                ),
                blocking=True,
                source_gate="compat_contract",
    ...
    return GateResult(ok=False, details=details), [
        Requirement(
            id="compat-contract-unvalidated",
            summary=f"Evolve has not validated OpenClaw {candidate_version}",
            remediation=(
                "No contract run has been recorded for this version. Wait for the "
                "nightly compatibility run, or run the suite on this pod "
                "(node dist/contract/cli.js --prefix <install> --json <out>). "
                "Absence of a run is not a pass."
            ),
            blocking=True,
            source_gate="compat_contract",
        ),
...
def run_preflight(
    ...
    *,
    ...
    # The compatibility contract: has Evolve been validated against THIS
    # candidate? Registry-independent in the sense that matters — it reads a
    # recorded contract run, never the network — so it runs even when the
    # registry fetch failed (in which case there is no candidate version and
    # the gate no-ops rather than inventing a blocker).
    cc_result, cc_reqs = gate_compat_contract(
        candidate.get("resolved_version"), shared_dir=shared_dir,
    )
    gates["compat_contract"] = cc_result
    requirements.extend(cc_reqs)
