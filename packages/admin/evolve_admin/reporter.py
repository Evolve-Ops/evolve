...
def _error_spike_component(fp: str) -> str:
    """Best-effort short component name from a log fingerprint.

    Fingerprints look like ``ERROR evolve_admin.secret_config_perms
    set_evolve_read_acl: ...`` — the second token is the logger name. The last
    dotted segment ("secret_config_perms") is enough to tell two concurrent
    spikes apart in a title without dumping the raw line there.
    """
    parts = fp.split(" ", 2)
...
def _plain_error_spike_title(fp: str) -> str:
    """Tier-A title for an error_spike Signal (docs/voice-guide.md).

    The old title was ``Admin server: <raw 80-char log excerpt>`` — a code dump
    in the headline slot. Lead with what is happening in plain words; keep the
    component tag so distinct spikes stay distinguishable in the Alerts list.
    The full raw line still travels in body + details for the technical reader.
    """
    component = _error_spike_component(fp)
    suffix = f" ({component})" if component else ""
    return f"Evolve's admin service keeps hitting an error{suffix}"


def _plain_error_spike_body(sample: str, occurrences: int) -> str:
    """Tier-A body: what happened + impact first, raw log line last."""
    times = f"{occurrences} times" if occurrences != 1 else "once"
    return (
        f"A background part of Evolve logged the same error {times} in the "
        "last hour. Your bots usually keep working, but something in Evolve "
        "isn't finishing its job. If this alert keeps coming back, check the "
        "Alerts page or ask Evo to look into it.\n\n"
        f"Technical detail: {sample[:400]}"
    )
...
def emit_error_signals(shared_dir: Path, *, lookback_hours: float = 1.0) -> int:
    ...
    Phase 5 of internal/spec-alerts-signal-store-2026-05-07.md. Ignores the
    ...
    for fp, occurrences in by_fp.items():
        ...
        try:
            signals_store.observe(
                ...
                severity=severity,
                scope="host",
                title=_plain_error_spike_title(fp),
                body=_plain_error_spike_body(sample, len(occurrences)),
                details={
                    "fingerprint": fp,
