...
Triggers 1 and 2 are the same question D1 asks — *does this admission key
already index into a person row?* — so since M1-B2b both are answered by one
``roster_identity.resolve_admission`` consult rather than two hand-rolled id
sets. That consult is exact-match on the recorded id; it never infers identity
from a name or handle (a wrong link would transfer privilege).

1. **Pod-admin claim** (existing) — the id resolves to ``pod.admins``
   (``network.json::pod.admins.external_ids[<channel>]``). Auto-approval
   reason: ``"known pod admin"``.

2. **Primary-owner claim** (new) — the id resolves to this bot's
   ``primary_user`` row, matching ANY of that person's ids in
   ``network.json::bots.<bot>.primary_user.external_ids[<channel>]``
   (a list since M1-B2; the reader tolerates the legacy scalar).
...
3. **Per-channel auto_admit mode** (new) — deliberately NOT a resolution;
   this trigger admits a genuine stranger, and a stranger IS a new person
   until an operator asserts otherwise (D1). The overlay's per-channel
...
Spec: internal/spec-user-roster-and-roles-2026-06-07.md §11, deferred
"Phase 1.1 ambient sweep" from internal/spec-per-bot-users-management-2026-05-29.md.
"""
...
def _sweep_channel(
    ...
    for req in requests:
        ...
        # D1 consult (M1-B2b): does this admission key already index into a
        # person row? Exact-match only — never a name/handle guess.
        admission = ri.resolve_admission(network, bot_id, channel, ext_id)
        reason = _resolve_auto_approve_reason(admission, mode)
        if reason is None:
            continue
        ...
        try:
            ...
            result.approved.append({
                ...
                # Which existing person this id resolved to, or None when the
                # auto-admit trigger admitted a genuine stranger (a new row).
                "person": admission.person_key,
            })
...
def _resolve_auto_approve_reason(
    admission: "ri.AdmissionResolution",
    mode: str,
) -> "str | None":
    ...
    The first two triggers are exactly D1's resolution question — "does this
    admission key already index into a person row?" — so since M1-B2b they read
    it off :func:`roster_identity.resolve_admission` rather than re-deriving two
    hand-rolled id sets. Identical behavior (``resolve_admission`` is bot-scoped
    to the pod-admin bag + this bot's primary, in that precedence order, and
    matches ANY of a person's ids on the channel), with the D1 invariant now
    stated once instead of copied. The third trigger is deliberately NOT a
    resolution: ``auto_admit`` admits a genuine stranger, and a stranger IS a
    new person until an operator asserts otherwise.
    """
    person = admission.person
    if person is not None and person.kind == ri.POD_ADMIN:
    ...
    if mode != "closed":
        ...
        if person is not None and person.kind == ri.PRIMARY_USER:
