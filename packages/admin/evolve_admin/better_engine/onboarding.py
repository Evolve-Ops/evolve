...
def build_onboarding_state(shared_dir: Path, network: dict) -> dict:
    ...
    Pod-wide signals added 2026-06-02 (pod-setup checklist):
      ...
      - pod_tiers_configured: primary bot's ~/.openclaw/evolve-tiers.json has
        tier2 + tier0 model bindings (baseline the rest of the pod inherits from)
    ...
    pod_admins_claimed = False
    if isinstance(pod_admins, dict):
        ext_ids = pod_admins.get("external_ids") or {}
        if isinstance(ext_ids, dict):
            for chan_ids in ext_ids.values():
                if isinstance(chan_ids, list) and any(
                    isinstance(eid, str) and eid.strip() for eid in chan_ids
                ):
    ...
    # both tier2 (workhorse — what runs human turns) AND tier0 (judge —
    # evaluates session quality) have at least one model. Mirrors the
    # per-bot ai_optim_tiers detector but specifically on the primary
    # bot — if the primary is tiered, the operator has visited AI
    # Optimization at least once.
        ...
        try:
            ...
            if tiers_path.is_file():
                ...
                if isinstance(tiers_doc, dict):
                    ...
                    # A.4) — a pod-base-only rung satisfying standard/judge
                    # would otherwise report the pod un-onboarded. Mirrors
                    # setup_checklist._detect_ai_optim_tiers.
                    pod_models = (network or {}).get("models") if isinstance(network, dict) else None
                    merged_doc = merge_model_catalog(pod_models, tiers_doc)

                    have = True
                    for required in ("tier2", "tier0"):
...
ONBOARDING_TASKS: list[OnboardingTask] = [
    ...
    OnboardingTask(
        ...
        context=(
            "Tiers map roles (workhorse, judge, power, grunt) to specific "
            "LLM models. The primary bot's tiers are the pod's baseline — "
            "they cascade as defaults for every other bot until that bot "
            "overrides them. Both tier2 (workhorse — what runs human turns) "
            "and tier0 (judge — evaluates session quality for Better Engine) "
            "need at least one model picked. Visit AI Optimization, pick a "
            "bot, then save."
        ),
        depends_on=["primary_installed"],
