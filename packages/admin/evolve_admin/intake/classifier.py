...
The classifier is Haiku-tier — small token budget, cheap, called once
...
See ``docs/spec-issue-inbox-2026-05-22.md`` for the design rationale
...
def _call_anthropic(
    *,
    ...
    """Same shape as ``evo.wizard.extractor._call_anthropic``.

    Lifted here rather than reused so the classifier doesn't take a
    hard dep on the wizard package; both modules use the same urllib
...
def _resolve_api_key() -> str | None:
    """Resolve the Anthropic API key. Falls through to None when the key
    isn't available — the caller surfaces a graceful "classifier
    disabled" response instead of crashing.

    Resolution order matches the rest of the evo subsystem:
      1. ``ANTHROPIC_API_KEY`` env var
      2. Primary-bot's auth-profiles.json
    """
...
# ─── Default classifier (live Anthropic call) ──────────────────────────────
...
def _default_classifier(message: str, ctx: ClassificationContext) -> Verdict:
    """Live Anthropic call. Falls back to a low-confidence ``local_env``
    verdict on any failure so the caller can degrade gracefully."""
    api_key = _resolve_api_key()
    if not api_key:
        return Verdict(
            category="local_env",
            confidence=0.0,
            reasoning="classifier disabled — no Anthropic API key available",
            in_chat_help=(
...
def _default_reviser(
    ...
    """Live Anthropic call. Falls back to a low-confidence "I couldn't
    revise" verdict on any failure so the caller can degrade gracefully."""
    api_key = _resolve_api_key()
    if not api_key:
        return ReviseVerdict(
            new_title=current_title, new_body=current_body,
            confidence=0.0,
            reasoning="reviser disabled — no Anthropic API key available",
        )
...
def _default_triager(
    ...
    """Live Anthropic call. Falls back to a low-confidence "unknown"
    verdict on any failure so callers can degrade gracefully."""
    api_key = _resolve_api_key()
    if not api_key:
        return TriageVerdict(
            confidence=0.0,
            reasoning="triager disabled — no Anthropic API key available",
        )
...
def classify_issue(
    ...
    *,
    ...
    Internally delegates to the active classifier (default: Anthropic;
    swappable via :func:`set_classifier` for tests). Any unhandled
