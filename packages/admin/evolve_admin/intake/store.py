...
def new_intake_id(shared_dir: Path | None = None) -> str:
    ...
    The ``shared_dir`` check only sees ids that are *on disk*, and it
    has to: several processes (the admin daemon, the issue watchers)
    mint into one store, so a process-local memory of past mints would
    be neither necessary nor sufficient. Mint immediately before
    persisting, as every caller here does; a batch of ids held unwritten
    is separated by entropy alone.

    The two claims above (multiple processes minting into one store; a
    held-but-unpersisted batch being out of contract) are documentation
    of intended use, not a tested guarantee — no test here enforces
    either. ``_id_taken`` is a single-process filesystem check; nothing
    stops a caller from minting a batch and persisting it later.
    """
    ...
    for _ in range(_ID_MINT_ATTEMPTS):
        ...
        if shared_dir is None or not _id_taken(shared_dir, candidate):
            return candidate
    # Astronomically unlikely at 16.7M values/day — widen the suffix once
    # rather than give up on the first miss. But still never hand back an
    # id we know is taken: check the widened draw too, and fail closed
    # (raise) rather than silently return a duplicate if even that collides.
    candidate = f"intake-{day}-{secrets.token_hex(_ID_SUFFIX_BYTES * 2)}"
    if shared_dir is not None and _id_taken(shared_dir, candidate):
        raise OSError(
            f"new_intake_id: could not mint an unused id after "
            f"{_ID_MINT_ATTEMPTS} attempts plus a widened fallback"
        )
    return candidate
