...
def pkg_display(pkg: dict) -> str:
    # identity: see resolve_app_id — display fallback ending at the catalog key; the alternatives are human strings.
...
def _run_teardown(
    ...
    # identity: NOT resolve_app_id — ``app_id`` here is the path segment of
    # ``DELETE /api/applications/<bot_id>/<app_id>``, which resolves the
    # manifest by FILENAME STEM (``id``). ``resolve_app_id`` returns the
    # ``pkg_id`` for a gallery install (AL-1.4 §6 delta 12), which is not a
    # manifest filename — substituting it would 404 every teardown.
...
def _dependency_gate_result(
    ...
    """
    result = AppResult(
        # identity: see resolve_app_id — this harness's own report column, keyed by gallery package (see model.AppResult).
...
def run_sweep(
    ...
    *,
    ...
    # brief §8 (D1). Every package-id read below is the GALLERY PACKAGE key
    # (required and format-checked by ``gallery.import_package``), used to join
    # against ``deps.required_dep_ids`` / ``installed_state`` / ``api.install``.
    # Gallery records carry their OWN ``app_id`` field holding the app script
    # name, which the resolver returns ahead of the legacy chain on main today
    # — re-keying the whole sweep from ``p-…`` to ``app_…``. PR #3681
    # supersedes this reason (it makes the resolver decline non-conforming
    # values); see the module note in ``deps.py``.
    ...
    def _dependency_blockers(pkg: dict, bot_id: str) -> list[tuple[str, bool]]:
        pkg_id = str(pkg.get("pkg_id") or "")  # identity: NOT resolve_app_id (§8 D1)
    ...
    def _pending_dependent_exists(d: DeferredTeardown) -> bool:
        ...
        pid = str(d.pkg.get("pkg_id"))  # identity: NOT resolve_app_id (§8 D1)
        return any(
