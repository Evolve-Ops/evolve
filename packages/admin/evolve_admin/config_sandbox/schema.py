...
_NETWORK: tuple[TunableKey, ...] = (
    ...
    TunableKey(
        ...
        stock_default="checkpoint",
        type_hint="enum",
        description=(
            "Action when the spend cap is hit. 'checkpoint' (default): stop "
            "background work and hold the next conversation turn behind a "
            "fixed 'cap reached' reply — the bot's owner answers continue "
            "(raising today's cap by a stated amount) or stop. "
            "'downgrade-tier': keep answering, but on the cheapest model for "
            "the rest of the day — replies get noticeably less careful, an "
            "app that declared a minimum model runs below it, and the bot "
            "keeps billing; NOT recommended. 'alert-only': tell the operator "
            "and change nothing."
        ),
    ),
    TunableKey(
    ...
    # "network.security.mode" ("Where review.py runs") and
    # "network.security.autoRejectRisk" retired 2026-08-14: review.py is gone
    # (#3641) and nothing reads either key. The wizards no longer write them
    # and the admin-UI Security card that surfaced them is removed.
    # ``network.security.botId`` stays out of this table for the same reason
    # it always was — it is set by the wizard, not tuned.
    TunableKey(
...
# Phase 2 of internal/spec-alert-notifier-2026-05-09.md. These entries surface
...
_ALERTS: tuple[TunableKey, ...] = (
    ...
    TunableKey(
        ...
        description=(
            ...
            "Phase 7 of internal/spec-alert-notifier-2026-05-09.md — flipped "
    ...
    # The "alerts.review.*" source-level toggles retired 2026-08-14 with
    # review.py (#3641). ``alerts.<source>.*`` keys are derived from a catalog
    # row's producer_source (see evo/tools/pod_state_subscriptions.py), and
    # no catalog row carries producer_source="review" any more, so nothing
    # can name these paths. They live only here — no defaults registry
    # mirrors them (unlike the plugin knobs of #3507), so removing them
    # cannot reset an operator edit; the dispatcher reads every
    # ``alerts.<source>.enabled`` defensively through _read_config_value and
    # falls back to _DEFAULT_SOURCE_ENABLED, which no longer lists "review"
    # either.
    ...
    TunableKey(
        ...
        stock_default=True,
        type_hint="bool",
        description="Push 'Apply Result' notifications (success / rollback / failure).",
    ),
    TunableKey(
    ...
    TunableKey(
        ...
        type_hint="int",
        description=(
            "Flap events (fires + standalone clears, i.e. pushes) within the "
            "window that trip demotion to the digest. Default 4 = two full "
            "fire+clear cycles — catches even a twice-a-day flap within "
            "hours while leaving a single fire-clear-fire blip (3 events) "
            "paging normally."
        ),
    ),
    TunableKey(
    ...
    TunableKey(
        ...
        description=(
            ...
            "demotion may self-lift and it pages normally again. The lift "
            "also requires the rolling window to have drained below the "
            "count threshold — a quiet afternoon does not amnesty a daily "
            "flapper. Default 6h (≈3× the motivating ~2h flap cadence)."
        ),
    ),
)
...
# internal/spec-openclaw-json-derived-artifact-2026-05-24.md §6.
#
...
_OPENCLAW: tuple[TunableKey, ...] = (
    ...
    TunableKey(
        path="openclaw.plugins.evolve.classifierModel",
        store=Store.OPENCLAW,
        target_path="openclaw.json::plugins.entries.evolve.config.classifierModel",
        strength=Strength.FREE,
        stock_default="anthropic/claude-haiku-4-5",
        type_hint="string",
        description=(
            "Tier-classifier model. Deploy derives the effective default from "
            "the bot's credentialed providers (plugin_defaults_for_bot); this "
            "TunableKey makes an operator's explicit pin survive redeploys "
            "instead of being reset to the derived default."
        ),
        per_bot=True,
    ),
    TunableKey(
        path="openclaw.plugins.evolve.layer2Enforce",
        store=Store.OPENCLAW,
        target_path="openclaw.json::plugins.entries.evolve.config.layer2Enforce",
        strength=Strength.FREE,
        stock_default=False,
        type_hint="bool",
        description=(
            "Arms the below-LLM Layer-2 per-role tool-call enforcement gate. "
            "False = observe-only (records would-blocks, allows all). Without "
            "this TunableKey the materializer reset a hand-armed True back to "
            "False on every deploy — silently disarming enforcement."
        ),
        per_bot=True,
    ),
    TunableKey(
        path="openclaw.plugins.evolve.prefixHashLedgerEnabled",
        store=Store.OPENCLAW,
        target_path="openclaw.json::plugins.entries.evolve.config.prefixHashLedgerEnabled",
        strength=Strength.FREE,
        stock_default=False,
        type_hint="bool",
        description=(
            "Context-observability Phase 0: per-turn prefix-hash records to "
            "turns/prefix-hashes-*.jsonl. Dark by default; armed per-bot to "
            "run the cache-invalidation confirming test."
        ),
        per_bot=True,
    ),
    TunableKey(
        path="openclaw.plugins.evolve.inTurnCheckpointUsd",
        store=Store.OPENCLAW,
        target_path="openclaw.json::plugins.entries.evolve.config.inTurnCheckpointUsd",
        strength=Strength.SHIPPED_POLICY,
        stock_default=5.0,
        type_hint="float",
        description=(
            "In-turn cost checkpoint (D-CS12): recorded spend in one run at "
            "which tool calls are vetoed and the owner is told. Floor $1.00."
        ),
        per_bot=True,
    ),
    TunableKey(
        path="openclaw.plugins.evolve.execFailureAbsorb",
        store=Store.OPENCLAW,
        target_path="openclaw.json::plugins.entries.evolve.config.execFailureAbsorb",
        strength=Strength.FREE,
        stock_default=False,
        type_hint="bool",
        description=(
            "Arms the exec-failure channel-hygiene absorber "
            "(design-exec-failure-hygiene-2026-08-31 A1). False = observe-only "
            "(raw '⚠️ 🛠️ Exec failed …' trailer matches are ledgered but "
            "delivered); True = matched trailer lines are stripped/cancelled "
            "before reaching a user channel. Only the exact boolean true arms."
        ),
        per_bot=True,
    ),
    TunableKey(
    ...
    # internal/spec-permission-posture-2026-05-10.md.
    #
