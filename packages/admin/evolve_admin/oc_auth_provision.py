...
The WRITE-side counterpart of :mod:`evolve_admin.oc_store` (the read-side
adapter). ``oc_store`` reads a bot's credentials regardless of which storage
...
# Per-agent layout, relative to the bot's home — matches oc_store._AGENT_RELDIR
# (the read-side adapter) so read and write target the same store.
