...
Spec trail: internal/decision-add-bot-m4-u1-proof-2026-06-11.md (finding 4 /
...
def _route_note(bot_block: dict[str, Any], bot_id: str, channels: set[str]) -> str:
    ...
    ext = _ext_ids.read_external_ids(bot_block.get("primary_user"))
    if any(ch in channels for ch in ext):
        return ""
    return (
...
def maybe_activate(
    ...
    * ``"no_channel"`` — called without a live messaging channel (the
    ...
    # ``pkg_id`` below is a GALLERY PACKAGE CONSTANT from ``_pack``, naming
    # which package the installer should fetch. Nothing is being read off a
    # manifest, so there is no identity to resolve: these are inputs to an
    # install, not the id of something already installed.
