...
def _gate_error_refusal(user: str, spec: str, exc: BaseException) -> str:
    ...
    try:
        ...
        return (
            ...
            f"(internal/design-plugin-install-provenance-gate-2026-08-11.md §4)."
        )
...
def install_externalized_plugin(
    ...
    **Provenance gate (Layer 1).** Before the command is built, the resolved
    ...
    Design: internal/design-plugin-install-provenance-gate-2026-08-11.md §4–§6.
    """
    ...
    # Consent flags probed from the installed runtime, not assumed. OC >= 2026.9
    # treats a missing --accept-capabilities as "no consent" and exits ZERO
    # having installed nothing, so without this an externalized-plugin repair
    # reports success and leaves the bot exactly as broken as it found it —
    # the 2026-09-07 failure. See oc_install_flags.
    #
    # ``--force`` stays the caller's decision — it means "overwrite an existing
    # install record", which is about intent, not about the runtime. Only the
    # consent flag is probed, so an older OC that never heard of it is not
    # handed it.
    from .oc_install_flags import probe_failed, required_install_flags

    cmd = ["sudo", "-u", user, "-H", "openclaw", "plugins", "install"]
    if force:
        cmd.append("--force")
    _flags = required_install_flags("openclaw")
    if "--accept-capabilities" in _flags:
        cmd.append("--accept-capabilities")
    elif probe_failed("openclaw"):
        # Loud, because this is the repair path: a `plugins install` that does
        # nothing here leaves the bot exactly as broken as it was found, and
        # the caller reports success. "Could not ask" must never look like
        # "needs nothing".
        _log.warning(
            "could not read `plugins install --help` — installing %s for %s "
            "without consent flags. If this runtime requires "
            "--accept-capabilities the install will not take; verify with "
            "`openclaw plugins list`.", spec, user,
        )
    cmd.append(spec)
