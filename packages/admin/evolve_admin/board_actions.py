...
def _google_granted(bot_id: str, network: dict[str, Any]) -> bool:
    return app_contract.google_configured(bot_id, network)
...
def _fast_rung_cost_per_token(
    ...
    Model selection and the pricing catalog are other platform services,
    reached through their contract rows (``model.tier_chain``,
    ``cost.model_price``); the bindings keep the lazy import, so a context
    that never asks for a cost estimate never loads the model-rungs code.
    """
    ...
    if not bare:
        return None
    rec = app_contract.model_price(Path(shared_dir), provider, bare)
    if not rec:
        return None
