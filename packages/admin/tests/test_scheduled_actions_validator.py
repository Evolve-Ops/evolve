...
def test_detect_ignores_negated_daemon_mention() -> None:
    # atlas-article-capture false positive: the ONLY daemon marker is a
    # negation ("No cron, no LaunchDaemon, no scheduled execution"). A
    # negated mention describes the *absence* of a scheduler, so it must
    # not trip the gate.
    spec = (
        "A reaction-driven capture app. "
        "No cron, no LaunchDaemon, no scheduled execution — entirely event-driven."
    )
    assert detect_unstructured_schedule(spec) == []


def test_detect_ignores_infra_mention_under_event_driven_disclaimer() -> None:
    # atlas-on-demand-research false positive: the bare "LaunchDaemon" word
    # is a passing reference to unrelated Evolve infra (the repo-puller),
    # NOT a scheduler this app installs — and the spec explicitly declares
    # itself event-driven. Suppressed.
    spec = (
        "No cron, no plist — entirely event-driven. "
        "The help directory is refreshed by the repo-puller LaunchDaemon every "
        "15 minutes via `git pull`; this app only reads it."
    )
    assert detect_unstructured_schedule(spec) == []


def test_detect_weak_word_still_trips_without_disclaimer() -> None:
    # The discriminator is the disclaimer, not the bare word: the SAME
    # infra-style phrasing WITHOUT a no-schedule declaration still trips,
    # so a genuine "the LaunchDaemon is the only scheduler" spec is caught
    # (the real Atlas Daily Digest bug this gate exists for).
    spec = (
        "The help directory is refreshed by the repo-puller LaunchDaemon every "
        "15 minutes."
    )
    assert "LaunchDaemon" in detect_unstructured_schedule(spec)


def test_detect_strong_marker_beats_disclaimer() -> None:
    # A spec cannot both emit a plist and be "entirely event-driven". If it
    # contradicts itself, the real emission wins so the gate still fires.
    spec = (
        "Entirely event-driven. "
        "## FILE: /Library/LaunchDaemons/com.bot.foo.plist\n<plist>...</plist>"
    )
    found = detect_unstructured_schedule(spec)
    assert any("/Library/LaunchDaemons/" in m for m in found)
...
# ── Atlas importable-manifest regression guard ───────────────────────────────
#
# docs/atlas-app-manifests/ holds importable gallery packages that live
# OUTSIDE gallery/ (imported via /api/gallery/import). The gallery conformance
# sweep (test_gallery_conformance.py) only walks gallery/index.json, so these
# would otherwise escape the scheduled_actions gate — which is exactly the
# false-positive class that had this validator excluded from that sweep. Now
# that the heuristic distinguishes a real daemon from a negated / event-driven
# mention, guard these four directly: article-capture and on-demand-research
# are event-driven (no action needed); daily-digest and weekly-recap declare
# a forge-installable launchd action.


def _atlas_manifest_dir() -> Path:
    here = Path(__file__).resolve()
    for parent in here.parents:
        candidate = parent / "docs" / "atlas-app-manifests"
        if candidate.is_dir():
            return candidate
    raise RuntimeError("Could not locate docs/atlas-app-manifests in repo tree")


def _atlas_manifests() -> list[Path]:
    return sorted(_atlas_manifest_dir().glob("*.json"))


@pytest.mark.parametrize(
    "manifest_path", _atlas_manifests(), ids=lambda p: p.stem
)
def test_atlas_manifests_pass_scheduled_actions_validator(manifest_path: Path) -> None:
    """Every importable Atlas manifest must validate clean.

    Regression guard for the two-class fix:
    - article-capture / on-demand-research were FALSE POSITIVES — event-driven
      apps whose build_spec mentions ``LaunchDaemon`` in a negation or a
      passing infra reference. The heuristic must not flag them.
    - weekly-recap was a REAL finding — it installs a weekly LaunchDaemon but
      declared no ``scheduled_actions[]``. It now carries a forge-installable
      ``launchd`` entry (mirroring atlas-daily-digest).
    """
    pkg = json.loads(manifest_path.read_text())
    result = validate_scheduled_actions(pkg)
    assert result["ok"], (
        f"{manifest_path.name} fails the scheduled_actions validator: "
        f"{result['message']}"
    )
