...
class TestAppDependenciesDictShape:
    ...
    def test_al_1_4b_identity_is_unchanged_for_every_shape_a_writer_emits(self):
        """AL-1.4b behavior-neutrality pin (internal/build-AL-1.4-app-id-canonical.md §3).

        ``_migrate_app_dependencies`` was swept onto
        ``app_identity.resolve_app_id`` — the WRITE side of the pair whose read
        side area 3 swept (``coherence_pass_a._declared_dependency_keys``). The
        sweep is only legitimate if the resolver returns exactly what the old
        ``entry.get("pkg_id") or entry.get("spec_id")`` chain returned for every
        shape a writer actually emits. Neither declared shape carries ``id`` or
        ``instance_id``, which is the only place the resolver's order could
        diverge from that chain.
        """
        import re

        spec_id_re = re.compile(r"^p-[a-f0-9]{8}$")  # mirrors migrate_v7._SPEC_ID_RE

        def pre_sweep(raw: list) -> list[dict]:
            """The chain exactly as it stood before AL-1.4b."""
            out: list[dict] = []
            for entry in raw:
                if isinstance(entry, str):
                    if spec_id_re.match(entry):
                        out.append({"spec_id": entry, "required": True})
                elif isinstance(entry, dict):
                    pkg_id = (entry.get("pkg_id") or entry.get("spec_id") or "").strip()
                    if not spec_id_re.match(pkg_id):
                        continue
                    out.append({
                        "spec_id": pkg_id,
        ...
        corpus = [
            ...
            {"pkg_id": "p-9bfa1c84", "display_name": "Task Manager",
             "required": True, "reason": "reads tasks.json"},
            {"spec_id": "p-12345678", "required": False, "purpose": "optional"},
            # Hybrid (a re-migrated manifest): migrate writes spec_id FROM
            # pkg_id, so the two are equal wherever both appear.
            {"pkg_id": "p-55555555", "spec_id": "p-55555555"},
            {"pkg_id": "p-12345678", "display_name": "Pizza Oven"},
            {"pkg_id": "  p-22222222  ", "required": True},
            {"display_name": "ghost", "required": True},
            {"pkg_id": "Task Manager"},
            {"pkg_id": ""},
            {},
            "not-a-dict-or-id",
        ]
        # Item by item, and as one list (order + count must also match).
        for entry in corpus:
            assert _migrate_app_dependencies([entry]) == pre_sweep([entry]), entry
        assert _migrate_app_dependencies(list(corpus)) == pre_sweep(list(corpus))

    def test_al_1_4b_a_canonical_app_id_on_a_dependency_entry_now_wins(self):
        """The one DELIBERATE delta, pinned so a reviewer sees it.

        No writer emits ``app_id`` on a dependency sub-record today (AL-1.4a
        stamps manifests, not dependency entries), so this is dormant until
        1.4c — and it is the same delta area 3 pinned on the read side, so the
        two stay symmetric when writers do start emitting it.
        """
        out = _migrate_app_dependencies(
            [{"app_id": "p-33333333", "pkg_id": "p-44444444"}]
        )
        assert out[0]["spec_id"] == "p-33333333"
        # A NON-conforming app_id falls through to the legacy chain (#3681 —
        # gallery/index.json's app_id holds the app SCRIPT name, not a slug).
        out = _migrate_app_dependencies(
            [{"app_id": "app_task_manager", "pkg_id": "p-44444444"}]
        )
        assert out[0]["spec_id"] == "p-44444444"
