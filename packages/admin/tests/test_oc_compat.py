...
def test_apply_proceeds_past_the_veto_once_an_override_is_recorded(client, monkeypatch):
    ...
    monkeypatch.setattr(rm, "_report_stale_reason", lambda _r: None)
    monkeypatch.setattr(su, "inflight_report_id", lambda: None)
    # The per-bot preflight gate (hold-fix-4277 item 2, freshness added by
    # hold-fix-4392) sits between the compat veto and the deploy lock; seed
    # a clean, FRESH cached report matching the fixture network's bot set
    # (`{"team-bot-a": {}}`, see the `client` fixture above) for this exact
    # pair so the test still measures the veto, not the new gate. Written in
    # oc_preflight_store's on-disk shape, as test_oc_upgrade_apply_endpoint
    # does, to stay independent of its API.
    from evolve_admin.oc_preflight import bot_set_hash as _bsh

    pf_root = shared / "oc_preflight" / "reports"
    pf_root.mkdir(parents=True, exist_ok=True)
    (pf_root / f"pair-{TESTED}__{UNTESTED}.json").write_text(json.dumps({
        "report_id": "pf-fixture", "target_version": UNTESTED,
        "installed_version": TESTED, "blocking": False,
        "summary": "clean", "rows": [],
        "checked_at": datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "bot_set_hash": _bsh({"bots": {"team-bot-a": {}}}),
    }))
