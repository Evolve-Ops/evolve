...
``tighten_shared_pii_trees`` (the post-``a+rX`` re-tighten) +
...
# ── tighten_shared_pii_trees — both trees in one deploy-time call ──────────────

class TestTightenSharedPiiTrees:
    ...
    def test_tightens_both_secrets_and_directory(self, tmp_path):
        ...
        assert scp.tighten_shared_pii_trees(shared) is True
