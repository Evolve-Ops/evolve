...
def test_stored_ambiguous_tier_passes_through_untouched(js: str):
    ...
    # value — pin the passthrough (role-shaped since #3662, translating a
    # still-tier-shaped stored value on the way out).
    assert "ambiguousRole: _priorRouting.ambiguousRole ?? (_AI_ROUTE_TIER_TO_ROLE[_priorRouting.ambiguousTier] ?? null)" in region
