...
def test_verify_evolve_access_reports_unreadable_secret_on_linux(tmp_path, monkeypatch):
    ...
    # .openclaw traverse + each secret + the queue write + workspace/ traverse.
    assert len(failures) == len(secret_config_perms.BOT_SECRET_CONFIG_RELPATHS) + 3
...
def test_heal_evolve_access_reasserts_mask_and_returns_true(tmp_path, monkeypatch):
    ...
    def _regrant(bot_id):
        # the recursive grant recomputes every clamped mask — .openclaw AND the
        # workspace/ root (facet 3), exactly what set_evolve_read_acl does live.
        fake.seed_acl(oc, "evolve", "search")
