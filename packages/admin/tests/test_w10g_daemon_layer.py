...
internal/diag-w10g-daemon-layer-2026-06-19.md). Host-mutation-free locks pinning
...
def _oc_dest(tmp_path, name):
    """A dest under a real ``.openclaw`` — the shape production always has.

    ``chmod_secret_config``'s D-2 gate anchors its ancestor walk on the bot home
    it derives from the path's ``.openclaw`` component (#3566 audit D-2
    residual), so a bare ``tmp_path/<name>`` is refused as off-shape. Building
    the real tree keeps these two tests about the ACL re-grant leg they exist
    for, rather than about the gate.
    """
...
def test_chmod_secret_config_regrants_read_ace_on_linux(monkeypatch, tmp_path):
    ...
    fake = FakePerms()
    set_perms(fake)
    f = _oc_dest(tmp_path, "auth-profiles.json")
...
def test_chmod_secret_config_no_acl_change_on_macos(monkeypatch, tmp_path):
    ...
    fake = FakePerms()
    set_perms(fake)
    f = _oc_dest(tmp_path, "openclaw.json")
    monkeypatch.setattr(secret_config_perms.subprocess, "run", lambda *a, **k: _Result())
...
def test_add_acl_sets_default_acl_on_linux(tmp_path, monkeypatch):
    ...
    def _record(self, args, *, path, timeout=10):
        # ``path`` is the redirect gate's view of the target and ``args[-1]`` is
        # what the root command will actually act on. A stub that ignored the
        # kwarg would let those two drift apart silently — which is precisely the
        # bypass the explicit-kwarg signature exists to make impossible.
        assert str(path) == args[-1], (path, args)
        calls.append(args)
        return True

    monkeypatch.setattr(LinuxPerms, "_setfacl", _record)
...
def test_cli_install_infra_jobs_evolve_dir_not_hardcoded():
    """`evolve-admin install-infra-jobs` must derive the evolve home from the
    platform profile, not from a /Users literal.

    It is the documented manual repair for a pod whose infra jobs are missing.
    Hardcoded, it wrote macOS log paths on a Linux pod. The repo_puller ratchet
    above pinned the same literal out of one caller; this pins the other, so
    the fix cannot come back through the CLI.

    Scoped to this command's own source on purpose: `setup evolve-user` in the
    same module is macOS-only by design (it exits on Linux telling the operator
    to run `setup --fresh`), so its /Users/evolve literals are correct there.
    """
    import inspect

    from evolve_admin.cli import install_infra_jobs

    src = inspect.getsource(install_infra_jobs.callback)
    assert "/Users/evolve" not in src
    assert 'get_profile().user_home_root) / "evolve"' in src  # the blessed root
