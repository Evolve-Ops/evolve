...
functions for the VALIDATE surfaces (still direct reads). The HANDLER
surfaces route every write through the admin daemon since 7.1 C2
(fail closed, no direct-write fallback), so handler tests mock
``admin_client.require_daemon_call`` and pin the endpoint + body +
response-mapping contract; the store side effects themselves are the
daemon routes' tests' job. The store-unchanged proof lives in
test_evo_actions_fail_closed.py.
"""
...
def _mock_daemon(monkeypatch, responder):
    """Stub the fail-closed daemon transport. ``responder(method, path,
    body)`` returns the (reachable, status, body) triple; every call is
    recorded so tests can pin the endpoint + payload contract."""
...
def _signal_view_response(sig_id: str, state: str, **extra):
    """A minimal /api/signals/<id>/* 200 response body."""
    view = {"id": sig_id, "title": "t", "state": state}
    view.update(extra)
    return True, 200, {"ok": True, "new_state": state, "signal": view}
...
def test_signal_snooze_handler_calls_daemon(tmp_path, monkeypatch):
    """Snooze posts /api/signals/<id>/snooze with the computed until +
    evo attribution; the store write is the daemon's (7.1 C2)."""
    calls = _mock_daemon(
        monkeypatch,
        lambda m, p, b: _signal_view_response("sig-1", "snoozed"),
    )
    result = action_signal._snooze_handler(
        shared_dir=tmp_path, signal_id="sig-1",
    ...
    assert result["verify_via"]["args"] == {"signal_id": "sig-1", "state": "snoozed"}
    assert "state=snoozed" in result["verify_via"]["expect"]

    # Daemon-call contract: right endpoint, until matches the returned
    # snoozed_until, actor + reason attributed to evo.
    assert len(calls) == 1
    method, path, body = calls[0]
    assert (method, path) == ("POST", "/api/signals/sig-1/snooze")
    assert body["until"] == result["snoozed_until"]
    assert body["actor"] == "evo"
    assert "wait" in body["reason"]


def test_signal_snooze_handler_default_duration_24h(tmp_path, monkeypatch):
...
def test_signal_snooze_handler_unknown_signal(tmp_path, monkeypatch):
    """Daemon 404 → handler returns the not-found error, doesn't crash.
    (Validate would have caught it earlier; this is the
    defense-in-depth path.)"""
    _mock_daemon(
        monkeypatch,
        lambda m, p, b: (True, 404, {"error": "Signal not found: ghost-id"}),
    )
    result = action_signal._snooze_handler(
...
def test_signal_dismiss_handler_terminal_state(tmp_path, monkeypatch):
    """Dismiss posts /api/signals/<id>/dismiss with evo attribution;
    the archive move is the daemon's write (7.1 C2)."""
    calls = _mock_daemon(
        monkeypatch,
        lambda m, p, b: _signal_view_response("sig-d", "dismissed"),
    )
    result = action_signal._dismiss_handler(
        shared_dir=tmp_path, signal_id="sig-d", reason="bad finding",
    )
    assert result["ok"] is True
    ...
    assert "verify_via" in result
    assert result["verify_via"]["tool"] == "pod_state.signals.history"
    assert result["verify_via"]["args"] == {"signal_id": "sig-d", "state": "dismissed"}

    method, path, body = calls[0]
    assert (method, path) == ("POST", "/api/signals/sig-d/dismiss")
    assert body["actor"] == "evo"
    assert body["reason"] == "bad finding"


def test_signal_dismiss_handler_with_verdict_sends_feedback_fields(
    tmp_path, monkeypatch,
):
    """When verdict is provided, it rides the daemon body (the daemon's
    dismiss endpoint owns the feedback.jsonl write — producer-tuning
    loop) and the reported count mirrors the motivated proposals."""
    calls = _mock_daemon(
        monkeypatch,
        lambda m, p, b: _signal_view_response(
            "sig-fb", "dismissed", motivated_proposals=["p1", "p2"],
        ),
    )
    result = action_signal._dismiss_handler(
        shared_dir=tmp_path, signal_id="sig-fb",
    ...
    assert body["note"] == "not actually broken"


def test_signal_dismiss_handler_no_verdict_no_feedback(tmp_path, monkeypatch):
    """Without a verdict, no verdict field rides the daemon body and
    the tool result carries no feedback count."""
    calls = _mock_daemon(
        monkeypatch,
        lambda m, p, b: _signal_view_response("sig-nofb", "dismissed"),
    )
    result = action_signal._dismiss_handler(
        shared_dir=tmp_path, signal_id="sig-nofb", reason="cleanup",
    )
    assert result["ok"] is True
...
def test_proposal_snooze_handler_transitions_to_snoozed(tmp_path, monkeypatch):
    """Snooze posts /api/arbiter/proposals/<id>/snooze with the
    computed until + evo attribution; the subdir move is the daemon's
    write (7.1 C2). The local read only enriches from_status/summary."""
    calls = _mock_daemon(
        monkeypatch,
        lambda m, p, b: (True, 200, {"ok": True, "new_status": "snoozed"}),
    )
    _seed_proposal(
    ...
    assert result["summary"] == "real snooze test"
    # verify_via hint (spec §3.7 reliability lever #4).
    assert "verify_via" in result
    assert result["verify_via"]["tool"] == "pod_state.proposals.snoozed"
    assert result["verify_via"]["args"] == {"proposal_id": "p-snooze-real"}

    # Daemon-call contract.
    method, path, body = calls[0]
    assert (method, path) == (
        "POST", "/api/arbiter/proposals/p-snooze-real/snooze",
    )
    assert body["until"] == result["snoozed_until"]
    assert body["actor"] == "evo"
    assert body["reason"] == "evo wants to wait"


def test_proposal_snooze_handler_default_duration_1w(tmp_path, monkeypatch):
...
def test_apply_handler_unknown_proposal(tmp_path, monkeypatch):
    """Daemon 404 → handler returns structured not-found error.
...
def test_apply_handler_routes_act_through_daemon(tmp_path, monkeypatch):
    """The linchpin contract post-7.1 C2: apply is a single daemon
    round-trip to /act with evo attribution — the approve → apply-chain
    → move sequence runs daemon-side (its own tests live with the
    routes), and the tool normalizes the response."""
    calls = _mock_daemon(
        monkeypatch,
        lambda m, p, b: (True, 200, {
            "ok": True, "new_status": "applied", "message": "investigated",
        }),
    )
    p_id = "p-apply-full"
    result = action_proposal_apply._apply_handler(
    ...
    method, path, body = calls[0]
    assert (method, path) == ("POST", f"/api/arbiter/proposals/{p_id}/act")
    assert body["actor"] == "evo"
    assert body["reason"] == "evo applying this from the resolver test"


def test_apply_handler_non_pending_maps_daemon_409(tmp_path, monkeypatch):
    """A proposal the daemon refuses to act on (snoozed, already
    approved, terminal — anything non-pending) → structured error with
    the daemon's detail attached, never a raise."""
    _mock_daemon(
        monkeypatch,
        lambda m, p, b: (True, 409, {
            "error": "cannot act on proposal in status 'snoozed'; expected 'pending'",
        }),
    )
    result = action_proposal_apply._apply_handler(
    ...
    assert "snoozed" in result["daemon_body"]["error"]
...
def test_apply_force_ask_kinds_set_is_documented(tmp_path):
    ...
    # AgentsAppend writes the same AGENTS.md SoulEdit(target="agents")
    # does, and eligibility classes it human-only ("free-text
    # instruction; needs read") — the confirm gate follows that.
    assert "AgentsAppend" in action_proposal_apply._FORCE_ASK_ACTION_KINDS
    # InstallApp puts new code on the bot; eligibility classes it human-only
    # for the same reason.
    assert "InstallApp" in action_proposal_apply._FORCE_ASK_ACTION_KINDS
...
def test_reject_handler_terminal_transition(tmp_path, monkeypatch):
    """Reject routes through the daemon's /dismiss endpoint (the
    surviving queue-clearing terminal action — /reject was retired) with
    evo attribution; the archive move + rejection-log + suppression side
    effects run daemon-side (7.1 C2)."""
    calls = _mock_daemon(
        monkeypatch,
        lambda m, p, b: (True, 200, {"ok": True, "new_status": "dismissed"}),
    )
    p_id = "p-reject-real"
    _seed_proposal(
    ...
    assert result["ok"] is True, result
    assert result["from_status"] == "pending"
    assert result["to_status"] == "dismissed"
    assert result["summary"] == "will be rejected"
    assert "verify_via" in result

    method, path, body = calls[0]
    assert (method, path) == (
        "POST", f"/api/arbiter/proposals/{p_id}/dismiss",
    )
    assert body["actor"] == "evo"
    assert body["reason"] == "evo's test"
...
def test_signal_resolve_handler_transitions_to_resolved(tmp_path, monkeypatch):
    """Resolve posts /api/signals/<id>/resolve with evo attribution;
    the archive move is the daemon's write (7.1 C2)."""
    calls = _mock_daemon(
        monkeypatch,
        lambda m, p, b: _signal_view_response("sig-r", "resolved"),
    )
    result = action_signal._resolve_handler(
        shared_dir=tmp_path, signal_id="sig-r",
    ...
    method, path, body = calls[0]
    assert (method, path) == ("POST", "/api/signals/sig-r/resolve")
    assert body["actor"] == "evo"
    assert body["reason"] == "fixed via manual restart"


def test_signal_resolve_handler_unknown_signal(tmp_path, monkeypatch):
    _mock_daemon(
        monkeypatch,
        lambda m, p, b: (True, 404, {"error": "Signal not found"}),
    )
    result = action_signal._resolve_handler(
...
def test_proposal_mark_complete_handler_promotes_to_succeeded(tmp_path, monkeypatch):
    """Handler keeps the local manual-completion-kind guard (a policy
    check the daemon's /complete endpoint doesn't make), then routes
    the applied → succeeded write through the daemon with evo
    attribution (7.1 C2)."""
    calls = _mock_daemon(
        monkeypatch,
        lambda m, p, b: (True, 200, {"ok": True, "new_status": "succeeded"}),
    )
    _seed_proposal(
    ...
    method, path, body = calls[0]
    assert (method, path) == (
        "POST", "/api/arbiter/proposals/p-applied-mc/complete",
    )
    assert body["actor"] == "evo"
    assert body["reason"] == "operator confirmed work done"
