...
def test_reclaim_chowns_root_owned_baseline_dirs(monkeypatch, tmp_path):
    ...
    # Pretend everything is owned by root (uid=0) so the helper proceeds.
    class _Stat:
        st_uid = 0
    monkeypatch.setattr(_deploy.Path, "stat", lambda self: _Stat())
...
def test_reclaim_skips_when_already_bot_owned(monkeypatch, tmp_path):
    ...
    class _Stat:
        st_uid = 502  # any non-zero — pretend bot user owns it
    monkeypatch.setattr(_deploy.Path, "stat", lambda self: _Stat())
