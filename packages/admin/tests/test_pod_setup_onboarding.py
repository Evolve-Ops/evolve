...
def test_pod_tiers_configured_task_replaces_them():
    ...
    (primary bot's evolve-tiers.json with tier2 + tier0 model bindings).
...
def test_pod_tiers_configured_true_when_primary_has_tier2_and_tier0(
        tmp_path, monkeypatch):
    """Happy path: primary's evolve-tiers.json has both tier2 (workhorse)
    and tier0 (judge) with at least one model each."""
...
def test_pod_tiers_configured_true_via_defaults_when_only_tier2_set(
        tmp_path, monkeypatch):
    """Workhorse picked, judge OMITTED → configured-via-defaults.

    Post-Addendum-2 (#2561): the DEFAULT_MODEL_CATALOG ships in code as the
    base layer of the keyed merge, so an absent judge (tier0) resolves to the
    blessed sonnet-class rung (provider-diverse from standard) at runtime. A
    fresh pod works out of the box — onboarding must NOT nag the operator to
    pick a tier the defaults already satisfy. Only an EXPLICIT-but-empty tier
    flags (see the ai_optim_tiers broken-config tests); an omitted one does not.
    """
    import json as _json
    monkeypatch.setattr(
        "evolve_admin.config.bot_home",
