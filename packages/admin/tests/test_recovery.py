...
def test_pause_all_sets_flag_before_dispatch(tmp_path: Path, monkeypatch):
    ...
    # Every bootout saw the flag in place. The list is filled from worker
    # threads, so assert on contents/length, never on order.
    assert len(captured["flag_existed"]) == 3
    assert all(captured["flag_existed"])
...
def test_pause_all_dry_run_touches_nothing(tmp_path: Path, monkeypatch):
    ...
    # `calls` is populated from ThreadPoolExecutor workers, so its ORDER is
    # scheduling-dependent — only the set of bots dispatched is meaningful.
    assert sorted(calls) == ["admin_bot", "team_bot_a", "team_bot_b"]
    # The input ordering contract lives on the return value, which
    # _dispatch_per_bot re-orders to match the input bot list.
    assert [b.bot_id for b in result.per_bot] == [
        "team_bot_a", "admin_bot", "team_bot_b",
    ]
