...
def test_every_function_this_chip_adds_has_a_non_test_caller():
    ...
    **The limit, stated rather than glossed.** Definitions are matched
    ...
    ENTRY_POINTS = {
        ...
        # Click command functions: the `@menu_group.command(...)` decorator
        # registers each and click dispatches by CLI name, so the only
        # reference is the decorator — the same shape as the Flask view above.
        # All five are driven through click by
        # packages/admin/tests/test_oc_runtime.py, which invokes them via
        # CliRunner on the group rather than calling the functions directly.
        "oc_install": "click command `evolve-admin oc install`; dispatched by name",
        "oc_list": "click command `evolve-admin oc list`; dispatched by name",
        "oc_remove": "click command `evolve-admin oc remove`; dispatched by name",
        "oc_pin": "click command `evolve-admin oc pin`; dispatched by name",
        "oc_adopt": "click command `evolve-admin oc adopt`; dispatched by name",
        # Click command function: the `@menu_group.command("preflight")`
        # decorator registers it and click dispatches by CLI name, so the only
        # reference is the decorator. Same shape as the Flask view above.
        # Driven by packages/admin/tests/test_oc_preflight.py, which exercises
        # every function the command composes.
        "oc_preflight_cmd": "click command `evolve-admin oc preflight`; "
                            "registered by decorator, dispatched by name",
        ...
        # The backup-repo visibility verdict. Flask view registered by its
        # `@app.post` decorator inside register_backup_bot_routes, so the only
        # in-repo reference is the decorator. DRIVEN FOR REAL, not merely
        # exempted, by packages/admin/tests/test_backup_bot_routes.py (every
        # branch, through the test client) and end-to-end against the real
        # client transport by test_backup_verdict_roundtrip.py.
        "api_backup_bot_visibility": "Flask route POST /api/backup-bot/"
                                     "visibility; dispatched by URL",
        ...
        # The click-command shape again: `@models.command("migrate-tier-
        # overrides")` inside tier_override_migration.register_cli, so the
        # only reference to the name is the decorator's own ast.Call on
        # `models.command`, never a call to the function. DRIVEN FOR REAL —
        # not merely exempted — by packages/admin/tests/
        # test_tier_override_migration.py, which invokes it through the real
        # `main` click group with a CliRunner for every disposition it can
        # report (move, conflict, blocked, nothing, idempotent re-run), which
        # is also the only thing proving cli.py's one-line registration
        # actually attached it to the `models` group.
        "migrate_tier_overrides": "click command registered by "
                                  "@models.command in "
                                  "tier_override_migration.register_cli; "
                                  "invoked on the command line, never by an "
                                  "in-repo call",
        ...
        # Cost-cap checkpoint (D-CC1..4). The Flask-view shape once more:
        # registered by its `@app.post` decorator inside
        # ``register_cost_checkpoint_bot_routes`` and dispatched by URL, so no
        # in-repo ast.Call ever names it. DRIVEN FOR REAL rather than merely
        # exempted, by packages/admin/tests/test_cost_checkpoint_bot_routes.py
        # through a Flask test client with simulated unix-socket peer
        # credentials — the peer-uid binding (TCP and unknown-uid refusals),
        # the owner-only continue for both owner roles, the participant and
        # blocked-user refusals, the "role never comes from the request"
        # case, stop, and every precondition/bad-input branch.
        "api_cost_checkpoint_answer": "Flask route POST /api/cost-checkpoint/"
                                      "answer; dispatched by URL",
        ...
        # board-bot-lane-worker (D-BI4 §5). Flask view registered by its
        # `@app.post` decorator inside register_board_routes, dispatched by
        # URL — the same shape as api_app_snapshot above. DRIVEN FOR REAL by
        # packages/admin/tests/test_board_routes.py's approval tests (grant,
        # decline, 409 with no pending request, 404 on an unknown card, 401
        # unauthenticated), through the real Flask test client.
        "api_board_approval": "Flask route POST /api/board/<bot_id>/cards/"
                              "<card_id>/approval; dispatched by URL",
        # The ast.Name-load shape once more, three times over: a handler
        # registered as a dict VALUE (_TOOL_HANDLERS = {"find_directions":
        # _handle_find_directions}, resolved by resolve_step_handler through
        # the dict, never called by name), and two callables handed to a
        # stdlib API as a VALUE rather than named in a call
        # (model_client=_refusing_model_client passed to run_loop; _term
        # passed to signal.signal). All three are DRIVEN FOR REAL by
        # packages/admin/tests/test_board_worker.py (find_directions' whole
        # accept/run/done path) and
        # packages/admin/tests/test_board_worker_runner.py (the refusing
        # client raises when actually called; the installed SIGTERM handler
        # actually exits — simulated by invoking whatever
        # signal.getsignal(SIGTERM) returns after main() installs it).
        "_handle_find_directions": "resolve_step_handler's _TOOL_HANDLERS "
                                   "dict entry; called through the dict",
        "_refusing_model_client": "board_worker_runner.main's default "
                                  "model_client=...; called through the "
                                  "parameter, never by name",
        "_term": "board_worker_runner.main's signal.signal(SIGTERM, _term); "
                "called by the OS through the registered handler",

        # Morning Board briefing panel (D-BI6d). The Flask-view shape once
        # more, twice: registered by their `@app.get`/`@app.post` decorators
        # inside ``register_board_routes`` and dispatched by URL, so no
        # in-repo ast.Call ever names them. DRIVEN FOR REAL rather than
        # merely exempted, by packages/admin/tests/test_board_routes.py
        # through a Flask test client — the read before any briefing exists,
        # the round trip against what the bot posted, dismiss-then-reread,
        # a stale-day dismiss as a quiet no-op, and both routes' 401s.
        "api_board_briefing": "Flask route GET /api/board/<bot_id>/"
                              "briefing; dispatched by URL",
        "api_board_briefing_dismiss": "Flask route POST /api/board/<bot_id>/"
                                      "briefing/dismiss; dispatched by URL",
        # The bot's half of the same feature: registered by its `@app.post`
        # decorator inside ``register_board_bot_routes`` and dispatched by
        # URL. DRIVEN FOR REAL by packages/admin/tests/test_board_bot_routes.py
        # through a Flask test client with simulated unix-socket peer
        # credentials — the peer-uid refusal, the text round-trip into the
        # store, and that a bot cannot write another bot's panel.
        "board_bot_briefing": "Flask route POST /api/board-bot/briefing; "
                              "dispatched by URL",
        # PR #4079's hold-fix. The click shape this list already covers, once
        # more: attached by cli.py's ``main.add_command(_pm_probe_cmd)`` — an
        # ast.Name load through an import alias, never an ast.Call. DRIVEN FOR
        # REAL rather than merely exempted, in
        # packages/admin/tests/test_pm_probe_sudoers.py: the whole --dry-run
        # body through a real CliRunner on the real ``main`` group (which is
        # also the only thing that proves the registration attached it), and
        # the off-root refusal, which is what keeps a half-install — a sudoers
        # grant naming a reader that was never written — from happening.
        "install_pm_probe_sudoers_cmd": "click command attached via cli.py's "
                                        "main.add_command; invoked as "
                                        "`sudo evolve-admin "
                                        "install-pm-probe-sudoers`",
        # Cache-thrash read for the Cost page's cache-tier row. Flask-view
        # shape: registered by `@app.get` inside
        # ``register_cost_measures_routes``, dispatched by URL. DRIVEN FOR
        # REAL by packages/admin/tests/test_cost_measures_cache_tier.py through
        # a Flask test client — the by-tokens figure and unreadable-is-null.
        "api_bot_cache_thrash": "Flask route GET /api/bot/cache-thrash; "
                                "dispatched by URL",
    }
