...
from evolve_admin import breakers_enforce, cli  # noqa: E402
...
def shared_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    """
    monkeypatch.setattr(cli, "_breakers_shared_dir", lambda ctx: tmp_path)
    # Stub network loading — return a minimal valid network dict.
    monkeypatch.setattr(cli, "load_network", lambda *_args, **_kw: {
