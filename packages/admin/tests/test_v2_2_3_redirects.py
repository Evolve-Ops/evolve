...
structure:
  ...
  1. page-apps has 4 subtabs: installed, gallery, create, forge-jobs
...
renamed to "watchlist", and "schedule" was absorbed into per-subscription
config. Tests covering those specific subtabs are now @pytest.mark.skip
...
def test_apps_page_has_installed_subtab():
    html = _load_html()
    # The subtab button must have data-subtab="installed" inside page-apps
    ...
    assert 'data-subtab="installed"' in block, (
        'Apps page missing "installed" subtab button'
    )
    assert 'id="apps-installed"' in block, (
        'Apps page missing subtab-page div id="apps-installed"'
    )
...
def test_apps_page_has_forge_jobs_subtab():
    html = _load_html()
    apps_match = re.search(r'id="page-apps"(.+?)(?=<div class="page"|$)', html, re.DOTALL)
    assert apps_match
    block = apps_match.group(1)
    assert 'data-subtab="forge-jobs"' in block, 'Apps page missing "forge-jobs" subtab'
    assert 'id="apps-forge-jobs"' in block, 'Apps page missing id="apps-forge-jobs"'
...
UNIQUE_IDS = [
    ...
    "apps-installed", "apps-gallery", "apps-forge-jobs",
