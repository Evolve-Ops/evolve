...
# ── two-writer mode contract (PR #3562 follow-up) ─────────────────────────
#
# The bot's gateway plugin is the OTHER writer/reader of this file
# (ModelRouter reads it on every routing decision; setStandingUserTierDefault
# read-merges then temp+renames). mkstemp's 0600 default would leave an
# evolve-owned file invisible to the bot user — the per-user pref silently
# never applies and the plugin's read-merge refuses to write. Both writers
# therefore pin 0644 before the rename.
...
def test_set_user_pref_pins_world_readable_mode(tmp_path):
    """Fresh write lands 0644 even under a restrictive umask (the daemon
    runs at umask 077 — mkstemp would otherwise mint 0600)."""
    import os
    ...
    try:
        ...
        os.umask(old_umask)
    assert _file_mode(tmp_path / "admin_bot" / "user-tier-prefs.json") == 0o644


def test_set_user_pref_auto_delete_repins_mode(tmp_path):
    """The auto (entry-delete) rewrite path pins 0644 too — every rename
    replaces the inode, so every write leg must re-pin."""
    from evolve_admin.evo.user_tier_prefs import set_user_pref
...
def test_set_user_pref_rewrites_over_plugin_written_file(tmp_path):
    """Writer matrix, evolve-over-bot leg: the admin-side writer replaces a
    file the plugin wrote (0644, its pinned mode), preserves the other
    user's entry, and re-pins 0644. (True cross-OWNER replacement needs two
    unix users; at this level we prove the merge + mode contract over a
    plugin-shaped file — the deploy-side ACL tests cover the dir
    permission half.)"""
    from evolve_admin.evo.user_tier_prefs import get_user_pref, set_user_pref

    bot_dir = tmp_path / "admin_bot"
    bot_dir.mkdir()
    prefs = bot_dir / "user-tier-prefs.json"
    # Byte-shape the plugin's writer produces (sorted keys, indent 2).
    prefs.write_text(json.dumps(
        {"users": {"ext:telegram:bob": {
            "defaultRole": "fast", "updated_at": "2026-08-03T00:00:00+00:00"}}},
        indent=2, sort_keys=True))
    prefs.chmod(0o644)

    set_user_pref(tmp_path, "admin_bot", "ext:telegram:alice", "max")
    assert _file_mode(prefs) == 0o644
    assert get_user_pref(tmp_path, "admin_bot", "ext:telegram:bob") == "fast"
    assert get_user_pref(tmp_path, "admin_bot", "ext:telegram:alice") == "max"


def test_set_user_pref_heals_a_stale_0600_file(tmp_path):
    """Writer matrix, self-heal leg: a file minted 0600 by a pre-fix write
    becomes 0644 on the next admin-side write (rename replaces the inode
    with the newly-pinned temp file)."""
    from evolve_admin.evo.user_tier_prefs import set_user_pref

    bot_dir = tmp_path / "admin_bot"
    bot_dir.mkdir()
    prefs = bot_dir / "user-tier-prefs.json"
    prefs.write_text(json.dumps({"users": {}}))
    prefs.chmod(0o600)

    set_user_pref(tmp_path, "admin_bot", "ext:telegram:alice", "standard")
    assert _file_mode(prefs) == 0o644
