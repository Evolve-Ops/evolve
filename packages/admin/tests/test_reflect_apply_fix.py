...
# Regression (2026-08-28): a bot's logical id (network.json key) may differ from
# its macOS/Linux ACCOUNT name (``bots.<id>.user``). This endpoint keyed BOTH of
# its filesystem-facing steps on the id:
#
#   • ``resolve_bot_paths(bot_id)`` with no ``user=`` resolved the workspace to
#     ``<home_root>/<bot_id>/…``, which does not exist on such a pod — so the
#     traversal guard 400'd on a perfectly legitimate in-workspace file; and
#   • the PermissionError fallback passed ``bot_id`` to
#     ``embed_marker_privileged``, whose root-side helper derives the bot from
#     the destination's own HOME-DIRECTORY component and refuses a mismatch —
#     so every privileged marker write was refused.
#
# Both are pinned here against a pod shaped like the real one
# (``team_bot_b`` runs as ``personal_bot_user``).


@pytest.fixture
def alias_env(tmp_path, monkeypatch):
    """A pod whose bot_id differs from its account name.

    ``resolve_bot_paths`` is patched with a stub that models the real one's
    ``actual_user = user or bot_id`` resolution, so a call site that forgets
    ``user=`` resolves a workspace that does not exist — exactly the live
    failure, rather than a stub that silently papers over it.
    """
    from evolve_admin.web import server as srv
    from evolve_admin.web.server import create_app
    ...
    shared_dir = tmp_path / "evolve"
    shared_dir.mkdir()
    homes = tmp_path / "bot-homes"
    workspace = homes / "personal_bot_user" / ".openclaw" / "workspace" / "scripts"
    workspace.mkdir(parents=True)

    network = {
        "networkId": "pod-test-alias",
        "sharedDir": str(shared_dir),
    ...
    def _fake_resolve(bid, user=None):
        seen_users.append(user)
        actual = user or bid  # the real resolve_bot_paths' fallback
        return {
            "workspace": str(homes / actual / ".openclaw" / "workspace"),
            "user": actual,
        }

    monkeypatch.setattr(srv, "resolve_bot_paths", _fake_resolve)

    app = create_app(network_path)
    app.config["TESTING"] = True
    return app.test_client(), {"workspace": workspace, "seen_users": seen_users}


class TestBotIdDiffersFromAccount:
    def _post(self, client, file_path: Path):
        return client.post("/api/bots/team_bot_b/reflect/apply-fix", json={
            "kind": "stamp_marker",
            "file_path": str(file_path),
            "spec_id": "p-aaaa1111",
            "file_id": "f-abc12345@2026.05.20-1.0",
    ...
    def test_workspace_resolution_uses_the_account(self, alias_env):
        """The traversal guard must resolve the workspace under the ACCOUNT's
        home. Keyed on bot_id it resolves ``<root>/team_bot_b/…`` — a directory
        that does not exist — and the legitimate file 400s as 'outside'."""
        client, env = alias_env
        f = env["workspace"] / "fresh.py"
        f.write_text("print('hello')\n")

        r = self._post(client, f)
        assert r.status_code == 200, r.get_json()
        assert env["seen_users"] == ["personal_bot_user"]
        assert "spec=p-aaaa1111" in f.read_text()

    def test_privileged_embed_is_passed_the_account(self, alias_env, monkeypatch):
        """The PermissionError fallback binds the privileged write to the
        ACCOUNT. The root-side helper compares its argv against the
        destination's home-directory component, so a bot_id here refuses every
        write on this pod shape."""
        import evolve_util
        from evolve_admin.applications import marker_embed_helper as meh

        client, env = alias_env
        f = env["workspace"] / "botowned.py"
        f.write_text("print('hello')\n")

        def _denied(*a, **kw):
            raise PermissionError("EACCES (bot-owned file)")

        monkeypatch.setattr(evolve_util, "atomic_write_text", _denied)

        calls: list[dict] = []

        def _fake_embed(target, *, new_text, bot_user):
            calls.append({"target": Path(target), "bot_user": bot_user})
            return True, ""

        monkeypatch.setattr(meh, "embed_marker_privileged", _fake_embed)

        r = self._post(client, f)
        assert r.status_code == 200, r.get_json()
        assert r.get_json()["applied_via"] == "privileged_helper"
        assert calls == [{"target": f, "bot_user": "personal_bot_user"}]

    def test_manual_cli_fallback_sudos_as_the_account(self, alias_env, monkeypatch):
        """`sudo -u` takes an OS account. When the privileged helper is
        unavailable the copy-paste hint must name the account, or the operator
        gets 'unknown user: team_bot_b'."""
        import evolve_util
        from evolve_admin.applications import marker_embed_helper as meh

        client, env = alias_env
        f = env["workspace"] / "botowned2.py"
        f.write_text("print('hello')\n")
        ...
        monkeypatch.setattr(evolve_util, "atomic_write_text", _denied)
        monkeypatch.setattr(
            meh, "embed_marker_privileged",
            lambda target, *, new_text, bot_user: (False, "no sudoers grant"),
        )

        r = self._post(client, f)
        assert r.status_code == 403
        manual_cli = r.get_json()["manual_cli"]
        assert "sudo -u personal_bot_user " in manual_cli
        assert "team_bot_b" not in manual_cli
