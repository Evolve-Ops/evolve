...
def test_list_rows_carry_only_the_choosing_fields(pod):
    ...
    # `has_actions` is a one-glyph presence flag (D-BI5 §6), not the actions
    # themselves — the note stays out, same as before.
    assert set(row) == {"id", "title", "lane", "owner", "cluster", "has_actions"}
...
def test_a_bot_drop_is_attributable_and_not_the_users_refusal(pod):
    """The learning loop's negative signal is only partly the user's.

    The bot can drop a card through ``board.move``, and that row sits in the
    same log as the user's own refusals — distinguishable ONLY by ``actor``.
    A preference detector that skips the filter would learn the bot's tidying
    habits and report them as what the user never wants. Pinned here so the
    field cannot quietly stop being written.
    """
    c = pod["client"]
    card_id = _add(pod, title="Standup", source="calendar").get_json()["card"]["id"]
    c.post(f"/api/board-bot/cards/{card_id}/move",
           json={"to_lane": "dropped", "reason": "already handled"},
           environ_overrides=_unix_env())
    rows = []
    for p in sorted((pod["shared"] / "boards" / BOT / "events").glob("*.jsonl")):
        rows += [json.loads(line) for line in p.read_text().splitlines() if line]
    drops = [e for e in rows if e["event"] == "dropped"]
    assert [e["actor"] for e in drops] == ["bot"]
    # The filter a detector must apply leaves nothing behind here — this drop
    # is the bot's, and says nothing about the user's preferences.
    assert [e for e in drops if e["actor"] == "user"] == []


# ── Morning Board briefing panel (D-BI6d) ──────────────────────────────────


def test_briefing_post_requires_peer_credentials(pod):
    r = pod["client"].post("/api/board-bot/briefing", json={"text": "Hi"})
    assert r.status_code == 403


def test_briefing_post_stores_the_exact_text_the_daily_message_carried(pod):
    r = pod["client"].post("/api/board-bot/briefing", json={"text": "Hi there"},
                           environ_overrides=_unix_env())
    assert r.status_code == 200
    briefing = r.get_json()["briefing"]
    assert briefing["text"] == "Hi there"
    assert bs.load_briefing(pod["shared"], BOT)["text"] == "Hi there"


def test_briefing_post_cannot_write_another_bots_panel(pod):
    pod["client"].post("/api/board-bot/briefing", json={"text": "Mine", "bot_id": OTHER},
                       environ_overrides=_unix_env())
    assert bs.load_briefing(pod["shared"], OTHER) is None
    assert bs.load_briefing(pod["shared"], BOT)["text"] == "Mine"
