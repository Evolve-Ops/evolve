...
class TestCheckBotSecretModes:
    ...
    def test_workspace_git_config_is_enforced(self, tmp_path, monkeypatch):
        # Under https_pat auth the backup remote URL embeds a GitHub PAT, so
        # the workspace .git/config is token-bearing. A 644 file (what the
        # pre-2026-07-28 rotate handler left behind) must read as drifted with
        # a chmod-600 repair on offer.
        assert "workspace/.git/config" in scp.BOT_SECRET_CONFIG_RELPATHS
        oc_dir = tmp_path / ".openclaw"
        git_dir = oc_dir / "workspace" / ".git"
        git_dir.mkdir(parents=True)
        cfg = git_dir / "config"
        cfg.write_text("[remote \"evolve-backup\"]\n\turl = https://ghp_x@github.com/o/r.git\n")
        os.chmod(cfg, 0o644)
        monkeypatch.setattr(scp, "_user_home", lambda u: tmp_path)
        checks = scp.check_bot_secret_modes("team-bot-a")
        cfg_check = [c for c in checks if c.target.endswith(".git/config")]
        assert cfg_check and not cfg_check[0].ok
        assert cfg_check[0].apply is not None
