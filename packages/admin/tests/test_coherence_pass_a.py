...
Spec: internal/spec-app-coherence-and-reconciliation-2026-06-05.md §6.1, §7.1.
...
#
# Spec: internal/spec-manifest-v7-2026-05-20.md. v7-arc Instances store their
...
# ── AL-1.4b: dependency identity resolves through the shared resolver ────────


def _dep_keys(manifest: dict) -> set:
    from evolve_admin.applications.coherence_pass_a import _declared_dependency_keys
    return _declared_dependency_keys(manifest)


def test_dependency_keys_unchanged_for_every_shape_writers_emit() -> None:
    """AL-1.4b behavior-neutrality pin.

    ``_declared_dependency_keys`` was swept onto ``app_identity.resolve_app_id``
    (internal/build-AL-1.4-app-id-canonical.md §3). The sweep is only legitimate if
    it returns the SAME keys for every dependency shape a writer actually
    emits: the ea-pack legacy shape ``{pkg_id, display_name, ...}`` and the
    v7-arc shape ``{spec_id, required, purpose}`` that
    ``migrate_v7._migrate_app_dependencies`` produces.
    """
    assert _dep_keys(
        {"app_dependencies": [{"pkg_id": "p-11111111", "display_name": "Task Manager"}]}
    ) == {"p-11111111", "task manager"}
    assert _dep_keys(
        {"apps": [{"spec_id": "p-33333333", "required": True, "purpose": "x"}]}
    ) == {"p-33333333"}
    # Hybrid manifest defensive path: the v7 loop used to read
    # ``spec_id or pkg_id`` while the canonical order puts pkg_id first. The
    # migration writes spec_id FROM pkg_id, so the two are equal wherever both
    # appear and the order flip is unobservable.
    assert _dep_keys(
        {"apps": [{"spec_id": "p-55555555", "pkg_id": "p-55555555"}]}
    ) == {"p-55555555"}
    assert _dep_keys({"apps": [{"pkg_id": "p-44444444"}]}) == {"p-44444444"}
    # Degenerate entries contribute nothing, before and after.
    assert _dep_keys({"app_dependencies": [{"display_name": "No Pkg"}]}) == {"no pkg"}
    assert _dep_keys({"app_dependencies": [{"pkg_id": ""}]}) == set()
    assert _dep_keys({"app_dependencies": ["not-a-dict"]}) == set()
    assert _dep_keys({"apps": [{}]}) == set()
    assert _dep_keys({}) == set()


def test_dependency_keys_honor_a_canonical_app_id_and_strip() -> None:
    """The two DELIBERATE deltas the sweep introduces, pinned so a reviewer
    sees them rather than discovering them on a pod.

    1. A dependency entry carrying a canonical ``app_id`` now resolves to it
       instead of to its mirrored legacy id. No writer emits ``app_id`` on a
       dependency sub-record today (AL-1.4a stamps manifests, not dependency
       entries), so this is dormant until 1.4c.
    2. The resolver strips, so a whitespace-padded id now matches a clean one
       instead of forming a second, never-matching key.
    """
    assert _dep_keys(
        {"app_dependencies": [{"app_id": "task-manager", "pkg_id": "p-88888888"}]}
    ) == {"task-manager"}
    assert _dep_keys(
        {"app_dependencies": [{"pkg_id": "  p-22222222  "}]}
    ) == {"p-22222222"}
