...
# ── an inbound connection that dies before its first byte ───────────────────
#
# The 2026-09-04 phone test: the bind line above logged success, the phone
# said "the network connection was lost", and the only evidence anywhere was
# `OSError: [Errno 57] Socket is not connected` repeating in a stderr file
# nobody reads. What these pin is that the same fact now reaches an operator
# with its cause and its repair — once per process, not once per retry.


@pytest.fixture()
def _fresh_drop_latch(monkeypatch):
    """A fresh daemon process, as far as the once-per-process latch knows.

    Set directly rather than through a reset helper: the latch is only ever
    cleared by a daemon restart in production, so a reset function would be a
    production symbol whose only caller is this fixture.
    """
    monkeypatch.setattr(bl, "_dropped_reported", False)


class _RecordingServer(bl._ThreadingWSGIServer):
    """A server object with no socket — ``handle_error`` needs neither."""

    def __init__(self, shared_dir=None):  # noqa: D107 - deliberately no bind
        self.shared_dir = shared_dir
        self.fell_through = 0

    def _fallback(self, request, client_address):
        self.fell_through += 1


def _raise_into_handle_error(server, exc, peer=("100.64.0.9", 51000)):
    """Drive ``handle_error`` from inside a real ``except``, the way
    ``socketserver.ThreadingMixIn.process_request_thread`` does — it reads
    ``sys.exc_info()``, so a synthesized call would test nothing."""
    try:
        raise exc
    except type(exc):
...
def test_a_dropped_connection_warns_once_and_records_one_signal(
    monkeypatch, caplog, tmp_path, _fresh_drop_latch,
):
    observed: list[dict] = []
    monkeypatch.setattr(
        bl, "_observe_dropped_signal",
        lambda shared_dir, host, exc: observed.append(
            {"shared_dir": shared_dir, "host": host, "exc": exc}
        ),
    )
    server = _RecordingServer(tmp_path)

    with caplog.at_level(logging.WARNING):
        _raise_into_handle_error(
            server, OSError(errno.ENOTCONN, "Socket is not connected")
        )
    first = "\n".join(r.getMessage() for r in caplog.records)
    assert "dropped before any bytes" in first
    assert "Application Firewall" in first
    assert bl.REPAIR_COMMAND in first
    assert "100.64.0.9" in first
    assert len(observed) == 1
    assert observed[0]["shared_dir"] == tmp_path

    caplog.clear()
    with caplog.at_level(logging.WARNING):
        _raise_into_handle_error(
            server, OSError(errno.ECONNRESET, "Connection reset by peer")
        )
    assert caplog.records == []
    assert len(observed) == 1
    assert server.fell_through == 0


def test_any_other_handler_error_keeps_the_default_behaviour(
    monkeypatch, tmp_path, _fresh_drop_latch,
):
    """A bug in a board route must stay exactly as loud as it was — the
    interception is for ONE errno family, not for handler errors at large."""
    monkeypatch.setattr(
        socketserver.BaseServer, "handle_error",
        lambda self, request, client_address: self._fallback(request, client_address),
    )
    server = _RecordingServer(tmp_path)
    _raise_into_handle_error(server, ValueError("a real bug"))
    assert server.fell_through == 1

    # An OSError with an unrelated errno is not the firewall shape either.
    _raise_into_handle_error(server, OSError(errno.EPIPE, "Broken pipe"))
    assert server.fell_through == 2


def test_the_signal_is_skipped_when_no_shared_dir_was_ever_learned(
    caplog, _fresh_drop_latch,
):
    """A bare server (a test harness, or a future caller that never set it)
    still warns — it just does not invent a store path to write into."""
    server = _RecordingServer(None)
    with caplog.at_level(logging.WARNING):
        _raise_into_handle_error(server, OSError(errno.ENOTCONN, "nope"))
    assert "dropped before any bytes" in "\n".join(
        r.getMessage() for r in caplog.records
    )


def test_a_started_listener_knows_where_its_signal_store_is(
    monkeypatch, network_file, inner_app, tmp_path,
):
    """``handle_error`` runs on a worker thread with no request context, so
    the shared dir has to be captured at bind time or the Signal is lost."""
    monkeypatch.setattr(
        bl, "resolve_bind_address",
        lambda: bl.BindAddress("127.0.0.1", bl.INTERFACE_SOURCE),
    )
    ...
    net = network_file({
        ...
        "board": {"tailnetListener": {"enabled": True, "port": port}},
    })

    result = bl.start_board_listener(inner_app, net, default_port=port)
    assert result is not None
    _thread, server = result
    try:
        assert server.shared_dir == shared
    finally:
        server.shutdown()
        server.server_close()


def test_the_real_signal_lands_in_the_store_the_alerts_page_reads(
    tmp_path, _fresh_drop_latch,
):
    """End-to-end through ``signals.store``, not a stub: the producer name,
    the type and the repair command are what an operator reads on the Alerts
    page, and a stubbed emitter would pin none of them."""
    from signals import store as signals_store

    server = _RecordingServer(tmp_path)
    _raise_into_handle_error(server, OSError(errno.ENOTCONN, "Socket is not connected"))

    firing = list(signals_store.iter_signals(tmp_path, subdirs=("firing",)))
    assert len(firing) == 1
    sig = firing[0]
    assert sig.producer == bl.PRODUCER
    assert sig.type == bl.TYPE_INBOUND_DROPPED
    assert sig.scope == "pod"
    assert bl.REPAIR_COMMAND in sig.body
    assert sig.details["errno"] == errno.ENOTCONN
