...
def test_step_4a_spawns_under_the_timed_chip_title(procedure):
    ...
    assert "`[META:<aspect>] HH:MM [<id>] <title>`" in procedure
...
def test_step_4d_poke_pins_the_created_time_and_session(procedure):
    ...
    """
    assert ("> `[META:<aspect>] [<id>] <title>` is prepared in the tray — created HH:MM "
...
def test_step_2c_writes_the_durable_card_surface(procedure):
    """D-CD3: `CARDS.md` is written every tick, from the SAME `cards[]` step 2 already
    computed — never a second scan of the lane — and lands on the standing branch like
    any other lane path."""
    assert "2c. **WRITE THE DURABLE CARD SURFACE (D-CD3).**" in procedure
    assert "python3 tools/meta-dispatch-move cards --json" in procedure
    assert "internal/dispatch/CARDS.md" in procedure
    assert "never a second scan of the lane" in procedure
    assert "NO-OP when the bytes are unchanged" in procedure


def test_step_4d_poke_repeats_on_age(procedure):
    """D-CD4 (2026-09-11): the launch-needs-a-click poke re-fires at 6h and 12h for a
    card still `prepared`/`stale`, each crossing with its OWN signature, so the existing
    edge rule still guarantees exactly one poke per event and nothing repeats past 12h."""
    assert "**REPEATS ON AGE (D-CD4, 2026-09-11).**" in procedure
    assert "`1:card:<id>:0h`" in procedure
    assert "`1:card:<id>:6h`" in procedure
    assert "`1:card:<id>:12h`" in procedure
    assert "never after 12" in procedure
    assert "prepared 6h — tap or decline" in procedure
