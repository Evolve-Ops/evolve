...
def test_settings_pack_points_at_config_bot_tool(html: str):
    """The pack's tool_pointers must signpost ``config.bot(bot_id=...)``
    — that's the fetch-on-demand path for any field the rendered cards
    elide (e.g. specific tool grants, hook config). Without the
    pointer the model has no fallback when an operator asks about a
    detail the snapshot didn't capture."""
    packs_start = html.find("const _EVO_CONTEXT_PACKS")
    assert packs_start > 0
    start = html.find("'settings':", packs_start)
    assert start > 0
    window = html[start:start + 5000]
    assert "config.bot" in window, (
        "the 'settings' pack no longer references config.bot in its "
...
def test_agents_md_bot_detail_row_present(agents_md: str):
    ...
    Bot" to ``config.bot(bot_id=...)`` and may default to vague
    "I'd need to look that up" replies."""
    ...
    for line in agents_md.splitlines():
        low = line.lower()
        if "bot detail" in low and "config.bot" in low:
            found = True
            break
    assert found, (
        "AGENTS.md no longer has a page-tool-map row pairing 'Bot detail' "
        "with `config.bot`. Either the row was deleted or the wording "
        "drifted — restore the pairing so the model has the signpost."
    )


def test_agents_md_bot_detail_row_mentions_pod_state_bots(agents_md: str):
    """The Bot-detail row should also signpost ``pod_state.bots`` —
    the runtime side (chips / status / activity) pairs with
    ``config.bot`` (the static openclaw.json). Operators often ask
    questions that need both ("why is team_bot_a showing a scan_needed chip
    while its archetype says ...")."""
    found = False
    for line in agents_md.splitlines():
        low = line.lower()
        if "bot detail" in low and "pod_state.bots" in low:
            found = True
            break
    assert found, (
        "AGENTS.md's Bot-detail row no longer signposts pod_state.bots. "
        "config.bot is the static config; pod_state.bots is the live "
        "tile. Operators routinely need both for bot-detail questions."
    )
