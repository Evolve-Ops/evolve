...
def test_malformed_requester_header_denied(app_with_path, tmp_path):
    """A garbage header is DENIED — never coerced onto the trusted-UI path
    (WO-H1-2 / CBR-1.2). The caller asserted an identity and failed; no
    legitimate caller sends a malformed header (the UI sends none, and both
    gateway senders build it from non-empty parts or refuse locally), so a
    parse failure must fail closed, even on the UI transport."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": ["111"]})
    for bad in ("this-is-not-valid",   # no colon
                "telegram:",           # empty stable_id
                ":999",               # empty platform
                " : "):               # both segments blank
        with app_with_path.test_client() as c:
            resp = c.patch(
                "/api/admin/bots/atlas/users/telegram/111",
                json={"role": "primary_user"},
                headers={"X-Requester-Identity": bad},
            )
        assert resp.status_code == 403, f"header {bad!r} was not denied"
        body = resp.get_json()
        assert body["error"] == "forbidden"
        assert "malformed X-Requester-Identity" in body["detail"]


def test_multi_colon_header_is_valid_parse_not_malformed(app_with_path, tmp_path):
    """``telegram:99:9`` parses as a VALID identity (partition on the first
    colon → stable_id ``"99:9"``) and takes the capability path — denied here
    as an unprivileged participant, with the role detail, NOT the malformed
    detail. Pins the valid-vs-malformed boundary this change hardens: stable
    ids are only ever exact-matched (roster_overlay / external_ids), so a
    colon-bearing id can't be re-split into someone else's identity."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": ["111"]})
    with app_with_path.test_client() as c:
        resp = c.patch(
            "/api/admin/bots/atlas/users/telegram/111",
            json={"role": "primary_user"},
            headers={"X-Requester-Identity": "telegram:99:9"},
        )
    assert resp.status_code == 403
    detail = resp.get_json()["detail"]
    assert "malformed" not in detail
    assert "bot.roster.mutate" in detail
    assert "telegram:99:9" in detail


def test_empty_requester_header_treated_as_absent(app_with_path, tmp_path):
    """An empty/whitespace-only header value asserts nothing — proxies can
    strip a header to empty — so it takes the ABSENT path (trusted here only
    because the conftest runs with device-auth disabled, the documented open
    -pod model), not the malformed-deny path."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": ["111"]})
    with app_with_path.test_client() as c:
        resp = c.patch(
            "/api/admin/bots/atlas/users/telegram/111",
            json={"role": "primary_user"},
            headers={"X-Requester-Identity": "   "},
        )
    assert resp.status_code == 200


def test_malformed_requester_header_over_socket_denied(app_with_path, tmp_path):
    """Malformed header over the unix socket → 403 with the malformed (not
    the header-absent) detail, proving it is denied before any transport
    trust question is asked."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": ["111"]})
    with app_with_path.test_client() as c:
        resp = c.patch(
            "/api/admin/bots/atlas/users/telegram/111",
            json={"role": "primary_user"},
            headers={"X-Requester-Identity": "not-an-identity"},
            environ_overrides={"REMOTE_TRANSPORT": "unix-socket",
                               "REMOTE_PEER_UID": 0},
        )
    assert resp.status_code == 403
    assert "malformed X-Requester-Identity" in resp.get_json()["detail"]


def test_malformed_requester_header_denied_even_authenticated_ui(
        app_with_path, tmp_path):
    """Malformed header on the AUTHENTICATED UI transport (device-auth
    enforced, valid cookie) is still denied: the deny is unconditional on
    transport, unlike the absent-header case which trusts the proven UI."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": ["111"]})
    ...
    try:
        with app_with_path.test_client() as c:
            resp = c.patch(
                "/api/admin/bots/atlas/users/telegram/111",
                json={"role": "primary_user"},
                headers={"X-Requester-Identity": "broken"},
            )
        assert resp.status_code == 403
        assert "malformed X-Requester-Identity" in resp.get_json()["detail"]
    finally:
...
# ── CBR-1.2 sibling — approve/revoke/reject capability gate (WO-H1-2) ────────
#
# The three DM pairing mutation routes share ``_handle_action``, which was the
# only mutation handler in routes_bot_users with NO ``_check_capability`` call:
# any socket peer that omitted X-Requester-Identity (and, on every pod, the
# trusted evo peer uid) could admit/revoke DM users with no capability check.
# The gate is ``bot.roster.mutate`` — same as patch/block/unblock/ignore — so
# these tests mirror the patch matrix above for the approve route, then pin
# that revoke and reject go through the same shared-handler gate.


def _allowfrom(creds: Path, channel: str = "telegram") -> list:
    return json.loads(
        (creds / f"{channel}-default-allowFrom.json").read_text()
    )["allowFrom"]


def test_approve_header_absent_over_socket_denied(app_with_path, tmp_path):
    """Header-absent on the unix socket → 403 and NO admission write."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": []})
    with app_with_path.test_client() as c:
        resp = c.post(
            "/api/admin/bots/atlas/users/approve",
            json={"channel": "telegram", "id": "444"},
            environ_overrides=_SOCKET_ENV,
        )
    assert resp.status_code == 403
    body = resp.get_json()
    assert body["error"] == "forbidden"
    assert "no requester identity" in body["detail"]
    assert "444" not in _allowfrom(creds)


def test_approve_header_absent_unauthenticated_tcp_denied(
        app_with_path, tmp_path):
    """Header-absent over TCP, device-auth enforced, no valid cookie → 403."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": []})
    import evolve_admin.web.admin_auth as _aa
...
def test_approve_header_absent_authenticated_ui_allowed(
        app_with_path, tmp_path):
    """The SPA Users page (header-less, authenticated UI transport) keeps
    working — the existing admin flow is preserved."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": []})
    import evolve_admin.web.admin_auth as _aa
    ...
    try:
        ...
        assert resp.status_code == 200
        assert "444" in _allowfrom(creds)
    finally:
        monkeypatch.undo()


def test_approve_malformed_header_over_socket_denied(app_with_path, tmp_path):
    """A garbage X-Requester-Identity over the socket is denied. (Over the
    socket the deny holds under both header semantics: pre-#3638 malformed
    parses as absent → socket deny; post-#3638 malformed is denied on every
    transport.)"""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": []})
...
def test_approve_with_participant_requester_403(app_with_path, tmp_path):
    """A participant cannot admit DM users — bot.roster.mutate is refused."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": ["333"]})
    with app_with_path.test_client() as c:
...
def test_approve_with_pod_admin_requester_succeeds(app_with_path, tmp_path):
    """A gateway-attested pod admin still has the capability → 200 + write."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": []})
    with app_with_path.test_client() as c:
...
def test_revoke_carries_same_capability_gate(app_with_path, tmp_path):
    """Revoke goes through the same shared-handler gate — participant → 403
    and the target stays admitted."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": ["333", "111"]})
    with app_with_path.test_client() as c:
        resp = c.post(
            "/api/admin/bots/atlas/users/revoke",
            json={"channel": "telegram", "id": "111"},
            headers={"X-Requester-Identity": "telegram:333"},
        )
    assert resp.status_code == 403
    assert "bot.roster.mutate" in resp.get_json()["detail"]
    assert "111" in _allowfrom(creds)


def test_reject_carries_same_capability_gate(app_with_path, tmp_path):
    """Reject goes through the same shared-handler gate — participant → 403
    and the pending request survives."""
    creds = _bot_creds_dir(tmp_path, "atlas")
    _write(creds / "telegram-default-allowFrom.json",
           {"version": 1, "allowFrom": ["333"]})
    _write(creds / "telegram-pairing.json", {
    ...
    with app_with_path.test_client() as c:
        resp = c.post(
            "/api/admin/bots/atlas/users/reject",
            json={"channel": "telegram", "id": "789"},
            headers={"X-Requester-Identity": "telegram:333"},
        )
    assert resp.status_code == 403
    pairing = json.loads((creds / "telegram-pairing.json").read_text())
    assert [r["id"] for r in pairing["requests"]] == ["789"]
...
# ── B3a: nested guild / guild-channel member lists ───────────────────────
#
# Discord's per-guild allowlists live BELOW the channel block
# (``channels.discord.guilds.<gid>.users`` and
# ``guilds.<gid>.channels.<cid>.users``). The top-level reader cannot see them,
# so a bot whose only group member sat in a guild list rendered as a
# gated-but-EMPTY channel — while the coherence Signal (which DOES harvest the
# nested lists) told the operator to come here and act. The GET now surfaces
# those lists with their provenance, and revoke targets the exact nested key.

_GUILD_ID = "900000000000000001"
_GUILD_CHANNEL_ID = "800000000000000001"


def _nested_discord_cfg() -> dict:
    """The live reference-pod shape: allowlist-gated, NO top-level list, one
    member in a guild list (plus a per-guild-channel list)."""
    return {
        "channels": {
            "discord": {
                "groupPolicy": "allowlist",
                ...
                "guilds": {
                    _GUILD_ID: {
                        ...
                        "users": [_DISCORD_ID_A],
                        "channels": {
                            _GUILD_CHANNEL_ID: {"users": [_DISCORD_ID_B]},
                        },
                    },
                },
            },
...
def test_get_surfaces_nested_guild_members_with_provenance(app, tmp_path):
    _bot_creds_dir(tmp_path, "team_bot_a")
    _write_openclaw(tmp_path, "team_bot_a", _nested_discord_cfg())
    with app.test_client() as c:
        data = c.get("/api/admin/bots/team_bot_a/users").get_json()
    discord = data["by_channel"]["discord"]
    # The top-level list is still empty — that is the shape that used to leave
    # the operator with nothing to act on.
    assert discord["group_access"] == []
    assert discord["group_allowlist_gated"] is True
    groups = discord["nested_group_access"]
    assert [g["config_path"] for g in groups] == [
        f"channels.discord.guilds.{_GUILD_ID}.users",
        f"channels.discord.guilds.{_GUILD_ID}.channels."
        f"{_GUILD_CHANNEL_ID}.users",
    ]
    assert [g["container_label"] for g in groups] == [
        f"guild {_GUILD_ID}",
        f"guild {_GUILD_ID} › channel {_GUILD_CHANNEL_ID}",
    ]
    assert [[m["id"] for m in g["members"]] for g in groups] == [
        [_DISCORD_ID_A], [_DISCORD_ID_B]]
    assert all(m["access_source"] == "group_allowlist_nested"
               for g in groups for m in g["members"])
    # The path is exactly what a revoke sends back.
    assert groups[0]["path"] == ["guilds", _GUILD_ID, "users"]
    # A channel with no nested lists carries an empty list, not a missing key.
    assert data["by_channel"]["slack"]["nested_group_access"] == []


def test_get_nested_members_are_not_active_not_admitted(app, tmp_path,
                                                        monkeypatch):
    """A nested member is admitted on the same gate, so they must not also show
    up in the "Active · not admitted" triage lane."""
    _bot_creds_dir(tmp_path, "team_bot_a")
    _write_openclaw(tmp_path, "team_bot_a", _nested_discord_cfg())

    import evolve_admin.evo.identity_discovery as idsc

    monkeypatch.setattr(idsc, "discover_candidates", lambda *a, **k: [
        {"channel": "discord", "external_id": _DISCORD_ID_A, "turn_count": 3},
    ])
    monkeypatch.setattr(idsc, "resolve_with_names", lambda net, c, **k: c)
    with app.test_client() as client:
        data = client.get("/api/admin/bots/team_bot_a/users").get_json()
    assert data["by_channel"]["discord"]["seen_recently"] == []

    # Control: with the channel not allowlist-gated the nested rows are not
    # surfaced, and the same candidate DOES land in the triage lane — so the
    # assertion above is the exclusion doing work, not a dead discovery path.
    ungated = _nested_discord_cfg()
    ungated["channels"]["discord"]["groupPolicy"] = "open"
    _write_openclaw(tmp_path, "team_bot_a", ungated)
    with app.test_client() as client:
        data = client.get("/api/admin/bots/team_bot_a/users").get_json()
    assert [s["id"] for s in data["by_channel"]["discord"]["seen_recently"]] \
        == [_DISCORD_ID_A]


def test_get_hides_nested_lists_when_channel_not_allowlist_gated(app, tmp_path):
    """Same gate as the top-level reader: under `open`/`disabled` there is no
    curated per-sender list, so we surface no rows rather than claiming an
    access grant OpenClaw does not make from that list."""
    _bot_creds_dir(tmp_path, "team_bot_a")
    cfg = _nested_discord_cfg()
    cfg["channels"]["discord"]["groupPolicy"] = "open"
    _write_openclaw(tmp_path, "team_bot_a", cfg)
    with app.test_client() as c:
        data = c.get("/api/admin/bots/team_bot_a/users").get_json()
    assert data["by_channel"]["discord"]["nested_group_access"] == []


def test_nested_revoke_removes_only_the_targeted_list(group_env, tmp_path):
    app, calls = group_env
    _bot_creds_dir(tmp_path, "team_bot_a")
    _write_openclaw(tmp_path, "team_bot_a", _nested_discord_cfg())
    with app.test_client() as c:
        resp = c.post(
            "/api/admin/bots/team_bot_a/users/group-allowlist/revoke",
            json={"channel": "discord", "id": _DISCORD_ID_A,
                  "path": ["guilds", _GUILD_ID, "users"]})
        data = resp.get_json()
    assert resp.status_code == 200 and data["ok"] is True
    oc = _oc_on_disk(tmp_path, "team_bot_a")
    guild = oc["channels"]["discord"]["guilds"][_GUILD_ID]
    assert guild["users"] == []
    # Everything else is byte-identical: the guild's other keys, the nested
    # per-channel list, the sibling channel, and the tokens.
    assert guild["requireMention"] is False
    assert guild["channels"][_GUILD_CHANNEL_ID]["users"] == [_DISCORD_ID_B]
    assert oc["channels"]["discord"]["token"] == "shh"
    assert oc["channels"]["slack"]["allowFrom"] == ["U001"]
    assert oc["gateway"] == {"token": "shh"}
    # Through the validating writer, with the audit reason naming the exact key.
    assert len(calls["write"]) == 1
    assert f"guilds.{_GUILD_ID}.users" in calls["write"][0]["reason"]
    assert [r["bot_id"] for r in calls["restart"]] == ["team_bot_a"]
    # The re-render no longer lists the revoked member.
    groups = data["by_channel"]["discord"]["nested_group_access"]
    assert [g["path"] for g in groups] == [
        ["guilds", _GUILD_ID, "channels", _GUILD_CHANNEL_ID, "users"]]


def test_nested_revoke_does_not_touch_dm_pairing_store(group_env, tmp_path):
    """Strict separation holds for the nested surface too."""
    app, _ = group_env
    creds = _bot_creds_dir(tmp_path, "team_bot_a")
    dm_path = creds / "discord-default-allowFrom.json"
    _write(dm_path, {"version": 1, "allowFrom": [_DISCORD_ID_A]})
    dm_before = dm_path.read_text()
    _write_openclaw(tmp_path, "team_bot_a", _nested_discord_cfg())
    with app.test_client() as c:
        resp = c.post(
...
def test_nested_revoke_is_idempotent_no_write_no_restart(group_env, tmp_path):
    app, calls = group_env
    _bot_creds_dir(tmp_path, "team_bot_a")
    _write_openclaw(tmp_path, "team_bot_a", _nested_discord_cfg())
    with app.test_client() as c:
        resp = c.post(
            "/api/admin/bots/team_bot_a/users/group-allowlist/revoke",
            json={"channel": "discord", "id": "111111111111111111",
                  "path": ["guilds", _GUILD_ID, "users"]})
    assert resp.status_code == 200
    assert calls["write"] == [] and calls["restart"] == []


def test_nested_approve_is_refused(group_env, tmp_path):
    """Nested lists are revoke-only — a grant inside someone's guild list is a
    config authoring decision, and the channel-level add box already covers
    "let this person in"."""
    app, calls = group_env
    _bot_creds_dir(tmp_path, "team_bot_a")
    _write_openclaw(tmp_path, "team_bot_a", _nested_discord_cfg())
    with app.test_client() as c:
        resp = c.post(
            ...
            json={"channel": "discord", "id": "111111111111111111",
                  "path": ["guilds", _GUILD_ID, "users"]})
    assert resp.status_code == 400
    assert "revoke-only" in resp.get_json()["error"]
    assert calls["write"] == []


def test_nested_revoke_refuses_a_path_that_is_not_a_member_list(group_env,
                                                               tmp_path):
    """Shape gate: the last segment must be an identity-list key, so a request
    can never name `token`, `roles`, or any other key as the write target."""
    app, calls = group_env
    _bot_creds_dir(tmp_path, "team_bot_a")
    _write_openclaw(tmp_path, "team_bot_a", _nested_discord_cfg())
    with app.test_client() as c:
        for bad in (["guilds", _GUILD_ID, "roles"],
                    ["channels", "discord", "token"],
                    ["allowFrom"],                      # top-level: not nested
                    "guilds.%s.users" % _GUILD_ID,      # not a list
                    [".."] * 12):                       # over the depth bound
            resp = c.post(
                "/api/admin/bots/team_bot_a/users/group-allowlist/revoke",
                json={"channel": "discord", "id": _DISCORD_ID_A, "path": bad})
            assert resp.status_code == 400, bad
    assert calls["write"] == []


def test_nested_revoke_refuses_a_path_the_config_does_not_have(group_env,
                                                              tmp_path):
    """Existence is decided against the live config's own enumerated lists —
    a path is never traversed as given, so an unknown guild cannot create a
    key or reach anything."""
    app, calls = group_env
    _bot_creds_dir(tmp_path, "team_bot_a")
    _write_openclaw(tmp_path, "team_bot_a", _nested_discord_cfg())
    with app.test_client() as c:
        resp = c.post(
            "/api/admin/bots/team_bot_a/users/group-allowlist/revoke",
            json={"channel": "discord", "id": _DISCORD_ID_A,
                  "path": ["guilds", "111111111111111111", "users"]})
    assert resp.status_code == 400
    assert "no nested member list" in resp.get_json()["error"]
    assert calls["write"] == []
    # The refusal did not invent the key either.
    oc = _oc_on_disk(tmp_path, "team_bot_a")
    assert list(oc["channels"]["discord"]["guilds"]) == [_GUILD_ID]


def test_nested_revoke_requires_allowlist_gate(group_env, tmp_path):
    """Same fail-loud rule as the top-level write: a channel that is not
    allowlist-gated has no curated list to manage."""
    app, calls = group_env
    _bot_creds_dir(tmp_path, "team_bot_a")
    cfg = _nested_discord_cfg()
    cfg["channels"]["discord"]["groupPolicy"] = "open"
...
def test_apply_nested_change_never_mutates_the_caller_config():
    cfg = _nested_discord_cfg()
    before = json.dumps(cfg, sort_keys=True)
    new, key = rbu._apply_group_allowlist_change(
        cfg, "discord", _DISCORD_ID_A, add=False,
        path=["guilds", _GUILD_ID, "users"])
    assert key == f"guilds.{_GUILD_ID}.users"
    assert new["channels"]["discord"]["guilds"][_GUILD_ID]["users"] == []
