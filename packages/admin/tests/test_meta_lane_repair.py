...
def test_the_request_schema_is_the_five_fields_and_a_closed_arg_shape(tmp_path):
    ...
    assert sorted(MOD.REPAIR_VERBS) == ["abandon-entry", "dismiss-card", "drop-stale-marker",
    ...
    assert MOD.REPAIR_VERBS["drop-stale-marker"] == {"required": ("id",), "optional": ()}


def test_there_is_no_sixth_verb_and_no_generic_escape_hatch(tmp_path):
    """The guardrail, stated as a test: a generic "run this" verb would make a committed
    file into a remote shell on the operator's machine with extra steps. `drop-stale-
    marker` (#4271) is the fifth and, like the first four, delegates its entire effect to
    an existing, already-safe lane verb (`done`) rather than composing a new one."""
    assert set(MOD.REPAIR_EXECUTORS) == set(MOD.REPAIR_VERBS)
    assert len(MOD.REPAIR_VERBS) == 5
...
# ── the first four verbs, each performing its effect and recording an outcome ─
...
# ── the fifth verb: drop-stale-marker (#4271) ──────────────────────────────────
#
# A `lane/state` branch cut before a chip's own PR carries its id to `done/` still shows
# that id in `inflight/`; merged as-is, the id lands in both dirs and the lane stops
# (`blocked_by: "lane-conflict"`). Before this verb the only fix was a PM PR an operator
# had to click merge on — eighteen hours wedged after #4271, repaired by hand in PR #4295.
# Delegated to the EXISTING `done` lane verb, not reimplemented — see requirement 5 of
# `internal/dispatch/queued/lane-state-pr-asserts-the-transition-it-came-from.md`.


def test_drop_stale_marker_drops_the_inflight_copy_and_keeps_done_untouched(tmp_path):
    """D-CS7 known-good: the id is genuinely present in two dirs, so the drop proceeds —
    and the terminal copy is never the one that moves."""
    root = _lane(tmp_path)
    marker = _entry(root, "resurrected-chip", state="inflight", launch="prepared")
    durable = root / "done" / "resurrected-chip.md"
    durable.write_text(BRIEF.format(id="resurrected-chip", launch="prepared"))
    before = durable.read_text()
    _request(root, "r-drop", "drop-stale-marker", {"id": "resurrected-chip"})

    report = MOD.repair_run(root, ledger_dir=str(_ledger(tmp_path, [])))
    rec = _by_id(report)["r-drop"]
    assert rec["ok"] and rec["reason"] == "ok", rec
    assert not marker.exists()
    assert durable.is_file() and durable.read_text() == before
    assert any("resurrected-chip" in c for c in rec["changed"])

    out = _outcome(root, "r-drop")
    assert out["verb"] == "drop-stale-marker"
    assert str(marker) in out["read"] and str(durable) in out["read"]


def test_drop_stale_marker_refuses_without_a_done_counterpart(tmp_path):
    """D-CS7 known-bad: an id genuinely still in flight, with nothing terminal to justify
    the drop, is never this verb's business — the precondition runs BEFORE `done` is ever
    called, so a live chip's only record is never at the mercy of `done`'s looser guard
    for a tracked marker with no durable copy."""
    root = _lane(tmp_path)
    marker = _entry(root, "still-flying", state="inflight", launch="started")
    _request(root, "r-nope", "drop-stale-marker", {"id": "still-flying"})

    report = MOD.repair_run(root, ledger_dir=str(_ledger(tmp_path, [])))
    rec = _by_id(report)["r-nope"]
    assert not rec["ok"] and rec["reason"] == "no-such-target"
    assert "still-flying" in rec["message"]
    assert marker.exists()


def test_drop_stale_marker_refuses_when_the_marker_itself_is_absent(tmp_path):
    """The anti-vacuity case, same as the other two entry verbs: no target, no quiet
    success."""
    root = _lane(tmp_path)
    (root / "done" / "ghost.md").write_text(BRIEF.format(id="ghost", launch="prepared"))
    _request(root, "r-ghost", "drop-stale-marker", {"id": "ghost"})
...
def test_drop_stale_marker_refuses_when_done_does_not_carry_the_marker_body(tmp_path):
    """A same-id `done/` entry that is a DIFFERENT brief is not evidence of anything —
    `done`'s own `_check_durable_carries_the_brief` refuses, and the repair queue records
    that as `refused`, never as a silent no-op."""
    root = _lane(tmp_path)
    marker = _entry(root, "swapped-chip", state="inflight", launch="prepared")
    (root / "done" / "swapped-chip.md").write_text(
        BRIEF.format(id="swapped-chip", launch="prepared").replace(
            "WHY: pinned by tests.", "WHY: an entirely different brief."))
    _request(root, "r-swap", "drop-stale-marker", {"id": "swapped-chip"})

    report = MOD.repair_run(root, ledger_dir=str(_ledger(tmp_path, [])))
    rec = _by_id(report)["r-swap"]
    assert not rec["ok"] and rec["reason"] == "refused"
    assert marker.exists()
...
def test_a_refusal_never_touches_the_pod_the_grants_or_any_code(tmp_path):
    """Stated as a test of the surface: every executor is one of five, and the only paths
