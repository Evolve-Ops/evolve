...
def test_snooze_records_body_actor(app):
    """7.1 C2: evo's fail-closed signal tools route writes through
    /api/signals/<id>/snooze|dismiss|resolve, passing actor in the body
    so state_history attributes the action to evo instead of 'user'."""
    shared = Path(app.config["_test_shared"])
    sig_id = _seed_firing_signal(shared, "sig_actor_evo")

    with app.test_client() as c:
        resp = c.post(
            f"/api/signals/{sig_id}/snooze",
            json={"duration": "24h", "actor": "evo",
                  "reason": "evo snoozed via tool"},
        )
        assert resp.status_code == 200, resp.get_json()

    from signals import store as signals_store
    sig = signals_store.find_signal(shared, sig_id)[0]
    last = sig.state_history[-1]
    assert last.actor == "evo"
    assert last.reason == "evo snoozed via tool"


def test_snooze_actor_defaults_to_user(app):
    """Browser callers send no actor field — attribution stays 'user'."""
    shared = Path(app.config["_test_shared"])
    sig_id = _seed_firing_signal(shared, "sig_actor_default")

    with app.test_client() as c:
        resp = c.post(f"/api/signals/{sig_id}/snooze", json={})
        assert resp.status_code == 200, resp.get_json()

    from signals import store as signals_store
    sig = signals_store.find_signal(shared, sig_id)[0]
    assert sig.state_history[-1].actor == "user"
