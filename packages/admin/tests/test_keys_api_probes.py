...
def test_should_list_rule_d_now_fires_for_llm_providers():
    """REVERSED 2026-09-04. This test used to assert the opposite.

    Its premise was that "google/anthropic/openai keep keys in .env or
    auth-profiles", so rule (d) would pre-list a "Setup required" row for a
    Gemini provider that works fine. The premise was measured and refuted:
    the keys live in OC's PER-AGENT runtime store, which
    ``OcAgentCatalogKeyProbe`` now reads. A provider with a key there is
    ACTIVE and never reaches rule (d); a provider that is enabled with no key
    in ANY store — and, per rule (d′), no turns in the last 30 days either —
    is a real gap, and hiding it is how a bot ships advertising a model it
    cannot call.
    """
    from evolve_admin.web.credentials_visibility import annotate_should_list
    rows = [
    ...
    annotate_should_list(rows, ["github"])
    for row in rows:
        assert row["should_list"] is True, row


def test_should_list_rule_d_still_silent_for_disabled_llm_provider():
    """Enabled is the capability claim; a disabled plugin claims nothing."""
    from evolve_admin.web.credentials_visibility import annotate_should_list
    rows = [
        {"provider": p, "status": "missing", "plugin_enabled": False}
        for p in ("google", "anthropic", "openai", "xai")
    ]
    annotate_should_list(rows, ["github"])
    for row in rows:
...
def test_inline_key_provider_sets_agree_across_surfaces():
    ...
    the class of mismatch #3219 had to fix. The shared set is
    INLINE_KEY_PROVIDERS = "providers Evolve can positively verify"; the
    Skills page's ``_INLINE_KEY_PATHS`` is the openclaw.json HALF of it (the
    LLM half is verified through the per-agent runtime store instead), so the
    invariant is containment plus exhaustive coverage — every member must be
    verifiable by one of the two mechanisms, or it silently falls back to the
    enabled-only rule on one page and not the other.
    """
    from evolve_admin.skills.inventory import (
        _INLINE_KEY_PATHS, _is_runtime_store_provider,
    )
    from evolve_admin.web.credentials_oc import (
        INLINE_KEY_PROVIDERS, RUNTIME_STORE_KEY_PROVIDERS,
    )
    assert set(_INLINE_KEY_PATHS) <= set(INLINE_KEY_PROVIDERS)
    assert set(RUNTIME_STORE_KEY_PROVIDERS) <= set(INLINE_KEY_PROVIDERS)
    unverifiable = {
        p for p in INLINE_KEY_PROVIDERS
        if p not in _INLINE_KEY_PATHS and not _is_runtime_store_provider(p)
    }
    assert not unverifiable, (
        f"{sorted(unverifiable)} are in INLINE_KEY_PROVIDERS but neither "
        "surface knows how to verify them"
    )
