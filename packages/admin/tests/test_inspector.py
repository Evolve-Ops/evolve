...
def test_suggest_read_tool_maps_gateway_domain():
    assert suggest_read_tool("the gateway is wedged") == "pod_state.bots()"
...
def test_branch_d_stub_names_domain_read_tool():
    ...
    assert "pod_state.backup_status" in text
    assert event.extra.get("followup_tool") == "pod_state.backup_status(bot_id=...)"
...
def test_branch_d_records_pending_followup_marker(tmp_path, monkeypatch):
    ...
    assert "pod_state.backup_status" in telemetry
...
def test_consume_pending_followup_pops_once(tmp_path, monkeypatch):
    ...
    assert record["read_tool"] == "pod_state.backup_status(bot_id=...)"
...
def test_format_followup_nudge_names_tool_and_excerpt():
    ...
    assert "pod_state.backup_status(bot_id=...)" in block
...
def test_format_followup_nudge_generic_without_tool():
    ...
    assert "pod_state.* read tool" in block
...
def test_ui_alternative_backup_kickstart_prefers_backup_now():
    ...
    assert "action.bot.backup_workspace" in alt
...
def test_backup_page_incident_2026_07_28_turn1_regression():
    ...
    assert "action.bot.backup_workspace" in text
