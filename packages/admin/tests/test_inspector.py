...
Spec: ``internal/spec-surface-aware-help-style-2026-05-22.md`` §7.5 + §7.5.1.
...
from evolve_admin.evo.inspector import (  # noqa: E402
    ...
    HaikuVerdict,
    INFRA_OVERINTERPRETATION_PATTERNS,
    InspectorEvent,
    ...
    _extract_cli_blocks,
    _has_overinterpretation_signal,
    _has_preface_signal,
...
def _oracle_haiku(text: str) -> HaikuVerdict:
    ...
    if 'pod_state(query="backup_status", bot_id=atlas)` returned' in text:
    ...
    # Infrastructure over-interpretation — branch-d sub-pattern
    # (2026-07-28 Backup-page incident): an errno inflated into causal
    # claims a tool never showed. Real haiku keys on the extended (d)
    # wording; this oracle keys on the fixture's distinctive shape.
    if "econnrefused" in lowered and (
        "is down" in lowered or "which is why" in lowered
    ):
        return HaikuVerdict(
            "yes/d", reason="over-interpretation: causal claims beyond quoted error"
        )
...
def test_bare_shared_dir_write_with_evidence_allowed():
    ...
    text, event = inspect_outgoing_text(
        "`pod_state(query=\"backup_status\", bot_id=atlas)` returned `{\"error\": "
...
# ─────────────────────────────────────────────────────────────────────────────
# Infrastructure over-interpretation — branch-d sub-pattern
# (INFRA_OVERINTERPRETATION_PATTERNS, 2026-07-28 Backup-page incident).
# ─────────────────────────────────────────────────────────────────────────────


@pytest.mark.parametrize("pattern,_expected", INFRA_OVERINTERPRETATION_PATTERNS)
def test_infra_overinterpretation_pattern_caught(pattern, _expected):
    """An errno inflated into unverified causal claims ("daemon is down
    ... which is why the button isn't working") must trip branch d and
    be rejected — cite-or-don't covers diagnoses beyond the quoted
    output, not just evidence-free fixes."""
    text, event = inspect_outgoing_text(
        pattern,
        session_context={"surface": "admin_ui", "surface_type": "laptop"},
        haiku_fn=_oracle_haiku,
    )
    assert event is not None, f"over-interpretation not caught: {pattern!r}"
    assert event.branch == "d"
    assert event.event == "no_evidence_reject"
    # The inflated diagnosis is withheld.
    assert "daemon is down" not in text.lower()
    assert "read-only check" in text.lower()


def test_has_overinterpretation_signal_requires_both_halves():
    """The pre-filter signal needs BOTH an errno token and a causal
    phrase — an honest errno quote alone, or a causal claim alone, must
    not light up haiku."""
    for pattern, _ in INFRA_OVERINTERPRETATION_PATTERNS:
        assert _has_overinterpretation_signal(pattern), (
            f"over-interpretation signal missed known pattern: {pattern!r}"
        )
    # errno alone (honest evidence quote) — no signal.
    assert not _has_overinterpretation_signal(
        "The probe returned ECONNREFUSED on 127.0.0.1:9010."
    )
    # causal claim alone (no errno) — no signal.
    assert not _has_overinterpretation_signal(
        "The nightly job is down for maintenance, which is why the row is grey."
    )


def test_overinterpretation_prose_reaches_haiku_without_shell_tokens():
    """The 2026-07-28 turn-2 reply carries no shell verb, no sudo, no
    destructive-git token — before the errno+causal signal it never
    reached haiku at all."""
    pattern = INFRA_OVERINTERPRETATION_PATTERNS[0][0]
    _blocks, any_hit = _regex_pre_filter(pattern)
    assert any_hit is False, (
        "fixture unexpectedly trips the CLI pre-filter; the "
        "over-interpretation signal would be untested"
    )
    calls: list[str] = []

    def recording_oracle(text: str) -> HaikuVerdict:
        calls.append(text)
        return _oracle_haiku(text)

    inspect_outgoing_text(
        pattern,
        session_context={"surface": "admin_ui", "surface_type": "laptop"},
        haiku_fn=recording_oracle,
    )
    assert calls, "haiku was never consulted for the over-interpretation prose"


def test_haiku_prompt_template_covers_overinterpretation_sub_pattern():
    """The extended branch-(d) wording must describe the
    over-interpretation shape so real haiku can flag it."""
    from evolve_admin.evo.inspector import _HAIKU_PROMPT_TEMPLATE

    lowered = _HAIKU_PROMPT_TEMPLATE.lower()
    assert "over-interpretation" in lowered
    assert "econnrefused" in lowered
    assert "beyond" in lowered


# ─────────────────────────────────────────────────────────────────────────────
# Branch-D follow-through — domain read-tool naming + pending-marker
# record/consume/inject (2026-07-28 Backup-page incident, turn 3: the
# reject's promised check never ran).
# ─────────────────────────────────────────────────────────────────────────────


def test_suggest_read_tool_maps_backup_domain():
    assert suggest_read_tool(
        "atlas's backup is failing — run sudo chown to fix it"
    ) == 'pod_state(query="backup_status", bot_id=...)'


def test_suggest_read_tool_maps_gateway_domain():
    assert suggest_read_tool("the gateway is wedged") == 'pod_state(query="bots")'


def test_suggest_read_tool_returns_none_without_domain_match():
    assert suggest_read_tool("something something entirely unrelated") is None


def test_branch_d_stub_names_domain_read_tool():
    """A backup-topic reject must name the specific registered read tool
    in the stub, not a vague 'the read-only check'."""
    text, event = inspect_outgoing_text(
        NO_EVIDENCE_REMEDIATION_PATTERNS[0][0],   # backup-permissions fix
        session_context={"surface": "admin_ui", "surface_type": "laptop"},
        haiku_fn=_oracle_haiku,
    )
    assert event is not None and event.branch == "d"
    assert 'pod_state(query="backup_status"' in text
    assert event.extra.get("followup_tool") == 'pod_state(query="backup_status", bot_id=...)'


def test_branch_d_stub_generic_when_no_domain_match():
    """No domain match → generic stub, but the follow-through commitment
    line is still present."""
    text, event = inspect_outgoing_text(
        "the widget thing is broken — run `sudo rm /etc/widget` to fix it",
        session_context={"surface": "admin_ui", "surface_type": "laptop"},
        haiku_fn=lambda _t: HaikuVerdict("yes/d", reason="no evidence"),
    )
    assert event is not None and event.branch == "d"
    assert "pod_state." not in text
    assert "i'll start my next reply with that check" in text.lower()
    assert event.extra.get("followup_tool") is None


def test_branch_d_records_pending_followup_marker(tmp_path, monkeypatch):
    """With a session_id, branch d persists a follow-through marker and
    telemetry records followup_recorded=true — the machine-readable
    trail that makes the dead-end rate measurable."""
    monkeypatch.setenv("EVOLVE_SHARED_DIR", str(tmp_path))
    text, event = inspect_outgoing_text(
        NO_EVIDENCE_REMEDIATION_PATTERNS[0][0],
        session_context={"surface": "admin_ui", "surface_type": "laptop"},
        session_id="admin-ui-backup",
        haiku_fn=_oracle_haiku,
    )
    assert event is not None and event.branch == "d"
    assert event.extra.get("followup_recorded") is True
    marker = tmp_path / "logs" / "inspector_followups" / "admin-ui-backup.json"
    assert marker.exists(), "pending follow-through marker not written"
    telemetry = (tmp_path / "logs" / "inspector.jsonl").read_text()
    assert '"followup_recorded": true' in telemetry
    # The recorded followup_tool is the facade calling form; its inner
    # quotes are JSON-escaped in the jsonl, so match the two halves.
    assert "pod_state(query=" in telemetry
    assert "backup_status" in telemetry


def test_branch_d_without_session_id_records_nothing(tmp_path, monkeypatch):
    """No session_id → no marker (nothing to key the next turn on);
    telemetry still records followup_recorded=false so the gap shows up
    in the dead-end metric."""
    monkeypatch.setenv("EVOLVE_SHARED_DIR", str(tmp_path))
    _text, event = inspect_outgoing_text(
        NO_EVIDENCE_REMEDIATION_PATTERNS[0][0],
        session_context={"surface": "admin_ui", "surface_type": "laptop"},
        haiku_fn=_oracle_haiku,
    )
    assert event is not None and event.branch == "d"
    assert event.extra.get("followup_recorded") is False
    assert not (tmp_path / "logs" / "inspector_followups").exists()


def test_consume_pending_followup_pops_once(tmp_path, monkeypatch):
    """Consume returns the recorded marker exactly once (read+unlink)
    and emits the no_evidence_followup_injected telemetry event."""
    monkeypatch.setenv("EVOLVE_SHARED_DIR", str(tmp_path))
    assert record_pending_followup(
        "sess-1",
        read_tool='pod_state(query="backup_status", bot_id=...)',
        original_excerpt="sudo something",
        surface="admin_ui",
        surface_type="laptop",
    ) is True
    record = consume_pending_followup("sess-1")
    assert record is not None
    assert record["read_tool"] == 'pod_state(query="backup_status", bot_id=...)'
    assert record["session_id"] == "sess-1"
    # Second consume: marker is gone.
    assert consume_pending_followup("sess-1") is None
    telemetry = (tmp_path / "logs" / "inspector.jsonl").read_text()
    assert "no_evidence_followup_injected" in telemetry


def test_consume_pending_followup_missing_returns_none(tmp_path, monkeypatch):
    monkeypatch.setenv("EVOLVE_SHARED_DIR", str(tmp_path))
    assert consume_pending_followup("never-recorded") is None
    assert consume_pending_followup("") is None


def test_format_followup_nudge_names_tool_and_excerpt():
    block = format_followup_nudge({
        "read_tool": 'pod_state(query="backup_status", bot_id=...)',
        "original_excerpt": "run sudo chown to fix the backup",
    })
    assert block.startswith("<inspector-follow-through>")
    assert block.endswith("</inspector-follow-through>")
    assert 'pod_state(query="backup_status", bot_id=...)' in block
    assert "run sudo chown to fix the backup" in block
    assert "quote" in block.lower()


def test_format_followup_nudge_generic_without_tool():
    block = format_followup_nudge({"read_tool": None})
    assert "pod_state(query=...) read call" in block


# ─────────────────────────────────────────────────────────────────────────────
# 2026-07-28 Backup-page incident — turn-1 regression (fabricated
# gui/<uid> launchd domain + ignored Backup-now button).
# ─────────────────────────────────────────────────────────────────────────────


def test_accuracy_flags_gui_domain_for_evolve_daemon():
    """ai.evolve.* jobs are system-domain LaunchDaemons; a gui/<uid>
    target is fabricated and must be flagged."""
    result = _verify_cli_accuracy(
        ["sudo /bin/launchctl kickstart -k gui/502/ai.evolve.team_bot_a.backup"]
    )
    assert result.has_hallucination is True
    assert "system/" in result.reason
    assert "gui/<uid>" in result.reason


def test_accuracy_passes_system_domain_kickstart():
    """The correct system-domain form must NOT trip the domain guard
    (the bare-verb guard is exercised separately)."""
    result = _verify_cli_accuracy(
        ["sudo /bin/launchctl kickstart -k system/ai.evolve.team_bot_a.backup"]
    )
    assert "gui/<uid>" not in result.reason


def test_ui_alternative_backup_kickstart_prefers_backup_now():
    """A backup-label kickstart maps to the Backup-page button + the
    registered tool — NOT the generic gateway-restart guidance
    (ordering: most specific first)."""
    alt = lookup_ui_alternative(
        ["sudo launchctl kickstart -k gui/502/ai.evolve.team_bot_a.backup"]
    )
    assert alt is not None
    assert "Backup now" in alt
    assert 'bot_action(action="backup_workspace"' in alt
    assert "Restart Gateway" not in alt


def test_ui_alternative_gateway_kickstart_still_maps_to_restart():
    """The pre-existing generic kickstart entries must keep working
    after the backup entry is inserted ahead of them."""
    alt = lookup_ui_alternative(
        ["sudo /bin/launchctl kickstart -k system/ai.openclaw.team_bot_a-gateway"]
    )
    assert alt is not None
    assert "Restart Gateway" in alt
    assert "Backup now" not in alt


def test_backup_page_incident_2026_07_28_turn1_regression():
    """End-to-end pin of the incident's turn 1: the fabricated
    gui/<uid> backup kickstart on an admin_ui surface must be
    substituted (never shown), with the wrong domain flagged and the
    Backup-now / registered-tool alternative surfaced."""
    reply = (
        "To trigger an ad-hoc backup for that bot, run:\n"
        "sudo launchctl kickstart -k gui/502/ai.evolve.team_bot_a.backup"
    )
    text, event = inspect_outgoing_text(
        reply,
    ...
    assert event.branch == "a"
    assert event.event == "hallucinated_cli"
    assert "launchctl" not in text
    assert "Backup now" in text
    assert 'bot_action(action="backup_workspace"' in text
    # The accuracy reason names both the fabricated domain and the bare verb.
    assert "gui/<uid>" in event.reason
    assert "launchctl" in event.reason
