...
def test_integrations_keys_pack_points_at_existing_tools(html: str):
    ...
    window = _locate_ik_pack(html)
    for tool_name in (
        # Reads — config.bot is the source-of-truth for plugins / mcp /
        # hooks / embeddings overrides. Without it the pack tells evo
        # nothing about WHERE the underlying data lives.
        "config.bot",
