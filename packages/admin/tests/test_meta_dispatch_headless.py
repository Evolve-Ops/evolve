...
def _load():
    """The ONE `meta_dispatch_headless` object for the whole test session.

    Reuse before re-exec, and the reuse is the load-bearing half. This module is
    a plain importable `.py`, so anything that puts `tools/` on `sys.path` gets it
    through a normal `import` — including `meta_rig_preflight`, which binds
    `mdh` at ITS import and keeps that reference for the life of the process.
    Unconditionally re-execing here REPLACED the `sys.modules` entry with a
    SECOND object while `mrp.mdh` still pointed at the first, so
    `monkeypatch.setattr(MOD.mdh, "probe_background_login", ...)` patched one copy
    while `mrp.rig_preflight` called the other. The probe then ran for real and the
    test's spy was never touched.

    Whether that happened at all depended on which test file was imported first —
    i.e. on alphabetical collection order and on how a shard happened to be
    partitioned. It broke three `probe_login` tests on 2026-09-23 when an
    unrelated new test file (`test_derived_from_pr.py`) sorted ahead of this one
    and imported `meta_rig_preflight` early. Nothing about those tests, this
    module, or that file was wrong; only the number of copies was.
    """
    existing = sys.modules.get("meta_dispatch_headless")
    if existing is not None and getattr(existing, "__file__", None) == str(_MOD):
        return existing
...
# ── the launcher breaker: availability that can report its own failure ───────
#
# 2026-09-13: four briefs routed headless in one day, every one of them producing no
# commit, no branch and not one byte written into its own worktree, while `check_prereqs`
# reported available throughout — correctly, because it answers permission and disclaimer
# and neither of those is "does a launched session do work".
# `internal/finding-headless-launch-is-0-for-4-2026-09-13.md`.


def test_one_stalled_session_closes_the_launcher():
    """One is enough. A launcher is not a thing that half works, and the cost of being
    wrong is a card the operator taps — throughput the lane has actually been getting."""
    ok, detail = mdh.launcher_state([mdh.ALIVE, mdh.STALLED])
    assert ok is False
    assert "1 in-flight headless session(s) are STALLED" in detail


def test_a_launcher_with_nothing_stalled_stays_open():
    ok, detail = mdh.launcher_state([mdh.ALIVE, mdh.ALIVE])
    assert ok is True and detail == ""


def test_no_headless_entries_at_all_never_trips_the_breaker():
    """A missing fact is not a negative fact — the same direction `classify_liveness`
    already fails in. An empty lane is silence about the launcher, not evidence."""
    ok, detail = mdh.launcher_state([])
    assert ok is True and detail == ""


def test_the_breaker_detail_names_the_finding_and_the_way_out():
    """The operator reads this line in the tick summary and must learn three things from
    it alone: what tripped, that it clears itself, and where the measurement is."""
    _, detail = mdh.launcher_state([mdh.STALLED])
    assert "clear or relaunch them and routing restores itself" in detail
    assert "finding-headless-launch-is-0-for-4-2026-09-13" in detail


def test_the_breaker_is_a_named_missing_prerequisite():
    """It rides the existing `missing` vocabulary so every reader that renders the
    prerequisite fallback learns about it without changing."""
    assert mdh.MISSING_STALLED == "stalled-launcher"
    assert mdh.MISSING_STALLED not in (mdh.MISSING_GRANT, mdh.MISSING_DISCLAIMER)


# ── probe_background_login (D-TP1) ────────────────────────────────────────────
#
# check_prereqs's own docstring says a probe is the wrong tool for the DISCLAIMER
# question — it changes once, ever, and a file read answers it for free. Login is the
# opposite shape: a session token that expires on its own clock, invisibly to a file
# read. These pin the three verdicts a real probe can reach, never a fourth.
...
def _probe_runner(bg_stdout, agents_rows_by_poll):
    """A runner: the first `claude --bg` call returns `bg_stdout`; each subsequent
    `claude agents --json` call pops the next entry off `agents_rows_by_poll` (a list of
    lists), repeating the last one once exhausted."""
    calls = {"agents": 0}

    def run(argv, cwd):
        if argv[:2] == ["claude", "--bg"]:
            return _FakeProc(0, bg_stdout)
        if argv[:2] == ["claude", "agents"]:
            i = min(calls["agents"], len(agents_rows_by_poll) - 1)
            calls["agents"] += 1
            return _FakeProc(0, json.dumps(agents_rows_by_poll[i]))
        return _FakeProc(9, "", "unexpected call")
    return run


def test_probe_ok_when_the_session_completes_with_the_reply():
    run = _probe_runner("backgrounded · 9988776\n",
                        [[{"id": "9988776", "status": "completed", "result": "PROBE_OK"}]])
    state, detail = mdh.probe_background_login(runner=run, sleep=lambda s: None,
                                               clock=lambda: 0)
    assert state == mdh.LOGIN_OK
    assert "PROBE_OK" in detail or "expected reply" in detail


def test_probe_login_expired_on_blocked_state():
    run = _probe_runner("backgrounded · 9988776\n",
                        [[{"id": "9988776", "status": "idle", "state": "blocked"}]])
    state, detail = mdh.probe_background_login(runner=run, sleep=lambda s: None,
                                               clock=lambda: 0)
    assert state == mdh.LOGIN_EXPIRED


def test_probe_login_expired_on_a_login_prompt_sentence():
    run = _probe_runner(
        "backgrounded · 9988776\n",
        [[{"id": "9988776", "status": "idle", "state": "idle",
           "last_message": "Login expired · Please run /login"}]])
    state, detail = mdh.probe_background_login(runner=run, sleep=lambda s: None,
                                               clock=lambda: 0)
    assert state == mdh.LOGIN_EXPIRED


def test_probe_unknown_when_the_session_never_appears():
    ticks = iter([0, 61])
    run = _probe_runner("backgrounded · 9988776\n", [[]])
    state, detail = mdh.probe_background_login(
        runner=run, sleep=lambda s: None, clock=lambda: next(ticks, 61))
    assert state == mdh.LOGIN_UNKNOWN
    assert "never appeared" in detail


def test_probe_unknown_when_launch_itself_fails():
    def run(argv, cwd):
        if argv[:2] == ["claude", "--bg"]:
...
def test_probe_unknown_when_the_launch_banner_has_no_parseable_id():
    def run(argv, cwd):
        if argv[:2] == ["claude", "--bg"]:
            return _FakeProc(0, "Starting background service...\n")
        return _FakeProc(9)
    state, detail = mdh.probe_background_login(runner=run, sleep=lambda s: None,
                                               clock=lambda: 0)
    assert state == mdh.LOGIN_UNKNOWN
...
def test_probe_unknown_when_claude_agents_errors_every_poll():
    def run(argv, cwd):
        if argv[:2] == ["claude", "--bg"]:
            return _FakeProc(0, "backgrounded · 9988776\n")
        if argv[:2] == ["claude", "agents"]:
            return _FakeProc(1, "", "agents unavailable")
        return _FakeProc(9)
    ticks = iter([0, 61])
    state, detail = mdh.probe_background_login(
        runner=run, sleep=lambda s: None, clock=lambda: next(ticks, 61))
...
def test_probe_never_returns_ok_on_an_unrecognised_shape():
    """The fail-safe direction: a schema this parser does not recognise is UNKNOWN, never
    OK — the same rule `check_prereqs` already enforces for its own two prerequisites."""
    run = _probe_runner("backgrounded · 9988776\n",
                        [[{"id": "9988776", "status": "some-future-status"}]])
    ticks = iter([0, 61])
    state, detail = mdh.probe_background_login(
        runner=run, sleep=lambda s: None, clock=lambda: next(ticks, 61))
    assert state == mdh.LOGIN_UNKNOWN
...
def test_probe_uses_the_real_launch_argv():
    """The whole point named in the brief: NOT a `claude -p` echo. It must call through
    `build_argv`, the same positional-prompt form the real headless launch uses."""
    seen = []

    def run(argv, cwd):
        seen.append(argv)
        if argv[:2] == ["claude", "--bg"]:
            return _FakeProc(0, "backgrounded · 1234567\n")
        return _FakeProc(0, json.dumps(
            [{"id": "1234567", "status": "completed", "result": "PROBE_OK"}]))

    mdh.probe_background_login(runner=run, sleep=lambda s: None, clock=lambda: 0)
    bg_call = seen[0]
    assert bg_call == mdh.build_argv(mdh.PROBE_PROMPT)
    assert "-p" not in bg_call


# ── one module object per tools module (2026-09-23) ──────────────────────────


def test_this_file_did_not_fork_the_shared_module():
    """`meta_rig_preflight` binds `mdh` at ITS import and keeps that reference, so
    a second `meta_dispatch_headless` object means `monkeypatch.setattr(mdh, ...)`
    patches a copy nobody calls.

    That is not hypothetical: it silently broke three `probe_login` tests in
    `test_meta_dispatch_move.py` on 2026-09-23, and only in CI shard 3/4 — an
    unrelated new test file sorted alphabetically ahead of this one, imported
    `meta_rig_preflight` first, and the unconditional re-exec here then replaced
    the object `mrp` was already holding. The tests that failed were correct; the
    number of copies was not.
    """
    # `tools/` may not be on sys.path when this file runs alone (it loads by
    # absolute path), and the invariant is about the REAL import chain — so put it
    # there and import mrp the way production does.
    if str(_MOD.parent) not in sys.path:
        sys.path.insert(0, str(_MOD.parent))
    import meta_rig_preflight as mrp

    assert mdh is sys.modules["meta_dispatch_headless"]
    assert mrp.mdh is mdh, (
        "meta_rig_preflight holds a DIFFERENT meta_dispatch_headless object than "
        "sys.modules — a patch applied to one will not be seen by the other")


def test_no_test_file_clobbers_a_shared_tools_module():
    """The class guard: a test may re-exec a tools module under the canonical name
    only if it REUSES an already-loaded one first.

    The hazard needs two things, and this checks for exactly that pair: the module
    is importable by name from `tools/`, and some other tools module imports it (so
    a stale reference can exist). Loading an EXTENSIONLESS script is not covered —
    nothing can hold a by-name reference to one — and loading under a unique pin
    name (`..._pr_parse_pin`, as `test_meta_dispatch_integrity.py` does) is the
    other sanctioned pattern, because it never touches the canonical entry.

    Static, so it answers the same way in every shard — the runtime check above can
    only fire when both modules happen to be loaded in the same process.
    """
    tools = Path(__file__).resolve().parents[3] / "tools"
    tools_src = "\n".join(
        p.read_text(encoding="utf-8", errors="replace")
        for p in sorted(tools.iterdir())
        if p.is_file() and (p.suffix == ".py" or p.name.startswith("meta-"))
    )
    offenders = []
    for test_file in sorted(Path(__file__).resolve().parent.glob("test_*.py")):
        text = test_file.read_text(encoding="utf-8", errors="replace")
        for name in re.findall(r'sys\.modules\[\s*"([A-Za-z_][A-Za-z0-9_]*)"\s*\]\s*=', text):
            if not (tools / f"{name}.py").is_file():
                continue                      # extensionless script — not bindable
            if f"import {name}" not in tools_src:
                continue                      # nothing else holds a reference
            if f'sys.modules.get("{name}")' in text:
                continue                      # reuses before re-execing — safe
            offenders.append("%s re-execs %s under its canonical name"
                             % (test_file.name, name))
    assert not offenders, (
        "a shared tools module must be reused, not re-exec'd:\n  "
        + "\n  ".join(offenders)
        + "\n(reuse it: `sys.modules.get(<name>)` when its __file__ matches, else "
          "load under a unique pin name)")
