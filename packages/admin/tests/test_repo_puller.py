...
@pytest.fixture(autouse=True)
def _forbid_real_sweep_reexec(monkeypatch):
    """The sweep re-exec runs `evolve-admin repo-redeploy-sweep`,
    which on a real box redeploys the whole fleet. A test that reaches it
    without stubbing subprocess.run must fail loudly, not silently spawn.
    Tests that exercise the re-exec stub `repo_puller.subprocess.run`
    themselves, which replaces this guard for their duration."""
    real_run = repo_puller.subprocess.run

    def guarded(argv, **kw):
        if any(repo_puller.SWEEP_ONLY_COMMAND in str(a) for a in (argv or [])):
            # pytest.fail raises an OutcomeException (a BaseException), NOT an
            # Exception — _reexec_lagging_sweep catches `except Exception` to
            # keep a spawn failure from breaking the pull, and a plain
            # AssertionError here would be swallowed into a generic
            # "could not start" instead of naming the real problem.
            pytest.fail(
                f"test spawned a REAL lagging-bot redeploy sweep: {argv}",
                pytrace=False,
            )
        return real_run(argv, **kw)

    monkeypatch.setattr(repo_puller.subprocess, "run", guarded)
...
# ── _repo_puller_verdict() — D-CS7 subject naming ──────────────────────────
#
# known_good.json / known_bad.json under
# tests/fixtures/controls/health__check_repo_puller_freshness/ are this
# control's registered fixture pair (tools/control-registry); these two
# tests are what actually replays them.

_REPO_ROOT = Path(__file__).resolve().parents[3]
_PULLER_FIXTURE_DIR = (
    _REPO_ROOT / "tests" / "fixtures" / "controls" / "health__check_repo_puller_freshness"
)


def _load_puller_fixture(tmp_path: Path, name: str) -> Path:
    payload = json.loads((_PULLER_FIXTURE_DIR / name).read_text())
    log = _write_log(tmp_path / "repo-puller.log", "\n".join(payload["log_lines"]) + "\n")
    import os
    old = log.stat().st_mtime - payload["age_seconds"]
    os.utime(log, (old, old))
    return log


def test_repo_puller_freshness_reports_the_run_id_it_judged(tmp_path: Path):
    """A healthy tick's Verdict must name the exact log file + mtime it
    read as its subject — not just assert "fine" with nothing behind it."""
    from evolve_admin import health
    log = _load_puller_fixture(tmp_path, "known_good.json")
    mtime = log.stat().st_mtime

    v = health._repo_puller_verdict(log)
    assert v.is_ok
    assert v.subject == f"{log}@{int(mtime)}"
    assert "age_seconds" in v.evidence


def test_known_bad_fixture_must_produce_broken(tmp_path: Path):
    from evolve_admin import health
    log = _load_puller_fixture(tmp_path, "known_bad.json")
    v = health._repo_puller_verdict(log)
    assert v.is_broken
    assert v.subject is not None  # the wedged log IS the subject, not nothing
    assert "Permission denied" in v.evidence["last_error"]


def test_repo_puller_verdict_is_unknown_not_ok_when_log_is_missing(tmp_path: Path):
    """The empty-source case (D-CS7's core failure mode): no log at all
    must never read as 'ok' — the control never looked at anything."""
    from evolve_admin import health
    v = health._repo_puller_verdict(tmp_path / "does-not-exist.log")
    assert v.is_unknown
...
def test_pull_kickstarts_bot_gateways_after_plugin_rebuild(
    ...
    with patch.object(repo_puller, "_git", fake), \
        ...
        result = repo_puller.pull(
            ...
            gateway_roster_fn=lambda: {"members": ["evolve", "team_bot_a", "admin_bot"]},
...
def test_pull_reconciles_nonmember_gateway_out_of_restart_set(
    tmp_path: Path, monkeypatch
):
    """A stale ai.openclaw.<non-member>-gateway plist discovered on disk is
    reconciled OUT of the restart set against network['members'] (∪ resolved
    primary) — only roster members + the primary get kickstarted, the residue
    is recorded in bot_gateways_skipped_nonmember and never kicked. macOS
    sibling of EVO-GATEWAY-RESIDUE-RERENDER (PR #3279); complementary to the
    #3167 orphan-sweeper, which REMOVES such units."""
    monkeypatch.setenv(repo_puller.AUTO_RESTART_ENV, "1")
    before, after = "a" * 40, "b" * 40
    fake = _make_git_runner({
        "rev-parse": [(0, before, ""), (0, after, "")],
    ...
    # Roster: primary "evo" + member "darwin"; "oldbot" is residue.
    roster = {"primary": "evo", "members": ["darwin"],
              "bots": {"evo": {"role": "primary"}, "darwin": {}}}
    ...
    with patch.object(repo_puller, "_git", fake), \
        ...
        result = repo_puller.pull(
            ...
            kickstart_fn=fake_kick,
            gateway_discovery_fn=fake_discover,
            gateway_roster_fn=lambda: roster,
            gateway_sleep_fn=lambda _: None,
        )

    assert result.success is True
    # The non-member was never kicked; members + primary were.
    assert kicks == ["ai.openclaw.evo-gateway", "ai.openclaw.darwin-gateway"]
    assert result.bot_gateways_restarted == [
        "ai.openclaw.evo-gateway", "ai.openclaw.darwin-gateway",
    ]
    assert result.bot_gateways_skipped_nonmember == ["ai.openclaw.oldbot-gateway"]
    assert any("skipping 1 non-member gateway" in s for s in result.steps)
...
def test_pull_records_gateway_kickstart_failures_without_failing_pull(
    ...
    with patch.object(repo_puller, "_git", fake), \
        ...
        result = repo_puller.pull(
            ...
            kickstart_fn=fake_kick,
            gateway_discovery_fn=fake_discover,
            gateway_roster_fn=lambda: {"members": ["team_bot_a", "admin_bot"]},
            gateway_sleep_fn=lambda _: None,
        )
...
def test_pull_gateway_kickstart_exception_does_not_crash_pull(
    ...
    with patch.object(repo_puller, "_git", fake), \
        ...
        result = repo_puller.pull(
            ...
            kickstart_fn=boom,
            gateway_discovery_fn=fake_discover,
            gateway_roster_fn=lambda: {"members": ["team_bot_a"]},
            gateway_sleep_fn=lambda _: None,
        )
...
# ── Convergence: the stamp a successful redeploy writes must make the next
# sweep skip the bot. Two things used to break that — the display-string
# comparison, and comparing against a version the checkout has moved past.


def _write_install_json_with_identity(shared_dir: Path, bot_versions: dict[str, dict]):
    """Helper: install.json whose per-bot entries carry sha/commit_count."""
    import json as _json
    (shared_dir / "install.json").write_text(_json.dumps({
        "bot_versions": bot_versions,
    }))


def test_find_lagging_bots_redeploys_a_bot_that_is_ahead_of_the_checkout():
    """`release rollback` force-moves the checkout BACKWARD and then runs this
    sweep (via run_hooks_only) to bring the fleet down onto stable. A bot on a
    newer commit than the checkout is drift, not an exemption — skipping it
    would leave the fleet on the code the operator just rolled back from."""
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        shared = Path(td)
        _write_install_json_with_identity(shared, {
            "team_bot_a": {"version": "2026.0818.3683", "sha": "a" * 40,
                           "commit_count": 5441},   # ahead — rolled back from
            "team_bot_c": {"version": "2026.0818.3683", "sha": "c" * 40,
                           "commit_count": 5439},   # behind
            "team_bot_b": {"version": "2026.0818.3661", "sha": "b" * 40,
                           "commit_count": 5440},   # on the checkout — skip
        })
        lagging = repo_puller._find_lagging_bots(
            shared, "2026.0818.3661",
            current_sha="b" * 40, current_count=5440,
        )
        assert sorted(b for b, _ in lagging) == ["team_bot_a", "team_bot_c"]


def test_find_lagging_bots_catches_same_version_string_different_commit():
    """Two direct pushes on the same day both render YYYY.MMDD.0, so the old
    string equality read them as in-sync and skipped a real redeploy."""
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        shared = Path(td)
        _write_install_json_with_identity(shared, {
            "team_bot_a": {"version": "2026.0818.0", "sha": "a" * 40,
                           "commit_count": 5439},
        })
        lagging = repo_puller._find_lagging_bots(
            shared, "2026.0818.0",
        ...
        assert [b for b, _ in lagging] == ["team_bot_a"]


def test_find_lagging_bots_without_identity_matches_legacy_string_equality():
    """Callers with no git identity (and legacy stamps carrying no sha) keep
    the pre-identity behaviour exactly."""
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        shared = Path(td)
        _write_install_json(shared, "2026.0603.2067", {
            "team_bot_a": "2026.0601.2010",
            "team_bot_c": "2026.0603.2067",
        })
        lagging = repo_puller._find_lagging_bots(shared, "2026.0603.2067")
        assert [b for b, _ in lagging] == ["team_bot_a"]


def test_loaded_deploy_code_is_current_fails_open_without_git(tmp_path: Path):
    """A path with no resolvable commit identity can't PROVE staleness, so the
    guard must not block the sweep (this is the shape every unit test uses)."""
    ok, why = repo_puller._loaded_deploy_code_is_current(tmp_path)
    assert ok is True
    assert why == ""


def test_sweep_never_runs_INLINE_against_stale_deploy_code(
    tmp_path: Path, monkeypatch,
):
    """On a tick that advanced HEAD, deploy.EVOLVE_* is still the PRE-pull
    commit (cli.py imports deploy before pull() runs), so stamping from THIS
    process would mark the fleet current at a superseded version and the next
    tick would redeploy everything again.

    The in-process sweep must therefore never run. What used to follow —
    "so defer to the next tick" — is no longer the disposition: deferring
    converges only on a NO-OP tick, and a merge burst can withhold one
    indefinitely (#3705 → #3721). The work is handed to a FRESH process that
    imported the post-pull deploy, on this same tick. See
    test_stale_deploy_code_reexecs_the_sweep_instead_of_deferring.
    """
    from evolve_admin import deploy as _deploy

    monkeypatch.setattr(_deploy, "EVOLVE_COMMIT_SHA", "a" * 40, raising=False)
    monkeypatch.setattr(
        repo_puller._vsync, "compute_commit_identity",
        lambda repo, log=None: ("b" * 40, 5440),
    )

    def boom(*a, **kw):
        raise AssertionError("sweep must not run INLINE against stale deploy code")

    monkeypatch.setattr(repo_puller, "_redeploy_lagging_bots", boom)

    spawned: list[list[str]] = []
    monkeypatch.setattr(repo_puller.subprocess, "run", lambda argv, **kw: (
        spawned.append(argv)
    ...
    assert result.lagging_bots_redeployed == []
    assert spawned, "stale code neither swept inline nor re-exec'd — the work was dropped"
    assert repo_puller.SWEEP_ONLY_COMMAND in spawned[0]
    # The staleness reason still has to reach the operator, now via the child's
    # own log rather than a skip line.
    assert result.redeploy_sweep_skipped == ""


def test_sweep_runs_when_loaded_deploy_code_matches_the_checkout(
    tmp_path: Path, monkeypatch,
):
    """The mirror of the skip case — a fresh process must still sweep."""
    from evolve_admin import deploy as _deploy

    monkeypatch.setattr(_deploy, "EVOLVE_COMMIT_SHA", "a" * 40, raising=False)
    monkeypatch.setattr(
        repo_puller._vsync, "compute_commit_identity",
        lambda repo, log=None: ("a" * 40, 5440),
    )
    monkeypatch.setattr(
        repo_puller, "_redeploy_lagging_bots",
        lambda repo, shared_dir: (["team_bot_a"], {}),
    )

    result = repo_puller.PullResult(success=True)
    repo_puller._run_lagging_bot_redeploy_sweep(result, tmp_path, tmp_path)

    assert result.lagging_bots_redeployed == ["team_bot_a"]
    assert not any("skipped redeploy sweep" in s for s in result.steps)


def test_quiet_noop_tick_still_logs_a_redeploy_sweep():
    """The sweep converges on the tick AFTER an advance — a no-op tick. The
    old quiet-mode early return dropped every action line on such ticks, so
    most full-fleet redeploys never reached the log at all."""
    out = repo_puller.format_for_log(
        repo_puller.PullResult(
            success=True, head_before="abc12345", head_after="abc12345",
            lagging_bots_redeployed=["team_bot_a", "team_bot_c"],
        ),
        quiet=True,
    )
    assert "redeployed 2 lagging bot(s)" in out
    assert "up to date at abc12345" in out


def test_quiet_noop_tick_with_no_action_still_logs_nothing():
    """...but the bare 15-minute no-op tick stays silent, which is the whole
    point of quiet mode."""
    out = repo_puller.format_for_log(
        repo_puller.PullResult(
            success=True, head_before="abc12345", head_after="abc12345",
...
# ── Retired per-bot job teardown: must NOT ride the lagging-bot sweep ──────
#
# The 2026-08-18 #3705 stall. Retiring ``ai.openclaw.evolve.apply.<bot>``
# deleted the script its units named, and teardown of the already-installed
# units lived only in ``deploy_bot`` step 6 — reachable on a hands-off pod
# only via the puller's lagging-bot redeploy sweep. That sweep is skipped on
# every HEAD-ADVANCING tick by ``_loaded_deploy_code_is_current`` (the tick
# process imported the pre-pull ``deploy`` module), so it converges only on a
# NO-OP tick. The retirement landed inside a run of six consecutive advancing
# ticks — merges kept arriving inside every 15-minute window — so no no-op
# tick ever came, and 9 launchd plists plus 2 systemd units went on firing
# against a deleted file until an operator ran ``deploy --all`` by hand.


def _stub_advancing_tick_git(before: str, after: str):
    return _make_git_runner({
        "rev-parse": [(0, before, ""), (0, after, "")],
        "pull": (0, "Updating ...", ""),
        "log": (0, "abc1234 retire the apply daemon (#3705)", ""),
        "diff": (0, "docs/CHANGELOG.md", ""),
    })


def test_retired_job_teardown_runs_on_a_tick_that_skips_the_lagging_sweep(tmp_path: Path):
    """THE regression. On an advancing tick the lagging-bot sweep is skipped,
    and teardown must happen anyway.

    Before the fix this was the only tick shape that ever ran during the
    stall, and it tore down nothing.
    """
    seen: list[Path] = []

    def fake_lagging(result, repo, shared_dir):
        # Exactly what the real guard does on every advancing tick.
        result.redeploy_sweep_skipped = "loaded deploy code is aaaaaaaa but the checkout is now bbbbbbbb"

    def fake_sweep(shared_dir, **kw):
        seen.append(shared_dir)
        return ["ai.openclaw.evolve.apply.team_bot_a"], {}

    with patch.object(repo_puller, "_git", _stub_advancing_tick_git("a" * 40, "b" * 40)), \
         patch.object(Path, "exists", return_value=True), \
         patch.object(repo_puller, "_run_lagging_bot_redeploy_sweep", fake_lagging), \
         patch("evolve_admin.retired_jobs.sweep_pod", fake_sweep):
        result = repo_puller.pull(repo=tmp_path, shared_dir=tmp_path)

    assert seen, (
        "retired-job teardown did not run on a HEAD-advancing tick — it is "
        "still gated behind the lagging-bot sweep, which skips every "
        "advancing tick (the #3705 stall)"
    )
    assert result.retired_jobs_removed == ["ai.openclaw.evolve.apply.team_bot_a"]


def test_retired_job_teardown_also_runs_on_a_no_op_tick(tmp_path: Path):
    """The no-op tick is the shape that never came during the stall; it must
    still sweep, so a pod whose puller only ever no-ops converges too."""
    calls: list[Path] = []
    sha = "c" * 40
    ...
    with patch.object(repo_puller, "_git", fake), \
         patch.object(Path, "exists", return_value=True), \
         patch.object(repo_puller, "_run_lagging_bot_redeploy_sweep", lambda *a, **k: None), \
         patch("evolve_admin.retired_jobs.sweep_pod", fake_sweep):
        repo_puller.pull(repo=tmp_path, shared_dir=tmp_path)

    assert calls, "retired-job teardown skipped on a no-op tick"


def test_retired_job_teardown_failure_never_breaks_the_pull(tmp_path: Path):
    ...
    with patch.object(repo_puller, "_git", _stub_advancing_tick_git("a" * 40, "b" * 40)), \
         patch.object(Path, "exists", return_value=True), \
         patch.object(repo_puller, "_run_lagging_bot_redeploy_sweep", lambda *a, **k: None), \
         patch("evolve_admin.retired_jobs.sweep_pod", boom):
        result = repo_puller.pull(repo=tmp_path, shared_dir=tmp_path)

    assert result.success is True
    assert "RuntimeError" in result.retired_job_errors["__sweep__"]


# ── Log visibility: a deferred sweep must leave a trace ────────────────────


def test_format_for_log_surfaces_a_skipped_redeploy_sweep():
    """The skip reason used to go only to ``result.steps``, which
    ``format_for_log`` never reads — so a sweep that deferred on EVERY tick
    was indistinguishable in repo-puller.log from one that ran and found
    nothing lagging. That blind spot is what forced the #3705 stall to be
    diagnosed by hand.
    """
    r = repo_puller.PullResult(
        success=True, head_before="a" * 40, head_after="b" * 40,
    )
    r.redeploy_sweep_skipped = "loaded deploy code is aaaaaaaa but the checkout is now bbbbbbbb"
    out = repo_puller.format_for_log(r, quiet=True)
    assert "redeploy sweep skipped" in out, out
    assert "aaaaaaaa" in out


def test_format_for_log_surfaces_retired_job_removals():
    r = repo_puller.PullResult(success=True, head_before="a" * 40, head_after="a" * 40)
    r.retired_jobs_removed = ["ai.openclaw.evolve.apply.team_bot_a"]
    out = repo_puller.format_for_log(r, quiet=True)
    assert "removed 1 retired per-bot job(s)" in out
    assert "ai.openclaw.evolve.apply.team_bot_a" in out


def test_format_for_log_stays_quiet_on_a_bare_no_op_tick():
    """The new lines must not un-suppress the routine tick — otherwise the
    daemon log grows a line every 15 minutes."""
    r = repo_puller.PullResult(success=True, head_before="a" * 40, head_after="a" * 40)
    assert repo_puller.format_for_log(r, quiet=True) == ""


# ── Sweep starvation: an advancing tick must converge, not defer ───────────
#
# `_loaded_deploy_code_is_current` is false on EVERY head-advancing tick (the
# tick process imported `deploy` before its own pull), so the sweep used to
# converge only on a NO-OP tick. On 2026-08-18 six consecutive ticks advanced
# with no no-op tick between them and the sweep never ran at all. The fix
# hands the sweep to a fresh process that imported the post-pull `deploy`,
# so it converges on the same tick that pulled.


def test_stale_deploy_code_reexecs_the_sweep_instead_of_deferring(tmp_path: Path):
    """THE regression. A tick whose loaded deploy code is stale must still
    sweep — via a fresh process — not defer to a tick that may never come."""
    spawned: list[list[str]] = []

    def fake_run(argv, **kw):
        spawned.append(argv)
        return subprocess.CompletedProcess(
            argv, 0, stdout="[repo-puller] redeployed 9 lagging bot(s): a, b\n", stderr="")

    r = repo_puller.PullResult(success=True)
    with patch.object(repo_puller, "_loaded_deploy_code_is_current",
                      return_value=(False, "loaded deploy code is aaaaaaaa but the checkout is now bbbbbbbb")), \
         patch.object(repo_puller.subprocess, "run", fake_run):
        repo_puller._run_lagging_bot_redeploy_sweep(r, tmp_path, tmp_path)

    assert spawned, (
        "a stale-code tick did not re-exec the sweep — it is still deferring "
        "to a no-op tick, which a merge burst can starve indefinitely"
    )
    assert repo_puller.SWEEP_ONLY_COMMAND in spawned[0]
    assert "redeployed 9 lagging bot(s)" in r.lagging_sweep_reexec
    assert r.redeploy_sweep_skipped == "", (
        "the tick reported a skip even though the fresh-process sweep ran"
    )


def test_reexec_is_not_attempted_when_the_loaded_code_is_already_current(tmp_path: Path):
    """A no-op tick sweeps inline — spawning a process for it would double the
    cost of the common case for nothing."""
    spawned: list[list[str]] = []
    r = repo_puller.PullResult(success=True)
    with patch.object(repo_puller, "_loaded_deploy_code_is_current", return_value=(True, "")), \
         patch.object(repo_puller.subprocess, "run",
                      lambda argv, **kw: spawned.append(argv)), \
         patch.object(repo_puller, "_redeploy_lagging_bots", return_value=([], {})):
        repo_puller._run_lagging_bot_redeploy_sweep(r, tmp_path, tmp_path)

    assert spawned == []
    assert r.lagging_sweep_reexec == ""


def test_the_child_never_spawns_a_grandchild(tmp_path: Path):
    """`allow_reexec=False` is the recursion stop. A child that somehow finds
    ITSELF stale (the checkout moved again under it) must skip the old way —
    a self-spawning sweep on a live pod is a fleet-redeploy loop."""
    spawned: list[list[str]] = []
    with patch.object(repo_puller, "_loaded_deploy_code_is_current",
                      return_value=(False, "still stale")), \
         patch.object(repo_puller.subprocess, "run",
                      lambda argv, **kw: spawned.append(argv)):
        r = repo_puller.run_redeploy_sweep_only(tmp_path, tmp_path)

    assert spawned == [], "the sweep child spawned a grandchild"
    assert "still stale" in r.redeploy_sweep_skipped


def test_reexec_failure_degrades_to_the_old_skip(tmp_path: Path):
    """A spawn/timeout/non-zero child must never break the pull, and must
    leave the pre-fix behaviour (defer + say so) rather than a silent no-op."""
    for failure in (
        subprocess.TimeoutExpired(cmd="x", timeout=1800),
        ...
        with patch.object(repo_puller, "_loaded_deploy_code_is_current",
             ...
             patch.object(repo_puller.subprocess, "run", boom):
            repo_puller._run_lagging_bot_redeploy_sweep(r, tmp_path, tmp_path)
        assert r.lagging_sweep_reexec_error, failure
        assert r.redeploy_sweep_skipped, (
            f"{failure!r}: re-exec failed but the tick reported no deferral — "
            "the old skip line is the required fallback"
        )


def test_reexec_reports_a_non_zero_child(tmp_path: Path):
    r = repo_puller.PullResult(success=True)
    with patch.object(repo_puller, "_loaded_deploy_code_is_current",
                      return_value=(False, "stale")), \
         patch.object(repo_puller.subprocess, "run",
                      lambda argv, **kw: subprocess.CompletedProcess(
                          argv, 3, stdout="", stderr="deploy_bot blew up")):
        repo_puller._run_lagging_bot_redeploy_sweep(r, tmp_path, tmp_path)

    assert "exited 3" in r.lagging_sweep_reexec_error
    assert "deploy_bot blew up" in r.lagging_sweep_reexec_error
    assert r.redeploy_sweep_skipped


def test_reexec_argv_resolves_beside_the_running_interpreter(tmp_path: Path):
    """Must not use release_manager.DEFAULT_EVOLVE_ADMIN — that is a hardcoded
    /Users/Shared literal and simply wrong on a Linux pod. sys.executable's
    sibling is by construction the venv this code was imported from."""
    import sys as _sys
    argv, why = repo_puller._sweep_reexec_argv(tmp_path)
    assert why == ""
    assert argv[1:] == [repo_puller.SWEEP_ONLY_COMMAND, "--quiet",
                        "--repo", str(tmp_path)] or argv[1] == "-m"
    assert not argv[0].startswith("/Users/Shared/"), argv
    assert str(Path(_sys.executable).parent) in argv[0] or argv[1] == "-m"


def test_reexec_argv_falls_back_to_dash_m_when_no_console_script(tmp_path: Path):
    with patch.object(Path, "exists", return_value=False):
        argv, why = repo_puller._sweep_reexec_argv(tmp_path)
    assert why == ""
    assert argv[1:3] == ["-m", "evolve_admin.cli"], argv
    assert repo_puller.SWEEP_ONLY_COMMAND in argv


def test_format_for_log_surfaces_the_fresh_process_sweep():
    r = repo_puller.PullResult(success=True, head_before="a" * 40, head_after="b" * 40)
    r.lagging_sweep_reexec = "[repo-puller] redeployed 9 lagging bot(s): a, b"
    out = repo_puller.format_for_log(r, quiet=True)
    assert "fresh process" in out
    assert "redeployed 9 lagging bot(s)" in out


def test_format_for_log_surfaces_a_failed_fresh_process_sweep():
    r = repo_puller.PullResult(success=True, head_before="a" * 40, head_after="b" * 40)
    r.lagging_sweep_reexec_error = "fresh-process redeploy sweep timed out (1800s)"
    out = repo_puller.format_for_log(r, quiet=True)
    assert "WARN post-pull redeploy sweep" in out
