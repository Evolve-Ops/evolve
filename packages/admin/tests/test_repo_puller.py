...
def test_sudoers_drift_signal_firing(tmp_path: Path):
    """True iff a sudoers_refresh_failed signature is in the signature index."""
    sig_dir = tmp_path / "signals"
    sig_dir.mkdir(parents=True, exist_ok=True)
    idx = sig_dir / "signature_index.json"
    assert repo_puller._sudoers_drift_signal_firing(tmp_path) is False  # no index
    idx.write_text(json.dumps({"some_other:thing:x": "id1"}))
    assert repo_puller._sudoers_drift_signal_firing(tmp_path) is False
    idx.write_text(json.dumps({"repo_puller_sudoers:sudoers_refresh_failed:evolve": "id2"}))
