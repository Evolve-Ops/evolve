...
def test_picker_renders_already_added_for_present_suggested(js: str):
    code = _strip_comments(js)
    # A recommended id that's ALSO already picked renders an "already"-marker
    # option (still starred), not excluded. The parenthetical is the caller's
    # `alreadyNote` when it supplies one; "already added" is the fallback for
    # callers that don't (the catalog picker).
    assert "'already added'" in code
    assert re.search(
        r"isSug && isAdded\) label = `★ \$\{label\} "
        r"\(\$\{alreadyNote \? alreadyNote\(id\) : 'already added'\}\)`",
        code,
