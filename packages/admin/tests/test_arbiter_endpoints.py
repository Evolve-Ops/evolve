...
def test_transition_records_body_actor_and_reason(arbiter_app):
    """7.1 C2: evo's fail-closed tools route writes through these
    endpoints, passing actor/reason in the body so the audit history
    still attributes the action to evo instead of 'user'."""
...
def test_transition_actor_defaults_to_user(arbiter_app):
    ...
    with app.test_client() as c:
        resp = c.post(f"/api/arbiter/proposals/{p.id}/dismiss")
        assert resp.status_code == 200
    on_disk = find_proposal(shared, p.id)[0]
    assert on_disk.history[-1].actor == "user"
