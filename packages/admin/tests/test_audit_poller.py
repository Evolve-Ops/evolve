...
# ── the drain order is a property of the records, not of the filesystem ──────
#
# Until 2026-09-18 `_list_outbox_files` sorted on `st_mtime` alone. A bot writes
# a run's records in one burst, so they routinely share a timestamp; Python's
# sort is stable, so equal keys left the order exactly as `iterdir()` produced
# it. `test_root_fully_drained_with_mixed_records` passed 1 run in 6 on the
# maintainer's machine and passed in CI — not because the two environments
# differ in kind, but because an unspecified order sometimes comes out right.
# These tests pin the order itself, so the next change to it fails here rather
# than in a counter three functions away.


def _same_mtime(*paths: Path) -> None:
    """Force every path to one timestamp — the burst the bot actually writes."""
    stamp = 1_700_000_000.0
    for p in paths:
        os.utime(p, (stamp, stamp))


def test_a_run_summary_is_drained_after_the_findings_it_summarises(
    tmp_root: Path,
) -> None:
    """The invariant the old docstring claimed and the old key did not enforce.

    A summary's sweep resolves every Signal whose signature it does not carry,
    so draining it BEFORE the findings of its own run resolves Signals those
    findings are about to re-observe — each one reopens instead of deduping.
    """
    outbox = _make_outbox(tmp_root, "team_bot_a")
    summary = _noop_run_summary(outbox, bot_id="team_bot_a", record_id="rec-hb")
    finding = _write_finding(outbox, bot_id="team_bot_a", app_id="journal",
                             record_id="rec-2")
    _same_mtime(summary, finding)

    order = [p.name for p in audit_poller._list_outbox_files("team_bot_a")]

    assert order == ["rec-2.json", "rec-hb.json"], (
        "a run summary sorted before a finding of the same instant — its sweep "
        "will resolve Signals the finding is about to re-observe"
    )


def test_the_drain_order_is_total_so_two_runs_agree(tmp_root: Path) -> None:
    """Records identical on every ordering key still get ONE fixed order.

    Without the final `name` key this function is right on one run and wrong on
    the next, which is the whole defect: not a wrong order, an UNDEFINED one.
    """
    outbox = _make_outbox(tmp_root, "team_bot_a")
    peers = [
        _write_finding(outbox, bot_id="team_bot_a", app_id="journal",
                       path="a.py", record_id="rec-c"),
        _write_finding(outbox, bot_id="team_bot_a", app_id="journal",
                       path="b.py", record_id="rec-a"),
        _write_finding(outbox, bot_id="team_bot_a", app_id="journal",
                       path="c.py", record_id="rec-b"),
    ]
    _same_mtime(*peers)

    runs = [[p.name for p in audit_poller._list_outbox_files("team_bot_a")]
            for _ in range(5)]

    assert runs[0] == ["rec-a.json", "rec-b.json", "rec-c.json"]
    assert all(r == runs[0] for r in runs), f"order varied across runs: {runs}"


def test_an_unreadable_record_still_gets_a_place_in_the_order(
    tmp_root: Path,
) -> None:
    """Sorting is not where a broken record becomes invisible.

    `_outbox_order_key` reads each file to learn its kind. A record that does
    not parse must still be RETURNED — the drain loop is what reports it as
    unreadable, and a key that raised (or that quietly dropped the file) would
    turn a corrupt record into a silently skipped one.
    """
    outbox = _make_outbox(tmp_root, "team_bot_a")
    good = _write_finding(outbox, bot_id="team_bot_a", app_id="journal",
                          record_id="rec-good")
    bad = outbox / "rec-bad.json"
    bad.write_text("{ this is not json")
    _same_mtime(good, bad)

    order = [p.name for p in audit_poller._list_outbox_files("team_bot_a")]

    assert set(order) == {"rec-good.json", "rec-bad.json"}
    # Not a summary (it cannot be read), so it sorts among the findings by ts
    # then name — the point is only that it is THERE and the order is fixed.
    assert order == sorted(order) or order[0] == "rec-good.json"
