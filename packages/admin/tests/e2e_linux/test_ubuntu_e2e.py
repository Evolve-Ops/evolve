...
# Step 6f — the per-bot apply + cost-converter daemons (the W7-deferred
# launchd-posture sites the real deploy_bot install path materializes).
APPLY_LABEL = f"ai.openclaw.evolve.apply.{BOT}"
...
def test_step6f_real_deploy_daemon_install_no_injection():
    ...
    try:
        ...
        # ── (c): apply + cost-converter render SYSTEMD units, not plists ─────
        result = deploy.DeployResult(bot_id=BOT, success=True)
        deploy._install_launchd_apply(BOT, OC_DIR / "workspace" / "evolve", result, user=BOT)
        deploy._install_launchd_cost_converter(BOT, result, user=BOT)
        apply_unit = Path(f"/etc/systemd/system/{APPLY_LABEL}.service")
        cost_unit = Path(f"/etc/systemd/system/{COST_LABEL}.service")
        assert apply_unit.exists(), (
            f"{apply_unit} missing — apply daemon did not render a systemd unit (fix 4)"
        )
        ...
        # and the units carry no .plist sibling.
        assert not Path("/Library/LaunchDaemons").exists(), (
            "/Library/LaunchDaemons exists on a Linux box — a launchd write was attempted"
        )
        for u in (apply_unit, cost_unit):
            assert not Path(str(u)[:-len(".service")] + ".plist").exists()
        assert get_scheduler().status(APPLY_LABEL)["installed"]
