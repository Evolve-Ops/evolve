...
def _stub_plugin_signature(monkeypatch):
    """install_oc_plugin verifies the deployed plugin's signed-bypass digest
    before passing ``--dangerously-force-unsafe-install`` to OC. Tests in this
...
class TestEnsurePluginConfig:
    ...
    def test_brave_install_gap_fill_when_missing(self):
        """When brave is not installed, ensure_plugin_config should attempt
        `openclaw plugins install @openclaw/brave-plugin` so an existing bot
        whose brave install was wiped (e.g. upstream API bump that required
        a fresh per-bot install) self-heals on the next deploy.

        Mirrors the entries-dict gap-fill from #1260 — extends it to the
        install itself. Without this, redeploys of existing bots stop short
        of repairing the missing install, and the operator must hand-run
        `sudo -u <bot> -H openclaw plugins install --force
        @openclaw/brave-plugin` per bot.
        """
        oc_json = _make_oc_json()
        net = _network()
        ...
        # The gap-fill must invoke `openclaw plugins install
        # @openclaw/brave-plugin`. Look for that cmd in the call history.
        brave_install_called = any(
        ...
        assert brave_install_called, (
            "ensure_plugin_config must attempt `openclaw plugins install "
            "@openclaw/brave-plugin` when brave is not yet installed"
        )
...
class TestInstallOcPluginSignedBypass:
    """Spec: docs/spec-plugin-install-trust-2026-06-06.md §1.

    install_oc_plugin must call ``verify_plugin_signature(PLUGIN_INSTALL_DIR)``
    before passing ``--dangerously-force-unsafe-install`` and refuse-to-install
    on mismatch. This closes PR #2293's blanket-bypass gap.
    ...
    def test_proceeds_with_bypass_when_signature_matches(self):
        ...
        # The bypass flag DID get passed — verification cleared.
        assert "--dangerously-force-unsafe-install" in install_cmd, (
            "verified signature must permit the --dangerously-force-unsafe-install "
            "flag — without it, OC 2026.6.1's scanner blocks TurnObserver"
        )
