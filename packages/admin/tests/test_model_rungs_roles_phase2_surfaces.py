...
def test_models_set_judge_is_refused(tmp_path):
    """The judge role is gone (judge-role collapse §5.4): `models set judge`
    is refused like any unknown role, and the config is untouched."""
    net_path = _make_network_with_rungs(tmp_path)
    before = json.loads(net_path.read_text())
    result = _invoke_models_set(["judge", "sonnet-class"], net_path)
    assert result.exit_code != 0
    assert "Unknown role" in result.output
    assert json.loads(net_path.read_text()) == before
