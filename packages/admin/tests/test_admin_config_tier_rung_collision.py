...
Root cause: on the new (rungs/roles) tier shape several legacy tier keys can
collapse onto one rung — historically ``standard``(tier2) and ``judge``(tier0)
both mapped to ``sonnet-class``; the judge role is gone (judge-role collapse,
2026-08-31) but a hand-authored roles map can still point any two roles at one
rung. ``oc_model.apply_tiers_update_new_shape`` folds a legacy
``{tierN: {models}}`` update into the rung store last-writer-wins, so an
...
# A new-shape file whose hand-authored roles map points fast AND standard at
# the same sonnet-class rung — the general collision geometry the guard
# defends (historically it was standard+judge; the judge role is gone but the
# collision class is not judge-specific).
...
def test_detect_no_collision_on_legacy_only_file():
    ...
    assert oc_model.detect_rung_collisions(
        legacy, {"tier2": {"models": ["a"]}, "tier3": {"models": ["b"]}}
    ) == []
...
def test_detect_no_collision_when_shared_rung_models_match():
    import oc_model
    # fast + standard both → sonnet-class with the SAME models is not a
    # conflict — it is exactly the editor's load state and a catalog re-save.
    same = ["anthropic/claude-sonnet-4-6"]
    assert oc_model.detect_rung_collisions(
        _evolve_shape_file(),
        {"tier2": {"models": same}, "tier3": {"models": list(same)}},
    ) == []
...
def test_detect_collision_when_shared_rung_models_differ():
    ...
    assert set(cols[0]["roles"]) == {"standard", "fast"}
    msg = oc_model.format_rung_collision_error(cols)
    assert "sonnet-class" in msg
    assert "standard" in msg and "fast" in msg


def test_tier0_never_collides_since_the_judge_collapse():
    import oc_model
    # tier0 (the retired judge slot) no longer maps to a role: its models
    # MERGE into the standard chain (append-dedup) instead of replacing any
    # rung, so it cannot clobber a sibling edit and never registers as a
    # collision (judge-role collapse §6).
...
def test_endpoint_409_on_rung_collision(app):
    with app["app"].test_client() as c:
        ...
        assert "standard" in err and "fast" in err
