...
def test_no_default_collision_between_standard_and_judge():
    import oc_model
    # Judge owns its own judge-class rung by default (spec §Addendum 16), so a
    # new-shape file with no explicit roles map no longer collides standard
    # (tier2 → sonnet-class) with judge (tier0 → judge-class): different rungs,
    # so both edits land. This is the decouple that fixes the pod-default judge
    # reorder revert.
