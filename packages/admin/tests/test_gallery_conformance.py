...
What it checks, and against what authority:
  ...
  8. The ``verification`` block validates against the harness schema
     (``gallery_verify.verification``), every path is workspace-relative,
     and every referenced script/artifact is grounded in the package.
  9. The repo-shipped importable manifests (``docs/atlas-app-manifests/``,
     imported on pods via POST /api/gallery/import) pass the same
     build-blocker validators preflight runs on every install
     (apps-inherit-bot-llm + privacy/audience_scoping pinned structure).
...
# packages/gallery/catalog.json is a GENERATED projection of
# gallery/index.json (scripts/backfill_application_tags.py --rebuild-index,
# via project_catalog()). It is the catalog the evo assistant
# (evo/handlers/gallery.py) and the RSI gallery recommender
# (analyzer/gallery_recommender.py) read. Before codebase-review 0.4 it was a
# hand-authored file of 14 placeholder apps fully disjoint from the real
# gallery, so every `evo install <N>` dead-ended in "Package not found" and
# the recommender scored phantom apps. These checks pin the projection: every
# catalog pkg_id must resolve in index.json, and name/description must agree
# with the index row — so a future divergence (or a reverted regeneration)
# fails CI instead of shipping phantom apps.
...
def _load_catalog() -> list[dict]:
    ...
    assert isinstance(catalog, list) and catalog, (
        "packages/gallery/catalog.json is empty or not a list"
    )
    return catalog
...
def test_evo_catalog_pkg_ids_resolve_in_gallery():
    ...
    assert not dangling, (
        ...
        f"regenerate with scripts/backfill_application_tags.py --rebuild-index"
    )


def test_evo_catalog_name_and_description_match_index():
    catalog = _load_catalog()
    by_pkg = {e["pkg_id"]: e for e in _INDEX}
    for entry in catalog:
        pkg_id = entry.get("pkg_id")
        index_row = by_pkg.get(pkg_id)
        assert index_row is not None, (
            f"catalog.json pkg_id {pkg_id!r} has no index.json row"
        )
        assert entry.get("name") == index_row.get("display_name"), (
            f"catalog.json {pkg_id}: name {entry.get('name')!r} disagrees "
            f"with index.json display_name {index_row.get('display_name')!r} "
            f"— catalog.json is a projection of index.json; regenerate with "
            f"scripts/backfill_application_tags.py --rebuild-index"
        )
        assert entry.get("description") == index_row.get("description"), (
            f"catalog.json {pkg_id}: description disagrees with index.json — "
            f"regenerate with scripts/backfill_application_tags.py "
            f"--rebuild-index"
        )


def test_evo_catalog_covers_every_gallery_app():
    # The projection is total: every installable gallery app appears in the
    # evo catalog (so `evo gallery` shows the real surface, not a subset).
    catalog = _load_catalog()
    catalog_ids = {c.get("pkg_id") for c in catalog}
    index_ids = {e["pkg_id"] for e in _INDEX}
    missing = index_ids - catalog_ids
    assert not missing, (
        f"gallery apps absent from packages/gallery/catalog.json: {missing} "
        f"— regenerate with scripts/backfill_application_tags.py "
        f"--rebuild-index"
    )
...
# ── 8: verification block (wave 3a) ──────────────────────────────────────────
# Every builtin package carries a machine-runnable ``verification`` block,
# executed by the gallery_verify harness's verify slot as the ``evolve`` user.
# The block must validate against the v1 schema (argv-vector commands on the
# #2602 exec posture, workspace-relative artifact checks) and only meta
# bundles (starter packs, which ship no files) may leave it empty.
...
_META_PKG_TAG = "starter-pack"


@_entry_param
def test_verification_block_present_and_valid(entry):
    pkg = _load_pkg(entry)
    block = pkg.get("verification")
    assert isinstance(block, list), (
        f"{entry['pkg_id']} ({entry.get('display_name')}): every builtin "
        f"package must carry a top-level 'verification' list (wave 3a) — "
        f"got {type(block).__name__}"
    )
    errors = validate_verification_block(block)
    assert not errors, (
        f"{entry['pkg_id']} ({entry.get('display_name')}): verification "
        f"block fails schema validation:\n  " + "\n  ".join(errors)
    )
    is_meta = _META_PKG_TAG in (pkg.get("application_tags") or [])
    if not is_meta:
        assert block, (
            f"{entry['pkg_id']} ({entry.get('display_name')}): only "
            f"starter-pack meta bundles may have an empty verification "
            f"block — a real app must verify at least its main script"
        )


@_entry_param
def test_verification_paths_grounded_and_relative(entry):
    """Every path a verification entry touches must be workspace-relative
    (never /Users, /home — the harness cwd's into the bot workspace) and
    grounded in the package itself (build_spec / files / interface_contract),
    so an entry can't probe files the app never creates."""
    pkg = _load_pkg(entry)
    # Ground against the package WITHOUT the block itself — entries must
    # not vouch for each other's paths.
    pkg_text = json.dumps({k: v for k, v in pkg.items() if k != "verification"})
    for i, e in enumerate(pkg.get("verification") or []):
        where = f"{entry['pkg_id']} verification[{i}] ({e.get('name')})"
        tokens = list(e.get("argv") or []) + [e.get("path") or ""]
        for tok in tokens:
            assert "/Users/" not in tok and "/home/" not in tok, (
                f"{where}: absolute home path in {tok!r} — all paths are "
                f"relative to the bot workspace"
            )
        if e.get("kind") == KIND_COMMAND:
            script = e["argv"][1]
            assert script in pkg_text, (
                f"{where}: command script {script!r} is not mentioned "
                f"anywhere else in the package (build_spec/files) — the "
                f"forge would never materialize it"
            )
        else:
            base = e["path"].split("*")[0].split("?")[0].rstrip("/")
            assert base and base in pkg_text, (
                f"{where}: artifact path {e['path']!r} (base {base!r}) is "
                f"not mentioned anywhere in the package — the app would "
                f"never create it"
            )


# ── 9: repo-shipped importable manifests (docs/atlas-app-manifests) ─────────
# The four Atlas manifests are gallery packages in every sense — operators
# import them via POST /api/gallery/import — but they live outside
# gallery/index.json, so the index-driven sweeps above never see them. That
# is exactly how the 2026-07-09 Gate-2 live sweep found "Atlas — Daily
# Digest" sitting in the mini's gallery unbuildable: the imported copy
# violated the apps-inherit-bot-llm principle and the pinned privacy/
# audience_scoping structure (base-spec §11.1), and no CI gate had ever run
# the preflight requirement validators against the source manifests. Sweep
# them here with the same two build-blocker validators preflight_check
# embeds, so a violating manifest can't ship (and then be imported) again.
#
# validate_scheduled_actions is swept separately: the heuristic's
# negated-mention / event-driven false-positive class was fixed (strong/weak
# marker tiers + negation and disclaimer guards), and the sweep lives next to
# the validator's own tests — see
# test_scheduled_actions_validator.test_atlas_manifests_pass_scheduled_actions_validator.
...
_ATLAS_MANIFEST_DIR = _REPO_ROOT / "docs" / "atlas-app-manifests"
_ATLAS_MANIFESTS = sorted(_ATLAS_MANIFEST_DIR.glob("*.json"))


def test_atlas_manifest_sweep_is_nonempty():
    # If the manifests move, move this sweep with them — an empty glob
    # would silently skip every parameterized case below.
    assert _ATLAS_MANIFESTS, (
        f"no importable manifests found under {_ATLAS_MANIFEST_DIR} — if the "
        f"Atlas manifests moved, update _ATLAS_MANIFEST_DIR (and the "
        f"docs/atlas-app-manifests/ path predicate in ci.yml + tools/preflight)"
    )


@pytest.mark.parametrize(
    "manifest_path", _ATLAS_MANIFESTS, ids=[p.name for p in _ATLAS_MANIFESTS]
)
def test_atlas_manifest_id_and_version_shape(manifest_path):
    pkg = json.loads(manifest_path.read_text(encoding="utf-8"))
    assert _PKG_ID_RE.match(pkg.get("pkg_id") or ""), (
        f"{manifest_path.name}: pkg_id {pkg.get('pkg_id')!r} does not match "
        f"the canonical p-<8 hex> shape — import_package rejects it (the "
        f"#1678 regression class)"
    )
    assert parse_pkg_version(pkg.get("pkg_version") or "") is not None, (
        f"{manifest_path.name}: pkg_version {pkg.get('pkg_version')!r} does "
        f"not parse as YYYY.MM.DD-major.minor — update detection against an "
        f"imported copy breaks"
    )
...
def test_atlas_manifest_passes_build_blocker_validators(manifest_path):
    pkg = json.loads(manifest_path.read_text(encoding="utf-8"))
    for validate in (validate_apps_inherit_bot_llm, validate_privacy_scoping):
        result = validate(pkg)
        assert result["ok"], (
            f"{manifest_path.name}: {validate.__name__} refuses this "
            f"manifest — every install of the imported package would "
            f"build_blocker at preflight forever:\n  "
            + "\n  ".join(result["errors"])
        )
