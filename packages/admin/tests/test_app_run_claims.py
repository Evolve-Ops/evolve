...
``write_app_run_claim`` is the Python-side writer of the AL-0.4 claim
contract. The live producer for scheduled app invocations is the generated
bash shim in ``app_run_shim`` — it runs inside the app's own cron process,
where importing this package is not available; ``test_app_run_shim.py`` pins
the two writers to the same shape. The plugin joins
``{shared}/{bot}/app-runs/<uuid>.json`` at ``before_agent_run``.
...
def test_forge_dispatcher_never_writes_claims():
    ...
    for token in ("write_app_run_claim", "app-runs", "app_runs", APP_RUNS_DIR_NAME):
        assert token not in src, (
            ...
            "unattributed (no claim files); see internal/build-AL-1.2-scheduled-"
