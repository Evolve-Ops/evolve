...
# ── D-BI3/D-BI5: read annotates actions live; a tap is an instruction ──────

def test_read_annotates_every_card_with_live_actions(pod):
    r = pod["client"].get("/api/board/personal-bot", headers=_auth(pod))
    cards = {c["title"]: c for c in r.get_json()["cards"]}
    actions = cards["Pick the trip weekend"]["actions"]
    assert actions  # a "travel" cluster card always gets a starter set
    for a in actions:
        assert "requires_rung" in a
        assert ("est_cost" in a) or ("disabled_reason" in a)
        # Never simply omitted (D-BI5): a gated action explains itself.
        if a.get("disabled_reason"):
            assert a["est_cost"] is None


def test_instruct_offers_the_card_and_queues_it(pod):
    card_id = _card_id(pod, "Pick the trip weekend")
    board = bs.load_board(pod["shared"], "personal-bot")
    card = bs.find_card(board, card_id)
    tool_action = next(a for a in card["actions"] if a["kind"] == "tool")
    r = pod["client"].post(f"/api/board/personal-bot/cards/{card_id}/instruct",
                           json={"action_id": tool_action["id"]}, headers=_auth(pod))
    assert r.status_code == 200
    card = r.get_json()["card"]
    assert card["owner"] == "bot"
    assert card["instructed"]["action_id"] == tool_action["id"]
    assert card["instructed"]["est_cost"] == 0.0


def test_instruct_unknown_action_is_404(pod):
    card_id = _card_id(pod, "Pick the trip weekend")
    r = pod["client"].post(f"/api/board/personal-bot/cards/{card_id}/instruct",
                           json={"action_id": "not-a-real-action"}, headers=_auth(pod))
    assert r.status_code == 404


def test_instruct_a_disabled_action_is_refused(pod):
    # "call_ahead" needs an integration (placing a phone call) that no pod
    # has ever built — always disabled, with a reason, never hidden.
    card_id = _card_id(pod, "Schedule the scan")  # cluster=health
    board = bs.load_board(pod["shared"], "personal-bot")
    card = bs.find_card(board, card_id)
    call_action = next(a for a in card["actions"] if a["id"] == "call_ahead")
    r = pod["client"].post(f"/api/board/personal-bot/cards/{card_id}/instruct",
                           json={"action_id": call_action["id"]}, headers=_auth(pod))
    assert r.status_code == 409
    assert "no calling integration" in r.get_json()["error"]


def test_instruct_requiring_confirm_needs_a_second_tap(pod, monkeypatch):
    # A granted, priced fast rung — otherwise every ``kind: llm`` action is
    # disabled on "no pricing"/"no integration" before confirmation is ever
    # reached, and this test would not exercise the confirm gate at all.
    from evolve_admin import board_actions
    monkeypatch.setattr(
        board_actions, "bot_action_context",
        lambda bot_id, network, shared: {"google": True, "llm_est_cost": 0.01})
    card = bs.create_card(
        pod["shared"], "personal-bot", title="Re: invoice", cluster="admin",
        source="email")
    reply = next(a for a in card["actions"] if a["id"] == "draft_reply")
    assert reply.get("requires_confirm") is True
    unconfirmed = pod["client"].post(
        f"/api/board/personal-bot/cards/{card['id']}/instruct",
        json={"action_id": reply["id"]}, headers=_auth(pod))
    assert unconfirmed.status_code == 409
    assert unconfirmed.get_json()["requires_confirm"] is True
    still = bs.find_card(bs.load_board(pod["shared"], "personal-bot"), card["id"])
    assert "instructed" not in still  # the unconfirmed tap changed nothing
    confirmed = pod["client"].post(
        f"/api/board/personal-bot/cards/{card['id']}/instruct",
        json={"action_id": reply["id"], "confirm": True}, headers=_auth(pod))
    assert confirmed.status_code == 200
    assert confirmed.get_json()["card"]["instructed"]["action_id"] == reply["id"]


def test_instruct_none_of_these_logs_and_leaves_the_card_alone(pod):
    card_id = _card_id(pod, "Pick the trip weekend")
    before = bs.find_card(bs.load_board(pod["shared"], "personal-bot"), card_id)
    r = pod["client"].post(f"/api/board/personal-bot/cards/{card_id}/instruct",
                           json={"none": True}, headers=_auth(pod))
    assert r.status_code == 200
    after = bs.find_card(bs.load_board(pod["shared"], "personal-bot"), card_id)
    assert before == after


def test_instruct_requires_token(pod):
    r = pod["client"].post("/api/board/personal-bot/cards/abc/instruct",
                           json={"action_id": "x"})
    assert r.status_code == 401


# ── D-BI4 §5: the approve/decline sheet resolves a pending approval ────────

def test_approval_grants_and_resumes_via_board_worker(pod):
    card_id = _card_id(pod, "Schedule the scan")
    bs.assign_card(pod["shared"], "personal-bot", card_id, "bot", actor="user")
    bs.record_approval_request(
        pod["shared"], "personal-bot", card_id, act="book",
        payload_summary="Proposed: Tuesday 2pm", est_cost=0.01, actor="personal-bot")

    r = pod["client"].post(f"/api/board/personal-bot/cards/{card_id}/approval",
                           json={"granted": True}, headers=_auth(pod))
    assert r.status_code == 200
    card = r.get_json()["card"]
    assert "pending_approval" not in card
    assert card["delegation"]["state"] == "done"
    assert "Proposed: Tuesday 2pm" in card["delegation"]["result"]["text"]


def test_approval_decline_blocks_the_delegation(pod):
    card_id = _card_id(pod, "Schedule the scan")
    bs.assign_card(pod["shared"], "personal-bot", card_id, "bot", actor="user")
    bs.record_approval_request(
        pod["shared"], "personal-bot", card_id, act="book",
        payload_summary="x", est_cost=0.01, actor="personal-bot")

    r = pod["client"].post(f"/api/board/personal-bot/cards/{card_id}/approval",
                           json={"granted": False}, headers=_auth(pod))
    assert r.status_code == 200
    card = r.get_json()["card"]
    assert card["delegation"]["state"] == "blocked"
    assert card["delegation"]["progress_note"] == "approval declined"


def test_approval_with_no_pending_request_is_a_409(pod):
    card_id = _card_id(pod, "Schedule the scan")
    bs.assign_card(pod["shared"], "personal-bot", card_id, "bot", actor="user")
    r = pod["client"].post(f"/api/board/personal-bot/cards/{card_id}/approval",
                           json={"granted": True}, headers=_auth(pod))
...
def test_approval_missing_granted_field_is_a_400(pod):
    card_id = _card_id(pod, "Schedule the scan")
    r = pod["client"].post(f"/api/board/personal-bot/cards/{card_id}/approval",
                           json={}, headers=_auth(pod))
    assert r.status_code == 400


def test_approval_unknown_card_is_a_404(pod):
    r = pod["client"].post("/api/board/personal-bot/cards/not-a-real-card/approval",
                           json={"granted": True}, headers=_auth(pod))
    assert r.status_code == 404


def test_approval_requires_token(pod):
    card_id = _card_id(pod, "Schedule the scan")
    r = pod["client"].post(f"/api/board/personal-bot/cards/{card_id}/approval",
                           json={"granted": True})
    assert r.status_code == 401

# ── Morning Board briefing panel (D-BI6d) ──────────────────────────────────


def test_briefing_requires_a_token(pod):
    assert pod["client"].get("/api/board/personal-bot/briefing").status_code == 401


def test_briefing_is_none_before_the_app_has_stocked_the_bot(pod):
    r = pod["client"].get("/api/board/personal-bot/briefing", headers=_auth(pod))
    assert r.status_code == 200
    assert r.get_json()["briefing"] is None


def test_briefing_reads_back_what_the_bot_posted_and_dismiss_round_trips(pod):
    saved = bs.save_briefing(pod["shared"], "personal-bot", "Morning.")
    r = pod["client"].get("/api/board/personal-bot/briefing", headers=_auth(pod))
    body = r.get_json()["briefing"]
    assert body == {"date": saved["date"], "text": "Morning.", "dismissed": False}

    r = pod["client"].post("/api/board/personal-bot/briefing/dismiss",
                           json={}, headers=_auth(pod))
    assert r.status_code == 200 and r.get_json()["dismissed"] is True

    r = pod["client"].get("/api/board/personal-bot/briefing", headers=_auth(pod))
    assert r.get_json()["briefing"]["dismissed"] is True


def test_dismiss_a_stale_day_is_a_quiet_no_op(pod):
    r = pod["client"].post("/api/board/personal-bot/briefing/dismiss",
                           json={}, headers=_auth(pod))
    assert r.status_code == 200
...
def test_briefing_get_and_dismiss_require_a_token(pod):
    assert pod["client"].get(
        "/api/board/personal-bot/briefing").status_code == 401
    assert pod["client"].post(
        "/api/board/personal-bot/briefing/dismiss").status_code == 401
