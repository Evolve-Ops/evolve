...
# ── every transition names its predecessor (D-CS7) ────────────────────────────
#
# A marker written into `inflight/` or `done/` asserts a state it left behind, and #4271
# happened because nothing on the marker itself said whether that position was still
# current: `from_state` + `from_sha` are the durable record. See `internal/dispatch/
# queued/lane-state-pr-asserts-the-transition-it-came-from.md` requirement 1.


def test_launch_stamps_from_state_and_from_sha(tmp_path):
    repo = _git_repo(tmp_path)
    root = _lane(repo)
    _write(root, "queued", "provenance-a")
    subprocess.run(["git", "-C", str(repo), "add", "-A"], check=True)
    subprocess.run(["git", "-C", str(repo), "commit", "-qm", "queue it"], check=True)
    head = subprocess.run(["git", "-C", str(repo), "rev-parse", "HEAD"],
                          capture_output=True, text=True).stdout.strip()

    MOD.launch(root, "provenance-a")
    moved = (root / "inflight" / "provenance-a.md").read_text()
    assert "from_state: queued" in moved
    assert "from_sha: %s" % head in moved


def test_launch_stamps_from_sha_null_outside_a_repo(tmp_path):
    """No repo, no HEAD to name — `null`, never a raised error over a provenance nicety;
    the transition's own safety never depends on this field."""
    root = _lane(tmp_path)
    _write(root, "queued", "provenance-b")
    MOD.launch(root, "provenance-b")
    moved = (root / "inflight" / "provenance-b.md").read_text()
    assert "from_sha: null" in moved


def test_abandon_stamps_from_state_inflight(tmp_path):
    repo = _git_repo(tmp_path)
    root = _lane(repo)
    _write(root, "inflight", "provenance-c")
    subprocess.run(["git", "-C", str(repo), "add", "-A"], check=True)
    subprocess.run(["git", "-C", str(repo), "commit", "-qm", "in flight"], check=True)
    head = subprocess.run(["git", "-C", str(repo), "rev-parse", "HEAD"],
                          capture_output=True, text=True).stdout.strip()

    MOD.abandon(root, "provenance-c")
    moved = (root / "done" / "provenance-c.md").read_text()
    assert "from_state: inflight" in moved
    assert "from_sha: %s" % head in moved


@pytest.mark.parametrize("src_state", ["queued", "inflight"])
def test_complete_stamps_from_state_matching_its_actual_source(tmp_path, src_state):
    """`complete` accepts either source dir — the stamp must name the ONE it actually
    used, not a fixed value, or it misdescribes a queued-brief chip's own closing move."""
    root = _lane(tmp_path)
    _write(root, src_state, "provenance-d")
    MOD.complete(root, "provenance-d")
    moved = (root / "done" / "provenance-d.md").read_text()
    assert ("from_state: %s" % src_state) in moved
...
def test_launch_refuses_when_the_id_already_exists_in_another_lane_dir(tmp_path):
    ...
    An id already in `done/` has terminated — re-entering flight is a re-queue, a
    different, explicit act, never a plain `launch`. The refusal names the id and BOTH
    paths, not just the conflicting one, so a reader does not have to re-derive where
    the brief was being moved FROM."""
    root = _lane(tmp_path)
    ...
    msg = str(e.value)
    assert "two lane dirs" in msg
    assert "twin" in msg
    assert str(root / "queued" / "twin.md") in msg
    assert str(root / "done" / "twin.md") in msg
...
def test_start_refuses_to_repoint_an_existing_branch(monkeypatch, tmp_path):
    """D-TP5: a second builder naming a different branch is exactly the claim conflict
    `AlreadyClaimed` (exit 4) exists for — the reconciler `ls-remote`s this value to
    decide whether a chip may be relaunched, so silently repointing it moves the
    relaunch guard onto a branch nobody chose. A human resolves it."""
    root, _ = _started_lane(tmp_path)
    _ledger(tmp_path, "substrate", [{"id": "chip-a", "branch": None}])
    _bind_ledger(monkeypatch, tmp_path)
    MOD.start(root, "chip-a", branch="claude/x-1")

    # `_started_lane`'s fixture carries `session: task_abc` (its tray-card task_id), and
    # the refusal prefers naming the session over the branch when both are known.
    with pytest.raises(MOD.AlreadyClaimed, match="already claimed by task_abc"):
...
def test_start_a_conflicting_ledger_row_blocks_the_entry_write_too(monkeypatch, tmp_path):
    ...
    with pytest.raises(MOD.AlreadyClaimed, match="already claimed by claude/other"):
        MOD.start(root, "chip-a", branch="claude/x-1")

    assert "launch: prepared" in entry.read_text()


def test_start_claims_a_queued_entry_with_branch_only(monkeypatch, tmp_path):
    """D-TP5: `start` IS the claim, and claiming happens BEFORE the dispatcher has
    moved anything — so a plain `queued/` entry (no dispatch fields at all) now gets
    its `branch:` written too. It must NOT gain `launch: started`, which would
    misstate that the queue dispatched it (spec §7)."""
    root = _lane(tmp_path)
    entry = _write(root, "queued", "chip-a")
    _ledger(tmp_path, "substrate", [{"id": "chip-a", "branch": None}])
    _bind_ledger(monkeypatch, tmp_path)

    out = MOD.start(root, "chip-a", branch="claude/x-1")

    assert out["entry_written"] is True
    text = entry.read_text()
    assert "branch: claude/x-1" in text
    assert "launch:" not in text
    assert out["ledger_written"] is True


def test_start_refuses_a_second_builder_on_a_claimed_queued_entry(monkeypatch, tmp_path):
    """The #4266/#4267 shape: a chip and an app session both build the same brief. The
    SECOND caller to claim a still-queued brief is refused, exit 4, naming the first."""
    root = _lane(tmp_path)
    _write(root, "queued", "chip-a")
    _ledger(tmp_path, "substrate", [{"id": "chip-a", "branch": None}])
    _bind_ledger(monkeypatch, tmp_path)
    MOD.start(root, "chip-a", branch="claude/first")

    with pytest.raises(MOD.AlreadyClaimed, match="already claimed by claude/first"):
        MOD.start(root, "chip-a", branch="claude/second")


def test_start_refuses_a_second_builder_on_a_headless_claimed_entry(monkeypatch, tmp_path):
    """The chip's own first `start` call on a headless entry is fine — see
    `test_the_chips_own_start_still_records_its_branch_afterwards`. A SECOND caller
    naming a DIFFERENT branch on that same entry is the #4266/#4267 collision, still
    caught by branch equality, and the message names the holder's session too."""
    root, entry = _started_lane(tmp_path)
    entry.write_text(entry.read_text().replace("launch: prepared", "launch: headless"))
    _ledger(tmp_path, "substrate", [{"id": "chip-a", "branch": None}])
    _bind_ledger(monkeypatch, tmp_path)
    MOD.start(root, "chip-a", branch="claude/first")

    with pytest.raises(MOD.AlreadyClaimed, match="task_abc"):
        MOD.start(root, "chip-a", branch="claude/second")


def test_start_cli_exits_already_claimed_and_names_the_holder(monkeypatch, tmp_path, capsys):
    root = _lane(tmp_path)
    _write(root, "queued", "chip-a")
    _ledger(tmp_path, "substrate", [{"id": "chip-a", "branch": None}])
    _bind_ledger(monkeypatch, tmp_path)
    assert MOD.main(["start", "chip-a", "--branch", "claude/first",
                     "--dir", str(root)]) == 0

    rc = MOD.main(["start", "chip-a", "--branch", "claude/second", "--dir", str(root)])

    assert rc == MOD.EXIT_ALREADY_CLAIMED == 4
    assert "claude/first" in capsys.readouterr().err
...
def test_start_leaves_a_done_entry_alone(monkeypatch, tmp_path):
    """A brief in `done/` is one whose chip is already over — there is nothing left to
    claim, and start evidence would only be stale."""
    root, entry = _started_lane(tmp_path, state="done")
    _ledger(tmp_path, "substrate", [{"id": "chip-a", "branch": None}])
    _bind_ledger(monkeypatch, tmp_path)

    out = MOD.start(root, "chip-a", branch="claude/x-1")

    assert out["entry_written"] is False and "done/" in out["entry_skipped"]
    assert "launch: prepared" in entry.read_text()
...
def test_land_includes_cards_md_alongside_lane_paths(tmp_path):
    """D-CD3's ONE named exception: `CARDS.md` sits directly under the dispatch dir,
    same as the README this suite pins as ignored one test up — but it is a tool-
    generated render, not an operator file, so it is the one path outside the four
    state dirs that DOES land."""
    work, root, _ = _land_repo(tmp_path)
    (root / MOD.CARDS_FILENAME).write_text(MOD.render_cards_md([], "2026-09-07T18:00:00Z"))
    res = _load_tool().land(root, dry_run=True)
    assert "internal/dispatch/%s" % MOD.CARDS_FILENAME in res["adds"]
    assert "internal/dispatch/%s" % MOD.CARDS_FILENAME in res["changed"]


def test_land_refuses_a_hand_edited_cards_md(tmp_path):
    """A `CARDS.md` without the generator's header is a hand edit: `land` refuses and
    names the file, rather than committing bytes the next tick silently overwrites."""
    work, root, _ = _land_repo(tmp_path)
    (root / MOD.CARDS_FILENAME).write_text("# my notes\n\nno cards\n")
    with pytest.raises(Exception, match="CARDS.md.*hand-edited"):
        _load_tool().land(root, dry_run=True)
...
def test_every_procedure_names_the_granted_state_verb(name):
    ...
    if name == "meta-dispatch":
        # D-MT1: the dispatch tick is `tools/meta-tick`; the state write is the RUNNER's,
        # so the pin moves to its specification and to the argv the runner builds.
        root = Path(__file__).resolve().parents[3]
        spec = (root / "internal" / "meta-tick-spec.md").read_text(encoding="utf-8")
        runner = (root / "tools" / "meta_tick.py").read_text(encoding="utf-8")
        assert "meta-dispatch-move state --lane dispatch" in spec
        assert '"state", "--lane", "dispatch"' in runner
        return
...
def test_headless_launch_stamps_model_unknown_when_none_given(tmp_path, monkeypatch):
    """D-MT4: `claude --bg`'s banner carries no model field today, so an unstamped
    launch writes the literal string `unknown`, never a guess."""
    repo, root, fake = _headless_repo(tmp_path, monkeypatch)
    _write(root, "queued", "open-work")
    r = MOD.launch(root, "open-work", headless=True)
    entry = root / "inflight" / "open-work.md"
    assert _fm(entry, "model") == "unknown"
    assert r["model"] == "unknown"


def test_headless_launch_stamps_a_given_model(tmp_path, monkeypatch):
    repo, root, fake = _headless_repo(tmp_path, monkeypatch)
    _write(root, "queued", "open-work")
    r = MOD.launch(root, "open-work", headless=True, model="sonnet-5")
    entry = root / "inflight" / "open-work.md"
    assert _fm(entry, "model") == "sonnet-5"
    assert r["model"] == "sonnet-5"


def test_a_plain_launch_with_no_session_writes_no_model(tmp_path, monkeypatch):
    """Nothing is running yet, so nothing names a model."""
    repo, root, fake = _headless_repo(tmp_path, monkeypatch, prereqs_ok=False)
    _write(root, "queued", "open-work")
    r = MOD.launch(root, "open-work", headless=True)  # falls back: prereqs unmet
    assert r["moved"] is False
    assert r.get("model") is None
...
def _fake_mrp_runner(gh_response):
    """`mrp._default_runner` replacement: real `git`, faked `gh` (there is no real GitHub
    repo behind these fixtures, only a local origin)."""
    def run(argv, cwd=None):
        if argv and argv[0] == "gh":
            return gh_response(argv, cwd)
        return subprocess.run(argv, cwd=(str(cwd) if cwd else None),
                              capture_output=True, text=True, timeout=60)
    return run


def _gh_all_green(argv, cwd=None):
    if argv[:3] == ["gh", "api", "-i"]:
        path = argv[3]
        if path in dict(MOD.mrp.TOKEN_READS):
            return subprocess.CompletedProcess(argv, 0, "HTTP/2.0 200 OK\r\n\r\n{}", "")
        return subprocess.CompletedProcess(argv, 9, "", "unexpected gh api -i call")
    if argv[:2] == ["gh", "api"]:
        path = argv[2]
        if "/jobs" in path:
            return subprocess.CompletedProcess(
                argv, 0, json.dumps({"total_count": 0, "jobs": []}), "")
        rows = [{"id": 1, "name": "ci", "status": "completed", "conclusion": "success",
                 "head_sha": "a" * 40, "updated_at": "2026-09-16T00:00:00Z"}]
        return subprocess.CompletedProcess(
            argv, 0, json.dumps({"total_count": 1, "workflow_runs": rows}), "")
    if argv[:3] == ["gh", "pr", "list"]:
        return subprocess.CompletedProcess(argv, 0, "[]", "")
    return subprocess.CompletedProcess(argv, 9, "", "unexpected gh call")


def test_rig_preflight_verb_all_clear_on_a_healthy_repo(tmp_path, monkeypatch):
    work, root, _ = _land_repo(tmp_path)
    monkeypatch.setattr(MOD.mrp, "_default_runner", _fake_mrp_runner(_gh_all_green))
    result = MOD.rig_preflight_verb(root, state_dir_override=tmp_path / "state")
    assert result == {"verb": "rig-preflight", "ok": True, "lines": [],
                      "may_reconcile": True, "may_launch": True,
                      "login_note": "login: not probed this tick (no cached verdict)"}


def test_rig_preflight_verb_probe_login_names_expired_first(tmp_path, monkeypatch):
    work, root, _ = _land_repo(tmp_path)
    monkeypatch.setattr(MOD.mdh, "probe_background_login",
                        lambda **kw: (MOD.mdh.LOGIN_EXPIRED, "please run /login"))
    monkeypatch.setattr(MOD.mrp, "_default_runner", _fake_mrp_runner(_gh_all_green))
    result = MOD.rig_preflight_verb(root, probe_login=True,
...
def test_rig_preflight_verb_probe_login_ok_writes_the_cache(tmp_path, monkeypatch):
    work, root, _ = _land_repo(tmp_path)
    state_dir = tmp_path / "meta-dispatch-state"
    monkeypatch.setattr(MOD.mdh, "probe_background_login",
                        lambda **kw: (MOD.mdh.LOGIN_OK, "stub"))
    monkeypatch.setattr(MOD.mrp, "_default_runner", _fake_mrp_runner(_gh_all_green))
    result = MOD.rig_preflight_verb(root, probe_login=True, state_dir_override=state_dir)
    assert result["ok"] is True and result["login_note"] == "login: probed"
    cached = MOD._read_login_cache(state_dir_override=state_dir)
    assert cached is not None and cached["state"] == MOD.mdh.LOGIN_OK

    # the NEXT call, even without --probe-login, reuses the fresh cache instead of
    # reporting "not probed" — this is the whole point of caching a LOGIN_OK verdict.
    monkeypatch.setattr(MOD.mdh, "probe_background_login",
                        lambda **kw: (_ for _ in ()).throw(
                            AssertionError("must not probe again this soon")))
    result2 = MOD.rig_preflight_verb(root, state_dir_override=state_dir)
    assert result2["ok"] is True
    assert result2["login_note"].startswith("login: ok (cached")


def test_rig_preflight_verb_names_the_wrong_branch(tmp_path, monkeypatch):
    work, root, _ = _land_repo(tmp_path)
    subprocess.run(["git", "-C", str(work), "checkout", "-qb", "feature-x"], check=True)
    monkeypatch.setattr(MOD.mrp, "_default_runner", _fake_mrp_runner(_gh_all_green))
    result = MOD.rig_preflight_verb(root, state_dir_override=tmp_path / "state")
    assert result["ok"] is False
    assert result["may_reconcile"] is False and result["may_launch"] is False
    assert any("feature-x" in l for l in result["lines"])


def test_the_cli_reports_rig_preflight_json(tmp_path, monkeypatch, capsys):
    work, root, _ = _land_repo(tmp_path)
    monkeypatch.setattr(MOD.mrp, "_default_runner", _fake_mrp_runner(_gh_all_green))
    assert MOD.main(["rig-preflight", "--dir", str(root), "--json",
                     "--state-dir", str(tmp_path / "state")]) == 0
    out = json.loads(capsys.readouterr().out)
    assert out["ok"] is True and out["lines"] == []
    assert out["may_reconcile"] is True and out["may_launch"] is True


def test_the_cli_prints_all_clear_in_text_mode(tmp_path, monkeypatch, capsys):
    work, root, _ = _land_repo(tmp_path)
    monkeypatch.setattr(MOD.mrp, "_default_runner", _fake_mrp_runner(_gh_all_green))
    assert MOD.main(["rig-preflight", "--dir", str(root),
                     "--state-dir", str(tmp_path / "state")]) == 0
    out = capsys.readouterr().out
    assert out.splitlines()[0] == "rig: all clear"
    assert "may_reconcile: True  may_launch: True" in out


def test_the_cli_probe_login_flag_reaches_the_verb(tmp_path, monkeypatch, capsys):
    work, root, _ = _land_repo(tmp_path)
    probed = {}

    def fake_probe(**kw):
        probed["called"] = True
        return (MOD.mdh.LOGIN_OK, "stub")

    monkeypatch.setattr(MOD.mdh, "probe_background_login", fake_probe)
    monkeypatch.setattr(MOD.mrp, "_default_runner", _fake_mrp_runner(_gh_all_green))
    assert MOD.main(["rig-preflight", "--dir", str(root), "--probe-login",
                     "--state-dir", str(tmp_path / "state")]) == 0
    assert probed.get("called") is True
    assert "login: probed" in capsys.readouterr().out


# ── recover-stalls (D-TP1 step 4) ──────────────────────────────────────────────


def _stalled_headless_brief(ident, started_iso):
    return ("---\nid: %s\naspect: apps\ntitle: \"A brief\"\nprivileged: false\n"
           "created: 2026-08-23\npm: fable-cowork\nlaunch: headless\n"
           "session: agent_deadbeef\nstarted: %s\nbranch: null\n---\n"
           "WHY: pinned by tests.\n" % (ident, started_iso))


def _no_evidence_runners():
    git_runner = lambda argv, cwd=None: subprocess.CompletedProcess(argv, 0, "", "")
    gh_runner = lambda argv, cwd=None: subprocess.CompletedProcess(argv, 0, "[]", "")
    return git_runner, gh_runner


def test_recover_stalls_abandons_and_requeues_when_nothing_corroborates(tmp_path):
    work, root, _ = _land_repo(tmp_path)
    _write(root, "inflight", "stall-me",
          _stalled_headless_brief("stall-me", "2026-01-01T00:00:00Z"))
    git_runner, gh_runner = _no_evidence_runners()
    now = _utc(2026, 1, 1, 2, 0, 0)     # 2h after started, past BRANCH_GRACE_S (45m)
    res = MOD.recover_stalls(root, repo=work, now=now, git_runner=git_runner,
                             gh_runner=gh_runner)
    assert res["recovered"] == [{"id": "stall-me", "successor": "stall-me-2"}]
    assert not (root / "inflight" / "stall-me.md").exists()
    done = root / "done" / "stall-me.md"
    assert done.is_file()
    assert MOD._read_fm_str(done.read_text(), "outcome") == "stalled-corroborated"
    succ = root / "queued" / "stall-me-2.md"
    assert succ.is_file()
    succ_text = succ.read_text()
    assert MOD._read_fm_str(succ_text, "id") == "stall-me-2"
    assert MOD._read_fm_str(succ_text, "supersedes") == "stall-me"
    for key in ("session", "started", "branch", "dispatched", "dispatched_at",
               "outcome", "pr", "launch"):
        assert MOD._read_fm_str(succ_text, key) is None
    assert "WHY: pinned by tests." in succ_text


def test_recover_stalls_takes_no_action_when_a_ref_corroborates(tmp_path):
    work, root, _ = _land_repo(tmp_path)
    _write(root, "inflight", "stall-me",
          _stalled_headless_brief("stall-me", "2026-01-01T00:00:00Z"))
    git_ref_present = lambda argv, cwd=None: subprocess.CompletedProcess(
        argv, 0, "deadbeef1234\n" if argv[:2] == ["git", "for-each-ref"] else "", "")
    _, gh_runner = _no_evidence_runners()
    now = _utc(2026, 1, 1, 2, 0, 0)
    res = MOD.recover_stalls(root, repo=work, now=now, git_runner=git_ref_present,
                             gh_runner=gh_runner)
    assert res["recovered"] == []
    assert (root / "inflight" / "stall-me.md").is_file()
    assert not (root / "done" / "stall-me.md").exists()
    assert any("UNKNOWN, no action" in l for l in res["lines"])


def test_recover_stalls_ignores_entries_still_inside_the_grace_window(tmp_path):
    work, root, _ = _land_repo(tmp_path)
    _write(root, "inflight", "fresh-chip",
          _stalled_headless_brief("fresh-chip", "2026-01-01T01:50:00Z"))
    git_runner, gh_runner = _no_evidence_runners()
    now = _utc(2026, 1, 1, 2, 0, 0)     # 10 min old — inside BRANCH_GRACE_S
    res = MOD.recover_stalls(root, repo=work, now=now, git_runner=git_runner,
                             gh_runner=gh_runner)
    assert res["recovered"] == []
    assert (root / "inflight" / "fresh-chip.md").is_file()


def test_recover_stalls_ignores_a_headless_entry_that_already_has_a_branch(tmp_path):
    work, root, _ = _land_repo(tmp_path)
    text = _stalled_headless_brief("branched-chip", "2026-01-01T00:00:00Z").replace(
        "branch: null", "branch: claude/meta-branched-chip")
    _write(root, "inflight", "branched-chip", text)
    git_runner, gh_runner = _no_evidence_runners()
    now = _utc(2026, 1, 1, 2, 0, 0)
    res = MOD.recover_stalls(root, repo=work, now=now, git_runner=git_runner,
                             gh_runner=gh_runner)
    assert res["recovered"] == []
    assert (root / "inflight" / "branched-chip.md").is_file()


def test_recover_stalls_is_idempotent_against_a_stale_reread(tmp_path):
    """The idempotency guarantee: an original that has ALREADY been superseded is never
    minted a second successor, even if a caller hands `recover_stalls` the same row twice
    (a stale snapshot, or a second sweep before the lane state it read was refreshed)."""
    work, root, _ = _land_repo(tmp_path)
    _write(root, "inflight", "stall-me",
          _stalled_headless_brief("stall-me", "2026-01-01T00:00:00Z"))
    git_runner, gh_runner = _no_evidence_runners()
    now = _utc(2026, 1, 1, 2, 0, 0)
    first = MOD.recover_stalls(root, repo=work, now=now, git_runner=git_runner,
                               gh_runner=gh_runner)
    assert first["recovered"] == [{"id": "stall-me", "successor": "stall-me-2"}]

    # Simulate a stale re-read: the original entry reappears in `inflight/` (as it would
    # if a caller re-derived its own entry list from a snapshot taken before the first
    # sweep's move landed).
    _write(root, "inflight", "stall-me",
          _stalled_headless_brief("stall-me", "2026-01-01T00:00:00Z"))
    second = MOD.recover_stalls(root, repo=work, now=now, git_runner=git_runner,
                                gh_runner=gh_runner)
    assert second["recovered"] == []
    assert any("already superseded" in l for l in second["lines"])
    assert not (root / "queued" / "stall-me-3.md").exists()
    # Exactly one successor exists.
    assert sorted(p.name for p in (root / "queued").glob("stall-me-*.md")) == [
        "stall-me-2.md"]


def test_recover_stalls_removes_the_worktree(tmp_path):
    """Corroboration's worktree check compares mtimes against `started` — so `started`
    has to be the REAL checkout instant here, not a fixed fictional date, or every file
    `git worktree add` itself writes would misread as chip-written evidence (the
    `WORKTREE_CHECKOUT_SLACK_S` window this pins is exactly what tells the two apart)."""
    work, root, _ = _land_repo(tmp_path)
    started = datetime.datetime.now(datetime.timezone.utc)
    _write(root, "inflight", "stall-me",
          _stalled_headless_brief("stall-me", MOD.mdi.dispatch_stamp(started)))
    wt_dir = work / ".worktrees" / "meta-chip-stall-me"
    # A real `git worktree add` so `git worktree remove` has something legitimate to drop.
    subprocess.run(["git", "-C", str(work), "worktree", "add", "-q", "-b",
                    "claude/meta-stall-me-wt", str(wt_dir), "main"], check=True)
    # Detach the branch this worktree holds so the corroboration ref check (which looks
    # for `claude/meta-stall-me`, not `-wt`) still finds nothing.
    git_runner, gh_runner = _no_evidence_runners()
    now = started + datetime.timedelta(hours=2)
    res = MOD.recover_stalls(root, repo=work, now=now, git_runner=git_runner,
                             gh_runner=gh_runner)
    assert res["recovered"] == [{"id": "stall-me", "successor": "stall-me-2"}]
...
# ── D-CD3: `cards` renders CARDS.md from the eligible tool's cards[] ──────────


GEN_AT = "2026-09-07T18:00:00Z"
CARDS_HEADER = ("<!-- generated by meta-dispatch-move cards at 2026-09-07T18:00:00Z"
                " — do not edit -->")


def test_render_cards_md_is_one_line_when_the_lane_is_empty():
    """No table — a table with nothing under it invites the same question a zero-state
    waiting line does: is the lane idle, or did the render break? The header and the
    dated title still lead, so `land` can tell the file is the generator's."""
    assert MOD.render_cards_md([], GEN_AT) == (
        CARDS_HEADER + "\n# Dispatch cards — written 2026-09-07 18:00 UTC\n\nno cards\n")


def test_render_cards_md_table_and_tray_titles():
    cards = [
        {"id": "went-quiet", "state": "stale", "stale_kind": "started", "age_h": 5,
         "age_from": "branch commit", "session": "task_a",
         "dispatched_at": "2026-09-07T10:00:00Z", "started": None, "declined_at": None,
         "action": "investigate",
         "tray_title": "[META:substrate] 10:00 [went-quiet] Widen the check"},
        {"id": "fresh-card", "state": "prepared", "stale_kind": None, "age_h": 0,
         "age_from": "dispatch", "session": "task_b",
         "dispatched_at": "2026-09-07T17:50:00Z", "started": None, "declined_at": None,
         "action": "tap", "tray_title": "[META:apps] 17:50 [fresh-card] Ship the thing"},
    ]
    lines = MOD.render_cards_md(cards, GEN_AT).splitlines()
    assert lines[0] == CARDS_HEADER
    # dated, and the time the file was written — never a card's own stamp
    assert lines[1] == "# Dispatch cards — written 2026-09-07 18:00 UTC"
    assert ("| went-quiet | stale (started) | 5h since last commit | task_a | "
            "investigate |") in lines
    assert "| fresh-card | prepared | 0h since dispatch | task_b | tap |" in lines
    fence = lines.index("```")
    assert lines[fence + 1:fence + 3] == [
        "[META:substrate] 10:00 [went-quiet] Widen the check",
        "[META:apps] 17:50 [fresh-card] Ship the thing"]


def test_render_cards_md_unknown_card_renders_an_unknown_age():
    """A card whose deciding stamp will not parse carries `age_h: None`; the row says
    `unknown`, never `0h` — an age the renderer invented would read as a fresh card —
    and its tray title carries `--:--`, never a clock time."""
    cards = [
        {"id": "no-stamp", "state": "unknown", "stale_kind": None, "age_h": None,
         "age_from": None, "session": None, "dispatched_at": None,
         "started": None, "declined_at": None, "action": "investigate",
         "tray_title": "[META:apps] --:-- [no-stamp] Ship the thing"},
    ]
    lines = MOD.render_cards_md(cards, GEN_AT).splitlines()
    assert lines[0] == CARDS_HEADER
    assert "| no-stamp | unknown | unknown | — | investigate |" in lines
    assert "[META:apps] --:-- [no-stamp] Ship the thing" in lines


def test_cards_verb_writes_the_file_atomically_and_is_a_noop_when_unchanged(tmp_path):
    """Re-running with nothing changed does not touch the file even though the clock
    moved (the header's time is when the BODY last changed), so `lane/state` does not
    churn on a quiet tick; no temp file survives the write."""
    root = _lane(tmp_path)
    _write(root, "inflight", "waiting-chip")

    r1 = MOD.cards(root, now="2026-09-07T18:00:00Z")
    assert r1["written"] is True
    assert r1["card_count"] == 1
    dest = root / MOD.CARDS_FILENAME
    first_bytes = dest.read_bytes()
    assert MOD.cards_header_ok(dest.read_text())

    r2 = MOD.cards(root, now="2026-09-07T19:00:00Z")
    assert r2["written"] is False
    assert dest.read_bytes() == first_bytes
    assert not list(root.glob("CARDS.md.*.tmp"))


def test_cards_verb_uses_the_given_cards_and_never_rescans(tmp_path, monkeypatch):
    """The tick hands over step 2's `cards[]`: the verb renders THOSE, and never
    reads the lane or calls `evaluate` a second time."""
    root = _lane(tmp_path)
    _write(root, "inflight", "on-disk-chip")

    def boom(*a, **k):
        raise AssertionError("second scan")
    monkeypatch.setattr(MOD, "_load_mde", boom)
    given = [{"id": "from-step-2", "state": "prepared", "age_h": 1,
              "age_from": "dispatch", "session": None, "action": "tap",
              "tray_title": "[META:apps] 17:00 [from-step-2] T"}]
    r = MOD.cards(root, given, now=GEN_AT)
    text = (root / MOD.CARDS_FILENAME).read_text()
    assert r["card_count"] == 1
    assert "from-step-2" in text and "on-disk-chip" not in text


def test_cards_verb_rewrites_a_file_whose_header_was_removed(tmp_path):
    root = _lane(tmp_path)
    MOD.cards(root, [], now=GEN_AT)
    dest = root / MOD.CARDS_FILENAME
    dest.write_text(dest.read_text().split("\n", 1)[1])      # header dropped by hand
    assert MOD.cards(root, [], now=GEN_AT)["written"] is True
    assert MOD.cards_header_ok(dest.read_text())


def test_cards_verb_reports_no_cards_for_an_empty_lane(tmp_path):
    root = _lane(tmp_path)
    r = MOD.cards(root)
    assert r["written"] is True
    assert r["card_count"] == 0
    assert (root / MOD.CARDS_FILENAME).read_text().endswith("\n\nno cards\n")


def test_the_cli_cards_verb_reports_json(tmp_path, capsys):
    root = _lane(tmp_path)
    assert MOD.main(["cards", "--dir", str(root), "--json"]) == 0
    out = json.loads(capsys.readouterr().out)
    assert out["verb"] == "cards"
    assert out["written"] is True
    assert out["card_count"] == 0


def test_the_cli_cards_verb_accepts_cards_json(tmp_path, capsys):
    root = _lane(tmp_path)
    given = [{"id": "a", "state": "unknown", "age_h": None, "session": None,
              "action": "investigate", "tray_title": "[META:apps] --:-- [a] T"}]
    assert MOD.main(["cards", "--dir", str(root), "--cards-json", json.dumps(given),
                     "--json"]) == 0
    assert json.loads(capsys.readouterr().out)["card_count"] == 1
    assert "| a | unknown | unknown |" in (root / MOD.CARDS_FILENAME).read_text()
# ── orphan-brief: D-TP5, every PR gets a lane row ─────────────────────────────
#
# #4266/#4267 built one brief twice partly because a self-directed app session's PR left
# NO trace in the lane at all — no queued brief, no bind, nothing the duplicate detector
# or a human skimming the digest could notice. This verb is the reconciler's fix for a PR
# that opened with no `bind`: a one-line `inflight/<slug>.md` so every PR has a row.


def test_orphan_brief_writes_a_lane_row(tmp_path):
    root = _lane(tmp_path)
    out = MOD.orphan_brief(root, 4321, "Fix the tile metrics regression",
                           aspect="ui")
    assert out["id"] == "fix-the-tile-metrics-regression"
    path = root / "inflight" / (out["id"] + ".md")
    assert path.is_file()
    text = path.read_text()
    assert "pr: 4321" in text and "pm: reconciler" in text and "why: orphan" in text
    assert "aspect: ui" in text and "privileged: true" in text  # fails closed by default


def test_orphan_brief_can_be_marked_non_privileged(tmp_path):
    root = _lane(tmp_path)
    out = MOD.orphan_brief(root, 1, "small fix", privileged=False)
    assert "privileged: false" in (root / "inflight" / (out["id"] + ".md")).read_text()


def test_orphan_brief_refuses_when_the_row_already_exists(tmp_path):
    root = _lane(tmp_path)
    MOD.orphan_brief(root, 1, "same title")
    with pytest.raises(MOD.Refused, match="already exists"):
        MOD.orphan_brief(root, 2, "same title")


def test_orphan_brief_refuses_an_id_already_live_elsewhere_in_the_lane(tmp_path):
    root = _lane(tmp_path)
    _write(root, "queued", "same-title")
    with pytest.raises(MOD.Refused, match="already exists"):
        MOD.orphan_brief(root, 1, "same title")


def test_orphan_brief_cli_reports_the_path(tmp_path, capsys):
    root = _lane(tmp_path)
    assert MOD.main(["orphan-brief", "--dir", str(root), "--pr", "9",
                     "--title", "A self-built fix"]) == 0
