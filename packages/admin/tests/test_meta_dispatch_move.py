...
def test_land_includes_cards_md_alongside_lane_paths(tmp_path):
    ...
    work, root, _ = _land_repo(tmp_path)
    (root / MOD.CARDS_FILENAME).write_text("# Dispatch cards — tick 10:00 UTC\n\nno cards\n")
    res = _load_tool().land(root, dry_run=True)
...
def test_render_cards_md_is_one_line_when_the_lane_is_empty():
    """No title, no table — a title with nothing under it invites the same question a
    zero-state waiting line does: is the lane idle, or did the render break?"""
    assert MOD.render_cards_md([]) == "no cards\n"


def test_render_cards_md_table_and_tray_titles():
    cards = [
        {"id": "went-quiet", "state": "stale", "age_h": 5,
         "session": "task_a", "dispatched_at": "2026-09-07T10:00:00Z",
         "started": None, "declined_at": None, "action": "investigate",
         "tray_title": "[META:substrate] 10:00 [went-quiet] Widen the check"},
        {"id": "fresh-card", "state": "prepared", "age_h": 0,
         "session": "task_b", "dispatched_at": "2026-09-07T17:50:00Z",
         "started": None, "declined_at": None, "action": "tap",
         "tray_title": "[META:apps] 17:50 [fresh-card] Ship the thing"},
    ]
    text = MOD.render_cards_md(cards)
    lines = text.splitlines()
    assert lines[0] == "# Dispatch cards — tick 17:50 UTC"     # newest stamp, not `now`
    assert "| went-quiet | stale | 5h | task_a | investigate |" in lines
    assert "| fresh-card | prepared | 0h | task_b | tap |" in lines
    assert "[META:substrate] 10:00 [went-quiet] Widen the check" in lines
    assert "[META:apps] 17:50 [fresh-card] Ship the thing" in lines


def test_render_cards_md_unknown_card_renders_a_question_mark_age():
    """A card whose deciding stamp will not parse carries `age_h: None`; the row says
    `?h`, not `0h` — an age the renderer invented would read as a fresh card."""
    cards = [
        {"id": "no-stamp", "state": "unknown", "age_h": None,
         "session": None, "dispatched_at": None,
         "started": None, "declined_at": None, "action": "investigate",
         "tray_title": "[META:apps] ??:?? [no-stamp] Ship the thing"},
    ]
    lines = MOD.render_cards_md(cards).splitlines()
    assert "| no-stamp | unknown | ?h | — | investigate |" in lines
    assert "[META:apps] ??:?? [no-stamp] Ship the thing" in lines


def test_render_cards_md_is_byte_stable_for_identical_input():
    """No wall-clock is sampled inside the renderer — the same `cards[]` always renders
    the same bytes, which is what lets the `cards` verb's no-op write skip a tick with
    no new card news."""
    cards = [{"id": "x", "state": "started", "age_h": 1, "session": None,
             "dispatched_at": "2026-09-07T17:00:00Z", "started": None,
             "declined_at": None, "action": "nothing",
             "tray_title": "[META:apps] 17:00 [x] Do the thing"}]
    assert MOD.render_cards_md(cards) == MOD.render_cards_md(cards)


def test_cards_verb_writes_the_file_atomically_and_is_a_noop_when_unchanged(tmp_path):
    """The verb renders from the SAME `cards[]` the eligible tool computes — never a
    second read of the lane — and re-running it with nothing changed does not touch
    the file (so `lane/state` does not churn on a quiet tick)."""
    root = _lane(tmp_path)
...
def test_cards_verb_reports_no_cards_for_an_empty_lane(tmp_path):
    ...
    assert (root / MOD.CARDS_FILENAME).read_text() == "no cards\n"
