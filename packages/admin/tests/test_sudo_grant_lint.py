...
def test_goldens_parse_to_grants():
    ...
    # git JOINED the contract with the §24 workspace-repo audit grants
    # (2026-07-29 VPS denial census wave 2); mv remains a representative
    # out-of-contract binary (deliberately no grant — see audit_poller's
    # no-/bin/mv note).
    assert "cat" in IN_CONTRACT and "git" in IN_CONTRACT
    assert "mv" not in IN_CONTRACT
...
@pytest.mark.parametrize("binary", ["rsync", "/bin/mv", "/usr/bin/touch", "pkill", "env"])
