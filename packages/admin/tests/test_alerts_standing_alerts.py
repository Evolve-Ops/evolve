...
def test_suppressed_delivery_does_not_count_as_announced(shared):
    """A delivery carrying a ``suppressed_reason`` is an attempt, not an
    announcement — audit records one when a CRITICAL page fails end-to-end,
    and that Signal stays firing. Counting it as announced would hide the
    exact condition this inventory exists to surface."""
    import signals.store as signals_store
    sa = _sa()
    sig = _seed(shared, signature="p:send-failed", severity="alert")
    signals_store.record_delivery(
        sig, shared, channel="telegram",
        suppressed_reason="not delivered — page did not reach the operator",
    )
    summary = sa.collect(shared, now=_NOW)
    assert summary.total == 1
    assert summary.never_announced == 1
    assert summary.worst_never_announced_severity == "alert"
    assert "never announced" in sa.render(summary)


def test_failed_then_successful_delivery_counts_as_announced(shared):
    """The retry that lands ends it: one unmarked entry anywhere in the
    trail is proof, even behind an earlier failure marker."""
    import signals.store as signals_store
    sa = _sa()
    sig = _seed(shared, signature="p:retry-landed", severity="alert")
    signals_store.record_delivery(
        sig, shared, channel="telegram",
        suppressed_reason="not delivered — page did not reach the operator",
    )
...
# ── R-3 event-vs-posture split in the standing board ─────────────────────────


def test_posture_findings_are_counted_and_reported_separately(shared):
    """The daily board separates "a list to go fix" from "something is wrong
    right now".

    R-3 classifies every critical audit finding event-vs-posture, but until
    this the classification's only consumer was a badge on the Alerts page —
    so the one line the operator actually reads every morning still showed a
    single undifferentiated total. That is the shape the design blames for
    training the operator to ignore the channel (design §1).
    """
    sa = _sa()
    _seed(shared, signature="audit:creds:bot_a", severity="alert",
          producer="audit", type_="audit_world_readable_creds",
          details={"finding_kind": "event"})
    for i in range(3):
        _seed(shared, signature=f"audit:sudoers:{i}", severity="alert",
              producer="audit", type_="audit_sudoers_drift",
              details={"finding_kind": "posture"})

    summary = sa.collect(shared, now=_NOW)
    assert summary.total == 4
    assert summary.posture_count == 3

    text = sa.render(summary)
    assert "3 of these are one-time fixes." in text


def test_posture_line_uses_plain_english_not_codebase_jargon(shared):
    """A chat message is a PRIMARY surface (docs/principle-plex-test.md §2),
    so the codebase's word for this classification must not reach it. The
    Alerts-page badge says "posture" and carries a tooltip; the phone-read
    daily line has no tooltip to lean on.

    Scope: this guards the word this change introduces, and nothing more. It
    is NOT a general plain-language gate on the section — `_fix_line` renders
    each Signal's own `fix_steps` verbatim, and those already carry internal
    strings (a sudoers-renderer function name among them). Closing that is a
    separate piece of work against the producers, not something this test
    should be read as covering."""
    sa = _sa()
    _seed(shared, signature="audit:sudoers:1", severity="alert",
          producer="audit", type_="audit_sudoers_drift",
          details={"finding_kind": "posture"})

    text = sa.render(sa.collect(shared, now=_NOW))
    assert "posture" not in text.lower()
    assert "1 of these is a one-time fix." in text


def test_posture_count_is_a_floor_not_a_complement(shared):
    """Unclassified is not the same as active.

    Only audit criticals carry finding_kind. Every warn and every non-audit
    producer has none, so the section must never characterise the remainder
    — not on the posture line, and not on any other line either.

    The first version of this test only inspected the tail of the posture
    line itself, so a whole extra sentence ("The other N are active and need
    attention right now.") passed it. It now reads the WHOLE rendered text,
    which is the property it always claimed to have.
    """
    sa = _sa()
    _seed(shared, signature="audit:sudoers:1", severity="alert",
          producer="audit", type_="audit_sudoers_drift",
          details={"finding_kind": "posture"})
    _seed(shared, signature="pod_health:gw:1", severity="warn",
          producer="pod_health")
    _seed(shared, signature="host_health:disk:1", severity="warn",
          producer="host_health", type_="disk_low")

    summary = sa.collect(shared, now=_NOW)
    assert summary.total == 3
    assert summary.posture_count == 1

    text = sa.render(summary)
    assert "1 of these is a one-time fix." in text
    lowered = text.lower()
    for claim in ("are active", "is active", "need attention",
                  "needs attention", "right now" + " and", "the other"):
        assert claim not in lowered, (
            f"the section must make no claim about the unclassified "
            f"remainder; found {claim!r} in:\n{text}"
        )


def test_no_posture_line_when_nothing_is_classified(shared):
    """A pod whose standing set is all warns and non-audit producers gets the
    section it always got — the new line appears only when it has something
    to say."""
    sa = _sa()
    _seed(shared, signature="pod_health:gw:1", severity="warn",
          producer="pod_health")
    ...
    summary = sa.collect(shared, now=_NOW)
    assert summary.posture_count == 0
    assert "one-time fix" not in sa.render(summary)


def test_an_active_event_is_offered_a_shortlist_slot(shared):
    """A three-day-old credential exposure must not be buried behind four
    fifty-day configuration chores of the same severity.

    Handled by displacing ONE posture entry, not by reordering the board —
    see test_unclassified_noise_never_displaces_a_security_finding for the
    regression that shape caused. An OFFER, not a guarantee: see
    test_reservation_is_a_no_op_when_the_board_holds_no_posture for the
    case it deliberately leaves unserved.
    """
    sa = _sa()
    for i, age in enumerate((900.0, 900.0, 400.0, 1200.0)):
        _seed(shared, signature=f"audit:posture:{i}", severity="alert",
              producer="audit", type_="audit_posture", age_hours=age,
              title=f"Standing config issue {i}",
              details={"finding_kind": "posture"})
    _seed(shared, signature="audit:creds", severity="alert",
          producer="audit", type_="audit_creds", age_hours=72.0,
          title="Credential found in workspace file",
          details={"finding_kind": "event"})

    top = sa.collect(shared, now=_NOW, top_n=3).top
    titles = [e.title for e in top]
    assert "Credential found in workspace file" in titles, (
        f"the active event must be named on the board; got {titles}"
    )
    assert len(top) == 3, "the reservation displaces, it does not grow the list"


def test_unclassified_noise_never_displaces_a_security_finding(shared):
    """The regression an earlier revision shipped, pinned so it cannot return.

    Ranking posture below unclassified looks like "don't demote what we
    haven't classified", but the unclassified population is operational
    noise while the posture population is security criticals. On a pod with
    ordinary noise it evicted every CRITICAL security finding from the board
    and replaced them with disk/probe/gateway lines.
    """
    sa = _sa()
    for i, (sig, title, age) in enumerate((
        ("host_health:disk", "Disk 91% full", 300.0),
        ("integration_probe:slack", "Slack probe failing", 250.0),
        ("pod_health:restarts", "Gateway restarted 6x", 200.0),
    )):
        _seed(shared, signature=sig, severity="alert",
              producer=sig.split(":")[0], type_="noise",
              age_hours=age, title=title)
    for sig, title in (
        ("audit:filevault", "FileVault is off"),
        ("audit:sshpw", "SSH password login is enabled"),
        ("audit:botadmin", "Bot account holds host admin privilege"),
    ):
        _seed(shared, signature=sig, severity="alert", producer="audit",
              type_="audit_posture", age_hours=1200.0, title=title,
              details={"finding_kind": "posture"})

    titles = [e.title for e in sa.collect(shared, now=_NOW, top_n=3).top]
    # Compared as a set: all three are the same severity AND the same age, so
    # their order relative to each other is not a property worth pinning.
    assert set(titles) == {
        "FileVault is off",
        "SSH password login is enabled",
        "Bot account holds host admin privilege",
    }, f"security criticals must lead the board, not noise; got {titles}"


def test_a_fresh_tamper_shaped_posture_finding_keeps_its_slot(shared):
    """#3918's edr co-sign accepted `.zshrc` drift as posture ONLY under the
    page-once-then-board contract, so the board half of that bargain has to
    hold. A change detection is born at age ~0 — "posture is old by
    definition" is false precisely for the tamper-shaped rows.
    """
    sa = _sa()
    _seed(shared, signature="audit:zshrc", severity="alert", producer="audit",
          type_="audit_shell_config", age_hours=6.0,
          title=".zshrc hash changed since baseline",
          details={"finding_kind": "posture"})
    _seed(shared, signature="deploy:tick", severity="alert",
          producer="deploy_drift_monitor", type_="release_tick_failed",
          age_hours=2.0, title="Release tick failed")
    _seed(shared, signature="setup:admin", severity="alert",
          producer="pod_health", type_="no_admin", age_hours=900.0,
          title="No pod admin configured")

    titles = [e.title for e in sa.collect(shared, now=_NOW, top_n=2).top]
    assert ".zshrc hash changed since baseline" in titles, (
        f"a six-hour-old tamper-shaped finding must stay on the board; got {titles}"
    )


def test_the_reservation_only_ever_displaces_a_posture_entry(shared):
    """The safety property that keeps the reservation from re-creating the
    regression above — and a deliberate limit on what it adjudicates.

    Displacing a posture entry is a claim this module can defend: an active
    audit event outranks an audit chore. Displacing an UNCLASSIFIED entry
    would be a different and much larger claim — that an audit event
    outranks a gateway-down or a pod-silent — and the unclassified set holds
    both real incidents and routine noise, so it is not one to make by
    default. With no posture on the board there is simply no swap.
    """
    sa = _sa()
    for sig, title, age in (
        ("host_health:disk", "Disk 91% full", 300.0),
        ("pod_health:gw", "Gateway is down", 250.0),
        ("integration_probe:slack", "Slack probe failing", 200.0),
    ):
        _seed(shared, signature=sig, severity="alert",
              producer=sig.split(":")[0], type_="noise",
              age_hours=age, title=title)
    _seed(shared, signature="audit:creds", severity="alert", producer="audit",
          type_="audit_creds", age_hours=1.0,
          title="Credential found in workspace file",
          details={"finding_kind": "event"})

    titles = [e.title for e in sa.collect(shared, now=_NOW, top_n=3).top]
    assert set(titles) == {
        "Disk 91% full", "Gateway is down", "Slack probe failing",
    }, (
        "with no posture entry on the board the reservation must be a no-op "
        f"— it must not displace an unclassified entry; got {titles}"
    )


def test_reservation_never_displaces_a_higher_ranked_entry(shared):
    """The swap must not undo the sort it runs after.

    Two attacks, both reproduced on the unguarded version:

    * an ALREADY-ANNOUNCED event displacing a NEVER-ANNOUNCED critical.
      This is the steady state, not an edge — R-1 pages events on create,
      so a standing event is normally announced while the cold-start
      silenced backlog (the population this module exists for) is not.
    * a WARN event displacing an ALERT posture critical. Latent today (no
      audit emit site pairs a non-critical level with "event") but
      __post_init__ permits it and collect() reads finding_kind from any
      producer.
    """
    sa = _sa()
    for k, title, age in (
        ("a1", "FileVault is off", 1200.0),
        ("a2", "SSH password login is enabled", 1100.0),
        ("a3", "Bot account holds host admin privilege", 1000.0),
    ):
        _seed(shared, signature=k, severity="alert", producer="audit",
              type_="audit_posture", age_hours=age, title=title,
              details={"finding_kind": "posture"})
    ev = _seed(shared, signature="e1", severity="alert", producer="audit",
               type_="audit_creds", age_hours=50.0,
               title="Credential found in workspace file",
               details={"finding_kind": "event"})
    # Only the event has been announced — exactly what R-1 produces.
    # NOTE: _announced() requires BOTH keys; setting only alerted_for_signal_id
    # leaves the entry un-announced and the test would pass for the wrong
    # reason (candidate and victim tie, so the guard is never consulted).
    _write_notifier_state(shared, {ev.signature: {
        "alerted_for_signal_id": ev.id,
        "last_fire_pushed_at": _NOW.isoformat().replace("+00:00", "Z"),
    }})

    titles = [e.title for e in sa.collect(shared, now=_NOW, top_n=3).top]
    assert "Bot account holds host admin privilege" in titles, (
        "an announced event must not displace a never-announced security "
        f"critical; got {titles}"
    )
    assert "Credential found in workspace file" not in titles

    # Severity half: a warn event must not displace an alert posture item.
    shared2 = shared.parent / "evolve2"
    for sub in ("firing", "snoozed", "archived"):
        (shared2 / "signals" / sub).mkdir(parents=True)
    _seed(shared2, signature="p1", severity="alert", producer="audit",
          type_="audit_posture", age_hours=900.0, title="FileVault is off",
          details={"finding_kind": "posture"})
    _seed(shared2, signature="p2", severity="alert", producer="audit",
          type_="audit_posture", age_hours=800.0, title="SSH password login on",
          details={"finding_kind": "posture"})
    _seed(shared2, signature="w1", severity="warn", producer="audit",
          type_="audit_minor", age_hours=1.0, title="Minor event",
          details={"finding_kind": "event"})
    titles2 = [e.title for e in sa.collect(shared2, now=_NOW, top_n=2).top]
    assert titles2 == ["FileVault is off", "SSH password login on"], (
        f"a warn event must not displace an alert posture critical; got {titles2}"
    )


def test_reservation_displaces_the_least_bad_posture_entry(shared):
    """`reversed(top)` is load-bearing: taking the FIRST posture instead of
    the last would drop the worst standing finding rather than the least-bad
    one. It decides whether FileVault-off or a far newer item falls off."""
    sa = _sa()
    _seed(shared, signature="p_worst", severity="alert", producer="audit",
          type_="audit_posture", age_hours=2000.0,
          title="Oldest posture finding", details={"finding_kind": "posture"})
    _seed(shared, signature="p_mid", severity="alert", producer="audit",
          type_="audit_posture", age_hours=1000.0,
          title="Middle posture finding", details={"finding_kind": "posture"})
    _seed(shared, signature="p_least", severity="alert", producer="audit",
          type_="audit_posture", age_hours=100.0,
          title="Newest posture finding", details={"finding_kind": "posture"})
    _seed(shared, signature="ev", severity="alert", producer="audit",
          type_="audit_creds", age_hours=50.0,
          title="Credential found in workspace file",
          details={"finding_kind": "event"})

    titles = [e.title for e in sa.collect(shared, now=_NOW, top_n=3).top]
    assert "Oldest posture finding" in titles, (
        f"the worst standing finding must survive the swap; got {titles}"
    )
    assert "Newest posture finding" not in titles, (
        f"the least-bad posture entry is the one displaced; got {titles}"
    )
    assert "Credential found in workspace file" in titles


def test_reservation_is_a_no_op_when_the_board_holds_no_posture(shared):
    """The case the rule deliberately leaves unserved, pinned so the limit is
    visible rather than surprising: with only unclassified entries on the
    board, a one-hour-old credential exposure stays in "+N more"."""
    sa = _sa()
    for sig, title, age in (
        ("host_health:disk", "Disk 91% full", 300.0),
        ("pod_health:gw", "Gateway is down", 250.0),
...
def test_the_posture_line_makes_no_delivery_or_triage_promise(shared):
    """Pins the ABSENCE of two clauses that were each shipped and removed.

    Every other assertion here is a prefix/substring check, so restoring
    either clause verbatim survived them all. The claims are the defect;
    the count is not.
    """
    sa = _sa()
    for i in range(2):
        _seed(shared, signature=f"audit:posture:{i}", severity="alert",
              producer="audit", type_="audit_posture",
              details={"finding_kind": "posture"})

    text = sa.render(sa.collect(shared, now=_NOW)).lower()
    assert "2 of these are one-time fixes." in text
    for banned in (
        "message you again",   # the delivery promise
        "while they stand",
        "rather than a response",  # the triage claim
        "right now",
        "not urgent",
        "no action needed",
    ):
        assert banned not in text, (
            f"the posture line must state what the finding is and claim "
            f"nothing further; found {banned!r} in:\n{text}"
        )
