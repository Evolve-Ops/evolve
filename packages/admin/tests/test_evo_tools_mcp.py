...
def _extract_handler(server, kind: str):
    """Pull the registered `tools/list` or `tools/call` handler off an
    ``mcp.server.Server``.

    mcp 2.x moved handler registration to constructor kwargs and keys the map
    by METHOD STRING (`"tools/call"`), exposing it through the PUBLIC
    ``get_request_handler(method)`` which returns a
    ``HandlerEntry(params_type, handler)``. This helper used to reach into
    ``request_handlers`` / ``_request_handlers`` keyed by the request CLASS,
    which is a private shape that 2.x no longer has — a test that pins a
    private shape fails on a migration that changed nothing it was testing.
    The public accessor is preferred; the class-keyed dict stays as a fallback
    so this file still runs against a 1.x SDK.
    """
    method = {"list_tools": "tools/list", "call_tool": "tools/call"}[kind]
    ...
    from mcp import types as _mt            # 1.x fallback: keyed by request class
    request_type = getattr(_mt, {"list_tools": "ListToolsRequest",
                                 "call_tool": "CallToolRequest"}[kind])
