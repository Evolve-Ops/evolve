...
# ── Pod-key resolution (deploy gap-fill source of truth) ─────────────────────
# resolve_pod_brave_key decides whether deploy may enable brave at all. It
# resolves this bot's own key first (never clobber a hand-set key), then the
# pod keystore's shared-scope brave key (the fleet fan-out the keystore was
# always documented to serve — see keystore.py's module docstring, which names
# "Brave API" as the canonical shared-scope example).

def _net(tmp_path):
    return {"sharedDir": str(tmp_path), "bots": {"bot_a": {}}}


def test_resolve_prefers_this_bots_existing_key(tmp_path):
    """A key already on the bot wins — gap-fill must never clobber it.

    `evolve-admin keys sync` is the deliberate fan-out path; deploy is not.
    """
    from evolve_admin.brave_key import resolve_pod_brave_key
    cfg = {"plugins": {"entries": {"brave": {"config": {"webSearch": {"apiKey": "BSA-mine"}}}}}}
    assert resolve_pod_brave_key("bot_a", _net(tmp_path), cfg) == "BSA-mine"


def test_resolve_falls_back_to_shared_keystore_key(tmp_path, monkeypatch):
    """No bot key → the pod keystore's shared brave key fans out."""
    from evolve_admin import brave_key as deploy

    class _FakeKs:
        def list_keys(self):
            return [{"name": "brave", "provider": "brave", "scope": "shared", "bots": None}]
    ...
    class _FakeMgr:
        ...
        def get_value(self, name):
            return "BSA-pod" if name == "brave" else None

    monkeypatch.setattr("evolve_admin.keystore.KeystoreManager", _FakeMgr)
    assert deploy.resolve_pod_brave_key("bot_a", _net(tmp_path), {}) == "BSA-pod"


def test_resolve_respects_group_scope_authorization(tmp_path, monkeypatch):
    """A group-scoped key only applies to its listed bots."""
    from evolve_admin import brave_key as deploy
    ...
    monkeypatch.setattr("evolve_admin.keystore.KeystoreManager", _FakeMgr)
    assert deploy.resolve_pod_brave_key("bot_a", _net(tmp_path), {}) is None


def test_resolve_none_when_nothing_registered(tmp_path, monkeypatch):
    """Empty keystore → None, which is deploy's signal NOT to enable brave."""
    from evolve_admin import brave_key as deploy

    class _FakeMgr:
        def __init__(self, *a, **kw):
            self.ks = type("K", (), {"list_keys": lambda self: []})()
...
def test_resolve_degrades_to_none_when_keystore_raises(tmp_path, monkeypatch):
    """A broken keystore must not fail the deploy, and must not enable brave.

    Fail-safe direction: install-but-don't-enable, never enable-without-key.
    """
    from evolve_admin import brave_key as deploy
    ...
    monkeypatch.setattr("evolve_admin.keystore.KeystoreManager", _boom)
    assert deploy.resolve_pod_brave_key("bot_a", _net(tmp_path), {}) is None
