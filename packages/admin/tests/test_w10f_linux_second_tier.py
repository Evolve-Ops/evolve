...
def test_apply_resolve_shared_dir_prefers_network_over_literal(tmp_path):
    """apply._resolve_shared_dir must read the (platform-keyed) network.json's
    sharedDir, NOT probe a hardcoded /Users/Shared/evolve first. Round-3: a
    stray /Users tree on the Linux box won over the real /var/lib shared dir,
    so apply.darwin wrote to /Users/Shared/evolve/proposals/quarantine."""
    import apply  # analyzer top-level

    net = tmp_path / "network.json"
    net.write_text('{"sharedDir": "/var/lib/evolve"}')
    resolved = apply._resolve_shared_dir(net, None)
    assert str(resolved) == "/var/lib/evolve"
    # And the source no longer probes the macOS literal.
    src = (_ANALYZER_DIR / "apply.py").read_text()
    assert 'Path("/Users/Shared/evolve")' not in src
