...
def test_staged_source_rejects_non_tmp_path():
    # A SYNTHESIZED absolute path, not a repo path: _staged_source_ok checks the
    # temp-dir prefix BEFORE it lstats, so the path need not exist — and a path
    # derived from the checkout would sit under a temp dir whenever the repo
    # itself is checked out under one (agent worktrees live in /private/tmp),
    # which sails past the gate under test and fails on readability instead.
    src = "/evolve-test-not-a-temp-dir/staged_marker.py"
    # Guard the premise rather than trusting it: if some pathological TMPDIR
    # ever contained this path, fail loudly instead of testing nothing.
    import tempfile
    tmp_roots = {os.path.realpath("/tmp"), os.path.realpath(tempfile.gettempdir())}
    assert not any(src == r or src.startswith(r + os.sep) for r in tmp_roots), tmp_roots

    ok, reason = meh._staged_source_ok(src)
