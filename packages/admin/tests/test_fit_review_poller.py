...
# The poller's gate loads the REAL gallery catalog internally
# (fit_review_poller → targeting._load_gallery_catalog), so the synthetic
# candidates here must suggest a real, installable gallery app. Task Manager
# (task-management) is a foundational app and matches the task-management
# observations _strong_report emits — so it lands on the recomputed shortlist
# AND resolves in the catalog. (Pre codebase-review 0.4 this pointed at the
# phantom "p-f6a7b8c9", which only resolved because catalog.json itself was
# fabricated.)
PKG_TASK_MANAGER = "p-9bfa1c84"
...
def _strong_report(*, bot_id="team-bot-a", installed=()):
    ...
    >= 8 distinct sessions → tier high, Task Manager shortlisted."""
...
def test_pass_emits_one_install_app_in_pending(tmp_path: Path):
    ...
    assert p.action.app_id == PKG_TASK_MANAGER
    assert p.altitude == 2
