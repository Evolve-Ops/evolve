...
def test_refine_validate_no_api_key_rejected(tmp_path):
    """Bot has no anthropic key → validate fails with key-config hint."""
    net = _write_network(tmp_path)
    _seed_proposal(tmp_path, proposal_id="p-no-key", bot_id="team_bot_a")
    # network.json has no anthropic key wired up for "team_bot_a"
    result = action_proposal_refine._refine_validate(
        shared_dir=tmp_path, network_path=net,
        proposal_id="p-no-key", feedback="reword",
    )
    assert result["ok"] is False
    assert "Anthropic" in result["reason"] or "api_key" in result["reason"]
