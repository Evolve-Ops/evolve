...
to the seven live signal-emitting monitors (integration_probe,
host_health, error_reporter, pod_health, cron_exit_monitor,
exec_failure_monitor, tool_profile_monitor) and returns per-monitor
outcomes. These tests pin the contract:

  - the seven expected monitors all show up in the response
...
def app(tmp_path, monkeypatch):
    ...
    from evolve_admin import exec_failure_monitor as _exec_failure
    from evolve_admin import tool_profile_monitor as _tool_profile
    ...
    monkeypatch.setattr(
        _exec_failure, "emit_exec_failure_signals",
        lambda shared, **kw: {"bots": 0, "lines": 0, "shapes": 0},
    )
    monkeypatch.setattr(
        _tool_profile, "emit_tool_profile_signals",
        lambda shared, **kw: {"bots": 0, "refusals": 0, "combinations": 0,
...
def test_refresh_returns_all_seven_monitors(app):
    ...
    assert names == [
        ...
        "pod_health",
        "cron_exit_monitor",
        "exec_failure_monitor",
        "tool_profile_monitor",
    ]
