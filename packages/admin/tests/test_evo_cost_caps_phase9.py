...
def test_cost_caps_returns_effective_when_per_bot_unset(pod_env):
    """Bot inherits compiled default L1 ($5) when no per-bot value set."""
    shared, net_path = pod_env
    _write_be(shared)  # empty per-bot budget
    from evolve_admin.evo.tools.pod_state_cost_caps import _cost_caps_handler
    result = _cost_caps_handler(net_path, "team_bot_a")
    assert result["ok"] is True
    assert result["effective"]["l1_breaker_usd"] == 5.0  # compiled default
