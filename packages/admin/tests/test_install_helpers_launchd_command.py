...
- ``install_launchd_command_action`` with mocked
  ``install_scheduled_jobspec`` (the seam-routed installer) — validates
  the end-to-end shape without touching the real filesystem or launchctl.
- ``install_launchd_system_daemon`` itself (one-shot dry-run with
  ``bootstrap=False``) — confirms it writes to /Library/LaunchDaemons/
  and rejects unsafe input. (Still the raw-XML materializer for the
  ``launchd_python_signal`` mechanism; the command mechanism no longer
  uses it — see test_install_scheduled_action_both_profiles.py.)
...
# ── install_launchd_command_action — end-to-end with mocked seam installer ───


def _patch_install_scheduled_jobspec(monkeypatch: pytest.MonkeyPatch) -> mock.MagicMock:
    """Replace ``install_scheduled_jobspec`` with a recording stub.

    Returns the stub so tests can assert on its call args. The real
    installer routes through the scheduler seam (cp / chown / launchctl
    or systemctl); none of that is appropriate to exercise from unit
    tests, so we capture the (bot_id, JobSpec) it would receive.
    """
    stub = mock.MagicMock(
...
def _stub_plist(stub: mock.MagicMock) -> str:
    """Render the JobSpec the stub captured through the launchd renderer —
    the same call the seam's macOS adapter makes, so the plist-level
    assertions below exercise the real rendered artifact."""
    from evolve_admin.runtime import render_launchd_plist
    return render_launchd_plist(stub.call_args.args[1])
...
def test_install_launchd_command_action_happy_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    stub = _patch_install_scheduled_jobspec(monkeypatch)
    ...
    # install_scheduled_jobspec received the substituted forms.
    call_args = stub.call_args
    assert call_args.args[0] == "atlas"             # bot_id
    assert call_args.args[1].label == "com.atlas.atlas-digest"  # label substituted
    d = _parse(_stub_plist(stub))
...
def test_install_launchd_command_action_returns_error_on_relative_program(
    ...
    _patch_network(monkeypatch)
    _patch_install_scheduled_jobspec(monkeypatch)
