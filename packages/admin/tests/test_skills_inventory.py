...
def _write_oc(tmp_path: Path, oc_data: dict, bot_user: str = "testbot") -> Path:
    ...
    # exist_ok: a test may plant per-agent runtime-store fixtures under
    # .openclaw/ before writing openclaw.json.
...
def _write_agent_catalog(
    home, agent: str, provider: str, key: str, *, provider_id: str = "",
) -> None:
    """Plant one ``agents/<a>/agent/plugins/<p>/catalog.json`` under *home*.

    The shape scanned live off the PoC bot on 2026-09-04: a ``generatedBy``
    marker plus a ``providers`` map whose entry carries the transport fields
    AND the key. Tests that only need "this provider is keyed" use it; the
    byte-preservation test uses the extra fields to prove a rotation leaves
    them alone.
    """
    rel = home / ".openclaw" / "agents" / agent / "agent" / "plugins" / provider
    rel.mkdir(parents=True, exist_ok=True)
    (rel / "catalog.json").write_text(json.dumps({
        "generatedBy": "openclaw-plugin-model-catalog-v1",
        "providers": {
            provider_id or provider: {
                "api": "anthropic-messages",
                "baseUrl": f"https://api.{provider}.test",
                "models": [{"id": "m-1", "name": "m-1", "maxTokens": 64000}],
                "apiKey": key,
            },
        },
    }))
...
class TestGetBotSkills:
    ...
    # 19. LLM providers are verified against OC's per-agent runtime store.
    #
    # This test used to assert that enabled=True ALONE meant "configured",
    # on the stated premise that "API keys live in env / auth.profiles".
    # Measured 2026-09-04: they live in
    # agents/<a>/agent/plugins/<p>/catalog.json. Both directions are pinned
    # now, because only pinning one is how the old rule survived being wrong.
    def test_llm_providers_with_runtime_key_read_configured(self, tmp_path):
        oc = _make_oc(plugin_entries={
        ...
        for provider in ("anthropic", "openai", "xai", "google"):
            _write_agent_catalog(home, "main", provider, f"key-{provider}-000")
        inv = self._get_with_home(tmp_path, oc)
        ...
        # google is the Gemini LLM provider, NOT google_workspace — must not
        # be needs_oauth
        assert statuses["google"] == "configured"

    def test_llm_provider_enabled_with_no_key_anywhere_reads_missing_config(
        self, tmp_path,
    ):
        """The defect signal that was silently dead for LLM providers.

        An enabled plugin is a capability claim. With no key in
        auth-profiles, openclaw.json OR the per-agent runtime store, the bot
        advertises a model it cannot call — the anthropic-shaped twin of the
        fleet-wide brave failure (2026-07-31).
        """
        oc = _make_oc(plugin_entries={"anthropic": {"enabled": True}})
        inv = self._get_with_home(tmp_path, oc)
        assert {s.id: s.status for s in inv.skills}["anthropic"] == "missing_config"

    def test_llm_provider_status_unchanged_when_store_cannot_be_read(self):
        """``runtime_keyed=None`` means "could not look" — don't cry wolf.

        Distinct from an empty set, which means "looked, found nothing". CLI
        callers with no access to the store get the pre-2026-09 answer.
        """
        from evolve_admin.skills.inventory import _resolve_plugin_status
        oc = {"plugins": {"entries": {"anthropic": {"enabled": True}}}}
        assert _resolve_plugin_status(
            "anthropic", {"enabled": True}, oc, None,
        ) == "configured"
        assert _resolve_plugin_status(
            "anthropic", {"enabled": True}, oc, frozenset(),
        ) == "missing_config"
        assert _resolve_plugin_status(
            "anthropic", {"enabled": True}, oc, frozenset({"anthropic"}),
        ) == "configured"
...
class TestGetPodSkills:
    ...
    # internal/openclaw-coverage-audit-2026-06-04.md.
    ...
    def test_channels_passthrough_covers_all_audit_phase1_targets(self):
        ...
        assert not missing, (
            ...
            f"(internal/openclaw-coverage-audit-2026-06-04.md); without entries "
    ...
    # internal/skills-deep-audit-2026-05-30.md for the rationale: the probe was
    ...
    def test_apple_local_does_not_appear_in_pod_wide_matrix(self, tmp_path):
        ...
        assert "apple_local" not in result["matrix"], (
            ...
            "internal/skills-deep-audit-2026-05-30.md. Restore inventory entry "
...
class TestSkillsEndpoints:
    ...
    def test_pod_capability_summaries_excludes_gemini_only_bot(self, tmp_path):
        ...
        # Give the bot a real Gemini key in OC's per-agent runtime store, so
        # the plugin reads "configured" for the contrast this test draws.
        # Since 2026-09-04 `enabled` alone no longer implies a credential for
        # LLM providers — the Credentials tab and the Skills page both verify
        # it against this file (see web/oc_agent_keys.py).
        _write_agent_catalog(home, "main", "google", "AIzaSy-gem-bot-key")
