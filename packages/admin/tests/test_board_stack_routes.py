...
def test_decision_on_a_bot_card_still_offered_is_a_409(pod):
    cid = pod["bot_card"]["id"]  # owner: bot
    bs.set_delegation_progress(pod["shared"], "personal-bot", cid,
                                state="offered", actor="bot")
    r = pod["client"].post(f"/api/board/personal-bot/cards/{cid}/decision",
                           headers=_auth(pod), json={"decision": "approve"})
    assert r.status_code == 409
    assert "offered" in r.get_json()["error"]


def test_decision_on_a_bot_card_in_progress_is_a_409(pod):
    cid = pod["bot_card"]["id"]
    bs.set_delegation_progress(pod["shared"], "personal-bot", cid,
    ...
    r = pod["client"].post(f"/api/board/personal-bot/cards/{cid}/decision",
                           headers=_auth(pod), json={"decision": "approve"})
    assert r.status_code == 409
    assert "in_progress" in r.get_json()["error"]
