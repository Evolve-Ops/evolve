...
def test_reset_clears_rungs_roles(app_and_calls):
    """mode=default writes empties for rungs/roles/roleCaps; the oc_model
    wholesale path drops them so the file carries no per-bot tier definitions
    and the merge inherits the pod/code default."""
    app, cfg_calls, audit_calls = app_and_calls
    with app.test_client() as c:
    ...
    assert len(cfg_calls) == 1
    updates = cfg_calls[0]["updates"]
    assert updates == {"rungs": [], "roles": {}, "roleCaps": {}}
