...
def test_unlink_failure_leaves_manifest_on_disk(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda target, *, bot_id: (False, "no sudoers grant in tests", False),
    )
...
def test_permission_error_falls_back_to_privileged_delete_and_archives(client, monkeypatch):
    ...
    def fake_privileged_delete(target, *, bot_id):
        privileged_calls.append((Path(target), bot_id))
