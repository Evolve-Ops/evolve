...
# (docs/spec-app-derived-permissions-2026-05-24.md §7)
#
...
def test_installed_package_version_normalizes_prerelease(tmp_path):
    pkg = tmp_path / "package.json"
    pkg.write_text(json.dumps({"version": "2026.5.20-beta.1"}))
    # canonical_version strips the prerelease tag so the banner doesn't show
    # "2026.5.20-beta.1" when the installed binary is the .0 release.
    assert uv.installed_package_version(package_json=pkg) == "2026.5.20"
