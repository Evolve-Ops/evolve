...
def read_screen(lines: list[str]) -> list[list[bool]]:
    """The module matrix a camera would see in a rendered block of CHARACTERS.

    The inverse of ``qr_terminal``'s two glyph renderers — the fallbacks a
    console without colour gets — written here rather than imported so that a
    renderer bug cannot cancel itself out: an inverted or transposed block
    decodes to garbage instead of round-tripping.

    The primary renderer paints background colours behind spaces, which no
    character reader can see; ``tests/_ansi_raster.py`` reads that one, and
    reads it as pixels, because glyph metrics were the 2026-09-04 failure.
    """
