...
def app(tmp_path: Path, monkeypatch):
    ...
    routes_cost_measures._THRASH_MEMO.clear()
    a = Flask(__name__)
...
def test_cache_thrash_endpoint_reports_tokens_not_just_turns(app, monkeypatch):
    """The 09-17 turn read 5.4M and wrote 8.2M: a HIT by the re-warm
    definition, thrash by this one, and rendered by magnitude."""
    import cache_shape
    rows = _sparse_turns(n=2, days=1) + [{
        "session_id": "s-thrash", "ts": DAY.isoformat(),
    ...
    with app.test_client() as c:
        body = c.get("/api/bot/cache-thrash?bot=admin_bot").get_json()
    t = body["thrash"]
    assert t["cache_thrash_turns"] == 1
    assert t["cache_thrash_write_tokens"] == 8_186_626
    assert t["window_days"] == cache_shape.AUTO_TIER_WINDOW_DAYS


def test_cache_thrash_endpoint_unreadable_is_null_not_zero(app, monkeypatch):
    import cache_shape
    monkeypatch.setattr(cache_shape, "load_turn_records", lambda bot_id, **kw: None)
    with app.test_client() as c:
        body = c.get("/api/bot/cache-thrash?bot=admin_bot").get_json()
    assert body["thrash"] is None
