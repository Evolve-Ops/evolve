...
# ── review FILES: the LAST `##` section that states a verdict is the record ───
#
# `extract_verdict` returns the FIRST labelled verdict in what it is given, and
# `--pm-verdict` used to hand it a whole review file. So a `## Re-review` could
# never be seen: on 2026-09-07 #4009/#4010/#4024/#4038 each carried an appended
# `Verdict: PASS`, all four still reported the original CONCERNS, and all four
# were merged by hand. `extract_file_verdict` is the fix, and these pin both
# what it changed and the three things it deliberately did not.
...
_TWO_PASS_CONCERNS = (
    ...
    "## Two-pass review\n"
    "Reviewer: PM lane\n"
    "Verdict: CONCERNS — the resolver stops one hop short of the real binary.\n"
    "\n"
    "### Pass 2 — findings\n"
    "The fixture cannot see this.\n"
    "\n"
    "## Hold\n"
    "1. `firewall_allow.py`: map the launcher to the framework binary.\n")


def test_a_re_review_supersedes_the_review_above_it():
    body = (_TWO_PASS_CONCERNS
            + "\n## Re-review 2026-09-07 — fix commit abc1234\n"
              "Verdict: PASS — the hold item is discharged with a fixture.\n")
    verdict = mvc.extract_file_verdict(body)
    assert mvc.verdict_is_pass(verdict) is True, verdict
    assert mvc.verdict_is_blocking(verdict) is False, verdict
    # The whole-file read this replaced returns the SUPERSEDED verdict.
    assert mvc.verdict_is_blocking(
        mvc.extract_verdict("## PM review\n" + body)) is True


def test_a_re_review_can_also_block_a_previously_passing_file():
    """Supersession runs both ways — 'latest wins' is not 'pass wins'."""
    body = ("# Review: PR #1 — a thing\n"
            "\n"
            "## Two-pass review\n"
            "Verdict: PASS — clean.\n"
            "\n"
            "## Re-review 2026-09-07 — fix commit abc1234\n"
            "Verdict: FAIL — the fix reopened the hole it closed.\n")
    verdict = mvc.extract_file_verdict(body)
    assert mvc.verdict_is_blocking(verdict) is True, verdict
    assert mvc.verdict_is_pass(verdict) is False, verdict


def test_a_trailing_hold_section_does_not_displace_the_verdict():
    """On a file with no re-review the LAST `##` section is `## Hold` — a remedy
    list, not a judgement. Counting it would let a numbered item displace the
    verdict it belongs to (the live #4036 / #4079 shape)."""
    verdict = mvc.extract_file_verdict(_TWO_PASS_CONCERNS)
    assert verdict is not None
    assert verdict.upper().startswith("CONCERNS"), verdict
    assert mvc.verdict_is_blocking(verdict) is True


def test_a_pass_shaped_line_in_a_subsection_does_not_displace_the_verdict():
    """`###` blocks are internals of ONE pass and their prose is full of
    verdict-shaped tokens; only `##` sections are revisions."""
    body = ("## Two-pass review\n"
            "Verdict: CONCERNS — one blocker remains.\n"
            "\n"
            "### Pass 2 — findings\n"
            "PASS on the unit tests; the integration leg is the problem.\n")
    verdict = mvc.extract_file_verdict(body)
    assert mvc.verdict_is_blocking(verdict) is True, verdict


def test_a_file_with_no_heading_falls_back_to_the_whole_body_read():
    """The fallback is why this change can only move a verdict FORWARD to a
    later section, never turn one it used to find into None."""
    assert mvc.verdict_is_pass(
        mvc.extract_file_verdict("Verdict: PASS — no headings anywhere.\n")) is True
    assert mvc.extract_file_verdict("no verdict here at all.\n") is None


def test_the_pr_body_rule_is_untouched_by_the_file_rule():
    """`verdict_qualifies` reads a PR BODY, where several sections are several
    INDEPENDENT reviewers and the body is author-editable — so any blocker
    still holds, and a later PASS section must NOT release it."""
    body = ("## Two-pass review\n"
            "Verdict: FAIL — the migration is unsafe.\n"
            "\n"
            "## Independent two-pass review\n"
            "Verdict: PASS — looks fine to me.\n")
    ok, _verdict, reason = mvc.verdict_qualifies(body)
    assert ok is False
    assert "blocking" in reason


@pytest.mark.parametrize("pr", [4009, 4010, 4024, 4038])
def test_the_real_stranded_review_files_now_read_their_re_review(pr):
    """The four files the incident was made of, read from the corpus."""
    path = _REPO / "internal" / "dispatch" / "reviews" / ("pr-%d.md" % pr)
    if not path.is_file():                  # synced consumer checkout: nothing to pin
        pytest.skip("%s not present in this checkout" % path)
    text = path.read_text(encoding="utf-8")
    if "## Re-review" not in text:          # a later edit may rewrite the top line instead
        pytest.skip("pr-%d.md no longer carries a re-review section" % pr)
    verdict = mvc.extract_file_verdict(text)
    assert mvc.verdict_is_pass(verdict) is True, verdict
    assert mvc.verdict_is_blocking(verdict) is False, verdict


def test_cli_review_verdict_exit_codes_and_json(tmp_path, capsys):
    path = tmp_path / "pr-1.md"
    path.write_text(_TWO_PASS_CONCERNS, encoding="utf-8")
    assert mvc.main(["--review-verdict", str(path), "--json"]) == 1
    out = json.loads(capsys.readouterr().out)
    assert out["verdict_is_blocking"] is True and out["verdict_is_pass"] is False

    path.write_text(_TWO_PASS_CONCERNS
                    + "\n## Re-review 2026-09-07 — fix commit abc1234\n"
                      "Verdict: PASS — discharged.\n", encoding="utf-8")
    assert mvc.main(["--review-verdict", str(path), "--json"]) == 0
    out = json.loads(capsys.readouterr().out)
    assert out["verdict_is_pass"] is True


def test_cli_review_verdict_refuses_a_pr_number_and_a_second_mode(tmp_path):
    path = tmp_path / "pr-1.md"
    path.write_text(_TWO_PASS_CONCERNS, encoding="utf-8")
    with pytest.raises(SystemExit) as e:
        mvc.main(["4038", "--review-verdict", str(path)])
    assert e.value.code == 2
    with pytest.raises(SystemExit) as e:
        mvc.main(["--review-verdict", str(path), "--hold", str(path)])
    assert e.value.code == 2


# ── D-MT4: the `Model:` line ──────────────────────────────────────────────────


def test_extract_model_reads_a_labelled_line():
    assert mvc.extract_model("Reviewer: fable\nModel: sonnet-5\n\nmore text") == "sonnet-5"


def test_extract_model_absent_is_none():
    assert mvc.extract_model("Reviewer: fable\n\nVerdict: PASS\n") is None


def test_extract_model_ignores_a_fenced_example():
    text = "```\nModel: not-a-real-model\n```\nModel: opus\n"
    assert mvc.extract_model(text) == "opus"


def test_cli_review_verdict_json_carries_model_and_warns_without_one(tmp_path, capsys):
    path = tmp_path / "pr-1.md"
    path.write_text("## Two-pass review\n\nReviewer: fable\nModel: sonnet-5\n\n"
                    "Verdict: PASS — fine.\n", encoding="utf-8")
    assert mvc.main(["--review-verdict", str(path), "--json"]) == 0
    out = json.loads(capsys.readouterr().out)
    assert out["model"] == "sonnet-5"

    path.write_text("## Two-pass review\n\nReviewer: fable\n\nVerdict: PASS — fine.\n",
                    encoding="utf-8")
    assert mvc.main(["--review-verdict", str(path), "--json"]) == 0  # warns, does not fail
...
def test_cli_review_verdict_require_model_fails_without_one(tmp_path):
    path = tmp_path / "pr-1.md"
    path.write_text("## Two-pass review\n\nReviewer: fable\n\nVerdict: PASS — fine.\n",
                    encoding="utf-8")
    assert mvc.main(["--review-verdict", str(path), "--require-model"]) == 1
    path.write_text("## Two-pass review\n\nReviewer: fable\nModel: sonnet-5\n\n"
                    "Verdict: PASS — fine.\n", encoding="utf-8")
    assert mvc.main(["--review-verdict", str(path), "--require-model"]) == 0
