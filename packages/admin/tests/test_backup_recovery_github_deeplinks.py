...
def test_backup_card_offers_guided_setup_when_unconfigured():
    ...
    # Slice to the NEXT top-level declaration rather than a fixed byte
    # window. The old `start + 8000` had ~200 bytes of headroom left, so
    # any edit that grew the function early on pushed `guidedSetupCTA`
    # out of view and reported it as MISSING — a false red pointing at
    # the wrong thing (hit 2026-08-31 by an unrelated two-line change).
    end = html.find("\nfunction ", start + 1)
    fn_body = html[start:end if end > start else len(html)]
