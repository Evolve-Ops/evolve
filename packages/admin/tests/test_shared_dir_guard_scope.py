...
def test_guard_is_active_for_an_ordinary_test():
    ...
    with pytest.raises(PermissionError, match=r"\[shared-dir-guard\]"):
...
def test_exempt_context_lets_the_write_through_to_the_os(monkeypatch):
    ...
    PermissionError, and one that let the guard be bypassed generally would be
    caught by test_guard_is_active_for_an_ordinary_test above.
    """
