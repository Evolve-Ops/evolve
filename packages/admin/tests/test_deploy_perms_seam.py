...
_EVO_WRITE_ACE = (
    # SOURCE form (what we hand to `chmod +a`), not the resolved form `ls -lde`
    # prints. `execute` is the traverse bit that lets keystore/ drop to 0750.
    "user:evo allow read,write,execute,delete,append,"
...
def test_fix_shared_dir_permissions_subdir_acl_argv_parity(
    ...
    # covered by test_tier_prefs_acl.py. Neutralize so the six-read-ACE
    ...
    expected_targets = [
        ...
        # Absorbed-trailer ledger (design-exec-failure-hygiene-2026-08-31 A1).
        # Same read-ACE reason as spans/cascade: the plugin writes it at the
        # gateway umask (077 → 0600), and exec_failure_monitor reads it as
        # evolve.
        str(shared / "examplebot" / "exec-failures"),
    ]
    assert acl_calls == [
...
def test_fix_shared_dir_permissions_precreates_app_runs_bot_owned(
        tmp_path, monkeypatch, deploy_runner, seam_runner):
    """AL-1.2's claim dir is pre-created here for the same reason spans/ and
    cascade/ are: the bot's ACE on ``{sharedDir}/{botId}/`` grants add_file but
    NOT add_subdirectory, so the app-run shim's own mkdir would EACCES on a pod
    where that parent is evolve-owned — and every scheduled turn would stay
    silently unattributed. Bot-owned 0755, and deliberately NO evolve-read ACE:
    claims are written and consumed by the bot alone."""
    run, seam_calls = seam_runner
    set_perms(MacOSPerms(runner=run))
    monkeypatch.setattr(deploy, "_bot_user_for", lambda bot_id: "examplebot")
    monkeypatch.setattr(deploy._tier_prefs_acl, "ensure_bot_tier_prefs_acl",
    ...
    claim_dir = shared / "examplebot" / "app-runs"
    assert claim_dir.is_dir()
    assert oct(claim_dir.stat().st_mode)[-3:] == "755"
    assert [c for c in deploy_runner
            if c[1].endswith("chown") and c[2].startswith("examplebot:")
            and c[-1] == str(claim_dir)], "claim dir must end up bot-owned"
    assert not any(str(claim_dir) in c for c in seam_calls), (
        "no evolve-read ACE on the claim dir — evolve never reads claims"
    )
