...
# Matches the convention used across this suite for tests that read a source
# file off disk rather than importing it.
...
def test_audit_critical_event_pages_and_is_distinct_from_the_umbrella():
    """audit.py's CRITICAL page-on-transition batch has its own IMMEDIATE
    catalog event, separate from the DAILY_DIGEST umbrella.

    The umbrella (security.audit_finding) is ERROR / DAILY_DIGEST on purpose
    — it carries the flap-prone WARN+ERROR traffic that caused the
    2026-06-28 flood. Delivery cadence is a property of the EVENT, not of the
    message, so annotating the CRITICAL batch with the umbrella made the
    dispatcher return DEFERRED for it: "page on transition" landed in the
    next daily digest, up to ~24h late. Pin the split so a future cleanup
    can't collapse the two keys back together."""
    crit = cat.by_key("security.audit_critical")
    umbrella = cat.by_key("security.audit_finding")
    assert crit is not None and umbrella is not None

    # Same producer, same operator Subscription group — different cadence.
    assert crit.producer_source == umbrella.producer_source == "audit"
    assert crit.subscription == umbrella.subscription == "security_findings"
    assert crit.category is Category.SECURITY

    # The critical key pages; the umbrella digests.
    assert crit.severity is Severity.CRITICAL
    assert crit.default_enabled is True
    assert crit.default_frequency is Frequency.IMMEDIATE
    assert crit.allowed_frequencies == (Frequency.IMMEDIATE,), (
        "immediate-only: offering a digest option here would re-open the "
        "routing hole this event closed"
    )
    assert umbrella.default_frequency is Frequency.DAILY_DIGEST

    # Confirm-before-mute at the event level too, matching cve_finding.
    assert crit.is_safety_critical is True

    # Sample payload renders cleanly with the approved critical header.
    out = render_event(crit, crit.sample_payload)
    assert out.lstrip().startswith("🔴 ")
    assert "CRITICAL Findings" in out


def test_audit_producer_annotates_its_critical_batch_with_the_page_key():
    """The producer side of the split: audit.py's dispatcher call carries
    security.audit_critical. Read from source rather than executing the
    module — the analyzer package isn't importable from the admin suite.

    Scoped to the _send_via_dispatcher BODY, not the whole file. A whole-file
    negative on the umbrella key would be a fragile ratchet: audit.py is
    ~6000 lines and security.audit_finding's own producer_source is "audit",
    so a future audit-side send that legitimately belongs on the umbrella (a
    warn roll-up, a posture summary) would fail this test for no reason.
    """
    src = (REPO_ROOT / "packages" / "analyzer" / "audit.py").read_text()

    start = src.index("def _send_via_dispatcher")
    # The next top-level def ends the function body.
    end = src.index("\ndef ", start + 1)
    body = src[start:end]

    assert 'catalog_event="security.audit_critical"' in body, (
        "audit.py's CRITICAL batch must dispatch under the IMMEDIATE key"
    )
    assert 'catalog_event="security.audit_finding"' not in body, (
        "the DAILY_DIGEST umbrella must not annotate this page-on-transition "
        "send — it resolves to DEFERRED on a default-configured pod"
    )
    assert body.count("catalog_event=") == 1, (
        "the page-on-transition send is the only dispatch in this function; "
        "a second one needs its own routing decision made consciously"
    )
...
def test_2026_06_28_flood_set_collapses_to_digest():
    ...
    finding, and the must-act path stays IMMEDIATE for it: the daily CVE scan
    via security.cve_finding, and the audit's own CRITICAL page-on-transition
    batch via security.audit_critical.

    (D1 originally recorded cve_finding alone as that carrier. It never was
    one for the audit producer — cve_finding is dispatched only by
    security-cve-scan — which is why audit's CRITICAL batch, annotated with
    the umbrella key, digested until security.audit_critical was added.)"""
    flood_to_digest = (
    ...
    for key in ("security.cve_finding", "security.audit_critical"):
        e = cat.by_key(key)
        assert e is not None, key
        assert e.severity is Severity.CRITICAL, key
        assert e.default_frequency is Frequency.IMMEDIATE, key
...
def test_audit_posture_findings_have_no_must_page_slot():
    """R-3 event-vs-posture taxonomy (design:
    internal/design-security-alert-fatigue-2026-08-31.md §2.3). Audit
    findings — posture-classified criticals included — deliver under the
    security.audit_finding umbrella (signal_notifier), so that key (and any
    other audit-produced security key) must never sit in
    MUST_PAGE_ALLOWLIST or it would flip every standing posture violation to
    an IMMEDIATE default and push it on every rebroadcast. Posture criticals
    page once on signal creation/reopen (R-1's transition gate); the
    allowlist is reserved for enforcement/liveness transitions. A
    genuine-tamper L2 event class earning a slot later must use a NEW key,
    not this umbrella - update this test consciously when that lands.

    (This docstring used to describe the allowlist as conferring a
    "rate-cap-bypass page". It does not: MUST_PAGE_ALLOWLIST is read at one
    site in production code, derive_default_frequency, and only picks a
    default cadence. Rate-cap bypass is dispatcher._is_bypass_alert, which
    keys off severity and is_safety_critical and never consults this
    frozenset. Corrected 2026-09-01 alongside security.audit_critical, which
    inherited the error.)

    security.audit_critical (2026-09-01) is NOT that reserved key. It carries
    audit's CRITICAL page-on-transition batch on a cadence split (page vs
    digest); dispatch_findings does not branch on any event-vs-posture
    classification, so both kinds page through it and the L2 tamper class
    remains unclaimed. #3918's PR body records an edr co-sign stating that
    posture create/reopen pages are non-negotiable (R-1's transition gate
    already gives "page once on discovery, then board"), so the
    non-branching reads as deliberate — though that co-sign text is
    unattributable: #3918 carries no review or comment artifact behind it.
    See design §4 for the precise scope of that evidence. It deliberately takes no allowlist slot
    either - for a CRITICAL entry a slot is simply redundant, since
    severity=CRITICAL already earns IMMEDIATE through
    derive_default_frequency."""
    assert "security.audit_finding" not in cat.MUST_PAGE_ALLOWLIST
    assert "security.config_drift" not in cat.MUST_PAGE_ALLOWLIST
    assert "security.audit_critical" not in cat.MUST_PAGE_ALLOWLIST
    security_keys = {
        k for k in cat.MUST_PAGE_ALLOWLIST if k.startswith("security.")
    }
    assert not security_keys, (
        "security.* keys in MUST_PAGE_ALLOWLIST would default every posture "
        f"finding to an IMMEDIATE push: {sorted(security_keys)}"
    )
