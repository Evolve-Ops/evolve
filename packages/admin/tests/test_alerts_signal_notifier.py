...
def test_audit_signals_stay_on_the_digest_umbrella_whatever_their_severity():
    """The invariant that keeps security.audit_critical rare.

    audit.py pages its CRITICAL page-on-transition batch directly under
    security.audit_critical (IMMEDIATE). signal_notifier separately walks the
    Signal store, and its mapping is per-PRODUCER, not per-severity — so
    re-pointing ``producer == "audit"`` at the CRITICAL key would send every
    warn-level audit Signal to an IMMEDIATE event and reproduce the 2026-06-28
    push flood exactly, on top of double-messaging each critical the batch
    already paged.

    Pinned here rather than left to a comment because it is the property three
    separate documents lean on when they justify leaving this mapping alone.
    A deliberate future change to the split must edit this test consciously.
    """
    from evolve_admin.alerts.signal_notifier import _catalog_event_for_signal

    class _S:
        def __init__(self, producer, sig_type="", severity="warn"):
            self.producer = producer
            self.type = sig_type
            self.severity = severity

    for severity in ("alert", "warn", "info"):
        assert _catalog_event_for_signal(
            _S("audit", "audit_world_readable_creds", severity=severity)
        ) == "security.audit_finding", (
            f"audit Signals of every severity ({severity}) stay on the "
            f"DAILY_DIGEST umbrella; only audit.py's own batch pages"
        )

    # And the notifier must not be routed around: "audit" is deliberately NOT
    # a direct-dispatch producer, so its Signals still reach the digest.
    # Excluding it would silence the warn-level standing board too, which is
    # the digest's whole job.
    #
    # This does pin the per-critical digest echo (page now, digest line next
    # morning) as an invariant, so read it before picking up the follow-up
    # named in design-security-alert-fatigue-2026-08-31.md §4: that follow-up
    # is a per-SIGNAL filter (suppress the echo when the Signal already
    # carries a non-suppressed delivery), not a producer exclusion. Adding
    # "audit" here is the wrong fix for it.
    from evolve_admin.alerts import signal_notifier as sn

    assert "audit" not in sn._DIRECT_DISPATCH_PRODUCERS
...
def test_catalog_event_mapping_for_host_health_and_error_reporter(env):
    ...
    # forge_engine Phase 4.5 materialize failure (audit slate S2): install-
    # shaped, rides the app_install_failed handle. Other forge_engine types
    # (cost overrun) stay on the digested catch-all.
    assert _catalog_event_for_signal(_S("forge_engine", "forge_scheduled_action_failed")) == "system.app_install_failed"
    assert _catalog_event_for_signal(_S("forge_engine", "forge_install_cost_overrun")) == "meta.unclassified"
...
_KNOWN_SIGNAL_PRODUCERS: tuple[tuple[str, str], ...] = (
    ...
    ("forge_engine", "forge_scheduled_action_failed"),
    ("forge_engine", "forge_install_cost_overrun"),  # → digested catch-all
    ...
    ("credential_access", "credential_store_unreadable"),
    ...
    # Layer 1 plugin-install provenance gate (2026-08-11).
    ("plugin_install_provenance", "plugin_install_refused_unlisted"),
    ("plugin_install_provenance", "plugin_install_gate_error"),
    # Absorbed exec-failure shapes (exec-failure hygiene A2). DELIBERATELY
    # unmapped: digest-only routing via the meta.unclassified catch-all is
    # the design ("quiet observation first"); a future A3 mapper branch
    # must not silently change that without revisiting the design.
    ("bot_exec_failures", "exec_failure_shape"),
...
def test_render_fire_truncates_at_text_boundary():
    """Long bodies are cut at a paragraph/word boundary, never mid-word.

    The 200-char hard cut beheaded the sudoers alert's remediation command
    ("sudo evolve-admi…" — VPS, 2026-07-29). The voice-guide body shape puts
    the plain what/impact/action first and "Technical detail:" last, so the
    cap must fit the full plain part and truncation should only eat the
    technical tail."""
    from evolve_admin.alerts.signal_notifier import (
        _BODY_EXCERPT_CHARS, _render_fire, _truncate_at_boundary,
    )

    plain = (
        "Evolve pulled an update that needs new admin permissions, but for "
        "safety it can't grant those to itself — an operator has to approve "
        "them by running one command on the pod (as root):\n\n"
    ...
    class _Sig:
        ...
        body = plain + "\n\nTechnical detail: " + "x" * 600

    out = _render_fire(_Sig())
    # The whole plain part — including the command — survives verbatim.
    assert "sudo evolve-admin refresh-sudoers" in out
    assert "This alert clears itself once the command has run." in out
    # Truncation happened (the technical tail is clipped), and not mid-word.
    assert out.endswith("…")
    # Boundary helper: cuts at the last paragraph break inside the limit…
    assert _truncate_at_boundary("a" * 300 + "\n\n" + "b" * 400, 600) == "a" * 300 + "…"
    # …or the last space when no newline is available…
    cut = _truncate_at_boundary("word " * 200, 600)
    assert cut.endswith("word…") and len(cut) <= 601
    # …and hard-cuts only when there is no boundary in the back half.
    assert _truncate_at_boundary("z" * 700, 600) == "z" * 600 + "…"
    # Short bodies pass through untouched.
    assert _truncate_at_boundary("short", _BODY_EXCERPT_CHARS) == "short"
...
def _seed_config_drift(shared, **overrides):
    ...
    base = dict(
        ...
        scope="pod",
        bot_id=None,
        title="pod file permissions need a repair run: 3 targets waiting",
...
# ── Cold-start observability trail (2026-07-29) ──────────────────────────────


def test_cold_start_leaves_a_never_announced_trail(tmp_path, monkeypatch, caplog):
    """The cold-start guard silences a backlog permanently — that has to be
    legible after the fact.

    On 2026-07-29 both live pods were found with a firing backlog nobody had
    ever been told about (95/99 on the mini, 34/36 on the Linux VPS), and the
    state file gave no way to distinguish "deliberately silenced at cold
    start" from "delivery is broken". Each synthesized entry now carries
    ``never_announced`` + ``cold_start_synced_at`` + ``cold_start_reason``,
    a pod-level ``cold_start`` record is appended, and the count is logged.
    """
    import logging

    env = _cold_env(tmp_path, monkeypatch)
    for bot_id in ("team_bot_a", "admin_bot"):
        _seed_signal(env["shared"], bot_id=bot_id,
                     signature=f"pod_health:pod_health_gateways:{bot_id}:gateway",
                     title=f"{bot_id} gateway down")

    with caplog.at_level(logging.WARNING,
                         logger="evolve_admin.alerts.signal_notifier"):
        env["sn"].run_once(env["shared"], env["network"])

    assert env["sent"] == []
    state = _state(env["shared"])

    for bot_id in ("team_bot_a", "admin_bot"):
        entry = state["signatures"][
            f"pod_health:pod_health_gateways:{bot_id}:gateway"
        ]
        assert entry["cold_start_synced"] is True
        assert entry["never_announced"] is True
        assert entry["cold_start_reason"] == "first_run"
        assert entry["cold_start_synced_at"]

    trail = state["cold_start"]
    assert len(trail) == 1
    assert trail[0]["synced_count"] == 2
    assert trail[0]["reason"] == "first_run"
    assert trail[0]["announced"] is False
    assert trail[0]["at"]

    assert any(
        "cold-start" in r.message and "WITHOUT any operator message" in r.message
        for r in caplog.records
    ), "the silenced count must be logged, not only recorded on disk"


def test_cold_start_trail_is_bounded():
    """A notifier whose state file keeps getting wiped must not grow the
    trail forever."""
    from evolve_admin.alerts import signal_notifier as sn

    state: dict = {"signatures": {}}
    now = datetime(2026, 7, 29, tzinfo=timezone.utc)
    for i in range(sn._COLD_START_TRAIL_MAX + 5):
        sn._record_cold_start(
            state, now + timedelta(hours=i), synced=i + 1, reason="first_run",
        )
    assert len(state["cold_start"]) == sn._COLD_START_TRAIL_MAX
    # Newest kept, oldest dropped.
    assert state["cold_start"][-1]["synced_count"] == sn._COLD_START_TRAIL_MAX + 5


def test_delivery_recovery_resync_is_tagged_as_such(env):
    """The other _sync_firing_as_known call site: a backlog re-synced after a
    dead delivery target healed is a DIFFERENT kind of silence from a first
    run, and the trail has to say which."""
    p = env["shared"] / "signals" / "notifier-state.json"
    state = json.loads(p.read_text())
    state["delivery_down"] = {"down_since": "2026-06-23T00:00:00+00:00"}
    p.write_text(json.dumps(state))
    ...
    assert stats.delivery_recovered == 1
    after = _state(env["shared"])
    trail = after["cold_start"]
    assert trail[-1]["reason"] == "delivery_recovery"
    entry = after["signatures"][
        "pod_health:pod_health_gateways:team_bot_a:gateway"
    ]
    assert entry["never_announced"] is True
    assert entry["cold_start_reason"] == "delivery_recovery"


def test_a_real_fire_push_clears_the_never_announced_marker(env):
    """Phase A rewrites the entry from scratch — that is what flips a
    signature from silenced to announced, and standing_alerts depends on it
    (otherwise the never-announced count could never go down)."""
    sig = _seed_signal(env["shared"], severity="warn")
    p = env["shared"] / "signals" / "notifier-state.json"
    p.write_text(json.dumps({"version": 1, "signatures": {
        sig.signature: {
            "alerted_for_signal_id": "sig_stale_other_id",
    ...
    stats = env["sn"].run_once(env["shared"], env["network"], now=now)
    assert stats.fired == 1
    entry = _state(env["shared"])["signatures"][sig.signature]
    assert entry["alerted_for_signal_id"] == sig.id
    assert "cold_start_synced" not in entry
    assert "never_announced" not in entry
