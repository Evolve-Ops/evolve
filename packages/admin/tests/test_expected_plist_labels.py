...
def test_usage_logger_in_expected_labels():
    """``ai.evolve.evolve.usage-logger`` is installed by
    _install_launchd_usage_logger("evolve") during install-infra-jobs.
    Daily usage aggregator feeding the Usage tab + Anthropic cost
    reconciliation; deleting it freezes both surfaces."""
    network = {"members": ["team_bot_a"], "bots": {}}
    labels = expected_plist_labels(network)
