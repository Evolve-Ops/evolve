...
def fake_workspace(tmp_path: Path, monkeypatch):
    """Tmp dir that pretends to be the bot home's .openclaw/workspace.

    The helper resolves the workspace via the ``user_home`` seam
    (profile-keyed — never a hardcoded /Users prefix), so redirecting the
    seam to a tmp home is all the harness needs.
    """
    home = tmp_path / "home"
