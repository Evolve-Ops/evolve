...
Design: internal/design-linux-port-2026-06-10.md §5 — ONE writer, TWO command
tables. Three layers:
...
# ── 5. §7c — the OC-upgrade root helper grant (spec-oc-upgrade-from-ui) ──────
#
# Verification item **S1**. The admin UI's "Run upgrade now" button needs root
# for the global `npm install -g`, and the grant that gets it there is the ONE
# thing standing between an HTTP endpoint and a root postinstall. Both halves
# are pinned: the grant exists in both renders (already visudo-checked above by
# test_render_passes_visudo_syntax_check), and the alternative that would have
# been a vulnerability is proven absent.
...
def test_oc_upgrade_helper_grant_is_rendered(monkeypatch, profile, oc_path) -> None:
    set_profile(profile)
    content = _render(monkeypatch, oc_path)
    expected = (
        f"evolve ALL=(root) NOPASSWD: {profile.venv_python} "
        f"{profile.deploy_checkout_default}/packages/admin/evolve_admin/"
        f"oc_upgrade_apply.py --report-id *\n"
    )
    assert expected in content, (
        "the §7c OC-upgrade helper grant is missing or drifted — sudo matches "
        "the command as a literal string, so a single changed byte is a dead "
        "grant and every UI-triggered upgrade fails with a password prompt"
    )
...
def test_no_npm_install_grant_ever_appears(monkeypatch, profile, oc_path) -> None:
    """The grant §7c deliberately is NOT.

    `<npm> install -g --prefix=<prefix> openclaw@*` looks narrow and isn't:
    sudo's `*` is fnmatch-style and matches npm's ALIAS syntax, so
    `openclaw@npm:some-other-package` satisfies it and runs an arbitrary
    package's postinstall as root — reachable from an HTTP endpoint. If this
    ever needs to change, it needs a design conversation, not a patch.
    """
    set_profile(profile)
    content = _render(monkeypatch, oc_path)
    for line in content.splitlines():
        if line.startswith("#") or "NOPASSWD" not in line:
            continue
        assert "openclaw@" not in line, f"a package-spec grant appeared: {line}"
        assert "npm install" not in line, f"an npm-install grant appeared: {line}"


def test_helper_path_constant_matches_the_grant(monkeypatch) -> None:
    """`oc_upgrade_apply.helper_script_path()` renders the byte-identical path
    the grant does — both off `platform_profile`'s deploy checkout, so the
    invoker's argv and the grant cannot drift."""
    set_profile(MACOS)
    from evolve_admin import oc_upgrade_apply  # noqa: PLC0415

    content = _render(monkeypatch, MAC_OC_PATH)
    assert f"NOPASSWD: {MACOS.venv_python} {oc_upgrade_apply.helper_script_path()} --report-id *" in content
