...
Coverage:
  ...
  - PUT (set) validates the candidate (non-empty ladder roles), writes
    rungs/roles into network.json, and PRESERVES models.embedding.
  - PUT (set) carrying a stale judge role has it dropped by the write-path
    canonicalization (the judge role is gone — judge-role collapse §5.4).
...
# A valid candidate pod default (standard spans two providers — a
# cross-vendor option for the derivation, though the endpoint no longer
# enforces it).
_VALID = {
    "rungs": [
...
def test_set_writes_rungs_roles_and_preserves_embedding(app_env):
    ...
    # the code default instead of accumulating: 4 canonical rungs (the judge
    # role and its rung are gone — judge-role collapse §5.4).
    ...
    # Each role points at its canonical rung; roles are uniformly slugs.
    ...
    assert "judge" not in models["roles"]
    # The clusters' members survive the rename.
...
def test_set_stale_judge_role_is_dropped_not_an_error(app_env):
    """A candidate still carrying the old structured judge role (a stale
    client) saves fine — the write-path canonicalization drops the stale role
    and its rung; the former provider-diversity 400 died with the judge role
    (judge-role collapse §5.4)."""
    app, network_path = app_env
    stale = json.loads(json.dumps(_VALID))
    stale["rungs"].append({"id": "judge-default", "models": ["prov_b/std"]})
    stale["roles"]["judge"] = {"rung": "judge-default", "provider": "not-standard"}
    with app.test_client() as c:
        resp = c.put("/api/admin/config/pod/models", json=stale)
    assert resp.status_code == 200, resp.get_json()
    models = _read_models(network_path)
    assert "judge" not in models["roles"]
    assert all(r["id"] != "judge-class" for r in models["rungs"])
    assert models["embedding"] == _EMBEDDING
