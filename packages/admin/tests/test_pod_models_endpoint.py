...
Spec: docs/spec-model-rungs-and-roles-2026-06-09.md §Addendum 5 — the POD-tab
...
def test_set_writes_rungs_roles_and_preserves_embedding(app_env):
    ...
    # the code default instead of accumulating. Assert the canonicalized shape,
    # not the synthetic-id input — judge folds into the standard (sonnet-class)
    # rung, so the saved catalog has 4 rungs (max included).
    saved_ids = [r["id"] for r in models["rungs"]]
    assert saved_ids == ["haiku-class", "sonnet-class", "opus-class", "fable-class"], saved_ids
    ...
    assert models["roles"]["judge"] == {"rung": "sonnet-class", "provider": "not-standard"}
    # The clusters' members survive the rename (judge's extra model folded in).
