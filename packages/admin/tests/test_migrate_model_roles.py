...
def test_migrate_models_block_basic():
    ...
    # Rungs are cost-ordered cheapest-first.
    ...
    # Roles map tier keys → role IDs; judge is structured + folds tier0 model
    # into the sonnet-class rung.
    assert out["roles"]["fast"] == "haiku-class"
    assert out["roles"]["standard"] == "sonnet-class"
    assert out["roles"]["power"] == "opus-class"
    assert out["roles"]["judge"] == {"rung": "sonnet-class", "provider": "not-standard"}
...
def test_default_canonicalization_renames_backfills_rederives_preserves_embedding():
    ...
    # Canonical ids, cost-ordered, judge folded into sonnet-class (4 rungs).
    ids = [r["id"] for r in out["rungs"]]
    assert ids == ["haiku-class", "sonnet-class", "opus-class", "fable-class"], ids
