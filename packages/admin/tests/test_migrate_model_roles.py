...
def test_migrate_models_block_basic():
    ...
    # Rungs are cost-ordered cheapest-first. No judge-class rung — tier0's
    # models merge into the standard chain (judge-role collapse §6).
    rung_ids = [r["id"] for r in out["rungs"]]
    assert rung_ids == ["haiku-class", "sonnet-class", "opus-class"]
    ...
    # Roles map tier keys → role IDs, uniformly slug-shaped; no judge role.
    ...
    # tier0's model landed on the standard chain (already there via the tier2
    # fallback in this fixture — dedup keeps it single).
    sonnet = next(r for r in out["rungs"] if r["id"] == "sonnet-class")
    assert sonnet["models"] == ["anthropic/claude-sonnet-4-6", "openai/gpt-4o"]
...
def test_migrate_user_tier_prefs():
    ...
    # tier0 (the retired judge slot) has no role mapping — the raw value
    # passes through and the router treats an unknown role as "auto".
    assert out["users"]["ext:tg:42"]["defaultRole"] == "tier0"
...
def _live_pod_buggy_models() -> dict:
    ...
    return {
        ...
        "rungs": [
            ...
            # a never-edited buggy seed the collapse DELETES (nothing curated).
...
def test_default_canonicalization_renames_backfills_rederives_preserves_embedding():
    ...
    # Canonical ids, cost-ordered; the never-edited buggy judge seed is
    # DELETED by the collapse (nothing curated to preserve), so 4 rungs.
    ...
    # Roles point at canonical ids, uniformly slugs; no judge role.
    assert out["roles"]["fast"] == "haiku-class"
...
# ── judge-role collapse (design-judge-role-collapse-2026-08-21 §6) ────────────
# Every config shape that can still carry judge artifacts must (a) collapse to
# a judge-free catalog with nothing operator-curated discarded, and (b) be
# IDEMPOTENT — a second run over the output changes nothing.


def _assert_idempotent(models: dict) -> dict:
    once, c1 = mmr.migrate_models_block(models)
    assert c1 is True
    twice, c2 = mmr.migrate_models_block(once)
    assert c2 is False, twice
    assert twice == once
    return once


def test_collapse_canonical_ids_operator_curated_judge_merges_into_standard():
    """Canonical-id shape, operator-curated judge-class contents: the curated
    models become fallback entries on the standard chain (dedup, order kept),
    the judge rung and role are gone. Idempotent.

    The judge rung deliberately OVERLAPS the standard chain
    (anthropic/claude-sonnet-4-6 appears in both) so the append-dedup in
    ``_collapse_judge_role`` is actually pinned: without the ``m not in``
    guard the merged chain would carry the model twice (mutation-verified)."""
    models = {
        "rungs": [
            {"id": "sonnet-class", "models": ["anthropic/claude-sonnet-4-6"], "costClass": "medium"},
            {"id": "judge-class",
             "models": ["xai/grok-4", "anthropic/claude-sonnet-4-6", "openai/gpt-4.1"],
             "costClass": "medium"},
        ],
        "roles": {
    ...
    out = _assert_idempotent(models)
    assert "judge" not in out["roles"]
    assert all(r["id"] != "judge-class" for r in out["rungs"])
    sonnet = next(r for r in out["rungs"] if r["id"] == "sonnet-class")
    # Curated picks appended after the existing chain, curated order kept;
    # the overlapping model is NOT duplicated.
    assert sonnet["models"] == [
        "anthropic/claude-sonnet-4-6", "xai/grok-4", "openai/gpt-4.1",
    ]


def test_collapse_default_ids_operator_curated_judge_merges_into_standard():
    """*-default-id shape with operator-edited judge contents: the collapse
    runs BEFORE canonicalization, so the curated models survive onto the
    standard chain instead of being dropped by the rename. Idempotent."""
    models = {
        "rungs": [
            {"id": "standard-default", "models": ["anthropic/claude-sonnet-4-6"]},
            {"id": "judge-default", "models": ["xai/grok-4"]},
        ],
        "roles": {
            "standard": "standard-default",
            "judge": {"rung": "judge-default", "provider": "not-standard"},
    ...
    assert "judge" not in out["roles"]
    ids = [r["id"] for r in out["rungs"]]
    assert ids == ["sonnet-class"], ids
    sonnet = out["rungs"][0]
    assert sonnet["models"] == ["anthropic/claude-sonnet-4-6", "xai/grok-4"]
    assert sonnet["costClass"] == "medium"


def test_collapse_bare_slug_judge_role_pointing_at_standards_rung():
    """The pre-decouple shape (roles.judge → standard's own rung, or a bare
    slug): only the role is dropped — the shared rung is untouched. Idempotent."""
    models = {
        "rungs": [
            {"id": "sonnet-class", "models": ["anthropic/claude-sonnet-4-6"], "costClass": "medium"},
        ],
        "roles": {"standard": "sonnet-class", "judge": "sonnet-class"},
    }
    out = _assert_idempotent(models)
    assert "judge" not in out["roles"]
    assert [r["id"] for r in out["rungs"]] == ["sonnet-class"]
    assert out["rungs"][0]["models"] == ["anthropic/claude-sonnet-4-6"]


def test_collapse_orphan_judge_rung_without_standard_rung_is_kept():
    """A file whose only rung is the judge rung (no standard rung of its own):
    the role is dropped, the rung stays as a parseable orphan — minting a
    standard chain from judge picks would silently change the workhorse.
    Idempotent (the surviving orphan never re-triggers a change)."""
    models = {
        "rungs": [
            {"id": "judge-class", "models": ["openai/gpt-4.1"], "costClass": "medium"},
        ],
        "roles": {"judge": {"rung": "judge-class", "provider": "not-standard"}},
    }
    out = _assert_idempotent(models)
    assert out["roles"] == {}
    assert [r["id"] for r in out["rungs"]] == ["judge-class"]
    assert out["rungs"][0]["models"] == ["openai/gpt-4.1"]


def test_collapse_legacy_tier0_merges_into_standard_chain():
    """Legacy tiers shape with tier0: its models land as fallback entries on
    the standard chain; no judge role or rung is minted. Idempotent."""
    models = {"tiers": {
        "tier2": {"models": ["anthropic/claude-sonnet-4-6"]},
        "tier0": {"models": ["openai/gpt-4.1", "google/gemini-2.5-pro"]},
    }}
    out = _assert_idempotent(models)
    assert "judge" not in out["roles"]
    assert [r["id"] for r in out["rungs"]] == ["sonnet-class"]
    assert out["rungs"][0]["models"] == [
        "anthropic/claude-sonnet-4-6", "openai/gpt-4.1", "google/gemini-2.5-pro",
    ]


def test_collapse_buggy_seed_judge_rung_is_deleted_not_merged():
    """A judge rung still carrying the never-edited buggy default seed is
    deleted outright — merging it would pollute the corrected standard chain
    with ghost model ids. Idempotent."""
    models = {
        "rungs": [
            {"id": "sonnet-class", "models": ["anthropic/claude-sonnet-4-6"], "costClass": "medium"},
            {"id": "judge-class",
             "models": sorted(mmr._OLD_BUGGY_DEFAULT_CLUSTERS["judge-class"]),
             "costClass": "medium"},
        ],
        "roles": {
            "standard": "sonnet-class",
    ...
    sonnet = next(r for r in out["rungs"] if r["id"] == "sonnet-class")
    assert sonnet["models"] == ["anthropic/claude-sonnet-4-6"]
    assert "gpt-5.5" not in " ".join(sonnet["models"])
