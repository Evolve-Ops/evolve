...
internal/design-phase4.3c-S0-plist-consolidation.md.
"""
...
_DEPLOY_CASES = [
    ("deploy_interval",
     ("ai.evolve.evolve.heal", "evolve",
      ...
      {"interval": 900}),
     {}),
    # These two carry the bot id as the label's trailing segment. The captured
    # goldens say `evolve-scan-status.log` / `evolve-audit.log` — the intended
    # name. Pre-2026-09-14 the emitter reached it by stripping the substring
    # `.<user>`, which only worked because these fixtures' account name happens
    # to equal the bot id; it mangled `doctor-pass.evolve` on account `evo`.
    # The `bot_id` kwarg is how the emitter reaches the SAME golden for every
    # account name — see test_daemon_log_path_naming.py.
...
def test_soft_file_limit_is_a_real_field():
    """soft_file_limit renders as SoftResourceLimits.NumberOfFiles — the
    2026-07-28 incident was an EMFILE storm under launchd's 256 soft
    default; the admin-ui jobspec now pins 4096 (see the regenerated
    deploy_admin_ui / service_admin_ui goldens)."""
    xml = render_launchd_plist(JobSpec(
        label="t", program_args=["/bin/true"], soft_file_limit=4096,
    ))
    assert plistlib.loads(xml.encode())["SoftResourceLimits"] == {
        "NumberOfFiles": 4096,
    }
    # unset → key omitted entirely (launchd default applies)
    xml = render_launchd_plist(JobSpec(label="t", program_args=["/bin/true"]))
    assert "SoftResourceLimits" not in plistlib.loads(xml.encode())


def test_admin_ui_jobspec_raises_nofile_limit():
    """Both admin-ui plist emitters carry the 4096 NOFILE cap — losing it
    in a refactor would silently reintroduce the 2026-07-28 EMFILE storm
    precondition on the next deploy."""
    from evolve_admin import deploy, service
    assert deploy._admin_ui_jobspec("ai.evolve.evolve.admin-ui").soft_file_limit == 4096
    xml = service.generate_plist(host="127.0.0.1", port=5050)
    assert plistlib.loads(xml.encode())["SoftResourceLimits"] == {
        "NumberOfFiles": 4096,
    }
