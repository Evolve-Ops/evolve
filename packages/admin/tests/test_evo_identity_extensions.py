...
def test_revoke_admin_removes_channel_external_id():
    ...
    # Emptying a channel DROPS the key rather than leaving `[]` (M1-B2):
    # `normalize` discards channels whose ids all wash out, so an empty
    # list was never observable through the reader anyway.
    assert ext_ids.ids_for(net["pod"]["admins"], "telegram") == []
    assert "telegram" not in net["pod"]["admins"]["external_ids"]
    # Slack entry untouched
    assert ext_ids.has_id(net["pod"]["admins"], "slack", "U123")
...
def test_evo_claim_accepts_per_bot_passphrase_override(tmp_path):
    ...
    assert ext_ids.ids_for(
        net["bots"]["admin_bot"]["primary_user"], "telegram"
    ) == ["789"]
...
def test_pod_default_still_works_on_bots_without_override(tmp_path):
    ...
    assert ext_ids.ids_for(
        net["bots"]["team_bot_a"]["primary_user"], "telegram"
    ) == ["111"]
