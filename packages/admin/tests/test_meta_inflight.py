...
# ── --repairs: a repair brief is never gated by the PR it repairs ──────────────
# 2026-09-25 23:01 PDT, the dispatch tick: `gated hold-fix-4440-…: actionable_count 1
# (top: The review tier is derived from the paths a PR touches, and the merge verb refu…)`
# — the single hit was #4440 itself, the PR that brief exists to repair. Two days of
# ticks, three free slots, nothing launched. A repair overlaps its PR by definition.


def _repair_candidates():
    return [
        {"kind": "pr", "aspect": "substrate", "scope": [], "note": "",
         "title": "dispatch: the review tier and merge gate are code (D-MT3, D-MT4)",
         "pr": 4440, "branch": "claude/meta-review-tier-and-merge-gate-are-code",
         "task_id": None, "id": None},
        {"kind": "chip", "aspect": "substrate", "scope": [], "note": "",
         "title": "[META:substrate] the review tier and merge gate are code",
         "pr": 4440, "branch": "claude/meta-review-tier-and-merge-gate-are-code",
         "task_id": "task_0c80", "id": "review-tier-and-merge-gate-are-code"},
        {"kind": "pr", "aspect": "substrate", "scope": [], "note": "",
         "title": "dispatch: a merge gate for the other lane", "pr": 4999,
         "branch": "claude/other-merge-gate", "task_id": None, "id": None},
    ]


def test_repair_tokens_name_the_pr_its_chip_row_and_its_branch():
    toks = mi.repair_tokens(["#4440"], _repair_candidates())
    assert toks == {"4440", "review-tier-and-merge-gate-are-code",
                    "claude/meta-review-tier-and-merge-gate-are-code"}
    assert mi.repair_tokens(["not-a-number"], _repair_candidates()) == set()
    assert mi.repair_tokens(None) == set()


def test_the_repaired_pr_is_excluded_and_an_unrelated_pr_on_the_same_words_is_not():
    cands = _repair_candidates()
    query = mi.make_query(aspect="substrate", keywords="merge,gate,repair,branch")
    # Without --repairs: the repaired PR (and its chip row) gate the brief — the defect.
    assert {o.get("pr") for o in mi.find_overlaps(cands, query)} >= {4440, 4999}
    dropped = []
    overlaps = mi.find_overlaps(cands, query, dropped_self=dropped,
                                self_handles=mi.repair_tokens([4440], cands))
    assert [o["pr"] for o in overlaps] == [4999]         # the unrelated PR still gates
    assert {d["pr"] for d in dropped} == {4440}


def test_cli_repairs_flag_folds_into_self_excluded(tmp_path, capsys):
    d = _write_fixture(tmp_path)
    cdir = _self_claims(tmp_path)
    rc = mi.main(["--aspect", "alpha", "--keywords", "dispatch,collision",
                  "--dir", str(d), "--no-prs", "--claims-dir", str(cdir),
    ...
    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    # 4242 matches nothing in this fixture, so nothing is dropped and nothing breaks:
    # the flag is inert when the PR is absent, never a hard stop.
    assert payload["scanned"]["self_excluded"] == 0


# ── --candidate: a brief must never be gated by its own prior chip row ─────────
# 2026-09-10: the dispatch tick reached the queue head, ran step 3's duplicate check, and
# got `actionable_count: 1` — the single hit was that brief's OWN prior chip row
# (bucket `returned_to_queue`, `pr: null`, no branch). Since the row never left the
# ledger, the lane skipped the queue head on every subsequent tick, forever. Two
# independent narrowings fix this: (1) a row that IS the candidate never gates it
# (`--candidate` / `is_own_prior_row`), and (2) a `returned_to_queue` row with no `pr`
# and no `branch` is not work in flight at all (`is_artifactless_returned_row`), even
# when it belongs to a genuinely different brief.

_CANDIDATE_ID = "board-tool-chat-parity-and-enrichment"


def _selfgate_own_row_only_ledger(tmp_path):
    """The exact 2026-09-10 shape, isolated: the ONLY hit is the candidate's own prior
    row (`returned_to_queue`, no pr, no branch)."""
    d = tmp_path / "meta-state"
    d.mkdir()
    (d / "board.json").write_text(json.dumps({
        "aspect": "board",
        "chips": [
            {"id": _CANDIDATE_ID, "bucket": "returned_to_queue", "pr": None,
             "title": "[META:board] board tool chat parity and enrichment"},
        ],
    }))
    return d


def test_own_prior_row_is_the_only_hit_and_is_excluded_not_actionable(tmp_path):
    d = _selfgate_own_row_only_ledger(tmp_path)
    query = mi.make_query(aspect="board", keywords="board,chat,parity")
    r = mi.run(d, query, "x/y", None, scan_prs=False, pr_limit=10,
              candidate_id=_CANDIDATE_ID)
    assert r["actionable_count"] == 0
    assert r["count"] == 0                       # the row vanishes from overlaps entirely
    assert r["self_rows"] == [_CANDIDATE_ID]
...
LEDGER_SELFGATE = {
    ...
    "updated": "2026-09-10",
    "chips": [
        # The candidate's own prior attempt — must never gate itself.
        {"id": _CANDIDATE_ID, "bucket": "returned_to_queue", "pr": None,
         "title": "[META:board] board tool chat parity and enrichment"},
        # A DIFFERENT brief, also returned_to_queue, no pr and no branch — the
        # artifact-less shape is not actionable regardless of whose brief it is.
        {"id": "other-chat-brief", "bucket": "returned_to_queue", "pr": None,
         "title": "[META:board] chat parity follow-up for a different brief"},
        # A DIFFERENT brief, returned_to_queue but carrying a PR — the narrowing must
        # NOT become a blanket exemption for the whole bucket.
        {"id": "other-chat-brief-with-pr", "bucket": "returned_to_queue", "pr": 4321,
         "title": "[META:board] chat parity enrichment picked back up with a PR"},
        # A genuine in-flight duplicate from a DIFFERENT brief — the guard must not
        # widen to swallow real collisions.
        {"id": "real-duplicate", "bucket": "dispatched", "pr": None,
         "title": "[META:board] board chat parity enrichment, a live duplicate"},
        # Bucket casing/underscore drift on the SAME artifact-less shape.
        {"id": "drifted-case-brief", "bucket": "Returned_To_Queue", "pr": None,
         "title": "[META:board] chat parity enrichment under a drifted bucket spelling"},
    ],
}


def _selfgate_ledgers(tmp_path):
    d = tmp_path / "meta-state"
    d.mkdir()
    (d / "board.json").write_text(json.dumps(LEDGER_SELFGATE))
    return d


def _run_selfgate(tmp_path, candidate_id=None):
    d = _selfgate_ledgers(tmp_path)
    query = mi.make_query(aspect="board", keywords="board,chat,parity")
    return mi.run(d, query, "x/y", None, scan_prs=False, pr_limit=10,
...
def _by_id(result):
    return {o["id"]: o for o in result["overlaps"]}


def test_returned_to_queue_with_no_pr_and_no_branch_is_not_actionable_for_another_brief(tmp_path):
    r = _run_selfgate(tmp_path, candidate_id=_CANDIDATE_ID)
    assert _by_id(r)["other-chat-brief"]["actionable"] is False


def test_returned_to_queue_with_a_pr_is_still_actionable(tmp_path):
    r = _run_selfgate(tmp_path, candidate_id=_CANDIDATE_ID)
    assert _by_id(r)["other-chat-brief-with-pr"]["actionable"] is True


def test_genuine_inflight_duplicate_from_another_brief_still_gates(tmp_path):
    r = _run_selfgate(tmp_path, candidate_id=_CANDIDATE_ID)
    assert _by_id(r)["real-duplicate"]["actionable"] is True
    assert r["actionable_count"] == 2              # other-chat-brief-with-pr + real-duplicate
    assert _CANDIDATE_ID not in _by_id(r)           # the candidate's own row vanished
    assert r["self_rows"] == [_CANDIDATE_ID]


def test_returned_to_queue_bucket_casing_and_underscore_drift_classify_the_same(tmp_path):
    r = _run_selfgate(tmp_path, candidate_id=_CANDIDATE_ID)
    assert _by_id(r)["drifted-case-brief"]["actionable"] is False
    assert mi.is_returned_to_queue_bucket("Returned_To_Queue")
    assert mi.is_returned_to_queue_bucket("returned-to-queue")
    assert mi.is_returned_to_queue_bucket("RETURNED_TO_QUEUE")
    assert not mi.is_returned_to_queue_bucket("dispatched")
    assert not mi.is_returned_to_queue_bucket(None)


def test_own_prior_row_excluded_even_without_returned_to_queue_bucket():
    """`is_own_prior_row` is unconditional on bucket — a candidate's own row must never
    gate it whatever state that row is in, distinct from the artifact-less narrowing."""
    cand = {"kind": "chip", "id": _CANDIDATE_ID, "bucket": "dispatched", "pr": None}
    assert mi.is_own_prior_row(cand, _CANDIDATE_ID.lower())
    assert not mi.is_own_prior_row(cand, "some-other-brief")
    assert not mi.is_own_prior_row(cand, None)
    # a PR/session/claim candidate's `id` is a different kind of handle (a session id, a
    # claim file stem) and must never be compared against a brief id.
    non_chip = {"kind": "pr", "id": None, "pr": 4242}
    assert mi.candidate_subject_id(non_chip) is None


def test_cli_candidate_flag_excludes_own_row_and_reports_self_rows(tmp_path, capsys):
    d = _selfgate_ledgers(tmp_path)
    rc = mi.main(["--aspect", "board", "--keywords", "board,chat,parity",
                  "--dir", str(d), "--no-prs", "--candidate", _CANDIDATE_ID, "--json"])
    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["self_rows"] == [_CANDIDATE_ID]
    assert payload["actionable_count"] == 2
    assert _CANDIDATE_ID not in {o["id"] for o in payload["overlaps"]}
