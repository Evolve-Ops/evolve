...
#: Per-test tmp shared dir, set by the autouse fixture below. Module-level
#: because ``_network()`` is a plain helper called from ~60 call sites with no
#: ``tmp_path`` in scope, and threading the fixture through all of them buys
#: nothing over one autouse hook.
_SHARED_DIR: Path | None = None


@pytest.fixture(autouse=True)
def _shared_dir_in_tmp(tmp_path):
    """Keep every ledger/state write these tools do inside the test's tree.

    ``google_service._send_ledger_path`` falls back to
    ``config.DEFAULT_SHARED_DIR`` (/Users/Shared/evolve) when the network dict
    has no ``sharedDir``, so the gmail_send tests below were appending rows to
    the DEVELOPER'S real pod ledger. The send guards are file-backed by design
    (a process counter would reset on daemon restart mid-storm), so those rows
    persisted and refused the next run inside SEND_DUPLICATE_WINDOW_SECONDS
    (15 min) as a duplicate — tests that passed once, then failed for a quarter
    of an hour. Same fix the sibling gmail modules already carry explicitly.
    """
    global _SHARED_DIR
    _SHARED_DIR = tmp_path / "shared"
    yield
    _SHARED_DIR = None


def _network(bot_id: str = "lex", with_persona: bool = True) -> dict:
    assert _SHARED_DIR is not None, "_network() called outside _shared_dir_in_tmp"
    net = {
        "sharedDir": str(_SHARED_DIR),
        "bots": {
            bot_id: {
                "role": "member",
