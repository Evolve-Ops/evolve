...
2. **raw()-debt census, shrink-only.** ``LaunchdScheduler.raw()`` and
   ...
   port's W3A migration worklist (internal/design-linux-port-2026-06-10.md
...
Scan scope is ``packages/**/*.py`` minus test code, matching the Phase-B
convention (the ``oc_cli`` import gate) — git-tracked, plus untracked-and-
not-ignored when ``EVOLVE_SCAN_UNTRACKED=1`` (see ``tools/scan_scope.py``).
...
def _load_scan_scope():
    """Load tools/scan_scope.py — the shared "what does a repo-wide gate
    scan?" definition. Same by-path load the other tools/-backed tests use."""
    path = _REPO_ROOT / "tools" / "scan_scope.py"
    spec = importlib.util.spec_from_file_location("scan_scope", path)
...
#
# RATCHET (2026-08-14): 23 → 21. The gateway restart/stop PAIR behind the admin
# UI dropped its two get_launchd_scheduler() calls. Both were launchd-only on
# a surface with no other implementation, so on a Linux pod the accessor's
# fail-fast raise made them structurally impossible to satisfy: /restart fell
# through to an openclaw-CLI fallback that then died on a hardcoded
# /Users/<bot> cwd, and /stop was a hard 500. Both now take get_scheduler() +
# an isinstance guard — Protocol restart()/disable() on a non-launchd adapter,
# the raw() bootout/kickstart domain loops only under launchd — the same shape
# routes_trust's defer-health probe uses. Those raw() sites are genuinely
# launchd-only and still count above (routes_admin's stop loop MOVED into
# oc_cli.oc_gateway_stop, so the raw census is unchanged, not reduced).
_RAW_BASELINE = 47
_GET_LAUNCHD_BASELINE = 21
...
def _scanned_non_test_python() -> list[str]:
    """Non-test ``packages/**/*.py`` this gate scans.

    Tracked at HEAD, PLUS untracked-and-not-ignored under
    ``EVOLVE_SCAN_UNTRACKED=1`` (``tools/preflight`` sets it). A brand-new
    module is exactly how a direct ``launchctl`` argv re-enters, and until it
    is ``git add``ed ``git ls-files`` cannot see it — so the local run of this
    gate used to be blind to the case it exists for. Scope and trade-off:
    ``tools/scan_scope.py``.
    """
    out: list[str] = []
    for rel in _scan_scope.scan_files(_REPO_ROOT, "packages/**/*.py"):
        parts = Path(rel).parts
        name = Path(rel).name
        if "tests" in parts or name.startswith("test_") or name == "conftest.py":
            continue
        if not (_REPO_ROOT / rel).is_file():
            continue  # tracked but deleted in a dirty working tree
        out.append(rel)
    assert len(out) > 100, "scan scope collapsed — the file scan returned too little"
    return out


def test_no_quoted_launchctl_argv_outside_the_seam():
    offenders: list[str] = []
    for rel in _scanned_non_test_python():
        if rel == _SEAM:
            continue
