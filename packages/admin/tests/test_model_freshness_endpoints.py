...
def test_check_freshness_has_no_judge_rows_or_advisory_sidecar(app):
    """The judge role — and its advisory_tiers side-car — died with the
    judge-role collapse (design-judge-role-collapse-2026-08-21 §5.4). The
    freshness summary must carry neither a judge severity row nor the
    advisory side-car; a single-provider pod is simply not flagged."""
    with app["app"].test_client() as c:
        body = c.post("/api/models/check-freshness", json={}).get_json()
        hb = body.get("hard_break_tiers") or []

        assert "advisory_tiers" not in body, "the advisory side-car is retired"
        assert [t for t in hb if t.get("role") == "judge"] == [], (
            "no judge severity rows may survive the collapse"
        )
...
# ── Behavior pins: sticky rollback enforcement (2026-08-21 recurrence) ────────
# `models rollback` pins the rejected (bot, tier, model); the two tier-write
# endpoints must refuse to reintroduce a pinned pair without an explicit
# override — the recurrence was a routine "Apply All" silently re-swapping the
# model a rollback had just backed out.


def _pin_tier2(app, model=None):
    import model_swap_ledger
    from model_registry import RECOMMENDED

    model = model or RECOMMENDED["anthropic"]["tier2"]["model"]
    assert model_swap_ledger.record_pin_event(
        "team_bot_a", "tier2", model, action="pin",
        reason="behavior-rejected by models rollback",
        source="cli_rollback", shared_dir=app["shared"],
    )
    return model


def test_update_tier_refuses_a_pinned_model(app):
    rec = _pin_tier2(app)
    before = list(app["bot_state"]["team_bot_a"]["tiers"]["tier2"]["models"])
    with app["app"].test_client() as c:
        resp = c.post("/api/models/update-tier", json={
            "bot_id": "team_bot_a", "tier": "tier2",
            "provider": "anthropic", "model": rec,
    ...
    assert "rolled back for behavior" in body["error"]
    assert app["bot_state"]["team_bot_a"]["tiers"]["tier2"]["models"] == before, \
        "a refused apply must not touch the config"


def test_update_tier_pin_is_rung_scoped(app):
    """A pin on tier2 must not block the same bot's tier3 apply."""
    from model_registry import RECOMMENDED
    _pin_tier2(app)
    rec3 = RECOMMENDED["anthropic"]["tier3"]["model"]
    with app["app"].test_client() as c:
        resp = c.post("/api/models/update-tier", json={
            "bot_id": "team_bot_a", "tier": "tier3",
            "provider": "anthropic", "model": rec3,
        })
    assert resp.status_code == 200, resp.get_json()


def test_update_tier_override_applies_and_unpins(app):
    """An explicit override is the operator revoking the rejection — the pin
    must lift so the NEXT routine apply isn't refused for a model the operator
    just chose."""
    import model_swap_ledger
    ...
    with app["app"].test_client() as c:
        resp = c.post("/api/models/update-tier", json={
            "bot_id": "team_bot_a", "tier": "tier2",
            "provider": "anthropic", "model": rec, "override_pin": True,
        })
    assert resp.status_code == 200, resp.get_json()
    assert app["bot_state"]["team_bot_a"]["tiers"]["tier2"]["models"][0] == rec
    assert model_swap_ledger.find_active_pin(
        "team_bot_a", "tier2", rec, app["shared"]) is None
    # History keeps both events — the pin was lifted, not erased.
    assert [e["action"] for e in model_swap_ledger.read_pin_events(app["shared"])] \
        == ["pin", "unpin"]


def test_update_tier_bulk_pinned_row_fails_without_blocking_the_rest(app):
    from model_registry import RECOMMENDED
    rec = _pin_tier2(app)
    rec3 = RECOMMENDED["anthropic"]["tier3"]["model"]
    with app["app"].test_client() as c:
    ...
    by_tier = {r["tier"]: r for r in body["results"]}
    assert by_tier["tier2"]["success"] is False
    assert by_tier["tier2"]["pinned"] is True
    assert "rolled back for behavior" in by_tier["tier2"]["error"]
    assert by_tier["tier3"]["success"] is True
    assert rec not in app["bot_state"]["team_bot_a"]["tiers"]["tier2"]["models"]


def test_update_tier_bulk_override_applies_and_unpins(app):
    import model_swap_ledger

    rec = _pin_tier2(app)
    with app["app"].test_client() as c:
        resp = c.post("/api/models/update-tier-bulk", json={
            "updates": [{"bot_id": "team_bot_a", "tier": "tier2",
                         "provider": "anthropic", "model": rec}],
            "override_pin": True,
        })
    assert resp.status_code == 200
...
def test_unreadable_pin_ledger_refuses_rather_than_failing_open(app):
    """Pin state unknown must not silently read as unpinned — that is the
    exact non-sticky-rollback failure the gate closes. The override escape
    hatch still works, so a corrupt ledger never bricks the surface."""
    import model_swap_ledger
    from model_registry import RECOMMENDED

    model_swap_ledger.pin_ledger_path(app["shared"]).write_text("garbage\n")
    rec = RECOMMENDED["anthropic"]["tier2"]["model"]
    with app["app"].test_client() as c:
        resp = c.post("/api/models/update-tier", json={
            "bot_id": "team_bot_a", "tier": "tier2",
        ...
        assert resp.status_code == 409
        assert "pin state is unknown" in resp.get_json()["error"]
        resp2 = c.post("/api/models/update-tier", json={
            "bot_id": "team_bot_a", "tier": "tier2",
            "provider": "anthropic", "model": rec, "override_pin": True,
        })
...
def test_check_freshness_tags_pinned_advisories(app):
    """The Model Freshness card must know a row is pinned BEFORE the operator
    clicks Apply — the UI routes it through the override confirm and keeps it
    out of Apply All."""
    rec = _pin_tier2(app)
    with app["app"].test_client() as c:
        resp = c.post("/api/models/check-freshness", json={})
        assert resp.status_code == 200
        advisories = resp.get_json()["advisories"]
        tier2 = [a for a in advisories
                 if a["bot_id"] == "team_bot_a" and a["tier"] == "tier2"
                 and a["provider"] == "anthropic"]
        assert len(tier2) == 1
        assert tier2[0]["behavior_pin"]["model"] == rec
        others = [a for a in advisories if a is not tier2[0]]
        assert all("behavior_pin" not in a for a in others)

        # The persisted-status read re-derives the tag (pins can change
        # between a check and a later page load).
        status = c.get("/api/models/freshness-status").get_json()
        tier2_status = [a for a in status["advisories"]
                        if a["bot_id"] == "team_bot_a" and a["tier"] == "tier2"
                        and a["provider"] == "anthropic"]
        assert tier2_status and tier2_status[0]["behavior_pin"]["model"] == rec
