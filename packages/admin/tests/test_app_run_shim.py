...
def test_unwritable_claim_dir_still_runs_the_app(rig, tmp_path: Path) -> None:
    ...
    # Fail-open, not fail-silent: the run survives AND says why.
    assert "not creatable" in proc.stderr
    assert str(blocked / "app-runs") in proc.stderr
    assert rig.claims() == []


@pytest.mark.skipif(os.geteuid() == 0, reason="root writes through any mode")
def test_existing_but_unwritable_claim_dir_leaves_a_breadcrumb(rig, tmp_path: Path) -> None:
    """The exact shape the 2026-09-03 drift produced, and the reason this
    breadcrumb exists.

    The claim dir EXISTS (so ``mkdir -p`` passes and the old code's only guard
    was satisfied) but the bot cannot write in it — on the reference pod
    because a missing sudoers grant left it ``evolve``-owned ``0755``. The
    claim WRITE is what fails, under ``2>/dev/null``. Before the breadcrumb an
    operator saw an empty err log here, which is byte-for-byte what a
    legitimately claim-free app produces.
    """
    readonly = tmp_path / "readonly-claims"
    readonly.mkdir()
    readonly.chmod(0o555)
    try:
        proc, argv = rig.run("agent", "-m", "ping",
                             env_overrides={ENV_CLAIM_DIR: str(readonly)})
    finally:
        readonly.chmod(0o755)

    assert proc.returncode == 0, "fail-open still holds — the app must run"
    assert argv == ["agent", "-m", "ping"], "no --session-id on an unclaimed run"
    assert "claim write failed" in proc.stderr, (
        f"the write-EACCES branch printed nothing — that silence IS the bug: "
        f"{proc.stderr!r}"
    )
    assert str(readonly) in proc.stderr, "the breadcrumb must name the dir to inspect"
    assert list(readonly.iterdir()) == [], "no temp file left behind"


def test_every_claim_failure_branch_says_unattributed(rig, tmp_path: Path) -> None:
    """One vocabulary across the branches, so an operator can grep one word.

    A branch that fails without this phrase is a branch that reintroduces the
    silence, so the assertion is on the phrase rather than on each message.
    """
    blocked = tmp_path / "nope"
    blocked.write_text("a file, not a dir")
    proc, _ = rig.run("agent", "-m", "ping",
                      env_overrides={ENV_CLAIM_DIR: str(blocked / "app-runs")})
    assert "turn runs unattributed" in proc.stderr


def test_breadcrumbs_stay_out_of_the_non_claimable_paths(rig) -> None:
    """An app whose cron never runs ``openclaw agent`` is NORMAL — the shim
    must not narrate it. This is the case that keeps the breadcrumb from
    becoming noise an operator learns to ignore: on the reference pod the only
    app with claim wiring is exactly this shape."""
    proc, argv = rig.run("--version")
    assert proc.returncode == 0
    assert argv == ["--version"]
