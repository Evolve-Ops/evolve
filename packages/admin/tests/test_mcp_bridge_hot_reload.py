...
def _write_net(path: Path, bots: dict, *, bump: bool = True) -> None:
    ...
    ``sharedDir`` is always written, pointing beside the temp network.json.
    Omitting it is not neutral: every ``{shared_dir}``-rooted path resolved
    from this config falls back to ``config.DEFAULT_SHARED_DIR`` — the REAL
    ``/Users/Shared/evolve`` on a maintainer's Mac — so the gmail_send
    duplicate-guard ledger below would be read from, and appended to, that
    machine's own dev-pod state. That is invisible on the Linux CI runner
    (no such path) and
    self-poisoning locally: the appended row makes the next run inside
    ``SEND_DUPLICATE_WINDOW_SECONDS`` fail with "duplicate send refused".
    """
    prev = path.stat().st_mtime if path.exists() else 0.0
    path.write_text(json.dumps(
        {"sharedDir": str(path.parent / "shared"), "bots": bots}, indent=2))
    if bump:
...
def _send_via_bridge(reg, net_path: Path) -> dict:
    """Drive ``tool_gmail_send`` through the bridge with the network + Google
    surface stubbed, and the send-guard clock PINNED.

    ``gmail_send`` stamps its duplicate/rate ledger with ``time.time()``; on
    the wall clock a test that exercises a real 15-minute window is red for
    15 minutes after any run that wrote a row. A fixed ``now`` makes these
    assertions depend on the code rather than on when the suite last ran.
    (Mirrors ``test_gmail_send_guards.py``.)
    """
    ...
    with patch("evolve_admin.mcp_bridge.google_tools.config.load_network",
         ...
         patch("evolve_admin.google_service.time.time", return_value=1000.0), \
        ...
        return asyncio.new_event_loop().run_until_complete(
            google_tools.tool_gmail_send(
                {"bot": "team_bot_a", "to": "x@y.com", "subject": "Hi",
                 "body": "Hello"},
                reg,
            )
        )
...
def test_gmail_tool_works_for_bot_configured_after_startup(tmp_path, registry_factory):
    ...
    result = _send_via_bridge(reg, net_path)
...
def test_the_send_ledger_stays_under_tmp_path(tmp_path, registry_factory):
    """Hermeticity, pinned so it cannot silently regress.

    ``gmail_send``'s duplicate/rate ledger is rooted at the network dict's
    ``sharedDir``; when that key is absent the resolver falls back to
    ``config.DEFAULT_SHARED_DIR`` — the real ``/Users/Shared/evolve``. This
    file used to omit it, so the end-to-end send above appended a row to the
    dev box's own ledger and read it back on the next run, where the
    15-minute duplicate guard refused the send. (A test-isolation defect on
    the maintainer's machine — it never reached a deployed pod.)

    That bug is invisible in two directions at once: green on Linux CI (no
    ``/Users/Shared``, so the read raises, ``except OSError`` returns ``[]``,
    and the guard sees an empty ledger), and on macOS a *delayed* failure —
    the polluting run passes and the NEXT run inside the window fails with an
    error about duplicate sends that says nothing about isolation.

    Same defect and same fix as ``test_gmail_send_attachments.py`` (#3984);
    it recurred here because that PR fixed one file rather than the class.

    The path is asserted as a LITERAL rather than by calling
    ``google_service._send_ledger_path``: a resolver that ignored
    ``sharedDir`` again would move both sides of the assertion and pass
    vacuously.
    """
    net_path = tmp_path / "network.json"
    _write_net(net_path, {
    ...
    assert _send_via_bridge(reg, net_path)["ok"] is True
    assert (tmp_path / "shared" / "google_send_ledger"
            / "team_bot_a.jsonl").is_file()
