...
# -- AL-1.4b: the extractor app identifier is the TEMPLATE-MATCHING key ------


def test_installed_apps_is_the_template_matching_key_not_the_canonical_app_id() -> None:
    """AL-1.4b left ``bot_evidence_from_files`` on its legacy chain
    (``id`` then ``pkg_id`` then ``name``) deliberately, and this pins why.

    ``installed_apps`` is matched (lowercased) against a bot template's
    ``ApplicationSpec.app_id`` / ``.name``, and a template's ``app_id`` is a
    gallery slug (``"app_task_manager"``), never a forge ``p-`` pkg_id. The
    canonical resolution order puts ``pkg_id`` ahead of ``id``, so sweeping
    this site would emit the pkg_id for every stamped manifest on a live pod
    and every template would silently stop matching. If a future stage sweeps
    this call site, this test is the alarm.
    """
    from evolve_admin.applications.app_identity import resolve_app_id
    from evolve_admin.pod_templates.extractor import bot_evidence_from_files

    manifest = {"id": "task-manager", "pkg_id": "p-cccccccc", "name": "Task Manager"}
    ev = bot_evidence_from_files(bot_cfg={}, oc_config={}, manifests=[manifest])

    assert ev.installed_apps == ("task-manager",)
    # The canonical resolver returns something DIFFERENT here -- that is the
    # whole point of not sweeping this site.
    assert resolve_app_id(manifest) == "p-cccccccc"
    assert resolve_app_id(manifest) not in ev.installed_apps
