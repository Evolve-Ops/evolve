...
B6 (spec-evolve-overhead-budget §B6) split the template into the
OC-ingested core (AGENTS.md) plus the on-demand reference library
(``evolve_bot/reference/*.md``, seeded to ``workspace/evolve/reference/``
and pointed at from core). The taught corpus is now the UNION: rules whose
substance moved into a reference file are guarded against the corpus via
the ``agents_md`` fixture below, and per-turn-critical floors (the
shell-snippet kernel, pronoun resolution, report_text awareness) are
guarded against the CORE via ``agents_md_core`` — so a future edit cannot
quietly demote an always-in-force rule to read-on-demand.
"""
...
def agents_md_core() -> str:
    """The OC-ingested core only — what rides in every turn's context."""
    assert _AGENTS_MD.exists(), f"{_AGENTS_MD} not found"
    return _AGENTS_MD.read_text(encoding="utf-8")


@pytest.fixture(scope="module")
def agents_md(agents_md_core: str) -> str:
    """The full taught corpus: core + the seeded reference library."""
    parts = [agents_md_core]
    ref_files = sorted(_REFERENCE_DIR.glob("*.md"))
    assert ref_files, f"no reference templates under {_REFERENCE_DIR}"
    parts += [p.read_text(encoding="utf-8") for p in ref_files]
    return "\n\n".join(parts)
...
def test_reference_resolution_covers_plural_pronouns(agents_md_core: str):
    ...
    for phrase in ('"all four"', '"those"', '"both"'):
        assert phrase in agents_md_core, (
...
def test_audit_readability_worked_example_present(agents_md: str):
    ...
    assert 'bot_action(action="repair_acls"' in agents_md, (
        "AGENTS.md no longer references the repair_acls bot_action. "
...
def test_in_process_proposals_worked_example_present(agents_md: str):
    ...
    assert 'pod_state(query="proposals.in_process"' in agents_md, (
        "AGENTS.md no longer references the proposals.in_process query. "
        "Without this signpost the model falls back to proposals.pending "
        "and comes up empty when the operator's on the In Process tab."
    )
    ...
    assert "mark_complete" in agents_md, (
        "AGENTS.md no longer references proposal_action's mark_complete "
        "action. That's the closing-out tool for in-process proposals — "
        "without it evo can find them but doesn't know how to close them."
    )
...
def test_scan_status_worked_example_in_operations_table(agents_md: str):
    ...
    assert 'pod_state(query="app_scan_status"' in agents_md, (
        "AGENTS.md no longer references the app_scan_status query. "
        "The model has no signpost for the scan_needed diagnostic "
        "path and may fall back to shell."
    )
    assert 'bot_action(action="rescan_apps"' in agents_md, (
        "AGENTS.md no longer references the rescan_apps bot_action. "
...
def test_accept_drift_operations_row_present(agents_md: str):
    ...
    assert 'pod_state(query="config_drift"' in agents_md, (
        "AGENTS.md no longer references the config_drift query. "
...
def test_backup_operations_row_present(agents_md: str):
    ...
    assert 'bot_action(action="backup_workspace"' in agents_md, (
        "AGENTS.md no longer references the backup_workspace bot_action. "
    ...
    assert 'pod_state(query="backup_status"' in agents_md, (
        "AGENTS.md no longer references the backup_status query. "
...
def test_chip_teaching_rows_present(agents_md: str):
    ...
    # exact phrasing.
    tool_names = (
        'pod_state(query="bots"',
        'pod_state(query="usage"',
        'pod_state(query="audit"',
...
def test_high_correction_row_describes_signal_correctly(agents_md: str):
    ...
    # same row.
    if 'pod_state(query="audit"' in row:
        assert ('NOT ``pod_state(query="audit")``' in row
                or "not surface" in row.lower()
                or "does not" in row.lower()), (
            "the high_correction row mentions the audit query but not "
...
def test_oc_slash_command_rule_present(agents_md: str):
    ...
    assert "/approve" in agents_md, (
        ...
        "on to recognize the forbidden ask. See internal/diagnosis-evo-"
...
def test_help_style_decision_tree_present(agents_md: str):
    ...
    assert "Help style — the four-rung escalation" in agents_md, (
        ...
        "spec (internal/spec-surface-aware-help-style-2026-05-22.md §5a) "
...
def test_page_tool_map_covers_all_current_admin_pages(agents_md: str):
    ...
    rows = [
        ...
        ("Model Economics", 'pod_state(query="usage"'),
        ("Users", 'roster_action(action="set_role"'),
        ("Skills", 'plugin_action(action="enable"'),
        ("Credentials", 'keys_action(action="add"'),  # integrations-keys page
        ("Getting Started", "Quick Start"),
        ("Backup", 'bot_action(action="backup_workspace"'),
        ("AI Optimization", "action.models.check_freshness"),
        ("Settings", 'pod_state(query="config_network"'),
...
def test_shell_snippet_floor_kernel_stays_in_core(agents_md_core: str):
    """B6 core-pinning: the shell-snippet floor must ride the INGESTED core
    (the pointer-block kernel), not only the on-demand reference file —
    evo won't read PAGE_CONTEXT.md before casually emitting a shell command.
    The full surface-conditional rule with worked examples lives in
    evolve/reference/PAGE_CONTEXT.md; this pins the always-in-force kernel."""
    lower = agents_md_core.lower()
    assert "never emit a shell command" in lower
    assert "accuracy-verified" in lower
    # The two recurring fabrication modes the kernel must keep naming:
    assert "`sed`" in agents_md_core and "json" in lower
    assert "/usr/sbin/chown" in agents_md_core


def test_reference_library_pointer_stays_in_core(agents_md_core: str):
    """B6 core-pinning: the pointer block is what makes the reference library
    reachable — without it the moved sections are as dead as when they were
    silently truncated."""
    for ref in ("evolve/reference/PAGE_CONTEXT.md",
                "evolve/reference/PLAYBOOKS.md",
                "evolve/reference/COMMANDS.md",
                "evolve/reference/GLOSSARY.md"):
        assert ref in agents_md_core, f"core lost the pointer to {ref}"
