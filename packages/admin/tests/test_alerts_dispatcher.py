...
def test_every_dispatching_source_is_registered():
    ...
    _SIGNAL_ONLY_PRODUCERS = frozenset({
        ...
        # system.app_script_failure catalog event. Spec: internal/spec-agent-
...
def test_catalog_event_daily_digest_defers_to_queue(env, monkeypatch):
    ...
    # Audit row lands in the QUEUED lane, not the "actually sent" lane.
    queued_log = _read_log(env["shared"], "dispatcher-queued.jsonl")
    assert queued_log[-1]["result"] == "deferred"
    assert _read_log(env["shared"], "dispatcher.jsonl") == []
...
def test_catalog_event_recorded_in_audit_log(env):
    ...
    Checked on both lanes: ``security.audit_finding`` defaults to
    DAILY_DIGEST, so a bare send lands the audit row in the QUEUED lane;
    an operator IMMEDIATE override puts the next one in the sent lane.
    The field must survive either route."""
    d = env["dispatcher"]
    out = d.send(
        shared_dir=env["shared"], network=env["network"],
        source="audit", message="hello",
    ...
    assert out.result == d.DispatchResult.DEFERRED
    rec = _read_log(env["shared"], "dispatcher-queued.jsonl")[-1]
    assert rec["catalog_event"] == "security.audit_finding"

    _force_immediate(env["shared"], "security.audit_finding")
    out2 = d.send(
        shared_dir=env["shared"], network=env["network"],
        source="audit", message="hello again",
        catalog_event="security.audit_finding",
        dedup_key="second",
    )
    assert out2.result == d.DispatchResult.SENT
    sent_rec = _read_log(env["shared"], "dispatcher.jsonl")[-1]
    assert sent_rec["catalog_event"] == "security.audit_finding"
...
#
# DEFERRED means "enqueued to a future digest", which is not what the
# Messages panel's label promises ("what the dispatcher actually sent …
# same messages your phone receives"). On the live pod 2026-08-25, 287 of
# 292 rows in that feed were heal config-drift deferrals. These pin the
# split so a future refactor can't silently fold DEFERRED back in.


def test_deferred_routes_to_queued_lane_not_sent_lane(env):
    """_log_filename_for is the single routing seam — pin it directly."""
    d = env["dispatcher"]
    outcome = d.DispatchOutcome(
        result=d.DispatchResult.DEFERRED,
        source="heal", severity=d.Severity.WARNING, dedup_key="k",
    )
    assert d._log_filename_for(outcome) == "dispatcher-queued.jsonl"


def test_sent_and_batched_rate_cap_stay_in_the_sent_lane(env):
    """BATCHED_RATE_CAP deliberately does NOT move with DEFERRED: its rows
    are the downgrade-to-digest paper trail the operator is meant to see."""
    d = env["dispatcher"]

    def _lane(result):
        return d._log_filename_for(d.DispatchOutcome(
            result=result, source="audit", severity=d.Severity.WARNING,
            dedup_key=None,
        ))

    assert _lane(d.DispatchResult.SENT) == "dispatcher.jsonl"
    assert _lane(d.DispatchResult.BATCHED_RATE_CAP) == "dispatcher.jsonl"
    assert _lane(d.DispatchResult.FAILED) == "delivery-failures.jsonl"
    assert _lane(d.DispatchResult.SUPPRESSED_COOLDOWN) == "dispatcher-suppressed.jsonl"


def test_flap_demoted_fire_lands_in_queued_lane(env, monkeypatch):
    """The recurring-flapper demotion also returns DEFERRED — a demoted fire
    never reached the phone, so it belongs in the queued lane too. Its flap_*
    explanation fields ride along (the operator can still see WHY)."""
    d = env["dispatcher"]
    ...
    class _Decision:
        ...
        flap_count = 6
        newly_demoted = True
        coalesce_key = "sig:abc"

    out = d._demote_flapper(
        env["shared"], datetime(2099, 5, 1, 12, 0, tzinfo=timezone.utc),
        source="heal", severity=d.Severity.WARNING, dedup_key="k",
        catalog_event="security.config_drift", message="drift on team-bot-a",
        digest_meta=None, decision=_Decision(),
        body_hash=d._body_hash("drift on team-bot-a"), effective_cooldown=86_400,
    )
    assert out.result == d.DispatchResult.DEFERRED
    assert _read_log(env["shared"], "dispatcher.jsonl") == []
    queued = _read_log(env["shared"], "dispatcher-queued.jsonl")
    assert len(queued) == 1
    assert queued[0]["result"] == "deferred"
    assert queued[0]["flap_demoted"] is True
    assert queued[0]["flap_reason"] == "flap_count=6"


def test_queued_lane_is_date_partitioned_like_the_others(env):
    """The new lane uses the same alerts/<basename>/<YYYY-MM-DD>.jsonl
    convention, so the retention sweep and both readers reach it."""
    d = env["dispatcher"]
    d._append_log(
        env["shared"], "dispatcher-queued.jsonl",
        {"ts": "2099-05-02T08:00:00Z", "result": "deferred", "source": "heal"},
    )
    day_file = env["shared"] / "alerts" / "dispatcher-queued" / "2099-05-02.jsonl"
    assert day_file.is_file()
    assert list(d.iter_log_records(
        env["shared"], "dispatcher-queued.jsonl",
    ))[0]["result"] == "deferred"


# ── Cooldown gates the digest enqueue (spec-delta-digest-audit-noise D1) ────
#
# The digest branch used to return DEFERRED *above* the cooldown / identical-
# content gates, so every digest-defaulted producer's cooldown was dead code
# on that path. Live pod 2026-08-25: heal ticks every 300s per bot, 2 bots ×
# 288 ticks = 287 rows carrying THREE distinct dedup_keys — 98.3% of the
# operator's Recent Messages feed, collapsing to 3 digest lines at flush.
#
# The fix is two halves and neither works alone:
#   1. consult the gate before enqueuing + logging;
#   2. record the enqueue in dispatcher-state.json so half 1 has something
#      to read. Without (2) every tick reads last_entry=None and all 287
#      rows are still written.
# The tests below are split so each half can be mutated out independently
# and observed to go red.

_DRIFT_EVENT = "security.config_drift"     # catalog default: DAILY_DIGEST
_DAY = 86_400                              # heal's declared config_drift cooldown
...
def _state_entries(shared):
    p = shared / "alerts" / "dispatcher-state.json"
    if not p.exists():
        return {}
    return json.loads(p.read_text()).get("last_dispatch", {})


def _drift_send(env, *, dedup_key, message, now, **kw):
    """One heal config-drift tick — the exact live shape (digest default,
    heal's 24h cooldown, per-content dedup_key)."""
    return env["dispatcher"].send(
        shared_dir=env["shared"], network=env["network"],
        source="heal", message=message,
        catalog_event=_DRIFT_EVENT, dedup_key=dedup_key,
        severity=env["dispatcher"].Severity.ERROR,
        now=now, **kw,
    )


# ── Half 2: the digest path records state ──────────────────────────────────


def test_digest_enqueue_records_dispatch_state(env):
    """HALF 2 in isolation. A DEFERRED enqueue must stamp the per-(source,
    dedup_key) cooldown identity — the map ``_consult_repeat_gates`` reads.
    Before this fix the digest path never wrote it, so the gate always found
    ``last_entry is None`` and the cooldown could never bite.

    Also pins the design decision: ONE result-aware ``last_dispatch`` map
    rather than a parallel ``last_enqueue`` one, so the cooldown identity is
    lane-independent. The recorded result is the REAL one — a deferred
    enqueue is not a chat push and must not claim to be."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    out = _drift_send(
        env, dedup_key="heal/config_drift/team-bot-a/2a3299ef",
        message="🔧 config drift on team-bot-a: tiers:rungs", now=t0,
    )
    assert out.result == d.DispatchResult.DEFERRED

    entry = _state_entries(env["shared"])["heal::heal/config_drift/team-bot-a/2a3299ef"]
    assert entry["result"] == "deferred"        # not the old hardcoded "sent"
    assert len(entry["body_hash"]) == 16
    assert entry["ts"].startswith("2026-08-25T06:00")


def test_sent_path_still_records_result_sent(env):
    """Widening the writer must not relabel the delivery path. An actual
    chat push still records ``result: sent``."""
    d = env["dispatcher"]
    _force_immediate(env["shared"], "security.audit_finding")
    d.send(
        shared_dir=env["shared"], network=env["network"],
        source="audit", message="a finding",
        catalog_event="security.audit_finding", dedup_key="audit/batch/abc",
    )
    assert _state_entries(env["shared"])["audit::audit/batch/abc"]["result"] == "sent"


# ── Half 1: the gate runs before the enqueue ───────────────────────────────


def test_digest_repeat_with_identical_body_is_suppressed_not_enqueued(env):
    """The live heal shape: same drift set → same content-hashed dedup_key →
    same body. The identical-content floor fires first and the tick is
    SUPPRESSED_IDENTICAL. Crucially it does NOT reach the queue or the
    queued audit lane — that is the 287 rows we are not writing."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    body = "🔧 config drift on team-bot-a: tiers:rungs, tiers:autoUpgrade"
    key = "heal/config_drift/team-bot-a/2a3299ef"

    first = _drift_send(env, dedup_key=key, message=body, now=t0)
    second = _drift_send(
        env, dedup_key=key, message=body, now=t0 + timedelta(seconds=300),
    )

    assert first.result == d.DispatchResult.DEFERRED
    assert second.result == d.DispatchResult.SUPPRESSED_IDENTICAL
    assert len(_queue_records(env["shared"])) == 1
    assert len(_read_log(env["shared"], "dispatcher-queued.jsonl")) == 1
    suppressed = _read_log(env["shared"], "dispatcher-suppressed.jsonl")
    assert suppressed[-1]["result"] == "suppressed_identical"
    assert suppressed[-1]["catalog_event"] == _DRIFT_EVENT


def test_digest_repeat_with_changed_body_is_suppressed_by_cooldown(env):
    """Same dedup_key, different body — the identical-content floor does not
    apply, so the SOURCE'S COOLDOWN is what has to hold. This is the arm the
    spec names: a within-cooldown repeat returns SUPPRESSED_COOLDOWN and
    lands in the suppressed lane instead of enqueuing a duplicate."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    key = "heal/config_drift/team-bot-a/2a3299ef"

    first = _drift_send(env, dedup_key=key, message="drift: 2 keys", now=t0)
    second = _drift_send(
        env, dedup_key=key, message="drift: 2 keys (rechecked)",
        now=t0 + timedelta(hours=6),
    )

    assert first.result == d.DispatchResult.DEFERRED
    assert second.result == d.DispatchResult.SUPPRESSED_COOLDOWN
    assert "cooldown:" in (second.error or "")
    assert len(_queue_records(env["shared"])) == 1


def test_digest_fires_again_once_the_cooldown_elapses(env):
    """The gate caps cadence, it does not mute the event. Past the 24h
    window the same key enqueues again — the operator keeps getting a daily
    reminder that the condition still holds."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    key = "heal/config_drift/team-bot-a/2a3299ef"
    _drift_send(env, dedup_key=key, message="drift: 2 keys", now=t0)
    later = _drift_send(
        env, dedup_key=key, message="drift: 2 keys (day 2)",
        now=t0 + timedelta(seconds=_DAY + 1),
    )
    assert later.result == d.DispatchResult.DEFERRED
    assert len(_queue_records(env["shared"])) == 2


# ── Invariants the spec pins ───────────────────────────────────────────────


def test_digest_first_fire_of_every_new_dedup_key_enqueues(env):
    """No key is ever silenced on its first fire — the gate only ever
    suppresses a REPEAT. Three distinct keys inside one cooldown window →
    three queue rows."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    keys = [
        "heal/config_drift/team-bot-a/2a3299ef",
        "heal/config_drift/team-bot-b/cf60be1b",
        "heal/config_drift/team-bot-a/0d16ffc1",
    ]
    for i, key in enumerate(keys):
        out = _drift_send(
            env, dedup_key=key, message=f"drift {i}",
            now=t0 + timedelta(seconds=60 * i),
        )
        assert out.result == d.DispatchResult.DEFERRED
    assert len(_queue_records(env["shared"])) == 3


def test_digest_content_change_mints_a_new_key_and_fires_fresh(env):
    """A content change mints a new dedup_key (heal hashes the drift list),
    so a TRANSITION reaches the operator even inside the old key's cooldown.
    That is the whole point of the per-content key and the fix must not
    break it."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    _drift_send(
        env, dedup_key="heal/config_drift/team-bot-a/2a3299ef",
        message="drift: tiers:rungs, tiers:autoUpgrade", now=t0,
    )
    # Operator reverted one key — new drift set, new hash, new key.
    out = _drift_send(
        env, dedup_key="heal/config_drift/team-bot-a/0d16ffc1",
        message="drift: tiers:rungs", now=t0 + timedelta(minutes=5),
    )
    assert out.result == d.DispatchResult.DEFERRED
    assert len(_queue_records(env["shared"])) == 2


def test_digest_dedup_key_none_callers_are_unaffected(env):
    """``dedup_key=None`` callers (resolve/recovery, the rate-cap downgrade)
    have no cooldown identity by construction and must keep flowing on every
    tick. The spec names this as a deliberate bound on D1's coverage, not a
    hole: flush-time ``auto:<source>:<event>:<body_hash>`` collapse still
    keeps the delivered digest to one line."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    for i in range(5):
        out = _drift_send(
            env, dedup_key=None, message="recovery notice",
            now=t0 + timedelta(seconds=300 * i),
        )
        assert out.result == d.DispatchResult.DEFERRED
    assert len(_queue_records(env["shared"])) == 5
    assert len(_read_log(env["shared"], "dispatcher-queued.jsonl")) == 5
    assert _state_entries(env["shared"]) == {}   # nothing to key state on


def test_digest_source_with_zero_cooldown_still_enqueues_every_tick(env):
    """The gate applies the SOURCE'S declared cooldown — it does not invent
    one. ``update_watcher`` declares 0, so its digest enqueues are untouched
    (it contributed 1 of the 292 live rows, not 287). Pins that this fix
    caps cadence only where a producer asked for a cap."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    assert d._DEFAULT_SOURCE_COOLDOWN_SECONDS.get("update_watcher") == 0
    for i in range(4):
        out = d.send(
            shared_dir=env["shared"], network=env["network"],
            source="update_watcher",
            catalog_event="updates.evolve_repo",   # weekly-digest only
            dedup_key="update_watcher/origin-main",
            message=f"🔄 origin/main has new commits (sha {i})",
            now=t0 + timedelta(seconds=900 * i),
        )
        assert out.result == d.DispatchResult.DEFERRED, i
    assert len(_queue_records(env["shared"], "weekly")) == 4


def test_must_page_allowlist_cannot_reach_the_digest_path(env):
    """The five MUST_PAGE events are immediate-ONLY, so no operator setting
    can route them through the gated digest branch. Pinned because the fix
    must NOT be built as though ``is_safety_critical`` carried the same
    guarantee — it does not (see the next test)."""
    from evolve_admin.alerts.catalog import MUST_PAGE_ALLOWLIST, Frequency, by_key
    for key in MUST_PAGE_ALLOWLIST:
        entry = by_key(key)
        assert entry is not None, key
        assert tuple(entry.allowed_frequencies) == (Frequency.IMMEDIATE,), key


def test_operator_lowered_safety_critical_event_still_fires_first(env):
    """``security.autonomy_posture_drift`` is safety-critical yet allows
    DAILY_DIGEST, so it DOES reach the gated branch once an operator lowers
    it. Nothing is lost: the first fire of a key still enqueues, and the
    within-cooldown repeat is one ``_dedup_records`` would have collapsed at
    flush anyway. The fix's safety rests on that collapse, not on the
    allowlist."""
    from evolve_admin.alerts import subscriptions as _subs
    from evolve_admin.alerts.catalog import Frequency
    d = env["dispatcher"]
    _subs.write_subscription(
        env["shared"], "security.autonomy_posture_drift",
        frequency=Frequency.DAILY_DIGEST,
    )
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    common = dict(
        shared_dir=env["shared"], network=env["network"], source="heal",
        catalog_event="security.autonomy_posture_drift",
        dedup_key="heal/posture/team-bot-a",
        severity=d.Severity.CRITICAL,
    )
    first = d.send(message="posture lowered on team-bot-a", now=t0, **common)
    repeat = d.send(
        message="posture lowered on team-bot-a",
        now=t0 + timedelta(seconds=300), **common,
    )
    assert first.result == d.DispatchResult.DEFERRED
    assert repeat.result == d.DispatchResult.SUPPRESSED_IDENTICAL
    assert len(_queue_records(env["shared"])) == 1


# ── The live reproduction + the content-preservation proof ─────────────────


def test_day_of_heal_ticks_collapses_to_the_distinct_key_count(env):
    """The reported defect, end to end. Two bots × 288 ticks/day carrying
    three distinct dedup_keys wrote 287 queue rows and 287 audit rows.
    Post-fix: one row per distinct key."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 0, 0, tzinfo=timezone.utc)
    bodies = {
        "heal/config_drift/team-bot-a/2a3299ef": "drift on team-bot-a: tiers:rungs",
        "heal/config_drift/team-bot-b/cf60be1b": "drift on team-bot-b: tiers:autoUpgrade",
    }
    for tick in range(288):          # StartInterval 300 for a full day
        for key, body in bodies.items():
            _drift_send(
                env, dedup_key=key, message=body,
                now=t0 + timedelta(seconds=300 * tick),
            )
    assert len(_queue_records(env["shared"])) == len(bodies)
    assert len(_read_log(env["shared"], "dispatcher-queued.jsonl")) == len(bodies)
    assert d.DispatchResult.SENT.value not in {
        r["result"] for r in _read_log(env["shared"], "dispatcher-queued.jsonl")
    }


def test_delivered_digest_is_byte_identical_before_and_after_the_gate(env):
    """Content preservation for the CONSTANT-BODY case — which is the live
    heal shape and the one the spec's 287-row evidence is drawn from. That
    "live heal shape" clause is an observation, not an inference from
    heal's render path: test_alerts_digest_live_census.py carries the census
    fixture recording the 3 distinct (dedup_key, message_excerpt) pairs found
    across all 287 rows, and replays the censused day through this same
    pre/post comparison.
    ``digest_dispatcher._dedup_records`` already collapses the queue on
    ``(catalog_event, digest_collapse_key)`` at flush, so the 287 suppressed
    enqueues were producing nothing the 3 surviving ones don't.

    Pre-fix queue = every tick enqueued (``_enqueue_digest`` directly, which
    is what the old short-circuit did). Post-fix queue = the same ticks
    through ``send``. The rendered bodies must match byte for byte.

    SCOPE, stated so this test is not read as proving more than it does:
    every collapsed group here has a constant body, so the queues differ
    only in LENGTH. That is deliberate — it isolates "does dropping the
    duplicate enqueues change the render?" — but it means this test cannot
    speak to a producer whose body MUTATES under a stable dedup_key.
    ``test_mutable_body_under_a_stable_dedup_key_keeps_the_first_body``
    below covers that case, where the render does change."""
    ...
    fires = [
        ("heal/config_drift/team-bot-a/2a3299ef", "🔧 drift on team-bot-a: tiers:rungs"),
        ("heal/config_drift/team-bot-b/cf60be1b", "🔧 drift on team-bot-b: tiers:autoUpgrade"),
        ("heal/config_drift/team-bot-a/0d16ffc1", "🔧 drift on team-bot-a: agents"),
    ]

    pre_shared = env["shared"] / "pre"
    pre_shared.mkdir()
    for tick in range(96):
        for key, body in fires:
            d._enqueue_digest(
                pre_shared, "daily_digest", t0 + timedelta(seconds=300 * tick),
                source="heal", catalog_event=_DRIFT_EVENT,
                severity=d.Severity.ERROR, dedup_key=key, message=body,
            )
            _drift_send(
                env, dedup_key=key, message=body,
                now=t0 + timedelta(seconds=300 * tick),
    ...
    assert len(pre) == 96 * len(fires)      # the old behaviour, reproduced
    assert len(post) == len(fires)          # the new one
    assert _render_digest(post, "daily") == _render_digest(pre, "daily")


def test_mutable_body_under_a_stable_dedup_key_renders_the_latest_body(env):
    """The mutable-body case, restored to parity with the constant-body one.

    ``cost.daily_threshold`` is a shipped DAILY_DIGEST-default event whose
    dedup_key is stable for a whole day
    (``spend_alert/threshold/<bot>/<date>``, spend_alert.py) while the FIRST
    line of its body — the only line ``digest_dispatcher._short_line`` keeps
    — carries a changing ``${amount:.2f}``. spend_alert is STATE_PERSISTS
    with an 86400s cooldown, and its caller only writes the daily flag-file
    on SENT, so a DEFERRED leaves the flag absent and the producer re-sends
    every tick all day.

    D1 alone froze the digest on the FIRST amount: the repeats were
    cooldown-suppressed, so the record ``_dedup_records`` used to pick as
    "most recent" no longer existed. $5.02 → $12.40 → $41.85 rendered
    **$5.02** — an understated cost figure is exactly the kind of number an
    operator acts on.

    Now the suppressed repeat REPLACES the pending record instead of
    appending a second one, so the digest reads $41.85 and the rendered body
    is byte-identical to the pre-D1 queue's. This is the same assertion
    ``test_delivered_digest_is_byte_identical_before_and_after_the_gate``
    makes for the constant-body case; between them the byte-identical claim
    now covers both shapes, with no producer left exempt.

    The suppression itself is untouched — the repeats still return
    SUPPRESSED_COOLDOWN and still write no queued audit row. That is the
    whole point: the noise fix and the content fix are separable, and this
    keeps both."""
    from evolve_admin.alerts.digest_dispatcher import _render_digest
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 8, 0, tzinfo=timezone.utc)
    key = "spend_alert/threshold/admin_bot/2026-08-25"
    ticks = [(0, 5.02), (3, 12.40), (7, 41.85)]   # hours past t0, $ spent

    pre_shared = env["shared"] / "pre"
    pre_shared.mkdir()
    for hours, amount in ticks:
        now = t0 + timedelta(hours=hours)
        out = d.send(
            shared_dir=env["shared"], network=env["network"],
            source="spend_alert", catalog_event="cost.daily_threshold",
            dedup_key=key, severity=d.Severity.WARNING,
            payload={"bot_id": "admin_bot", "amount": amount, "threshold": 5.00},
            now=now,
        )
        # Same ticks into a pre-fix queue: the old branch enqueued every time.
        d._enqueue_digest(
            pre_shared, "daily_digest", now,
            source="spend_alert", catalog_event="cost.daily_threshold",
            severity=d.Severity.WARNING, dedup_key=key,
            message=f"💰 admin_bot's daily spend hit ${amount:.2f}\nThreshold: $5.00.",
        )
        expected = d.DispatchResult.DEFERRED if hours == 0 else d.DispatchResult.SUPPRESSED_COOLDOWN
        assert out.result == expected, (hours, out.result)

    pre_body = _render_digest(_queue_records(pre_shared), "daily")
    post_body = _render_digest(_queue_records(env["shared"]), "daily")

    assert "$41.85" in pre_body and "$5.02" not in pre_body    # most recent won
    assert "$41.85" in post_body and "$5.02" not in post_body  # ...and still does
    assert post_body == pre_body     # byte-identical, mutable body included

    # D1's noise fix is intact: the repeats never reached either lane.
    assert len(_queue_records(env["shared"])) == 1
    assert len(_read_log(env["shared"], "dispatcher-queued.jsonl")) == 1


def test_mutable_body_queue_stays_one_row_across_a_full_day_of_ticks(env):
    """The bound, pinned. The refresh REPLACES the pending record; it must
    never append a second one, or we reopen the growth failure mode by the
    back door (the 2026-06 leak was 4804/4827 records).

    288 ticks with a body that changes EVERY tick — the pathological shape
    approach "just enqueue when the body changed" could not bound — leaves
    exactly one queue row, one queued audit row, and renders the last
    figure."""
    from evolve_admin.alerts.digest_dispatcher import _render_digest
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 0, 0, tzinfo=timezone.utc)
    ...
    for tick in range(288):
        d.send(
            ...
            dedup_key=key, severity=d.Severity.WARNING,
            payload={
                "bot_id": "admin_bot", "amount": 5.0 + tick * 0.13,
                "threshold": 5.00,
            },
            now=t0 + timedelta(seconds=300 * tick),
        )
    assert len(_queue_records(env["shared"])) == 1
    assert len(_read_log(env["shared"], "dispatcher-queued.jsonl")) == 1
    assert f"${5.0 + 287 * 0.13:.2f}" in _render_digest(
        _queue_records(env["shared"]), "daily",
    )


def test_refresh_does_not_extend_the_cooldown_window(env):
    """Refreshing the pending record must not re-stamp the cooldown identity.

    If it did, a producer ticking every 5 minutes would push the window
    forward on every tick and the key would never re-fire — the digest would
    keep one silently-updating row forever instead of reminding the operator
    daily. The window is measured from the ENQUEUE, so past 24h the key
    enqueues again (a second row, then two after collapse — a fresh
    reminder), exactly as
    ``test_digest_fires_again_once_the_cooldown_elapses`` pins for the
    constant-body case."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    key = "heal/config_drift/team-bot-a/2a3299ef"
    ...
    for hours in (6, 12, 18, 23):
        out = _drift_send(
            env, dedup_key=key, message=f"drift: 2 keys (h{hours})",
            now=t0 + timedelta(hours=hours),
        )
        assert out.result == d.DispatchResult.SUPPRESSED_COOLDOWN, hours
    assert len(_queue_records(env["shared"])) == 1
    later = _drift_send(
        env, dedup_key=key, message="drift: 2 keys (day 2)",
        now=t0 + timedelta(seconds=_DAY + 1),
...
def test_constant_body_repeat_never_claims_the_queue(env, monkeypatch):
    """The refresh must be free for the producer this whole spec-delta exists
    to quiet. heal's repeat differs from the pending record only in ``ts``,
    and nothing downstream renders ``ts``, so the refresh returns BEFORE the
    ``os.replace`` claim. Pinned with a spy rather than by outcome, because
    the outcome (one queue row) is identical either way — what we are
    protecting is that 288 ticks/day do not rotate the shared queue file 288
    times under every other producer's appends."""
    from evolve_admin.alerts import digest_dispatcher as dd
    claims = []
    real = os.replace
    ...
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    key = "heal/config_drift/team-bot-a/2a3299ef"
    body = "🔧 config drift on team-bot-a: tiers:rungs"
    _drift_send(env, dedup_key=key, message=body, now=t0)
    monkeypatch.setattr(dd.os, "replace", _spy)
    ...
    # POSITIVE CONTROL. Without this the assertion above is vacuous: a spy
    # whose filter never matches anything also reports an empty list, so a
    # renamed _REFRESH_CLAIM_INFIX would silently turn this test into a
    # no-op. One changed body must make the same spy fire exactly once.
    _drift_send(
        env, dedup_key=key, message=body + " (and agents)",
        now=t0 + timedelta(seconds=300 * 20),
    )
    assert len(claims) == 1


def test_a_corrupt_queue_byte_does_not_take_down_a_producer_tick(env):
    """A non-UTF-8 byte in the queue must not escape ``send``.

    ``_read_jsonl`` decodes strictly, so a corrupt byte raises
    ``UnicodeDecodeError`` — a ``ValueError``. Before the refresh existed
    only ``flush`` read this file, so the blast radius was one flush; the
    gated repeat now reads it on the producer's own tick. Deliberately
    caught at ``_refresh_digest_queue`` rather than papered over inside
    ``_read_jsonl`` with ``errors="replace"``: #3797 settled that an
    undecodable byte is a reportable finding, and softening the shared
    decode would undo that for the flush path too.

    Degrades to D1 behaviour — the digest keeps the older figure — rather
    than crashing the producer over queue maintenance."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    key = "heal/config_drift/team-bot-a/2a3299ef"
    _drift_send(env, dedup_key=key, message="drift: 2 keys", now=t0)
    ...
    queue = env["shared"] / "alerts" / "digest-pending" / "daily.jsonl"
    queue.write_bytes(queue.read_bytes() + b"\xff\xfe not utf-8\n")

    repeat = _drift_send(
        env, dedup_key=key, message="drift: 2 keys (rechecked)",
        now=t0 + timedelta(hours=6),
    )
    assert repeat.result == d.DispatchResult.SUPPRESSED_COOLDOWN


def test_refresh_is_skipped_when_the_key_was_never_queued(env):
    """The cooldown identity is lane-INDEPENDENT (D1), so the gate can fire
    on a digest tick whose previous fire went out on the IMMEDIATE lane. No
    pending record exists to refresh, and the refresh must not invent one —
    an event the operator already saw pushed must not also materialize in the
    digest."""
    from evolve_admin.alerts import subscriptions as _subs
    from evolve_admin.alerts.catalog import Frequency
    d = env["dispatcher"]
    ...
    _force_immediate(env["shared"], _DRIFT_EVENT)
    first = _drift_send(env, dedup_key=key, message="drift: 2 keys", now=t0)
    assert first.result != d.DispatchResult.DEFERRED
    assert _queue_records(env["shared"]) == []

    _subs.write_subscription(
        env["shared"], _DRIFT_EVENT, frequency=Frequency.DAILY_DIGEST,
    )
    repeat = _drift_send(
        env, dedup_key=key, message="drift: 2 keys (rechecked)",
        now=t0 + timedelta(hours=1),
...
# ── The second digest path: recurring-flapper demotion ─────────────────────


def _demote(env, *, now, dedup_key="heal/flap/team-bot-a", message="drift on team-bot-a"):
    d = env["dispatcher"]
    ...
    return d._demote_flapper(
        env["shared"], now,
        source="heal", severity=d.Severity.WARNING, dedup_key=dedup_key,
        catalog_event=_DRIFT_EVENT, message=message,
        digest_meta=None, decision=_Decision(),
        body_hash=d._body_hash(message), effective_cooldown=_DAY,
    )


def test_flap_demotion_records_state_and_gates_its_repeat(env):
    """The demotion path has the same shape as the digest short-circuit —
    enqueue, log, return DEFERRED — so it gets the same two halves. A
    demoted signature ticking every few minutes queues once per cooldown
    window, not once per tick."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)

    first = _demote(env, now=t0)
    assert first.result == d.DispatchResult.DEFERRED
    entry = _state_entries(env["shared"])["heal::heal/flap/team-bot-a"]
    assert entry["result"] == "deferred"

    repeat = _demote(env, now=t0 + timedelta(seconds=300))
    assert repeat.result == d.DispatchResult.SUPPRESSED_IDENTICAL
    assert len(_queue_records(env["shared"])) == 1
    assert len(_read_log(env["shared"], "dispatcher-queued.jsonl")) == 1


def test_flap_demotion_repeat_refreshes_its_pending_record(env):
    """Both digest paths are in scope. The demotion path enqueues, gates and
    records exactly like the ``send`` short-circuit, so it refreshes too."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    first = _demote(env, now=t0, message="drift on team-bot-a: 2 keys")
    assert first.result == d.DispatchResult.DEFERRED
    repeat = _demote(
        env, now=t0 + timedelta(seconds=300),
        message="drift on team-bot-a: 5 keys",
    )
    assert repeat.result == d.DispatchResult.SUPPRESSED_COOLDOWN
    rows = _queue_records(env["shared"])
    assert len(rows) == 1
    assert rows[0]["message"] == "drift on team-bot-a: 5 keys"
    assert len(_read_log(env["shared"], "dispatcher-queued.jsonl")) == 1


def test_flap_demotion_first_fire_of_a_key_still_queues(env):
    """The demotion gate suppresses repeats only — a fresh coalesce key
    still reaches the digest, so a newly-demoted signature is never lost."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    a = _demote(env, now=t0, dedup_key="heal/flap/team-bot-a", message="a")
    b = _demote(env, now=t0, dedup_key="heal/flap/team-bot-b", message="b")
    assert a.result == b.result == d.DispatchResult.DEFERRED
    assert len(_queue_records(env["shared"])) == 2


def test_flap_demotion_without_dedup_key_still_queues_every_tick(env):
    """``dedup_key=None`` on the demotion path keeps flowing too — same
    invariant as the digest short-circuit."""
    d = env["dispatcher"]
    t0 = datetime(2026, 8, 25, 6, 0, tzinfo=timezone.utc)
    for i in range(3):
        out = _demote(env, now=t0 + timedelta(seconds=300 * i), dedup_key=None)
        assert out.result == d.DispatchResult.DEFERRED
    assert len(_queue_records(env["shared"])) == 3
    assert _state_entries(env["shared"]) == {}
