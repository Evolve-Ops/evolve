...
#
# A ``sys.addaudithook`` hook sees only THIS interpreter, so a write performed
# by a child process is invisible to it -- and this codebase is full of writers
# whose fallback for "I cannot write that path" is to shell out to ``sudo``.
# On a CI runner (passwordless sudo) those escalations SUCCEED, so a guard that
# only watched Python writes left the host genuinely mutated: a shard run used
# to end with a root-owned /Users/Shared/evolve/{logs,better-engine,network.json}
# on the runner. Two consequences are handled here:
#
#   * the hook raises ``_SharedDirGuardError``, an OSError that is deliberately
#     NOT a PermissionError, because ``except PermissionError:`` is the exact
#     branch those writers use to decide "escalate to root" (config.save_network,
#     evolve_config._patch_network_json, deploy.write_install_json,
#     deploy._create_dir_with_mode, ...). Staying an OSError keeps the
#     swallow-and-carry-on behaviour the 52 baselined files already rely on;
#     ceasing to be a PermissionError is what stops the guard from handing the
#     write to sudo.
#   * the hook also watches ``subprocess.Popen`` and blocks a child whose argv
#     names a guarded root AND whose command is a known mutating one (mkdir, cp,
#     chmod, ...). Known gap, deliberate: a child that mutates the root some
#     other way (an interpreter of its own) is still invisible -- a shell
#     one-liner is tokenized, an arbitrary child is not -- and
#     READS through subprocess (``sudo /bin/cat``) stay allowed -- the guard has
#     never guarded reads, and ~6k of them exist.
#
...
class _SharedDirGuardError(OSError):
    """Raised by the audit hook for a blocked shared-dir mutation.

    An OSError (so the ``except OSError`` / ``except Exception`` paths that
    already swallow the block keep swallowing it -- the autouse fixture, not
    this exception, is what makes an offence visible) but NOT a
    PermissionError, so it cannot be mistaken for "this process lacks the
    permission, retry under sudo" by a production fallback.
    """
...
# Commands whose whole job is to mutate a path. A subprocess is blocked only
# when its argv names a guarded root AND one of these appears in it, so the
# many legitimate READS through a child (``sudo /bin/cat``, ``git -C``) are
# untouched -- this guard has never guarded reads.
_MUTATING_BINS = frozenset({
    "mkdir", "rmdir", "cp", "mv", "rm", "ln", "chmod", "chown", "chgrp",
    "touch", "tee", "truncate", "install", "dd", "shred", "unlink",
})


def _mutating_argv(argv: object) -> list[str] | None:
    """Return argv's tokens when it invokes a mutating command, else None."""
    if isinstance(argv, (str, bytes)):
        return None
    try:
        parts = [
            a.decode("utf-8", "replace") if isinstance(a, bytes) else os.fspath(a)
            for a in argv
        ]
    except (TypeError, ValueError):
        return None
    # ``shell=True`` arrives here as ["/bin/sh", "-c", "mkdir -p /x"], so the
    # verb and the path are buried in one element. Split on whitespace so both
    # are seen; a path with a space in it is then missed, which over-blocks
    # nothing and is the direction this guard should err in.
    tokens = [
        t for part in parts if isinstance(part, str)
        for t in (part.split() if " " in part else (part,))
    ]
    if not any(os.path.basename(t) in _MUTATING_BINS for t in tokens):
        return None
    return tokens
...
# EXEMPT: the Linux e2e suite. Everything above is about UNIT tests, where a
# write under a canonical shared dir is always a bug -- a fixture network.json
# missing "sharedDir" falling back to DEFAULT_SHARED_DIR. tests/e2e_linux/ is
# the opposite: roadmap 8.3's exit criterion is a LIVE deploy that provisions a
# real pod at the real /var/lib/evolve on a throwaway runner. Blocking it does
# not merely mis-report -- it breaks the thing under test (the apply unit fails
# to start, the owned-jobs registry and app-cron-map cannot be written), which
# is how this reddened main from 2026-09-11 to 2026-09-14.
#
# Safe because that suite gates itself far more narrowly than this guard could:
# sys.platform linux AND CI set AND EVOLVE_PLATFORM=linux AND not sharded --
# see its module docstring, "excludes real Linux pods/dev boxes". A developer
# machine (either platform) never runs it, so the host this guard protects is
# never the host it writes to.
#
# Scoped by PATH, not by baselining the file: the baseline only silences the
# assertion while the audit hook still blocks the write, so it would leave the
# deploy just as broken and merely hide the evidence. This is also why the
# exemption lives here, next to the guard, rather than in a child conftest --
# an exemption invisible from the guard's own source is one nobody will find.
_E2E_EXEMPT_DIR = str(_TESTS_DIR / "e2e_linux") + os.sep


def _current_test_is_exempt() -> bool:
    """True while a tests/e2e_linux/ test is running (see _E2E_EXEMPT_DIR)."""
    return _current_test.get("path", "").startswith(_E2E_EXEMPT_DIR)


def _shared_dir_audit_hook(event: str, args: tuple) -> None:
    if event == "subprocess.Popen":
        # args = (executable, args, cwd, env). A child process is invisible to
        # this hook once it starts, so the argv is the only chance to see it.
        parts = _mutating_argv(args[1] if len(args) > 1 else None)
        if parts is None:
            return
        # The e2e exemption applies HERE too. tests/e2e_linux/ exists to
        # deploy a real pod on a throwaway CI VM, and it does that almost
        # entirely through sudo children -- `sudo /bin/mkdir -p <shared>/logs`
        # is the first one. Checking the exemption only on the in-process
        # branch below blocked every one of them, so the Linux e2e job died on
        # its first write with a guard error while the guard's own exemption
        # said it should not have. Same rule, both branches.
        if _current_test_is_exempt():
            return
        for part in parts:
            ap = _guarded_path(part)
            if ap is None:
                continue
            _shared_dir_writes.setdefault(_current_test["nodeid"], set()).add(ap)
            raise _SharedDirGuardError(
                f"[shared-dir-guard] blocked a test subprocess from mutating "
                f"the real pod shared dir: {ap} (argv: {' '.join(parts)})"
            )
        return
    if event == "open":
        ...
        return
    # Checked here, not at function entry: the hook runs on every audited
    # event in the process, and only write-shaped events get this far.
    if _current_test_is_exempt():
        return
...
def pytest_runtest_protocol(item, nextitem):  # noqa: D103
    ...
    _current_test["path"] = str(getattr(item, "fspath", "") or "")
...
def _no_real_shared_dir_writes():
    ...
    yield
    if _GUARD_OFF or _current_test_is_exempt():
        return
    ...
    pytest.fail(
        ...
        "The other cause is a path this test never chose: an EARLIER file in "
        "the same shard left a real file under the shared dir (via a sudo "
        "child process, which the hook now blocks too), and this test merely "
        "read it. Check what the run created before blaming this file -- and "
        "never silence it by adding the file to the baseline.\n"
