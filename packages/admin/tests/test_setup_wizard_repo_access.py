...
def test_unregistered_key_is_loud_and_prints_walkthrough(tmp_path, capsys):
    ...
    summary, out = _run(
        ...
        anon_probe_fn=lambda u: False,  # origin not anonymously readable
    )
...
# ── 2b. public HTTPS origin → anonymous pull verified, no deploy-key alarm ────


def test_public_https_origin_verifies_anonymously_and_stays_quiet(tmp_path, capsys):
    """The standard public-clone install: key unregistered, but the origin is
    a public HTTPS repo — a credential-free read succeeds, so the step reports
    verified and never shows the deploy-key walkthrough (a beta tester cannot
    register a deploy key on a repo they don't admin)."""
    net_path, network = _net(tmp_path)
    ...
    summary, out = _run(
        ...
        register_fn=lambda *a, **k: register_calls.append(1) or _Reg(added=True),
        anon_probe_fn=lambda u: u == "https://github.com/acme/evolve",
    )

    assert summary["auth_ok"] is True
    assert summary["loud"] is False
    assert summary["registered"] is False
    assert register_calls == []  # no GitHub API call when anonymous read works
    assert "will NOT stay current" not in out
    assert "verified" in out.lower()


def test_ssh_origin_never_takes_anonymous_shortcut(tmp_path, capsys):
    """A pod cloned over SSH pulls over SSH — even if the repo is publicly
    readable over HTTPS, the anonymous probe must not short-circuit the
    deploy-key walkthrough (web_url normalizes git@… to https://…, so the
    gate must use the RAW origin URL)."""
    net_path, network = _net(tmp_path)
    probes: list[str] = []
    summary, out = _run(
        net_path, network, tmp_path, capsys,
        discover_fn=lambda n: _RepoRemote(
            True, "https://github.com/acme/evolve",
            raw_url="git@github.com:acme/evolve.git",
        ),
        deploy_key_fn=lambda: _DK(auth_test_ok=False),
        load_pat_fn=lambda sd: None,
        anon_probe_fn=lambda u: probes.append(u) or True,  # would false-green
    )

    assert probes == []  # gate rejects SSH transport before probing
    assert summary["auth_ok"] is False
    assert summary["loud"] is True
    assert "will NOT stay current" in out
...
def test_auto_register_with_pat_uses_readonly_and_reverifies(tmp_path, capsys):
    ...
    summary, out = _run(
        ...
        anon_probe_fn=lambda u: False,  # private origin: PAT path stays live
    )
