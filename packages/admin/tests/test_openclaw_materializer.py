...
Phase 3b of internal/spec-openclaw-json-derived-artifact-2026-05-24.md.
...
_FAKE_MANIFEST = {
    ...
    "configSchema": {
        ...
        "properties": {
            ...
            "prefixHashLedgerEnabled": {"type": "boolean"},
        },
    },
}
...
def test_hand_armed_flag_survives_materialize(shared_dir, manifest, fixed_now):
    """Regression: 2026-07-31, an operator hand-set
    ``prefixHashLedgerEnabled: true`` in openclaw.json and the next deploy
    reset it to False. Cause: the field was in the defaults registry but had
    no config_sandbox TunableKey, so Pass 2's drift detection (which only
    walks the tunable map) never saw the edit and Pass 3b wrote the default.
    With the TunableKey present, the edit must auto-promote and survive.
    ``layer2Enforce`` had the same latent bug — a hand-armed enforcement
    gate was silently disarmed by any deploy."""
    current = {"botId": "team_bot_a", "prefixHashLedgerEnabled": True}
    r = materialize_evolve_plugin_config(
        "team_bot_a", _NETWORK, current,
        defaults_registry=_DEFAULTS, shared_dir=shared_dir,
        plugin_manifest=manifest, now=fixed_now,
    )
    assert r.new_block["prefixHashLedgerEnabled"] is True
    assert "prefixHashLedgerEnabled" in r.auto_promoted
    entry = read_bot_overrides(shared_dir, "team_bot_a").get(
        "openclaw.plugins.evolve.prefixHashLedgerEnabled")
    assert entry is not None and entry.value is True
...
# ─── materialize: override-clobber surfacing ──────────────────────────────
# Live incident 2026-07-31: a tier A/B test hand-set tier=off, which was
# auto-promoted to a needs_review override. Every subsequent hand-set tier
# (monitor / manage / full, including the restore) was then clobbered back
# to "off" by the recorded override on the next deploy — silently. These
# tests pin the loud surface: MaterializeResult.override_clobbered + a
# WARN log line.


def test_override_clobbering_hand_edit_is_surfaced(shared_dir, manifest, fixed_now, caplog):
    """A recorded override overwriting a DIFFERENT live value must land in
    override_clobbered and emit a WARN — the incident shape."""
    import logging
    write_override(
        shared_dir, "team_bot_a", "openclaw.plugins.evolve.tier",
        "off", set_by="auto:drift_2026-07-30T00:00:00Z",
        needs_review=True, now=fixed_now,
    )
    current = {"botId": "team_bot_a", "tier": "monitor"}   # later hand-edit
    with caplog.at_level(logging.WARNING, logger="evolve_admin.openclaw_materializer"):
        r = materialize_evolve_plugin_config(
            "team_bot_a", _NETWORK, current,
            defaults_registry=_DEFAULTS, shared_dir=shared_dir,
    ...
    # Behavior is unchanged: the override still wins...
    assert r.new_block["tier"] == "off"
    assert r.kept_overrides == ["tier"]
    # ...but the clobber is now loud.
    assert len(r.override_clobbered) == 1
    clobber = r.override_clobbered[0]
    assert clobber.field == "tier"
    assert clobber.override_value == "off"
    assert clobber.live_value == "monitor"
    assert "OVERRODE hand-edited value" in clobber.message
    assert "openclaw.plugins.evolve.tier" in clobber.message
    assert any(
        "OVERRODE hand-edited value" in rec.message and "'monitor'" in rec.message
        for rec in caplog.records
    )


def test_override_clobbers_restore_to_default(shared_dir, manifest, fixed_now):
    """The incident's cruelest edge: hand-restoring the DEFAULT value is
    still clobbered by the recorded override (Pass 2 skips drift checks
    for override-backed fields), so it must be flagged too."""
    write_override(
        shared_dir, "team_bot_a", "openclaw.plugins.evolve.tier",
        "off", set_by="auto:drift_x", needs_review=True, now=fixed_now,
    )
    current = {"botId": "team_bot_a", "tier": "full"}   # restore to default
    r = materialize_evolve_plugin_config(
        "team_bot_a", _NETWORK, current,
        defaults_registry=_DEFAULTS, shared_dir=shared_dir,
        plugin_manifest=manifest, now=fixed_now,
    ...
    assert r.new_block["tier"] == "off"
    assert [c.field for c in r.override_clobbered] == ["tier"]
    assert r.override_clobbered[0].live_value == "full"


def test_override_matching_current_is_not_a_clobber(shared_dir, manifest, fixed_now):
    """Override value == live value → nothing was overwritten; stay quiet."""
    write_override(
        shared_dir, "team_bot_a", "openclaw.plugins.evolve.tier",
        "monitor", set_by="operator", now=fixed_now,
    )
    ...
    assert r.override_clobbered == []


def test_override_filling_absent_field_is_not_a_clobber(shared_dir, manifest, fixed_now):
    """Override applied where the live block has no value at all — that's
    plain consumption, not a clobber."""
    write_override(
        shared_dir, "team_bot_a", "openclaw.plugins.evolve.tier",
        "monitor", set_by="operator", now=fixed_now,
    )
    r = materialize_evolve_plugin_config(
        "team_bot_a", _NETWORK, {"botId": "team_bot_a"},
        defaults_registry=_DEFAULTS, shared_dir=shared_dir,
        plugin_manifest=manifest,
    )
    assert r.new_block["tier"] == "monitor"
...
def test_same_deploy_auto_promote_is_not_a_clobber(shared_dir, manifest, fixed_now):
    """The deploy that auto-promotes a hand-edit writes an override whose
    value IS the live value — no clobber on that same pass."""
    current = {"botId": "team_bot_a", "tier": "monitor"}
    r = materialize_evolve_plugin_config(
        "team_bot_a", _NETWORK, current,
        defaults_registry=_DEFAULTS, shared_dir=shared_dir,
...
def test_clobber_detected_on_non_registry_schema_field(shared_dir, manifest, fixed_now):
    """Pass 3c fields (schema-known, no defaults_registry entry —
    e.g. dashboardEnabled) get the same clobber surfacing."""
    write_override(
        shared_dir, "team_bot_a", "openclaw.plugins.evolve.dashboardEnabled",
        True, set_by="operator", now=fixed_now,
    )
    current = {"botId": "team_bot_a", "dashboardEnabled": False}
    r = materialize_evolve_plugin_config(
        "team_bot_a", _NETWORK, current,
        defaults_registry=_DEFAULTS, shared_dir=shared_dir,
        plugin_manifest=manifest,
    )
    assert r.new_block["dashboardEnabled"] is True
    assert [c.field for c in r.override_clobbered] == ["dashboardEnabled"]
    assert r.override_clobbered[0].live_value is False


def test_multiple_clobbers_sorted_by_field(shared_dir, manifest, fixed_now):
    write_override(
        shared_dir, "team_bot_a", "openclaw.plugins.evolve.tier",
        "off", set_by="auto:drift_x", needs_review=True, now=fixed_now,
    )
    write_override(
        ...
        5, set_by="operator", now=fixed_now,
    )
    current = {"botId": "team_bot_a", "tier": "manage", "summarizerMinTurns": 9}
    r = materialize_evolve_plugin_config(
        "team_bot_a", _NETWORK, current,
        defaults_registry=_DEFAULTS, shared_dir=shared_dir,
        plugin_manifest=manifest,
    )
    assert [c.field for c in r.override_clobbered] == ["summarizerMinTurns", "tier"]
...
# The one value whose STORED form does not say what will be deployed. The
# resolution happens at materialize time so the tier tracks the bot's traffic
# rather than freezing whatever it looked like on the day someone clicked;
# these pin what it resolves to and, more importantly, what it does when it
# cannot decide. Fixtures mirror
# internal/finding-cost-forensics-power-bot-2026-09-04.md §2 — turns ~18
# minutes apart against a 5-minute cache.

#: ``auto`` will not move a tier on less than ``MIN_WINDOW_DAYS_FOR_AUTO``
#: days of OBSERVED history, and the observed span is derived from the records
#: themselves — so every fixture that expects a decision spans real days, one
#: session per day.
_AUTO_FIXTURE_DAYS = 7


def _sparse_turns(
    n: int = 12, gap_minutes: int = 18, days: int = _AUTO_FIXTURE_DAYS,
) -> list[dict]:
    ...
    for d in range(days):
        for i in range(n):
            rows.append({
                "session_id": f"s{d}",
                "ts": (base + timedelta(days=d, minutes=gap_minutes * i)).isoformat(),
...
def _dense_turns(n: int = 30, days: int = _AUTO_FIXTURE_DAYS) -> list[dict]:
    from datetime import timedelta
    base = datetime(2026, 9, 4, 12, 0, tzinfo=timezone.utc)
    rows = []
    for d in range(days):
        for i in range(n):
            rows.append({
                "session_id": f"s{d}",
                "ts": (base + timedelta(days=d, seconds=20 * i)).isoformat(),
                "cache_write_tokens": 45_000 if i == 0 else 0,
                "cache_read_tokens": 0 if i == 0 else 45_000,
            })
    return rows


def _gapped_turns(factor_rows: int, *, gap_minutes: int) -> list[dict]:
    """A week of sessions whose in-session gap is ``gap_minutes``.

    Used to park a bot near the break-even: every gap outlives the 5-minute
    cache, none outlives the 1-hour one, so ``writes_short / writes_long``
    is ``(sessions + gaps) / sessions`` and the caller can dial it.
    """
    from datetime import timedelta
    base = datetime(2026, 9, 4, 12, 0, tzinfo=timezone.utc)
    rows = []
    for d in range(_AUTO_FIXTURE_DAYS):
...
def turns(monkeypatch):
    """Patch the turn reader ``cache_shape`` uses, so the materializer's
    ``auto`` path runs against a fixture instead of the pod's turn files."""
    import cache_shape

    holder: dict[str, object] = {"rows": []}

    def _load(bot_id, **kwargs):
        return holder["rows"]

    monkeypatch.setattr(cache_shape, "load_turn_records", _load)
    return holder


def test_auto_resolves_to_long_on_the_findings_gap_distribution(
    shared_dir, fixed_now, _be_config_importable, turns,
):
    turns["rows"] = _sparse_turns()
    _write_be_config(shared_dir, "team-bot-a", per_bot_cache_retention="auto")
    cfg = _kit_like_catalog_cfg()
    r = materialize_openclaw_policy_slice(cfg, "team-bot-a", shared_dir)

    models = cfg["agents"]["defaults"]["models"]
    assert models["anthropic/claude-sonnet-4-6"]["params"]["cacheRetention"] == "long"
    assert "openclaw.agents.defaults.models.cacheRetention" in r.applied_overrides
    # "auto" itself is never written to openclaw.json — OC would not know it.
    for mid, mcfg in models.items():
        assert (mcfg.get("params") or {}).get("cacheRetention") != "auto", mid


def test_auto_resolves_to_short_on_a_dense_distribution(
    shared_dir, fixed_now, _be_config_importable, turns,
):
    turns["rows"] = _dense_turns()
    _write_be_config(shared_dir, "team-bot-a", per_bot_cache_retention="auto")
    cfg = _kit_like_catalog_cfg()
    materialize_openclaw_policy_slice(cfg, "team-bot-a", shared_dir)
    models = cfg["agents"]["defaults"]["models"]
    assert models["anthropic/claude-sonnet-4-6"]["params"]["cacheRetention"] == "short"


def test_auto_that_cannot_decide_leaves_the_catalog_alone(
    shared_dir, fixed_now, _be_config_importable, turns,
):
    """Too few gaps to size the break-even. The knob must not fall through to
    a premium nothing has shown the bot earns — declining leaves the catalog
    exactly as it was, which is the pod's behavior today."""
    turns["rows"] = _sparse_turns(n=2, days=2)
    _write_be_config(shared_dir, "team-bot-a", per_bot_cache_retention="auto")
    cfg = _kit_like_catalog_cfg()
    before = json.dumps(cfg, sort_keys=True)
    r = materialize_openclaw_policy_slice(cfg, "team-bot-a", shared_dir)
...
def test_auto_with_unreadable_turns_leaves_the_catalog_alone(
    shared_dir, fixed_now, _be_config_importable, monkeypatch,
):
    """A cache tier is not worth failing a deploy over."""
    import cache_shape
    monkeypatch.setattr(
        cache_shape, "load_turn_records", lambda bot_id, **kw: None,
    )
    _write_be_config(shared_dir, "team-bot-a", per_bot_cache_retention="auto")
    cfg = _kit_like_catalog_cfg()
    before = json.dumps(cfg, sort_keys=True)
...
def test_auto_does_not_override_an_explicit_pin(
    shared_dir, fixed_now, _be_config_importable, turns,
):
    """An operator's explicit "short" is not re-argued against data that
    would have chosen "long"."""
    turns["rows"] = _sparse_turns()
    _write_be_config(shared_dir, "team-bot-a", per_bot_cache_retention="short")
    cfg = _kit_like_catalog_cfg()
    materialize_openclaw_policy_slice(cfg, "team-bot-a", shared_dir)
...
def test_unset_is_not_a_hidden_auto(
    shared_dir, fixed_now, _be_config_importable, turns,
):
    """Turning "the operator has not chosen" into a measured choice would
    move every bot's cache tier on the next deploy with nobody asking."""
    turns["rows"] = _sparse_turns()
    _write_be_config(shared_dir, "team-bot-a", per_bot_daily_hard_usd=5.0)
    cfg = _kit_like_catalog_cfg()
    before = json.dumps(cfg, sort_keys=True)
    materialize_openclaw_policy_slice(cfg, "team-bot-a", shared_dir)
    assert json.dumps(cfg, sort_keys=True) == before
...
def _resolved_tier(cfg: dict) -> str | None:
    """The tier the materializer WROTE, or ``None`` if it wrote nothing.

    Read off a model that carries no ``cacheRetention`` in the fixture
    catalog, so "wrote short" is distinguishable from "left it alone" —
    the other Anthropic entries ship ``short`` already.
    """
    entry = cfg["agents"]["defaults"]["models"]["anthropic/claude-opus-4-7"]
    return (entry.get("params") or {}).get("cacheRetention")


def _materialize_once(shared_dir) -> str | None:
    cfg = _kit_like_catalog_cfg()
    materialize_openclaw_policy_slice(cfg, "team-bot-a", shared_dir)
    return _resolved_tier(cfg)


def _breakeven_band_turns(*, above: bool) -> list[dict]:
    """A week whose re-write factor sits just inside the 15% margin band.

    ``writes_short / writes_long`` is ``1 + short_gaps / (sessions +
    long_gaps)``: a gap that outlives the 5-minute cache but not the 1-hour
    one is a write the longer tier avoids, and one that outlives both is a
    write either way. Ten sessions with six 90-minute gaps each and five or
    four 18-minute gaps puts the ratio at 1.71x or 1.57x — the finding's
    oscillation, both readings inside the band around the ~1.65x break-even.
    """
    from datetime import timedelta
    base = datetime(2026, 9, 4, 12, 0, tzinfo=timezone.utc)
    short_gaps = 5 if above else 4
    rows = []
    for d in range(10):
        at = base + timedelta(days=d)
        rows.append({"session_id": f"b{d}", "ts": at.isoformat(),
                     "cache_write_tokens": 45_000, "cache_read_tokens": 0})
        for _ in range(short_gaps):
            at += timedelta(minutes=18)
            rows.append({"session_id": f"b{d}", "ts": at.isoformat(),
                         "cache_write_tokens": 45_000, "cache_read_tokens": 0})
        for _ in range(6):
...
def test_the_oscillation_fixtures_straddle_the_breakeven_inside_the_margin():
    """The premise of the dwell tests, asserted rather than assumed."""
    import cache_shape

    above = cache_shape.gap_stats(_breakeven_band_turns(above=True))
    below = cache_shape.gap_stats(_breakeven_band_turns(above=False))
    assert above.rewrite_factor == pytest.approx(1.714, abs=5e-3)
    assert below.rewrite_factor == pytest.approx(1.571, abs=5e-3)
    assert below.rewrite_factor < cache_shape.LONG_RETENTION_BREAKEVEN
    assert above.rewrite_factor > cache_shape.LONG_RETENTION_BREAKEVEN
    assert above.has_economics and below.has_economics
    assert min(above.window_days, below.window_days) >= (
        cache_shape.MIN_WINDOW_DAYS_FOR_AUTO
    )
    # Neither clears the 15% margin, so neither may overturn the other.
    assert not cache_shape.clears_flip_margin("long", above)
    assert not cache_shape.clears_flip_margin("short", below)


def test_an_oscillation_across_the_breakeven_writes_one_tier_not_three(
    shared_dir, fixed_now, _be_config_importable, turns,
):
    """Finding 4: ``auto`` re-resolves on EVERY materialize and used to write
    whatever came out, so a bot parked at the threshold wrote long, short,
    long across three deploys — 1.71x one week, 1.57x the next as a weekend
    rolled out of the 7-day window. Every flip invalidates the live cache and
    re-primes it at full price: the cost the tier exists to avoid, paid for a
    decision the operator never made and only saw in an ``_log.info`` line.
    """
    import better_engine_config as bec

    _write_be_config(shared_dir, "team-bot-a", per_bot_cache_retention="auto")
    seen = []
    for above in (True, False, True):
        turns["rows"] = _breakeven_band_turns(above=above)
        seen.append(_materialize_once(shared_dir))
    assert seen == ["long", "long", "long"], (
        "three deploys, one tier — every flip is a full-price prefix rewrite"
    )
    value, stamp = bec.load(shared_dir).per_bot_cache_retention_decision(
        "team-bot-a",
    )
    assert value == "long" and stamp


def test_a_reading_that_clears_the_margin_does_flip_the_tier(
    shared_dir, fixed_now, _be_config_importable, turns, caplog,
):
    """Dwell damps noise, it does not freeze the tier. A bot whose traffic
    genuinely changed clears the margin and moves — loudly."""
    import logging

    _write_be_config(shared_dir, "team-bot-a", per_bot_cache_retention="auto")
    turns["rows"] = _breakeven_band_turns(above=True)
    assert _materialize_once(shared_dir) == "long"

    # Turns 90 minutes apart throughout: the 1-hour cache expires on every
    # one of them, so it buys nothing (1.0x) — far outside the margin band.
    turns["rows"] = _sparse_turns(n=6, gap_minutes=90)
    with caplog.at_level(logging.WARNING):
        assert _materialize_once(shared_dir) == "short"
    assert any(
        "FLIPPED" in r.message or "FLIPPED" in r.getMessage()
        for r in caplog.records
    ), "a flip is a warning, not an info line an operator has to go looking for"


def test_a_decline_records_no_standing_decision(
    shared_dir, fixed_now, _be_config_importable, turns,
):
    """"auto could not size this" is not a decision to start a dwell clock on."""
    import better_engine_config as bec

    turns["rows"] = _sparse_turns(n=2, days=2)
    _write_be_config(shared_dir, "team-bot-a", per_bot_cache_retention="auto")
    assert _materialize_once(shared_dir) is None
    assert bec.load(shared_dir).per_bot_cache_retention_decision("team-bot-a") == (
        None, None,
    )


def test_a_week_old_decision_may_be_overturned_without_the_margin(
    shared_dir, fixed_now, _be_config_importable, turns,
):
    """The margin damps noise; it must not freeze a bot on last month's
    pattern. A standing decision older than the dwell window yields to a
    reading that would otherwise be held."""
    import better_engine_config as bec

    _write_be_config(shared_dir, "team-bot-a", per_bot_cache_retention="auto")
    cfg = bec.load(shared_dir)
    cfg.set_per_bot_cache_retention_decision(
        "team-bot-a", "short", "2026-01-01T00:00:00Z",
    )
    bec.save(cfg, shared_dir)

    turns["rows"] = _breakeven_band_turns(above=True)   # inside the margin
    assert _materialize_once(shared_dir) == "long"
