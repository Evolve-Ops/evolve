...
# ─────────────────────────────────────────────────────────────────────────────
# Call-site guard — shared_dir is not optional in practice
# ─────────────────────────────────────────────────────────────────────────────


class TestCallSitesPassSharedDir:
    """``shared_dir`` is keyword-optional on ``enforce_trip`` only for the
    L2 callers that genuinely do not need it — omitting it on a ``cost``
    breaker makes enforcement return ``ok=False`` with "shared_dir
    required for cost enforcement" and do NOTHING: no heartbeat stash, no
    exec-approval passthrough, no user-channel message. The breaker file
    is written and the operator is told the trip fired.

    Four production call sites had drifted into that state (the two web
    routes, the two evo in-process fallbacks) plus the breakers runner,
    while the CLI and spend_alert paths passed it correctly. Nothing in
    the type system catches it, so this walks the AST instead.
    """

    @staticmethod
    def _call_sites(func_name: str) -> list[str]:
        import ast

        pkgs = Path(__file__).parent.parent.parent  # packages/
        out: list[str] = []
        for path in sorted(pkgs.rglob("*.py")):
            rel = path.relative_to(pkgs)
            if "tests" in rel.parts or rel.parts[-1].startswith("test_"):
                continue
            try:
            ...
            for node in ast.walk(tree):
                ...
                # The breakers runner calls through an injected seam
                # (``enforce_trip_fn``); the omission is just as invisible
                # there, so match the seam names too.
                if name not in (func_name, f"{func_name}_fn"):
                    continue
                kwargs = {k.arg for k in node.keywords}
                # ``**kwargs`` forwarding (arg is None) passes it through.
                if "shared_dir" in kwargs or None in kwargs:
                    continue
                out.append(f"{rel}:{node.lineno}")
        return out

    def test_every_production_enforce_trip_call_passes_shared_dir(self) -> None:
        offenders = self._call_sites("enforce_trip")
        assert offenders == [], (
            "enforce_trip called without shared_dir — cost enforcement "
            "silently no-ops at these sites:\n  " + "\n  ".join(offenders)
        )

    def test_the_guard_can_actually_see_a_missing_shared_dir(self) -> None:
        """Guard the guard: the AST walk must be finding call sites at
        all, or the assertion above passes vacuously forever."""
        import ast

        pkgs = Path(__file__).parent.parent.parent
        found = 0
        for path in sorted(pkgs.rglob("*.py")):
            rel = path.relative_to(pkgs)
            if "tests" in rel.parts or rel.parts[-1].startswith("test_"):
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Call) and name in (
                    "enforce_trip", "enforce_trip_fn",
                ):
                    found += 1
        assert found >= 5, f"expected several enforce_trip call sites, saw {found}"
