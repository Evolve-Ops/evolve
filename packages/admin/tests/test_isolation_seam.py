...
# ── list_accounts: host account enumeration (orphaned-account monitor) ──────


def test_list_accounts_filters_system_and_underscore_records(monkeypatch):
    """macOS `getpwall()` yields `_`-prefixed service records and duplicate
    root/nobody rows; Linux yields the FHS system users. Neither is a login
    account a caller should have to filter itself."""
    import pwd as _pwd

    class _PW:
        def __init__(self, name, uid, dir_):
            self.pw_name, self.pw_uid, self.pw_dir = name, uid, dir_

    monkeypatch.setattr(_pwd, "getpwall", lambda: [
        _PW("root", 0, "/var/root"),
        _PW("root", 0, "/var/root"),          # macOS yields this twice
        _PW("_assetcache", 200, "/var/empty"),
        _PW("nobody", 4294967294, "/var/empty"),
        _PW("team_bot_a", 502, "/Users/team_bot_a"),
        _PW("pod_operator", 501, "/Users/pod_operator"),
    ])

    accounts = MacOSIsolation().list_accounts()

    assert [a.user for a in accounts] == ["pod_operator", "team_bot_a"]  # uid order
    assert accounts[1].uid == 502
    assert str(accounts[1].home) == "/Users/team_bot_a"


def test_list_accounts_raises_rather_than_reporting_an_empty_host(monkeypatch):
    """An empty enumeration is never true on a live host — it means the
    directory service is not answering. A sweep-style caller that took it at
    face value would conclude 'no accounts exist' and auto-resolve every
    finding, so this must fail loudly instead."""
    import pwd as _pwd

    monkeypatch.setattr(_pwd, "getpwall", lambda: [])
    with pytest.raises(IsolationError, match="not answering"):
        MacOSIsolation().list_accounts()


def test_list_accounts_raises_when_the_directory_service_errors(monkeypatch):
    import pwd as _pwd

    def _boom():
        raise OSError("NSS unavailable")
    monkeypatch.setattr(_pwd, "getpwall", _boom)
    with pytest.raises(IsolationError):
        MacOSIsolation().list_accounts()


def test_fake_isolation_list_accounts_round_trip():
    fake = FakeIsolation()
    fake.seed_user("team_bot_a", 502)
    fake.seed_user("shared_acct", 503, home="/Users/shared_acct")

    accounts = fake.list_accounts()
    assert [a.user for a in accounts] == ["team_bot_a", "shared_acct"]
    assert accounts[0].uid == 502


def test_fake_isolation_list_accounts_mirrors_the_fail_loud_contract():
    """An unseeded Fake must exercise the caller's BLIND path, not silently
    assert against an empty sweep."""
    with pytest.raises(IsolationError):
        FakeIsolation().list_accounts()


def test_both_real_adapters_share_the_posix_enumeration(monkeypatch):
    """macOS and Linux use the identical `pwd.getpwall()` walk — there is
    nothing platform-specific to branch on, so no dscl/getent subprocess and
    no sudoers grant."""
    import pwd as _pwd
    ...
    monkeypatch.setattr(_pwd, "getpwall", lambda: [_PW("team_bot_a", 502, "/x")])

    def _explode(*a, **kw):  # any subprocess here is a bug
        raise AssertionError(f"unexpected subprocess: {a}")

    assert MacOSIsolation(runner=_explode).list_accounts() == (
        LinuxUserIsolation(runner=_explode).list_accounts()
    )
