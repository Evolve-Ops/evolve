...
# ── D-BI3: typed enrichment fields ──────────────────────────────────────────

def test_typed_enrichment_fields_round_trip(tmp_path: Path):
    card = bs.create_card(
        tmp_path, "b", title="Team sync", cluster="work", source="calendar",
        enrichment={
            "contacts": {
                "value": [{"name": "Alex", "phone": "+15551234567",
                           "email": "alex@example.com"}],
                "source": "calendar",
            },
            "location": {
                "value": {"text": "12 Main St", "maps_url": "https://maps.example/x"},
                "source": "calendar",
            },
            "when": {"value": {"start": "2026-09-20T09:00:00Z",
                               "end": "2026-09-20T09:30:00Z"}, "source": "calendar"},
            "links": {"value": [{"label": "Doc", "url": "https://docs.example/x"}],
                      "source": "calendar"},
        })
    e = card["enrichment"]
    assert e["contacts"]["value"][0]["name"] == "Alex"
    assert e["location"]["value"]["text"] == "12 Main St"
    assert e["when"]["value"]["start"] == "2026-09-20T09:00:00Z"
    assert e["links"]["value"][0]["url"] == "https://docs.example/x"


def test_when_still_accepts_a_bare_string_from_chip_1(tmp_path: Path):
    # board-tool-chat-parity-and-enrichment shipped `when` as a plain ISO
    # string; D-BI3's {start, end?} is additive, not a replacement.
    card = bs.create_card(
        tmp_path, "b", title="x", cluster="work",
        enrichment={"when": {"value": "2026-09-20T09:00:00Z", "source": "calendar"}})
    assert card["enrichment"]["when"]["value"] == "2026-09-20T09:00:00Z"


@pytest.mark.parametrize("field,value", [
    ("contacts", "not-a-list"),
    ("contacts", [{"phone": "555"}]),  # missing name
    ("contacts", [{"name": "A", "unexpected": 1}]),
    ("location", "not-an-object"),
    ("location", {"maps_url": "https://x"}),  # missing text
    ("location", {"text": "x", "unexpected": 1}),
    ("when", 123),
    ("when", {"end": "2026-09-20T09:30:00Z"}),  # missing start
    ("links", "not-a-list"),
    ("links", [{"label": "no url"}]),
    ("links", [{"url": "https://x"}]),  # missing label
])
def test_typed_enrichment_rejects_malformed_shapes(tmp_path: Path, field, value):
    board = bs.load_board(tmp_path, "b")
    with pytest.raises(ValueError):
        bs.add_card(board, title="x", cluster="work",
                    enrichment={field: {"value": value, "source": "calendar"}})
    # Refused BEFORE the card is appended — the board is exactly as it was.
    assert board["cards"] == []


def test_contacts_and_links_are_capped(tmp_path: Path):
    board = bs.load_board(tmp_path, "b")
    with pytest.raises(ValueError):
        bs.add_card(board, title="x", cluster="work", enrichment={
            "contacts": {"value": [{"name": f"n{i}"} for i in range(bs.MAX_CONTACTS + 1)],
                        "source": "calendar"}})
    with pytest.raises(ValueError):
        bs.add_card(board, title="x", cluster="work", enrichment={
            "links": {"value": [{"label": "l", "url": "https://x"}
                                for _ in range(bs.MAX_LINKS + 1)],
                     "source": "calendar"}})


# ── D-BI5: actions[] ─────────────────────────────────────────────────────────

def test_new_card_gets_a_starter_actions_list_capped_at_three(tmp_path: Path):
    card = bs.create_card(tmp_path, "b", title="Buy paint", cluster="home")
    assert 1 <= len(card["actions"]) <= 3
    assert all({"id", "label", "kind"} <= set(a) for a in card["actions"])


def test_calendar_and_email_sources_get_their_own_vocabulary(tmp_path: Path):
    cal = bs.create_card(tmp_path, "b", title="Standup", cluster="work",
                         source="calendar")
    email = bs.create_card(tmp_path, "b", title="Re: invoice", cluster="admin",
                           source="email")
    cal_ids = {a["id"] for a in cal["actions"]}
    email_ids = {a["id"] for a in email["actions"]}
    assert "prepare_dossier" in cal_ids
    assert "draft_reply" in email_ids
    # D-BI6: an email-derived card never carries a send/book/pay action, and
    # its one action with an email-derived target needs confirmation.
    assert all(a["kind"] == "llm" for a in email["actions"])
    reply = next(a for a in email["actions"] if a["id"] == "draft_reply")
    assert reply.get("requires_confirm") is True


def test_a_cluster_with_no_dedicated_vocabulary_falls_back_to_admin(tmp_path: Path):
    card = bs.create_card(tmp_path, "b", title="x", cluster="not-a-real-cluster")
    assert card["actions"]


# ── D-BI5: a tap is an instruction ──────────────────────────────────────────

def test_instruct_offers_the_card_to_the_bot_and_marks_it_queued(tmp_path: Path):
    card_id = _one_card(tmp_path, cluster="home")
    card = bs.instruct_card(tmp_path, "b", card_id, action_id="find_directions",
                            action_label="Find directions", est_cost=0.0, actor="user")
    assert card["owner"] == "bot"
    assert card["delegation"]["state"] == "offered"
    assert card["instructed"] == {
        "action_id": "find_directions", "label": "Find directions",
        "est_cost": 0.0, "ts": card["instructed"]["ts"]}
    events = [e for e in _events(tmp_path) if e["event"] == "instruction"]
    assert len(events) == 1
    assert events[0]["action_id"] == "find_directions"
    assert events[0]["actor"] == "user"


def test_instruct_on_a_bot_owned_card_does_not_re_offer(tmp_path: Path):
    card_id = _one_card(tmp_path, cluster="home", owner="bot")
    before = bs.find_card(bs.load_board(tmp_path, "b"), card_id)["delegation"]["updated_at"]
    bs.instruct_card(tmp_path, "b", card_id, action_id="find_directions",
                     action_label="Find directions", est_cost=0.0, actor="user")
    after = bs.find_card(bs.load_board(tmp_path, "b"), card_id)["delegation"]["updated_at"]
    assert before == after
    assigned_events = [e for e in _events(tmp_path) if e["event"] == "assigned"]
    assert assigned_events == []  # no re-assignment event fired


def test_taking_a_card_back_clears_the_instructed_marker(tmp_path: Path):
    card_id = _one_card(tmp_path, cluster="home")
    bs.instruct_card(tmp_path, "b", card_id, action_id="find_directions",
                     action_label="Find directions", est_cost=0.0, actor="user")
    card = bs.assign_card(tmp_path, "b", card_id, "me", actor="user")
    assert "instructed" not in card
    assert "delegation" not in card


def test_instruction_none_logs_without_touching_the_card(tmp_path: Path):
    card_id = _one_card(tmp_path, cluster="home")
    before = bs.find_card(bs.load_board(tmp_path, "b"), card_id)
    bs.record_instruction_none(tmp_path, "b", card_id, actor="user")
    after = bs.find_card(bs.load_board(tmp_path, "b"), card_id)
    assert before == after
    events = [e for e in _events(tmp_path) if e["event"] == "instruction_none"]
    assert len(events) == 1


def test_instruct_unknown_card_raises_keyerror(tmp_path: Path):
    with pytest.raises(KeyError):
        bs.instruct_card(tmp_path, "b", "nope", action_id="x", action_label="x",
                         est_cost=0.0, actor="user")


# ── zero model calls in the store layer ─────────────────────────────────────

def test_board_store_and_board_actions_import_no_model_client(tmp_path: Path):
    # D-BI3 build note: "Deterministic first ... a model call only for a
    # field nothing deterministic can fill." The schema/vocabulary layer
    # itself must never be able to reach a model — a fixture built the way a
    # calendar/directory lookup would build it (deterministically, by hand
    # here) round-trips with no LLM-shaped import anywhere in this module.
    import inspect
    for mod in (bs, board_actions):
        src = inspect.getsource(mod)
        for banned in ("anthropic", "openai", "ChatCompletion", "llm_client"):
            assert banned not in src, f"{mod.__name__} references {banned!r}"
    card = bs.create_card(
        tmp_path, "b", title="Dentist", cluster="health", source="calendar",
        enrichment={
            "when": {"value": {"start": "2026-09-21T14:00:00Z"}, "source": "calendar"},
            "location": {"value": {"text": "123 Elm St"}, "source": "calendar"},
            "contacts": {"value": [{"name": "Dr. Lee", "phone": "+15550000000"}],
                        "source": "directory"},
        })
    assert card["enrichment"]["contacts"]["value"][0]["name"] == "Dr. Lee"


# ── D-BI4 §4/§5: result-on-progress, approval-request/decision writers ─────

def _bot_owned_card(tmp_path: Path, **kw) -> str:
    card_id = _one_card(tmp_path, **kw)
    bs.assign_card(tmp_path, "b", card_id, "bot", actor="user")
    return card_id


def test_set_delegation_progress_result_lands_on_a_terminal_transition(tmp_path: Path):
    card_id = _bot_owned_card(tmp_path)
    card = bs.set_delegation_progress(
        tmp_path, "b", card_id, state="done", cost_to_date=0.001,
        result={"text": "A draft reply.", "cost": 0.001}, actor="bot")
    assert card["delegation"]["result"] == {"text": "A draft reply.", "cost": 0.001}


def test_set_delegation_progress_rejects_an_overlong_result_text(tmp_path: Path):
    card_id = _bot_owned_card(tmp_path)
    with pytest.raises(ValueError):
        bs.set_delegation_progress(
            tmp_path, "b", card_id, state="done",
            result={"text": "x" * 10_000}, actor="bot")


def test_record_approval_request_and_decision_lifecycle(tmp_path: Path):
    card_id = _bot_owned_card(tmp_path)
    card = bs.record_approval_request(
        tmp_path, "b", card_id, act="book", payload_summary="Dear Sam...",
        est_cost=0.002, actor="bot")
    assert card["pending_approval"]["act"] == "book"

    decided = bs.record_approval_decision(tmp_path, "b", card_id, granted=True, actor="user")
    assert "pending_approval" not in decided
...
def test_record_approval_decision_declined(tmp_path: Path):
    card_id = _bot_owned_card(tmp_path)
    bs.record_approval_request(
        tmp_path, "b", card_id, act="book", payload_summary="x", est_cost=None, actor="bot")
    bs.record_approval_decision(tmp_path, "b", card_id, granted=False, actor="user")
    events = [e["event"] for e in _events(tmp_path)]
    assert "approval_declined" in events

# ── Morning Board briefing panel (D-BI6d/e) ────────────────────────────────


def test_save_and_load_briefing_round_trips(tmp_path: Path):
    assert bs.load_briefing(tmp_path, "bot-a") is None
    briefing = bs.save_briefing(tmp_path, "bot-a", "Good morning.",
                                date="2026-09-12")
    assert briefing["date"] == "2026-09-12"
    assert briefing["dismissed_at"] is None
    loaded = bs.load_briefing(tmp_path, "bot-a")
    assert loaded["text"] == "Good morning."


def test_dismiss_marks_the_same_day_and_rejects_a_stale_one(tmp_path: Path):
    bs.save_briefing(tmp_path, "bot-a", "Hi", date="2026-09-12")
    dismissed = bs.dismiss_briefing(tmp_path, "bot-a", date="2026-09-12")
    assert dismissed["dismissed_at"]
    with pytest.raises(KeyError):
        bs.dismiss_briefing(tmp_path, "bot-a", date="2026-09-11")


def test_a_resend_on_the_same_day_keeps_the_dismissal(tmp_path: Path):
    bs.save_briefing(tmp_path, "bot-a", "Hi", date="2026-09-12")
    bs.dismiss_briefing(tmp_path, "bot-a", date="2026-09-12")
    resent = bs.save_briefing(tmp_path, "bot-a", "Hi (forced re-run)",
                              date="2026-09-12")
    assert resent["dismissed_at"] is not None


def test_a_new_day_starts_undismissed(tmp_path: Path):
    bs.save_briefing(tmp_path, "bot-a", "Hi", date="2026-09-12")
    bs.dismiss_briefing(tmp_path, "bot-a", date="2026-09-12")
    tomorrow = bs.save_briefing(tmp_path, "bot-a", "Hi again",
                                date="2026-09-13")
    assert tomorrow["dismissed_at"] is None


def test_briefing_text_is_bounded(tmp_path: Path):
    with pytest.raises(ValueError):
        bs.save_briefing(tmp_path, "bot-a", "x" * (bs.MAX_NOTE_CHARS + 1))


# ── D-MB6 legacy migration: hand-planted board-events.jsonl ────────────────


def test_legacy_events_settle_matching_cards_by_title(tmp_path: Path):
    bs.import_tasks_into_board(tmp_path, "bot-a", _TASKS_MD)
    events = "\n".join([
        json.dumps({"title": "Crown replacement", "lane": "done"}),
        json.dumps({"title": "Backup system audit", "dropped": True}),
    ])
    settled = bs.import_legacy_board_events(tmp_path, "bot-a", events)
    assert settled == 2
    board = bs.load_board(tmp_path, "bot-a")
    by_title = {c["title"]: c["lane"] for c in board["cards"]}
    assert by_title["Crown replacement"] == "done"
    assert by_title["Backup system audit"] == "dropped"


def test_legacy_events_are_idempotent_and_skip_the_unmatched(tmp_path: Path):
    bs.import_tasks_into_board(tmp_path, "bot-a", _TASKS_MD)
    events = "\n".join([
        json.dumps({"title": "Crown replacement", "lane": "done"}),
        json.dumps({"title": "No such card", "lane": "done"}),
        "not json at all",
        json.dumps(["not", "an", "object"]),
        json.dumps({"title": "Crown replacement"}),  # no disposition
    ])
    assert bs.import_legacy_board_events(tmp_path, "bot-a", events) == 1
    # Re-running the same file a second time settles nothing further.
    assert bs.import_legacy_board_events(tmp_path, "bot-a", events) == 0


def test_legacy_events_never_resurrect_a_live_card_into_the_wrong_lane(
    tmp_path: Path,
):
    card = bs.create_card(tmp_path, "bot-a", title="Live one", cluster="home")
    bs.move_card(tmp_path, "bot-a", card["id"], "today", actor="user")
    events = json.dumps({"title": "Live one", "done": True})
    assert bs.import_legacy_board_events(tmp_path, "bot-a", events) == 1
    board = bs.load_board(tmp_path, "bot-a")
    assert board["cards"][0]["lane"] == "done"


def test_disable_legacy_cron_label_and_shape(tmp_path: Path):
    assert bs.default_legacy_cron_label("bot-a") == "ai.evolve.bot-a.morning-board"


def test_disable_legacy_cron_reports_already_gone(monkeypatch):
    import subprocess
    ...
    ok, message = bs.disable_legacy_cron("ai.evolve.bot-a.morning-board")
    assert ok is False
