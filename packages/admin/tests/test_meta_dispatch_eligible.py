...
def test_a_read_error_is_tap_only_when_the_card_is_untapped(monkeypatch):
    """FAIL TOWARD THE CARD, never toward a duplicate: whatever goes wrong inside
    `card_state`, the card is never dropped and never an exception — but the fallback
    is `prepared`/`tap` ONLY for an untapped entry. An in-motion entry (or one whose
    in-motion-ness itself cannot be read) is `unknown`/`investigate`, the same invariant
    `test_stale_started_is_investigate_not_tap` pins for the non-error path."""
    def boom(*a, **k):
        raise RuntimeError("boom")
    monkeypatch.setattr(mde, "_precise_age_hours", boom)

    untapped = {"id": "x", "dispatched_at": "2026-09-07T18:00:00Z"}
    assert mde.card_state(untapped, NOW, None) == (
        mde.CARD_STATE_PREPARED, None, mde.CARD_ACTION_TAP)

    moving = {"id": "y", "launch": "started", "branch": "claude/y",
              "dispatched_at": "2026-09-07T18:00:00Z"}
    assert mde.card_state(moving, NOW, None) == (
