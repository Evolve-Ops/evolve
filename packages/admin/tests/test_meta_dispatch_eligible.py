...
def brief(root, state, ident, *, aspect="substrate", title=None, privileged=False,
          ...
          pr=None, launch=None, branch=None, touches=None,
...
def test_a_claimed_queued_brief_is_held_not_dispatched(tmp_path):
    """D-TP5: `tools/meta-dispatch-move start` wrote a `branch:` onto this still-queued
    brief — a chip or an app session already claimed it. The dispatcher must not also
    prepare a card or launch a session for it (the #4266/#4267 shape)."""
    root = lane(tmp_path)
    brief(root, "queued", "claimed-chip", branch="claude/someone-already-on-it")
    r = mde.evaluate(root)
    assert r["next"] is None and r["eligible"] == []
    (held,) = r["held"]
    assert held["id"] == "claimed-chip" and held["reason"] == "claimed"
    assert "claude/someone-already-on-it" in held["detail"]


def test_a_queued_brief_bound_to_a_pr_is_held_and_does_not_ride_alone(tmp_path):
    """2026-09-23: a hand-launched chip's PR was bound to its brief while the brief
    stayed in `queued/`. The helper offered it as the sole candidate every tick and,
    with no `touches:`, it rode alone and held every other brief behind it. A `pr:` on
    a queued brief is a claim like `branch:` — held, and the queue flows past it."""
    root = lane(tmp_path)
    brief(root, "queued", "hand-launched", pr=4424, created="2026-08-15")
    brief(root, "queued", "next-in-line", created="2026-08-16",
          touches=["packages/admin/evolve_admin/x.py"])
    r = mde.evaluate(root)
    held = {h["id"]: h for h in r["held"]}
    assert held["hand-launched"]["reason"] == "claimed"
    assert "4424" in held["hand-launched"]["detail"]
    assert [e["id"] for e in r["eligible"]] == ["next-in-line"]
...
def prepared(root, ident, *, touches=None):
    """A card the dispatcher prepared and nobody has clicked: no branch, no pr."""
    brief(root, "inflight", ident, launch="prepared", touches=touches)
...
def test_the_cap_is_six_and_it_counts_work_in_motion(tmp_path):
    ...
    assert r["prepared_count"] == 0 and r["prepared_slots"] == 4
...
def test_three_started_plus_one_prepared_still_allows_one_more_launch(tmp_path):
    ...
    assert r["slots"] == 3 and r["prepared_count"] == 1 and r["prepared_slots"] == 3
    assert r["blocked_by"] is None
    assert r["next"]["id"] == "a-first"
    assert [e["id"] for e in r["eligible"]] == ["a-first", "b-second"]


def test_four_prepared_cards_block_dispatch_even_with_free_build_slots(tmp_path):
    """The prepared cap is a SEPARATE budget, and it binds first here: 2/6 in motion says
    the lane could build more, but the dispatcher only ever prepares cards, and four are
    already waiting on the operator's click. Four, not two: D-PM13 raised the prepared cap
    because a tick now prepares up to three cards, and a tray of two would have been full
    before the first tick finished."""
    root = lane(tmp_path)
    for i in range(2):
        started(root, "busy-%d" % i, pr=100 + i)
    for name in ("a", "b", "c", "d"):
        prepared(root, "unclicked-%s" % name)
    brief(root, "queued", "waiting")
    r = mde.evaluate(root)
    assert r["in_motion_count"] == 2 and r["slots"] == 4        # in motion 2/6: room
    assert r["prepared_count"] == 4 and r["prepared_slots"] == 0  # prepared 4/4: full
    assert r["blocked_by"] == "prepared-cap"
    assert r["next"] is None and r["dispatchable"] is False
    assert r["candidates"] == []
    assert [d["blocked_by"] for d in r["deferred"]] == ["prepared-cap"]
    assert "prepared and unclicked (prepared cap 4)" in mde.render_text(r)
...
def test_the_cap_outranks_the_prepared_cap_when_both_are_full(tmp_path):
    ...
    for i in range(6):
        started(root, "busy-%d" % i, pr=100 + i)
    for name in ("a", "b", "c", "d"):          # fill the tray to the prepared cap
        prepared(root, "unclicked-%s" % name)
    brief(root, "queued", "waiting")
...
def test_the_summary_line_states_both_counts_both_caps_and_the_holds(tmp_path):
    ...
    assert lines[1] == ("  in flight: 2/6 · prepared: 1/4 · slots: 4 in-motion, "
                        "3 prepared · 1 held: #3976 · headless: unavailable "
                        "· base: unknown")
...
def test_the_summary_line_omits_the_held_clause_when_nothing_is_held(tmp_path):
    ...
    started(root, "chip-a", pr=3976)
    lines = mde.render_text(mde.evaluate(root)).splitlines()
    assert lines[1] == ("  in flight: 1/6 · prepared: 0/4 · slots: 5 in-motion, "
                        "4 prepared · headless: unavailable · base: unknown")
...
def test_the_prepared_cap_default_has_one_definition(tmp_path):
    ...
    assert mde.DEFAULT_PREPARED_CAP == mq.PM_PREPARED_CAP == 4
...
def test_two_entries_on_one_pr_cost_one_slot_and_the_summary_names_the_sharing_pr(tmp_path):
    ...
    assert line == ("  in flight: 1/6 · prepared: 0/4 · slots: 5 in-motion, 4 prepared "
...
def test_the_prepared_cap_still_blocks_with_the_corrected_count(tmp_path):
    ...
    assert r["prepared_count"] == 4 and r["prepared_slots"] == 0
...
def test_cli_json_and_exit_codes(tmp_path, capsys):
    ...
    assert (out["next"]["id"] == "chip-a" and out["cap"] == 6
            and out["prepared_cap"] == 4 and out["per_tick_cap"] == 3)
...
def test_a_front_matter_write_after_stamping_leaves_the_entry_eligible(tmp_path):
    ...
    for k, v in (("dispatched", "2026-08-27"), ("session", "task_x")):
        text = mde.mdi.set_front_matter_field(text, k, v)
    p.write_text(text, encoding="utf-8")
    assert mde.evaluate(root)["next"]["id"] == "still-fine"
    # A `pr:` write is ALSO integrity-neutral — but a queued brief carrying a PR is a
    # claim (see test_a_queued_brief_bound_to_a_pr_is_held_and_does_not_ride_alone), so
    # it reads as held-claimed, never as invalid.
    text = mde.mdi.set_front_matter_field(text, "pr", 4001)
    p.write_text(text, encoding="utf-8")
    r = mde.evaluate(root)
    assert r["invalid"] == [] and r["held"][0]["reason"] == "claimed"
...
def test_lane_state_pr_refuses_a_stale_snapshot_that_resurrects_a_done_id(tmp_path):
    """#4271, reproduced exactly (`internal/finding-...` and
    `internal/dispatch/queued/lane-state-pr-asserts-the-transition-it-came-from.md`).

    A `lane/state` branch is cut while `chip-r` sits in `inflight/`. Before it merges, a
    SEPARATE PR — the chip's own — carries `chip-r` to `done/` on `origin/main`. The
    stale branch still adds `inflight/chip-r.md`, and that resurrected copy's body is
    BYTE-IDENTICAL to the one now in `done/` — which is exactly what let the #4271 shape
    through: conjunct 4 (the body survives) can only see that the body matches
    SOMEWHERE, not that it went in the wrong direction. This must fail even though the
    body check alone would pass it."""
    work, root, _ = _lane_repo(tmp_path)
    head = _land_like(work, root, lambda r: (r / "inflight" / "chip-r.md").write_text(
        _entry("chip-r", extra="launch: prepared\n")))

    # The chip's own PR merges AFTER the branch above was cut — main never carried
    # chip-r in inflight/ at all; the dispatch position was local-only until the now-
    # stale branch tried to commit it.
    (root / "done" / "chip-r.md").write_text(_entry("chip-r", extra="pr: 4280\n"))
    _commit_push(work, "chip-r's own PR carries it to done/")

    v = mde.lane_state_pr(root, 4100, _lane_pr_json(
        head, paths=["internal/dispatch/inflight/chip-r.md"]))
    assert v["qualifies"] is False, v["reason"]
    assert v["reason"].startswith("resurrected: ")
    # Names the id, both paths, and the remedy — never a bare refusal.
    assert "chip-r" in v["reason"]
    assert "internal/dispatch/inflight/chip-r.md" in v["reason"]
    assert "internal/dispatch/done/chip-r.md" in v["reason"]
    assert "drop-stale-marker" in v["reason"]
...
def test_waiting_line_is_reported_on_a_blocked_tick_too(tmp_path):
    ...
    root = lane(tmp_path)
    # Four cards, because D-PM13 raised the prepared cap from 2 to 4 — the state this
    # test is about is "the tray is full", and that is now four.
    for i, name in enumerate(("a", "b", "c", "d")):
        _card(root, "waiting-%s" % name,
              at="2026-09-07T%02d:00:00Z" % (10 + i), session="task_%s" % name)
    brief(root, "queued", "ready-work")

    r = mde.evaluate(root)

    assert r["blocked_by"] == "prepared-cap" and r["next"] is None
    assert r["cards_waiting_line"].startswith("cards waiting for a tap: 4 — ")
...
def test_next_carries_a_chip_title_with_the_local_time(tmp_path):
    ...
    assert re.fullmatch(r"\[META:substrate\] \d{2}:\d{2} \[ready-work\] Widen the drift "
                        r"check", timed)
    assert timed == ("[META:substrate] %s [ready-work] Widen the drift check"
                     % r["now_local"])
...
def test_card_title_carries_the_lane_id(tmp_path):
    """D-CD1: the lane id sits in square brackets immediately after the time, verbatim
    — the fix for "confusion around which cards have been started and which haven't"
    (operator, 2026-09-11). The id must appear BEFORE the title text, and untouched."""
    root = lane(tmp_path)
    brief(root, "queued", "widen-the-drift-check", aspect="substrate",
          title="Widen the drift check")

    r = mde.evaluate(root)
    timed = r["next"]["chip_title_timed"]

    assert "[widen-the-drift-check]" in timed
    assert timed.index("[widen-the-drift-check]") < timed.index("Widen the drift check")
    assert timed == mde.compose_chip_title_timed(
        "substrate", r["now_local"], "widen-the-drift-check", "Widen the drift check")
...
def test_a_full_tray_no_longer_freezes_a_non_privileged_brief(tmp_path, monkeypatch):
    ...
    root = lane(tmp_path)
    for name in ("one", "two", "three", "four"):   # fill the tray to the prepared cap
        prepared(root, "card-%s" % name)
...
# Corroboration stubs for the launcher breaker. The real `_stall_corroboration` reads a git
# checkout and a chip worktree; a lane fixture is neither, so every fixture would otherwise
# land on "could not look" and only one of the two directions could be pinned.
def _git_repo(path):
    """A real (empty) git repo. The corroborator asks git whether a chip ref exists, and
    "no such ref" must be distinguishable from "git could not answer" — which can only be
    exercised against a real repo."""
    import subprocess
...
NOTHING_FOUND = lambda entry, repo: (False, "")            # noqa: E731 — a stub, not logic
SOMETHING_FOUND = lambda entry, repo: (True, "entry records pr #4266")  # noqa: E731
...
def test_a_headless_chip_with_no_branch_after_45_minutes_is_reported_stalled(tmp_path):
    """...and only when nothing corroborates it. See the UNKNOWN sibling below."""
    root = lane(tmp_path)
    headless(root, "gone-quiet", started_at="2020-01-01T00:00:00Z")
    (live,) = mde.evaluate(root, corroborate=NOTHING_FOUND)["headless_liveness"]
...
def test_a_stalled_headless_chip_is_named_in_the_text_report(tmp_path):
    root = lane(tmp_path)
    headless(root, "gone-quiet", started_at="2020-01-01T00:00:00Z")
    text = mde.render_text(mde.evaluate(root, corroborate=NOTHING_FOUND))
...
# ── the launcher breaker: a stall is acted on, not only reported (2026-09-13) ─
#
# The defect: four briefs were routed headless in one day into a launcher that produced
# nothing at all — no commit, no branch, not one byte written into their worktrees — while
# the prerequisite check reported `headless: available` throughout, correctly, because it
# answers permission and disclaimer and neither is "does a launched session do work".
# `internal/finding-headless-launch-is-0-for-4-2026-09-13.md`.


def test_a_stalled_session_fails_headless_routing_closed(tmp_path, monkeypatch):
    """Both prerequisites met and one stalled session in flight: the next brief takes the
    PREPARED route instead. A tap is throughput the lane actually gets; a slot spent on a
    session that will not start is not."""
    headless_available(monkeypatch)
    root = lane(tmp_path)
    headless(root, "gone-quiet", started_at="2020-01-01T00:00:00Z")
    brief(root, "queued", "ready-work", created="2026-08-21")
    r = mde.evaluate(root, corroborate=NOTHING_FOUND)
    assert r["next"]["id"] == "ready-work"
    assert r["next"]["route"] == "prepared"
    assert r["headless_slots"] == 0
    assert r["headless"]["available"] is False
    assert r["headless"]["missing"] == [mde.mdh.MISSING_STALLED]


def test_the_breaker_is_named_last_among_the_missing(tmp_path):
    """The operator's two setup steps come first — those are the ones a human fixes by
    hand — and the breaker, which clears itself, comes last."""
    root = lane(tmp_path)
    headless(root, "gone-quiet", started_at="2020-01-01T00:00:00Z")
    r = mde.evaluate(root, corroborate=NOTHING_FOUND)
    assert r["headless"]["missing"] == [mde.mdh.MISSING_GRANT, mde.mdh.MISSING_STALLED]
    assert mde.mdh.MISSING_STALLED in r["headless"]["note"]
    assert "STALLED" in r["headless"]["detail"]


def test_a_live_headless_session_leaves_routing_open(tmp_path, monkeypatch):
    """Not "any headless entry" — a session that cut its branch is alive however long ago
    it launched, and the next brief still goes headless."""
    headless_available(monkeypatch)
    root = lane(tmp_path)
    brief(root, "inflight", "working", launch="headless", branch="claude/working",
          extra="session: agent_live\nstarted: 2020-01-01T00:00:00Z")
    brief(root, "queued", "ready-work", created="2026-08-21")
    r = mde.evaluate(root)
    assert r["headless_liveness"][0]["state"] == "alive"
    assert r["headless"]["available"] is True
    assert r["next"]["route"] == "headless"


def test_an_empty_lane_never_trips_the_breaker(tmp_path, monkeypatch):
    """A missing fact is not a negative fact. No headless entry is silence about the
    launcher, not evidence against it."""
    headless_available(monkeypatch)
    root = lane(tmp_path)
    brief(root, "queued", "ready-work", created="2026-08-21")
    r = mde.evaluate(root)
...
def test_the_breaker_holds_until_the_stalled_entry_is_cleared(tmp_path, monkeypatch):
    """The honest cost, pinned so nobody meets it as a surprise: ONE uncleared stalled
    entry keeps headless routing closed until somebody clears it. That is the trade taken
    deliberately — the alternative is 2026-09-13, where nothing closed and four of six
    build slots went to silence for a day."""
    headless_available(monkeypatch)
    root = lane(tmp_path)
    headless(root, "gone-quiet", started_at="2020-01-01T00:00:00Z")
    brief(root, "queued", "ready-work", created="2026-08-21")
    assert mde.evaluate(root, corroborate=NOTHING_FOUND)["next"]["route"] == "prepared"
    (root / "inflight" / "gone-quiet.md").unlink()
    assert mde.evaluate(root, corroborate=NOTHING_FOUND)["next"]["route"] == "headless"


def test_the_stalled_line_says_routing_has_failed_closed(tmp_path):
    """The report has to say what the lane DID, not only what it saw — the old line said
    only that the reconciler decides, which was true and is no longer the whole story."""
    root = lane(tmp_path)
    headless(root, "gone-quiet", started_at="2020-01-01T00:00:00Z")
    text = mde.render_text(mde.evaluate(root, corroborate=NOTHING_FOUND))
    assert "FAILED CLOSED" in text
    assert "output of record" in text, "the PR is still the output of record"


# ── corroboration: a clock reading is not a death certificate (2026-09-14) ───
#
# The 09-13 breaker tripped on `classify_liveness`'s 45-minute no-branch window alone. Four
# sessions were read as dead on it; three began writing ~21 hours later and delivered, and
# the fourth already had two open PRs against its brief while its entry still read
# `stalled`. Had the breaker been on main it would have closed headless routing fleet-wide
# on three chips that were about to land.
# `internal/finding-headless-launch-is-0-for-4-2026-09-13.md` §5-6.


def test_an_uncorroborated_stall_is_unknown_not_stalled(tmp_path):
    """The window elapsed AND something says work exists → `unknown`, and the reason says
    which evidence contradicted the clock."""
    root = lane(tmp_path)
    headless(root, "gone-quiet", started_at="2020-01-01T00:00:00Z")
    (live,) = mde.evaluate(root, corroborate=SOMETHING_FOUND)["headless_liveness"]
    assert live["state"] == mde.mdh.UNKNOWN
    assert "pr #4266" in live["reason"] and "UNKNOWN, not stalled" in live["reason"]


def test_an_unknown_never_trips_the_breaker(tmp_path, monkeypatch):
    """THE 2026-09-13 ACCEPTANCE FIXTURE. Three chips past the window, all corroborated:
    headless routing must stay open and the next brief must still route headless."""
    headless_available(monkeypatch)
    root = lane(tmp_path)
    for ident in ("board-card-detail", "cards-are-identified", "morning-board"):
        headless(root, ident, started_at="2020-01-01T00:00:00Z")
    brief(root, "queued", "ready-work", created="2026-08-21")
    r = mde.evaluate(root, corroborate=SOMETHING_FOUND)
    assert {row["state"] for row in r["headless_liveness"]} == {mde.mdh.UNKNOWN}
    assert r["headless"]["available"] is True
    assert mde.mdh.MISSING_STALLED not in r["headless"]["missing"]
    assert r["next"]["route"] == "headless"


def test_a_recorded_pr_alone_corroborates(tmp_path):
    """The real corroborator, on the shape that actually occurred: the entry carried
    `pr: 4266` while its branch field was empty and its worktree untouched."""
    found, detail = mde._stall_corroboration(
        {"id": "backup-pat", "pr": 4266, "started": "2020-01-01T00:00:00Z"}, None)
    assert found is True and "4266" in detail


def test_corroboration_that_cannot_run_reports_found(tmp_path):
    """The fail-safe direction, applied to the check itself: no repo to look in is not
    evidence of death. A corroboration that could not run never licenses `stalled`."""
    found, detail = mde._stall_corroboration(
        {"id": "gone-quiet", "started": "2020-01-01T00:00:00Z"}, None)
    assert found is True and "no repo" in detail


def test_a_worktree_written_after_checkout_corroborates(tmp_path):
    """A session writing into its own worktree is work, whatever the lane entry records."""
    repo = _git_repo(tmp_path / "repo")
    wt = repo / mde.HEADLESS_WORKTREE_SUBDIR / "meta-chip-busy-chip"
    wt.mkdir(parents=True)
    started = "2020-01-01T00:00:00Z"
    f = wt / "src.py"
    f.write_text("work", encoding="utf-8")
    import os
    late = mde.mdh._epoch(started) + mde.WORKTREE_CHECKOUT_SLACK_S + 600
    os.utime(f, (late, late))
    found, detail = mde._stall_corroboration(
        {"id": "busy-chip", "started": started}, repo)
    assert found is True and "worktree written" in detail


def test_a_worktree_untouched_since_checkout_does_not_corroborate(tmp_path):
    """The other direction, so the check can still say `stalled`: nothing written since the
    checkout, no pr, no ref → not corroborated."""
    repo = _git_repo(tmp_path / "repo")
    wt = repo / mde.HEADLESS_WORKTREE_SUBDIR / "meta-chip-dead-chip"
    wt.mkdir(parents=True)
    started = "2020-01-01T00:00:00Z"
    f = wt / "src.py"
    f.write_text("checkout", encoding="utf-8")
    import os
    at_checkout = mde.mdh._epoch(started) + 2
    os.utime(f, (at_checkout, at_checkout))
    found, _ = mde._stall_corroboration({"id": "dead-chip", "started": started}, repo)
    assert found is False


def test_the_unknown_line_says_routing_stays_open(tmp_path):
    """The report must say what the lane DID. `unknown` and `stalled` differ in exactly one
    consequence — whether routing closed — so the two lines must not read alike."""
    root = lane(tmp_path)
    headless(root, "gone-quiet", started_at="2020-01-01T00:00:00Z")
    text = mde.render_text(mde.evaluate(root, corroborate=SOMETHING_FOUND))
    assert "UNKNOWN" in text and "Routing stays OPEN" in text
...
# ── D-CD2: a tray card's computed state (2026-09-11) ──────────────────────────


def _tray_card(root, ident, *, dispatched_at=None, started=None, session=None,
               declined_at=None, launch=None, branch=None, pr=None, title=None):
    """An `inflight/` entry with exactly the D-CD2 fields under test, and nothing else
    implied — unlike `_card` (which always writes `launch: prepared`), this helper lets
    a test choose `launch`/`branch`/`pr` independently so it can build the `started` and
    `stale` fixtures the prepared-only helper cannot."""
    lines = []
    if dispatched_at is not None:
        lines.append("dispatched_at: %s" % dispatched_at)
        lines.append("dispatched: %s" % dispatched_at[:10])
    if started is not None:
        lines.append("started: %s" % started)
    if session is not None:
        lines.append("session: %s" % session)
    if declined_at is not None:
        lines.append("declined_at: %s" % declined_at)
    return brief(root, "inflight", ident, title=title, launch=launch, branch=branch,
                pr=pr, extra="\n".join(lines))


def test_cards_state_prepared_started_stale_declined(tmp_path):
    """One fixture per state, each threshold crossed by exactly one minute either side
    — `STALE_PREPARED_H` (6h) and `STALE_STARTED_H` (4h) are boundaries a real card
    crosses mid-tick, not on the hour, so the decision must be precise to the minute
    even though the reported `age_h` stays a floored whole hour."""
    root = lane(tmp_path)
    _tray_card(root, "fresh-prepared", dispatched_at="2026-09-07T12:31:00Z",
              session="task_a")                                     # 5h59m -> prepared
    _tray_card(root, "old-prepared", dispatched_at="2026-09-07T12:29:00Z",
              session="task_b")                                     # 6h01m -> stale
    _tray_card(root, "boundary-prepared", dispatched_at="2026-09-07T12:30:00Z",
              session="task_c")                                     # exactly 6h -> prepared
    _tray_card(root, "healthy-started", dispatched_at="2026-09-07T14:31:00Z",
              launch="started", branch="claude/healthy")            # 3h59m -> started
    _tray_card(root, "stale-started", dispatched_at="2026-09-07T14:29:00Z",
              launch="started", branch="claude/stale")               # 4h01m -> stale
    _tray_card(root, "boundary-started", dispatched_at="2026-09-07T14:30:00Z",
              launch="started", branch="claude/boundary")            # exactly 4h -> started
    _tray_card(root, "headless-healthy", started="2026-09-07T16:30:00Z",
              launch="headless", session="task_hl")                  # 2h -> started
    _tray_card(root, "already-declined", declined_at="2026-09-07T17:30:00Z")
    _tray_card(root, "graduated", dispatched_at="2026-09-07T10:00:00Z",
              launch="started", branch="claude/done", pr=4300)       # has a PR -> excluded

    inflight = mde.scan_lane(root)[0]["inflight"]
    cards = mde.build_cards(inflight, NOW, None)
    by_id = {c["id"]: c for c in cards}

    assert "graduated" not in by_id
    assert by_id["fresh-prepared"]["state"] == "prepared"
    assert by_id["fresh-prepared"]["action"] == "tap"
    assert by_id["fresh-prepared"]["age_h"] == 5
    assert by_id["old-prepared"]["state"] == "stale"
    assert by_id["old-prepared"]["action"] == "decline"
    assert by_id["boundary-prepared"]["state"] == "prepared"
    assert by_id["healthy-started"]["state"] == "started"
    assert by_id["healthy-started"]["action"] == "nothing"
    assert by_id["stale-started"]["state"] == "stale"
    assert by_id["stale-started"]["action"] == "investigate"
    assert by_id["boundary-started"]["state"] == "started"
    assert by_id["headless-healthy"]["state"] == "started"
    assert by_id["headless-healthy"]["age_h"] == 2
    assert by_id["already-declined"]["state"] == "declined"
    assert by_id["already-declined"]["action"] == "nothing"

    # stale first, then prepared, then started, then declined
    order = [c["state"] for c in cards]
    assert order.index("stale") < order.index("prepared") < order.index("started")


def test_stale_started_is_investigate_not_tap(tmp_path):
    """A card that stamped `start` and then went quiet reads `stale`/`investigate` —
    never `prepared`/`tap`, which would invite the operator to spawn a duplicate on a
    chip that may already be mid-flight."""
    root = lane(tmp_path)
    _tray_card(root, "went-quiet", dispatched_at="2026-09-07T10:00:00Z",
              launch="started", branch="claude/went-quiet")

    inflight = mde.scan_lane(root)[0]["inflight"]
    cards = mde.build_cards(inflight, NOW, None)

    assert cards[0]["state"] == "stale"
    assert cards[0]["action"] == "investigate"
    assert cards[0]["action"] != "tap"


def test_a_missing_deciding_stamp_is_unknown_on_both_paths(tmp_path):
    """Absence of evidence is never the reassuring answer. Untapped with no
    `dispatched_at` is not a `prepared` card that stays fresh forever (and so never
    crosses D-CD4's 6h/12h pokes); in motion with nothing `_card_last_activity` can
    resolve is not `started`/`nothing`. Both are `unknown`/`investigate`, sorted first."""
    root = lane(tmp_path)
    _tray_card(root, "fresh-prepared", dispatched_at="2026-09-07T17:31:00Z",
              session="task_a")
    _tray_card(root, "old-prepared", dispatched_at="2026-09-07T10:00:00Z",
              session="task_b")                                     # stale
    _tray_card(root, "untapped-no-stamp", session="task_c")          # no dispatched_at
    _tray_card(root, "moving-no-stamp", launch="started",
              branch="claude/moving-no-stamp")                       # repo None, no stamps

    inflight = mde.scan_lane(root)[0]["inflight"]
    cards = mde.build_cards(inflight, NOW, None)
    by_id = {c["id"]: c for c in cards}

    for ident in ("untapped-no-stamp", "moving-no-stamp"):
        assert by_id[ident]["state"] == mde.CARD_STATE_UNKNOWN, ident
        assert by_id[ident]["action"] == mde.CARD_ACTION_INVESTIGATE, ident
        assert by_id[ident]["age_h"] is None, ident
    assert [c["state"] for c in cards][:2] == ["unknown", "unknown"]
    assert cards[2]["state"] == "stale"


def test_a_card_with_no_stamp_never_borrows_the_clock_for_its_tray_title(tmp_path):
    """No composed clock time on a card that has no stamp of its own — the composer's
    `??:??` stands, so the title neither mismatches the tile nor moves every tick."""
    root = lane(tmp_path)
    _tray_card(root, "no-stamp", session="task_a", title="Do the thing")

    inflight = mde.scan_lane(root)[0]["inflight"]
    (card,) = mde.build_cards(inflight, NOW, None)

    assert card["tray_title"] == "[META:substrate] --:-- [no-stamp] Do the thing"
    assert NOW.astimezone().strftime("%H:%M") not in card["tray_title"]


def test_a_raising_probe_is_investigate_never_tap(monkeypatch):
    """FAIL TOWARD THE CARD, NEVER TOWARD A TAP: whatever goes wrong inside
    `card_state`, the card is never dropped and never an exception — and it is
    `unknown`/`investigate` for EVERY entry, untapped included. An error means the tool
    cannot tell, and `tap` on a chip that may already be mid-flight spawns a duplicate."""
    def boom(*a, **k):
        raise RuntimeError("boom")
    monkeypatch.setattr(mde, "_precise_age_hours", boom)

    untapped = {"id": "x", "dispatched_at": "2026-09-07T18:00:00Z"}
    moving = {"id": "y", "launch": "started", "branch": "claude/y",
              "dispatched_at": "2026-09-07T18:00:00Z"}
    for entry in (untapped, moving):
        detail = {}
        assert mde.card_state(entry, NOW, None, detail=detail) == (
            mde.CARD_STATE_UNKNOWN, None, mde.CARD_ACTION_INVESTIGATE)
        assert detail == {"age_from": None, "stale_kind": None}

    monkeypatch.setattr(mde, "in_motion", boom)
    state, _, action = mde.card_state(untapped, NOW, None)
    assert (state, action) == (mde.CARD_STATE_UNKNOWN, mde.CARD_ACTION_INVESTIGATE)


def test_a_none_stamp_is_unknown_on_both_paths():
    """A `None` deciding stamp never falls through to a reassuring branch: not
    `prepared`/`tap` untapped, not `started`/`nothing` in motion."""
    for entry in ({"id": "u", "dispatched_at": None},
                  {"id": "m", "launch": "started", "dispatched_at": None, "started": None}):
        assert mde.card_state(entry, NOW, None) == (
            mde.CARD_STATE_UNKNOWN, None, mde.CARD_ACTION_INVESTIGATE)


def test_cards_carry_stale_kind_and_what_the_age_measures_from(tmp_path):
    """D-CD4's repeat poke reads `stale_kind`, a field, rather than inferring
    stale-prepared from `action`; each card names what its `age_h` counts from."""
    old = (NOW - datetime.timedelta(hours=7)).strftime("%Y-%m-%dT%H:%M:%SZ")
    root = lane(tmp_path)
    _tray_card(root, "old-card", dispatched_at=old, session="task_a")
    _tray_card(root, "old-start", dispatched_at=old, session="task_b",
               launch="started", started=old)
    inflight = mde.scan_lane(root)[0]["inflight"]
    by_id = {c["id"]: c for c in mde.build_cards(inflight, NOW, None)}
    assert (by_id["old-card"]["stale_kind"], by_id["old-card"]["age_from"]) == (
        "prepared", "dispatch")
    assert (by_id["old-start"]["stale_kind"], by_id["old-start"]["age_from"]) == (
        "started", "start")


def test_cards_is_in_the_evaluate_payload(tmp_path):
    """`evaluate()["cards"]` is the SAME computation `build_cards` performs directly —
    one source, so a caller reading the JSON and a test calling the function can never
    disagree about a card's state. `evaluate` samples the REAL clock (unlike
    `build_cards`, which takes `now` as an argument), so the fixture's `dispatched_at`
    is computed from it rather than pinned, to stay `prepared` whenever this runs."""
    root = lane(tmp_path)
    recent = (datetime.datetime.now(datetime.timezone.utc)
             - datetime.timedelta(minutes=5)).strftime("%Y-%m-%dT%H:%M:%SZ")
    _tray_card(root, "a-card", dispatched_at=recent, session="task_a")

    r = mde.evaluate(root)

    assert len(r["cards"]) == 1
    assert r["cards"][0]["id"] == "a-card"
    assert r["cards"][0]["state"] == "prepared"
    assert r["cards"][0]["tray_title"].startswith("[META:")
    assert "[a-card]" in r["cards"][0]["tray_title"]
# ── duplicate_prs: D-TP5, name the keeper ─────────────────────────────────────


def test_duplicate_prs_flags_two_prs_bound_to_one_id(tmp_path):
    a = {"number": 100, "title": "Fix A", "id": "chip-a", "created_at": "2026-09-10T00:00:00Z"}
    b = {"number": 101, "title": "Fix A but different", "id": "chip-a",
        "created_at": "2026-09-10T01:00:00Z"}
    (dup,) = mde.duplicate_prs([a, b])
    assert dup["reason"] == "bound-twice:chip-a"
    assert dup["keeper"] == 100 and dup["duplicate"] == 101


def test_duplicate_prs_flags_similar_titles(tmp_path):
    a = {"number": 200, "title": "Fix the tile metrics regression",
        "created_at": "2026-09-10T00:00:00Z"}
    b = {"number": 201, "title": "fix tile metrics regression",
        "created_at": "2026-09-10T02:00:00Z"}
    (dup,) = mde.duplicate_prs([a, b])
    assert dup["reason"].startswith("title-match:")
    assert dup["keeper"] == 200 and dup["duplicate"] == 201


def test_duplicate_prs_flags_overlapping_changed_files(tmp_path):
    a = {"number": 300, "title": "unrelated title one", "created_at": None,
        "files": ["tools/meta-dispatch-move", "packages/admin/tests/test_x.py"]}
    b = {"number": 299, "title": "totally different wording", "created_at": None,
        "files": ["tools/meta-dispatch-move"]}
    (dup,) = mde.duplicate_prs([a, b])
    assert dup["reason"] == "touches-overlap"
    # no created_at on either: falls back to the lower PR number as the keeper
    assert dup["keeper"] == 299 and dup["duplicate"] == 300


def test_duplicate_prs_reports_nothing_for_unrelated_prs(tmp_path):
    a = {"number": 1, "title": "Add the foo widget", "files": ["a.py"]}
    b = {"number": 2, "title": "Remove the bar service", "files": ["b.py"]}
    assert mde.duplicate_prs([a, b]) == []


def test_duplicate_prs_cli_exits_1_and_names_the_keeper(tmp_path, capsys):
    prs = [{"number": 1, "title": "same", "id": "x", "created_at": "2026-09-10T00:00:00Z"},
          {"number": 2, "title": "same", "id": "x", "created_at": "2026-09-10T01:00:00Z"}]
    path = tmp_path / "prs.json"
    path.write_text(json.dumps(prs))
    rc = mde.main(["--duplicate-prs", str(path)])
    assert rc == 1
    assert "keeper #1" in capsys.readouterr().out


# ── the ordering classes (2026-09-17) ────────────────────────────────────────
#
# Oldest-first on `created` is right for a backlog and wrong for a repair. On
# 2026-09-17 the four briefs written that morning to unblock OPEN PRs sat at
# positions 71-74 of 74 eligible, behind August briefs that block nothing, and the
# only lever available was back-dating `created` — which would have "worked" and
# would have been a lie to the sort. These pin the class that replaced it.
...
def repair(root, ident, pr, *, created, since=None, **kw):
    extra = "class: repair\nrepair_pr: %d" % pr
    if since is not None:
        extra += "\nrepair_since: %s" % since
    return brief(root, "queued", ident, created=created, extra=extra, **kw)


def order(root, **kw):
    return [e["id"] for e in mde.evaluate(root, **kw)["eligible"]]


def test_a_brief_with_no_class_is_backlog_and_the_order_is_unchanged(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "older", created="2026-08-01")
    brief(root, "queued", "newer", created="2026-09-01")
    r = mde.evaluate(root)
    assert [e["id"] for e in r["eligible"]] == ["older", "newer"]
    assert {e["class"] for e in r["eligible"]} == {mde.CLASS_BACKLOG}
    assert {e["class_effective"] for e in r["eligible"]} == {mde.CLASS_BACKLOG}


def test_a_repair_sorts_ahead_of_the_whole_backlog(tmp_path):
    """The defect this exists for: a brief written today to unblock an open PR,
    behind a backlog written weeks earlier."""
    root = lane(tmp_path)
    brief(root, "queued", "august-backlog", created="2026-08-01")
    brief(root, "queued", "also-backlog", created="2026-08-02")
    repair(root, "unblocks-an-open-pr", 4137, created=days_ago(0))
    assert order(root)[0] == "unblocks-an-open-pr"


def test_repairs_are_still_oldest_first_among_themselves(tmp_path):
    """One dimension is added to the sort. Nothing else changes — there is no
    priority number and no per-brief knob, so the older repair goes first."""
    root = lane(tmp_path)
    repair(root, "newer-repair", 2, created=days_ago(1))
    repair(root, "older-repair", 1, created=days_ago(3))
    brief(root, "queued", "backlog", created="2026-08-01")
    assert order(root) == ["older-repair", "newer-repair", "backlog"]


def test_a_repair_must_name_the_pr_it_unblocks(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "classy", extra="class: repair")
    r = mde.evaluate(root)
    assert r["eligible"] == []
    assert "repair_pr" in r["invalid"][0]["error"]


def test_repair_pr_without_the_class_is_refused_not_ignored(tmp_path):
    """A priority field that silently does nothing is the worst of both."""
    root = lane(tmp_path)
    brief(root, "queued", "half-classed", extra="repair_pr: 4137")
    r = mde.evaluate(root)
    assert r["eligible"] == []
    assert "without `class: repair`" in r["invalid"][0]["error"]


def test_an_unknown_class_is_refused_loudly_not_sorted_last(tmp_path):
    """The silent-control shape: `class: urgent` treated as backlog would leave the
    PM believing the brief is at the front while the queue disagrees, with nothing
    saying so. Refused, it lands in invalid[] which the procedure never dispatches."""
    root = lane(tmp_path)
    brief(root, "queued", "typo-class", extra="class: urgent")
    r = mde.evaluate(root)
    assert r["eligible"] == []
    assert "class 'urgent'" in r["invalid"][0]["error"]


def test_the_class_expires_when_the_named_pr_is_observed_merged(tmp_path):
    """Merged-ness comes from the injected facts and nowhere else — the same rule
    the duplicate classifier follows."""
    root = lane(tmp_path)
    brief(root, "queued", "backlog", created="2026-08-01")
    repair(root, "repairs-4137", 4137, created=days_ago(0))
    assert order(root)[0] == "repairs-4137"

    r = mde.evaluate(root, facts={"merged_prs": [4137]})
    assert [e["id"] for e in r["eligible"]] == ["backlog", "repairs-4137"]
    done = [e for e in r["eligible"] if e["id"] == "repairs-4137"][0]
    assert done["class_expired_reason"] == "repair-pr-merged"


def test_an_expired_repair_is_demoted_never_held(tmp_path):
    """An expiry changes RANK and nothing else. A repair that expires is an ordinary
    queued brief in its ordinary place — never held, hidden or dropped."""
    root = lane(tmp_path)
    repair(root, "repairs-4137", 4137, created=days_ago(0))
    r = mde.evaluate(root, facts={"merged_prs": [4137]})
    assert [e["id"] for e in r["eligible"]] == ["repairs-4137"]
    assert r["held"] == [] and r["invalid"] == []


def test_the_class_expires_after_the_window(tmp_path):
    """A fast lane with no exit becomes the lane. Absent the merged fact the class
    stands, so the window is what bounds it."""
    root = lane(tmp_path)
    brief(root, "queued", "backlog", created="2026-08-01")
    repair(root, "stale-repair", 4137,
           created=days_ago(mde.REPAIR_WINDOW_DAYS + 5),
           since=days_ago(mde.REPAIR_WINDOW_DAYS + 1))
    r = mde.evaluate(root)
    assert [e["id"] for e in r["eligible"]] == ["backlog", "stale-repair"]
    stale = [e for e in r["eligible"] if e["id"] == "stale-repair"][0]
    assert stale["class_expired_reason"] == "repair-window-elapsed"


def test_the_window_is_measured_from_the_class_not_the_brief(tmp_path):
    """The case that proved the rule: the chip unblocking #4079 was written
    2026-08-23 and became a repair on 09-17 — 25 days old, zero days in the class. A
    window on `created` would demote it on the day it was classed."""
    root = lane(tmp_path)
    brief(root, "queued", "backlog", created="2026-08-01")
    repair(root, "old-brief-new-repair", 4079,
           created=days_ago(mde.REPAIR_WINDOW_DAYS * 2), since=days_ago(0))
    assert order(root)[0] == "old-brief-new-repair"


def test_repair_since_defaults_to_created(tmp_path):
    root = lane(tmp_path)
    repair(root, "fresh", 1, created=days_ago(1))
    repair(root, "ancient", 2, created=days_ago(mde.REPAIR_WINDOW_DAYS + 1))
    r = mde.evaluate(root)
    by_id = {e["id"]: e for e in r["eligible"]}
    assert by_id["fresh"]["class_effective"] == mde.CLASS_REPAIR
    assert by_id["ancient"]["class_effective"] == mde.CLASS_BACKLOG
    assert by_id["fresh"]["repair_since"] == by_id["fresh"]["created"]


def test_repair_since_cannot_predate_the_brief(tmp_path):
    """Backdating `repair_since` is the same lie as backdating `created`, one field
    over — and it buys a LONGER fast lane rather than a place in the queue."""
    root = lane(tmp_path)
    repair(root, "backdated", 1, created="2026-09-01", since="2026-08-01")
    r = mde.evaluate(root)
    assert r["eligible"] == []
    assert "cannot predate" in r["invalid"][0]["error"]


def test_the_human_line_says_why_a_repair_outranks_an_older_brief(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "backlog", created="2026-08-01")
    repair(root, "repairs-4137", 4137, created=days_ago(0))
    text = mde.render_text(mde.evaluate(root))
    line = [ln for ln in text.splitlines() if "repairs-4137" in ln][0]
    assert "repair #4137" in line


# ── D-PM13: the per-tick selection, the prepared cap and the file-set rule ───
#
# The rule this replaced was step 5's "ONE candidate per run, always" — correct for a
# `*/30` tick, starvation at the hourly one. Two limits bound the new behaviour and they
# are different limits: `--per-tick` is the RATE (cards per tick), `--prepared-cap` is the
# DEPTH (unclicked cards at once). The file-set rule is what keeps the extra rate from
# buying conflicts instead of throughput. Since D-PM6' there is a THIRD limit that is not
# a number at all: a brief's ROUTE. A headless brief spends a build slot and a prepared
# one spends a tray slot, so the two are counted separately and a full tray no longer
# stops a headless candidate riding the same tick.

PLUGIN = ["packages/plugin/src/observer/TurnObserver.ts"]
ADMIN = ["packages/admin/evolve_admin/web"]
TOOLS = ["tools/meta-dispatch-move"]


def test_three_non_overlapping_candidates_are_returned_in_queue_order(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=ADMIN)
    brief(root, "queued", "c-three", created="2026-08-03", touches=TOOLS)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-one", "b-two", "c-three"]
    assert r["next"]["id"] == "a-one"        # unchanged meaning: candidates[0]
    assert r["dispatchable"] is True and r["deferred"] == []
    assert r["per_tick_cap"] == 3


def test_a_fourth_eligible_brief_waits_for_the_next_tick(tmp_path):
    """The per-tick cap binds, and it says so by name — a reader that cannot tell WHICH
    of the three limits held the fourth card cannot tell a busy lane from a rate limit."""
    root = lane(tmp_path)
    for i, paths in enumerate((PLUGIN, ADMIN, TOOLS, ["docs/help"])):
        brief(root, "queued", "chip-%d" % i, created="2026-08-0%d" % (i + 1),
              touches=paths)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["chip-0", "chip-1", "chip-2"]
    (d,) = r["deferred"]
    assert d["id"] == "chip-3" and d["blocked_by"] == "per-tick-cap"


def test_a_fourth_is_held_by_the_prepared_cap_when_the_tray_is_not_empty(tmp_path):
    """One card already unclicked leaves room for three, so the prepared cap — not the
    per-tick cap — is what the fourth is held for, and it is named that way."""
    root = lane(tmp_path)
    prepared(root, "already-in-the-tray")
    for i, paths in enumerate((PLUGIN, ADMIN, TOOLS, ["docs/help"])):
        brief(root, "queued", "chip-%d" % i, created="2026-08-0%d" % (i + 1),
              touches=paths)
    r = mde.evaluate(root)
    assert r["prepared_count"] == 1 and r["prepared_slots"] == 3
    assert len(r["candidates"]) == 3
    (d,) = r["deferred"]
    assert d["id"] == "chip-3" and d["blocked_by"] == "prepared-cap"
    assert "prepared room 3" in d["detail"]


def test_the_build_cap_bounds_the_tick_too(tmp_path):
    """A tick never prepares more cards than the build cap could take. `slots` is the
    smallest of the three here, so it is the one named."""
    root = lane(tmp_path)
    for i in range(5):
        started(root, "busy-%d" % i, pr=100 + i)
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    ...
    r = mde.evaluate(root)
    assert r["slots"] == 1
    assert [c["id"] for c in r["candidates"]] == ["a-one"]
    assert r["deferred"][0]["blocked_by"] == "cap"


def test_two_briefs_on_the_same_files_never_ride_the_same_tick(tmp_path):
    """The collision the rule exists for: on 2026-09-07 five head-of-queue briefs all
    edited TurnObserver.ts, so three cards a tick would have bought two rebases."""
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=PLUGIN)
    brief(root, "queued", "c-three", created="2026-08-03", touches=ADMIN)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-one", "c-three"]
    (d,) = r["deferred"]
    assert d["id"] == "b-two" and d["blocked_by"] == "files"
    assert d["detail"] == ("b-two overlaps a-one on "
                           "packages/plugin/src/observer/TurnObserver.ts")
    assert ("  held for files: b-two overlaps a-one on "
            "packages/plugin/src/observer/TurnObserver.ts") in mde.render_text(r)


def test_a_directory_prefix_overlaps_a_file_beneath_it(tmp_path):
    """Prefixes are prefixes: the reported one is the SHORTER of the pair, because the
    shared ground is the directory, not one brief's private file."""
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01",
          touches=["packages/plugin/src/observer"])
    brief(root, "queued", "b-two", created="2026-08-02", touches=PLUGIN)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-one"]
    assert r["deferred"][0]["detail"].endswith("on packages/plugin/src/observer")


def test_a_trailing_slash_is_the_same_declaration(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01", touches=["tools/"])
    brief(root, "queued", "b-two", created="2026-08-02", touches=["tools"])
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-one"]
    assert r["deferred"][0]["blocked_by"] == "files"


def test_a_brief_with_no_touches_is_prepared_alone(tmp_path):
    """Absent means "touches everything" on the candidate side — which is exactly the
    pre-D-PM13 behaviour, so every brief written before the rule keeps getting it."""
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01")           # no touches
    brief(root, "queued", "b-two", created="2026-08-02", touches=ADMIN)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-one"]
    (d,) = r["deferred"]
    assert d["id"] == "b-two" and d["blocked_by"] == "files"
    assert "declares no touches" in d["detail"]


def test_a_brief_with_no_touches_may_not_ride_alongside_one_already_selected(tmp_path):
    """The other direction of the same rule — undeclared second, not first."""
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01", touches=ADMIN)
    brief(root, "queued", "b-two", created="2026-08-02")            # no touches
    brief(root, "queued", "c-three", created="2026-08-03", touches=TOOLS)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-one", "c-three"]
    assert [d["id"] for d in r["deferred"]] == ["b-two"]
    assert r["deferred"][0]["blocked_by"] == "files"


def test_an_overlap_with_work_already_in_motion_holds_the_candidate(tmp_path):
    root = lane(tmp_path)
    brief(root, "inflight", "running", launch="started", branch="claude/running",
          touches=PLUGIN)
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=ADMIN)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["b-two"]
    (d,) = r["deferred"]
    assert d["id"] == "a-one" and d["blocked_by"] == "files"
    assert d["detail"] == ("a-one overlaps in-motion running on "
                           "packages/plugin/src/observer/TurnObserver.ts")


def test_a_repair_brief_is_not_fenced_by_the_pr_it_repairs(tmp_path):
    """A repair is its PR's continuation (D-PM7): touching the held PR's files is the job.
    Before this, `hold-fix-4443` sat deferred on the brief bound to #4443 — the fence
    refused the one brief written to clear it."""
    root = lane(tmp_path)
    brief(root, "inflight", "tier-chip", launch="started", branch="claude/tier-chip",
          pr=4443, touches=PLUGIN)
    review(root, 4443, "Verdict: CONCERNS — red shard.\n")
    brief(root, "queued", "hold-fix-4443", created="2026-08-01", touches=PLUGIN,
          extra="class: repair\nrepair_pr: 4443")
    r = mde.evaluate(root)
    assert r["held_prs"] == [4443]
    assert [c["id"] for c in r["candidates"]] == ["hold-fix-4443"]
    assert r["deferred"] == []


def test_a_held_prs_brief_keeps_its_slot_but_fences_no_files(tmp_path):
    """D-PM6 keeps the slot; D-LT2 says a held PR holds no build. The file fence follows
    the second: the branch is stopped, nobody is editing those paths, and a repair merges
    main first. On 2026-09-25 three held PRs fenced ~40 briefs on `packages/admin/tests`."""
    root = lane(tmp_path)
    brief(root, "inflight", "tier-chip", launch="started", branch="claude/tier-chip",
          pr=4443, touches=PLUGIN)
    review(root, 4443, "Verdict: CONCERNS — red shard.\n")
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    r = mde.evaluate(root)
    assert r["in_motion_count"] == 1 and r["held_prs"] == [4443]   # slot still charged
    assert [c["id"] for c in r["candidates"]] == ["a-one"]
    assert r["deferred"] == []


def test_an_unheld_running_pr_still_fences_its_files(tmp_path):
    """The exemption is for HELD PRs only — a PASS-reviewed chip still in motion is live
    work, and its fence stands exactly as before."""
    root = lane(tmp_path)
    brief(root, "inflight", "running", launch="started", branch="claude/running",
          pr=4001, touches=PLUGIN)
    review(root, 4001, "Verdict: PASS — fine.\n")
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    r = mde.evaluate(root)
    assert r["held_prs"] == []
    assert r["candidates"] == []
...
def test_an_in_motion_entry_with_no_touches_blocks_nothing(tmp_path):
    """"Unknown" is not "everything" on the in-motion side. Read the other way, the first
    undeclared chip in flight would hold the entire queue behind it — and every entry in
    flight when this rule shipped was one of those."""
    root = lane(tmp_path)
    brief(root, "inflight", "running", launch="started", branch="claude/running")
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=ADMIN)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-one", "b-two"]
    assert r["deferred"] == []


def test_an_overlap_with_a_prepared_unclicked_card_holds_the_candidate(tmp_path):
    """The file-set rule covers the WHOLE of `inflight/`, not just the in-motion half.

    Under the transport deviation a card waits in the tray until the operator taps it, and
    the prepared cap of 4 exists so several can wait at once — so the common path is a
    card from an earlier tick with no start evidence. Checking only `moving` let tick 2
    prepare a second card on tick 1's file set, and the tick poke invites the operator to
    tap both in one pass, buying exactly the conflict D-PM13 exists to prevent.
    """
    root = lane(tmp_path)
    prepared(root, "waiting", touches=PLUGIN)
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=ADMIN)
    r = mde.evaluate(root)
    # The waiting card is in the prepared partition, not the in-motion one.
    assert r["in_motion_count"] == 0 and r["prepared_count"] == 1
    assert [c["id"] for c in r["candidates"]] == ["b-two"]
    (d,) = r["deferred"]
    assert d["id"] == "a-one" and d["blocked_by"] == "files"
    # `prepared`, not `in-motion`: a hold the operator can clear by tapping the card is a
    # different fact from one they cannot clear at all, and the line must not conflate them.
    assert d["detail"] == ("a-one overlaps prepared waiting on "
                           "packages/plugin/src/observer/TurnObserver.ts")


def test_a_prepared_card_with_no_touches_blocks_nothing(tmp_path):
    """"Unknown" is not "everything" on the in-flight side whichever partition it sits in.

    The same reading as the in-motion case above, and for the same reason: every card in
    the tray when this rule shipped was undeclared, and holding the queue behind them
    would stop the lane on the day it landed.
    """
    root = lane(tmp_path)
    prepared(root, "waiting")
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=ADMIN)
    r = mde.evaluate(root)
    assert r["prepared_count"] == 1
    assert [c["id"] for c in r["candidates"]] == ["a-one", "b-two"]
    assert r["deferred"] == []


def test_an_undeclared_candidate_is_not_held_against_in_flight_work(tmp_path):
    """The narrowing the README and the docstring now state outright: "prepared alone" is
    about the TICK. `_clash` needs a declared set on both sides, so a brief with no
    `touches:` rides while a declared chip edits the same files — the pre-D-PM13
    behaviour, and why declaring `touches:` is what buys the in-flight hold."""
    root = lane(tmp_path)
    brief(root, "inflight", "running", launch="started", branch="claude/running",
          touches=PLUGIN)
    brief(root, "queued", "a-one", created="2026-08-01")            # no touches
...
def _unreadable_inflight(root, ident, text):
    """An `inflight/` entry the schema reader cannot parse, written byte-for-byte.

    The `brief()` helper only emits WELL-FORMED front matter, which is the point: these
    two fixtures are the shapes an author actually produces by hand, and neither can be
    expressed through the helper.
    """
    path = root / "inflight" / ("%s.md" % ident)
    path.write_text(text, encoding="utf-8")
    return path


def test_an_inflight_entry_with_a_scalar_touches_holds_the_whole_tick(tmp_path):
    """FAIL CLOSED on the in-flight side of the switch (#4137 hold item 1).

    `touches:` written as a bare scalar instead of a list is the exact mistake the
    candidate-side schema error exists for, and on the in-flight side it is worse: the
    entry drops out of `scan_lane` into `invalid[]`, so `_clash` never sees it and the
    file hold it was carrying silently disappears. That is a fail-open on the one guard
    D-PM13 was built to provide — a chip is editing those files right now and the tick
    would prepare a second card on them with nothing in the report saying why.

    So an `inflight/` entry that does not parse holds the tick: `candidates[]` is empty
    and every eligible brief is deferred under ONE grouped line naming the unreadable
    path. `invalid[]` still carries the class-3 poke that tells the PM which file to fix,
    so no new notification is added.
    """
    root = lane(tmp_path)
    _unreadable_inflight(root, "running", (
        "---\n"
        "id: running\n"
        "aspect: substrate\n"
        "title: a chip already editing the observer\n"
        "privileged: false\n"
        "launch: started\n"
        "branch: claude/running\n"
        "touches: packages/plugin/src/observer\n"      # scalar, not a list
        "created: 2026-08-01\n"
        "pm: fable-cowork\n"
        "---\n\nBuild the thing.\n"))
    ...
    # The entry is unreadable, not absent: it is reported so the PM can fix the file.
    assert [Path(b["path"]).stem for b in r["invalid"]] == ["running"]
    assert r["invalid"][0]["state"] == "inflight"
    # Nothing rides. Not just the overlapping brief — the overlap CANNOT BE COMPUTED, so
    # `b-two` is held too: the restrictive direction is to select nothing rather than to
    # select a brief whose overlap is unknown.
    assert r["candidates"] == [] and r["next"] is None
    assert [d["id"] for d in r["deferred"]] == ["a-one", "b-two"]
    assert {d["group"] for d in r["deferred"]} == {"unreadable-inflight"}
    assert {d["blocked_by"] for d in r["deferred"]} == {"files"}
    # One grouped line, and it names the path — the thing the PM has to open.
    (detail,) = {d["detail"] for d in r["deferred"]}
    assert str(root / "inflight" / "running.md") in detail
    assert "held for files: 2 brief(s) wait for the next tick" in mde.render_text(r)


def test_an_inflight_entry_with_truncated_front_matter_holds_the_whole_tick(tmp_path):
    """The same fail-closed direction reached by the other route.

    A truncated file (the closing `---` never written — an interrupted edit, or a bad
    merge resolution) fails in `split_front_matter` rather than in `_touches`, so it
    takes a different path into `invalid[]`. The tick's answer has to be the same one:
    an entry whose declared file set cannot be read is an entry whose overlap cannot be
    ruled out.
    """
    root = lane(tmp_path)
    _unreadable_inflight(root, "running", (
    ...
    (d,) = r["deferred"]
    assert d["id"] == "a-one" and d["blocked_by"] == "files"
    assert d["group"] == "unreadable-inflight"
    assert str(root / "inflight" / "running.md") in d["detail"]


def test_an_unreadable_entry_outside_inflight_does_not_hold_the_tick(tmp_path):
    """The hold is as narrow as its reason. `queued/` and `done/` carry no file set the
    tick has to respect — a queued brief that does not parse is simply not eligible, and
    a done one is finished — so only `inflight/` fails the tick closed. Without this the
    fix would be a lane-wide halt wearing the file rule's name."""
    root = lane(tmp_path)
    _unreadable_inflight(root, "half-written", "---\nid: half-written\n")
    (root / "inflight" / "half-written.md").rename(root / "queued" / "half-written.md")
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    r = mde.evaluate(root)
    assert [Path(b["path"]).stem for b in r["invalid"]] == ["half-written"]
    assert [c["id"] for c in r["candidates"]] == ["a-one"]
    assert r["deferred"] == []


def test_an_in_motion_overlap_is_reported_before_a_prepared_one(tmp_path):
    """A candidate overlapping both names the RUNNING chip: it is the more actionable of
    the two facts, and the order has to be deterministic for the pin to mean anything."""
    root = lane(tmp_path)
    brief(root, "inflight", "running", launch="started", branch="claude/running",
          touches=PLUGIN)
    prepared(root, "waiting", touches=PLUGIN)
...
def test_a_privileged_greenlit_brief_is_still_prepared_alone(tmp_path):
    """D-PM1 is untouched: the greenlight is still required, and a greenlit privileged
    brief still gets its own tick even when the rest of the queue would fit beside it."""
    root = lane(tmp_path)
    brief(root, "queued", "a-priv", created="2026-08-01", privileged=True,
          greenlight=True, touches=ADMIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=TOOLS)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-priv"]
    (d,) = r["deferred"]
    assert d["id"] == "b-two" and d["blocked_by"] == "privileged"


def test_a_privileged_brief_never_joins_a_tick_already_carrying_one(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01", touches=ADMIN)
    brief(root, "queued", "b-priv", created="2026-08-02", privileged=True,
          greenlight=True, touches=TOOLS)
    brief(root, "queued", "c-three", created="2026-08-03", touches=PLUGIN)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-one", "c-three"]
    assert [d["id"] for d in r["deferred"]] == ["b-priv"]
    assert r["deferred"][0]["blocked_by"] == "privileged"


def test_a_privileged_brief_without_a_greenlight_is_still_held_not_deferred(tmp_path):
    """`held[]` (ineligible in principle) and `deferred[]` (not this tick) must not read
    the same — the poke class for one is "the operator must act", for the other nothing."""
    root = lane(tmp_path)
    brief(root, "queued", "a-priv", created="2026-08-01", privileged=True, touches=ADMIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=TOOLS)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["b-two"]
    assert [h["id"] for h in r["held"]] == ["a-priv"]
    assert r["held"][0]["reason"] == "privileged-without-greenlight"
    assert r["deferred"] == []


def test_the_selection_never_reorders_the_queue(tmp_path):
    """Oldest-first is a prefix walk, not a search for the three that pack best. A head
    of queue that keeps colliding is a PM-level signal, and reordering would hide it."""
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=PLUGIN)
    brief(root, "queued", "c-three", created="2026-08-03", touches=PLUGIN)
    brief(root, "queued", "d-four", created="2026-08-04", touches=ADMIN)
    r = mde.evaluate(root)
    assert [c["id"] for c in r["candidates"]] == ["a-one", "d-four"]


def test_a_blocked_lane_selects_nothing_at_all(tmp_path):
    """Every top-level block means the lane is busy or wrong, and neither becomes
    dispatchable by taking fewer cards."""
    root = lane(tmp_path)
    for i in range(6):
        started(root, "busy-%d" % i, pr=100 + i)
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    r = mde.evaluate(root)
    assert r["blocked_by"] == "cap"
    assert r["candidates"] == [] and r["deferred"] == [] and r["next"] is None


def test_per_tick_is_overridable_and_reported(tmp_path):
    root = lane(tmp_path)
    for i, paths in enumerate((PLUGIN, ADMIN, TOOLS)):
        brief(root, "queued", "chip-%d" % i, created="2026-08-0%d" % (i + 1),
              touches=paths)
    r = mde.evaluate(root, per_tick=1)
    assert r["per_tick_cap"] == 1 and [c["id"] for c in r["candidates"]] == ["chip-0"]
    assert [d["blocked_by"] for d in r["deferred"]] == ["per-tick-cap", "per-tick-cap"]


def test_one_candidate_still_renders_the_original_next_line(tmp_path):
    """One card is still the common tick, so its line keeps the shape a reader learned —
    id, arrow, the SPAWN title, which carries the tick's local `HH:MM` (#4123)."""
    root = lane(tmp_path)
    brief(root, "queued", "only-one")
    r = mde.evaluate(root)
    out = mde.render_text(r)
    assert ("  NEXT: only-one  →  %s" % r["next"]["chip_title_timed"]) in out
    assert r["next"]["chip_title_timed"].endswith("only one")
    assert "\n    1. " not in out          # no numbered list for a single card


def test_a_multi_card_tick_lists_every_card_for_one_poke(tmp_path):
    """Step 4d's poke lists every card prepared this tick, oldest first, so the operator
    taps them in one pass instead of reading N notifications."""
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=ADMIN)
    out = mde.render_text(mde.evaluate(root))
    assert "NEXT: 2 cards this tick, oldest first" in out
    assert "ONE poke listing every one." in out
    lines = [ln for ln in out.splitlines() if ln.startswith("    ")]
    assert lines[0].startswith("    1. a-one  →  ")
    assert lines[1].startswith("    2. b-two  →  ")


def test_the_dry_run_plan_covers_every_card(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01", touches=PLUGIN)
    brief(root, "queued", "b-two", created="2026-08-02", touches=ADMIN)
    out = mde.render_text(mde.evaluate(root), dry_run=True)
    assert "card 1 of 2" in out and "card 2 of 2" in out
    assert "touches    : packages/plugin/src/observer/TurnObserver.ts" in out
    assert out.count("body head  :") == 2


def test_a_grouped_deferral_prints_once_with_its_count(tmp_path):
    """Thirty copies of one sentence is a report nobody reads. A reason that is a fact
    about the TICK collapses to one line; a brief's own collision still prints in full."""
    root = lane(tmp_path)
    brief(root, "queued", "a-one", created="2026-08-01")            # no touches: alone
    for i in range(5):
        brief(root, "queued", "chip-%d" % i, created="2026-08-1%d" % i, touches=ADMIN)
    out = mde.render_text(mde.evaluate(root))
    assert out.count("held for files:") == 1
    assert "5 brief(s) wait for the next tick" in out


# ── `touches:` as a schema field ─────────────────────────────────────────────


def test_touches_is_parsed_and_normalized(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "a-one", touches=["./tools/meta-queue", "docs/help/"])
    r = mde.evaluate(root)
    assert r["next"]["touches"] == ["tools/meta-queue", "docs/help"]


def test_an_absent_touches_is_none_not_an_empty_list(tmp_path):
    """The two absent-ish values mean opposite things, so the payload must keep them
    apart: None is "touches everything", and `[]` never reaches a reader at all."""
    root = lane(tmp_path)
    brief(root, "queued", "a-one")
    assert mde.evaluate(root)["next"]["touches"] is None


def test_an_empty_touches_list_is_a_schema_error(tmp_path):
    """A half-written file set is the one input this tool refuses to guess at. Read as
    "touches nothing" it would ride alongside every other brief — the worst reading."""
    root = lane(tmp_path)
    brief(root, "queued", "a-one", extra="touches:")
    r = mde.evaluate(root)
    assert r["next"] is None
    assert "touches is empty" in r["invalid"][0]["error"]


def test_a_non_list_touches_is_a_schema_error(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "a-one", extra="touches: packages/admin")
    r = mde.evaluate(root)
    assert r["next"] is None
    assert "touches must be a list" in r["invalid"][0]["error"]


def test_prefix_overlap_is_deliberately_not_segment_aware(tmp_path):
    """`packages/admin` and `packages/admin-ui` read as overlapping. A false overlap
    costs one brief one tick; a missed one costs a conflict and a re-review."""
    assert mde.prefixes_overlap("packages/admin", "packages/admin-ui") is True
    assert mde.prefixes_overlap("packages/admin", "packages/plugin") is False
    assert mde.overlapping_prefix(["a/b/c"], ["a/b"]) == "a/b"
    assert mde.overlapping_prefix(["a/b"], ["c/d"]) is None


# ── the `directed` class (D-AS2's lever) ──────────────────────────────────────
#
# The same gap as `repair`, in the other direction. On 2026-09-22 the operator
# ratified D-AS2 and directed a second track up alongside it; the briefs sat at
# positions 29, 41 and 44 of 86 and NOTHING in the lane could express either
# ruling. `repair` was unavailable (a directive unblocks no PR), which left
# back-dating `created` — the lie the class system exists to remove.


def decision_doc(tmp_path, rel="internal/decision-thing.md", text="D-AS2 ratified.\n"):
    doc = tmp_path / rel
...
def directed(root, ident, *, created, by="D-AS2 internal/decision-thing.md",
             since=None, **kw):
    """A `directed` brief. Callers pass RELATIVE dates (`days_ago`) for a directive
    that is meant to be LIVE: unlike a repair, a directive's only exit is the window,
    so a fixed date in the past silently expires the fixture the moment the calendar
    passes it (the clock-coupling that reds a suite overnight)."""
    extra = "class: directed\ndirected_by: %s" % by
    if since is not None:
        extra += "\ndirected_since: %s" % since
    return brief(root, "queued", ident, created=created, extra=extra, **kw)


def test_directed_sorts_ahead_of_backlog_behind_repair(tmp_path):
    """The rank, stated once: a repair unblocks work already in motion, a directive
    reorders work that has not started, so the half-built thing goes first."""
    decision_doc(tmp_path)
    root = lane(tmp_path)
    brief(root, "queued", "a-backlog", created="2026-08-01")
    directed(root, "b-directed", created=days_ago(3))
    repair(root, "c-repair", 4137, created=days_ago(0))
    assert order(root) == ["c-repair", "b-directed", "a-backlog"]


def test_directed_sorts_oldest_first_within_the_class(tmp_path):
    decision_doc(tmp_path)
    root = lane(tmp_path)
    directed(root, "newer", created=days_ago(1))
    directed(root, "older", created=days_ago(5))
    brief(root, "queued", "backlog", created="2026-07-01")
    assert order(root) == ["older", "newer", "backlog"]


def test_directed_without_directed_by_is_refused(tmp_path):
    """A priority claim nobody can check is a mood, not a class."""
    root = lane(tmp_path)
    brief(root, "queued", "unsourced", extra="class: directed")
    r = mde.evaluate(root)
    assert r["eligible"] == []
    assert "must name the ratified decision" in r["invalid"][0]["error"]


def test_directed_by_without_the_class_is_refused(tmp_path):
    """A field that silently does nothing is the worst of both."""
    decision_doc(tmp_path)
    root = lane(tmp_path)
    brief(root, "queued", "half-classed",
          extra="directed_by: D-AS2 internal/decision-thing.md")
    r = mde.evaluate(root)
    assert "without `class: directed`" in r["invalid"][0]["error"]


def test_directed_since_without_the_class_is_refused(tmp_path):
    root = lane(tmp_path)
    brief(root, "queued", "half-classed", extra="directed_since: 2026-09-01")
    r = mde.evaluate(root)
    assert "without `class: directed`" in r["invalid"][0]["error"]


def test_directed_by_naming_an_absent_decision_is_refused(tmp_path):
    root = lane(tmp_path)
    directed(root, "points-nowhere", created="2026-09-01",
             by="D-AS2 internal/decision-that-does-not-exist.md")
    r = mde.evaluate(root)
    assert r["eligible"] == []
    assert "does not exist" in r["invalid"][0]["error"]


def test_directed_by_naming_a_document_that_omits_the_id_is_refused(tmp_path):
    """The sharper of the two failures: the document is there and does not record
    the ruling, which a plain "not found" would have hidden."""
    decision_doc(tmp_path, text="This document rules on D-TM11 and nothing else.\n")
    root = lane(tmp_path)
    directed(root, "wrong-doc", created="2026-09-01")
    r = mde.evaluate(root)
    assert "does not mention it" in r["invalid"][0]["error"]


def test_directed_by_must_name_a_document_not_just_a_decision(tmp_path):
    root = lane(tmp_path)
    directed(root, "id-only", created="2026-09-01", by="D-AS2")
    r = mde.evaluate(root)
    assert "names no document" in r["invalid"][0]["error"]


def test_directed_expires_after_the_window_measured_from_directed_since(tmp_path):
    """The window bounds how long a brief may HOLD the priority, not how long the
    brief has existed — so an August brief directed today is zero days in."""
    decision_doc(tmp_path)
    root = lane(tmp_path)
    brief(root, "queued", "backlog", created="2026-08-01")
    directed(root, "old-brief-new-directive",
             created=days_ago(mde.DIRECTED_WINDOW_DAYS * 3), since=days_ago(0))
    assert order(root)[0] == "old-brief-new-directive"

    root2 = lane(tmp_path / "second")
    directed(root2, "stale-directive",
             created=days_ago(mde.DIRECTED_WINDOW_DAYS * 3),
             since=days_ago(mde.DIRECTED_WINDOW_DAYS + 1))
    r = mde.evaluate(root2)
    (e,) = r["eligible"]
    assert e["class_effective"] == mde.CLASS_BACKLOG
    assert e["class_expired_reason"] == "directed-window-elapsed"


def test_directed_since_defaults_to_created(tmp_path):
    decision_doc(tmp_path)
    root = lane(tmp_path)
    directed(root, "fresh", created=days_ago(1))
    directed(root, "ancient", created=days_ago(mde.DIRECTED_WINDOW_DAYS + 1))
    by_id = {e["id"]: e for e in mde.evaluate(root)["eligible"]}
    assert by_id["fresh"]["class_effective"] == mde.CLASS_DIRECTED
    assert by_id["ancient"]["class_effective"] == mde.CLASS_BACKLOG
    assert by_id["fresh"]["directed_since"] == by_id["fresh"]["created"]


def test_directed_since_cannot_predate_the_brief(tmp_path):
    decision_doc(tmp_path)
    root = lane(tmp_path)
    directed(root, "backdated", created="2026-09-01", since="2026-08-01")
    r = mde.evaluate(root)
    assert "cannot predate" in r["invalid"][0]["error"]


def test_expired_directive_falls_back_to_oldest_first_and_is_not_held(tmp_path):
    """An expiry changes RANK and nothing else — never held, hidden or dropped."""
    decision_doc(tmp_path)
    root = lane(tmp_path)
    # The expired directive is NEWER than the backlog brief, so falling back to
    # oldest-first must put it BEHIND — demoted, not dropped. (A directive that is
    # genuinely older would correctly sort first, which proves nothing here.)
    brief(root, "queued", "aaa-older-backlog", created=days_ago(60))
    directed(root, "zzz-expired", created=days_ago(30),
             since=days_ago(mde.DIRECTED_WINDOW_DAYS + 5))
    r = mde.evaluate(root)
    assert {e["id"] for e in r["eligible"]} == {"aaa-older-backlog", "zzz-expired"}
    assert order(root) == ["aaa-older-backlog", "zzz-expired"]
    assert r["held"] == []
    (exp,) = [e for e in r["eligible"] if e["id"] == "zzz-expired"]
    assert exp["class"] == mde.CLASS_DIRECTED          # what the brief declares
    assert exp["class_effective"] == mde.CLASS_BACKLOG  # what it sorts under now
    assert exp["class_expired_reason"] == "directed-window-elapsed"


def test_directed_brief_still_held_by_unmet_dependency(tmp_path):
    """A class is a sort key and only a sort key: every hold still applies."""
    decision_doc(tmp_path)
    root = lane(tmp_path)
    directed(root, "waits-on-something", created="2026-09-01",
             depends_on=["not-built-yet"])
    r = mde.evaluate(root)
    assert r["eligible"] == []
    (h,) = r["held"]
    assert h["reason"] == "depends-on-unmet"


def test_directed_privileged_brief_still_needs_greenlight(tmp_path):
    decision_doc(tmp_path)
    root = lane(tmp_path)
    directed(root, "priv-directive", created="2026-09-01", privileged=True)
    r = mde.evaluate(root)
    assert r["eligible"] == []
    (h,) = r["held"]
    assert h["reason"] == "privileged-without-greenlight"


def test_class_unknown_is_refused_not_sorted_last(tmp_path):
    """A typo'd class that quietly sorts last is indistinguishable from one at the
    front — the silent-control shape D-CS7 is about."""
    root = lane(tmp_path)
    brief(root, "queued", "typo-class", extra="class: directd")
    r = mde.evaluate(root)
    assert r["eligible"] == []
    assert "is not one of" in r["invalid"][0]["error"]
    assert "directed" in r["invalid"][0]["error"]


# ── D-MT3: the review tier is a function of the paths a PR touches ───────────
#
# Each row's paths are the ACTUAL `git diff --name-only <parent>..<merge-commit>` of a
# real merged PR from September 2026 (see the PR body for shas): #4277/#4392/#4402
# (oc_preflight.py), #4408/#4398 (breakers/cost_profiles), #4191 (peer_auth.py),
# #4423 (setup_wizard.py sudoers renderer), #4141 (preflight_prior.py / ModelRouter),
# #4372/#4311 (docs-only / lane tooling — neither is one of the four D-TP9 tiers).
TEN_REAL_SEPTEMBER_PRS = [
    (4277, ["packages/admin/evolve_admin/oc_preflight.py",
            "packages/admin/evolve_admin/oc_install_flags.py",
            "packages/admin/evolve_admin/oc_neutralize.py",
            "packages/admin/evolve_admin/health.py"], "upgrade_path"),
    (4408, ["packages/admin/evolve_admin/breakers_enforce.py",
            "packages/admin/evolve_admin/alerts/dispatcher.py",
            "packages/analyzer/cost_profiles.py"], "money"),
    (4392, ["packages/admin/evolve_admin/oc_preflight.py",
            "packages/admin/evolve_admin/oc_preflight_store.py"], "upgrade_path"),
    (4402, ["packages/admin/evolve_admin/oc_preflight.py",
            "packages/admin/evolve_admin/oc_preflight_store.py",
            "packages/admin/evolve_admin/health.py"], "upgrade_path"),
    (4372, ["internal/dispatch/queued/better-engine-log-path.md"], "none"),
    (4311, ["tools/meta-dispatch-eligible", "tools/meta-dispatch-move",
            "internal/dispatch/README.md", "CLAUDE.md"], "none"),
    (4191, ["packages/admin/evolve_admin/web/peer_auth.py",
            "packages/admin/evolve_admin/board_store.py",
            "packages/admin/evolve_admin/web/routes_board.py"], "credentials"),
    (4141, ["packages/analyzer/preflight_prior.py",
            "packages/plugin/dist/observer/ModelRouter.js",
            "packages/admin/evolve_admin/deploy.py"], "routing"),
    (4398, ["packages/analyzer/breakers/store.py",
            "packages/analyzer/breakers/audit_generator.py",
            "packages/admin/evolve_admin/health.py"], "money"),
    (4423, ["packages/admin/evolve_admin/setup_wizard.py",
            "packages/admin/tests/test_sudoers_argv_shape_drift.py"], "credentials"),
]


@pytest.mark.parametrize("pr,paths,expected", TEN_REAL_SEPTEMBER_PRS,
                         ids=[str(p) for p, _paths, _t in TEN_REAL_SEPTEMBER_PRS])
def test_tier_table_against_ten_real_september_prs(pr, paths, expected):
    assert mde.classify_tier(paths) == expected


@pytest.mark.parametrize("path", [
    "packages/admin/evolve_admin/deploy.py",
    "packages/admin/evolve_admin/user_tier_override.py",
])
def test_the_two_model_choosing_admin_files_are_routing_tier(path):
    """pr-4440 review finding 3: both decide which model answers a user turn."""
    assert mde.classify_tier([path]) == mde.TIER_ROUTING


def test_a_pr_touching_two_tiers_resolves_to_the_more_severe_one():
    """money outranks routing (TIER_PRIORITY): a PR that trips a breaker AND a router
    is reviewed as money, never split or silently downgraded to the lesser tier."""
    paths = ["packages/admin/evolve_admin/breakers_enforce.py",
             "packages/analyzer/preflight_prior.py"]
    assert mde.classify_tier(paths) == mde.TIER_MONEY


# ── D-MT3/D-MT4: the merge gate ───────────────────────────────────────────────


def _pr_json(head, pr, *, base="main", head_ref="feature"):
    return {"number": pr, "baseRefName": base, "headRefName": head_ref,
           "commits": [{"oid": head}]}


def _review(root, name, text):
    (root / "reviews" / name).write_text(text)


_FABLE_PASS = ("## Two-pass review\n\nReviewer: PM lane (Cowork, fable)\nModel: sonnet-5\n\n"
              "Verdict: PASS — fine.\n")
_OPUS_PASS = ("## Two-pass review\n\nReviewer: PM lane (Cowork, opus)\nModel: opus\n\n"
             "Verdict: PASS — fine.\n")


def test_merge_gate_refuses_a_stacked_pr(tmp_path):
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9001.md", _FABLE_PASS)
    head = _commit_push(work, "add review")
    v = mde.merge_qualifies(root, 9001, _pr_json(head, 9001, base="some-feature-branch"))
    assert v["qualifies"] is False
    assert any("base is" in r for r in v["refusals"])


def test_merge_gate_refuses_with_no_review_file_on_main(tmp_path):
    work, root, _ = _lane_repo(tmp_path)
    (root / "queued" / "unrelated.md").write_text("placeholder\n")
    head = _commit_push(work, "nothing to review")
    v = mde.merge_qualifies(root, 9002, _pr_json(head, 9002))
    assert v["qualifies"] is False
    assert any("no review file on main" in r for r in v["refusals"])


def test_merge_gate_qualifies_a_single_pass_pr(tmp_path):
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9003.md", _FABLE_PASS)
    head = _commit_push(work, "add review")
    v = mde.merge_qualifies(root, 9003, _pr_json(head, 9003))
    assert v["qualifies"] is True, v["refusals"]
    assert v["tier"] == mde.TIER_NONE
    assert v["second_pass_required"] is False


def test_merge_gate_refuses_a_risk_tier_pr_with_only_one_lanes_pass(tmp_path, monkeypatch):
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9004.md", _FABLE_PASS)
    head = _commit_push(work, "add review")
    monkeypatch.setattr(
        mde, "_changed_paths_from_git",
        lambda repo, base, h: (["packages/admin/evolve_admin/breakers_enforce.py"], None))
    v = mde.merge_qualifies(root, 9004, _pr_json(head, 9004))
    assert v["qualifies"] is False
    assert v["tier"] == mde.TIER_MONEY
    assert any("second pass" in r and "D-TP9" in r for r in v["refusals"])


def test_merge_gate_qualifies_a_risk_tier_pr_reviewed_by_two_different_lanes(
        tmp_path, monkeypatch):
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9005.md", _OPUS_PASS)
    _review(root, "pr-9005-second-pass-fable.md", _FABLE_PASS)
    head = _commit_push(work, "add both reviews")
    monkeypatch.setattr(
        mde, "_changed_paths_from_git",
        lambda repo, base, h: (["packages/admin/evolve_admin/breakers_enforce.py"], None))
    v = mde.merge_qualifies(root, 9005, _pr_json(head, 9005))
    assert v["qualifies"] is True, v["refusals"]
    assert {r["lane"] for r in v["reviews"]} == {"opus", "fable"}


def test_merge_gate_refuses_the_same_lane_twice_on_a_risk_tier_pr(tmp_path, monkeypatch):
    """Two files, one lane: the SECOND pass must be by the OTHER lane, so one lane's
    own PASS repeated across two files still does not satisfy D-TP9."""
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9006.md", _FABLE_PASS)
    _review(root, "pr-9006-recheck-fable.md", _FABLE_PASS)
    head = _commit_push(work, "same lane twice")
    monkeypatch.setattr(
        mde, "_changed_paths_from_git",
        lambda repo, base, h: (["packages/admin/evolve_admin/breakers_enforce.py"], None))
    v = mde.merge_qualifies(root, 9006, _pr_json(head, 9006))
    assert v["qualifies"] is False
    assert any("second pass" in r for r in v["refusals"])


def test_merge_gate_refuses_a_concerns_with_no_hold_as_malformed_not_pass_equivalent(
        tmp_path):
    """D-PM7: a CONCERNS with no `## Hold` states no remedy — it never merge-forwards,
    however plainly the finding names spend/deletion."""
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9007.md",
           "## Two-pass review\n\nReviewer: PM lane (Cowork, fable)\nModel: sonnet-5\n\n"
           "Verdict: CONCERNS — spends real money, no remedy stated.\n")
    head = _commit_push(work, "concerns no hold")
    v = mde.merge_qualifies(root, 9007, _pr_json(head, 9007))
    assert v["qualifies"] is False
    assert any("pass-equivalent verdict" in r for r in v["refusals"])


def test_merge_gate_qualifies_a_concerns_with_a_non_destructive_hold(tmp_path):
    """D-TP2 merge-forward: CONCERNS + a `## Hold` reads as pass-equivalent (the fix
    chip rides the same landing) — this is `_hold_decision`, reused not reinvented."""
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9008.md",
           "## Two-pass review\n\nReviewer: PM lane (Cowork, fable)\nModel: sonnet-5\n\n"
           "Verdict: CONCERNS — rename needed before merge.\n\n"
           "## Hold\n\nrename the helper in foo.py:12 to bar\n")
    head = _commit_push(work, "concerns with hold")
    v = mde.merge_qualifies(root, 9008, _pr_json(head, 9008))
    assert v["qualifies"] is True, v["refusals"]


@pytest.mark.parametrize("verdict", [
    "CONCERNS — held; not merged forward.",
    "CONCERNS — held on red CI; the change itself is correct.",
])
def test_merge_gate_refuses_a_concerns_the_reviewer_marked_held(tmp_path, verdict):
    """PM-RULINGS 09-19 (#4351): a CONCERNS that says HELD is the named exception to
    merge-forward — the fix goes on the PR's branch and is re-reviewed before merge.
    A `## Hold` under it still means "dispatch the fix chip", never "merge now"."""
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9013.md",
           "## Two-pass review\n\nReviewer: PM lane (Cowork, fable)\nModel: sonnet-5\n\n"
           "Verdict: %s\n\n## Hold\n\nadd the bookkeeping class on the branch\n" % verdict)
    head = _commit_push(work, "concerns held")
    v = mde.merge_qualifies(root, 9013, _pr_json(head, 9013))
    assert v["qualifies"] is False
    assert v["reviews"][0]["held"] is True
    assert v["reviews"][0]["pass_equivalent"] is False
    assert any("HELD" in r for r in v["refusals"])


def test_merge_gate_a_pass_saying_claims_held_is_still_a_pass(tmp_path):
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9014.md",
           "## Two-pass review\n\nReviewer: PM lane (Cowork, fable)\nModel: sonnet-5\n\n"
           "Verdict: PASS — all five claims held.\n")
    head = _commit_push(work, "pass held")
    v = mde.merge_qualifies(root, 9014, _pr_json(head, 9014))
    assert v["qualifies"] is True, v["refusals"]


def test_merge_gate_refuses_a_fail_verdict_even_with_a_hold_section(tmp_path):
    """D-PM7: FAIL / DO NOT MERGE is a human hold, never a fix-forward — a `## Hold`
    under it changes nothing."""
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9009.md",
           "## Two-pass review\n\nReviewer: PM lane (Cowork, fable)\nModel: sonnet-5\n\n"
           "Verdict: FAIL — do not merge, auth bypass.\n\n"
           "## Hold\n\nsee above\n")
    head = _commit_push(work, "fail with hold")
    v = mde.merge_qualifies(root, 9009, _pr_json(head, 9009))
    assert v["qualifies"] is False


def _priv_entry(ident, pr):
    body = "WHY: privileged merge-gate test."
    return ("---\nid: %s\naspect: substrate\ntitle: t\nprivileged: true\n"
           "created: 2026-09-06\npm: fable-cowork\npr: %d\nbody_sha256: %s\n---\n%s\n"
           % (ident, pr, mdi.body_digest(body), body))


def test_merge_gate_refuses_a_privileged_pr(tmp_path):
    work, root, _ = _lane_repo(tmp_path)
    (root / "inflight" / "priv-chip.md").write_text(_priv_entry("priv-chip", 9010))
    _review(root, "pr-9010.md", _FABLE_PASS)
    head = _commit_push(work, "privileged chip")
    v = mde.merge_qualifies(root, 9010, _pr_json(head, 9010))
    assert v["qualifies"] is False
    assert any("privileged" in r for r in v["refusals"])


def test_merge_gate_requires_pr_json():
    root = Path("/nonexistent-does-not-matter")
    v = mde.merge_qualifies(root, 9011, None)
    assert v["qualifies"] is False
    assert any("no PR metadata" in r for r in v["refusals"])


def test_merge_gate_cli_mode_matches_the_function(tmp_path, capsys):
    work, root, _ = _lane_repo(tmp_path)
    _review(root, "pr-9012.md", _FABLE_PASS)
    head = _commit_push(work, "add review")
    rc = mde.main(["--dir", str(root), "--merge-gate", "9012", "--pr-json",
                  json.dumps(_pr_json(head, 9012)), "--json"])
    assert rc == 0
    out = json.loads(capsys.readouterr().out)
    assert out["qualifies"] is True


def test_gh_py_merge_calls_the_shared_gate_by_import_not_copy():
    """D-MT3 item 2: `internal/pm/gh.py merge` must load `merge_qualifies` from
    tools/meta-dispatch-eligible (import, not a re-derived copy), refuse before its
    API call, and carry no override flag."""
    src = (_TOOL.parents[1] / "internal" / "pm" / "gh.py").read_text()
    assert "meta-dispatch-eligible" in src
    assert "merge_qualifies" in src
    assert "qualifies" in src and "raise SystemExit" in src
    assert "def merge(n,remote='origin'):" in src, \
        "merge() takes the PR number and the remote to read — no override flag"
    assert "--force" not in src.replace("No --force", "")


def test_gh_py_merge_passes_remote_through_to_fetch_and_gate():
    """pr-4440 review finding 2: the Fable lane's clone calls GitHub `gh`, not `origin`;
    `--remote` must reach both the `git fetch` and `merge_qualifies`, and the refusal
    must name what it read."""
    src = (_TOOL.parents[1] / "internal" / "pm" / "gh.py").read_text()
    assert "['git','fetch',remote,'main']" in src
    assert "merge_qualifies(repo,int(n),pr_json,remote=remote)" in src
    assert "'--remote'" in src
    assert "looked at %s" in src


# ── D-TP2: the bookkeeping class (pr-4440 review finding 1) ────────────────────


def _pr_head(work: Path, files: dict, branch: str = "feature") -> str:
    """Commit `files` on `branch` off main, push it, return its sha — the PR head. The
    working tree is left back on main so later main commits are unaffected."""
    subprocess.run(["git", "-C", str(work), "checkout", "-q", "-B", branch, "main"],
                   check=True)
    for rel, content in files.items():
        f = work / rel
        f.parent.mkdir(parents=True, exist_ok=True)
        f.write_text(content)
    head = _commit_push(work, "pr head", branch=branch)
    subprocess.run(["git", "-C", str(work), "checkout", "-q", "main"], check=True)
    return head


_DOCS_ONLY = {
    "internal/dispatch/reviews/pr-4433.md": "Verdict: PASS\n",
    "internal/dispatch/queued/some-brief.md": "---\nid: some-brief\n---\nWHY\n",
    "internal/dispatch/CARDS.md": "cards\n",
    "internal/followups/thing.md": "followup\n",
    "internal/pm/PM-STATE-opus.md": "state\n",
    "internal/pm/PM-RULINGS.md": "rulings\n",
    "internal/decision-something-2026-09-26.md": "decision\n",
}


@pytest.mark.parametrize("path,expected", [
    ("internal/dispatch/reviews/pr-1.md", True),
    ("internal/dispatch/queued/x.md", True),
    ("internal/dispatch/CARDS.md", True),
    ("internal/followups/x.md", True),
    ("internal/pm/PM-RULINGS.md", True),
    ("internal/decision-x.md", True),
    ("internal/pm/gh.py", False),
    ("internal/pm/pm-land.sh", False),
    ("internal/pm/sub/x.md", False),
    ("internal/specs/x.md", False),
    ("internal/dispatch/inflight/x.md", False),
    ("internal/dispatch/README.md", False),
    ("docs/help/x.md", False),
    ("tools/meta-dispatch-eligible", False),
])
def test_bookkeeping_path_table(path, expected):
    assert mde._is_bookkeeping_path(path) is expected


def test_merge_gate_qualifies_a_docs_only_pr_with_no_review_file(tmp_path):
    work, root, _ = _lane_repo(tmp_path)
    head = _pr_head(work, _DOCS_ONLY)
    v = mde.merge_qualifies(root, 9101, _pr_json(head, 9101))
    assert v["qualifies"] is True, v["refusals"]
    assert v["bookkeeping"] is True
    assert v["bookkeeping_paths_outside"] == []
    assert v["reviews"] == []


def test_merge_gate_refuses_a_docs_pr_that_also_touches_gh_py(tmp_path):
    """Code under internal/pm/ is lane tooling, not paperwork: one line in gh.py makes
    the docs PR a code PR, and the refusal names the file that did it."""
    work, root, _ = _lane_repo(tmp_path)
    head = _pr_head(work, dict(_DOCS_ONLY, **{"internal/pm/gh.py": "x = 1\n"}))
    v = mde.merge_qualifies(root, 9102, _pr_json(head, 9102))
    assert v["qualifies"] is False
    assert v["bookkeeping"] is False
    assert v["bookkeeping_paths_outside"] == ["internal/pm/gh.py"]
    assert any("no review file on main" in r for r in v["refusals"])


def test_merge_gate_refuses_a_docs_only_stacked_pr(tmp_path):
    work, root, _ = _lane_repo(tmp_path)
    head = _pr_head(work, _DOCS_ONLY)
    v = mde.merge_qualifies(root, 9103, _pr_json(head, 9103, base="some-feature"))
    assert v["bookkeeping"] is True
    assert v["qualifies"] is False
    assert any("base is" in r for r in v["refusals"])


def test_merge_gate_refuses_a_docs_only_privileged_pr(tmp_path):
    work, root, _ = _lane_repo(tmp_path)
    (root / "inflight" / "priv-docs.md").write_text(_priv_entry("priv-docs", 9104))
    _commit_push(work, "privileged lane entry")
    head = _pr_head(work, _DOCS_ONLY)
    v = mde.merge_qualifies(root, 9104, _pr_json(head, 9104))
    assert v["bookkeeping"] is True
    assert v["qualifies"] is False
    assert any("privileged" in r for r in v["refusals"])


def test_merge_gate_refuses_a_docs_only_operator_merge_pr(tmp_path, monkeypatch):
    work, root, _ = _lane_repo(tmp_path)
    head = _pr_head(work, _DOCS_ONLY)
    monkeypatch.setattr(mde, "_merge_operator_merge", lambda pr, led: (True, "ledger"))
    v = mde.merge_qualifies(root, 9105, _pr_json(head, 9105))
    assert v["bookkeeping"] is True
    assert v["qualifies"] is False
    assert any("operator_merge" in r for r in v["refusals"])


def test_merge_gate_empty_diff_is_not_bookkeeping_and_still_refuses(tmp_path):
    """`all()` over zero paths is True — an empty diff must not waive the review file."""
    work, root, _ = _lane_repo(tmp_path)
    head = subprocess.run(["git", "-C", str(work), "rev-parse", "HEAD"],
                          capture_output=True, text=True).stdout.strip()
    v = mde.merge_qualifies(root, 9106, _pr_json(head, 9106))
    assert v["bookkeeping"] is False
    assert v["qualifies"] is False
    assert any("no review file on main" in r for r in v["refusals"])


def test_merge_gate_cli_prints_the_bookkeeping_fields(tmp_path, capsys):
    work, root, _ = _lane_repo(tmp_path)
    head = _pr_head(work, dict(_DOCS_ONLY, **{"internal/pm/gh.py": "x = 1\n"}))
    pj = json.dumps(_pr_json(head, 9107))
    rc = mde.main(["--dir", str(root), "--merge-gate", "9107", "--pr-json", pj, "--json"])
    assert rc == 1
    out = json.loads(capsys.readouterr().out)
    assert out["bookkeeping"] is False
    assert out["bookkeeping_paths_outside"] == ["internal/pm/gh.py"]
    rc = mde.main(["--dir", str(root), "--merge-gate", "9107", "--pr-json", pj])
    assert rc == 1
    text = capsys.readouterr().out
    assert "bookkeeping: false" in text
    assert "bookkeeping_paths_outside: internal/pm/gh.py" in text
