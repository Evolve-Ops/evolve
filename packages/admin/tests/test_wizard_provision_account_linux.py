...
def test_linux_missing_unit_uses_systemd_service_path(monkeypatch) -> None:
    _, printed = _drive(monkeypatch, LINUX, venv_present=False)
    joined = "\n".join(printed)
    # Derived from per_bot_evolve_plist_labels, so this asserts the SUFFIX +
    # DIR are platform-keyed, not a specific (retirable) daemon name.
    assert (
        f"/etc/systemd/system/ai.openclaw.evolve.cost-converter.{BOT}.service"
        in joined
    )
    assert "ai.openclaw.evolve.apply." not in joined, (
        "the retired apply daemon is being reported as missing"
    )
    assert "ai.openclaw.evolve.test." not in joined, (
        "the retired app-test daemon is being reported as missing"
    )
...
def test_macos_missing_unit_uses_launchd_plist_path(monkeypatch) -> None:
    ...
    joined = "\n".join(printed)
    assert (
        f"/Library/LaunchDaemons/ai.openclaw.evolve.cost-converter.{BOT}.plist"
        in joined
    )
    assert "ai.openclaw.evolve.apply." not in joined
