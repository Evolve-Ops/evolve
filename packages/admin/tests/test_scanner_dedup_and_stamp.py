...
# -- AL-1.4b: owned_by rewrite stays in the pkg_id namespace -----------------


def test_owned_by_rewrite_is_pkg_id_only_and_no_ops_without_a_pkg_id() -> None:
    """AL-1.4b left the ``winner``/``loser`` ``pkg_id`` reads in
    ``_merge_two_manifests`` unswept, and this pins the behavior that
    protects.

    ``files[*].owned_by`` is written in the pkg_id namespace. Resolving
    canonically would let a manifest with no ``pkg_id`` fall back to its
    ``id``, and the rewrite below would then fire with a value no
    ``owned_by`` tag has ever held -- silently retagging files to a key
    ``scanner_repair._gather_live_pkg_ids`` cannot see.
    """
    from evolve_admin.applications.scanner import _merge_two_manifests

    winner_pkg = {
        "id": "task-manager", "pkg_id": "p-cccccccc", "name": "Task Manager",
        "files": [{"path": "a.py", "owned_by": "p-cccccccc"}],
    }
    loser_pkg = {
        "id": "task-mgr", "pkg_id": "p-dddddddd", "name": "Task Mgr",
        "files": [{"path": "b.py", "owned_by": "p-dddddddd"}],
    }
    merged = _merge_two_manifests(winner_pkg, loser_pkg)
    by_path = {f["path"]: f for f in merged["files"]}
    assert by_path["b.py"]["owned_by"] == "p-cccccccc", "loser tag must be rewritten"

    # No pkg_id on either side -> the rewrite must NOT fire, even though both
    # manifests have perfectly good (and different) ``id`` values.
    winner_nopkg = {
        "id": "task-manager", "name": "Task Manager",
        "files": [{"path": "a.py", "owned_by": ""}],
    }
    loser_nopkg = {
        "id": "task-mgr", "name": "Task Mgr",
        "files": [{"path": "b.py", "owned_by": "task-mgr"}],
    }
    merged2 = _merge_two_manifests(winner_nopkg, loser_nopkg)
    by_path2 = {f["path"]: f for f in merged2["files"]}
    assert by_path2["b.py"]["owned_by"] == "task-mgr", "no pkg_id -> no rewrite"


def test_merged_from_records_the_filename_stem_not_the_app_identity() -> None:
    """``merged_from`` is an operator-audit breadcrumb naming the file that
    went away: ``id`` is the stem for legacy manifests, ``instance_id`` is the
    stem for v7-arc Instances, and the sibling site in ``_dedup_manifests``
    appends ``loser["stem"]`` literally. AL-1.4b keeps that chain for exactly
    that reason -- resolving canonically would emit the SPEC id for every
    v7-arc loser and diverge from the sibling.
    """
    from evolve_admin.applications.scanner import _merge_two_manifests

    winner = {"id": "keeper", "name": "Keeper"}
    legacy_loser = {"id": "goner", "pkg_id": "p-cccccccc", "name": "Goner"}
    assert _merge_two_manifests(winner, legacy_loser)["merged_from"] == ["goner"]

    v7_loser = {
        "manifest_shape": "v7-arc", "instance_id": "i-1111",
        "provenance": {"spec_id": "p-aaaaaaaa", "spec_version": 1},
    }
    assert _merge_two_manifests(winner, v7_loser)["merged_from"] == ["i-1111"]
