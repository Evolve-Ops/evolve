...
def test_the_reader_promises_exactly_the_marker_drops_the_writer_performs(tmp_path):
    """The no-fork property for the THIRD shape (2026-09-10), in the direction it has.

    `tools/meta-dispatch-eligible` REPORTS an `inflight/`+`done/` pair with a merged `pr:`
    in `repairable[]` and `tools/meta-dispatch-move done` DROPS the marker. The two are
    NOT the same set here, and deliberately so: `done` is also the operator's explicit
    close-out verb, called by hand on a chip that was abandoned rather than merged, so it
    accepts pairs the unattended reader must never release on its own.

    **The guarantee is one-directional: reader ⊆ writer.** A reader promising a drop the
    writer refuses is a lane that reports itself healthy and stays stuck — that is the
    direction this pins. The reverse is the reader being narrower, which is the whole
    design, so `unmerged-marker` is carried as its named witness rather than left to
    read as drift.

    The one predicate they DO share is containment: the writer's
    `_check_durable_carries_the_brief` and the reader's `classify_merged_chip_marker` must
    refuse `diverged-marker` together, because a merged PR does not make a `done/` body
    that lost the brief safe to drop text against.
    """
    mdm = _load("meta-dispatch-move", "meta_dispatch_move_marker_pin")
    mde = _load("meta-dispatch-eligible", "meta_dispatch_eligible_marker_pin")
    ...
    # (id, layout) — two the merge explains, three it does not. `merged` is what the run
    # positively observed; #77 is deliberately absent from it (open, closed-unmerged or
    # simply unobserved — the injection carries merged numbers and nothing else).
    merged = [4183]
    reader_releases = {"merged-marker", "merged-marker-appended"}
    writer_refuses = {"diverged-marker"}
    cases = {
        "merged-marker": lambda r: (
            write(r, "inflight", "merged-marker", "WHY: x.", stamp=True),
            write(r, "done", "merged-marker", "WHY: x.", pr=4183)),
        "merged-marker-appended": lambda r: (
            write(r, "inflight", "merged-marker-appended", "WHY: x.", stamp=True),
            write(r, "done", "merged-marker-appended", "WHY: x.\n\nOUTCOME: merged.",
                  pr=4183)),
        "unmerged-marker": lambda r: (
            write(r, "inflight", "unmerged-marker", "WHY: x.", stamp=True),
            write(r, "done", "unmerged-marker", "WHY: x.", pr=77)),
        "unbound-marker": lambda r: (
            write(r, "inflight", "unbound-marker", "WHY: x.", stamp=True),
            write(r, "done", "unbound-marker", "WHY: x.")),
        "diverged-marker": lambda r: (
            write(r, "inflight", "diverged-marker", "WHY: x.", stamp=True),
            write(r, "done", "diverged-marker", "WHY: something else.", pr=4183)),
    }
    ...
    for ident, lay in cases.items():
        ...
        reported = {rp["id"] for rp in
                    mde.evaluate(root, facts={"merged_prs": merged})["repairable"]}
        try:
            # `method` is the discriminator: an already-absent marker returns
            # `already-absent` (the verb is idempotent) and that is NOT a drop.
            performed = mdm.done(root, ident)["method"] == "plain"
        except mdm.Refused:
            performed = False

        assert (ident in reported) == (ident in reader_releases), ident
        if ident in reported:
            assert performed, "%s: the reader promised a drop the writer refused" % ident
        assert performed is not (ident in writer_refuses), ident

    # The named asymmetry, stated rather than inferred: the writer WOULD drop this one on
    # an explicit operator call, and the reader still refuses to release it unattended.
    assert "unmerged-marker" not in reader_releases


# ── `dispatched_at` — one spelling, three readers (2026-09-07) ───────────────
#
# `tools/meta-dispatch-move` WRITES the stamp at `launch`, `tools/meta-dispatch-eligible`
# renders the lane's "cards waiting for a tap" line from it, and `tools/meta-queue`
# renders group (E) from the copy in the meta-state chip row. It lives here for the same
# reason `split_front_matter` does: three parsers for one field is three chances to
# disagree about whether a card is three hours old or unreadable.


def test_dispatch_stamp_is_second_resolution_utc_with_a_literal_z():
    """Same shape as the mover's `now` verb and its heartbeat's `run`, so the three
    stamps a tick produces are comparable without a format negotiation."""
    at = mdi.dispatch_stamp(datetime.datetime(2026, 9, 7, 21, 4, 22, 987654,
                                              tzinfo=datetime.timezone.utc))
    assert at == "2026-09-07T21:04:22Z"


def test_dispatch_stamp_converts_a_non_utc_instant_rather_than_relabelling_it():
    tz = datetime.timezone(datetime.timedelta(hours=-7))
    at = mdi.dispatch_stamp(datetime.datetime(2026, 9, 7, 18, 30, tzinfo=tz))
    assert at == "2026-09-08T01:30:00Z"


def test_parse_dispatch_stamp_rejects_a_naive_value():
    """A local wall clock wearing a `Z` is how a whole timezone offset gets reported as
    an age. Rejected outright: an unreadable stamp costs the AGE, never the line."""
    assert mdi.parse_dispatch_stamp("2026-09-07T21:04:22") is None
    assert mdi.parse_dispatch_stamp("2026-09-07 21:04:22") is None


@pytest.mark.parametrize("bad", [None, "", "   ", "not-a-time", 20260907, True, [],
                                 "2026-13-01T00:00:00Z"])
def test_parse_dispatch_stamp_never_raises_on_junk(bad):
    assert mdi.parse_dispatch_stamp(bad) is None


@pytest.mark.parametrize("raw", ["2026-09-07T21:04:22Z", "2026-09-07T21:04:22z",
                                 "2026-09-07T14:04:22-07:00", "2026-09-07T21:04:22+00:00"])
def test_parse_dispatch_stamp_normalizes_every_accepted_spelling_to_one_instant(raw):
    got = mdi.parse_dispatch_stamp(raw)
    assert got == datetime.datetime(2026, 9, 7, 21, 4, 22, tzinfo=datetime.timezone.utc)


def test_age_hours_floors_and_clamps():
    """Floored, so a card prepared 40 minutes ago reads `0h` — the honest answer to
    "did this hour's tick make it?". Clamped at 0, so a stamp from the future (a clock
    the run cannot trust) is not reported as a negative age."""
    now = datetime.datetime(2026, 9, 7, 18, 30, tzinfo=datetime.timezone.utc)
    assert mdi.age_hours("2026-09-07T17:50:00Z", now) == 0
    assert mdi.age_hours("2026-09-07T18:30:00Z", now) == 0
    assert mdi.age_hours("2026-09-07T15:29:00Z", now) == 3
    assert mdi.age_hours("2026-09-06T15:30:00Z", now) == 27
    assert mdi.age_hours("2026-09-07T20:00:00Z", now) == 0     # future stamp, clamped


def test_age_label_says_question_mark_rather_than_omitting_the_card():
    """The card whose bookkeeping went wrong is precisely the one the operator has
    lost, so it must still occupy a slot on the waiting line."""
    assert mdi.age_label(None) == "?h"
    assert mdi.age_label("garbage") == "?h"
    now = datetime.datetime(2026, 9, 7, 18, 30, tzinfo=datetime.timezone.utc)
    assert mdi.age_label("2026-09-07T15:30:00Z", now) == "3h"


def test_local_hhmm_renders_the_host_zone_and_none_for_junk():
    at = "2026-09-07T21:04:22Z"
    expected = (datetime.datetime(2026, 9, 7, 21, 4, 22, tzinfo=datetime.timezone.utc)
                .astimezone().strftime("%H:%M"))
    assert mdi.local_hhmm(at) == expected
    assert mdi.local_hhmm("2026-09-07T21:04:22") is None      # naive: rejected
    assert mdi.local_hhmm(None) is None


def test_set_front_matter_field_writes_dispatched_at_without_touching_the_body():
    """The whole reason `body_sha256` covers the body and not the file: the lane
    legitimately writes front matter at every transition."""
    text = ("---\nid: a-brief\naspect: apps\ntitle: t\nprivileged: false\n"
            "created: 2026-09-07\npm: fable-cowork\n---\nWHY: pinned.\n")
    before = mdi.check(text)
    after = mdi.set_front_matter_field(text, mdi.DISPATCHED_AT, "2026-09-07T21:04:22Z")
    after = mdi.set_front_matter_field(after, mdi.DISPATCHED, "2026-09-07")

    assert "dispatched_at: 2026-09-07T21:04:22Z" in after
    assert mdi.check(after).computed == before.computed
    assert mdi.split_front_matter(after).body == mdi.split_front_matter(text).body


def test_the_backgrounded_banner_id_is_parsed_and_a_bare_number_alone_is_not():
    """CLI v2.1.268 prints `backgrounded · 13440514` — a BARE NUMERIC id matching none of
    the prefixed shapes, so before 2026-09-12 every headless launch recorded
    `session: null` even when it started cleanly, and the operator had no handle for
    `claude logs <id>` / `claude stop <id>` when the stall poke told them to look.

    The second half is the guardrail: the bare-number shape is recognised ONLY next to the
    banner word. A loose `\\d+` alternation would also match a pid, a duration or a byte
    count, and a WRONG session id is worse than none — it is the #3849 orphan-id shape,
    a lane keyed on a value nothing else holds.
    """
    mdm = _load("meta-dispatch-move", "meta_dispatch_move_session_banner_pin")

    real = ("Starting background service…\n"
            "backgrounded · 13440514\n"
            "  claude agents             list sessions\n")
    assert mdm._parse_session_id(real) == "13440514"
    assert mdm._parse_session_id("", real) == "13440514", "stderr is read too"
    assert mdm._parse_session_id("backgrounded: abc123de") == "abc123de"

    # No banner word: nothing is claimed.
    assert mdm._parse_session_id("pid 13937 started in 4120 ms") is None
    assert mdm._parse_session_id("") is None

    # A prefixed id still wins when both are present — the loose shapes are tried first.
    both = "backgrounded · 13440514 (task_2d54e3e4)"
    assert mdm._parse_session_id(both) == "task_2d54e3e4"
