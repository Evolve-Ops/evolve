...
def _openai_target():
    """A non-Anthropic infra_llm target (#3466 flow-through)."""
...
def test_summary_has_expected_keys(tmp_path: Path) -> None:
    ...
    summary = _run_static_analysis_phase(
        ...
        critic_target=None, current_files=[],
    )
...
def test_constraint_critic_skipped_when_no_provider(tmp_path: Path) -> None:
    """No credentialed LLM provider (critic_target=None) → no LLM call →
    empty constraint findings. (The other two checks still run; they're
    pure-Python.)"""
    manifest = _manifest(
    ...
    summary = _run_static_analysis_phase(
        _job(), manifest, tmp_path,
        critic_target=None,  # no provider credentialed
        current_files=[],
    )
...
def test_constraint_critic_runs_when_target_present(tmp_path: Path) -> None:
    """With a resolved critic target, _call_llm is invoked. Patch it to
    avoid network — the target can be any credentialed provider (#3466);
    a non-Anthropic one proves the flow-through."""
    manifest = _manifest(
        constraints={"boundaries": ["Fail silently when API is unreachable"]},
    )
    canned = '[{"verdict":"absent","evidence":"no try/except around api call"}]'
    with patch("evolve_admin.applications.forge_engine._call_llm",
               return_value=canned):
        summary = _run_static_analysis_phase(
            _job(), manifest, tmp_path,
            critic_target=_openai_target(),
