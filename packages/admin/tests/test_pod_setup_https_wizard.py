...
def test_https_banner_link_opens_wizard_not_new_window():
    """The HTTPS-nudge banner's "Show me how" link must open the in-SPA
    wizard modal via onclick. The desktop shell's Tauri webview silently
    drops target="_blank" new-window requests, so a bare doc link is
    inert there; the href stays as a middle-click / no-JS fallback for
    real browsers."""
    html = _html()
    banner = re.search(
        r'<div id="https-banner".+?</div>',
        html, re.DOTALL,
    )
    assert banner, "https-banner markup not found"
    link = re.search(r'<a class="hb-link"[^>]+>', banner.group(0))
    assert link, "hb-link anchor not found in banner"
    tag = link.group(0)
    assert 'href="/docs/https-setup"' in tag, \
        "href fallback to the doc must stay for middle-click / no-JS"
    assert "event.preventDefault()" in tag and "openHttpsSetupWizard()" in tag, \
        "banner link must preventDefault and open the in-SPA wizard"


def test_wizard_modal_links_to_full_doc():
    """The wizard modal offers a plain same-tab link to the full doc for
    operators who want more than the three-step flow."""
    html = _html()
    sub = re.search(
        r'<div id="https-setup-modal-sub".+?</div>',
        html, re.DOTALL,
    )
    assert sub, "https-setup-modal-sub not found"
    assert 'href="/docs/https-setup"' in sub.group(0)
