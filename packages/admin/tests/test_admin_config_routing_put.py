...
def test_partial_put_cannot_clear_enabled_false(app):
    ...
    # The tier the caller sent lands as its ROLE (#3662 boundary translation).
    assert stored["maintenanceRole"] == "standard", "the intended edit still landed"
    assert not [k for k in stored if k.endswith("Tier")]
...
def test_tier_write_evicts_its_role_sibling(app):
    ...
    stored = json.loads(app["tiers_path"].read_text())["routing"]
    # #3662: the caller's tier edit lands ON the role slot (translated), so the
    # stale role is REPLACED rather than left to shadow the edit — and no
    # *Tier key is ever persisted (the runtime refuses the shape on sight).
    assert stored["maintenanceRole"] == "fast", "stale role would shadow the edit"
...
def test_full_spa_block_round_trips(app):
    ...
    (role-shaped since #3662; the card never PUTs a ``*Tier`` key)."""
    payload = {
        "enabled": True,
    ...
    # The response is the PROJECTED (tier-shaped) view the card renders from.
    projected = {
        "enabled": True,
...
def test_unprojectable_role_is_replaced_by_a_card_save(app):
    ...
    stored = json.loads(app["tiers_path"].read_text())["routing"]
    # The tier the card posted lands as its role — ``max`` replaced (#3662).
...
def test_read_modify_write_of_the_api_view_lands_on_a_migrated_bot(app):
    ...
    stored = json.loads(app["tiers_path"].read_text())["routing"]
    # The plugin resolves maintenanceRole ?? maintenanceTier — the edit landed
    # on the WINNING half (translated, #3662), replacing the stale ``power``.
...
def test_applier_shaped_rmw_lands_on_an_unprojectable_role(tmp_path, monkeypatch):
    ...
    # Both-halves rule keeps the caller's tier (the intent), which then lands
    # as its role (#3662) — the stale ``max`` is replaced, never shadowing.
    assert stored["maintenanceRole"] == "power", "stale role would win Role ?? Tier"
    assert not [k for k in stored if k.endswith("Tier")]
