...
Pins the contract that the internalized events no longer reach the
...
(meta.digest, meta.unclassified) keep dispatching. The mechanism under test:
  ...
  - The removed events set ``notify=False``; digest/unclassified (and
...
# The events internalized by this chip — produced, but no longer pushed to
# chat. Must match the notify=False entries in catalog.py exactly.
# ``decisions.proposal_rejected`` was a seventh until 2026-08-14, when it
# retired with review.py's last advertising surfaces (#3641 deleted its only
# emitter, so there was nothing left to internalize).
STOPPED_EVENTS = (
...
def test_removed_events_are_notify_false():
    """Each internalized event carries notify=False."""
...
def test_notify_for_resolves_legacy_and_unknown_keys():
    ...
    assert catalog.notify_for("decisions.proposal_applied") is False
...
def test_every_other_event_still_notifies():
    """Only the internalized events are notify=False; nothing else."""
    from evolve_admin.alerts import catalog
...
def test_stopped_event_is_not_chat_dispatched(env, event_key):
    ...
    out = d.send(
        ...
        source="apply", message="should not reach chat",
...
def test_proposal_applied_chat_suppressed_internal_record_preserved(env, tmp_path):
    """proposal_applied's chat message is stopped, but the internal
    proposals/applied/ record (written by apply.py, NOT the dispatcher)
    is untouched — the two paths are independent.

    This pins the preservation invariant: suppressing the chat push must
    not touch any internal record the apply feeds. We assert the
    independence directly — apply.py writes the proposal to disk and only
    THEN dispatches, so the dispatcher suppressing the alert cannot affect
    the on-disk record.

    (Anchored on ``decisions.proposal_rejected`` until 2026-08-14; that
    event retired with review.py, so the invariant moved to its live
    sibling in the same internalized set.)
    """
    d = env["dispatcher"]
    # The chat side: suppressed.
    out = d.send(
        shared_dir=env["shared"], network=env["network"],
        source="apply", message="📋 Proposal prop_1 applied",
        catalog_event="decisions.proposal_applied",
        dedup_key="apply/applied/prop_1",
    )
    assert out.result == d.DispatchResult.SUPPRESSED_DISABLED
    assert env["sent"] == []

    # The internal record side: a caller can still write + read its own
    # applied-proposal record independent of the (now-suppressed) alert.
    applied_dir = tmp_path / "proposals" / "applied"
    applied_dir.mkdir(parents=True)
    rec = applied_dir / "prop_1.json"
    rec.write_text('{"id": "prop_1", "status": "applied"}', encoding="utf-8")
    assert rec.exists(), "applied-proposal record must survive chat suppression"
