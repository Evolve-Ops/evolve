...
def mock_dispatch(monkeypatch: pytest.MonkeyPatch) -> dict[str, list]:
    ...
    # breakers_enforce now calls dispatcher.dispatch_message, which tries
    # direct Telegram HTTP first (breaker-notify-survives-openclaw-config-
    # validation) before falling back to the CLI mocked above. Force the
    # fallthrough deterministically rather than depending on whether
    # /Users/evolve/.openclaw/openclaw.json happens to exist on the machine
    # running the suite.
    monkeypatch.setattr(
...
class TestRender:
    ...
    def test_cost_trip_message_says_paused_not_halted(self) -> None:
        ...
        # Distinct from L2: this is "paused" not "halted" — the gateway
        # stays up.
        assert "paused" in msg.lower()
        assert "Spent $5.08 of $5.00 daily cap" in msg
        assert "2026-06-05T21:36:00+00:00" in msg
        # Operator decision D-CC2 (2026-09-04): the copy no longer promises
        # an unchanged conversation. It used to assert "talk to me" here,
        # while an undisclosed third action answered every remaining turn on
        # the cheapest model — see
        # internal/decision-cost-cap-checkpoint-2026-09-04.md.
        assert "talk to me" not in msg.lower()
...
class TestCostNotifiesUser:
    ...
    def test_cost_trip_posts_user_channel_pause_message(
        ...
        # The copy names the background actions it took (D-CC2) and no
        # longer promises an unchanged conversation.
        assert "heartbeat" in sent["message"].lower()
        assert "scheduled jobs" in sent["message"].lower()
        assert "talk to me" not in sent["message"].lower()
        # Enforcement was a no-op (no heartbeat.every to disable) but the
        # courtesy message happens regardless.
        assert result.no_op is True

    def test_cost_trip_message_offers_the_checkpoint_choices(
        self, network: dict, mock_dispatch: dict, tmp_path: Path,
    ) -> None:
        """When the cap-driven path supplies the structured action, the same
        post carries the two choices and the stated increment (D-CC3)."""
        home = tmp_path / "home"
        self._stub_empty_openclaw(home)
        ...
        breakers_enforce.enforce_trip(
            ...
            cap_action="checkpoint", cap_usd=20.0, spend_usd=20.57,
            increment_usd=10.0,
        )
        msg = mock_dispatch["sends"][0]["message"]
        assert '"continue"' in msg and '"stop"' in msg
        assert "$10.00" in msg
        assert "$20.57" in msg

    def test_cost_trip_ledgers_the_delivery_verdict(
        self, network: dict, mock_dispatch: dict, tmp_path: Path,
    ) -> None:
        """A cap trip that may not have reached anyone is not enforcement —
        every attempt lands a tri-state row in the breaker ledger."""
        ...
        breakers_enforce.enforce_trip(
            scope="team_bot_a", breaker_type="cost", network=network,
            shared_dir=tmp_path, home_override=home, reason="cap",
        )
        rows = [
        ...
        assert rows
        assert rows[0]["delivery"] in {
            "delivered", "not_delivered", "cannot_tell",
        }
