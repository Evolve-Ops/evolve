...
def test_new_intake_id_is_distinct_across_many_mints(tmp_path):
    """Many ids in one process must all be distinct.
    ...
    in one test drew the same id). 6 hex chars makes that ~1/16.7M, and
    passing shared_dir re-mints against what is already on disk.
    """
    from evolve_admin.intake import store

    ids = [store.new_intake_id(tmp_path) for _ in range(500)]
    assert len(set(ids)) == len(ids)

    # And with records actually on disk, a taken id is never handed back.
    for i in ids[:50]:
        store.write_intake(_make_intake(id=i), tmp_path)
    assert store.new_intake_id(tmp_path) not in set(ids[:50])
