...
def test_report_written_with_symlink(tmp_path):
    ...
    with patch.object(su, "_fetch_registry_metadata", return_value=fake_metadata), \
         ...
         patch.object(su, "gate_compat_contract",
                      return_value=(su.GateResult(ok=True, details={"state": "tested"}), [])):
        # gate_compat_contract is stubbed for the same reason gate_send_surface
        # is: this test is about report PERSISTENCE, and the compat gate's real
        # answer depends on a recorded contract run that has nothing to do with
        # writing a file. Its own behaviour is covered in test_oc_compat.py.
