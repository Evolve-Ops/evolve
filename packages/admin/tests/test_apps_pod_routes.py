"""Tests for the pod-first Apps reads (AL-1.8a, internal/build-AL-1.8a-apps-shell.md §4).
...
def test_detail_returns_bots_facts_and_absent_bots(pod):
    ...
    # ``evolve`` is NOT here (ALPHA-3a, audit P4): the pod's own service
    # account can hold a manifest, which is why it is in the bot list at all,
    # but it is never a place an operator installs an app — offering it as a
    # target is a remediation that cannot work.
    assert body["bots_without"] == [BOT_B]
