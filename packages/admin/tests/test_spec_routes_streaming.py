...
pin the contract:
  ...
  * ``_resolve_spec_target`` resolves the operator's configured tier2 by
    default and tier1 when ``power=True`` through the provider-agnostic
    infra_llm resolver — never a hardcoded literal, never a presumed
    provider. Anthropic targets stream; other providers single-shot
    through infra_llm.complete (#3466).
        ...
        monkeypatch.setattr(
            ...
            lambda power=False: (
                ...
                InfraLLMTarget(
                    provider="anthropic",
                    model=(
                        "anthropic/claude-opus-4-6" if power
...
def test_resolve_spec_target_defaults_to_tier2(monkeypatch):
    """Default path resolves the tier2 role through infra_llm — never a
    hardcoded literal or presumed provider. Operator's choice wins."""
    import infra_llm
    from infra_llm import InfraLLMTarget
    from evolve_admin.web import spec_routes as sr

    calls: list[tuple[str, dict]] = []
    resolved = InfraLLMTarget(
        provider="anthropic",
        model="anthropic/claude-sonnet-4-6-imagined",
        api_key="sk-fake",
    )

    def fake_resolve(role, *, network=None):
        calls.append((role, network))
        return resolved

    monkeypatch.setattr(infra_llm, "resolve_infra_llm", fake_resolve)
    ...
    tier, target = sr._resolve_spec_target(power=False)
    assert tier == "tier2"
    assert target is resolved
    assert calls == [("tier2", {"_test": True})]


def test_resolve_spec_target_power_uses_tier1(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda role, *, network=None: InfraLLMTarget(
            provider="anthropic", model=f"anthropic/{role}-model", api_key="sk-fake",
        ),
    )
    monkeypatch.setattr(
    ...
    tier, target = sr._resolve_spec_target(power=True)
    assert tier == "tier1"
    assert target.model == "anthropic/tier1-model"
    assert policy_calls and policy_calls[0][0] == "tier1"


def test_resolve_spec_target_errors_when_no_provider(monkeypatch):
    """No credentialed provider → loud RuntimeError with a fix-it
    message (the old Anthropic-only rejection is gone, #3466)."""
    import infra_llm
    from evolve_admin.web import spec_routes as sr

    monkeypatch.setattr(
        infra_llm, "resolve_infra_llm", lambda role, *, network=None: None
    )
    monkeypatch.setattr("models.check_tier_policy", lambda *a, **k: None)
    monkeypatch.setattr("evolve_config.load_config", lambda: {})

    with pytest.raises(RuntimeError, match="No LLM provider credentialed"):
        sr._resolve_spec_target(power=False)


def test_build_draft_events_non_anthropic_single_shot(tmp_path, monkeypatch):
    """A non-Anthropic target takes the infra_llm single-shot path: the
    full text arrives as one delta, tokens are estimated, and the draft
    parses — no Anthropic streaming involved (#3466)."""
    import infra_llm
    from infra_llm import InfraLLMTarget
    from evolve_admin.web import spec_routes as sr
    ...
    monkeypatch.setattr(sr, "_resolve_spec_target", lambda power=False: ("tier2", target))

    seen: dict = {}

    def fake_complete(t, *, prompt=None, system="", **kwargs):
        seen["provider"] = t.provider
        return json.dumps(_VALID_DRAFT_JSON)

    monkeypatch.setattr(infra_llm, "complete", fake_complete)

    events = list(sr._build_draft_events(
        description="A small app.", target_bots=[], shared_dir=tmp_path,
    ))
    ...
    assert model_event["model"] == "openai/gpt-4o-mini"
...
def test_build_draft_events_surfaces_no_provider_error(tmp_path, monkeypatch):
    """No LLM provider credentialed → single error event, no crash."""
    from evolve_admin.web import spec_routes as sr

    def _raise(power=False):
        raise RuntimeError("No LLM provider credentialed for the pod's primary bot.")

    monkeypatch.setattr(sr, "_resolve_spec_target", _raise)

    events = list(sr._build_draft_events(
        description="anything", target_bots=[], shared_dir=tmp_path,
    ))
    assert events[-1]["type"] == "error"
    assert "No LLM provider credentialed" in events[-1]["message"]
...
def test_post_specs_worker_failure_propagates_to_session(tmp_path, monkeypatch, sync_workers):
    ...
    def _raise(power=False):  # forces the no-provider error
        raise RuntimeError("No LLM provider credentialed for the pod's primary bot.")
    ...
    assert "No LLM provider credentialed" in detail["generation"]["error"]
...
def test_build_draft_events_forwards_keepalive(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sr, "_resolve_spec_target",
        lambda power=False: ("tier2", InfraLLMTarget(
            provider="anthropic",
