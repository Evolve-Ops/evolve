...
def test_two_identities_of_one_person_are_one_row_with_the_combined_count(
        monkeypatch):
    """The roster maps a Slack id and a Telegram id to one person: one row.

    The detail pane lists both identities. An id no person row holds keeps
    its own row, labelled with its identity kind. Counts are only summed.
    """
    from evolve_admin import roster_resolver

    monkeypatch.setattr(roster_resolver, "resolve_display_name",
                        lambda *a, **k: (None, "none"))
    network = {"bots": {"bot_a": {"primary_user": {
        "external_ids": {"slack": ["U1"], "telegram": ["555"]}}}},
        # The pod-admin bag holds U1 too; it is several people, so it never
        # decides who U1 is.
        "pod": {"admins": {"external_ids": {"slack": ["U1", "U2"]}}}}
    module = _users(3, top=[("slack:U1", 30), ("slack:U2", 25),
                            ("telegram:555", 20)])
    view = rd._module_view({}, module, [],
                           rd._name_resolver(network, Path("/nope")),
                           rd._person_grouper(network))
    rows = view["bars"]["rows"]
    assert [r["value"] for r in rows] == [50, 25]
    assert rows[1]["label"] == "Slack user · U2"
    assert view["detail"]["identities_merged_into_one_person"] == {
        "slack:U1": ["slack:U1", "telegram:555"]}
