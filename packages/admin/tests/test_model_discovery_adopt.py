...
def test_picker_roles_is_always_the_four_ladder_roles():
    # The former conditional Judge appendage died with the judge role
    # (judge-role collapse §5.4) — even a stale roles.judge entry in the pod
    # config must not resurrect the button.
    assert mda.picker_roles({"models": {"roles": {"standard": "sonnet-class"}}}) == [
        "fast", "standard", "power", "max",
    ]
    stale = mda.picker_roles(
        {"models": {"roles": {"standard": "sonnet-class", "judge": "haiku-class"}}})
    assert stale == ["fast", "standard", "power", "max"]
...
# ── Auto-update preview column (spec-model-auto-upgrade Phase 1, READ-ONLY) ────
# The card gains a "would this apply on its own?" verdict per row plus a
# card-level preview block. Nothing here applies anything — these tests pin that
# the surface is honest (a held row says why) and inert (config is untouched).

def _write_pricing(shared, rows):
    """Persist a pricing cache the cost guard reads off ($/token)."""
    import json

    (Path(shared) / "model-pricing.json").write_text(json.dumps({
        "models": [
...
def test_upgrade_row_carries_the_auto_update_verdict(shared):
    net = _net_with_rung("sonnet-class", ["anthropic/claude-sonnet-4-5"])
    _write_listings(shared, {"anthropic": ["claude-sonnet-4-5", "claude-sonnet-5"]})
    _write_pricing(shared, [("claude-sonnet-4-5", 3e-6, 15e-6),
                            ("claude-sonnet-5", 3e-6, 15e-6)])
    row = mda.list_version_upgrades(shared, net)[0]
    auto = row["auto_upgrade"]
    # Ships dark: eligible by the rules, but no scope has the toggle on.
    assert auto["verdict"] == "eligible"
    assert "turn automatic updates on" in auto["summary"]
    assert auto["scopes"] == ["pod"] and auto["enabled_scopes"] == []


def test_repriced_upgrade_row_reads_as_held_with_its_reason(shared):
    net = _net_with_rung("sonnet-class", ["anthropic/claude-sonnet-4-5"])
    _write_listings(shared, {"anthropic": ["claude-sonnet-4-5", "claude-sonnet-5"]})
    _write_pricing(shared, [("claude-sonnet-4-5", 3e-6, 15e-6),
                            ("claude-sonnet-5", 6e-6, 15e-6)])
    auto = mda.list_version_upgrades(shared, net)[0]["auto_upgrade"]
    assert auto["verdict"] == "held"
    assert auto["reason_code"] == "cost_regression"
    assert auto["durable"] is True
    assert "costs more" in auto["summary"]


def test_preview_reads_the_first_seen_ledger_and_never_writes_it(shared):
    """Phase 3a wiring: the card's read-only column shows the veto countdown
    from the persisted first-seen ledger, and this route READS only — the
    clock is advanced by the daily model_discovery generator run, because a
    clock advanced by page views would never start on a pod nobody opens."""
    import model_auto_upgrade as mau
    import model_upgrade_store as mus
    ...
    # Detected just now → inside the minVisibleDays floor, so the row that
    # read "eligible" with no ledger now reads as held with a countdown.
    key = mau.upgrade_key("anthropic", "sonnet-class", "claude-sonnet-5")
    mus.stamp_first_seen(Path(shared), [key])
    before = mus.read_first_seen(Path(shared))

    auto = mda.list_version_upgrades(shared, net)[0]["auto_upgrade"]
    assert auto["verdict"] == "held"
    assert auto["reason_code"] == "not_visible_long_enough"

    # Rendering the card advanced nothing.
    assert mus.read_first_seen(Path(shared)) == before


def test_unpriced_upgrade_row_is_held_not_waved_through(shared):
    net = _net_with_rung("sonnet-class", ["anthropic/claude-sonnet-4-5"])
    _write_listings(shared, {"anthropic": ["claude-sonnet-4-5", "claude-sonnet-5"]})
    auto = mda.list_version_upgrades(shared, net)[0]["auto_upgrade"]
    assert auto["verdict"] == "held"
    assert auto["reason_code"] == "target_price_unknown"


def test_discoveries_route_carries_the_preview_summary(tmp_path, shared, net_state):
    from evolve_admin.web.server import create_app
    ...
    app = create_app(network_path)
    app.config["TESTING"] = True
    payload = app.test_client().get("/api/models/discoveries").get_json()

    assert payload["upgrades"][0]["auto_upgrade"]["verdict"] == "held"
    auto = payload["auto_upgrade"]
    assert auto["considered"] == 1 and auto["heldCount"] == 1
    assert auto["governance"]["podEnabled"] is False
    assert "Pod defaults (off)" in auto["governance"]["note"]
    # Phase 1 mutates nothing — the models config is untouched by the preview
    # (create_app itself backfills unrelated top-level keys, so compare the
    # block this feature would ever write to).
    assert json.loads(network_path.read_text())["models"] == net["models"]


def test_preview_reports_a_provider_whose_listing_failed(shared):
    net = _net_with_rung("sonnet-class", ["anthropic/claude-sonnet-4-5"])
    md.write_listings_cache(
        Path(shared),
        [md.ProviderEnumeration(provider="anthropic", ok=False, reason="HTTP 503")],
        refreshed_at="2026-07-31T00:00:00Z",
    )
    summary = mda.auto_upgrade_summary(mda.auto_upgrade_report(shared, net))
    assert [p["provider"] for p in summary["skippedProviders"]] == ["anthropic"]
    assert "not the same as being up to date" in summary["skippedProviders"][0]["message"]


def test_held_item_with_no_row_still_reaches_the_card(shared, monkeypatch):
    """The card drops upgrade rows with no rung to extend; the preview summary
    must still carry them, or "never silently dropped" holds in the engine and
    fails at the only surface an operator looks at."""
    net = _net_with_rung("sonnet-class", ["anthropic/claude-sonnet-4-5"])
    _write_listings(shared, {"anthropic": ["claude-sonnet-4-5", "claude-sonnet-5"]})

    real = mda._compute_upgrades

    def _unlocated(shared_dir, network):
        return [
            md.VersionUpgrade(
                provider=u.provider, family=u.family,
                current_model=u.current_model, latest_model=u.latest_model,
                rung_slug="", roles=[],
            ) for u in real(shared_dir, network)
        ]

    monkeypatch.setattr(mda, "_compute_upgrades", _unlocated)
    rows, report = mda.version_upgrade_rows(shared, net)
    assert rows == []                       # nothing renderable
    summary = mda.auto_upgrade_summary(report)
    assert summary["considered"] == 1
    assert len(summary["heldWithoutRow"]) == 1
    assert summary["heldWithoutRow"][0]["holds"][0]["message"]


def test_card_still_renders_when_the_preview_engine_raises(shared, monkeypatch):
    """Fail-open: losing the preview must not take the upgrade rows with it."""
    net = _net_with_rung("sonnet-class", ["anthropic/claude-sonnet-4-5"])
    _write_listings(shared, {"anthropic": ["claude-sonnet-4-5", "claude-sonnet-5"]})
    import model_auto_upgrade as mau

    monkeypatch.setattr(mau, "compute_auto_upgrades", lambda *a, **k: 1 / 0)
    rows, report = mda.version_upgrade_rows(shared, net)
    assert report is None
    assert len(rows) == 1 and rows[0]["auto_upgrade"] is None
    assert mda.auto_upgrade_summary(None) is None
