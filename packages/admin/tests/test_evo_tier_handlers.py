...
def test_user_tier_override_enabled_reads_explicit_false(tmp_path, monkeypatch):
    ...
    Stubs the inner Path constructor cleanly so the recursion-prone
    `Path.__init__` monkeypatch isn't necessary — we redirect the
    /Users/<bot>/.openclaw/evolve-tiers.json path to our tmp dir."""
    from evolve_admin.evo.handlers import tier as tier_handler

    home = tmp_path / "fake-bot"
    oc_dir = home / ".openclaw"
    oc_dir.mkdir(parents=True)
    tiers_path = oc_dir / "evolve-tiers.json"

    # Stand a real Path object pointing at the tmp file; have the
    # handler use it whenever it asks for the fake-bot's tiers path.
    real_path_cls = Path

    class _PathStub:
        def __new__(cls, p):
            if isinstance(p, str) and "/Users/fake-bot/" in p:
                return real_path_cls(str(tiers_path))
            return real_path_cls(p)

    monkeypatch.setattr(tier_handler, "Path", _PathStub)

    # Case 1: enabled: false → False
    tiers_path.write_text(json.dumps({
        "userTierOverride": {"enabled": False},
    }))
    assert tier_handler._user_tier_override_enabled("fake-bot") is False

    # Case 2: enabled: true → True
    tiers_path.write_text(json.dumps({
        "userTierOverride": {"enabled": True},
    }))
    assert tier_handler._user_tier_override_enabled("fake-bot") is True
