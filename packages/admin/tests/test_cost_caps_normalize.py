...
def test_pod_thresholds_stock_downgrade_flips_instead_of_mapping(
    shared_dir, network_path, _be_config_importable,
):
    """Operator decision D-CC4 (2026-09-04) replaced this case.

    ``downgrade-tier`` was the value ``config.py`` scaffolded into every
    fresh network.json and the schema labelled "(recommended)" — an
    un-chosen default, not an operator's pick. This migration used to
    enshrine it as a ``tier_downgrade_usd`` rung, which made the un-chosen
    default permanent. It now flips to the checkpoint FIRST, so the rung is
    never written; see
    internal/decision-cost-cap-checkpoint-2026-09-04.md and
    packages/admin/tests/test_cost_cap_checkpoint.py for the full contract.
    """
    _write_network(network_path, {
        "bots": {},
        "thresholds": {
    ...
    from evolve_admin.migrations.cost_caps_normalize import run
    result = run(shared_dir, network_path)
    assert result.pod_spend_cap_action_flipped is True
    assert result.pod_tier_downgrade_migrated is False
    be = _read_be_config(shared_dir)
    assert not be["pod_defaults"]["budget"].get("tier_downgrade_usd")
    net = _read_network(network_path)
