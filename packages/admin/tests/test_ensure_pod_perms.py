...
class TestEnsurePodPerms:
    ...
    def test_apply_invokes_fixes_for_drift_only(self, tmp_path, monkeypatch):
        ...
        # Same hermetic reason: the role-cap ledger check resolves
        # {sharedDir}/cost/tier-usage from network.json, which this fixture
        # leaves at the canonical default — unstubbed, its apply would mkdir
        # outside tmp_path and its dir-owner fix would fail for want of an
        # `evolve` account on the runner.
        monkeypatch.setattr(
            deploy, "_check_tier_usage_dir",
            lambda shared_dir: [_PermCheck(
                category="tier-usage", target="t", ok=True, detail="ok")],
        )
...
# ── macOS Application Firewall allow-rule wiring ──────────────────────────────
#
# The board's tailnet listener bound fine and every phone request died with
# ENOTCONN because the firewall blocked the daemon's Homebrew interpreter
# (2026-09-04 phone test). The rule is now Evolve's, which only holds if the
# pod-wide pass actually asks for it — a check nothing calls is the same
# silence the incident already had.

class TestEnsurePodPermsIncludesFirewallCheck:

    def test_the_firewall_check_runs_in_the_pod_wide_pass(self, tmp_path, monkeypatch):
        from evolve_admin import firewall_allow as fw
        ...
        ok_check = _PermCheck(category="x", target="y", ok=True, detail="ok")
        fw_check = _PermCheck(
            category=fw.PERM_CHECK_CATEGORY, target="/some/Python", ok=True,
        )
        with patch.object(deploy, "_check_proposals_dir", return_value=[ok_check]):
            with patch.object(deploy, "_check_alerts_dir", return_value=[ok_check]):
                with patch.object(
                    deploy._fw, "check_daemon_firewall_allow",
                    return_value=[fw_check],
                ) as spy:
                    result = ensure_pod_perms(
                        bot_id=None, network_path=np, check_only=True,
                    )
        assert spy.call_count == 1
        assert fw_check in result.checks

    def test_drift_from_the_firewall_check_reaches_the_result(
        self, tmp_path, monkeypatch,
    ):
        """It is the drift list that the hourly pod_perms_drift_monitor turns
        into a Signal, so a check whose failure never lands there is invisible
        between deploys — the exact gap this fix closes."""
        from evolve_admin import firewall_allow as fw
        ...
        blocked = _PermCheck(
            category=fw.PERM_CHECK_CATEGORY, target="/some/Python", ok=False,
            detail="blocked", fix_description="unblock it",
        )
        with patch.object(deploy, "_check_proposals_dir", return_value=[]):
            with patch.object(deploy, "_check_alerts_dir", return_value=[]):
                with patch.object(
                    deploy._fw, "check_daemon_firewall_allow", return_value=[blocked],
                ):
                    result = ensure_pod_perms(
                        bot_id=None, network_path=np, check_only=True,
                    )
        assert blocked in result.drift
class TestPmProbePrivilegedHalf:
    """The PM probe's two pod-side artifacts (PR #4079's hold).

    `/etc/sudoers.d/pm-probe` grants root for ONE fixed-argv reader, so that
    file has to be there and has to be the rendered one — whatever lives at
    the granted path is what root runs. `/tmp/pm` has to be owned by the admin
    account, because /tmp is world-writable and sticky: an unowned `/tmp/pm`
    is any bot's to create, and the probe runs scripts staged there.

    Both checks are gated on the sudoers file existing, so a pod that never
    installed the probe gets an informational pass and no new files.
    """

    @pytest.fixture()
    def probe_paths(self, tmp_path, monkeypatch):
        from evolve_admin import pm_probe_install as ppi

        monkeypatch.setattr(ppi, "STAGING_DIR", str(tmp_path / "pm"))
        monkeypatch.setattr(ppi, "WRAPPER_PATH", str(tmp_path / "pm-probe-cat"))
        monkeypatch.setattr(ppi, "probe_sudoers_installed", lambda: True)
        monkeypatch.setattr(ppi, "admin_user", lambda: "opadmin")
        return ppi

    def test_both_checks_no_op_without_the_sudoers_file(self, monkeypatch):
        from evolve_admin import pm_probe_install as ppi

        monkeypatch.setattr(ppi, "probe_sudoers_installed", lambda: False)
        for check in (ppi.check_pm_probe_wrapper(), ppi.check_pm_staging_dir()):
            assert check.ok
            assert check.apply is None
            assert "not installed" in check.detail

    def test_an_unreadable_sudoers_dir_reads_as_not_installed(self, monkeypatch, tmp_path):
        """`/etc/sudoers.d` is 0700 on Linux: `exists()` RAISES there rather
        than answering False, and an uncaught raise here takes the whole
        ensure-pod-perms pass down for every non-root caller."""
        from evolve_admin import pm_probe_install as ppi

        blocked = tmp_path / "sudoers.d"
        blocked.mkdir(mode=0o700)
        blocked.chmod(0o000)
        monkeypatch.setattr(ppi, "SUDOERS_PATH", str(blocked / "pm-probe"))
        try:
            assert ppi.probe_sudoers_installed() is False
            assert ppi.check_pm_probe_wrapper().ok
            assert ppi.check_pm_staging_dir().ok
        finally:
            blocked.chmod(0o700)

    def test_staging_dir_missing_is_drift_with_a_fix(self, probe_paths):
        check = probe_paths.check_pm_staging_dir()
        assert not check.ok
        assert "missing" in check.detail
        assert callable(check.apply)

    def test_staging_dir_owned_by_someone_else_is_drift(self, probe_paths, monkeypatch):
        Path(probe_paths.STAGING_DIR).mkdir(mode=0o755)
        monkeypatch.setattr(
            probe_paths, "admin_user", lambda: "somebody-else-entirely")
        check = probe_paths.check_pm_staging_dir()
        assert not check.ok
        assert "expected 'somebody-else-entirely'" in check.detail

    def test_staging_dir_with_the_right_owner_and_mode_passes(self, probe_paths, monkeypatch):
        import pwd

        d = Path(probe_paths.STAGING_DIR)
        d.mkdir(mode=0o755)
        me = pwd.getpwuid(os.getuid()).pw_name
        monkeypatch.setattr(probe_paths, "admin_user", lambda: me)
        check = probe_paths.check_pm_staging_dir()
        assert check.ok, check.detail
        assert f"owner={me}" in check.detail

    def test_world_writable_staging_dir_is_drift(self, probe_paths, monkeypatch):
        """0777 is the shape /tmp hands you by default — and the one that lets
        a bot replace a staged script under the operator's nose."""
        import pwd
        ...
        check = probe_paths.check_pm_staging_dir()
        assert not check.ok
        assert "0o777" in check.detail

    def test_missing_reader_is_drift_because_the_grant_names_it(self, probe_paths):
        check = probe_paths.check_pm_probe_wrapper()
        assert not check.ok
        assert "missing" in check.detail
        assert check.apply is probe_paths.install_wrapper

    def test_stale_reader_content_is_drift(self, probe_paths):
        Path(probe_paths.WRAPPER_PATH).write_text("#!/usr/bin/python3\nprint('not it')\n")
        check = probe_paths.check_pm_probe_wrapper()
        assert not check.ok
        assert "content differs" in check.detail

    def test_rendered_content_satisfies_the_content_half(self, probe_paths):
        """The rendered bytes are what the check compares against — and a
        non-root owner is still drift on its own, because whatever lives at
        the granted path is what root runs."""
        from platform_profile import MACOS, set_profile

        set_profile(MACOS)
        w = Path(probe_paths.WRAPPER_PATH)
        w.write_text(probe_paths.render_wrapper())
        w.chmod(0o755)
        check = probe_paths.check_pm_probe_wrapper()
        assert "content differs" not in check.detail
        assert "mode=" not in check.detail
        if os.getuid() != 0:
            # A test file is owned by the runner, so only the owner half fails.
            assert not check.ok and "owner uid" in check.detail
        else:  # pragma: no cover — only a root runner reaches this
            assert check.ok, check.detail

    def test_pod_wide_pass_includes_both_checks(self, tmp_path, monkeypatch):
        from evolve_admin import pm_probe_install as ppi
        ...
        np.write_text(__import__("json").dumps(net))
        monkeypatch.setattr(deploy, "POD_CELLAR_ROOT", tmp_path / "no-cellar")
        sentinel = _PermCheck(category="pm-probe", target="sentinel", ok=True, detail="stubbed")
        monkeypatch.setattr(ppi, "check_pm_probe_wrapper", lambda: sentinel)
        monkeypatch.setattr(ppi, "check_pm_staging_dir", lambda: sentinel)
        with patch.object(deploy, "_check_bot_acl", return_value=[]):
            result = ensure_pod_perms(bot_id=None, network_path=np, check_only=True)
        assert sum(1 for c in result.checks if c.target == "sentinel") == 2
