...
class TestAclUserPresent:
    ...
    def test_resolves_source_form_perms_on_directory_acl(self, tmp_path):
        ...
        # read,write,delete,append,readattr,writeattr,readextattr,
...
class TestCheckApplyLock:
    def test_missing_lock_reports_drift(self, monkeypatch, tmp_path):
        monkeypatch.setattr(deploy, "APPLY_LOCK_TEMPLATE",
                            str(tmp_path / "apply_processed-{bot}.lock"))
        check = _check_apply_lock("admin_bot", "admin_bot")
        assert check.ok is False
        assert check.detail == "missing"
        assert callable(check.apply)

    def test_present_with_correct_owner_passes(self, monkeypatch, tmp_path):
        # We can't chown to a different user in a test, so pretend the current
        # user IS the bot user (using their real username).
        import pwd as _pwd
        current_user = _pwd.getpwuid(os.getuid()).pw_name
        lock = tmp_path / f"apply_processed-{current_user}.lock"
        lock.write_text("")
        monkeypatch.setattr(deploy, "APPLY_LOCK_TEMPLATE",
                            str(tmp_path / "apply_processed-{bot}.lock"))
        check = _check_apply_lock(current_user, current_user)
        assert check.ok is True
        assert "owner=" in check.detail

    def test_wrong_owner_reports_drift(self, monkeypatch, tmp_path):
        # The lock exists but is owned by the current user, while the call
        # asks for a *different* bot user → drift.
        import pwd as _pwd
        current_user = _pwd.getpwuid(os.getuid()).pw_name
        fake_bot = "nonexistent-bot-zzzz"
        if fake_bot == current_user:
            pytest.skip("test username collides with current user")
        lock = tmp_path / f"apply_processed-{fake_bot}.lock"
        lock.write_text("")
        monkeypatch.setattr(deploy, "APPLY_LOCK_TEMPLATE",
                            str(tmp_path / "apply_processed-{bot}.lock"))
        # bot_id=fake_bot names the lock; bot_user=fake_bot is the expected
        # owner; file is owned by current_user → drift
        check = _check_apply_lock(fake_bot, fake_bot)
        assert check.ok is False
        assert "owner is" in check.detail
        assert callable(check.apply)

    def test_bot_id_differs_from_bot_user_uses_bot_id_for_path(self, monkeypatch, tmp_path):
        # Regression: when a bot is renamed (e.g. team_bot_b→personal_bot_user), the lock file
        # must be named by bot_id ("team_bot_b") but owned by bot_user ("personal_bot_user").
        # Simulated here by naming the lock after bot_id and creating it owned
        # by the current OS user, then asserting the check looks at the right path.
        import pwd as _pwd
        current_user = _pwd.getpwuid(os.getuid()).pw_name
        bot_id = "renamed-bot-id"
        bot_user = current_user  # simulate: bot now runs as current OS user
        lock = tmp_path / f"apply_processed-{bot_id}.lock"
        lock.write_text("")
        monkeypatch.setattr(deploy, "APPLY_LOCK_TEMPLATE",
                            str(tmp_path / "apply_processed-{bot}.lock"))
        check = _check_apply_lock(bot_id, bot_user)
        # Lock exists at the bot_id path and is owned by bot_user → passes
        assert check.ok is True, f"expected ok, got: {check.detail}"


# ── _check_apply_plist + _read_plist_username ─────────────────────────────────


class TestReadPlistUsername:
    """Direct plistlib reader added 2026-06-06 to replace the sudo
    PlistBuddy call that silently yielded UserName='' when the caller
    lacked the sudo grant (pod_perms_drift_monitor running as evolve
    daemon hit this — every plist was reported as drifted)."""

    def _write_plist(self, path: Path, body: dict) -> None:
        import plistlib
        with path.open("wb") as f:
            plistlib.dump(body, f)

    def test_returns_username_value(self, tmp_path):
        p = tmp_path / "demo.plist"
        self._write_plist(p, {"Label": "x", "UserName": "team-bot-a"})
        assert deploy._read_plist_username(p) == "team-bot-a"

    def test_returns_empty_string_when_username_absent(self, tmp_path):
        """The previous PlistBuddy round-trip surfaced '' on missing keys
        too; preserve that semantics so downstream equality check still
        flags it as drift."""
        p = tmp_path / "demo.plist"
        self._write_plist(p, {"Label": "x"})
        assert deploy._read_plist_username(p) == ""

    def test_returns_none_on_unreadable_file(self, tmp_path):
        """Permission denied / file disappeared mid-read should
        propagate as None so the caller surfaces 'plist read failed'
        rather than treating it as an empty UserName."""
        p = tmp_path / "missing.plist"
        # File doesn't exist — plistlib.load raises FileNotFoundError
        # (subclass of OSError) which the reader catches.
        assert deploy._read_plist_username(p) is None

    def test_returns_none_on_corrupt_plist(self, tmp_path):
        p = tmp_path / "junk.plist"
        p.write_bytes(b"not a plist at all")
        assert deploy._read_plist_username(p) is None

    def test_works_on_root_owned_world_readable_plist(self, tmp_path):
        """End-to-end: the operating constraint we're working around is
        that /Library/LaunchDaemons/*.plist is root:wheel mode 644. We
        can't simulate root ownership in a test, but we CAN simulate
        the equivalent — a plist the test process can read but cannot
        modify the owner of — and confirm direct plistlib.load works
        without subprocess.

        The bug this catches is regression to subprocess-via-sudo. If
        someone reverts this function back to shelling out, a future
        non-root caller (like pod_perms_drift_monitor) hits the silent-
        empty-stdout failure again. Lock the implementation to a
        non-subprocess path so the regression can't sneak back.
        """
        p = tmp_path / "apply.plist"
        self._write_plist(p, {"Label": "apply", "UserName": "evo"})

        # Spy on subprocess.run inside the deploy module — the reader
        # must NOT shell out.
        called = []
        ...
        deploy.subprocess.run = _spy_run
        try:
            result = deploy._read_plist_username(p)
        finally:
            deploy.subprocess.run = original_run

        assert result == "evo"
        assert called == [], (
            f"Reader must not shell out — got subprocess invocations: {called!r}"
        )
...
class TestEnsurePodPerms:
    ...
    def test_check_only_does_not_invoke_fixes(self, tmp_path, monkeypatch):
        ...
        calls = {"apply": 0}
        with patch.object(deploy, "_check_apply_plist",
                          return_value=_PermCheck(
            ...
            with patch.object(deploy, "_check_apply_lock",
                              return_value=_PermCheck(
                                  category="lock-file", target="y", ok=True,
                                  detail="ok",
                              )):
        ...
        # At least one drift detected (the plist mock), but no apply ran.
    ...
    def test_apply_invokes_fixes_for_drift_only(self, tmp_path, monkeypatch):
        ...
        with patch.object(deploy, "_check_apply_plist",
            ...
            with patch.object(deploy, "_check_apply_lock",
                              return_value=_PermCheck(
                                  category="lock-file", target="L", ok=True,
                                  detail="ok",
                                  # If this fix were ever invoked it would prove
                                  # the apply phase skipped the ok-gate.
                                  apply=_ok_fix,
                              )):
                with patch.object(deploy, "_check_bot_acl", return_value=[]):
    ...
    def test_no_drift_means_no_changes(self, tmp_path, monkeypatch):
        ...
        with patch.object(deploy, "_check_apply_plist", return_value=ok_check):
            with patch.object(deploy, "_check_apply_lock", return_value=ok_check):
    ...
    def test_idempotency_second_pass_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch.object(deploy, "_check_apply_plist", side_effect=lambda bid, u: _make_check()):
            with patch.object(deploy, "_check_apply_lock", side_effect=lambda bid, u: _make_check()):
