...
privileged-read guard (a symlinked store component is refused), and the
bot_id -> OS-account resolution that decides WHICH home is read at all.
...
# ── bot_id → account resolution (aliased bots) ────────────────────────────────
#
# bot_id is a LOGICAL name; the OS account may differ (network.json `user`).
# Until 2026-08-18 `_resolve_account` guarded its network load with
# `from evolve_config import load_network` — a name that has never existed
# (evolve_config exports `load_config`; `load_network` is the admin package's).
# The import always raised, the handler substituted `network = {}`, and `{}`
# — not being None — then defeated get_bot_user's own
# `if config is None: config = load_config()`. Every aliased bot silently
# resolved to its bot_id, so the reader looked in a home that was either absent
# (no such home) or STALE — `evolve` resolved to /Users/evolve, the abandoned
# pre-account-separation profile, where it read the WRONG credentials and
# reported success. Every non-aliased bot was fine, which is why it hid.
#
# These pin the resolution itself, not just the read, because the read looked
# healthy for 7 of 9 bots on the mini.


@pytest.fixture
def aliased_network() -> dict:
    """`team_bot_b` lives on OS account `personal_bot_user`; `plain` does not."""
    return {
        "bots": {
...
def test_resolve_account_maps_aliased_bot_id_via_explicit_network(aliased_network):
    import oc_auth_store as s

    assert s._resolve_account("team_bot_b", network=aliased_network) == "personal_bot_user"


def test_resolve_account_maps_aliased_bot_id_when_network_not_supplied(
    monkeypatch, aliased_network
):
    """THE regression test.

    With no `network`, `_resolve_account` must let get_bot_user load the
    config itself. Substituting `{}` here is what broke aliasing — and `{}`
    fails silently, resolving to the bot_id rather than raising.
    """
    import evolve_config

    monkeypatch.setattr(evolve_config, "load_config", lambda *a, **kw: aliased_network)

    import oc_auth_store as s

    assert s._resolve_account("team_bot_b") == "personal_bot_user"


def test_resolve_account_falls_back_to_bot_id_for_non_aliased_bot(aliased_network):
    """The common case must keep working — it is what masked the bug."""
    import oc_auth_store as s

    assert s._resolve_account("plain", network=aliased_network) == "plain"
    assert s._resolve_account("absent-from-network", network=aliased_network) == (
        "absent-from-network"
    )


def test_resolve_account_explicit_user_still_wins(aliased_network):
    """`user` is already an account name; it must short-circuit the mapping."""
    import oc_auth_store as s

    assert s._resolve_account(
        "team_bot_b", user="someone-else", network=aliased_network
    ) == "someone-else"


def test_read_auth_store_reads_the_ACCOUNT_home_for_an_aliased_bot(
    monkeypatch, tmp_path, aliased_network
):
    """End-to-end: the store lives under the ACCOUNT's home, and there is no
    bot_id-named home at all — the shape that made the aliased bot
    unreadable."""
    import evolve_config

    account_home = tmp_path / "personal_bot_user"
    _build_sqlite_store(_agent_dir(account_home), _profiles_json(key="sk-ant-personal_bot_user"))
    assert not (tmp_path / "team_bot_b").exists()

    monkeypatch.setattr(evolve_config, "load_config", lambda *a, **kw: aliased_network)
    monkeypatch.setattr(evolve_config, "user_home", lambda user: tmp_path / user)

    from evolve_admin import oc_store as s

    assert s.read_anthropic_key("team_bot_b") == "sk-ant-personal_bot_user"


def test_read_auth_store_does_not_read_a_stale_bot_id_home(
    monkeypatch, tmp_path, aliased_network
):
    """The `evolve` shape, and the worse half of the bug: a bot_id-named home
    EXISTS and holds stale credentials. Resolving to it returns the wrong key
    while looking like success, so a read that merely 'succeeds' proves
    nothing — assert on WHICH key came back."""
    import evolve_config

    _build_sqlite_store(
        _agent_dir(tmp_path / "team_bot_b"), _profiles_json(key="sk-ant-STALE")
    )
    _build_sqlite_store(
        _agent_dir(tmp_path / "personal_bot_user"), _profiles_json(key="sk-ant-LIVE")
    )
    ...
    assert s.read_anthropic_key("team_bot_b") == "sk-ant-LIVE"
