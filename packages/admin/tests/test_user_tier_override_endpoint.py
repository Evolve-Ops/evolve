...
Coverage:
  ...
  - Audit log entry has action=config.user_tier_override.set,
    oc_keys={"tiers"}, full details payload
...
def app_and_calls(tmp_path, monkeypatch):
    ...
    def _stub_cfg_set_err(bot_id, updates, **kw):
        ...
        # Mirror the real shape — the endpoint echoes userTierOverride.
        return ({"userTierOverride": updates.get("userTierOverride", {})}, None)
...
def test_put_default_tier_each_choice(app_and_calls, choice):
    ...
    # heal.py credits this endpoint as the writer of tiers — i.e. of
    # evolve-tiers.json, not openclaw.json — by checking oc_keys.
    assert audit_calls[0]["oc_keys"] == {"tiers"}
