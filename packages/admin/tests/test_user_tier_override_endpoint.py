...
def app_and_calls(tmp_path, monkeypatch):
    ...
    #: The bot's on-disk config, as the stubbed getter reports it. Tests that
    #: care about the ``roleCaps`` read-merge seed this before the request.
    on_disk: dict = {}
    ...
    def _stub_cfg_set_err(bot_id, updates, **kw):
        ...
        # (``tiersKeysWritten``) so the endpoint can declare them. ``roleCaps``
        # rides along on a dailyCap write and is echoed the same way, because
        # the writer verifies it landed before reporting success.
        on_disk.update(updates)
        return ({
            "userTierOverride": updates.get("userTierOverride", {}),
            "roleCaps": updates.get("roleCaps", on_disk.get("roleCaps", {})),
            "tiersKeysWritten": sorted(updates),
        }, None)

    def _stub_cfg_get(bot_id, **kw):
        # Without this the endpoint's read-merge would shell out to the real
        # ``sudo -u <bot> python3`` and fail closed — and a green suite would
        # then be proving nothing about the merge.
        return dict(on_disk)
    ...
    monkeypatch.setattr(oc_cli, "oc_full_config_get", _stub_cfg_get)
...
def test_put_each_field_independently(app_and_calls, field, value):
    ...
    The ONE coupling is deliberate and is the 2026-09-10 fix: ``dailyCap``
    pulls ``roleCaps.power.maxPerDayPerBot`` into the same write, because the
    gateway enforces that key and shadows ``dailyCap`` on every pod. The two
    must land together or the two readers report different caps.
    """
    ...
    assert resp.status_code == 200, resp.get_json()
    expected = {"userTierOverride": {field: value}}
    if field == "dailyCap":
        expected["roleCaps"] = {"power": {"maxPerDayPerBot": value}}
    assert cfg_set_calls[0]["updates"] == expected
