...
//   6. A FAST FLICK commits even short of the 45% distance threshold
//      (as long as it has travelled past 15%).
...
//  14-17. D-ST10, the card is a physical object: a commit flies the top
//      NODE off and promotes the peek node (content is never rewritten into
//      a node on screen); a sub-threshold drag changes nothing at all; a
//      fast short nudge and a slow short drag both do not commit; reduced
//      motion never sets a transform.
//
...
/** The live node carrying `id` inside the card stack (cards are built per
 *  card, so `top-card`/`card-body`/`drop-reasons` move between nodes). */
function live(h, id) {
  return walk(h.byId['card-stack']).find((n) => n.id === id) || null;
}
function cardNodes(h) { return h.byId['card-stack'].children; }
function topId(h) { const t = live(h, 'top-card'); return t ? t.dataset.cardId : null; }
function peekId(h) { const p = live(h, 'peek-card'); return p ? p.dataset.cardId : null; }
...
function drive(h, points) {
  const top = live(h, 'top-card');
  top.rect = RECT;
  h.byId['card-stack'].fire('pointerdown', { pointerId: 1, clientX: points[0].x, clientY: points[0].y,
...
{
  const h = await boot();
  // Right, past the 45% threshold (0.45*320=144) — a clean commit.
  ...
  check('2. the card leaves the top of the queue', live(h, 'card-body').children.some(
    (n) => n.textContent === 'Approve the reply'));
}

{
  const h = await boot();
  drive(h, [{ x: 0, y: 0, ts: 0 }, { x: 0, y: -30, ts: 50 }, { x: 0, y: -300, ts: 400 }]);
  await flush();
  ...
  check('1. the passed card rotates to the BACK, not off the queue',
    live(h, 'card-body').children.some((n) => n.textContent === 'Approve the reply'));
}
...
// ── 3: drop shows reasons, then posts ─────────────────────────────────────
{
  ...
  const overlay = live(h, 'drop-reasons');
  ...
  check('3. …and the overlay is gone', live(h, 'drop-reasons').hidden === true);
}
...

{
  ...
  check('4. …and the card is back on top',
    live(h, 'card-body').children.some((n) => n.textContent === 'Book dentist'));
}
...
// ── 6: a fast flick commits short of the distance threshold ──────────────
{
  ...
  // past the 0.6px/ms flick speed; 50px total is far under the 144px (45%
  // of 320) distance threshold but past the 48px (15%) flick floor.
...
// ── 10: taps — decision, Later, action ────────────────────────────────────
{
  ...
  const approve = live(h, 'card-body').children.find(
...

{
  ...
  const later = live(h, 'card-body').children.find((n) => n.id === 'later-btn');
  later.onclick();
...

{
  ...
  const actionBtn = live(h, 'card-body').children.find(
...
// ── 12: the same persist-then-animate guarantee on the Drop path ─────────
{
  ...
  await flush();
  h.failNextPost(500, 'boom');
  live(h, 'drop-reasons').children.find((b) => b.textContent === 'never').onclick();
  await flush();
...
// ── 14: a commit removes the top NODE and promotes the peek node ─────────
{
  const h = await boot({ items: [...fixtureItems(), {
    kind: 'card', card: { id: 'card3', title: 'Call the plumber', cluster: 'home',
                          lane: 'today', owner: 'me', source: 'manual', why_line: 'added by you' },
  }] });
  const top0 = live(h, 'top-card'), peek0 = live(h, 'peek-card');
  check('14. top and peek are two distinct nodes holding two distinct cards',
    top0 && peek0 && top0 !== peek0 && topId(h) === 'card1' && peekId(h) === 'card2');
  const status0 = h.byId['status-text'].textContent;
  drive(h, [{ x: 0, y: 0, ts: 0 }, { x: 30, y: 0, ts: 50 }, { x: 200, y: 0, ts: 400 }]);
  await flush();
  check('14. the committed node flies off the way it was pushed (right)',
    top0.classList.contains('is-leaving') && /translate\(150%/.test(top0.style.transform),
    top0.style.transform);
  check('14. …its content was never rewritten',
    top0.dataset.cardId === 'card1' &&
    walk(top0).some((n) => n.textContent === 'Book dentist'));
  check('14. the PEEK node is promoted to top (same node, id moved)',
    live(h, 'top-card') === peek0 && topId(h) === 'card2');
  check('14. …and a NEW node mounts as peek beneath it',
    live(h, 'peek-card') !== top0 && peekId(h) === 'card3');
  check('14. the header ticks down with emphasis',
    h.byId['status-text'].textContent !== status0 &&
    h.byId['status-text'].classList.contains('tick'), h.byId['status-text'].textContent);
  eq('14. the toast names the outcome and the card', h.byId['undo-text'].textContent,
    'Done: Book dentist');
  h.fireTimer(280);
  check('14. the departed node is removed once off screen', !cardNodes(h).includes(top0));
  eq('14. …leaving exactly two card nodes', cardNodes(h).length, 2);
}

{
  const h = await boot();
  const top0 = live(h, 'top-card');
  drive(h, [{ x: 0, y: 0, ts: 0 }, { x: 0, y: -30, ts: 50 }, { x: 0, y: -300, ts: 400 }]);
  await flush();
  check('14. a pass flies the node UP, it does not re-render in place',
    top0.classList.contains('is-leaving') && /translate\(0, -150%/.test(top0.style.transform)
    && topId(h) === 'card2');
  eq('14. …the pass toast names the card', h.byId['undo-text'].textContent,
    'Passed: Book dentist');
}

// ── 15: below threshold = nothing, byte-identical ────────────────────────
{
  const h = await boot();
  const snap = () => JSON.stringify({
    top: topId(h), peek: peekId(h), status: h.byId['status-text'].textContent,
    nodes: cardNodes(h).length, toast: h.byId['undo-toast'].hidden,
  });
  const before = snap();
  const top0 = live(h, 'top-card');
  // 140px right (just under 144) over a slow second.
  drive(h, [{ x: 0, y: 0, ts: 0 }, { x: 70, y: 0, ts: 500 }, { x: 140, y: 0, ts: 1000 }]);
  await flush();
  eq('15. a below-threshold drag sends no request', h.posted, []);
  eq('15. …queue, header count, top/peek ids and toast are identical', snap(), before);
  check('15. …the same node is on top and springs back to rest',
    live(h, 'top-card') === top0 && top0.style.transform === '' &&
    !top0.classList.contains('dragging'));
}

// ── 16: a fast short nudge does not commit; a slow short drag does not ───
{
  const h = await boot();
  // 40px (12.5% — under the 15% flick floor) at 3px/ms: an accidental brush.
  drive(h, [{ x: 0, y: 0, ts: 0 }, { x: 10, y: 0, ts: 5 }, { x: 40, y: 0, ts: 15 }]);
  await flush();
  eq('16. a fast but SHORT nudge (under 15%) commits nothing', h.posted, []);
  check('16. …the card is still on top', topId(h) === 'card1');
}

{
  const h = await boot();
  // 130px (40%) slowly: would have committed under the old 35% threshold.
  drive(h, [{ x: 0, y: 0, ts: 0 }, { x: 65, y: 0, ts: 600 }, { x: 130, y: 0, ts: 1200 }]);
  await flush();
  eq('16. a slow drag short of 45% commits nothing', h.posted, []);
}

{
  const h = await boot();
  drive(h, [{ x: 0, y: 0, ts: 0 }, { x: 30, y: 0, ts: 10 }, { x: 60, y: 0, ts: 20 }]);
  await flush();
  eq('16. a fast flick past 15% commits', h.posted, [{
    path: '/api/board/dom/cards/card1/move', method: 'POST', body: { to_lane: 'done' },
  }]);
}

// ── 16b: a failed write springs the card back, same node, no fly-off ─────
{
  const h = await boot();
  const top0 = live(h, 'top-card');
  h.failNextPost(500, 'boom');
  drive(h, [{ x: 0, y: 0, ts: 0 }, { x: 30, y: 0, ts: 50 }, { x: 200, y: 0, ts: 400 }]);
  await flush();
  check('16b. a rejected Done springs the SAME node back, never flies it',
    live(h, 'top-card') === top0 && top0.style.transform === '' &&
    !top0.classList.contains('is-leaving'));
}

// ── 17: reduced motion never sets a transform; two-node rule holds ───────
{
  const h = await boot({ reducedMotion: true });
  const top0 = live(h, 'top-card'), peek0 = live(h, 'peek-card');
  h.byId['btn-right'].onclick();
  await flush();
  check('17. reduced motion: the top node fades out (opacity), not flies',
    top0.classList.contains('is-leaving') && top0.style.opacity === '0');
  check('17. …the peek node is promoted', live(h, 'top-card') === peek0);
  check('17. …and no card node ever carries a transform',
    walk(h.byId['card-stack']).every((n) => !n.style.transform) && !top0.style.transform);
  h.fireTimer(180);
  check('17. …the faded node is removed', !cardNodes(h).includes(top0));
}
