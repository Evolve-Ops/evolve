...
// ── 7: reduced motion disables the gesture; buttons post the same bodies ──
{
  const h = await boot({ reducedMotion: true });
  check('7. the four-button fallback is shown', h.byId['button-row'].hidden === false);
  drive(h, [{ x: 0, y: 0, ts: 0 }, { x: 200, y: 0, ts: 400 }]);
  await flush();
  eq('7. the pointer gesture is inert under reduced motion', h.posted, []);
  h.byId['btn-right'].onclick();
  await flush();
  eq('7. the button posts the identical body a swipe would', h.posted, [{
    path: '/api/board/dom/cards/card1/move', method: 'POST', body: { to_lane: 'done' },
  }]);
}
