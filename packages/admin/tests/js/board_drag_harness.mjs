...
//  11. ONE GESTURE, ONE FINGER. A second pointer's move/up neither steers nor
//      ends a drag the first one owns; an event carrying no pointerId at all
//      still works, so a browser that omits it does not lose the gesture.
//
...
// ── 11: a second finger must not take over the drag ──────────────────────
{
  const h = await boot();
  const tile = h.tile('aaaa1111');
  tile.rect = { left: 0, top: 0, right: 200, bottom: 60, width: 200, height: 60 };
  ...
  const today = h.laneHead('Today');
  today.rect = RECT(150, 400);
  const dropped = h.laneHead('Dropped');
  dropped.rect = RECT(150, 600);

  // The owning finger moves over Today.
  h.doc('pointermove', { pointerId: 1, clientX: 150, clientY: 400 });
  // A second finger wanders over Dropped and lifts.
  h.doc('pointermove', { pointerId: 2, clientX: 150, clientY: 600 });
  h.doc('pointerup', { pointerId: 2, clientX: 150, clientY: 600 });
  await new Promise((r) => setImmediate(r));
  eq('11. a second finger neither steers nor ends the drag', h.posted, []);
  // …and specifically did not drop the card into the lane IT was over: the
  // stray release landed on Dropped, whose sheet must never have opened.
  check('11. …and did not open the drop sheet the stray finger was over',
    h.byId['drop-sheet'].style.display !== 'block',
    h.byId['drop-sheet'].style.display);
  check('11. …and the card is still held',
    h.body.children.some((n) => n.className.includes('drag-ghost')));

  // The owning finger finishes where IT was, not where the other one went.
  h.doc('pointerup', { pointerId: 1, clientX: 150, clientY: 400 });
  await new Promise((r) => setImmediate(r));
  eq('11. the owning finger completes its own drop', h.posted, [{
    path: '/api/board/dom/cards/aaaa1111/move', method: 'POST',
    body: { to_lane: 'today' },
  }]);
}

// ── 11b: an event with no pointerId at all must still work ───────────────
{
  const h = await boot();
  const tile = h.tile('aaaa1111');
  tile.rect = { left: 0, top: 0, right: 200, bottom: 60, width: 200, height: 60 };
  // A browser (or a synthetic event) that omits pointerId entirely: the
  // guard must not silently swallow the whole gesture.
  tile.fire('pointerdown', { button: 0, clientX: 20, clientY: 30 });
  h.fireHoldTimer();
  h.laneHead('Today').rect = RECT(150, 400);
  h.doc('pointermove', { clientX: 150, clientY: 400 });
  h.doc('pointerup', { clientX: 150, clientY: 400 });
  await new Promise((r) => setImmediate(r));
  eq('11. an id-less gesture still completes', h.posted, [{
    path: '/api/board/dom/cards/aaaa1111/move', method: 'POST',
    body: { to_lane: 'today' },
  }]);
}
