...
WHAT THESE PIN:
  ...
  * on a colour console that code is PAINTED (background-coloured cells),
    which is what survives a font whose block glyphs do not fill the cell;
    a console with no colour gets the glyph fallback and is told so;
...
class _Utf8Buffer(io.StringIO):
    """A capture buffer that answers ``.encoding`` the way a tty's stdout
    does — ``_stdout_encoding`` reads it to pick the glyph vs ASCII path."""

    encoding = "utf-8"


def _colour_console(monkeypatch, *, width: int = 200) -> io.StringIO:
    """Swap the module console for one that reports colour, and hand back its
    buffer. A CliRunner's stdout is not a tty, so the CLI would otherwise
    always take the no-colour fallback and the painted path would go
    untested."""
    buf = io.StringIO()
    monkeypatch.setattr(board_cli, "console",
                        Console(file=buf, force_terminal=True, width=width,
                                color_system="standard"))
    return buf


def _painted_matrix(ansi: str) -> list[list[bool]]:
    """The module grid carried by the painted rows in ``ansi``."""
    rows = [ln for ln in ansi.split("\n")
            if len(ln) > 20 and not _ANSI.sub("", ln).strip()]
    if not rows:
        raise AssertionError(f"no painted QR block in output:\n{ansi}")
    return raster.modules_from_backgrounds(
        raster.parse_cells("\n".join(rows)),
        columns_per_module=qr_terminal.CELL_COLUMNS)
...
def test_a_colour_terminal_gets_the_painted_code(pod, monkeypatch):
    """The path a real operator is on: modules are background-coloured cells,
    so the code does not depend on the terminal font's block glyphs."""
    buf = _colour_console(monkeypatch)
    result = _run(pod, "token", BOT)
    assert result.exit_code == 0, result.output
    out = buf.getvalue()
    url = _url_from(_ANSI.sub("", out))
    assert decode(_painted_matrix(out)) == url
    assert "Drawn without colour" not in _ANSI.sub("", out)

    # And it is the credential the board accepts — the painted code is of the
    # link, not of some other string that merely looks like one.
    token = url.split("?t=", 1)[1]
    assert pod["client"].get(f"/api/board/{BOT}?t={token}").status_code == 200


def test_a_console_without_colour_falls_back_to_glyphs_and_says_so(pod):
    """CliRunner's stdout is not a tty, so this is the no-colour path. The
    glyph rendering is what failed a phone once; the operator is told."""
    result = _run(pod, "token", BOT)
    assert result.exit_code == 0, result.output
    lines = _qr_lines(result.output)
    assert any(ch in "█▀▄" for line in lines for ch in line)
    assert decode(read_screen(lines)) == _url_from(result.output)
    assert "Drawn without colour" in result.output


def test_no_color_on_a_colour_terminal_gets_glyphs_not_blank_rows(pod,
                                                                  monkeypatch):
    """``NO_COLOR`` keeps rich's ``color_system`` and strips the SGR at print
    time instead, so asking only that question paints an invisible code. The
    console must be asked both, and this is the tty that proves it."""
    # A real tty's stdout reports an encoding; a bare StringIO does not, and
    # the renderer would degrade to ASCII for that reason instead of the one
    # under test.
    buf = _Utf8Buffer()
    monkeypatch.setattr(board_cli, "console",
                        Console(file=buf, force_terminal=True, width=200,
                                color_system="standard", no_color=True))
    result = _run(pod, "token", BOT)
    assert result.exit_code == 0, result.output
    out = _ANSI.sub("", buf.getvalue())
    lines = _qr_lines(out)
    assert any(ch in "█▀▄" for line in lines for ch in line)
    assert decode(read_screen(lines)) == _url_from(out)
    assert "Drawn without colour" in out


def test_the_painted_code_is_refused_rather_than_wrapped(pod, monkeypatch):
    """Painting costs width — two columns a module. A default 80-column
    window cannot hold a board link, and saying so beats printing a code that
    reflows into something no camera can read."""
    buf = _colour_console(monkeypatch, width=80)
    result = _run(pod, "token", BOT)
    assert result.exit_code == 0, result.output
    out = " ".join(_ANSI.sub("", buf.getvalue()).split())
    assert "Terminal too narrow" in out
    needed = re.search(r"\((\d+) columns needed, 80 available\)", out)
    # Pinned to the renderer's own figure, not merely "> 80": the number the
    # operator is quoted is the width they must size the window to, so a
    # change to CELL_COLUMNS or the quiet zone has to break a test rather
    # than silently desync it. (This fixture's link has no host, so it is a
    # module row shorter than the 90 columns a real board link needs — the
    # 90 x 45 figure the help text carries is pinned in test_board_qr.py.)
    assert needed, out
    url = _url_from(_ANSI.sub("", buf.getvalue()))
    assert int(needed.group(1)) == qr_terminal.render_qr(url).width, out
    assert f"/board/{BOT}?t=" in out
