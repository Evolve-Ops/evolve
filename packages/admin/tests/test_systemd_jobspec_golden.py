...
1. **Unit-file golden tests**: every JobSpec-built job *shape* in the
   ...
   from internal/design-linux-port-2026-06-10.md §3.
...
def test_soft_file_limit_renders_limit_nofile():
    """soft_file_limit → LimitNOFILE= in [Service] (the systemd sibling of
    launchd SoftResourceLimits.NumberOfFiles; 2026-07-28 EMFILE incident)."""
    units = render_systemd_units(JobSpec(
        label="ai.evolve.t", program_args=["/bin/true"], soft_file_limit=4096,
    ))
    svc = _parse_units(units["ai.evolve.t.service"])["Service"]
    assert ("LimitNOFILE", "4096") in svc
    # unset → key omitted entirely (systemd default applies)
    units = render_systemd_units(JobSpec(
        label="ai.evolve.t", program_args=["/bin/true"],
    ))
    svc = _parse_units(units["ai.evolve.t.service"])["Service"]
    assert not any(k == "LimitNOFILE" for k, _ in svc)
...
def test_remove_handles_the_pair_atomically(tmp_path):
    ...
    # daemon-reload retracts the unit files; reset-failed then retracts the
    # recorded failure, which deleting the files does NOT do — without it a
    # unit that was failing when it was retired keeps showing up in
    # `systemctl list-units --state=failed` as `not-found / failed` forever.
    # Order matters: before daemon-reload systemd still has the old unit
    # loaded and re-derives the failed state.
    assert runner.systemctl_calls()[-2:] == [
        ["daemon-reload"],
        ["reset-failed",
         f"{INTERVAL_SPEC.label}.service", f"{INTERVAL_SPEC.label}.timer"],
    ]
...
def test_install_skip_then_explicit_restart_via_real_call_path(tmp_path):
    ...
    Runs on DAEMON_SPEC (``keep_alive=True``): the guarantee is a daemon
    contract. ``systemctl restart <label>.service`` on a timer-activated
    oneshot RUNS the job rather than restarting it, so the wrapper
    deliberately skips the bounce there — pinned by the companion test
    below.
    """
...
def test_install_skip_does_not_force_run_a_timer_oneshot(tmp_path):
    """The other half, against the REAL systemd adapter.

    ``restart`` targets the ``.service``, not the ``.timer`` — see
    ``SystemdScheduler.restart``'s own docstring ("For timer jobs this runs
    the job now"). Units are byte-identical on nearly every deploy and the
    puller deploys every 15 min, so the old unconditional skip-path restart
    force-ran ~30 timer jobs per deploy on this platform, including the real
    tokens of ``model_liveness_monitor``.
    """
    from evolve_admin import deploy
    ...
    assert deploy._install_job_ensuring_restart(INTERVAL_SPEC)[0]
    before = len(runner.systemctl_calls())

    ok, msg = deploy._install_job_ensuring_restart(INTERVAL_SPEC)
    assert ok, msg
    assert "no bounce" in msg
    assert runner.systemctl_calls()[before:] == [], (
        "byte-identical timer oneshot was force-RUN on the skip path; "
        f"calls={runner.systemctl_calls()[before:]!r}"
    )
