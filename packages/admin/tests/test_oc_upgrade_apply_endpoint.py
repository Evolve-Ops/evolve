...
Spec: internal/spec-oc-upgrade-from-ui-2026-07-28.md §6 + verification items
**A1–A6** and **U1**.
...
from evolve_admin import oc_compat  # noqa: E402
...
def client(tmp_path, monkeypatch):
    ...
    # TARGET counts as validated for this module. The endpoint gained a
    # separate veto in 2026-09 — it refuses a target Evolve's compatibility
    # contract has not passed on, re-derived rather than read off the report
    # (internal/design-oc-upgrade-safety-2026-09-08.md §2 row 3). Every test
    # here is about a DIFFERENT refusal, so the compat manifest is pointed at
    # a fixture that lists TARGET. The veto's own behaviour — including that
    # it fires on a green, non-stale report — is covered in test_oc_compat.py.
    compat_repo = tmp_path / "compat-repo"
    (compat_repo / "packages" / "plugin").mkdir(parents=True)
    (compat_repo / "packages" / "plugin" / "compat.json").write_text(
        json.dumps({"evolveVersion": "0.1.0", "openclaw": {"tested": [TARGET]}})
    )
    monkeypatch.setattr(oc_compat, "_repo_root", lambda: compat_repo)
...
def _write_preflight_cache(
    shared: Path, *, installed: str = INSTALLED, target: str = TARGET,
    blocking: bool = False, checked_at: str | None = None,
    bots: dict | None = None,
) -> None:
    """Seed a passing (or blocking) oc_preflight cache entry for the pair —
    the gate api_oc_upgrade_apply added under
    hold-fix-4277-preflight-fails-closed-and-is-wired-to-the-upgrade item 2,
    now with the freshness fields hold-fix-4392 checks (a fresh
    ``checked_at`` and a ``bot_set_hash`` matching the fixture network's
    ``team_bot_a``, both overridable by the tests that specifically exercise
    staleness). Written directly in oc_preflight_store's own on-disk shape
    rather than imported, so this test stays independent of that module's
    internals.
    """
    from evolve_admin.oc_preflight import bot_set_hash as _bsh

    root = shared / "oc_preflight" / "reports"
    root.mkdir(parents=True, exist_ok=True)
    now = checked_at or datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    net = bots if bots is not None else {"team_bot_a": {"user": "team_bot_a"}}
    (root / f"pair-{installed}__{target}.json").write_text(json.dumps({
        "report_id": "pf-fixture", "target_version": target,
        "installed_version": installed, "blocking": blocking,
        "summary": "1 blocker" if blocking else "clean", "rows": [],
        "checked_at": now, "bot_set_hash": _bsh({"bots": net}),
    }))
...
#
# hold-fix-4277-preflight-fails-closed-and-is-wired-to-the-upgrade: the
# Update control must be backed by a passing oc_preflight run for the EXACT
# from → to pair — enforced here server-side, not only by a disabled button.


def test_no_preflight_cache_for_the_pair_is_409(client):
    _write_report(client["shared"], _report())
    # No _write_preflight_cache() call — this pair has never been checked.
    resp = client["client"].post("/api/oc/upgrade/apply",
                                 json={"reportId": REPORT_ID, "confirm": True})
    assert resp.status_code == 409
    assert "No per-bot OpenClaw preflight" in resp.get_json()["error"]
    assert client["calls"]["privileged"] == []


def test_a_blocking_preflight_cache_is_409(client):
    _write_report(client["shared"], _report())
    _write_preflight_cache(client["shared"], blocking=True)
    resp = client["client"].post("/api/oc/upgrade/apply",
                                 json={"reportId": REPORT_ID, "confirm": True})
    assert resp.status_code == 409
    assert "blocking issues" in resp.get_json()["error"]
    assert client["calls"]["privileged"] == []


def test_a_preflight_cache_for_a_different_pair_does_not_satisfy_the_gate(client):
    # Cached for a DIFFERENT target — an operator who checked 2026.7.2 must
    # not have that count for an apply that resolves to 2026.7.1.
    _write_report(client["shared"], _report())
    _write_preflight_cache(client["shared"], target="2026.7.2")
    resp = client["client"].post("/api/oc/upgrade/apply",
                                 json={"reportId": REPORT_ID, "confirm": True})
    assert resp.status_code == 409
    assert "No per-bot OpenClaw preflight" in resp.get_json()["error"]
...
# ── Preflight freshness gate (hold-fix-4392) ────────────────────────────────
#
# A green preflight is only good for FRESHNESS_WINDOW_H, and only against
# the bot set it actually ran against — a run from before a bot was added,
# or from too long ago, must 409 the same as no run at all.


def test_a_preflight_cache_past_the_freshness_window_is_409_stale(client):
    _write_report(client["shared"], _report())
    old = (datetime.now(timezone.utc) - timedelta(hours=25)).strftime("%Y-%m-%dT%H:%M:%SZ")
    _write_preflight_cache(client["shared"], checked_at=old)
    resp = client["client"].post("/api/oc/upgrade/apply",
                                 json={"reportId": REPORT_ID, "confirm": True})
    assert resp.status_code == 409
    assert "old" in resp.get_json()["error"]
    assert client["calls"]["privileged"] == []


def test_a_preflight_cache_from_before_a_bot_was_added_is_409_bot_set_changed(client):
    _write_report(client["shared"], _report())
    # The cache reflects a registry that had only team_bot_a; the fixture's
    # LIVE network (see the `client` fixture above) already carries it too —
    # so seed the cache against a DIFFERENT bot set to simulate one added since.
    _write_preflight_cache(client["shared"], bots={"team_bot_a": {}, "team_bot_z": {}})
    resp = client["client"].post("/api/oc/upgrade/apply",
                                 json={"reportId": REPORT_ID, "confirm": True})
    assert resp.status_code == 409
    assert "bot was added or removed" in resp.get_json()["error"]
    assert client["calls"]["privileged"] == []
...
def test_a5_deploy_lock_held_is_409(client, monkeypatch):
    _write_report(client["shared"], _report())
    _write_preflight_cache(client["shared"])
...
def test_u1_apply_button_requires_ok_and_not_stale():
    ...
    # The button only renders behind that gate — "Run upgrade now" appears
    # exactly once in the source, inside the `if (canApply)` arm.
    arm = re.search(r"if \(canApply\) \{(.+?)\n  \} else ", src, re.S)
    assert arm, "the apply row is not gated on canApply"
    assert "Run upgrade now" in arm.group(1)
    # Comments stripped — a comment naming the button isn't a second button.
    assert _JS_LINE_COMMENT_RE.sub("", src).count("Run upgrade now") == 1


def test_u1_withheld_button_explains_itself():
    """A yellow "update available" with no button and no words is the state
    the operator can do nothing with: they can't tell whether Evolve applies
    the upgrade, and the banner carries no command to fall back to (nor could
    it — see test_u1_no_sudo_cli_hint_survives_in_the_banner). Every
    not-canApply branch must say what it's waiting for."""
    src = _js()
    m = re.search(r"\} else if \(!running\) \{(.+?)\n  \}", src, re.S)
    assert m, "no explanatory branch for a withheld apply button"
    branch = m.group(1)
    assert "Run the safety check" in branch          # no report yet
    assert "failing checks" in branch                # red report
    assert "Re-check" in branch                      # stale report
    assert "can't confirm which OpenClaw version" in branch   # unreadable
    assert not _SUDO_HINT_RE.search(_JS_LINE_COMMENT_RE.sub("", branch))
...
def test_root_helper_runs_as_a_script_with_analyzer_already_on_sys_path(tmp_path):
    """The §7b helper must survive `sudo <venv_python> …/oc_upgrade_apply.py`.

    The deploy venv installs evolve-analyzer as a compat-mode editable (a
    plain-path .pth), so ``packages/analyzer`` is already on sys.path —
    *behind* site-packages. The shim's old `if _p not in sys.path` guard
    therefore never hoisted it, leaving the script's own directory
    (…/evolve_admin) at sys.path[0], where the `runtime` subpackage shadows
    the analyzer's top-level one. `evolve_admin.runtime.scheduler`'s
    `from runtime.scheduler import …` then resolved to itself and the helper
    died at import, before the first event — every apply from the UI failing
    with a traceback tail no operator could act on (mini pod, 2026-08-01).

    PYTHONPATH reproduces that ordering exactly: it lands after sys.path[0].
    """
    script = _ADMIN_DIR / "evolve_admin" / "oc_upgrade_apply.py"
    env = dict(os.environ, PYTHONPATH=str(_ADMIN_DIR.parent / "analyzer"))
    r = subprocess.run(
        [sys.executable, str(script), "--report-id", "20260801T000000Z-deadbeef"],
    ...
    # It got far enough to validate the id and refuse the unknown report —
    # i.e. every import the sequence needs resolved.
    assert r.returncode == apply_mod.EXIT_REFUSED, combined[-2000:]
