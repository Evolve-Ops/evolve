...
# ── The deploy checkout's GROUP is the operator's, never the fleet's ──────
#
# 2026-09-23: this chown rendered `evolve:staff` with a companion
# `chmod -R g+rwX`, so the human admin could write the checkout alongside the
# daemon. But `staff` is — by platform_profile's own definition — the primary
# group of EVERY bot account, and the deploy venv is editable-installed from
# that checkout. Any bot with shell could rewrite code that later runs as root
# (`sudo evolve-admin …`) or as the evolve daemon: the self-grant path #2759
# closed for the `evolve` user, reopened one group wider. Verified on the
# reference pod — a non-admin bot account had write on `setup_wizard.py`.


def _repo_chowns(shared, tmp_path, monkeypatch):
    """Run deploy_shared_dir against an existing checkout, returning every
    recorded `chown`/`chmod` argv that named the deploy checkout."""
    from evolve_admin import deploy
    ...
    # Re-point ONLY the checkout path; every other field (notably ops_group
    # and bot_shared_group, which the assertions read back) stays the real
    # profile's, so the test pins production values rather than fixtures.
    stub = dataclasses.replace(
        deploy._get_profile(), deploy_checkout_default=str(repo))
    monkeypatch.setattr(deploy, "_get_profile", lambda: stub)
    calls: list[list[str]] = []
    ...
    monkeypatch.setattr(deploy.subprocess, "run", _record)
    deploy.deploy_shared_dir(shared)
    return [c for c in calls if str(repo) in " ".join(c)], repo


def test_deploy_checkout_group_is_the_operator_group(shared, tmp_path,
                                                     monkeypatch):
    from platform_profile import get_profile

    calls, repo = _repo_chowns(shared, tmp_path, monkeypatch)
    chowns = [c for c in calls if any("chown" in part for part in c)]
    assert chowns, "deploy_shared_dir no longer chowns the deploy checkout"
    owners = {c[c.index("-R") + 1] for c in chowns}
    assert owners == {f"evolve:{get_profile().ops_group}"}, owners


def test_deploy_checkout_is_never_chowned_to_the_all_bots_group(
        shared, tmp_path, monkeypatch):
    """The regression guard. `bot_shared_group` is every bot account; the
    companion `chmod -R g+rwX` turns group ownership into fleet-wide write
    on code root executes."""
    from platform_profile import get_profile

    calls, _repo = _repo_chowns(shared, tmp_path, monkeypatch)
    bots = get_profile().bot_shared_group
    offenders = [c for c in calls if f"evolve:{bots}" in c]
    assert not offenders, (
        f"deploy checkout chowned to the all-bots group ({bots!r}): {offenders}")


def test_operator_group_contains_no_bot_account_by_construction():
    """The two groups must stay distinct on every platform — the whole fix is
    that one of them is the fleet and the other is the person."""
    from platform_profile import LINUX, MACOS

    for prof in (MACOS, LINUX):
        assert prof.ops_group != prof.bot_shared_group, prof.name
        # …and distinct from the gid-0 group, which the operator is NOT in
        # (chowning the checkout there would lock the human out entirely).
        assert prof.ops_group != prof.admin_group, prof.name
