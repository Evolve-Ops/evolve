...
def _run_install_bot_docs(*, role, fname, substitute, src_text, existing_text,
    ...
    stubbed. Returns (result, writes) where writes = [(fname, content)]."""
    import tempfile
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(deploy, "_doc_plan_for_role",
             ...
             patch.object(deploy, "_append_evo_glossary",
                          side_effect=lambda text, result: text + glossary), \
...
def test_install_bot_docs_primary_agents_rewrites_over_stale_glossary():
    ...
    old glossary, well over 1500 bytes) does NOT block the redeploy — the new
    base + freshly-rendered glossary is written. This is the bug #2915 hit."""
    stale = ("o" * 2000) + "\n\n---\n\nGLOSSARY (old)\n"
    result, writes = _run_install_bot_docs(
    ...
    assert "new shipped base" in written["AGENTS.md"]
    assert "GLOSSARY" in written["AGENTS.md"]  # glossary still appended
...
def test_install_bot_docs_member_preserves_operator_edit_endtoend():
    ...
    (substantive + differs from the template) is left untouched."""
