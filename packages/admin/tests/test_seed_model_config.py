...
def test_seeds_anthropic_catalog_when_bot_has_anthropic_key(network_path):
    ...
    # Post-#71-degraded: the judge role IS now seeded with the same-provider
    # cheap tier (rather than left unset). The original PR #1729 left it unset
    ...
    #
    # tier0 → roles.judge → judge-class rung (TIER_TO_ROLE / TIER_TO_RUNG).
    assert updates["roles"]["judge"]["rung"] == "judge-class"
    assert _rungs["judge-class"]["models"] == ["anthropic/claude-haiku-4-5"]
...
@pytest.mark.parametrize("providers", [
    ["anthropic"],            # single provider → degraded same-provider judge
    ["anthropic", "openai"],  # two providers → cross-provider judge
])
...
def test_seed_payload_matches_what_the_migrator_would_produce(network_path):
    ...
    # Every input is derived INDEPENDENTLY from the seed's own output — if the
    # expected tier0 were read back out of `updates`, a regression in
    # `_pick_judge_provider` would reconstruct itself and the test would pass
    # while the judge silently moved to the wrong model or provider.
    _, catalog, legacy_tiers = provisioning._default_catalog_for_provider(
        "anthropic", role="member",
    )
    legacy_tiers = dict(legacy_tiers)
    judge_provider, judge_model, _degraded = provisioning._pick_judge_provider(
        ["anthropic", "openai"], "anthropic",
    )
    assert judge_provider == "openai" and judge_model, (
        "sanity: two credentialed providers must yield a cross-provider judge"
    )
    legacy_tiers["tier0"] = {"models": [judge_model]}
