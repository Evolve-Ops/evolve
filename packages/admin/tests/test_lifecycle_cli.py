...
def test_pre_flight_preview_helper_tolerates_missing_state(tmp_path: Path, capsys):
    ...
    from evolve_admin.cli import _print_lifecycle_preview, console

    bogus_network_path = tmp_path / "no-such-network.json"

    # Calling it should not raise even when the network can't be loaded.
    # (Output capture via console.file isn't trivial; just verify no exception.)
    try:
        _print_lifecycle_preview("admin_bot", bogus_network_path, action="detach")
