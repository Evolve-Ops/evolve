...
3. **``SystemdScheduler``** — the second platform, REAL since W3A (the
   Linux port's L1 step, docs/design-linux-port-2026-06-10.md §3): held
...
PROTOCOL_VERBS = ("install", "remove", "restart", "list", "status", "running", "supervision", "kill")
...
def test_jobspec_lifecycle_runs_end_to_end_on_fake_scheduler():
    ...
    assert fake.status(SPEC.label)["pid"] != first_pid, "the bounce must be real"

    # Read side through the real health path — Protocol surface, no raw().
    assert health._launchd_loaded(SPEC.label) is True
    assert fake.list(prefix="ai.evolve.") == [SPEC.label]

    # Remove, then the read path reports gone.
    ok, _ = get_scheduler().remove(SPEC.label)
    assert ok
    assert health._launchd_loaded(SPEC.label) is False
    assert fake.list() == []
