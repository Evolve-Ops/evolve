...
import { resolveInTurnCheckpointUsd } from "./breakers/InTurnCheckpoint.js";
...
export interface EvolveConfig {
  ...
  // Exec-failure hygiene A1 (design-exec-failure-hygiene-2026-08-31): when
  // armed, the message_sending hook ABSORBS raw `⚠️ 🛠️ Exec failed …`
  // trailers instead of letting them reach a user channel (matches are
  // always ledgered either way). DEFAULT false = observe-only: matches are
  // ledgered as would_absorb but delivered unchanged. Only the exact
  // boolean ``true`` arms absorption (layer2Enforce posture — a typo must
  // never silently start cancelling channel messages).
  ...
  // flip. Spec: internal/spec-user-roster-and-roles-2026-06-07.md §8 (Layer 2);
  ...
  // In-turn cost checkpoint (D-CS12; breakers/InTurnCheckpoint.ts): plugin
  // config ``inTurnCheckpointUsd``, default 5.00, below 1.00 refused. Not under
  // ``agents.defaults`` — OpenClaw rejects unknown keys there.
  inTurnCheckpointUsd: number;
  inTurnCheckpointWarning: string | null;
}
...
export function resolveLayer2Enforce(
  ...
  return resolveStrictBooleanArm(
    present,
    (value) =>
 ...
 */
export function resolveStrictBooleanArm(
  present: unknown,
  buildWarning: (present: unknown) => string,
): { armed: boolean; warning: string | null } {
...
export function resolveExecFailureAbsorb(
  pluginConfig: Record<string, unknown>,
): { armed: boolean; warning: string | null } {
  return resolveStrictBooleanArm(
    pluginConfig.execFailureAbsorb as unknown,
    (value) =>
      `Evolve exec-failure absorber: execFailureAbsorb is set to a ` +
...
export function resolveConfig(
  ...
  return {
    ...
    ...((): { execFailureAbsorb: boolean; execFailureAbsorbWarning: string | null } => {
    ...
    ...((): { inTurnCheckpointUsd: number; inTurnCheckpointWarning: string | null } => {
      const itc = resolveInTurnCheckpointUsd(pluginConfig);
      return { inTurnCheckpointUsd: itc.usd, inTurnCheckpointWarning: itc.warning };
    })(),
  };
}
