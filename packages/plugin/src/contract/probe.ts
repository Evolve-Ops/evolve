...
import type { CliResult, ConfigValidation, OcProbe } from "./types.js";
 ...
 */
export function cliUnavailable(r: CliResult): boolean {
  const text = `${r.stdout}\n${r.stderr}`;
...
export function assertReadOnlyArgs(args: readonly string[]): void {
  for (const a of args) {
    if (WRITE_FLAGS.has(a)) {
      throw new Error(
        `contract probe refused a write-capable OpenClaw invocation: ${a} ` +
...
function collectTextFiles(root: string, maxFiles: number): string[] {
  const out: string[] = [];
  const stack = [root];
...
export function createCliProbe(opts: {
  ...
  const readRuntimeText = (): string => {
    if (runtimeText !== null) return runtimeText;
    const parts: string[] = [];
  ...
  return {
    async cliAvailable(): Promise<boolean> {
      if (cliOk === null) cliOk = !cliUnavailable(await cli(["--version"]));
      return cliOk;
    },
    ...
    async scratchDir(name: string): Promise<string> {
      const dir = join(home, name);
      mkdirSync(dir, { recursive: true });
      return dir;
    },

    async stageConfig(config: unknown): Promise<void> {
      writeFileSync(join(home, ".openclaw", "openclaw.json"), JSON.stringify(config, null, 2), "utf8");
    },

    async validateConfig(config: unknown): Promise<ConfigValidation> {
      writeFileSync(join(home, ".openclaw", "openclaw.json"), JSON.stringify(config, null, 2), "utf8");
    ...
    async runtimeMentions(needle: string): Promise<boolean> {
      return readRuntimeText().includes(needle);
    },

    async evolveSource(repoRelPath: string): Promise<string | null> {
      try {
