...
import {
  evolveAuthoredConfig,
  EVOLVE_AUTHORED_KEYS,
  EVOLVE_PLUGIN_INSTALL_FLAGS,
  REQUIRED_HOOK_CTX_FIELDS,
} from "./fixtures.js";
import type { CheckResult, CliResult, ContractCheck, OcProbe } from "./types.js";
 ...
 */
async function stagedEvolveConfig(probe: OcProbe): Promise<Record<string, unknown>> {
...
function verdict(
  check: { id: string; title: string; incident: string },
 ...
 */
export function looksLikeUnknownCommand(r: CliResult): boolean {
  const text = `${r.stdout}\n${r.stderr}`;
...
export function hookHandlerSource(source: string, hook: string): string | null {
...
const subagentReentryGuarded: ContractCheck = {
  ...
  async run(probe: OcProbe): Promise<CheckResult> {
    ...
    const routeAt = Math.min(
      ...["preflightRouter", "handleBeforeModelResolve"]
    ...
    if (guardAt < 0) {
      return verdict(
        this,
        "fail",
        "The before_model_resolve handler has no classifyEvolveSubagentKey guard. Under OC 2026.9.2 the hook fires for the plugin's own subagent sessions, so the preflight router classifies its own prompt — 2,998 calls in four hours on one bot.",
        evidence,
      );
    }
    if (routeAt !== Number.MAX_SAFE_INTEGER && guardAt > routeAt) {
      return verdict(
        this,
        "fail",
        "The classifyEvolveSubagentKey guard runs AFTER the routing call it is supposed to prevent, so a self-call still reaches the preflight router.",
        evidence,
      );
    }
...
const silentTokenStaysSilent: ContractCheck = {
  ...
  async run(probe: OcProbe): Promise<CheckResult> {
    ...
    const roomEventAccepted =
      v.ok || !v.messages.some((m) => /unmentionedInbound|room_event/i.test(m));
...
const evolveConfigKeysAccepted: ContractCheck = {
...
const beforeAgentReplyShortCircuit: ContractCheck = {
...
const hookCtxFieldsPresent: ContractCheck = {
  ...
  async run(probe: OcProbe): Promise<CheckResult> {
    ...
    for (const field of REQUIRED_HOOK_CTX_FIELDS) {
      if (!(await probe.runtimeMentions(field))) missing.push(field);
    }
...
const pluginInstallFlagsAccepted: ContractCheck = {
...
const doctorPreservesModelRefs: ContractCheck = {
...
const channelPluginVersionsMatch: ContractCheck = {
  ...
  async run(probe: OcProbe): Promise<CheckResult> {
    ...
    for (const e of entries) {
      const id = String((e as { id?: unknown; name?: unknown }).id ?? (e as { name?: unknown }).name ?? "");
      const ver = String((e as { version?: unknown }).version ?? "");
      if (!id.startsWith("@openclaw/") || !ver) continue;
      if (line && !ver.startsWith(`${line}.`)) mismatched.push(`${id}@${ver}`);
    }
...
const gatewayStatusDeepSurface: ContractCheck = {
...
export const CONTRACT_CHECKS: readonly ContractCheck[] = [
