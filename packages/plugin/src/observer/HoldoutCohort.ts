 ...
 *
 * Spec: internal/spec-tier-cascade-2026-05-26.md § 2.3 Component 5.
 *
...
export const DEFAULT_HOLDOUT_CONFIG: Required<HoldoutCohortConfig> = {
  enabled: true,
  target_rate: 0.02,
 ...
 */
function hashScore(botId: string, sessionId: string): number {
 ...
 */
export function shouldBeInHoldout(
  botId: string,
  sessionId: string,
  config?: HoldoutCohortConfig,
): boolean {
  const cfg: Required<HoldoutCohortConfig> = { ...DEFAULT_HOLDOUT_CONFIG, ...(config ?? {}) };
  if (cfg.enabled === false) return false;
  if (!botId || !sessionId) return false;
  if (cfg.target_rate <= 0) return false;
  if (cfg.target_rate >= 1) return true;
 ...
 */
export function inheritFromParent(parentInHoldout: boolean): boolean {
...
export function checkOversized(
  observedRate: number,
  config?: HoldoutCohortConfig,
): { oversized: boolean; observed_rate: number; threshold: number } {
  const cfg: Required<HoldoutCohortConfig> = { ...DEFAULT_HOLDOUT_CONFIG, ...(config ?? {}) };
  return {
