...
const SHELL_ERROR_KEYWORDS: RegExp = new RegExp(
 ...
 */
export function isShellErrorPaste(text: string): boolean {
 ...
 */
const BOT_SELF_CORRECTION_PATTERNS: RegExp[] = [
 ...
 * Pure function — exported for tests.
 */
export function isBotSelfCorrection(text: string): boolean {
...
interface SessionState {
  /** Recent turn observations — bounded ring of last MAX_HISTORY entries. */
  history: Array<{
    endedAt: number;          // epoch ms
    userIsShellErrorPaste: boolean;
    botIsSelfCorrection: boolean;
  }>;
}
...
const EMPTY_SIGNAL: SessionStruggleSignal = Object.freeze({
...
export class SessionStruggleAggregator {
  ...
  getSessionSignal(sessionId: string): SessionStruggleSignal {
    ...
    for (const entry of h) {
      if (entry.userIsShellErrorPaste) shell += 1;
      if (entry.botIsSelfCorrection) correction += 1;
    }
 ...
 */
export function isSessionStruggleElevated(signal: SessionStruggleSignal): boolean {
