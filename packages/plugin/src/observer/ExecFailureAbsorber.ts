 ...
 */
const UNCLASSIFIED_RUN_FAILURE_RE = new RegExp(
  `^(?:>\\s*)?${WARN}\\s*Agent run failed \\((?:model|provider):\\s*[^)\\r\\n]{1,200}\\)\\.$`,
  "iu",
);
 ...
 */
export function isUnclassifiedRunFailureLine(line: string): boolean {
 ...
 */
export function decideAbsorb(
  content: string,
  matches: (line: string) => boolean = isExecFailureTrailerLine,
): AbsorbDecision | null {
...
export class ExecFailureAbsorber {
  ...
  private absorbRunFailure(
    ...
    if (!scheduled) {
      this.logger.info(
        `ExecFailureAbsorber: unclassified run-failure banner on a ` +
    ...
    this.logger.info(
      `ExecFailureAbsorber: absorbed an unclassified run-failure banner on ` +
