...
function evolvePythonBin(): string {
  try {
 ...
 * (docs/spec-agent-freelance-bypass-phase2-2026-06-06.md). Built by
...
export class TurnObserver {
   ...
   */
  private _capBlock: { text: string; at: number } | null = null;
   ...
   */
  private _dirDigestBlock: { text: string; at: number } | null = null;
  ...
  private async runTaskExtractor(sessionId: string, turns: TurnRecord[]): Promise<void> {
    // Only extract tasks if the feature is enabled
    if (!this.config.enableTaskExtraction) return;
  ...
  private resolveModelRouting(ctx: any, sessionKey: string): Record<string, string> {
    ...
    if (modelOverride) result.modelOverride = modelOverride;
  ...
  private async handleBeforeAgentRun(
    ...
    try {
      const _evt = event as { senderId?: string; senderIsOwner?: boolean; channelId?: string };
      captureSender(ctx?.runId, {
        ...
        platform: (ctx?.channelId as string | undefined) ?? _evt.channelId ?? null,
  ...
  private writeTurnToShared(sessionId: string, llm: SessionLlmData | undefined, costEstimated = 0, ctx?: any): void {
