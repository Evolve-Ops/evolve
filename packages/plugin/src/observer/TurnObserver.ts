...
import {
  ...
  type EvolveSubagentTriggerKind,
} from "./subagentRun.js";
import { PreflightCallCap } from "./preflightCallCap.js";
import { classifySessionKind } from "../tools/ToolProfiles.js";
import {
  estimateCost, priceCall, EXACT_MODEL_COSTS, _resetPricingCatalogCache,
  type CostSource, type PricedCall,
...
import {
  readCostBreakerDecision,
  type VetoDecision,
} from "../breakers/BreakerStateReader.js";
import {
  classifyCheckpointAnswer,
  readCostCheckpoint,
  renderAnswerUnavailableMessage,
  renderCapReachedMessage,
  renderContinuedMessage,
  renderDeclinedMessage,
  renderNotOwnerMessage,
} from "../breakers/CostCheckpoint.js";
...
): { llm: SessionLlmData; costEstimated: number; priced: PricedCall } | null {
 ...
 * Pure function — no I/O, no module state — for testability.
 */
export function _buildScheduledToolDisciplineNote(): string {
  return (
    `[EVOLVE BACKGROUND TURN] No one is waiting on this turn, so spend calls ` +
 ...
 * Pure function — no I/O, no module state — for testability.
 */
export function _buildRoutingNotice(
  driver: string | null,
  model: string,
  role: string | null,
 ...
 * banner alone.
 */
export function _fallbackNoticeActiveModel(text: string): string | null {
 ...
 */
export function _sameModelRef(active: string, override: string): boolean {
 ...
 */
export function _isFallbackNoticePayload(payload: any): boolean {
  if (!payload) return false;
  if (payload.isFallbackNotice === true) return true;
...
export type { CostSource, PricedCall };
...
export class TurnObserver {
   ...
   */
  private heartbeatDueCheck: HeartbeatDueCheck;
   ...
   */
  private _dueWakeByRun: Map<string, DueItem[]> = new Map();
   ...
   */
  private _loggedMissingHeartbeatWorkspaceDir = false;
  private static readonly _DUE_WAKE_MAX_RUNS = 64;
   ...
   */
  private classifierGate!: ClassifierGateReader;
   ...
   */
  private readonly _pendingJudgeTail: Set<string> = new Set();
   ...
   * a hook later.
   */
  private readonly _armedJudgeTail: Set<string> = new Set();
  /**
   ...
   */
  private _surfaceForCtx(ctx: any): string | null {
    const raw =
      ctx?.channel ?? ctx?.channelKind ?? ctx?.channel_id ?? ctx?.channelId ?? null;
   ...
   */
  private decideRuleForCtx(ctx: any, triggerKind: string): RoutingRuleDecision {
    let backgroundRole: string | null = null;
    let maintenanceRole: string | null = null;
    try {
   ...
   */
  private readonly _preflightCap = new PreflightCallCap();
  /**
   ...
   */
  private _housekeepingRuns: Map<string, HousekeepingKind> = new Map();
  private _housekeepingLever: HousekeepingLeverReader | null = null;
   ...
   */
  private _breakerSpokenConversations: Set<string> = new Set();
   ...
   */
  private _evolveRoutedRuns: Map<
    string,
    {
      driver: string | null;
      model: string;
      role: string | null;
      /** For driver "role_cap": the role whose daily budget ran out. */
      degradedFrom?: string | null;
    }
  > = new Map();
  /**
   ...
   */
  private _routingNoticeRuns: Set<string> = new Set();
  /**
  ...
  constructor(config: EvolveConfig, logger: PluginLogger, api?: any) {
    ...
    this.modelRouter = new ModelRouter(
      this.loadModelRouterConfig(), config.sharedDir, config.botId, logger,
    );
    ...
    this.sessionJudge = new SessionStruggleJudge(
      config,
      logger,
      api,
      (againstRole?: string) => this.modelRouter.resolveCrossVendor(againstRole),
    );
  ...
  register(api: any): void {
    ...
    api.on(
      ...
      async (event: any, ctx: any) =>
        this.handleBeforeAgentReply(ctx?.runId, ctx, event),
      ...
      api.on("before_prompt_build", async (_event: any, ctx: any) => {
        try {
          ...
          if (runId && this._dueWakeByRun.has(String(runId))) {
            ...
            if (due.length > 0) duePrepend = buildWakeContext(due);
          }
          ...
          try {
            const kind = classifySessionKind(
              ctx?.sessionKey ?? ctx?.sessionId ?? null,
              null,
            ).kind;
            if (kind === "scheduled") {
    ...
    try {
      api.on(
        ...
        async (event: any) =>
          withInTurnOwnerNotice(event, this.handleReplyPayloadSending(event)),
        { name: "evolve-model-routing-notice" },
      );
    } catch (err) {
      ...
      this.logger.warn(
        `Evolve model-routing notice: reply_payload_sending not supported ` +
    ...
    api.on("before_model_resolve", async (event: any, ctx: any) => {
      try {
        ...
        if (
          classifyEvolveSubagentKey(sessionKey) !== null
        ...
        try {
          ...
          if (intercepted) {
            ...
            return this._withJudgeTail(
              _ctx,
              caps.modelRouting ? this.resolveModelRouting(_ctx, sessionKey) : {},
            );
          }
        } catch (err) {
        ...
        if (betterResult.systemAppend) {
          ...
          return this._withJudgeTail(
            _ctx,
            { ...modelResult, systemAppend: betterResult.systemAppend },
          );
        }
   ...
   */
  handleReplyPayloadSending(event: any): { payload?: any } | undefined {
    ...
    if (typeof _tailPayload?.text === "string" && hasTailMarker(_tailPayload.text)) {
      ...
      if (parsed.found) {
        this.logger.debug(
          `Evolve judge: stripped verdict tail from an outbound payload ` +
          `(run ${String(event?.runId ?? "").slice(0, 8)})`
        );
        return { payload: { ..._tailPayload, text: parsed.stripped } };
      }
      this.logger.warn(
        `Evolve judge: an outbound payload carries an unparseable verdict ` +
    ...
    try {
      ...
      if (!_isFallbackNoticePayload(payload)) return undefined;
      const text = typeof payload.text === "string" ? payload.text : "";
      if (!text) return undefined;
      if (this._routingNoticeRuns.has(key)) return undefined;
      ...
      if (!active || !_sameModelRef(active, routed.model)) {
        ...
        this.logger.info(
          `Evolve model-routing notice: banner names ${active ?? "an unparseable model"} ` +
      ...
      if (this._routingNoticeRuns.size >= 1024) {
        ...
        if (oldest !== undefined) this._routingNoticeRuns.delete(oldest);
      }
      this._routingNoticeRuns.add(key);

      const notice = _buildRoutingNotice(
        routed.driver, active, routed.role, routed.degradedFrom,
      );
      this.logger.info(
        `Evolve model-routing notice: replaced OC's fallback banner ` +
   ...
   */
  private _breakerConversationKey(ctx?: any, event?: any): string {
    const candidates = [
  ...
  private _markBreakerSpoken(key: string): void {
    if (this._breakerSpokenConversations.size
      ...
      if (!oldest.done) this._breakerSpokenConversations.delete(oldest.value);
    }
  ...
  private _breakerReplyClaim(
    ...
    try {
      ...
      this.logger.warn(
        `Evolve cost breaker reply-claim check failed (treating the bot as ` +
   ...
   */
  private _recordBreakerRefusal(
    trigger: string, decision: { scope?: string; tripId?: string },
    note: string,
  ): void {
    const now = Date.now();
    if (now - this._lastBreakerRefusalLogMs < 60_000) return;
    this._lastBreakerRefusalLogMs = now;
    this.logger.info(
      `Evolve cost breaker: outcome=breaker_refused ` +
  ...
  handleBeforeAgentReply(
    ...
    ctx?: {
      ...
      sessionId?: unknown;
      jobId?: unknown;
      // Read by the cost-breaker reply-claim, which keys its
      // one-message-per-trip budget by conversation.
      sessionKey?: unknown;
      chatId?: unknown;
      channelId?: unknown;
    } | null,
    ...
    try {
      ...
      if (breakerClaim !== undefined) return breakerClaim;
      if (runId && this._directSentRuns.has(runId)) {
      ...
      if (dueDecision) return dueDecision;
    } catch (err) {
  ...
  private _decideHeartbeatDue(
    ...
    // never be suppressed.
    if (!DUE_CHECK_TRIGGERS.includes(trigger)) return undefined;
    ...
    if (!workspaceDir) {
      ...
      if (!this._loggedMissingHeartbeatWorkspaceDir) {
        ...
        this.logger.warn(
          `Evolve heartbeat: a ${trigger} trigger reached the due-check with no ` +
    ...
    if (verdict.decision === "skip") {
      ...
      this.logger.info(
        `Evolve heartbeat: no model call — ${trigger} ${verdict.reason} ` +
    ...
    if (runId && verdict.due.length > 0) {
      if (this._dueWakeByRun.size >= TurnObserver._DUE_WAKE_MAX_RUNS) {
        ...
        if (oldest !== undefined) this._dueWakeByRun.delete(oldest);
      }
    ...
    this.logger.info(
      `Evolve heartbeat: waking the model — ${trigger} ${verdict.reason} ` +
      `(run ${String(runId ?? "?").slice(0, 8)})`,
    );
    return undefined;
  }
  ...
  private async handleTurn(event: any, ctx: any): Promise<void> {
    ...
    const { userMessage, assistantMessage: _rawAssistantMessage } =
      extractMessages(event?.messages);
    ...
    if (this._armedJudgeTail.delete(sessionId) && hasTailMarker(assistantMessage)) {
      ...
      if (parsed.verdict) {
        ...
        this.logger.debug(
          `Evolve judge: session ${String(sessionId).slice(0, 8)} self-reported ` +
          `${parsed.verdict} in its own turn (no second model call)`
        );
      } else if (parsed.malformed) {
        this.logger.debug(
          `Evolve judge: session ${String(sessionId).slice(0, 8)} emitted an ` +
    ...
    if (cachedLlmClass) {
      ...
      if (
        turnCount === 1 && classResult.confidence < 0.65 && userMessage &&
        this.classifierGate.read(this.preflightRouter.hasConfidentPrior()).enabled
      ) {
        this.llmClassifier.classify(
          userMessage, this.config.classifierHints,
    ...
    try {
      ...
      if (this.cascadeTelemetry) {
        ...
        const ruleDecisionForTurn = this.decideRuleForCtx(
          { ...ctx, channel: llm?.channel ?? ctx?.channel },
          triggerKind,
        );
        ...
        const judgeGate = this.classifierGate.read(
          this.preflightRouter.hasConfidentPrior(),
        );
        const judgeSampled = shouldSampleSession(
          this.config.botId, sessionId, judgeGate.sampleRate,
        );
          ...
          if (triggeredBy !== null && judgeSampled) {
            if (judgeGate.enabled) {
              ...
              if (this._pendingJudgeTail.size >= 512) {
                ...
                if (oldest !== undefined) this._pendingJudgeTail.delete(oldest);
              }
  ...
  private resolveModelRouting(ctx: any, sessionKey: string): Record<string, string> {
    ...
    if (!this.modelRouter.getSessionType(sessionKey) && ruleDecision.sessionClass) {
      ...
      this.logger.info(
        `Evolve routing rule: ${String(sessionKey).slice(0, 8)} → ` +
    ...
    if (modelOverride) {
      ...
      if (ctx?.runId) {
        if (this._evolveRoutedRuns.size >= 1024) {
          ...
          if (oldest !== undefined) this._evolveRoutedRuns.delete(oldest);
        }
   ...
   */
  private resolveHousekeepingRouting(
    ctx: any,
    sessionKey: unknown,
    ...
    if (runId && this._housekeepingRuns.has(runId)) {
      this.logger.info(
        `Evolve housekeeping: ${kind} run ${runId.slice(0, 8)} re-resolving (failover lane) — standing down`,
      );
      return {};
    }
    if (runId) {
      if (this._housekeepingRuns.size >= 1024) {
        ...
        if (oldest !== undefined) this._housekeepingRuns.delete(oldest);
      }
    ...
    if (!this._housekeepingLeverOn()) {
      this.logger.info(
        `Evolve housekeeping: ${kind} for ${String(sessionKey ?? "").slice(0, 24)} left on the ` +
        `session model — lever housekeeping_cheap_rung is off for this bot`,
      );
      return {};
    }
    if (ctx?.userModelOverride) return {};
    const picked = resolveHousekeepingModel(
      this.modelRouter.resolveFastRoleOverride(),
      this.config.classifierModel,
    );
    if (!picked) return {};
    this.logger.info(
      `Evolve housekeeping: routing ${kind} run ${runId ? runId.slice(0, 8) : "<no-run>"} ` +
  ...
  private _housekeepingKindForRun(ctx: any): HousekeepingKind | null {
   ...
   */
  private _recordHousekeepingTurn(sessionId: string, ctx: any, kind: HousekeepingKind): void {
    const llm = this.sessionLlmData.get(sessionId);
    this.sessionLlmData.delete(sessionId);
    if (typeof ctx?.runId === "string") this._housekeepingRuns.delete(ctx.runId);
    if (!llm) return;
    const tagged: SessionLlmData = { ...llm, source: kind };
    const priced = priceCall(
      tagged.model, tagged.inputTokens, tagged.outputTokens,
      tagged.cacheWriteTokens, tagged.cacheReadTokens,
      tagged.provider, this.config.sharedDir,
    );
    this.writeTurnToShared(sessionId, tagged, priced, ctx);
    this.logger.info(
      `Evolve housekeeping: recorded ${kind} for session ${String(sessionId).slice(0, 8)} ` +
  ...
  private _housekeepingLeverOn(): boolean {
    if (!this.config.sharedDir) return true;
   ...
   */
  private async _handleCostCheckpointTurn(
    userMessage: string,
    ctx: any,
  ): Promise<BeforeAgentRunResult | null> {
    ...
    const blockOf = (message: string, note: string): BeforeAgentRunResult => ({
      outcome: "block",
    ...
    if (answer === null) {
      return blockOf(
        status.state === "declined"
          ...
          : renderCapReachedMessage(status, botName),
        status.state === "declined" ? "already declined" : "awaiting an answer",
      );
    }

    const sender = getSender(ctx?.runId);
    const outcome = await this._recordCheckpointAnswer(
      answer, sender?.platform ?? null, sender?.senderId ?? null,
    );
    ...
    if (outcome === "not_owner") {
      this.logger.info(
        `Evolve cost checkpoint: non-owner "continue" refused for ` +
        `bot=${this.config.botId}`,
      );
    ...
    if (answer === "stop") {
      this.logger.info(
        `Evolve cost checkpoint: declined for bot=${this.config.botId}`,
      );
      return blockOf(renderDeclinedMessage(botName), "declined by owner");
    }
    this.logger.info(
      `Evolve cost checkpoint: continued for bot=${this.config.botId} ` +
      `(+$${(status.incrementUsd ?? 0).toFixed(2)})`,
    );
    return blockOf(
      renderContinuedMessage(status, botName, status.incrementUsd),
      "continued by owner",
    );
  }
   ...
   */
  private async _recordCheckpointAnswer(
    answer: "continue" | "stop",
    platform: string | null,
    stableId: string | null,
  ): Promise<"recorded" | "not_owner" | "unavailable"> {
    let res;
    try {
      ...
      this.logger.warn(
        `Evolve cost checkpoint: daemon call failed: ${err?.message ?? err}`,
      );
      return "unavailable";
    }
    if (res.status < 200 || res.status >= 300) {
      this.logger.warn(
        `Evolve cost checkpoint: daemon returned ${res.status} — no change`,
      );
      return "unavailable";
    }
  ...
  private async handleBeforeAgentRun(
    ...
    if (this.config.capabilities.modelRouting) {
      try {
        const _cpTrigger = (ctx?.trigger as string | undefined) ?? "";
        const _cpChannel = (ctx?.channelId as string | undefined)
        ...
        this.logger.warn(
          `Evolve cost checkpoint check failed (allowing turn): ${err?.message ?? err}`,
        );
      }
    }
    ...
    try {
      ...
      if (isAutoSource({ source: trigger, channel: channelId })) {
        ...
        if (decision.vetoed) {
          this._recordBreakerRefusal(
            trigger || "unknown", decision,
   ...
   */
  private _preflightAdmitted(ctx: any, sessionKey: string): boolean {
    const verdict = this._preflightCap.admit(ctx?.runId, sessionKey);
    if (verdict.logLine) this.logger.warn(verdict.logLine);
  ...
  private writeTurnToShared(
    sessionId: string,
    llm: SessionLlmData | undefined,
    priced: PricedCall = { costUsd: null, costSource: "unpriced" },
    ctx?: any,
    appAttribution?: AppAttributionResult,
  ): void {
