 ...
 */
// ── The prompt-borne self-call sentinel ─────────────────────────────────────
//
// `classifyEvolveSubagentKey` recognises Evolve's own runs by the SHAPE of a
// session key OC derives from our idempotencyKey
// ("agent:main:explicit:evolve:<tag>:…"). That derivation is OC's, not ours:
// it is undocumented, it changed behaviour once already (2026.9.2 began
// firing before_model_resolve on the plugin-subagent lane at all — the
// 2026-09-08 loop), and a future change to how OC builds the key would
// silently reopen the recursion this guard exists to close.
//
// So every model call the plugin makes FOR ITSELF also carries a marker in
// the prompt text, which only this module writes and only the hook reads.
// The prompt is ours end to end — no OC derivation sits between writing the
// marker and matching it — so the second check holds even if the first stops
// matching. The hook applies both, independently: either one alone stops the
// loop.
//
// Deliberately not a nonce. The hook has to recognise a call it did not make
// (a different process, a run that outlived a gateway restart), so the marker
// has to be a constant both sides compile in. It is not a secret and grants
// nothing: a forged marker makes Evolve skip ITS OWN routing for that turn,
// which is the safe direction — the turn is answered by the bot's configured
// primary, exactly as it is when the router abstains.
export const EVOLVE_INTERNAL_PROMPT_SENTINEL = "[evolve:internal-model-call]";

/**
 * Does this prompt text carry the marker `runPinnedSubagent` stamps on every
 * plugin-internal model call?
 *
 * Matched as a literal substring, never a regex: the marker must be
 * recognisable in a message OC has wrapped, timestamped or prefixed on its
 * way to the hook, and a substring test survives all three.
 */
export function isEvolveInternalPrompt(message: unknown): boolean {
  return typeof message === "string"
    && message.includes(EVOLVE_INTERNAL_PROMPT_SENTINEL);
}
...
export async function runPinnedSubagent(
  ...
  const { model, interactiveTurn: _interactiveTurn, ...params_ } = params;
  // Stamp the self-call marker on the prompt. Every plugin-internal model
  // call funnels through here, so this is the one place that has to remember
  // — a new call site gets the marker by construction, the same way it gets
  // the breaker gate. See EVOLVE_INTERNAL_PROMPT_SENTINEL.
  const rest = {
    ...params_,
    message: isEvolveInternalPrompt(params_.message)
      ? params_.message
