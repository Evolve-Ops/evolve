...
/**
 * Per-feature saturation thresholds. Each feature is normalized as
 * `min(raw / saturation, 1)`, so a saturation of 3 means "three of these
 * is full signal." Set via SATURATION below; intentionally not configurable
 * yet — Phase 4 audit can propose changes once we have data.
 */
interface StruggleSaturation {
  tool_error_count: number;     // 2 = two tool errors saturates
  tool_retry_count: number;     // 3 = three retries saturates
  restart_markers: number;      // 2 = two restart phrases saturates
  clarification_loops: number;  // 1 = single user correction saturates
  tokens_per_progress: number;  // ms-per-tool-call ratio; saturation in ms
  tool_count_per_turn: number;  // 8 = eight tool calls saturates (see WEIGHTS note)
}
...
export const DEFAULT_STRUGGLE_WEIGHTS: StruggleWeights = {
...
const SATURATION: StruggleSaturation = {
  tool_error_count: 2,
  tool_retry_count: 3,
  restart_markers: 2,
 ...
 */
export function countToolErrors(messages: unknown[]): number {
  let n = 0;
  for (const b of userBlocks(messages)) {
    if (b.type === "tool_result" && b.is_error === true) n += 1;
  }
  return n;
}
 ...
 */
export function countToolRetries(messages: unknown[]): number {
 ...
 */
export function countRestartMarkers(messages: unknown[]): number {
  let n = 0;
  for (const b of assistantBlocks(messages)) {
 ...
 */
export function countClarificationLoops(messages: unknown[]): number {
 ...
 */
export function tokensPerProgressRatio(
  messages: unknown[],
  durationMs?: number,
): number {
...
export function countToolCalls(messages: unknown[]): number {
  ...
  for (const m of messages) {
    ...
    if (Array.isArray(msg.tool_calls)) {
      n += (msg.tool_calls as unknown[]).length;
    }
 ...
 * insurance.)
 */
export function computeStruggle(input: ComputeStruggleInput): StruggleSignal {
  const weights: StruggleWeights = {
  ...
  try { raw_tool_error_count = countToolErrors(messages); } catch { /* keep 0 */ }
  try { raw_tool_retry_count = countToolRetries(messages); } catch { /* keep 0 */ }
  try { raw_restart_markers = countRestartMarkers(messages); } catch { /* keep 0 */ }
  try { raw_clarification_loops = countClarificationLoops(messages); } catch { /* keep 0 */ }
  try { raw_tokens_per_progress = tokensPerProgressRatio(messages, input.durationMs); } catch { /* keep 0 */ }
  try { raw_tool_count_per_turn = countToolCalls(messages); } catch { /* keep 0 */ }
  ...
  const contrib: Record<keyof StruggleWeights, number> = {
 ...
 */
function _emptySignal(reason: NonNullable<StruggleSignal["payload_drift"]>): StruggleSignal {
  return {
    score: null,
    features: {
      tool_error_count: 0,
