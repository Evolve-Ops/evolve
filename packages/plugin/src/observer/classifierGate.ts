 ...
 */
export const CLOSED_GATE: Readonly<ClassifierGate> = Object.freeze({
  enabled: false,
  sampleRate: DEFAULT_SAMPLE_RATE,
  source: "unreadable" as const,
});

function clampRate(raw: unknown, fallback: number): number {
 ...
 */
export function readClassifierGate(
  sharedDir: string,
  botId: string,
  hasConfidentPrior: boolean = false,
): ClassifierGate {
  ...
  try {
    ...
    if (bot?.enabled === true) enabled = true;
    else if (bot?.enabled === false) enabled = false;

    const sampleRate = clampRate(
      bot?.sample_rate,
      clampRate(
        hasConfidentPrior ? pod?.sample_rate_with_prior : pod?.sample_rate,
        defaultRate,
      ),
    );
...
export class ClassifierGateReader {
  ...
  private cached: { gate: ClassifierGate; withPrior: boolean; at: number } | null = null;
  ...
  read(hasConfidentPrior: boolean = false): ClassifierGate {
    const now = Date.now();
    if (
      this.cached &&
