 ...
 * (docs/spec-agent-freelance-bypass-phase2-2026-06-06.md). The TurnObserver's
 ...
 */
export const KNOWN_PROTOCOLS: ReadonlyArray<ProtocolName> = [
  "atlas_research",
  "atlas_capture",
  "raw_text",
];

export function isKnownProtocol(name: unknown): name is ProtocolName {
  return typeof name === "string" && KNOWN_PROTOCOLS.includes(name as ProtocolName);
}
...
// (docs/spec-agent-freelance-bypass-phase2-2026-06-06.md) — it lets Layer C
...
function parseRawText(stdout: string): ParsedReply {
  const text = stdout.trim();
  if (!text) return { text: null, outcome: "silent" };
...
function parseAtlasResearch(stdout: string): ParsedReply {
...
function parseAtlasCapture(stdout: string): ParsedReply {
  const lines = stdout.split(/\r?\n/);
  for (const rawLine of lines) {
    const line = rawLine.trim();
    if (!line) continue;
