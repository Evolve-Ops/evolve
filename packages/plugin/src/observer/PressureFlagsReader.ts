...
export class PressureFlagsReader {
  ...
  private _cached: { flags: PressureFlags | null; at: number } | null = null;
   ...
   */
  read(nowMs: number = Date.now()): PressureFlags | null {
    if (this._cached && nowMs - this._cached.at < CACHE_TTL_MS) {
      return this._cached.flags;
    }
    let flags: PressureFlags | null = null;
    try {
      if (fs.existsSync(this.filePath)) {
        const raw = fs.readFileSync(this.filePath, "utf8");
        flags = JSON.parse(raw) as PressureFlags;
