...
const _SESSION_CONTEXT_BLOCK_RE =
  /<session-context>[\s\S]*?<\/session-context>/i;

// The page-context tag carries attributes (surface=, page=, view=, …) —
// see format_page_context in evolve_admin/evo/proxy.py.
const _PAGE_CONTEXT_BLOCK_RE =
  /<page-context\b[^>]*>[\s\S]*?<\/page-context>/i;
...
const _LEADING_TIMESTAMP_RE =
  /^\s*\[(?:[A-Za-z]{3},?\s+)?\d{4}-\d{2}-\d{2}\s+\d{1,2}:\d{2}(?::\d{2})?(?:\s+[^\]\n]{1,14})?\]\s*/;
 ...
 * trimming.
 */
export function unwrapUserMessage(raw: string | null | undefined): string {
