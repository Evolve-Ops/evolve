...
export function buildDecisionRecord(args: BuildDecisionArgs): HeartbeatDecisionRecord {
...
export function decisionsFilePath(sharedDir: string, botId: string, day: string): string {
...
export function appendDecision(
  sharedDir: string,
  record: HeartbeatDecisionRecord,
  ...
  try {
    ...
    if (code === "EACCES" || code === "EPERM") {
      logger?.debug?.(
        `Evolve heartbeat: decisions ledger for ${record.instance} is owned by ` +
