...
import { runPinnedSubagent, subagentPinDenied } from "./subagentRun.js";
 ...
 */
export function _parseJudgeResponse(response: string | null | undefined): {
  verdict: JudgeVerdict;
  reason: string;
} {
  if (!response) {
  ...
  const hasAlt =
    (first !== "STRUGGLING" && /\bSTRUGGLING\b/.test(text)) ||
 ...
 * Pure function — exported for tests.
 */
export function _buildConversationSnippet(
  turns: ReadonlyArray<{ userMessage: string; assistantMessage: string }>,
): string {
...
export class SessionStruggleJudge {
  ...
  private readonly crossVendorResolver: CrossVendorResolver | null;

  constructor(
    config: EvolveConfig,
    logger: PluginLogger,
    api: unknown,
    crossVendorResolver?: CrossVendorResolver,
  ) {
    this.config = config;
    this.logger = logger;
    this.api = api;
    this.crossVendorResolver = crossVendorResolver ?? null;
  }
   ...
   */
  private _pickJudgeModel(): { model: string | undefined; crossVendor: boolean } {
    if (!subagentPinDenied()) {
      try {
        const derived = this.crossVendorResolver?.("standard") ?? null;
        if (derived) return { model: derived, crossVendor: true };
      } catch (err) {
        this.logger.debug(`Evolve: cross-vendor judge derivation failed: ${err}`);
      }
    }
    return {
   ...
   * reply path).
   */
  async judge(input: JudgeInput): Promise<JudgeDecision> {
    const start = Date.now();
    const { model, crossVendor } = this._pickJudgeModel();
    const finishAt = (verdict: JudgeVerdict, reason: string): JudgeDecision => ({
      verdict,
      reason,
      latency_ms: Date.now() - start,
      triggered_by: input.triggeredBy,
      // Re-checked at stamp time: the pin may be denied DURING this very run
      // (runPinnedSubagent retries unpinned on the first denial), in which
      // case the judge ran on the session-default model regardless of what
      // was picked — self-judged, not cross-vendor.
      cross_vendor: crossVendor && !subagentPinDenied(),
    });
    ...
    try {
      ...
      // Cross-vendor first (design judge-role-collapse §5.3): judge on a
      // provider other than the one that produced the work when the pod has
      // one; otherwise classifierModel exactly as before (self-judged).
      ...
      return finishAt(parsed.verdict, parsed.reason);
    } catch (err) {
...
export function shouldRunJudge(
  signal: SessionStruggleSignal,
): JudgeInput["triggeredBy"] | null {
  const reasons: JudgeInput["triggeredBy"][] = [];
  if (signal.shell_error_paste_count >= 1) reasons.push("shell_paste");
  if (signal.bot_self_correction_count >= 1) reasons.push("self_correction");
  if (
    signal.turn_velocity_per_min !== null
