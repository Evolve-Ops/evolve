...
export interface ModelRouterConfig {
  ...
  rungs: Rung[];
  // Role ID -> rung slug (or the structured JudgeRole for `judge`).
  roles: {
    fast?: string;
    standard?: string;
    power?: string;
...
export const DEFAULT_MODEL_CATALOG: any = {
  rungs: [
    ...
    {
      // judge's OWN rung (spec §Addendum 16 — judge decoupled from standard).
      // Same medium-cost, provider-diverse cluster standard ships with, so
      // judge routing is unchanged, but judge now has an independent model
      // list + order the editors can reorder/curate without sharing standard's.
      id: "judge-class",
      costClass: "medium",
      models: [
        "anthropic/claude-sonnet-4-6",
  ...
  roles: {
    ...
    power: "opus-class",
    max: "fable-class",
    // judge rides its OWN judge-class rung (spec §Addendum 16), still
    // diversity-constrained to a provider other than the standard role's
    // primary (judge-class holds several non-anthropic providers so judge
    // diversity is satisfiable from defaults alone). A separate rung lets the
    // editors reorder/curate judge independently of standard.
...
export class ModelRouter {
  ...
  resolveRoleToModel(role: string): string | null {
    if (role === "judge") return this._resolveJudgeModel();
   ...
   */
  private _resolveJudgeModel(): string | null {
    ...
    for (const m of models) {
      if (this._providerOf(m) !== standardProvider) return m;
    }
  ...
  resolveRoleAvailability(role: string): {
    ...
    if (role === "judge") {
      ...
      const rung = this._rung(
        typeof this.config.roles?.judge === "string"
          ? this.config.roles.judge
          : this.config.roles?.judge?.rung,
      );
      const providers = this._rungProviders(rung);
      let reason: "uncredentialed" | "unconfigured" | "same_vendor_as_standard" | null;
      if (model) reason = sameVendor ? "same_vendor_as_standard" : null;
      else reason = providers.length ? "uncredentialed" : "unconfigured";
      return {
        requestedRole: "judge",
  ...
  private _resolveJudgeModelAvailable(
    avail: Set<string>,
  ): { model: string | null; sameVendor: boolean } {
    const judge = this.config.roles?.judge;
    if (!judge) return { model: null, sameVendor: false };
    const slug = typeof judge === "string" ? judge : judge.rung;
    const rung = this._rung(slug);
    const models = rung?.models ?? [];
    if (models.length === 0) return { model: null, sameVendor: false };
   ...
   */
  private _warnIfJudgeSameVendor(): void {
    const judge = this.config.roles?.judge;
    if (!judge || typeof judge === "string") return;  // no preference to check
    if (judge.provider !== "not-standard") return;
    const rung = this._rung(judge.rung);
    const models = rung?.models ?? [];
    if (models.length === 0) return;  // empty-rung warn handled elsewhere
    ...
    if (models.some((m) => this._providerOf(m) !== standardProvider)) return;
    try {
      // eslint-disable-next-line no-console
      console.warn(
        `[Evolve ModelRouter] NOTE at startup: the 'judge' role prefers a ` +
  ...
  getRoleForModel(modelString: string | undefined | null): string | null {
    ...
    const roleIds: RoleId[] = ["fast", "standard", "power", "max", "judge"];
