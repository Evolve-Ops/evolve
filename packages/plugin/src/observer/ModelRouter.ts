 ...
 * not Greenwich's — see internal/spec-user-tier-control-2026-05-26.md §"Pod-local
...
function spendCapFlagPath(sharedDir: string, botId: string): string {
 ...
 *
 * Only the EXPLICIT ``downgrade-tier`` action is enforced here. The default
 * action since 2026-09-04 is ``checkpoint`` (operator decision D-CC2,
 * internal/decision-cost-cap-checkpoint-2026-09-04.md), which this predicate
 * deliberately does NOT match: under the checkpoint a tripped cap must leave
 * routing alone and hold the turn instead — a held turn never reaches model
 * resolution at all, and the turns after an owner says "continue" are answered
 * at the bot's normal quality on the cap the owner just raised. Silently
 * pinning them to `fast` is precisely the undisclosed third action the
 * decision removed.
 *
 * The other actions (pause-crons, suspend-bot) are executed by spend_alert.py
 * at trigger time and likewise do not route here.
 */
export function isSpendCapActive(sharedDir: string, botId: string): boolean {
  try {
...
function _IS_SPECIFIC_CLASS(c: unknown): boolean {
...
export function splitProviderModelRef(
  ...
  if (slash <= 0 || slash >= ref.length - 1) return { modelOverride: ref };
  return { providerOverride: ref.slice(0, slash), modelOverride: ref.slice(slash + 1) };
}

export interface AccountTier {
  description?: string;
  profiles: string[];           // auth profile IDs e.g. "anthropic:user@example.com"
  for_session_types: string[];  // session types this tier handles
}
 ...
 */
const _CLASSIFIER_ROLES: ReadonlySet<string> = new Set(["fast", "standard", "power"]);

/** A capability/cost cluster in the rung catalog (cheapest first). */
export interface Rung {
  id: string;                 // stable slug, e.g. "sonnet-class"
  models: string[];           // cross-provider cluster: primary first
  costClass?: "low" | "medium" | "high" | "premium";
}

export interface ModelRouterConfig {
  // Ordered catalog: array position = cost rank (cheapest first). Each
  // role points at a rung slug; role resolution returns the rung's first
  // model. The legacy `models.tiers.tierN` shape is no longer synthesized
  // here — it is refused loudly (see LegacyTierShapeError).
  rungs: Rung[];
  // Role ID -> rung slug, uniformly. (An un-migrated config may still carry
  // a stale `judge` key — bare slug or the old structured object; it parses
  // as an extra key nothing reads. The judge-role collapse removed the role.)
  roles: {
    fast?: string;
    standard?: string;
    power?: string;
  ...
  // Per-role daily caps (spec §max semantics #6). Generalizes the legacy
  // single tier1 cap. On cap hit a role degrades down the chain
  // (max -> power -> standard); see canEscalateToRole.
  roleCaps?: {
    power?: { maxPerDayPerBot?: number };  // default 10
    max?: { maxPerDayPerBot?: number };    // default 5
  };
  routing?: {
    maintenanceRole?: string;        // default: "fast"
    backgroundRole?: string;         // default: "fast"
    ambiguousRole?: string | null;   // default: null (use bot default)
    enabled?: boolean;               // default: true
    confidenceThreshold?: number;    // default: 0.65
    // May the CONTENT classifier downgrade a human conversation it labels
    // `maintenance` (system-upkeep chatter) to the maintenance role?
    // Default false: keyword misreads have routed real work to the cheap
    // model (the OC 2026.7 incident class), so content-based downgrades
    // are opt-in. Trigger-anchored maintenance (Evolve's own scaffolding
    // subagents) and background (cron/heartbeat) sessions are routed
    // regardless — their provenance is objective, not guessed.
    classifierDowngrade?: boolean;   // default: false
  };
  // From network.json accounts section
  accountTiers?: Record<string, AccountTier>;
  accountRouting?: {
    enabled?: boolean;
  };
  // Runaway-rate hard cap (spec § 2.6 cost management).
  // The single per-session hard cap retained — catches runaway loops,
  // NOT a cost-control mechanism. Defaults from spec: $20 in 5min.
  runawayRateCap?: {
    enabled?: boolean;          // default: true
    dollarsPerWindow?: number;  // default: 20.0
    windowMinutes?: number;     // default: 5
    // Pod-wide trip count threshold to escalate Signal severity.
    // After this many trips in 24h, fire CRITICAL instead of WARNING.
    criticalTripsPer24h?: number; // default: 3
  };
  // Tier-cascade Phase 3 cutover gate (spec § 2.2 + § 2.6 precedence).
  // When `enabled: true`, the cascade controller's verdict drives
  // routing (inserted between user-override and classifier in the
  // precedence ladder). When false/absent, the controller still
  // computes verdicts for shadow telemetry but does NOT influence
  // model selection — Phase 2 shadow-mode behavior. Defaults to
  // false so a config-omitted bot stays on the classifier until the
  // operator explicitly opts in per-bot after the cutover review.
  cascade?: {
    enabled?: boolean;          // default: false (shadow-only)
  };
  // Per-bot operator-controlled defaults for user-turn / unknown
  // sessions (audit #69 Phase A).
  //
  //   enabled       — gates the admin-UI chip + session_set_tier surface
  //                   (already used pre-Phase-A by SetTierTool /
  //                   home_chat_routes).
  //   dailyCap      — LEGACY: per-bot power-role daily cap. Superseded by
  //                   models.roleCaps.power.maxPerDayPerBot; read here
  //                   only as a back-compat fallback for un-migrated
  //                   configs (mapped into roleCaps.power by the loader).
  //   allowBotInitiated — per-role: { power: <bool>, max: <bool> }. When
  //                   power is true the agent may self-escalate to power
  //                   via session_set_tier; max defaults to false (§max
  //                   semantics #4 — a bot may forward a user's explicit
  //                   ask but may not unilaterally pin Fable). A legacy
  //                   boolean value maps to { power: <value>, max: false }.
  //   defaultRole   — Phase A picker, now role-shaped: "auto" (legacy) /
  //                   "fast" / "standard" / "power". `max` is rejected
  //                   (pull-only). Routed by _resolveOperatorDefaultRole
  //                   and slots into the precedence ladder between
  //                   classifier-returns-null and bot-default.
  //
  // All fields are optional; the absence of the block is equivalent to
  // {enabled: false, defaultRole: "auto"} — full legacy behavior.
  userTierOverride?: {
    enabled?: boolean;
    dailyCap?: number;  // legacy power cap (mapped into roleCaps.power)
    allowBotInitiated?: boolean | { power?: boolean; max?: boolean };
    defaultRole?: string;
    defaultTier?: string;  // legacy alias for defaultRole (loader maps it)
  };
  // Per-user-per-bot persistent default tier (audit #69 Phase C).
  //
  // Loaded from ``{sharedDir}/{botId}/user-tier-prefs.json`` and
  // consulted by _resolveOperatorDefaultTier BEFORE
  // userTierOverride.defaultTier — so a user's personal pref wins
  // over the operator's bot-wide default but still loses to a
  // session-scoped user override (chip / `evo tier X`). Set via
  // ``evo tier-default X`` running through the admin-side handler.
  //
  // Keyed by user_key (from derive_user_key). The plugin matches
  // entries against the current session's user_key, which the
  // TurnObserver computes per-turn from (channel, sender_external_id)
  // and pins on the session via ``setSessionUserKey``.
  //
  // Absence of the block (or empty users map) is the legacy / pre-
  // Phase-C behavior — only the operator default applies.
  userTierPrefs?: {
    // Values are role-shaped (`defaultRole`); `defaultTier` is read as a
    // legacy alias by _resolveRoleFromChoice. A per-user `max` default is
    // honored here (a user pinning Fable as their personal default is an
    // explicit pull), unlike the operator default which rejects it.
    users: Record<string, { defaultRole?: string; defaultTier?: string }>;
  };
  // Set (to the full remediation message) when a legacy tier0-tier3-shaped
  // config was caught at a load seam. _resolveModelAndTier short-circuits on
  // this before every other branch, returning LEGACY_CONFIG_REFUSE_SENTINEL
  // so every turn refuses loudly instead of routing on untrusted shape.
 ...
 */
function loadTiersFile(sharedDir: string, botId: string): any {
 ...
 */
export class LegacyTierShapeError extends Error {
  constructor(detail: string) {
    super(
      `[Evolve ModelRouter] Unmigrated legacy model-tier config: ${detail}. ` +
      "The tier0-tier3 runtime fallback was removed after the fleet " +
      "migration (2026-08-15); this shape no longer routes — affected turns " +
      "are refused rather than silently misrouted. Remediation: run " +
      "`sudo evolve-admin migrate-model-roles --apply` on the pod to " +
      "rewrite the config to the rungs/roles shape.",
    );
    this.name = "LegacyTierShapeError";
  }
}
 ...
 */
export const LEGACY_CONFIG_REFUSE_SENTINEL = "evolve/legacy-tiers-config-unmigrated"; // provider-literal-allow: sentinel, "evolve" is not a real provider

const _LEGACY_TIER_KEY_RE = /^tier[0-3]$/;

/** tier0-3 keys carried by a legacy `tiers` block ([] when none/not legacy). */
function _legacyTierKeys(tiers: unknown): string[] {
  if (!tiers || typeof tiers !== "object" || Array.isArray(tiers)) return [];
  return Object.keys(tiers).filter((k) => _LEGACY_TIER_KEY_RE.test(k));
}

/**
 * Throw LegacyTierShapeError when a models-block-shaped source carries a
 * legacy `tiers.tierN` entry. Checked BEFORE any rungs-wins shortcut so the
 * mixed-shape pollution class (rungs present + stale `tiers` key — the old
 * freshness-advisory write bug, see migrate_model_roles.migrate_models_block)
 * is refused too, not silently shadowed. A vestigial EMPTY `tiers: {}` (no
 * tierN keys) carries no routing intent and is tolerated.
 */
function _assertNoLegacyTierShape(source: any, where: string): void {
  if (!source || typeof source !== "object") return;
  const keys = _legacyTierKeys(source.tiers);
  if (keys.length > 0) {
    throw new LegacyTierShapeError(
      `${where} carries models.tiers keys [${keys.join(", ")}]`,
    );
  }
}

let _loggedLegacyRefusal = false;
 ...
 */
export function legacyTiersRefuseConfig(message: string): ModelRouterConfig {
  if (!_loggedLegacyRefusal) {
    _loggedLegacyRefusal = true;
    try {
...
export const DEFAULT_MODEL_CATALOG: any = {
  rungs: [
    {
      id: "haiku-class",
      costClass: "low",
  ...
  roles: {
    ...
    power: "opus-class",
    max: "fable-class",
    // The former `judge` role (its own judge-class rung + structured shape)
    // was collapsed 2026-08 — cross-vendor checking is derived from the
    // standard chain (resolveCrossVendor); design-judge-role-collapse §5.4.
  },
  roleCaps: {
...
export function defaultModelCatalog(): any {
 ...
 */
export function synthesizeRungsRoles(source: any): {
  rungs: Rung[];
  ...
  // Legacy shape check FIRST — a mixed-shape source (rungs present + stale
  // tiers.tierN) must refuse, not silently win on the rungs discriminator.
  _assertNoLegacyTierShape(src, "models block");

  if (Array.isArray(src.rungs) && src.rungs.length > 0) {
    return {
      rungs: src.rungs as Rung[],
      roles: (src.roles ?? {}) as ModelRouterConfig["roles"],
      roleCaps: src.roleCaps as ModelRouterConfig["roleCaps"] | undefined,
    };
  }
...
function usableModels(rung: any): string[] {
  if (!rung || typeof rung !== "object" || !Array.isArray(rung.models)) return [];
  return rung.models.filter((m: any) => typeof m === "string" && m.trim() !== "");
}
 ...
 */
function mergeOneRung(baseRung: any, overRung: any): any {
  if (usableModels(overRung).length > 0) return overRung;
  return { ...baseRung, ...overRung, models: baseRung?.models ?? [] };
}

function mergeTwo(base: any, override: any): any {
  ...
  const merged: any = { ...b, ...o }; // override wins for scalar/other keys

  // rungs: merge by id.
  const overById = new Map<string, any>();
  const overOrder: string[] = [];
...
export function mergeModelCatalog(
  ...
  // Legacy tier0-tier3 layers are refused, not normalized: without the old
  // synthesis a legacy layer would be silently SHADOWED by the code-default
  // rungs (mergeTwo passes the dangling `tiers` key through but resolution
  // keys off `rungs`), routing on defaults the operator never chose. Throw
  // instead — see LegacyTierShapeError.
  _assertNoLegacyTierShape(base, "pod (base) layer");
  _assertNoLegacyTierShape(override, "bot (override) layer");
  if (opts && opts.includeDefaults === false) {
    return mergeTwo(base, override);
  }
  const withPod = mergeTwo(defaultModelCatalog(), base);
  return mergeTwo(withPod, override);
}
 ...
 */
export function normalizeRouting(raw: any): ModelRouterConfig["routing"] {
  return _normalizeRouting(raw);
}

function _normalizeRouting(raw: any): ModelRouterConfig["routing"] {
  const r = raw && typeof raw === "object" ? raw : {};
  const legacyKeys = ["maintenanceTier", "backgroundTier", "ambiguousTier"]
    .filter((k) => k in r);
  if (legacyKeys.length > 0) {
    throw new LegacyTierShapeError(
      `routing block carries legacy keys [${legacyKeys.join(", ")}]`,
    );
  }
  const toRole = (field: string, v: unknown): string | null | undefined => {
    if (v === null) return null;
    if (typeof v !== "string") return undefined;
    if (_LEGACY_TIER_KEY_RE.test(v)) {
      throw new LegacyTierShapeError(
        `routing.${field} carries legacy tier value "${v}"`,
      );
    }
    return v;
  };
  return {
    ...
    maintenanceRole: toRole("maintenanceRole", r.maintenanceRole) ?? undefined,
    backgroundRole: toRole("backgroundRole", r.backgroundRole) ?? undefined,
    // ambiguousRole may legitimately be null (use bot default).
    ambiguousRole:
      "ambiguousRole" in r ? toRole("ambiguousRole", r.ambiguousRole) : null,
    // Bool-only: any non-bool value (string "true", 1, …) is not a
    // decision to enable content-based downgrades — treat as unset.
    classifierDowngrade:
      typeof r.classifierDowngrade === "boolean" ? r.classifierDowngrade : undefined,
  };
}
 ...
 * (`power_cap.sanitize_daily_cap` — the one Python resolver, which also
 * carries `resolve_effective_power_cap`, the twin of `_mergeRoleCaps` +
 * `_roleCap` below; and `home_chat_routes._read_user_tier_override`, the
 * legacy-key reader for the admin chat gate): a cap is valid only when it is
 ...
 */
export function sanitizeDailyCap(raw: unknown, fallback: number): number {
  if (typeof raw !== "number") return fallback;
  // NaN, ±Infinity, negatives and >100 all fail this one guard.
 ...
 * enforcement point.
 */
export function defaultRoleCap(role: "power" | "max"): number {
  return DEFAULT_MODEL_CATALOG.roleCaps[role].maxPerDayPerBot;
}
 ...
 */
function loadUserTierPrefsFile(
  sharedDir: string,
  botId: string,
): { users: Record<string, { defaultRole?: string; defaultTier?: string }> } {
...
/**
 * Recursively sort object keys — serialization helper for
 * ``setStandingUserTierDefault``. Makes ``JSON.stringify(x, null, 2)``
 * byte-equivalent to the admin-side Python writer's
 * ``json.dumps(data, indent=2, sort_keys=True)`` so the two
 * user-tier-prefs.json writers produce stable, diffable output.
 */
function _sortKeysDeep(v: unknown): unknown {
  if (Array.isArray(v)) return v.map(_sortKeysDeep);
  if (v && typeof v === "object") {
    const out: Record<string, unknown> = {};
    for (const k of Object.keys(v as object).sort()) {
      out[k] = _sortKeysDeep((v as Record<string, unknown>)[k]);
    }
    return out;
  }
  return v;
...
const _TIER_DIRECTIVE_RE =
  /\[evolve-routing\s+nonce=[A-Za-z0-9_-]{8,}\]\s+tier=([a-z]+)/i;
 ...
 * several.
 */
function _rolePreferenceRank(role: string): number {
  switch (role) {
...
export class ModelRouter {
  ...
  private sessionTypes: Map<string, string>; // sessionKey → 'productive'|'maintenance'|'ambiguous'|'background'
  // WHO decided the session type: 'anchor' = the trigger-kind
  // pre-classification (objective fact — heartbeat/cron/subagent),
  // 'classifier' = the content classifier's guess at agent_end. The
  // routing branch consults this to gate content-based maintenance
  // downgrades behind routing.classifierDowngrade (default off).
  // 'anchor' is sticky: a later lateral reclassification by the
  // classifier must not demote objective provenance to guess-land
  // (a scaffolding subagent that chats about `sudo` is still a
  // scaffolding subagent).
  private sessionTypeSources: Map<string, "anchor" | "classifier">;
  private sessionUserTiers: Map<string, UserTierChoice>; // sessionKey → operator's per-turn pick
  ...
  private sessionConsentSources: Map<
    string,
    "ui_chip" | "ask_hint_agreed" | "bot_initiated" | "evo_keyword"
  >;
  ...
  private sessionUserKeys: Map<string, string>;
  ...
  private sessionCostHistory: Map<string, Array<{ ts: number; cost: number }>>;
  private sessionRunawayTripped: Map<string, number>; // sessionKey → trip count this session
  ...
  private _runawayTripsToday: { count: number; dayIso: string } = { count: 0, dayIso: "" };
  ...
  private _tier1ActiveSessions: Set<string> = new Set();
  private _tier1ActiveFileInitialized: boolean = false;
  ...
  private _tier1CallsToday: { count: number; dayIso: string } = { count: 0, dayIso: "" };
  ...
  private _maxCallsToday: { count: number; dayIso: string } = { count: 0, dayIso: "" };
  ...
  // classifier's choice).
  private sessionCascadeVerdicts: Map<
    string,
    { tier: "tier1" | "tier2" | "tier3"; decidedAt: number }
  > = new Map();
  ...
  private sessionLastDecisionDriver: Map<
    string,
    | "legacy_config"
    | "runaway"
    | "spend_cap"
  ...
  private sessionPreflightDecisions: Map<
    string,
    { tier: "tier1" | "tier2" | "tier3"; reason: string }
  > = new Map();

  // Cache for auth-profiles.json — re-read at most once per minute
  private _authProfilesCache: { profiles: Record<string, any>; at: number } | null = null;
  private static readonly _AUTH_PROFILES_TTL = 60_000;
  ...
  private _credentialedProvidersOverride: Set<string> | null = null;
   ...
   */
  private static _normalizeConfig(config: ModelRouterConfig): ModelRouterConfig {
    const c = (config ?? {}) as any;
    try {
      _assertNoLegacyTierShape(c, "router config");
      let roleCaps = c.roleCaps;
      // Fold legacy power cap into roleCaps.power, sanitized (#3566
      // audit E-4) so an invalid legacy value never materializes as a
      // new-shape cap; _roleCap re-applies the same resolver at read.
      if (!roleCaps && typeof c.userTierOverride?.dailyCap === "number") {
        roleCaps = {
          power: {
            maxPerDayPerBot:
              sanitizeDailyCap(c.userTierOverride.dailyCap, defaultRoleCap("power")),
          },
        };
      }
      return {
        ...c,
        rungs: Array.isArray(c.rungs) ? c.rungs : [],
        roles: c.roles ?? {},
        roleCaps,
   ...
   */
  private logger?: { warn?: (m: string) => void; info?: (m: string) => void };
  ...
  constructor(
    ...
    this.sessionTypeSources = new Map();
  ...
  private _rung(slug: string | undefined): Rung | null {
    if (!slug) return null;
  ...
  private _providerOf(model: string | undefined | null): string | null {
  ...
  resolveRoleToModel(role: string): string | null {
    const slug = (this.config.roles ?? {})[role as keyof ModelRouterConfig["roles"]];
  ...
  private _llmProvidersFromCatalog(): Set<string> {
   ...
   */
  private _credentialedProviders(): Set<string> {
    ...
    for (const [key, profile] of Object.entries(profiles)) {
      ...
      for (const field of ["key", "token", "api_key"]) {
        ...
        if (typeof val === "string" && val.trim().length > 0) { hasKey = true; break; }
      }
      if (!hasKey) continue;
      ...
      let provider = (profile as any).provider;
  ...
  availableProviders(): Set<string> {
    ...
    const out = new Set<string>();
    for (const p of credentialed) if (llm.has(p)) out.add(p);
    return out;
  }
  ...
  resolveRoleAvailability(role: string): {
    ...
    const seen = new Set<string>();
    let cur: string | null = role;
    while (cur && !seen.has(cur)) {
      seen.add(cur);
      const slug = (this.config.roles ?? {})[cur as keyof ModelRouterConfig["roles"]];
      const rung = typeof slug === "string" ? this._rung(slug) : null;
      const models: string[] = Array.isArray(rung?.models) ? rung!.models : [];
   ...
   * `cross_vendor: false`).
   */
  resolveCrossVendor(againstRole: string = "standard"): string | null {
    const avail = this.availableProviders();
    const res = this.resolveRoleAvailability(againstRole);
    if (!res.model) return null;  // nothing produces the judged work
    const againstProvider = this._providerOf(res.model);
    const resolvedRole = res.resolvedRole ?? againstRole;
    const entry = (this.config.roles ?? {})[
      resolvedRole as keyof ModelRouterConfig["roles"]
    ];
    // Roles are uniformly rung slugs (J3 — the structured judge shape is gone).
    const slug = typeof entry === "string" ? entry : undefined;
    const rung = this._rung(slug);
    for (const m of (rung?.models ?? [])) {
      const p = this._providerOf(m);
      if (!p || p === againstProvider) continue;
      if (avail.has(p)) return m;
    }
    return null;
  }

  /** Sorted provider set of a rung's models. */
  private _rungProviders(rung: Rung | null | undefined): string[] {
   ...
   */
  private _degradeRole(role: string): string | null {
    switch (role) {
      case "max":      return "power";
      case "power":    return "standard";
      case "standard": return "fast";
      default:         return null;  // fast, unknown
    }
  }
  ...
  private _roleToCascadeTier(
    ...
    switch (role) {
      ...
      // tier0/judge is gone (judge-role collapse) — nothing maps to it.
      default:         return null;
    }
  }
   ...
   *
   * NOTE the legacy fold is unreachable from the production load path:
   * `fromTiersFile` is `synthesizeRungsRoles(mergeModelCatalog(...))`, and
   * mergeModelCatalog folds DEFAULT_MODEL_CATALOG in as its base layer —
   * which ships `roleCaps.power.maxPerDayPerBot`. So `explicit` is always
   * truthy for a pod configured by Evolve and `dailyCap` never routes. The
   * fold stays for a hand-rolled config that carries `roleCaps: {}`; it is
   * NOT a supported way to set a cap, and `evolve-admin models cap` writes
   * `roleCaps.power.maxPerDayPerBot` for exactly this reason.
   */
  private _mergeRoleCaps(
    fromTiersFile: ModelRouterConfig["roleCaps"] | undefined,
    fromNetwork: ModelRouterConfig["roleCaps"] | undefined,
    legacyOverride: { dailyCap?: number } | undefined,
  ): ModelRouterConfig["roleCaps"] {
    const explicit = fromTiersFile ?? fromNetwork;
    if (explicit) return explicit;
    const legacyCap = legacyOverride?.dailyCap;
    if (typeof legacyCap === "number") {
      // Sanitize at fold time so an invalid legacy value never
      // materializes as a new-shape cap (#3566 audit E-4); _roleCap
      // re-applies the same resolver at the enforcement point.
      return {
        power: {
          maxPerDayPerBot: sanitizeDailyCap(legacyCap, defaultRoleCap("power")),
        },
      };
    }
    return undefined;
  ...
  private _cascadeTierToRole(tier: string): RoleId | null {
    switch (tier) {
   ...
   */
  private _safetyNetRefusalWarned: boolean = false;
  private _safetyNetDowngradeModel(driver: "runaway" | "spend_cap"): string {
    ...
    if (!this._safetyNetRefusalWarned) {
      ...
      try {
        // eslint-disable-next-line no-console
        console.warn(
          `[Evolve ModelRouter] WARN: ${driver} safety-net fired but the ` +
   ...
   */
  private _warnIfSafetyNetWithoutFastRole(): void {
    // Legacy-poisoned router already refuses every turn (and logged its own
    // remediation) — a second "fast unconfigured" warning is just noise.
    if (this.config.legacyConfigError) return;
    const fastHasModels = !!this.resolveRoleToModel("fast");
    if (fastHasModels) return;
    const runawayWired = this.config.runawayRateCap?.enabled !== false; // default true
    const spendCapWired = !!(this.sharedDir && this.botId);
    if (!runawayWired && !spendCapWired) return;
    try {
      // eslint-disable-next-line no-console
      console.warn(
        `[Evolve ModelRouter] WARN at startup: the 'fast' role has no ` +
  ...
  private _lastResolvedDefaultSource: "user" | "operator" | null = null;
  ...
  private _resolveRoleFromChoice(
    ...
    if (!norm || norm === "auto") return [null, null];

    const VALID = allowMax
      ? new Set(["fast", "standard", "power", "max"])
      : _CLASSIFIER_ROLES;
    if (!VALID.has(norm)) return [null, null];
  ...
  isUserTierSurfaceEnabled(): boolean {
    const override = (this.config.userTierOverride ?? {}) as { enabled?: boolean };
  ...
  setStandingUserTierDefault(userKey: string, choice: string | null): void {
    if (!userKey) throw new Error("user_key is required");
    if (!this.sharedDir || !this.botId) {
      throw new Error("sharedDir/botId not configured on this router");
    }
    if (choice !== null && !["fast", "standard", "power", "max"].includes(choice)) {
      throw new Error(`unknown standing choice '${choice}'`);
    }

    const prefsPath = path.join(this.sharedDir, this.botId, "user-tier-prefs.json");

    // Read-merge so concurrent admin-side entries for OTHER users survive.
    // ENOENT → fresh file. Malformed JSON → start fresh (matches the
    // admin-side _load's fail-open posture on JSONDecodeError). Any other
    // read error (EACCES etc.) → throw; overwriting a file we cannot read
    // would destroy other users' prefs.
    ...
    try {
      ...
      if (err?.code !== "ENOENT") {
        throw new Error(
          `cannot read ${prefsPath} (${err?.code ?? err}) — refusing to ` +
          `overwrite a file this process cannot read`,
        );
      }
    }
    if (raw !== null) {
      ...
      } catch { /* malformed → fresh shape, admin-side parity */ }
    }
    ...
    if (choice === null) {
      ...
      // Second-precision UTC with the "+00:00" suffix — byte-identical
      // format to the Python writer's timestamp.
      const stamp = new Date().toISOString().slice(0, 19) + "+00:00";
      data.users[userKey] = { defaultRole: choice, updated_at: stamp };
    }

    const jsonText = JSON.stringify(_sortKeysDeep(data), null, 2);
    ...
    try {
      ...
      throw new Error(
        `cannot write ${prefsPath} (${err?.code ?? err})`,
      );
    }

    // In-memory effect — next _resolveOperatorDefaultRole lookup sees the
    // new entry immediately, no reload needed.
    this.config.userTierPrefs = { users: data.users };
  }
  ...
  getRoleForModel(modelString: string | undefined | null): string | null {
    ...
    const roleIds: RoleId[] = ["fast", "standard", "power", "max"];
    ...
    for (const role of roleIds) {
      ...
      if (!roleDef) continue;
      const slug = typeof roleDef === "string" ? roleDef : undefined;
      const rung = this._rung(slug);
   ...
   */
  private _loadAuthProfiles(): Record<string, any> {
    const now = Date.now();
   ...
   */
  private _profileHasKey(profileId: string): boolean {
    const profiles = this._loadAuthProfiles();
  ...
  setSessionType(sessionKey: string, sessionType: string): void {
    ...
    // Raw setter = the trigger-anchor path (and explicit reclassification
    // callers) — objective provenance.
    this.sessionTypeSources.set(sessionKey, "anchor");
  }
  ...
  setSessionTypeIfMoreSpecific(sessionKey: string, newType: string): void {
    ...
    // This is the content classifier's path. Record 'classifier'
    // provenance only when no trigger anchor owns the session — the
    // anchor is an objective fact and stays authoritative even across
    // a lateral reclassification (see sessionTypeSources docstring).
    if (this.sessionTypeSources.get(sessionKey) !== "anchor") {
      this.sessionTypeSources.set(sessionKey, "classifier");
    }
  }
  ...
  setUserTier(
    ...
    if (v === "fast" || v === "standard" || v === "power" || v === "max") {
      this.sessionUserTiers.set(sessionKey, v as UserTierChoice);
   ...
   */
  private _allowBotInitiated(role: "power" | "max"): boolean {
    const raw = (this.config.userTierOverride ?? {}).allowBotInitiated;
    if (typeof raw === "boolean") {
      return role === "power" ? raw : false;
    }
    if (raw && typeof raw === "object") {
      if (role === "power") return raw.power !== false;  // default true
   ...
   */
  private _roleCap(role: "power" | "max"): number {
    // Python twin: `power_cap.resolve_effective_power_cap` (evolve_admin).
    // It exists because `evolve-admin models cap` has to WRITE the key this
    // reads, and a writer that guesses the read order is how the legacy
    // `dailyCap` ended up reaching routing on no pod at all — see that
    // module's docstring. Change the order here and change it there.
    const fallback = defaultRoleCap(role);
    const caps = this.config.roleCaps ?? {};
    const fromNew = caps[role]?.maxPerDayPerBot;
    if (typeof fromNew === "number") return sanitizeDailyCap(fromNew, fallback);
    // Legacy fallback: userTierOverride.dailyCap was the power cap.
    if (role === "power") {
      const legacy = (this.config.userTierOverride ?? {}).dailyCap;
      if (typeof legacy === "number") return sanitizeDailyCap(legacy, fallback);
    }
    return fallback;
  }

  /** Per-role used-today counter, rolled at pod-local midnight. */
  private _roleUsedToday(role: "power" | "max"): number {
  ...
  canEscalateToRole(role: string): {
    ...
    if (override.enabled === false) {
      return {
        ...
        detail:
          "userTierOverride.enabled is false in evolve-tiers.json — " +
    ...
    if (!this._allowBotInitiated(role)) {
      return {
        ...
        detail:
          `allowBotInitiated.${role} is false in evolve-tiers.json — ` +
    ...
    if (used >= cap) {
      return {
        ...
        detail:
          `'${role}' daily cap reached (${used}/${cap} turns today). ` +
  ...
  degradeRoleOnCap(role: string): string {
    if (role === "max") return "power";
    if (role === "power") return "standard";
    return "standard";
  }
   ...
   * end.
   */
  private sessionCapDegradedFrom: Map<string, string> = new Map();
  ...
  getLastCapDegradedFrom(sessionKey: string): string | null {
    return this.sessionCapDegradedFrom.get(sessionKey) ?? null;
  }
   ...
   * not as a courtesy.
   */
  private _sessionAlreadyInRole(sessionKey: string, role: string): boolean {
    if (!sessionKey) return false;
    if (role === "power") return this._tier1ActiveSessions.has(sessionKey);
    if (role === "max") return this._lastResolvedRoleWasMax.has(sessionKey);
    return false;
  }
   ...
   */
  private _capDefaultRole(sessionKey: string, role: string | null): string | null {
    let current = role;
    // Bounded walk: the chain is max→power→standard and terminates at an
    // uncapped role, so this cannot spin even if degradeRoleOnCap changes.
    for (let hop = 0; hop < 4; hop++) {
      if (current !== "power" && current !== "max") return current;
      if (this._sessionAlreadyInRole(sessionKey, current)) return current;
      if (this._roleUsedToday(current) < this._roleCap(current)) return current;
      const next = this.degradeRoleOnCap(current);
  ...
  getConsentSource(
    sessionKey: string,
  ): "ui_chip" | "ask_hint_agreed" | "bot_initiated" | "evo_keyword" | null {
  ...
  setCascadeVerdict(
    sessionKey: string,
    verdict: { tier: "tier1" | "tier2" | "tier3" } | null,
   ...
   */
  getCascadeVerdict(sessionKey: string): { tier: "tier1" | "tier2" | "tier3" } | null {
   ...
   * changes.
   */
  setSessionPreflightDecision(
    sessionKey: string,
    decision: { tier: "tier1" | "tier2" | "tier3"; reason: string } | null,
  ): void {
    if (!sessionKey) return;
    if (decision === null) {
   ...
   */
  getSessionPreflightDecision(
    sessionKey: string,
  ): { tier: "tier1" | "tier2" | "tier3"; reason: string } | null {
  ...
  clearSession(sessionKey: string): void {
    ...
    this.sessionTypeSources.delete(sessionKey);
    this.sessionUserTiers.delete(sessionKey);
    this.sessionConsentSources.delete(sessionKey);
    this.sessionUserKeys.delete(sessionKey);
    ...
    this.sessionCapDegradedFrom.delete(sessionKey);
   ...
   */
  private _markSessionTier(sessionKey: string, resolvedRole: string | null): void {
    if (!sessionKey) return;
    ...
    // multi-turn session). Read against roleCaps.power.maxPerDayPerBot by
    // canEscalateToRole("power") for escalation REQUESTS and by
    // _capDefaultRole for operator/user DEFAULTS. Reset at pod-local
    // midnight.
    if (isPower && !wasPower) {
      const dayIso = localDateYMD();
    ...
    if (isPower === wasPower) return;   // no change → no I/O
    if (isPower) {
  ...
  // session end.
  private _lastResolvedRoleWasMax: Set<string> = new Set();
  ...
  protected _writeTier1ActiveFile(): void {
    if (!this.sharedDir || !this.botId) return;
    try {
      ...
      if (err?.code === "EACCES" && !this._tier1ActiveWarnedEACCES) {
        ...
        try {
          ...
          this.logger?.warn?.(
            `ModelRouter: cannot write tier1_active.json under ` +
  ...
  private _tier1ActiveWarnedEACCES: boolean = false;
   ...
   */
  private _clearTier1ActiveFileOnce(): void {
    if (this._tier1ActiveFileInitialized) return;
    if (!this.sharedDir || !this.botId) return;
  ...
  private _roleToDiskTierField(role: "power" | "max"): "tier1" | "max" {
  ...
  private _tierUsageLogPath(): string {
    return path.join(
      this.sharedDir,
      "cost",
      "tier-usage",
   ...
   */
  private _appendTierUsageRecord(role: "power" | "max", model: string | null): void {
    if (!this.sharedDir || !this.botId) return;
    try {
      ...
      if (!this._tierUsageWarned) {
        ...
        try {
          this.logger?.warn?.(
            `ModelRouter: FAILED to append tier-usage record under ` +
  ...
  private _tierUsageWarned: boolean = false;
   ...
   */
  private _seedRoleCountersFromDisk(): void {
    if (!this.sharedDir || !this.botId) return;
    const dayIso = localDateYMD();
    let tier1 = 0;
    let max = 0;
   ...
   */
  recordTurnCost(sessionKey: string, costUsd: number, tsMs: number = Date.now()): void {
   ...
   */
  checkRunawayRate(sessionKey: string, tsMs: number = Date.now()): {
    tripped: boolean;
    totalUsd: number;
    ...
    const cfg = this.config.runawayRateCap ?? {};
    if (cfg.enabled === false) return { tripped: false, totalUsd: 0 };
    ...
    if (total <= threshold) return { tripped: false, totalUsd: total };
    ...
    const severity: "warning" | "critical" =
      tripsToday >= criticalThreshold ? "critical" : "warning";
  ...
  isSpendCapForced(sessionKey: string): boolean {
    if (this.isRunawayTripped(sessionKey)) return true;
   ...
   */
  /**
   * The effective ``routing`` block — read-only.
   *
   * Exposed so the routing rule (routingRule.ts) can reproduce step 5
   * of ``_resolveModelAndTier`` exactly: a pod that has moved
   * ``maintenanceRole`` or ``backgroundRole`` off ``fast`` must see the
   * rule pick the SAME rung the ladder would have picked, or the rule
   * would quietly change which model answers its heartbeats — the one
   * thing the overhead work is not allowed to do.
   */
  getRoutingConfig(): Readonly<NonNullable<ModelRouterConfig["routing"]>> | null {
    return this.config.routing ?? null;
  }
   ...
   */
  private _resolveModelAndTier(sessionKey: string): [string | null, string | null] {
    // Poisoned by a legacy tier0-tier3 config (see LegacyTierShapeError):
    // refuse EVERY turn, before every other branch including the
    // routing-disabled fail-open. None of the shape-derived config can be
    // trusted, so no branch below may run; the sentinel is unresolvable,
    // the turn errors loudly, and the one-time console.error at the load
    // seam already named the remediation (migrate-model-roles --apply).
    if (this.config.legacyConfigError) {
      this.sessionLastDecisionDriver.set(sessionKey, "legacy_config");
      return [LEGACY_CONFIG_REFUSE_SENTINEL, null];
    }

    // Fail open: routing explicitly disabled
    if (this.config.routing?.enabled === false) return [null, null];
    ...
    // Spec: internal/spec-model-rungs-and-roles-2026-06-09.md §Routing
    // precedence + internal/spec-user-tier-control-2026-05-26.md.
    ...
    if (userRole) {
      ...
      if (a.model) return [a.model, a.resolvedRole ?? userRole];
    ...
    if (this.isCascadeEnabled()) {
      ...
      if (verdict) {
        ...
        // Defensive: a cascade verdict that maps to no classifier role
        // (a stale tier0, or junk) is dropped to standard rather than routed.
    ...
    // Content-based maintenance downgrade is OPT-IN (routing.
    // classifierDowngrade, default false): when the CLASSIFIER labeled a
    // human conversation `maintenance` from its content, treat it like
    // productive (fall through to defaults) unless the operator enabled
    // the downgrade. Trigger-anchored maintenance (scaffolding subagents)
    // and background are objective and route regardless.
    const classifierMaintenanceSuppressed =
      sessionType === "maintenance" &&
      this.sessionTypeSources.get(sessionKey) === "classifier" &&
      this.config.routing?.classifierDowngrade !== true;
    if (
      !sessionType || sessionType === "productive" || sessionType === "ambiguous" ||
      classifierMaintenanceSuppressed
    ) {
      ...
      if (opModel) {
        // A default spends like a request does, so it is counted and bounded
        // like one: _markSessionTier already bumps the per-day counter on a
        // transition into power/max whatever the driver, and this is the gate
        // that reads it back. Degrading down the chain beats refusing the
        // turn — the session keeps working, one rung cheaper.
        const capped = this._capDefaultRole(sessionKey, opRole);
        // `capped` can only differ from `opRole` by walking the degrade
        // chain, which yields a concrete role — a null `opRole` never enters
        // the loop. The truthiness check is what tells the compiler that.
        if (capped && capped !== opRole) {
          this.sessionCapDegradedFrom.set(sessionKey, String(opRole));
          this.sessionLastDecisionDriver.set(sessionKey, "role_cap");
          return [this.resolveRoleToModel(capped), capped];
        }
        this.sessionCapDegradedFrom.delete(sessionKey);
  ...
  resolveAuthProfileOverride(sessionKey: string): string | null {
    if (!this.config.accountRouting?.enabled) return null;
    if (!this.config.accountTiers) return null;
  ...
  reloadConfig(networkPath: string, tiersJsonPath?: string, sharedDir?: string, botId?: string): void {
    try {
      ...
      if (tiersJsonPath) {
        try { tiersFile = JSON.parse(fs.readFileSync(tiersJsonPath, "utf8")); } catch { /* ok */ }
      }
      ...
      try { network = JSON.parse(fs.readFileSync(networkPath, "utf8")); } catch { /* ok */ }
      ...
      // A legacy {tiers:{tierN}} shape REFUSES (LegacyTierShapeError): the
      // router adopts the poisoned refuse config instead of keeping the old
      // in-memory config — keep-old would silently mask a new legacy-shape
      // producer behind a stale-but-working catalog.
      let synthesized: ReturnType<typeof synthesizeRungsRoles>;
      let normalizedRouting: ModelRouterConfig["routing"];
      ...
      try {
        ...
        normalizedRouting = _normalizeRouting(rawRouting);
      } catch (e) {
        if (e instanceof LegacyTierShapeError) {
          this.config = legacyTiersRefuseConfig(e.message);
          if (sharedDir) this.sharedDir = sharedDir;
      ...
      this.config = {
        ...
        roleCaps: this._mergeRoleCaps(
          synthesized.roleCaps,
          network.models?.roleCaps,
          tiersFile.userTierOverride ?? network.userTierOverride,
        ),
        ...
        userTierOverride:
          tiersFile.userTierOverride ?? this.config?.userTierOverride,
        ...
        userTierPrefs: (() => {
          ...
          if (fresh.users && Object.keys(fresh.users).length > 0) return fresh;
