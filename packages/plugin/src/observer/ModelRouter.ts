...
export interface ModelRouterConfig {
  ...
  // model. Synthesized from a legacy `models.tiers.tierN` shape by the
  // loader when an old config is encountered (tier3->fast etc).
  rungs: Rung[];
 ...
 */
const _LEGACY_TIER_TO_ROLE: Record<string, RoleId> = {
  tier3: "fast",
  tier2: "standard",
  tier1: "power",
  tier0: "judge",
...
const _LEGACY_TIER_TO_RUNG: Record<string, string> = {
  tier3: "haiku-class",
  tier2: "sonnet-class",
  tier1: "opus-class",
...
export function synthesizeRungsRoles(source: any): {
  ...
  if (tiers && typeof tiers === "object") {
    const rungs: Rung[] = [];
    const roles: ModelRouterConfig["roles"] = {};
    // Emit rungs in cost order so array position stays a valid cost rank.
    const COST_ORDER: Array<[string, Rung["costClass"]]> = [
      ["tier3", "low"],
      ["tier2", "medium"],
      ["tier1", "high"],
    ];
    ...
    // tier0 (judge) synthesizes its OWN judge-class rung (spec §Addendum 16)
    // with the provider-diversity constraint, so a legacy judge list keeps its
    // independent order instead of folding into standard's sonnet-class rung.
    const tier0 = tiers.tier0;
    const tier0Models = Array.isArray(tier0?.models) ? tier0.models : [];
    if (tier0Models.length > 0) {
      const judgeRung = rungs.find((r) => r.id === "judge-class");
      if (judgeRung) {
        for (const m of tier0Models) {
          if (!judgeRung.models.includes(m)) judgeRung.models.push(m);
        }
      } else {
        rungs.push({ id: "judge-class", models: tier0Models, costClass: "medium" });
      }
    ...
    if (synthesized && !_loggedLegacySynthesis) {
      ...
      try {
        ...
        console.warn(
          "[Evolve ModelRouter] DEPRECATION: synthesized rungs/roles from a " +
 ...
 */
function _normalizeLegacyLayer(layer: any): any {
  if (!layer || typeof layer !== "object") return layer;
  if (Array.isArray(layer.rungs) && layer.rungs.length > 0) return layer;
  if (!layer.tiers || typeof layer.tiers !== "object") return layer;
  const synth = synthesizeRungsRoles(layer);
  if (!synth.rungs.length) return layer;
 ...
 */
export function tierOverrideIsBroken(override: any, tier: string): boolean {
  ...
  if (Array.isArray(override.rungs) && override.rungs.length > 0) {
    ...
    if (typeof entry === "string" && entry) rungId = entry;
    else if (entry && typeof entry === "object" && typeof entry.rung === "string") rungId = entry.rung;
    else if (role) rungId = _LEGACY_TIER_TO_RUNG[tier];
    if (rungId) {
      const r = override.rungs.find((x: any) => x && typeof x === "object" && x.id === rungId);
      if (r) return usableModels(r).length === 0;
    }
  }
  ...
  if (override.tiers && typeof override.tiers === "object" && tier in override.tiers) {
    ...
    if (cfg && typeof cfg === "object") {
      const models = Array.isArray(cfg.models)
        ? cfg.models.filter((m: any) => typeof m === "string" && m.trim() !== "")
        : [];
...
function _normalizeRouting(raw: any): ModelRouterConfig["routing"] {
  ...
  const toRole = (v: unknown): string | null | undefined => {
  ...
  return {
    ...
    ambiguousRole:
      "ambiguousRole" in r ? toRole(r.ambiguousRole)
