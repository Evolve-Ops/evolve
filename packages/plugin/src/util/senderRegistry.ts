 ...
 *
 * Phase C.3 of docs/spec-user-roster-and-roles-2026-06-07.md. The OC SDK's
...
/** The four messaging platforms the roster / role layer keys identities
 *  on. Mirrors ``KNOWN_PROVIDERS`` in routes_bot_users.py and
 *  ``CHANNEL_ENUM`` in RosterTools.ts. */
const KNOWN_PLATFORMS = ["telegram", "slack", "discord", "whatsapp"] as const;


/**
 * Normalize an OC channel-TYPE value (what OC threads as
 * ``channelId`` / ``ctx.channelId`` on the before_agent_run event — the
 * channel *type* name like "telegram"/"slack", NOT the numeric chat id;
 * see TurnObserver._handleEvoFallback) to one of the four roster
 * platforms, or ``null`` when it isn't a recognized messaging platform
 * (e.g. "web", "unknown", "").
 *
...
export function normalizePlatform(raw: string | null | undefined): string | null {
  ...
  for (const p of KNOWN_PLATFORMS) {
    if (v === p || v.startsWith(`${p}_`) || v.startsWith(`${p}:`)) return p;
  }
  return null;
}
...
const _senders: Map<string, SenderRecord> = new Map();

/** Soft cap on entries. The hot path is at most ~1 entry per concurrent
 *  active turn; 1024 leaves headroom for any reasonable load. */
const MAX_ENTRIES = 1024;

/** How long a captured sender stays valid. Tools must run their daemon
 *  call within this window or the registry returns null and they
 *  refuse with a "could not identify sender" envelope. 5 minutes is
 *  generous — typical tool latencies are sub-second. */
...
export function captureSender(
  ...
  fields: {
    ...
    /** Raw channel-TYPE / platform hint from OC; normalized here. */
    platform?: string | null;
    senderIsOwner?: boolean | null;
  },
): void {
  if (!runId) return;
  if (!fields.senderId) return;
  // FIFO eviction when at capacity.
  if (_senders.size >= MAX_ENTRIES) {
    const oldestKey = _senders.keys().next().value;
    if (oldestKey) _senders.delete(oldestKey);
  }
  _senders.set(runId, {
    ...
    senderIsOwner: !!fields.senderIsOwner,
    capturedAt: Date.now(),
  });
}
 ...
 */
export function getSender(runId: string | undefined | null): SenderRecord | null {
  if (!runId) return null;
  const rec = _senders.get(runId);
  if (!rec) return null;
  if (Date.now() - rec.capturedAt > TTL_MS) {
    _senders.delete(runId);
    return null;
  }
  return rec;
}
