 ...
 *
 * Phase C.2 of internal/spec-user-roster-and-roles-2026-06-07.md. Lets
 ...
 */
export function adminDaemonSocketPath(sharedDir: string): string {
  return joinPath(sharedDir, "admin-daemon.sock");
}

export interface AdminSocketRequest {
  method: "GET" | "POST" | "PATCH" | "PUT" | "DELETE";
  path: string;
  /** JSON-serializable body, or undefined for no body. */
  body?: unknown;
  /** Headers in addition to Content-Type/Length. */
  headers?: Record<string, string>;
  /** Per-call socket override (testing); defaults to DEFAULT_ADMIN_DAEMON_SOCKET. */
  socketPath?: string;
  /** Connection + request timeout. Defaults to 10s. */
  timeoutMs?: number;
}

export interface AdminSocketResponse {
  status: number;
  /** Parsed JSON when the response body parses; raw string otherwise; null on empty body. */
  body: unknown;
}
 ...
 */
export async function adminSocketRequest(
  req: AdminSocketRequest,
): Promise<AdminSocketResponse> {
  return new Promise((resolve, reject) => {
    ...
    const r = httpRequest(reqOpts, (res) => {
      ...
      res.on("error", (err) => reject(new AdminSocketUnavailable(
        `response error from ${req.method} ${req.path}: ${err.message}`,
      )));
    });
    ...
    r.on("error", (err: NodeJS.ErrnoException) => {
      ...
      if (
        ...
        reject(new AdminSocketUnavailable(
          `cannot reach admin daemon at ${reqOpts.socketPath}: ${err.message}`,
        ));
      } else {
        reject(err);
      }
    ...
    r.on("timeout", () => {
      r.destroy();
      reject(new AdminSocketUnavailable(
        `timeout calling ${req.method} ${req.path}`,
      ));
    });

    if (bodyJSON) r.write(bodyJSON);
    r.end();
  });
}
 ...
 */
export function extractTelegramIdFromSessionKey(
