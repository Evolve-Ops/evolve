...
const NON_WRITE_TOOL_SUBSTRINGS = ["read", "search", "grep", "list", "glob"] as const;

export function isFileWriteTool(name: unknown): boolean {
  if (typeof name !== "string" || !name) return false;
  const n = name.toLowerCase();
  if (NON_WRITE_TOOL_SUBSTRINGS.some((s) => n.includes(s))) return false;
...
export function isReadCommandInput(input: unknown): boolean {
 ...
 */
export function extractFileWrites(messages: unknown): ObservedWrite[] {
  if (!Array.isArray(messages)) return [];

  const errorById = new Set<string>();
  for (const msg of messages) {
    ...
    for (const block of content) {
      ...
      if (typeof b.tool_use_id === "string") errorById.add(b.tool_use_id);
    }
  }
  ...
  const push = (name: unknown, id: unknown, input: unknown) => {
    ...
    if (isReadCommandInput(input)) return;
    if (typeof id === "string" && errorById.has(id)) return;
  ...
  for (const msg of messages) {
    ...
    if (Array.isArray(m.content)) {
      for (const block of m.content) {
        const b = block as Record<string, unknown> | null;
        if (b && b.type === "tool_use") push(b.name, b.id, b.input);
      }
    }
 ...
 */
export function normalizeAppPath(raw: unknown, workspaceRoot: string): string | null {
...
export function indexKey(relPath: string): string {
 ...
 */
export function pathAliases(key: string): string[] {
  const out = [key];
  if (key.startsWith("workspace/")) out.push(key.slice("workspace/".length));
  else out.push(`workspace/${key}`);
  return out;
}
 ...
 */
export function parseEvolveMarkerIds(text: unknown): string[] {
...
function emptyIndex(): OwnershipIndex {
  return { byPath: new Map(), footprintByPath: new Map(), byMarkerId: new Map() };
}

function manifestDeclaredPaths(m: Record<string, unknown>): string[] {
  const out: string[] = [];
 ...
 */
function manifestFootprint(
  m: Record<string, unknown>,
  workspaceRoot: string,
): Map<string, string[]> {
  const out = new Map<string, string[]>();
  const add = (rawPath: unknown, token: string) => {
...
export function buildOwnershipIndex(
  ...
  for (const name of names) {
    ...
    if (!appId || appId === NO_APP_ID) continue;
    ...
    for (const raw of manifestDeclaredPaths(m)) {
      ...
      for (const alias of pathAliases(indexKey(rel))) {
        if (!index.byPath.has(alias)) index.byPath.set(alias, appId);
      }
    }
 ...
 */
export function growthAppDir(sharedDir: string, botId: string, appId: string | null): string {
...
export class GrowthLog {
  private readonly config: GrowthLogConfig;
  private readonly logger: GrowthLogger;

  private _index: OwnershipIndex | null = null;
  private _indexMtime = 0;
  private _indexCheckedAt = 0;

  private _dnt: boolean | null = null;
  private _dntCheckedAt = 0;

  /** Per day-file record counts, so the cap costs no stat per write. */
  private readonly _counts = new Map<string, number>();
  private readonly _initializedDirs = new Set<string>();
  private _prunedAt = 0;
  ...
  private _recordTurn(input: RecordTurnInput): GrowthRecord[] {
    ...
    // app_id (or null for the unattributed bucket) → accumulating delta.
    interface Bucket {
      appId: string | null;
      attribution: GrowthAttribution;
      files: string[];
    ...
    for (const w of writes) {
      ...
      if (!rel) continue; // outside the workspace — not app surface
      const key = indexKey(rel);

      let appId: string | null = null;
      let attribution: GrowthAttribution = "none";
      for (const alias of pathAliases(key)) {
        const owner = index.byPath.get(alias);
        if (owner) { appId = owner; attribution = "manifest"; break; }
      }
      if (!appId) {
        const viaMarker = this._appIdFromMarker(rel, index);
        if (viaMarker) { appId = viaMarker; attribution = "marker"; }
      }
      // An unowned write is app growth only when the TURN is app-attributed.
      if (!appId && turnGrade === "none") continue;
      ...
      if (!b.files.includes(rel) && b.files.length < MAX_FILES_PER_RECORD) {
        ...
        for (const alias of pathAliases(key)) {
          for (const token of index.footprintByPath.get(alias) ?? []) {
            if (!b.footprint.includes(token)) b.footprint.push(token);
          }
        }
      }
      if (!b.tools.includes(w.tool)) b.tools.push(w.tool);
      // A manifest match anywhere in the bucket outranks a marker match.
      if (attribution === "manifest") b.attribution = "manifest";
    ...
    const causeSource: GrowthCauseSource =
      dnt ? "dnt" : rawCause ? "user_request" : "none";
    const cause = causeSource === "user_request"
      ? rawCause.slice(0, MAX_CAUSE_CHARS)
      : null;

    const written: GrowthRecord[] = [];
    for (const b of buckets.values()) {
      ...
      if (this._append(record)) written.push(record);
    }
  ...
  private _appIdFromMarker(relPath: string, index: OwnershipIndex): string | null {
    if (index.byMarkerId.size === 0) return null;
    let head: string;
    try {
  ...
  private _ownershipIndex(): OwnershipIndex {
    const now = Date.now();
    ...
    if (this._index !== null && mtime === this._indexMtime) return this._index;
    this._index = buildOwnershipIndex(this.config.manifestsDir, this.config.workspaceRoot);
    this._indexMtime = mtime;
    return this._index;
  }

  private _isDntEnabled(): boolean {
    const now = Date.now();
    if (this._dnt !== null && now - this._dntCheckedAt < DNT_CACHE_TTL_MS) return this._dnt;
    let dnt = false;
    try {
      const network = JSON.parse(
        fs.readFileSync(path.join(this.config.sharedDir, "network.json"), "utf8"),
      );
      if (network?.bots?.[this.config.botId]?.growthLog === false) dnt = true;
    } catch {
  ...
  private _append(record: GrowthRecord): boolean {
    ...
    if (seen !== undefined && seen >= MAX_RECORDS_PER_DAY_FILE) return false;

    if (!this._initializedDirs.has(dir)) {
      if (!this._ensureDirs(dir)) return false;
      this._initializedDirs.add(dir);
      if (seen === undefined) this._counts.set(file, this._countLines(file));
      if ((this._counts.get(file) ?? 0) >= MAX_RECORDS_PER_DAY_FILE) return false;
    }
    ...
    try {
      ...
      if (code === "ENOENT") this._initializedDirs.delete(dir);
  ...
  private _countLines(file: string): number {
    try {
   ...
   */
  private _pruneOwnSubtreeOnce(): void {
    const now = Date.now();
    if (this._prunedAt && now - this._prunedAt < 24 * 60 * 60_000) return;
    this._prunedAt = now;
    ...
    try {
      for (const appSeg of fs.readdirSync(botRoot)) {
        ...
        let entries: string[];
        try { entries = fs.readdirSync(appDir); } catch { continue; }
