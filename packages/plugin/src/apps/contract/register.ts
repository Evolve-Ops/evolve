...
function neededRow(name: string): string {
  return (
    `app contract ${CONTRACT_VERSION}: tool_verb '${name}' has no contract row. ` +
...
export function requireToolVerbRows(tool: string, verbs: readonly string[]): void {
  const missing = verbs
    .map((v) => `${tool}.${v}`)
 ...
 */
export function registerAppTool<F>(
  api: { registerTool: (factory: F) => unknown },
  tool: string,
  ...
  try {
    ...
    logger.error(`Evolve: tool '${tool}' refused — ${(err as Error).message}`);
    return false;
  }
