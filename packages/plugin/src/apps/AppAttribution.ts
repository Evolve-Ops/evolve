...
interface SessionState {
  appId: string;
  /** Scheduled sessions are single-app by construction — never flip, never
   *  expire on signal-less turns (design §5). */
  scheduled: boolean;
  /** Which scheduled join produced the stamp — "oc_cron_map" | "claim_file"
   *  (AL-1.2; kept distinct for calibration, design §3). Absent on explicit
   *  stickiness entries. */
  scheduledSource?: string;
  /** Consecutive resolved turns with no fresh explicit signal. */
  signallessTurns: number;
  updatedAt: number;
}
...
const _runSignals: Map<string, RunSignal> = new Map();
const _sessions: Map<string, SessionState> = new Map();
const _warnedReasons: Set<string> = new Set();

let _config: AttributionConfig | null = null;
let _logger: AttributionLogger | null = null;
...
export function configureAppAttribution(
...
function warnOnce(reason: string, err: unknown): void {
  ...
  try {
    _logger?.warn(
      `Evolve app-attribution: ${reason} — resolving "none" (warns once per ` +
...
function evictOldest(map: Map<string, unknown>, max: number): void {
  ...
  if (oldestKey !== undefined) map.delete(oldestKey as string);
}


function getFreshRunSignal(runId: string): RunSignal | null {
...
function getFreshSession(sessionId: string): SessionState | null {
...
function appendConflict(record: Record<string, unknown>): void {
  try {
    if (!_config) return;
    const dir = path.join(_config.sharedDir, _config.botId);
    const file = path.join(dir, CONFLICT_LEDGER_FILENAME);
    try {
      if (fs.statSync(file).size > MAX_CONFLICT_LEDGER_BYTES) return;
    } catch {
...
function stickExplicit(sessionId: string | null | undefined, appId: string): void {
  if (!sessionId) return;
  const existing = getFreshSession(sessionId);
  if (existing) {
    if (existing.scheduled) return; // single-app by construction — never flip
 ...
 */
export function recordExplicit(
  ...
  try {
    ...
    if (runId) {
      ...
      if (!prior) evictOldest(_runSignals, MAX_RUN_ENTRIES);
...
export function recordScheduled(
  ...
  try {
    if (!sessionId || typeof appId !== "string" || !appId.trim()) return;
    evictOldest(_sessions, MAX_SESSION_ENTRIES);
    _sessions.set(sessionId, {
      appId: appId.trim(),
      scheduled: true,
...
const NONE: AppAttributionResult = Object.freeze({
  app_id: null,
  app_attribution: "none" as const,
  app_confidence: null,
