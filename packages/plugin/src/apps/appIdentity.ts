 ...
 *
 * AL-1.4a (docs/build-AL-1.4-app-id-canonical.md §2; decision in
 * docs/design-app-spec-and-discovery-2026-08-15.md §3). `app_id` is the
...
export const APP_ID_RESOLUTION_ORDER = ["app_id", ...LEGACY_APP_ID_FIELDS] as const;

/** The middleware's sentinel for a manifest that declared no identity. */
export const NO_APP_ID = "unknown";

function firstNonEmpty(manifest: unknown, fields: readonly string[]): string {
...
export function appIdOf(manifest: unknown): string {
  ...
  if (canonical && isCanonicalAppId(canonical)) return canonical;
 ...
 */
export function draftIdOf(manifest: unknown): string {
  return firstNonEmpty(manifest, ["draft_id"]);
}

/** True when `value` is already a conforming, lowercase app-id slug. */
export function isCanonicalAppId(value: unknown): boolean {
