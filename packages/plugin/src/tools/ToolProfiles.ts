...
export const EVOLVE_TAGS = [":evolve:", ":explicit:evolve"] as const;
/** Substrings marking a key as schedule-triggered. */
export const SCHEDULED_TAGS = [":cron:", ":heartbeat", ":scheduled"] as const;
/** Route-segment values that are NOT channel names. */
export const NON_CHANNEL_ROUTES: ReadonlySet<string> = new Set([
 ...
 */
export const CONSOLE_SESSION_PREFIXES = ["admin-ui"] as const;
...
export function classifySessionKind(
  ...
  channel?: string | null,
): { kind: SessionKind; channel: string | null } {
  ...
  if (ch) return { kind: "user", channel: ch };
  ...
  if (route === EXPLICIT_ROUTE) {
    ...
    if (console_) return { kind: "user", channel: console_ };
    return { kind: "oneshot", channel: null };
  }
  if (route === SUBAGENT_ROUTE) return { kind: "subagent", channel: null };
 ...
 */
export const NO_LIVE_SPEAKER_TOOLS: readonly string[] = [
  ...
  // The Morning Board's stocking run is a scheduled session with no live
  // speaker, and stocking IS board.add — trimming this would leave that run
  // with nowhere to put a card (D-MB5).
  "board",                 // the user's task board; list/add/move/assign/progress
...
export const TOOL_PROFILES: Readonly<Record<string, ToolProfile>> = {
  ...
  no_live_speaker: {
    ...
    why:
      "this session has no live human speaker, so the roster, channel-config, " +
  ...
  evolve_dispatch: {
    ...
    tools: [],
    why:
      "Evolve dispatched this session for one judgment and reads only the " +
 ...
 */
export const PROFILE_BY_KIND: Readonly<Partial<Record<SessionKind, string>>> = {
...
export function resolveToolProfile(kind: SessionKind): ToolProfile {
...
export function profileAllows(profile: ToolProfile, toolName: string): boolean {
  return profile.tools === "full" || profile.tools.includes(toolName);
}

// ── Runtime: the registration filter ────────────────────────────────────────
...
export function trimmedDescription(profile: ToolProfile, kind: SessionKind): string {
  return (
    `Not available in this session (tool profile "${profile.id}", session kind ` +
...
export function refusalText(
  toolName: string, profile: ToolProfile, kind: SessionKind,
): string {
  return (
    `${toolName} is not available in this session. Tool profile ` +
 ...
 */
export function writeToolProfileRefusalLedger(
  config: ToolProfileConfig,
  rec: ToolProfileRefusalRecord,
 ...
 */
export function trimToolDefinition(
  def: ToolDefinition,
  profile: ToolProfile,
  kind: SessionKind,
  ...
  return {
    ...
    async execute() {
      ...
      try {
        logger.info?.(
          `Evolve tool profiles: refused ${name} under profile ${profile.id} ` +
          `(session kind ${kind})`,
        );
...
export function applyToolProfile(
  ...
  return (ctx: ToolFactoryContext) => {
    ...
    try {
      ...
      if (profileAllows(profile, name)) return def;
 ...
 */
export function installToolProfileFilter<
  T extends { registerTool: (f: ToolFactory, ...rest: unknown[]) => unknown },
