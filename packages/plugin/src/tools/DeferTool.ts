...
const MODE_MESSAGE = "message" as const;
const MODE_ACTION = "action" as const;
const STATUS_PENDING = "pending" as const;

export const DeferToolParamsSchema = Type.Object({
  due_at: Type.String({
    description:
      "When to fire, as an ISO 8601 UTC timestamp (e.g. '2026-05-05T13:04:00Z'). " +
  ...
  message: Type.Optional(
    Type.String({
      description:
        "Literal text to deliver to the user when fired. Use this when you " +
  ...
  action: Type.Optional(
    Type.String({
      description:
        "Instruction for a follow-up turn that needs work later (e.g. 'check " +
...
interface DeferRow {
  ...
  // Routing-style session key (e.g. agent:main:telegram:direct:...) —
  // diagnostic only; the CLI does NOT accept this format.
  session_key: string | null;
  fires_at: string;
  created_at: string;
  mode: typeof MODE_MESSAGE | typeof MODE_ACTION;
  message: string | null;
  action: string | null;
...
function validateParams(params: DeferToolParams): {
  ok: true;
  fires_at_iso: string;
} | {
  ok: false;
  ...
  if (target.getTime() < now - MAX_PAST_MS) {
    return {
      ok: false,
      error:
        `'due_at' (${due_at}) is in the past — defer needs a future time. ` +
...
function appendRow(row: DeferRow): void {
 ...
 */
export function createDeferToolFactory(
  config: { botId: string },
  logger: PluginLogger,
) {
  return (ctx: {
    ...
    logger.debug(
      `defer: factory invoked agentId=${ctx.agentId ?? "?"} ` +
    ...
    async execute(_toolCallId: string, rawParams: unknown) {
      const params = rawParams as DeferToolParams;
      const v = validateParams(params);
      if (!v.ok) {
        return {
          content: [{ type: "text" as const, text: `defer: ${v.error}` }],
          isError: true,
        };
      }
      ...
      if (!ctx.sessionId) {
        logger.warn(
          "defer: ctx.sessionId (UUID) is undefined — runner will be unable " +
      ...
      if (!config.botId || config.botId === "unknown") {
        logger.warn(
          `defer: config.botId looks unset (${JSON.stringify(config.botId)}) — queue ` +
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `defer: failed to enqueue — ${msg}` }],
          isError: true,
        };
      }

      logger.info(
        `defer: queued ${row.defer_id.slice(0, 8)} mode=${row.mode} ` +
