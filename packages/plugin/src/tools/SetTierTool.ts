...
export const SetTierToolParamsSchema = Type.Object({
  ...
  scope: Type.Optional(
    Type.Union(
      [Type.Literal("session"), Type.Literal("standing")],
      {
        description:
          "'session' (default) = this conversation only — for 'this " +
...
interface SetTierResult {
  ...
  // "session" | "standing" — echoed so the bot's ack states the right
  // scope (G4 addendum: the ack is the user's ONLY confirmation).
  scope?: string;
  // Standing-scope only: guidance for phrasing the user-facing ack.
  ack_hint?: string;
  error?: string;
...
function _resultContent(r: SetTierResult): {
  ...
  if (r.scope !== undefined) body.scope = r.scope;
  if (r.ack_hint !== undefined) body.ack_hint = r.ack_hint;
        ...
        if (scope === "standing") {
          if (!modelRouter.isUserTierSurfaceEnabled()) {
            return _resultContent({
              ok: false,
              error:
                "tier-control surfaces are disabled on this bot " +
          ...
          if (!userKey) {
            return _resultContent({
              ok: false,
              error:
                "this session has no pinned user identity (internal/" +
          ...
          try {
            modelRouter.setStandingUserTierDefault(
              userKey,
            ...
            logger.warn(
              `session_set_tier: bot=${botId} standing write FAILED for ` +
              `user_key=${userKey}: ${msg}`,
            );
            return _resultContent({
              ok: false,
              error:
                `standing default write failed: ${msg}. Nothing was saved. ` +
          ...
          const standingReason = params.reason
            ...
            : "";
          logger.info(
            `session_set_tier: bot=${botId} scope=standing ` +
