...
export const RecordApplicationToolParamsSchema = Type.Object({
  app_id: Type.String({
    description:
      "Slug for this application (lowercase alphanumeric + hyphens, " +
  ...
  name: Type.Optional(
    Type.String({
      description:
        "Human-readable name (e.g. 'Protein Tracker'). Defaults to a " +
  ...
  purpose: Type.Optional(
    Type.String({
      description:
        "1–2 sentences on what this app does and for whom. The forge " +
  ...
  files: Type.Optional(
    Type.Array(Type.String(), {
      description:
        "All scripts, data files, and assets that belong to this app. " +
    ...
    Type.Array(
      Type.Object({
        schedule: Type.String({
          description:
            "Cron schedule string ('0 21 * * *') or @-keyword ('@daily').",
        }),
        script: Type.String({
          description:
            "Path to the script the cron runs. Should also appear in `files`.",
        }),
      }),
      {
        description:
          "Cron entries the bot installed for this app. List them here even " +
  ...
  inputs: Type.Optional(
    Type.Array(Type.String(), {
      description:
        "What the app consumes (e.g. 'user voice messages', 'tasks.json'). " +
  ...
  outputs: Type.Optional(
    Type.Array(Type.String(), {
      description:
        "What the app produces (e.g. 'daily summary message', 'log entry'). " +
        "Free-form short phrases.",
    }),
  ),
  test_command: Type.Optional(
    Type.String({
      description:
        "Optional shell command to run the app's test suite from the " +
  ...
  update: Type.Optional(
    Type.Boolean({
      description:
        "Set true when amending an existing manifest with the same " +
...
export type RecordApplicationToolParams = Static<
  typeof RecordApplicationToolParamsSchema
>;
...
function botEvolveDir(botId: string): string {
  ...
  if (override) return path.join(override, botId);
...
function validateParams(params: RecordApplicationToolParams): {
  ok: true;
} | {
  ok: false;
  error: string;
  ...
  if (typeof app_id !== "string" || !APP_ID_PATTERN.test(app_id)) {
    return {
      ok: false,
      error:
        `'app_id' must be lowercase alphanumeric + hyphens, 3–48 chars, ` +
        `starting and ending with an alphanumeric — got ${JSON.stringify(app_id)}.`,
    };
  }
  if (files !== undefined) {
    if (!Array.isArray(files)) return { ok: false, error: "'files' must be an array of strings." };
  ...
  if (crons !== undefined) {
    if (!Array.isArray(crons)) return { ok: false, error: "'crons' must be an array." };
    for (const c of crons) {
      if (
        !c || typeof c !== "object" ||
        typeof (c as CronEntry).schedule !== "string" || (c as CronEntry).schedule.trim() === "" ||
        typeof (c as CronEntry).script !== "string"   || (c as CronEntry).script.trim() === ""
      ) {
        return {
          ok: false,
...
function buildRow(
  params: RecordApplicationToolParams,
  ctx: {
    botId: string;
...
function appendRow(row: ReflexRow): void {
...
function ensureQueueFileGroupWritable(file: string): void {
  try {
 ...
 */
export function createRecordApplicationToolFactory(
  config: { botId: string },
  logger: PluginLogger,
) {
  return (ctx: {
    ...
    return {
      ...
      async execute(_toolCallId: string, rawParams: unknown) {
        const params = rawParams as RecordApplicationToolParams;
        const v = validateParams(params);
        if (!v.ok) {
          return {
            content: [{ type: "text" as const, text: `record_application: ${v.error}` }],
            isError: true,
          };
        }
        ...
        if (!config.botId || config.botId === "unknown") {
          logger.warn(
            `record_application: config.botId looks unset (${JSON.stringify(config.botId)}) — ` +
        ...
        try {
          ...
          return {
            content: [
              { type: "text" as const, text: `record_application: failed to enqueue — ${msg}` },
            ],
            isError: true,
          };
        }
        ...
        logger.info(
          `record_application: queued ${row.reflex_id.slice(0, 8)} app=${row.app_id} ` +
