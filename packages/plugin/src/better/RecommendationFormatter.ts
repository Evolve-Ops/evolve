...
export class RecommendationFormatter {
  ...
  parseReply(
    text: string,
    pendingRecExists: boolean = false,
  ): FollowUpAction | null {
    const normalized = text.trim().toLowerCase();

    // Accept
    if (
      ["yes", "accept", "do it", "ok", "sure"].includes(normalized)
    ) {
      return "accept";
    }
    ...
    // Snooze
    if (
      ["snooze", "later", "not now", "remind me"].includes(normalized)
    ) {
      return "snooze";
    }
  ...
  private getTitle(rec: any, surface: Surface): string {
  ...
  private getDetail(rec: any, surface: Surface): string {
  ...
  private urgencyEmoji(rec: any): string {
    const tags: string[] = rec.tags ?? [];
    if (tags.includes("urgency:critical")) return "🔴";
    if (tags.includes("urgency:high")) return "🟠";
    if (tags.includes("urgency:medium")) return "🟡";
    return "🟢";
  }
}
