...
export interface EvoDispatchResult {
   ...
   */
  session_tier_override: { choice: string; consent_source: string } | null;
}
 ...
 */
export type EvoDispatchFailureReason =
  | "unreachable"
  | "unauthorized"
  | "empty"
  | "error";
...
/** Internal: raw admin response with the HTTP status preserved. */
...
export function classifyDispatchResponse(
  ...
  return { ok: true, result: body as EvoDispatchResult };
}
 ...
 * fall-through site.
 */
export function evoFailureUserMessage(reason: EvoDispatchFailureReason): string {
  switch (reason) {
    case "unreachable":
      return (
        "⚠️ evo can't reach its admin service right now (looks like a " +
        "connection issue). Your message wasn't lost — try again shortly, " +
        "or check the admin logs."
      );
    case "unauthorized":
      return (
        "⚠️ evo can't reach its admin service right now (looks like an " +
        "auth/pairing issue). Your message wasn't lost — try again shortly, " +
        "or check the admin logs."
      );
    case "empty":
      return (
        "⚠️ evo hit an internal error handling that command. Nothing was " +
        "changed — try again shortly, or check the admin logs."
      );
    case "error":
    default:
      return (
        "⚠️ evo hit an error reaching its admin service. Try again shortly, " +
        "or check the admin logs."
      );
  }
}
...
export class EvoDispatchClient {
  ...
  async findActiveWizard(
    ...
    try {
      ...
      if (senderExternalId) params.set("sender_external_id", senderExternalId);
  ...
  async wizardTurn(
    ...
    try {
      ...
      if (typeof result.system_append !== "string") return null;
      ...
      return result as WizardTurnResult;
    } catch {
      return null;
    }
  }
