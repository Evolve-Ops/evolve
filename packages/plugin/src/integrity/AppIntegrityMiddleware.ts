 ...
 */
export function makeAppIntegrityHandler(
  registry: { lookup(command: string): { appId: string; script: string } | null },
  ...
  return (
    ...
    try {
      ...
      if (!command) return; // not a shell tool / no command → passthrough

      const match = registry.lookup(command);
      if (!match) return; // command did not run a known app script → passthrough

      // Explicit app-attribution signal (AL-1.1, design §4.2 source 2): this
      // tool call ran a KNOWN app's script — record it for the turn regardless
      // of the script's outcome (a failing run still served that app).
      // recordExplicit never throws; the middleware's behavior and its
      // never-fabricate invariant are untouched.
      recordExplicit(context?.runId, context?.sessionId, match.appId, "script_middleware");
 ...
 */
export function registerAppIntegrityMiddleware(
  api: AgentToolResultMiddlewareApi,
  config: EvolveConfig,
  logger: MiddlewareLogger,
): void {
  if (typeof api.registerAgentToolResultMiddleware !== "function") {
    logger.info(
      "Evolve app-integrity: gateway does not expose " +
  ...
  const handler = makeAppIntegrityHandler(registry, logger, () =>
    checkGateLivenessOnToolActivity(config, logger),
  );

  api.registerAgentToolResultMiddleware(handler, { runtimes: ["openclaw"] });
  logger.info(
    `Evolve app-integrity: tool-result middleware registered for ${config.botId} ` +
