...
import { inTurnCheckpoint } from "../breakers/InTurnCheckpoint.js";
import type {
 ...
 */
export const SENSITIVE_CAPABILITY_BY_TOOL: Readonly<Record<string, string>> = Object.freeze({
 ...
 */
export const KNOWN_SAFE_TOOLS: ReadonlySet<string> = Object.freeze(
  new Set<string>([
 ...
 */
export const CAPABILITY_BY_TOOL_KIND: Readonly<Record<string, string>> = Object.freeze({
  code_mode_exec: "bot.code.modify",
} as Record<PluginHookToolKind, string>);
 ...
 */
export function normalizeToolName(raw: string | undefined | null): string {
...
function shortRun(runId: string | null | undefined): string {
 ...
 */
export function evaluateGatedCall(
  config: Pick<EvolveConfig, "botId" | "sharedDir">,
  requiredCap: string,
  ...
  if (!sender || !sender.senderId) {
    ...
    return {
      ...
      platform: null,
      reason:
        `Evolve Layer-2 gate: the speaker for this turn could not be identified ` +
  ...
  if (!sender.platform) {
    return {
      ...
      platform: null,
      reason:
        `Evolve Layer-2 gate: speaker id=${sender.senderId} has no recognized ` +
 ...
 */
export function writeGateDecisionLedger(
  config: Pick<EvolveConfig, "botId" | "sharedDir">,
  rec: GateLedgerRecord,
  logger: GateLogger,
): void {
  try {
...
const _driftSeen = new Set<string>();

function logToolDrift(
  config: Pick<EvolveConfig, "botId">,
  toolName: string,
  toolKind: string | null,
  logger: GateLogger,
): void {
  const key = `${config.botId}::${toolName}`;
  if (_driftSeen.has(key)) return;
  _driftSeen.add(key);
  try {
    logger.warn(
      `Evolve Layer-2 gate: DRIFT — tool '${toolName}'` +
 ...
 */
export function checkGateLivenessOnToolActivity(
  config: Pick<EvolveConfig, "botId" | "layer2Enforce">,
  logger: GateLogger,
): void {
  if (!config.layer2Enforce) return; // observe-only — nothing armed to be dead
  if (_beforeToolCallEverFired) return; // provably live
  if (_armedButDeadWarned) return; // one-shot
  _armedButDeadWarned = true;
  try {
    logger.error(
      `Evolve Layer-2 gate: ARMED BUT NOT LIVE — layer2.enforce is true for ` +
...
export function detectOcVersion(api: {
  ...
  const candidates: unknown[] = [
    (api as { openclawVersion?: unknown })?.openclawVersion,
    (api as { version?: unknown })?.version,
  ...
  for (const c of candidates) {
    if (typeof c === "string" && parseVersion(c)) return c;
  }
  return null;
}
 ...
 */
export function enforceAllowedByVersion(
  api: { config?: Record<string, unknown> },
  config: Pick<EvolveConfig, "botId">,
  logger: GateLogger,
): boolean {
  const raw = detectOcVersion(api);
  if (!raw) return true; // undiscoverable → don't refuse on version alone
  const got = parseVersion(raw);
  const min = parseVersion(OC_MIN_VERSION_FOR_HOOK)!;
  if (got && versionLt(got, min)) {
    logger.error(
      `Evolve Layer-2 gate: REFUSING to arm — gateway OC version ${raw} is below ` +
...
export function makeBeforeToolCallHandler(
  ...
  return (event, ctx) => {
    ...
    // In-turn cost checkpoint (D-CS12) — a cost brake, not authorization, so
    // it vetoes regardless of layer2Enforce. Fail-open: it fires only on a
    // readable run total at or over the threshold.
    try {
      const d = inTurnCheckpoint.evaluate(
        (event?.runId ?? ctx?.runId ?? null) as string | null,
        (ctx?.sessionId ?? null) as string | null,
      );
      if (d.kind === "veto") return { block: true, blockReason: d.reason };
    } catch (err) {
      try { logger.warn(`Evolve in-turn checkpoint: evaluation error, allowed: ${err}`); } catch { /* */ }
    }
    ...
    let requiredCap: string | undefined;
    let toolName = "";
    let toolKind: string | null = null;
    let runId: string | null = null;
    try {
      toolName = String(event?.toolName ?? ctx?.toolName ?? "");
      toolKind = (event?.toolKind ?? ctx?.toolKind ?? null) as string | null;
      runId = (event?.runId ?? ctx?.runId ?? null) as string | null;
      ...
      if (!requiredCap) return; // known-safe tool → allow (void)

      const evaluation = evaluateGatedCall(config, requiredCap, runId);
      if (evaluation.allowed) return; // authorized → allow (void)

      // ── would-block ─────────────────────────────────────────────────────────
      const enforcing = config.layer2Enforce;
      const mode: "observe" | "enforce" = enforcing ? "enforce" : "observe";
      const action: GateLedgerRecord["action"] = enforcing ? "block" : "would_block";

      writeGateDecisionLedger(
        config,
        {
      ...
      logger.warn(
        `Evolve Layer-2 gate [${mode}] ${enforcing ? "BLOCKED" : "would block"} ` +
      ...
      if (requiredCap) {
        ...
        try {
          logger.error(
            `Evolve Layer-2 gate: internal error evaluating gated tool ` +
        ...
        if (enforcing) {
          return {
            block: true,
            blockReason:
              "Evolve Layer-2 gate: could not resolve authorization for this tool; " +
              "blocked (fail-closed).",
          };
        }
...
export function registerToolCallGate(
  ...
  if (typeof api.on !== "function") {
    logger.info(
      `Evolve Layer-2 gate: gateway api.on unavailable — gate inactive for ${config.botId}`,
    );
    return;
  }
  ...
  const versionSafe =
    !config.layer2Enforce || enforceAllowedByVersion(api, config, logger);
  const effectiveConfig: EvolveConfig =
    config.layer2Enforce && !versionSafe
      ? { ...config, layer2Enforce: false }
      : config;

  const handler = makeBeforeToolCallHandler(effectiveConfig, logger);
  if (config.inTurnCheckpointWarning) logger.warn(config.inTurnCheckpointWarning);
  try {
    api.on(
      "before_tool_call",
      async (event: PluginHookBeforeToolCallEvent, ctx?: PluginHookToolContext) =>
        handler(event, ctx),
      { name: "evolve-before-tool-call-gate" },
    );
    inTurnCheckpoint.configure({
      sharedDir: config.sharedDir, botId: config.botId,
      thresholdUsd: config.inTurnCheckpointUsd, registered: true, logger,
    });
    logger.info(
      `Evolve Layer-2 gate: before_tool_call registered for ${config.botId} ` +
    ...
    inTurnCheckpoint.configure({
      sharedDir: config.sharedDir, botId: config.botId,
      thresholdUsd: config.inTurnCheckpointUsd, registered: false, logger,
    });
    logger.info(
      `Evolve Layer-2 gate: before_tool_call not supported by this gateway ` +
