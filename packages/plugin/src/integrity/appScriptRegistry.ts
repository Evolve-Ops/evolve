 ...
 */
export function defaultWorkspaceRoot(botId: string, logger?: RegistryLogger): string {
  let home = "";
  try {
    home = os.homedir() || "";
  } catch {
  ...
  if (!home.trim()) {
    ...
    logger?.debug(
      `Evolve app-integrity: no resolvable process home (HOME unset/empty and ` +
 ...
 *
 * AL-1.4a: the implementation moved to `apps/appIdentity.ts` so TS has ONE
 * resolver instead of a copy per consumer, and so the `app_id` canonical field
 * lands ahead of the legacy chain in one edit. Re-exported under this name
 * because this module is the documented import site for the middleware and for
 * tests/appIdResolution.test.mjs.
 */
export { appIdOf };

/** Resolve a manifest-declared path to absolute (workspace-relative ⇒ abs). */
export function toAbs(p: string, workspaceRoot: string): string | null {
...
export function toWorkspaceRel(abs: string, workspaceRoot: string): string {
...
export function collectScriptPaths(manifest: unknown, workspaceRoot: string): string[] {
  const out: string[] = [];
  ...
  const add = (p: unknown): void => {
    ...
    if (!abs) return;
    if (!SCRIPT_EXTS.has(path.extname(abs).toLowerCase())) return;
  ...
  if (Array.isArray(realized)) {
    for (const rf of realized) {
      if (rf && typeof rf === "object") add((rf as Record<string, unknown>)["path"]);
    }
  }
  ...
  if (Array.isArray(triggers)) {
    for (const t of triggers) {
      const inv =
        t && typeof t === "object" ? (t as Record<string, unknown>)["invocation"] : null;
      if (inv && typeof inv === "object") add((inv as Record<string, unknown>)["script"]);
    }
  }
  ...
  const bfiles =
    blueprint && typeof blueprint === "object"
      ? (blueprint as Record<string, unknown>)["files"]
      : null;
  ...
  const cli = ic && typeof ic === "object" ? (ic as Record<string, unknown>)["cli"] : null;
  if (Array.isArray(cli)) {
    for (const c of cli) {
      const cmd = c && typeof c === "object" ? (c as Record<string, unknown>)["command"] : null;
      if (typeof cmd === "string") {
        for (const tok of tokenize(cmd)) {
          const t = stripQuotes(tok);
          if (t.includes("/") && SCRIPT_EXTS.has(path.extname(t).toLowerCase())) add(t);
        }
      }
    }
  }
...
export function knownScriptsForManifest(
  manifest: unknown,
...
export function coverageOf(scripts: KnownScript[]): AppCoverage[] {
...
export function lookupCommand(
  command: string,
  scripts: KnownScript[],
): { appId: string; script: string } | null {
  if (!command || typeof command !== "string" || scripts.length === 0) return null;
  const tokens = tokenize(command);
  for (let i = 0; i < tokens.length; i++) {
    if (!isExecutablePosition(tokens, i)) continue;
    const tok = stripQuotes(tokens[i] ?? "");
    if (!tok) continue;
    const match = matchToken(tok, scripts);
    if (match) return { appId: match.appId, script: match.absPath || match.relPath };
  }
  return null;
}

function matchToken(tok: string, scripts: KnownScript[]): KnownScript | null {
  // Exact absolute-path match first.
  for (const ks of scripts) {
    if (ks.absPath === tok) return ks;
  }
  // Workspace-relative: token equals rel, or token ends with "/<rel>".
  for (const ks of scripts) {
    if (!ks.relPath) continue;
    if (tok === ks.relPath || tok.endsWith("/" + ks.relPath)) return ks;
  }
  return null;
}
...
export function stripQuotes(tok: string): string {
  if (tok.length >= 2) {
    const a = tok[0];
 ...
 */
export function isExecutablePosition(tokens: string[], i: number): boolean {
  if (i === 0) return true;
  let j = i - 1;
  while (j >= 0) {
    const prev = tokens[j] ?? "";
    ...
    if (SHELL_OPS.has(prev)) return true;
    if (INTERPRETERS.has(prev)) return true;
...
export class AppScriptRegistry {
  ...
  private scripts: KnownScript[] = [];
  private coverage: AppCoverage[] = [];

  private loaded = false;
  private lastScanAt = 0;
  private lastDirMtime = -1;
  ...
  lookup(command: string): { appId: string; script: string } | null {
    this.refresh();
  ...
  private refresh(): void {
    const now = Date.now();
    if (this.loaded && now - this.lastScanAt < RESCAN_THROTTLE_MS) return;
  ...
  private setEmpty(now: number): void {
    this.scripts = [];
    this.coverage = [];
    this.loaded = true;
    this.lastScanAt = now;
