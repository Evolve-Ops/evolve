...
import {
  ...
  renderInTurnCheckpointMessage,
} from "./CostCheckpoint.js";
...
export function resolveInTurnCheckpointUsd(
  pluginConfig: Record<string, unknown>,
): { usd: number; warning: string | null } {
  const raw = pluginConfig.inTurnCheckpointUsd;
  if (raw === undefined || raw === null) return { usd: IN_TURN_CHECKPOINT_DEFAULT_USD, warning: null };
  ...
  return {
    ...
    warning:
      `Evolve in-turn checkpoint: inTurnCheckpointUsd=${JSON.stringify(raw)} refused — ` +
...
export function renderInTurnVeto(spendUsd: number): string {
  return (
    `Evolve checkpoint: this turn has spent ${money(spendUsd)} (recorded) — reply to ` +
    `the user now with what you have and say what is unfinished. The owner can continue it.`
  );
}

/** Every later veto in the same run. */
export const IN_TURN_VETO_REPEAT =
  "Evolve checkpoint: tool calls are stopped for this turn — reply to the user now.";
...
export type InTurnDecision =
  | { kind: "unknown" }
  | { kind: "allow"; spendUsd: number }
  | { kind: "veto"; first: boolean; spendUsd: number; reason: string };
...
export class InTurnCheckpoint {
  ...
  private trippedSessions = new Map<string, number>();
  private sharedDir = "";
  private botId = "";
  private thresholdUsd = IN_TURN_CHECKPOINT_DEFAULT_USD;
  ...
  configure(opts: { sharedDir: string; botId: string; thresholdUsd: number; registered: boolean; logger?: Logger }): void {
  ...
  private run(runId: string, sessionId: string | null, now: number): RunState {
  ...
  recordCost(runId: string | null | undefined, sessionId: string | null, costUsd: number, now = new Date()): void {
    if (!runId) return;
    const r = this.run(String(runId), sessionId, now.getTime());
    r.llmCalls += 1;
    if (r.llmCalls === 1) this.counters.spend_runs += 1;
    if (r.llmCalls === 2) this.counters.multi_call_runs += 1; // a tool call sat between them
    if (Number.isFinite(costUsd) && costUsd > 0) r.spendUsd += costUsd;
    else this.counters.zero_cost_calls += 1; // estimateCost's 0 = "could not price"
    this.writeStatus(now);
  }

  /** The before_tool_call decision. */
  evaluate(runId: string | null | undefined, sessionId: string | null, now = new Date()): InTurnDecision {
    if (!runId) {
    ...
    if (!r) return { kind: "allow", spendUsd: 0 };
    r.toolCalls += 1;
    r.lastSeenMs = now.getTime();
    if (r.toolCalls === 1) this.counters.evaluated_runs += 1;
    if (r.tripped) return { kind: "veto", first: false, spendUsd: r.tripSpendUsd, reason: IN_TURN_VETO_REPEAT };
    if (r.spendUsd < r.thresholdUsd) return { kind: "allow", spendUsd: r.spendUsd };
    ...
    if (sid) this.trippedSessions.set(sid, now.getTime());
    this.append({
    ...
    this.logger?.warn(
      `Evolve in-turn checkpoint TRIPPED bot=${this.botId} run=${String(runId).slice(0, 8)} ` +
  ...
  takeOwnerNotice(runId: string | null | undefined, now = new Date()):
    { spendUsd: number; thresholdUsd: number; incrementUsd: number; llmCalls: number; toolCalls: number } | null {
    if (!runId) return null;
    const r = this.runs.get(String(runId));
    if (!r || !r.noticePending) return null;
  ...
  noteTurnStart(runId: string | null | undefined, sessionId: string | null, message: string, now = new Date()): void {
    if (!runId || !sessionId || !this.trippedSessions.has(sessionId)) return;
    if (classifyCheckpointAnswer(message) !== "continue") return;
  ...
  endRun(runId: string | null | undefined, now = new Date()): void {
    const r = runId ? this.runs.get(String(runId)) : undefined;
    if (r && r.endedMs === null) r.endedMs = now.getTime();
  }

  /** A trip whose notice never went out (NO_REPLY, no text) is ledgered. */
  private drop(runId: string, r: RunState, now: number): void {
    if (r.noticePending) {
      this.append({
  ...
  private prune(now: number): void {
    for (const [k, r] of this.runs) {
      if (now - r.lastSeenMs > RUN_IDLE_MS || (r.endedMs !== null && now - r.endedMs > ENDED_KEEP_MS)) this.drop(k, r, now);
    }
    for (const [k, t] of this.trippedSessions) if (now - t > 24 * RUN_IDLE_MS) this.trippedSessions.delete(k);
  }
...
export function withInTurnOwnerNotice(
  event: { runId?: unknown; payload?: { text?: unknown } } | null | undefined,
  result: { payload?: any } | undefined,
  ledger: InTurnCheckpoint = inTurnCheckpoint,
): { payload?: any } | undefined {
  try {
    const payload = result?.payload ?? event?.payload;
    if (!payload || typeof payload.text !== "string" || !payload.text.trim()) return result;
