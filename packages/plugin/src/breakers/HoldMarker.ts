...
export function recordInteractiveHold(opts: {
  ...
  try {
    ...
    if ((err?.code === "EACCES" || err?.code === "EPERM") && !warnedEACCES) {
      warnedEACCES = true;
      opts.logger?.warn(
        `Evolve cost checkpoint: cannot record interactive-hold evidence ` +
      ...
      opts.logger?.warn(
        `Evolve cost checkpoint: failed to record interactive-hold ` +
