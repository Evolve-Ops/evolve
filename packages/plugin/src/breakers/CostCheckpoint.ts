...
export interface CheckpointStatus {
  ...
  /**
   * The spend that CROSSED the cap ($). Null when unreadable.
   *
   * Measured from the operator's reactivation marker when there is one, so
   * after a reactivation this is spend SINCE that moment, not the day's
   * running total. ``acceptedUsd`` says how much came before it.
   */
  readonly spendUsd: number | null;
  /**
   * Spend the operator accepted when they last reactivated this bot today
   * ($), or null if they have not. The breaker counts from here.
   */
  readonly acceptedUsd: number | null;
  /** ``HH:MM`` (gateway-local) of that reactivation, or null. */
  readonly acceptedAtLabel: string | null;
  /** The day's raw running total ($) — what the Cost page calls today's spend. */
  readonly spendTotalUsd: number | null;
 ...
 */
function localHourMinute(iso: string | null): string | null {
  if (!iso) return null;
 ...
 */
function readSpendCapFlag(
  sharedDir: string, botId: string, now: Date,
): SpendCapFlag | null {
  try {
    const fp = path.join(
      sharedDir, "spend-caps", `${botId}-${localDateYMD(now)}.json`,
    );
    const data = JSON.parse(fs.readFileSync(fp, "utf8"));
    if (!data || typeof data !== "object" || data.cleared) return null;
    return {
      action: typeof data.action === "string" ? data.action : "",
      cap: typeof data.cap === "number" ? data.cap : null,
      spend:
        typeof data.spend_at_trigger === "number" ? data.spend_at_trigger : null,
      acceptedUsd:
        typeof data.accepted_usd === "number" ? data.accepted_usd : null,
      acceptedAt:
        typeof data.accepted_at === "string" ? data.accepted_at : null,
      spendTotal:
        typeof data.spend_total === "number" ? data.spend_total : null,
    };
  } catch {
    return null;
  }
...
export function readCostCheckpoint(opts: {
  ...
  const recPath = path.join(
    opts.sharedDir, "breakers", opts.botId, "cost.json",
  );
  const rec = readBreakerFile(recPath);
  if (rec === null || isExpired(rec, now)) return null;
  ...
  try {
    raw = JSON.parse(fs.readFileSync(recPath, "utf8")) as Record<string, unknown>;
  } catch {
    return null;
  }
  ...
  if (state !== "pending" && state !== "declined") return null;
  ...
  const baseCapUsd =
    capUsd === null ? null : Math.max(capUsd - (grantedUsd ?? 0), 0) || capUsd;
  return {
    state,
    capUsd,
 ...
 */
export function classifyCheckpointAnswer(
  ...
  const text = String(message ?? "")
    ...
    .replace(/[.!?]+$/, "")
    .trim();
  if (!text || text.length > _MAX_ANSWER_CHARS) return null;
  if (_CONTINUE_RE.test(text)) return "continue";
  if (_STOP_RE.test(text)) return "stop";
  return null;
}
 ...
 */
export function renderAcceptedLine(status: CheckpointStatus): string | null {
  ...
  return (
    `You accepted ${money(status.acceptedUsd)} at ${at ?? "reactivation"}; ` +
    `counting from then.`
  );
}

export function renderCapReachedMessage(
  status: CheckpointStatus, botName: string,
): string {
...
export function renderNotOwnerMessage(
  status: CheckpointStatus, botName: string,
): string {
  const accepted = renderAcceptedLine(status);
  return [
...
export function renderContinuedMessage(
  status: CheckpointStatus, botName: string, grantedUsd: number | null,
): string {
  const raised =
    grantedUsd !== null && status.capUsd !== null
...
export function renderDeclinedMessage(botName: string): string {
  return (
    `💤 ${botName} is paused until tomorrow — today's spending cap was ` +
...
export function renderAnswerUnavailableMessage(): string {
  return (
    "I couldn't record that — my admin daemon isn't answering, so I've " +
...
/** The in-turn checkpoint's owner notice (D-CS12), appended once per run. */
export function renderInTurnCheckpointMessage(f: {
  spendUsd: number; thresholdUsd: number; incrementUsd: number; llmCalls: number;
}): string {
  return [
    `💰 Evolve checkpoint — I was stopped mid-task. This turn had spent ` +
      `${money(f.spendUsd)} (recorded, catalog prices) over ${f.llmCalls} ` +
      `model calls, past the ${money(f.thresholdUsd)} per-turn checkpoint, so ` +
      "my tool calls were stopped and the answer above is what I had.",
    "",
    `  • Reply "continue" — I pick it up, and that turn's checkpoint is raised ` +
      `by ${money(f.incrementUsd)} to ${money(f.thresholdUsd + f.incrementUsd)}.`,
    "",
    "Only my owner can say continue. Nothing was downgraded.",
  ].join("\n");
}
