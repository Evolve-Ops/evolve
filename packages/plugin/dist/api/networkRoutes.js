...
export function registerNetworkRoutes(api, config) {
    ...
    function parseParam(req, name, position = -1) {
        if (req.params?.[name])
    ...
    api.registerHttpRoute({
        ...
        handler: async (req, res) => {
            ...
            try {
                ...
                const { stdout } = await execFileAsync(evolvePythonBin(), ["-m", "arbiter.security_screen", "--proposal", pendingPath], { cwd: resolveAnalyzerDir(config), timeout: 15_000 });
            ...
            const proposal = readJSON(pendingPath);
    ...
    api.registerHttpRoute({
        ...
        handler: async (req, res) => {
            ...
            proposal.rejection_reason = rejectBody?.reason ?? "other";
            proposal.rejection_note = rejectBody?.note ?? "";
            ...
            fs.appendFileSync(rejectionsPath, JSON.stringify({
                proposal_id: id,
                pattern_key: proposal.pattern_key,
                target_bot: proposal.target_bot,
                type: proposal.type,
                reason: rejectBody?.reason,
                note: rejectBody?.note,
    ...
    api.registerHttpRoute({
        ...
        handler: async (req, res) => {
            ...
            const cfgPath = networkConfigPath();
            const cfg = (readJSON(cfgPath) ?? {});
            cfg.members = (cfg.members ?? []).filter((m) => m !== botId);
            const bots = cfg.bots ?? {};
            delete bots[botId];
            cfg.bots = bots;
            writeJSON(cfgPath, cfg);
    ...
    const manifestsDir = () => path.join(`/Users/${config.botId}/.openclaw/workspace`, "manifests");
        ...
        handler: async (_req, res) => {
            ...
            if (fs.existsSync(dir)) {
                for (const f of fs.readdirSync(dir)) {
                    if (!f.endsWith(".json") || f.startsWith(".") || f.includes("_history"))
                        continue;
