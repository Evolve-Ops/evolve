...
export function cronJobIdFromSessionKey(key) {
...
function readJsonCached(filePath, logger) {
    let st;
    try {
...
function jobNameById(jobsData, jobId) {
...
function jobNameFromSqliteStore(home, jobId, logger) {
    const cached = _sqliteNameCache.get(jobId);
    if (cached && Date.now() - cached.at < SQLITE_NAME_TTL_MS)
        return cached.name;
    ...
    if (_sqliteNameCache.size >= MAX_SQLITE_NAME_ENTRIES) {
        ...
        if (oldest !== undefined)
            _sqliteNameCache.delete(oldest);
    }
...
function sweepStaleClaims(claimDir, logger) {
    const now = Date.now();
    if (now - _lastSweepAt < SWEEP_MIN_INTERVAL_MS)
        return;
    _lastSweepAt = now;
    let names;
    try {
    ...
    for (const name of names) {
        ...
        try {
            if (now - fs.statSync(file).mtimeMs > CLAIM_MAX_AGE_MS)
                fs.unlinkSync(file);
        }
        catch (err) {
...
function consumeClaim(claimDir, sessionId, logger) {
 ...
 * throws.
 */
export function captureScheduledAttribution(event, ctx, opts) {
    try {
        const ev = event;
        const c = ctx;
        ...
        if (consumeClaim(claimDir, sessionId, opts.logger))
            return;
        ...
        const jobs = readJsonCached(path.join(home, ".openclaw", "cron", "jobs.json"), opts.logger);
        const name = jobNameById(jobs, jobId) ?? jobNameFromSqliteStore(home, jobId, opts.logger);
        if (!name)
            return;
        const map = readJsonCached(path.join(opts.sharedDir, opts.botId, CRON_MAP_FILENAME), opts.logger);
        const appId = map && typeof map === "object" && !Array.isArray(map)
            ? map[name]
            : undefined;
...
export function _resetScheduledAttributionForTests() {
    _jsonCache.clear();
    _sqliteNameCache.clear();
    _warnedReasons.clear();
    _lastSweepAt = 0;
}
//# sourceMappingURL=scheduledAttribution.js.map