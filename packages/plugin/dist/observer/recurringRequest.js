 ...
 */
export function normalizeRequestLabel(text) {
    ...
    if (SENTINEL_TEXTS.has(lowered.replace(/[^a-z_]/g, "")))
        return null;
    ...
    for (const tok of tokens) {
        if (tok.length < 3)
            continue; // "to", "of", stray letters
        if (VOLATILE_TOKENS.has(tok))
            continue;
        ...
        seen.add(tok);
        kept.push(tok);
        if (kept.length >= MAX_LABEL_TOKENS)
            break;
    }
    if (kept.length < MIN_LABEL_TOKENS)
        return null;
    return kept.join(" ");
}
/**
...
export function composeLabel(head, appTags) {
    const cleaned = [];
...
function readJsonCached(filePath, slot, nowMs) {
 ...
 */
export function isRecurringRequestEnabled(sharedDir, botId, nowMs = Date.now()) {
 ...
 */
export function isRequesterExcluded(sharedDir, botId, platform, senderId, nowMs = Date.now()) {
    const overlay = readJsonCached(path.join(sharedDir, "rosters", `${botId}.json`), "overlay", nowMs);
    if (!overlay)
        return false; // no overlay → no exclusions (fresh bot)
...
export function _resetRecurringRequestCaches() {
    _networkCache = null;
    _overlayCache = null;
}
/**
 ...
 */
export function buildRecurringRequest(input) {
    ...
    if (input.sharedDir && input.botId) {
        // Gate 1 — per-bot DNT.
        if (!isRecurringRequestEnabled(input.sharedDir, input.botId, nowMs))
            return null;
 ...
 */
export function firstUserTurn(turns) {
    for (const t of turns) {
        if (typeof t?.userMessage === "string" && t.userMessage.trim())
            return t;
    }
    return null;
}
//# sourceMappingURL=recurringRequest.js.map