...
import { FULL_PROFILE_ID, PROFILE_BY_KIND, TOOL_PROFILES, profileAllows, stubChars, } from "../tools/ToolProfiles.js";
 ...
 */
function widestKindFor(profileId) {
    let widest = "";
    for (const [kind, id] of Object.entries(PROFILE_BY_KIND)) {
        if (id === profileId && kind.length > widest.length)
            widest = kind;
    }
    return (widest || profileId);
}
/**
 ...
 */
export function profileWeights(tools) {
    const out = {};
    for (const [id, profile] of Object.entries(TOOL_PROFILES)) {
        ...
        const sum = (rows) => rows.reduce((acc, r) => acc + Math.max(0, r.chars), 0);
