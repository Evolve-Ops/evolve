...
export const EXACT_MODEL_COSTS = {
    // Anthropic
...
export function _resetPricingCatalogCache() {
    catalogCache.clear();
}
function buildIndex(doc) {
    ...
    for (const [bare, rec] of bareSeen)
        if (rec)
            byBareId.set(bare, rec);
    const refreshed = doc?.refreshed_at;
    return {
        byKey,
        byBareId,
 ...
 */
export function loadPricingCatalog(sharedDir) {
...
export function catalogRefreshedAt(sharedDir) {
...
function candidateKeys(model, provider) {
    const segs = model.split("/").map((s) => s.trim()).filter(Boolean);
    if (segs.length === 0)
        return { keys: [], bare: "" };
    if (segs.length > 1 && segs[0] === provider)
        segs.shift();
...
function rateFromRecord(rec, field) {
    const raw = rec[field];
...
function catalogRates(index, model, provider, tableRates) {
    ...
    if (!rec && bare)
        rec = index.byBareId.get(bare.toLowerCase());
    if (!rec)
        return null;
...
function tableRatesFor(model, provider) {
    const { keys, bare } = candidateKeys(model, provider);
    for (const key of keys) {
...
function applyRates(r, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens) {
    const cost = (inputTokens / MTOK) * r.input +
 ...
 */
export function priceCall(model, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens, provider, sharedDir) {
    ...
    if (!resolved)
        return { costUsd: null, costSource: "unpriced" };
    return {
        costUsd: applyRates(resolved, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens),
 ...
 */
export function estimateCost(model, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens, provider, sharedDir) {
    return priceCall(model, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens, provider, sharedDir).costUsd ?? 0;
}
