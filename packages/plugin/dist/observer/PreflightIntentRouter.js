...
import { priorRoleForSurface, DEFAULT_PRIOR_MIN_CONFIDENCE, } from "./routingRule.js";
/**
 ...
 */
const TECHNICAL_NOUNS = "system|architecture|solution|api|schema|database|service|workflow|process|algorithm|protocol|infrastructure|framework|pipeline|deployment";
const TIER1_PATTERNS = Object.freeze([
    {
    ...
    {
        rx: new RegExp(`\\bdesign (a |an |the |my |our |some |this )(${TECHNICAL_NOUNS})\\b`, "i"),
        reason: "regex:design_imperative",
    },
    {
        rx: new RegExp(`\\barchitect (a |an |the |my |our |some |this )(${TECHNICAL_NOUNS})\\b`, "i"),
 ...
 */
const TIER3_PATTERNS = Object.freeze([
    {
 ...
 */
export function _parseHaikuTier(response) {
    if (!response)
        return null;
    ...
    if (hitCount !== 1)
        return null; // 0 → abstain (AMBIGUOUS / garbage); >1 → unclear
    if (hasT1)
        return "tier1";
    if (hasT2)
        return "tier2";
...
function _roleToPreflightTier(role) {
 ...
 * Exported for tests.
 */
export function _parseBotPrior(preflight) {
    ...
    if (typeof rawPrior !== "string")
        return null;
    const role = _TIER_TO_ROLE[rawPrior] ??
        (rawPrior === "fast" || rawPrior === "standard" || rawPrior === "power"
            ? rawPrior
            : undefined);
    if (!role)
        return null;
    const ev = (pf.prior_evidence ?? null);
    // No evidence block → an operator set this by hand. Full confidence.
    const confidence = ev && typeof ev.confidence === "number" ? ev.confidence : 1.0;
    const surfaces = {};
    const rawSurfaces = ev && typeof ev.surfaces === "object" ? ev.surfaces : null;
    if (rawSurfaces) {
        for (const [k, v] of Object.entries(rawSurfaces)) {
            const surfaceRole = typeof v === "string"
                ? (_TIER_TO_ROLE[v] ??
                    (v === "fast" || v === "standard" || v === "power" ? v : undefined))
                : undefined;
            if (surfaceRole)
                surfaces[k.toLowerCase()] = surfaceRole;
        }
    }
    return {
        role,
...
export class PreflightIntentRouter {
     ...
     */
    _botPriorCache = null;
    static _BOT_PRIOR_TTL_MS = 60_000;
    /**
     ...
     */
    _getBotPrior(surface) {
        const now = Date.now();
        let prior = this._botPriorCache?.prior ?? null;
        if (!this._botPriorCache ||
     ...
     * case is +150ms.
     */
    async _classifyWithHaiku(text, botId, start, trigger) {
    ...
    async classify(input) {
        ...
        try {
            ...
            if (this._isHaikuEnabled()) {
                const haikuDecision = await this._classifyWithHaiku(text, input.botId, start, input?.trigger);
                if (haikuDecision)
                    return haikuDecision;
            }
...
//# sourceMappingURL=PreflightIntentRouter.js.map