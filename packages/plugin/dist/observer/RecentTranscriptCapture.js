...
export class RecentTranscriptCapture {
    ...
    recordUserTurn(input) {
        ...
        try {
            ...
            if (code === "EACCES" || code === "EPERM") {
                this.logger.debug(`Evolve: recent-transcripts dir for ${this.config.botId} owned by another gateway, skipping`);
                return;
            }
    ...
    ensureDir(dir) {
        ...
        try {
            ...
            const code = err?.code;
            if (code === "EACCES" || code === "EPERM") {
                this.logger.debug(`Evolve: metrics dir for ${this.config.botId} owned by another gateway, skipping`);
                return false;
            }
 ...
 */
export function applyRetention(entries, nowMs) {
    const cutoff = nowMs - RETENTION_MS;
    const fresh = entries.filter((e) => {
        const t = Date.parse(e.ts);
        return Number.isFinite(t) && t >= cutoff;
    });
    if (fresh.length <= MAX_TURNS)
        return fresh;
    return fresh.slice(fresh.length - MAX_TURNS);
}
//# sourceMappingURL=RecentTranscriptCapture.js.map