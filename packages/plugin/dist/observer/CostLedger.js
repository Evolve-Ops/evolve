 ...
 */
export function inferCacheState(cacheReadTokens, cacheWriteTokens, inputTokens) {
    ...
    if (readRatio >= 0.80)
        return "warm";
    if (cacheWriteTokens > 0 && readRatio <= 0.10)
        return "invalidated";
    return "unknown";
}
 ...
 */
export function inferTriggerKind(ctxTrigger, modelHint) {
    const t = typeof ctxTrigger === "string" ? ctxTrigger.toLowerCase() : "";
    if (t.includes("user") || t.includes("message") || t === "chat")
        return "user_turn";
    if (t.includes("heartbeat") || t.includes("cron") || t.includes("timer"))
        return "heartbeat";
    if (t.includes("summar"))
        return "summarizer";
    if (t.includes("classif") || t.includes("tier"))
        return "classifier";
    if (t.includes("task") && t.includes("extract"))
        return "task_extractor";
    if (t.includes("fallback") || t.includes("retry"))
        return "fallback";
    ...
    //
    // NOTE (2026-08-01): post-#3531 this heuristic is UNRELIABLE — the pin
    // denial means Evolve's subagent calls run on the bot's default model, so
    // a summarizer/classifier call need not be Haiku and a Haiku call may be
    // ordinary tier-routed bot work. This function is not on the live tagging
    // path (cost_event_converter re-derives trigger_kind from the turn
    // record's source, which TurnObserver's llm_output capture sets via
    // classifyEvolveSubagentKey in subagentRun.ts); kept for the legacy
    // plugin-emission schema doc + tests only.
    if (modelHint && modelHint.toLowerCase().includes("haiku"))
        return "classifier";
    return "unknown";
}
 ...
 */
export function buildCostEvent(input, now = new Date()) {
    return {
        ...
        cache_state: inferCacheState(input.cacheReadTokens, input.cacheWriteTokens, input.inputTokens),
...
//# sourceMappingURL=CostLedger.js.map