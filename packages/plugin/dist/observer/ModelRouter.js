 ...
 */
export function isSpendCapActive(sharedDir, botId) {
    try {
 ...
 */
export function splitProviderModelRef(ref) {
...
export class LegacyTierShapeError extends Error {
    constructor(detail) {
        super(`[Evolve ModelRouter] Unmigrated legacy model-tier config: ${detail}. ` +
...
function _legacyTierKeys(tiers) {
 ...
 */
function _assertNoLegacyTierShape(source, where) {
    if (!source || typeof source !== "object")
        return;
    const keys = _legacyTierKeys(source.tiers);
    if (keys.length > 0) {
        throw new LegacyTierShapeError(`${where} carries models.tiers keys [${keys.join(", ")}]`);
    }
}
 ...
 */
export function legacyTiersRefuseConfig(message) {
...
function _normalizeRouting(raw) {
    ...
    if (legacyKeys.length > 0) {
        throw new LegacyTierShapeError(`routing block carries legacy keys [${legacyKeys.join(", ")}]`);
    }
    const toRole = (field, v) => {
        if (v === null)
            return null;
        if (typeof v !== "string")
            return undefined;
        if (_LEGACY_TIER_KEY_RE.test(v)) {
            throw new LegacyTierShapeError(`routing.${field} carries legacy tier value "${v}"`);
        }
        return v;
    };
    return {
        ...
        ambiguousRole: "ambiguousRole" in r ? toRole("ambiguousRole", r.ambiguousRole) : null,
        // Bool-only: any non-bool value (string "true", 1, …) is not a
        // decision to enable content-based downgrades — treat as unset.
        classifierDowngrade: typeof r.classifierDowngrade === "boolean" ? r.classifierDowngrade : undefined,
    };
}
/**
 ...
 */
export function sanitizeDailyCap(raw, fallback) {
 ...
 * enforcement point.
 */
export function defaultRoleCap(role) {
...
export class ModelRouter {
    ...
    static _normalizeConfig(config) {
        ...
        try {
            ...
            if (!roleCaps && typeof c.userTierOverride?.dailyCap === "number") {
                roleCaps = {
                    power: {
                        maxPerDayPerBot: sanitizeDailyCap(c.userTierOverride.dailyCap, defaultRoleCap("power")),
                    },
                };
            }
            return {
     ...
     */
    logger;
    constructor(config, sharedDir = "", botId = "", logger) {
        this.logger = logger;
     ...
     */
    setStandingUserTierDefault(userKey, choice) {
        if (!userKey)
        ...
        try {
            ...
            if (err?.code !== "ENOENT") {
                throw new Error(`cannot read ${prefsPath} (${err?.code ?? err}) — refusing to ` +
        ...
        try {
            ...
            throw new Error(`cannot write ${prefsPath} (${err?.code ?? err})`);
        }
    ...
    getRoleForModel(modelString) {
        ...
        const roleIds = ["fast", "standard", "power", "max"];
        let bestRole = null;
     ...
     * not as a courtesy.
     */
    _sessionAlreadyInRole(sessionKey, role) {
        if (!sessionKey)
            return false;
        if (role === "power")
            return this._tier1ActiveSessions.has(sessionKey);
        if (role === "max")
            return this._lastResolvedRoleWasMax.has(sessionKey);
        return false;
    }
    /**
     ...
     */
    _capDefaultRole(sessionKey, role) {
        let current = role;
        ...
        for (let hop = 0; hop < 4; hop++) {
            if (current !== "power" && current !== "max")
                return current;
            if (this._sessionAlreadyInRole(sessionKey, current))
                return current;
            if (this._roleUsedToday(current) < this._roleCap(current))
                return current;
    ...
    _resolveModelAndTier(sessionKey) {
        ...
        const classifierMaintenanceSuppressed = sessionType === "maintenance" &&
