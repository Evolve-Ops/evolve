...
let _calibratedProductiveKws = [...TIER1_KEYWORDS];
let _calibratedMaintenanceKws = [...TIER2_KEYWORDS];
let _calibratedCorrectionPatterns = [...CORRECTION_PATTERNS];
...
function _applyDelta(base, add, remove) {
 ...
 */
export function loadCalibrationOverrides(sharedDir) {
    ...
    _calibratedProductiveKws = _applyDelta(TIER1_KEYWORDS, cal["productive_keywords_add"], cal["productive_keywords_remove"]);
    _calibratedMaintenanceKws = _applyDelta(TIER2_KEYWORDS, cal["maintenance_keywords_add"], cal["maintenance_keywords_remove"]);
    _calibratedCorrectionPatterns = _applyDelta(CORRECTION_PATTERNS, cal["correction_patterns_add"], cal["correction_patterns_remove"]);
 ...
 */
export function getCalibratedCorrectionPatterns() {
    return _calibratedCorrectionPatterns;
}
export function classifyTierByKeywords(userMessage, assistantMessage, _sessionId, hints) {
    ...
    // ── User-text priority (2026-07-31 incident regression guard) ─────────────
    // `combined` mixes the USER's request with the ASSISTANT's reply. Error
    // vocabulary in the assistant text ("failed to", "error:", …) routinely
    // pollutes a substantive user request into a maintenance/tie verdict —
    // which then sticks on the session via setSessionTypeIfMoreSpecific and
    // downgrades every subsequent turn to the fast rung. The user's own words
    // are the intent signal: when they alone score strictly productive, the
    // class stays productive regardless of the assistant's vocabulary. A user
    // who pastes errors/commands still classifies maintenance — their own
    // text carries the maintenance signal.
    if (maintenanceScore >= productiveScore) {
        const userText = userMessage.toLowerCase();
        const userProductiveHits = allProductiveKws.filter((kw) => userText.includes(kw));
        const userMaintenanceHits = allMaintenanceKws.filter((kw) => userText.includes(kw));
        if (userProductiveHits.length > userMaintenanceHits.length) {
            const confidence = Math.min(max, base + (userProductiveHits.length - userMaintenanceHits.length) * per_signal);
            return { class: "productive", signals: userProductiveHits.slice(0, 5), confidence };
        }
    }
    if (maintenanceScore > productiveScore) {
