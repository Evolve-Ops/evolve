...
export class LLMTierClassifier {
     ...
     *   refused.
     */
    async classify(firstUserMessage, hints, opts) {
        ...
        try {
            ...
            const result = await runPinnedSubagent(this.api, this.logger, {
                ...
                maxTurns: 1,
                // This classifier fixes the session's tier — and so its model — for
                // every later turn, so on an interactive turn the cost-breaker gate
                // must not silence it into the keyword fallback. Background work
                // stays refused.
                interactiveTurn: isInteractiveTrigger(opts?.trigger),
            });
            ...
            const loud = this.logger.error ?? this.logger.warn;
            loud.call(this.logger, `Evolve: LLM tier classification failed, using keyword fallback ` +
