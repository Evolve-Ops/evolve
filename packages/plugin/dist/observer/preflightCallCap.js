 ...
 */
const ADMITTED = { admitted: true, logLine: null };
export class PreflightCallCap {
    /** Ceiling per session per rolling minute. */
    static PER_MINUTE_PER_SESSION = 10;
     ...
     */
    admit(runId, sessionKey, now = Date.now()) {
        ...
        if (hits.length >= PreflightCallCap.PER_MINUTE_PER_SESSION) {
            ...
            return this._verdict(key, now, `${hits.length} classifications in the last minute on this session ` +
    ...
    _verdict(sessionKey, now, why) {
        ...
        return {
            admitted: false,
            logLine: `Evolve: preflight classification capped for session ${sessionKey} — ${why}. ` +
...
//# sourceMappingURL=preflightCallCap.js.map