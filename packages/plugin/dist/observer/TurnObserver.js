...
import { classifyEvolveSubagentKey } from "./subagentRun.js";
...
export class TurnObserver {
        ...
        api.on("before_model_resolve", async (event, ctx) => {
            try {
                ...
                try {
                    ...
                    this.logger.info(`Evolve diag: before_model_resolve fired sessionKey=${String(sessionKey ?? "<none>").slice(0, 12)} ` +
    ...
    async handleBeforeModelResolve(ctx, sessionKey, userMessage) {
        ...
        if (userMessage && !DUE_CHECK_TRIGGERS.includes(_trigger) && this._isPreflightEnabled()) {
            try {
