 ...
 */
export function _buildEvolveSubagentTurn(kind, event) {
    ...
    return {
        ...
        costEstimated: estimateCost(model, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens, provider),
    };
}
/**
 ...
 * Prices must stay in sync with the offline tables in
 * packages/analyzer/turn_cost.py (OFFLINE_MODEL_PRICING /
 * OFFLINE_PROVIDER_PRICING) — the pod's one per-turn cost rule.
 *
 * Coverage matters more than precision here: until 2026-08 this table held
 * only the Anthropic families, so every non-Anthropic turn (grok, gpt,
 * gemini) was priced $0 — which fed a silent zero into the session-budget
 * breaker, the runaway-rate cap, and the annotation's cost_estimated. An
 * unmeasured turn must never present as free.
 */
const MODEL_COSTS = {
    // Anthropic. claude-3-haiku is significantly cheaper than claude-haiku-4-x;
    // must appear before the "haiku" fallback so the substring match hits the
    // right entry.
    ...
    // OpenAI — no cache-write price difference; cacheRead is the discounted rate.
    "gpt-4o-mini": { input: 0.15, output: 0.60, cacheWrite: 0.00, cacheRead: 0.075 },
    "gpt-4o": { input: 2.50, output: 10.00, cacheWrite: 0.00, cacheRead: 1.25 },
    "gpt-4.1-nano": { input: 0.10, output: 0.40, cacheWrite: 0.00, cacheRead: 0.05 },
    "gpt-4.1-mini": { input: 0.40, output: 1.60, cacheWrite: 0.00, cacheRead: 0.20 },
    "gpt-4.1": { input: 2.00, output: 8.00, cacheWrite: 0.00, cacheRead: 1.00 },
    "o4-mini": { input: 1.10, output: 4.40, cacheWrite: 0.00, cacheRead: 0.275 },
    // Google
    "gemini-2.0-flash-lite": { input: 0.075, output: 0.30, cacheWrite: 0.00, cacheRead: 0.00 },
    "gemini-2.0-flash": { input: 0.10, output: 0.40, cacheWrite: 0.00, cacheRead: 0.00 },
    "gemini-1.5-pro": { input: 1.25, output: 5.00, cacheWrite: 0.00, cacheRead: 0.00 },
    "gemini": { input: 1.25, output: 10.00, cacheWrite: 0.00, cacheRead: 0.00 }, // pro-tier family default
    // xAI
    "grok-4-1-fast": { input: 5.00, output: 25.00, cacheWrite: 0.00, cacheRead: 0.00 },
    "grok-3-mini": { input: 0.30, output: 0.50, cacheWrite: 0.00, cacheRead: 0.00 },
    "grok": { input: 3.00, output: 15.00, cacheWrite: 0.00, cacheRead: 0.00 }, // grok-3 / grok-4 list price
};
/**
 * Provider-level fallback when no model-family key matches — a known
 * provider's unknown model must not be priced $0. Mirrors
 * OFFLINE_PROVIDER_PRICING in packages/analyzer/turn_cost.py (mid-range
 * list prices, keep in sync).
 */
const PROVIDER_COSTS = {
    anthropic: { input: 3.00, output: 15.00, cacheWrite: 3.75, cacheRead: 0.30 },
    openai: { input: 2.50, output: 10.00, cacheWrite: 0.00, cacheRead: 1.25 },
    google: { input: 1.25, output: 5.00, cacheWrite: 0.00, cacheRead: 0.00 },
    xai: { input: 3.00, output: 15.00, cacheWrite: 0.00, cacheRead: 0.00 },
    mistral: { input: 2.00, output: 6.00, cacheWrite: 0.00, cacheRead: 0.00 },
    groq: { input: 0.59, output: 0.79, cacheWrite: 0.00, cacheRead: 0.00 },
};
/**
 * Token counts → estimated USD. Resolution: model-family substring match,
 * then provider-level fallback (explicit `provider` arg, else the model's
 * `provider/...` prefix), then 0. A 0 from the no-match tail means "can't
 * price", not "free" — the Python analyzer (turn_cost.py) re-estimates
 * zero-cost turns and counts the truly unpriceable ones instead of summing
 * them, so the honest surfacing happens downstream; this estimator's job
 * is coverage on the hot path. Exported for tests.
 */
export function estimateCost(model, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens, provider) {
    const lower = (model || "").toLowerCase();
    const key = Object.keys(MODEL_COSTS).find((k) => lower.includes(k));
    let p = key ? MODEL_COSTS[key] : undefined;
    if (!p) {
        const prov = (provider || (lower.includes("/") ? lower.split("/")[0] : ""))
            .toLowerCase().trim();
        p = PROVIDER_COSTS[prov];
    }
    if (!p)
        return 0;
...
export class TurnObserver {
        ...
        api.on("llm_output", async (event, ctx) => {
            try {
                ...
                try {
                    ...
                    const callCost = estimateCost(String(model || ""), Number(usage?.input ?? 0), Number(usage?.output ?? 0), Number(usage?.cacheWrite ?? 0), Number(usage?.cacheRead ?? 0), typeof event?.provider === "string" ? event.provider : null);
    ...
    async handleTurn(event, ctx) {
        ...
        const costEstimated = estimateCost(modelSelected, llm?.inputTokens ?? 0, llm?.outputTokens ?? 0, llm?.cacheWriteTokens ?? 0, llm?.cacheReadTokens ?? 0, llm?.provider ?? null);
    ...
    writeTurnToShared(sessionId, llm, costEstimated = 0, ctx, appAttribution) {
