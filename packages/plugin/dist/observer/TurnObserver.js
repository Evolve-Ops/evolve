...
import { recordExplicit as recordExplicitAppAttribution, resolveForTurn as resolveAppAttributionForTurn, } from "../apps/AppAttribution.js";
...
import { resolveSpeakerRole, buildSpeakerContextBlock, renderDirectoryDigestBlock } from "../util/roleResolver.js";
import { adminDaemonSocketPath, adminSocketRequest } from "../util/adminSocket.js";
import { classifyTierByKeywords, loadCalibrationOverrides, getCalibratedCorrectionPatterns, } from "./TierClassifier.js";
import { SessionSummarizer } from "./SessionSummarizer.js";
import { classifyEvolveSubagentKey, isEvolveInternalPrompt, } from "./subagentRun.js";
import { PreflightCallCap } from "./preflightCallCap.js";
import { classifySessionKind } from "../tools/ToolProfiles.js";
import { estimateCost, priceCall, EXACT_MODEL_COSTS, _resetPricingCatalogCache, } from "./ModelPricing.js";
...
import { PreflightIntentRouter } from "./PreflightIntentRouter.js";
import { decideRoutingRule, triggerKindToSessionClass, } from "./routingRule.js";
import { ClassifierGateReader, shouldSampleSession } from "./classifierGate.js";
import { buildJudgeTailRequest, parseStructuredTail, hasTailMarker, } from "./structuredTail.js";
import { SessionStruggleAggregator, } from "./SessionStruggleAggregator.js";
import { SessionStruggleJudge, shouldRunJudge, } from "./SessionStruggleJudge.js";
import { ModelRouter, synthesizeRungsRoles, normalizeRouting, mergeModelCatalog, parseTierDirective, defaultRoleCap, sanitizeDailyCap, splitProviderModelRef, LegacyTierShapeError, legacyTiersRefuseConfig, } from "./ModelRouter.js";
import { HOUSEKEEPING_TOOLS_ALLOW, HousekeepingLeverReader, housekeepingKindForTrigger, resolveHousekeepingModel, } from "./housekeeping.js";
import { computeStruggle } from "./StruggleDetector.js";
import { OutwardActionLedger } from "./OutwardActionLedger.js";
import { PrefixHashLedger } from "./PrefixHashLedger.js";
import { HeartbeatDueCheck, buildWakeContext, DUE_CHECK_TRIGGERS, } from "./HeartbeatDueCheck.js";
import { buildDecisionRecord, appendDecision } from "./HeartbeatSkipLedger.js";
import { StickyBlockCache, NarrativeStableCache, composeEvolvePrefix } from "./BlockStability.js";
import { computePushback } from "./PushbackDetector.js";
import { CascadeTelemetry } from "./CascadeTelemetry.js";
import { detectDangerousCombo } from "./DangerousComboDetector.js";
import { shouldBeInHoldout } from "./HoldoutCohort.js";
import { computeTriviality } from "./TrivialityDetector.js";
import { CascadeController, DEFAULT_CASCADE_CONFIG, } from "./CascadeController.js";
...
import { EvoDispatchClient, evoFailureUserMessage, } from "../better/EvoDispatchClient.js";
import { RecommendationFormatter, } from "../better/RecommendationFormatter.js";
import { KeywordHandler } from "../better/KeywordHandler.js";
import { readCostBreakerDecision, } from "../breakers/BreakerStateReader.js";
import { classifyCheckpointAnswer, readCostCheckpoint, renderAnswerUnavailableMessage, renderCapReachedMessage, renderContinuedMessage, renderDeclinedMessage, renderNotOwnerMessage, } from "../breakers/CostCheckpoint.js";
import { inTurnCheckpoint, withInTurnOwnerNotice } from "../breakers/InTurnCheckpoint.js";
import { PLUGIN_VERSION, recordInteractiveHold } from "../breakers/HoldMarker.js";
import { isAutoSource, shouldRetagHeartbeatSource } from "../breakers/sourceClassifier.js";
import { SessionCostMonitor, readSessionBudgetBreaker, readSessionBudgetCap, } from "./SessionCostMonitor.js";
import { fileURLToPath } from "url";
import { parseProtocol, isKnownProtocol, } from "./triggerProtocols.js";
...
export function evolvePythonBin() {
    try {
 ...
 */
export function resolveAnalyzerDir(config) {
 ...
 */
function inferTriggerKind(source, channel, ctxTrigger) {
    const src = (source ?? "").toLowerCase();
    const ch = (channel ?? "").toLowerCase();
    const trg = (ctxTrigger ?? "").toLowerCase();
    if (src === "human" || src === "user")
        return "user_turn";
    if (src === "heartbeat")
        return "heartbeat";
    if (src === "cron")
    ...
    if (src === "summarizer" || src === "classifier" ||
        src === "task_extractor" || src === "fallback")
        return src;
    // OC housekeeping — the pre-compaction memory flush (ctx.trigger "memory")
    // and compaction. Its own kind, never "unknown": the receipt reports it on
    // a separate line and the routing rule must not anchor the conversation's
    // session class from it (triggerKindToSessionClass → null).
    const hk = housekeepingKindForTrigger(src) ?? housekeepingKindForTrigger(trg);
    if (hk)
        return hk;
    // Channel-based fallback for OC versions that don't populate source.
    if (ch === "heartbeat")
        return "heartbeat";
    if (ch === "cron-event" || ch === "cron")
        return "cron_app";
    if (ch === "subagent" || ch === "exec-event")
        return "subagent";
 ...
 */
export function _triggerKindToSessionClass(triggerKind) {
    // Delegates to routingRule so the hot path, the cascade input and the
    // replay test cannot drift apart. Kept as an export because callers
    // and tests already import this name.
    return triggerKindToSessionClass(triggerKind);
}
/**
 ...
 */
export function _buildEvolveSubagentTurn(kind, event, sharedDir) {
    ...
    const priced = priceCall(model, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens, provider, sharedDir);
    return {
        llm: {
            model,
            provider,
        ...
        costEstimated: priced.costUsd ?? 0,
        priced,
    };
}
/**
 ...
 */
export function _computeChosenBy(routerDriver, userTierForChosenBy, userModelOverride, cascadeEnabled, modelTier) {
    if (routerDriver === "runaway" || routerDriver === "spend_cap") {
        return "spend_cap";
    }
    if (routerDriver === "role_cap") {
        // A per-role daily cap degraded the operator/user default. Kept
        // separate from "spend_cap" (the cost breaker) so calibration can tell
        // a tier budget from a money budget.
        return "role_cap";
    }
    ...
    if (routerDriver === "user_request")
        return "user_request";
    if (routerDriver === "cascade" && cascadeEnabled)
        return "cascade";
    if (routerDriver === "preflight")
        return "preflight";
    if (routerDriver === "classifier")
        return "classifier";
 ...
 *
 * The USER-facing half of the attribution is the rewritten banner
 * (_buildRoutingNotice, delivered via reply_payload_sending). This note is the
 * BOT-facing half: a bot cannot observe its own routing, so without it a
 * follow-up question ("why are you on Haiku?") is answered by confabulation
 * even though the chat now carries the true reason.
 *
 * Pure function — no I/O, no module state — for testability.
 */
export function _buildCostDowngradeNotice(driver, model) {
    const cause = driver === "runaway"
        ? "this session tripped the runaway-rate cost cap"
        : "this bot reached its daily spending cap";
    return (`[EVOLVE COST DOWNGRADE] Evolve intentionally routed this turn to ${model} ` +
        `because ${cause} (cost breaker). This is a policy downgrade, NOT a provider ` +
        `outage — the configured model is available; Evolve overrode it to stop spend. ` +
        `If the user asks about the model change, attribute it to the cost cap (daily ` +
        `caps reset at midnight; the pod operator can clear the breaker early). Do not ` +
        `speculate about provider availability.`);
}
/**
 * System-prompt note injected on SCHEDULED turns (heartbeat / cron) only.
 *
 * Background turns run on the cheap rung by design (routingRule sends
 * heartbeat / cron_app to `background`), and the cheap rung is measurably
 * worse at OC's stricter tool contracts. On one team bot's logs (2026-07-28 →
 * 2026-09-09) the same three refusals account for the bulk of ~1.4k failed
 * tool calls, and each one costs a full extra model round-trip to discover:
 *
 *   - `exec preflight: complex interpreter invocation detected` — a
 *     heredoc / `python -c` / shell-pipeline invocation, refused before it
 *     runs;
 *   - `EISDIR: illegal operation on a directory, read` — `read` aimed at a
 *     directory rather than a file;
 *   - `Explicit message target required for this run` / `Unknown channel:
 *     <id>` — a `message` call carrying a guessed or stale channel id.
 *
 * Each of those is a contract the model can satisfy on the FIRST call if it
 * knows the rule up front, which is what this note supplies. It is scoped to
 * scheduled sessions deliberately: user turns run on a stronger rung, already
 * get several other per-turn blocks, and should not pay these bytes.
 *
 * This states the tool CONTRACTS, not OC's error strings — an upstream
 * reword must not silently turn this into stale advice. It is guidance, not
 * enforcement: the tools still refuse on their own terms if the model
 * ignores it.
 *
 * Pure function — no I/O, no module state — for testability.
 */
export function _buildScheduledToolDisciplineNote() {
    return (`[EVOLVE BACKGROUND TURN] No one is waiting on this turn, so spend calls ` +
        `carefully — three tool contracts reject a first attempt often enough to ` +
        `be worth stating up front. exec: run a script FILE directly ` +
        `(\`python path/to/script.py\`); an inline \`-c\`, a heredoc, or a shell ` +
        `pipeline is refused by preflight before it runs, so write the file first ` +
        `and then run it. read: the path must be a FILE — list a directory to see ` +
        `what is in it, do not read the directory itself. message: pass an ` +
        `explicit target, and only a channel or conversation id you have actually ` +
        `seen this session — a remembered or guessed id fails as an unknown ` +
        `channel. If a tool refuses, read what it asked for and change the call ` +
        `shape; do not re-send the same one.`);
}
/**
 * The user-facing replacement for OC's fallback banner.
 *
 * OC labels ANY divergence between the session's configured model and the one
 * actually used a "Model Fallback", and builds the reason exclusively from
 * provider-failure attempts. An Evolve `before_model_resolve` override has
 * zero attempts, so the banner falls through to the literal "selected model
 * unavailable" — it reports a provider outage that did not happen, on a turn
 * whose model Evolve chose on purpose. `mergeBeforeModelResolve` keeps only
 * `{modelOverride, providerOverride}`, so there is no reason/label field to
 * correct it through; the payload itself is the only seam we own.
 *
 * So we replace the banner rather than annotate it. Same slot, same cadence
 * (OC emits one per fallback TRANSITION, not per turn), but it states what
 * actually happened in one line — no failure, no unavailability, just which
 * model is in use and why.
 *
 * Pure function — no I/O, no module state — for testability.
 */
export function _buildRoutingNotice(driver, model, role, degradedFrom) {
    const tier = role ? `the ${role} tier` : null;
    let cause;
    switch (driver) {
        case "role_cap":
            // The one driver that needs TWO roles to be legible: the tier whose
            // budget ran out is the interesting half, and it is not the tier the
            // turn ended up on.
            cause = degradedFrom
                ? `this bot's daily ${degradedFrom}-tier limit is used up` +
                    (tier ? `, so it is on ${tier}` : "")
                : "this bot's daily limit for its usual tier is used up";
            break;
        case "spend_cap":
            cause = "Evolve switched models: this bot reached its daily spending cap";
            break;
        case "runaway":
            cause = "Evolve switched models: this session tripped its runaway-rate cost cap";
            break;
        case "user_request":
            cause = tier ? `you asked for ${tier}` : "you asked for this model";
            break;
        case "user_default":
            cause = tier ? `your default is ${tier}` : "it is your default";
            break;
        case "operator_default":
            cause = tier ? `this bot defaults to ${tier}` : "it is this bot's default";
            break;
        default:
            cause = tier ? `Evolve routed this session to ${tier}` : "routed by Evolve";
    }
    return `↪️ Model: ${model} — ${cause}.`;
}
/**
 * The ACTIVE model ref out of an OC fallback banner —
 * ``↪️ Model Fallback: <active> (selected <configured>; <reason>)``.
 *
 * Used as the rewrite's safety interlock, not for display: the banner may only
 * be replaced when the model OC is reporting is the one Evolve actually asked
 * for. If OC walked its own fallback chain PAST our override, a provider
 * genuinely failed and the banner is telling the truth — see
 * _routingNoticeApplies.
 *
 * Returns null when the text does not parse, which fails toward leaving OC's
 * banner alone.
 */
export function _fallbackNoticeActiveModel(text) {
    const m = /Model Fallback:\s*(.+?)\s*\(selected\b/i.exec(text);
    const active = m ? m[1].trim() : "";
    return active === "" ? null : active;
}
/**
 * True when `active` (as OC rendered it) names the same model Evolve
 * overrode to.
 *
 * ModelRouter emits `provider/model` refs, and OC re-renders the pair through
 * formatProviderModelRef, so the common case is an exact match. The
 * bare-model comparison covers a rung configured without its provider prefix,
 * which splitProviderModelRef would have paired with the lane's own provider.
 */
export function _sameModelRef(active, override) {
    const a = active.toLowerCase();
    const o = override.toLowerCase();
    if (a === o)
        return true;
    const aBare = a.slice(a.lastIndexOf("/") + 1);
    const oBare = o.slice(o.lastIndexOf("/") + 1);
    return aBare !== "" && aBare === oBare;
}
/**
 * True when an outbound reply payload is OC's own model-fallback banner.
 *
 * OC pushes the banner as its OWN payload (``buildFallbackNotice`` →
 * ``markReplyPayloadForSourceSuppressionDelivery({text, isFallbackNotice:
 * true})``, prepended to the reply array in agent-runner.runtime), so it
 * arrives at reply_payload_sending as a separate, identifiable payload —
 * which is why the trailer can attach to the banner instead of to the bot's
 * answer. The flag is the primary test; the text prefix is a fallback for a
 * gateway that stops setting it (the emoji is optional for the same reason
 * ExecFailureAbsorber's patterns treat VS16 as optional).
 */
export function _isFallbackNoticePayload(payload) {
    if (!payload)
        return false;
    if (payload.isFallbackNotice === true)
        return true;
    const text = typeof payload.text === "string" ? payload.text : "";
    return /^\s*(?:\u21AA\uFE0F?\s*)?Model Fallback:/i.test(text);
}
// ── Cost estimation ───────────────────────────────────────────────────────────
//
// The rule itself lives in ModelPricing.ts: price by EXACT model id from the
// pod's mirrored catalog, then an exact-id offline table, then say you can't
// (finding-cost-forensics-power-bot-2026-09-04 §1 — the family-substring
// table this replaces priced a current-generation id at the previous
// generation's rates and overstated the power model ~3x). Re-exported here
// because ``estimateCost`` is this module's long-standing public name.
export { estimateCost, priceCall, EXACT_MODEL_COSTS, _resetPricingCatalogCache };
 ...
 */
export function _shouldCaptureStruggleSample(event, signal) {
    if (!event || !signal)
        return false;
 ...
 */
export function _sanitizeMessagesForShape(messages) {
    ...
    const sample = messages.slice(startIdx).map((m, i) => {
        ...
        if (Array.isArray(content)) {
            ...
            summary.blocks = content.slice(0, STRUGGLE_SAMPLE_MAX_BLOCKS).map((b) => {
                ...
                if (typeof block?.is_error === "boolean")
                    blk.is_error = block.is_error;
                if (typeof block?.name === "string")
                    blk.has_name = true;
                if (typeof block?.tool_use_id === "string")
                    blk.has_tool_use_id = true;
                if (typeof block?.tool_call_id === "string")
                    blk.has_tool_call_id = true;
                ...
                if (Array.isArray(block?.tool_calls)) {
                    blk.tool_calls_count = block.tool_calls.length;
                }
                if (block?.function_call)
                    blk.has_function_call = true;
                if (typeof block?.text === "string") {
                    blk.text_len = block.text.length;
                }
                ...
                if (input && typeof input === "object" && !Array.isArray(input)) {
                    blk.input_keys = Object.keys(input).slice(0, 20);
                }
                return blk;
            });
            if (blocksTruncated)
                summary.blocksTruncated = true;
        }
        else if (typeof content === "string") {
            summary.contentType = "string";
            summary.text_len = content.length;
        }
        ...
        if (Array.isArray(msg?.tool_calls)) {
            summary.top_tool_calls_count = msg.tool_calls.length;
        }
        if (typeof msg?.tool_call_id === "string") {
            summary.top_has_tool_call_id = true;
        }
        if (typeof msg?.name === "string")
            summary.top_has_name = true;
        return summary;
    });
 ...
 */
const INACTIVE_TRIGGER_STATUSES = new Set([
    "paused",
    "hidden",
    "dormant",
    "deprecated",
 ...
 */
export function _manifestStatusAllowsTriggers(manifest) {
 ...
 */
export function _layerCAppId(manifest) {
...
export class TurnObserver {
     ...
     */
    _heartbeatTriggeredSessions = new Set();
    /**
     * The model-free "is anything due" check for heartbeat / cron triggers
     * (D-OH3). Always constructed; it self-disables per bot by returning
     * ``unavailable`` when the bot has no ``HEARTBEAT.json``, which is every
     * bot until an operator writes one.
     */
    heartbeatDueCheck;
    /**
     * Runs whose heartbeat/cron turn the due-check let through, and the items
     * that let it through. Read by ``before_prompt_build`` so the woken turn
     * carries ONLY what is due (the context reduction, not just a nicety).
     * Entries are dropped on read; the FIFO cap bounds the map if a run never
     * reaches prompt build.
     */
    _dueWakeByRun = new Map();
    /**
     * True once the "no workspaceDir on the hook context" warning has been
     * emitted. One line per process — the condition is a gateway-shape
     * regression, not a per-tick event.
     */
    _loggedMissingHeartbeatWorkspaceDir = false;
    static _DUE_WAKE_MAX_RUNS = 64;
    summarizer;
    modelRouter;
    /**
     ...
     * Since D-OH2 its LLM layer is off by default and its strongest
     * layer is the per-bot prior the nightly analyzer job learns offline
     * — so on a default pod this is a regex scan and a cached file read,
     * not a model call.
     */
    preflightRouter;
    /**
     * The one switch every Evolve-owned model call reads (D-OH2/D-OH5).
     * Default OFF, fails CLOSED, TTL-cached. See classifierGate.ts.
     */
    classifierGate;
    /**
     * Sessions for which a struggle judgement is wanted from the PRIMARY
     * model's own next turn — a structured tail it appends and we strip —
     * rather than from a second model call (D-OH4). Set at agent_end when
     * the free struggle thresholds trip on a sampled session; consumed by
     * the next before_model_resolve, which appends the request.
     */
    _pendingJudgeTail = new Set();
    /**
     * Sessions whose CURRENT turn carries a tail request, so agent_end
     * knows to look for one. Separate from the pending set because the
     * request is consumed at before_model_resolve and the answer arrives
     * a hook later.
     */
    _armedJudgeTail = new Set();
    /**
     ...
     */
    _sessionPreflightDecisions = new Map();
    /**
     ...
     */
    _preflightEnabled = null;
    _preflightEnabledCheckedAt = 0;
    static _PREFLIGHT_CACHE_TTL_MS = 60_000;
    recentTranscript;
    /**
     ...
     */
    _pushbackEnabled = null;
    _pushbackEnabledCheckedAt = 0;
    static _PUSHBACK_DNT_CACHE_TTL_MS = 60_000;
    /**
     ...
     * fix established.
     */
    _getLastAssistantText(sessionKey) {
    ...
    /**
     * Resolve the surface (OC channel) a turn arrived on, lowercased.
     * Null when nothing in ctx names one — the learned prior then falls
     * back to its bot-wide value rather than guessing at a surface.
     */
    _surfaceForCtx(ctx) {
        const raw = ctx?.channel ?? ctx?.channelKind ?? ctx?.channel_id ?? ctx?.channelId ?? null;
        const s = typeof raw === "string" ? raw.trim().toLowerCase() : "";
        return s ? s : null;
    }
    /**
     * Merge the structured judge-tail request into a before_model_resolve
     * result (D-OH4).
     *
     * A struggle judgement was wanted for this session. Instead of a
     * second model call over a snippet of the conversation, we ask the
     * model that is about to answer this turn — it already has the whole
     * conversation in context — for one word at the end of its reply. The
     * word is stripped before the user sees it
     * (``handleReplyPayloadSending``) and read at ``agent_end``.
     *
     * Called at every return point of the before_model_resolve hook, so
     * the request rides whichever branch this turn took. The pending flag
     * is CONSUMED here: one armed turn per trip of the thresholds, so a
     * long struggling session does not carry the instruction forever.
     *
     * Appends to any systemAppend already present rather than replacing
     * it — a keyword injection and a judge request are both wanted.
     */
    _withJudgeTail(ctx, result) {
        const out = {};
        ...
        if (!sessionId || !this._pendingJudgeTail.delete(sessionId))
            return out;
        if (this._armedJudgeTail.size >= 512) {
            const oldest = this._armedJudgeTail.values().next().value;
            if (oldest !== undefined)
                this._armedJudgeTail.delete(oldest);
        }
        this._armedJudgeTail.add(sessionId);
        const request = buildJudgeTailRequest();
        out.systemAppend = out.systemAppend ? `${out.systemAppend}\n\n${request}` : request;
        this.logger.debug(`Evolve judge: asking session ${sessionId.slice(0, 8)}'s own turn for a ` +
            `verdict (structured tail) — no second model call`);
        return out;
    }
    /**
     * Run the routing rule for one turn's ctx.
     *
     * Two callers, deliberately the same function: ``resolveModelRouting``
     * (before the model runs, to anchor the session) and ``handleTurn``
     * (at agent_end, to give the cascade controller a rule-derived
     * intent instead of a classifier-derived one — D-OH2 item 5). If
     * these two ever disagreed, the cascade would be shadow-grading a
     * decision nobody made.
     *
     * The operator's configured maintenance/background roles are passed
     * through so the rule reproduces ``_resolveModelAndTier``'s step 5
     * exactly, including a pod that has moved maintenance off ``fast``.
     */
    decideRuleForCtx(ctx, triggerKind) {
        let backgroundRole = null;
        let maintenanceRole = null;
        try {
            const routing = this.modelRouter.getRoutingConfig();
            backgroundRole = routing?.backgroundRole ?? null;
            maintenanceRole = routing?.maintenanceRole ?? null;
        }
        catch {
            // Router not configured (legacy pod) — the rule's own `fast`
            // default is the same one _resolveModelAndTier would apply.
        }
        return decideRoutingRule({
            triggerKind,
            surface: this._surfaceForCtx(ctx),
            // The prior routes through the preflight slot, which
            // PreflightIntentRouter already owns and which ran earlier in
            // this same hook. Passing null here keeps ONE consumer of the
            // prior — this call is about the trigger anchor and the cascade
            // intent, and a second reader could disagree with the first.
            prior: null,
            backgroundRole,
            maintenanceRole,
        });
     ...
     */
    async _fireJudgeAsync(sessionId, triggeredBy, turns) {
     ...
     *
     * Fail-open is correct HERE and only here: with the classifier gate
     * closed (the default since D-OH2) the router costs a regex scan and
     * a TTL-cached file read, so an unreadable network.json leaving it
     * enabled spends nothing. The switch that guards actual model calls
     * is ``classifierGate``, and that one fails closed.
     */
    _isPreflightEnabled() {
        const now = Date.now();
        if (this._preflightEnabled !== null &&
     ...
     */
    _sessionHoldoutAssignment = new Map();
    _isHoldoutSession(sessionId) {
     ...
     */
    _pendingKeywordInjection = new Map();
    /**
     ...
     */
    _evoHandledRuns = new Set();
    /**
     * Ceiling on preflight classifications (one per runId, ten per session per
     * minute). The self-call guard on before_model_resolve stops the loop we
     * diagnosed; this stops the one we have not met, because it counts calls
     * instead of trying to recognise them. See preflightCallCap.ts.
     */
    _preflightCap = new PreflightCallCap();
    /**
     ...
     */
    _routedRunIds = new Set();
    /**
     * Housekeeping runs (OC's pre-compaction memory flush) this plugin routed
     * to the cheap rung, keyed by runId → kind. Read by before_prompt_build
     * (tool allow-list, no injections) and handleTurn (tagged row, nothing
     * else). A repeat before_model_resolve for a runId already here is OC's
     * failover walk — we stand down, same as _routedRunIds. FIFO at 1024.
     */
    _housekeepingRuns = new Map();
    _housekeepingLever = null;
    ...
    _directSentRuns = new Map();
    /**
     * ``<trip_id>|<conversation>`` pairs this bot has already answered once
     * while a cost breaker was tripped.
     *
     * The breaker's refusal is news exactly once per trip per conversation.
     * Before this set existed the hold re-emitted its message on EVERY
     * blocked attempt: on 2026-09-07 a bot's Slack workspace filled with
     * "Your message could not be sent: Background activity paused…" every
     * few minutes, sent to people who are not the operator and can do
     * nothing about it. Keyed on the trip id (not the bot) so a genuinely
     * new trip speaks again, and cleared wholesale the moment no breaker is
     * tripped — reactivation resets the budget of one message per
     * conversation along with everything else.
     */
    _breakerSpokenConversations = new Set();
    /** Bound on the set above. A bot with thousands of conversations under
     *  one trip would otherwise grow it without limit; at the cap the
     *  oldest entries are dropped, which can only cause a repeat message,
     *  never a wrong one. */
    static _BREAKER_SPOKEN_CAP = 512;
    /** Last time a background refusal was logged (ms). Per minute, not per
     *  attempt — see ``_breakerReplyClaim``. */
    _lastBreakerRefusalLogMs = 0;
    /**
     ...
     */
    _llmEchoRuns = new Map();
    /**
     * Runs whose model Evolve overrode, and what drove the choice. Written by
     * resolveModelRouting; read by reply_payload_sending (to replace OC's
     * fallback banner with the true reason) and by before_prompt_build (which
     * takes only the cost-safety-net entries, to brief the bot).
     *
     ...
     * differs from the session's configured model. The reason comes exclusively
     * from provider-failure attempts; a hook-driven override has zero attempts,
     * so the banner falls through to a literal claim of provider outage on a
     * turn where nothing failed. `driver` is what turns that into a true
     * sentence, and `model` is what proves the banner is describing OUR
     * override rather than a real fallback that walked past it.
     *
     ...
     */
    _evolveRoutedRuns = new Map();
    /**
     * Runs whose fallback banner has already been rewritten.
     * reply_payload_sending fires once per PAYLOAD and a turn can emit several,
     * so this keeps one run to one rewrite. Bounded at 1024 with FIFO eviction,
     * same as the map above.
     */
    _routingNoticeRuns = new Set();
    /**
     ...
     */
    _capBlock; // constructed in constructor (statics declared below)
    /** Dedupes concurrent capability-block computes (one in-flight at a time). */
    _capBlockInflight = null;
    /**
     ...
     */
    _lastSpeakerBlockBySession = new Map();
    /**
    ...
    constructor(config, logger, api) {
        ...
        this.modelRouter = new ModelRouter(this.loadModelRouterConfig(), config.sharedDir, config.botId, logger);
        ...
        this.classifierGate = new ClassifierGateReader(config.sharedDir, config.botId);
        ...
        if (this.isCascadeTelemetryEnabled() && config.botId && config.botId !== "unknown") {
            this.cascadeTelemetry = new CascadeTelemetry({ sharedDir: config.sharedDir, botId: config.botId }, logger);
        }
        // Outward-action ledger (autonomy ladder Phase B). See the field
        // docstring for why there is no kill-switch.
        if (config.botId && config.botId !== "unknown") {
            this.outwardActionLedger = new OutwardActionLedger({ sharedDir: config.sharedDir, botId: config.botId }, logger);
        ...
        // Heartbeat / cron due-check (D-OH3). No config knob by design: the
        // presence of a valid HEARTBEAT.json beside the bot's HEARTBEAT.md IS
        // the opt-in, and its absence keeps today's behaviour exactly.
        this.heartbeatDueCheck = new HeartbeatDueCheck({
            botId: config.botId,
            sharedDir: config.sharedDir,
            logger: {
        ...
        this._capBlock = new StickyBlockCache(TurnObserver._CAP_BLOCK_TTL_MS, TurnObserver._BLOCK_MAX_STALE_MS);
        this._dirDigestBlock = new StickyBlockCache(TurnObserver._DIR_DIGEST_TTL_MS, TurnObserver._BLOCK_MAX_STALE_MS);
        ...
        // The cross-vendor resolver (design judge-role-collapse §5.3) lets the
        // judge run on a provider other than the one producing the judged work;
        // closure over this.modelRouter so config reloads stay visible.
        this.sessionJudge = new SessionStruggleJudge(config, logger, api, (againstRole) => this.modelRouter.resolveCrossVendor(againstRole));
    ...
    _emitSessionBudgetSignal(rec) {
        this.logger.warn(`Evolve session-budget breaker emit bot=${rec.bot_id} `
    ...
    _extractSenderExternalId(ctx, surface) {
        if (surface === "member_bot") {
    ...
    register(api) {
        ...
        api.on("llm_output", async (event, ctx) => {
            try {
                ...
                if (evolveKind) {
                    const built = _buildEvolveSubagentTurn(evolveKind, event, this.config.sharedDir);
                    if (built) {
                        // ctx deliberately NOT passed: the enrichment path would read
                        // the session key's trailing Date.now() as a channel id.
                        this.writeTurnToShared(String(sessionId), built.llm, built.priced);
                        this.logger.debug(`Evolve: recorded ${evolveKind} subagent call ` +
                ...
                this.sessionLlmData.set(sessionId, {
                    ...
                    // Housekeeping runs carry their canonical kind ("memory_flush"),
                    // not OC's raw "memory" — the receipt and cost_event key on it.
                    source: housekeepingKindForTrigger(ctx?.trigger) ?? ctx?.trigger ?? existing?.source ?? "unknown",
                ...
                try {
                    ...
                    const callCost = estimateCost(String(model || ""), Number(usage?.input ?? 0), Number(usage?.output ?? 0), Number(usage?.cacheWrite ?? 0), Number(usage?.cacheRead ?? 0), typeof event?.provider === "string" ? event.provider : null, this.config.sharedDir);
                    // In-turn checkpoint (D-CS12): the same per-call cost, keyed by
                    // run — ToolCallGate vetoes tool calls once the run crosses it.
                    inTurnCheckpoint.recordCost(event?.runId ?? ctx?.runId, String(sessionId), callCost);
                    ...
                    if (result.trippedThisCall) {
                        this.logger.info(`Evolve session-budget breaker TRIPPED bot=${this.config.botId} `
                    ...
                    this.logger.warn(`Evolve session-budget monitor error (continuing): ${innerErr}`);
                }
            }
            catch (err) {
        ...
        api.on("agent_end", async (event, ctx) => {
            try {
                inTurnCheckpoint.endRun(ctx?.runId ?? event?.runId);
                ...
                if (sid && !this.sessionLlmData.has(sid)) {
                    ...
                    for (let waited = 0; waited < 500; waited += 50) {
                        ...
                        if (this.sessionLlmData.has(sid))
                            break;
                    }
                }
        ...
        //
        // REGISTERED UNCONDITIONALLY — on every tier, capabilities or not.
        //
        // This hook is the ONLY seam that can refuse a turn quietly:
        // ``before_agent_run``'s ``block`` decision is fed to OC's
        // ``resolveBlockMessage``, which always produces channel text (with no
        // ``message`` field it still emits "Your message could not be sent:
        // blocked by evolve"). So while this registration sat inside the
        // capability gate below, a pod tier with ``injectPodConduct`` and
        // ``injectKeywords`` both off had no silent path at all: a tripped cost
        // breaker fell through to the ``before_agent_run`` veto and posted one
        // refusal per blocked heartbeat, cron tick and inbound message into a
        // channel, to people who could do nothing about it — the exact
        // behaviour design principle 6 forbids ("refusals never post to a
        // channel per attempt"). ``_breakerReplyClaim`` lives on this hook, so
        // the hook has to exist wherever a breaker can trip, which is
        // everywhere.
        //
        // The two capability-driven behaviours inside ``handleBeforeAgentReply``
        // (stay-silent suppression, evo direct-send / llm-echo) self-gate via
        // the run-tracking Sets ``_directSentRuns`` / ``_llmEchoRuns``, which
        // only the injectKeywords keyword-intercept populates — so on a tier
        // without those capabilities they stay empty and the handler does
        // nothing but the breaker claim. Registering it here changes no
        // behaviour on the tiers that already had it.
        api.on("before_agent_reply", async (event, ctx) => this.handleBeforeAgentReply(ctx?.runId, ctx, event), { name: "evolve-before-agent-reply" });
        // Gate: injectPodConduct OR injectKeywords — the SAME gate as session_start
        // (line ~1390). This block registers before_prompt_build only. What DOES
        // run for `manage` is the per-turn regular-turn injection (capability
        // block + speaker context + primary-only narrative). It MUST: those are
        // the blocks that reach long-running sessions, and the capability block in
        // particular was injected at session_start (injectPodConduct-gated) before
        // this migration — narrowing it to injectKeywords here would silently drop
        // it for `manage`.
        if (caps.injectPodConduct || caps.injectKeywords) {
            ...
            api.on("before_prompt_build", async (_event, ctx) => {
                try {
                    ...
                    // ── Housekeeping (memory flush): allow-list only, no injections ──
                    // The flush needs read + its append-only write and nothing Evolve
                    // adds to a conversation turn (capability block, speaker context,
                    // narrative). Every block appended here would be re-cached on the
                    // cheap model for a prompt that reads none of it. toolsAllow is
                    // intersected with OC's own flush allow-list, so this can only
                    // narrow — it holds if a future OC loosens its list.
                    if (this._housekeepingKindForRun(ctx)) {
                        return { toolsAllow: [...HOUSEKEEPING_TOOLS_ALLOW] };
                    }
                    // ── D-OH3: narrow a woken heartbeat/cron to what is actually due ─
                    // The due-check already decided this turn earns a model call; the
                    // model's job is now "do these two things", not "read HEARTBEAT.md
                    // and work out what applies". prependContext (not
                    // appendSystemContext) because OC merges it into the USER-side
                    // turn context — the same slot heartbeat_prompt_contribution feeds
                    // — so it reads as this turn's instruction rather than as a
                    // standing system rule, and it does not churn the cached system
                    // prefix (post-mortem §2).
                    let duePrepend = "";
                    if (runId && this._dueWakeByRun.has(String(runId))) {
                        const due = this._dueWakeByRun.get(String(runId)) ?? [];
                        this._dueWakeByRun.delete(String(runId));
                        if (due.length > 0)
                            duePrepend = buildWakeContext(due);
                    }
                    if (runId && this._directSentRuns.has(runId)) {
                        ...
                        this.logger.info(`Evolve evo: injecting stay-silent system context ` +
                    ...
                    if (runId && this._llmEchoRuns.has(runId)) {
                        ...
                        this.logger.info(`Evolve evo: injecting LLM-directive system context ` +
                    ...
                    // (marker set in resolveModelRouting), tell the bot WHY. The chat
                    // now carries the true reason too (the rewritten banner), but a bot
                    // that can't observe its own routing still needs this to ANSWER a
                    // follow-up rather than confabulate. Cost drivers only: every other
                    // driver is self-evident to the user (they asked, or it's the
                    // configured default) and does not earn per-turn prompt bytes.
                    // Entry is intentionally not deleted on consume (prompt rebuilds on
                    // the same run must re-inject identically); FIFO eviction bounds
                    // the map.
                    let costDowngradeBlock = "";
                    try {
                        const routed = runId
                            ? this._evolveRoutedRuns.get(String(runId))
                            : undefined;
                        if (routed && (routed.driver === "spend_cap" || routed.driver === "runaway")) {
                            costDowngradeBlock = _buildCostDowngradeNotice(routed.driver, routed.model);
                        }
                    }
                    catch { /* attribution is best-effort; never break the turn */ }
                    // ── Scheduled-turn tool discipline ─────────────────────────────
                    // Heartbeat / cron turns only (SCHEDULED_TAGS via
                    // classifySessionKind). Background turns run on the cheap rung by
                    // design and burn a steady drip of round-trips rediscovering three
                    // tool contracts; stating them costs a few hundred bytes on turns
                    // that no user is waiting on, and nothing at all on user turns.
                    let toolDisciplineBlock = "";
                    try {
                        const kind = classifySessionKind(ctx?.sessionKey ?? ctx?.sessionId ?? null, null).kind;
                        if (kind === "scheduled") {
                            toolDisciplineBlock = _buildScheduledToolDisciplineNote();
                        }
                    }
                    catch { /* guidance is best-effort; never break the turn */ }
                    // Block ORDER is a contract, not the order these locals happen to
                    // be declared in: composeEvolvePrefix concatenates most-stable
                    // first so the volatile blocks (speaker, costDowngrade) sit in the
                    // tail, where a change displaces the least. See
                    // BlockStability.EVOLVE_PREFIX_BLOCK_ORDER for the rationale and
                    // tests/prefixBlockOrder.test.mjs for the pin. Before 2026-09 this
                    // was a bare array literal that opened with costDowngrade — the
                    // most volatile block in the worst position.
                    const composed = composeEvolvePrefix({
                        capabilities: capabilitiesBlock,
                        toolDiscipline: toolDisciplineBlock,
                        digest: directoryDigest,
                        narrative,
                        speaker: speakerBlock,
                        costDowngrade: costDowngradeBlock,
                    });
                    const combined = composed.text;
                    ...
                    if (combined) {
                        return {
                            appendSystemContext: combined,
                            ...(duePrepend ? { prependContext: duePrepend } : {}),
                        };
                    }
                    if (duePrepend) {
                        return { prependContext: duePrepend };
                    }
                }
                catch (err) {
            ...
            void this._renderCapabilitiesBlock().catch(() => { });
            ...
            void this._renderDirectoryDigestBlock().catch(() => { });
        }
        // ── Model-routing notice (reply_payload_sending) ──────────────────────
        // before_model_resolve marked this run as one Evolve routed; OC then
        // emits its own "Model Fallback … selected model unavailable" banner as a
        // separate reply payload, reporting a provider outage that did not
        // happen. This hook REPLACES that payload's text with the true reason.
        //
        // Not gated by an arming flag, because it adds no surface: the user is
        // already being shown a sentence about the model on exactly these turns.
        // The only question is whether that sentence is true, and delivering a
        // false one is not a safer default. The interlock that keeps it honest is
        // in the handler — a banner naming a model Evolve did not pick is left
        // alone, because then a provider really did fail.
        //
        // Scope is the banner, not the turn: OC emits one per fallback
        // TRANSITION, so a run of routed turns produces exactly one notice.
        // Diagnosis: internal/diagnosis-cost-downgrade-attribution-2026-09-03.md.
        try {
            api.on("reply_payload_sending", async (event) => withInTurnOwnerNotice(event, this.handleReplyPayloadSending(event)), { name: "evolve-model-routing-notice" });
        }
        catch (err) {
            // Same posture as ExecFailureAbsorber's message_sending guard: a
            // gateway too old for the hook must not abort the rest of init.
            this.logger.warn(`Evolve model-routing notice: reply_payload_sending not supported ` +
                `by this gateway (${err}) — OC's mislabeled fallback banner will be ` +
                `delivered unchanged.`);
        }
        ...
        if (caps.observer) {
            try {
                ...
                this.logger.info(caps.injectKeywords
        ...
        api.on("before_model_resolve", async (event, ctx) => {
            try {
                ...
                const sessionKey = _ctx.sessionKey ??
                    ...
                    _event.sessionKey ??
                    _event.channelId;
                const _rawUserMessage = _event.prompt ??
                    _ctx.userMessage ??
                    _ctx.message?.content ??
                    _ctx.message?.text ??
                    _ctx.input ??
                    "";
                // ── Self-call guard — MUST stay the first thing this hook does ───────
                // internal/finding-tier-router-self-call-loop-2026-09-07.md.
                //
                // The preflight router classifies a turn by asking a cheap model, and
                // it asks by running a plugin subagent. OC 2026.9.2 began firing
                // before_model_resolve for that subagent's own session, so the router
                // saw its own prompt as a turn to route and classified it again. The
                // router prompt is ambiguous to the regex layers BY DESIGN — that is
                // what sends a turn to the model layer in the first place — so every
                // level reached the model layer and spawned the next. Measured on the
                // pod: a bot whose steady state was 12 hook fires a DAY did 461 in
                // three minutes on 2026-09-08, ten of them inside the first 500 ms,
                // each nested call under its own fresh runId, seeded by a single
                // scheduled reminder.
                //
                // Two independent checks, because each covers the other's blind spot:
                //
                //   1. The session key. OC derives it from the idempotencyKey we
                //      chose, so `classifyEvolveSubagentKey` recognises the family
                //      ("agent:main:explicit:evolve:preflight:…"). This is the same
                //      test `llm_output` already applies to divert the cost record —
                //      the plugin knew how to recognise its own sessions for BILLING
                //      and never applied it to ROUTING.
                //   2. The prompt marker. The key derivation is OC's and undocumented;
                //      a change to it would silently reopen the loop, which is exactly
                //      the class of change 2026.9.2 turned out to be. The marker is
                //      written by runPinnedSubagent, on a prompt that is ours end to
                //      end. See EVOLVE_INTERNAL_PROMPT_SENTINEL.
                //
                // Returning {} means no injection and no model override: the run
                // proceeds on the model OC already resolved. Nothing about which model
                // answers a USER turn changes — this path is only ever Evolve's own
                // classification call, which has no business being routed.
                //
                // Ahead of the diag line deliberately: 461 diag lines for one reminder
                // is how this hid, and a guard that runs after the logging it is
                // supposed to silence is not a guard.
                if (classifyEvolveSubagentKey(sessionKey) !== null
                    || isEvolveInternalPrompt(_rawUserMessage)) {
                    return {};
                }
                // ── Housekeeping (memory flush) → cheap rung ─────────────────────────
                // OC's pre-compaction flush runs on the conversation's own session key
                // with ctx.trigger "memory". Left to the normal path it inherits that
                // session's (power) routing, keyword injection and all — the
                // $1.90 turn in internal/finding-cost-forensics-power-bot-2026-09-04.md
                // §2. Decided before anything below reads or WRITES per-session state:
                // the flush must not set the conversation's tier preference, user key
                // or session class, and it has no user message to intercept.
                const _hkKind = housekeepingKindForTrigger(_ctx.trigger);
                if (_hkKind) {
                    return caps.modelRouting ? this.resolveHousekeepingRouting(_ctx, sessionKey, _hkKind) : {};
                }
                ...
                if (sessionKey && caps.modelRouting) {
                    ...
                    this.modelRouter.setUserTier(String(sessionKey), _directiveTier ?? process.env.EVOLVE_TIER_PREFERENCE);
                    ...
                    try {
                        ...
                        const _userKey = _channel && _sender ? `ext:${_channel}:${_sender}` : null;
                ...
                try {
                    ...
                    this.logger.info(
                    // Full key, not the first 12 chars. The truncation printed
                    // "agent:main:e" for every one of the 461 looping fires on
                    // 2026-09-08 — the part it cut, "explicit:evolve:preflight:…",
                    // is precisely the part that says whose call it is, so the
                    // forensics needed a second pass over the log to learn what the
                    // first pass was already looking at.
                    `Evolve diag: before_model_resolve fired sessionKey=${String(sessionKey ?? "<none>")} ` +
                ...
                try {
                    ...
                    if (intercepted) {
                        ...
                        return this._withJudgeTail(_ctx, caps.modelRouting ? this.resolveModelRouting(_ctx, sessionKey) : {});
                    }
                }
                catch (err) {
                ...
                if (betterResult.systemAppend) {
                    ...
                    if (_ctx.runId && this.keywordHandler.parseEvoCommand(userMessage) !== null) {
                        if (this._evoHandledRuns.size >= 1024) {
                            ...
                            if (_oldest !== undefined)
                                this._evoHandledRuns.delete(_oldest);
                        }
                    ...
                    return this._withJudgeTail(_ctx, { ...modelResult, systemAppend: betterResult.systemAppend });
                }
    ...
    /**
     * reply_payload_sending handler — replaces OC's mislabeled model-fallback
     * banner with the true reason on a run Evolve routed.
     *
     * Correlation is by ``event.runId``, the SAME key before_model_resolve
     * marked: the dispatcher populates it from the agent run
     * (``buildReplyPayloadSendingBeforeDeliver`` → ``runState.runId``, set by
     * ``onAgentRunStart``). Note this is the one outbound seam that carries it
     * — ``message_sending`` / ``message_sent`` do NOT have the run id plumbed
     * through (OC's own hook-types doc says so), which is why the notice lives
     * here and not beside the exec-failure absorber.
     *
     * The interlock: OC re-fires before_model_resolve on every failover
     * attempt, and resolveModelRouting stands down on the repeat — so a
     * provider failure AFTER our override lands the run on some other rung
     * while the run stays marked. Rewriting there would replace a true outage
     * report with "Evolve routed this session to the power tier", which is the
     * same class of lie in the other direction. Comparing the banner's ACTIVE
     * model against the one we asked for separates the two cases, and anything
     * that does not parse or does not match is left alone.
     *
     * Fail-open by construction: anything that is not a confident match on a
     * marked run returns undefined (deliver unchanged), and any internal error
     * is caught — an attribution nicety must never eat or delay a real reply.
     */
    handleReplyPayloadSending(event) {
        // ── Structured judge tail strip (D-OH4) ─────────────────────────────
        // Runs FIRST and independently of the routing notice below: the tail
        // is our instruction's output and must never reach the user,
        // whatever else is true about this run. A well-formed tail is
        // removed; a malformed one is left alone rather than guessed at
        // (structuredTail.ts). Nothing here can throw — the parse is pure —
        // but the whole handler is wrapped anyway a few lines down.
        const _tailPayload = event?.payload;
        if (typeof _tailPayload?.text === "string" && hasTailMarker(_tailPayload.text)) {
            const parsed = parseStructuredTail(_tailPayload.text);
            if (parsed.found) {
                this.logger.debug(`Evolve judge: stripped verdict tail from an outbound payload ` +
                    `(run ${String(event?.runId ?? "").slice(0, 8)})`);
                return { payload: { ..._tailPayload, text: parsed.stripped } };
            }
            this.logger.warn(`Evolve judge: an outbound payload carries an unparseable verdict ` +
                `tail — delivering it unchanged rather than guessing where it ends. ` +
                `The model did not follow the structured-tail contract.`);
        }
        try {
            ...
            const routed = this._evolveRoutedRuns.get(key);
            if (!routed)
                return undefined;
            const payload = event?.payload;
            if (!_isFallbackNoticePayload(payload))
                return undefined;
            const text = typeof payload.text === "string" ? payload.text : "";
            if (!text)
                return undefined;
            if (this._routingNoticeRuns.has(key))
                return undefined;
            const active = _fallbackNoticeActiveModel(text);
            if (!active || !_sameModelRef(active, routed.model)) {
                // A real fallback walked past our override — OC's banner is accurate.
                this.logger.info(`Evolve model-routing notice: banner names ${active ?? "an unparseable model"} ` +
                    `but Evolve routed to ${routed.model} (run ${key.slice(0, 8)}) — ` +
                    `a provider failure moved the turn, delivering OC's banner unchanged.`);
                return undefined;
            }
            if (this._routingNoticeRuns.size >= 1024) {
                const oldest = this._routingNoticeRuns.values().next().value;
                if (oldest !== undefined)
                    this._routingNoticeRuns.delete(oldest);
            }
            this._routingNoticeRuns.add(key);
            const notice = _buildRoutingNotice(routed.driver, active, routed.role, routed.degradedFrom);
            this.logger.info(`Evolve model-routing notice: replaced OC's fallback banner ` +
                `(run ${key.slice(0, 8)}, driver=${routed.driver ?? "none"}, ` +
                `role=${routed.role ?? "none"}, model=${active}).`);
            return { payload: { ...payload, text: notice } };
        }
        catch (err) {
            this.logger.warn(`Evolve model-routing notice error: ${err}`);
            return undefined;
        }
    }
    ...
    _getManifestTriggers() {
        ...
        for (const fname of files) {
            ...
            try {
                ...
                if (m?.invocation_mode !== "plugin_intercept")
                    continue;
                ...
                if (!_manifestStatusAllowsTriggers(m))
                    continue;
                ...
                this.logger.warn(`Evolve Layer C: manifest ${fname} failed to load — ${err}`);
            }
        }
        ...
        if (compiled.length > 0) {
            this.logger.info(`Evolve Layer C: compiled ${compiled.length} trigger(s) ` +
     ...
     */
    _compileTrigger(t, manifest, workspaceRoot) {
        ...
        try {
            ...
            this.logger.warn(`Evolve Layer C: trigger ${t.id ?? "?"} has invalid pattern — ${err}`);
            return null;
        }
        ...
        if (typeof match.exclude_pattern === "string" && match.exclude_pattern) {
            try {
                ...
                this.logger.warn(`Evolve Layer C: trigger ${t.id ?? "?"} has invalid exclude_pattern — ${err}`);
            }
        }
        const stdoutProtocol = invocation.stdout_protocol;
        if (!isKnownProtocol(stdoutProtocol))
            return null;
     ...
     */
    _substituteTemplate(template, tokens) {
        let out = template;
     ...
     */
    _substitutePayload(payload, tokens) {
     ...
     * with non-zero exit.
     */
    _runScript(scriptAbsolutePath, requestFilePath, cwd) {
     ...
     */
    async _interceptManifestTrigger(ctx, userMessage) {
        if (!userMessage)
            return false;
        ...
        for (const t of triggers) {
            try {
                ...
                if (t.channel !== "any" && channelHint !== "unknown") {
                    ...
                    const channelPartsInHint = t.channel.split("_").some((p) => p && channelHint.includes(p));
                    if (!hintInChannel && !channelPartsInHint)
                        continue;
                }
                if (t.excludePattern && t.excludePattern.test(userMessage))
                    continue;
                if (!t.pattern.test(userMessage))
                    continue;
            }
            catch (err) {
                this.logger.warn(`Evolve Layer C: trigger ${t.appId}:${t.triggerId} match error — ${err}`);
                continue;
            }
            ...
            const tokens = {
                ...
                message_text_minus_mention: userMessage.replace(/^\s*@[A-Za-z0-9_]+\s*/, "").trim(),
                message_text_minus_command: userMessage.replace(/^\s*\/[A-Za-z0-9_]+\s*/, "").trim(),
            };
            ...
            this.logger.info(`Evolve Layer C: trigger matched ${t.appId}:${t.triggerId} ` +
            ...
            this.logger.info(`Evolve Layer C: script ${path.basename(t.scriptAbsolutePath)} ` +
            ...
            this.logger.info(`Evolve Layer C: protocol=${t.stdoutProtocol} outcome=${parsed.outcome} ` +
            ...
            this._markDirectSent(ctx?.runId, `layer-c:${t.appId}:${t.triggerId}:${parsed.outcome}`);
            return true;
        }
        return false;
    }
     ...
     */
    async _handleTriggerFailure(t, ctx, reason) {
        this.logger.warn(`Evolve Layer C: trigger ${t.appId}:${t.triggerId} failed — ${reason}`);
        ...
        this._markDirectSent(ctx?.runId, `layer-c-fallback:${t.appId}:${t.triggerId}`);
        return true;
    }
    _markDirectSent(runId, subcommandBrief) {
        if (!runId)
            return;
        if (this._directSentRuns.size >= 1024) {
            const oldestKey = this._directSentRuns.keys().next().value;
            if (oldestKey !== undefined)
                this._directSentRuns.delete(oldestKey);
        }
     ...
     */
    _markLLMEcho(runId, systemAppendText) {
        if (!runId || !systemAppendText)
            return;
        if (this._llmEchoRuns.size >= 1024) {
            const oldestKey = this._llmEchoRuns.keys().next().value;
            if (oldestKey !== undefined)
                this._llmEchoRuns.delete(oldestKey);
        }
     ...
     */
    _composedReplyDirective(body, opts = {}) {
        ...
        return ("SYSTEM: Your reply for this turn has already been composed for you — it " +
     ...
     */
    _llmEchoVerbatimInstruction(wrappedBody) {
     ...
     */
    _recordEvoDispatchFailure(reason, meta) {
        try {
     ...
     */
    _evoErrorRelayInstruction(body) {
        return this._composedReplyDirective(body, {
            extraProhibitions: "The user typed an `evo` command that could not be handled, so this is " +
     ...
     */
    async _deliverEvoFailure(ctx, surface, reason, meta) {
        ...
        if (surface === "member_bot") {
            ...
            this.logger.info(`Evolve evo failure: direct-send unavailable (reason=${reason}, ` +
     ...
     */
    async _runEvoSelfTest() {
        const botId = this.config.botId;
        const surface = this.getBetterSurface();
        try {
            // channel/sender null: `evo help` is generic + read-only (no wizard
            // state, no per-user routing), so the lightest possible probe.
            const outcome = await this.evoDispatchClient.dispatch(botId, null, null, "evo help");
            if (outcome.ok) {
                this.logger.info(`Evolve evo self-test: PASS — admin dispatch reachable ` +
                    `(bot=${botId}, subcommand=${outcome.result.subcommand})`);
            }
            else {
                this.logger.warn(`Evolve evo self-test: FAIL — reason=${outcome.reason}` +
            ...
            this.logger.warn(`Evolve evo self-test: FAIL — exception ${err} (bot=${botId})`);
        }
    }
    /**
     * The conversation this turn belongs to, for once-per-trip bookkeeping.
     *
     * ``sessionKey`` first because it is the value OC itself keys a
     * conversation by (``agent:main:slack:channel:g0t79fgse``). ``channelId``
     * is a LAST resort and never a type — on OC ≥2026.7 it carries the chat
     * id, so it identifies a conversation but not a platform. Empty string
     * when nothing resolves, which collapses every unattributable turn onto
     * one budget of one message: the safe direction, since the failure it
     * exists to prevent is repetition.
     */
    _breakerConversationKey(ctx, event) {
        const candidates = [
            ctx?.sessionKey, ctx?.sessionId, ctx?.chatId,
    ...
    /** Record that this trip has now spoken in this conversation, with a cap. */
    _markBreakerSpoken(key) {
        if (this._breakerSpokenConversations.size
            >= TurnObserver._BREAKER_SPOKEN_CAP) {
            // Sets iterate in insertion order — drop the oldest.
            const oldest = this._breakerSpokenConversations.values().next();
            if (!oldest.done)
                this._breakerSpokenConversations.delete(oldest.value);
        }
        this._breakerSpokenConversations.add(key);
    }
    /**
     * Claim (and silence) a turn that a tripped cost breaker must refuse.
     *
     * **Why this hook.** ``before_agent_run`` cannot refuse quietly. Its
     * ``block`` decision is fed to OC's ``resolveBlockMessage``, which ALWAYS
     * produces text — with no ``message`` field it still emits "Your message
     * could not be sent: blocked by evolve" — and that text comes back as the
     * turn's ``promptError``, which the channel posts. Verified against the
     * fleet build (OC 2026.9.2, ``dist/hook-runner-global-*.js``). So every
     * refusal on that hook is, by construction, a channel message.
     *
     * ``before_agent_reply`` is the seam that can. It fires once per run for
     * triggers ``cron`` | ``heartbeat`` | ``user``, and on a truthy
     * ``handled`` OC short-circuits to ``{text: "NO_REPLY"}`` — its silent
     * sentinel — and returns BEFORE ``executePreparedEmbeddedRun``, so the
     * model is never dispatched and ``before_agent_run`` never runs. Zero
     * spend, zero bubbles. (Verified in ``dist/embedded-agent-*.js``;
     * ``buildHandledBeforeAgentReplyPayloads`` defaults the reply to
     * ``NO_REPLY``.)
     *
     * The rules it implements:
     *
     *   * **background** (heartbeat / cron) → always claimed, always silent.
     *     Nobody is waiting on a heartbeat; a refusal posted to a shared
     *     channel is pure noise to people who cannot act on it.
     *   * **user** → claimed only when a cost CHECKPOINT is actually holding
     *     this bot AND this trip has already spoken once in this
     *     conversation. The first such turn is left to
     *     ``_handleCostCheckpointTurn`` (it has something to say); every
     *     later one is silence. A bot whose cap action is not the checkpoint
     *     is untouched — D-CC1 keeps user chat working there.
     *   * **a checkpoint ANSWER is never claimed.** "continue" / "stop" must
     *     reach ``before_agent_run`` or the owner could not lift their own
     *     cap — silencing the hold would make it unliftable from chat.
     *
     * Returns ``undefined`` to let the turn proceed.
     *
     * **Fail CLOSED on unknown breaker state.** An *absent*
     * ``breakers/<bot>/cost.json`` legitimately means "not tripped" and lets
     * the turn proceed as it does today. A file that EXISTS and does not
     * parse — a truncated write, a disk-full, a hand-edit, a schema change
     * mid-deploy — means "unknown", and a turn on a bot whose breaker state
     * is unknown is treated exactly as a tripped one: claimed silently for
     * background work, and put under the same once-per-conversation rule for
     * a person. A truncated write during a trip is precisely when the file is
     * unreadable, which is the moment the breaker must hold.
     */
    _breakerReplyClaim(ctx, event) {
        let trigger;
        try {
            trigger = String(ctx?.trigger ?? "").trim().toLowerCase();
        }
        catch {
            return undefined;
        }
        if (trigger !== "heartbeat" && trigger !== "cron" && trigger !== "user") {
            return undefined;
        }
        let decision;
        try {
            ...
            // "cannot happen" branch — and unknown state holds the breaker.
            this.logger.warn(`Evolve cost breaker reply-claim check failed (treating the bot as ` +
                `paused — unknown breaker state holds): ${err?.message ?? err}`);
            decision = { vetoed: false, unreadable: true };
        }
        if (!decision.vetoed && !decision.unreadable) {
            // Nothing tripped — the per-trip message budget resets with the bot.
            if (this._breakerSpokenConversations.size > 0) {
                this._breakerSpokenConversations.clear();
            }
            return undefined;
        }
        // An unreadable record carries no trip_id, so every unknown-state turn
        // shares one budget key — the safe direction, since the failure this
        // budget exists to prevent is repetition.
        const tripId = decision.tripId ?? "no-trip-id";
        if (trigger === "user") {
            // Only a live checkpoint hold silences a person. Without one the
            // breaker does not gate user chat at all (D-CC1) and claiming here
            // would mute an ordinary conversation.
            let status;
            try {
                status = readCostCheckpoint({
                    sharedDir: this.config.sharedDir,
                    botId: this.config.botId,
                });
            }
            ...
            if (status === null)
                return undefined;
            if (classifyCheckpointAnswer(event?.cleanedBody) !== null) {
                // An answer to the hold. Let it through — this is how the cap
                // gets lifted.
                return undefined;
            }
            const key = `${tripId}|${this._breakerConversationKey(ctx, event)}`;
            if (!this._breakerSpokenConversations.has(key)) {
                // First blocked turn of this trip in this conversation: mark it
                // now and let before_agent_run emit the ONE cap-reached message.
                // Marked before the message rather than after because this hook
                // is the only point that sees every attempt; the cost of the
                // ordering is at worst one lost message, against the flood it
                // replaces.
                this._markBreakerSpoken(key);
                // D-CS13 evidence: this IS the first interactive hold of this trip
                // in this conversation. decision.tripId (not the "no-trip-id"
                // fallback) — an unreadable breaker record has no real trip to
                // correlate the marker against.
                if (decision.tripId) {
                    recordInteractiveHold({
                        sharedDir: this.config.sharedDir,
                        botId: this.config.botId,
                        tripId: decision.tripId,
                        session: this._breakerConversationKey(ctx, event) || null,
            ...
            this._recordBreakerRefusal(trigger, decision, "already answered once");
            return {
                handled: true,
                reason: "evolve cost breaker: already replied once for this trip",
            };
        }
        this._recordBreakerRefusal(trigger, decision, "background trigger");
        return {
            handled: true,
            reason: "evolve cost breaker: background activity paused",
        };
    }
    /**
     * Log a refused attempt with ``outcome=breaker_refused``.
     *
     * Rate-limited to one line a minute. A paused bot still receives
     * heartbeats, cron ticks and inbound messages; one line each buries the
     * log an operator reads to understand the pause, which is the same
     * failure as the channel flood one level down.
     */
    _recordBreakerRefusal(trigger, decision, note) {
        const now = Date.now();
        if (now - this._lastBreakerRefusalLogMs < 60_000)
            return;
        this._lastBreakerRefusalLogMs = now;
        this.logger.info(`Evolve cost breaker: outcome=breaker_refused ` +
            `(bot=${this.config.botId}, trigger=${trigger}, ` +
            `scope=${decision.scope ?? "bot"}, ` +
            `trip_id=${(decision.tripId ?? "").slice(0, 8)}, ${note}). ` +
            `Refused silently — no channel message. Logged once a minute.`);
    }
    /**
     ...
     */
    handleBeforeAgentReply(runId, ctx, event) {
        try {
            // A tripped cost breaker claims the turn HERE, silently, before the
            // direct-send check — see ``_breakerReplyClaim`` for why this hook
            // and not ``before_agent_run``.
            const breakerClaim = this._breakerReplyClaim(ctx, event);
            if (breakerClaim !== undefined)
                return breakerClaim;
            if (runId && this._directSentRuns.has(runId)) {
                ...
                this.logger.info(`Evolve evo: suppressing LLM reply (already direct-sent) ` +
            ...
            const dueDecision = this._decideHeartbeatDue(runId, ctx);
            if (dueDecision)
                return dueDecision;
        }
        catch (err) {
    ...
    /**
     * D-OH3 — the "is anything due" gate for a heartbeat or cron trigger.
     *
     * Returns ``{handled:true}`` ONLY when a valid ``HEARTBEAT.json`` says
     * nothing is due; OC then short-circuits the run to ``NO_REPLY`` without
     * dispatching the model (verified against the OC 2026.9.2 bundle — see
     * HeartbeatDueCheck's module docstring). Every other path returns
     * ``undefined`` and the turn proceeds exactly as it does today:
     *
     *   * a ``user`` trigger — never gated, whatever the message says;
     *   * a bot with no conditions file — the default for every bot;
     *   * a ``cron`` job the file does not name in its ``cron`` map — a cron
     *     job carries its own instruction and is never gated on the
     *     heartbeat's conditions;
     *   * a file that is unreadable, invalid, disabled or empty — logged once,
     *     model runs (the one deliberate fail-open in Evolve: the alternative
     *     to a spurious call is a silently dead heartbeat);
     *   * nothing due but the operator-side max-silence floor passed — the
     *     model runs anyway, because the file that said "nothing due" is one
     *     the bot itself can write;
     *   * something due — the model runs, and ``before_prompt_build`` narrows
     *     the turn to exactly those items.
     *
     * Context fields read, all present on OC 2026.9.2's ``before_agent_reply``
     * hook context (``embedded-agent-*.js``, the ``hookCtx`` literal passed to
     * ``runBeforeAgentReplyForTurn``): ``trigger``, ``workspaceDir``,
     * ``sessionId``, ``runId``, and ``jobId`` (the cron store's job id, set
     * from ``params.job.id`` for a cron run).
     */
    _decideHeartbeatDue(runId, ctx) {
        const trigger = String(ctx?.trigger ?? "").toLowerCase();
        // Strict allowlist, not a heuristic: OC only ever passes cron / heartbeat
        // / user here, and a user turn that merely TALKS about heartbeats must
        // never be suppressed.
        if (!DUE_CHECK_TRIGGERS.includes(trigger))
            return undefined;
        const workspaceDir = typeof ctx?.workspaceDir === "string" && ctx.workspaceDir.trim()
            ? ctx.workspaceDir
            : null;
        if (!workspaceDir) {
            // A NEW dependency on OC's hook-context shape (the pre-D-OH3
            // TurnObserver never read workspaceDir on any hook). If a point
            // release drops or renames the field the check goes permanently
            // inert — every bot back to full idle burn — and the receipt would
            // keep printing "0 skipped" as though no bot had opted in. Say so
            // once rather than degrading in silence.
            if (!this._loggedMissingHeartbeatWorkspaceDir) {
                this._loggedMissingHeartbeatWorkspaceDir = true;
                this.logger.warn(`Evolve heartbeat: a ${trigger} trigger reached the due-check with no ` +
                    `workspaceDir on the hook context — the check cannot run and every ` +
                    `${trigger} tick will call the model as before. Expected on OC 2026.9.2's ` +
                    `before_agent_reply context; a gateway upgrade may have renamed it.`);
            }
            return undefined;
        }
        const jobId = typeof ctx?.jobId === "string" ? ctx.jobId : null;
        const verdict = this.heartbeatDueCheck.evaluate(workspaceDir, { trigger, jobId });
        if (verdict.decision === "unavailable") {
            this.heartbeatDueCheck.warnOnce(verdict.warn, `${workspaceDir}`);
            return undefined;
        }
        const sessionId = ctx?.sessionId ? String(ctx.sessionId) : null;
        if (verdict.decision === "skip") {
            appendDecision(this.config.sharedDir, buildDecisionRecord({
                botId: this.config.botId,
                trigger,
                outcome: "skipped_nothing_due",
                sessionId,
                runId: runId ?? null,
                conditionsEvaluated: verdict.conditionsEvaluated,
                conditionsSha256: verdict.conditionsSha256,
                cronJobId: verdict.cronJobId,
                reason: verdict.reason,
            }), this.logger);
            this.logger.info(`Evolve heartbeat: no model call — ${trigger} ${verdict.reason} ` +
                `(run ${String(runId ?? "?").slice(0, 8)})`);
            return { handled: true, reason: `${trigger}: nothing due` };
        }
        // Wake. The fired conditions are marked served HERE, at the decision, and
        // not after the turn: this hook is the only point in the run that is
        // guaranteed to execute, so there is no later place to commit from.
        //
        // The trade-off, stated plainly: if the model turn then fails outright
        // (provider error, timeout), a cadence condition waits out its interval
        // and a ``file_changed`` misses that one edit — it re-fires on the NEXT
        // edit, because the state records the mtime we observed rather than
        // "now". The alternative — commit on success — needs a success signal
        // this seam does not have, and would re-fire the same item every tick
        // for any turn that legitimately answered NO_REPLY.
        this.heartbeatDueCheck.commitState(verdict.nextState);
        if (runId && verdict.due.length > 0) {
            if (this._dueWakeByRun.size >= TurnObserver._DUE_WAKE_MAX_RUNS) {
                const oldest = this._dueWakeByRun.keys().next().value;
                if (oldest !== undefined)
                    this._dueWakeByRun.delete(oldest);
            }
            this._dueWakeByRun.set(String(runId), verdict.due);
        }
        appendDecision(this.config.sharedDir, buildDecisionRecord({
            botId: this.config.botId,
            trigger,
            ...
            runId: runId ?? null,
            conditionsEvaluated: verdict.conditionsEvaluated,
            dueIds: verdict.due.map((d) => d.id),
            conditionsSha256: verdict.conditionsSha256,
            cronJobId: verdict.cronJobId,
            reason: verdict.reason,
        }), this.logger);
        this.logger.info(`Evolve heartbeat: waking the model — ${trigger} ${verdict.reason} ` +
            `(run ${String(runId ?? "?").slice(0, 8)})`);
        return undefined;
    }
    /**
     ...
     */
    _stayQuietSystemContext(subcommandBrief) {
        ...
        if (subcommandBrief) {
            lines.push("WHAT THE USER ASKED FOR:", subcommandBrief, "");
        }
        lines.push("WHAT JUST HAPPENED:", "The user's most recent message either started or advanced an", "Evolve plugin flow. Evolve is a chat-driven admin layer for", "this bot's pod (admin commands, gallery installs, OAuth setup,", "bot health checks, etc.) invoked via `evo <subcommand>` keywords", "or, mid-flow, by replying to an Evolve wizard with data. The", "plugin already understood the user's request and just delivered", "the user-visible response directly to their chat via channel", "transport (Telegram Bot API), framed by these markers (which", "appear literally in the delivered message):", "", "    ═══ evo ═══", "    [the message the user already received]", "    ═══ end evo ═══", "", "You don't need to figure out what the user's keyword means or", "what the plugin did — both are already handled" +
            (subcommandBrief ? " (see above)" : "") + ".", "", "WHO OWNS THIS CONVERSATION RIGHT NOW:", "The plugin is the speaker. The user's next reply belongs to", "the plugin's wizard / dispatcher, not to you — they're", "answering the plugin, not asking you something. The wizard is", "multi-turn; you will see their answer in your context next", "turn. Continue this behavior every turn while you see", "═══ evo ═══ markers in the recent conversation.", "", "NO TOOL CALLS THIS TURN:", "Make ZERO tool calls. No exec / shell / python, no file reads", "or writes, no web fetches, no MCP calls, no search, no nothing.", "The plugin already did all the work — there is nothing to look", "up, verify, or investigate. In particular: do NOT try to run", "anything under /Users/Shared/evolve, /Users/Shared/evolve-repo,", "or the analyzer/admin Python packages. Those are plugin", "infrastructure, not commands you should invoke. If a tool call", "would be blocked by gateway policy (exec.security, sandbox,", "etc.), that is a stop sign — never offer to disable, weaken,", "or route around any security policy to make a command work.", "", "YOUR OUTPUT THIS TURN — SINGLE INSTRUCTION:", "Produce NO visible output at all. The plugin already delivered", "the user's reply, so there is nothing for you to say. Output the", "exact token NO_REPLY (uppercase, by itself, nothing else) — the", "runtime recognizes it as a silent turn and shows the user no", "additional message. Do NOT emit a period, a sentence, an emoji,", "a quoted version of the user's message, an offer to relay their", "answer, or a suggestion to redo anything. Just NO_REPLY.", "═══════════════════════════════════════════════════════════════════════");
        return lines.join("\n");
    }
    // ── Per-turn Home-narrative injection ──────────────────────────────────────
    //
    ...
    static _HOME_NARRATIVE_MAX_AGE_MS = 6 * 60 * 60 * 1000; // 6h — matches _HOME_NARRATIVE_MAX_AGE_S in
    ...
    _renderPerTurnNarrativeBlock() {
        ...
        if (this.config.role !== "primary")
            return "";
        const cachePath = path.join(this.config.sharedDir, "home-narrative-cache.json");
        let payload;
        try {
    ...
    static _CAP_BLOCK_TTL_MS = 15 * 60 * 1000; // 15 min
    static _CAP_BLOCK_TIMEOUT_MS = 8_000;
    ...
    static _DIR_DIGEST_TTL_MS = 3 * 60 * 1000; // 3 min
    static _DIR_DIGEST_TIMEOUT_MS = 4_000;
    /**
     ...
     */
    static _BLOCK_MAX_STALE_MS = 24 * 60 * 60 * 1000; // 24h
    /**
    ...
    async _renderCapabilitiesBlock() {
        ...
        this._capBlockInflight = (async () => {
            ...
            try {
                ...
                let sharedDir = this.config.sharedDir;
                try {
                    const net = JSON.parse(fs.readFileSync(path.join(this.config.sharedDir, "network.json"), "utf8"));
                    if (net.sharedDir)
                        sharedDir = net.sharedDir;
                }
                catch { /* no network.json — use default */ }
                const { stdout } = await execFileAsync(evolvePythonBin(), [
                    scriptPath,
                    "--capabilities-only",
                    "--bot", this.config.botId,
                    "--shared-dir", sharedDir,
                ...
                this.logger.warn(`Evolve capability block render failed (exit ${err?.code}): ` +
                    `${err?.stderr?.trim() ?? err}`);
            }
            ...
            if (rendered) {
                ...
                if (text) {
                    ...
                    this.logger.warn(`Evolve: capability block render failed — serving last-good ` +
            ...
            this.logger.info(`Evolve: capability block ${text ? text.length + " chars" : "empty"} ` +
     ...
     */
    async _renderDirectoryDigestBlock() {
        if (!this.config.capabilities.injectPodConduct)
            return "";
        const fresh = this._dirDigestBlock.getFresh(Date.now());
        if (fresh !== null)
            return fresh;
        if (this._dirDigestInflight)
            return this._dirDigestInflight;
        this._dirDigestInflight = (async () => {
            let text = "";
            let rendered = false;
            try {
                ...
                if (res.status === 200 && res.body && typeof res.body === "object") {
                    text = renderDirectoryDigestBlock(res.body);
                    rendered = true;
                }
                else {
                    // 403 (this bot's gateway uid maps to no/ambiguous bot), 5xx, or a
                    // non-JSON body — never inject a partial/garbled block.
                    this.logger.warn(`Evolve directory digest: unexpected response (status ${res.status})`);
                }
            }
            catch (err) {
            ...
            if (rendered) {
                ...
                if (text) {
                    ...
                    this.logger.warn(`Evolve: directory digest fetch failed — serving last-good ` +
                        `(${Math.round((age ?? 0) / 60000)}m old) for ${this.config.botId}`);
                }
            }
            this.logger.info(`Evolve: directory digest ${text ? text.length + " chars" : "empty"} ` +
    ...
    _buildSpeakerContextBlock(ctx) {
        ...
        if (!platform)
            return "";
        const resolution = resolveSpeakerRole(this.config.botId, platform, sender.senderId, { sharedDir: this.config.sharedDir });
    ...
    _pruneSessionMapsIfOversized() {
        const limit = 500;
        if (this.sessionTurns.size <= limit)
            return;
        ...
        for (const sid of this.sessionTurns.keys()) {
            ...
            if (t !== undefined) {
                clearTimeout(t);
                this._pendingSummaryTimers.delete(sid);
            }
            this._summarizedSessions.delete(sid);
            pruned++;
        }
        this.logger.warn(`Evolve: evicted ${pruned} stale sessions from memory — session_end may not be firing.`);
    }
    async handleTurn(event, ctx) {
        ...
        // ── Housekeeping (memory flush): one tagged row, nothing else ──────────
        // The flush shares the conversation's sessionId. Run through the rest of
        // this method it would count as a conversation turn, feed the flush
        // prompt ("store durable memories now") to the session classifier, the
        // struggle and pushback detectors and the summariser's transcript, and
        // bill as user demand. It is none of those: record its cost under its
        // own source so the receipt shows it on a separate line, and stop.
        const _hkKind = this._housekeepingKindForRun(ctx);
        if (_hkKind) {
            this._recordHousekeepingTurn(sessionId, ctx, _hkKind);
            return;
        }
        ...
        try {
            ...
            this.logger.info(`Evolve diag: agent_end fired sessionId=${String(sessionId).slice(0, 8)} ` +
            ...
            if (ctx && typeof ctx === "object") {
                for (const k of Object.keys(ctx)) {
                    ...
                    if (typeof v === "string" || typeof v === "number" || typeof v === "boolean") {
                        ...
                        try {
                            _ctxDump[k] = JSON.stringify(v).slice(0, 200);
                        }
                        catch {
                            _ctxDump[k] = "<unserializable>";
                        }
                    }
                }
            }
            this.logger.info(`Evolve diag: agent_end ctx dump = ${JSON.stringify(_ctxDump)}`);
        }
        ...
        const { userMessage, assistantMessage: _rawAssistantMessage } = extractMessages(event?.messages);
        // ── Structured judge tail (D-OH4) ───────────────────────────────────────
        // A prior turn asked THIS turn's own model for a struggle verdict as a
        // delimited line at the end of its reply. Read it here and record it
        // exactly where a second model call would have recorded it, so every
        // consumer downstream (the cascade controller, the span) is unchanged.
        //
        // The user never sees the line: handleReplyPayloadSending strips it on
        // the outbound seam. It is stripped HERE too, from the text every other
        // reader of this turn gets — the struggle detector, the turn record, the
        // summariser's transcript. Our own instruction's output is not part of
        // what the bot said, and leaving it in would have the detectors scoring
        // a sentence Evolve wrote. A malformed tail is left visible and yields
        // no verdict — see structuredTail.ts for why ugly beats lossy here.
        let assistantMessage = _rawAssistantMessage;
        if (this._armedJudgeTail.delete(sessionId) && hasTailMarker(assistantMessage)) {
            const parsed = parseStructuredTail(assistantMessage);
            assistantMessage = parsed.stripped;
            if (parsed.verdict) {
                this._sessionJudgeVerdicts.set(sessionId, {
                    verdict: parsed.verdict,
                    reason: "structured_tail",
                    // Zero, honestly: the judgement rode a turn that was happening
                    // anyway, so it cost no wall clock of its own. A latency SLO
                    // reader that sums this sees the truth — nothing was waited on.
                    latency_ms: 0,
                    triggered_by: "multiple",
                    // Self-judged by construction — it IS the model that did the work.
                    cross_vendor: false,
                });
                this.logger.debug(`Evolve judge: session ${String(sessionId).slice(0, 8)} self-reported ` +
                    `${parsed.verdict} in its own turn (no second model call)`);
            }
            else if (parsed.malformed) {
                this.logger.debug(`Evolve judge: session ${String(sessionId).slice(0, 8)} emitted an ` +
                    `unparseable verdict tail — left visible, no verdict recorded`);
            }
        }
        ...
        // turns, but the runaway behavior was masked while exec ran silently.
        {
            ...
            if (_runId && this._directSentRuns.has(_runId)) {
                ...
                if (_trimmed.length > STAY_QUIET_TOLERANCE) {
                    this.logger.warn(`Evolve evo: LLM ignored stay-quiet directive on direct-sent run ` +
        ...
        {
            const _allMsgs = Array.isArray(event?.messages) ? event.messages : [];
            ...
            for (const _m of _allMsgs) {
                if (_m?.role !== 'user')
                    continue;
                const _mText = typeof _m.content === 'string' ? _m.content
                    : Array.isArray(_m.content)
                        ? _m.content
                            ...
                            .join(' ')
                        : String(_m.content ?? '');
                const _contentType = typeof _m.content === 'string' ? 'string'
                    : Array.isArray(_m.content) ? 'array'
                        : typeof _m.content;
                const _hasMarker = _mText.includes('(untrusted metadata)');
                this.logger.info(`Evolve diag: agent_end userMsg[${_diagUserMsgIdx}] ` +
            ...
            if (_lastUserText !== null) {
                ...
                if (/^(evo|evolve)$/i.test(_lastUserText)) {
                    ...
                    if (_runId && this._evoHandledRuns.has(_runId)) {
                        ...
                        this.logger.info(`Evolve evo: agent_end fallback skipped — before_model_resolve handled run ${String(_runId).slice(0, 8)}`);
                    }
                    else {
        ...
        if (cachedLlmClass) {
            ...
            classResult = classifyTierByKeywords(userMessage, assistantMessage, sessionId, this.config.classifierHints);
            ...
            //
            // Gated OFF by default since D-OH2. This call bought a session class
            // that the routing rule now derives from the trigger for free, and it
            // arrived at agent_end — after the turn it was meant to route. When
            // the gate is closed the keyword classifier's verdict stands, which is
            // what a bot with an unreadable config or a slow classifier got anyway.
            if (turnCount === 1 && classResult.confidence < 0.65 && userMessage &&
                this.classifierGate.read(this.preflightRouter.hasConfidentPrior()).enabled) {
                this.llmClassifier.classify(userMessage, this.config.classifierHints, 
                // The cost-breaker gate refuses this helper on background work
                // but not on a person's turn — refusing it there would move the
                // session's tier to the keyword fallback instead of stopping it.
                { trigger: ctx?.trigger ?? null })
                    ...
                    if (!existing || existing.confidence < llmResult.confidence) {
                        ...
                        this.logger.info(`Evolve: LLM classified session ${sessionId.slice(0, 8)} as ${llmResult.class} (confidence ${llmResult.confidence})`);
                    }
                })
                    .catch((err) => {
        ...
        const correctionDetected = getCalibratedCorrectionPatterns().some((pattern) => userMessage.toLowerCase().includes(pattern));
        // Resolution turn: which turn number we're on (used by measure.py)
        const resolutionTurn = turnCount;
        // Cost estimation — catalog by exact model id, else the exact-id offline
        // table, else honestly unpriced (ModelPricing.priceCall). ``costEstimated``
        // stays a number for the in-process breakers below, which treat 0 as
        // "contributed nothing"; ``pricedTurn`` carries the honest null + source
        // onto the turn record.
        const pricedTurn = priceCall(modelSelected, llm?.inputTokens ?? 0, llm?.outputTokens ?? 0, llm?.cacheWriteTokens ?? 0, llm?.cacheReadTokens ?? 0, llm?.provider ?? null, this.config.sharedDir);
        const costEstimated = pricedTurn.costUsd ?? 0;
        ...
        try {
            ...
            if (runaway.tripped) {
                this.logger.warn(`Evolve cascade: runaway-rate cap TRIPPED for session ${String(sessionId).slice(0, 8)} — ` +
        ...
        if (struggleSignal?.payload_drift && !this._loggedDriftReasons.has(struggleSignal.payload_drift)) {
            ...
            this.logger.warn(`Evolve cascade: agent_end payload drift detected — reason=${struggleSignal.payload_drift}. ` +
        ...
        const appAttribution = resolveAppAttributionForTurn(ctx?.runId, sessionId !== "unknown" ? sessionId : null);
        const annotation = {
            ...
            // Which resolution produced cost_estimated (provider / catalog / table
            // / unpriced). Rides beside the number so usage_by_app and the per-app
            // rollups can tell a catalog price from a fallback-table estimate.
            cost_source: pricedTurn.costSource,
        ...
        if (llm &&
            ...
            this.logger.debug(`Evolve: re-tagged turn source=heartbeat (was ${llm.source}) for ` +
        ...
        try {
            ...
            this.writeTurnToShared(sessionId, llm, pricedTurn, ctx, appAttribution);
            ...
            if (this.cascadeTelemetry) {
                const now = new Date();
                const startedAt = typeof event?.durationMs === "number"
                ...
                const routerDriver = ctxSessionKey
                    ...
                    : null;
                const chosenBy = _computeChosenBy(routerDriver, userTierForChosenBy, userModelOverride, this.modelRouter.isCascadeEnabled(), modelTier);
                ...
                try {
                    ...
                    if (dangerousCombo.matched) {
                        this.logger.warn(`Evolve cascade: DANGEROUS COMBO detected for session ${String(sessionId).slice(0, 8)} — ` +
                ...
                // ── Cascade intent comes from the RULE, not the classifier ──────
                // D-OH2 item 5: the cascade keeps escalating on the free struggle
                // signals, but the tier it grades itself against is now the rule's
                // decision. Grading against a classifier we no longer run would be
                // shadow-comparing against a verdict nobody made.
                //
                // A user turn the rule declines to decide (no confident prior)
                // falls back to the classifier-derived tier, which is what the
                // ladder actually does on that turn: the bot's primary.
                const ruleDecisionForTurn = this.decideRuleForCtx({ ...ctx, channel: llm?.channel ?? ctx?.channel }, triggerKind);
                const tierIntended = ruleDecisionForTurn.tier ?? modelTier;
                // ── Sampling gate for the judge (D-OH4) ──────────────────────────
                // Read once per turn, TTL-cached underneath. `judgeSampled` is a
                // stable hash of (bot, session) — a session is in or out for its
                // whole life, so a struggling conversation is not judged on turn 3
                // and ignored on turn 4.
                const judgeGate = this.classifierGate.read(this.preflightRouter.hasConfidentPrior());
                const judgeSampled = shouldSampleSession(this.config.botId, sessionId, judgeGate.sampleRate);
                ...
                let sessionAggregateSignal;
                try {
                    this.sessionAggregator.observeTurn(sessionId, userMessage, assistantMessage, now);
                ...
                try {
                    ...
                    shadowVerdict = this.cascadeController.decide({
                        ...
                        triggerKind: triggerKind ?? "unknown",
                ...
                //
                // Two changes since D-OH2/D-OH4, both about what the judgement
                // COSTS rather than what it says:
                //
                //   - It runs on a SAMPLE of sessions (default 10%, 5% for a bot
                //     with a confident learned prior), not on every session whose
                //     thresholds trip. The rate is in network.json and rides the
                //     span so the ledger shows it.
                //   - When the classifier gate is closed — the default — the
                //     second model call does not happen at all. The judgement is
                //     asked of the PRIMARY model inside its own next turn, as a
                //     structured tail we strip before the user sees it. One word
                //     on a turn that was happening anyway, instead of a whole
                //     extra call over a re-sent conversation snippet.
                if (sessionAggregateSignal) {
                    const triggeredBy = shouldRunJudge(sessionAggregateSignal);
                    if (triggeredBy !== null && judgeSampled) {
                        if (judgeGate.enabled) {
                            this._fireJudgeAsync(sessionId, triggeredBy, this.sessionTurns.get(sessionId) ?? []).catch((err) => {
                                this.logger.debug(`Evolve: judge fire-and-forget failed: ${err}`);
                            });
                        }
                        else if (!this._armedJudgeTail.has(sessionId)) {
                            // Ask this session's own next turn instead. Skipped when a
                            // request is already outstanding — one in flight is enough.
                            if (this._pendingJudgeTail.size >= 512) {
                                const oldest = this._pendingJudgeTail.values().next().value;
                                if (oldest !== undefined)
                                    this._pendingJudgeTail.delete(oldest);
                            }
                            this._pendingJudgeTail.add(sessionId);
                        }
                    }
                }
                this.cascadeTelemetry.recordTurnSpan({
                    // What Evolve's own machinery was allowed to spend on this
                    // turn: whether the classifier gate was open at all, and the
                    // sample rate the kept judgements ran at (D-OH4 — "the ledger
                    // shows it"). Recorded on every span so an operator reading a
                    // week of spans can tell a quiet pod from a disabled one.
                    classifiersEnabled: judgeGate.enabled,
                    classifierSampleRate: judgeGate.sampleRate,
                    ruleDriver: ruleDecisionForTurn.driver,
                    sessionId,
                    turnIndex: resolutionTurn,
                    startedAt,
                    endedAt: now,
                    ...
                    dangerousComboContextTokens: dangerousCombo?.matched ? llm?.inputTokens : undefined,
                    ...
                    sessionJudge: this._sessionJudgeVerdicts.get(sessionId)
                        ? {
                            ...
                            cross_vendor: this._sessionJudgeVerdicts.get(sessionId).cross_vendor,
                        }
                        : undefined,
                });
            }
     ...
     * said 'evo' or 'evolve'.
     */
    async _handleEvoFallback(ctx, sessionId, chatIdOverride) {
     ...
     * round-trip).
     */
    async _sendEvoDirectToTelegram(ctx, rec, formatted) {
        const _sessionKey = String(ctx?.sessionKey ?? '');
        const _match = _sessionKey.match(/:(\d+)$/);
        if (!_match) {
            this.logger.info(`Evolve evo: cannot direct-send (no chatId in sessionKey=${_sessionKey.slice(0, 60)})`);
            return false;
        }
        ...
        try {
            ...
            this.logger.info(`Evolve evo: direct-sent ${what} to chat=${chatId} (${this.config.botId})`);
        }
        catch (err) {
    ...
    async _sendTelegramMessage(token, chatId, text) {
    ...
    async handleSessionEnd(event, ctx) {
        ...
        this._pendingJudgeTail.delete(sessionId);
        this._armedJudgeTail.delete(sessionId);
     ...
     */
    resolveModelRouting(ctx, sessionKey) {
        if (ctx.userModelOverride)
            return {};
        ...
        if (_runId &&
            ...
            this.logger.info(`Evolve ModelRouter: run ${_runId.slice(0, 8)} re-resolving (failover lane) — ` +
                `standing down, no override`);
            return {};
        }
        // ── The routing rule (D-OH2) ────────────────────────────────────────
        // Routing is decided HERE, from ctx alone, before any model runs and
        // without any model call of Evolve's own. Three inputs, all free:
        // the turn's trigger kind, the surface it arrived on, and the bot's
        // prior — which the nightly analyzer job learned offline from this
        // bot's own turns, not from a classifier fired at this one.
        //
        // What this replaced: the tier classifier ran at agent_end — AFTER
        // the turn — so a single-turn auto session (every heartbeat, every
        // cron) was always turn 1 and the classification never landed in
        // time. The workaround was to pre-classify from the trigger anyway,
        // which is to say: the free signal was doing the work, and the
        // model calls around it were paying for a verdict that arrived too
        // late to route anything. Decision §2 measured what they bought:
        // ~$3-4/day of heartbeat/cron savings, produced by a rule; up to
        // four Evolve model calls per real turn, produced by the machinery.
        //
        // Precedence is untouched. This only supplies the anchor the
        // classifier used to supply; the operator's tier preference and the
        // cost breakers still sit above it in _resolveModelAndTier, and an
        // existing classification from a prior turn still wins over the
        // anchor (we never overwrite a real verdict).
        const triggerKind = inferTriggerKind(ctx?.source, ctx?.channel ?? ctx?.channelId ?? ctx?.channel_id, ctx?.trigger);
        const ruleDecision = this.decideRuleForCtx(ctx, triggerKind);
        if (!this.modelRouter.getSessionType(sessionKey) && ruleDecision.sessionClass) {
            this.modelRouter.setSessionType(sessionKey, ruleDecision.sessionClass);
            this.logger.info(`Evolve routing rule: ${String(sessionKey).slice(0, 8)} → ` +
                `${ruleDecision.sessionClass} (driver=${ruleDecision.driver}, ` +
                `reason=${ruleDecision.reason}) — decided from the trigger, no model call`);
        }
        ...
        if (_runId) {
            if (this._routedRunIds.size >= 1024) {
                ...
                if (_oldest !== undefined)
                    this._routedRunIds.delete(_oldest);
            }
            this._routedRunIds.add(_runId);
        }
        if (!modelOverride && !authProfileOverride)
            return {};
        const result = {};
        ...
        if (modelOverride)
            Object.assign(result, splitProviderModelRef(modelOverride));
        if (authProfileOverride)
            result.authProfileOverride = authProfileOverride;
        if (modelOverride) {
            // EVERY hook-driven model change makes OC render a "selected model
            // unavailable" fallback banner (see _buildRoutingNotice), so every one
            // of them gets a per-run marker: reply_payload_sending reads it to
            // replace that banner with the true reason, and before_prompt_build
            // reads the cost-safety-net subset to brief the bot as well.
            const driver = this.modelRouter.getLastDecisionDriver(sessionKey);
            const role = this.modelRouter.getRoleForModel(modelOverride);
            const degradedFrom = driver === "role_cap"
                ? this.modelRouter.getLastCapDegradedFrom?.(sessionKey) ?? null
                : null;
            const costDriver = driver === "spend_cap" || driver === "runaway" ? driver : null;
            this.logger.info(`Evolve ModelRouter: routing session ${String(sessionKey).slice(0, 8)} to ${modelOverride}` +
                ` (driver=${driver ?? "none"}, role=${role ?? "none"}` +
                (costDriver ? " — cost-breaker downgrade" : "") +
                `; any OC fallback banner on this turn is THIS routing, not a provider failure)`);
            if (ctx?.runId) {
                if (this._evolveRoutedRuns.size >= 1024) {
                    const oldest = this._evolveRoutedRuns.keys().next().value;
                    if (oldest !== undefined)
                        this._evolveRoutedRuns.delete(oldest);
                }
                this._evolveRoutedRuns.set(String(ctx.runId), {
                    driver,
                    model: modelOverride,
                    role,
                    degradedFrom,
    ...
    /**
     * Route a housekeeping run (OC's pre-compaction memory flush) to the cheap
     * rung. See housekeeping.ts for what OC 2026.9.2 lets us enforce here and
     * what it doesn't (thinking level, output cap — no hook surface).
     *
     * Returns ``{}`` — the run proceeds on the model OC already chose — when:
     *   - the bot opted out (network.json bots.<id>.cost.levers.
     *     housekeeping_cheap_rung: false, or the pod-level key)
     *   - routing is disabled, or no cheap model resolves
     *   - this runId was already answered (OC's failover walk re-fires the
     *     hook for the same run; re-pinning the model whose failure started
     *     the walk is the 2026-07-31 incident — stand down, let OC fall back)
     *
     * No _evolveRoutedRuns marker: a flush has no user-visible reply, so there
     * is no fallback banner to explain.
     */
    resolveHousekeepingRouting(ctx, sessionKey, kind) {
        const runId = typeof ctx?.runId === "string" && ctx.runId ? ctx.runId : null;
        if (runId && this._housekeepingRuns.has(runId)) {
            this.logger.info(`Evolve housekeeping: ${kind} run ${runId.slice(0, 8)} re-resolving (failover lane) — standing down`);
            return {};
        }
        if (runId) {
            if (this._housekeepingRuns.size >= 1024) {
                const oldest = this._housekeepingRuns.keys().next().value;
                if (oldest !== undefined)
                    this._housekeepingRuns.delete(oldest);
            }
            // Recorded even when we emit nothing: the turn is still housekeeping
            // and still gets its own tagged row and tool allow-list.
            this._housekeepingRuns.set(runId, kind);
        }
        if (!this._housekeepingLeverOn()) {
            this.logger.info(`Evolve housekeeping: ${kind} for ${String(sessionKey ?? "").slice(0, 24)} left on the ` +
                `session model — lever housekeeping_cheap_rung is off for this bot`);
            return {};
        }
        if (ctx?.userModelOverride)
            return {};
        const picked = resolveHousekeepingModel(this.modelRouter.resolveFastRoleOverride(), this.config.classifierModel);
        if (!picked)
            return {};
        this.logger.info(`Evolve housekeeping: routing ${kind} run ${runId ? runId.slice(0, 8) : "<no-run>"} ` +
            `(session ${String(sessionKey ?? "").slice(0, 24)}) to ${picked.model} ` +
            `(source=${picked.source}) — housekeeping, not the conversation's model`);
        return { ...splitProviderModelRef(picked.model) };
    }
    /** Housekeeping kind for a hook ctx: the routed-run record, else the trigger. */
    _housekeepingKindForRun(ctx) {
        const runId = typeof ctx?.runId === "string" ? ctx.runId : null;
        return (runId ? this._housekeepingRuns.get(runId) : undefined)
            ?? housekeepingKindForTrigger(ctx?.trigger);
    }
    /**
     * Write the housekeeping run's turn row (source = its kind) and drop the
     * session's accumulated llm data so the NEXT conversation turn does not
     * inherit the flush's tokens. Priced exactly like any other turn.
     */
    _recordHousekeepingTurn(sessionId, ctx, kind) {
        const llm = this.sessionLlmData.get(sessionId);
        this.sessionLlmData.delete(sessionId);
        if (typeof ctx?.runId === "string")
            this._housekeepingRuns.delete(ctx.runId);
        if (!llm)
            return;
        const tagged = { ...llm, source: kind };
        const priced = priceCall(tagged.model, tagged.inputTokens, tagged.outputTokens, tagged.cacheWriteTokens, tagged.cacheReadTokens, tagged.provider, this.config.sharedDir);
        this.writeTurnToShared(sessionId, tagged, priced, ctx);
        this.logger.info(`Evolve housekeeping: recorded ${kind} for session ${String(sessionId).slice(0, 8)} ` +
            `(${tagged.model}, in=${tagged.inputTokens} cw=${tagged.cacheWriteTokens} ` +
            `cr=${tagged.cacheReadTokens} out=${tagged.outputTokens}, ` +
            `$${(priced.costUsd ?? 0).toFixed(4)})`);
    }
    /** Lever state, TTL-cached; ON when sharedDir is unknown (the default). */
    _housekeepingLeverOn() {
        if (!this.config.sharedDir)
            return true;
        if (!this._housekeepingLever) {
            this._housekeepingLever = new HousekeepingLeverReader(this.config.sharedDir, this.config.botId);
        }
        return this._housekeepingLever.enabled();
    }
    /**
     ...
     */
    _evoModelOverride(ctx, sessionKey) {
        if (ctx?.userModelOverride)
            return {};
        ...
        if (!model)
            return {};
        this.logger.info(`Evolve evo: forcing grunt model for echo/silent turn ` +
     ...
     */
    /**
     * The daily-cap checkpoint gate for ONE interactive turn.
     *
     * Returns a ``block`` decision when the turn is held, or ``null`` when it
     * should proceed. Every branch either dispatches nothing or emits a fixed
     * string from ``CostCheckpoint`` — there is no path through this method
     * that reaches a model, so no branch adds a spend path.
     *
     * The four cases (D-CC2/D-CC3):
     *   pending  + "continue" from an OWNER → daemon records the grant, the
     *              hold lifts, and the NEXT turn routes normally.
     *   pending  + "continue" from anyone else → the cap message plus "ask
     *              your owner". No extension.
     *   pending  + "stop" → recorded ``declined``.
     *   pending  + anything else → the fixed cap-reached message.
     *   declined + "continue" from an owner → same as the pending case (an
     *              owner may change their mind); otherwise the short refusal.
     *
     * Authorization is NOT decided here. The daemon resolves the answering
     * identity's role from the roster overlay and network.json; this method
     * forwards the platform-captured ``(platform, stable_id)`` and renders
     * whichever answer comes back. A model cannot reach the decision, and a
     * daemon that cannot be reached grants nothing (``renderAnswerUnavailable``)
     * rather than falling back to a local allow.
     */
    async _handleCostCheckpointTurn(userMessage, ctx) {
        const status = readCostCheckpoint({
            sharedDir: this.config.sharedDir,
            botId: this.config.botId,
        });
        if (status === null)
            return null;
        const botName = this.config.botId;
        const answer = classifyCheckpointAnswer(userMessage);
        const blockOf = (message, note) => ({
            outcome: "block",
            category: "evolve.cost_checkpoint",
            reason: `daily cost cap checkpoint (${status.state}): ${note}`,
            message,
            metadata: {
                checkpoint_state: status.state,
                cap_usd: String(status.capUsd ?? ""),
                spend_usd: String(status.spendUsd ?? ""),
                answer: answer ?? "none",
            },
        });
        if (answer === null) {
            return blockOf(status.state === "declined"
                ? renderDeclinedMessage(botName)
                : renderCapReachedMessage(status, botName), status.state === "declined" ? "already declined" : "awaiting an answer");
        }
        const sender = getSender(ctx?.runId);
        const outcome = await this._recordCheckpointAnswer(answer, sender?.platform ?? null, sender?.senderId ?? null);
        if (outcome === "unavailable") {
            return blockOf(renderAnswerUnavailableMessage(), "daemon unreachable");
        }
        if (outcome === "not_owner") {
            this.logger.info(`Evolve cost checkpoint: non-owner "continue" refused for ` +
                `bot=${this.config.botId}`);
            return blockOf(renderNotOwnerMessage(status, botName), "not an owner");
        }
        if (answer === "stop") {
            this.logger.info(`Evolve cost checkpoint: declined for bot=${this.config.botId}`);
            return blockOf(renderDeclinedMessage(botName), "declined by owner");
        }
        this.logger.info(`Evolve cost checkpoint: continued for bot=${this.config.botId} ` +
            `(+$${(status.incrementUsd ?? 0).toFixed(2)})`);
        return blockOf(renderContinuedMessage(status, botName, status.incrementUsd), "continued by owner");
    }
    /**
     * Ask the admin daemon to record a checkpoint answer.
     *
     * Returns ``"recorded"``, ``"not_owner"`` (the daemon resolved the speaker
     * and they are not an owner), or ``"unavailable"`` (socket down, non-2xx,
     * or a raced/absent checkpoint). **Fails closed**: every non-success maps
     * to a refusal that grants nothing, matching the ``require_daemon_call``
     * posture of evo's proposal/signal tools — a fallback that wrote locally
     * would stay inducible by killing the socket.
     */
    async _recordCheckpointAnswer(answer, platform, stableId) {
        let res;
        try {
            res = await adminSocketRequest({
                method: "POST",
                path: "/api/cost-checkpoint/answer",
                body: { answer, platform: platform ?? "", stable_id: stableId ?? "" },
                socketPath: adminDaemonSocketPath(this.config.sharedDir),
            });
        }
        catch (err) {
            this.logger.warn(`Evolve cost checkpoint: daemon call failed: ${err?.message ?? err}`);
            return "unavailable";
        }
        if (res.status < 200 || res.status >= 300) {
            this.logger.warn(`Evolve cost checkpoint: daemon returned ${res.status} — no change`);
            return "unavailable";
        }
        const body = (res.body ?? {});
        if (body.ok !== true)
            return "unavailable";
        return body.authorized === false ? "not_owner" : "recorded";
    }
    async handleBeforeAgentRun(event, ctx) {
        ...
        const _rawUserMessage = event.userMessage ??
            event.message ??
            event.text ??
            ctx?.userMessage ??
            ctx?.message?.content ??
        ...
        const userMessage = unwrapUserMessage(_rawUserMessage);
        // An owner's "continue" after an in-turn checkpoint raises THIS run's
        // checkpoint by the D-CC3 increment (InTurnCheckpoint.noteTurnStart).
        try {
            inTurnCheckpoint.noteTurnStart(ctx?.runId, ctx?.sessionId ?? null, userMessage);
        }
        catch { /* never block the turn */ }
        // ── Daily-cap CHECKPOINT (D-CC1..4) ────────────────────────────────────────
        // Decision: internal/decision-cost-cap-checkpoint-2026-09-04.md.
        //
        // The L1 veto below stops BACKGROUND work; this is the other half of the
        // same trip. When the cap tripped under the default ``checkpoint``
        // action, the first INTERACTIVE turn is held here and answered with a
        // fixed message — no model is resolved, none is dispatched, and the
        // turn's cost is $0 because ``outcome: "block"`` short-circuits the run
        // before OpenClaw reaches model resolution. This is the same mechanism
        // the per-session budget breaker below uses to block user turns.
        //
        // TIER: gated on ``modelRouting``, so it runs at ``manage`` as well as
        // ``full`` — and deliberately ABOVE the ``injectKeywords`` gate. It needs
        // no keyword injection, and a `manage` bot is exactly the case where the
        // gap bites: before the D-CC4 default flip such a bot was at least
        // downgraded on trip, so skipping the hold there would leave it answering
        // every turn at full price with a flag that says the cap is enforced —
        // "warns but never enforces", the anti-pattern the principle names.
        //
        // Placed BEFORE the ``!userMessage`` early return so an empty-bodied
        // interactive turn is held too, and before session-key resolution so a
        // held turn never starts the wizard / evo pipeline.
        //
        // Auto-source turns are excluded: the L1 veto owns them, and it already
        // blocks them with the background wording.
        if (this.config.capabilities.modelRouting) {
            try {
                const _cpTrigger = ctx?.trigger ?? "";
                const _cpChannel = ctx?.channelId
                    ?? event.channelId
                    ?? "";
                if (!isAutoSource({ source: _cpTrigger, channel: _cpChannel })) {
                    const held = await this._handleCostCheckpointTurn(userMessage, ctx);
                    if (held)
                        return held;
                }
            }
            catch (err) {
                // Fail-open, same posture as the two breaker checks below: a bug in
                // the checkpoint layer must never brick the bot's conversation.
                this.logger.warn(`Evolve cost checkpoint check failed (allowing turn): ${err?.message ?? err}`);
            }
        }
        // ── Tier gate: capture-only below tier=full ────────────────────────────────
        // Sender capture above runs at every active tier so the Layer-2 gate can
        // attribute tool calls (audit tier-asymmetry fix). The daily-cap checkpoint
        // above runs wherever ``modelRouting`` does (manage + full). The keyword
        // short-circuit + L1 cost-breaker veto below are tier=full only
        // (injectKeywords); at monitor/manage we capture and PASS — never blocking
        // the turn.
        ...
        try {
            ...
            if (isAutoSource({ source: trigger, channel: channelId })) {
                ...
                if (decision.vetoed) {
                    this._recordBreakerRefusal(trigger || "unknown", decision, `channel=${channelId || "unknown"}, ` +
                        `reason="${decision.reason ?? ""}" (before_agent_run fallback)`);
                    // NO ``message``. Every string this hook returns is wrapped by
                    // OC's ``resolveBlockMessage`` into "Your message could not be
                    // sent: <message> (blocked by evolve)" and posted to the channel
                    // as the turn's promptError — one post per blocked heartbeat,
                    // cron tick and inbound message, which is exactly the flood
                    // removed here. On the fleet build this branch is unreachable
                    // for cron/heartbeat: ``_breakerReplyClaim`` on
                    // ``before_agent_reply`` claims those turns silently BEFORE the
                    // prepared run that hosts this hook. It is kept as the fail-safe
                    // for a build (or a tier) where that hook does not fire — a
                    // still-stopped turn with OC's own generic wording is a much
                    // smaller harm than a spending bot.
                    return {
                        outcome: "block",
                        category: "evolve.cost_breaker",
                        reason: `cost breaker tripped (scope=${decision.scope})`,
                        metadata: {
                            outcome: "breaker_refused",
        ...
        try {
            const earlySessionId = event.sessionId
                ?? ctx?.sessionId
                ?? ctx?.sessionKey
                ?? ctx?.session?.id;
            if (earlySessionId) {
                ...
                if (sessionBreaker) {
                    this.logger.info(`Evolve session-budget breaker BLOCKED turn bot=${this.config.botId} `
                    ...
                    return {
                        ...
                        message: (`Session paused: this conversation exceeded its cost budget `
            ...
            this.logger.warn(`Evolve session-budget breaker check failed (allowing turn): ${err?.message ?? err}`);
        }
        if (!userMessage)
            return { outcome: "pass" };
        ...
        const sessionKey = _candidateKeys[0] ?? "";
        if (!sessionKey)
            return { outcome: "pass" };
        const surface = this.getBetterSurface();
        const botId = this.config.botId;
        ...
        if (!state.wizardSessionId
            ...
            if (channel && senderExternalId) {
                const recovered = await this.evoDispatchClient.findActiveWizard(botId, channel, senderExternalId);
                if (recovered) {
                    state.wizardSessionId = recovered;
                    this.logger.info(`Evolve evo wizard: recovered session=${recovered} for bot=${botId} ` +
        ...
        if (state.wizardSessionId && this.keywordHandler.parseEvoCommand(userMessage) !== null) {
            this.logger.info(`Evolve evo wizard: abandoning session=${state.wizardSessionId} ` +
        ...
        if (state.wizardSessionId) {
            ...
            if (turn === null) {
                ...
                this.logger.warn(`Evolve evo wizard: turn HTTP failed (session=${wsid}) — surfacing honest error, keeping session for retry`);
                const injection = await this._deliverEvoFailure(ctx, surface, "unreachable", { site: "before_agent_run_wizard", phase: "wizard_turn" });
                for (const k of _candidateKeys) {
                    this._pendingKeywordInjection.set(k, injection);
                }
                return { outcome: "pass" };
            }
            let injection = turn.system_append || null;
            let wizardDirectSent = false;
            if (turn.direct_send_message && surface === "member_bot") {
                const sent = await this._sendEvoDirectToTelegram(ctx, null, turn.direct_send_message);
                if (sent) {
                    injection = this.keywordHandler.buildStaySilentInjection(turn.direct_send_message);
                    ...
                    this.logger.info(`Evolve evo wizard: direct-sent verbatim phase=${turn.phase} ` +
                        `(session=${String(wsid).slice(0, 8)})`);
                }
                else {
                    this.logger.info(`Evolve evo wizard: direct-send unavailable for verbatim phase=${turn.phase}, ` +
            ...
            if (turn.wizard_session_id === null) {
                state.wizardSessionId = null;
                this.logger.info(`Evolve evo wizard: session=${wsid} completed (phase=${turn.phase})`);
            }
            return { outcome: "pass" };
        }
        ...
        if (parsedEvo) {
            ...
            const outcome = await this.evoDispatchClient.dispatch(botId, this._channelForSurface(surface), this._extractSenderExternalId(ctx, surface), userMessage);
            ...
            // keep the LLM silent.
            if (!outcome.ok) {
                const injection = await this._deliverEvoFailure(ctx, surface, outcome.reason, { site: "before_agent_run", subcommand: parsedEvo.subcommand, status: outcome.status });
                for (const k of _candidateKeys) {
                    this._pendingKeywordInjection.set(k, injection);
                }
                return { outcome: "pass" };
            ...
            if (dispatchResult.system_append) {
                ...
                if (dispatchResult.wizard_session_id) {
                    ...
                    this.logger.info(`Evolve evo wizard: started session=${dispatchResult.wizard_session_id} ` +
                        `for bot=${botId}`);
                }
                ...
                if (dispatchResult.session_tier_override) {
                    ...
                    this.logger.info(`Evolve evo tier: session=${String(sessionKey).slice(0, 8)} ` +
                ...
                if (dispatchResult.direct_send_message &&
                    surface === "member_bot") {
                    const sent = await this._sendEvoDirectToTelegram(ctx, null, dispatchResult.direct_send_message);
                    if (sent) {
                        injection = this.keywordHandler.buildStaySilentInjection(dispatchResult.direct_send_message);
                        directSent = true;
                        this._markDirectSent(ctx?.runId, dispatchResult.subcommand_brief);
                        this.logger.info(`Evolve evo dispatch: direct-sent subcommand="${dispatchResult.subcommand}" ` +
                            `to Telegram (session=${String(sessionKey).slice(0, 8)})`);
                    }
                    else {
                        this.logger.info(`Evolve evo dispatch: direct-send unavailable for subcommand="${dispatchResult.subcommand}", ` +
                            `falling back to LLM-echo`);
                    }
                }
                ...
                this.logger.info(`Evolve evo dispatch: subcommand=${dispatchResult.subcommand} mode=${dispatchResult.mode} ` +
                ...
                const injection = await this._deliverEvoFailure(ctx, surface, "empty", { site: "before_agent_run", subcommand: parsedEvo.subcommand });
                for (const k of _candidateKeys) {
                    this._pendingKeywordInjection.set(k, injection);
                }
            }
        ...
        if (state.pendingRecId || state.pendingRec) {
            this.logger.info(`Evolve BetterEngine: clearing stale plugin-side pendingRec (session ${String(sessionKey).slice(0, 8)}) — wizard sessions own follow-ups now`);
            state.pendingRecId = null;
            state.pendingRec = null;
        }
     ...
     */
    /**
     * Ask the preflight cap whether this turn may be classified, and emit the
     * operator line on the fire that trips it.
     *
     * Counting is the side effect, so this is called only from the point where
     * a classification would otherwise happen — never as a predicate anyone
     * might reorder ahead of the cheaper checks.
     */
    _preflightAdmitted(ctx, sessionKey) {
        const verdict = this._preflightCap.admit(ctx?.runId, sessionKey);
        if (verdict.logLine)
            this.logger.warn(verdict.logLine);
        return verdict.admitted;
    }
    async handleBeforeModelResolve(ctx, sessionKey, userMessage) {
        // ── Pre-flight intent router (Phase 1 of
        ...
        // layer.
        //
        // Gating: only fires when (a) the bot has the router enabled, (b) we
        // have a non-empty userMessage, AND (c) the trigger is not a heartbeat
        // or cron tick.
        //
        // (c) is D-OH3's other half, and it is a gate on the TRIGGER because the
        // old assumption behind (b) — "heartbeat sessions have an empty
        // userMessage" — is false on OC 2026.9.2: a heartbeat turn arrives with
        // OC's own heartbeat prompt in ``event.prompt`` ("Follow the heartbeat
        // monitor scratch context when provided… reply HEARTBEAT_OK"), which is
        // a perfectly classifiable string. With the classifier gate open, that
        // string reaches the router's haiku layer and spends a model call on the
        // very tick the due-check is about to answer for free. There is nothing
        // to route on a tick whose whole question is "is anything due"; the
        // answer is a stat() call, and the tier is settled by trigger anyway.
        //
        ...
        const _trigger = String(ctx?.trigger ?? "").toLowerCase();
        // The cap is LAST in this chain on purpose: `admit` counts the call, so
        // it must run only where a classification would actually have happened.
        // Over the cap the block is skipped and the turn proceeds UNROUTED —
        // the same outcome as an abstaining router, answered by the bot's
        // configured primary. See preflightCallCap.ts.
        if (userMessage
            && !DUE_CHECK_TRIGGERS.includes(_trigger)
            && this._isPreflightEnabled()
            && this._preflightAdmitted(ctx, sessionKey)) {
            try {
                const decision = await this.preflightRouter.classify({
                    userMessage,
                    botId: this.config.botId,
                    lastAssistantMessage: this._getLastAssistantText(sessionKey),
                    // Same reason as the tier classifier above: this layer picks the
                    // rung that answers THIS turn, so the cost-breaker gate must not
                    // silence it out from under a waiting person.
                    trigger: ctx?.trigger ?? null,
                    // The surface selects the per-surface entry of the learned
                    // prior (D-OH2 item 2) — a bot whose Slack turns are answered
                    // by a different rung than its Telegram turns says so here.
                    surface: this._surfaceForCtx(ctx),
                });
        ...
        if (pendingInjection) {
            ...
            this.logger.info(`Evolve BetterEngine: delivering pending injection for session ${String(sessionKey).slice(0, 8)}`);
        ...
        if (!userMessage) {
            ...
            if (!state.hintFired && !state.evoCalled) {
                ...
                if (hint) {
                    ...
                    this.logger.info(`Evolve BetterEngine: contextual hint fired for session ${String(sessionKey).slice(0, 8)}, rec=${hint.rec_id}`);
                    const hintBlock = `[Discovery hint — use your judgment, do not force:]\n` +
        ...
        if (state.wizardSessionId) {
            ...
            if (turn === null) {
                ...
                // next reply retries.
                this.logger.warn(`Evolve evo wizard: turn HTTP failed (session=${wsid}, before_model_resolve fallback) — surfacing honest error, keeping session for retry`);
                const injection = await this._deliverEvoFailure(ctx, surface, "unreachable", { site: "before_model_resolve_wizard", phase: "wizard_turn" });
                return { ...this._evoModelOverride(ctx, sessionKey), systemAppend: injection };
            }
            if (turn.wizard_session_id === null) {
                state.wizardSessionId = null;
                this.logger.info(`Evolve evo wizard: session=${wsid} completed (phase=${turn.phase}, before_model_resolve fallback)`);
            }
            ...
            if (injection) {
                ...
                this.logger.info(`Evolve evo wizard: turn handled (session=${String(wsid).slice(0, 8)}, phase=${turn.phase}, before_model_resolve fallback)`);
                return {
        ...
        if (parsedSub) {
            ...
            if (!outcome.ok) {
                const injection = await this._deliverEvoFailure(ctx, surface, outcome.reason, { site: "before_model_resolve", subcommand: parsedSub.subcommand, status: outcome.status });
                return { ...this._evoModelOverride(ctx, sessionKey), systemAppend: injection };
            }
            const dispatchResult = outcome.result;
            if (dispatchResult.wizard_session_id) {
            ...
            if (dispatchResult.system_append) {
                ...
                if (dispatchResult.direct_send_message &&
                    ...
                    if (sent) {
                        ...
                        return {
                            ...this._evoModelOverride(ctx, sessionKey),
                            systemAppend: this.keywordHandler.buildStaySilentInjection(dispatchResult.direct_send_message),
                        };
                    }
                    this.logger.info(`Evolve evo dispatch: direct-send unavailable for subcommand=${dispatchResult.subcommand} ` +
                ...
                let llmDirective = null;
                if (dispatchResult.direct_send_message) {
                    llmDirective = this._llmEchoVerbatimInstruction(dispatchResult.direct_send_message);
                }
            ...
            const emptyInjection = await this._deliverEvoFailure(ctx, surface, "empty", { site: "before_model_resolve", subcommand: parsedSub.subcommand });
        ...
        if (state.pendingRecId && state.pendingRec) {
            ...
            if (action !== null) {
                const followUp = await this.keywordHandler.handleFollowUp(action, state.pendingRec, botId, surface);
                ...
                this.logger.info(`Evolve BetterEngine: follow-up action="${action}" (legacy path) for session ${String(sessionKey).slice(0, 8)}`);
     ...
     */
    loadMatchingHint(botId, userMessage) {
        try {
            const hintsPath = path.join(`/Users/${botId}/.openclaw/workspace/evolve`, "rec-hints.json");
            let raw;
            try {
            ...
            for (const hint of hints) {
                // Only eligible types
                if (!eligibleTypes.has(hint.type))
                    continue;
                const triggers = hint.triggers ?? [];
                const matched = triggers.some((trigger) => normalizedMsg.includes(trigger.toLowerCase()));
                if (!matched)
                    continue;
    ...
    writeTurnToShared(sessionId, llm, priced = { costUsd: null, costSource: "unpriced" }, ctx, appAttribution) {
        ...
        if (!this._initializedDirs.has(turnsDir)) {
            try {
                ...
                try {
                    fs.chmodSync(sharedDir, 0o1777);
                }
                catch { /* not our dir to chmod */ }
                ...
                if (mkdirErr?.code === "EACCES") {
                    this.logger.warn(`Evolve: cannot write turns — ${turnsDir} is not writable. ` +
        ...
        try {
            ...
            try {
                ...
                if (!userId) {
                    try {
                        ...
                        if (_captured?.senderId)
                            userId = _captured.senderId;
                    }
            ...
            const record = {
                ...
                cache_write_tokens: llm?.cacheWriteTokens ?? 0,
                cache_read_tokens: llm?.cacheReadTokens ?? 0,
                // ``null`` — never 0 — when nothing could price this model: a zero
                // reads as "free" to every downstream sum, which is the silent zero
                // docs/principle-tri-state-status.md forbids. ``cost_source`` says
                // which of the four resolutions produced the number, so a reader can
                // prefer the catalog over a table estimate (turn_cost.turn_cost).
                cost: priced.costUsd,
     ...
     */
    _writeStruggleSample(event, signal, sessionId) {
        if (!signal)
            return;
        ...
        if (this._struggleSamplesToday >= STRUGGLE_SAMPLE_DAILY_CAP)
            return;
        ...
        const dir = path.join(this.config.sharedDir, this.config.botId, "cascade", "struggle-debug");
        if (!this._initializedDirs.has(dir)) {
            try {
                ...
                const code = err?.code;
                if (code === "EACCES" || code === "EPERM") {
                    this.logger.debug(`Evolve: struggle-debug dir for ${this.config.botId} owned by another gateway, skipping`);
                    return;
                }
    ...
    writeAnnotation(annotation) {
        ...
        const annotationDir = path.join(this.config.sharedDir, "annotations", this.config.botId);
...
//# sourceMappingURL=TurnObserver.js.map