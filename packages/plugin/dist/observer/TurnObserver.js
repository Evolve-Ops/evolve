...
import { ModelRouter, synthesizeRungsRoles, normalizeRouting, mergeModelCatalog, parseTierDirective, } from "./ModelRouter.js";
 ...
 * task_extractor.py) — from plugin config.
 *
 ...
 *
 * Base-spec §8.4 steps 3/4 (docs/spec-manifest-v7-2026-05-20.md): any
...
export class TurnObserver {
     ...
     * Same shape/discipline as the capability block: caches success AND failure for
     * the TTL so the hot path issues at most one socket round-trip per window, and
     * a directory-read fault degrades to "" (today's behavior) instead of throwing.
     */
    _dirDigestBlock = null;
    ...
    constructor(config, logger, api) {
        ...
        // Cascade telemetry — Phase 1 of docs/spec-tier-cascade-2026-05-26.md.
    ...
    loadModelRouterConfig() {
        ...
        const roleCaps = explicitCaps
            ?? (typeof legacyOverride?.dailyCap === "number"
                ? { power: { maxPerDayPerBot: legacyOverride.dailyCap } }
                : undefined);
        return {
    ...
    register(api) {
        ...
        api.on("llm_output", async (event, ctx) => {
            try {
                ...
                this.sessionLlmData.set(sessionId, {
                    ...
                    channel: ctx?.channelId ?? existing?.channel ?? "unknown",
                ...
                try {
                    ...
                    const callCost = estimateCost(String(model || ""), Number(usage?.input ?? 0), Number(usage?.output ?? 0), Number(usage?.cacheWrite ?? 0), Number(usage?.cacheRead ?? 0));
            ...
            api.on("before_prompt_build", async (_event, ctx) => {
                try {
                    ...
                    if (runId && this._directSentRuns.has(runId)) {
                        ...
                        return {
                            appendSystemContext: this._stayQuietSystemContext(brief),
                        };
                    }
                    // ── LLM-directive path ─────────────────────────────────────────
                    ...
                    // refused. Spec: docs/spec-user-roster-and-roles-2026-06-07.md
                    ...
                    // docs/spec-bot-capability-awareness-2026-06-22.md §5.
                    ...
                    // turn. Spec: docs/spec-user-directory-2026-06-22.md §5.
                    let directoryDigest = "";
                    try {
                        directoryDigest = await this._renderDirectoryDigestBlock();
                    }
                    catch { /* never let directory digest render break the turn */ }
                    const combined = [capabilitiesBlock, directoryDigest, narrative, speakerBlock]
        ...
        if (!caps.modelRouting && !caps.injectKeywords) {
            // Capture-side hooks above are sufficient at this tier; nothing else to
            // register. before_agent_run is also skipped below for the same reason.
            return;
        }
        api.on("before_model_resolve", async (event, ctx) => {
            try {
                ...
                // (docs/spec-user-tier-control-2026-05-26.md). Two transports,
                // checked in order:
                ...
                // docs/spec-agent-freelance-bypass-phase2-2026-06-06.md.
        ...
        // ── before_agent_run (keyword short-circuit) ─────────────────────────────
        // Registered with try/catch — gracefully degrades on gateway versions that
        // don't support this hook yet. When supported, keywords are handled with
        // zero tokens and zero LLM latency. The before_model_resolve path above
        // remains the active fallback until then. Skipped entirely below tier=full
        // — its sole purpose is keyword detection for the injection pipeline.
        if (caps.injectKeywords) {
            try {
                ...
                this.logger.info("Evolve: before_agent_run hook registered — keyword intercept active (zero-token path)");
            }
            catch {
    ...
    async handleSessionStart(event) {
        ...
        if (caps.injectPodConduct) {
            ...
            const args = [
                ...
                // load_help_sidebar_block. Spec: docs/spec-primary-bot-interface-
                // 2026-05-14.md §4.2, §7.
    ...
    // ── Layer C: manifest trigger interception ──────────────────────────────
    //
    // Phase 2.3 of docs/spec-agent-freelance-bypass-phase2-2026-06-06.md.
    ...
    _compileTrigger(t, manifest, workspaceRoot) {
        ...
        const requestPayload = (invocation.request_payload && typeof invocation.request_payload === "object")
            ...
            : {};
        const appId = (typeof manifest?.pkg_id === "string" && manifest.pkg_id)
    ...
    // LLM (see docs/diagnosis-evo-briefing-context-gap-2026-05-26.md and the
    ...
    _renderPerTurnNarrativeBlock() {
        ...
        if (generatedAt) {
            lines.push("");
            lines.push(`(Generated ${generatedAt}. May be moments older than the ` +
    ...
    // ── Per-turn [INSTALLED CAPABILITIES] injection ────────────────────────────
    //
    // Spec: docs/spec-bot-capability-awareness-2026-06-22.md §5 (P1 delivery).
    ...
    async _renderCapabilitiesBlock() {
        ...
        if (!this.config.capabilities.injectPodConduct)
            return "";
        const now = Date.now();
        if (this._capBlock && now - this._capBlock.at < TurnObserver._CAP_BLOCK_TTL_MS) {
            return this._capBlock.text;
        }
        ...
        this._capBlockInflight = (async () => {
            ...
            // Cache success AND failure for the TTL — a transient fault shouldn't
            // spawn Python on every subsequent turn.
            this._capBlock = { text, at: Date.now() };
    ...
    async _renderDirectoryDigestBlock() {
        if (!this.config.capabilities.injectPodConduct)
            return "";
        const now = Date.now();
        if (this._dirDigestBlock &&
            now - this._dirDigestBlock.at < TurnObserver._DIR_DIGEST_TTL_MS) {
            return this._dirDigestBlock.text;
        }
        ...
        this._dirDigestInflight = (async () => {
            ...
            try {
                ...
                if (res.status === 200 && res.body && typeof res.body === "object") {
                    ...
                    // non-JSON body — never inject a partial/garbled block; degrade to "".
                ...
                // the turn. Cached as "" below so a transient fault doesn't hammer the
                // socket every turn.
                this.logger.warn(`Evolve directory digest render failed: ${err?.message ?? err}`);
                text = "";
            }
            this._dirDigestBlock = { text, at: Date.now() };
    ...
    _buildSpeakerContextBlock(ctx) {
        ...
        // G-N2, #3378). Fall back to telegram only when the platform wasn't
        // threaded, preserving prior behavior without regressing Telegram.
        const platform = sender.platform || "telegram";
    ...
    async handleTurn(event, ctx) {
        ...
        const costEstimated = estimateCost(modelSelected, llm?.inputTokens ?? 0, llm?.outputTokens ?? 0, llm?.cacheWriteTokens ?? 0, llm?.cacheReadTokens ?? 0);
        ...
        const annotation = {
            ...
            // schema_version 4 (was 3): adds model_role alongside model_tier per
            // spec-model-rungs-and-roles-2026-06-09 (readers accept both during
            // the transition). schema_version 3 added user_pushback_* per
            // spec-user-pushback-signal-2026-05-30. Consumers tolerate unknown
            // fields via .get(field, default); no consumer gates on the integer.
            schema_version: 4,
            turn_id: turnId,
        ...
        try {
            ...
            this.writeTurnToShared(sessionId, llm, costEstimated, ctx);
    ...
    async handleSessionEnd(event, ctx) {
        ...
        if (!turns || turns.length === 0)
            return;
        // Run session summarizer and task extractor in parallel
        const summaryPromise = this.summarizer.summarize(sessionId, turns, (record) => {
            this.writeAnnotation(record);
        });
        const extractPromise = this.runTaskExtractor(sessionId, turns);
        await Promise.allSettled([summaryPromise, extractPromise]);
    ...
    async runTaskExtractor(sessionId, turns) {
        // Only extract tasks if the feature is enabled
        if (!this.config.enableTaskExtraction)
            return;
        // Convert TurnRecord[] to the plain object format task_extractor.py expects
        const turnDicts = turns.map((t) => ({
            role: t.role,
            content: t.content ?? "",
        ...
        const execFileAsync = promisify(execFile);
        const analyzerDir = resolveAnalyzerDir(this.config);
        const scriptPath = path.join(analyzerDir, "task_extractor.py");
        const networkPath = path.join(this.config.sharedDir, "network.json");
        // Pass turns as JSON on stdin, use --stdin flag
        const turnsJson = JSON.stringify(turnDicts);
        // Write turns to a temp file (avoid shell arg size limits)
        const fs = await import("fs");
        const os = await import("os");
        // Verify script exists before attempting to spawn — gives a clear error instead
        // of a cryptic python3 "no such file" failure buried in the catch block.
        try {
            await fs.promises.access(scriptPath, fs.constants.R_OK);
        }
        catch {
            this.logger.warn(`Evolve task extractor not found at ${scriptPath} — skipping extraction`);
            return;
        }
        const tmpFile = path.join(os.tmpdir(), `evolve-turns-${sessionId}.json`);
        await fs.promises.writeFile(tmpFile, turnsJson);
        try {
            await execFileAsync(evolvePythonBin(), [
                scriptPath,
                "--shared-dir", this.config.sharedDir,
                "--network", networkPath,
                "--bot", this.config.botId,
            ...
            this.logger.warn(`Evolve task extractor error for session ${sessionId}: ${err}`);
        }
        finally {
            try {
    ...
    async handleBeforeAgentRun(event, ctx) {
        ...
        // actual speaker — e.g. roster_set_role per docs/spec-user-roster-and-roles-
        ...
        try {
            ...
            captureSender(ctx?.runId, {
                ...
                // Real platform of the sender. OC threads the channel-TYPE name
                // (e.g. "telegram"/"slack", NOT the numeric chat id — see
                // _handleEvoFallback) as channelId on ctx/event; captureSender
                // normalizes it to one of the four roster platforms. Threaded so
                // the roster tools + speaker-context resolve the correct
                // (platform, id) key off-Telegram (audit R1a G-N2, #3378).
                platform: ctx?.channelId ?? _evt.channelId ?? null,
                senderIsOwner: _evt.senderIsOwner ?? null,
            });
        }
        catch { /* never crash the hot path over diagnostic capture */ }
        // ── L1 cost-breaker veto (Phase 3b) ────────────────────────────────────────
        // Spec: docs/spec-circuit-breakers-2026-05-21.md §5.2.
    ...
    writeTurnToShared(sessionId, llm, costEstimated = 0, ctx) {
