...
export class TurnObserver {
    ...
    register(api) {
        ...
        if (caps.injectPodConduct || caps.injectKeywords) {
            api.on("before_agent_reply", async (_event, ctx) => this.handleBeforeAgentReply(ctx?.runId), { name: "evolve-before-agent-reply" });
    ...
    resolveModelRouting(ctx, sessionKey) {
        ...
        if (!this.modelRouter.getSessionType(sessionKey)) {
            ...
            if (anchoredClass) {
                ...
                this.logger.info(`Evolve ModelRouter: pre-classified ${String(sessionKey).slice(0, 8)} ` +
    ...
    async handleBeforeAgentRun(event, ctx) {
        ...
        try {
            ...
            if (isAutoSource({ source: trigger, channel: channelId })) {
                ...
                if (decision.vetoed) {
                    this.logger.info(`Evolve cost breaker BLOCKED auto-source turn (scope=${decision.scope}, ` +
                    ...
                    return {
                        ...
                        message: (`Background activity paused: cost circuit breaker is tripped. ` +
