 ...
 */
export const HOUSEKEEPING_TOOLS_ALLOW = Object.freeze(["read", "write"]);
/**
 ...
 */
export function housekeepingKindForTrigger(trigger) {
    const t = typeof trigger === "string" ? trigger.trim().toLowerCase() : "";
    if (t === "memory" || t === "memory_flush")
        return "memory_flush";
    if (t === "compaction")
        return "compaction";
    return null;
...
export class HousekeepingLeverReader {
    ...
    _value = null;
    _checkedAt = 0;
    constructor(sharedDir, botId, now = Date.now) {
    ...
    enabled() {
        const t = this.now();
        if (this._value !== null && t - this._checkedAt < LEVER_CACHE_TTL_MS)
            return this._value;
        let value = true;
        try {
 ...
 */
export function resolveHousekeepingModel(fastRoleModel, classifierModel) {
