...
export class SessionCostMonitor {
    ...
    breakerFilePath(sessionId) {
        const sanitized = sessionId
            .replace(/[^A-Za-z0-9_-]/g, "_")
            .slice(0, 64);
        return path.join(this.sharedDir, "breakers", this.botId, `session-${sanitized}.json`);
    }
    ...
    peek(sessionId) {
        ...
        if (!b)
            return { accumulatedUsd: 0, tripped: false };
    ...
    recordCost(opts) {
        ...
        if (opts.userId)
            bucket.lastUserId = opts.userId;
        if (opts.channelId)
            bucket.lastChannelId = opts.channelId;
        if (opts.channelKind)
            bucket.lastChannelKind = opts.channelKind;
        ...
        if (!alreadyTripped
            ...
            const rec = {
                ...
                reason: (`Session cost $${round6(bucket.accumulatedUsd).toFixed(4)} `
    ...
    pruneIdle(maxAgeMs, now = new Date()) {
...
export function readSessionBudgetCap(opts) {
    ...
    if (!isPlainObject(defaults))
        return null;
    const cap = defaults.sessionBudgetCapUsd;
    if (cap === null || cap === undefined)
        return null;
...
export function readSessionBudgetBreaker(opts) {
    const sanitized = opts.sessionId
        .replace(/[^A-Za-z0-9_-]/g, "_")
        .slice(0, 64);
    const filePath = path.join(opts.sharedDir, "breakers", opts.botId, `session-${sanitized}.json`);
    let text;
    try {
...
//# sourceMappingURL=SessionCostMonitor.js.map