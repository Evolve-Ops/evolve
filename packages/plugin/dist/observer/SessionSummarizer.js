...
export class SessionSummarizer {
    ...
    async summarize(sessionId, turns, writeFn) {
        ...
        // LLM outcome extraction — optional, falls back to transcript inference.
        //
        // Gated OFF by default since D-OH2/D-OH4: "summaries come from
        // transcripts in batch." With the gate closed there is no live
        // subagent call at session end at all — ``inferOutcome`` reads the
        // outcome off the transcript this method already holds, which is
        // the same source the LLM was being handed, minus a model call, a
        // cache write and 15s of wall clock per session. The gate is the
        // whole switch; ``enableLLMSummarization`` still works as the
        // per-bot opt-out it always was.
        //
        ...
        const llmSummaryAllowed = this.config.enableLLMSummarization !== false && this._gate.read().enabled;
        if (llmSummaryAllowed && turns.length >= minTurns) {
            try {
        ...
        else if (llmSummaryAllowed && turns.length < minTurns) {
