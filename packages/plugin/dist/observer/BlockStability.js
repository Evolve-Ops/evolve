...
export class StickyBlockCache {
    ...
    staleAgeMs(now) {
        if (!this.entry || this.entry.goodAt === null)
            return null;
 ...
 */
export function renderHomeNarrativeBlock(text, generatedAt) {
    const lines = [
 ...
 */
export const EVOLVE_PREFIX_TAIL_BLOCKS = [
    "speaker",
    "costDowngrade",
];
...
function joinBlocks(blocks, names) {
    return names
        .map((n) => blocks[n])
        .filter(isPresent)
 ...
 */
export function composeEvolvePrefix(blocks) {
    const headNames = EVOLVE_PREFIX_BLOCK_ORDER.filter((n) => !EVOLVE_PREFIX_TAIL_BLOCKS.includes(n));
...
//# sourceMappingURL=BlockStability.js.map