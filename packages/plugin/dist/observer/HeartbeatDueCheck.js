...
export const DUE_CHECK_TRIGGERS = ["heartbeat", "cron"];
 ...
 */
function parseConditionList(rawConditions, label, keyPrefix) {
    if (rawConditions !== undefined && !Array.isArray(rawConditions)) {
        return { error: `${label} must be an array` };
    }
    const conditions = [];
    const seen = new Set();
    for (const [i, entry] of (rawConditions ?? []).entries()) {
        ...
        if (!id)
            return { error: `${label}[${i}] needs a non-empty 'id'` };
        if (seen.has(id))
            return { error: `duplicate condition id ${JSON.stringify(id)} in ${label}` };
        seen.add(id);
        const when = typeof c.when === "string" ? c.when.trim() : "";
        if (!CONDITION_KINDS.includes(when)) {
            return {
                error: `condition ${JSON.stringify(id)} has unknown 'when' ${JSON.stringify(c.when)} ` +
        ...
        if (!wake) {
            return {
                error: `condition ${JSON.stringify(id)} needs a 'wake' string — it becomes the ` +
                    `model's instruction when this condition is what woke it`,
            };
        }
        const cond = { id, when, wake, stateKey: `${keyPrefix}${id}` };
        if (when === "time_window") {
            ...
            if (after >= before) {
                return {
                    error: `condition ${JSON.stringify(id)}: 'after' (${c.after}) must be earlier ` +
            ...
            if (parseInterval(c.interval) === null) {
                return {
                    error: `condition ${JSON.stringify(id)} needs an 'interval' like "30m", "6h" or "1d"`,
                };
            }
            ...
            if (!rel)
                return { error: `condition ${JSON.stringify(id)} needs a 'path'` };
            const bad = unsafeRelativePath(rel);
            if (bad)
                return { error: `condition ${JSON.stringify(id)} path ${JSON.stringify(rel)}: ${bad}` };
            cond.path = rel;
        }
...
export function cronStateKeyPrefix(jobId) {
    return `cron:${jobId}:`;
}
/**
 ...
 */
export function parseDueConditions(raw) {
    let doc;
    try {
        doc = JSON.parse(raw);
    }
    ...
    if ("error" in heartbeat)
        return { error: heartbeat.error };
    const cron = {};
    const rawCron = obj.cron;
    if (rawCron !== undefined) {
        if (typeof rawCron !== "object" || rawCron === null || Array.isArray(rawCron)) {
            return {
                error: "'cron' must be an object mapping each cron job's id to its own " +
                    "conditions — the heartbeat's conditions never gate a cron job",
            };
        }
        for (const [jobId, rawList] of Object.entries(rawCron)) {
            const key = jobId.trim();
            if (!key)
                return { error: "'cron' has an empty job id key" };
 ...
 */
export function unsafeRelativePath(rel) {
    if (path.isAbsolute(rel))
        return "must be relative to the workspace, not absolute";
 ...
 */
export function evaluateConditions(conditions, workspaceDir, state, now, probe = realFsProbe) {
    const due = [];
    const nextState = {
        version: 1,
    ...
    for (const c of conditions) {
        ...
        switch (c.when) {
            ...
            case "path_missing":
                if (!probe.exists(abs))
                    fired = `${c.path} is missing`;
                break;
            case "dir_non_empty": {
                if (probe.isDirectory(abs)) {
                    const n = probe.entryCount(abs);
                    if (n > 0)
                        fired = `${c.path} holds ${n} ${n === 1 ? "entry" : "entries"}`;
                }
                break;
            }
            case "file_changed": {
                ...
                if (mtime === null)
                    break; // absent file cannot have changed
            ...
            case "time_window": {
                ...
                if (Number.isFinite(lastMs) && lastMs >= windowOpenedMs)
                    break;
 ...
 */
export function buildWakeContext(due) {
    const lines = [
    ...
    lines.push("", "Nothing else on the checklist is due. If none of the above needs a " +
...
export function formatSilenceWindow(ms) {
    if (ms % 3_600_000 === 0)
 ...
 */
export function readMaxSilenceMs(sharedDir, botId) {
    try {
...
export class HeartbeatDueCheck {
    ...
    shaChangeWarned = new Set();
    maxSilenceOverride;
    cachedMaxSilenceMs = null;
    maxSilenceCheckedAt = 0;
    static MAX_SILENCE_CACHE_TTL_MS = 60_000;
    constructor(opts) {
    ...
    maxSilenceMs() {
        if (this.maxSilenceOverride)
            return this.maxSilenceOverride();
        const nowMs = Date.now();
        if (this.cachedMaxSilenceMs !== null &&
    ...
    evaluate(workspaceDir, scope) {
        ...
        if ("error" in parsed) {
            return {
                ...
                warn: `Evolve heartbeat: ${cfgPath} is invalid — ${parsed.error}. Running the ` +
        ...
        const { due, nextState } = evaluateConditions(conditions, ws, state, now, this.probe);
        ...
        if (due.length === 0) {
            ...
            const silentTooLong = !Number.isFinite(lastWokeMs) || now.getTime() - lastWokeMs >= floorMs;
     ...
     */
    noteConditionsDigest(cfgPath, raw) {
        ...
        if (prior && prior !== sha && !this.shaChangeWarned.has(cfgPath)) {
            ...
            this.logger.warn(`Evolve heartbeat: ${cfgPath} changed while the gateway was running ` +
...
//# sourceMappingURL=HeartbeatDueCheck.js.map