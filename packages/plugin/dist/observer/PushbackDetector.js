 ...
 *
 * Spec: docs/spec-user-pushback-signal-2026-05-30.md.
 *
 ...
 */
// ── Defaults ─────────────────────────────────────────────────────────────────
export const DEFAULT_PUSHBACK_WEIGHTS = {
 ...
 */
export function countClarificationHits(text) {
    if (!text)
        return 0;
    let n = 0;
...
export function tokenizeForJaccard(text) {
    ...
    for (const tok of cleaned.split(/\s+/)) {
        if (tok.length < 2)
            continue; // single letters add noise
 ...
 */
export function isShortFollowup(currentText, priorAssistantText) {
    const t = (currentText ?? "").trim();
    if (t.length === 0)
        return false;
    if (t.length > SHORT_FOLLOWUP_MAX_CHARS)
        return false;
    if (t.includes("?"))
        return false;
    const prior = (priorAssistantText ?? "").trim();
    if (prior.length < SHORT_FOLLOWUP_MIN_PRIOR_ASSISTANT_CHARS)
        return false;
    return true;
}
// ── Combiner ─────────────────────────────────────────────────────────────────
 ...
 * host anyway.
 */
export function computePushback(input) {
    // ── DNT check (spec § 6) ──────────────────────────────────────────────
    ...
    let score = contrib.clarification_regex +
...
//# sourceMappingURL=PushbackDetector.js.map