...
const KNOWN_PLATFORMS = ["telegram", "slack", "discord", "whatsapp"];
/**
...
export function normalizePlatform(raw) {
    ...
    for (const p of KNOWN_PLATFORMS) {
        if (v === p || v.startsWith(`${p}_`) || v.startsWith(`${p}:`))
            return p;
    }
    return null;
}
 ...
 */
export function captureSender(runId, fields) {
    if (!runId)
        return;
    if (!fields.senderId)
        return;
 ...
 */
export function getSender(runId) {
    if (!runId)
        return null;
...
//# sourceMappingURL=senderRegistry.js.map