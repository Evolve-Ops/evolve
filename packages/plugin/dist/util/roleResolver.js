...
function overlayPath(sharedDir, botId) {
    return path.join(sharedDir, "rosters", `${botId}.json`);
}
function networkPath(sharedDir) {
...
function podAdminIdsFor(network, platform) {
    const pod = (network && network.pod) || {};
    const admins = pod.admins || {};
    const ext = admins.external_ids || {};
    const list = Array.isArray(ext[platform]) ? ext[platform] : [];
    return new Set(list.map((x) => String(x)).filter(Boolean));
}
function primaryOwnerIdFor(network, botId, platform) {
    const bots = (network && network.bots) || {};
    const bot = bots[botId] || {};
    const primary = bot.primary_user || {};
    const ext = primary.external_ids || {};
    const val = ext[platform];
    return val ? String(val) : null;
}
function identityKey(platform, stableId) {
 ...
 */
export function resolveSpeakerRole(botId, platform, stableId, opts = {}) {
    ...
    if (entry && typeof entry.role === "string") {
        const explicit = entry.role;
        if (explicit === "admin" || explicit === "primary_user" ||
    ...
    if (stableId === primaryOwnerIdFor(network, botId, platform)) {
        return {
            role: "primary_user",
 ...
 */
export function buildSpeakerContextBlock(resolution, identity) {
 ...
 */
export function renderDirectoryDigestBlock(digest) {
    ...
    for (const p of rows) {
        ...
        const head = [p.display || p.handle || p.identity || p.person_id];
        if (p.handle)
            head.push(p.handle);
        if (p.identity)
            head.push(`[${p.identity}]`);
        if (p.primary_email)
            head.push(p.primary_email);
        lines.push(`  • ${head.join("  ")} · ${p.role}`);
    }
    const overflow = Math.max(0, (digest?.total ?? rows.length) - rows.length);
    if (overflow > 0) {
        lines.push(`  (+${overflow} more not shown — call directory_lookup(@handle | name | email) ` +
    ...
    lines.push("To save or correct anyone's email, messaging ID, or contact details, call " +
