...
import { createDescribeBotToolFactory, createListAuditsToolFactory, createListProposalsToolFactory, createListSignalsToolFactory, createPodStatusToolFactory, createRecentTurnsToolFactory, createRecentWatchdogToolFactory, createSpendRollupToolFactory, } from "./tools/PodStateTools.js";
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        if (config.role === "primary") {
            ...
            // ── Pod-state read tools (Bundle 3) ─────────────────────────────────────
            // Grounded reads against {shared_dir}/* via /api/primary/state/*, over the
            // same admin-daemon unix socket.
            // Spec: docs/spec-primary-bot-interface-2026-05-14.md §5.
            api.registerTool(createPodStatusToolFactory(api.logger, adminSocketPath));
            api.registerTool(createListSignalsToolFactory(api.logger, adminSocketPath));
            api.registerTool(createListProposalsToolFactory(api.logger, adminSocketPath));
            api.registerTool(createRecentWatchdogToolFactory(api.logger, adminSocketPath));
            api.registerTool(createSpendRollupToolFactory(api.logger, adminSocketPath));
            api.registerTool(createRecentTurnsToolFactory(api.logger, adminSocketPath));
            api.registerTool(createDescribeBotToolFactory(api.logger, adminSocketPath));
            api.registerTool(createListAuditsToolFactory(api.logger, adminSocketPath));
        }
