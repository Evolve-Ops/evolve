...
import { createHelpReadToolFactory, createHelpSearchToolFactory, createSubmitIntakeToolFactory, } from "./tools/PrimaryBotTools.js";
import { createPodStateToolFactory } from "./tools/PodStateTools.js";
import { createRosterSetRoleToolFactory, createRosterBlockToolFactory, createRosterUnblockToolFactory, createChannelSetNewcomerModeToolFactory, } from "./tools/RosterTools.js";
import { adminDaemonSocketPath } from "./util/adminSocket.js";
import { createGoogleToolFactories, googleConfiguredForBot, } from "./tools/GoogleTools.js";
import { createDirectoryLookupToolFactory, createDirectoryUpsertToolFactory, } from "./tools/DirectoryTools.js";
import { createUsersWhoamiToolFactory, createUsersListToolFactory, } from "./tools/UsersTools.js";
import { BOARD_VERBS, createBoardToolFactory } from "./tools/BoardTool.js";
import { registerAppTool } from "./apps/contract/register.js";
...
import { configureSubagentBreakerGate } from "./observer/subagentRun.js";
import { ExecFailureAbsorber } from "./observer/ExecFailureAbsorber.js";
import { createExpandAppToolFactory } from "./tools/ExpandAppTool.js";
import { installToolProfileFilter } from "./tools/ToolProfiles.js";
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        if (_registeredApis.has(api)) {
            api.logger.warn(`Evolve register() called twice on same api instance — skipping duplicate init (bot=${config.botId})`);
            return;
        }
        _registeredApis.add(api);
        api.logger.info(`Evolve starting — bot=${config.botId} role=${config.role} network=${config.networkId} tier=${config.tier}`);
        // ── Tier gate ─────────────────────────────────────────────────────────────
        ...
        if (!config.capabilities.observer) {
            api.logger.info(`Evolve: tier=${config.tier} — skipping plugin-side integration for ${config.botId}`);
            return;
        }
        // ── Per-session-type tool profiles (context-economy CE-2b) ────────────────
        // Wraps api.registerTool so each factory resolves the CALLING session's
        // kind from its sessionKey and, for a background/one-shot/Evolve-dispatch
        // session, registers a one-line stand-in instead of the full schema for
        // any tool that session's profile does not carry. `default: full` — a
        // user session, a subagent run, and anything that does not classify keep
        // every tool unchanged. A trimmed tool still exists and refuses by name,
        // so a call is never a silent "tool not found".
        // Design: internal/design-pa-context-economy-2026-08-31.md §3 CE-2.
        //
        // Installed BEFORE the footprint wrap on purpose: the footprint must see
        // the ORIGINAL factories so it can weigh each one under every profile.
        installToolProfileFilter(api, { botId: config.botId, sharedDir: config.sharedDir }, api.logger);
        ...
        // ── Cost-breaker gate on Evolve's own model calls ─────────────────────────
        // Points runPinnedSubagent (the single funnel for the tier classifier,
        // struggle judge, preflight router and session summarizer) at this bot's
        // breaker files, so a tripped cost breaker stops Evolve's spending too and
        // not just the bot's replies. Reads the same files the turn veto reads, so
        // reactivation lifts both at once. Wired BEFORE the observer constructs
        // those helpers — an unconfigured gate is inert, which would spend.
        configureSubagentBreakerGate({ sharedDir: config.sharedDir, botId: config.botId }, api.logger);
        // ── Turn observer ─────────────────────────────────────────────────────────
        ...
        configureAppAttribution({ sharedDir: config.sharedDir, botId: config.botId }, api.logger);
        // ── HTTP routes ───────────────────────────────────────────────────────────
        ...
        // Spec: internal/spec-app-invocation-just-works-2026-06-29.md §2.2.
        ...
        // ── Exec-failure channel hygiene (A1) ─────────────────────────────────────
        // message_sending hook that recognizes OpenClaw's raw exec-failure
        // trailers (`⚠️ 🛠️ Exec failed: …` and kin) on CHANNEL-BOUND payloads,
        // ledgers every match to {shared}/{bot}/exec-failures/, and — only when
        // armed via ``execFailureAbsorb: true`` — strips/cancels them so the
        // operator never sees a raw trailer. Observe-only by default; merging is
        // non-arming. Model-visible tool results are untouched (the bot still
        // adapts to its own failures). Registered for every active-tier bot —
        // channel hygiene is a fleet-wide guarantee, not a role/feature surface.
        // Design: internal/design-exec-failure-hygiene-2026-08-31.md.
        new ExecFailureAbsorber({
            sharedDir: config.sharedDir,
            botId: config.botId,
            armed: config.execFailureAbsorb,
            armedWarning: config.execFailureAbsorbWarning,
        }, api.logger).register(api);
        ...
        if (config.capabilities.modelRouting) {
            api.registerTool(createSetTierToolFactory({ botId: config.botId, modelRouter: observer.getModelRouter() }, api.logger));
        }
        ...
        if (config.capabilities.recordApplicationTool) {
            api.registerTool(createRecordApplicationToolFactory({ botId: config.botId }, api.logger));
        }
        // ── Primary-bot interface tools ───────────────────────────────────────────
        // Bundle 2B of internal/spec-primary-bot-interface-2026-05-14.md. Only the
        ...
        if (config.role === "primary") {
            ...
            api.registerTool(createSubmitIntakeToolFactory({ botId: config.botId, socketPath: adminSocketPath }, api.logger));
            ...
            // internal/spec-evolve-overhead-budget-2026-07-31.md §B2.
            api.registerTool(createPodStateToolFactory(api.logger, adminSocketPath));
        }
        // ── Roster admin tools (Phase C.2 / Path B) ────────────────────────────
        // Spec: internal/spec-user-roster-and-roles-2026-06-07.md §10.
        //
        ...
        api.registerTool(createRosterSetRoleToolFactory({ botId: config.botId, socketPath: adminSocketPath }, api.logger));
        api.registerTool(createRosterBlockToolFactory({ botId: config.botId, socketPath: adminSocketPath }, api.logger));
        api.registerTool(createRosterUnblockToolFactory({ botId: config.botId, socketPath: adminSocketPath }, api.logger));
        api.registerTool(createChannelSetNewcomerModeToolFactory({ botId: config.botId, socketPath: adminSocketPath }, api.logger));
        // ── Directory lookup tool (user-directory Phase 3a) ─────────────────────
        // Spec: internal/spec-user-directory-2026-06-22.md §5 item 1.
        //
        ...
        api.registerTool(createDirectoryLookupToolFactory({ sharedDir: config.sharedDir, botId: config.botId }, api.logger));
        // ── Directory upsert tool (user-directory Phase 3b — the WRITE half) ─────
        // Spec: internal/spec-user-directory-2026-06-22.md §5 item 2.
        //
        ...
        api.registerTool(createDirectoryUpsertToolFactory({ sharedDir: config.sharedDir, botId: config.botId }, api.logger));
        // ── Users identity read tools (D-AP5 — the app-facing "who is asking") ──
        // internal/dispatch/done/users-roster-read-only-surface.md item 2;
        // design-app-access-2026-08-15.md §"Identity per turn".
        //
        // ``users_whoami`` / ``users_list`` let the bot answer "who is asking"
        // without importing the roster's internals. Registered here with no
        // ``appId`` — a bare bot-conversation call, which carries no audience to
        // enforce (same default DirectoryTools already has). An app-scoped
        // instantiation (a specific app's own ``appId`` baked in, so its
        // ``audience`` is actually enforced) is the app-facing-contract chip's
        // job to wire generically once it exists; this registers the verb.
        api.registerTool(createUsersWhoamiToolFactory({ sharedDir: config.sharedDir, botId: config.botId }, api.logger));
        api.registerTool(createUsersListToolFactory({ sharedDir: config.sharedDir, botId: config.botId }, api.logger));
        // ── Board tool: chat parity with the user's board (D-MB4) ──────────────
        // Design: internal/design-pa-mobile-board-2026-08-31.md D-MB1/D-MB4, as
        // amended by design-pa-board-interface-v2-2026-09-04.md D-BI7.
        //
        // The board the user sees on their phone and the board the bot triages in
        // chat are ONE store, written by ONE process (the admin daemon). This tool
        // is the bot's only way in: five verbs, each a single daemon call over the
        // unix socket, fail closed when the daemon is unreachable. Without it the
        // bot keeps a second task list somewhere — a hand-planted BOARD.md — and a
        // second source of truth is the failure D-MB6 exists to end.
        //
        // Registered unconditionally, like DirectoryTools: the server is the
        // security boundary. It binds the acting bot from the socket peer uid, so
        // the tool is scoped to the caller's OWN board by construction and carries
        // no privilege of its own. A bot whose board was never minted simply has an
        // empty one.
        //
        // An app-facing tool: every verb must carry an app-contract v1 row, or
        // the tool is refused (apps/contract/register.ts).
        registerAppTool(api, "board", BOARD_VERBS, createBoardToolFactory({ sharedDir: config.sharedDir, botId: config.botId, socketPath: adminSocketPath }, api.logger), api.logger);
        // ── App capability index: expand_app (Tier-2 on-demand disclosure) ──────
        // Spec: internal/spec-app-invocation-just-works-2026-06-29.md §2.1 (recognition).
        //
        ...
        api.registerTool(createExpandAppToolFactory({ sharedDir: config.sharedDir, botId: config.botId, socketPath: adminSocketPath }, api.logger));
        // ── Curated Google tools (Gmail / Calendar / Drive) ─────────────────────
        // Spec: internal/spec-google-integration-architecture-2026-06-20.md (P1).
        //
        ...
        if (googleMode) {
            const factories = createGoogleToolFactories({ sharedDir: config.sharedDir, botId: config.botId }, api.logger);
            for (const factory of factories) {
                api.registerTool(factory);
            }
            api.logger.info(`Evolve: registered ${factories.length} Google tools for ` +
