...
export const BOARD_LANES = ["inbox", "today", "later", "done", "dropped"];
/** Who a card is on (D-BI7). `bot` is an offer, not an instruction. */
export const BOARD_OWNERS = ["me", "bot"];
/**
 ...
 */
export const MOVE_TO_BOT_REFUSAL = "'bot' is not a lane. Lanes answer WHEN (inbox/today/later/done/dropped); " +
...
export const BOARD_VERBS = ["list", "add", "move", "assign", "progress"];
/**
 ...
 */
export const DAEMON_UNREACHABLE_REFUSAL = "The board is unavailable right now — the Evolve admin daemon is not " +
...
export const BoardParamsSchema = Type.Object({
    verb: Type.Union(BOARD_VERBS.map((v) => Type.Literal(v)), {
        description: "list=read it; add=new card; move=change WHEN; assign=change WHO; " +
    ...
    cluster: Type.Optional(Type.String({
        description: "Life area (lowercase slug): health, fitness, travel, work, social, " +
    ...
    reason: Type.Optional(Type.String({
        description: "to_lane=dropped only. One of: " + DROP_REASONS.join("|"),
    })),
    ...
    owner: Type.Optional(Type.Union(BOARD_OWNERS.map((o) => Type.Literal(o)), {
        description: "WHO: me (the user) or bot (you). On assign, 'bot' OFFERS it to you — " +
    ...
    enrichment: Type.Optional(Type.Record(Type.String(), EnrichmentFieldSchema, {
        description: "add only: facts worth keeping, captured ONCE now — runtime_min, " +
 ...
 */
export function renderList(payload) {
    ...
    for (const card of cards) {
        ...
        const line = [
            ...
            // D-BI5 §6: a one-glyph "this card has actions[] to offer" marker —
            // the actions themselves (and their live cost/gating) cost a fetch by
            // id, same as enrichment values; this is a per-turn presence flag.
            + (card.has_actions ? " ⚡" : "");
...
function renderCard(card, verb) {
    const id = String(card.id ?? "");
    const delegation = card.delegation;
    const owner = card.owner === "bot"
        ? `bot${delegation?.state ? ` (${delegation.state})` : ""}`
        : "me";
    return (`${verb}: ${id.slice(0, ID_PREFIX_CHARS)} · ${String(card.title ?? "")} · ` +
...
function buildRequest(p) {
    switch (p.verb) {
        case "list": {
            ...
            for (const key of ["cluster", "lane", "owner"]) {
        ...
        case "add": {
            ...
            if (!title)
                return "board add: `title` is required — what is the card?";
            const body = {
                title,
            ...
            if (p.note)
                body.note = p.note;
            if (p.source_id)
                body.source_id = p.source_id.trim();
            if (p.enrichment !== undefined)
                body.enrichment = p.enrichment;
            return { method: "POST", path: `${BOARD_BASE}/cards`, body };
        }
        case "move": {
            const id = (p.id ?? "").trim();
            if (!id)
                return "board move: `id` is required (list the board first).";
            if (!p.to_lane) {
                return `board move: \`to_lane\` is required — one of ${BOARD_LANES.join(", ")}.`;
            }
            if (p.to_lane === "bot")
                return MOVE_TO_BOT_REFUSAL;
            const body = { to_lane: p.to_lane };
        ...
        case "assign": {
            const id = (p.id ?? "").trim();
            if (!id)
                return "board assign: `id` is required (list the board first).";
            if (!p.owner)
                return "board assign: `owner` is required — 'me' or 'bot'.";
            return {
        ...
        case "progress": {
            const id = (p.id ?? "").trim();
            if (!id)
                return "board progress: `id` is required (list the board first).";
            if (!p.state) {
            ...
            if (p.note)
                body.note = p.note;
            if (typeof p.cost_to_date === "number")
                body.cost_to_date = p.cost_to_date;
            return {
                method: "POST",
 ...
 */
export function createBoardToolFactory(config, logger) {
        ...
        async execute(_toolCallId, rawParams) {
            ...
            try {
                ...
                if (res.status >= 200 && res.status < 300) {
                    if (params.verb === "list")
                        return textResult(renderList(payload));
                    ...
                    if (payload.skipped) {
                        const settled = payload.card;
                        return textResult(`board add: skipped — the user already ${payload.lane === "dropped" ? "dropped" : "finished"} this (${String(settled?.title ?? "")}). Not re-added.`);
                    }
                    const card = payload.card;
                    if (card && typeof card === "object") {
                        return textResult(renderCard(card, params.verb));
                    }
...
//# sourceMappingURL=BoardTool.js.map