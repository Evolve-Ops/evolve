...
export const SetTierToolParamsSchema = Type.Object({
    choice: Type.Union([
        ...
        description: "fast = the quick/cheap role (Haiku-class), standard = the default " +
    ...
    scope: Type.Optional(Type.Union([Type.Literal("session"), Type.Literal("standing")], {
        description: "'session' (default) = this conversation only — for 'this " +
            "one / right now'. 'standing' = the user's persistent default " +
            "on this bot, all future conversations — ONLY for 'always / " +
            "from now on / by default'. Ambiguous → 'session'.",
    })),
    reason: Type.Optional(Type.String({
        description: "Brief paraphrase (1-2 sentences) of the user's intent that led " +
...
const DESCRIPTION = [
    "Set the model capability tier — for this session, or as the user's",
    "standing default.",
    "",
    ...
    "",
    "Scope: 'always / from now on / by default / for our chats' →",
    "scope='standing' (persistent per-user default; choice='auto' clears",
    "it). Otherwise scope='session'; if ambiguous, mention the standing",
    "option once.",
    "",
    ...
    "  - 'Done — that's your default for our future chats.' (standing)",
    "",
    "The ack must state the scope; it is the user's only confirmation —",
    "never claim which model is currently answering.",
    "",
...
function _resultContent(r) {
    ...
    if (r.scope !== undefined)
        body.scope = r.scope;
    if (r.ack_hint !== undefined)
        body.ack_hint = r.ack_hint;
    if (r.requested_choice !== undefined)
        body.requested_choice = r.requested_choice;
    if (r.tier1_blocked_reason !== undefined)
        body.tier1_blocked_reason = r.tier1_blocked_reason;
    if (r.tier1_blocked_detail !== undefined)
        body.tier1_blocked_detail = r.tier1_blocked_detail;
    return {
        content: [{ type: "text", text: JSON.stringify(body) }],
    };
}
export function createSetTierToolFactory(deps, logger) {
    ...
    return (ctx) => {
        return {
            ...
            async execute(_toolCallId, rawParams) {
                ...
                const scope = (params.scope ?? "session").trim().toLowerCase();
                if (!["session", "standing"].includes(scope)) {
                    return _resultContent({
                        ok: false,
                        error: `unknown scope '${params.scope}'. Valid: session | standing`,
                    });
                }
                // ── Session context ─────────────────────────────────────────────────
                const sessionKey = ctx.sessionKey ?? ctx.sessionId;
                if (!sessionKey) {
                    logger.warn("session_set_tier: ctx.sessionKey/sessionId missing — cannot " +
                ...
                // branches.
                let consentSource = "bot_initiated";
                if (hasRecentAskHint && hasRecentAskHint(String(sessionKey))) {
                    consentSource = "ask_hint_agreed";
                }
                // ── scope="standing" — write the per-user standing default ─────────
                // G4 of the spec-user-tier-control 2026-08-03 addendum: "always /
                // from now on / by default" intent lands HERE (the per-user
                // ``user-tier-prefs.json`` entry that ``evo tier-default`` writes),
                // never in a session pin. The write mirrors the admin-side handler:
                //
                //   • gated only on the surface-enable flag (same refusal the admin
                //     handler gives when userTierOverride.enabled === false). The
                //     power/max degrade chain below is deliberately NOT applied — a
                //     per-user standing default MAY be max (explicit user pull, §max
                //     semantics #1, exactly like ``evo tier-default max``); it is a
                //     user-owned preference, not a bot-driven session escalation, and
                //     the per-day role caps continue to govern actual usage.
                //   • MUST be attributable: the write targets the calling session's
                //     pinned user_key (TurnObserver's setSessionUserKey). No
                //     user_key (heartbeat / internal turn) → refuse, never guess.
                //   • can fail on PERMISSIONS (the per-bot shared dir is typically
                //     evolve-owned; the plugin runs as the bot user) → fail LOUDLY
                //     with the fallback instruction, never silently no-op.
                if (scope === "standing") {
                    if (!modelRouter.isUserTierSurfaceEnabled()) {
                        return _resultContent({
                            ok: false,
                            error: "tier-control surfaces are disabled on this bot " +
                                "(userTierOverride.enabled is false in evolve-tiers.json). " +
                                "Ask the pod admin to re-enable them from the AI " +
                                "Optimization page.",
                        });
                    }
                    const userKey = modelRouter.getSessionUserKey(String(sessionKey));
                    if (!userKey) {
                        return _resultContent({
                            ok: false,
                            error: "this session has no pinned user identity (internal/" +
                                "heartbeat/anonymous turn), so a standing per-user default " +
                                "cannot be attributed to anyone. Nothing was saved. The " +
                                `user can set it themselves by typing 'evo tier-default ${choice}'.`,
                        });
                    }
                    try {
                        modelRouter.setStandingUserTierDefault(userKey, choice === "auto" ? null : choice);
                    }
                    catch (err) {
                        const msg = err instanceof Error ? err.message : String(err);
                        logger.warn(`session_set_tier: bot=${botId} standing write FAILED for ` +
                            `user_key=${userKey}: ${msg}`);
                        return _resultContent({
                            ok: false,
                            error: `standing default write failed: ${msg}. Nothing was saved. ` +
                                "Tell the user they can set it themselves by typing " +
                                `'evo tier-default ${choice}', or ask the pod admin to set ` +
                                "it from the admin UI.",
                        });
                    }
                    const standingReason = params.reason
                        ? ` reason="${params.reason.slice(0, 80)}"`
                        : "";
                    logger.info(`session_set_tier: bot=${botId} scope=standing ` +
                        `user_key=${userKey} choice=${choice} ` +
                        `consent_source=${consentSource}${standingReason}`);
                    return _resultContent({
                        ok: true,
                        applied_choice: choice,
                        consent_source: consentSource,
                        scope: "standing",
                        ack_hint: choice === "auto"
                            ? "Standing default cleared — tell the user their " +
                                "conversations go back to this bot's normal defaults " +
                                "from their next turn. Do not claim which model is " +
                                "currently active."
                            : "Standing default saved — tell the user it now applies " +
                                "to all their future conversations with this bot, not " +
                                "just this one. Do not claim which model is currently " +
                                "active.",
                    });
                }
                ...
                if (choice === "power" || choice === "max") {
                    ...
                    if (role !== choice) {
                        ...
                        logger.warn(`session_set_tier: bot=${botId} session=${String(sessionKey).slice(0, 12)} ` +
                ...
                // ── Apply ───────────────────────────────────────────────────────────
                try {
                    modelRouter.setUserTier(String(sessionKey), appliedChoice === "auto" ? null : appliedChoice, consentSource);
                }
                catch (err) {
                    const msg = err instanceof Error ? err.message : String(err);
                ...
                const blockedSuffix = costGateBlocked
                    ...
                    : "";
                logger.info(`session_set_tier: bot=${botId} session=${String(sessionKey).slice(0, 12)} ` +
...
//# sourceMappingURL=SetTierTool.js.map