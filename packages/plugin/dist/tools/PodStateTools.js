...
export function createPodStatusToolFactory(logger, socketPath) {
    return (_ctx) => ({
        name: "pod_status",
...
export const ListSignalsParamsSchema = Type.Object({
    state: Type.Optional(Type.Union([
        ...
        description: "Signal lifecycle state. Default 'firing' — most 'show me " +
            "alerts' questions want live, not historical.",
    })),
    producer: Type.Optional(Type.String({
        description: "Filter to one monitor/generator (e.g. 'cost_alert', " +
...
export function createListSignalsToolFactory(logger, socketPath) {
    return (_ctx) => ({
        name: "list_signals",
...
export const ListProposalsParamsSchema = Type.Object({
    state: Type.Optional(Type.Union([
        ...
        description: "Proposal lifecycle state. 'pending' is the default and the " +
...
export function createListProposalsToolFactory(logger, socketPath) {
    return (_ctx) => ({
        name: "list_proposals",
...
export const RecentWatchdogParamsSchema = Type.Object({
    hours: Type.Optional(Type.Integer({
        description: "Lookback window in hours. Default 24, max 168 (one week). " +
...
export function createRecentWatchdogToolFactory(logger, socketPath) {
    return (_ctx) => ({
...
export const SpendRollupParamsSchema = Type.Object({
    window: Type.Optional(Type.Union([Type.Literal("1d"), Type.Literal("7d"), Type.Literal("30d")], {
        description: "Time window for the rollup. Default '7d'. Use '1d' for " +
            "'today', '30d' for monthly trends.",
    })),
    bot: Type.Optional(Type.String({
        description: "Filter to one bot. Omit for pod-wide totals across every " +
            "member bot.",
    })),
});
...
export function createSpendRollupToolFactory(logger, socketPath) {
    return (_ctx) => ({
        name: "spend_rollup",
...
export const RecentTurnsParamsSchema = Type.Object({
    bot: Type.String({
        description: "Which bot's recent user turns to retrieve. Required — there " +
...
export function createRecentTurnsToolFactory(logger, socketPath) {
    return (_ctx) => ({
        name: "recent_turns",
...
export function createDescribeBotToolFactory(logger, socketPath) {
    return (_ctx) => ({
        name: "describe_bot",
...
export const ListAuditsParamsSchema = Type.Object({
    bot: Type.Optional(Type.String({
        description: "Filter to one bot's latest audit. Omit to get the latest " +
...
export function createListAuditsToolFactory(logger, socketPath) {
    return (_ctx) => ({
        name: "list_audits",
