...
export const ExpandAppParamsSchema = Type.Object({
    app_id: Type.String({
        description: "Which installed app to expand — the id shown in the Installed Apps menu " +
 ...
 */
export function createExpandAppToolFactory(config, logger) {
    const socketPath = config.socketPath ?? join(config.sharedDir, SOCKET_NAME);
    const transport = config.transport ?? adminSocketRequest;
    return (ctx) => ({
        name: "expand_app",
        description: "Show how to use one of your installed apps — its commands, arguments, and " +
        ...
        async execute(_toolCallId, rawParams) {
            ...
            if (!appId) {
                return textResult("Provide app_id — the id of the installed app to expand (see your " +
            ...
            try {
                ...
                if (res.status === 200 && obj.ok === true) {
                    ...
                    const resolvedId = typeof obj.app_id === "string" && obj.app_id.trim()
                        ? obj.app_id.trim()
                        : appId;
                    ...
                    return textResult(`No usage details are recorded for ${JSON.stringify(appId)} yet.`);
                }
                if (res.status === 404) {
                    const available = Array.isArray(obj.available)
                        ? obj.available.filter((x) => typeof x === "string")
                        : [];
                    const hint = available.length
                        ? ` Installed apps you can expand: ${available.join(", ")}.`
                        : "";
                    return textResult(`No installed app matches ${JSON.stringify(appId)}.${hint}`, true);
                }
                const detail = "detail" in obj || "error" in obj
                ...
                if (err instanceof AdminSocketUnavailable) {
                    ...
                    return textResult("App details are unavailable right now (admin daemon unreachable). " +
...
//# sourceMappingURL=ExpandAppTool.js.map