 ...
 *
 * Spec: docs/spec-user-directory-2026-06-22.md §5 items 1–2 + §10 invariants. Together
...
export const DirectoryLookupParamsSchema = Type.Object({
    ref: Type.String({
        description: "Who to look up — a person's @handle, full or display name, email address, or " +
 ...
 */
export function createDirectoryLookupToolFactory(config, logger) {
    ...
    return (_ctx) => ({
        ...
        description: "Look up a person in THIS bot's user directory — its admitted users and saved " +
        ...
        async execute(_toolCallId, rawParams) {
            ...
            if (!ref) {
                return textResult("Provide who to look up — a name, @handle, email, or id.", true);
            }
            try {
                ...
                if (res.status === 200) {
                    const person = (res.body && typeof res.body === "object"
                        ? res.body.person
                        : null) ?? null;
                    if (!person) {
                        return textResult(`No one in this bot's directory matches ${JSON.stringify(ref)}. ` +
                ...
                if (err instanceof AdminSocketUnavailable) {
                    ...
                    return textResult("The directory is unavailable right now (admin daemon unreachable). " +
...
const UpsertEmailSchema = Type.Object({
    ...
    rank: Type.Optional(Type.Union([Type.Literal("primary"), Type.Literal("secondary")], {
        description: "'primary' for the person's main address (at most one), else 'secondary'. " +
...
export const DirectoryUpsertParamsSchema = Type.Object({
    ref: Type.String({
        description: "Who to record — the @handle, name, email, or stable id of an existing person to " +
    ...
    emails: Type.Optional(Type.Array(UpsertEmailSchema, {
        description: "Contact email addresses to record (their inbox — distinct from your send-as " +
            "From address). At most one 'primary'.",
    })),
    contact: Type.Optional(Type.Record(Type.String(), Type.String(), {
        description: "Open contact attributes as a flat map of text values, e.g. " +
 ...
 */
export function createDirectoryUpsertToolFactory(config, logger) {
    const socketPath = config.socketPath ?? join(config.sharedDir, SOCKET_NAME);
    const transport = config.transport ?? adminSocketRequest;
    return (_ctx) => ({
        name: "directory_upsert",
        description: "Record or correct a person's identity and contact details in THIS bot's user " +
        ...
        async execute(_toolCallId, rawParams) {
            ...
            const ref = typeof params.ref === "string" ? params.ref.trim() : "";
            if (!ref) {
                return textResult("Provide `ref` — who to record (a name, @handle, email, or id).", true);
            }
            ...
            try {
                ...
                if (res.status === 200) {
                    const obj = res.body && typeof res.body === "object"
                        ? res.body
                        : {};
...
//# sourceMappingURL=DirectoryTools.js.map