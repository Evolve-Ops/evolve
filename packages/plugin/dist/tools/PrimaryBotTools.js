 ...
 *
 * Spec: docs/spec-primary-bot-interface-2026-05-14.md.
 *
 ...
 */
function adminPost(socketPath, urlPath, body) {
...
export const HelpSearchParamsSchema = Type.Object({
    query: Type.String({
        description: "Free-text question about Evolve (e.g. 'how do I add a bot?', " +
    ...
    k: Type.Optional(Type.Integer({
        description: "How many top hits to return (default 3, max 10). Each hit is a " +
...
export function createHelpSearchToolFactory(logger, socketPath) {
    return (_ctx) => ({
...
export const HelpReadParamsSchema = Type.Object({
    doc_id: Type.String({
        description: "Stable id for a help doc — exactly the string returned in " +
...
export function createHelpReadToolFactory(logger, socketPath) {
    return (_ctx) => ({
        ...
        async execute(_toolCallId, rawParams) {
            ...
            try {
                ...
                if (res.status === 404) {
                    return textResult(`evolve_help_read: no doc with id '${docId}'. Try evolve_help_search first.`, true);
                }
                if (res.status === 503) {
                    const err = (res.body && res.body.error) || "help index not built";
...
export const SubmitIntakeParamsSchema = Type.Object({
    kind: Type.Union([Type.Literal("bug"), Type.Literal("feature"), Type.Literal("question")], {
        description: "What kind of intake this is. 'bug' for misbehavior, 'feature' " +
            "for a request, 'question' for ambiguous things admin wants on " +
            "the record. Pick one — when uncertain, ask admin first.",
    }),
    body: Type.String({
        description: "The intake body — admin's own words, near-verbatim. Don't " +
    ...
    promote: Type.Optional(Type.Boolean({
        description: "If true, after capture also file this as a GitHub issue. Only " +
    ...
    include_transcript: Type.Optional(Type.Boolean({
        description: "When promote=true: if true, attach the redacted recent-turns " +
...
export function createSubmitIntakeToolFactory(config, logger) {
    ...
    return (ctx) => ({
        ...
        async execute(_toolCallId, rawParams) {
            ...
            try {
                ...
                if (!intakeId) {
                    return textResult("submit_intake: admin server returned no intake id.", true);
                }
            }
            catch (err) {
                const msg = err instanceof Error ? err.message : String(err);
            ...
            try {
                const promoteRes = await adminPost(socketPath, `/api/evo/intake/${encodeURIComponent(intakeId)}/promote`, { include_transcript: includeTranscript });
                if (promoteRes.status === 200) {
                    const githubUrl = promoteRes.body?.intake?.promotion?.github_issue_url ?? null;
...
//# sourceMappingURL=PrimaryBotTools.js.map