...
function resolveSenderId(ctx, logger) {
    const captured = getSender(ctx.runId ?? null);
    ...
    logger.warn(`UsersTools: could not resolve sender — runId=${ctx.runId ?? "?"} ` +
        `sessionKey=${sessionKey || "?"}`);
    return { id: null, platform: null };
}
function noSenderRefusal() {
    return textResult("Could not identify who is speaking (or which platform they are on) — " +
        "this needs a verified sender identity to answer 'who is asking'.", true);
}
async function callUsersBot(path, config, senderId, platform, logger, toolName) {
    ...
    try {
        ...
        if (err instanceof AdminSocketUnavailable) {
            ...
            return textResult("The roster is unavailable right now (admin daemon unreachable). " +
...
export function createUsersWhoamiToolFactory(config, logger) {
    return (ctx) => ({
        name: "users_whoami",
        description: "Who is the person you're currently talking to? Returns THEIR OWN roster " +
        ...
        async execute(_toolCallId, _rawParams) {
            ...
            return await callUsersBot("/api/users-bot/whoami", config, senderId, platform, logger, "users_whoami");
        },
    });
}
export const UsersListParamsSchema = Type.Object({});
/** Build the ``users_list`` tool factory — the roster this app may see. */
export function createUsersListToolFactory(config, logger) {
    return (ctx) => ({
        name: "users_list",
        description: "List the people this bot knows. If this app is open to everyone on the " +
        ...
        async execute(_toolCallId, _rawParams) {
            ...
            if (!senderId || !platform)
                return noSenderRefusal();
            return await callUsersBot("/api/users-bot/list", config, senderId, platform, logger, "users_list");
        },
    });
}
//# sourceMappingURL=UsersTools.js.map