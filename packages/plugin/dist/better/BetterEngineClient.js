...
export class BetterEngineClient {
     ...
     */
    async getTopRecommendation(botId, surface) {
        try {
            ...
            const result = await this.adminRequest("GET", `/api/better/recommendations?${params.toString()}`);
            // Server returns {recommendations: [...], never_run: bool} or bare array
            const recs = Array.isArray(result)
                ? result
    ...
    async acceptRecommendation(recId) {
        try {
            await this.adminRequest("POST", `/api/better/recommendations/${encodeURIComponent(recId)}/accept`);
        }
        catch {
    ...
    async rejectRecommendation(recId, reason) {
        try {
            await this.adminRequest("POST", `/api/better/recommendations/${encodeURIComponent(recId)}/reject`, reason ? { reason } : {});
        }
        catch {
            // Fire-and-forget
        }
    ...
    async snoozeRecommendation(recId) {
        try {
            await this.adminRequest("POST", `/api/better/recommendations/${encodeURIComponent(recId)}/snooze`);
        }
        catch {
            // Fire-and-forget
        }
     ...
     */
    async getNextRecommendation(botId, surface, afterRecId) {
        try {
            const params = new URLSearchParams({
                surface,
                scope_id: botId,
            ...
            const next = recs.find((r) => r.id !== afterRecId);
            return next ?? null;
        }
        catch {
            return null;
     ...
     */
    async recordIgnored(recId) {
        try {
            // The API records a reject with reason "ignored" — treated as neutral
            // in the learning layer (distinct from an explicit rejection).
            await this.adminRequest("POST", `/api/better/recommendations/${encodeURIComponent(recId)}/reject`, { reason: "ignored" });
        }
        catch {
            // Fire-and-forget
        }
     ...
     */
    async addPendingAdminTask(botId, rec, acceptedAt) {
        try {
...
//# sourceMappingURL=BetterEngineClient.js.map