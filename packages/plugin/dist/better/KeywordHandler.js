...
export class KeywordHandler {
     ...
     */
    buildKeywordInjection(formattedMessage) {
        return (`IMPORTANT: The user has typed 'evo' or 'evolve'. ` +
     ...
     */
    buildFollowUpInjection(responseMessage) {
        return (`IMPORTANT: The user has replied to a pending recommendation. ` +
            `Respond ONLY with the following message, verbatim. ` +
     ...
     */
    buildStaySilentInjection(sentMessage) {
        ...
        const sentBlock = preview
            ...
            : "";
        return (`CRITICAL — STAY SILENT. DO NOT RESPOND TO THE USER'S 'evo' MESSAGE.\n\n` +
     ...
     */
    async handleFollowUp(action, pendingRec, botId, surface, channel = "other") {
        switch (action) {
            case "accept": {
                if (pendingRec.bridge_strategy === "task_queue") {
                    // Queue it for the admin; confirm to the user
                    await this.client.addPendingAdminTask(botId, pendingRec, new Date().toISOString());
                    await this.client.acceptRecommendation(pendingRec.id);
                    return {
                        message: "Got it — I've flagged this for your Evolve admin.\n" +
            ...
            case "reject": {
                ...
                const next = await this.client.getNextRecommendation(botId, surface, pendingRec.id);
            ...
            case "next": {
                ...
                if (!next) {
                    return {
                        message: "You're all caught up 🎉 I'll let you know when there's something worth your attention.\n" +
            ...
            case "context": {
                ...
                const contextText = pendingRec.context?.trim() || "No additional context available.";
...
//# sourceMappingURL=KeywordHandler.js.map