...
export function makeBeforeToolCallHandler(config, logger) {
    return (event, ctx) => {
        ...
        try {
            const d = inTurnCheckpoint.evaluate((event?.runId ?? ctx?.runId ?? null), (ctx?.sessionId ?? null));
            if (d.kind === "veto")
                return { block: true, blockReason: d.reason };
        }
        catch (err) {
            try {
                logger.warn(`Evolve in-turn checkpoint: evaluation error, allowed: ${err}`);
            }
            catch { /* */ }
        }
...
export function registerToolCallGate(api, config, logger) {
    ...
    if (config.inTurnCheckpointWarning)
        logger.warn(config.inTurnCheckpointWarning);
    try {
