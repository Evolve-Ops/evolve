...
test("priorRoleForSurface: null prior, null surface, junk — all yield null", () => {
  // The positives live in this test on purpose. Every assertion below used to
  // be a `null`, which is also what a `priorRoleForSurface` that resolved
  // NOTHING would return — so the three refusals passed without anything
  // proving the function still answers. A negative and its positive control
  // live or die together or neither is a pin.
  ...
  // A confident per-surface entry resolves, and says it came from the SURFACE.
  assert.deepEqual(
    priorRoleForSurface(
      { role: "fast", confidence: 1, surfaces: { slack: "power" } },
      "Slack ",
    ),
    { role: "power", scope: "surface" },
    "a per-surface prior resolves, case- and whitespace-insensitively",
  );

  // No entry for this surface ⇒ the bot-wide prior, tagged `bot`. The two
  // scopes are the function's whole reason to exist, so a version that
  // collapsed them would still satisfy the refusals above.
  assert.deepEqual(
    priorRoleForSurface(
      { role: "fast", confidence: 1, surfaces: { slack: "power" } },
      "telegram",
  ...
  // A surface entry naming a role the RULE may not pick (`max` is pull-only)
  // falls back to the bot-wide prior rather than refusing outright — the
  // behaviour the source comments, pinned.
  assert.deepEqual(
    priorRoleForSurface(
      { role: "standard", confidence: 1, surfaces: { slack: "max" } },
      "slack",
    ),
    { role: "standard", scope: "bot" },
  );

  // And the confidence floor still refuses — a negative that now cannot be
  // satisfied by "the function resolves nothing", because the lines above
  // prove it resolves.
  assert.equal(
    priorRoleForSurface({ role: "power", confidence: 0.79, surfaces: {} }, "slack"),
    null,
  );
});
