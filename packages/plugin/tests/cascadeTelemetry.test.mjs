...
test("buildSpan: sessionJudge carries cross_vendor so grading can segment verdicts", () => {
  // J2 (design judge-role-collapse §5.3): the judge-accuracy grading must be
  // able to separate self-judged verdicts (cross_vendor:false) from
  // cross-judged ones (true). Both values must round-trip onto the span.
  const t = new CascadeTelemetry({ sharedDir: "/tmp/x", botId: "team_bot_a" }, fakeLogger());
  for (const crossVendor of [true, false]) {
    const span = t.buildSpan({
      sessionId: "s1", turnIndex: 1, startedAt: T0, endedAt: T1,
      tierUsed: "tier2", tierIntended: "tier2", tierChosenBy: "classifier",
      sessionJudge: {
        verdict: "STRUGGLING",
        reason: "shell errors piling up",
        latency_ms: 850,
        triggered_by: "shell_paste",
        cross_vendor: crossVendor,
      },
    });
    assert.equal(span.attributes["cascade.session.judge.verdict"], "STRUGGLING");
    assert.equal(span.attributes["cascade.session.judge.cross_vendor"], crossVendor);
  }
});
