...
/**
 * A config whose shared dir has the classifier gate OPEN.
 *
 * The haiku layer is off by default and fails closed since D-OH2
 * (internal/decision-evolve-overhead-2026-09-07.md) — it is the only
 * layer in this router that costs money. Every haiku test below has to
 * open the gate explicitly, which is the point: on a default pod none
 * of these code paths run.
 */
function gateOpenConfig(extra = {}) {
  const dir = tmpSharedDir();
  fs.writeFileSync(
    path.join(dir, "network.json"),
    JSON.stringify({ cascade: { classifiers: { enabled: true } }, ...extra }),
  );
  return { botId: "team_bot_a", sharedDir: dir };
}
...
test("haiku: returns tier1 when haiku responds TIER1", async () => {
  ...
  const r = new PreflightIntentRouter(gateOpenConfig(), fakeLogger(), api);
  const d = await r.classify({
    userMessage: "Can you read my last email from Sarah?",
    botId: "team_bot_a",
  });
...
test("haiku: NOT called when bot_prior is set", async () => {
  // Gate deliberately OPEN so the assertion below means "bot_prior
  // short-circuited the layer", not "the layer was off anyway".
  const dir = tmpSharedDir();
  fs.writeFileSync(
    path.join(dir, "network.json"),
    JSON.stringify({
      cascade: { classifiers: { enabled: true } },
      bots: { team_bot_a: { preflight: { bot_prior: "tier1" } } },
    }),
  );
  const api = makeHaikuApi("TIER3"); // would override if called
  const r = new PreflightIntentRouter(
...
// ── The classifier gate (D-OH2) — one switch, default off, fail closed ─────

test("gate: closed by default — no network.json means no haiku call", async () => {
  const dir = tmpSharedDir(); // no network.json written at all
  const api = makeHaikuApi("TIER1");
  const r = new PreflightIntentRouter(
    { botId: "team_bot_a", sharedDir: dir },
    fakeLogger(),
  ...
  const d = await r.classify({
    userMessage: "an ambiguous prompt that would hit haiku if the gate were open",
    botId: "team_bot_a",
  });
  assert.equal(d.tier, null, "default must be abstain, not a model call");
  assert.equal(api.calls.length, 0, "the default pod makes NO Evolve model call");
});

test("gate: an unparseable network.json fails CLOSED", async () => {
  // The old haiku gate failed OPEN: a broken config turned the LLM layer
  // ON, on every ambiguous user turn, on a pod that could least afford it.
  // D-OH5 inverts that — every Evolve switch fails closed.
  const dir = tmpSharedDir();
  fs.writeFileSync(path.join(dir, "network.json"), "{ this is not json");
  const api = makeHaikuApi("TIER1");
  const r = new PreflightIntentRouter(
    { botId: "team_bot_a", sharedDir: dir },
    fakeLogger(),
  ...
  assert.equal(d.tier, null, "a broken config must not enable the LLM layer");
  assert.equal(api.calls.length, 0);
});

test("gate: pod-level classifiers.enabled=false skips the haiku layer", async () => {
  const dir = tmpSharedDir();
  fs.writeFileSync(
    path.join(dir, "network.json"),
    JSON.stringify({ cascade: { classifiers: { enabled: false } } }),
  );
  const api = makeHaikuApi("TIER1");
  const r = new PreflightIntentRouter(
    { botId: "team_bot_a", sharedDir: dir },
  ...
  assert.equal(d.tier, null, "expected abstain when the gate is closed");
  assert.equal(api.calls.length, 0, "haiku must NOT be called when gated off");
});

test("gate: per-bot opt-in beats pod-level off", async () => {
  const dir = tmpSharedDir();
  fs.writeFileSync(
    path.join(dir, "network.json"),
    JSON.stringify({
      cascade: { classifiers: { enabled: false } },
      bots: { team_bot_a: { classifiers: { enabled: true } } },
    }),
  );
  const api = makeHaikuApi("TIER1");
  const r = new PreflightIntentRouter(
...
test("gate: per-bot opt-out beats pod-level on", async () => {
  const dir = tmpSharedDir();
  fs.writeFileSync(
    path.join(dir, "network.json"),
    JSON.stringify({
      cascade: { classifiers: { enabled: true } },
      bots: { team_bot_a: { classifiers: { enabled: false } } },
    }),
  );
  const api = makeHaikuApi("TIER1");
  const r = new PreflightIntentRouter(
...
test("haiku: null api skips layer gracefully (no crash)", async () => {
  const r = new PreflightIntentRouter(gateOpenConfig(), fakeLogger(), null);
  const d = await r.classify({
...
test("haiku: partial api stub (no subagent) skips layer gracefully", async () => {
  ...
  const r = new PreflightIntentRouter(gateOpenConfig(), fakeLogger(), {});
  const d = await r.classify({
    userMessage: "an ambiguous prompt",
    botId: "team_bot_a",
  });
...
// ── The learned prior (D-OH2) ───────────────────────────────────────────────

test("prior: an operator's hand-set tier scalar parses at full confidence", () => {
  // No evidence block means a person typed it. They meant it.
  const p = _parseBotPrior({ bot_prior: "tier3" });
  assert.equal(p.role, "fast");
  assert.equal(p.confidence, 1.0);
});

test("prior: the analyzer's role vocabulary parses with its measured confidence", () => {
  const p = _parseBotPrior({
    bot_prior: "standard",
    prior_evidence: {
      turns: 412, confidence: 0.91, surfaces: { Slack: "fast" },
      computed_at: "2026-09-07T03:20:00Z",
    },
  });
  assert.equal(p.role, "standard");
  assert.equal(p.confidence, 0.91);
  assert.equal(p.turns, 412);
  assert.deepEqual(p.surfaces, { slack: "fast" }, "surfaces are keyed lowercase");
  assert.equal(p.computedAt, "2026-09-07T03:20:00Z");
});

test("prior: junk, absence and unknown roles all parse to null", () => {
  assert.equal(_parseBotPrior(null), null);
  assert.equal(_parseBotPrior({}), null);
  assert.equal(_parseBotPrior({ bot_prior: 3 }), null);
  assert.equal(_parseBotPrior({ bot_prior: "turbo" }), null);
});

test("prior: a low-confidence learned prior does NOT route", () => {
  // The parse keeps it; priorRoleForSurface refuses it. Splitting those
  // two lets the evidence stay readable in network.json while the
  // routing decision stays conservative.
  const dir = tmpSharedDir();
  fs.writeFileSync(
    path.join(dir, "network.json"),
    JSON.stringify({
      bots: {
        team_bot_a: {
          preflight: {
            bot_prior: "fast",
            prior_evidence: { turns: 80, confidence: 0.55, surfaces: {} },
          },
        },
      },
    }),
  ...
  const r = new PreflightIntentRouter(
    { botId: "team_bot_a", sharedDir: dir }, fakeLogger(), {},
  );
  return r
    .classify({ userMessage: "an ambiguous prompt", botId: "team_bot_a" })
    .then((d) => {
      assert.equal(d.tier, null, "an unconfident prior must leave the bot on its primary");
      assert.equal(d.layer, "abstain");
    });
});

test("prior: a per-surface entry routes that surface differently", () => {
  const dir = tmpSharedDir();
  fs.writeFileSync(
    path.join(dir, "network.json"),
    JSON.stringify({
      bots: {
        team_bot_a: {
          preflight: {
            bot_prior: "standard",
            prior_evidence: { turns: 400, confidence: 0.93, surfaces: { slack: "fast" } },
          },
        },
      },
    }),
  ...
  return Promise.all([
    r.classify({ userMessage: "a question", botId: "team_bot_a", surface: "slack" }),
    r.classify({ userMessage: "a question", botId: "team_bot_a", surface: "telegram" }),
  ]).then(([slack, telegram]) => {
    assert.equal(slack.tier, "tier3");
    assert.equal(telegram.tier, "tier2", "a surface with no entry uses the bot-wide value");
    assert.equal(slack.layer, "bot_prior");
  });
});

test("prior: hasConfidentPrior reports whether the bot has one", () => {
  const bare = new PreflightIntentRouter(
    { botId: "team_bot_a", sharedDir: tmpSharedDir() }, fakeLogger(), {},
  );
  assert.equal(bare.hasConfidentPrior(), false);

  const dir = tmpSharedDir();
  fs.writeFileSync(
    path.join(dir, "network.json"),
    JSON.stringify({ bots: { team_bot_a: { preflight: { bot_prior: "fast" } } } }),
  );
  const withPrior = new PreflightIntentRouter(
    { botId: "team_bot_a", sharedDir: dir }, fakeLogger(), {},
  );
  assert.equal(withPrior.hasConfidentPrior(), true);
});
