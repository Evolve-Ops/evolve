 ...
 *
 * Spec: docs/spec-model-rungs-and-roles-2026-06-09.md §Addendum 2.
 *
...
const ROLES = ["fast", "standard", "power", "max", "judge"];
...
function rungModels(merged, role) {
  ...
  const rungId = typeof entry === "string" ? entry
    : (entry && typeof entry === "object" ? entry.rung : undefined);
  if (!rungId) return [];
...
function resolvePrimary(merged, role) {
  ...
  // `judge` is provider-diversity-constrained (spec §judge provider-diversity;
  // ModelRouter._resolveJudgeModel): the first model in the judge rung whose
  // provider differs from standard's resolved primary, or null if the rung
  // holds no provider-diverse option. Other roles take rung.models[0].
  if (role === "judge") {
    const standardProvider = providerOf(resolvePrimary(merged, "standard"));
    return models.find((m) => providerOf(m) !== standardProvider) ?? null;
  }
  return models[0];
}
