...
/**
 * A registry pointed at a workspace root that doesn't exist: the manifests
 * dir lookup fails, so refresh() takes the setEmpty() path — which still
 * writes the coverage file (empty covered set). That exercises the real
 * write path hermetically — the default (homedir-derived) workspace root
 * would scan the test runner's own ~/.openclaw on a dev machine.
 */
function registryFor(sharedDir, botId) {
  const workspaceRoot = path.join(sharedDir, "no-such-workspace");
  return new AppScriptRegistry({ botId, sharedDir, logger, workspaceRoot });
}
