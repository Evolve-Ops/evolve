...
// ── App attribution (AL-1.1, design §4.2 source 1) ────────────────────────────
// A daemon-confirmed expand records an EXPLICIT attribution signal for the
// current run using the SERVER-resolved canonical id (the raw param may be a
// name/display-name); misses and failures record nothing.

test("a 200 hit records explicit attribution with the server-resolved id", async () => {
  _resetAttributionForTests();
  const logger = fakeLogger();
  const transport = captureTransport();
  const factory = createExpandAppToolFactory(
    { sharedDir: "/tmp/shared", botId: "atlas", transport }, logger,
  );
  const tool = factory({ runId: "run-attr-1", sessionId: "sess-attr-1" });
  transport.queue({ status: 200, body: { ok: true, app_id: "task_manager", detail: DETAIL } });
  await tool.execute("call-1", { app_id: "Task Manager" }); // display name in, id out
  assert.deepEqual(resolveAttributionForTurn("run-attr-1", "sess-attr-1"), {
    app_id: "task_manager",
...
test("a 404 miss records NO attribution (never invent an app id)", async () => {
  _resetAttributionForTests();
  const logger = fakeLogger();
  const transport = captureTransport();
  const factory = createExpandAppToolFactory(
  ...
  const tool = factory({ runId: "run-attr-2", sessionId: "sess-attr-2" });
  transport.queue({ status: 404, body: { ok: false, error: "no match", available: [] } });
  await tool.execute("call-1", { app_id: "nope" });
  assert.equal(resolveAttributionForTurn("run-attr-2", "sess-attr-2").app_attribution, "none");
});

test("a factory ctx without runId/sessionId still works and records nothing", async () => {
  _resetAttributionForTests();
  const { tool, transport } = buildTool(); // ctx = {}
  transport.queue({ status: 200, body: { ok: true, app_id: "task_manager", detail: DETAIL } });
  const res = await tool.execute("call-1", { app_id: "task_manager" });
  assert.equal(res.isError, undefined);
});
