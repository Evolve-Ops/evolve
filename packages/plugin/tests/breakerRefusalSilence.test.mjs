...
//
// internal/decision-cost-single-turn-2026-09-18.md, from the 2026-09-17
// incident: a trip held `pending` while $4.15 was spent on the operator's
// DM in the next seven minutes, with nothing on disk saying whether the
// plugin live at that minute carried the hold. The "first blocked turn of
// this trip in this conversation" branch above is exactly that hold —
// it must leave a marker for breakers.store to fold into the trip record.

function readHoldRows(shared, botId = BOT) {
  const p = path.join(shared, "breakers", botId, "holds.jsonl");
  if (!fs.existsSync(p)) return [];
  return fs.readFileSync(p, "utf8").trim().split("\n").filter(Boolean).map((l) => JSON.parse(l));
}

test("the first held turn of a trip appends a marker for that trip_id", async () => {
  const h = makeHarness();
  armBreaker(h.shared, { tripId: "trip-evidenced" });
  await h.reply("user", "hi");
  const rows = readHoldRows(h.shared);
  assert.equal(rows.length, 1);
  assert.equal(rows[0].trip_id, "trip-evidenced");
  assert.equal(typeof rows[0].ts, "string");
});

test("a later held turn in the SAME conversation does not append a second marker", async () => {
  // The budget this reuses (_breakerSpokenConversations) is exactly the
  // once-per-trip-per-conversation gate; the marker should follow it 1:1.
  const h = makeHarness();
  armBreaker(h.shared, { tripId: "trip-evidenced" });
  await h.reply("user", "hi");
  await h.reply("user", "hi again");
  await h.reply("user", "hello?");
  assert.equal(readHoldRows(h.shared).length, 1);
});

test("a background trigger never appends a marker — only an interactive hold does", async () => {
  const h = makeHarness();
  armBreaker(h.shared, { tripId: "trip-evidenced" });
  await h.reply("heartbeat", "heartbeat");
  await h.reply("cron", "job");
  assert.equal(readHoldRows(h.shared).length, 0);
});

test("no checkpoint armed → no hold, no marker", async () => {
  const h = makeHarness();
  armBreaker(h.shared, { checkpoint: null });
  await h.reply("user", "hello");
  assert.equal(readHoldRows(h.shared).length, 0);
...
test("an unreadable breaker record never appends a marker — no real trip_id to correlate", async () => {
  const h = makeHarness();
  corruptBreaker(h.shared);
  await h.reply("heartbeat", "heartbeat");
  assert.equal(readHoldRows(h.shared).length, 0);
