 ...
 *   7. the retired `judge` role no longer resolves (judge-role collapse —
 *      internal/design-judge-role-collapse-2026-08-21.md §5.4); a stale
 *      structured roles.judge entry in config is inert, not an error.
 *
...
// ── the retired judge role (judge-role collapse §5.4) ─────────────────────

test("judge no longer resolves; a stale structured roles.judge entry is inert", () => {
  // Read-compat: an un-migrated config carrying the old structured judge
  // entry must PARSE and route its real roles normally — the judge key is
  // simply an extra key nothing reads, and resolving "judge" yields null.
  ...
  assert.equal(r.resolveRoleToModel("standard"), "anthropic/claude-sonnet-4-6");
  assert.equal(r.resolveRoleToModel("judge"), null);
});
