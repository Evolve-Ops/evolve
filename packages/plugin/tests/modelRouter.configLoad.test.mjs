...
const TIER_FIXTURE = {
  tiers: {
    tier1: { models: ["power/v1"] },
    tier2: { models: ["workhorse/v1"] },
    tier3: { models: ["grunt/v1"] },
  },
...
const LEGACY_FIXTURE = {
  tiers: {
    tier1: { models: ["legacy-power/v0"] },
    tier2: { models: ["legacy-workhorse/v0"] },
    tier3: { models: ["legacy-grunt/v0"] },
  },
  routing: { enabled: true },
};
...
test("reloadConfig: reads ~/.openclaw/evolve-tiers.json when present", () => {
  ...
  try {
    ...
    const r = new ModelRouter({ tiers: {}, routing: { enabled: true } }, shared, "team_bot_a");
...
test("reloadConfig: neither file → empty config, fall through to network.json", () => {
  ...
  try {
    fs.writeFileSync(path.join(shared, "network.json"), JSON.stringify({
      models: {
        tiers: {
          tier1: { models: ["network-power/v0"] },
          tier2: { models: ["network-workhorse/v0"] },
          tier3: { models: ["network-grunt/v0"] },
        },
        routing: { enabled: true },
      },
    }));
...
test("reloadConfig: explicit tiersJsonPath override still wins for tests/programmatic use", () => {
  ...
  try {
    ...
    fs.writeFileSync(explicitFile, JSON.stringify({
      tiers: {
        tier1: { models: ["explicit-power/v0"] },
        tier2: { models: ["explicit-workhorse/v0"] },
        tier3: { models: ["explicit-grunt/v0"] },
      },
      routing: { enabled: true },
    }));
