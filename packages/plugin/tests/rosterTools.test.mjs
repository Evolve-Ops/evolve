...
test("sender registry path: tool uses senderId captured by before_agent_run", async () => {
  ...
  captureSender("run-xyz", {
    ...
    platform: "telegram",        // before_agent_run threads the real platform
    senderIsOwner: false,
  });
...
test("G-N2: registry sender with no threaded platform refuses (never defaults to telegram)", async () => {
  // Resolve-or-refuse (audit R1a G-N2): a captured sender whose platform was
  // NOT threaded must NOT default to `telegram` — that mis-attributes a
  // possibly-foreign id onto the privileged telegram id-space. Refuse locally
  // and never call the daemon header-less-of-platform.
  ...
  const result = await tool.execute("call-1", { channel: "telegram", target_id: "555" });
  assert.equal(result.isError, true);
  assert.match(result.content[0].text, /platform|identify|verified sender/i);
  assert.equal(transport.calls.length, 0);
});
