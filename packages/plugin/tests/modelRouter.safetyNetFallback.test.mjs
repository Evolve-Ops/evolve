...
function configWithEmptyTier3() {
  return {
    ...
    // (from legacy fixture) tier3 deliberately missing — the bug scenario.
    routing: { enabled: true },
    runawayRateCap: { enabled: true, dollarsPerWindow: 20, windowMinutes: 5 },
  };
}
...
test("constructor does not warn when safety nets are disabled even if tier3 is empty", () => {
  ...
  const cfg = {
    rungs: [
      { id: "sonnet-class", models: ["y"], costClass: "medium" },
      { id: "opus-class", models: ["x"], costClass: "high" },
    ],
    roles: { standard: "sonnet-class", power: "opus-class" }, // no fast role
    routing: { enabled: true },
