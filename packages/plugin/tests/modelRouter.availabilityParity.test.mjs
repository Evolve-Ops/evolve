...
for (const scenario of scenarios) {
  ...
  // J2 seam parity (design judge-role-collapse §5.2): resolveCrossVendor must
  // return the fixture's expectCrossVendor model (or null) per against-role —
  // the same values the Python side (resolve_cross_vendor) is pinned to.
  test(`cross-vendor: ${scenario.name}`, () => {
    const r = routerFor(scenario);
    for (const [againstRole, exp] of Object.entries(scenario.expectCrossVendor ?? {})) {
      assert.equal(
        r.resolveCrossVendor(againstRole),
        exp,
        `cross-vendor against ${againstRole}`,
      );
    }
  });
}
