 ...
 * registry (internal/design-app-attribution-2026-08-15.md §4–§5).
 *
...
import { knownScriptsForManifest } from "../dist/integrity/appScriptRegistry.js";
...
// ── AL-1.4a: the id that reaches an annotation IS the manifest's app_id ──────

test("attribution carries the manifest's canonical app_id, not a legacy field", () => {
  // AL-1.4a made `app_id` the head of the resolution order. Attribution has
  // no id of its own: the middleware records whatever the script registry
  // resolved from the manifest. This pins the whole chain — manifest.app_id →
  // appIdOf → KnownScript.appId → recordExplicit → the turn annotation — so a
  // manifest that has been given a canonical id can never be attributed under
  // the legacy id sitting next to it.
  const manifest = {
    app_id: "morning-briefing",
    pkg_id: "p-a3f91c8b",           // legacy field, deliberately different
    id: "morning-briefing-legacy",
    realized_files: [{ path: "scripts/brief.py" }],
  };
  const [script] = knownScriptsForManifest(manifest, "/ws");
  assert.equal(script.appId, manifest.app_id);

  recordExplicit("run-canon", "sess-canon", script.appId, "script_middleware");
  assert.equal(resolveForTurn("run-canon", "sess-canon").app_id, manifest.app_id);
});
