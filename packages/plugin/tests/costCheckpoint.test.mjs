...
// ── The acceptance line (reactivate accepts the window) ──────────────────────
//
// Reactivating a bot writes a per-bot "count from now" marker; the trip that
// follows is measured from it. The copy has to say so, or "$5.20 of $5.00"
// reads as a lie next to a Cost page showing $32.69 for the day.

test("the card names the accepted spend and the time it was accepted", () => {
  const msg = renderCapReachedMessage(status({
    spendUsd: 5.2, capUsd: 5.0,
    acceptedUsd: 27.49, acceptedAtLabel: "21:40", spendTotalUsd: 32.69,
  }), BOT);
  assert.match(msg, /\$5\.20 of \$5\.00/);
  assert.match(msg, /accepted \$27\.49 at 21:40/i);
  assert.match(msg, /counting from then/i);
});

test("the non-owner card carries the same acceptance line", () => {
  const msg = renderNotOwnerMessage(status({
    spendUsd: 5.2, capUsd: 5.0, acceptedUsd: 27.49, acceptedAtLabel: "21:40",
  }), BOT);
  assert.match(msg, /accepted \$27\.49 at 21:40/i);
});

test("no acceptance line when the operator has not reactivated today", () => {
  for (const overrides of [{}, { acceptedUsd: null }, { acceptedUsd: 0 }]) {
    const msg = renderCapReachedMessage(status(overrides), BOT);
    assert.ok(!/counting from then/i.test(msg), JSON.stringify(overrides));
  }
});

test("an unparseable acceptance timestamp costs the clock, not the line", () => {
  const msg = renderCapReachedMessage(status({
    acceptedUsd: 27.49, acceptedAtLabel: null,
  }), BOT);
  assert.match(msg, /accepted \$27\.49 at reactivation/i);
});

test("the status reader surfaces the marker fields from the flag", () => {
  const shared = tmpShared();
  writeBreaker(shared, { checkpoint: "pending" });
  writeFlag(shared, {
    action: "checkpoint", cap: 5.0, spend_at_trigger: 5.2,
    accepted_usd: 27.49, accepted_at: "2026-09-07T21:40:00+00:00",
    spend_total: 32.69,
  });
  const st = readCostCheckpoint({ sharedDir: shared, botId: BOT });
  assert.equal(st.acceptedUsd, 27.49);
  assert.equal(st.spendTotalUsd, 32.69);
  assert.match(st.acceptedAtLabel, /^\d{2}:\d{2}$/);
});

test("a flag without marker fields reads as no acceptance", () => {
  const shared = tmpShared();
  writeBreaker(shared, { checkpoint: "pending" });
  writeFlag(shared, { action: "checkpoint", cap: 5.0, spend_at_trigger: 27.49 });
  const st = readCostCheckpoint({ sharedDir: shared, botId: BOT });
  assert.equal(st.acceptedUsd, null);
  assert.equal(st.acceptedAtLabel, null);
  assert.equal(st.spendTotalUsd, null);
});
