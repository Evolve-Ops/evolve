...
test("background turns are the L1 veto's business, not the checkpoint's", async () => {
  // A heartbeat turn under a pending checkpoint is blocked by the
  // pre-existing auto-source veto with the BACKGROUND wording — unchanged
  // behaviour, and never the checkpoint's cap-reached message.
  const h = makeHarness();
  armCheckpoint(h.shared);
  ...
  assert.match(res.message, /background activity paused/i);
});
