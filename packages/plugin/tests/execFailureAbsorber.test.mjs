...
/**
 * A shared dir the ledger CANNOT be written under, portably.
 *
 * The parent is a regular file, so the absorber's `mkdirSync(..., {recursive:
 * true})` fails ENOTDIR immediately — on every platform, for any user, for a
 * structural reason rather than a permission one.
 *
 * Of the two obvious alternatives, one is disqualified and one is merely
 * weaker. A path under `/proc` can BLOCK the calling thread indefinitely
 * instead of returning an error — which is what this fixture used to do,
 * hanging `node --test tests/*.mjs` on Linux with no output naming the test.
 * `chmod 0o500` works, but only for an unprivileged user: root ignores mode
 * bits, so the same fixture means "must fail" or "succeeds silently"
 * depending on who runs the suite. (Corrected 2026-09-17: an earlier version
 * of this comment claimed CI runs as root. It does not — `ci.yml` declares no
 * `container:` on any job, so every job runs as the runner's own unprivileged
 * user. The preference for a uid-independent failure stands; the reason given
 * for it did not.)
 */
function unwritableSharedDir() {
  const notADirectory = path.join(tmpDir(), "not-a-directory");
  fs.writeFileSync(notADirectory, "");
  return path.join(notADirectory, "shared");
}
...
// ── Unclassified agent-run-failure banner (second family, 2026-09-09) ───────
//
// OC renders `⚠️ Agent run failed (model: X).` only when it could NOT
// classify the failure (renderAssistantRequestFailureCopy's last branch), so
// the model id is a placeholder rather than a diagnosis. Absorbed on
// scheduled sessions WITHOUT execFailureAbsorb; delivered everywhere else.

// A real heartbeat key, verbatim from the 2026-09-09 incident. Note the
// channel is a live Slack DM — the gate must read the KEY, not the channel.
const SCHED_CTX = {
  channelId: "slack",
  accountId: "default",
  sessionKey: "agent:main:main:heartbeat",
};
const BANNER = "⚠️ Agent run failed (model: anthropic/claude-haiku-4-5).";

test("run-failure: matches the unclassified banner, model and provider forms", () => {
  assert.ok(isUnclassifiedRunFailureLine(BANNER));
  assert.ok(isUnclassifiedRunFailureLine("⚠️ Agent run failed (provider: anthropic)."));
  // VS16 dropped by a normalizing channel adapter must still match.
  assert.ok(isUnclassifiedRunFailureLine("⚠ Agent run failed (model: anthropic/claude-haiku-4-5)."));
  assert.ok(isUnclassifiedRunFailureLine("> " + BANNER));
});

test("run-failure: leaves OC's CLASSIFIED copy alone (it is actionable)", () => {
  // Every branch above the bare form renders a reason the reader can act on.
  for (const line of [
    "⚠️ anthropic/claude-haiku-4-5 request failed (rate limit, HTTP 429). This is usually temporary — try again shortly.",
    "⚠️ anthropic request failed (auth). Re-authenticate the provider and try again.",
    "⚠️ LLM request failed (server error, HTTP 500). This is usually temporary — try again shortly.",
  ]) {
    assert.equal(isUnclassifiedRunFailureLine(line), false, line);
  }
  // And it is not an exec trailer either.
  assert.equal(isExecFailureTrailerLine(BANNER), false);
});

test("run-failure: absorbed on a scheduled session even though armed=false", () => {
  const { absorber, sharedDir } = makeAbsorber({ armed: false });
  const result = absorber.handleMessageSending({ to: "u1", content: BANNER }, SCHED_CTX);
  assert.deepEqual(result, {
    cancel: true,
    cancelReason: "evolve_run_failure_absorbed",
  });
  const rows = readLedger(sharedDir);
  assert.equal(rows.length, 1);
  assert.equal(rows[0].action, "absorbed");
  assert.equal(rows[0].family, "unclassified_run_failure");
  assert.equal(rows[0].session_kind, "scheduled");
  assert.equal(rows[0].armed, false);
  assert.deepEqual(rows[0].matched_lines, [BANNER]);
  assert.equal(rows[0].full_content, BANNER);
});

test("run-failure: delivered unchanged on a user session, ledgered as unscheduled", () => {
  const { absorber, sharedDir } = makeAbsorber({ armed: true });
  const result = absorber.handleMessageSending({ to: "u1", content: BANNER }, CTX);
  assert.equal(result, undefined, "a person waiting on a reply still hears something");
  const rows = readLedger(sharedDir);
  assert.equal(rows.length, 1);
  assert.equal(rows[0].action, "would_absorb_unscheduled");
  assert.equal(rows[0].family, "unclassified_run_failure");
  // No near_miss row: the family matched, it just was not this session kind.
  assert.equal(rows.filter((r) => r.action === "near_miss").length, 0);
});

test("run-failure: a missing sessionKey delivers and says so in the ledger", () => {
  const { absorber, sharedDir } = makeAbsorber({ armed: true });
  const result = absorber.handleMessageSending(
    { to: "u1", content: BANNER },
    { channelId: "slack" },
  );
  assert.equal(result, undefined);
  const rows = readLedger(sharedDir);
  assert.equal(rows.length, 1);
  assert.equal(rows[0].action, "would_absorb_unscheduled");
  assert.equal(rows[0].session_kind, "other");
  assert.equal(rows[0].session_key, null);
});

test("run-failure: cron and scheduled keys absorb too, not just heartbeat", () => {
  for (const sessionKey of [
    "agent:main:cron:nightly-digest",
    "agent:main:scheduled:weekly",
  ]) {
    const { absorber } = makeAbsorber({ armed: false });
    const result = absorber.handleMessageSending(
      { to: "u1", content: BANNER },
      { channelId: "slack", sessionKey },
    );
    assert.equal(result?.cancel, true, sessionKey);
  }
});

test("run-failure: mixed content on a scheduled session strips only the banner", () => {
  const { absorber, sharedDir } = makeAbsorber({ armed: false });
  const content = `Heartbeat report: 3 items reviewed.\n${BANNER}`;
  const result = absorber.handleMessageSending({ to: "u1", content }, SCHED_CTX);
  assert.deepEqual(result, { content: "Heartbeat report: 3 items reviewed." });
  const rows = readLedger(sharedDir);
  assert.equal(rows[0].action, "stripped");
  // A partial strip keeps the surviving prose out of the ledger.
  assert.equal(rows[0].full_content, undefined);
});

test("run-failure: banner inside a code fence is never absorbed", () => {
  const { absorber } = makeAbsorber({ armed: false });
  const content = "Here is what I saw:\n```\n" + BANNER + "\n```";
  const result = absorber.handleMessageSending({ to: "u1", content }, SCHED_CTX);
  assert.equal(result, undefined);
});

test("run-failure: ledger write failure refuses to absorb (absorbed ≠ vanished)", () => {
  const { absorber } = makeAbsorber({ armed: false, sharedDir: unwritableSharedDir() });
  const result = absorber.handleMessageSending({ to: "u1", content: BANNER }, SCHED_CTX);
  assert.equal(result, undefined, "no record means no absorb");
});
