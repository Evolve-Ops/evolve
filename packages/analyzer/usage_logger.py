...
DEMOTED to fallback (AL-1.3, 2026-08-17). This mtime sweep is structural
inference — it measures whether an app's FILES were touched, not whether
the app served any turns (internal/audit-app-framework-2026-07-02.md §6.3-6.4).
``usage_by_app.py`` now rolls up real per-turn attribution from the
plugin's turn annotations, and the Usage / Apps readers prefer
``usage-by-app.json``; they fall back to this file only for bots with zero
attributed turns in the window. Still installed, still useful for apps that
work on files without conversation — just no longer the primary signal.
Do not delete.

Read by:
  - api_analytics_applications (admin UI — shows last-touched on app cards)
    as the fallback when usage-by-app.json has no attributed turns
