...
(spec: ``docs/spec-model-economics-page-2026-06-13.md``).
...
# literals), so the role rollup renders fast→standard→power→max→judge regardless
# of dict iteration order. Unknown roles trail in name order.
_ROLE_ORDER: dict[str, int] = {
    "fast": 0, "standard": 1, "power": 2, "max": 3, "judge": 4,
}
...
def _bot_model_matrix(
    ...
    if not turns:
        return [], {}
    from usage_analytics import _estimate_turn_cost  # single-source cost rule
    ...
    for t in turns:
        ...
        recorded = float(t.get("cost") or 0.0)
    ...
    for cell in agg.values():
        ...
        cell["cost_per_turn"] = round(cell["cost"] / calls, 6) if calls > 0 else None
        matrix.append(cell)
