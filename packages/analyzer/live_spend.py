...
def spend_by_local_day(
    ...
    *,
    ...
    turn_filter: Callable[[dict], bool] | None = None,
    log: Callable[[str], None] | None = None,
    ...
    ``turn_filter`` keeps only the turns it returns True for — the receipt's
    housekeeping line (``housekeeping_cost``) sums one kind of turn through
    this same window rather than re-deriving the local-day rule.
    ...
    for t in turns:
        if exempt_subkinds and t.get("forge_subkind") in exempt_subkinds:
            continue
        if turn_filter is not None and not turn_filter(t):
            continue
...
def total_over_local_days(
    ...
    *,
    ...
    """
    by_day = spend_by_local_day(bot_id, days=days, now=now, turn_filter=turn_filter, log=log)
    if by_day is None:
        return None
