...
class Charter:
    ...
    # Pending-proposal TTL backstop (arbiter.auto_resolve). When set, a
    # pending proposal from this generator that has seen no operator
    # engagement AND no re-assertion (dedup-refresh) for this many days is
    # archived as ``resolved_externally``. Complements
    # ``resolves_when_silent`` — the sweep only runs while the generator
    # itself runs; the TTL drains proposals orphaned by a generator that
    # stopped running (or predates a fingerprint-scheme change). None
    # (default) = no TTL; proposals live until dismissed/resolved.
    ...
    def from_dict(cls, data: dict) -> "Charter":
        return cls(
            ...
            pending_ttl_days=(
                int(data["pending_ttl_days"])
                if data.get("pending_ttl_days") is not None
                else None
            ),
