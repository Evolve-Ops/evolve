...
Status = Literal[
    ...
    # See internal/spec-take-this-on-evo-dispatch-2026-06-04.md.
    "dispatched",
]
...
@dataclass
class PromoteApp:
    """Promote a discovered app to ``defined`` — the vouch, as an Action (AL-1.7).

    Design ``design-app-spec-and-discovery-2026-08-15.md`` §4 (the
    discovered → defined edge) and §7.2, which requires that "the promotion
    action executes server-side — the bot's LLM never holds a privileged tool".
    Making promotion an Action rather than a tool is what holds that boundary:
    the bot pitches, the user answers in their own channel, the *arbiter*
    mutates.

    ``app_id`` is the id the promotion will carry, already decided by
    ``evolve_admin.applications.app_promotion.adopt_or_confer_app_id`` — the
    §7.3a **ADOPT** decision. ``identity_mode`` records which branch produced it
    (``adopted`` = the draft already had a resolvable canonical id and keeps it;
    ``conferred`` = one was minted from the name) so the applier never has to
    re-derive an identity decision, and a reviewer can tell the two apart on the
    stored proposal.

    ``manifest_stem`` is the on-disk filename stem, which can diverge from
    ``app_id`` on gallery-installed v7-arc-pre manifests. Read AND write must hit
    the same file (the finding routes_app_definition._resolve_manifest_stem
    exists for), so the stem travels on the action rather than being guessed at
    apply time.

    Reversible: the applier snapshots the prior manifest and restores it, which
    is the same shape ``DeprecateApp`` uses.
    """

    app_id: str
    manifest_stem: str
    identity_mode: Literal["adopted", "conferred"] = "adopted"
    app_audience: str = "everyone"
    kind: Literal["PromoteApp"] = "PromoteApp"
...
class BuildApp:
    ...
    sequence) which is then run asynchronously.

    identity: see ``applications.app_identity.resolve_app_id`` — the only
    mention of ``pkg_id`` in this module is this prose. ``BuildApp.app_id``
    below is already the canonical field; the ForgeJob's ``pkg_id`` is a
    freshly MINTED package id (``build_app._synthetic_pkg_id``) that names
    the build, not the app, and the two are deliberately different strings. The proposal lands in
...
class InstallMcpServer:
    ...
    Spec: internal/spec-mcp-administration-2026-05-10.md §3.5 + §5.2.
...
class EnablePluginEntry:
    ...
    Spec: internal/spec-plugin-inventory-2026-05-10.md §3.4. Sets
...
class EnableWebhookIngress:
    ...
    Spec: internal/spec-hook-governance-2026-05-10.md §3.5. Operator-
...
class UpdateExecApproval:
    ...
    Spec: internal/spec-permission-posture-2026-05-10.md §3.4.
...
class UpdateAutonomyPosture:
    ...
    Spec: internal/spec-autonomy-ladder-2026-06-10.md §3.2 (Phase B). The
...
class UpdateContentScanCatalog:
    ...
    Spec: internal/spec-prompt-injection-scanner-2026-05-10.md §3.5.
...
# normalization (spec: internal/spec-cost-caps-2026-06-05.md): the two knobs
...
class UpdateAgentDefaults:
    ...
    Spec: internal/spec-cache-retention-tunable-2026-05-30 (PR A) for the
    motivating knob; internal/spec-openclaw-json-derived-artifact-2026-05-24.md
...
class ValueEstimate:
    ...
    Spec: internal/spec-fit-reviewer-2026-06-12.md §5.1 / §3.6 (Gate B).
...
class Proposal:
    ...
    # Spec: internal/spec-recommendations-rework-2026-06-02.md (Phase 1
    ...
    # Design: internal/design-recommendation-legibility-2026-06-12.md
    ...
    # Spec: internal/spec-fit-reviewer-2026-06-12.md §5. Altitude is the
    ...
    # ── Integrity ───────────────────────────────────────────────────────────
    # LEGACY, no longer written. Was an HMAC stamped by the retired
    # evolve_config.sign_proposal (removed 2026-08-18, see
    # internal/design-proposal-signing-key-2026-08-18.md). Retained because it
    # is persisted on historical proposals and migrations/proposal_v1_to_v2
    # still reads it. Nothing verifies it; do not treat a present value as
