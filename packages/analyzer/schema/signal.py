...
Spec: internal/spec-alerts-signal-store-2026-05-07.md §4.
...
# Why a Signal reached ``resolved``. ``None`` (the default, and what
# every signal resolved before 2026-08-31 carries) means the ordinary
# case: a producer's ``sweep_resolve`` observed the condition actually
# clear, or the operator resolved it by hand.
#
# ``ttl_backstop`` is the one exception, written by
# ``signals.stale_ttl.sweep_stale_firing``: the condition was NOT
# observed to clear — the producer simply stopped talking about it for
# ``ttl_days``. That is a much weaker claim, and downstream consumers
# that act on "the condition cleared" must not treat it as equivalent.
# ``arbiter.auto_resolve`` is the first such consumer; see the
# engagement gate there.
#
# The vocabulary lives here (not in stale_ttl) so writer and reader
# share one constant instead of matching on a prose reason string that
# rots the moment someone rewords it.
TTL_BACKSTOP_RESOLUTION = "ttl_backstop"
ResolutionKind = Literal["ttl_backstop"]
...
PRODUCER_CATEGORY_DEFAULT: dict[str, Category] = {
    ...
    # Identities present in a bot's OpenClaw config but absent from every Evolve
    # roster source — the roster-is-canonical invariant inverted, so the same
    # least-privilege governance concern. M1-B3; detection-only.
    "roster_coherence": "security",
    # Accounts carrying an Evolve-provisioned OpenClaw install that back no
    # roster member — a decommissioned bot whose channel/gateway tokens and SSH
    # keys are still valid. Un-revoked credential exposure; detection-only.
    "orphaned_bot_account": "security",
    # Cost & efficiency
    "cost_watchdog": "cost",
    ...
    # Evolve is locked out of a bot's credential store and its own inline ACL
    # heal did not restore access (#3477). A filesystem/permissions integrity
    # fault, not a security finding — the credentials are intact, Evolve just
    # cannot reach them.
    "credential_access": "platform",
    ...
    # The board's tailnet listener reporting that inbound connections are
    # being dropped before the first byte (macOS Application Firewall
    # blocking the daemon's interpreter). Host/daemon integrity — platform.
    "board_listener": "platform",
    ...
    # Model liveness probe — a routed model that resolves in the catalog but
    # fails real dispatch (wrong provider transport, rejected override — the
    # #3489 shape). Platform-integrity probe, sibling to evo_path_probe.
    "model_liveness_probe": "platform",
    ...
    # OC-ingested bootstrap files at/past the bot's injection caps — the
    # over-cap tail is silently absent from every turn's context (the
    # 2026-08-01 AGENTS.md truncation incident). Config-integrity shaped,
    # not spend-shaped: the harm is lost behavioral rules, so "platform".
    "bootstrap_size": "platform",
    ...
    # Absorbed exec-failure shapes (exec-failure hygiene A2) — channel-
    # hygiene advisory: raw trailers kept out of user channels, recurring
    # shapes surfaced here instead.
    "bot_exec_failures": "hygiene",
}
...
class Signal:
    ...
    # Why this Signal is resolved — see ResolutionKind above. Only
    # meaningful while ``state == "resolved"``; ``apply_transition``
    # clears it on any transition away from resolved (re-open, snooze,
    # dismiss) so a re-opened Signal never carries a stale marker.
