...
class PlatformProfile:
    ...
    *admin/root* group — distinct from a bot account's PRIMARY group
    ...
    ops_group: str = "admin"
    """Group of the HUMAN operator(s) — and of nobody else. The group that
    co-owns the deploy checkout with the ``evolve`` service user, so the
    daemon (owner) and the person at the keyboard (group) can both write it.

    macOS ``admin`` / Linux ``sudo`` — on both, the group a person is put in
    to administer the box, and one no bot account is ever created into
    (``runtime.isolation`` mints bots with the platform's plain user group).

    **Never :attr:`bot_shared_group` for anything executable.** That was the
    2026-09-23 finding: the deploy checkout was chowned ``evolve:staff`` with
    ``g+rwX`` so the operator could write it — but ``staff`` is, by this
    module's own definition, *the group every bot account belongs to*, and the
    venv is editable-installed from that checkout. Any bot with shell could
    rewrite code that later runs as root via ``sudo evolve-admin`` or as the
    ``evolve`` daemon. Group-write on a path root executes must name the
    operator, not the fleet.

    Distinct from :attr:`admin_group` (the gid-0 group owning root-owned
    artifacts, which the operator is NOT in) and from
    :attr:`bot_shared_group` (every bot)."""
...
MACOS = PlatformProfile(
    ...
    ops_group="admin",  # the human operator group; no bot account is in it
...
LINUX = PlatformProfile(
    ...
    ops_group="sudo",  # Debian/Ubuntu's administrator group (RHEL's is 'wheel')
