...
See ``internal/spec-app-audit-2026-05-16.md §3.1`` for the assertion catalog.
"""
...
ASSERTION_IDS = (
    ...
    # v13 — scheduled-action contracts (internal/spec-audit-extensions-2026-05-17.md §3.3)
    ...
    # v16 — install-site verification (internal/spec-forge-side-effects-2026-06-02.md §7)
...
#
# See internal/spec-audit-extensions-2026-05-17.md §3.3.
...
def _has_script_realized_file(manifest: dict) -> bool:
    ...
    See internal/alert-root-cause-audit-2026-06-09.md.
...
def check_v7_arc_instance_has_spec_binding(
    ...
    identity: see ``applications.app_identity.resolve_app_id`` — every
    ``spec_id`` below is ``provenance.spec_id``, the Instance's BINDING to a
    Spec, and it is used to build the gallery path
    ``gallery/<tier>/<spec_id>/<spec_version>.json``. Build brief §3 and §5
    name this as the semantically-required case: it is a nested field naming
    ANOTHER artifact, not a top-level answer to "which app is this?" — the
    resolver reads top-level only and would return the Instance's own id,
    which points at no Spec file. The whole point of this check is that the
    binding can be null while the Instance still has a perfectly good
    identity, so collapsing the two would delete the defect it detects.
    """
    if manifest.get("manifest_shape") != "v7-arc":
        return []
