  ...
  * :func:`estimate_turn_cost` — token counts → USD, or ``None`` ("can't price").
    Resolution order is **catalog → offline table → can't-price**:
      ...
      2. :data:`OFFLINE_MODEL_PRICING` / :data:`OFFLINE_PROVIDER_PRICING` — the
         ...
         for this model). The two tables report as DIFFERENT resolutions
         (``table`` / ``provider_table``): the second is a provider-level
         mid-range guess, and :class:`TurnCostTotal` counts it so a receipt can
         say "N turns priced at a provider-level guess" rather than presenting
         it as the model's own price.
      3. ``None``. Never ``0.0``. We do not invent a price, and we do not widen
         the offline table as a substitute for reading the catalog.
  * :func:`turn_cost` / :func:`turn_cost_detail` — the recorded-else-estimate
    rule every reader used to spell out inline (``recorded if recorded else
    _estimate_turn_cost(t)``). A recorded cost wins **when the writer stamped
    it ``cost_source: "provider"``** (the gateway's own billed figure) or
    ``"catalog"``. A ``"table"`` stamp — and an ABSENT stamp, which is a
    record written before 2026-09-04 by the family-substring estimator — is
    re-priced from the catalog whenever the catalog can price the exact model
    id, so a historical turn is corrected on read and no backfill script is
    needed. See :func:`turn_cost_detail`.
...
def _offline_pricing_detail(
    turn: dict,
) -> tuple[tuple[float, float, float, float], str] | None:
    """``(pricing, rung)`` from the offline tables, or ``None``.

    The two rungs are NOT the same claim and are therefore not reported under
    one name:

      * ``"table"`` — :data:`OFFLINE_MODEL_PRICING` has a row for this exact
        model id. A published price for the model actually run.
      * ``"provider_table"`` — only :data:`OFFLINE_PROVIDER_PRICING` matched:
        a mid-range rate for *some* model of that provider, charged to a model
        we have no price for. B6's floor against a silent zero, and still a
        guess. Folding it into ``"table"`` is how a provider-level guess for
        ``claude-opus-5`` reached the cap, the checkpoint and the receipt
        wearing the same label as a real row (PR #4038 review, F2).
    """
...
def _offline_pricing(turn: dict) -> tuple[float, float, float, float] | None:
    ...
    The rate only — see :func:`_offline_pricing_detail` when the caller has to
    tell an exact-model row from a provider-level guess.
    """
    detail = _offline_pricing_detail(turn)
    return None if detail is None else detail[0]


def _apply_pricing(pricing: tuple[float, float, float, float], turn: dict) -> float:
    """Token counts x per-MTok rates -> USD. The arithmetic half of
    :func:`estimate_turn_cost`, split out so the resolution order can be
    reused without re-walking it."""
...
def estimate_turn_cost(
    ...
    *,
    ...
    if pricing is None:
        return None
    return _apply_pricing(pricing, turn)


#: Resolutions :func:`turn_cost_detail` can report, best-first. ``provider``
#: is the gateway's own billed figure; ``catalog`` is the model's published
#: price for the exact id; ``table`` is the offline table's row for that exact
#: id; ``recorded`` is a writer's number we could not improve on;
#: ``provider_table`` is the provider-level mid-range GUESS — a rate for some
#: model of that provider, charged to a model nothing could price — and
#: ``unpriced`` is "can't price".
COST_RESOLUTIONS = (
    "provider", "catalog", "table", "recorded", "provider_table", "unpriced",
)

#: The resolutions that are a guess rather than a published price for the model
#: that actually ran. Counted separately so a surface can say so out loud.
GUESS_RESOLUTIONS = frozenset({"provider_table"})

#: ``cost_source`` values that mark a RECORDED cost as authoritative — a
#: figure the provider itself billed, which no estimate should override.
_AUTHORITATIVE_SOURCES = frozenset({"provider"})


def turn_cost_detail(
    turn: dict,
    *,
    catalog: dict | None = None,
    shared_dir: Path | str | None = None,
) -> tuple[float | None, str]:
    """``(usd, resolution)`` for one turn — the rule, plus where it came from.

    The recorded-cost-wins shortcut is qualified by the writer's own
    ``cost_source`` stamp (added to the turn record 2026-09-04):

      * ``provider`` — the gateway billed this; it is truth and always wins.
      * ``catalog`` — already priced from the pod's catalog by exact model id;
        taken as-is rather than recomputed.
      * ``table`` / absent — a fallback-table estimate, or a **pre-fix record
        written by the family-substring estimator**. Both are re-priced from
        the catalog when the catalog can price the exact id, which is why no
        backfill script exists: a historical turn is corrected on read.
        Measured on the PoC bot for UTC 2026-09-04, the substring estimator
        read $32.09 against a provider console figure of $11.08
        (``internal/finding-cost-forensics-power-bot-2026-09-04.md`` §1).
      * ``unpriced`` — the writer said so; ``cost`` is ``None`` and the
        estimate path runs as usual.

    Only after the catalog declines does a recorded non-zero cost stand, and
    only after that the offline estimate — which reports ``"table"`` for the
    offline table's row for this exact model id and ``"provider_table"`` when
    only the provider-level mid-range rate matched. The second is a guess: a
    rate for *some* model of that provider charged to a model nothing could
    price. It is a separate resolution because a surface that cannot tell the
    two apart shows a guess with the confidence of a published price
    (PR #4038 review, F2). ``(None, "unpriced")`` is the honest tail — never
    ``0.0`` (B6).
    """
    try:
        ...
        recorded = 0.0
    source = str(turn.get("cost_source") or "").strip().lower()
    if recorded and source in _AUTHORITATIVE_SOURCES:
        return recorded, "provider"
    if recorded and source == "catalog":
        return recorded, "catalog"

    cat = catalog if catalog is not None else load_pricing_catalog(shared_dir)
    pricing = _catalog_pricing(turn, cat)
    if pricing is not None:
        return _apply_pricing(pricing, turn), "catalog"
    if recorded:
        return recorded, "recorded"
    offline = _offline_pricing_detail(turn)
    if offline is not None:
        pricing, rung = offline
        return _apply_pricing(pricing, turn), rung
...
def turn_cost(
    ...
    *,
    ...
    """The pod's per-turn cost rule: the best price available for this turn,
    or ``None`` ("can't price this turn").
    ...
    one function so their totals can never disagree. See
    :func:`turn_cost_detail` for the resolution order and why a recorded cost
    no longer wins unconditionally.
    """
    return turn_cost_detail(turn, catalog=catalog, shared_dir=shared_dir)[0]
...
class TurnCostTotal:
    ...
    #: Turns whose RECORDED cost was superseded by the catalog's price for
    #: the exact model id — pre-fix records written by the family-substring
    #: estimator, corrected on read. Reported so a surface can say the total
    #: is not the number the writer stamped, rather than quietly differing
    #: from it.
    repriced_turns: int = 0
    #: Turns priced from :data:`OFFLINE_PROVIDER_PRICING` — a mid-range rate
    #: for *some* model of that provider, charged to a model neither the
    #: catalog nor the exact-id table could price. Counted beside the total
    #: (never folded into it) so the receipt can name the guess as a guess.
    provider_guess_turns: int = 0
    #: The providers those guesses were charged at, for the operator sentence.
    guess_providers: tuple[str, ...] = ()
    ...
    def fully_published(self) -> bool:
        """True when no turn in the window was priced at a provider-level
        guess. A ``usd`` that is ``measurable`` but not ``fully_published`` is
        a complete total built partly out of estimates for models we have no
        price for — a distinction the cap and the receipt both need."""
        return self.provider_guess_turns == 0
...
def sum_turn_costs(
    ...
    *,
    ...
    guesses = 0
    providers: list[str] = []
    guess_providers: list[str] = []
    for t in turns:
        cost, resolution = turn_cost_detail(t, catalog=cat)
        if resolution == "catalog" and _recorded_differs(t, cost):
            repriced += 1
        if resolution in GUESS_RESOLUTIONS:
            guesses += 1
            gp = turn_provider(t)
            if gp not in guess_providers:
                guess_providers.append(gp)
    ...
    return TurnCostTotal(
        ...
        repriced_turns=repriced,
        provider_guess_turns=guesses,
        guess_providers=tuple(sorted(guess_providers)),
    )


def _recorded_differs(turn: dict, resolved: float | None) -> bool:
    """True when the turn carried a non-zero cost the resolver did not use.

    Sub-cent agreement is not a re-price: a catalog row that reproduces the
    writer's own figure has corrected nothing, and counting it would make
    "repriced" mean "read", which is not a fact worth reporting.
    """
    if resolved is None:
        return False
    try:
        ...
        return False
    return bool(recorded) and abs(recorded - resolved) > 1e-9


def provider_guess_note(
    provider_guess_turns: int, providers: Iterable[str] = (),
) -> str:
    """One operator-facing sentence for a provider-level-guess count, or ``""``.

    The counterpart to :func:`unpriced_note`: those turns ARE in the total, but
    at a rate published for a different model of the same provider. The single
    spelling of the phrase, so the CLI and the receipt say the same thing.
    """
    n = int(provider_guess_turns or 0)
    if n <= 0:
        return ""
    ...
    return (
        f"{n} {turn_word} priced at a provider-level guess{tail} — "
        f"no published price for that model"
    )
