...
_LISTING_PROVIDERS = {"anthropic", "openai", "google", "xai", "deepseek", "mistral"}
...
def _read_auth_profiles(bot_user: str) -> dict | None:
    ...
    SOURCE is ``evolve_admin.oc_store.read_auth_store`` (sqlite → legacy json →
    bak, agent-dir discovered, platform-aware). Before OpenClaw 2026.6.9's
    ...
    raw: str | None
    try:
        from evolve_admin.oc_store import read_auth_store  # type: ignore
    except Exception:
        # ``evolve_admin`` not importable in this context — fall back to the
        # legacy direct read so un-migrated / legacy pods still resolve.
...
def _read_auth_profiles_legacy(bot_user: str) -> dict | None:
    ...
    Fallback for contexts where ``evolve_admin.oc_store`` can't be imported.
    Reads the JSON directly (evolve ACL) with a ``sudo /bin/cat`` fallback; does
    NOT see the post-2026.6.9 SQLite store, so it resolves only un-migrated pods.
    """
