...
def _band_to_rung_map() -> dict[str, tuple[str, str]]:
    ...
    try:
        ...
        return {}
    # rung-slug → role (invert the catalog roles map; entries are uniformly
    # bare rung slugs since the judge-role collapse — anything else is junk
    # and skipped).
    rung_role: dict[str, str] = {}
    for role, entry in (cat.get("roles") or {}).items():
        if isinstance(entry, str) and entry:
            rung_role.setdefault(entry, role)
...
FIT_ROLES: tuple[str, ...] = ("fast", "standard", "power", "max")
...
def _rung_to_roles(catalog: dict) -> dict[str, list[str]]:
    """Invert a catalog's ``roles`` map → ``{rung_slug: [role, ...]}``. Role
    entries are uniformly bare rung slugs (judge-role collapse); junk values
    are skipped."""
    out: dict[str, list[str]] = {}
    for role, entry in (catalog.get("roles") or {}).items():
        if isinstance(entry, str) and entry:
            out.setdefault(entry, []).append(role)
    return out
