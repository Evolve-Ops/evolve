...
from dossier.edition import _default_bot_home, _pod_roster, build_edition
...
def run_modules(
    ...
    *,
    ...
    # One app-name table for the whole report, never a lookup per row.
    net = network or {}
    table = names.app_names(shared_dir, _pod_roster(net),
                            bot_home=_default_bot_home(net))
    payload = mod.build_modules(edition, priors, now=now, app_names=table)
