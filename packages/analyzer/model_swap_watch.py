...
:data:`MIN_TARGET_TURNS` and correctly do not fire. A swap that REINTRODUCES
a model previously rolled back or pinned on the same rung
(``model_swap_ledger.previously_rejected_models``) is instead held to the
lower :data:`MIN_TARGET_TURNS_REPEAT` floor — the 2026-08-21 recurrence
diverged cleanly but sat below the cold-start floor (n=21) for days. ``team-bot-a`` is the bot from
...
from model_swap_ledger import (  # noqa: E402
    latest_swaps_by_rung, model_key as _model_key, previously_rejected_models,
)
...
MIN_TARGET_TURNS = 30
# Early-warning floor for a REPEAT of a previously-rejected swap — one whose
# new model was already rolled back (or pinned) on this rung. The prior is
# strong there: the question is not "did behavior change?" from scratch but
# "did the known regression come back?", so far less evidence suffices. The
# 2026-08-21 recurrence measured a clean divergence (target 35.9% → 9.5%,
# control flat) yet stayed silent for days at post-window n=21 because it was
# held to the cold-start floor above.
MIN_TARGET_TURNS_REPEAT = 10
...
def evaluate(arms: dict, *, min_target_turns: int = MIN_TARGET_TURNS,
             repeat_of_rejected: bool = False) -> dict:
    ...
    ``repeat_of_rejected`` marks a swap whose new model was already rolled
    back (or pinned) on this rung; callers pass the lower
    :data:`MIN_TARGET_TURNS_REPEAT` floor alongside it. Only the sample floor
    softens on a repeat — every other gate (prior silence, collapse ratio,
    control stability) is about WHAT the numbers say, not how many there are,
    and the prior does not change what a collapse looks like.
    """
    ...
    verdict = {
        "diverged": False,
        "reason": "",
        "repeat_of_rejected": repeat_of_rejected,
...
def _divergence_signal(swap: dict, verdict: dict) -> dict:
    ...
    repeat_note = (
        "\n\n**This is a repeat.** A model this swap reintroduces was already "
        "rolled back (or pinned) on this rung, so this Signal fired at the "
        f"early-warning sample floor ({MIN_TARGET_TURNS_REPEAT} turns/side "
        f"instead of {MIN_TARGET_TURNS}) — the prior from the earlier "
        "rollback is doing part of the work. Treat the rate above as an early "
        "read, and the rollback below as the known-good undo."
        if verdict.get("repeat_of_rejected") else ""
    )
    return dict(
        ...
        body=(
            ...
            f"change, not a change in what the bot is being asked.{repeat_note}\n\n"
...
def run(shared_dir: Path, *, dry_run: bool = False, now=None) -> dict:
    ...
    # {(bot, tier): {model_key}} ever rolled back or pinned — a swap that
    # reintroduces one of these is judged at the lower repeat floor.
    rejected = previously_rejected_models(shared_dir)
    ...
    for (bot_id, tier), swap in sorted(latest_swaps_by_rung(shared_dir).items()):
        ...
        # A rollback record's own "new" models are the RESTORED ones — never a
        # repeat. Only a forward swap that reintroduces a rejected model is.
        repeat = any(
            _model_key(m) in rejected.get((bot_id, tier), set())
            for m in (swap.get("new_models") or [])
        )
        verdict = evaluate(
            arms,
            min_target_turns=MIN_TARGET_TURNS_REPEAT if repeat else MIN_TARGET_TURNS,
            repeat_of_rejected=repeat,
        )
