...
Spec: internal/spec-fit-reviewer-2026-06-12.md §3.6 (Gate A cite-or-don't + Gate B
...
def _catalog_index(catalog: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    identity: see ``applications.app_identity.resolve_app_id`` — this indexes
    ``gallery/index.json`` rows by their own primary key, and every ``pkg_id``
    downstream in this module is a lookup into or a label out of this index.
    See the package docstring in ``fit_review/__init__.py`` for why a catalog
    row must not be run through the manifest resolver.
    """
...
def evaluate_candidate(
    ...
    *,
    ...
    # identity: see resolve_app_id — the gallery catalog key the candidate named; see fit_review/__init__.py.
...
def build_install_app_proposal(
    ...
    *,
    ...
    # identity: see resolve_app_id — the gate's own carried catalog key, asserted present before it becomes an InstallApp.
