...
Spec: internal/spec-fit-reviewer-2026-06-12.md §3.3 + §7 (Bite 1).
...
class GalleryMatch:
    ...
    identity: see ``applications.app_identity.resolve_app_id`` — a
    ``gallery/index.json`` row's primary key, carried verbatim so the
    eventual ``InstallApp`` names something the installer can find. See
    ``fit_review/__init__.py``.
    """
...
class TargetingReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            # identity: see resolve_app_id — GalleryMatch's own catalog key, surfaced in this report dict.
...
def _gallery_shortlist(
    ...
    for entry in catalog:
        if not isinstance(entry, dict):
            continue
        # identity: see resolve_app_id — reads the catalog row's primary key while building the shortlist.
...
def _decide(
    ...
    return (
        ...
        # identity: see resolve_app_id — the lead match's catalog key, rendered into the decision rationale.
...
def _installed_coverage(
    ...
    for f in sorted(app_dir.iterdir()):
        ...
        if not isinstance(data, dict):
            continue
        # identity: see resolve_app_id — an ACCUMULATOR, not a resolution.
        # This set is subtracted from the shortlist, so it must be a superset
        # of every id shape a catalog row might be recognised under; a
        # resolver returns ONE string and stops at the first non-empty field,
        # which would let an app the bot already has back onto the shortlist
        # whenever the manifest records its gallery key under a later rung.
        # Erring wide is the safe direction here: a missed exclusion is a
        # duplicate-install recommendation to the operator.
