...
Injects several things into every session's system prompt:
  ...
  3. Home report banner (primary bot only) — the LLM-generated narrative
     ...
     ``internal/diagnosis-evo-briefing-context-gap-2026-05-26.md`` failure
...
# ── Apps reference — progressive disclosure (overhead-budget B1) ─────────────
# Spec: internal/spec-evolve-overhead-budget-2026-07-31.md Phase B1.
#
# The coherence vocab, weekly app posture, and app-repair playbook together
# were ~64% (~2.4k tokens) of the session_start injection, carried by every
# session of every bot whether or not apps ever came up. They now live in ONE
# on-demand reference file in the bot's workspace — a location OpenClaw does
# NOT auto-ingest (its bootstrap set is a fixed filename list; verified
# against the installed gateway 2026-07-31) — and the prompt carries a
# compact stub telling the bot when to read it.
#
# Fail-safe: if the reference file cannot be written (read-only workspace,
# ad-hoc invocation outside the bot account), the blocks are inlined exactly
# as before. The bot must never silently lose access to the content.

APPS_REFERENCE_RELPATH = Path("evolve") / "reference" / "APPS_GUIDE.md"


def build_apps_reference_doc(
    coherence_vocab: str,
    app_posture_block: str,
    app_repair_block: str,
) -> str:
    """The full on-demand reference document. Pure."""
    parts = [
        "# Apps guide — vocabulary, posture, repair playbook",
        "",
        "Read this when a conversation involves your apps: app findings,",
        "coherence assertions (`C-A1`-style ids), app repair, or questions",
        "about your app inventory. It is intentionally NOT inlined into your",
        "system prompt — read it on demand, then act on what it says.",
    ]
    for section in (coherence_vocab, app_posture_block, app_repair_block):
        if section:
            parts += ["", section]
    return "\n".join(parts) + "\n"


def build_apps_reference_stub(relpath: Path, sections: "list[str]") -> str:
    """The compact inline block that replaces the three app sections."""
    return (
        "[APPS REFERENCE — read on demand]\n"
        f"Your {', '.join(sections)} live in `{relpath}` inside your "
        "workspace. READ that file BEFORE discussing app findings, coherence "
        "assertion ids, app repair, or your app inventory — it is the "
        "authoritative playbook and is deliberately not inlined here."
    )


def ensure_apps_reference(workspace_dir: Path, content: str) -> "Path | None":
    """Idempotently write the reference doc; return its path, or None on
    any failure (caller falls back to inlining). Atomic temp+rename; skips
    the write when content is unchanged."""
    try:
        dest = workspace_dir / APPS_REFERENCE_RELPATH
        dest.parent.mkdir(parents=True, exist_ok=True)
        import contextlib
        with contextlib.suppress(OSError):  # missing/unreadable → write fresh
        ...
        print(
            f"[session_surface] WARNING: could not write apps reference under "
            f"{workspace_dir}: {e} — inlining app blocks instead",
            file=sys.stderr,
        )
        return None
...
def load_autonomy_block(bot_id: str | None, shared_dir: Path) -> str:
    ...
    Spec: internal/spec-autonomy-ladder-2026-06-10.md §2.3 (bot_guidance
...
#
# Spec: internal/diagnosis-evo-briefing-context-gap-2026-05-26.md (Option B1).
...
def load_firing_signals_block(role: str | None, shared_dir: Path) -> str:
    ...
    is still reachable via ``pod_state(query="signals.firing")``. Grouped by
...
def _format_firing_signals_block(
    ...
    lines = [
        "[FIRING SIGNALS — live Signal-store snapshot at session start; "
        "use pod_state(query=\"signals.firing\") for an update mid-session]",
    ]
    ...
    if total > shown:
        lines.append(
            ...
            "For full list call pod_state(query=\"signals.firing\").)"
        )
    else:
        lines.append(
            f"({total} signal{'s' if total != 1 else ''} total. "
            "For details call pod_state(query=\"signals.firing\").)"
        )
...
def _truncate_at_line_boundary(text: str, cap_bytes: int) -> str:
    ...
    if len(encoded) <= cap_bytes:
        return text
    tail = "\n(...truncated; call pod_state(query=\"signals.firing\") for the full list.)"
...
def load_primary_block(role: str | None) -> str:
    ...
    Spec: internal/spec-primary-bot-interface-2026-05-14.md §7. Tells the
...
def load_help_sidebar_block(role: str | None, shared_dir: Path) -> str:
    ...
    Spec: internal/spec-primary-bot-interface-2026-05-14.md §4.2.
...
_APP_FINDINGS_DEEP_DIVE_FOOTER = (
    ...
    "\n"
    "Assertion ids (C-A1…C-A8, C1-1…C1-4), severity meanings, and "
    "reconciliation status values are defined in the [COHERENCE VOCAB] "
    "reference — inlined earlier in this prompt, or in "
    "`evolve/reference/APPS_GUIDE.md` in your workspace (read it before "
...
def load_capabilities_block(bot_id: str | None, shared_dir: Path) -> str:
    ...
    Spec: internal/spec-bot-capability-awareness-2026-06-22.md §3 (PUSH). Lists
...
def build_session_prefix(
    ...
    # Overhead-budget B1 (progressive disclosure): when the apps reference
    # file was written, main() passes the stub here and blanks the three
    # inline app blocks; inline_coherence_vocab=False suppresses the
    # module-level vocab constant. Defaults preserve the legacy inline
    # behavior for existing callers/tests.
    apps_reference_stub: str = "",
    inline_coherence_vocab: bool = True,
) -> str:
    """
    ...
    if COHERENCE_VOCAB and inline_coherence_vocab:
        ...
        # Suppressed (inline_coherence_vocab=False) when the vocab ships
        # via the apps reference file instead (overhead-budget B1).
        parts.append(COHERENCE_VOCAB)
    if apps_reference_stub:
        # B1 progressive disclosure: the compact pointer to the on-demand
        # apps reference (vocab + posture + repair). Lands where the vocab
        # sat — early, above any app_findings block, so the bot knows where
        # id definitions live before it reads findings that use them.
        parts.append(apps_reference_stub)
    if guide_block:
    ...
    if capabilities_block:
        ...
        # reason — see internal/spec-bot-capability-awareness §5 P1). The
...
def main() -> None:
    ...
    parser.add_argument(
        "--workspace-dir",
        help=(
            "Bot workspace dir for the on-demand apps reference file "
            "(default: ~/.openclaw/workspace — correct when invoked by the "
            "gateway as the bot user). Unwritable → app blocks inline as "
            "before (fail-safe)."
        ),
    )
    parser.add_argument(
    ...
    # ── B1 progressive disclosure: apps blocks → on-demand reference ─────────
    # The vocab + posture + repair trio was ~64% of this injection. Write it
    # to the workspace reference file and inject the compact stub instead.
    # Any write failure falls back to inlining all three (fail-safe: content
    # must never be silently lost). Spec: spec-evolve-overhead-budget §B1.
    apps_reference_stub = ""
    inline_vocab = True
    apps_sections: "list[str]" = []
    if COHERENCE_VOCAB:
        apps_sections.append("app-finding vocabulary (coherence assertion ids)")
    if app_posture_block:
        apps_sections.append("weekly app-posture inventory")
    if app_repair_prompt_block:
        apps_sections.append("app-repair playbook")
    if apps_sections:
        workspace_dir = (
            Path(args.workspace_dir) if args.workspace_dir
            else Path.home() / ".openclaw" / "workspace"
        )
        doc = build_apps_reference_doc(
            COHERENCE_VOCAB, app_posture_block, app_repair_prompt_block)
        if ensure_apps_reference(workspace_dir, doc) is not None:
            apps_reference_stub = build_apps_reference_stub(
                APPS_REFERENCE_RELPATH, apps_sections)
            inline_vocab = False
            app_posture_block = ""
            app_repair_prompt_block = ""
    ...
    if args.as_json:
        print(json.dumps({
            ...
            # B1: non-empty when the apps blocks shipped via the reference
            # file (app_posture/app_repair_prompt above are then blank and
            # coherence_vocab is NOT injected despite being listed here).
            "apps_reference_stub": apps_reference_stub,
        }))
    else:
        print(build_session_prefix(
            ...
            apps_reference_stub=apps_reference_stub,
            inline_coherence_vocab=inline_vocab,
        ))
