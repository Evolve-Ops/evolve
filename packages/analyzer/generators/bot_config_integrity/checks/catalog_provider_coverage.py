...
def run(ctx, cfg: dict) -> list[Proposal]:
    ...
    for provider in sorted(credentialed):
        ...
        # Cross-vendor improvement (judge-role collapse §5.5 reframing): a
        # credentialed provider whose vendor differs from the one Standard
        # actually routes on buys failover AND a cross-vendor option for the
        # ``resolve_cross_vendor`` derivation once its models are on the
        # chains. An improvement, not an operational fix — tracked separately
        # so the copy stays honest.
        _std_model = (severities.get("standard") or {}).get("model") or ""
        _std_vendor = (
            _std_model.split("/", 1)[0].lower() if "/" in _std_model else None
        )
        adds_cross_vendor = bool(_std_vendor) and provider != _std_vendor
        if fixed_roles:
            urgency = "operational_urgent"
        elif adds_cross_vendor:
            urgency = "improvement"
        else:
            urgency = "hygiene"
        ...
        if fixed_roles:
            ...
            closing = (
                f"\n\nRecommended (not required): {provider} is a different "
                f"provider than the one Standard routes on, so adding its "
                f"models buys {ctx.bot_id} failover when the primary vendor is "
                f"rate-limited or down, and a cross-vendor option for "
                f"independent self-checking. Everything routes today — this is "
                f"a quality improvement, not a fix."
            )
        else:
            closing = (
        ...
        proposal = Proposal(
            ...
            provenance=Provenance(
                ...
                signals={
                    ...
                    # leaves a role with no routable model; "advisory" if it
                    # only adds a cross-vendor option (failover + independent
                    # self-checking — judge-role collapse §5.5); else "soft"
                    # hygiene. Plus the roles the fix repairs.
                    "coverage_severity": (
                        "hard_break" if fixed_roles
                        else "advisory" if adds_cross_vendor
                        else "soft"
                    ),
                    "fixes_hard_break_roles": fixed_roles,
                    "adds_cross_vendor_option": adds_cross_vendor,
                },
