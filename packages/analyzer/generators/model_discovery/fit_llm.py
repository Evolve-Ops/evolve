...
a prompt-cached system prompt, env-var-or-auth-profiles key resolution,
JSON-only response with a strict fallback, and **FAIL-OPEN** on every error
(no role resolves, no key, SDK missing, call error, unparseable / low-confidence
output → ``None``, so the caller keeps the deterministic verdict).

Two invariants set it apart from the precedent:

  - **No hardcoded model id** (the no-provider-literals rule): the model to call
    is resolved from the pod's ``fast`` (else ``judge``) ROLE via the catalog
    roles map — :func:`resolve_fit_model`. The precedent hardcodes
    ``claude-haiku-4-5``; this must not.
...
def make_fit_classifier(
    client: Any, *, model: str, max_tokens: int = DEFAULT_MAX_TOKENS,
    known_families: list[str] | None = None,
) -> Callable[[Any], dict | None]:
    """Build a classifier callable backed by ``client.messages.create``.

    ``model`` is resolved from the pod's role map by :func:`resolve_fit_model`,
    NEVER hardcoded. ``known_families`` is the pod's adopted model-family list
    (trusted, from :func:`resolve_known_families`) so the model can perform the
    ``mode_variant`` family tie; it is constant for the run, so it is bound once
    here rather than re-resolved per finding. The returned callable takes a
    ``DiscoveryFinding`` and returns ``{verdict, recommended_role, confidence,
    reason}`` or ``None`` (fail-open on any API/parse error)."""
...
# ── Model + key resolution (no provider/model literals) ───────────────────────

def resolve_fit_model(network: dict | None) -> str | None:
    """Bare model id to run the fit call on — the pod's ``fast`` role (else
    ``judge``), resolved through the catalog roles map. NEVER a hardcoded id.

    Returns the bare id (provider prefix stripped for the SDK call) or ``None``
    when neither role resolves to a model (the caller then runs deterministic
    only). Provider-neutral: a remapped role pointing at a non-Anthropic model
    simply yields that model's id; the Anthropic-keyed call then fails and the
    caller falls open — no provider branch here."""
    try:
    ...
    try:
        catalog = merge_model_catalog(pod_models, None)
    except Exception:
        return None
    for role in ("fast", "judge"):
        try:
            res = resolve_role_with_availability(catalog, role, None)
        except Exception:
            continue
...
def _resolve_key(network: dict | None) -> str | None:
    """Resolve the API key for the fit call (env var → primary bot auth-profiles).
    Reuses the engine's existing background-call key resolver."""
    try:
        from primary_bot import read_primary_bot_anthropic_key  # type: ignore
        return read_primary_bot_anthropic_key(network) or None
    except Exception:
        return None


def _default_client_factory(key: str) -> Any | None:
    """Build the SDK client, or ``None`` when the SDK isn't installed / fails."""
    try:
        from anthropic import Anthropic  # type: ignore[import-not-found]
    except ImportError as exc:
        logger.warning("fit_llm: anthropic SDK not installed (%s); deterministic only", exc)
        return None
    try:
        return Anthropic(api_key=key)
    except Exception as exc:  # noqa: BLE001
        logger.warning("fit_llm: client build failed: %s", type(exc).__name__)
        return None
...
def build_default_classifier(
    network: dict | None,
    *,
    model_resolver: Callable[[dict | None], str | None] | None = None,
    key_resolver: Callable[[dict | None], str | None] | None = None,
    ...
    Returns ``None`` (the caller then runs deterministic-only) when any
    prerequisite is missing — no ``fast``/``judge`` role resolves, no key, or
    the SDK isn't installed. Every seam is injectable so tests exercise each
    fail-open branch without touching the network. This is FAIL-OPEN by
    construction: discovery never depends on the LLM."""
    resolve_model = model_resolver or resolve_fit_model
    resolve_key = key_resolver or _resolve_key
    make_client = client_factory or _default_client_factory

    model = resolve_model(network)
    if not model:
        logger.info("fit_llm: no fast/judge role resolves to a model; deterministic only")
        return None
    key = resolve_key(network)
    if not key:
        logger.info("fit_llm: no API key available; deterministic only")
        return None
    ...
    return make_fit_classifier(client, model=model, known_families=known_families)
