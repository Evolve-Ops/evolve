...
def _build_proposal(sig, ctx: EngagementAmplifierContext) -> Proposal | None:
    ...
    return Proposal(
        ...
        dimension=DIMENSION,
        # Value-free trigger identity ONLY. The arbiter dedup fingerprint
        # hashes trigger_observations, so any measured value here (the
        # running engagement total, the alignment classification) mints a
        # fresh fingerprint every cycle and defeats dedup entirely — the
        # 2026-08-30 alerts review found 32 near-duplicate pendings from
        # exactly that. Measured values live in provenance.signals and the
        # summary/explanation prose, which refresh on re-detection.
        ...
        dismiss_scope="kind",
        # Producer-legibility ratchet: value-free per-cluster key (same
        # identity as the dismiss signature) + count-agnostic title. A
        # re-emission of the same (bot, noun, verb) cluster folds into the
        # existing pending parent instead of minting a new file.
        coalesce_key=_dismiss_signature(bot_id, noun, verb),
        human_title=headline,
    )
