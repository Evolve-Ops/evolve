...
Builds a fast-role prompt that takes (transcript, current_profile) and
asks "what new facts about the speaker came up that aren't already
recorded?" Returns a list of FactCandidate. Per-section confidence
thresholds are applied by the caller (hook.py) — the extractor itself
just produces candidates with confidence scores.

I/O-free: takes a callable for the LLM (so tests inject a stub) and
returns dataclasses. Hook layer handles the file writing (and resolves
the bot's credentialed provider — #3466 provider-agnostic).

Cost target: one fast-tier call per session. Typical session is ~2-3K
...
# Sentinel meaning "the model resolved from the bot's own credentials"
# (#3466: provider-agnostic — the production LLMCallable built by
# hook._build_default_llm_call carries the resolved model in its target
# and ignores this argument; tests inject stubs).
DEFAULT_MODEL: str = ""
...
# a deterministic stub without any network dependency.
    ...
    *,
    ...
    Args:
        ...
        llm_call: callable that runs the LLM. Tests inject a stub; production
            wires it to the bot's own credentialed provider (hook.py).
        model: which model to use. Default "" = the model carried by the
            production callable's resolved target.
