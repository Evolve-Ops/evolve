...
The LLM call uses the bot user's OWN credentials, looked up from
``~/.openclaw/credentials/<provider>.json`` (the canonical bot-side
credential location) for any known LLM provider — never presuming a
specific one (#3466 / docs/principle-llm-provider-agnostic.md). The
hook never sees the evolve-user or admin-side key store. If no key is
available the hook logs and exits 0 — DNT is the only condition under
which a user's session is silently skipped; auth-failure is loud
    ...
    *,
    ...
    Args:
        ...
        llm_call: testing seam — production callers pass None and we wire
            up the bot's own credentialed provider; tests pass a stub.
    ...
    # Resolve the LLM call. Tests pass their own; production wires the
    # bot's own credentialed provider via infra_llm.
    if llm_call is None:
...
def _build_default_llm_call(home_dir: Path) -> Optional[LLMCallable]:
    ...
    ``~/.openclaw/credentials/<provider>.json`` — any known LLM
    provider's file counts; none is presumed present (#3466).

    Returns None when no provider is credentialed — the hook then logs
    and exits as ``no_llm_credentials``.

    The returned callable ignores the ``model`` argument threaded
    through :func:`extract_facts` (its default is the ``""`` sentinel):
    the resolved target already carries the provider-appropriate fast
    model. A non-empty explicit ``model`` from the caller still wins.
    """
    target = _resolve_llm_target(home_dir)
    if target is None:
        logger.warning(
            "user_profile_inferrer: no LLM provider credentialed for bot at %s; "
            "skipping",
            home_dir,
        )
        return None
    ...
    def _call(system: str, user_msg: str, model: str, max_tokens: int) -> str:
        # Explicit caller pin (bare id) rides on the resolved provider.
        call_target = replace(target, model=model) if model else target
        return complete(
            call_target,
            prompt=user_msg,
            system=system,
...
_CREDENTIALS_DIR_REL = ".openclaw/credentials"


def _read_credential_file(path: Path) -> Optional[str]:
    """Extract the secret from one ``credentials/<provider>.json`` file."""
    try:
...
def _resolve_llm_target(home_dir: Path):
    """Resolve an ``infra_llm.InfraLLMTarget`` from the bot's OWN credentials.

    Reads ``~/.openclaw/credentials/<provider>.json`` for every known LLM
    provider (the generalization of the old anthropic-only reader — no
    provider is presumed), with the per-provider env vars
    (``ANTHROPIC_API_KEY``, ``OPENAI_API_KEY``, …) as overrides for
    ad-hoc test harnesses. The model is the catalog's fast-tier pick for
    the first credentialed provider (``models.derive_default_tiers``).

    Deliberately does NOT use ``infra_llm.resolve_infra_llm`` — that
    reads the PRIMARY bot's key store; this hook runs bot-side and must
    only ever see the bot's own credentials.
    """
    ...
    from primary_bot import _PROVIDER_KEY_ENV_VARS  # type: ignore

    keys: dict[str, str] = {}
    cred_dir = Path(home_dir) / _CREDENTIALS_DIR_REL
    for provider in sorted(_KNOWN_LLM_PROVIDERS):
        secret = _read_credential_file(cred_dir / f"{provider}.json")
        if secret:
            keys[provider] = secret
    for provider, env_names in _PROVIDER_KEY_ENV_VARS:
    ...
    tier = normalize_to_tier("fast")
    derived = derive_default_tiers(set(keys))
    for model in derived.get(tier, {}).get("models", []):
        if "/" not in model:
            continue
        provider = model.split("/", 1)[0]
        if provider in keys:
            return InfraLLMTarget(provider=provider, model=model, api_key=keys[provider])
    return None
