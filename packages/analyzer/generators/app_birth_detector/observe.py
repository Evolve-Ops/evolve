...
def observe(ctx: DetectorContext) -> list[Proposal]:
    ...
    """
    # ctx.bot_id is the logical name, not the macOS account (team-bot-b runs on
    # the personal-bot-user account) — resolve it rather than joining onto /Users/.
    workspace = ctx.workspace_root
    if workspace is None:
        from evolve_config import bot_home
        workspace = bot_home(ctx.bot_id) / ".openclaw" / "workspace"
    try:
        if not workspace.exists():
            return []
    except (PermissionError, OSError):
        # A bare .exists() under a bot's .openclaw RAISES rather than
        # returning False when the OC gateway clamps the ACL mask to ---
        # (#3184, Linux/Py3.12). Unreachable is not the same as absent, but
        # this detector has nothing to observe either way — skip this pass
