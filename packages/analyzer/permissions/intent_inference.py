...
def _call_llm(prompt: str, *,
    ...
    """Invoke ``openclaw llm complete`` with the given prompt. Returns
    raw stdout on success, ``None`` on any failure.

    Single-shot wrapper that exists primarily as a test seam — every
    test of the inference layer monkey-patches this function and asserts
    on the prompt it received, never running a real LLM. Production
    invocations shell out to the same ``openclaw`` CLI the rest of
    Evolve's analyzer code uses (see ``community_intel.py``,
    ``digest_source_audit.py``).
    """
    try:
        proc = subprocess.run(
