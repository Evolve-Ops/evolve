...
Spec: internal/spec-exec-outcome-watchdog-2026-05-28.md.
...
def read_turn_annotations(
    ...
    *,
    ...
    record_type: str = "turn_annotation",
) -> list[dict]:
    ...
    ``record_type`` selects which record kind to return. The file is a
    MIXED store — ``TurnObserver`` appends ``turn_annotation`` rows and
    ``SessionSummarizer`` appends ``session_summary`` rows to the same
    daily file — so a caller that wants the other kind gets it here
    rather than opening the same paths a second time. This function is
    the single windowed reader for that store (``usage_by_app`` reads
    the turn rows through it, ``usage_by_user`` the summary rows); the
    default keeps every pre-existing caller on the turn rows unchanged.
    """
