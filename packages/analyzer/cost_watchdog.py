...
DEFAULTS: dict[str, Any] = {
    ...
    # internal/forensic-security_bot-model-override-2026-05-21.md).
    ...
    # surface. See internal/incident-cost-alerting-blackout-2026-05-20.md.
...
# Spec: internal/spec-severity-framework-2026-05-18.md §2.2
...
def _resolve_per_bot_cap(
    ...
    Resolution:
      1. ``BetterEngineConfig.resolve_budget_ladder(bot_id)["l1_breaker"]`` —
         the single reader shared with ``spend_alert`` (the enforcer),
         ``spend_caps.get_caps_config`` (the UI) and the evo cost-cap tools.
         It covers monthly-budget derivation, the explicit per-bot override,
         the graduated new-bot default, and the ``pod_defaults.budget``
         fallback. This detector previously read only the explicit per-bot
         field and so saw ``None`` — i.e. sized its heartbeat-cost warnings
         against no cap at all — for every bot running on the pod default.
      2. ``network.json::dailySpendCapUsd`` (legacy pod-wide cap; emptied by
         the Phase-8 migration, retained for un-migrated installs).
    ...
    if shared_dir is not None:
        try:
            ...
            cap = be_config.resolve_budget_ladder(bot_id)["l1_breaker"]
            if cap is not None:
                return cap
        except Exception:
    ...
    *,
    ...
    fire). Otherwise returns:
      ...
      - magnitude: 1 | 2 | 3 (matches the cost-vector anchor table in
        internal/spec-severity-framework-2026-05-18.md §2.2 — the bigger
