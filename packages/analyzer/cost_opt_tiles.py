...
def build_tile(
    ...
    *,
    ...
    # Which experience-preserving cost levers are on for this bot (D-CS5:
    # on by default, per-bot opt-out). Its own field, not a chip — the chip
    # row truncates at three, and "is this lever on?" must always be legible.
    try:
        from cost_levers import levers_for_bot
        levers = levers_for_bot(network or {}, bot_id)
    except Exception:
        levers = {}
