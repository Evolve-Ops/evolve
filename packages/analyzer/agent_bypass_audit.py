...
``docs/spec-agent-freelance-bypass-2026-06-05.md``): when ``bot_guidance``
...
(docs/spec-agent-freelance-bypass-phase2-2026-06-06.md), the audit
...
def _build_at_risk_apps_from_manifest(manifest: Any) -> list[AtRiskApp]:
    ...
    manifest_id = (
        getattr(manifest, "pkg_id", None)
        or getattr(manifest, "id", None)
        or getattr(manifest, "display_name", None)
        or "unknown"
    )
...
def _spec_for_bypass(totals: BotAppAuditTotals, lookback_hours: int) -> dict:
    ...
    body_lines.append(
        "See `docs/spec-agent-freelance-bypass-2026-06-05.md` for the "
