...
L1, internal/design-linux-port-2026-06-10.md §2; tests supply
``FakeIsolation``). Design:
``internal/design-phase4.3c-scheduler-isolation-seams-2026-06-10.md`` (Track I).
...
* Extra read primitives (``user_exists`` / ``used_uids`` / ``next_free_uid``
  / ``group_members`` / ``list_accounts``) and account attributes (``set_password`` /
...
class Account:
    """One host login account, as ``list_accounts`` reports it.

    Deliberately NOT an :class:`Identity`: an Account has no ``bot_id``,
    because the whole point of enumerating accounts is to find the ones that
    no longer map to a bot. Resolution runs the other direction (roster →
    account, via ``resolve``); this is the raw host inventory.
    """
...
# Login accounts that are never bot accounts on either platform. Kept minimal
# on purpose: the real filter is "does this account carry an Evolve install",
# not a name list. These are only here so an enumerator doesn't hand callers
# obvious noise (macOS's ``_``-prefixed service records are filtered by the
# adapter itself, which is a platform detail rather than policy).
SYSTEM_ACCOUNTS: frozenset[str] = frozenset(
    {"root", "daemon", "nobody", "Guest", "sync", "bin", "sys"}
)
...
def _getpwall_accounts() -> "tuple[Account, ...]":
    """Host login accounts via ``pwd.getpwall()`` — the shared real-adapter leg.

    POSIX, so macOS and Linux use the identical implementation: on macOS
    ``getpwall()`` enumerates the DirectoryService local node (verified against
    the reference pod — it returns every ``/Users`` record, uid and home
    included), and on Linux it walks NSS. That is why this is not a
    ``dscl``/``getent`` subprocess pair: there is nothing platform-specific to
    branch on, and no argv means no sudoers grant and no platform-path-lint
    surface.

    Filtering is deliberately minimal — macOS's ``_``-prefixed service records
    and :data:`SYSTEM_ACCOUNTS`. Anything else is a real login account and the
    caller decides what it means. Duplicate rows (``getpwall`` yields ``root``
    and ``nobody`` twice on macOS) are collapsed by username.

    Raises ``IsolationError`` when the enumeration comes back empty. An empty
    list is never true on a live host, so it means NSS/DirectoryService is not
    answering — and a caller that took it at face value would conclude "no
    accounts exist", which for a sweep-style monitor reads as "every finding
    cleared". Failing loudly is the only safe direction.
    """
    seen: "dict[str, Account]" = {}
    try:
        rows = pwd.getpwall()
    except OSError as exc:  # NSS/DirectoryService unreachable
        raise IsolationError(f"account enumeration failed: {exc}") from exc
    for pw in rows:
        name = pw.pw_name
        if not name or name.startswith("_") or name in SYSTEM_ACCOUNTS:
            continue
        seen.setdefault(
            name, Account(user=name, uid=pw.pw_uid, home=Path(pw.pw_dir))
        )
    if not seen:
        raise IsolationError(
            "account enumeration returned no login accounts — the directory "
            "service is not answering (an empty host is not a real state)"
        )
    return tuple(sorted(seen.values(), key=lambda a: a.uid))
...
class MacOSIsolation:
    ...
    def list_accounts(self) -> "tuple[Account, ...]":
        """Every non-system login account on the host (see
        :func:`_getpwall_accounts` for why this is not a ``dscl`` call)."""
        return _getpwall_accounts()
...
class LinuxUserIsolation:
    ...
    (Linux port L1, internal/design-linux-port-2026-06-10.md §2). A direct
    ...
    def list_accounts(self) -> "tuple[Account, ...]":
        """Every non-system login account on the host — the identical
        ``pwd.getpwall()`` walk the macOS adapter uses (POSIX; nothing here
...
class FakeIsolation:
    ...
    def list_accounts(self) -> "tuple[Account, ...]":
        """The seeded user table as Accounts.

        Mirrors the real adapters' fail-loud contract: an empty table raises
        rather than reporting "no accounts", so a test that forgets to seed
        exercises the caller's blind path instead of silently asserting
        against an empty sweep. Seed at least one user (``seed_user``) to get
        a normal listing.
        """
        if not self.users:
            raise IsolationError(
                "account enumeration returned no login accounts "
                "(FakeIsolation has no seeded users)"
            )
            ...
            sorted(
                (
                    Account(
                        user=user,
                        uid=uid,
                        home=self.homes.get(user) or home_root / user,
                    )
                    for user, uid in self.users.items()
                ),
                key=lambda a: a.uid,
            )
        )
