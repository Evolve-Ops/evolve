...
Design: internal/design-linux-port-2026-06-10.md §4. Evolve's read/write
...
**No privileged ACL command is issued against a redirected path.** Every
argv this seam emits — ``setfacl``'s grant / carve-out / mask-repair forms
and macOS's ``chmod +a`` / ``chmod -N`` — is a ROOT command that FOLLOWS a
symlink given as its path argument (live-verified on the Ubuntu pod: a root
``setfacl -m m::rwX <link>`` widens the LINK TARGET's mask; ``-P`` skips the
argument but silently returns 0 and would need new sudoers grants). The seam
is the chokepoint where that is refused: :func:`_redirect_safe` gates both
backends' privileged emitters and the Linux ``getfacl`` probes that decide
whether to fire one, via ``evolve_util.assert_no_symlink_in_path``. The gate
lives HERE rather than at the call sites because the call sites are ~25
``grant*`` / ``clear_acl`` / ``reassert_mask`` invocations across deploy.py,
audit.py and secret_config_perms.py, and several of them (the secret-relpath
parent dirs, the ``workspace/`` retro-grant loop) name paths whose every
component below ``.openclaw`` the bot user can replace.
...
from evolve_util import assert_no_symlink_in_path as _assert_no_symlink_in_path
...
def _redirect_safe(path: "Path | str", what: str) -> bool:
    """``assert_no_symlink_in_path`` in bool form — the shape this seam needs.

    Every emitter here is best-effort (``bool`` return, callers converge on the
    next pass or escalate off a fresh VERIFY), so a refusal is a False plus a
    log, never a raise into a deploy.

    Two log levels, because ``reporter.emit_error_signals`` mirrors every ERROR
    admin-log line into the Signal store — so the level chosen here decides
    whether a refusal pages an operator:

    * a replaced component (symlink / hard link / ``..``) is attack-shaped and
      an operator SHOULD see it → ERROR;
    * an unverifiable component (``.unverifiable``; in practice an EACCES from a
      gateway mask clamp racing the hourly reassert) is benign and self-healing
      on the next pass → WARNING. The pre-existing behaviour in exactly that
      state was a SILENT no-op — the getfacl probe returned nothing and the
      repair skipped itself — so a warning is already strictly more visible
      than what it replaces, and an ERROR there would manufacture an alert for
      a condition that was never one.
    """
    try:
        _assert_no_symlink_in_path(path)
        return True
    except PermissionError as e:
        log = _log.warning if getattr(e, "unverifiable", False) else _log.error
        log("%s refused: %s", what, e)
        return False
...
class MacOSPerms:
    ...
    def _chmod_plus_a(self, ace: str, path: Path, *, recursive: bool, timeout: int) -> bool:
        ...
        ``chmod`` follows a symlink argument (``-h`` would not, but adding it
        changes the byte-exact argv the sudoers grants match), so the
        destination is gated first. macOS's ``reassert_mask`` is a no-op and so
        is not the reachable leg here — ``grant``/``clear_acl`` are: the
        ``workspace/`` retro-grant loop in ``set_evolve_read_acl`` selects
        members with ``is_file()``, which FOLLOWS, so a bot-planted link there
        aims this root ``chmod +a`` at its target."""
        if not _redirect_safe(path, f"chmod +a {ace}"):
            return False
    ...
    def clear_acl(self, path: Path, *, recursive: bool = False) -> bool:
        ...
        timeout matches its historical call site. Gated like ``+a`` — this is
        the carve-out primitive, so following a planted link would STRIP a
        victim's ACL rather than add one."""
        if not _redirect_safe(path, "chmod -N"):
            return False
...
class LinuxPerms:
    ...
    def _setfacl(self, args: "list[str]", *, path: "Path | str",
                 timeout: int = 10) -> bool:
        """One ``sudo setfacl`` — the single privileged-argv chokepoint.

        ``path`` is the target the command will resolve; it is also the last
        element of ``args``, but it is passed EXPLICITLY rather than sniffed
        from ``args[-1]`` so a future argv shape cannot quietly opt out of the
        gate. ``setfacl`` follows a symlink given as its path argument
        (verified live on the Ubuntu 24.04 pod, acl 2.3.2 — the man page's
        "``-P`` ... only effective in combination with ``-R``" caveat is
        misleading, but ``-P`` returns 0 while doing nothing and would need
        matching sudoers grants, so a pre-check beats a flag). ``-R`` does NOT
        descend through child symlinks, so the argument is the whole exposure.
        """
        if not _redirect_safe(path, f"setfacl {' '.join(args[:-1])}"):
            return False
        try:
    ...
    def grant_traverse(self, path: Path, user: str) -> bool:
        ...
        return self._setfacl(["-m", f"u:{user}:--x", str(path)], path=path)
    ...
    def clear_acl(self, path: Path, *, recursive: bool = False) -> bool:
        ...
        timeout = 30 if recursive else 10
        ok = self._setfacl([*rflag, "-b", str(path)], path=path, timeout=timeout)
        if recursive or _is_dir(path):
            self._setfacl([*rflag, "-k", str(path)], path=path, timeout=timeout)
        return ok
    ...
    def _getfacl_lines(self, path: Path) -> "list[str]":
        ...
        Gated on the same redirect check as the privileged emitters even though
        ``getfacl`` itself is unprivileged and NOT sudo-granted (setup_wizard
        §9b3 removed those grants in 2026-06-11). Two reasons: it FOLLOWS a
        symlink argument too (verified live), so on a planted path it reports
        the VICTIM's ACL — and ``reassert_mask`` uses exactly that report to
        decide whether to fire the root ``setfacl``. Refusing here means the
        decision is never taken on a redirected observation. The existing
        ``[]``-on-error contract carries the refusal: the effective-perm checks
        read it as "contract not satisfied" and escalate, which is the right
        answer for a path whose components have been replaced."""
        if not _redirect_safe(path, "getfacl"):
            return []
        try:
    ...
    def _masked_paths_recursive(self, path: Path) -> "list[str]":
        ...
        Gated on the root argument like every other emitter. The enumerated
        CHILDREN need no separate gate on this account — ``getfacl -R`` does not
        descend through a child symlink (verified live) — but they get one
        anyway when each is handed back to ``_setfacl``."""
        if not _redirect_safe(path, "getfacl -R -s"):
            return []
        try:
            ...
            return []
        # Only paths genuinely UNDER the argument are returned. `getfacl`'s
        # `# file:` headers are attacker-influenced data — the filenames come
        # from the bot's own tree — and each one becomes the argv of a root
        # `setfacl`. acl >= 2.2.52 escapes `\n` in those headers (acl 2.3.2 on
        # the VPS does), so a filename cannot forge a block boundary today; this
        # filter makes that a belt rather than the whole trousers, since the
        # cost is one string compare and the failure mode is a root ACL write on
        # a path of the bot's choosing. Components inside the returned paths are
        # still re-gated when each goes back through `_setfacl`.
        ...
        for line in (proc.stdout or "").splitlines() + [""]:
            ...
            if text.startswith("# file:"):
                ...
                if (current is not None and has_mask and capped
                        and (current == root or current.startswith(prefix))):
                    out.append(current)
    ...
    def reassert_mask(self, path: Path, *, recursive: bool = False) -> bool:
        ...
        Redirect gate FIRST, ahead of the getfacl probe, so a planted component
        is refused with a False rather than reaching the ``[]``-on-refusal path
        below — which the non-recursive form would read as "no mask, nothing to
        do" and report as a healthy ``True``. This is the leg the hourly
        ``reassert_evolve_access`` sweep aims at ``<oc>/agents``,
        ``<oc>/agents/main``, ``<oc>/agents/main/agent``, ``<oc>/workspace`` and
        ``<oc>/workspace/.git`` — every one of them a directory the BOT can
        replace with a symlink, on a root command, once an hour.
        """
        if not _redirect_safe(path, "reassert_mask"):
            return False
        if not recursive:
            lines = self._getfacl_lines(path)
            if not (self._has_access_mask(lines) and self._mask_caps_entries(lines)):
                return True
            return self._setfacl(["-m", "m::rwX", str(path)], path=path)
        ok = True
        for masked in self._masked_paths_recursive(path):
            ok = self._setfacl(["-m", "m::rwX", masked], path=masked) and ok
        return ok
