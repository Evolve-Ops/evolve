...
class LinuxPerms:
    ...
    def _masked_paths_recursive(self, path: Path) -> "list[str]":
        ...
        mask entry. ``getfacl -R -s`` skips base-entries-only files (so
        carve-outs like credentials/ never appear); ``-p`` keeps paths
        absolute. Output parses regardless of exit code — ``-R`` returns
        nonzero if any one subpath errored but still lists the rest, and
        mask repair is best-effort."""
        try:
        ...
        for line in (proc.stdout or "").splitlines() + [""]:
            ...
            if text.startswith("# file:"):
                ...
                if current is not None and has_mask:
                    out.append(current)
                current, has_mask = None, False
        return out
    ...
    def reassert_mask(self, path: Path, *, recursive: bool = False) -> bool:
        ...
        ACL where none belongs — e.g. the credentials/ carve-out). The
        recursive form therefore never runs ``setfacl -R``: it
        enumerates the ACL'd paths under the tree (``getfacl -R -s``)
        and repairs only those, so a tree whose root has no ACL still
        gets masked children repaired, and non-ACL'd children are never
        touched.
        """
        if not recursive:
            if not self._has_access_mask(self._getfacl_lines(path)):
                return True
