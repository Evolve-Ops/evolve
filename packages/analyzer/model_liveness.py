...
Spec: [docs/spec-model-auto-upgrade-2026-07-30.md](../../docs/spec-model-auto-upgrade-2026-07-30.md)
...
fleet's measurements), plus a one-word output. At the daily cadence
(:data:`CADENCE_SECONDS`) and the :data:`MAX_PROBES_PER_RUN` cap that is
bounded (~8 turns/day, roughly low-single-digit dollars/month depending on
which rungs resolve), and it lands on each probing bot's own cost
accounting, where it is indistinguishable from real usage. The fresh
per-probe session ids also accrue small OC session files in the bot's
state; accepted — the alternative (one stable session) grows turn context
