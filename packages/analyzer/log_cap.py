...
DEFAULT_TARGETS: tuple[Path, ...] = (
    ...
    # spend_alert.log was the one daemon log missing here: its siblings were
    # all capped, so nothing bounded the 5-minute tick's append. The
    # `daemon_operator_logs` output declaration names this function as its
    # pruner, and a declaration whose pruner does not actually touch the file
    # is the kind of control that passes by saying nothing (D-CS7).
    _LOG_DIR / "spend_alert.log",
