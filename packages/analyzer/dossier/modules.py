...
def _app_label(app_id: str, names: dict[str, str] | None) -> str:
    """An app's name from the report's one table, or an honest stand-in.

    ``None`` means no table was built (a caller that predates it), and the
    id is said out loud as before. With a table, an id it cannot name reads
    ``unknown app (<id>)`` — visible, never silently the id, never dropped.
    """
    if names is None:
        return hl.humanize_id(app_id)
    return names.get(app_id) or hl.say("apps.unknown_app", app_id=app_id)


def _apps(edition: dict[str, Any],
          names: dict[str, str] | None = None) -> _Draft:
    ...
    if names is not None:
        # A missing manifest is a finding: say which ids had no name.
        unnamed = [a for a, _ in used if a not in names]
        detail["apps_without_a_name"] = {"count": len(unnamed), "ids": unnamed}
    values = {
        "apps_used": len(used),
        "requests_total": total_turns,
        "top": [{"app_id": a, "name": _app_label(a, names), "requests": t}
    ...
    if starved:
        ...
        sentence = hl.say(key, app=_app_label(top_id, names),
                          turns=top_turns, count=len(used))
        # "unknown app (…)" opens the sentence; a sentence opens capitalised.
...
def _reliability(edition: dict[str, Any],
                 names: dict[str, str] | None = None) -> _Draft:
    ...
    ranked = [
        {**row, "app_id": app_id, "name": _app_label(app_id, names)}
...
def _cost(edition: dict[str, Any], _names: Any = None) -> _Draft:
...
def _users(edition: dict[str, Any], _names: Any = None) -> _Draft:
...
#: Every builder is ``(edition, app_names) -> _Draft``; the table is the
#: report's one app-name lookup (``dossier.names``), or ``None``.
_Builder = Callable[[dict[str, Any], "dict[str, str] | None"], _Draft]

SPECS: tuple[tuple[ModuleSpec, _Builder], ...] = (
...
def build_module(spec: ModuleSpec, build: _Builder,
                 edition: dict[str, Any],
                 priors: list[dict[str, Any]],
                 app_names: dict[str, str] | None = None) -> dict[str, Any]:
    """One module: headline, values, trend, detail."""
    draft = build(edition, app_names)
...
def build_modules(edition: dict[str, Any], priors: list[dict[str, Any]], *,
    ...
    assertions rather than approximations. ``app_names`` is the report's
    one app-name table (``dossier.names.app_names``), built by the caller so
    this stays off the disk; ``None`` says ids out loud as before.
    """
    return {
        ...
        "modules": [build_module(spec, build, edition, priors, app_names)
