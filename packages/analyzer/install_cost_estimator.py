...
def _bot_context_bytes(bot_id: str, network: dict | None = None) -> int:
    ...
    ``network`` is the caller's already-loaded config. Passing it is what
    keeps :func:`estimate_install_cost`'s "no disk writes" promise true:
    ``bot_home(bot_id)`` with no config calls ``evolve_config.load_config()``,
    which resolves the CANONICAL ``network.json`` and, when its schema
    migrates, writes the migrated file back. So a function documented as pure
    could write to the pod's real shared dir — which is how it reddened an
    admin CI shard from inside a route that had the right dict in hand all
    along. Resolving twice was also a correctness bug in its own right: the
    estimate could be computed against a different config than the caller
    decided with.

    ``None`` still means "resolve it from disk" — the behaviour every caller
    that passes nothing has today, unchanged.
    ...
    try:
        path = _bot_home(bot_id, network) / ".openclaw" / "openclaw.json"
        if not path.exists():
            return _BOT_CONTEXT_FLOOR_BYTES
...
def estimate_install_cost(
    ...
    *,
    ...
    """
    # Kept BEFORE the coalesce below: ``None`` and ``{}`` mean different
    # things to _bot_context_bytes. None = "I have no config, go read one"
    # (today's behaviour for callers that pass nothing); {} = "I have one and
    # it says nothing about bots" — and must not send it to the disk.
    ...
    if bot_context_bytes_override is not None:
        ...
        context_bytes = _bot_context_bytes(bot_id, supplied_network)
