...
_HUMAN_ONLY_ACTION_KINDS: frozenset[str] = frozenset(
    {
        ...
        # (.openclaw/workspace/memory/ — the MEMORY.md index, dated journals
        # and topical subdirs). It was decidable + low-risk, so a `rewrite`
        # could have auto-applied while a one-line AGENTS.md append could
        # not. Nothing emits the kind yet and no spec defines what
        # `target_selector` selects, so "the action has a computable right
        # answer" was never true of it. Human-only until a producer lands
        # AND defines the selector language. (2026-08-31)
        "MemoryCurate",          # destructive edits to the bot's own memory
