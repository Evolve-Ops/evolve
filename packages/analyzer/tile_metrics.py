...
# Audit B6 — the one spelling of the can't-price phrase (turn_cost.py).
from turn_cost import unpriced_note as _unpriced_note
...
# Key: (bot_id, local_today_iso, pod_tz, newest_mtime). The tz is in the key
# because the same bot + same local day buckets differently under a changed
# pod.timezone — without it an operator's tz edit would serve a stale tile.
_LIVE_OVERLAY_CACHE: dict[tuple[str, str, str, float], dict] = {}
...
# Key: (bot_id, local_today_iso, days, pod_tz, newest_mtime) — see above for
# why the tz participates.
_LIVE_WINDOW_CACHE: dict[tuple[str, str, int, str, float], dict] = {}
_LIVE_WINDOW_CACHE_MAX = 32


# ── Pod-local day vs UTC storage day ─────────────────────────────────────────
#
# The tile mixes TWO data sources that date themselves DIFFERENTLY:
#
#   * ``metrics/{date}/{bot}.json`` — written by the measure.py cron, whose
#     ``--date`` defaults to ``date.today()``. POD-LOCAL.
#   * ``turns-{date}.jsonl``        — written by TurnObserver as
#     ``new Date().toISOString().slice(0, 10)``, and every ``ts`` is a ``Z``
#     timestamp. UTC.
#
# ``today`` here is the POD-LOCAL day: it is what the metrics files are keyed
# on, and it is the day the operator's wall clock shows. That makes it the
# right axis for the whole tile — so the JSONL side is what has to convert.
#
# Two consequences, and both are load-bearing:
#   1. A pod-local day straddles TWO UTC files, so any window covering N local
#      days must read N+1 UTC days.
#   2. A turn belongs to the local day of its INSTANT, so bucketing must be
#      ``ts -> aware UTC -> pod-local -> .date()``. The old ``ts[:10]`` prefix
#      compare was wrong by the UTC offset for part of every day: west of UTC
#      it filed the whole local evening under tomorrow, which is why an
#      operator's "today" cost went blank after 17:00 Pacific.


def _pod_tz() -> tzinfo:
    """The pod's timezone. Single seam — tests patch this."""
    try:
...
def _pod_today() -> date:
    """Today on the pod's calendar, falling back to the system local date."""
    try:
...
def _utc_days_for_local_day(day: date, tz: tzinfo) -> list[str]:
    """The UTC date strings a single pod-local day can touch.

    One local day spans at most two UTC dates (exactly two unless the offset
    is zero). Returned oldest-first.
    """
    ...
    if end.strftime("%Y-%m-%d") != days[0]:
        days.append(end.strftime("%Y-%m-%d"))
    return days


def _utc_end_for_local_day(day: date, tz: tzinfo) -> datetime:
    """The UTC instant at the END of a pod-local day.

    This is the ``end_date`` a ``load_turns`` window must be anchored on to
    include everything that local day can contain.
    """
    return datetime.combine(day, time(23, 59, 59), tzinfo=tz).astimezone(timezone.utc)


def _local_day_iso(ts: object, tz: tzinfo) -> str | None:
    """A turn's pod-local date, from its UTC ``ts``. None if unparseable.

    Returning None rather than guessing matters: a malformed ``ts`` must drop
    out of the day buckets, not silently land in one.
    """
...
def _today_jsonl_mtime(bot_id: str, today: date, tz: tzinfo | None = None) -> float:
    """Newest mtime across the UTC turn files that local ``today`` touches.

    Used only for cache-busting. It takes the MAX rather than the first hit
    because the local day spans two UTC files and either may be the one
    currently being appended to — keying on the wrong one would serve a stale
    tile for the rest of the day.
    """
    tz = tz if tz is not None else _pod_tz()
    newest = 0.0
    try:
        from usage_analytics import _find_turns_dirs  # type: ignore[import]
        for d in _find_turns_dirs(bot_id):
            for day_str in _utc_days_for_local_day(today, tz):
                p = Path(d) / f"turns-{day_str}.jsonl"
                try:
...
def _live_today_overlay(bot_id: str, today: date) -> dict:
    ...
    """
    tz = _pod_tz()
    cache_key = (bot_id, today.isoformat(), str(tz),
                 _today_jsonl_mtime(bot_id, today, tz))
    ...
    out = {
        ...
        # Audit B6: turns neither the pricing catalog nor the offline tables
        # could price. Carried beside the dollars, never folded in as $0.
        "unpriced_today": 0, "unpriced_yesterday": 0,
        "unpriced_providers": [],
    }
    try:
    ...
    # established bot).
    # The two local days we report span at most three UTC files.
    candidate_files = {
        f"turns-{s}.jsonl"
        for d0 in (today, today - timedelta(days=1))
        for s in _utc_days_for_local_day(d0, tz)
    }
        ...
        for d in _find_turns_dirs(bot_id):
            try:
                if Path(d).is_dir():
                    ...
                    if any((Path(d) / f).exists() for f in candidate_files):
    ...
    try:
        # THREE UTC days, not two: today + yesterday in POD-LOCAL terms reach
        # back across one extra UTC boundary. Anchored at the END of local
        # today so the currently-being-written UTC file is always included.
        turns = load_turns(bot_id, days=3,
                           end_date=_utc_end_for_local_day(today, tz))
    except Exception:
        return out
    ...
    for t in turns:
        d = _local_day_iso(t.get("ts"), tz)
        resolved = _resolve_turn_cost(t, catalog=catalog)
        cost = resolved if resolved is not None else 0.0
        if resolved is None and d in (today_iso, yest_iso):
            p = turn_provider(t)
            if p not in unpriced_providers:
                unpriced_providers.append(p)
        sid = t.get("session_id") or ""
        if d == today_iso:
            out["cost_today"] += cost
            out["turns_today"] += 1
            if resolved is None:
                out["unpriced_today"] += 1
            if sid:
                sids_today.add(sid)
        elif d == yest_iso:
            ...
            if resolved is None:
                out["unpriced_yesterday"] += 1
            if sid:
                sids_yest.add(sid)
    out["unpriced_providers"] = sorted(unpriced_providers)
...
def _live_window_costs(bot_id: str, today: date, days: int) -> dict:
    ...
    """
    tz = _pod_tz()
    cache_key = (bot_id, today.isoformat(), days, str(tz),
                 _today_jsonl_mtime(bot_id, today, tz))
    ...
    out: dict = {
        ...
        # Audit B6: per-date unpriced counts, so a window sum can report how
        # much of itself is missing rather than presenting a confident total.
        "unpriced_providers": [],
    }
    try:
    ...
    try:
        for dpath in _find_turns_dirs(bot_id):
            try:
                ...
                if p.is_dir():
                    any_dir_readable = True
                    if any((p / f).exists() for f in candidate_files):
                        any_file_present = True
                        break
            except OSError:
                continue
    ...
    try:
        # days + 1: a window of N POD-LOCAL days reaches across one more UTC
        # boundary than it has days. Anchored at the END of local today so the
        # UTC file currently being appended to is always in range.
        turns = load_turns(bot_id, days=days + 1,
                           end_date=_utc_end_for_local_day(today, tz))
    except Exception:
        return out
    ...
    unpriced_providers: list[str] = []
    for t in turns:
        # per_date keys are POD-LOCAL dates, matching the metrics-file axis
        # the rest of the tile is built on.
        d = _local_day_iso(t.get("ts"), tz)
        if d is None:
            continue
        resolved = _resolve_turn_cost(t, catalog=catalog)
        cost = resolved if resolved is not None else 0.0
        slot = out["per_date"].setdefault(
            d, {"cost": 0.0, "turns": 0, "unpriced": 0},
        )
        slot["cost"] += cost
        slot["turns"] += 1
        if resolved is None:
...
def compute_tile_data(
    ...
    """
    # POD-LOCAL, and deliberately via pod_time rather than date.today(): it
    # honours an explicit ``pod.timezone`` in network.json, which a pod whose
    # host clock is UTC but whose operator is not needs in order to agree with
    # the cap-rollover boundary. Falls back to the system local TZ, i.e. to
    # exactly what date.today() returned before.
    ...
    # outage.
    unpriced_overlay_1d = int(live.get("unpriced_today") or 0)
    overlay_unpriced_providers = list(live.get("unpriced_providers") or [])
    ...
    # Audit B6 tri-state — how many turns in each window Evolve could NOT price.
    # A non-zero count means the usd_* figure beside it is a floor, not a total.
    unpriced_1d = 0
    unpriced_7d = 0
    unpriced_28d = 0
    unpriced_providers: list[str] = []
    ...
    if live_window["ok"]:
        ...
        unpriced_1d = int(
            _live_window_sum(live_window, cur_1_start_live, live_end, "unpriced")
        )
        unpriced_7d = int(
            _live_window_sum(live_window, cur_7_start_live, live_end, "unpriced")
        )
        unpriced_28d = int(
            _live_window_sum(live_window, cur_28_start_live, live_end, "unpriced")
        )
        unpriced_providers = list(live_window.get("unpriced_providers") or [])
    ...
    return {
        ...
        "cost": {
            ...
            # ── Audit B6: measurability beside the number ──────────────
            # Turns in each window that neither the pricing catalog nor the
            # offline tables could price. They are NOT summed as $0 — when a
            # count is non-zero the matching usd_* figure is a FLOOR and the
            # UI must render ``unpriced_note`` next to it, so a stranger reads
            # "can't price 41 turns" instead of a confident total.
            "unpriced_1d": max(unpriced_1d, unpriced_overlay_1d),
            "unpriced_7d": unpriced_7d,
            "unpriced_28d": unpriced_28d,
            "unpriced_providers": sorted(
                set(unpriced_providers) | set(overlay_unpriced_providers)
            ),
            "unpriced_note": _unpriced_note(
                max(
                    unpriced_28d,
                    unpriced_7d,
                    unpriced_1d,
