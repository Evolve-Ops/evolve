...
def _infer_trigger_kind(source: str | None, channel: str | None) -> str:
    ...
    summarizer | classifier | task_extractor | fallback | memory_flush |
    compaction | unknown.
    ...
    if src in {"summarizer", "classifier", "task_extractor", "fallback"}:
        ...
        return src
    if src in {"memory_flush", "compaction"}:
        # OC housekeeping — the pre-compaction memory flush, tagged at the
        # plugin (housekeeping.ts; OC's raw trigger is "memory"). Its own
        # kind so the receipt can show it on a separate line
        # (housekeeping_cost.py). Not Evolve overhead: OC does this work
        # for the bot whether or not Evolve is installed.
        return src
    if src == "memory":
        return "memory_flush"
...
def turn_to_cost_event(
    ...
    *,
    ...
    return {
        ...
        # Recorded verbatim — OC writes cost 0 for providers it has no
        # pricing for (observed 2026-08-31: xai/grok-4 turns with real
        # token counts and usage.cost all zeros). Readers must therefore
        # resolve cost through turn_cost's recorded-else-estimate rule
        # (cost_rollup does), never sum this field raw.
