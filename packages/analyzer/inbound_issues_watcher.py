...
def _capture_inbound(
    ...
    intake_id = _intake_store.new_intake_id(shared_dir)
    intake = Intake(
        id=intake_id,
    ...
    try:
        ...
        print(f"[inbound_issues_watcher] failed to write {intake.id}: {e}",
              file=sys.stderr)
        return None

    # create_intake re-mints on an id collision — report what landed.
    return intake.id
