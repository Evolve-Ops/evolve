...
def _emit_ast_unavailable_signal(shared_dir: Path) -> None:
    """Emit a Signal that the AST security layer is unavailable (fail-closed).

    Called once per review run when ``review_ast`` could not be imported.
    ``signals.store.observe`` find-or-create dedup on the signature collapses
    repeats into a single firing Signal until the condition clears (the daily
    sweep / a healthy run re-resolves it). Best-effort: a Signal-store failure
    must never crash the reviewer — the per-proposal fail-closed flag in
    ``evaluate_proposal`` is the load-bearing guarantee; this is the operator's
    ...
    try:
        ...
        signals_store.observe(
            shared_dir,
            signature=make_signature("review_gate", "ast_analyzer_unavailable", "pod"),
            producer="review_gate",
            type="ast_analyzer_unavailable",
            severity="alert",
            scope="pod",
            title="Security AST analyzer unavailable — proposal gate failing closed",
            body=(
                "review_ast could not be imported, so the AST security layer of the "
                "proposal reviewer is offline. Every proposal reviewed while this "
                "persists is force-flagged to human review (fail-closed) and cannot "
                "auto-approve. The static security_rules.json patterns alone miss "
                "the type-spoofed / obfuscated code the AST layer catches."
            ),
            details={
                "vector": "security",
                "magnitude": 2,
                "what_it_means": (
                    "The proposal security reviewer's AST analysis layer "
                    "(review_ast) failed to import. This is a packaging/deploy "
                    "fault — the module is missing from the deployed analyzer "
                    "package. The gate has NOT silently weakened: it is failing "
                    "closed (force-flagging every proposal to human review), but "
                    "no proposal can clear to auto-approval until AST coverage is "
                    "restored."
                ),
                "fix_steps": (
                    "1. Confirm review_ast.py is present in the deployed analyzer "
                    "package on the reviewing bot\n"
                    "2. Redeploy the analyzer package to restore the module\n"
                    "3. Re-run the reviewer; the Signal auto-resolves once "
                    "review_ast imports cleanly again"
                ),
            },
        )
    except Exception as exc:  # noqa: BLE001 — never let telemetry crash the gate
        print(
            f"[evolve/review] WARN: could not emit ast_analyzer_unavailable Signal "
            f"({exc}); the review gate is still failing closed.",
            file=sys.stderr,
        )
...
def main() -> None:
    ...
    # Fail-closed telemetry: if the AST security layer could not be imported,
    # every proposal this run is force-flagged (evaluate_proposal's else-branch).
    # Emit a Signal once per run so operators SEE the degraded gate instead of it
    # being invisible. find-or-create dedup collapses repeats.
    if not _AST_AVAILABLE:
        _emit_ast_unavailable_signal(shared_dir)
...
def evaluate_proposal(
    ...
    if _AST_AVAILABLE and _ast_analyze_proposal is not None:
        ...
        # ── AST analyzer UNAVAILABLE — fail CLOSED ────────────────────────────
        # The AST layer is load-bearing: it catches the type-spoofed and
        # obfuscated code that the static security_rules.json patterns miss
        # (see test_review_redteam.py). If review_ast could not be imported,
        # we must NOT silently degrade to a weaker static-only review and pass
        # the proposal — that is a fail-OPEN hole where a packaging/deploy fault
        # quietly weakens the gate. Instead force a non-approving outcome so the
        # proposal routes to human review and can never be auto-approved without
        # AST coverage. A real static auto-reject has already `return`ed above,
        # so this branch never downgrades a genuine rejection. Mirrors the
        # ast_analyzer_error handling above (degraded-AST → flag, don't crash).
        review.triggered_rules.append(
            RuleResult(
                rule_id="ast_analyzer_unavailable",
                triggered=True,
                description="security AST analyzer unavailable — failing closed",
                action="flag",
                detail="review_ast import failed; AST security layer skipped",
            )
        )
        review.flags.append(
            "ast_analyzer_unavailable: security AST analyzer "
