...
Reads on-disk telemetry only (no LLM):
  - {shared_dir}/alerts/dispatcher.jsonl          — successful send
  ...
  Deferred-to-digest records were split out of dispatcher.jsonl into
  alerts/dispatcher-queued.jsonl on 2026-08-25. This monitor is unaffected:
  ``alert_repeat_loop`` only ever counted ``result == "sent"`` rows (a
  deferral is not a page), and ``dispatcher_failures`` reads the failures
  stream. Both detectors report the same numbers before and after.
