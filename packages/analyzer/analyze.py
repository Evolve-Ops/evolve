    ...
    for cap, promise_count in app_promise_counts.items():
        ...
        proposals.append({
            ...
            "root_cause": (
                ...
                f"and there's no mechanism to carry them forward. "
                f"This application is a good candidate for the task queue system — "
                f"deferred work should become tracked tasks, not verbal commitments."
            ),
            "alternatives_considered": [],
            "confidence": 0.70,
            "proposed_change": {
                "description": (
                    ...
                    f"written to memory/tasks.md with a concrete next-action. "
                    f"Alternatively, flag this application as a priority candidate "
                    f"for the Continuity Engine task queue when that ships."
                ),
                "action": "workflow_change",
            },
...
def write_proposal(proposal: dict, shared_dir: str) -> str:
    ...
    try:
        ...
        sig = sign_proposal(full_proposal)
        if sig:
            full_proposal["evolve_sig"] = sig
