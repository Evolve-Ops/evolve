...
def _synthesize_with_llm(
    data: dict, config: dict, shared_dir: Path | None = None
) -> tuple[dict | None, str]:
    """Use the fast (tier3) role to synthesize decisions + recommended actions.

    Returns ``(synthesis_or_None, outcome)`` where outcome is one of
    ``engine_llm.OK`` / ``UNCREDENTIALED`` / ``FAILED``. The caller must keep
    those apart: an uncredentialed pod degrading to a data-only report is
    expected, a FAILED call is breakage (and has raised a Signal).
    """
    try:
...
- Output ONLY the JSON object, no other text"""
    ...
    # Text came back but is not the JSON we asked for — broken enrichment,
    # not an absent one, so the extractor raises a Signal rather than letting
    # it degrade silently into the data-only report.
    parsed = extract_json_object(output, job="weekly_review", shared_dir=shared_dir)
...
def build_report(data: dict, synthesis: dict | None, review_date: date) -> str:
    ...
    # Pod-maturity gate (Mechanism 3, internal/spec-transient-signal-suppression-
...
def send_report(
    ...
    Phase C of internal/spec-alert-subscriptions-2026-05-10.md. Operator
...
def main() -> None:
    ...
    if warming_up:
        ...
        print("[weekly_review] Synthesizing with the fast-role model...")
        from engine_llm import FAILED, UNCREDENTIALED  # type: ignore

        synthesis, outcome = _synthesize_with_llm(data, config, shared_dir)
        if outcome == UNCREDENTIALED:
            print(
                "[weekly_review] no credentialed LLM provider — data-only report "
                "(expected)",
                file=sys.stderr,
            )
        elif outcome == FAILED:
            # Distinct from the line above ON PURPOSE: this one means the pod
            # HAS a provider and the enrichment is broken. A Signal is firing.
            print(
                "[weekly_review] ERROR: LLM synthesis FAILED against a "
                "credentialed provider — shipping a data-only report and "
                "raising an engine_llm_call_failed Signal",
                file=sys.stderr,
            )
