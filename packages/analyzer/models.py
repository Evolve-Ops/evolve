...
def print_tier_table(config: dict[str, Any]) -> None:
    ...
    for tier_id in sorted(tiers.keys()):
        if tier_id == "tier0":
            # Legacy judge slot — the judge role is collapsed (design-judge-
            # role-collapse-2026-08-21); the engine tier stays resolvable for
            # read-compat but is no longer presented as a routing row.
            continue
        cfg = tiers[tier_id]
...
def print_tier_detail(tier: str, config: dict[str, Any]) -> None:
    ...
    requested_role = tier if tier in {"fast", "standard", "power", "max"} else None
    tier = normalize_to_tier(tier)
