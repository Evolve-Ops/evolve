...
(network.json → alerts). Three buckets, only shown when non-empty:
  ...
  Standing — inventory of currently-firing Signals (count, severity split,
             how many were never announced, worst few with their own
             remediation lines). Built by
             ``evolve_admin.alerts.standing_alerts``; emitted at most once
             per configured window, silent on an empty backlog.
...
Config (network.json → pod_report):
    ...
    rig_line       — true appends the dev rig's D-TP10 line (default: false; see
                     collect_rig_line — it describes the rig, not the pod)
...
class ReportLine:
    ...
    signal_type — stable type slug for the alerts/signal-store consolidation
                  (Phase 2 of internal/spec-alerts-signal-store-2026-05-07.md).
...
def _read_audit_snapshot(shared_dir: Path) -> dict:
    ...
    # R-3 event-vs-posture split. audit._write_findings_snapshot stamps
    # ``finding_kind`` ("event" | "posture") on every critical it writes,
    # and names this reader as an intended consumer. Three properties the
    # rest of this module depends on:
    #
    #   * ``.get()``, never ``[]`` — the key is additive, so a snapshot
    #     written by a pod that has not pulled the R-3 audit yet has no
    #     ``finding_kind`` at all. That pod must keep rendering the line it
    #     always rendered, not crash the daily report.
    #   * Criticals only. The snapshot's ``warn`` entries are written
    #     without the key by construction, so warns can never be counted
    #     here — which matters because the rendered clause sits next to the
    #     CRITICAL count and would otherwise misattribute.
    #   * A FLOOR, not a complement. We count what is explicitly marked
    #     "posture" and nothing else. ``critical_count - posture_count`` is
    #     the UNCLASSIFIED remainder, which is a mix of real "event"
    #     criticals and criticals from an older snapshot that carry no
    #     classification at all — it is emphatically not "the active ones",
    #     and no caller may render it as such.
    posture = [f for f in criticals if f.get("finding_kind") == "posture"]
    ...
    return {
        ...
        "critical_posture_count": len(posture) if state == "fresh" else 0,
    }
...
def _fix_once_clause(posture_count: int) -> str:
    """The "N of these are standing config, not an incident" qualifier.

    Renders as an empty string when nothing is classified, so a pod on a
    pre-R-3 audit gets byte-identical output to what it got before.

    Three constraints shape the wording, and each one has already cost a
    revision somewhere in this codebase:

    * **No "posture".** A chat message is a PRIMARY surface under
      docs/principle-plex-test.md §2, so the codebase's own word for this
      concept stays in the code. "one-time fix" is the phrasing the
      standing-alerts section of this same message uses; the two surfaces
      describe overlapping sets, so they must not invent separate
      vocabularies for the same distinction.
    * **A floor, never a complement.** We say how many ARE one-time fixes
      and stop. Rendering the remainder ("and the other N are active")
      would be false: only audit criticals carry a classification at all,
      so the remainder is "unclassified", which includes every critical
      written by a pod that has not pulled the R-3 audit yet.
    * **No promise about what happens next.** Not what the operator should
      do about it (that is their triage call — audit.py itself comments
      some of the standing rows "urgent"), and not what Evolve will do
      about it. In particular this line may never imply these stop being
      reported: it is itself the daily report that reports them.

    Placement is load-bearing too. The clause sits immediately after the
    CRITICAL count rather than at the end of the line, because the line
    ends with "· N warn" — a trailing qualifier would read as though it
    counted warns, which can never carry a classification.
    """
    if posture_count <= 0:
        return ""
    if posture_count == 1:
        return " (1 one-time fix)"
    return f" ({posture_count} one-time fixes)"
...
def collect_broken(
    ...
    if sec["state"] == "missing":
        ...
        if crits > 0:
            text = (
                f"Audit: {crits} CRITICAL"
                f"{_fix_once_clause(sec['critical_posture_count'])}"
                f" — {_format_finding_titles(sec['critical_findings'])}"
            )
            if warns > 0:
                text += f" · {warns} warn"
            lines.append(ReportLine(
...
def collect_trending(
    ...
    # In-turn cost checkpoints (D-CS12) — runs stopped in the last 7 days.
    try:
        from breakers import in_turn
        itc = in_turn.count_by_bot(shared_dir, members, days=7)
    except Exception:  # noqa: BLE001 — evidence line, never fail the report
        itc = {}
    if itc:
        total = sum(itc.values())
        lines.append(ReportLine(
            bucket="trending", severity="yellow",
            text=(f"In-turn checkpoints: {total} this week ("
                  + ", ".join(f"{b} {n}" for b, n in sorted(itc.items())) + ")"),
            deeplink="/admin/cost",
            meta={"affected_bots": sorted(itc)},
            signal_type="in_turn_checkpoint",
        ))
...
def collect_touch_report(shared_dir: Path, members: list[str]) -> list[ReportLine]:
    """D-TM10: per bot, last 7 days — touches due / on time / missed;
    captures proposed / kept / dropped (rendered "unknown" until
    ``capture-from-turns-is-gated-and-lands-as-a-proposal`` lands — the
    sibling chip that fills the second half).

    One line per bot that actually had something due this week, or whose
    store came back unreadable (D-CS7 — a control names the subject it
    judged, it does not go quiet). A bot with zero touches due — no cards
    on a pace yet, or the touch scheduler simply hasn't fired — produces
    no line, the same "quiet day, quiet report" contract every other
    collector here already holds (:func:`collect_queue`'s own
    ``if blocked > 0`` gate; the empty-state test pins exactly one Pod
    usage line on a clean pod).

    Best-effort per bot: ``evolve_admin.board_touch.weekly_report_stats``
    already renders ``unknown`` for a bot whose store it could not read,
    so nothing here needs a second failure path beyond the import itself
    (a pod predating this chip's module).
    """
    lines: list[ReportLine] = []
    try:
        from evolve_admin.board_touch import format_report_line, weekly_report_stats
    except Exception as exc:  # noqa: BLE001
        print(f"[pod_report] board_touch import failed; touch report skipped: {exc}",
              file=sys.stderr)
        return lines
    for bot_id in members:
        stats = weekly_report_stats(shared_dir, bot_id)
        if not stats.get("unknown") and not stats.get("touches_due"):
            continue
        lines.append(ReportLine(
            bucket="queue", severity="info",
            text=format_report_line(bot_id, stats),
            signal_type="board_touch_report",
            meta={"bot_id": bot_id, **{k: v for k, v in stats.items() if k != "bot_id"}},
        ))
    return lines
...
def render_report(
    ...
    *,
    pod_usage_line: str = "",
    standing_section: str = "",
    standing_worst_unannounced: str | None = None,
    rig_line: str = "",
) -> tuple[str, str]:
    ...
    ``standing_section`` (new 2026-07-29): the firing-Signal *inventory*
    digest built by ``evolve_admin.alerts.standing_alerts``. Appended
    last — today's news reads first, the standing backlog is context
    below it. ``standing_worst_unannounced`` is the highest severity
    among standing Signals the operator has NEVER been told about; it
    raises ``overall`` so a ``notify_on: red_only`` / ``yellow_or_red``
    gate can't hide an invisible alert-severity condition. A standing set
    the operator has already been paged about does not raise ``overall``
    at all — it is a reminder, not news.
    """
    ...
    if rig_line:  # informational: never raises ``overall``
        sections.append(rig_line)

    text = "\n\n".join(sections)
    overall = "red" if broken else ("yellow" if trending else "green")
    if standing_section and overall != "red":
        # alert → red, warn → yellow, info → no change. An info-tier
        # condition nobody announced is genuinely low-stakes; an operator
        # on ``yellow_or_red`` opted out of that tier and we honor it.
        if standing_worst_unannounced == "alert":
            overall = "red"
        elif standing_worst_unannounced == "warn":
            overall = "yellow"
    return text, overall


def collect_rig_line(config: dict | None) -> str:
    """D-TP10 (``internal/decision-rig-throughput-2026-09-15.md``): the Evolve dev rig's
    trailing-24 h line plus its 48-hour-PR list, computed by ``tools/rig_line.py`` from
    this checkout. OPT-IN (``pod_report.rig_line: true``) — it describes the development
    rig, not the pod, so an ordinary install never sees it. Read-only, and the running
    user's reach is the reach: a source it cannot read renders ``unknown``. A checkout
    without the tool, or any failure, says so in one line rather than vanishing."""
    if not ((config or {}).get("pod_report") or {}).get("rig_line"):
        return ""
    tools = Path(__file__).resolve().parents[2] / "tools"
    try:
        if str(tools) not in sys.path:
            sys.path.insert(0, str(tools))
        out = importlib.import_module("rig_line").report(tools.parent)
    except Exception as exc:  # noqa: BLE001 — best-effort, like the standing section
        return f"rig line: unavailable — {exc}"
...
def run_report(
    ...
    queue.extend(collect_touch_report(shared_dir, members))
    ...
    # Standing-alerts inventory (2026-07-29). signal_notifier announces
    # transitions only, and its cold-start guard permanently silences
    # whatever backlog existed at first run — 95 of 99 firing Signals on
    # the mini had never been announced. This section is the inventory
    # channel. Best-effort: a standing-alerts failure must never cost the
    # operator the rest of the report. Emits at most once per configured
    # window (see standing_alerts.build_section), so the hourly pod-report
    # tick can't repeat it.
    standing_section, standing_worst = "", None
    try:
        from evolve_admin.alerts import (  # pyright: ignore[reportMissingImports]
            standing_alerts,
        )
        # pod_report's ``when`` is naive *local* (``datetime.now()``);
        # standing_alerts works in UTC because Signal timestamps are UTC.
        # Converting rather than calling the wall clock again keeps the
        # once-per-window gate injectable from tests — the clock-coupling
        # trap that produced overnight-only reds elsewhere in this codebase.
        standing_now = (
            when if when.tzinfo is not None else when.astimezone(timezone.utc)
        )
        standing_section, standing_worst = standing_alerts.build_section(
            shared_dir, label=label, now=standing_now,
        )
    except Exception as exc:  # noqa: BLE001
        print(f"[pod_report] standing-alerts section skipped: {exc}",
              file=sys.stderr)

    rig_line = collect_rig_line(config)
    text, overall = render_report(
        label, broken, trending, queue, empty_summary,
        pod_usage_line=pod_usage_line,
        standing_section=standing_section,
        standing_worst_unannounced=standing_worst,
        rig_line=rig_line,
    )
    ...
    # (internal/spec-alerts-signal-store-2026-05-07.md): mirror every report
    ...
    # Convert each bot's nightly `openclaw doctor --lint` findings into
    # Signals. The bot-user runner can't write the Signal store, so it drops
    # an artifact in its own workspace and this pod-side pass (running as
    # evolve) emits from it. Homes are resolved per bot rather than derived
    # from the bot id — the account name is not the bot id.
    try:
        from doctor_lint_signal import emit_doctor_lint_signals
        from evolve_config import bot_home
        emit_doctor_lint_signals(
            shared_dir, {b: bot_home(b, config) for b in members},
        )
    except Exception as exc:  # noqa: BLE001 — never break the report run
        # Logged, not swallowed: this pass is the only thing that surfaces
        # doctor findings, so its failure must not itself be invisible.
        print(f"[pod_report] doctor-lint Signal emission failed: {exc}")
...
# internal/spec-severity-framework-2026-05-18.md. audit_critical lives on
...
_POD_REPORT_SEVERITY_TAG: dict[str, tuple[str, int]] = {
    ...
    "in_turn_checkpoint": ("cost",     2),  # a run was stopped at the per-turn threshold
    # ── queue ────────────────────────────────────────────────────────────────
...
_OPERATOR_DOCS: dict[str, tuple[str, str]] = {
    ...
    "in_turn_checkpoint": (
        "A turn reached the per-turn checkpoint (`inTurnCheckpointUsd`, "
        "default $5.00 recorded) and its tool calls were stopped; the owner "
        "was told in the conversation. Recurring stops mean a loop or a "
        "cache problem upstream.",
        "1. Read `{shared_dir}/breakers/<bot>/in-turn-checkpoints.jsonl`\n"
        "2. Inspect those runs for repeated tool calls or a pinned cache read\n"
        "3. Legitimate work: the owner replies \"continue\" (+50 %)",
    ),
    "tasks_blocked": (
...
def main() -> None:
    ...
    # Turn-failure autopsy rides the daily report tick (no launchd job of
    # its own). Best-effort by design: an autopsy crash must never cost the
    # pod its daily report, and vice-versa the report never waits on it.
    try:
        from turn_autopsy import run_sweep as _autopsy_sweep
        _autopsy_sweep(Path(shared_dir))
    except Exception as exc:  # noqa: BLE001
        print(f"[pod_report] turn_autopsy sweep failed (non-fatal): {exc}",
              file=sys.stderr)
