...
def test_overhead_reads_tool_footprint(tmp_path: Path) -> None:
    ...
    # A v1 footprint has no per-profile block; say so rather than showing zeros.
    assert "per-profile weights unmeasured" in out


def test_overhead_reports_per_profile_tool_weights(tmp_path: Path) -> None:
    """Footprint schema v2 (context-economy CE-2a): the same tools weighed
    under each tool profile, so the audit can say what a background session
    pays without waiting for one to run."""
    from context_health import collect_overhead, render_overhead

    bot_dir = tmp_path / "team-bot-a" / "turns"
    bot_dir.mkdir(parents=True)
    (bot_dir / "context-footprint.json").write_text(json.dumps({
        "schema_version": 2, "tier": "full", "tool_count": 2,
        "total_chars": 4226,
        "tools": [{"name": "record_application", "chars": 2772},
                  {"name": "roster_block", "chars": 1454}],
        "profiles": {
            "full": {"tools": [{"name": "record_application", "chars": 2772},
                               {"name": "roster_block", "chars": 1454}],
                     "tool_count": 2, "total_chars": 4226,
                     "trimmed": [], "trimmed_chars": 0},
            "no_live_speaker": {
                "tools": [{"name": "record_application", "chars": 2772}],
                "tool_count": 1, "total_chars": 2772,
                "trimmed": [{"name": "roster_block", "chars": 1454}],
                "trimmed_chars": 1454},
        },
        "kind_profiles": {"scheduled": "no_live_speaker", "oneshot": "no_live_speaker"},
    }), encoding="utf-8")
    out = render_overhead(
        collect_overhead(tmp_path, "team-bot-a", 1, [], [], workspace_dir=None))
    assert "per tool profile" in out
    assert "no_live_speaker" in out
    # The kinds that get the profile are named beside it, so the reader needs
    # no second source to say who pays which column.
    assert "oneshot, scheduled" in out
    assert "per-profile weights unmeasured" not in out
