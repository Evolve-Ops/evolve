...
def test_oc_gateway_restart_broken_adapter_fails_closed(tmp_path, monkeypatch):
    """An adapter that honors neither raw() nor the Protocol verbs must keep
    oc_gateway_restart's never-raise dict contract."""
    import oc_cli
    ...
    set_scheduler(NotAScheduler())
    res = oc_cli.oc_gateway_restart("zzztestbot", network_path=net)
    assert res["ok"] is False
    # No launchd vocabulary on a non-launchd adapter — a systemd pod has no
    # plists by design, so "Plist not found" would be a phantom diagnosis.
    assert "Plist" not in res["error"]


def test_oc_gateway_restart_on_systemd_uses_seam_restart(tmp_path, monkeypatch):
    """On a Linux pod the restart goes through the Protocol restart() verb
    against the one unit — never a launchctl call, and never the
    openclaw-CLI fallback (which used to be the ONLY path here, and died on a
    hardcoded /Users/<bot> cwd before it could run)."""
    import oc_cli
    ...
    unit_dir.mkdir()
    (unit_dir / "ai.openclaw.zzztestbot-gateway.service").write_text("[Unit]\n")

    def _fallback_must_not_run():
        raise AssertionError("openclaw-CLI fallback ran on a healthy systemd unit")

    monkeypatch.setattr(oc_cli, "_find_openclaw", _fallback_must_not_run)
    ...
    res = oc_cli.oc_gateway_restart("zzztestbot", network_path=net)
    assert res["ok"] is True
    assert res["method"] == "scheduler-restart"
    assert res["service"] == "ai.openclaw.zzztestbot-gateway"
    assert [
        "/usr/bin/systemctl", "restart", "ai.openclaw.zzztestbot-gateway.service",
    ] in calls
    assert all("launchctl" not in " ".join(c) for c in calls)


def test_oc_gateway_restart_on_systemd_missing_unit_reports_deploy_gap(
    tmp_path, monkeypatch
):
    """No unit on disk is a deploy gap. Say that — don't let the openclaw-CLI
    fallback start an unsupervised gateway and report success."""
    import oc_cli
    from runtime.scheduler import SystemdScheduler
    ...
    set_scheduler(
        SystemdScheduler(
            unit_dir=tmp_path / "empty", use_sudo=False,
            runner=lambda argv: (4, "", "Unit not found"),
        )
    )

    res = oc_cli.oc_gateway_restart("zzztestbot", network_path=net)
    assert res["ok"] is False
    assert "no unit installed" in res["error"]


# ── oc_cli.oc_gateway_stop ───────────────────────────────────────────────────


def test_oc_gateway_stop_launchd_boots_out_gui_then_system(tmp_path, monkeypatch):
    """launchd stop stays a raw() bootout (remove() would delete the plist and
    /restart needs it on disk), gui/<uid> first, then system."""
    import oc_cli

    net = _write_network(tmp_path, "zzztestbot", "zzztestuser")
    monkeypatch.setattr(oc_cli, "_bot_uid", lambda u: 501)
    runner = _RecordingRunner(
        reply=lambda argv: (0, "", "") if "system/" in argv[-1] else (1, "", "no gui")
    )
    set_scheduler(LaunchdScheduler(runner=runner))

    res = oc_cli.oc_gateway_stop("zzztestbot", network_path=net)
    assert res["ok"] is True
    assert res["domain"] == "system"
    svc = "ai.openclaw.zzztestbot-gateway"
    assert runner.calls == [
        ["sudo", "/bin/launchctl", "bootout", f"gui/501/{svc}"],
        ["sudo", "/bin/launchctl", "bootout", f"system/{svc}"],
    ]


def test_oc_gateway_stop_on_systemd_uses_disable_not_remove(tmp_path):
    """On a Linux pod, stop is ``disable --now`` — the unit file must SURVIVE
    so /restart can bring the gateway back. Before this branch existed the
    call reached the fail-fast launchd accessor and 500'd the endpoint."""
    import oc_cli
    from runtime.scheduler import SystemdScheduler
    ...
    unit_dir = tmp_path / "systemd"
    unit_dir.mkdir()
    unit = unit_dir / "ai.openclaw.zzztestbot-gateway.service"
    ...
    assert [
        ...
        "ai.openclaw.zzztestbot-gateway.service",
    ] in calls
    assert unit.exists(), "disable() must not delete the unit file — /restart needs it"
    assert all("launchctl" not in " ".join(c) for c in calls)
