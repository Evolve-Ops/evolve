...
def test_cron_jobs_read_denied_is_skipped(tmp_path: Path):
    """personal_bot-style 'cannot read cron/jobs.json' is a capability gap, not anomaly."""
    fake_home = tmp_path / "personal_bot"
    fake_home.mkdir()
    # No cron/jobs.json file AND sudo cat will fail → cap-gap.
    class _R:
        returncode = 1
        stdout = ""
        stderr = ""
...
def _seed_fake_binary(tmp_path: Path, mtime_secs: float) -> Path:
    """Create a fake openclaw binary at a known mtime and steer the audit's
    candidate-list at it via _OC_BINARY_CANDIDATES override."""
...
def test_oc_binary_mtime_clean_upgrade_refreshes_baseline(tmp_path: Path, monkeypatch):
    ...
    fake = _seed_fake_binary(tmp_path, mtime_secs=2000.0)
    monkeypatch.setattr(audit, "_OC_BINARY_CANDIDATES", [fake])
...
def test_oc_binary_mtime_unchanged_emits_ok(tmp_path: Path, monkeypatch):
    ...
    fake = _seed_fake_binary(tmp_path, mtime_secs=1000.0)
    monkeypatch.setattr(audit, "_OC_BINARY_CANDIDATES", [fake])
    monkeypatch.setattr(audit, "_read_openclaw_version",
                        lambda _p: "openclaw 2026.4.29")
    baseline = tmp_path / "security" / "oc-binary-mtime.baseline"
