...
def test_dispatch_findings_writes_snapshot_in_normal_mode(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(audit, "_send_security_alert", lambda *a, **k: None)
    monkeypatch.setattr(audit, "_send_warn_log", lambda *a, **k: None)
    monkeypatch.setattr(audit, "_should_send_critical", lambda *a, **k: False)
...
def test_dispatch_writes_snapshot_even_if_alert_dispatch_raises(tmp_path: Path, monkeypatch):
    """Real-world failure mode: alerts/ dir not writable → _record_critical_sent
    raised PermissionError on the live pod 2026-05-07. The snapshot must still
    land — it's the report's source of truth and can't be hostage to alert
    delivery bookkeeping."""
    monkeypatch.setattr(audit, "_send_security_alert", lambda *a, **k: None)
    monkeypatch.setattr(audit, "_send_warn_log", lambda *a, **k: None)
    monkeypatch.setattr(audit, "_should_send_critical", lambda *a, **k: True)

    def boom(*a, **k):
        raise PermissionError("alerts/ not writable")
    monkeypatch.setattr(audit, "_record_critical_sent", boom)
