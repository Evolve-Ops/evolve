...
Pins the migration contract:
  ...
  - send_alert / _send_cap_alert / _maybe_send_weekly_summary all hand
...
def test_cap_alert_uses_distinct_dedup_key_namespace(env):
    spend_alert._send_cap_alert(
        "admin_bot", 12.50, 10.0, "downgrade-tier", "downgraded ok",
        shared_dir=env["shared_dir"], network=env["network"],
        today_iso="2026-05-09",
    )
    assert len(env["captured"]) == 1
    ...
    assert call["dedup_key"] == "spend_alert/cap/admin_bot/2026-05-09"
    assert call["catalog_event"] == "cost.hard_cap_hit"
    # Different dedup_key namespace from threshold alert — same bot, same
    # day, threshold + cap can both fire (operator wants to know about
    # both events).
    assert call["dedup_key"] != "spend_alert/threshold/admin_bot/2026-05-09"
    # Phase F3: payload-driven.
    assert call["message"] is None
    assert call["payload"]["bot_id"] == "admin_bot"
    assert call["payload"]["spend"] == 12.5
    assert call["payload"]["cap"] == 10.0
    assert "Downgraded" in call["payload"]["action_label"]
