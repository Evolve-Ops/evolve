...
def test_default_cascade_drops_costlier_tail():
    ...
    # Since the 2026-08-31 provider-affinity change, haiku (same provider
    # as the sonnet primary) precedes the cross-provider peers.
    assert flat == [
...
def test_real_pod_tiers_file_yields_opus_free_chain(tmp_path):
    ...
    # Provider affinity (2026-08-31): the anthropic rung below the primary
    # precedes the cross-provider peers.
    assert fallbacks == [
        "anthropic/claude-haiku-4-5",
        "google/gemini-3.1-pro-preview",
        "google/gemini-3-flash-preview",
