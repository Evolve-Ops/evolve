...
class TestReflect:
    ...
    def test_no_provider_returns_failed_result(self, monkeypatch):
        from app_posture_reflect import reflect
        # No provider credentialed anywhere → resolution yields None.
        monkeypatch.setattr("app_posture_reflect._resolve_target", lambda b: None)
    ...
    def test_api_error_bubbles_to_failed_result(self, monkeypatch):
        from app_posture_reflect import reflect
        monkeypatch.setattr(
            "app_posture_reflect._resolve_target", lambda b: _fake_target()
        )
        monkeypatch.setattr("app_posture_reflect._call_llm", lambda *a, **kw: "")
        result = reflect(_make_posture())
        assert result.ok is False
        assert "empty response" in (result.error or "")

    def test_non_anthropic_target_flows_through(self, monkeypatch):
        """An openai-only pod runs the reflection through its openai
        target — no Anthropic presumption anywhere (#3466)."""
        from infra_llm import InfraLLMTarget
        from app_posture_reflect import reflect
        openai_target = InfraLLMTarget(
            provider="openai", model="openai/gpt-4o-mini",
            api_key="sk-openai-fake-test-key",
        )
        seen: dict = {}
        ...
        monkeypatch.setattr(
            "app_posture_reflect._resolve_target", lambda b: openai_target
        )
