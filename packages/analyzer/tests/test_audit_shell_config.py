...
def test_changed_critical(tmp_path: Path):
    """A changed .zshrc is a CRITICAL posture finding.

    Posture, not event, on purpose: this check is deliberately outside the L2
    authorized-change gate (internal/spec-drift-alert-taxonomy-2026-06-26.md,
    build record). The message is the exact string
    ``_remediation_for_finding`` keys on to attach the reset action.
    """
    home, shared_dir = _setup(tmp_path)
    ...
    assert crits and ".zshrc hash changed since baseline" in crits[0].message
    assert crits[0].finding_kind == "posture"


def test_changed_stays_posture_and_names_the_reset_affordance(tmp_path: Path):
    """The shell check does not consult the L2 gate, and says how to clear itself.

    Nothing in Evolve writes a bot's .zshrc and no admin-surface record can
    name the file (``oc_keys`` are openclaw.json top-level keys), so no
    allow-set for this kind could ever fire in production. An operator
    action on the same bot — in the shape production actually logs — must
    therefore change nothing: the finding stays a posture CRITICAL, the
    baseline is NOT re-recorded by the mismatch branch, and the fix steps
    name the one affordance that clears it: the Alerts-page "Accept current
    .zshrc as new baseline" action, which is the ``reset_baseline`` shell
    remediation ``_remediation_for_finding`` attaches to this exact message.
    """
    home, shared_dir = _setup(tmp_path)
    (home / ".zshrc").write_text("changed content")
    _baseline_path(shared_dir).parent.mkdir(parents=True)
    _baseline_path(shared_dir).write_text('{"bot": "old_hash_abcdef"}')
    (shared_dir / "audit-log.jsonl").write_text(json.dumps({
    ...
    assert len(crits) == 1
    assert crits[0].finding_kind == "posture"
    assert ".zshrc hash changed since baseline" in crits[0].message
    # The steps name the real affordance, not a re-baseline that never happens.
    assert "Accept current .zshrc as new baseline for bot" in crits[0].fix_steps
    assert "next run" not in crits[0].fix_steps
    assert "sudo" not in crits[0].fix_steps
    # The mismatch branch does not re-record the baseline — only the reset does.
    assert json.loads(_baseline_path(shared_dir).read_text())["bot"] == "old_hash_abcdef"
    remediation = audit._remediation_for_finding(crits[0])
    assert remediation is not None
    assert remediation.kind == "reset_baseline"
    assert remediation.params == {"bot_id": "bot", "kind": "shell"}
    assert remediation.label == "Accept current .zshrc as new baseline for bot"
