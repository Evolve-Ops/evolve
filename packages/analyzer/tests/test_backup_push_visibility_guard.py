...
def _drive(tmp_path, monkeypatch, *, pat, visibility):
    ...
    Returns (status, error_msg). ``pat`` controls _load_github_pat_state output;
    ...
    monkeypatch.setattr(
        "backup._load_github_pat_state",
        lambda network: ((bv.PAT_PRESENT, pat) if pat
                         else (bv.PAT_ABSENT, None)),
    )
