...
Covers:
  - applier: create rung at position, extend existing rung, optional role
    re-point, cap seed (max default 5), idempotency, stale-judge refusal
...
# ── Applier: the retired judge role is refused (judge-role collapse §5.4) ─────

def test_apply_refuses_judge_role_mapping():
    """The judge role is gone — a proposal that still names it (a stale
    generator, an old queued proposal) is refused cleanly with no write,
    ...
    try:
        ...
        assert "role_mapping" in result.details
        # No write happened (role validation runs before any persist).
