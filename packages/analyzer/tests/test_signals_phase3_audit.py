...
Spec: docs/spec-alerts-signal-store-2026-05-07.md (Phase 3).
...
Covers:
  ...
  - Suppressed (dedup'd) Telegram alerts still record a Delivery with
    a suppressed_reason — visible in the audit trail
"""
...
def _critical(category: str, bot_id: str | None, message: str) -> Finding:
    return Finding(level="critical", category=category, bot_id=bot_id,
...
def test_suppressed_duplicate_alert_records_suppression_reason(tmp_path):
    """The 7-day dedup suppresses repeat Telegram messages, but the
    audit trail still records WHY delivery was skipped — spec calls
    this 'single delivery path, not two'."""
    f = _critical("identity", "admin_bot", "ssh key wrong perms")

    # First run sends and records
    dispatch_findings([f], tmp_path, config={}, dry_run=False)

    # Second run — same finding, dedup suppresses Telegram
    dispatch_findings([f], tmp_path, config={}, dry_run=False)

    sigs = list(signals_store.iter_active(tmp_path, producer="audit"))
    assert len(sigs) == 1
    # Two delivery entries: one sent, one suppressed
    deliveries = sigs[0].deliveries
    assert len(deliveries) == 2
    assert deliveries[0].suppressed_reason is None
    assert deliveries[1].suppressed_reason is not None
    assert "dedup" in deliveries[1].suppressed_reason
