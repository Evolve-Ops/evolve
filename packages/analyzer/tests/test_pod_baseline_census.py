...
def test_context_profile_unset_and_custom():
    ...
    # A mutated builtin no longer matches it. Mutate a field the profile
    # still declares — matching is subset semantics (extra, UNdeclared live
    # keys don't break recognition by design, see _cost_settings_match), so
    # the mutated field has to be one the profile actually checks.
    cfg = _config_with_profile_applied("balanced")
    cfg["agents"]["defaults"]["contextPruning"]["ttl"] = "99m"
