...
# ── Retired bot accounts are not "new" accounts (2026-08-02) ──


def test_retired_bot_account_is_not_reported_as_new(tmp_path):
    """The 2026-08-02 finding: `ledger` was retired on 2026-06-15 and this
    check called it a NEW account every cycle for seven weeks. Because the
    message was visibly wrong about what it was looking at, it read as a false
    positive and nobody acted — while the account really did still hold a live
    channel token. A retirement archive is proof the account is a
    decommissioned bot, not a new one."""
    _baseline(tmp_path, "pod_admin_user\nevolve\nteam_bot_a\n")
    (tmp_path / "archived-bots" / "retired_bot-2026-06-15").mkdir(parents=True)
    config = {"members": ["team_bot_a"], "bots": {"team_bot_a": {"user": "team_bot_a"}}}

    with _patch_dscl(["pod_admin_user", "evolve", "team_bot_a", "retired_bot"]):
        findings = audit._check_user_accounts(tmp_path, config)

    criticals = [f for f in findings if f.level == "critical"]
    assert criticals == [], f"a retired bot is not a new account. Got: {[f.message for f in findings]}"
    warns = [f for f in findings if f.level == "warn" and "retired_bot" in f.message]
    assert len(warns) == 1
    assert "retired bot account(s) still present" in warns[0].message
    # The accurate framing names the real condition and hands off the detail.
    assert "still valid until revoked" in (warns[0].detail or "")


def test_a_genuinely_new_account_is_still_critical_alongside_a_retired_one(tmp_path):
    """Splitting retired accounts out must not swallow real intrusion alerts."""
    _baseline(tmp_path, "pod_admin_user\nevolve\nteam_bot_a\n")
    (tmp_path / "archived-bots" / "retired_bot-2026-06-15").mkdir(parents=True)
    config = {"members": ["team_bot_a"], "bots": {"team_bot_a": {"user": "team_bot_a"}}}

    with _patch_dscl(["pod_admin_user", "evolve", "team_bot_a", "retired_bot", "attacker"]):
        findings = audit._check_user_accounts(tmp_path, config)

    criticals = [f for f in findings if f.level == "critical"]
    assert len(criticals) == 1
    assert "attacker" in criticals[0].message
    assert "retired_bot" not in criticals[0].message


def test_archive_dir_suffix_variants_resolve_to_the_account(tmp_path):
    """`_resolve_archive_root` appends `-<date>` and, on a same-day re-run,
    `-<date>-<n>`. Both must resolve back to the account name."""
    root = tmp_path / "archived-bots"
    (root / "retired_bot-2026-06-15").mkdir(parents=True)
    (root / "other_bot-2026-06-01-2").mkdir(parents=True)
    assert audit._retired_bot_accounts(tmp_path) == {"retired_bot", "other_bot"}


def test_missing_archive_dir_is_conservative(tmp_path):
    """No archive dir → empty set → a retired-looking account falls back to the
    CRITICAL branch rather than being silently excused."""
    assert audit._retired_bot_accounts(tmp_path) == set()
