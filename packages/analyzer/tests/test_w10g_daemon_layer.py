...
def test_synthesizer_resolves_key_from_primary_bot_on_linux(monkeypatch, tmp_path):
    """Round-9 #2: run as the evolve SERVICE user, the synthesizer's target
    resolution (infra_llm) must read the PRIMARY bot's auth-profiles — on a
    fresh Linux pod the evolve account has NO OpenClaw instance, so the key
    lives only in /home/<primary>. The pre-fix code hardcoded
    read_bot_anthropic_key("evolve", …) → "no key" → exit 1 on every fire
    even though the key was present in the primary's home."""
    set_profile(LINUX)
    _clear_provider_env(monkeypatch)
    import primary_bot
    from infra_llm import resolve_infra_llm
    ...
    target = resolve_infra_llm("fast", network=network)
    assert target is not None
    assert target.api_key == "sk-ant-primary-key"
    assert target.provider == "anthropic"


def test_synthesizer_key_resolver_no_key_when_primary_absent(monkeypatch, tmp_path):
    """No key anywhere (no env, primary has no auth-profiles) → None, so
    synthesize.main surfaces the clear "no LLM provider credentialed" error
    ...
    assert resolve_infra_llm("fast", network={"primary": "evo"}) is None
