...
def test_action_reader_parity_on_a_rung_only_pod(shared):
    """The SAME divergence, one field over: what a tripped cap DOES.

    A pod normalized in 2026-06 expresses the old stock ``downgrade-tier``
    only as ``pod_defaults.budget.tier_downgrade_usd`` — network.json carries
    no ``spendCapAction`` at all. ``spend_caps.get_caps_config`` passes that
    budget to the resolver and reports ``downgrade-tier``; the enforcer used
    to call the same resolver WITHOUT it and got ``checkpoint``. One resolver,
    two answers: the Cost page and the enforcer describing different pods.
    """
    # No L2 rung: the resolver checks ``l2_breaker_usd`` first, and this pod
    # expressed only the downgrade mirror.
    _write_be(shared, pod={
        "per_bot_daily_warn_usd": 5.0,
        "per_bot_daily_hard_usd": 20.0,
        "tier_downgrade_usd": 20.0,
    })
    network = {"thresholds": {"dailySpendAlertUsd": 5.0}}

    ui_action = spend_caps.get_caps_config(network, shared)["spendCapAction"]
    enforced_action = spend_alert._resolve_spend_cap_action(network, shared)

    assert ui_action == enforced_action == "downgrade-tier"


def test_action_reader_parity_when_nothing_is_configured(shared):
    """No rung, no key: both readers land on the D-CC2 checkpoint default."""
    _write_be(shared, pod={"per_bot_daily_hard_usd": 20.0})
    network: dict = {}

    assert (
        spend_caps.get_caps_config(network, shared)["spendCapAction"]
        == spend_alert._resolve_spend_cap_action(network, shared)
        == "checkpoint"
    )
