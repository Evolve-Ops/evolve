...
# ── Image-turn cache pin (2026-09-20 image-turn-prompt-cache-thrash) ────────


def _calls(*pairs: tuple[int, int]) -> list[dict]:
    """Per-call rows from ``(cache_read_tokens, cache_write_tokens)`` pairs —
    the minimal shape :func:`cs.image_turn_cache_pin` reads."""
    return [{"cache_read_tokens": r, "cache_write_tokens": w} for r, w in pairs]


def test_pinned_and_growing_fires():
    """The post-mortem's §3 shape, compressed: one read figure repeated while
    writes climb every call."""
    calls = _calls((46_007, 7_702), (46_007, 46_613), (46_007, 111_517), (46_007, 180_936))
    verdict, reason = cs.image_turn_cache_pin(calls)
    assert verdict == "pinned"
    assert "46,007" in reason


def test_growing_reads_do_not_fire():
    """The healthy shape: later calls read a larger prefix as the
    conversation grows — not pinned."""
    calls = _calls((0, 20_000), (20_000, 5_000), (25_000, 4_000), (29_000, 3_000))
    verdict, _ = cs.image_turn_cache_pin(calls)
    assert verdict == "not_pinned"


def test_fewer_than_three_calls_is_unknown():
    calls = _calls((46_007, 7_702), (46_007, 46_613))
    verdict, reason = cs.image_turn_cache_pin(calls)
    assert verdict == "unknown"
    assert "2 call" in reason


def test_missing_per_call_data_is_unknown():
    calls = [
        {"cache_read_tokens": 46_007, "cache_write_tokens": 7_702},
        {"cache_read_tokens": 46_007, "cache_write_tokens": None},
        {"cache_read_tokens": 46_007, "cache_write_tokens": 111_517},
    ]
    verdict, _ = cs.image_turn_cache_pin(calls)
    assert verdict == "unknown"


def test_pin_that_heals_partway_through_the_run_still_fires():
    """A run that heals once a later user-role message becomes active (the
    post-mortem's seq 1121-1124: reads jump back up, writes fall to
    hundreds) is still pinned for the calls before that point — a
    whole-sequence equality check would miss this and under-count the
    post-mortem's own incident turns, which both end in a short healed
    tail."""
    calls = _calls(
        (46_007, 7_702), (46_007, 46_613), (46_007, 111_517), (46_007, 180_936),
        (232_693, 510),  # healed: a later user message is now active
    )
    verdict, reason = cs.image_turn_cache_pin(calls)
    assert verdict == "pinned"
    assert "4 consecutive calls (of 5 given)" in reason


def test_constant_write_does_not_fire_as_pinned():
    """Reads pinned but writes NOT growing is not this defect — e.g. a run
    that made no further tool calls after the first re-warm."""
    calls = _calls((46_007, 7_702), (46_007, 7_702), (46_007, 7_702))
    verdict, _ = cs.image_turn_cache_pin(calls)
    assert verdict == "not_pinned"
...
# ── Cache thrash (incident-post-mortem-2026-09-17 §5) ────────────────────────


def test_post_mortem_aggregate_is_thrash_not_a_rewarm():
    read, write = 5_382_819, 8_186_626
    assert cs.is_cache_thrash(read, write) is True
    assert cs.is_prefix_rewarm(read, write) is False


def test_thrash_counts_by_magnitude_and_includes_first_turns():
    turns = [
        # first-of-session thrash turn — must count; a priming write must not
        {"session_id": "s1", "ts": "2026-09-17T19:27:28Z",
         "cache_read_tokens": 5_382_819, "cache_write_tokens": 8_186_626},
        {"session_id": "s2", "ts": "2026-09-17T10:00:00Z",
         "cache_read_tokens": 0, "cache_write_tokens": 45_000},
        {"session_id": "s2", "ts": "2026-09-17T10:30:00Z",
         "cache_read_tokens": 20_000, "cache_write_tokens": 150_000},
    ]
    ct = cs.cross_turn_cache(turns)
    assert ct.thrash_turns == 2
    assert ct.thrash_write_tokens == 8_336_626
    assert ct.thrash_max_write_tokens == 8_186_626
    d = ct.as_dict()
    assert d["cache_thrash_write_tokens"] == 8_336_626
    # The re-warm count is untouched by the new predicate.
    assert ct.rewarms == 0 and d["prefix_rewarms"] == 0


def test_receipt_renders_thrash_by_tokens_largest_bot_first(monkeypatch):
    big = [{"session_id": "a", "ts": "2026-09-17T19:27:28Z",
            "cache_read_tokens": 5_382_819, "cache_write_tokens": 8_186_626}]
    small = [{"session_id": "b", "ts": "2026-09-17T09:00:00Z",
              "cache_read_tokens": 0, "cache_write_tokens": 120_000},
             {"session_id": "c", "ts": "2026-09-17T11:00:00Z",
              "cache_read_tokens": 0, "cache_write_tokens": 110_000}]
    monkeypatch.setattr(
        cs, "load_turn_records",
        lambda bot_id, **kw: big if bot_id == "bot_big" else small,
    )
    monkeypatch.setattr(cs, "read_idle_reset_minutes", lambda bot_id: 120)
    lines = cs.receipt_lines(["bot_small", "bot_big"])
    total = next(ln for ln in lines if ln.startswith("cache thrash"))
    assert "3 turns, 8,416,626 tokens written, largest 8,186,626" in total
    per_bot = [ln.strip() for ln in lines if "cache-thrash turns" in ln]
    assert per_bot[0].startswith("bot_big: 1 cache-thrash turns, 8,186,626")
    assert per_bot[1].startswith("bot_small: 2 cache-thrash turns, 230,000")
    # Named apart from the re-warm on the surface (4146's rule).
    assert "re-warm" not in total
