...
# ``day_start`` is the UTC instant the POD-LOCAL day began. These cases pin the
# projection arithmetic, so they use a UTC-aligned pod (day_start == 00:00Z) and
# read as "N hours into the local day".
_DAY_START = _at(0)


def test_projection_at_noon_doubles_spend() -> None:
    projected, elapsed = spend_alert._projected_full_day_spend(
        2.50, _at(12), day_start=_DAY_START,
    )
...
def test_projection_floors_elapsed_at_one_hour_to_avoid_runaway_extrapolation() -> None:
    ...
    projected, elapsed = spend_alert._projected_full_day_spend(
        0.50, _at(0, 30), day_start=_DAY_START,
    )
...
def test_projection_at_midnight_treats_as_one_hour_elapsed() -> None:
    ...
    projected, _ = spend_alert._projected_full_day_spend(
        10.0, _at(23, 59), day_start=_DAY_START,
    )
    # 24 / 23.98 ≈ 1.001 → projected ≈ $10.01
    assert projected == pytest.approx(10.01, abs=0.05)


def test_projection_elapsed_is_pod_local_not_utc() -> None:
    """A Pacific pod at 21:00 local is 04:00 the next day in UTC.

    ``spend_usd`` is the pod-LOCAL day's total, so anchoring elapsed hours on
    UTC midnight divided a nearly-complete day's spend by ~4h and projected
    ~5x the real end-of-day figure. Anchor on local midnight instead.
    """
    # 2026-06-04 21:00 PDT == 2026-06-05 04:00Z; local day began 07:00Z.
    now = datetime(2026, 6, 5, 4, 0, tzinfo=timezone.utc)
    local_midnight = datetime(2026, 6, 4, 7, 0, tzinfo=timezone.utc)

    projected, elapsed = spend_alert._projected_full_day_spend(
        21.0, now, day_start=local_midnight,
    )
    assert elapsed == pytest.approx(21.0)
    assert projected == pytest.approx(24.0)

    # The old UTC-midnight anchor saw 4h elapsed and projected $126.
    utc_projected, utc_elapsed = spend_alert._projected_full_day_spend(
        21.0, now, day_start=now.replace(hour=0, minute=0),
    )
    assert utc_elapsed == pytest.approx(4.0)
    assert utc_projected > projected * 4
