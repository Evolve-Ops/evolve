...
@pytest.mark.parametrize("tiers,expect_rungs", [
    # tier0 (the legacy judge slot) no longer mints a judge-class rung — its
    # models merge into the standard chain (judge-role collapse §6).
    (LADDER, {"haiku-class", "sonnet-class", "opus-class"}),
...
def test_created_file_reads_as_new_shape_to_the_gateway(fake_bot_env):
    ...
    # The legacy read projection renders the three ladder tiers; tier0's
    # models MERGED into the standard chain (judge-role collapse §6) and the
    # tier0 key itself is gone from the projection.
    legacy = oc_model.synthesize_legacy_tiers(written)
    assert "tier0" not in legacy
    assert legacy["tier1"] == LADDER["tier1"]
    assert legacy["tier3"] == LADDER["tier3"]
    assert legacy["tier2"]["models"] == [
