"""Phase C — proposal-lifecycle trio (analyze, apply, outcome) routes
through alerts.dispatcher.

The three sources cover the proposal pipeline:

  analyze (proposals generated) → decisions.proposal_ready
  apply (proposal applier ran)   → decisions.proposal_applied
...
class _ApplyResult:
    def __init__(self, *, success=True, rollback=False,
                 bot_id="team_bot_a", proposal_id="prop_abc",
                 action="applied", details="ok"):
        self.success = success
        self.rollback_triggered = rollback
        self.bot_id = bot_id
...
def test_apply_routes_proposal_applied_success(env, monkeypatch):
    # apply._alert reads network.json off shared_dir; stub load_json.
    monkeypatch.setattr(
        apply_mod, "load_json",
        lambda _p: {"alerts": {"channel": "telegram", "chatId": "12345"}},
    )
    proposal = {"problem": "Lower context cap by 30%"}
    result = _ApplyResult(success=True, action="applied", proposal_id="prop_abc1234")
    apply_mod._alert(proposal, result, env["shared_dir"])
    call = env["captured"][0]
    assert call["source"] == "apply"
    assert call["catalog_event"] == "decisions.proposal_applied"
    assert call["dedup_key"] == "apply/prop_abc1234/applied"
    assert call["severity"] == env["Severity"].INFO
    # Phase F5: payload-driven render.
    assert call["message"] is None
    assert call["payload"]["action"] == "applied"
    assert call["payload"]["bot_id"] == "team_bot_a"
    assert call["payload"]["proposal_id"] == "prop_abc1234"


def test_apply_rollback_uses_warning_severity(env, monkeypatch):
    monkeypatch.setattr(
        apply_mod, "load_json",
        lambda _p: {"alerts": {"channel": "telegram", "chatId": "12345"}},
    )
    proposal = {"problem": "Lower context cap by 30%"}
    result = _ApplyResult(success=False, rollback=True,
                          action="rolled_back", details="metric regressed")
    apply_mod._alert(proposal, result, env["shared_dir"])
    call = env["captured"][0]
    assert call["severity"] == env["Severity"].WARNING
    # Phase F5: payload carries the action + rollback context in
    # problem_summary; catalog template renders.
    assert call["message"] is None
    assert "rolled back" in call["payload"]["problem_summary"].lower()


def test_apply_distinct_proposals_get_distinct_keys(env, monkeypatch):
    monkeypatch.setattr(
        apply_mod, "load_json",
        lambda _p: {"alerts": {"channel": "telegram", "chatId": "12345"}},
    )
    apply_mod._alert(
        {"problem": "p1"},
        _ApplyResult(proposal_id="prop_x", action="applied"),
        env["shared_dir"],
    )
    apply_mod._alert(
        {"problem": "p2"},
        _ApplyResult(proposal_id="prop_y", action="applied"),
        env["shared_dir"],
    )
    keys = [c["dedup_key"] for c in env["captured"]]
    assert keys == ["apply/prop_x/applied", "apply/prop_y/applied"]
