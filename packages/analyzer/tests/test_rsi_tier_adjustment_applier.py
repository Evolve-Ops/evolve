...
def test_revert_reports_failure_when_a_non_canonical_role_cannot_be_restored(
    ...
    try:
        ...
        # apply is correct here: the stale role is evicted and the tier lands.
        assert applier.apply(action, "evolve").ok
        assert _stored_routing(tiers_path)["maintenanceTier"] == "tier2"
        assert _stored_routing(tiers_path).get("maintenanceRole") is None
...
def test_revert_restores_a_projectable_role_and_reports_ok(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert applier.apply(action, "evolve").ok
        assert _stored_routing(tiers_path)["maintenanceTier"] == "tier3"

        revert = applier.revert(snap, "evolve")
        assert revert.ok, revert.message
        assert _stored_routing(tiers_path)["maintenanceTier"] == "tier1"
    finally:
        set_config_io(None, None)
...
def test_revert_restores_an_unprojectable_but_canonical_role(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert applied["maintenanceTier"] == "tier3"
        assert applied.get("maintenanceRole") is None
