...
def test_aggregate_empty():
    ...
    assert agg["unpriced_events"] == 0
    assert agg["by_model"] == {}


def test_aggregate_reprices_recorded_zero_from_known_provider(tmp_path):
    """The 2026-08-31 PoC-bot regression: OC records cost 0 for providers
    it has no pricing for (xai/grok-4 turns with real token counts and
    usage.cost all zeros). The converter passes that zero through as
    cost_usd, so a raw sum silently counted the provider as free. The
    rollup must re-price a zero-cost event from its tokens via turn_cost
    (offline xai provider rate here — no catalog in tmp_path)."""
    events = [
        _ev(model="grok-4", provider="xai", cost_usd=0.0,
            input_tokens=50_000, output_tokens=1_000),
    ]
    agg = cr.aggregate(events, shared_dir=tmp_path)
    # xai offline provider rate $3/$15 per MTok: 0.15 + 0.015
    assert agg["total_usd"] == pytest.approx(0.165)
    assert agg["unpriced_events"] == 0
    assert agg["by_model"]["grok-4"]["cost_usd"] == pytest.approx(0.165)


def test_aggregate_counts_unpriceable_events_instead_of_summing_zero(tmp_path):
    """House doctrine: unmeasured must never present as zero. An event no
    pricing source can cost (unknown provider AND unknown model) is
    counted in unpriced_events — total_usd is then a floor, not $0."""
    events = [
        _ev(model="mystery-model", provider="mystery-cloud", cost_usd=0.0,
            input_tokens=10_000, output_tokens=500),
        _ev(model="claude-sonnet-4-6", cost_usd=0.5, input_tokens=10),
    ]
    agg = cr.aggregate(events, shared_dir=tmp_path)
    assert agg["total_usd"] == pytest.approx(0.5)
    assert agg["event_count"] == 2
    assert agg["unpriced_events"] == 1
    assert agg["by_model"]["mystery-model"]["unpriced_events"] == 1
    assert agg["by_model"]["mystery-model"]["cost_usd"] == 0.0


def test_aggregate_zero_token_zero_cost_event_is_genuinely_free(tmp_path):
    """A zero-cost event that burned no tokens is a measurement ($0), not
    an unpriceable one — it must not inflate unpriced_events."""
    agg = cr.aggregate([_ev(model="grok-4", provider="xai")], shared_dir=tmp_path)
    assert agg["total_usd"] == 0.0
    assert agg["unpriced_events"] == 0
