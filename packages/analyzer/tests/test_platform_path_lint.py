...
# ── binpath rule (platform-divergent command-binary paths: chown|lsof|openclaw) ─
...
def test_binpath_openclaw_hardcoded_path_is_blocked(tmp_path):
    # The bot_forge Linux-forge incident: a hardcoded macOS Homebrew openclaw
    # path in a sudo argv can never match the Linux sudoers SETENV grant
    # (/usr/bin/openclaw on the VPS) — "sudo: a password is required", forge
    # dead. Any of the six resolver candidates hardcoded whole-string is a hit.
    r = _lint_binpath(
        tmp_path,
        "import subprocess\n"
        'OPENCLAW_BIN = "/opt/homebrew/bin/openclaw"\n'
        'subprocess.run(["/usr/bin/sudo", "-H", "-u", "bot", OPENCLAW_BIN, "agent"])\n',
    )
    assert r.returncode == 1, r.stdout + r.stderr
    assert "/opt/homebrew/bin/openclaw" in r.stdout


def test_binpath_openclaw_linux_symlink_literal_is_blocked(tmp_path):
    # Symmetric: a hardcoded Linux openclaw path breaks the macOS grant.
    r = _lint_binpath(
        tmp_path,
        'cmd = ["/usr/bin/openclaw", "gateway", "status"]\n',
    )
    assert r.returncode == 1, r.stdout + r.stderr


def test_binpath_openclaw_resolver_routed_usage_passes(tmp_path):
    # The blessed replacement: the shared resolver, called at dispatch time.
    r = _lint_binpath(
        tmp_path,
        "import subprocess\n"
        "from platform_profile import find_openclaw_cli\n"
        'subprocess.run([find_openclaw_cli() or "openclaw", "agent"])\n',
    )
    assert r.returncode == 0, r.stdout + r.stderr
...
def test_binpath_baseline_exists_and_rule_is_green():
    ...
    # added a hardcoded /usr/sbin/chown, /usr/sbin/lsof, or absolute-openclaw
    # literal in the admin daemon package — route it through
    # platform_profile.get_profile().chown / .lsof, or for openclaw
    # platform_profile.find_openclaw_cli() (resolved at call time).
