...
def test_resolve_fit_model_comes_from_catalog_role_not_literal():
    """The fit model is resolved from the pod's fast role via the catalog — a
    non-empty BARE id (provider stripped), never a hardcoded literal."""
    model = fit_llm.resolve_fit_model(_NETWORK)
    assert isinstance(model, str) and model
    assert "/" not in model  # bare id for the SDK call
...
def test_build_default_classifier_failopen_no_model():
    assert fit_llm.build_default_classifier({}, model_resolver=lambda _n: None) is None


def test_build_default_classifier_failopen_no_key():
    out = fit_llm.build_default_classifier(
        {}, model_resolver=lambda _n: "some-model", key_resolver=lambda _n: None)
    assert out is None


def test_build_default_classifier_failopen_sdk_missing():
    out = fit_llm.build_default_classifier(
        {}, model_resolver=lambda _n: "some-model",
        key_resolver=lambda _n: "sk-key", client_factory=lambda _k: None)
    assert out is None
...
def test_build_default_classifier_wires_and_calls():
    ...
    classify = fit_llm.build_default_classifier(
        {}, model_resolver=lambda _n: "resolved-model",
        key_resolver=lambda _n: "sk-key",
    ...
    # model id came from the resolver (no hardcoded literal).
    assert captured["model"] == "resolved-model"
    # prompt-cached system prompt, like the precedent.
...
def test_fit_classifier_failopen_on_api_error():
    ...
    classify = fit_llm.make_fit_classifier(_BoomClient(), model="m")
