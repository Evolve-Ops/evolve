...
def replay_env(tmp_path, monkeypatch):
    ...
    # The fixtures are a UTC day (``turns-2026-05-20.jsonl``) and the incident's
    # $16.29 / $12.78 are UTC-day totals, so these replays only reproduce on a
    # UTC pod. ``load_today_spend`` buckets turns by the POD-LOCAL day, so pin
    # the pod TZ rather than inheriting the runner's — otherwise the assertions
    # measure the developer's timezone as much as the detector.
...
def test_load_today_spend_returns_none_on_discovery_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **kw: live_spend.LIVE_LOAD_FAILED,
    )
...
def test_burst_window_returns_negative_on_discovery_failure(tmp_path, monkeypatch):
...
def test_sweep_archives_stale_burst_signals(tmp_path):
    ...
    spend_alert._sweep_stale_signals(
        tmp_path, types={"cost_burst"}, bot_ids={bot_id},
        kept_signatures=set(),
...
def test_sweep_does_not_touch_unchecked_bots(tmp_path):
    ...
    spend_alert._sweep_stale_signals(
        tmp_path, types={"cost_burst"}, bot_ids={fine},
        kept_signatures=set(),
        reason="auto-resolve: burst window no longer above threshold",
    )
