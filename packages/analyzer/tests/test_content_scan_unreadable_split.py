...
The split:
  ...
  * undecodable      → content_scan_file_unreadable   (warn)  — DAMAGE, won't self-clear.
  * everything else  → content_scan_file_unreadable   (warn)  — access flap, digest.

2026-08-25 addendum: ``UnicodeDecodeError`` is a ``ValueError``, so it matched
none of ``_read_text``'s arms and propagated out of ``_scan_target``, aborting
the whole scan run (swallowed upstream in ``audit.py``). A doc truncated
mid-multibyte character is exactly the damage this producer exists to report,
so the signal could not fire for the case that matters most. Twin of the
seeding-side fix in ``bot_doc_seeding.read_existing_doc`` (#3795).
...
# ── undecodable bytes: present, readable, and damaged ────────────────────────

def _truncated_mid_character() -> bytes:
    """A real workspace line with the 3-byte U+2014 EM DASH cut after its first
    byte — what a partial write or an interrupted restore actually leaves.

    The cut point is COMPUTED, not a hardcoded slice: an offset tuned by hand
    silently stops truncating anything the moment someone edits the text above
    it, and a fixture that decodes cleanly would make both tests below vacuous.
    """
    text = "# USER.md\n\nRoster \u2014 Slack IDs\n"
    raw = text.encode("utf-8")
    dash_start = raw.index("\u2014".encode("utf-8"))
    return raw[: dash_start + 1]          # keep byte 1 of 3 → invalid UTF-8


_TRUNCATED = _truncated_mid_character()


def test_undecodable_file_is_reported_not_raised(tmp_path: Path) -> None:
    """THE regression: a real file of undecodable bytes must produce a finding
    rather than an exception that takes the whole scan run down with it."""
    shared_dir = tmp_path / "evolve"
    shared_dir.mkdir()
    doc = tmp_path / "USER.md"
    doc.write_bytes(_TRUNCATED)
    with pytest.raises(UnicodeDecodeError):      # the shape really is undecodable
        doc.read_text()

    # Real file, real read — _read_text is NOT stubbed here on purpose.
    assert _scanner._read_text(doc) == (None, "undecodable")

    findings = _scan_one(shared_dir, doc)

    assert len(findings) == 1
    f = findings[0]
    assert f["type"] == "content_scan_file_unreadable"   # not "disappeared"
    assert f["severity"] == "warn"
    assert f["details"]["read_error"] == "undecodable"


def test_undecodable_wording_does_not_promise_it_will_self_clear(tmp_path: Path) -> None:
    """The access-failure body tells the operator this is a short-lived
    permission hiccup that clears on its own. For a damaged file that is
    actively wrong — both read paths work and nothing will fix itself — so the
    undecodable case must carry its own words. A signal that fires with the
    wrong advice is barely better than one that never fires."""
    shared_dir = tmp_path / "evolve"
    shared_dir.mkdir()
    doc = tmp_path / "USER.md"
    doc.write_bytes(_TRUNCATED)

    body = _scan_one(shared_dir, doc)[0]["body"]

    assert "clears on its own" not in body
