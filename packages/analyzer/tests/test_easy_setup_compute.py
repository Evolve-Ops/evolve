...
cluster are simply not present). Filtered to credentialed providers. The former
judge role is gone (judge-role collapse); cross-vendor checking is now the
resolve_cross_vendor derivation over the reordered standard chain, covered
below.
...
def test_cross_vendor_derivable_after_reorder():
    base = _base()
    # Anthropic-first preference puts anthropic at the head of standard; the
    # cross-vendor derivation must still find a non-anthropic model in the
    # enriched sonnet-class chain (openai/google all live there).
    order = ["anthropic", "openai", "google"]
    cred = {"anthropic", "openai", "google"}
    out = compute_easy_setup_catalog(base, order, credentialed_providers=cred)
    xv = resolve_cross_vendor("standard", catalog=out, credentialed=cred)
    assert xv is not None and provider_of(xv) != "anthropic"
    assert validate_pod_default_catalog(out) is None


def test_single_provider_easy_setup_saves_and_derives_no_cross_vendor():
    base = _base()
    # Only anthropic credentialed: every tier filters to anthropic. The
    # validator must NOT reject a single-provider candidate — it saves, and
    # the cross-vendor derivation returns None (the meaningful null: a
    # single-provider pod gets no cross-vendor option — design 5.2).
    order = ["anthropic"]
    cred = {"anthropic"}
    out = compute_easy_setup_catalog(base, order, credentialed_providers=cred)
    ...
    assert validate_pod_default_catalog(out) is None
    assert resolve_cross_vendor("standard", catalog=out, credentialed=cred) is None
...
# ── Addendum 7 item 14 (as amended by the judge-role collapse): cross-vendor
# derivation is secondary-first ────────────────────────────────────────────────
...
def test_cross_vendor_derivation_is_secondary_preference_first():
    """After easy-setup with a provider_order, standard resolves to the PRIMARY
    preference and resolve_cross_vendor resolves to the SECONDARY preference
    (the highest-pref provider that is NOT standard's) — secondary-first, the
    diversity goal Addendum 7 item 14 originally pinned for the judge role,
    now carried by the derivation (design-judge-role-collapse 5.2)."""
    base = _base()
    cred = set(_FOUR)
    for order in (
        ...
        xv_p = provider_of(
            resolve_cross_vendor("standard", catalog=out, credentialed=cred)
        )
        assert std_p == order[0], f"{order}: standard={std_p}, expected primary"
        assert xv_p == order[1], (
            f"{order}: cross-vendor={xv_p}, expected secondary (order[1]); "
            "the derivation must lead with the highest-pref non-standard provider"
        )
