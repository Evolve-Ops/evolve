...
def test_resolve_roles_with_provenance_rank_on_junk_pod_merge():
    ...
    # Judge has no rank (off-ladder).
    assert out["judge"]["rank"] is None
...
def test_canonicalize_renames_and_backfills_costclass():
    ...
    # judge owns its own judge-class rung (spec §Addendum 16), cost-ordered next
    # to sonnet-class (both medium).
    assert [r["id"] for r in out["rungs"]] == [
        "haiku-class", "sonnet-class", "judge-class", "opus-class", "fable-class",
    ]
    assert [r["costClass"] for r in out["rungs"]] == ["low", "medium", "medium", "high", "premium"]
    assert out["roles"]["max"] == "fable-class"
    assert out["roles"]["judge"] == {"rung": "judge-class", "provider": "not-standard"}
    # judge models live in judge-class, NOT folded into standard's sonnet-class.
    judge = next(r for r in out["rungs"] if r["id"] == "judge-class")
    assert judge["models"] == ["openai/gpt-4.1"]
...
def test_canonicalized_catalog_overlays_code_default_in_merge():
    ...
    # OVERLAY, not accumulate — 5 canonical rungs (judge-class its own now), not 9.
