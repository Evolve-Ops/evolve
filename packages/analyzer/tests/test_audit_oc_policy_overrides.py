...
member-bot default — see docs/spec-app-derived-permissions-2026-05-24.md
...
def test_policy_override_table_carries_spec_links():
    ...
    for check_id, reason in overrides.items():
        ...
        assert (
            "docs/" in reason or "spec-" in reason or "openclaw#" in reason
        ), (
            f"entry for {check_id} must reference a docs/ spec path or "
            f"upstream issue: got reason={reason!r}"
        )
