...
# See internal/decision-retire-primary-model-floor-advisor-2026-06-04.md.
...
def test_build_tile_reports_which_cost_levers_are_on(tmp_path):
    """D-CS5: the Cost page shows each lever's state per bot. On by default;
    network.json bots.<id>.cost.levers.<name>: false is the opt-out."""
    network = {"bots": {
        "admin_bot": {"role": "member", "user": "admin_bot"},
        "personal_bot": {"role": "member", "user": "personal_bot",
                         "cost": {"levers": {"housekeeping_cheap_rung": False}}},
    }}
    tiles = cot.build_all_tiles(
        tmp_path, ["admin_bot", "personal_bot"], network, today=date(2026, 5, 28),
    )
    assert tiles[0]["levers"] == {"housekeeping_cheap_rung": True}
    assert tiles[1]["levers"] == {"housekeeping_cheap_rung": False}
