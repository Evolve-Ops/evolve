...
def test_silence_rule_is_in_the_injected_summary():
    """Not just in the prose — the summary block is the injected surface.

    Asserted on the rule's SCOPE and its sentinel, never on its title. The title
    assertion here read "Group-chat silence" and #4160 broadened the rule to cover
    scheduled wakes and retitled it "Silence"; because #4160 was docs-only, the CI
    path filter skipped every Python suite, it merged green, and main was red for
    every Python PR that followed. A title is not the contract — the scope and the
    sentinel are.
    """
    summary = _summary_block()
    assert "NO_REPLY" in summary, "the rule must name the sentinel the runtime honors"
    assert "not addressed" in summary, "the group-channel scope must survive a retitle"
    assert "scheduled wake" in summary.lower(), (
        "the scheduled-wake scope (#4160, the hourly all-clear into a 1:1 DM) must "
        "survive a retitle"
    )
...
def test_full_prose_section_exists_for_the_rule():
    ...
    The section is located by its NUMBER, not its title — see the note in
    ``test_silence_rule_is_in_the_injected_summary``. Section 15 may be retitled;
    it may not lose the mechanical detail the one-liner cannot carry.
    """
    text = POD_CONDUCT.read_text()
    m = re.search(r"^## 15\. .+$", text, re.MULTILINE)
    assert m, "POD_CONDUCT.md has no section 15"
    body = text[m.end():]
