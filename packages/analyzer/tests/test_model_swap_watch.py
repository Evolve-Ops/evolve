...
# ── Repeat of a rolled-back swap: the early-warning floor ────────────────────
# The 2026-08-21 recurrence measured a clean divergence but sat silent for
# days at post-window n=21 because the cold-start floor (30/side) applied to a
# swap whose model had ALREADY been rolled back once — a strong prior.


def test_evaluate_honors_a_caller_supplied_floor():
    n = watch.MIN_TARGET_TURNS_REPEAT + 2
    arms = _arms((int(n * 0.8), n), (int(n * 0.1), n))
    assert watch.evaluate(arms)["diverged"] is False, "cold-start floor holds"
    v = watch.evaluate(arms, min_target_turns=watch.MIN_TARGET_TURNS_REPEAT,
                       repeat_of_rejected=True)
    assert v["diverged"] is True
    assert v["repeat_of_rejected"] is True


def _reswap_pod(tmp_path, *, with_rollback):
    """OLD→NEW swap (optionally preceded by a rollback of NEW on this rung),
    with target-arm volume between the repeat floor and the cold-start floor."""
    if with_rollback:
        ledger.record_swap("team-bot-a", "standard", "anthropic", [OLD], [NEW],
                           source="admin_ui_bulk", shared_dir=tmp_path)
        ledger.record_swap("team-bot-a", "standard", "anthropic", [NEW], [OLD],
    ...
    for i, r in enumerate(rows):
        # Earlier history well before the measurement window; the re-swap at SWAP_AT.
        r["ts"] = (SWAP_AT - timedelta(days=40 - i)).isoformat() \
            if i < len(rows) - 1 else SWAP_AT.isoformat()
    path.write_text("".join(json.dumps(r) + "\n" for r in rows))

    n = watch.MIN_TARGET_TURNS_REPEAT + 5  # below MIN_TARGET_TURNS
    assert n < watch.MIN_TARGET_TURNS
    _write_turns(tmp_path, "team-bot-a",
                 _arm(OLD, n, int(n * 0.8), era="pre") + _arm(NEW, n, int(n * 0.1), era="post")
                 + _arm(CONTROL, n, int(n * 0.3), era="pre")
                 + _arm(CONTROL, n, int(n * 0.3), era="post"))
    return tmp_path


def test_reswap_of_a_rolled_back_model_fires_at_the_repeat_floor(tmp_path):
    summary = watch.run(_reswap_pod(tmp_path, with_rollback=True), now=NOW)
    assert summary["diverged"] == 1
    verdict = summary["evaluations"][0]
    assert verdict["repeat_of_rejected"] is True
    assert verdict["min_target_turns"] == watch.MIN_TARGET_TURNS_REPEAT


def test_first_time_swap_at_the_same_volume_stays_below_the_floor(tmp_path):
    """The lower floor is earned by the prior rollback, not handed to every
    low-volume swap — without the history the cold-start floor still holds."""
    summary = watch.run(_reswap_pod(tmp_path, with_rollback=False), now=NOW)
    assert summary["diverged"] == 0
    verdict = summary["evaluations"][0]
    assert verdict["repeat_of_rejected"] is False
    assert "sample floor" in verdict["reason"]


def test_repeat_signal_names_the_prior_rollback(tmp_path):
    shared = _reswap_pod(tmp_path, with_rollback=True)
    watch.run(shared, now=NOW)
    firing = list((shared / "signals" / "firing").glob("*.json"))
    assert len(firing) == 1
    sig = json.loads(firing[0].read_text())
    assert "repeat" in sig["body"].lower()
    assert sig["details"]["repeat_of_rejected"] is True


def test_a_rollback_itself_is_not_a_repeat(tmp_path):
    """A rollback RESTORES models; judging the restore at the loose floor
    (because the backed-out model is 'rejected') would be self-triggering."""
    ledger.record_swap("team-bot-a", "standard", "anthropic", [OLD], [NEW],
                       source="admin_ui_bulk", shared_dir=tmp_path)
    ledger.record_swap("team-bot-a", "standard", "anthropic", [NEW], [OLD],
                       source=ledger.ROLLBACK_SOURCE, shared_dir=tmp_path)
    path = ledger.swap_ledger_path(tmp_path)
    rows = [json.loads(x) for x in path.read_text().splitlines() if x.strip()]
    rows[0]["ts"] = (SWAP_AT - timedelta(days=10)).isoformat()
    rows[1]["ts"] = SWAP_AT.isoformat()
    path.write_text("".join(json.dumps(r) + "\n" for r in rows))
    n = watch.MIN_TARGET_TURNS_REPEAT + 5
    _write_turns(tmp_path, "team-bot-a",
                 _arm(NEW, n, int(n * 0.8), era="pre") + _arm(OLD, n, int(n * 0.1), era="post"))
    summary = watch.run(tmp_path, now=NOW)
    verdict = summary["evaluations"][0]
    assert verdict["repeat_of_rejected"] is False
    assert verdict["min_target_turns"] == watch.MIN_TARGET_TURNS
