...
def test_synthesize_projects_new_shape_to_legacy_view():
    ...
    # tier0 (the legacy judge slot) is gone from the projection — the judge
    # role was collapsed; a stale structured roles.judge entry stays inert.
...
def test_wholesale_rungs_roles_write_materializes_custom(fake_bot_env):
    ...
    written = json.loads(fake_bot_env["tiers_path"].read_text())
    # The ladder rungs land verbatim; the write-path canonicalization drops
    # the stale structured judge entry (the role is gone — judge-role
    # collapse §5.4; the sanctioned merge path is migrate-model-roles).
    assert [r["id"] for r in written["rungs"]] == [
        "haiku-class", "sonnet-class", "opus-class",
    ]
    assert written["roles"]["standard"] == "sonnet-class"
    assert "judge" not in written["roles"]
...
def test_wholesale_synthetic_ids_canonicalized_on_write(fake_bot_env):
    ...
    # The stale judge role + its judge-default rung are dropped by the
    # canonicalizer (judge-role collapse §5.4) — roles are uniformly slugs.
    assert "judge" not in written["roles"]
