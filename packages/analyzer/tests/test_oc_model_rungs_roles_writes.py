...
def test_wholesale_rungs_roles_write_materializes_custom(fake_bot_env):
    ...
    written = json.loads(fake_bot_env["tiers_path"].read_text())
    # The ladder rungs land verbatim; the write canonicalizes the pre-decouple
    # judge (roles.judge → sonnet-class) into its OWN judge-class rung (spec
    # §Addendum 16), cost-ordered next to sonnet-class.
    assert [r["id"] for r in written["rungs"]] == [
        "haiku-class", "sonnet-class", "judge-class", "opus-class",
    ]
    assert written["roles"]["standard"] == "sonnet-class"
    assert written["roles"]["judge"] == {"rung": "judge-class", "provider": "not-standard"}
    judge = next(r for r in written["rungs"] if r["id"] == "judge-class")
    assert judge["models"] == ["anthropic/claude-sonnet-4-6"]
...
def test_wholesale_synthetic_ids_canonicalized_on_write(fake_bot_env):
    ...
    assert written["roles"]["judge"] == {"rung": "judge-class", "provider": "not-standard"}
    # judge-default's cluster lands in judge-class, not folded into sonnet-class.
    judge = next(r for r in written["rungs"] if r["id"] == "judge-class")
    assert judge["models"] == ["openai/gpt-4.1", "anthropic/claude-sonnet-4-6"]
