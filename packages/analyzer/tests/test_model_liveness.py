...
# ── interim cost brake: SCHEDULED_PROBE_EXCLUSIONS ───────────────────────────
#
# claude-opus-4-8 is ~$0.75 of the run's measured ~$0.89 (84%), paid by the
# standing daily sweep. These pin the two halves of the brake: the scheduled
# sweep skips it, the --model spot-check path still reaches it. Interim — when
# the tuple empties, the first two tests go with it.


def test_scheduled_sweep_excludes_the_costly_model():
    """The routed (scheduled) set drops every SCHEDULED_PROBE_EXCLUSIONS entry."""
    assert "claude-opus-4-8" in ml.SCHEDULED_PROBE_EXCLUSIONS, (
        "test pins the shipped brake; update it deliberately, not incidentally"
    )
    resolver = _resolver_for({
        "admin_bot": {
            "max": {"resolvedModel": "anthropic/claude-opus-4-8"},
            "fast": {"resolvedModel": "anthropic/claude-haiku-4-5"},
        },
        "team_bot_a": {}, "team_bot_b": {},
    })
    targets, _dropped = ml.routed_probe_targets(
        _NETWORK, roles_resolver=resolver,
        exclude=ml.SCHEDULED_PROBE_EXCLUSIONS,
    )
    assert [t.model for t in targets] == ["anthropic/claude-haiku-4-5"]


def test_explicit_model_spot_check_still_probes_an_excluded_model(tmp_path):
    """The brake is scheduled-path-only — `--model claude-opus-4-8` must still
    dispatch a real probe, or the escape hatch the brake depends on is gone."""
    runner = FakeRunner(ml.AgentRunResult(payload=_envelope(model="claude-opus-4-8")))
    summary = ml.run_probes(
        tmp_path, _NETWORK, runner=runner,
        targets=[ml.ProbeTarget(bot_id="admin_bot", model="anthropic/claude-opus-4-8")],
    )
    assert summary["probed"] == 1
    assert [r.model for r in runner.requests] == ["anthropic/claude-opus-4-8"]
    # Injected targets are never filtered, so nothing is declared excluded.
    assert summary["excluded_from_schedule"] == []


def test_exclusion_applies_before_the_cost_cap():
    """An excluded model must not consume a probe slot — otherwise the brake
    silently costs coverage of a target that would have been probed."""
    view = {
        "max": {"resolvedModel": "anthropic/claude-opus-4-8"},
        "a": {"resolvedModel": "anthropic/m-a"},
        "b": {"resolvedModel": "anthropic/m-b"},
    }
    resolver = _resolver_for({
        "admin_bot": view, "team_bot_a": {}, "team_bot_b": {},
    })
    targets, dropped = ml.routed_probe_targets(
        _NETWORK, roles_resolver=resolver, max_targets=2,
        exclude=("claude-opus-4-8",),
    )
    assert [t.model for t in targets] == ["anthropic/m-a", "anthropic/m-b"]
    assert dropped == [], "the excluded model ate a slot that m-b needed"


def test_scheduled_run_names_its_exclusions_in_the_summary(tmp_path):
    """No silent caps: a run that declines to probe something says so. Also
    proves the exclusion reaches run_probes' routed path end to end, not just
    routed_probe_targets in isolation."""
    resolver = _resolver_for({
        "admin_bot": {
            "max": {"resolvedModel": "anthropic/claude-opus-4-8"},
            "fast": {"resolvedModel": "anthropic/claude-haiku-4-5"},
    ...
    runner = FakeRunner(ml.AgentRunResult(payload=_envelope(model="claude-haiku-4-5")))
    summary = ml.run_probes(
        tmp_path, _NETWORK, runner=runner, roles_resolver=resolver, dry_run=True,
    )
    assert summary["excluded_from_schedule"] == list(ml.SCHEDULED_PROBE_EXCLUSIONS)
    # The costly model was resolved for admin_bot and still never dispatched.
    assert [r.model for r in runner.requests] == ["anthropic/claude-haiku-4-5"]
