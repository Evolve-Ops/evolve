...
def test_cache_retention_accepts_auto():
    """"auto" is stored as-is; the materializer resolves it to short/long at
    deploy time (cache_shape.resolve_retention). Storing the RESOLVED value
    instead would freeze the tier on whatever the traffic looked like the day
    an operator clicked."""
    cfg = BetterEngineConfig.default()
    cfg.set_per_bot_cache_retention("team_bot_a", "auto")
    assert cfg.per_bot_cache_retention("team_bot_a") == "auto"
    restored = BetterEngineConfig.from_dict(cfg.to_dict())
    assert restored.per_bot_cache_retention("team_bot_a") == "auto"


def test_cache_retention_enum_has_one_home():
    """The accepted values come from cache_shape, which owns the resolver
    that interprets them. A literal here would be a second place to forget.

    The import IS guarded — better_engine_config is the read path for every
    budget, cap and ladder value in the pod, and an unimportable cache_shape
    must fail the cache knob rather than the whole config. That guard carries
    a fallback literal, so pin the two against each other: the fallback exists
    to keep a deploy alive, not to let the enum drift.
    """
    import better_engine_config as bec
    import cache_shape
    assert set(cache_shape.RETENTION_VALUES) == {"short", "long", "auto"}
    assert tuple(bec.CACHE_RETENTION_VALUES) == tuple(cache_shape.RETENTION_VALUES)


def test_a_missing_cache_shape_does_not_take_be_config_down():
    """The whole-config outage finding 5 named: a top-level import of a cache
    enum could take every budget, cap and remediation-ladder read with it."""
    import importlib
    import sys

    import better_engine_config as bec

    saved = sys.modules.get("cache_shape")
    sys.modules["cache_shape"] = None          # import raises ImportError
    try:
        reloaded = importlib.reload(bec)
        assert reloaded.CACHE_RETENTION_VALUES == ("short", "long", "auto")
        cfg = reloaded.BetterEngineConfig.default()
        cfg.set_per_bot_cache_retention("team_bot_a", "auto")
        assert cfg.per_bot_cache_retention("team_bot_a") == "auto"
    finally:
        if saved is None:
            sys.modules.pop("cache_shape", None)
        else:
            sys.modules["cache_shape"] = saved
        importlib.reload(bec)


def test_the_resolved_cache_tier_is_recorded_beside_the_knob():
    """The dwell rule needs to know what was last DEPLOYED, which is not
    recoverable from the gap distribution alone."""
    cfg = BetterEngineConfig.default()
    assert cfg.per_bot_cache_retention_decision("team_bot_a") == (None, None)
    cfg.set_per_bot_cache_retention_decision(
        "team_bot_a", "long", "2026-09-01T00:00:00Z",
    )
    restored = BetterEngineConfig.from_dict(cfg.to_dict())
    assert restored.per_bot_cache_retention_decision("team_bot_a") == (
        "long", "2026-09-01T00:00:00Z",
    )
    # A junk value reads as "no standing decision" rather than pinning a tier.
    junk = BetterEngineConfig.from_dict({
        "bots": {"b": {"budget": {"per_bot_cache_retention_resolved": "auto"}}},
    })
    assert junk.per_bot_cache_retention_decision("b") == (None, None)


def test_unrecognised_stored_cache_retention_reads_as_none():
    """A hand-edited BE config must not wedge a deploy over a cache knob."""
    cfg = BetterEngineConfig.from_dict({
        "bots": {"team_bot_a": {"budget": {"per_bot_cache_retention": "forever"}}},
    })
    assert cfg.per_bot_cache_retention("team_bot_a") is None
