...
def test_macos_acl_check_resolves_dir_form_perm_names(tmp_path):
    """`chmod +a "user:evo allow read,write,execute,append" <dir>` stores the
    directory-resolved names (list/add_file/add_subdirectory/search) and
    ...
    The resolved string is captured verbatim from a fresh grant on the
    reference pod 2026-08-18, hence the kernel's ordering rather than the
    source order — `execute` lands as `search` between `add_file` and `delete`.
    """
    resolved = (
...
@pytest.mark.parametrize("verbs,bits", [
    ...
    # The evo write grant once ``execute`` joined it (keystore 0750 traverse):
    # collapse is UNCHANGED — a write-ish set already implies ``X``, so the
    # Linux backend was granting dirs ``x`` all along. This row is what makes
    # "adding execute is a Linux no-op in granted bits" a pinned claim rather
    # than a comment.
    ("read,write,execute,delete,append", "rwX"),
    ("read,execute,readattr", "rX"),             # execute alone → still rX
...
def test_linux_recursive_reassert_repairs_only_capped_paths(tmp_path):
    ...
    whose ACCESS-ACL mask is actually CAPPING an entry (getfacl's
    ``#effective:`` annotation): a default-ACL-only dir is listed by -s
    but must be left alone, and so must a masked-but-fully-effective
    path — re-widening healthy masks on every hourly sweep was ~12k
    privileged execs/day on the two-bot VPS (2026-07-29 denial audit)."""
    root = tmp_path / "plugins"
    enumerated = (
        # mask::r-- caps the named rX ACE → must be repaired
        f"# file: {root}\n# owner: evolve\n# group: staff\n"
        "user::rwx\nuser:evolve:r-x\t#effective:r--\ngroup::r-x\t#effective:r--\n"
        "mask::r--\nother::r-x\n"
        "\n"
        # default-ACL only (no access mask) → left alone
        f"# file: {root}/defaults-only\n# owner: evolve\n# group: staff\n"
        "user::rwx\ngroup::r-x\nother::r-x\n"
        "default:user::rwx\ndefault:user:evolve:r-x\n"
        "default:group::r-x\ndefault:mask::r-x\ndefault:other::r-x\n"
        "\n"
        # mask present but capping nothing (no #effective) → left alone
...
def test_linux_reassert_mask_skips_healthy_mask(tmp_path):
    """Non-recursive guard: a mask that caps nothing (no ``#effective:``
    annotation anywhere) is already healthy — the reassert must be a
    query-only no-op so the hourly Tier-1 sweeps cost zero privileged
    execs in steady state."""
    healthy = (
        f"# file: {tmp_path}\n# owner: evolve\n# group: staff\n"
        "user::rwx\nuser:evolve:r-x\ngroup::r-x\nmask::rwx\nother::---\n"
    )
    run, calls = _recording_runner({(GETFACL,): _Result(0, stdout=healthy)})
    assert LinuxPerms(runner=run).reassert_mask(tmp_path) is True
    assert [c[0][0:2] for c in calls] == [[GETFACL, "-p"]]  # no setfacl ran


def test_linux_reassert_mask_repairs_capped_path(tmp_path):
    """Non-recursive guard: a clamped mask (entry annotated with
    ``#effective:``) fires exactly one ``setfacl -m m::rwX``."""
    clamped = (
        f"# file: {tmp_path}\n# owner: darwin\n# group: staff\n"
        "user::rwx\nuser:evolve:r-x\t#effective:---\ngroup::---\nmask::---\nother::---\n"
    )
    run, calls = _recording_runner({
        (GETFACL,): _Result(0, stdout=clamped),
        ("sudo", SETFACL): _Result(0),
    })
    assert LinuxPerms(runner=run).reassert_mask(tmp_path) is True
    assert calls[-1][0] == ["sudo", SETFACL, "-m", "m::rwX", str(tmp_path)]
