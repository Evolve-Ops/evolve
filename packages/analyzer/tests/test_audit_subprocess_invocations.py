...
def test_send_security_alert_fallback_routes_through_dispatcher(
    ...
    assert call["catalog_event"] == "security.audit_critical", (
        ...
        f"take effect. The key is the CRITICAL/IMMEDIATE one, not the "
        f"security.audit_finding umbrella — that umbrella is DAILY_DIGEST by "
        f"design, so annotating this page-on-transition send with it made the "
        f"dispatcher defer every newly-firing CRITICAL. got: "
        f"{call['catalog_event']}"
    )
...
def test_send_security_alert_dispatcher_success_skips_direct(
    ...
    direct_calls: list = []
    def _direct(token, chat_id, msg, shared_dir):
        direct_calls.append((token, chat_id, msg))
        return True
    ...
    monkeypatch.setattr(audit, "_send_via_dispatcher",
                        lambda msg, shared_dir, config: audit._DISPATCH_DELIVERED)
...
def test_send_security_alert_falls_back_to_keystore_when_dispatcher_fails(
    ...
    monkeypatch.setattr(audit, "_send_via_dispatcher",
                        lambda msg, shared_dir, config: audit._DISPATCH_BROKEN)
