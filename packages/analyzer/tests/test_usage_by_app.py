...
# ── B6: unmeasured must never present as zero (2026-08-31) ──────────────────

def test_zero_cost_annotation_from_known_provider_is_repriced(shared: Path) -> None:
    """The PoC-bot regression: grok-4 turns landed with cost_estimated 0
    (the TS estimator had an Anthropic-only table) and real token counts.
    30 days of those records sit in this rollup's window, so the rollup
    re-prices a recorded zero from model/provider + tokens via turn_cost
    (offline xai provider rate here — no pricing catalog under tmp_path)."""
    _write_annotations(shared, "team_bot_a", TODAY, [
        _annotation(
            app_id="app-a", grade="explicit", cost=0.0, tokens=0,
            model_selected="grok-4", provider="xai",
    ...
    assert window["total"]["cost_estimated"] == pytest.approx(0.165)
    assert window["total"]["unpriced_turns"] == 0
    assert payload["coverage"]["d1"]["unpriced_turns"] == 0


def test_unpriceable_turn_is_counted_not_summed_as_free(shared: Path) -> None:
    """An annotation no pricing source can cost (unknown provider AND
    unknown model, recorded $0, real tokens) is surfaced as an unpriced
    turn — in its metrics bucket and in coverage — never silently summed
    as $0 spend."""
    _write_annotations(shared, "team_bot_a", TODAY, [
        _annotation(
            app_id="app-a", grade="explicit", cost=0.0, tokens=0,
            model_selected="mystery-model", provider="mystery-cloud",
    ...
    payload = rollup_bot(shared, "team_bot_a", today=TODAY)
    window = payload["apps"]["app-a"]["d1"]
    assert window["total"]["turns"] == 2
    assert window["total"]["cost_estimated"] == pytest.approx(0.20)
    assert window["total"]["unpriced_turns"] == 1
    assert payload["coverage"]["d1"]["unpriced_turns"] == 1


def test_recorded_nonzero_cost_still_wins(shared: Path) -> None:
    """A plugin-recorded non-zero cost_estimated is used verbatim — the
    re-estimate path only fires on the zero-with-tokens shape."""
    _write_annotations(shared, "team_bot_a", TODAY, [
        _annotation(
            app_id="app-a", grade="explicit", cost=0.42,
            model_selected="grok-4", provider="xai",
        ),
    ])

    payload = rollup_bot(shared, "team_bot_a", today=TODAY)
    assert payload["apps"]["app-a"]["d1"]["total"]["cost_estimated"] == pytest.approx(0.42)
