...
def test_resolve_per_bot_cap_falls_back_to_be_pod_default(tmp_path) -> None:
    """No per-bot value → the BE-config pod default, the same rung the
    enforcer trips on and the UI displays.

    This detector used to read only the explicit per-bot field, so it sized
    its heartbeat-cost projections against "no cap" for every bot running on
    the pod default. See test_cost_cap_reader_parity.py.
    """
    import json
    be_payload = {
        "schema_version": 1,
        "pod_defaults": {"budget": {"per_bot_daily_hard_usd": 20.0}},
        "bots": {},
    }
    (tmp_path / "better-engine-config.json").write_text(json.dumps(be_payload))
    assert cost_watchdog._resolve_per_bot_cap(
        "security-bot", None, tmp_path
    ) == 20.0


def test_resolve_per_bot_cap_falls_back_to_pod_global_without_be_config(
    tmp_path,
) -> None:
    """The legacy network.json::dailySpendCapUsd global is now reachable only
    when BE config can't be consulted at all (no shared_dir / read failure).
    Retained for un-migrated installs."""
    cfg = {"dailySpendCapUsd": 20.0}
    assert cost_watchdog._resolve_per_bot_cap("security-bot", cfg, None) == 20.0


def test_resolve_per_bot_cap_none_when_nothing_set(tmp_path) -> None:
    import json
    be_payload = {"schema_version": 1, "pod_defaults": {}, "bots": {}}
    (tmp_path / "better-engine-config.json").write_text(json.dumps(be_payload))
    ...
    assert cost_watchdog._resolve_per_bot_cap("security-bot", {}, None) is None
...
def test_resolve_per_bot_cap_ignores_legacy_network_json_per_bot(tmp_path) -> None:
    ...
    # Stored BE config with nothing set + no pod-wide global → no cap (was 5.0
    # pre-Phase-4). Written explicitly so the compiled-defaults layer isn't
    # what supplies the answer — this test is about the legacy per-bot key.
    be_payload = {"schema_version": 1, "pod_defaults": {}, "bots": {}}
    (tmp_path / "better-engine-config.json").write_text(json.dumps(be_payload))
