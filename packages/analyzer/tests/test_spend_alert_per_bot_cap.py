...
#   * the plugin (ModelRouter.isSpendCapActive under the explicit
#     ``downgrade-tier`` action; the before_agent_run checkpoint gate, which
#     reads the cap and spend for its fixed reply, under the default);
...
#
# The ``action`` it carries is resolved through
# ``spend_caps.resolve_spend_cap_action`` — the one reader shared with the
# UI — rather than pinned to ``downgrade-tier`` as it was before operator
# decision D-CC2 (internal/decision-cost-cap-checkpoint-2026-09-04.md).
...
def test_trip_breaker_writes_spend_cap_enforcement_flag(
    ...
    assert flag.exists(), "the plugin + the Cost Measures page read this file"
    data = json.loads(flag.read_text())
    # ``fake_network`` expresses no preference, so the default applies: the
    # trip HOLDS the next conversation instead of downgrading it.
    ...
    # match the plugin's localDateYMD() for the flag to be found at all.
    assert data["date"] == "2026-07-31"


def test_trip_arms_a_pending_checkpoint_by_default(
    fake_shared, fake_network, monkeypatch,
):
    """D-CC1: the trip is the question; ``pending`` is it being asked."""
    from breakers import store as bstore

    monkeypatch.setattr(spend_alert, "_dispatch", lambda **kw: True)
    import evolve_admin
    monkeypatch.setattr(evolve_admin, "breakers_enforce", MagicMock())

    spend_alert._trip_breaker_for_cost_cap(
        bot_id="security_bot", spend=33.78, cap=20.0,
        shared_dir=fake_shared, network=fake_network, today_iso="2026-07-31",
    )
    rec = bstore.read_trip(fake_shared, "security_bot", "cost")
    assert rec.checkpoint == "pending"


def test_explicit_downgrade_pod_keeps_the_downgrade_and_no_checkpoint(
    fake_shared, fake_network, monkeypatch,
):
    """An operator who chose the downgrade keeps it — flag action and all —
    and gets no checkpoint (there is nothing to answer)."""
    from breakers import store as bstore
    ...
    net = dict(fake_network)
    net["thresholds"] = {"spendCapAction": "downgrade-tier"}
    spend_alert._trip_breaker_for_cost_cap(
        bot_id="security_bot", spend=33.78, cap=20.0,
        shared_dir=fake_shared, network=net, today_iso="2026-07-31",
    )
    flag = json.loads(
        (fake_shared / "spend-caps" / "security_bot-2026-07-31.json").read_text()
    )
    assert flag["action"] == "downgrade-tier"
    assert bstore.read_trip(fake_shared, "security_bot", "cost").checkpoint is None


def test_continued_checkpoint_retrips_only_at_the_raised_ceiling(
    fake_shared, fake_network, monkeypatch,
):
    """D-CC3: "continue" buys +$X until the day boundary, not an unlimited
    day. Below cap+increment the trip dedups; above it, a fresh checkpoint."""
    from breakers import store as bstore
    ...
    def trip(spend):
        spend_alert._trip_breaker_for_cost_cap(
            bot_id="security_bot", spend=spend, cap=20.0,
            shared_dir=fake_shared, network=fake_network,
            today_iso="2026-07-31",
        )

    trip(20.57)
    first = bstore.read_trip(fake_shared, "security_bot", "cost")
    bstore.answer_checkpoint(
        shared_dir=fake_shared, scope="security_bot", breaker_type="cost",
        state="continued", answered_by="user:telegram:1", role="primary_user",
        increment_usd=10.0,
    )

    # Still under $30 — the grant holds, no new checkpoint.
    trip(25.00)
    mid = bstore.read_trip(fake_shared, "security_bot", "cost")
    assert mid.checkpoint == "continued"
    assert mid.trip_id == first.trip_id

    # Past the raised ceiling — ask again, at the higher number, and keep
    # the operator's grant on the record.
    trip(30.01)
    after = bstore.read_trip(fake_shared, "security_bot", "cost")
    assert after.checkpoint == "pending"
    assert after.trip_id != first.trip_id
    assert after.checkpoint_increment_usd == 10.0
    assert "$30.00" in after.reason
    # The OFFER is 50% of the BASE cap, not of the raised ceiling: a fixed
    # +$10 step, not +10 / +15 / +22.50 compounding across checkpoints.
    assert fake_be.enforce_trip.call_args.kwargs["increment_usd"] == 10.0
    assert fake_be.enforce_trip.call_args.kwargs["cap_usd"] == 30.0


def test_yesterdays_grant_does_not_raise_todays_ceiling(
    fake_shared, fake_network, monkeypatch,
):
    """D-CC3's "until the day boundary" is midnight, not the 24h TTL.

    Yesterday's "continue" put +$10 on a record that is still unexpired this
    morning. Left alone, today's $25 would sit under yesterday's raised $30
    ceiling and sail through un-held, on a day whose spend the operator never
    agreed to. The stale record is reset at the boundary and the cap re-trips
    at its configured $20 with no carried grant.
    """
    from breakers import store as bstore
    ...
    today = date.today()          # the record is stamped at real "now"
    ...
    assert not bstore.is_expired(
        bstore.read_trip(fake_shared, "security_bot", "cost")
    ), "the TTL must still be live — that is the whole point"

    # New pod-local day, $25 spent: under yesterday's $30, over today's $20.
    trip(25.00, tomorrow)

    after = bstore.read_trip(fake_shared, "security_bot", "cost")
    assert after.checkpoint == "pending"
    assert after.trip_id != first.trip_id
    assert after.checkpoint_increment_usd is None, "the grant did not survive"
    assert "$20.00" in after.reason
    flag = json.loads(
        (fake_shared / "spend-caps"
         / f"security_bot-{tomorrow.isoformat()}.json").read_text()
    )
    assert flag["cap"] == 20.0
