...
Spec: docs/spec-alerts-signal-store-2026-05-07.md (Phase 3 exit gate).
...
def test_findings_appear_in_maintenance_lane(admin_client):
    ...
    findings = [
        Finding(level="critical", category="identity", bot_id="admin_bot",
        ...
        Finding(level="critical", category="machine", bot_id=None,
...
def test_critical_findings_dispatch_and_record_delivery(admin_client):
    ...
    findings = [
        ...
        Finding(level="critical", category="config", bot_id="team_bot_a",
...
def test_repeat_run_records_suppression_audit(admin_client):
    """Second run with same findings: Telegram suppressed (dedup)
    but the Signal records the suppression so the audit trail is
    complete."""
    client, shared = admin_client
    f = Finding(level="critical", category="identity", bot_id="admin_bot",
                message="ssh key wrong perms", detail="")
    ...
    sent, suppressed = deliveries
    assert sent["suppressed_reason"] is None
    assert suppressed["suppressed_reason"] is not None
    assert "dedup" in suppressed["suppressed_reason"]
