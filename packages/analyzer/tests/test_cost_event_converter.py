...
def test_infer_trigger_kind_evolve_analysis_passthrough():
    """source ∈ {summarizer, classifier, task_extractor, fallback} is set by
    TurnObserver's llm_output capture of Evolve's own subagent calls (OC's
    plugin-subagent lane never fires agent_end, so llm_output is the only
    hook that sees them). Before this branch these enum values were
    UNREACHABLE — the Phase A2 overhead rollup structurally read $0.0000
    for Evolve-initiated kinds no matter how often the summarizer ran."""
    assert cec._infer_trigger_kind("summarizer", "subagent") == "summarizer"
    assert cec._infer_trigger_kind("classifier", "subagent") == "classifier"
    assert cec._infer_trigger_kind("task_extractor", None) == "task_extractor"
    assert cec._infer_trigger_kind("fallback", None) == "fallback"


def test_turn_to_cost_event_summarizer_is_internal():
    """A summarizer turn (as written by the plugin's llm_output capture)
    lands with trigger_kind=summarizer and channel_kind=internal — no
    external user ids ever ride on Evolve's own analysis calls."""
    turn = {
        "ts": "2026-08-01T21:55:12.489000+00:00",
        "instance": "admin_bot",
        "channel": "subagent",
        "source": "summarizer",
        ...
        "session_id": "5dd90f95-e72b-41e1-ade4-2dab54bb3f2e",
    }
    event = cec.turn_to_cost_event(turn, bot_id="admin_bot")
    assert event is not None
    assert event["trigger_kind"] == "summarizer"
    assert event["channel_kind"] == "internal"
    assert event["user_id"] is None
    assert event["channel_id"] is None
    assert event["cost_usd"] == pytest.approx(0.034)
...
# ─────────────────────────────────────────────────────────────────────────────
# App attribution pass-through (AL-1.1, design-app-attribution-2026-08-15 §6.2)
# ─────────────────────────────────────────────────────────────────────────────


def test_app_attribution_absent_defaults_to_none():
    """A pre-AL-1.1 turn record (no app fields) reads as app_id=None/'none'."""
    event = cec.turn_to_cost_event(_example_turn(), bot_id="admin_bot")
    assert event is not None
    assert event["app_id"] is None
    assert event["app_attribution"] == "none"


def test_app_attribution_passthrough():
    """A plugin-stamped record's fields ride through as trigger_kind siblings."""
    turn = _example_turn()
    turn["app_id"] = "task_manager"
    turn["app_attribution"] = "explicit"
    event = cec.turn_to_cost_event(turn, bot_id="admin_bot")
    assert event is not None
    assert event["app_id"] == "task_manager"
    assert event["app_attribution"] == "explicit"


def test_app_attribution_grade_none_drops_the_id():
    """'none' means NO signal — an id alongside it is a writer bug; normalize
    to the honest pair rather than surfacing a contradictory record."""
    turn = _example_turn()
    turn["app_id"] = "task_manager"
    turn["app_attribution"] = "none"
    event = cec.turn_to_cost_event(turn, bot_id="admin_bot")
    assert event["app_id"] is None
    assert event["app_attribution"] == "none"


def test_app_attribution_invalid_values_fail_toward_no_signal():
    turn = _example_turn()
    turn["app_id"] = "   "
    turn["app_attribution"] = "explicit"  # grade without a usable id
    event = cec.turn_to_cost_event(turn, bot_id="admin_bot")
    assert event["app_id"] is None
    assert event["app_attribution"] == "none"
    ...
    turn = _example_turn()
    turn["app_id"] = "task_manager"
    turn["app_attribution"] = "EXPLICIT"  # unrecognized grade casing
    event = cec.turn_to_cost_event(turn, bot_id="admin_bot")
    assert event["app_id"] is None
    assert event["app_attribution"] == "none"

    turn = _example_turn()
    turn["app_id"] = 42  # non-string id
    turn["app_attribution"] = "explicit"
    event = cec.turn_to_cost_event(turn, bot_id="admin_bot")
    assert event["app_id"] is None
    assert event["app_attribution"] == "none"
