...
Spec: docs/spec-model-auto-upgrade-2026-07-30.md (Phase 1 — ships dark).
...
# one model in several rungs on purpose (the judge rung is by design an alias of
# Standard's cluster). Judging condition 7 against the merged view therefore
# reported the single most common real upgrade — Sonnet N → N+1 — as "sits in
# more than one tier" while its own Update button had exactly one target.

def _stock_standard_model() -> str:
    """The model the shipped catalog puts in BOTH sonnet-class and judge-class."""
    from primary_bot import DEFAULT_MODEL_CATALOG

    rungs = {r["id"]: list(r.get("models") or []) for r in DEFAULT_MODEL_CATALOG["rungs"]}
    shared = set(rungs["sonnet-class"]) & set(rungs["judge-class"])
    assert shared, "catalog no longer aliases judge-class to standard's cluster"
...
def test_a_stock_ladder_upgrade_is_not_falsely_ambiguous():
    ...
    net = {
        "models": {
            "rungs": [{"id": "sonnet-class", "costClass": "medium",
                       "models": [stock]}],
            "roles": {"standard": "sonnet-class", "judge": "judge-class"},
        },
        "bots": {},
    }
    up = md.VersionUpgrade(
