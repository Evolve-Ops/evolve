...
def _turn(
    *,
    ...
    ``cost_source`` is omitted by default: a record without it is what the
    writer produced before 2026-09-04, and most of this file's cases are
    about that shape.
    """
    record = {
        "ts": ts,
...
def test_recorded_cost_stands_when_nothing_can_reprice_it(pod_with_catalog):
    """A recorded non-zero cost is used when no catalog row supersedes it.

    Narrowed 2026-09-04: a recorded cost used to win unconditionally, which
    is what let the plugin's family-substring estimate ride every reader at
    ~3x for months. A record the catalog CAN price by exact model id is now
    re-priced on read (see ``turn_cost_detail`` and
    ``test_turn_cost_cost_source.py``); a record it cannot still stands.
    """
    unpriceable = _turn(
    ...
    # A cost the gateway itself billed is truth and is never re-priced.
    billed = _turn(
        model=OFF_TABLE_MODEL, provider=OFF_TABLE_PROVIDER,
        ts="2026-08-25T10:00:00Z", cost=0.42, cost_source="provider",
    )
    assert tc.turn_cost(billed) == pytest.approx(0.42)
    # Without that stamp the catalog's own price for the exact id wins.
    estimated = _turn(
        model=OFF_TABLE_MODEL, provider=OFF_TABLE_PROVIDER,
        ts="2026-08-25T10:00:00Z", cost=0.42,
    )
    assert tc.turn_cost(estimated) == pytest.approx(10.0)
...
def live_turns(pod, monkeypatch):
    ...
    and ``spend_alert.load_today_spend_detail`` both bucket turns by the
    pod-LOCAL day of each ``ts``, so a test staging
