...
# ─────────────────────────────────────────────────────────────────────────────
# Bot-side credential resolution (#3466: provider-agnostic)
# ─────────────────────────────────────────────────────────────────────────────
...
def _write_credential(home: Path, provider: str, key: str) -> None:
    cred_dir = home / ".openclaw" / "credentials"
    cred_dir.mkdir(parents=True, exist_ok=True)
    (cred_dir / f"{provider}.json").write_text(json.dumps({"api_key": key}))


def test_resolve_llm_target_reads_any_provider_file(tmp_path, _no_ambient_keys):
    """The generalized reader: a credentials/openai.json (not anthropic)
    yields an openai fast-tier target — no provider presumed."""
    from generators.user_profile_inferrer import hook as hk

    _write_credential(tmp_path, "openai", "sk-test-not-a-real-key")
    target = hk._resolve_llm_target(tmp_path)
    assert target is not None
    assert target.provider == "openai"
    assert target.model.startswith("openai/")
    assert target.api_key == "sk-test-not-a-real-key"


def test_resolve_llm_target_none_without_credentials(tmp_path, _no_ambient_keys):
    from generators.user_profile_inferrer import hook as hk

    assert hk._resolve_llm_target(tmp_path) is None


def test_resolve_llm_target_env_var_override(tmp_path, _no_ambient_keys, monkeypatch):
    from generators.user_profile_inferrer import hook as hk
...
def test_hook_skips_with_no_llm_credentials(tmp_path, _no_ambient_keys):
    summary = run_hook(
        bot_id="atlas",
        user_key="alice",
        home_dir=tmp_path,
        transcript_text="hello there",
        llm_call=None,  # production path: resolve from bot credentials
    )
    assert summary["skipped"] == "no_llm_credentials"


def test_default_llm_call_flows_through_infra_llm(tmp_path, _no_ambient_keys, monkeypatch):
    """Non-Anthropic flow-through (#3466): the production callable routes
    the prompt through infra_llm.complete with the openai target resolved
    from the bot's own credential file."""
