...
def test_unmapped_tier_id_is_refused_not_written_as_null_role(
    ...
    """``_is_tier_id_or_none`` admits any tierN but only tier0-3 have a role
    mapping. A tier7 must land in ``routingKeysRefused`` like any other
    rejected value — not silently become ``<x>Role: null`` (#3662 review
    follow-up)."""
    ...
    assert result["routingKeysRefused"] == ["maintenanceTier"]
    stored = json.loads(tiers_path.read_text())["routing"]
    # The refused slot is untouched; the mappable one translates (tier0 is a
    # real tier — the judge role — not a refusal).
    assert stored == {
        "enabled": True, "maintenanceRole": "fast", "backgroundRole": "judge",
    }
