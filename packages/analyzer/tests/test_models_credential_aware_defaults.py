...
def test_derive_with_moonshot_only_picks_kimi_models():
    """A Kimi-only pod (Moonshot-credentialed, nothing else) must derive
    usable tiers — the provider-switcher scenario the moonshot rows in
    _PROVIDER_PICKS exist for."""
    out = derive_default_tiers({"moonshot"})
    assert out["tier1"]["models"] == ["moonshot/kimi-k3"]
    assert out["tier2"]["models"] == ["moonshot/kimi-k3"]
    assert out["tier3"]["models"] == ["moonshot/kimi-k2.6"]
    for tier_id in ("tier0", "tier1", "tier2", "tier3"):
        for m in out[tier_id]["models"]:
            assert m.startswith("moonshot/"), (
                f"Moonshot-only pod should derive only moonshot models, "
                f"got {m} for {tier_id}"
            )
...
# ── derive_seed_models (fresh-pod primary + classifier seed) ────────────────


def test_seed_models_anthropic_pin():
    """Regression pin: anthropic-credentialed pods seed exactly today's
    models — sonnet workhorse primary, haiku classifier."""
    assert derive_seed_models({"anthropic"}) == (
        "anthropic/claude-sonnet-4-6", "anthropic/claude-haiku-4-5",
    )


def test_seed_models_openai_only():
    assert derive_seed_models({"openai"}) == (
        "openai/gpt-4o", "openai/gpt-4o-mini",
    )


def test_seed_models_unknown_provider_only_returns_empty():
    """Unlike derive_default_tiers, the seed helper must NOT fall back to
    the Anthropic-pinned DEFAULT_TIERS for a provider with no
    _PROVIDER_PICKS entry — empty means "leave the field unseeded" so
    wizard_verify fires loudly instead of seeding a dead model."""
    assert derive_seed_models({"mistral"}) == ("", "")


def test_seed_models_no_providers_returns_empty():
    assert derive_seed_models(set()) == ("", "")
    assert derive_seed_models(None) == ("", "")


def test_seed_models_moonshot_only():
    assert derive_seed_models({"moonshot"}) == (
        "moonshot/kimi-k3", "moonshot/kimi-k2.6",
    )


def test_seed_models_mixed_known_and_unknown_uses_known():
    assert derive_seed_models({"mistral", "openai"}) == (
        "openai/gpt-4o", "openai/gpt-4o-mini",
    )
