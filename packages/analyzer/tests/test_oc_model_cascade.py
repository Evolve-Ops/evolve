...
def test_cascade_write_preserves_existing_tiers_and_routing(
    ...
    written = json.loads(tiers_path.read_text())
    assert written["cascade"] == {"enabled": True}
    # Read the tier allocations through the shape-agnostic projection: the
    # pre-populate step above lands on a bot with no tiers file, which since
    # #3566 is created on the rungs/roles shape rather than the deprecated
    # ``tiers`` one. What this test is about is that the cascade flip does not
    # nuke them, which is a merge question, not a shape question.
    assert oc_model.synthesize_legacy_tiers(written) == {
        "tier3": {"models": ["anthropic/claude-haiku-4-5"]},
    }
