...
def test_the_weekly_run_writes_a_module_set_beside_the_edition(pod):
    ...
    got = _by_id(payload)
    # The weekly run builds the report's app-name table; this fixture pod
    # installs no manifest for the id, so it is named as unknown, not
    # silently printed as the id.
    assert got["apps_leaderboard"]["headline"].startswith(
        "Unknown app (morning_brief) was")
...
def test_a_module_set_is_re_said_over_a_sealed_week_without_force(pod, capsys,
    ...
    out = capsys.readouterr().out
    assert "re-said" in out and "Unknown app (morning_brief)" in out
...
# ── names: the tile leads with the app's name, never its manifest id ────────

def _apps_module(edition: dict, names: dict | None) -> dict:
    payload = mod.build_modules(edition, [], now=NOW_MIDWEEK, app_names=names)
    return _by_id(payload)["apps_leaderboard"]


def test_three_ids_read_as_three_names_and_the_headline_names_the_leader():
    edition = _edition("2026-W35", apps={"p-0000aaa1": 135, "i-0000bbb2": 40,
                                         "p-0000ccc3": 12})
    names = {"p-0000aaa1": "Morning Brief", "i-0000bbb2": "Inbox Sweep",
             "p-0000ccc3": "Task Manager"}
    card = _apps_module(edition, names)
    assert [r["name"] for r in card["values"]["top"]] == [
        "Morning Brief", "Inbox Sweep", "Task Manager"]
    assert card["headline"].startswith("Morning Brief was the busiest")
    assert card["detail"]["apps_without_a_name"] == {"count": 0, "ids": []}


def test_an_id_the_catalogue_cannot_name_is_shown_and_counted_not_dropped():
    edition = _edition("2026-W35", apps={"p-0000dead": 135,
                                         "p-0000aaa1": 40})
    card = _apps_module(edition, {"p-0000aaa1": "Morning Brief"})
    assert [r["name"] for r in card["values"]["top"]] == [
        "unknown app (p-0000dead)", "Morning Brief"]
    # The sentence opens capitalised; the id stays visible.
    assert card["headline"].startswith("Unknown app (p-0000dead) was")
    assert card["detail"]["apps_without_a_name"] == {
        "count": 1, "ids": ["p-0000dead"]}


def test_the_name_table_is_built_from_the_catalogue_and_installed_manifests(
        tmp_path: Path):
    from dossier import names

    index = tmp_path / "index.json"
    index.write_text(json.dumps([
        {"pkg_id": "p-0000aaa1", "display_name": "Morning Brief"},
        {"pkg_id": "p-0000spec", "display_name": "Inbox Sweep"},
    ]))
    mdir = tmp_path / "home" / "bot-a" / ".openclaw" / "workspace" / "manifests"
    mdir.mkdir(parents=True)
    (mdir / "p-0000ccc3.json").write_text(json.dumps(
        {"pkg_id": "p-0000ccc3", "title": "Task Manager", "name": "tm"}))
    # A v7-arc Instance names nothing itself; its Spec's catalogue name does.
    (mdir / "i-0000bbb2.json").write_text(json.dumps(
        {"manifest_shape": "v7-arc", "instance_id": "i-0000bbb2",
         "provenance": {"spec_id": "p-0000spec", "spec_version": "1"}}))
    (mdir / "broken.json").write_text("{not json")

    table = names.app_names(tmp_path / "shared", ["bot-a"],
                            bot_home=lambda b: tmp_path / "home" / b,
                            gallery_index=index)
    assert table == {"p-0000aaa1": "Morning Brief", "p-0000spec": "Inbox Sweep",
                     "p-0000ccc3": "Task Manager", "i-0000bbb2": "Inbox Sweep"}
