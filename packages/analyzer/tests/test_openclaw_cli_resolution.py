...
def test_doctor_pass_runner_invokes_the_resolved_binary(tmp_path: Path, monkeypatch):
    ...
    class _R:
        returncode = 0
        stdout = '{"ok": true, "checksRun": 30, "findings": []}'
        stderr = ""

    monkeypatch.setattr(doctor_pass_runner.subprocess, "run",
                        lambda argv, **_k: seen.append(argv) or _R())
    # Never let a test write into the developer's real home.
    monkeypatch.setattr(doctor_pass_runner, "write_artifact",
                        lambda payload, **_k: tmp_path / "doctor-lint.json")
    monkeypatch.setattr(sys, "argv", ["doctor_pass_runner.py", "--bot-id", "demo_bot"])

    assert doctor_pass_runner.main() == 0
    # --lint --json, not --fix: under OC 2026.9.2 `--fix` cannot enter
    # maintenance from a timer and repairs nothing (see the module docstring).
    assert seen == [[str(fake), "doctor", "--lint", "--json"]]
