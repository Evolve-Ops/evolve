...
def _write_audit_snapshot(shared_dir: Path, age_minutes: float, criticals: int = 0,
                          warns: int = 0, kinds: list[str] | None = None,
                          warn_kinds: list[str] | None = None):
    """Write a current-findings snapshot.

    ``kinds`` stamps R-3 ``finding_kind`` values onto the criticals,
    positionally. Left ``None`` (the default every pre-existing caller
    uses) the key is OMITTED entirely — which is the real shape of a
    snapshot on a pod that has not yet pulled the R-3 audit, and the case
    the reader's ``.get()`` exists for.

    ``warn_kinds`` does the same for warns. audit._write_findings_snapshot
    never writes the key there; the param exists so a test can prove the
    reader ignores it even if one somehow appears.
    """
    completed_at = datetime.now(timezone.utc) - timedelta(minutes=age_minutes)

    def _crit(i: int) -> dict:
        f = {"category": "machine", "bot_id": None,
             "message": f"crit-{i}", "detail": ""}
        if kinds is not None and i < len(kinds):
            f["finding_kind"] = kinds[i]
        return f

    def _warn(i: int) -> dict:
        f = {"category": "config", "bot_id": "admin_bot",
             "message": f"warn-{i}", "detail": ""}
        if warn_kinds is not None and i < len(warn_kinds):
            f["finding_kind"] = warn_kinds[i]
        return f
    ...
    payload = {
        ...
        "critical": [_crit(i) for i in range(criticals)],
        "warn": [_warn(i) for i in range(warns)],
    }
...
def test_audit_line_marks_fix_once_findings(tmp_path: Path):
    """R-3 payoff on the Broken line: standing config is not an incident.

    The undifferentiated "Audit: N CRITICAL" is what this fixes. The
    standing-alerts section of the SAME daily message already splits the
    two (PR #3941), so leaving this line flat made one message carry a
    differentiated and an undifferentiated statement about overlapping
    sets.
    """
    members = ["team_bot_a"]
    ref_date = date(2026, 5, 7) - timedelta(days=1)
    _write_audit_snapshot(
        tmp_path, age_minutes=3, criticals=5, warns=2,
        kinds=["posture", "event", "posture", "posture", "event"],
    )
    _write_status(tmp_path, "team_bot_a", reachable=True)

    broken = collect_broken(tmp_path, members, ref_date, DEFAULT_OVERRIDES)
    line = [l for l in broken if "Audit:" in l.text][0].text

    # Bound to the CRITICAL count, not trailing after "· N warn" — warns
    # never carry a classification, so a trailing clause would read as
    # though it counted them.
    assert "Audit: 5 CRITICAL (3 one-time fixes) —" in line, line
    assert "2 warn" in line


def test_audit_fix_once_singular(tmp_path: Path):
    members = ["team_bot_a"]
    ref_date = date(2026, 5, 7) - timedelta(days=1)
    ...
    broken = collect_broken(tmp_path, members, ref_date, DEFAULT_OVERRIDES)
    line = [l for l in broken if "Audit:" in l.text][0].text
    assert "Audit: 2 CRITICAL (1 one-time fix) —" in line, line


def test_audit_fix_once_count_is_a_floor_not_a_complement(tmp_path: Path):
    """Unclassified is not the same as active.

    Only audit criticals carry finding_kind, and only once the pod has
    pulled the R-3 audit. So ``critical_count - posture_count`` is the
    unclassified remainder, and the line must characterise it nowhere —
    not in the fix-once clause and not anywhere else in the text.

    Mirrors test_posture_count_is_a_floor_not_a_complement on the
    standing-alerts side: it reads the WHOLE rendered line, because a
    version that only inspected the clause would pass while a separate
    "the other 2 are active" phrase sat beside it.
    """
    members = ["team_bot_a"]
    ref_date = date(2026, 5, 7) - timedelta(days=1)
    _write_audit_snapshot(
        tmp_path, age_minutes=3, criticals=4, warns=1,
        # Deliberately mixed: one classified posture, one classified
        # event, and two criticals with no classification at all.
        kinds=["posture", "event"],
    )
    _write_status(tmp_path, "team_bot_a", reachable=True)
    ...
    assert "(1 one-time fix)" in line, line
    lowered = line.lower()
    for claim in ("are active", "is active", "need attention",
                  "needs attention", "the other", "the rest"):
        assert claim not in lowered, (
            f"the line must make no claim about the unclassified "
            f"remainder; found {claim!r} in:\n{line}"
        )


def test_audit_line_unchanged_on_a_snapshot_without_classification(tmp_path: Path):
    """A pod that has not pulled the R-3 audit yet renders what it always did.

    ``finding_kind`` is an additive key, so its ABSENCE is the live state
    on any pod mid-rollout. The reader must .get() it, and the clause must
    disappear entirely rather than render "(0 one-time fixes)".
    """
    members = ["team_bot_a"]
    ref_date = date(2026, 5, 7) - timedelta(days=1)
    _write_audit_snapshot(tmp_path, age_minutes=3, criticals=3, warns=1)
    _write_status(tmp_path, "team_bot_a", reachable=True)

    broken = collect_broken(tmp_path, members, ref_date, DEFAULT_OVERRIDES)
    line = [l for l in broken if "Audit:" in l.text][0].text
    assert line.startswith("Audit: 3 CRITICAL — "), line
    assert "one-time" not in line


def test_audit_all_events_render_no_fix_once_clause(tmp_path: Path):
    """Classified, but nothing standing — no clause, not "(0 one-time fixes)"."""
    members = ["team_bot_a"]
    ref_date = date(2026, 5, 7) - timedelta(days=1)
    _write_audit_snapshot(tmp_path, age_minutes=3, criticals=2,
                          kinds=["event", "event"])
    ...
    broken = collect_broken(tmp_path, members, ref_date, DEFAULT_OVERRIDES)
    line = [l for l in broken if "Audit:" in l.text][0].text
    assert line.startswith("Audit: 2 CRITICAL — "), line
    assert "one-time" not in line


def test_warns_are_never_counted_as_fix_once(tmp_path: Path):
    """The clause qualifies the CRITICAL count and nothing else.

    audit._write_findings_snapshot writes warn entries without the key by
    construction. This pins that the reader would still ignore one — the
    clause sits adjacent to the critical count, so counting a warn there
    would state a number about the wrong set.
    """
    members = ["team_bot_a"]
    ref_date = date(2026, 5, 7) - timedelta(days=1)
    _write_audit_snapshot(tmp_path, age_minutes=3, criticals=2, warns=3,
                          kinds=["posture", "event"],
                          warn_kinds=["posture", "posture", "posture"])
    _write_status(tmp_path, "team_bot_a", reachable=True)

    broken = collect_broken(tmp_path, members, ref_date, DEFAULT_OVERRIDES)
    line = [l for l in broken if "Audit:" in l.text][0].text
    assert "Audit: 2 CRITICAL (1 one-time fix) —" in line, line
    assert "3 warn" in line


def test_stale_snapshot_suppresses_the_fix_once_clause(tmp_path: Path):
    """Stale suppresses every count on this line; the new one is no exception."""
    members = ["team_bot_a"]
    ref_date = date(2026, 5, 6)
    _write_audit_snapshot(tmp_path, age_minutes=120, criticals=4,
                          kinds=["posture", "posture", "posture", "posture"])
    _write_status(tmp_path, "team_bot_a", reachable=True)

    broken = collect_broken(tmp_path, members, ref_date, DEFAULT_OVERRIDES)
    stale = [l for l in broken if "stale" in l.text][0].text
    assert "one-time" not in stale
