...
# _read_auth_profiles now reads the SOURCE through the shared oc_auth_store
# ladder (admin's oc_store delegates to the same module since #3475) so the
...
def test_read_auth_profiles_falls_back_when_shared_reader_unimportable(tmp_path, monkeypatch):
    """When the shared reader can't be imported, the legacy direct read is used
    so un-migrated / legacy pods still resolve.

    The reader is ``oc_auth_store`` since #3475 (an analyzer sibling — this
    module used to reach up into ``evolve_admin.oc_store``, against the
    dependency direction).
    """
    import sys
    # Force the shared-reader import to fail (ImportError on `from … import`).
    monkeypatch.setitem(sys.modules, "oc_auth_store", None)
...
#
# Spec: internal/design-recommendation-legibility-2026-06-12.md. A provider that
