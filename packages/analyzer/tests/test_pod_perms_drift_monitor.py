...
def test_drift_fires_one_pod_signal(stub_admin_imports):
    ...
    # Flap hysteresis (internal/spec-transient-signal-suppression-2026-06-23.md):
    ...
    # L1 vocabulary (internal/spec-drift-alert-taxonomy-2026-06-26.md): the
    # operator-facing title says "repair run", not "drift" — this is Evolve's
    # own permission contract waiting for its repair pass, not tampering. The
    # producer and TYPE are unchanged on purpose (asserted above), because a
    # type is half of every Signal signature.
    assert "need a repair run" in sig["title"]
...
def test_body_puts_each_drift_target_on_its_own_line():
    """The body is read as Markdown on the Alerts page — every drift entry
    must be its OWN line.

    Two boundaries are pinned here, because they fail independently:

      * intra-category — two targets under one header must not run
        together onto a single line;
      * inter-category — the first target of a category must not run onto
        the end of its own ``**category**`` header.

    Both were broken when the entries were concatenated with ``"".join``
    (the headers' leading ``\\n`` made the grouping *look* right while every
    ``- `` entry ran together). Asserted on the SPLIT, not on a substring —
    a substring check passes either way.
    """
    drifted = [
        _FakePermCheck(
            category="dir-owner",
            target="/Users/Shared/evolve/proposals/pending",
            ok=False,
            detail="owner=team_bot_d != expected evolve",
            fix_description="chown evolve:wheel proposals/pending",
        ),
        _FakePermCheck(
            category="dir-owner",
            target="/Users/Shared/evolve/proposals/archived",
            ok=False,
            detail="owner=team_bot_a != expected evolve",
            fix_description="chown evolve:wheel proposals/archived",
        ),
        _FakePermCheck(
            category="ACL",
            target="/Users/Shared/evolve/proposals",
            ok=False,
            detail="evolve-allow ACE missing",
        ),
    ]
    body = monitor._drift_signal(drifted)["body"]
    lines = body.split("\n")

    # Every drift entry is its own line — one bullet per drifted target.
    entry_lines = [ln for ln in lines if ln.startswith("- `")]
    assert len(entry_lines) == len(drifted), (
        f"expected {len(drifted)} entry lines, got {len(entry_lines)}: {lines!r}"
    )

    # Intra-category: the two dir-owner targets are on DIFFERENT lines.
    for target in ("proposals/pending", "proposals/archived"):
        holders = [ln for ln in lines if target in ln]
        assert len(holders) == 1, f"{target} on {len(holders)} lines: {holders!r}"
    pending_line = next(ln for ln in lines if "proposals/pending" in ln)
    assert "proposals/archived" not in pending_line

    # Inter-category: each header stands alone on its line — no entry text
    # trailing off the end of it.
    for cat in ("dir-owner", "ACL"):
        header_lines = [ln for ln in lines if f"**{cat}**" in ln]
        assert header_lines == [f"**{cat}**"], (
            f"{cat} header not on its own line: {header_lines!r}"
        )

    # Categories are separated by a blank line, and the first header is
    # separated from the intro paragraph the same way.
    for cat in ("dir-owner", "ACL"):
        assert f"\n\n**{cat}**\n- `" in body
...
def test_stale_checkphase_finding_dropped_when_bot_heals(stub_admin_imports):
    """The check-only pass runs BEFORE the per-bot reassert, so its
    evolve-access finding is a stale snapshot once the reassert heals the bot
    (V-2, evolve-vps 2026-08-31): a gateway re-clamp that self-heals within
    the same tick must not ride the drift Signal on the pre-heal snapshot."""
    from evolve_config import user_home

    calls, net, shared = stub_admin_imports
    _net_with_members(net, ["evo"])
    calls["_planned_checks"] = [
        _FakePermCheck(
            category="evolve-access",
            target=str(user_home("evo") / ".openclaw"),
            ok=False,
            detail="evolve cannot TRAVERSE .../agents/main/agent (ACL mask clamp)",
        ),
    ]
    calls["_reassert_result"] = {"evo": (True, [])}
    report = monitor.run(net)
    assert report["drifted"] == 0
    assert report["signals_fired"] == 0
...
def test_stale_drop_is_scoped_to_evolve_access_and_healed_bots(stub_admin_imports):
    """The drop must never hide a genuine finding: a non-evolve-access check on
    a healed bot's tree keeps drifting, and an evolve-access finding for a bot
    the reassert did NOT heal keeps drifting too."""
    from evolve_config import user_home

    calls, net, _shared = stub_admin_imports
    _net_with_members(net, ["evo", "darwin"])
    calls["_planned_checks"] = [
        # Same healed bot, different category → kept.
        _FakePermCheck(
            category="config-mode",
            target=str(user_home("evo") / ".openclaw" / "openclaw.json"),
            ok=False,
            detail="mode 0644, expected 0600",
        ),
        # evolve-access on the UNHEALED bot → kept.
        _FakePermCheck(
            category="evolve-access",
            target=str(user_home("darwin") / ".openclaw"),
            ok=False,
            detail="evolve cannot TRAVERSE (ACL mask clamp)",
        ),
    ]
    ...
    report = monitor.run(net, dry_run=True)
    # both stale-drop survivors + the appended unhealed finding for darwin
    assert report["drifted"] == 3


def test_post_heal_lockout_survives_a_deploy_window(stub_admin_imports, capsys):
    """Inside a deploy window the L2 gate explains the CHECK-PHASE finding for
    the bot that was deployed — but a bot still locked out AFTER the heal is a
    genuine fault whatever deploy happened, so that finding must survive. The
    gate runs before the post-heal lockouts are appended, never over them."""
    from evolve_config import user_home
    ...
    (shared / "install.json").write_text(json.dumps({
        ...
        "installed_at": stamp,
        "bot_versions": {
            "evo": {"version": "2026.0901.3999", "deployed_at": stamp},
            "darwin": {"version": "2026.0901.3999", "deployed_at": stamp},
        },
    }))
    calls["_planned_checks"] = [
        _FakePermCheck(
    ...
    report = monitor.run(net, dry_run=True)
    # evo's pre-heal drift is explained by evo's own deploy; darwin's post-heal
    # lockout is NOT dropped even though darwin was deployed just as recently.
    assert report["drifted"] == 1
    payload = next(
        json.loads(line)["would_observe"]
        for line in capsys.readouterr().out.splitlines()
        if line.startswith('{"would_observe"')
    )
    drifted = payload["details"]["drifted"]
    assert [d["target"] for d in drifted] == ["/home/darwin/.openclaw"]
    assert drifted[0]["category"] == "evolve-access"
    explained = payload["details"]["explained_by_authorized_change"]
    assert [e["target"] for e in explained] == [
        str(user_home("evo") / ".openclaw" / "openclaw.json"),
    ]
...
# ── credential_access clear side (#3477) ──────────────────────────────────


def _observe_credential_signal(shared: Path, bot_id: str) -> None:
    """Fire the reader-side credential_access Signal directly.

    The producer is ``oc_auth_store``'s inline heal path; this daemon only owns
    the CLEAR side, so the fixture writes the Signal rather than reproducing a
    clamped filesystem here (that half is pinned in
    ``test_oc_auth_store_clamp_heal.py``).
    """
    import credential_access_signal as cred

    cred.observe_unreadable(
        bot_id=bot_id,
        clamped_paths=[f"/home/{bot_id}/.openclaw/agents/main/agent"],
        heal_outcome="contract NOT fully restored",
        shared_dir=shared,
    )


def _cred_firing(shared: Path) -> list:
    import credential_access_signal as cred

    out = []
    for p in (shared / "signals" / "firing").glob("*.json"):
        sig = json.loads(p.read_text())
        if sig.get("producer") == cred.PRODUCER:
            out.append(sig.get("bot_id"))
    return sorted(out)


def test_credential_access_signal_clears_when_the_bot_reasserts(stub_admin_imports):
    """A bot the periodic reassert restores has its reader-side
    credential_access alert auto-archived — the alert's only clear path."""
    calls, net, shared = stub_admin_imports
    _net_with_members(net, ["evo"])
    _observe_credential_signal(shared, "evo")
    assert _cred_firing(shared) == ["evo"]

    calls["_reassert_result"] = {"evo": (True, [])}
    report = monitor.run(net)
    assert report["credential_access_resolved"] == 1
    assert _cred_firing(shared) == []


def test_credential_access_signal_survives_a_still_locked_out_bot(stub_admin_imports):
    """A bot the reassert could NOT restore keeps its alert — clearing it while
    the pod is still engine-dark would be worse than never firing."""
    calls, net, shared = stub_admin_imports
    _net_with_members(net, ["evo"])
    _observe_credential_signal(shared, "evo")

    calls["_reassert_result"] = {
        "evo": (False, ["evolve cannot TRAVERSE /home/evo/.openclaw"]),
    }
    report = monitor.run(net)
    assert report["credential_access_resolved"] == 0
    assert _cred_firing(shared) == ["evo"]


def test_credential_access_sweep_is_skipped_on_dry_run(stub_admin_imports):
    """``--dry-run`` must not mutate the Signal store."""
    calls, net, shared = stub_admin_imports
    _net_with_members(net, ["evo"])
    _observe_credential_signal(shared, "evo")
