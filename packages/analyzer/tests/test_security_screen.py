...
and obfuscated payloads must DENY; legitimate proposals must not), and adds:
  ...
  * the verify_review_stamp verdict assertion — a ``flagged`` stamp can
    never verify (and therefore can never apply), signed or unsigned
...
# ── verify_review_stamp — the verdict assertion (roadmap 1.7) ────────────────


class TestReviewStampVerdict:
    def _stamped(self, result: str, sign: bool) -> dict:
        stamp = {
            "proposal_id": "prop-x",
        ...
        if sign:
            sig = evolve_config.sign_review_stamp(stamp)
            if sig:
                stamp["sig"] = sig
        return {"id": "prop-x", "review_stamp": stamp}

    @pytest.fixture
    def keyed_pod(self, tmp_path, monkeypatch):
        key_path = tmp_path / "signing.key"
        key_path.write_text("ab" * 32)  # hex-encoded 32-byte key
        monkeypatch.setattr(evolve_config, "SIGNING_KEY_PATH", key_path)
        monkeypatch.setattr(
            evolve_config, "SIGNING_ENFORCED_MARKER", tmp_path / ".enforced"
        )

    @pytest.fixture
    def unkeyed_pod(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            evolve_config, "SIGNING_KEY_PATH", tmp_path / "absent.key"
        )
        monkeypatch.setattr(
            evolve_config, "SIGNING_ENFORCED_MARKER", tmp_path / ".absent-marker"
        )

    def test_flagged_stamp_never_verifies_on_keyed_pod(self, keyed_pod):
        """The negative path: a FLAGGED stamp with a VALID signature must not
        verify — pre-2026-08 only the HMAC was checked, so this applied."""
        proposal = self._stamped("flagged", sign=True)
        assert proposal["review_stamp"].get("sig"), "fixture must be signed"
        assert evolve_config.verify_review_stamp(proposal) is False

    def test_flagged_stamp_never_verifies_on_unkeyed_pod(self, unkeyed_pod):
        """Unkeyed bootstrap pods verified EVERYTHING pre-2026-08; the verdict
        assertion must hold even there."""
        proposal = self._stamped("flagged", sign=False)
        assert evolve_config.verify_review_stamp(proposal) is False

    def test_rejected_stamp_never_verifies(self, keyed_pod):
        assert evolve_config.verify_review_stamp(self._stamped("rejected", sign=True)) is False

    def test_approved_signed_stamp_verifies(self, keyed_pod):
        assert evolve_config.verify_review_stamp(self._stamped("approved", sign=True)) is True

    def test_missing_stamp_still_fails_on_keyed_pod(self, keyed_pod):
        assert evolve_config.verify_review_stamp({"id": "prop-x"}) is False
