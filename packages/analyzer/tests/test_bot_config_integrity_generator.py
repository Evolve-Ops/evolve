...
def test_catalog_provider_coverage_advisory_severity_for_cross_vendor_add(tmp_path):
    """Judge-role collapse §5.5 reframing: a credentialed-but-uncataloged
    provider whose vendor differs from the one Standard routes on buys
    failover AND a cross-vendor option for resolve_cross_vendor — an
    improvement nudged above pure hygiene, NOT an operational fix.

    team_bot_a's catalog is anthropic-only, so google/xai/openai each add a
    cross-vendor option."""
    ctx = BotConfigIntegrityContext(bot_id="team_bot_a", shared_dir=tmp_path)
    with patch.object(catalog_provider_coverage, "_read_auth_providers_safe",
                      return_value={"anthropic", "google", "xai", "openai"}):
    ...
    assert proposals
    for p in proposals:
        # Advisory, not hard_break — everything already routes.
        assert p.provenance.signals["coverage_severity"] == "advisory"
        assert p.provenance.signals["fixes_hard_break_roles"] == []
        assert p.provenance.signals["adds_cross_vendor_option"] is True
        # Cross-vendor improvement → "improvement", NOT operational_urgent.
...
def test_catalog_provider_coverage_cross_vendor_improvement_when_all_route(tmp_path):
    """No hard break (a diverse in-catalog credentialed model already routes
    every role) → a still-missing cross-vendor provider stays the soft
    'advisory' improvement (failover + cross-vendor checking), never
    operational_urgent."""
    cfg = _team_bot_a_shaped_config()
    # Put openai/gpt-4o (named in tier2) into the catalog so every role has a
    # routable model and nothing is broken.
    ...
    assert google[0].provenance.signals["coverage_severity"] == "advisory"
    assert google[0].provenance.signals["fixes_hard_break_roles"] == []
    assert google[0].provenance.signals["adds_cross_vendor_option"] is True
    assert google[0].urgency == "improvement"
