...
Spec: docs/spec-model-rungs-and-roles-2026-06-09.md §Addendum 5. The per-bot
...
# ── validate_pod_default_catalog (judge diversity + structure) ────────────────


def _diverse_catalog():
    """A minimal valid pod default: standard from one provider, judge from
    another (diversity satisfiable). No real model names — placeholders."""
    return {
        "rungs": [
        ...
        "roles": {
            ...
            "power": "power-class",
            "max": "max-class",
            "judge": {"rung": "std-class", "provider": "not-standard"},
        },
    }
...
def test_validate_rejects_undiverse_judge_when_diversity_satisfiable():
    """A ≥2-provider candidate whose judge rung shares Standard's provider is
    rejected — cross-vendor diversity IS satisfiable here, so the strict guard
    applies (the §Addendum 5 invariant)."""
    cat = _diverse_catalog()
    for rung in cat["rungs"]:
        # Collapse std-class (where judge points) to a single provider so judge
        # has no provider distinct from standard's ...
        if rung["id"] == "std-class":
            rung["models"] = ["prov_a/std"]
        # ... but keep a SECOND provider in another rung so the candidate still
        # spans ≥2 providers (diversity satisfiable → the hard check fires).
        if rung["id"] == "fast-class":
            rung["models"] = ["prov_a/fast", "prov_b/fast"]
    err = primary_bot.validate_pod_default_catalog(cat)
    assert err and "diversity" in err.lower()


def test_validate_accepts_single_provider_candidate():
    """A single-provider candidate (every rung from one provider, e.g. a
    credential-matched anthropic-only pod) is NOT rejected for judge diversity —
    diversity is unsatisfiable and the runtime routes such a judge with the soft
    `same_vendor_as_standard` advisory (#3040). The non-empty ladder + judge
    checks still apply."""
    cat = _diverse_catalog()
    # Collapse EVERY rung to one provider. judge → std-class is now same-vendor
    # as standard, but with a single provider in play the cross-vendor check is
    # not enforceable, so the candidate must save.
    for rung in cat["rungs"]:
...
def test_redundant_not_flagged_for_genuinely_custom_bot(bot_env):
    ...
    bot_env["tiers_path"].write_text(json.dumps({
        ...
        "roles": {
            ...
            "judge": {"rung": "fast-class", "provider": "not-standard"},
        },
    }))
