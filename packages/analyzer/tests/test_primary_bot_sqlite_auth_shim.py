...
These tests pin the shim that closes that gap:
  - the read-only SQLite recovery (``read_agent_auth_store_json``), including a
    live WAL sidecar (mirrors the running-gateway case), and
  - the reader precedence env → SQLite store → legacy JSON.

Since #3475 the source ladder itself lives in ``oc_auth_store`` (shared with
``evolve_admin.oc_store``, which delegates to it) — ``primary_bot`` no longer
carries a second, narrower ladder. That unification flipped the on-disk
precedence to SQLite-before-JSON, matching the admin-side reader; the two
``sqlite_wins_over_legacy_json`` tests below carry the reasoning. Broader
source coverage (bak rung, non-``main`` agents, multi-provider, the
platform-keyed network.json path) lives in
``test_infra_llm_credential_sources.py``.
...
def _no_ambient_key(monkeypatch):
    ...
    Also stubs out the root ``sudo sqlite3 -readonly`` belt in the shared
    reader: it is a production fallback for a pre-ACL bot, and a test must
    never shell out to sudo (it would prompt, or depend on the runner's
    sudoers). monkeypatch tears both down.
    """
...
def test_anthropic_key_sqlite_wins_over_legacy_json(primary_home):
    """Both present: the SQLite store is the LIVE source and wins (#3475).

    Flipped from "legacy JSON wins" when the two readers were unified on
    ``oc_auth_store`` (the admin-side reader had always preferred sqlite).
    Post-migration a leftover ``auth-profiles.json`` is either stale or the
    doctor's partial write-back holding exactly the profiles OpenClaw could NOT
    verify into SQLite — preferring it prefers the unverified credential. A
    pre-migration pod has no sqlite store, so nothing changes there.
    """
    agent = _agent_dir(primary_home)
...
def test_keys_by_provider_sqlite_wins_over_legacy_json(primary_home):
    """Same precedence as the anthropic reader — one ladder, one order (#3475)."""
    agent = _agent_dir(primary_home)
    _write_legacy_json(agent, _profiles_blob(_JSON_KEY))
    _build_sqlite_store(agent, _profiles_blob(_SQLITE_KEY))
    keys = primary_bot.read_primary_bot_keys_by_provider(_NET)
