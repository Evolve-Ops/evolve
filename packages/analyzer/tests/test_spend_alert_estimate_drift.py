...
def _quiet_log():
    """The log needs no redirecting now — ``_log`` writes under the caller's
    ``shared_dir``, which every test here already hands it as ``tmp_path``.
    The monkeypatch this fixture used to carry pinned a module constant that
    no longer exists (see ``test_spend_alert_log_path_is_derived.py``)."""
