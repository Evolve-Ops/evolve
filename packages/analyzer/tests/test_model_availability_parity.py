...
(The former judge role's off-ladder dispatch died with the judge-role
collapse — design-judge-role-collapse-2026-08-21 §5.4; expectCrossVendor
pins its replacement, the resolve_cross_vendor derivation.)
"""
...
@pytest.mark.parametrize("scenario", _load_scenarios(), ids=lambda s: s["name"])
def test_python_cross_vendor_matches_shared_fixture(scenario):
    """J2 seam parity (design judge-role-collapse §5.2): resolve_cross_vendor
    must return the fixture's expectCrossVendor model (or None) per
    against-role — the same values the TS mirror is pinned to."""
    merged = merge_model_catalog(scenario["pod"], scenario["bot"])
    credentialed = {str(p).lower() for p in scenario["credentialed"]}
    for against_role, exp in scenario.get("expectCrossVendor", {}).items():
        got = resolve_cross_vendor(
            against_role, catalog=merged, credentialed=credentialed
        )
        assert got == exp, (
            f"{scenario['name']}: cross-vendor against {against_role} "
            f"{got!r} != {exp!r}"
        )
