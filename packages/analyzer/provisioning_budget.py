...
# The pod's ONE per-turn cost rule (audit B6) — never a local zero-shim.
...
def _turn_cost(turn: dict, *, catalog: dict | None = None) -> float | None:
    ...
    Delegates to ``turn_cost.turn_cost`` — the pod's ONE per-turn rule — so
    this backstop's totals agree with the Usage page and the spend alerter to
    the penny.

    Returns ``None`` for a turn that cannot be priced (audit B6). The old
    ImportError shim returned ``0.0``, a second source of the silent zero:
    a provisioning ceiling that reads unpriceable spend as free would let a
    new bot's standup run unbounded.
    """
    ...
    *,
    ...
    for t in turns:
        ...
        if dt is None or dt < created_at:
            continue
        in_window.append(t)
    totals = sum_turn_costs(in_window)
    if totals.unpriced_turns:
        # Audit B6: unpriced turns are real spend of unknown size. Reporting the
        # priced floor as if it were the standup total would let the ceiling
        # read an unmeasurable window as "plenty of headroom", so this returns
        # ``None`` — the caller's documented "can't read the ledger" state.
        _log.warning(
            "provisioning_budget: %s standup window %s — ceiling cannot be "
            "enforced on this window",
            bot_id,
            unpriced_note(totals.unpriced_turns, totals.unpriced_providers),
        )
        return None
