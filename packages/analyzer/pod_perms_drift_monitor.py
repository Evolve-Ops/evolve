...
def _bot_for_target(target: str, bot_homes: "dict[str, Path]") -> str | None:
    """The bot whose home ``target`` sits under, or ``None`` for a pod-wide path."""
    try:
        path = Path(target)
...
def _drop_explained(
    drifted: list, shared_dir: Path, now: datetime | None,
    bot_homes: "dict[str, Path] | None" = None,
) -> "tuple[list, list[dict]]":
    """Split ``drifted`` into what still needs the operator and what does not.

    Returns ``(remaining, explained)``. ``explained`` entries carry the
    evidence line so the Signal (when one still fires) can say what Evolve
    already accounts for.

    ``bot_homes`` (``{bot_id: home}``) scopes each check to the bot whose
    home its target sits under, so the gate credits only THAT bot's deploy
    stamp. A target under no bot's home (the pod-wide shared dirs) is
    passed with no bot and gets no deploy credit — a deploy re-asserts the
    pod-perms contract rather than breaking it.

    A gate that raises leaves everything in ``remaining``: a suppression gate
    that cannot evaluate itself must never suppress.
    """
    try:
        import drift_authorization
    except Exception as exc:  # noqa: BLE001 — no gate means no suppression
        print(f"[pod_perms_drift] authorized-change gate unavailable: {exc}",
              flush=True)
        return drifted, []
    ...
    for check in drifted:
        target = str(getattr(check, "target", ""))
        try:
            found = drift_authorization.explain(
                drift_authorization.DriftChange(
                    kind=drift_authorization.KIND_POD_PERMS,
                    bot_id=_bot_for_target(target, bot_homes or {}),
                    target=target,
                ),
                shared_dir,
                now=now,
            )
        except Exception as exc:  # noqa: BLE001 — same failure direction
            print(f"[pod_perms_drift] gate failed for {target}: {exc}",
                  flush=True)
            found = None
        if found is None:
            remaining.append(check)
        else:
            explained.append({"target": target, "evidence": found.evidence})
    return remaining, explained


def _drift_signal(drifted: list, explained: list[dict] | None = None) -> dict:
    ...
    ``explained`` is what the L2 authorized-change gate already accounted
    for. Those targets are NOT in ``drifted`` — they are carried in the
    Signal's details so the operator can see what Evolve is not asking them
    about, rather than having to trust that the number went down for a
    good reason.
    """
    ...
    # L1 vocabulary split: "drift" reads as tampering, and this is not that
    # — it is Evolve's own permission contract waiting for its repair pass to
    # run. Only the OPERATOR-FACING wording changes; the Signal ``type`` stays
    # ``pod_perms_drift`` so signatures (and therefore every stored Signal,
    # every dwell counter and every operator snooze keyed on them) survive.
    title = (
        f"pod file permissions need a repair run: {n} "
        f"target{'s' if n != 1 else ''} waiting"
    )
    ...
    return dict(
        ...
        details=dict(
            ...
            explained_by_authorized_change=list(explained or []),
            what_it_means=(
...
def run(
    ...
    *,
    ...
    # Drop the check-only pass's STALE evolve-access findings for bots the
    # reassert just healed: ensure_pod_perms ran BEFORE the reassert, so its
    # evolve-access snapshot predates the heal. Without this drop, a gateway
    # re-clamp that self-heals within this very tick still rides the drift
    # Signal — violating the "a bot that self-heals emits no Signal" contract
    # above (V-2). Only the evolve-access category is dropped, and only for
    # bots whose POST-heal verify passed (ok=True is a verify fresher than the
    # finding); every other category — and every unhealed bot — keeps its
    # finding, so this never hides a genuine lockout.
    from evolve_config import user_home

    bot_homes = {bid: user_home(bu) for bid, bu in bot_pairs}
    if healed:
        healed_targets = {
            str(bot_homes[bid] / ".openclaw") for bid in healed
        }
        drifted = [
            c for c in drifted
            if not (
                str(getattr(c, "category", "")) == "evolve-access"
                and str(getattr(c, "target", "")) in healed_targets
            )
        ]
    # L2 authorized-change gate (internal/spec-drift-alert-taxonomy-2026-06-26.md):
    # drop any check-phase finding a recent deploy OF THAT BOT accounts for
    # before deciding this is something the operator should see.
    #
    # This runs on the check-only pass's findings and NOTHING ELSE — the
    # post-heal lockouts appended just below are added after it on purpose.
    # A bot still locked out AFTER the escalated re-grant is a genuine fault
    # whatever deploy happened this morning, and must never be dropped on a
    # deploy window.
    #
    # The gate's OTHER relevant source — the gateway re-locking its own
    # settings folder — is deliberately NOT consulted here, because the block
    # above already answers it with better evidence: it re-runs the grant and
    # re-verifies, so a bot whose access came back is dropped on a fresh
    # check rather than on a pattern match. See drift_authorization's module
    # docstring.
    explained_targets: list[dict] = []
    if drifted:
        drifted, explained_targets = _drop_explained(
            drifted, shared_dir, now, bot_homes,
        )
    ...
    if drifted:
        signal = _drift_signal(drifted, explained_targets)
        if dry_run:
