...
def _resolve_home_dir(user: str) -> str:
    ...
    Falls back to ``/Users/{user}`` if the system has no record of the
    user (covers dev machines without bot user accounts; production has
    real entries via macOS local users). Tests can patch this seam.
    """
    try:
        import pwd as _pwd
...
# caught up. The diagnosis doc (docs/diagnosis-spend-alert-openclaw-flag-2026-05-21.md
...
def _find_openclaw() -> str | None:
    """Find openclaw binary. Checks PATH first, then known install locations.

    On Apple Silicon Homebrew the canonical user-facing path is
    /opt/homebrew/bin/openclaw (a symlink into ../lib/node_modules/openclaw/openclaw.mjs).
    The deeper node_modules paths below `bin/openclaw` don't exist for npm-style
    installs that use a top-level `.mjs` entry — so we must check the symlink
    paths directly. evolve's launchd PATH does not include /opt/homebrew/bin,
    so shutil.which() returns None even when the binary is present.
    """
    found = shutil.which("openclaw")
    if found:
        return found
    for candidate in [
        "/opt/homebrew/bin/openclaw",          # Apple Silicon Homebrew (symlink)
        "/usr/local/bin/openclaw",             # Intel Homebrew (symlink)
...
def oc_command_raw(
    ...
    try:
        import pwd as _pwd
        _home = _pwd.getpwnam(user).pw_dir
    except Exception:
        _home = f"/Users/{user}"
...
def _discover_gateway_domain(svc: str, user: str, bot_uid: "int | None") -> "tuple[str, str] | None":
    ...
    checks: list[tuple[Path, str]] = [
        ...
        (Path(f"/Users/{user}/Library/LaunchAgents/{svc}.plist"),
         f"gui/{bot_uid}" if bot_uid is not None else "system"),
    ]
...
def oc_gateway_restart(
    ...
    """Restart the bot's openclaw gateway via launchctl kickstart -k.
    ...
    except Exception as e:  # non-launchd adapter injected, or import failure
    ...
    return {
        ...
        "error": (
            ...
            + (f"Plist found at {discovered_plist}. " if discovered_plist else "Plist not found in standard locations. ")
            + f"Tried: {'; '.join(errors)}"
        ),
        "bot": bot_id,
        "service": svc,
        ...
        "hint": (
            "Add service_domain to this bot's entry in network.json to override domain detection. "
            "Valid values: 'system', 'gui/{uid}' (e.g. 'gui/502')."
        ) if not discovered_plist else None,
    }
...
def oc_full_config_set_with_error(
    ...
    bots = _load_bots(network_path)
    if bots and bot_id not in bots:
        msg = (
