...
PROTECTIONS: dict[str, ProducerProtection] = dict(
    [
        ...
        _e(
            ...
            (NONE,),
            KIND_SIGNAL,
            emits_from=("packages/admin/evolve_admin/breakers_enforce.py",),
            justification="The breaker's push to the operator/bot user could not "
            "be delivered because the openclaw CLI refused to start (its own "
            "config fails schema validation) — the pod's spend safety net going "
            "silent (breaker-notify-survives-openclaw-config-validation). No gate: "
            "on a fresh pod this fires only on a REAL breaker trip attempt (never "
            "a sweep), and a startup config refusal is exactly as urgent on day 1 "
            "as day 100 — settling it would just delay the first notice.",
        ),
        _e(
            "repo_puller",
            (NONE,),
        ...
        _e(
            ...
            (NONE,),
            KIND_SIGNAL,
            emits_from=("packages/admin/evolve_admin/web/board_listener.py",),
            justification="Emitted only from an observed inbound connection that died "
            "before its first byte, once per daemon process — so it cannot fire on a "
            "fresh pod at all: the listener is off unless an operator enabled it, and "
            "silence until a real device connects is the normal state, not immaturity. "
            "The condition is structural (the macOS Application Firewall blocking this "
            "daemon's interpreter), so it never self-heals and never flaps; the whole "
            "mobile board surface is dark until one command is run.",
        ),
        _e(
            "bot_exec_failures",
            (NONE,),
        ...
        _e(
            ...
            (NONE,),
            KIND_SIGNAL,
            emits_from=("packages/analyzer/doctor_lint_signal.py",),
            justification="Read-only `openclaw doctor --lint` findings per bot "
            "(plaintext secrets in config, unreachable node onboarding, tool-policy "
            "gaps). OC's own health checks; a real finding is real on day one. The "
            "stale-artifact class means the nightly lint has STOPPED running — the "
            "2026-09-14 silent-daemon failure it exists to catch.",
        ),
        _e(
            "alerts_loop_monitor",
            (NONE,),
