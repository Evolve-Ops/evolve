...
PROTECTIONS: dict[str, ProducerProtection] = dict(
    [
        ...
        # RETIRED PRODUCERS (review.py retirement, decided 2026-07-28,
        # executed 2026-08-14): review.py — the only emitter of both the
        # "review" dispatch source and the "review_gate" Signal — is gone;
        # its deny mandate + AST layer now run inside
        # arbiter/security_screen.py, which emits nothing (the autonomy gate
        # fails closed in-line instead of paging). The entries stay because
        # the name-reconciliation check (lint check 3) requires every
        # PRODUCER_SEVERITY / dispatcher-source name to reconcile here, and
        # the alerts catalog still carries producer_source="review" rows
        # (subscribable keys need the LEGACY_KEY_MIGRATION treatment to
        # retire — a follow-up chip, not this one).
        _e(
            "review",
            (NONE,),
            KIND_DISPATCH,
            justification="RETIRED 2026-08-14 (emitter deleted; see block comment "
            "above). Was: per-proposal review outcome (rejected/reviewed/"
            "quarantined) with per-proposal dedup_key.",
        ),
        _e(
            "review_gate",
            (NONE,),
            KIND_SIGNAL,
            justification="RETIRED 2026-08-14 (emitter deleted; see block comment "
            "above). Was: security floor for the reviewer's AST layer failing to "
            "import; the fail-closed posture now lives in "
            "arbiter.routing.is_autonomous_eligible via arbiter/security_screen.py.",
        ),
        _e(
            "analyze",
            (NONE,),
