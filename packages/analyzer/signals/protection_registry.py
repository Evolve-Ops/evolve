...
PROTECTIONS: dict[str, ProducerProtection] = dict(
    [
        ...
        _e(
            ...
            (NONE,),
            KIND_SIGNAL,
            emits_from=("packages/analyzer/review.py",),
            justification="Security floor. The proposal reviewer's AST security layer "
            "failed to import — the gate is failing closed (force-flagging every "
            "proposal) but the operator must see the degraded gate immediately, on a "
            "fresh pod as much as a mature one. Delaying a security-gate-degraded "
            "finding is the worst failure here. find-or-create dedup on a single "
            "pod-scope signature caps it to one firing Signal.",
        ),
        _e(
            "analyze",
            (NONE,),
