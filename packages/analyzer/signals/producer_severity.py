...
PRODUCER_SEVERITY: dict[str, Severity] = {
    ...
    "host_health": "alert",
    # "review_gate" retired 2026-08-14 with review.py (#3641). The AST layer
    # it guarded now runs inside arbiter/security_screen.py, whose
    # fail-closed posture is enforced in-line by
    # arbiter.routing.is_autonomous_eligible (a proposal with no AST
    # coverage is never autonomy-eligible) rather than by paging.
    # A bot's on-disk provider credentials are unreadable AND the reader's own
    # inline ACL heal already failed (#3477). On the primary bot this is the
    # engine going dark — every background LLM feature reports "no provider
    # credentialed" — and it cannot self-clear, so it must page now. A clamp
    # the inline heal DID repair emits nothing at all (see oc_auth_store).
    "credential_access": "alert",

    # An engine-side LLM enrichment call (weekly_review synthesis, expansion
    # suggestions, community_intel summary, permission-intent inference)
    # failed against a provider the pod IS credentialed for. Sibling to
    # credential_access directly above: that one is the engine going dark
    # because the key is unreadable, this one is the engine going dark with
    # the key working fine. Same blast radius — every background LLM feature
    # silently stops enriching — so the same tier.
    #
    # Deliberately NOT warn. This fault is self-concealing by construction:
    # each job still ships plausible data-only output, so nothing downstream
    # looks wrong. That is exactly how the dead `openclaw llm complete` call
    # sites survived months of Sunday reports. A warn an operator can skim
    # past reproduces the original bug in slower motion; alert forces the
    # look. The no-key case emits nothing at all (see engine_llm) — this
    # producer only ever fires on genuine breakage.
    "engine_llm": "alert",
    ...
    # internal/spec-openclaw-json-derived-artifact-2026-05-24.md.
    ...
    "audit": "warn",
    # The stale-firing TTL backstop's leaking-producer meta-signal
    # (signals/stale_ttl.py): a producer whose signals only leave the
    # queue when the TTL ages them out is missing its sweep_resolve —
    # a real defect in that monitor's lifecycle wiring, not telemetry.
    "signals_retention": "warn",
    ...
    "pod_report": "warn",
    # Turn-failure autopsy (output-cap hits, mid-conversation provider
    # swaps, tool retry loops, unclassified incomplete turns). Each finding
    # is user-affecting (the user saw a failed or off-brand reply) and
    # operator-fixable, but never a page — the sweep's job is explaining
    # yesterday, not alarming tonight.
    ...
    # The board's tailnet listener accepted a connection that died before
    # its first byte — on macOS, the Application Firewall blocking the
    # daemon's own interpreter (2026-09-04 phone test). Deterministic, not a
    # flap, and the whole mobile surface is dark until an operator runs one
    # command; warn tier (maintenance-shaped repair, not a pod-wide outage).
    "board_listener": "warn",
    ...
    # page rather than buried in a log file. Background: internal/diagnosis-spend-alert-openclaw-flag-2026-05-21.md.
    "oc_cli": "warn",
    ...
    # Identities a bot's openclaw.json will accept that no Evolve roster source
    # knows about (M1-B3). Same governance exposure class as the drift sibling —
    # a sender who can spend the bot's tokens while carrying no role, limits, or
    # observation settings — but not a user-facing outage; warn. The sibling
    # "unreadable" finding (coherence check blind on a bot) rides the same warn
    # default. Detection-only. See packages/analyzer/roster_coherence_monitor.py.
    "roster_coherence": "warn",
    # A decommissioned bot's account still holding live credentials. Warn
    # rather than alert: nothing is on fire and nothing is degrading — the
    # account is inert — but the exposure is OUTWARD-facing (a channel token
    # stays valid at the platform regardless of this host) and it does not
    # decay on its own, so it must not sit in the info tier where it is hidden
    # by default. The sibling "unreadable" finding rides the same warn default.
    # Detection-only. See packages/analyzer/orphaned_bot_account_monitor.py.
    "orphaned_bot_account": "warn",
    ...
    # is on fire. Spec: internal/spec-agent-freelance-bypass-2026-06-05.md.
    ...
    # Model liveness probe — one real agent turn per routed model through the
    # gateway dispatch path. Fires only when a model the pod routes to fails
    # to dispatch (or the gateway silently serves the turn with a different
    # model): every rung routing to it is dead and fallback chains walk past
    # it to costlier models — the #3489 weeks-long-silent class. Alert tier
    # (sibling to evo_path_probe: a proven pod-wide path break).
    "model_liveness_probe": "alert",
    ...
    "repo_puller": "warn",
    # OC-ingested bootstrap files at/past the bot's injection caps
    # (bootstrapMaxChars / bootstrapTotalMaxChars). Default warn covers the
    # near-limit band (≥85% of a cap — about to start losing content); the
    # producer escalates per-emit to alert at ≥100%, where OC is actively
    # truncating and everything past the cap is silently absent from every
    # turn's context (the 2026-08-01 AGENTS.md incident: 128k chars against
    # a 40k cap silently dropped every anti-confabulation rule). The sibling
    # "unreadable" finding (size check blind on a bot) rides the same warn
    # default. See packages/analyzer/bootstrap_size_monitor.py.
    "bootstrap_size": "warn",
    # Post-model-swap behavior divergence (design-model-swap-behavior-guard-
    # 2026-08-19). Warn, not alert: the bot is running and answering — what
    # changed is HOW it behaves on the rung that was swapped (the 2026-08-14
    # incident: a new model stopped honoring OC's NO_REPLY sentinel and posted
    # its should-I-reply deliberation into four Slack channels). That is a
    # queue-and-fix with a one-command undo (`evolve-admin models rollback`),
    # not a page. The producer never escalates per-emit — a divergence is a
    # divergence; the operator decides whether the new behavior is wanted.
    # See packages/analyzer/model_swap_watch.py.
    "model_swap_watch": "warn",
    ...
    # Absorbed bot exec-failure shapes (exec-failure hygiene A2, design-
    # exec-failure-hygiene-2026-08-31). Advisory by design: A2 is quiet
    # observation of recurring failure shapes (digest visibility via the
    # meta.unclassified catalog fallback); escalation with plain-language
    # remediation is A3 and gets its own thresholds once the ledger shows
    # real distributions. The bot itself already sees and adapts to each
    # failure — the model-facing result is untouched.
    "bot_exec_failures": "info",
    # Out-of-profile tool calls (context-economy CE-2b, design-pa-context-
    # economy-2026-08-31 §3). Advisory: a refusal means a background session
    # reached for a tool its tool profile does not carry. Nothing is broken —
    # the call was refused with a message naming the profile, and the row is
    # the evidence the profile table asks for before it is widened. Escalating
    # this would page an operator for a model's guess.
    "tool_profile": "info",
    ...
    # (internal/spec-value-baseline-2026-06-10.md §6.4 / §11.2).
    "value_baseline": "info",
}
...
PRODUCER_FLAVOR: dict[str, Flavor] = {
    ...
    # Recurring absorbed exec-failure shapes (exec-failure hygiene A2).
    # Quiet observation of what the channel-hygiene absorber caught; the
    # actionable layer is A3's escalation, not these roll-ups. Advisory.
    "bot_exec_failures": "activity",
    # Out-of-profile tool refusals (CE-2b). Observation that feeds a table
    # edit when the evidence accumulates — not a queued repair. Advisory.
    "tool_profile": "activity",
}
