    ...
    *,
    ...
    if reopen_window_seconds > 0:
        ...
        if recent is not None:
            ...
            # This path bypasses apply_transition, so clear the
            # resolution marker here too — a re-opened Signal is firing
            # again and must not look TTL-resolved to downstream readers.
            recent.resolution_kind = None
...
def apply_transition(
    ...
    *detected* (``IllegalTransitionError`` from the fresh state) rather
    ...
    if resolution_kind is not None and to_state != "resolved":
        raise ValueError(
            "apply_transition: resolution_kind is only meaningful for "
            f"to_state='resolved' (got {to_state!r})"
        )
    ...
    with _flock(lock_path(shared_dir)):
        ...
        # Set on the way into resolved; cleared on every transition out
        # of it (re-open, snooze, dismiss) so a Signal that comes back
        # never carries a stale marker from a previous resolution.
        signal.resolution_kind = (
            resolution_kind if to_state == "resolved" else None
        )
