...
Retention rules:
  ...
  - firing signals: kept until terminal, EXCEPT the stale-firing TTL
    backstop (signals.stale_ttl, spec §6's "rely on TTL or manual
    resolution"): a firing signal not re-observed for 14 days (default)
    is auto-resolved with an explicit backstop reason, and a
    per-producer leak meta-signal reports the producer whose signals
    aged out. Snoozed signals are never TTL'd (a snooze is an operator
    statement with its own wake timer).
  ...
  - alerts/{dispatcher,dispatcher-queued,dispatcher-suppressed,delivery-failures}/
    <YYYY-MM-DD>.jsonl:
...
# Alert dispatcher logs (dispatcher/, dispatcher-queued/,
# dispatcher-suppressed/, delivery-failures/) — noisier than signals/log (one record per dispatch
...
# {bot}/exec-failures/exec-failures-<YYYY-MM-DD>.jsonl — the plugin
# ExecFailureAbsorber's absorbed-trailer ledger (design-exec-failure-
# hygiene-2026-08-31 A1). The only reader (exec_failure_monitor, A2) scans
# a 24h window; 30 days keeps enough history for A3's threshold derivation
# ("set after the ledger shows real distributions") without letting a bot
# stuck in a failure loop grow the stream forever.
DEFAULT_EXEC_FAILURES_RETENTION_DAYS = 30
...
_ALERTS_LOG_STREAMS: tuple[str, ...] = (
    "dispatcher",
    # Deferred-to-digest sends (dispatcher._log_filename_for). Split out of
    # ``dispatcher`` on 2026-08-25 so the "actually sent" feed means what it
    # says; same 30-day window — it is the same append-per-decision shape and
    # was in fact the bulk of the ``dispatcher`` volume before the split.
    "dispatcher-queued",
...
class PruneResult:
    ...
    exec_failures_pruned: int = 0
    exec_failures_kept: int = 0
    stale_firing_resolved: int = 0
    stale_firing_exempt: int = 0
...
def prune_retention(
    ...
    *,
    ...
    exec_failures_days: int = DEFAULT_EXEC_FAILURES_RETENTION_DAYS,
    stale_firing_ttl_days: int = stale_ttl.DEFAULT_STALE_FIRING_TTL_DAYS,
    ...
    0. **Stale-firing TTL backstop** (``signals.stale_ttl``) — any firing
       Signal not re-observed for ``stale_firing_ttl_days`` (default 14;
       0 disables) is resolved through the state machine with an explicit
       backstop reason, and a per-producer ``producer_signal_leak``
       meta-signal names the producer whose signals aged out. Snoozed
       signals and TTL-exempt types (``stale_ttl.TTL_EXEMPT_TYPES``) are
       never touched. Runs first so freshly-aged signals enter the
       archive with a current mtime and ride the normal 90-day window.
    ...
    5. ``alerts/<stream>/<YYYY-MM-DD>.jsonl`` — date-partitioned
       dispatcher log streams (``dispatcher``, ``dispatcher-queued``,
       ``dispatcher-suppressed``, ``delivery-failures``) older than
       ``alerts_days``. The dispatcher-suppressed stream is the one that
       fills the disk (~36k entries/day under a stuck producer); the
       others grow slower but get the same sweep for symmetry.
    ...
    Active signal/proposal states are never *pruned* (file deletion);
    the TTL backstop above resolves stale firing signals through the
    state machine, which is a transition, not a deletion.
    """
    ...
    exec_failures_cutoff_date: date = (
        cutoff_now - timedelta(days=exec_failures_days)
    ).date()

    # ── 0. stale-firing TTL backstop (signals.stale_ttl) ─────────────────────
    # Before the archive prune so freshly-aged signals land in archived/
    # with a current mtime (they then age out on the normal 90-day window).
    stale_result = stale_ttl.sweep_stale_firing(
        shared_dir,
        ttl_days=stale_firing_ttl_days,
        now=cutoff_now,
    )
    ...
    # ── 13. {bot}/exec-failures/exec-failures-<YYYY-MM-DD>.jsonl ─────────────
    # Absorbed-trailer ledger (plugin ExecFailureAbsorber); date-named with a
    # stream prefix, age on the filename's date suffix.
    exec_failures_pruned = 0
    exec_failures_kept = 0
    shared_root = Path(shared_dir)
    if shared_root.is_dir():
        for bot_dir in shared_root.iterdir():
            ledger_dir = bot_dir / "exec-failures"
            if not ledger_dir.is_dir():
                continue
            for path in ledger_dir.glob("exec-failures-*.jsonl"):
                try:
                    file_date = date.fromisoformat(
                        path.stem.removeprefix("exec-failures-")
                    )
                except ValueError:
                    # Non-conforming filename — keep, don't risk deleting.
                    exec_failures_kept += 1
                    continue
                if file_date < exec_failures_cutoff_date:
                    try:
                        path.unlink()
                        exec_failures_pruned += 1
                    except OSError:
                        exec_failures_kept += 1
                else:
                    exec_failures_kept += 1
    ...
    return PruneResult(
        ...
        exec_failures_pruned=exec_failures_pruned,
        exec_failures_kept=exec_failures_kept,
        stale_firing_resolved=stale_result.stale_resolved,
        stale_firing_exempt=stale_result.exempt_skipped,
        leak_signals_emitted=stale_result.leak_signals_emitted,
        leak_signals_resolved=stale_result.leak_signals_resolved,
    )
...
def _main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--exec-failures-days",
        type=int,
        default=DEFAULT_EXEC_FAILURES_RETENTION_DAYS,
        help="Days to retain {bot}/exec-failures/ absorbed-trailer JSONL (default: 30)",
    )
    parser.add_argument(
        "--stale-firing-ttl-days",
        type=int,
        default=stale_ttl.DEFAULT_STALE_FIRING_TTL_DAYS,
        help=(
            "Auto-resolve firing signals not re-observed for this many days "
            "(default: 14; 0 disables the TTL backstop)"
        ),
    )
    ...
    result = prune_retention(
        ...
        exec_failures_days=args.exec_failures_days,
        stale_firing_ttl_days=args.stale_firing_ttl_days,
    )
    audit_pruned = (
    ...
    print(
        ...
        f" · exec_failures pruned={result.exec_failures_pruned} kept={result.exec_failures_kept}"
        f" · stale-firing resolved={result.stale_firing_resolved}"
        f" exempt={result.stale_firing_exempt}"
        f" leaks emitted={result.leak_signals_emitted}"
        f" resolved={result.leak_signals_resolved}"
    )
    return 0
