...
as an **injected** mapping, and its only production caller (the admin route
``evolve_admin.web.model_discovery_adopt.auto_upgrade_report``) passes
nothing. So ``visible_days`` is ``None`` on every live pod,
``HOLD_NOT_VISIBLE_LONG_ENOUGH`` has never once fired, and operator decision
1's guarantee — "the veto window comes from the item's own age, not the
calendar" — has no clock behind it. P3b cannot be correct until it does.
