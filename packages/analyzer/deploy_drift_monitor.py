...
def detect_deploy_drift(
    ...
    if ahead_only:
        body_lines.extend(
            [
                "This is the inverse of deploy drift and is usually benign: "
    ...
    if ahead_only:
        what_it_means = (
            ...
            "Evolve than the admin server's current code — the inverse of "
            "deploy drift. This is usually benign: the admin server pulls "
    ...
    # (drift, advisory), 3+ = mag 2 (pod-wide degradation of "running latest
    ...
    if not dry_run:
        try:
            resolved = signals_store.sweep_resolve(
                ...
                reason="auto-resolve: deploy drift cleared on next run",
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="deploy_drift_monitor — Signal producer for evolve-deploy drift",
    )
