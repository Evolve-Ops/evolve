...
DEFAULT_MODEL_CATALOG: dict[str, Any] = {
    "rungs": [
        ...
        },
        {
            ...
            # Ships with the same medium-cost, provider-diverse cluster standard
            # uses so judge routing is unchanged (the diversity resolver still
            # prefers a non-standard provider), but judge now has an INDEPENDENT
            # model list + order: the pod-default / per-bot editors can reorder or
            # curate judge without it sharing (and reverting to) standard's rung.
            "id": "judge-class",
            "costClass": "medium",
            "models": [
                "anthropic/claude-sonnet-4-6",
...
def canonicalize_catalog_rung_ids(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    ``_CANONICAL_RUNG_COST_CLASS``. ``judge`` owns its own canonical rung
    (``judge-class``, spec §Addendum 16): a source ``judge`` rung — whatever slug
    it arrives under (a synthetic ``judge-default``, or a legacy ``sonnet-class``
    from a pre-decouple pod) — is renamed to ``judge-class`` with its own
    cluster, so judge's independent order survives the write instead of folding
    into standard's rung. Roles are rewritten to point at the canonical id
    (judge keeps its ``{rung, provider}`` shape).
    ...
    # judge → its OWN canonical rung (judge-class), keeping the diversity
    # constraint. Only when the source defines judge; the source rung's cluster
    # (whatever slug it arrived under) becomes judge-class verbatim, so judge's
    # independent order is preserved rather than folded into standard's rung.
    ...
    # stays a valid cost rank. The ladder loop already emits in cost order, but
    # judge-class (medium, appended after the ladder) must sort back next to
    # sonnet-class rather than trail fable-class (premium). Stable sort preserves
    # the ladder's relative order for same-cost rungs.
...
def _normalize_legacy_layer(layer: dict | None) -> dict | None:
    ...
    for tier_key, cost_class in cost_order:
        ...
        synthesized = True
    # tier0 (judge) synthesizes its OWN judge-class rung (spec §Addendum 16) with
    # the diversity constraint, so a legacy judge list keeps its independent
    # order instead of folding into standard's sonnet-class rung.
    ...
    if tier0_models:
        ...
        new_roles["judge"] = {"rung": "judge-class", "provider": "not-standard"}
        synthesized = True
    if not synthesized:
        return layer
