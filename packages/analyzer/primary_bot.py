...
Both the admin and analyzer packages import from here. Keep it dependency-free
(stdlib only) AT IMPORT TIME so it loads cleanly from either side; the analyzer
siblings it needs (``evolve_config`` for the platform-keyed network.json path,
``platform_profile`` for the home-construction fallback, ``oc_auth_store`` for
the credential-store ladder) are imported lazily inside the functions that use
them, and each of those imports degrades rather than raising.
"""
...
def primary_bot_home(network: dict[str, Any]) -> Path | None:
    ...
    Resolution is ``pwd`` first (the account's REAL home, whatever the
    platform), then the platform-keyed construction fallback for a pod where
    the account does not exist yet. The fallback goes through
    ``platform_profile`` — a ``/Users/{user}`` literal here resolved to a
    non-existent path on every Linux pod.
    """
    ...
    try:
        ...
        logger.debug(
            "primary_bot: pwd lookup failed for account %r (%s); constructing the "
            "home path from the platform profile",
            user,
            exc,
        )
    try:
        from platform_profile import get_profile  # analyzer sibling, stdlib-only
        return Path(get_profile().user_home_root) / user
    except Exception as exc:
        logger.error(
            "primary_bot: cannot resolve a home dir for account %r — pwd failed and "
            "platform_profile is unavailable (%s); primary-bot credential reads "
            "will degrade to env-var-only",
            user,
            exc,
        )
        return None
...
def _load_network_default() -> dict[str, Any]:
    """Load network.json from the canonical location for THIS platform.

    Used as a fallback when callers don't have a network dict handy — which
    is the common case for engine LLM work (``infra_llm.resolve_infra_llm``
    with no ``network=``).

    Resolution goes through ``evolve_config`` (``resolve_network_path`` +
    ``load_config``), which is platform-keyed: ``/Users/Shared/evolve`` on
    macOS, ``/var/lib/evolve`` on Linux. This module previously hardcoded the
    macOS path, so on a Linux pod EVERY primary-bot reader resolved against an
    empty network dict → no primary bot → no home → no keys → every engine
    feature reported "no provider credentialed" while the pod's credentials sat
    readable on disk (#3475, confirmed live on the reference VPS pod).

    Falls back to the ``EVOLVE_NETWORK`` / ``EVOLVE_NETWORK_JSON`` env
    overrides when ``evolve_config`` is unimportable (a context carrying only
    this module). Returns ``{}`` on total failure — the caller's auth lookup
    then degrades to env-var-only.
    """
    try:
        from evolve_config import load_config, resolve_network_path  # type: ignore

        data = load_config(str(resolve_network_path()))
        if isinstance(data, dict) and data:
            return data
    except Exception as exc:
        logger.debug(
            "primary_bot: evolve_config network resolution unavailable (%s); "
            "falling back to the EVOLVE_NETWORK env overrides",
            exc,
        )
    for candidate in (
        os.environ.get("EVOLVE_NETWORK", ""),
...
# ── Auth-store sources: ONE reader, shared with the admin side ────────────────
#
...
# ``auth-profiles.json.sqlite-import.<epoch_ms>.bak`` — and, once every profile
# verifies into SQLite, REMOVING the JSON outright.
#
# The source ladder for that lives in exactly one place: ``oc_auth_store``
# (which ``evolve_admin.oc_store`` also delegates to). Before #3475 this module
# carried its own narrower ladder — JSON candidates, then a hardcoded ``main``-
# agent sqlite path, and no ``.bak`` rung, no non-``main`` agent discovery, no
# privileged-read guard, no root ``sqlite3 -readonly`` belt. Two readers, one
# blind. The engine path (judge, summarizer, classifier, spec extractor, help
# bot, proposal synthesis, …) now walks the SAME ladder the scanner does.


def primary_bot_agent_sqlite_path(network: dict[str, Any]) -> Path | None:
    """Path to the primary bot's conventional ``main``-agent SQLite auth store.

    Convenience for callers that want the canonical path itself (diagnostics,
    the ownership/permission passes). Credential READS go through
    :func:`_iter_primary_bot_auth_payloads`, which discovers every agent id
    rather than assuming ``main``. Returns ``None`` when the primary bot / its
...
def read_agent_auth_store_json(db_path: Path) -> dict[str, Any] | None:
    ...
    Delegates to ``oc_auth_store.read_sqlite_store_json``, which walks the one
    sqlite read ladder: a WAL-safe read-only open (``mode=ro`` + ``PRAGMA
    query_only``) first so a row the running gateway committed but has not
    checkpointed is still seen, then the root ``sudo sqlite3 -readonly`` belt for
    a pre-ACL bot, then ``immutable=1`` as the last-ditch main-file read (it
    ignores the WAL, so it can only ever be the last rung).
    ...
    """
    try:
        from oc_auth_store import read_sqlite_store_json  # type: ignore
    except Exception:
        return None
    return read_sqlite_store_json(db_path)
...
def _iter_primary_bot_auth_payloads(network: dict[str, Any]) -> Iterator[Any]:
    ...
    The shared ladder first (``oc_auth_store.iter_auth_store_payloads``: the
    per-agent SQLite store → legacy ``auth-profiles.json`` → newest
    ``.sqlite-import.*.bak``, across every discovered agent id), then the two
    ANCIENT non-per-agent JSON layouts this module has always tolerated
    (``~/.openclaw/agents/main/auth-profiles.json`` and
    ``~/.openclaw/auth-profiles.json``), which no post-2026.6 pod writes.

    Each yielded payload is a parsed object ready for
    ...
    fewer payloads, never raises. Lazy throughout: a caller satisfied by the
    first payload never opens the later sources.

    Precedence note (#3475): SQLite now precedes the legacy JSON, matching the
    admin-side reader. Post-migration the JSON is either absent or a partial
    write-back holding exactly the profiles OpenClaw could NOT verify into
    SQLite — preferring it would prefer the unverified credential. Callers walk
    every payload, so a SQLite store that carries nothing they need still falls
    through to the JSON.
    """
    home = primary_bot_home(network)
    if home is not None:
        # Import hoisted out of the iteration so ONLY a genuine missing-module
        # failure takes the degraded branch (an ImportError raised while
        # iterating would otherwise be misreported as "reader unavailable").
        _iter_shared = None
        try:
            from oc_auth_store import iter_auth_store_payloads as _iter_shared  # type: ignore
        except ImportError as exc:
            # The shared reader is an analyzer sibling and should always be
            # importable; if it is not, the ancient-layout candidates below are
            # the only remaining source — degraded, but never a hard failure.
            # Logged at ERROR: silently reading fewer credential sources than
            # the pod has is exactly the failure mode #3475 is about.
            logger.error(
                "primary_bot: oc_auth_store unavailable (%s) — the SQLite store and "
                "the transitional .bak snapshot cannot be read; primary-bot "
                "credential resolution is degraded to the legacy JSON layouts",
                exc,
            )
        if _iter_shared is not None:
            # ``network`` is passed (not just ``home``) so the shared ladder's
            # inline ACL heal (#3477) resolves the bot's real OS account for
            # ``heal_evolve_access`` and writes its Signal into THIS pod's
            # ``sharedDir`` rather than the platform default.
            for raw in _iter_shared(
                primary_bot_id(network), home=home, network=network
            ):
                try:
                    yield json.loads(raw)
                except Exception:
                    continue
    # Ancient layouts the shared per-agent ladder does not cover. Anything under
    # an ``…/agent/`` dir IS the per-agent layout the ladder already walked —
    # skip it rather than re-read (matched on the parent dir, not a list index,
    # so reordering the candidate list can't silently double-read or skip).
...
def read_primary_bot_keys_by_provider(
    ...
    Walks the shared auth-store ladder (:func:`_iter_primary_bot_auth_payloads`
    — SQLite store, legacy ``auth-profiles.json``, transitional ``.bak``),
    returning the first source that yields keys. Handles canonical, flat, and
    ultra-flat shapes.
    Prefers ``api_key`` profiles over ``token`` profiles (the latter are
...
def _profile_secret(pdata: dict[str, Any]) -> str:
    """Return the secret string carried by one auth profile dict, or ``""``.

    Field precedence mirrors :func:`extract_anthropic_key`: ``api_key`` /
    ``key`` first (direct API credentials), then ``token`` / ``value``
    (OAuth or session material — some engine paths can still use these).
    """
...
def _extract_llm_keys(raw: Any) -> dict[str, str]:
    """Extract ``{provider: secret}`` from one auth-profiles payload,
    honouring the store's ``lastGood`` pointers.

    Generalization of :func:`extract_anthropic_key` keyed on each profile's
    explicit ``"provider"`` field — NOT on id substrings — per the
    provider-agnostic principle (docs/principle-llm-provider-agnostic.md).

    Precedence per provider:
      1. The profile ``lastGood[provider]`` points at (the gateway's
         known-working credential for that provider).
      2. Any ``api_key``-typed profile for the provider.
      3. Any ``token``-typed profile (OAuth/session material) — included so
         callers can see the provider is configured, though direct API
         calls may not accept it.

    Canonical / flat / ultra-flat payload shapes are all tolerated (same
    tolerance set as :func:`_extract_keys_by_provider`). When a profile has
    no ``provider`` field, the profile id's prefix before ``":"`` is used
    (canonical ids look like ``"openai:api_key"``).
    """
    keys: dict[str, str] = {}
    if not isinstance(raw, dict):
        return keys
    ...
    if not isinstance(profiles, dict):
        return keys
    # 1. lastGood-pointed profiles win for their provider.
    last_good = raw.get("lastGood")
    if not isinstance(last_good, dict):
        last_good = {}
    for prov, pid in last_good.items():
        pdata = profiles.get(pid) if isinstance(pid, str) else None
        if isinstance(pdata, dict):
            secret = _profile_secret(pdata)
            if isinstance(prov, str) and prov and secret:
                keys[prov.lower()] = secret
    # 2./3. Fill remaining providers: api_key profiles preferred over token.
    for prefer_api_key in (True, False):
        for pid, pdata in profiles.items():
            if isinstance(pdata, str):
                prov = str(pid).split(":", 1)[0].lower()
                secret = pdata.strip()
            elif isinstance(pdata, dict):
                ptype = (pdata.get("type") or "api_key").lower()
                if prefer_api_key and ptype == "token":
                    continue
                prov = str(pdata.get("provider") or str(pid).split(":", 1)[0]).lower()
                secret = _profile_secret(pdata)
            else:
                continue
            if prov and secret and prov not in keys:
                keys[prov] = secret
    return keys


# Env-var override names per provider (operator override, also covers tests).
# Adapter data (three-homes rule bucket b) — the names each provider's own
# tooling conventions use for its key.
# provider-literal-allow-begin: provider env-var name table (adapter data)
_PROVIDER_KEY_ENV_VARS: tuple[tuple[str, tuple[str, ...]], ...] = (
...
def read_primary_bot_llm_keys(network: dict[str, Any] | None = None) -> dict[str, str]:
    """Return ``{provider: secret}`` for every LLM provider credentialed on
    the primary bot — the provider-agnostic generalization of
    :func:`read_primary_bot_anthropic_key` (which stays for its existing
    callers until they migrate to ``infra_llm``).

    Resolution order:
      1. The primary bot's auth store via the shared ladder
         (:func:`_iter_primary_bot_auth_payloads` — the per-agent
         ``openclaw-agent.sqlite``, then the legacy ``auth-profiles.json``,
         then the transitional ``.sqlite-import.*.bak``) — first source that
         yields any keys wins (same walk as the anthropic reader).
         ``lastGood``-pointed profiles are preferred per provider;
         ``api_key`` profiles beat ``token`` profiles.
      2. Per-provider env-var overrides (``ANTHROPIC_API_KEY``,
         ``OPENAI_API_KEY``, ``GEMINI_API_KEY``/``GOOGLE_API_KEY``,
         ``XAI_API_KEY``) — each overrides/extends its own provider's
         entry, never another provider's.

    Returns ``{}`` when nothing is credentialed — callers (``infra_llm``'s
    resolver) treat that as "no infra LLM available" and keep their degrade
    paths.
    """
    if network is None:
        network = _load_network_default()
    keys: dict[str, str] = {}
    for raw in _iter_primary_bot_auth_payloads(network):
        keys = _extract_llm_keys(raw)
        if keys:
            break
    for prov, env_names in _PROVIDER_KEY_ENV_VARS:
...
def provider_key_env_assignments(network: dict[str, Any] | None = None) -> list[str]:
    """``NAME=value`` env assignments for every credentialed provider key.

    For handing engine LLM credentials across a sudo boundary (``sudo -u
    <bot> SETENV: …``): a subprocess running as a bot user cannot read the
    primary bot's auth store, but ``infra_llm`` resolution honors the
    per-provider env overrides, so injecting these assignments makes
    ``resolve_infra_llm`` work inside the subprocess. Uses each provider's
    canonical env var (the first name in :data:`_PROVIDER_KEY_ENV_VARS`).
    """
    keys = read_primary_bot_llm_keys(network)
    out: list[str] = []
    for prov, env_names in _PROVIDER_KEY_ENV_VARS:
        secret = keys.get(prov, "")
        if secret:
            out.append(f"{env_names[0]}={secret}")
    return out
...
def read_primary_bot_anthropic_key(network: dict[str, Any] | None = None) -> str:
    ...
    .. deprecated:: #3466
        Use :func:`read_primary_bot_llm_keys` (provider-agnostic) or
        ``infra_llm.resolve_infra_llm``. #3466 PR-3 (#3474) migrated the
        production call sites; as of #3475 the ONLY remaining caller is the
        Linux e2e assertion in
        ``packages/admin/tests/e2e_linux/test_ubuntu_e2e.py`` (round9 #2 —
        "the key resolver targets the PRIMARY bot's home"). Retiring this
        function therefore means porting that assertion to
        :func:`read_primary_bot_llm_keys` first; deliberately not bundled
        into #3475 (that suite is Linux-only and unverifiable from a macOS
        dev box).

    Resolution order:
      1. ``ANTHROPIC_API_KEY`` env var (operator override, also covers tests).
      2. The primary bot's auth store via the shared ladder
         (:func:`_iter_primary_bot_auth_payloads` — per-agent
         ``openclaw-agent.sqlite`` → legacy ``auth-profiles.json`` →
         transitional ``.sqlite-import.*.bak``).
...
# The AI-Optimization page and ``evolve-admin models show`` both render in
# this order. The former off-ladder ``judge`` role was collapsed into the
# cross-vendor derivation (:func:`resolve_cross_vendor`) —
# internal/design-judge-role-collapse-2026-08-21.md §5.4.
ROLE_ORDER: tuple[str, ...] = ("fast", "standard", "power", "max")
...
# must NOT inflate the step count. This is the role order, not a model literal.
...
def ladder_rank_view(catalog: dict[str, Any]) -> dict[str, dict[str, int]]:
    ...
    ladder role that resolves to a rung.
    """
...
def resolve_roles_with_provenance(
    ...
    """Resolve all four roles through the defaults ← pod ← bot merge, tagging
    each with the **winning layer** that supplied its rung.

    Spec: internal/spec-model-rungs-and-roles-2026-06-09.md §Addendum 2.3 — the AI
    Optimization page renders ALL FOUR roles unconditionally, each labeled with
    ...
    over default. A role always resolves (defaults map all four), so there is
    no "unconfigured" row.
    """
    pod_models = (network or {}).get("models")
    ...
    # only rungs a ladder role actually points at count.
    ladder_view = ladder_rank_view(cat_bot)
    # One canonical ladder rung count for every row (the meter draws this many
    # steps). Any ladder entry carries the same count; fall back to 0.
    ...
    for role in ROLE_ORDER:
        ...
        # Rank (1-based, cost-ordered) for the meter; None for any ladder
        # role whose rung isn't in the catalog.
...
def read_bot_tiers_doc(network: dict[str, Any], bot_id: str) -> dict:
    ...
    Spec: internal/spec-model-rungs-and-roles-2026-06-09.md §Addendum 5 — the
...
def tiers_doc_is_custom(doc: dict[str, Any]) -> bool:
    """Pure Custom-flip predicate over a RAW per-bot tiers doc.

    Spec §Addendum 5 "Strict all-or-nothing": Custom ⇔ the doc carries a
    non-empty ``rungs`` array. The single home for the rule — callers that
    already hold a doc (pod_baseline.census) share it with
    :func:`bot_has_custom_tiers` instead of copying the expression.
    """
...
def bot_has_custom_tiers(network: dict[str, Any], bot_id: str) -> bool:
    ...
    Decided from the RAW per-bot doc, never the merged view — see
    :func:`read_bot_tiers_doc`. Rule lives in :func:`tiers_doc_is_custom`.
    """
    return tiers_doc_is_custom(read_bot_tiers_doc(network, bot_id))
...
#   - ``validate_pod_default_catalog`` — shape + resolvability checks run
#     BEFORE writing a pod override (every ladder role must resolve to a
#     non-empty rung cluster).
...
def validate_pod_default_catalog(catalog: dict[str, Any]) -> str | None:
    ...
    Checks (spec §Addendum 5):
      1. ``rungs`` is a non-empty list of ``{id, models[]}`` clusters.
      2. Every ladder role (fast/standard/power/max) resolves to a non-empty
         rung cluster.

    The former check 3 — the judge provider-diversity invariant — was removed
    with the judge role itself (design-judge-role-collapse-2026-08-21 §5.4):
    cross-vendor checking is now a call-site derivation over the standard
    chain (:func:`resolve_cross_vendor`), not a config invariant.
    """
...
# Pinned dated-snapshot suffix ("-20251001", "-2025-10-01", "@20251001").
# Deliberately DATED-ONLY — narrower than model_discovery._DATE_SUFFIX_RE:
# stripping "-latest"/"-preview" here would drop the alias the operator most
# wants to ride (e.g. a "-latest" rolling id next to its pinned base).
# primary_bot cannot import model_discovery (it imports us), so this stays local.
_DATED_SNAPSHOT_SUFFIX_RE = re.compile(r"[-@](?:\d{8}|\d{4}-\d{2}-\d{2})$")


def _dated_snapshot_base(model_id: str) -> str:
    """``anthropic/claude-haiku-4-5-20251001`` → ``anthropic/claude-haiku-4-5``.

    Returns the id unchanged when it carries no dated-snapshot suffix. Iterates
    for stacked suffixes; the provider prefix is preserved.
    """
    prev = None
    cur = model_id
    while cur != prev:
        prev = cur
        cur = _DATED_SNAPSHOT_SUFFIX_RE.sub("", cur)
    return cur


def _drop_shadowed_snapshots(models: list[str]) -> list[str]:
    """Drop a pinned dated snapshot when its rolling base id is ALSO in the list.

    A rung carrying both ``claude-haiku-4-5-20251001`` and ``claude-haiku-4-5``
    (adopt-path residue from before discovery excluded dated aliases) offers the
    same model twice; the rolling base is the one to ride. A snapshot with no
    base sibling is kept — pinning is a legitimate operator choice.
    """
    present = {m.lower() for m in models if isinstance(m, str)}
    out: list[str] = []
    for m in models:
        if isinstance(m, str):
            base = _dated_snapshot_base(m)
            if base.lower() != m.lower() and base.lower() in present:
                continue
        out.append(m)
    return out
...
def compute_easy_setup_catalog(
    ...
    ``roles`` / ``roleCaps`` are carried through unchanged; the reorder only
    touches each rung's ``models`` list. The result is a full
    ...
    for rung in (base.get("rungs") or []):
        ...
        new_rung["models"] = _reorder_models_by_preference(
            _drop_shadowed_snapshots(models), provider_order or [], credentialed_providers,
        )
...
# introduced by spec-model-rungs-and-roles-2026-06-09. Mirrors TIER_TO_ROLE /
# TIER_TO_RUNG in migrate_model_roles.py — keep the two in sync. (ModelRouter.ts
# no longer carries the mapping: its runtime legacy fallback was removed
# 2026-08-15; a legacy-shaped config fails loudly there instead. The Python
# side keeps translating because ADMIN reads must still understand un-migrated
# files to display/diagnose them and to drive the migrator.)
#
...
TIER_TO_ROLE: dict[str, str] = {
    ...
    "tier2": "standard",
    "tier1": "power",
    # tier0 (the legacy judge slot) has no role since the judge-role collapse
    # (design-judge-role-collapse-2026-08-21 §5.4). Legacy ``tiers.tier0``
    # content stays PARSEABLE: ``_normalize_legacy_layer`` folds it into an
    # ordinary extra rung (no role points at it), never an error.
}
...
DEFAULT_MODEL_CATALOG: dict[str, Any] = {
    ...
    "roles": {
        ...
        "power": "opus-class",
        "max": "fable-class",
        # The former ``judge`` role (its own judge-class rung + structured
        # {rung, provider} shape) was collapsed 2026-08 — cross-vendor checking
        # is now derived from the standard chain (resolve_cross_vendor);
        # design-judge-role-collapse-2026-08-21 §5.4.
    },
    "roleCaps": {
...
def _resolve_role_to_rung(data: dict, role: str) -> str | None:
    ...
    provider}`` shape died with the judge role — design-judge-role-collapse
    §5.4; a stale structured entry in an un-migrated file is simply not a
    slug, so it falls through to the default map like any junk value).
    Falls back to the canonical default rung for the role when the file
    carries rungs but no explicit ``roles`` entry (a partially-migrated
    file), so a read still finds the right cluster.
    """
...
def canonicalize_catalog_rung_ids(catalog: dict[str, Any]) -> dict[str, Any]:
    ...
    ``_CANONICAL_RUNG_COST_CLASS``. Roles are rewritten to point at the
    canonical id.
    ...
    # minting an empty rung here would brick the merge for that role.
    ...
    # Keep the rungs[] array cost-ascending (cheapest first) so array position
    # stays a valid cost rank. The ladder loop already emits in cost order;
    # the stable sort preserves relative order for same-cost rungs and guards
    # any future non-ladder emission order.
    def _cost_rank(rung: dict) -> int:
        c = rung.get("costClass")
        return _COST_CLASS_ORDER.index(c) if c in _COST_CLASS_ORDER else len(_COST_CLASS_ORDER)
    new_rungs.sort(key=_cost_rank)
...
def easy_setup_catalog_for(
    ...
    The result runs through ``canonicalize_and_validate_pod_catalog`` (same
    enforcement as the manual pod write).
    """
...
# fast terminal. This is the AVAILABILITY chain — the cap-exhaustion path
...
# cheaper credentialed rung. Mirrors ``_degradeRole`` in ModelRouter.ts (the
# cap-vs-availability split is documented there).
...
def resolve_role_with_availability(
    ...
    Mirrors ModelRouter.ts ``resolveRoleAvailability``.
    """
    seen: set[str] = set()
    cur: str | None = role
...
def resolve_cross_vendor(
    against_role: str = "standard",
    *,
    catalog: dict[str, Any],
    credentialed: set[str] | None,
) -> str | None:
    """Derive a cross-vendor judge model against ``against_role``'s provider.

    The judge-role collapse seam (internal/design-judge-role-collapse-2026-08-21.md
    §5.2): *"the first credentialed model whose provider differs from the one
    that produced the work being judged — or nothing, if the operator only has
    one provider."* Mirrors ``ModelRouter.resolveCrossVendor`` — keep in sync.

    - ``against_role`` resolves through the ordinary availability ladder
      (:func:`resolve_role_with_availability`), so the comparison provider is
      the one that would actually produce the judged work — degradation-aware.
    - The walk covers the rung the resolution actually LANDED in (the resolved
      role's rung, not the requested role's, when they differ): that rung's
      ``models[]`` order is the operator-curated fallback chain, already sorted
      into ``provider_order`` rank at write time by the easy-setup wizard
      (:func:`_reorder_models_by_preference`) — walking it head-to-tail IS the
      provider-preference walk.
    - ``credentialed`` is the raw credentialed-provider set (intersected here
      against the catalog's LLM-capable set); ``None`` means credential state
      unknown and fails open (presentation readers only — the routing path
      passes a concrete set).

    Returns ``None`` when no credentialed different-provider model exists in
    the chain. ``None`` is meaningful, not a failure: a single-provider pod
    gets NO cross-vendor judge, and each call site decides what that means
    (design §5.3 — SessionStruggleJudge falls back to ``classifierModel``
    flagged ``cross_vendor: false``; a future auto-approval scorer refuses).
    """
    avail = (
        None if credentialed is None
        else available_providers_for_resolution(catalog, credentialed)
    )
    res = resolve_role_with_availability(catalog, against_role, avail)
    against_model = res.get("model")
    if not against_model:
        return None  # nothing will produce the judged work — nothing to diff
    against_provider = provider_of(against_model)
    resolved_role = res.get("resolved_role") or against_role
    rung_id = _resolve_role_to_rung(catalog, resolved_role)
    models = _rung_models(catalog, rung_id) if rung_id else []
    for m in models:
        mp = provider_of(m)
        if not mp or mp == against_provider:
            continue
        if avail is None or mp in avail:
            return m
    return None
...
def classify_role_severity(
    ...
    (The former judge-only ``advisory`` severity — routes, but same vendor as
    standard — died with the judge role; design-judge-role-collapse §5.4.)
    ...
    where ``providers`` is the resolved rung's provider set (the remedy
    targets) and ``dormant_models`` is the inert uncredentialed fallback ids
    in that rung.
    """
...
_LEGACY_TIER_TO_RUNG: dict[str, str] = {
    ...
    "tier2": "sonnet-class",
    "tier1": "opus-class",
    # tier0 (the legacy judge slot) folds into an ORDINARY extra rung — see
    # the tier0 read-compat branch in _normalize_legacy_layer.
}
...
def _normalize_legacy_layer(layer: dict | None) -> dict | None:
    ...
    Python-side ONLY since 2026-08-15: the TS mirror (`_normalizeLegacyLayer`
    in ModelRouter.ts) was removed with the runtime legacy fallback — the TS
    runtime now REFUSES a legacy-shaped layer (LegacyTierShapeError) instead
    of normalizing it. This Python normalization stays because admin reads
    must still understand un-migrated files (display, diagnosis, and the
    migrate-model-roles driver). ``tiers.tierN.fallbacks`` are folded into
    the synthesized cluster here (F2 of the #2561 review). A layer that
    already carries ``rungs`` (or has no legacy ``tiers``) is returned
    unchanged.
    """
    ...
    for tier_key, cost_class in cost_order:
        ...
        synthesized = True
    # tier0 read-compat (judge-role collapse §6.1): the legacy judge slot no
    # longer maps to a role, but a legacy file carrying it must stay PARSEABLE.
    # Its models fold into an ORDINARY extra rung (kept under the historical
    # ``judge-class`` slug so the keyed merge stays stable) that no role points
    # at — visible in the merged catalog, never routed, never an error.
    ...
    if tier0_models:
        judge_rung = next((r for r in new_rungs if r["id"] == "judge-class"), None)
        if judge_rung is not None:
            for m in tier0_models:
                if m not in judge_rung["models"]:
                    judge_rung["models"].append(m)
        else:
            new_rungs.append({"id": "judge-class", "models": tier0_models, "costClass": "medium"})
        synthesized = True
    if not synthesized:
        return layer
...
def resolve_tier_chain(data: dict, tier: str) -> list[str]:
    ...
    Resolution order:
      1. **New shape (rungs/roles wins):** map ``tierN`` → role
         (tier3→fast, tier2→standard, tier1→power; tier0 no longer maps —
         judge-role collapse), resolve the
...
def tier_override_is_broken(override: dict | None, tier: str) -> bool:
    ...
    entry flags. Python-only since 2026-08-15: the TS mirror
    (``tierOverrideIsBroken``) was removed with ModelRouter.ts's legacy
    runtime fallback — this admin-side detector is the sole home now.
    """
