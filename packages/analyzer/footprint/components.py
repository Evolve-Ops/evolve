...
Spec: internal/spec-footprint-2026-06-18.md (§ "Auto-generated output declaration
contract"). Audit that motivated it: internal/footprint-disk-output-audit-2026-06-28.md.
    ...
    [
        ...
        _c(
            ...
            outputs=(
                ...
                OutputDeclaration(
                    path_glob="model_swaps.jsonl",
                    surface=SURFACE_SHARED,
                    writer="packages/analyzer/model_swap_ledger.py:record_swap",
                    cadence="One append per model-rung change (an operator "
                    "applying a model-freshness advisory, or a rollback). A "
                    "no-op re-apply appends nothing.",
                    volume_files="1 file, append-only",
                    volume_bytes="~250 bytes/swap; a handful of swaps per pod "
                    "per month, so KB-scale for the life of a pod",
                    retention=Retention(
                        pruner="none (deliberate)",
                        window="unbounded",
                        justification="The record's whole purpose is that a "
                        "swap from months ago is still attributable and still "
                        "undoable (internal/design-model-swap-behavior-guard-"
                        "2026-08-19.md) — the 2026-08-14 incident cost days "
                        "precisely because the previous model had to be "
                        "reconstructed by hand. Pruning it would restore the "
                        "gap. Cardinality is operator-gated, not "
                        "traffic-driven: a line is appended only when a human "
                        "changes a model, so this cannot become sediment.",
                    ),
                    consumer=named(
                        "model_swap_watch (post-swap behavior check) + "
                        "`evolve-admin models swaps|rollback`",
                        "packages/analyzer/model_swap_watch.py + "
                        "packages/admin/evolve_admin/model_swap_cli.py",
                    ),
                ),
                OutputDeclaration(
                    path_glob="model_swap_pins.jsonl",
                    surface=SURFACE_SHARED,
                    writer="packages/analyzer/model_swap_ledger.py:record_pin_event",
                    cadence="One append per pin/unpin — a `models rollback` "
                    "rejecting a model for behavior, an operator override "
                    "re-allowing it, or `models unpin`.",
                    volume_files="1 file, append-only",
                    volume_bytes="~200 bytes/event; rollbacks are rare, so "
                    "byte-scale for the life of a pod",
                    retention=Retention(
                        pruner="none (deliberate)",
                        window="unbounded",
                        justification="The pin is the sticky-rollback record "
                        "the 2026-08-21 recurrence showed was missing: a "
                        "behavior-rejected (bot, tier, model) must stay "
                        "refused by the tier-write endpoints indefinitely, "
                        "and even lifted pins feed model_swap_watch's "
                        "repeat-swap early-warning floor. Cardinality is "
                        "operator-gated (one event per rollback/override).",
                    ),
                    consumer=named(
                        "tier-write endpoints (behavior-pin gate) + "
                        "model_swap_watch repeat floor + "
                        "`evolve-admin models pins|unpin`",
                        "packages/admin/evolve_admin/web/model_tier_update_routes.py + "
                        "packages/analyzer/model_swap_watch.py",
                    ),
                ),
                OutputDeclaration(
        ...
        _c(
            "dossier_spine",
            summary="Weekly raw-metric editions of the pod dossier — the "
            "longitudinal spine — plus the module set that says in plain "
            "English what each week's numbers mean. Operator-facing today "
            "only through the CLI report: the Pod Intelligence surface that "
            "reads both lands later, deliberately, on a spine that by then "
            "has history.",
            outputs=(
                OutputDeclaration(
                    path_glob="dossier/editions/<YYYY>-W<WW>.json",
                    surface=SURFACE_SHARED,
                    writer="packages/analyzer/dossier/store.py:write_edition",
                    cadence="Once per ISO week (Mon 04:10, pod-wide, as evolve); "
                    "an in-week re-run overwrites only its own unsealed edition.",
                    volume_files="52/year; 260 at the 5-year steady state",
                    volume_bytes="~10-40 KB/edition, ~1-2 MB/year",
                    retention=Retention(
                        pruner="packages/analyzer/dossier/store.py:prune_editions",
                        window="5y",
                        justification="Cardinality is bounded by construction (one "
                        "file per ISO week), but a bound that is only argued is not "
                        "a bound — the prune runs at the end of every write and "
                        "enforces it.",
                    ),
                    consumer=named(
                        "dossier module synthesis (the weekly headlines + their "
                        "cross-edition trends)",
                        "packages/analyzer/dossier/modules.py:build_modules",
                    ),
                ),
                OutputDeclaration(
                    path_glob="dossier/modules/<YYYY>-W<WW>.json",
                    surface=SURFACE_SHARED,
                    writer="packages/analyzer/dossier/store.py:write_modules",
                    cadence="Once per ISO week, in the same run that writes the "
                    "edition; re-said on demand (--modules-only) without "
                    "touching the measurement.",
                    volume_files="52/year; 260 at the 5-year steady state",
                    volume_bytes="~3-8 KB/module set, well under 1 MB/year",
                    retention=Retention(
                        pruner="packages/analyzer/dossier/store.py:prune_modules",
                        window="5y",
                        justification="Same bound and same horizon as the edition "
                        "it renders — a module set outliving its edition would be "
                        "a headline with no measurement behind it.",
                    ),
                    consumer=named(
                        "the Pod Intelligence page (D-T5/T9) — it renders the "
                        "current week's module set and reads earlier ones for "
                        "each card's trend line, which is why the synthesis "
                        "accrued from the day the writer shipped: a trend needs "
                        "two weeks and the second one cannot be backdated",
                        "packages/admin/evolve_admin/web/routes_dossier.py:"
                        "register_dossier_routes",
                    ),
                ),
                OutputDeclaration(
                    path_glob="dossier/profile.json",
                    surface=SURFACE_SHARED,
                    writer="packages/analyzer/dossier/profile.py:save_profile",
                    cadence="Only when the operator rates, reorders, or turns "
                    "down a card on the Pod Intelligence page. Nothing "
                    "scheduled writes it; a pod nobody has arranged never has "
                    "the file at all.",
                    volume_files="exactly 1, forever",
                    volume_bytes="under 4 KB — three capped lists of module ids",
                    retention=Retention(
                        pruner="overwrite-in-place",
                        window="overwrite",
                        justification="A bounded STATE file, not a record "
                        "store: one path, rewritten whole on each save, with "
                        "every list capped at dossier.profile.MAX_ENTRIES. "
                        "There is nothing to prune because nothing "
                        "accumulates — the same shape as the breaker and "
                        "autonomy state files. Declared anyway rather than "
                        "left to the contract's state-store exemption, "
                        "because a reader asking what lives under "
                        "{shared_dir}/dossier/ should find all three answers "
                        "in one place.",
                    ),
                    consumer=named(
                        "the Pod Intelligence page, which reads it to arrange "
                        "the module grid; design §4a rule 1 routes a second "
                        "reader (proposal ranking) to the rsi lane, deposited "
                        "and not yet built",
                        "packages/admin/evolve_admin/web/routes_dossier.py:"
                        "register_dossier_routes",
                    ),
        ...
        _c(
            ...
            outputs=(
                OutputDeclaration(
                    path_glob="alerts/{dispatcher,dispatcher-queued,dispatcher-suppressed,"
                    "delivery-failures}/<YYYY-MM-DD>.jsonl",
                    surface=SURFACE_SHARED,
                    writer="packages/admin/evolve_admin/alerts/dispatcher.py:_append_log",
                    cadence="One append per dispatch decision.",
                    volume_files="4 streams × 1 file/day, 30-day window",
        ...
        _c(
            "daemon_operator_logs",
            summary="The plain-text tick log the spend-alert daemon appends "
                    "to, read by an operator during a spend incident.",
            outputs=(
                OutputDeclaration(
                    path_glob="logs/spend_alert.log",
                    surface=SURFACE_SHARED,
                    writer="packages/analyzer/spend_alert.py:_log",
                    cadence="One append per log line on each 5-minute tick.",
                    volume_files="1 file, append-only",
                    volume_bytes="size-capped",
                    retention=Retention(
                        pruner="packages/analyzer/log_cap.py:cap_logs",
                    ...
                    consumer=no_consumer(
                        "Operator forensics only — the trail an operator tails "
                        "when a cap trips or a burst alert fires. Everything a "
                        "program needs is in the Signal store instead; the log "
                        "carries the surrounding narration, not state."
                    ),
                    note="Went to a hard-coded absolute path until the log "
                         "path was derived from shared_dir — which is what "
                         "made this write visible to the lint at all. "
                         "better_engine.log is the same shape and is still a "
                         "module constant; it gets its own declaration when "
                         "that half lands.",
                ),
            ),
        ),
...
# ─────────────────────────────────────────────────────────────────────────────
# Spec: internal/spec-footprint-2026-06-18.md § "FD-5 engine design" (part 1, the
# component graph). Authoritative edge list + classification: the VERIFIED-from-
# code graph in internal/footprint-dependency-graph-2026-06-18.md (the F-2.5 audit) —
...
_GRAPH_NODES: tuple[FootprintComponent, ...] = (
    ...
    _node(
        "apply",
        kind=KIND_MODULE, classification=CLASS_CASCADE,
        summary="The applier — realizes approved proposals into bot config. "
        "The L2 write path couples it to the sudoers/ACL infra floor. NOTE: the "
        "legacy per-bot applier daemon (packages/analyzer/apply.py) that this "
        "toggle gated was retired 2026-08-18 as structurally unreachable "
        "(internal/design-proposal-signing-key-2026-08-18.md); the live applier is "
        "arbiter.apply, which this node does not currently gate. Re-pointing or "
        "retiring the toggle is a footprint-aspect decision, deliberately not "
        "made here.",
        footprint_dims=(DIM_MUTATION, DIM_PRIVILEGE),
        requires=("analysis", "rsi", "sudoers", "openclaw_acls"),
        gate="(none — the apply.py gate was removed 2026-08-18; toggle needs a "
             "footprint-owned decision)",
    ),
    _node(
        "outcomes",
        kind=KIND_MODULE, classification=CLASS_CASCADE,
