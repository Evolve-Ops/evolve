...
from backup_visibility import (
    PAT_ABSENT as _PAT_ABSENT,
    PAT_UNREADABLE as _PAT_UNREADABLE,
    check_repo_visibility,
    load_pat_state as _load_github_pat_state,
    parse_github_repo as _parse_github_repo,
    visibility_via_daemon as _visibility_via_daemon,
)
...
def _backup_bot_attempt(
    ...
    # Which of the three PAT states we are in decides how this run is
    # CLASSIFIED, not just what we print. Only a genuinely-unconfigured PAT is
    # a setup step the operator hasn't done; everything else is a failure that
    # must accumulate so backup_signal's threshold can see it.
    pat_state, pat = _load_github_pat_state(network)
    if pat_state == _PAT_ABSENT:
    ...
    if pat_state == _PAT_UNREADABLE:
        # A PAT *is* stored; this uid cannot decrypt it (the #3696 clamp). Ask
        # the admin daemon — which runs as evolve and owns the vault — for a
        # verdict instead. It returns private/public/unknown and never the
        # token, so the vault stays as shut to bots as #3696 made it.
        answer = _visibility_via_daemon(backup_url)
        if answer is None:
            # Neither route worked. Before #4196 this returned the wizard
            # message above and was recorded "skipped", so consecutive_failures
            # never left 0 and no Signal could fire — nine bots, twenty-three
            # silent nights. It is a failure, and it names the real cause.
            msg = (
                "refusing to push: a pod GitHub PAT is stored but this "
                "process cannot read it (keystore key `github_pat`; the vault "
                "and `.machine-key` are readable only by `evolve`), AND the "
                "admin daemon's visibility endpoint was unreachable, so the "
                "backup repo's visibility could not be verified by either "
                "route. This is NOT a missing setup step — do not re-run the "
                "Backup → Cloud wizard. Check that the admin daemon is "
                "running and that `{shared}/admin-daemon.sock` is reachable "
                "from this bot's user."
            )
            print(f"[backup] {bot_id}: {msg}", file=sys.stderr)
            _refresh_local_drift_baseline(bot_id, shared_dir)
            return "failed", msg
        visibility, daemon_reason = answer
    else:
        visibility = check_repo_visibility(backup_url, pat=pat)
        daemon_reason = None

    if visibility != "private":
        # Two distinct conditions share this guard, and their remediations
        # point at different things — collapsing them misled the 2026-07-28
        # triage ("make the repo private" for a repo that WAS private;
        # the actual problem was an expired PAT → lookup 401 → 'unknown').
        #   - "public":  GitHub CONFIRMED the repo is public → fix the repo.
        #   - "unknown": the lookup itself failed (expired/revoked PAT,
        #     missing repo access, network error) → fix the PAT, not the
        #     repo. Fail-safe still blocks the push either way.
        if visibility == "unknown" and daemon_reason is not None:
            # The verdict came from the admin daemon, which could read the PAT
            # and still couldn't get an answer. Say which hop failed — "fix
            # your PAT" and "the daemon refused this peer" have nothing to do
            # with each other, and the 2026-07-28 triage was lost to exactly
            # this kind of collapsed message.
            msg = (
                f"refusing to push: backup repo visibility is 'unknown' — "
                f"this bot cannot read the pod PAT, and the admin daemon's "
                f"visibility endpoint answered {daemon_reason!r} rather than a "
                f"verdict. The repo was NOT detected as public. If the reason "
                f"names the PAT, fix the pod-wide PAT (keystore key "
                f"`github_pat`) on the Backup page; if it names the peer or "
                f"the repo URL, this bot's `backupRepoUrl` in network.json "
                f"and the daemon's view of it disagree."
            )
        elif visibility == "unknown":
            msg = (
                "refusing to push: backup repo visibility is 'unknown' — "
                "the GitHub API lookup failed even though a PAT is "
                "configured (expired or revoked token, no access to this "
                "repo, or a network error). The repo was NOT detected as "
                "public. Fix the pod-wide PAT (keystore key `github_pat` — "
                "re-run the GitHub setup wizard on the Backup page) or "
                "check connectivity to resume backups."
            )
        else:
    ...
    # internal/spec-backup-and-data-classification-2026-05-28.md §"Phase 3".
...
def _copy_with_sudo(src: Path, dst: Path, bot_id: str) -> tuple[bool, str]:
    ...
    internal/diagnosis-accept-drift-regression-2026-05-21.md for why a stale
...
def commit_baseline_local(bot_id: str, shared_dir: Path) -> tuple[bool, str]:
    ...
    if not ok:
        ...
        # internal/diagnosis-accept-drift-regression-2026-05-21.md.
