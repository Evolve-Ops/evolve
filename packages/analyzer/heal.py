...
def run_once(config: dict, shared_dir: Path, heal_cfg: dict, only_bot: str | None, dry_run: bool) -> None:
    ...
    # internal/spec-circuit-breakers-2026-05-21.md §3.5).
    #
    ...
    # Pod-wide: re-register Evolve daemons that are on disk but not loaded.
    # Runs AFTER the per-bot loop — a bot whose gateway heal just restarted
    # should not also have its per-bot daemons re-bootstrapped in the same
    # tick, and the per-bot work is what the operator cares about first.
    #
    # Skipped while paused, for the same reason the cost-breaker reaper is:
    # bootstrapping daemons during an operator panic-stop fights the
    # operator. An L2 breaker does NOT gate it — L2 stops a bot's gateway,
    # it does not mean "leave Evolve's own scheduled maintenance unloaded".
    if not paused:
        _sweep_unloaded_daemons(config, shared_dir, dry_run=dry_run)


def _sweep_unloaded_daemons(
    config: dict, shared_dir: Path, *, dry_run: bool = False,
) -> None:
    """Repair on-disk-but-unloaded Evolve daemons; report what happened.

    See ``packages/analyzer/launchd_reload.py`` for the mechanism and for
    why the repair is a plain bootstrap rather than ``Scheduler.enable``.

    Fails open in every direction: an unimportable module, an unavailable
    admin package, or a raising sweep must never take down the heal cycle
    that restarts gateways.
    """
    try:
        import launchd_reload
    except Exception as exc:            # noqa: BLE001 — optional repair path
        print(f"[heal] launchd_reload unavailable, skipping sweep: {exc}")
        return

    try:
        result = launchd_reload.sweep(config, shared_dir, dry_run=dry_run)
    except Exception as exc:            # noqa: BLE001 — a raise is not fatal
        print(f"[heal] launchd reload sweep raised, continuing: {exc}")
        return

    if result.skipped:
        print(f"[heal] launchd reload sweep skipped: {result.skipped}")
        return
    if result.unprobed:
        # Never silent: a probe we couldn't run is the state in which this
        # sweep is blind, and blindness that looks like health is the exact
        # failure mode the sweep exists to end.
        print(f"[heal] launchd: {len(result.unprobed)} daemon(s) unprobeable "
              f"— {'; '.join(result.unprobed[:5])}")
    if not result.unloaded:
        print(f"[heal] launchd: {result.checked} expected daemons, all loaded")
        return

    for outcome in result.outcomes:
        print(f"[heal] launchd {outcome.label}: {outcome.result}"
              f"{' — ' + outcome.detail if outcome.detail else ''}")

    # Report every repair. A self-heal nobody hears about turns a visible
    # outage into an invisible one — the whole reason the 2026-09-07
    # doctor-pass bootout went unnoticed for 7 days was that nothing said
    # anything. Digest cadence, not IMMEDIATE: this is "you should know",
    # not "act now".
    if result.repaired and not dry_run:
        _dispatch_heal(
            config=config,
            catalog_event="system.daemon_reloaded",
            # Keyed on the label set, so a recurring bootout of the SAME
            # daemons coalesces while a new one still breaks through.
            dedup_key=f"heal/daemon_reloaded/{'+'.join(sorted(result.repaired))}",
            severity_name="warning",
            payload={
                "count": len(result.repaired),
                "labels": ", ".join(sorted(result.repaired)),
            },
        )

    # Escalate once, on the cycle a label exhausts its attempt budget.
    # Re-alerting every 5 minutes for a unit that cannot load is noise.
    if result.newly_parked and not dry_run:
        _dispatch_heal(
            config=config,
            catalog_event="system.daemon_reload_failed",
            dedup_key=f"heal/daemon_reload_failed/{'+'.join(sorted(result.newly_parked))}",
            severity_name="error",
            payload={
                "count": len(result.newly_parked),
                "labels": ", ".join(sorted(result.newly_parked)),
                "attempts": launchd_reload.MAX_ATTEMPTS,
            },
        )
...
def _read_l2_breaker_state(shared_dir: str | Path) -> tuple[set[str], bool]:
    ...
    Spec: internal/spec-circuit-breakers-2026-05-21.md §3.5.
    """
...
def detect_backup_drift_keys(bot_id: str, shared_dir: Path, config: dict, os_user: str | None = None) -> list[str]:
    ...
    # openclaw.json keys and the SAME "tiers:<key>" namespace this function
    # emits for evolve-tiers.json keys, so both sides of `diffs` are matched
    # against the declaration set verbatim.
    #
    # HISTORY (spec-delta-digest-audit-noise-2026-08-25 D3): this comment used
    # to assert the tiers:-prefixed declaration as existing fact. It did not
    # exist. Every tier-write endpoint declared a bare {"agents"} (or, in the
    # user-tier-override case, a bare {"tiers"} that matches nothing this
    # function emits), and no call site anywhere in the repo emitted a
    # "tiers:"-prefixed key — so the entire tiers: namespace was structurally
    # unexplainable and an authorized Evolve write was indistinguishable from
    # a hand edit, forever, on every bot with an evolve-tiers.json. The
    # writers now declare what they landed: json_full_config_set diffs the
    # tiers doc across the write and reports the changed top-level keys, and
    # each audit call site turns that into "tiers:<key>" via
    # routes_shared.tier_write_oc_keys. A HAND edit still has no declaration
    # and still surfaces here, which is the point.
...
def _get_recent_admin_ui_writes(
    ...
    """Return the config keys touched by recent admin-UI writes.

    Two namespaces share the one set: bare names are top-level
    ``openclaw.json`` keys; ``tiers:<key>`` names are top-level
    ``evolve-tiers.json`` keys (the same namespacing
    :func:`detect_backup_drift_keys` emits). Writers declare both, since one
    tier write usually lands in both files.
...
def _get_recently_applied_config_keys(bot_id: str, shared_dir: Path) -> set[str]:
    ...
    The extraction (both record shapes the retired apply daemon wrote) is
    ``drift_authorization.applied_config_keys`` — one reader for the
    directory, shared with the L2 authorized-change gate. A record belongs
    to ``bot_id`` when its file name carries the bot prefix or its
    ``bot_id`` field names the bot.
    """
    from drift_authorization import applied_config_keys
    ...
    for f in sorted(results_dir.glob("*.json")):
        ...
        if not isinstance(r, dict):
            continue
        if not (f.name.startswith(f"{bot_id}-") or r.get("bot_id") == bot_id):
            continue
        keys |= applied_config_keys(r)
    return keys
...
def check_pod_conduct_injection(
    ...
    # Repair: strip openclaw.json keys Evolve used to write that the
    # installed OpenClaw now rejects outright (breaker-notify-survives-
    # openclaw-config-validation item 2). Same shape as the agents.main
    # strip above — this is the faster of the two propagation paths the
    # fix reaches the live pod by (the other is the next `evolve-admin
    # deploy`, which calls the SAME oc_retired_keys.strip_retired_openclaw_keys,
    # so the two can never drift on which keys are retired).
    config_text, _ = _read_with_sudo_fallback(config_path)
    if config_text is not None:
        try:
            config = json.loads(config_text)
        ...
        if config is not None:
            from evolve_admin.oc_retired_keys import strip_retired_openclaw_keys  # type: ignore[import]
            if strip_retired_openclaw_keys(config):
                print(
                    f"[heal] Removing retired openclaw.json key(s) from "
                    f"{bot_id} (see breaker-notify-survives-openclaw-"
                    f"config-validation)"
                )
                from staging import secure_stage
                tmp = secure_stage(json.dumps(config, indent=2))
                try:
                    r3 = subprocess.run(["sudo", "/bin/cp", str(tmp), str(config_path)],
                                        capture_output=True, text=True)
                    subprocess.run(["sudo", "/usr/sbin/chown", f"{os_user}:staff", str(config_path)],
                                   capture_output=True)
                    if r3.returncode == 0:
                        print(f"[heal] retired key(s) removed — restarting {bot_id} gateway")
                        from runtime.agent_runtime import get_runtime
                        get_runtime().gateway_restart(bot_id)
                finally:
                    tmp.unlink(missing_ok=True)
