...
def run_once(config: dict, shared_dir: Path, heal_cfg: dict, only_bot: str | None, dry_run: bool) -> None:
    ...
    for bot_id in members:
        ...
        oc_config_path = Path(f"/Users/{os_user}/.openclaw/openclaw.json")
        conduct_file = Path("/Users/Shared/evolve/POD_CONDUCT.md")
        if not conduct_file.exists():
            print(f"[heal] warning: /Users/Shared/evolve/POD_CONDUCT.md does not exist — skipping injection check for {bot_id}")
            conduct_injected = None
        else:
            conduct_injected = check_pod_conduct_injection(bot_id, oc_config_path, os_user=os_user)
...
def restart_gateway(bot_id: str, os_user: str | None = None) -> bool:
    ...
    try:
        r = subprocess.run(
            ["sudo", "-u", os_user, "env", f"HOME=/Users/{os_user}",
...
def detect_config_drift(bot_id: str, shared_dir: Path, config: dict, os_user: str | None = None) -> list[str]:
    ...
    oc_path = Path(f"/Users/{os_user}/.openclaw/openclaw.json")
...
def detect_backup_drift_keys(bot_id: str, shared_dir: Path, config: dict, os_user: str | None = None) -> list[str]:
    ...
    if committed_tiers is not None:
        live_tiers = _read_live_json(
            Path(f"/Users/{os_user}/.openclaw/evolve-tiers.json")
        )
...
def check_pod_conduct_injection(bot_id: str, config_path: Path, os_user: str | None = None) -> bool:
    ...
    if os_user is None:
        os_user = bot_id
    conduct_src = "/Users/Shared/evolve/POD_CONDUCT.md"
...
def _write_status_file(
    ...
    try:
        ...
        recent_errors: list[str] = []
        recent_errors_ts: list[str | None] = []
        err_path = Path(f"/Users/{user}/.openclaw/logs/gateway.err.log")
