...
Spec: internal/spec-circuit-breakers-2026-05-21.md §8 Phase 5.
...
default ``true`` — ARMED — since the §5.2 arming PR; the 2026-06→08
calibration soak banked 7,576 clean cycles with zero false trips)
gates step 4b-d only. The operator can disarm with ``sudo
evolve-admin breaker disarm`` (writes ``auto_trip_enabled: false``),
which returns the runner to observe-only mode: the detector still
runs and runner-log entries are still written (subject to the
step-4 source-cut above).
...
def read_auto_trip_enabled(network: dict | None) -> bool:
    """Read ``breakers.auto_trip_enabled`` from network.json.

    Default ``True`` (armed). The §5.2 calibration soak (2026-06-15 →
    2026-08, 7,576 observe-only cycles, zero would-trip decisions)
    cleared the false-positive bar, so auto-trip is now the product
    default. The operator opt-out is ``sudo evolve-admin breaker
    disarm``, which records ``breakers.auto_trip_enabled: false`` and
    returns the runner to observe-only mode (detector runs, decisions
    are logged, no real trips).

    A malformed/missing network config reads as ``False`` — never
    enforce against a pod whose config we couldn't read (moot in
    practice: without a config there are no bots to iterate either).
    """
    if not isinstance(network, dict):
        return False
    breakers_cfg = network.get("breakers") or {}
    return bool(breakers_cfg.get("auto_trip_enabled", True))
...
def run_once(
    *,
    ...
    auto_trip_enabled = read_auto_trip_enabled(network)
...
def _process_one_bot(
    *,
    ...
    # 7) Enforce. ``shared_dir`` is REQUIRED for cost enforcement — it
    #    locates the heartbeat / exec-approval stashes and the spend-cap
    #    flag. Omit it and enforce_trip returns ok=False with
    #    "shared_dir required for cost enforcement" and does nothing,
    #    which is how auto-trips from this runner silently skipped every
    #    L1 action while the runner-log recorded the trip as fired.
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "Acts on trips unless network.json::breakers.auto_trip_enabled "
            "is false (default: true — armed; disarm with "
            "`evolve-admin breaker disarm`)."
        ),
    )
    parser.add_argument(
