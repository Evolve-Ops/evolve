...
def quiet_network() -> dict:
    """One bot, auto-trip disabled — the safe default."""
    return {
        "primary": "team_bot_a",
        "members": ["team_bot_a"],
...
def auto_network() -> dict:
    """Same shape but auto_trip_enabled=True."""
    return {
        "primary": "team_bot_a",
        "members": ["team_bot_a"],
        "bots": {"team_bot_a": {"user": "team_bot_a"}},
...
# ─────────────────────────────────────────────────────────────────────────────
# Observe-only (flag off) — the safe default
# ─────────────────────────────────────────────────────────────────────────────
...
class TestFlagReader:
    @pytest.mark.parametrize("net", [
        None, {}, {"breakers": {}}, {"breakers": {"auto_trip_enabled": False}},
        {"some": "other"}, "not a dict",
    ])
    def test_defaults_to_false(self, net) -> None:
        assert runner._read_auto_trip_enabled(net) is False

    def test_explicit_true(self) -> None:
        assert runner._read_auto_trip_enabled(
            {"breakers": {"auto_trip_enabled": True}}
        ) is True
