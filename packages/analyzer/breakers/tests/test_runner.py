...
class TestAutoTrip:
    def test_trip_fires_when_flag_on_and_decision_is_trip(
        ...
        # shared_dir is REQUIRED for cost enforcement — without it
        # enforce_trip returns ok=False / "shared_dir required for cost
        # enforcement" and performs no L1 action at all (no heartbeat
        # stash, no exec-approval passthrough, no user-channel message),
        # while this runner still logs the trip as fired.
        assert enforce_calls[0]["shared_dir"] == shared_dir
