...
class TestAuditLog:
    ...
    def test_audit_append_failure_does_not_abort_reset(
        self, tmp_path: Path, caplog,
    ) -> None:
        """2026-09-04 reset incident regression.

        A root-owned ``breakers/log/<date>.jsonl`` made the audit append
        raise ``PermissionError`` AFTER ``reset()`` had already unlinked
        the breaker file. The exception propagated to the route, which
        therefore never ran ``enforce_reset`` — the bot was left with no
        breaker (so nothing flagged it, and the UI hid Reactivate) and no
        restored heartbeat.

        The append is bookkeeping: it must degrade to a warning.
        """
        trip(
            shared_dir=tmp_path, scope="team_bot_a", breaker_type="cost",
            duration=timedelta(hours=24), initiated_by="auto",
        ...
        with caplog.at_level("WARNING"):
            with pytest.MonkeyPatch.context() as mp:
                mp.setattr(Path, "open", _deny_audit_open)
                prior = reset(
                    shared_dir=tmp_path, scope="team_bot_a",
                    breaker_type="cost", initiated_by="web",
                    reason="operator reset",
                )

        # The reset completed: the caller gets the prior record back and
        # can go on to run the bring-up half.
        assert prior is not None
        assert prior.bot_id == "team_bot_a"
        assert read_trip(tmp_path, "team_bot_a", "cost") is None
        # ...and the failure is visible rather than silent.
        assert any(
            "audit append failed" in r.getMessage() for r in caplog.records
        )

    def test_audit_append_failure_does_not_abort_trip(
        self, tmp_path: Path,
    ) -> None:
        """Same contract on the trip side — the state write is committed
        before the append, so a failed append must not make the caller
        think the trip did not happen."""
        real_open = Path.open
        ...
        assert read_trip(tmp_path, "team_bot_a", "cost") is not None
...
# ─────────────────────────────────────────────────────────────────────────────
# Interactive-hold evidence (D-CS13;
# internal/decision-cost-single-turn-2026-09-18.md)
# ─────────────────────────────────────────────────────────────────────────────


def _write_hold(tmp_path: Path, bot_id: str, trip_id: str, ts: str,
                 plugin_version: str = "0.1.0", session: str = "slack:C1") -> None:
    """Mirrors the plugin's HoldMarker.ts append — one JSONL line."""
    p = holds_log_path(tmp_path, bot_id)
    p.parent.mkdir(parents=True, exist_ok=True)
    with p.open("a", encoding="utf-8") as fh:
        fh.write(json.dumps({
            "trip_id": trip_id, "ts": ts, "session": session,
            "plugin_version": plugin_version,
        }) + "\n")


class TestReadEarliestHold:
    def test_absent_file_reads_as_no_marker(self, tmp_path: Path) -> None:
        assert read_earliest_hold(tmp_path, "team_bot_a", "trip-1") is None

    def test_returns_the_earliest_matching_entry(self, tmp_path: Path) -> None:
        _write_hold(tmp_path, "team_bot_a", "trip-1", "2026-09-17T19:55:00Z")
        _write_hold(tmp_path, "team_bot_a", "trip-1", "2026-09-17T19:52:47Z",
                    plugin_version="0.1.0-canary")
        _write_hold(tmp_path, "team_bot_a", "trip-1", "2026-09-17T19:58:00Z")
        result = read_earliest_hold(tmp_path, "team_bot_a", "trip-1")
        assert result == ("2026-09-17T19:52:47Z", "0.1.0-canary")

    def test_ignores_lines_for_a_different_trip(self, tmp_path: Path) -> None:
        _write_hold(tmp_path, "team_bot_a", "trip-other", "2026-09-17T19:00:00Z")
        assert read_earliest_hold(tmp_path, "team_bot_a", "trip-1") is None

    def test_tolerates_garbage_lines(self, tmp_path: Path) -> None:
        p = holds_log_path(tmp_path, "team_bot_a")
        p.parent.mkdir(parents=True, exist_ok=True)
        p.write_text("not json\n" + json.dumps({"trip_id": "trip-1"}) + "\n")
        # Missing `ts` on the one structurally-valid line -> still no marker.
        assert read_earliest_hold(tmp_path, "team_bot_a", "trip-1") is None


class TestHoldMarkerFoldedIntoTrip:
    """The store, not the plugin, owns cost.json — but every write folds in
    whatever the plugin has evidenced in holds.jsonl (see module docstring
    and _fold_hold_marker)."""

    def test_extend_folds_a_marker_written_after_the_trip(self, tmp_path: Path) -> None:
        rec = trip(
            shared_dir=tmp_path, scope="team_bot_a", breaker_type="cost",
            duration=timedelta(hours=24), initiated_by="auto",
            reason="x", checkpoint="pending", now=FIXED_NOW,
        )
        assert rec.first_hold_at is None  # nothing evidenced yet
        _write_hold(tmp_path, "team_bot_a", rec.trip_id, "2026-05-21T16:07:00Z",
        ...
        assert updated.first_hold_at == "2026-05-21T16:07:00Z"
        assert updated.hold_plugin_version == "7.1.2"

    def test_marker_survives_a_further_rewrite(self, tmp_path: Path) -> None:
        """Once folded, a later write (answer_checkpoint here) must not
        drop the evidence even though it never reads holds.jsonl for a
        DIFFERENT, still-earlier entry."""
        rec = trip(
            shared_dir=tmp_path, scope="team_bot_a", breaker_type="cost",
            duration=timedelta(hours=24), initiated_by="auto",
            reason="x", checkpoint="pending", now=FIXED_NOW,
        ...
        answered = answer_checkpoint(
            shared_dir=tmp_path, scope="team_bot_a", breaker_type="cost",
            state="continued", answered_by="user:slack:U1",
            increment_usd=2.5, now=FIXED_NOW + timedelta(minutes=20),
        )
        assert answered.first_hold_at == "2026-05-21T16:07:00Z"
        # Read back from disk too — this is what a live dashboard reads.
        again = read_trip(tmp_path, "team_bot_a", "cost")
        assert again.first_hold_at == "2026-05-21T16:07:00Z"

    def test_no_marker_leaves_first_hold_at_none(self, tmp_path: Path) -> None:
        trip(
            shared_dir=tmp_path, scope="team_bot_a", breaker_type="cost",
            duration=timedelta(hours=24), initiated_by="auto",
            reason="x", checkpoint="pending", now=FIXED_NOW,
        ...
        updated = update_audit_fields(
            shared_dir=tmp_path, scope="team_bot_a", breaker_type="cost",
            audit_summary="no interactive spend",
        )
        assert updated.first_hold_at is None

    def test_pod_wide_scope_is_never_folded(self, tmp_path: Path) -> None:
        """CostCheckpoint.ts never consults the pod-wide breaker for a
        checkpoint — there is no per-bot cap to quote — so a marker under
        the pod scope (which should never exist in practice) is ignored."""
        rec = trip(
            shared_dir=tmp_path, scope=POD_SCOPE, breaker_type="cost",
            duration=timedelta(hours=1), initiated_by="admin:pod_admin",
            reason="pod-wide pause", now=FIXED_NOW,
        )
        _write_hold(tmp_path, POD_SCOPE, rec.trip_id, "2026-05-21T16:07:00Z")
        updated = extend(
            shared_dir=tmp_path, scope=POD_SCOPE, breaker_type="cost",
            additional=timedelta(hours=1), initiated_by="admin:pod_admin",
        )
    ...
    def test_full_type_is_never_folded(self, tmp_path: Path) -> None:
        """A "full" halt carries no checkpoint — nothing to hold."""
        rec = trip(
            shared_dir=tmp_path, scope="team_bot_a", breaker_type="full",
            duration=timedelta(hours=1), initiated_by="admin:pod_admin",
            reason="manual pause", now=FIXED_NOW,
        )
        _write_hold(tmp_path, "team_bot_a", rec.trip_id, "2026-05-21T16:07:00Z")
        updated = extend(
            shared_dir=tmp_path, scope="team_bot_a", breaker_type="full",
