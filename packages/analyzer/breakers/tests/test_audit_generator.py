...
class TestAnalyzeTrip:
    ...
    def test_falls_through_to_manual_review_when_no_pattern_matches(
        ...
        assert summary is not None
        assert recommendation is not None
        # Not a shrug: names the turns and the predicates it evaluated.
        assert "Top 3 turns by cost" in summary
        assert "Evaluated, did not fire" in summary
...
# ─────────────────────────────────────────────────────────────────────────────
# Interactive-hold evidence line (D-CS13;
# internal/decision-cost-single-turn-2026-09-18.md)
# ─────────────────────────────────────────────────────────────────────────────


def _trip_checkpoint_bot(shared_dir: Path, bot_id: str) -> store.BreakerRecord:
    """A checkpoint-armed trip — the only kind that can hold a turn."""
    return store.trip(
        shared_dir=shared_dir,
        scope=bot_id,
        breaker_type="cost",
...
class TestInteractiveHoldLine:
    def test_appends_not_recorded_when_no_marker_exists(self, tmp_path: Path) -> None:
        _trip_checkpoint_bot(tmp_path, "team_bot_a")
        process_pending_audits(
            shared_dir=tmp_path, now=FIXED_NOW, read_turns_fn=lambda *a, **kw: [],
        )
        reread = store.read_trip(tmp_path, "team_bot_a", "cost")
        assert "interactive hold: not recorded." in reread.audit_summary

    def test_appends_recorded_at_when_a_marker_exists(self, tmp_path: Path) -> None:
        rec = _trip_checkpoint_bot(tmp_path, "team_bot_a")
        p = store.holds_log_path(tmp_path, "team_bot_a")
        p.parent.mkdir(parents=True, exist_ok=True)
        p.write_text(json.dumps({
            "trip_id": rec.trip_id, "ts": "2026-05-21T16:07:00Z",
            "session": "slack:C1", "plugin_version": "0.1.0",
        }) + "\n")
        process_pending_audits(
            shared_dir=tmp_path, now=FIXED_NOW, read_turns_fn=lambda *a, **kw: [],
        )
        reread = store.read_trip(tmp_path, "team_bot_a", "cost")
        assert "interactive hold: recorded at 2026-05-21T16:07:00Z." in reread.audit_summary

    def test_no_line_when_trip_has_no_checkpoint(self, tmp_path: Path) -> None:
        _trip_bot(tmp_path, "team_bot_a")  # no checkpoint armed
        turns = []
        for i in range(4):
            turns.append(_t(minutes_ago=5 + i, model="anthropic/claude-sonnet-4-5"))
        for i in range(6):
            turns.append(_t(minutes_ago=20 + i, model="anthropic/claude-haiku-4-5"))
        process_pending_audits(
            shared_dir=tmp_path, now=FIXED_NOW, read_turns_fn=lambda *a, **kw: turns,
        )
        reread = store.read_trip(tmp_path, "team_bot_a", "cost")
        assert "interactive hold" not in reread.audit_summary
