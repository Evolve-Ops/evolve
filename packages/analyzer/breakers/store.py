...
Semantics:
  ...
  - ``first_hold_at`` / ``hold_plugin_version`` are EVIDENCE, not control
    state (D-CS13; internal/decision-cost-single-turn-2026-09-18.md). The
    plugin never writes this file — it appends what it observes to its own
    ``breakers/<bot_id>/holds.jsonl`` (trip_id, ts, session, plugin_version),
    and every write this module makes to a cost trip folds the earliest
    matching entry in here first (see :func:`read_earliest_hold`). A
    checkpoint whose hold was never evidenced this way is exactly the
    2026-09-17 incident: the trip record read ``checkpoint: null`` with no
    way to tell whether the plugin live at that minute carried the hold.
...
class BreakerRecord:
    ...
    # ISO-8601 timestamp of the EARLIEST holds.jsonl entry matching this
    # trip_id, or None if no marker has been observed yet. Folded in by
    # _fold_hold_marker on every write; never set by a constructor call
    # directly (see that function's docstring).
    first_hold_at: str | None = None
    hold_plugin_version: str | None = None
    ...
    def from_json(cls, data: dict[str, Any]) -> "BreakerRecord":
        return cls(
            ...
            first_hold_at=(
                data["first_hold_at"]
                if isinstance(data.get("first_hold_at"), str) else None
            ),
            hold_plugin_version=(
                data["hold_plugin_version"]
                if isinstance(data.get("hold_plugin_version"), str) else None
            ),
        )
...
#: The plugin's own marker file — see the module docstring's ``first_hold_at``
#: entry. One JSONL line per hold it observes: ``{trip_id, ts, session,
#: plugin_version}``. This module only ever READS it.
HOLDS_FILENAME: str = "holds.jsonl"


def holds_log_path(shared_dir: Path, bot_id: str) -> Path:
    """Return the path to ``bot_id``'s interactive-hold marker log."""
    return breakers_dir(shared_dir) / bot_id / HOLDS_FILENAME


def read_earliest_hold(
    shared_dir: Path, bot_id: str, trip_id: str,
) -> tuple[str, str | None] | None:
    """Return ``(ts, plugin_version)`` of the EARLIEST holds.jsonl line
    matching ``trip_id``, or None if the file is absent, unreadable, or has
    no matching line.

    This is the single source of truth both :func:`_fold_hold_marker` (fold
    into the live ``cost.json``) and the admin health control (evidence for
    trips that have already cleared and whose file may be long gone) read
    from — a health check must see the same marker whether or not this
    trip's ``cost.json`` still exists to be folded into.

    Fail-open: any I/O or parse error reads as "no marker yet". A control
    that spends money must fail closed on unreadable state (see
    BreakerStateReader.ts); this only supplies EVIDENCE, and a bad line
    here must not brick the fold it feeds nor manufacture a false "held".
    """
    if not trip_id:
        return None
    path = holds_log_path(shared_dir, bot_id)
    if not path.is_file():
        return None
        ...
        with path.open("r", encoding="utf-8") as fh:
            for line in fh:
                ...
                try:
                    ...
                    continue
                if not isinstance(entry, dict) or entry.get("trip_id") != trip_id:
                    continue
                ts = entry.get("ts")
                if not isinstance(ts, str) or not ts:
                    continue
                version = entry.get("plugin_version")
...
def _fold_hold_marker(
    shared_dir: Path, scope: str, breaker_type: str, record: BreakerRecord,
) -> BreakerRecord:
    """Fold the earliest recorded interactive hold for this trip into
    ``record`` before it is written (D-CS13; see the module docstring).

    Only a per-bot cost trip can be interactively held — CostCheckpoint.ts
    never consults the pod-wide breaker for this, and a "full" halt has no
    checkpoint — so every other (scope, type) passes ``record`` through
    unchanged. Re-derives from ``holds.jsonl`` on every call rather than
    trusting a previously-folded value, so a marker that lands between two
    writes is still picked up on the next one.
    """
    if breaker_type != "cost" or scope == POD_SCOPE or not record.trip_id:
        return record
    earliest = read_earliest_hold(shared_dir, scope, record.trip_id)
    if earliest is None:
        return record
    ts, plugin_version = earliest
    if record.first_hold_at is not None and record.first_hold_at <= ts:
        return record
    return replace(record, first_hold_at=ts, hold_plugin_version=plugin_version)
...
def trip(
    *,
    ...
    record = _fold_hold_marker(shared_dir, scope, breaker_type, record)
...
def extend(
    *,
    ...
    new_record = _fold_hold_marker(shared_dir, scope, breaker_type, new_record)
