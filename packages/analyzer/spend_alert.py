...
def daily_cap_decision(
    ...
    *,
    ...
    tripped = [
        ...
        if rungs.get(rung) is not None and day.usd >= float(rungs[rung])  # type: ignore[arg-type]
    ]
...
def _write_spend_cap_flag(
    ...
    try:
        _spend_caps.write_enforcement_flag(
            shared_dir, bot_id, action, spend, cap,
...
def _trip_breaker_for_cost_cap(
    *,
    ...
    _write_spend_cap_flag(
        ...
        today_iso=today_iso, action=action,
    )
...
def _maybe_send_weekly_summary(
    ...
    per_bot_breakdown = "\n".join(
        [per_bot_breakdown] + _accuracy_lines(shared_dir),
    )
