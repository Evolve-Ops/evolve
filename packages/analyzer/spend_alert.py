...
# The pod's ONE per-turn cost rule (audit B6). Imported at module level, not
# behind a try/except that could fall back to a manufactured zero.
from turn_cost import (
    turn_cost as _resolve_turn_cost,
    unpriced_note,
)
import live_spend as _live_spend
...
def spend_alert_log_path(shared_dir: Path) -> Path:
    """The operator-facing spend-alert log for the pod rooted at ``shared_dir``.

    Derived, never a module constant. A hard-coded ``/Users/Shared/evolve``
    made every test that drove the alert paths append its fixture lines — cap
    trips, breaker trips, tier downgrades — to the REAL log an operator greps
    when a bot's spend runs away, whatever ``tmp_path`` the test was handed.
    Benign on Linux CI, where ``/Users`` is not creatable by a non-root user
    and the ``mkdir`` raises into the fail-soft ``except OSError`` — so the
    pollution is invisible exactly where it is checked. Same
    ``{shared_dir}/logs/...`` shape the rest of the pod's state already uses,
    and the same fix ``audit_log_path`` already carries.
    """
    return shared_dir / "logs" / "spend_alert.log"
...
def _log(msg: str, shared_dir: Path | None) -> None:
    print(msg, flush=True)
    if shared_dir is None:
        # No pod root in hand means there is no pod log to append to. The
        # line still prints; it does NOT fall back to an absolute path,
        # which is the whole point of the parameter having no default.
        return
    log_file = spend_alert_log_path(shared_dir)
    try:
...
# The live-JSONL reader lives in ``live_spend``, one layer below this module
# and below ``spend_caps``, because BOTH need it and ``spend_alert`` already
# imports ``spend_caps`` — an import back would be a cycle. These names are
# the module's established API surface; they stay, as re-exports.
#
# ``live_spend`` also owns the SEAMS (``pod_tz_or_local``, ``load_live_turns``).
# There is deliberately only one of each: two copies of the pod-TZ resolver
# could disagree, and a turn that falls outside every bucket is the silent
# zero this whole area keeps relapsing into.
_LIVE_LOAD_FAILED = _live_spend.LIVE_LOAD_FAILED


def _turn_cost(turn: dict, *, catalog: dict | None = None) -> float | None:
    """Recorded cost when non-zero; otherwise the token-based estimate.

    Delegates to ``turn_cost.turn_cost`` — the pod's ONE per-turn cost rule —
    so the daemon's totals and the Usage page's totals never disagree.

    Returns ``None`` when the turn cannot be priced at all (audit B6). The old
    ImportError shim here returned ``0.0``, which was a SECOND source of the
    silent zero on exactly the cap path: a cap that cannot see spend must say
    so, not report ``$0.00``.
    """
    return _resolve_turn_cost(turn, catalog=catalog)
...
DaySpend = _live_spend.DaySpend


def load_today_spend_detail(
    ...
    *,
    ...
    """Today's spend for ``bot_id`` as a :class:`DaySpend`, or ``None`` when
    live-JSONL discovery failed (same "did not run" contract as
    :func:`load_today_spend`).

    This is the shape the cap path uses: it needs to tell "the bot spent
    $0.00 today" apart from "232 turns ran and I could not price any of them",
    which the bare float cannot express.

    The UTC-file / pod-local-day crossing this depends on lives in
    ``live_spend`` — see that module for why one local day needs two UTC
    files and why the turn's INSTANT, not its ``ts[:10]`` prefix, picks the
    bucket.
    """
    return _live_spend.load_day_spend_detail(
        bot_id, today, now=now, exempt_subkinds=exempt_subkinds,
        log=lambda m: _log(m, shared_dir),
    )
...
def load_today_spend(
    ...
    *,
    ...
    NOTE (audit B6): the float covers the turns that could be **priced**. A day
    containing unpriced turns still returns a float here, so this function
    cannot distinguish "spent nothing" from "could not price anything" — the
    cap path uses :func:`load_today_spend_detail` for exactly that reason.
    ...
    """
    detail = load_today_spend_detail(
        shared_dir, bot_id, today, now=now, exempt_subkinds=exempt_subkinds,
    )
    if detail is None:
        return None
...
def burst_window_spend(
    ...
    *,
    ...
    turns = _live_spend.load_live_turns(
        bot_id, days=days, end=now, log=lambda m: _log(m, shared_dir),
    )
    ...
    for t in turns:
        ...
        if window_start <= dt <= now:
            ...
            if cost:
                ...
                # Unpriced (audit B6) — cannot join the burst total. The daily
                # path emits the ``spend_unpriced_turns`` Signal for this bot,
                # so the operator learns about it there rather than through a
                # burst number that silently understates.
                unpriced += 1
    if unpriced:
        _log(
            f"[spend_alert] {bot_id}: burst window "
            f"{unpriced_note(unpriced)} — burst total is a floor",
            shared_dir,
        )
...
def _day_signature(signal_type: str, bot_id: str, today: date) -> str | None:
    """Per-(bot, day) Signal signature for ``signal_type``.

    Single source of truth shared by the per-day emitters
    (``emit_velocity_forecast_signal``, ``emit_unpriced_spend_signal``)
    and ``main()``'s end-of-run sweep — the same contract
    ``_burst_signature`` provides for ``cost_burst``. A day-bucketed
    signature REQUIRES this pairing: without the sweep, yesterday's
    signature is orphaned in firing/ forever at midnight rollover
    (signals from 2026-06-09 were still firing on 2026-08-30). Returns
    None if the schema import fails — the sweep treats that as "no kept
    signature" and the writer aborts entirely.
    """
    try:
        from schema.signal import make_signature  # type: ignore[import]
    except Exception:
        return None
    return make_signature(
        "spend_alert", signal_type, f"{bot_id}/{today.isoformat()}"
    )


def _self_failure_signature(now: datetime) -> str | None:
    """Per-day signature for the pod-scoped ``self_failure_jsonl_discovery``
    Signal. Shared by ``_emit_self_failure_signal`` and ``main()``'s sweep
    (same contract as ``_day_signature``, minus the bot key — this Signal
    covers the whole pod)."""
    try:
        from schema.signal import make_signature  # type: ignore[import]
    except Exception:
        return None
    return make_signature(
        "spend_alert", "self_failure_jsonl_discovery", now.date().isoformat()
    )
...
def emit_burst_signal(
    *,
    ...
    try:
        ...
        _log(f"[spend_alert] signals import failed; burst Signal skipped: {exc}", shared_dir)
        return None

    hour_key = _hour_bucket(now)
    signature = _burst_signature(bot_id, now)
    if signature is None:
        _log(f"[spend_alert] make_signature unavailable; burst Signal skipped", shared_dir)
        return None
    ...
    try:
        ...
        _log(f"[spend_alert] signals.observe({bot_id}) raised: {exc}", shared_dir)
        return None


def _local_day_start_utc(day: date, tz=None) -> datetime:
    """The UTC instant pod-local ``day`` begins. Fallback mirrors ``_pod_tz``."""
    try:
        from pod_time import pod_local_day_start_utc  # type: ignore[import]
        return pod_local_day_start_utc(day, tz)
    except ImportError:
        zone = tz if tz is not None else _live_spend.pod_tz_or_local()
        return datetime.combine(day, dt_time(0, 0), tzinfo=zone).astimezone(timezone.utc)


def _projected_full_day_spend(
    spend_usd: float, now: datetime, *, day_start: datetime,
) -> tuple[float, float]:
    ...
    ``day_start`` is the UTC instant the POD-LOCAL day began, and it is
    required rather than derived from ``now``: ``spend_usd`` is a pod-local
    day total, so dividing it by UTC hours elapsed mixes two calendars. On a
    Pacific pod at 21:00 local that read 4h elapsed instead of 21h and
    over-projected by ~5×.
    """
    hours_elapsed = max(1.0, (now - day_start).total_seconds() / 3600.0)
    projected = spend_usd * (24.0 / hours_elapsed)
    return projected, hours_elapsed


def _velocity_forecast_applies(
    *, spend_usd: float, cap_usd: float, warn_fraction: float, now: datetime,
    day_start: datetime,
) -> bool:
    """True iff the ``cost_velocity_forecast`` condition holds right now.

    Single source of truth for the predicate, shared by
    ``emit_velocity_forecast_signal`` (which writes the Signal) and
    ``main()`` (which records the signature in this tick's keep-set) —
    the same pairing ``_day_signature`` provides for the signature.

    The keep-set MUST be driven by this predicate rather than by the
    emitter's return value: a ``None`` return also covers "the store
    write raised", and dropping the signature on a transient
    ``observe()`` failure lets the sweep resolve a Signal whose
    condition is still true, pushing a false "Cleared …" on exactly the
    tick the store misbehaved.
    """
    if cap_usd <= 0:
        return False
    projected, _hours = _projected_full_day_spend(
        spend_usd, now, day_start=day_start,
    )
    if projected / cap_usd < warn_fraction:
        return False
    # Actuals already over the cap: the breaker path owns it, so the
    # projection Signal is genuinely not applicable (condition cleared).
    return spend_usd < cap_usd
...
def emit_velocity_forecast_signal(
    *,
    ...
    """
    day_start = _local_day_start_utc(today)
    if not _velocity_forecast_applies(
        spend_usd=spend_usd, cap_usd=cap_usd,
        warn_fraction=warn_fraction, now=now, day_start=day_start,
    ):
        # No cap, projection back under the warn fraction, or actuals
        # already over the cap (the breaker path owns that one).
        return None
    projected, hours_elapsed = _projected_full_day_spend(
        spend_usd, now, day_start=day_start,
    )
    ...
    try:
        ...
        return None
    signature = _day_signature("cost_velocity_forecast", bot_id, today)
    if signature is None:
        _log("[spend_alert] make_signature unavailable; velocity Signal skipped", shared_dir)
        return None
...
def _dispatch(
    *,
    ...
    try:
        ...
        _log(f"[spend_alert] dispatcher import failed; alert dropped: {exc}", shared_dir)
        return False
    ...
    try:
        ...
        _log(f"[spend_alert] dispatcher.send raised; alert dropped: {exc}", shared_dir)
        return False
...
def send_alert(
    ...
    *,
    ...
    if sent:
        _log(f"[spend_alert] {bot_id}: alert sent (${amount:.2f} > ${threshold:.2f})", shared_dir)
    return sent
...
# ── Daily cap verdict (audit B6 tri-state) ────────────────────────────────────


def daily_cap_decision(
    day: "DaySpend | None",
    *,
    threshold: float,
    ladder: dict[str, float | None] | None = None,
    accepted_usd: float = 0.0,
) -> dict:
    """The daily path's verdict for one bot-day — a pure function, so the
    decision can be asserted in a test rather than inferred from a printed
    number.

    Verdicts:
      * ``load_failed``  — live JSONL discovery failed; nothing was measured.
      * ``unmeasurable`` — turns ran that neither the pricing catalog nor the
        offline tables could price. ``usd`` is a FLOOR. Before audit B6 this
        case returned ``ok`` with ``$0.00`` against a day of real turns, and
        the cap sat green on a pod it could not see. It is now its own verdict:
        the day is never reported OK, and the daemon raises the
        ``spend_unpriced_turns`` Signal.
      * ``over_threshold`` / ``ok`` — the day was fully priced.

    ``tripped`` lists the ladder rungs the PRICED portion crosses. An
    unmeasurable day still trips on whatever it *could* price — a floor over
    the cap is still over the cap — but a rung is never tripped on the strength
    of an unknown: we do not invent a spend figure any more than we invent a
    price. Silence about the unknown is what B6 was; the Signal is the answer
    to it, not a manufactured trip.

    ``accepted_usd`` is the operator's reactivation marker
    (``spend_caps.read_accepted_through``): spend up to that figure has been
    accepted and the rungs measure from there. ``usd`` stays the RAW day
    total — that is what the operator means by "today's spend", and the
    alert lane still reports it — while ``effective_usd`` is what the rungs
    compare. With no marker the two are identical and this reduces exactly to
    the pre-marker behaviour. The marker moves the ORIGIN of the measurement;
    it never raises a cap, so a fresh spike of ``cap`` dollars past the
    acceptance point still trips (that is the test that keeps this honest).
    """
    if day is None:
        return {
            "verdict": "load_failed",
            "usd": None,
            "accepted_usd": max(accepted_usd, 0.0),
            "effective_usd": None,
            "unpriced_turns": 0,
            "unpriced_providers": (),
            "tripped": [],
    ...
    rungs = ladder or {}
    effective_usd = max(day.usd - max(accepted_usd, 0.0), 0.0)
    tripped = [
        rung for rung in ("tier_downgrade", "l1_breaker", "l2_breaker")
        if rungs.get(rung) is not None and effective_usd >= float(rungs[rung])  # type: ignore[arg-type]
    ]
    if day.unpriced_turns:
        verdict = "unmeasurable"
    elif day.usd > threshold:
    ...
    return {
        ...
        "unpriced_turns": day.unpriced_turns,
        "unpriced_providers": day.unpriced_providers,
        "tripped": tripped,
        "measurable": day.measurable,
    }


def emit_unpriced_spend_signal(
    *,
    shared_dir: Path,
    bot_id: str,
    day: "DaySpend",
    ...
    """Fire the ``spend_unpriced_turns`` Signal for a bot-day the cap could
    not fully measure (audit B6).

    This is the reported half of the tri-state contract: an unpriced turn is a
    state the operator is TOLD about, not one that quietly becomes ``$0.00``.
    Per-(bot, day) signature so it stays one open Signal across the day's
    ticks; it auto-resolves once the catalog can price the pod again.
    """
    if not day.unpriced_turns:
        return None
    try:
        from signals import store as signals_store  # type: ignore[import]
    except Exception as exc:
        _log(
            f"[spend_alert] signals import failed; unpriced Signal skipped: {exc}",
            shared_dir,
        )
        return None
    signature = _day_signature("spend_unpriced_turns", bot_id, today)
    if signature is None:
        _log("[spend_alert] make_signature unavailable; unpriced Signal skipped", shared_dir)
        return None
    note = unpriced_note(day.unpriced_turns, day.unpriced_providers)
    provs = ", ".join(day.unpriced_providers) or "unknown"
    # Alert when NOTHING could be priced (the cap is fully blind for this bot);
    # warn when only part of the day is missing.
    severity = "alert" if day.priced_turns == 0 else "warn"
    cap_line = (
        f"Its daily cap is ${cap_usd:.2f}."
        if cap_usd is not None else
        "No daily cap is configured for this bot."
    )
    body = (
        f"{bot_id} ran {day.unpriced_turns + day.priced_turns} turns today and "
        f"Evolve {note}. The ${day.usd:.2f} figure covers only the "
        f"{day.priced_turns} turn(s) it could price, so it is a FLOOR, not the "
        f"day's spend. {cap_line} The cap cannot enforce what it cannot "
        f"measure — this Signal is the honest state, in place of the $0.00 "
        f"that a missing price used to read as."
    )
    try:
        sig = signals_store.observe(
            shared_dir,
            ...
            details={
                "bot_id": bot_id,
                "unpriced_turns": day.unpriced_turns,
                "priced_turns": day.priced_turns,
                "unpriced_providers": list(day.unpriced_providers),
                "priced_usd_floor": round(day.usd, 4),
                "cap_usd": cap_usd,
                "today_iso": today.isoformat(),
                "vector": "cost",
                "magnitude": 3 if severity == "alert" else 2,
                "what_it_means": (
                    f"Evolve has no price for {provs}, so today's spend for "
                    f"{bot_id} is unknown and the daily cap is not enforcing "
                    f"on those turns."
                ),
                "fix_steps": (
                    "1. Check the pricing catalog exists and is fresh:\n"
                    "   {shared_dir}/model-pricing.json (refreshed by the "
                    "model-discovery sweep).\n"
                    "2. If the file is missing or stale, run the discovery "
                    "sweep so the catalog is fetched.\n"
                    f"3. If the catalog is fresh but still has no record for "
                    f"{provs}, that provider is not in the upstream pricing "
                    "catalogs — record the cost on the turn at the gateway, or "
                    "treat this bot's cap as unenforced until it is.\n"
                    "4. Do NOT read the displayed total as the real spend "
                    "while this Signal is open."
                ),
            },
        )
        return sig.id
        ...
        _log(
            f"[spend_alert] signals.observe({bot_id} unpriced_turns) "
            f"raised: {exc}",
            shared_dir,
        )
        return None
...
class _LadderUnavailable(Exception):
    """The budget ladder could not be READ this tick.

    Distinct from a ladder that resolved to all-``None`` (the operator
    configured no caps) — that is a real answer. Raised only by
    ``_resolve_per_bot_caps(..., strict=True)``; the default
    (``strict=False``) keeps the historical all-None fallback for the
    enforcement callers that cannot act on the distinction anyway.
    """


def _empty_ladder() -> dict[str, float | None]:
    """A fresh all-``None`` ladder — "no enforcement at any rung"."""
    return {rung: None for rung in _LADDER_RUNGS}
...
def _resolve_per_bot_caps(
    ...
    ``strict=True`` raises :class:`_LadderUnavailable` instead of returning
    the all-None fallback when the reader could not be consulted at all
    (no shared_dir, BE config import/load failure). Callers that only
    enforce want the fallback — "could not read" and "no cap configured"
    both mean "do not trip a breaker". Callers that decide whether a
    condition was OBSERVED (the ``cost_velocity_forecast`` sweep in
    ``main()``) must tell the two apart: resolving a live Signal because
    the ladder was unreadable is a false all-clear.
    ...
    if shared_dir is None:
        if strict:
            raise _LadderUnavailable("no shared_dir")
        return empty
    try:
        ...
        if strict:
            raise _LadderUnavailable(f"better-engine config unreadable: {exc}") from exc
        return empty
...
def _read_accepted_usd(shared_dir: Path, bot_id: str, today: date) -> float:
    """This bot's accepted-spend origin for ``today``, or 0.0.

    0.0 on every failure path — no marker, an unreadable one, or an
    unimportable ``spend_caps``. That is the pre-marker behaviour
    (measure the raw window), so a broken read can only make the breaker
    MORE willing to trip, never less. The inverse default would turn an
    I/O glitch into an open-ended pass on a live bleed.
    """
    try:
        ...
        return 0.0
    try:
        marker = _spend_caps.read_accepted_through(shared_dir, bot_id, today)
    except Exception as exc:  # noqa: BLE001
        _log(
            f"[spend_alert] {bot_id}: acceptance marker unreadable ({exc}) — "
            f"measuring the raw window",
            shared_dir,
        )
        return 0.0
    return marker.accepted_usd if marker is not None else 0.0


def _pod_budget_defaults(shared_dir: Path | None) -> dict | None:
    """``better-engine-config.json::pod_defaults.budget``, or None.

    The rung half of the action resolution. Read here so the enforcer feeds
    ``resolve_spend_cap_action`` the SAME second argument ``GET
    /api/spend-caps`` feeds it (``spend_caps.get_caps_config`` passes the
    pod budget); without it the resolver is one function with two answers —
    a pod whose 2026-06 normalization left the choice as a
    ``tier_downgrade_usd`` rung read as ``checkpoint`` here and
    ``downgrade-tier`` on the Cost page.

    Unreadable BE config returns None, which is the pre-existing behaviour
    (network-only resolution): an enforcement tick must not die because the
    config file is momentarily unreadable.
    """
    if shared_dir is None:
        return None
    try:
        from better_engine_config import load as _load_be  # type: ignore[import]
        budget = _load_be(shared_dir).pod_defaults.get("budget")
    except Exception as exc:  # noqa: BLE001
        _log(f"[spend_alert] pod budget defaults unreadable ({exc})", shared_dir)
        return None
    return budget if isinstance(budget, dict) else None


def _resolve_spend_cap_action(
    network: dict, shared_dir: Path | None = None,
) -> str:
    """What a tripped daily cap does on this pod.

    Delegates to ``spend_caps.resolve_spend_cap_action`` — the ONE reader
    shared with ``GET /api/spend-caps``, so the enforcer and the operator's
    view of it cannot disagree. That equality only holds when both callers
    pass the same inputs, so ``shared_dir`` is threaded through to supply
    ``pod_budget=`` exactly as the API reader does. Lazy import (matching
    this module's other ``spend_caps`` touch points), and a missing module
    falls back to the documented default rather than crashing the tick.
    """
    try:
        import spend_caps as _spend_caps  # type: ignore[import]
    except ImportError:
        return "checkpoint"
    try:
        return _spend_caps.resolve_spend_cap_action(
            network, pod_budget=_pod_budget_defaults(shared_dir),
        )
    except Exception as exc:  # noqa: BLE001
        _log(f"[spend_alert] spendCapAction resolve failed ({exc}) — checkpoint", shared_dir)
        return "checkpoint"


def _checkpoint_increment_usd(cap: float) -> float | None:
    """The "+$X" a continue answer grants, or None when unresolvable.

    Same lazy-import seam as :func:`_resolve_spend_cap_action`; returning
    None just means the trip copy offers "raise today's cap" without a
    number rather than inventing one.
    """
    try:
        import spend_caps as _spend_caps  # type: ignore[import]
        return _spend_caps.checkpoint_increment_usd(cap)
    except Exception:  # noqa: BLE001
        return None


# ── 80% soft warning (D-CC4) ─────────────────────────────────────────────────

# Fraction of the cap at which the operator gets a heads-up in the ALERT
# lane (never a page — the principle explicitly blesses soft warnings at
# 80/90/100 as hygiene, and "100 means stop, not warn harder").
SOFT_CAP_WARN_FRACTION = 0.80


def _soft_cap_flag_path(shared_dir: Path, bot_id: str, today_iso: str) -> Path:
    return shared_dir / "alerts" / f"softcap-{bot_id}-{today_iso}.flag"


def maybe_warn_soft_cap(
    *,
    bot_id: str,
    spend: float,
    cap: float | None,
    ...
    """Warn once per bot per day when spend crosses 80% of the L1 cap.

    D-CC4 keeps $20/bot/day — the number was not the 2026-09-03 failure —
    and adds the principle-sanctioned soft warning "so the conversation
    happens before the trip". Alert lane, not a page: the catalog entry is
    a WARNING that digests by default.

    Returns True iff a warning was sent this call. No-ops when no cap is
    configured, when spend is below the fraction, when the cap has already
    tripped (the trip is louder and says more), or when today's flag file
    already exists.

    The flag is written only on a SENT dispatch, matching ``write_flag`` —
    a suppressed or failed send leaves the flag absent so the next tick can
    retry rather than silently swallowing the only warning of the day.
    """
    if not cap or cap <= 0 or spend < cap * SOFT_CAP_WARN_FRACTION:
        return False
    # Already over the cap, or already tripped: the trip event owns this
    # bot-day. A "you're at 80%" line under a "BREAKER TRIPPED" line is
    # noise that makes the real message harder to find.
    if spend >= cap or _breaker_already_tripped(shared_dir, bot_id):
        return False
    flag = _soft_cap_flag_path(shared_dir, bot_id, today_iso)
    if flag.exists():
        return False
    sent = _dispatch(
        shared_dir=shared_dir,
        network=network,
        dedup_key=f"spend_alert/softcap/{bot_id}/{today_iso}",
        severity_name="warning",
        catalog_event="cost.soft_cap_warning",
        payload={
            "bot_id": bot_id,
            "spend": round(spend, 2),
            "cap": round(float(cap), 2),
            "pct": int(round(spend / float(cap) * 100)),
        },
    )
    if not sent:
        return False
    try:
        flag.parent.mkdir(parents=True, exist_ok=True)
        flag.write_text(
            json.dumps({"bot_id": bot_id, "spend": spend, "cap": cap}),
            encoding="utf-8",
        )
    except OSError as exc:
        # The warning went out; losing the dedup flag costs at most one
        # duplicate on the next tick. Never fail the tick over it.
        _log(f"[spend_alert] {bot_id}: soft-cap flag write failed: {exc}", shared_dir)
    _log(
        f"[spend_alert] {bot_id}: SOFT CAP warning sent "
        f"(${spend:.2f} ≥ {int(SOFT_CAP_WARN_FRACTION * 100)}% of ${cap:.2f})",
        shared_dir,
    )
    return True
...
def _write_spend_cap_flag(
    ...
    action: str, spend_total: float | None = None,
) -> None:
    ...
    two consumers the breaker record alone doesn't reach:
      ...
      * the OpenClaw plugin — ``ModelRouter.isSpendCapActive`` (in-band
        per-turn downgrade to the ``fast`` role, ONLY under the explicit
        ``downgrade-tier`` action) and, under the default ``checkpoint``
        action, the ``before_agent_run`` gate that reads the cap and spend
        for the fixed cap-reached reply.
    ...
    ``action`` comes from ``spend_caps.resolve_spend_cap_action`` — the one
    reader shared with ``GET /api/spend-caps``, so the operator-visible
    action and the enforced one cannot diverge. It used to be pinned to
    ``"downgrade-tier"``; D-CC2 makes ``checkpoint`` the default and leaves
    ``downgrade-tier`` reachable as an explicit choice. The heavier legacy
    actions (``pause-crons``, ``suspend-bot``) are owned by the L2 rung.
    ...
    try:
        ...
        _log(
            ...
            f"enforcement flag not written; the plugin will not see the cap "
            f"(L1 breaker is still tripped)",
            shared_dir,
        )
        return
    day = date.fromisoformat(today_iso)
    try:
        accepted = _spend_caps.read_accepted_through(shared_dir, bot_id, day)
    except Exception:  # noqa: BLE001
        accepted = None
    try:
        _spend_caps.write_enforcement_flag(
            shared_dir, bot_id, action, spend, cap, day,
            accepted=accepted, spend_total=spend_total,
        )
    except Exception as exc:  # noqa: BLE001
        _log(
            f"[spend_alert] {bot_id}: enforcement flag write failed: {exc}. "
            f"L1 breaker is tripped but the plugin will not see the cap "
            f"(action={action}).",
            shared_dir,
        )


def _pod_local_date(iso_ts: object) -> str | None:
    """The pod-local calendar date of an ISO-8601 timestamp, or None.

    Breaker records stamp ``tripped_at`` in UTC; caps roll at the pod's
    LOCAL midnight (the same boundary ``pod_today_str`` and the spend-caps
    flag filename use). Comparing the raw UTC date prefix would move the
    boundary by the pod's UTC offset — the ``turn_files_are_utc_named``
    shape. Returns None for an absent or unparseable stamp.
    """
    ...
    try:
        ...
        tz = None
    return (dt.astimezone(tz) if tz is not None else dt.astimezone()).date().isoformat()


def _tripped_before_today(rec: object, today_iso: str) -> bool:
    """True iff ``rec`` was tripped on an earlier pod-local day.

    An absent or unparseable ``tripped_at`` returns False: "cannot tell"
    must never reset a live breaker.
    """
    stamped = _pod_local_date(getattr(rec, "tripped_at", None))
    return stamped is not None and stamped < today_iso
...
def _trip_breaker_for_cost_cap(
    *,
    ...
    ``spend`` is spend measured from the operator's acceptance marker —
    the number that actually crossed ``cap``. ``spend_total`` is the raw
    day total when the two differ; it is recorded on the enforcement flag
    so the card can say "accepted $X at HH:MM; counting from then" instead
    of leaving an operator to reconcile "$5.20 >= $5.00" against a Cost
    page showing $32.69 for the day.
    ...
    One exception (D-CC3): a tripped breaker whose checkpoint an owner
    answered ``continued`` carries a granted increment on today's cap.
    Once spend crosses that RAISED ceiling the trip re-fires — a fresh
    ``pending`` checkpoint at the higher number — because the grant was
    "+$X until the day boundary", not "unlimited for the rest of today".
    The grant is carried forward by ``store.trip``.

    Sequence:
      1. Write the breaker record (24h TTL, initiated_by="auto:spend_alert"),
         arming ``checkpoint="pending"`` when the resolved action is the
         checkpoint.
    ...
    if _breaker_already_tripped(shared_dir, bot_id):
        existing = None
        try:
            existing = _bstore.read_trip(shared_dir, bot_id, "cost")
        except Exception as exc:  # noqa: BLE001
            _log(f"[spend_alert] {bot_id}: breaker re-read failed: {exc}", shared_dir)

        # ── Day boundary, not the TTL (D-CC3) ──────────────────────────────
        # The grant is "+$X until the day boundary". The breaker record's
        # 24h TTL is a DIFFERENT clock: a 21:18 trip is still unexpired at
        # 21:18 tomorrow, hours after the spend-caps flag rolled and today's
        # spend restarted at $0. Carrying that record into a new pod-local
        # day would let yesterday's "+$10" raise today's ceiling to $30 and
        # keep a stale hold in force. A record stamped on an earlier
        # pod-local day is therefore reset and re-tripped from scratch.
        day_boundary_reset = False
        if existing is not None and _tripped_before_today(existing, today_iso):
            _log(
                f"[spend_alert] {bot_id}: L1 cost breaker was tripped on "
                f"{_pod_local_date(existing.tripped_at)} and today is "
                f"{today_iso} — resetting at the day boundary and tripping "
                f"fresh (no grant carried)",
                shared_dir,
            )
            try:
                _bstore.reset(
                    shared_dir=shared_dir, scope=bot_id, breaker_type="cost",
                    initiated_by="auto:day_boundary",
                    reason=f"pod-local day rolled over to {today_iso}",
                )
            except Exception as exc:  # noqa: BLE001
                _log(
                    f"[spend_alert] {bot_id}: day-boundary breaker reset "
                    f"failed ({exc}); leaving yesterday's record in place",
                    shared_dir,
                )
            else:
        ...
        if not day_boundary_reset:
            granted = (
                (existing.checkpoint_increment_usd or 0.0)
                if existing is not None else 0.0
            )
            raised = cap + granted
            if not (
                existing is not None
                and existing.checkpoint == "continued"
                and spend >= raised
            ):
                _log(
                ...
                return
            effective_cap = raised
            _log(
                f"[spend_alert] {bot_id}: continued checkpoint ceiling "
                f"${raised:.2f} crossed (${spend:.2f}) — re-tripping with a "
                f"fresh checkpoint",
                shared_dir,
            )

    action = _resolve_spend_cap_action(network, shared_dir)
    reason = (
        f"per-bot daily cap exceeded: ${spend:.2f} ≥ ${effective_cap:.2f}"
    )
    try:
        rec = _bstore.trip(
            shared_dir=shared_dir,
            ...
            reason=reason,
            checkpoint="pending" if action == "checkpoint" else None,
        )
    except Exception as exc:
        _log(
    ...
    _write_spend_cap_flag(
        shared_dir, bot_id, spend=spend, cap=effective_cap,
        today_iso=today_iso, action=action, spend_total=spend_total,
    )
    ...
    try:
        ...
        enforce_result = _be.enforce_trip(
            ...
            # Structured inputs for the trip copy: it must name every
            # action it performed and, under the checkpoint, the two
            # choices with the actual increment (D-CC2/D-CC3).
            cap_action=action, cap_usd=effective_cap, spend_usd=spend,
            # BASE cap, never ``effective_cap``: the grant is "+50% of the
            # cap" (D-CC3), a fixed step. Computing it from the raised
            # ceiling compounds it — +10, +15, +22.50 across three
            # checkpoints on a $20 cap.
            increment_usd=_checkpoint_increment_usd(cap),
        )
    except Exception as exc:
        enforce_raised = True
        _log(
    ...
    base_reason = f"Spent ${spend:.2f} of ${effective_cap:.2f} daily cap"
    if enforce_raised:
        reason_text = (
    ...
    _log(
        f"[spend_alert] {bot_id}: HARD CAP (per-bot) ${spend:.2f} ≥ "
        f"${effective_cap:.2f} — L1 cost breaker tripped "
        f"(action={action}, checkpoint={rec.checkpoint or 'none'}, "
        f"trip_id={rec.trip_id[:8]}, expires={rec.expires_at})",
        shared_dir,
    )
...
def _apply_tier_downgrade(
    *, bot_id: str, shared_dir: Path, network: dict, spend: float,
    ...
    try:
        ...
        _log(f"[spend_alert] {bot_id}: tier_downgrade flag write failed: {exc}", shared_dir)
...
def main() -> None:
    ...
    # Per-tick state for the post-loop sweeps that auto-archive stale
    # Signals. See ``_sweep_stale_signals``: bots whose discovery
    # succeeded but whose condition no longer holds this tick get their
    # stranded Signals archived (previous hour bucket for cost_burst;
    # previous day — or a cleared same-day condition — for the per-(bot,
    # day) types). Bots whose discovery failed are NOT swept — their
    # Signal history is untouched until the next clean tick.
    kept_burst_sigs: set[str] = set()
    checked_bots: set[str] = set()
    # One (checked-bots, keep-set) pair PER TYPE, not per bucket shape: a
    # bot is swept for a type only on a tick where that type's own
    # preconditions were actually evaluated. Bundling the two per-(bot,
    # day) types under one gate was the 2026-08-30 review's concern 1 —
    # they do not share inputs.
    #   spend_unpriced_turns: depends only on the daily JSONL read, so
    #     day_checked_bots (day_spend is not None) IS its precondition.
    kept_unpriced_sigs: set[str] = set()
    day_checked_bots: set[str] = set()
    #   cost_velocity_forecast: additionally needs the budget ladder to
    #     have been READ — an unreadable ladder looks exactly like "no cap
    #     configured" (all-None), and sweeping on it clears a live Signal
    #     while the bot is still burning. A ladder that resolved to no cap
    #     IS a real answer and still sweeps (the forecast is meaningless
    #     without a cap, so a Signal from before the cap was removed
    #     should resolve).
    kept_velocity_sigs: set[str] = set()
    velocity_checked_bots: set[str] = set()
    ...
    for bot_id in members:
        ...
        burst_cost, burst_turns = burst_window_spend(
            shared_dir, bot_id, now=now, window_minutes=burst_window_min,
        ...
        day_spend = load_today_spend_detail(
        ...
        if day_spend is None:
            ...
            continue
        # Daily JSONL read succeeded — eligible for the per-(bot, day)
        # sweep below. Tracked regardless of whether either per-day
        # Signal fires: a bot whose condition cleared (or whose Signal
        # is from a previous day) is exactly what the sweep archives.
        day_checked_bots.add(bot_id)
        spend = day_spend.usd
        # The operator's "count from now" origin for today (0.0 when the
        # bot has not been reactivated). ``spend`` stays the raw day total
        # — the alert lane, the velocity forecast and the operator-facing
        # print all mean "today's spend" by it — while ``effective_spend``
        # is what every REMEDIATION rung measures. Resolved once per bot
        # per tick so the ladder, the soft warning and the trip copy can
        # never disagree about where the window starts.
        accepted_usd = _read_accepted_usd(shared_dir, bot_id, today)
        effective_spend = max(spend - accepted_usd, 0.0)
        if accepted_usd > 0:
            _log(
                f"[spend_alert] {bot_id}: ${accepted_usd:.2f} of today's "
                f"${spend:.2f} was accepted at reactivation — enforcing on "
                f"${effective_spend:.2f} measured since then",
                shared_dir,
            )
        ...
        # anywhere) ladder.
        #
        # ``strict=True`` so an unreadable ladder is distinguishable from a
        # ladder that legitimately carries no caps. Enforcement below is
        # unchanged either way (an all-None ladder trips nothing); only the
        # velocity sweep needs the distinction. A raise from the reader —
        # including one out of ``resolve_budget_ladder`` itself — no longer
        # aborts the whole tick: this bot is simply not velocity-checked.
        try:
            ladder = _resolve_per_bot_caps(
                bot_id, shared_dir, now=now, strict=True,
            )
            ...
            _log(
                f"[spend_alert] {bot_id}: budget ladder unreadable ({exc}) — "
                f"enforcing nothing and NOT sweeping velocity Signals this tick",
                shared_dir,
            )
        if ladder_resolved:
            # Eligible for the cost_velocity_forecast sweep below, whether or
            # not a cap is actually set: "no cap" is an observed answer.
            velocity_checked_bots.add(bot_id)
        # The L1 threshold is the cap of record for the velocity forecast.
        per_bot_cap = ladder["l1_breaker"]

        # ── Audit B6: the day's verdict, including "I could not price it" ──
        # ``spend`` above is the PRICED floor. When part (or all) of the day is
        # unpriced the ladder still enforces on that floor — a floor over the
        # cap is still over the cap — but the day is never reported OK, and the
        # operator gets a Signal saying exactly how many turns are missing.
        decision = daily_cap_decision(
            day_spend, threshold=threshold, ladder=ladder,
            accepted_usd=accepted_usd,
        )
        if decision["verdict"] == "unmeasurable":
            _log(
                f"[spend_alert] {bot_id}: UNMEASURABLE — "
                f"{unpriced_note(day_spend.unpriced_turns, day_spend.unpriced_providers)}; "
                f"${spend:.4f} is a floor over {day_spend.priced_turns} priced turn(s)",
                shared_dir,
            )
            # Keep today's signature so the post-loop sweep doesn't archive
            # the Signal this tick just (re-)observed. The keep records the
            # PREDICATE — this bot-day has unpriced turns, which is exactly
            # what ``verdict == "unmeasurable"`` means — not the write's
            # outcome: a transient observe() raise must not hand the sweep a
            # still-true Signal to resolve (2026-08-30 review, concern 2).
            sig_keep = _day_signature("spend_unpriced_turns", bot_id, today)
            if sig_keep is not None:
                kept_unpriced_sigs.add(sig_keep)
            emit_unpriced_spend_signal(
                shared_dir=shared_dir,
                bot_id=bot_id,
                day=day_spend,
                cap_usd=per_bot_cap,
        ...
        # Which rungs the day crosses is decided ONCE, in
        # ``daily_cap_decision`` — the actuators below just act on that list,
        # so the verdict a test asserts is the verdict the daemon enforces.
        tripped = decision["tripped"]

        # 1. tier_downgrade: switch primary model for the rest of the day.
        # The ``is not None`` clauses are type-narrowing no-ops — a rung only
        # lands in ``tripped`` when its cap is set — kept so the actuator's
        # ``cap=`` argument stays a plain float.
        td_cap = ladder["tier_downgrade"]
        if "tier_downgrade" in tripped and td_cap is not None:
            _apply_tier_downgrade(
                bot_id=bot_id, shared_dir=shared_dir, network=config,
                spend=effective_spend, cap=td_cap, today_iso=str(today),
            )

        # 1b. Soft warning at 80% of the L1 cap (D-CC4) — alert lane, once
        # per bot per day, so the operator can act BEFORE the trip holds a
        # conversation. Self-gates on "already over" / "already tripped".
        maybe_warn_soft_cap(
            bot_id=bot_id, spend=effective_spend, cap=per_bot_cap,
            shared_dir=shared_dir, network=config, today_iso=str(today),
        )

        # 2. L1 breaker: pause heartbeat + background; user chat keeps working.
        if "l1_breaker" in tripped and per_bot_cap is not None:
        ...
        if "l2_breaker" in tripped and l2_cap is not None:
            _trip_l2_breaker_for_cost_cap(
                bot_id=bot_id, spend=effective_spend, cap=l2_cap,
                shared_dir=shared_dir, network=config, today_iso=str(today),
            )
        ...
        if velocity_cap > 0 and spend > 0:
            velocity_warn_fraction = float(
                thresholds_cfg.get("velocityForecastWarnFraction", 0.70)
            )
            # Keep on the PREDICATE, not on the emitter's return (see the
            # unpriced keep above). ``_velocity_forecast_applies`` is the
            # same gate the emitter runs, so a False here means the
            # condition genuinely cleared — projection back under the warn
            # fraction, or actuals over the cap — and the sweep resolving
            # the same-day Signal is the wanted behaviour.
            if _velocity_forecast_applies(
                spend_usd=spend, cap_usd=velocity_cap,
                warn_fraction=velocity_warn_fraction, now=now,
                day_start=_local_day_start_utc(today),
            ):
                sig_keep = _day_signature(
                    "cost_velocity_forecast", bot_id, today
                )
                if sig_keep is not None:
                    kept_velocity_sigs.add(sig_keep)
            emit_velocity_forecast_signal(
                shared_dir=shared_dir,
                bot_id=bot_id,
                spend_usd=spend,
                cap_usd=velocity_cap,
                warn_fraction=velocity_warn_fraction,
                today=today,
                now=now,
            )

        if decision["verdict"] == "unmeasurable":
            # Not OK, not over-threshold — unknown. Printing OK here against a
            # day of real-but-unpriced turns is precisely the B6 failure.
            print(
                f"[spend_alert] {bot_id}: UNMEASURABLE "
                f"(${spend:.4f} priced floor; "
                f"{unpriced_note(day_spend.unpriced_turns, day_spend.unpriced_providers)})"
            )
            continue
    ...
    # ── Auto-archive stale Signals (all bucketed types) ───────────────
    # Comprehensive sweep contract: any firing/snoozed Signal from a bot
    # we successfully checked, whose signature isn't in this tick's
    # keep-set, no longer holds and should be archived. Without this, a
    # bucket rollover (hour boundary for cost_burst, midnight for the
    # per-(bot, day) types) leaves the old-bucket Signal stranded in
    # firing/ forever once the condition stops — cost_burst got this
    # sweep on 2026-06-06 ("2 findings" duplicate); the 2026-08-30
    # alerts-noise review found the lesson was never generalized, and
    # cost_velocity_forecast Signals from 2026-06-09 were still firing.
    #
    # One sweep per TYPE, each with the bot set whose preconditions that
    # type actually had this tick: a bot is swept for a type only where
    # that type's condition was genuinely evaluated and found false —
    # never where it could not be evaluated at all.
    if checked_bots:
        _sweep_stale_signals(
            shared_dir,
            types={"cost_burst"},
            bot_ids=checked_bots,
            kept_signatures=kept_burst_sigs,
            reason="auto-resolve: burst window no longer above threshold",
        )
    if day_checked_bots:
        _sweep_stale_signals(
            shared_dir,
            types={"spend_unpriced_turns"},
            bot_ids=day_checked_bots,
            kept_signatures=kept_unpriced_sigs,
            reason="auto-resolve: the day's turns are priced, or day rolled over",
        )
    if velocity_checked_bots:
        _sweep_stale_signals(
            shared_dir,
            types={"cost_velocity_forecast"},
            bot_ids=velocity_checked_bots,
            kept_signatures=kept_velocity_sigs,
            reason=(
                "auto-resolve: projection back under the warn fraction, "
                "or day rolled over"
            ),
        )
    ...
    kept_self_failure_sigs: set[str] = set()
    if discovery_failures:
        _emit_self_failure_signal(shared_dir, discovery_failures, now)
        sig_keep = _self_failure_signature(now)
        if sig_keep is not None:
            kept_self_failure_sigs.add(sig_keep)
    # Companion sweep for the per-day self-failure signature: runs even
    # when today's Signal fired (yesterday's still resolves), and with
    # no bot filter — the Signal is pod-scoped (bot_id=None), which a
    # bot_ids filter would never match.
    _sweep_stale_signals(
        shared_dir,
        types={"self_failure_jsonl_discovery"},
        bot_ids=None,
        kept_signatures=kept_self_failure_sigs,
        reason="auto-resolve: JSONL discovery healthy or day rolled over",
    )
...
def _sweep_stale_signals(
    ...
    *,
    ...
    """Archive spend_alert Signals of ``types`` not in the keep-set.

    Wraps ``signals.store.sweep_resolve`` with the spend_alert producer
    + a type filter + an optional bot-id filter (``bot_ids=None`` for
    pod-scoped types whose Signals carry no ``bot_id``). Every bucketed
    signature this module emits (per-(bot, hour) cost_burst, per-(bot,
    day) cost_velocity_forecast / spend_unpriced_turns, per-day
    self_failure_jsonl_discovery) needs a companion sweep — without one,
    the bucket rollover orphans the old signature in firing/ forever.
    Failures are logged but never raised — a sweep miss leaves a stale
    Signal for one extra tick, which is recoverable; a thrown exception
    ...
    try:
        ...
        _log(
            ...
            f"stale {types_str} Signals NOT archived this tick: {exc}",
            shared_dir,
        )
        return
    try:
        ...
        _log(f"[spend_alert] sweep_resolve raised; continuing: {exc}", shared_dir)
        return
    if resolved:
        _log(
            f"[spend_alert] auto-archived {len(resolved)} stale {types_str} "
            f"Signal(s) for: "
            f"{', '.join(sorted({s.bot_id or 'pod' for s in resolved}))}",
            shared_dir,
        )
...
def _emit_self_failure_signal(
    ...
    try:
        ...
        _log(f"[spend_alert] self-failure Signal import failed: {exc}", shared_dir)
        return

    signature = _self_failure_signature(now)
    if signature is None:
        _log("[spend_alert] make_signature unavailable; self-failure Signal skipped", shared_dir)
        return
    ...
    try:
        ...
        _log(f"[spend_alert] self-failure Signal observe raised: {exc}", shared_dir)
...
def _weekly_spend(
    shared_dir: Path,  # noqa: ARG001 — kept for callers that pass it
    members: list[str],
    today: date,  # noqa: ARG001 — window is anchored on ``now``, see below
    *,
    now: datetime | None = None,
) -> tuple[float, dict[str, float | None]]:
    """Return (total_7d, {bot_id: spend}) over the last 7 POD-LOCAL days.

    A bot maps to ``None`` when live-JSONL discovery failed for it — "I
    could not look", rendered as ``n/a`` rather than folded into the total
    as $0.00. ``None`` is excluded from the total, so the headline figure
    is a floor when any bot is unreadable, never a confident understatement.

    This used to sum 7 days of ``{shared_dir}/metrics/<date>/<bot>.json``.
    Those files are written by ``measure.py`` from launchd at 01:00
    pod-local with ``--date`` defaulting to ``date.today()``, so each one
    holds only what had accrued in the first hour of the day it is named
    for, and nothing regenerates it. The partiality is uniform across the
    seven days, which is exactly what made it invisible: the weekly total
    looked plausible while being a fixed fraction of real spend. Measured
    on the mini for 2026-09-03, that fraction was 1.7% ($0.95 of $55.52
    pod-wide) — and this total is
    compared against a real dollar threshold (``weeklySpendAlertUsd``,
    default $20), which a 1.7%-scaled number can essentially never cross.

    The window is anchored on ``now`` rather than counted back from
    ``today`` because ``live_spend`` buckets by the pod-local day of each
    turn's instant; the two agree in production, and ``now`` is the one the
    bucketing actually uses.
    """
    per_bot: dict[str, float | None] = {}
    for bot_id in members:
        result = _live_spend.total_over_local_days(
            bot_id, days=7, now=now, log=lambda m: _log(m, shared_dir),
        )
        if result is None:
            _log(f"[spend_alert] weekly: live turn discovery failed for {bot_id}", shared_dir)
            per_bot[bot_id] = None
            continue
        usd, measurable = result
        if not measurable:
            _log(f"[spend_alert] weekly: {bot_id} has unpriced turns — "
                 f"${usd:.4f} is a floor", shared_dir)
        per_bot[bot_id] = round(usd, 4)
    total = sum(v for v in per_bot.values() if v is not None)
    return round(total, 4), per_bot


def _accuracy_lines(shared_dir: Path) -> list[str]:
    """The receipt's estimate-vs-bill and catalog-freshness lines.

    Always non-empty: a receipt that omits the comparison when nothing has
    been reconciled reads as "checked and fine", which is the confusion the
    reconcile exists to end. An unimportable reconciler says so rather than
    dropping the lines silently.
    """
    try:
        from cost_reconcile import receipt_lines  # type: ignore[import]
    except Exception as exc:  # noqa: BLE001 — analyzer path unavailable
        _log(f"[spend_alert] cost_reconcile unavailable: {exc}", shared_dir)
        return ["estimate vs provider bill: reconciler unavailable"]
    try:
        return receipt_lines(shared_dir)
    except Exception as exc:  # noqa: BLE001
        _log(f"[spend_alert] receipt_lines raised: {exc}", shared_dir)
        return ["estimate vs provider bill: reconcile unreadable"]


def _heartbeat_lines(shared_dir: Path, members: list[str]) -> list[str]:
    """The receipt's idle-heartbeat line (D-OH3).

    Always non-empty, for the same reason :func:`_accuracy_lines` is: on a
    pod where no bot has a ``HEARTBEAT.json`` yet, silence would read as
    "nothing to report" while the pod pays the full idle burn the due-check
    exists to remove.
    """
    try:
        from heartbeat_savings import receipt_lines as _hb_lines  # type: ignore[import]
    except Exception as exc:  # noqa: BLE001 — analyzer path unavailable
        _log(f"[spend_alert] heartbeat_savings unavailable: {exc}", shared_dir)
        return ["heartbeats: savings reader unavailable"]
    try:
        return _hb_lines(shared_dir, members, days=7)
    except Exception as exc:  # noqa: BLE001
        _log(f"[spend_alert] heartbeat receipt_lines raised: {exc}", shared_dir)
        return ["heartbeats: savings unreadable"]

def _cache_shape_lines(shared_dir: Path, members: list[str]) -> list[str]:
    """The receipt's cross-turn cache lines.

    The CROSS-turn hit rate, deliberately — not the headline figure the
    per-call surfaces report. On the measured day
    (internal/finding-cost-forensics-power-bot-2026-09-04.md §2) those two
    numbers were 79-87% and near-zero respectively, and only the second one
    was about money: the first counts one turn's thirteen internal calls
    reading the prefix its own first call had just written.

    Always non-empty for the same reason ``_accuracy_lines`` is — a receipt
    that drops the line when nothing was readable reads as "checked and fine".
    """
    try:
        from cache_shape import receipt_lines as _cache_receipt  # type: ignore[import]
    except Exception as exc:  # noqa: BLE001 — analyzer path unavailable
        _log(f"[spend_alert] cache_shape unavailable: {exc}", shared_dir)
        return ["cross-turn cache hit: measurement unavailable"]
    try:
        return _cache_receipt(members)
    except Exception as exc:  # noqa: BLE001
        _log(f"[spend_alert] cache_shape.receipt_lines raised: {exc}", shared_dir)
        return ["cross-turn cache hit: measurement failed"]


def emit_estimate_drift(
    *, shared_dir: Path, network: dict, iso_week: str,
) -> bool:
    """Raise the cost-estimate drift Signal + digest event for this week.

    Fires only when the most recent reconcile puts our estimate outside the
    0.9-1.1 band against the provider's own figure. Signature and dedup key
    are per ISO week, so a standing drift is one item a week rather than a
    stream; it auto-resolves the week after a reconcile lands back in band.

    Returns True when the operator message was SENT (the Signal is written
    either way — the Alerts page carries the state even when the digest
    suppresses the message).
    """
    try:
        from cost_reconcile import (  # type: ignore[import]
            drift_signal_body, latest_reconcile, ratio_out_of_band,
        )
    except Exception as exc:  # noqa: BLE001
        _log(f"[spend_alert] cost_reconcile unavailable; drift check skipped: {exc}", shared_dir)
        return False
    doc = latest_reconcile(shared_dir)
    if not doc:
        return False
    ratio = doc.get("ratio")
    if not ratio_out_of_band(ratio):
        return False
    day = (doc.get("days") or [{}])[0].get("date") or "?"
    estimate = float(doc.get("estimate_usd") or 0)
    billed = float(doc.get("console_usd") or 0)
    title, body = drift_signal_body(doc)
    try:
        from signals import store as signals_store  # type: ignore[import]
        signature = f"spend_alert:cost_estimate_drift:{iso_week}"
        signals_store.observe(
            shared_dir,
            signature=signature,
            producer="spend_alert",
            type="cost_estimate_drift",
            flavor="activity",
            severity="warn",
            scope="pod",
            title=title,
            ...
            details={
                ...
                "console_usd": round(billed, 4),
                "ratio": ratio,
                "vector": "cost",
                "magnitude": 2,
                "what_it_means": (
                    "Evolve's cost estimate and the provider's bill disagree "
                    "by more than 10%. The daily cap, the 80% warning, the "
                    "checkpoint message and the weekly receipt all read the "
                    "estimate."
                ),
                "fix_steps": (
                    "1. Check the pricing catalog is fresh:\n"
                    "   {shared_dir}/model-pricing.json (refreshed by the "
                    "model-discovery sweep).\n"
                    "2. Re-run the reconcile against a fresh console export:\n"
                    "   sudo evolve-admin cost reconcile --console-csv "
                    "costs.csv --provider <provider>\n"
                    "3. If the catalog has no row for the models this pod "
                    "runs, they are priced from the offline fallback table — "
                    "the estimate is only as good as that table until the "
                    "sweep can reach the catalog sources."
                ),
            },
        )
    except Exception as exc:  # noqa: BLE001
        _log(f"[spend_alert] signals.observe(cost_estimate_drift) raised: {exc}", shared_dir)
    return _dispatch(
        shared_dir=shared_dir,
        network=network,
        dedup_key=f"spend_alert/estimate_drift/{iso_week}",
        severity_name="warning",
        catalog_event="cost.estimate_drift",
        payload={
            "day": day,
            "estimate": estimate,
            "console": billed,
...
def _per_bot_receipt_lines(
    shared_dir: Path, per_bot: dict[str, float | None],
) -> list[str]:
    """Each bot's conversation line, then its housekeeping line.

    Compaction and the memory flush are turns nobody asked for; the receipt
    shows them apart from the conversation (brief
    compaction-and-memory-flush-on-cheap-rung §4). The weekly ``total`` stays
    every dollar — this splits a bot's number, it never drops part of it.
    """
    try:
        from housekeeping_cost import (  # type: ignore[import]
            housekeeping_over_local_days, receipt_bot_lines,
        )
    except Exception as exc:  # noqa: BLE001 — analyzer path unavailable
        _log(f"[spend_alert] housekeeping_cost unavailable: {exc}", shared_dir)
        return [
            f"  {b}: " + ("n/a (could not read turns)" if v is None else f"${v:.2f}")
            for b, v in sorted(per_bot.items())
        ]
    lines: list[str] = []
    for bot_id, total in sorted(per_bot.items()):
        hk = None
        if total is not None:
            try:
                hk = housekeeping_over_local_days(
                    bot_id, days=7, log=lambda m: _log(m, shared_dir),
                )
            except Exception as exc:  # noqa: BLE001 — one bot never sinks the receipt
                _log(f"[spend_alert] housekeeping for {bot_id} raised: {exc}", shared_dir)
        lines.extend(receipt_bot_lines(bot_id, total, hk))
    return lines
...
def _maybe_send_weekly_summary(
    ...
    _log(f"[spend_alert] weekly summary: ${total:.2f} over 7d (threshold ${weekly_threshold:.2f})", shared_dir)

    per_bot_breakdown = "\n".join(_per_bot_receipt_lines(shared_dir, per_bot))
    # Every figure above is an ESTIMATE. The receipt says how that estimate
    # last compared to the provider's own bill, or that nothing checked it.
    per_bot_breakdown = "\n".join(
        [per_bot_breakdown]
        + _accuracy_lines(shared_dir)
        # D-OH3: what the model-free due-check kept off the bill this week.
        + _heartbeat_lines(shared_dir, members)
        + _cache_shape_lines(shared_dir, members),
    )
    ...
    if sent:
        ...
        _log(f"[spend_alert] weekly summary sent for {iso_week}", shared_dir)
    # Independent of the summary's own send outcome: a suppressed receipt is
    # not a reason to withhold the drift warning, and the drift event carries
    # its own per-week dedup key.
    if emit_estimate_drift(
        shared_dir=shared_dir, network=network, iso_week=iso_week,
    ):
        _log(f"[spend_alert] estimate-drift warning sent for {iso_week}", shared_dir)
