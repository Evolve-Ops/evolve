...
def load_today_spend_detail(
    ...
    *,
    ...
    return _live_spend.load_day_spend_detail(
        bot_id, today, now=now, exempt_subkinds=exempt_subkinds, log=_log,
    )
...
def burst_window_spend(
    ...
    *,
    ...
    turns = _live_spend.load_live_turns(bot_id, days=days, end=now, log=_log)
...
def emit_velocity_forecast_signal(
    *,
    ...
    if signature is None:
        _log("[spend_alert] make_signature unavailable; velocity Signal skipped")
        return None
...
def emit_unpriced_spend_signal(
    *,
    ...
    if signature is None:
        _log("[spend_alert] make_signature unavailable; unpriced Signal skipped")
        return None
...
def _pod_budget_defaults(shared_dir: Path | None) -> dict | None:
    ...
    try:
        ...
        _log(f"[spend_alert] pod budget defaults unreadable ({exc})")
        return None
...
def _resolve_spend_cap_action(
    ...
    try:
        ...
        _log(f"[spend_alert] spendCapAction resolve failed ({exc}) — checkpoint")
        return "checkpoint"
...
def maybe_warn_soft_cap(
    *,
    ...
    try:
        ...
        _log(f"[spend_alert] {bot_id}: soft-cap flag write failed: {exc}")
    _log(
    ...
    *,
    ...
    if _breaker_already_tripped(shared_dir, bot_id):
        ...
        try:
            ...
            _log(f"[spend_alert] {bot_id}: breaker re-read failed: {exc}")
...
def _emit_self_failure_signal(
    ...
    if signature is None:
        _log("[spend_alert] make_signature unavailable; self-failure Signal skipped")
        return
    ...
    *,
    ...
    for bot_id in members:
        result = _live_spend.total_over_local_days(
            bot_id, days=7, now=now, log=_log,
        )
        if result is None:
            _log(f"[spend_alert] weekly: live turn discovery failed for {bot_id}")
            per_bot[bot_id] = None
            continue
...
def _accuracy_lines(shared_dir: Path) -> list[str]:
    ...
    try:
        ...
        _log(f"[spend_alert] cost_reconcile unavailable: {exc}")
        return ["estimate vs provider bill: reconciler unavailable"]
    try:
        return receipt_lines(shared_dir)
    except Exception as exc:  # noqa: BLE001
        _log(f"[spend_alert] receipt_lines raised: {exc}")
...
def _heartbeat_lines(shared_dir: Path, members: list[str]) -> list[str]:
    ...
    try:
        ...
        _log(f"[spend_alert] heartbeat_savings unavailable: {exc}")
        return ["heartbeats: savings reader unavailable"]
    try:
        return _hb_lines(shared_dir, members, days=7)
    except Exception as exc:  # noqa: BLE001
        _log(f"[spend_alert] heartbeat receipt_lines raised: {exc}")
...
def emit_estimate_drift(
    *, shared_dir: Path, network: dict, iso_week: str,
    ...
    try:
        ...
        _log(f"[spend_alert] cost_reconcile unavailable; drift check skipped: {exc}")
        return False
    ...
    try:
        ...
        _log(f"[spend_alert] signals.observe(cost_estimate_drift) raised: {exc}")
    return _dispatch(
        shared_dir=shared_dir,
        network=network,
...
def _maybe_send_weekly_summary(
    ...
    if emit_estimate_drift(
        ...
        _log(f"[spend_alert] estimate-drift warning sent for {iso_week}")
