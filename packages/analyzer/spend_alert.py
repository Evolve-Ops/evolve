...
def _resolve_per_bot_cap(
    ...
    *, now: datetime | None = None,
    ...
    Reads ``bots.<bot>.budget.per_bot_daily_hard_usd``. Returns the cap as a
    float, or None when unset / unparseable / ≤ 0 (zero / negative means
    "no cap" — same convention as the global ``dailySpendCapUsd``). When the
    bot has no explicit cap but is inside its new-bot activation window, the
    graduated new-bot default applies (see ``_resolve_per_bot_caps``).
...
def _resolve_per_bot_caps(
    ...
    Phase 6 spec: docs/spec-cost-caps-2026-06-05.md §"Enforcement
    implementation".
    """
    ...
    budget = be.bots.get(bot_id, {}).get("budget", {})
    l1 = _pos(budget.get("per_bot_daily_hard_usd"))
    if l1 is None:
        # New-bot graduated default: a freshly-created bot with no explicit
        # cap still gets a real L1 backstop ($10) during its activation
        # window, then falls back to None (current behavior) once mature.
        # This is the fix for the "cap backstop absent on new bots" incident
        # (the per-bot L1 trip is what actually pauses heartbeat + background
        # spend; without a per-bot value it never fired). Mature bots without
        # an explicit cap are unchanged — the pod default still governs them
        # via the Budget Hawk guardian veto + the pod-wide spend_caps path.
        # ``new_bot_graduated_hard_cap_usd`` guards its own parsing (returns
        # None on a missing / malformed / future stamp).
        l1 = be.new_bot_graduated_hard_cap_usd(bot_id, now=now)
    return {
        "daily_warn": _pos(budget.get("per_bot_daily_warn_usd")),
        "weekly_warn": _pos(budget.get("weekly_warn_usd")),
        "tier_downgrade": _pos(budget.get("tier_downgrade_usd")),
        "l1_breaker": l1,
        "l2_breaker": _pos(budget.get("l2_breaker_usd")),
        "per_session": _pos(budget.get("per_bot_session_cost_cap_usd")),
    }
...
def _trip_breaker_for_cost_cap(
    *,
    ...
    Sequence:
      ...
      2. Run synchronous enforcement (heartbeat-disable + gateway kickstart).
      3. Send the Telegram alert via the existing ``cost.hard_cap_hit``
         catalog event (operator already subscribed; same body shape as
         the legacy spend_caps path so the operator's mental model
         carries over).
...
# spec: docs/spec-cost-caps-2026-06-05.md §"Enforcement implementation".


# Module-level handles for the admin-side helpers used by Phase 6
# tier_downgrade + L2 enforcement. Lazily resolved at first call so
# tests can override without needing the admin package importable in
# the test env.
...
def _get_tier_downgrade_writer():
    ...
    try:
        from evolve_admin.cost_profiles import write_openclaw_cost_settings  # type: ignore
...
def _apply_tier_downgrade(
    *, bot_id: str, shared_dir: Path, network: dict, spend: float,
    ...
    it explicitly. Spec: docs/spec-cost-caps-2026-06-05.md.
    """
    ...
    # Best-effort openclaw.json write. Admin helper handles the
    # /tmp + sudo /bin/cp path. Failures land in the alert reason so
    # the operator knows what to fix. The writer is resolved through
    # ``_get_tier_downgrade_writer()`` so tests can monkeypatch the
    # module-level handle without needing the admin package importable.
    write_ok = False
    write_detail = ""
    try:
        writer = _get_tier_downgrade_writer()
        if writer is None:
            write_detail = "writer unavailable (evolve_admin not importable)"
        else:
...
def _trip_l2_breaker_for_cost_cap(
    *, bot_id: str, spend: float, cap: float, shared_dir: Path,
    ...
    gateway entirely. Spec: docs/spec-cost-caps-2026-06-05.md.
...
def _send_cap_alert(
    bot_id: str, spend: float, cap: float, action: str, action_result: str,
    *,
    shared_dir: Path,
    network: dict,
    today_iso: str | None = None,
) -> None:
    """Hard-cap enforcement alert (separate dedup_key from threshold alert)."""
    action_labels = {
        "alert-only": "Alert only (no automated action)",
        "downgrade-tier": "Downgraded to tier 3 for rest of day",
        "pause-crons": "⏸ Cron jobs paused for rest of day",
        "suspend-bot": "🛑 Bot gateway suspended",
    }
    ...
    sent = _dispatch(
        shared_dir=shared_dir,
        network=network,
        dedup_key=f"spend_alert/cap/{bot_id}/{today_iso}",
        severity_name="critical",
        catalog_event="cost.hard_cap_hit",
        # action_result not in the catalog template — operator opens
        # admin UI → Cost → Spending Caps via the action line.
        payload={
            "bot_id": bot_id,
            "spend": spend,
            "cap": cap,
    ...
    if sent:
        _log(f"[spend_alert] {bot_id}: cap alert sent")
...
def main() -> None:
    ...
    # Hard cap config. Two layers:
    #   1. Per-bot ``bots.{bot}.daily_cap_usd`` (preferred since 2026-05-23
    #      — set per-bot so chatty bots like team_bot_a/admin_bot can sit at $20 while
    #      heartbeat-heavy bots like security_bot/team_bot_b sit at $5)
    #   2. Pod-wide ``thresholds.dailySpendCapUsd`` (legacy global; still
    #      honored as the fallback when no per-bot value is set, so existing
    #      installs don't lose their cap on upgrade)
    # Per-bot caps trip the L1 cost breaker (heartbeat-disable enforcement
    # via breakers_enforce.enforce_trip) — that's the path with provable
    # halt-the-bleed semantics. The legacy global path stays on the
    # spend_caps.execute_enforcement_action route (downgrade-tier /
    # pause-crons / suspend-bot) for backwards compat.
    daily_cap = thresholds_cfg.get("dailySpendCapUsd")  # None = no global cap
    cap_action = thresholds_cfg.get("spendCapAction", "downgrade-tier")
    bots_cfg = config.get("bots", {})

    # Lazily import spend_caps — only needed when a cap is configured
    _spend_caps = None
    if daily_cap is not None:
        try:
            import spend_caps as _spend_caps  # type: ignore[import]
        except ImportError:
            _log("[spend_alert] spend_caps module not found — hard cap enforcement disabled")
    ...
    for bot_id in members:
        ...
        # Legacy variable for the velocity-forecast block below; uses
        # the L1 threshold as the cap of record.
        ...
        velocity_cap = (
            per_bot_cap
            if per_bot_cap is not None
            else (float(daily_cap) if daily_cap is not None else 0.0)
        )
        ...
        # ── Pod-wide hard cap (legacy spend_caps path) ────────────
        if daily_cap is not None and _spend_caps is not None:
            cap = float(daily_cap)
            if spend >= cap and not _spend_caps.enforcement_already_triggered(shared_dir, bot_id, today):
                _log(f"[spend_alert] {bot_id}: HARD CAP HIT ${spend:.4f} >= ${cap:.2f} — action: {cap_action}")
                _spend_caps.write_enforcement_flag(shared_dir, bot_id, cap_action, spend, cap, today)
                bot_cfg = bots_cfg.get(bot_id, {})
                action_result = _spend_caps.execute_enforcement_action(
                    cap_action, bot_id, shared_dir, bot_cfg, alerts_cfg, spend, cap,
                )
                # Send a cap-specific alert (separate from the threshold alert)
                _send_cap_alert(
                    bot_id, spend, cap, cap_action, action_result,
                    shared_dir=shared_dir, network=config, today_iso=str(today),
                )
                # Flag urgent Better Engine refresh — hard cap hit
                _flag_urgent_refresh(shared_dir, source="spend_alert",
                                     reason="hard_cap_hit", bot_id=bot_id)
