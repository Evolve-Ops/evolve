...
def load_today_spend_detail(
    ...
    *,
    ...
    selected = [
        t for t in turns
        if (t.get("ts") or "")[:10] == today_iso
        and not (exempt_subkinds and t.get("forge_subkind") in exempt_subkinds)
    ]
    total = sum_turn_costs(selected)
    return DaySpend(
...
def main() -> None:
    ...
    # Same pair for the per-(bot, day) types (cost_velocity_forecast +
    # spend_unpriced_turns). Membership requires the DAILY JSONL read to
    # have succeeded (day_spend is not None) — a bot whose burst check
    # passed but whose day read failed is not day-checked.
    kept_day_sigs: set[str] = set()
    ...
    for bot_id in members:
        ...
        if decision["verdict"] == "unmeasurable":
            ...
            unpriced_id = emit_unpriced_spend_signal(
                shared_dir=shared_dir,
                bot_id=bot_id,
                day=day_spend,
                cap_usd=per_bot_cap,
            ...
            if unpriced_id is not None:
                # Keep today's signature so the post-loop sweep doesn't
                # archive the Signal we just (re-)observed. A transient
                # observe() failure returns None → not kept → swept; the
                # store's reopen window absorbs that on the next tick.
                sig_keep = _day_signature(
                    "spend_unpriced_turns", bot_id, today
                )
                if sig_keep is not None:
                    kept_day_sigs.add(sig_keep)
        ...
        if velocity_cap > 0 and spend > 0:
            velocity_id = emit_velocity_forecast_signal(
                shared_dir=shared_dir,
                bot_id=bot_id,
                spend_usd=spend,
            ...
            if velocity_id is not None:
                # Keep today's signature (see the unpriced keep above).
                # A None return also covers "projection dropped below the
                # warn fraction" and "actuals crossed the cap" — in both
                # cases the sweep resolving the same-day Signal is the
                # desired behavior.
                sig_keep = _day_signature(
    ...
    if day_checked_bots:
        _sweep_stale_signals(
            shared_dir,
            types={"cost_velocity_forecast", "spend_unpriced_turns"},
            bot_ids=day_checked_bots,
            kept_signatures=kept_day_sigs,
            reason="auto-resolve: condition cleared or day rolled over",
        )
