"""observations.llm_extractor — fast-role LLM tuple extractor.

Wires ``observations.extract.set_extractor()`` to a fast-role
``infra_llm`` call (#3466: provider-agnostic — the pod's credentialed
provider, not necessarily Anthropic) that takes a Transcript +
ExtractorVocabulary and returns raw tuple dicts in the shape
``observations.extract.extract_tuples`` expects.

Falls back silently to the pluggable default when NO LLM provider is
credentialed on the primary bot — production is never silently broken;
it just emits no tuples until wiring succeeds.
"""
...
def make_llm_extractor(
    ...
    *,
    ...
    """Build an ExtractorCallable backed by ``infra_llm.complete``.

    Args:
        target: a resolved :class:`infra_llm.InfraLLMTarget` (fast role —
            the model rides in the target, never hardcoded here).
        max_tokens: response cap. 600 is plenty for ≤2 tuples.
        transport: test seam, forwarded to ``complete``.
...
def wire_default_extractor() -> bool:
    """Wire observations.extract's pluggable extractor to a fast-role
    ``infra_llm`` call.

    Returns True if wiring succeeded, False if it fell back to the
    no-op default (no LLM provider credentialed). Logs at INFO/WARNING;
    never raises.
    """
    try:
        target = resolve_infra_llm("fast")
    except Exception as e:  # noqa: BLE001
        logger.warning("llm_extractor: LLM resolution failed (%s); remaining on stub", e)
        return False
    if target is None:
        logger.info(
            "llm_extractor: no LLM provider credentialed; "
            "remaining on stub (no tuples will be extracted)"
        )
        return False

    try:
        set_extractor(make_llm_extractor(target))
    except Exception as e:  # noqa: BLE001
        logger.warning("llm_extractor: wiring failed (%s); remaining on stub", e)
        return False

    logger.info("llm_extractor: wired LLM-backed extractor (model=%s)", target.model)
    return True
