    ...
    for path in candidates:
        if path.exists():
            try:
                ...
                # Migrate IN MEMORY only. This used to write the migrated dict
                # back (tmp + rename) whenever `changed`, which made a pure read
                # a write to the canonical network.json — on EVERY call, because
                # nothing else stamps `evolveVersion`, so a file without it was
                # re-migrated forever. Two costs: the admin suite's
                # real-shared-dir guard turned innocent callers (anything
                # reaching bot_home() -> get_bot_user() -> load_config()) into
                # teardown failures blaming the wrong test, and on a real pod a
                # read escalated to a root sudo fallback in sibling writers. The
                # returned dict is migrated either way, so no reader loses
                # anything; `_default_network()` now stamps the version at write
                # time, which is where a schema stamp belongs.
                data, _changed = _migrate_network(data)
                return data
...
DEFAULT_MODULES: dict[str, Any] = {
    ...
    "apply":            {"enabled": True},
    # Continuity Engine v2 (defer tool + defer_runner). Only `enabled` is
    # read (defer_runner.is_bot_module_enabled); the v1 tuning keys
    # (idleThresholdMin / maxAgentTasksPerRun / budgetFloor) died with the
    # v1 extraction stack (PR #835).
...
def is_module_enabled(config: dict[str, Any], module_name: str) -> bool:
    ...
    slack_signals and community_intel which default to False)."""
...
# ── HMAC signing (Phase 3a) — REMOVED 2026-08-18 ─────────────────────────────
#
# SIGNING_KEY_PATH, SIGNING_ENFORCED_MARKER, _SIGNED_FIELDS, _signing_enforced,
# _mark_signing_enforced, _load_signing_key, generate_signing_key,
# sign_proposal / verify_proposal_sig, sign_review_stamp / verify_review_stamp,
# and sign_forge_result / verify_forge_result_sig all lived here.
#
# The only verifier on the pod was packages/analyzer/apply.py, a per-bot
# daemon that polled {shared_dir}/proposals/approved/ — a directory no
# arbiter status maps to (arbiter/store.py::_STATUS_TO_SUBDIR), so it could
# not see a modern proposal even in principle and had applied nothing in its
# entire logged history. Because that verifier ran as each bot's own user,
# the shared HMAC secret had to be bot-readable; retiring it removed the
# topology constraint that kept a pod secret open.
#
# Layer 1's replacement is filesystem ACL plus the arbiter actor/routing
# model, not another signature scheme: after this change every writer and
# every verifier on the live path is the evolve or evo user, in-process,
# over a shared directory, so a signature attests nothing the ACL does not.
# The residual structural gap (the evo in-process apply fallback can
# self-promote pending -> approved_auto under its own uid) is NOT closed by
# signing and is tracked as diligence backlog 7.1 C.
#
# See internal/design-proposal-signing-key-2026-08-18.md,
# internal/spec-security-protocol.md (Layer 1), docs/threat-model.md.
#
# schema.proposal.Proposal.signature is retained as a persisted legacy field
# (migrations/proposal_v1_to_v2.py reads it); nothing writes it any more.
