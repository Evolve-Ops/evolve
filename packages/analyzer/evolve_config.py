...
def verify_review_stamp(proposal: dict) -> bool:
    """Verify review_stamp.sig in an approved proposal — and its VERDICT.

    Fails closed when signing is enforced but the key is missing (see
    ``verify_proposal_sig``).

    A stamp whose ``result`` is anything but ``"approved"`` (e.g. a
    ``"flagged"`` stamp from the retired review.py era) never verifies —
    key or no key. The pre-2026-08 version validated only the HMAC, so a
    flagged proposal with an intact signature sailed through apply
    (roadmap-codebase-review item 1.7).
    """
    stamp = proposal.get("review_stamp")
    if isinstance(stamp, dict) and stamp.get("result") != "approved":
        logger.error(
            "Proposal %s carries a non-approving review stamp "
            "(result=%r); refusing to verify.",
            proposal.get("id", "<unknown>"),
            stamp.get("result"),
        )
        return False
