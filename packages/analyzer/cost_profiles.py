...
# ── Prune/cache coherence invariant ───────────────────────────────────────────
#
# ``contextPruning.mode = "cache-ttl"`` does NOT mean "keep context for ttl".
# It is the opposite: OC SKIPS pruning while the prompt cache is still warm,
# because pruning rewrites the prefix and would throw away a live cache. The
# guard in OC's contextPruningExtension is literally:
#
#     if (ttlMs > 0 && Date.now() - lastTouch < ttlMs) return;   // don't prune yet
#
# So ``ttl`` is "how long to wait before it is free to prune", and the correct
# value is the effective prompt-cache TTL — prune exactly when the cache dies
# and the rewrite is happening anyway. OC's own default is 300s, matching
# Anthropic's 5-minute default cache.
#
# Anthropic offers exactly two cache TTLs: 5 minutes (default) and 1 hour
# (``cache_control.ttl = "1h"``, reached via ``cache_retention: "long"``).
# **1 hour is the maximum**, so a prune ttl above 1h is incoherent under EVERY
# possible cache configuration — there is no setting that makes it correct.
# A too-large ttl silently defeats pruning: tool results are never trimmed
# during the window between cache death and prune eligibility, and each cache
# expiry re-writes all of that untrimmed bulk at 1.25x-2x input price.
#
# Enforced by test_cost_profiles_prune_cache_coherence.py over every built-in
# profile and over deploy.py's _BALANCED_COST_DEFAULTS.
MAX_COHERENT_PRUNE_TTL_SECONDS = 3600  # Anthropic's longest cache TTL (1h)



def prune_ttl_for_cache_retention(retention: str | None) -> str:
    """The only coherent ``contextPruning.ttl`` for a given ``cache_retention``.

    These two settings are NOT independently choosable. ``contextPruning.ttl``
    is a prune DELAY — OC skips pruning while the prompt cache is still warm,
    because pruning rewrites the prefix and would discard a live cache. So::

        prune <  cache  ->  prunes while warm, throws away a paid-for cache
        prune >  cache  ->  cache dies first; untrimmed context is re-written on
                            every expiry until pruning is finally allowed
        prune == cache  ->  prune exactly when the rewrite is free.  Correct.

    Deriving one from the other removes the possibility of divergence. Three
    separate incoherent pairings turned up on 2026-07-31 alone: the shipped
    ``4h`` against a 5-minute cache (pruning effectively disabled), and then
    ``1h`` against a 5-minute cache when cache retention was reverted without
    the prune ttl following it. That is enough evidence that this should not be
    two hand-maintained numbers.

    Callers must not hand-pick a prune ttl — use this.
    ``test_cost_profiles_prune_cache_coherence.py`` asserts every built-in
    profile obeys it.
    """
    # Anthropic offers exactly two cache TTLs: 5 minutes (default) and 1 hour.
    return {"long": "1h", "short": "5m", None: "5m"}.get(retention, "5m")


# GATED PENDING MEASUREMENT (2026-07-31).
#
# Both cost profiles shipped ``cache_retention: "long"`` on the reasoning that a
# 1-hour cache beats a 5-minute one because it avoids TTL-invalidation rewrites
# across conversational gaps. **That reasoning assumes the cached prefix is
# STABLE between turns.** The 2026-07-31 post-mortem identifies a root-cause
# candidate in which it is not: Evolve appends per-turn-varying content to the
# system prompt, which may invalidate the whole prefix on its own schedule
# regardless of TTL.
#
# If that holds, ``long`` is strictly worse — 2x per cache write instead of
# 1.25x, buying no additional reuse. The default is therefore held at "short"
# until the Phase 0 experiment in
# internal/spec-context-observability-2026-07-30.md returns a number.
#
# To flip it after Phase 0: change this ONE constant. Both profiles and both
# prune ttls follow automatically, and the coherence test enforces the pairing.
# See internal/incident-post-mortem-2026-07-31-cost-containment.md §2.
_DEFAULT_CACHE_RETENTION: str = "short"
...
BUILTIN_PROFILES: dict[str, dict] = {
    "conservative": {
        ...
        "settings": {
            ...
            "contextPruning": {
                "mode": "cache-ttl",
                "ttl": prune_ttl_for_cache_retention(_DEFAULT_CACHE_RETENTION),
                # No "keepLastAssistants" — OpenClaw 2026.9.2 retired it (a
                # strict-schema `Unrecognized key` under the installed
                # runtime, not a rename); canonical built-in defaults now
                # apply. See breaker-notify-survives-openclaw-config-validation.
            },
            "compaction": {
                "mode": "safeguard",
                # A byte budget, not the retired reserveTokensFloor (OC
                # 2026.9.2 rejects it; the reserve is a fixed 20k, so with no
                # budget a 1M-window model compacts near 980k tokens). The
                # flush threshold sits below it so the memory note is always
                # written before the compaction it precedes. See
                # evolve_admin/compaction_housekeeping.py and
                # docs/configuration.md "Compaction budget".
                "maxActiveTranscriptBytes": 256 * 1024,
                "memoryFlush": {
                    "enabled": True, "softThresholdTokens": 6000,
                    "forceFlushTranscriptBytes": 204 * 1024,
                },
            },
            ...
            "cache_retention": _DEFAULT_CACHE_RETENTION,
        },
    },
    "balanced": {
        "name": "balanced",
        ...
        "settings": {
            ...
            # ttl MUST NOT exceed the effective prompt-cache TTL — see
            # MAX_COHERENT_PRUNE_TTL_SECONDS below. Was "4h" until 2026-07-31,
            # which silently defeated pruning: `cache-ttl` mode SKIPS pruning
            # while the cache is still warm (OC's guard is
            # `if (now - lastTouch < ttlMs) return`), so a 4h ttl meant tool
            # results were never trimmed during the 3h window after the cache
            # died — and every expiry re-wrote all of that untrimmed bulk.
            "contextPruning": {
                "mode": "cache-ttl",
                "ttl": prune_ttl_for_cache_retention(_DEFAULT_CACHE_RETENTION),
                # No "keepLastAssistants" — see the conservative profile's
                # comment above; same retirement, same fix.
            },
            "compaction": {
                "mode": "safeguard",
                # Same budget as deploy._BALANCED_COST_DEFAULTS — see the
                # conservative profile's comment.
                "maxActiveTranscriptBytes": 400 * 1024,
                "memoryFlush": {
                    "enabled": True, "softThresholdTokens": 10000,
                    "forceFlushTranscriptBytes": 320 * 1024,
                },
            },
    ...
    "unrestricted-debug": {
        ...
        "settings": {
            ...
            # Sets no transcript budget of its own (a merge keeps whatever
            # the bot already has; with none, OC compacts at window − 20k).
...
def read_openclaw_primary_model(bot_id: str) -> str | None:
    """Read ``agents.defaults.model.primary`` from a bot's openclaw.json.

    Returns None when the file is unreadable or the key is absent. Used by
    spend_alert's tier_downgrade path to record the pre-downgrade model so
    the midnight auto-revert can restore it.
    """
    oc_path = _bot_home(bot_id) / ".openclaw" / "openclaw.json"
    data = _read_oc_json(oc_path, bot_id)
    if data is None:
        return None
    try:
        val = data["agents"]["defaults"]["model"]["primary"]
