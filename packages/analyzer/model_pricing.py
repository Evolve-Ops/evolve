...
class PricedModel:
    ...
    ``cache_read_cost_per_token`` / ``cache_write_cost_per_token`` carry the
    prompt-cache rates in the same $/token units when the source publishes
    them, and are ``None`` when it does not.
    """
    ...
    source: str = ""
    # Prompt-cache rates, when the source publishes them. ``None`` means the
    # catalog does not know — NOT that caching is free. Added for audit B6:
    # ``turn_cost`` prices cache tokens from these when present, and a missing
    # rate must never be read as a zero rate (a cache-heavy agent turn is
    # mostly cache-read tokens, so a silent zero there is the same silent-zero
    # failure at a finer grain).
...
def normalize_litellm(raw: dict) -> list[PricedModel]:
    ...
    for model_id, spec in (raw or {}).items():
        ...
        out.append(PricedModel(
            ...
            cache_read_cost_per_token=_as_float(
                spec.get("cache_read_input_token_cost"),
            ),
            cache_write_cost_per_token=_as_float(
...
def normalize_modelsdev(raw: dict) -> list[PricedModel]:
    ...
    for raw_provider, pdata in (raw or {}).items():
        ...
        for model_id, spec in models.items():
            ...
            cr_per_m = _as_float(cost.get("cache_read"))
            cw_per_m = _as_float(cost.get("cache_write"))
            cr_cost = cr_per_m / 1e6 if cr_per_m is not None else None
            cw_cost = cw_per_m / 1e6 if cw_per_m is not None else None
            ...
            out.append(PricedModel(
                ...
                cache_read_cost_per_token=cr_cost,
                cache_write_cost_per_token=cw_cost,
            ))
    return out
...
def build_pricing_cache(
    *,
    ...
    for r in litellm_records:
        ...
        if r.family is None and cross is not None and cross.family:
            r.family = cross.family
        # Cache rates enrich the same way family does: LiteLLM is primary on
        # input/output, but a row missing a cache rate borrows the cross-check's
        # rather than leaving the cost layer to guess at one (audit B6).
        if cross is not None:
            if r.cache_read_cost_per_token is None:
                r.cache_read_cost_per_token = cross.cache_read_cost_per_token
            if r.cache_write_cost_per_token is None:
                r.cache_write_cost_per_token = cross.cache_write_cost_per_token
        merged[key] = r
...
def write_pricing_cache(shared_dir: Path, doc: dict) -> Path:
    ...
    os.replace(tmp, path)
    # Drop the cost layer's in-process memo of this file. It is keyed on
    # (path, mtime), and mtime granularity is a whole second on some
    # filesystems — a refresh landing in the same second as an earlier read
    # would otherwise keep serving the pre-refresh catalog to this process,
    # which for a long-lived daemon means pricing from a stale table until
    # restart. Imported lazily: turn_cost reads model_pricing, not the reverse.
    try:
        from turn_cost import reset_pricing_catalog_cache  # type: ignore[import]
        reset_pricing_catalog_cache()
    except Exception:  # pragma: no cover — cost layer absent (docs/CI tooling)
        _log.debug("model_pricing: turn_cost memo not reset (import failed)")
    return path
