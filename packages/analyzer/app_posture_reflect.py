...
Spec: internal/spec-manifest-reflex.md §"App posture review (PR4 — inventory
...
LLM dispatch goes through the provider-agnostic ``infra_llm`` client
(#3466): the bot's own tier3 model pin is honored when its provider is
credentialed on the primary bot, otherwise resolution walks the pod's
credentialed providers. No provider is ever presumed.

Failure-soft throughout: no credentialed provider, HTTP error,
validation mismatch — each degrades to "no reflection appended this
week", never breaks the inventory doc that PR4 already wrote.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# LLM target + model resolution (provider-agnostic via infra_llm, #3466)
# ─────────────────────────────────────────────────────────────────────────────


def _resolve_model(bot_id: str) -> str:
    """Pick the bot's tier3 model from its evolve-tiers config. Returns the
    provider-qualified model string or "" when nothing resolves — the
    caller falls back to pod-level ``infra_llm`` resolution. tier3 is the
    cost-conscious tier — the reflection is a once-a-week call so we
    don't pay the standard tier for it."""
    user = _bot_user(bot_id)
    tiers_path = _user_home(user) / ".openclaw" / "evolve-tiers.json"
    try:
...
def _resolve_target(bot_id: str):
    """Resolve the (provider, model, key) target for the reflection call.

    The bot's own tier3 pin wins when its provider is credentialed on
    the primary bot; otherwise pod-level ``infra_llm`` resolution walks
    the credentialed providers. Returns ``None`` when no LLM provider is
    credentialed (or resolution fails at all — failure-soft, per the
    module ethos) — the caller degrades with a clear error.
    """
    try:
        from infra_llm import credentialed_target, resolve_infra_llm  # type: ignore

        return credentialed_target(_resolve_model(bot_id)) or resolve_infra_llm(
            REFLECT_TIER
        )
    except Exception as e:  # noqa: BLE001 — never break the inventory doc
        print(f"[app_posture] LLM target resolution failed: {e}", file=sys.stderr)
        return None


def _call_llm(target, prompt: str, *, timeout: int = REFLECT_TIMEOUT_SEC) -> str:
    """One-shot LLM call via ``infra_llm.complete``. Returns text on
    success, "" on any failure (logged to stderr)."""
    ...
    try:
        ...
        print(f"[app_posture] LLM call failed: {e}", file=sys.stderr)
        return ""
...
def reflect(
    ...
    *,
    ...
    response = _call_llm(target, prompt)
    if not response:
        return ReflectionResult(
            ok=False, text=None, raw_response=None,
