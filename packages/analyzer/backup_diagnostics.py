...
_CAUSES: tuple[tuple[re.Pattern[str], _Cause], ...] = (
    # The pod PAT is stored but this bot's uid cannot decrypt the vault,
    # and the admin daemon's verdict endpoint was also unreachable.
    # FIRST in the table: this error names a GitHub PAT and a repo, so
    # several of the looser patterns below would otherwise claim it and
    # send the operator to the wrong page.
    #
    # The fix steps deliberately do NOT say "run the Backup → Cloud
    # wizard". That is the sentence the old code emitted for this exact
    # state, and it is why the 2026-08-18 outage read as an unfinished
    # setup step for 23 nights: the operator HAD stored a PAT, and
    # storing it again would not have granted the bot vault access.
    (
        re.compile(
            r"cannot read it \(keystore key `github_pat`",
            re.IGNORECASE | re.DOTALL,
        ),
        _Cause(
            cause_id="pat_unreadable",
            title=(
                "The pod's GitHub PAT exists, but this bot cannot read the "
                "vault — and the admin daemon couldn't be reached to answer "
                "on its behalf"
            ),
            fix_steps=(
                "This is NOT a missing setup step. A PAT *is* stored. "
                "Re-running the Backup → Cloud wizard will not fix it — "
                "the vault (`{shared}/keystore/vault`) and its "
                "`.machine-key` are readable only by `evolve`, and the "
                "backup daemon runs as the bot's own user.",
                "The bot is supposed to ask the admin daemon for a "
                "private/public verdict instead of reading the PAT. Check "
                "the daemon is up: `sudo launchctl print "
                "system/ai.evolve.evolve.admin-ui`.",
                "Check the socket is reachable from the bot's user: "
                "`{shared}/admin-daemon.sock` should be mode 0660 owned by "
                "`evolve` with group `staff` (macOS) or carry the "
                "`evolve-bots` group ACE (Linux). "
                "`sudo evolve-admin ensure-pod-perms` re-asserts it.",
                "Do NOT widen the vault or `.machine-key` to fix this. "
                "PR #3696 clamped them deliberately — a world-readable "
                "machine key lets any prompt-injected bot decrypt the "
                "stored PAT.",
            ),
        ),
    ),
