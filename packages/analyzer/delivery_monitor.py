...
U2.1 of the user-value roadmap (internal/roadmap-user-value-2026-06-10.md §5);
spec: internal/spec-proactive-delivery-monitor-2026-06-10.md. The #1 stated
...
-------------------------------------
* ``launchd`` — stdout-log mtime (``<app-cron log dir>/<label>.out.log``,
  the profile-keyed path ``install_launchd_command_action`` wires:
  ``/tmp`` on macOS, ``{bot_home}/.openclaw/logs`` on Linux — both sides
  derive it from ``platform_profile.app_cron_log_dir`` so the probe can
  never drift from the installer) plus the job's load state via
...
def _stdout_log_path(action: MonitoredAction) -> Path | None:
    ...
    if action.label:
        ...
        # Same profile-keyed derivation install_launchd_command_action uses
        # (/tmp on macOS, {bot_home}/.openclaw/logs on Linux) so the probe
        # reads the file the unit actually writes. workspace is
        # {bot_home}/.openclaw/workspace — see _discover's construction.
        bot_home = action.workspace.parent.parent
        return Path(get_profile().app_cron_log_dir(bot_home)) / f"{action.label}.out.log"
    return None
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "user-facing apps. Spec: internal/spec-proactive-delivery-monitor-"
            "2026-06-10.md."
        ),
    )
