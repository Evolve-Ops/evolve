...
# Accepted values for ``bots.<id>.budget.per_bot_cache_retention``, imported
# rather than re-declared: ``cache_shape`` owns the enum because it owns the
# resolver that interprets it, and a second literal here is how a store and
# its resolver drift apart.
#
# GUARDED, and the only guarded import in this module: better_engine_config is
# the read path for every budget, cap and remediation-ladder value in the pod.
# An unimportable ``cache_shape`` — analyzer path not set up, a partial
# deploy, a future import cycle — must fail the cache knob, not the whole
# config. The fallback is the same tuple; ``test_cache_retention_enum_has_one_home``
# pins the two against each other so the copy cannot drift unnoticed.
try:
    from cache_shape import RETENTION_VALUES as CACHE_RETENTION_VALUES
except Exception:  # noqa: BLE001 — analyzer path unavailable in this embedding
    CACHE_RETENTION_VALUES = ("short", "long", "auto")
...
class BetterEngineConfig:
    ...
    def per_bot_cache_retention(self, bot_id: str) -> str | None:
        ...
        Returns ``"short"`` (5m), ``"long"`` (1h) or ``"auto"`` when explicitly
        set, else ``None`` (inherit OC's default of "short" — the materializer
        skips the per-model fan-out when this is None).

        ``"auto"`` is not a value OpenClaw understands: it is Evolve's
        instruction to pick the tier from the bot's own measured inter-turn
        gaps at deploy time. ``cache_shape.resolve_retention`` turns it into
        ``"short"``/``"long"``/``None`` (declined), and the materializer
        writes only what comes out of that. Anything unrecognised on disk
        reads as ``None`` rather than raising — a hand-edited BE config must
        not wedge a deploy over a cache knob.
        """
        if bot_id is None:
            return None
        bot_config = self.bots.get(bot_id, {})
        ...
        if value is _SENTINEL or value is None:
            return None
        if value not in CACHE_RETENTION_VALUES:
            return None
        return value
    ...
    def set_per_bot_cache_retention(self, bot_id: str, value: str | None) -> None:
        ...
        Accepts ``"short"``, ``"long"``, ``"auto"``, or ``None`` (clear override).
        """
        if value is not None and value not in CACHE_RETENTION_VALUES:
            raise ValueError(
                f"per_bot_cache_retention must be one of "
                f"{', '.join(repr(v) for v in CACHE_RETENTION_VALUES)}, or None; "
                f"got {value!r}"
            )
        bot = self.bots.setdefault(bot_id, {})
        budget = bot.setdefault("budget", {})
    ...
    def per_bot_cache_retention_decision(
        self, bot_id: str,
    ) -> tuple[str | None, str | None]:
        """``(last tier ``auto`` resolved to, ISO-8601 stamp)`` for this bot.

        ``(None, None)`` when ``auto`` has never resolved for it. Recorded
        beside the knob rather than derived on the fly because the dwell rule
        the materializer applies — do not overturn a tier that has not cleared
        the break-even by a margin, unless the standing decision is a week old
        — needs to know what was last DEPLOYED, and that is not recoverable
        from the gap distribution alone.
        """
        if bot_id is None:
            return None, None
        budget = _walk(self.bots.get(bot_id, {}), ("budget",))
        if budget is _SENTINEL or not isinstance(budget, dict):
            return None, None
        value = budget.get("per_bot_cache_retention_resolved")
        at = budget.get("per_bot_cache_retention_resolved_at")
    ...
    def set_per_bot_cache_retention_decision(
        self, bot_id: str, value: str | None, at: str | None,
    ) -> None:
        """Record what ``auto`` resolved to and when. Caller persists with ``save``."""
        if value is not None and value not in ("short", "long"):
            raise ValueError(
                f"per_bot_cache_retention_resolved must be 'short', 'long', "
                f"or None; got {value!r}"
            )
        bot = self.bots.setdefault(bot_id, {})
        budget = bot.setdefault("budget", {})
        if value is None:
            budget.pop("per_bot_cache_retention_resolved", None)
            budget.pop("per_bot_cache_retention_resolved_at", None)
            return
        budget["per_bot_cache_retention_resolved"] = value
        if at is None:
            budget.pop("per_bot_cache_retention_resolved_at", None)
        else:
            budget["per_bot_cache_retention_resolved_at"] = at
