...
See internal/spec-app-audit-2026-05-16.md §5.2 (whitelist), §5.6 (conflict).
"""
...
class ConflictReport:
    ...
    and ``app_id`` are BOTH columns of this report's own schema, carried side
    by side on purpose: the operator reading a conflict_notice needs to know
    the gallery package (to decide whether upgrading it is the fix) as well as
    the app. Collapsing them onto one resolved id would delete information the
    notice exists to convey.
    """
    file_path: str
...
def find_conflicts(
    ...
    for m in other_manifests:
        ...
        if role is None:
            continue
        affected.append({
            # identity: see resolve_app_id — report-schema columns, see
            # ConflictReport's docstring. ``other_id`` stays the manifest's
            # own ``id`` because it is compared against ``auditing_app_id``,
            # which the caller reads from the SAME field; the comparison is
            # symmetric and swapping one side alone would break it.
            "pkg_id": m.get("pkg_id", ""),
            "app_id": other_id,
...
#   4. Document the safety contract in internal/spec-app-audit-2026-05-16.md §5.2.
