...
def load_outcome_calibration(shared_dir: str, detector_name: str) -> dict:
    """
    Load outcome statistics for a given detector — used by analyze.py to
    calibrate thresholds. Returns: {thumbs_up: N, thumbs_down: N, expired: N}
    """
    outcomes_path = Path(shared_dir) / "feedback" / "outcomes.jsonl"
    pending_path = Path(shared_dir) / "feedback" / "pending-outcomes.jsonl"
    if not outcomes_path.exists():
        return {"thumbs_up": 0, "thumbs_down": 0, "expired": 0}

    # Match outcomes to proposals by detector name via pending log
    proposal_ids_for_detector: set[str] = set()
            ...
            for line in f:
                ...
                if line:
                    try:
                        ...
                        if record.get("detector_name") == detector_name:
                            proposal_ids_for_detector.add(record.get("outcome_id", ""))
                    except Exception:
                        pass

    counts: dict[str, int] = {"thumbs_up": 0, "thumbs_down": 0, "expired": 0}
        ...
        for line in f:
            ...
            if line:
                try:
                    ...
                    if oid in proposal_ids_for_detector:
                        r = record.get("result", "")
                        if r in counts:
                            counts[r] += 1
                except Exception:
