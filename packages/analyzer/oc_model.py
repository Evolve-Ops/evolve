...
ROUTING_ROLE_NAMES = ("fast", "standard", "power", "max", "judge")
...
def detect_rung_collisions(tiers_file: dict, updates_tiers: dict) -> list[dict]:
    ...
    if an operator points them there (judge no longer shares standard's rung by
    default — spec §Addendum 16 gives judge its own ``judge-class`` — but the
    guard stays as a general safety net for any hand-authored collision).
...
#: the ladder in cost order, then judge (which lands next to standard once
#: ``_ensure_role_and_rung`` places it by cost), then ``max``. Without this,
#: a payload listing tier0 before tier2 produced ``judge-class`` ahead of
#: ``sonnet-class`` — inert today (judge is excluded from the ladder rank), but
#: array position IS the cost rank for every other rung, so it is not a
#: property to leave dependent on dict iteration order.
_CREATE_TIER_ORDER: tuple[str, ...] = ("tier3", "tier2", "tier1", "tier0", "max")
...
def generate_fallback_list(tiers: dict, cascade: list[str]) -> list[str]:
    ...
    **Fallback degrades; it never escalates.** Tiers costlier than the one
    ...
    Example:
        ...
        -> ["opus", "sonnet", "gpt-4o", "haiku", "gpt-4o-mini"]
...
def json_full_config_set(
    ...
    (unknown key or a value the whitelist rejects), or ``["routing"]`` when
    the whole block was a non-dict. The key is ABSENT when nothing was
    ...
    if "tiers" in updates:
        ...
        # one rung with different models (judge now has its own judge-class
        # rung, but a hand-authored roles map could still collide), the fold is
        ...
        if _file_is_new_shape(tiers_file):
            ...
            # each role, its canonically-named rung at the cost-ordered
            # position with ``costClass``, and the structured ``judge`` entry.
    ...
    if "routing" in updates:
        ...
        if isinstance(incoming_routing, dict):
            ...
            if _dropped:
                ...
                _routing_refused = sorted(_dropped)
                print(
