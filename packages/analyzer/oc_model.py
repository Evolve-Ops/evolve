...
def json_full_config(bot: str, oc_json_path: Path | None = None) -> dict:
    ...
    Returns:
        {
          ...
          roleCaps           # per-role daily caps ({} when the bot has none)
        }
    """
    path = oc_json_path or _DEFAULT_OC_JSON
    data = _load_json(path)
    ...
    # separate read path.
    # ONE read for the three evolve-tiers.json keys below. Was one read each;
    # a third would have made this endpoint open and parse the same file three
    # times per call, and it is polled by the AI Optimization page.
    _tiers_doc = _load_tiers_file(bot)

    cascade = _tiers_doc.get("cascade") or {}
    ...
    user_tier_override = _tiers_doc.get("userTierOverride") or {}
    if not isinstance(user_tier_override, dict):
        user_tier_override = {}

    # roleCaps — the per-bot per-role daily caps, and the key the gateway
    # ACTUALLY enforces the Power cap from (``ModelRouter._roleCap`` reads
    # ``roleCaps.power.maxPerDayPerBot``; the ``userTierOverride.dailyCap``
    # above is the legacy sibling it shadows — see ``power_cap`` for why that
    # shadowing is total). Surfaced here because ``roleCaps`` is a WHOLESALE
    # replace on the write side: a caller setting ONE role's cap has to read
    # the block back to merge it, and this is the same seam its write goes
    # through. Absent (the common case — most bots inherit the pod/code
    # layer) reads as {}.
    role_caps = _tiers_doc.get("roleCaps") or {}
