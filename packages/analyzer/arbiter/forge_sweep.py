"""arbiter.forge_sweep — Close out applied forge-backed proposals.

Two action kinds hand work to the forge and return immediately: ``BuildApp``
(generate a new app) and ``InstallApp`` (install a gallery package). Both are
in ``apply._EXTERNAL_COMPLETION_KINDS``, which means apply.py deliberately
does NOT close them out — this sweep does, from the forge job's own status.
A kind whose applier dispatches to forge but which this sweep skips would sit
in ``applied/`` forever.

After such a proposal is applied, its applier kicks off a forge job
...
from schema.proposal import BuildApp, InstallApp
...
def sweep(
    ...
    *,
    ...
    for proposal in list(iter_proposals(shared_dir, subdirs=("applied",))):
        if not isinstance(proposal.action, (BuildApp, InstallApp)):
            continue
        ...
        if not job_id:
            counts["missing"] += 1
            log(
                f"[forge_sweep] proposal {proposal.id} ({kind}) has no "
