...
def _refresh_existing(existing: Proposal, *, incoming: Proposal, actor: str) -> None:
    ...
    - Overwrite the proposal's user-facing surface (problem, action,
      provenance, claim, admin_surface_summary) with the incoming values so
      the operator sees the *current* framing — current dollar amounts,
      latest recommended config value, etc. The dedup fingerprint is the
      only thing that has to stay constant for collapse to work; the
      framing should freshen on every re-detection. Without this the queue
      shows "admin_bot daily spend $4.23" days after the actual figure has
      moved on.
