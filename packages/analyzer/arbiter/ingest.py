...
L1 ships no signature verification, and no other layer supplies it either.
This paragraph previously claimed ``evolve_config.verify_proposal_sig``
owned that job and that ingest called it when a signature was present —
both were false. Ingest never called it, and the only verifier the pod
ever had was the per-bot ``apply.py`` daemon, which polled a directory no
arbiter status maps to. The whole HMAC surface was retired 2026-08-18
(internal/design-proposal-signing-key-2026-08-18.md). Proposal provenance on
the live path rests on filesystem ACL plus the actor recorded by
``arbiter.state_machine.transition``; the residual structural gap is
tracked as diligence backlog 7.1 C.
"""
