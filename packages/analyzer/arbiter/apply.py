...
_EXTERNAL_COMPLETION_KINDS = frozenset({"BuildApp"})
...
# (docs/spec-effectiveness-layer-2026-06-09.md §11) routes these out of the
...
def apply(
    ...
    *,
    ...
    # Don't apply config changes against a tripped bot. Operator-facing
    # kinds (Investigation, WorkflowInstruction, AddSignalCollection,
    # BuildApp) don't write to bot config, so they proceed normally.
    ...
    # naturally.
    if shared_dir is not None and not is_deferred_completion_kind(kind):
