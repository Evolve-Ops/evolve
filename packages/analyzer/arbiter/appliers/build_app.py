...
def _default_kickoff_runner(
    ...
    """Production path: run the forge pipeline with auto-approve.

    The dispatch itself (lazy forge_engine import, crash → mark the job
    failed so ``forge_sweep`` can close the proposal) is shared with the
    InstallApp applier — see ``arbiter.appliers._forge_kickoff``.
    """
...
def _synthetic_pkg_id() -> str:
    ...
    ``pkg_id`` in this module is this MINTED value, not a read of anything.
    It names the build (the ForgeJob's package slot) and travels alongside
    ``action.app_id``, which is the app's identity and is already canonical.
    A resolver has nothing to resolve here: the string does not exist until
