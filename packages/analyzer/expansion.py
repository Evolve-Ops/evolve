...
def enrich_themes_with_llm(
    ...
    """
    Use the fast (tier3) role to turn raw theme clusters into specific
    capability suggestions. Returns list of enriched suggestion dicts.

    Falls back to basic (non-enriched) suggestions when the LLM cannot be
    used. The two reasons for that are kept apart: no credentialed provider
    is expected and quiet, while a FAILED call against a credentialed
    provider raises an ``engine_llm_call_failed`` Signal — the basic
    suggestions that come back look identical either way, which is exactly
    how the dead ``openclaw llm complete`` call site stayed invisible.
    """
    if not themes:
        return []
...
Output only JSON lines, no other text."""
    ...
    from engine_llm import FAILED, OK, UNCREDENTIALED, engine_complete  # type: ignore

    output, outcome = engine_complete(
        prompt,
        job="expansion",
        shared_dir=shared_dir,
        model_hint=tier3_model,
        role="fast",
        max_tokens=600,
        timeout=45,
    )
    if outcome == UNCREDENTIALED:
        print(
            "[expansion] no credentialed LLM provider — basic suggestions "
            "(expected)",
            file=_sys.stderr,
        )
        return [_basic_suggestion(t) for t in themes[:max_suggestions]]
    if outcome == FAILED:
        # Distinct from the branch above ON PURPOSE: an engine_llm_call_failed
        # Signal is now firing.
        print(
            "[expansion] ERROR: LLM enrichment FAILED against a credentialed "
            "provider — falling back to basic suggestions and raising an "
            "engine_llm_call_failed Signal",
            file=_sys.stderr,
        )
        return [_basic_suggestion(t) for t in themes[:max_suggestions]]
    ...
    # "NO_GAPS" is a legitimate model verdict (no genuine gaps found), not a
    # failure — basic suggestions, no Signal.
    if "NO_GAPS" in output:
        return [_basic_suggestion(t) for t in themes[:max_suggestions]]
...
def run_expansion(
    ...
    if themes:
        suggestions = enrich_themes_with_llm(
            themes, user_profile, network_config, MAX_SUGGESTIONS, shared_dir
        )
