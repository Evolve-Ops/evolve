...
# The pod's ONE per-turn cost rule (audit B6). The plugin's recorded
# cost_estimated wins when non-zero; a zero with real token counts is
# re-estimated from the annotation's model/provider (the TS estimator
# prices models it can't match at $0 — the silent-zero shape B6 forbids),
# and a turn neither can price is COUNTED, never summed as zero.
from turn_cost import load_pricing_catalog, turn_cost as _turn_cost

# ── Constants ────────────────────────────────────────────────────────────────

# v2 (2026-08-31): additive — metrics dicts gain ``unpriced_turns`` and
# coverage gains a windowed ``unpriced_turns`` total; zero-cost annotations
# with real tokens are re-priced via turn_cost (audit B6). Readers use
...
def _empty_metrics() -> dict[str, Any]:
    return {
        ...
        "output_tokens": 0,
        "cost_estimated": 0.0,
        # Turns whose cost could not be resolved at all (recorded $0, and
        # neither the pricing catalog nor the offline tables know the
        # model/provider). Carried BESIDE the dollar figure so a reader
        # sees "cost is a floor over N unpriced turns", never a confident
        # total that silently counted them as free (audit B6).
        "unpriced_turns": 0,
    }


def _resolve_annotation_cost(
    rec: dict, catalog: dict | None, shared_dir: Path | None = None,
) -> float | None:
    """One annotation's USD cost via the pod's per-turn cost rule.

    The plugin-recorded ``cost_estimated`` wins when non-zero. A zero
    with real token counts is re-estimated from the annotation's
    ``model_selected``/``provider`` + token fields — until 2026-08 the
    TS estimator priced every non-Anthropic model (grok, gpt, gemini)
    at $0, and 30 days of those records stay in this rollup's window.
    Returns ``None`` when the turn cannot be priced at all; a turn that
    burned no tokens is genuinely $0.
    """
    recorded = _float(rec.get("cost_estimated"))
    if recorded:
        return recorded
    tokens = (
        _int(rec.get("input_tokens")) + _int(rec.get("output_tokens"))
        + _int(rec.get("cache_read_tokens")) + _int(rec.get("cache_write_tokens"))
    )
    if tokens <= 0:
        return 0.0
    return _turn_cost(
        {
            "model": rec.get("model_selected") or "",
            "provider": rec.get("provider") or "",
            "cost": recorded,
            "input_tokens": _int(rec.get("input_tokens")),
            "output_tokens": _int(rec.get("output_tokens")),
            "cache_read_tokens": _int(rec.get("cache_read_tokens")),
            "cache_write_tokens": _int(rec.get("cache_write_tokens")),
        },
        catalog=catalog,
...
def _add_metrics(acc: dict[str, Any], rec: dict, cost: float | None) -> None:
    """Fold one annotation's usage into an accumulator (defensive casts).

    ``cost`` is the pre-resolved :func:`_resolve_annotation_cost` value —
    resolved once per record by the caller, since the same record lands
    in several window buckets.
    """
    acc["turns"] += 1
...
def _sum_metrics(parts: Iterable[dict[str, Any]]) -> dict[str, Any]:
    ...
    for part in parts:
        ...
        out["unpriced_turns"] += part.get("unpriced_turns", 0)
    return out
...
def rollup_bot(
    ...
    *,
    ...
    # Pricing catalog for the zero-cost re-estimate path — read once per
    # rollup, not per record. Absent catalog → offline tables → unpriced.
    ...
    for offset in range(MAX_WINDOW_DAYS):
        ...
        for rec in records:
            ...
            for key in windows_hit:
                _add_metrics(app[key][grade], rec, cost)
...
def _assemble(
    ...
    for key in WINDOWS:
        ...
        coverage[key] = {
            ...
            # Turns no pricing source could cost (audit B6): when non-zero,
            # every cost_estimated in this window is a floor, not a total.
            "unpriced_turns": (
                attributed["unpriced_turns"]
                + inferred["unpriced_turns"]
                + un["unpriced_turns"]
            ),
...
def write_rollup_json(out_path: Path, payload: dict[str, Any], *, prefix: str) -> Path:
    """Atomically write one rollup payload at ``out_path``, mode 0644.
    ...
    Shared by every ``{shared}/{bot}/`` rollup written by the evolve
    user (``usage-by-app.json`` here, ``usage-by-user.json`` in
    ``usage_by_user``) so the mode-before-rename ordering has one
    implementation rather than one per file.
    """
    out_path.parent.mkdir(parents=True, exist_ok=True)
    fd, tmp = tempfile.mkstemp(dir=str(out_path.parent), prefix=prefix, suffix=".tmp")
    try:
        ...
        try:
            ...
            print(f"[rollup] temp file vanished during cleanup: {tmp}",
                  file=sys.stderr)
        raise
    return out_path
...
def write_usage_by_app(
    ...
    *,
    ...
    if dry_run:
        ...
        return out_path
    return write_rollup_json(out_path, payload, prefix=".usage-by-app-")
