...
Coverage contract — READ BEFORE USING metrics/<date>/<bot>.json
---------------------------------------------------------------
This module is scheduled by ``ai.openclaw.evolve.measure`` at
``StartCalendarInterval {Hour: 1, Minute: 0}`` — 01:00 **pod-local**, once a
day — and passes no ``--date``, so ``--date`` falls back to ``date.today()``.
Two consequences, both load-bearing for every reader:

1. **The file named for day D is written one hour into D, and nothing
   regenerates it.** Every file in ``metrics/2026-09-02/`` has mtime
   ``Sep 2 01:00``; every file in ``metrics/2026-09-03/`` has ``Sep 3 01:01``.
   So the rollup for D permanently contains only what had accrued by 01:00
   on D.

2. **The label and the source use different calendars.** ``date.today()`` is
   pod-local; ``load_annotations`` reads a UTC-named file. The file called
   ``metrics/<D>/`` therefore describes the first ~8 hours of *UTC* day D,
   which west of UTC is mostly the *previous* pod-local evening.

Measured on the mini for 2026-09-03: the pod's files held **$0.95 against
$55.52** of real pod-local-day spend — **1.7%** — and the heaviest bot's
said ``$0.0078`` where the day's live total was ``$51.45``. Judged on its
own terms it is no better: summing the very annotations this module reads,
the UTC day each file is NAMED for came to $25.28, of which it captured
$0.95 (3.8%).

**This file is therefore not "day D's totals" and must never be read as
"today's spend."** The surfaces that did — the Cost Measures cap-warning
chip, the trust-page heartbeat spend reason, and the weekly spend summary —
moved to ``live_spend``, which reads turn JSONL and buckets on the pod-local
day. Aggregate/analytic readers (session counts, maintenance ratios, trend
charts) are left as they are: the truncation is uniform across days, so
shapes and ratios survive it even though dollar figures do not.

Both points above are defects in this module, not just reader misuse, and
neither is fixed here — fixing them changes what every file already on disk
means and needs its own change with a backfill decision. See the PR that
added this note.
"""
import argparse
import json
...
def load_annotations(shared_dir: str, bot_id: str, target_date: date) -> list[dict]:
    """Load a bot's turn annotations for one **UTC** day.

    NOT a local date, despite what this docstring said until 2026-09-04.
    The plugin names these files off ``toISOString()``, so
    ``annotations/<bot>/2026-09-03.jsonl`` holds the UTC day: on the Pacific
    pod it was still being appended to at 16:55 local on Sep 3 (23:55 UTC)
    and the Sep 4 file opened at 17:00 local on Sep 3. ``main`` passes a
    ``date.today()`` that is pod-LOCAL, so the two calendars are crossed
    here without conversion — see the module docstring.
    """
