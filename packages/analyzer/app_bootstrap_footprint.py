...
# identity: this module was SWEPT onto applications.app_identity.resolve_app_id in AL-1.4b (area 4c): the per-app
# label came from an ``id or pkg_id`` chain — the REVERSE of canonical — and
# now calls the resolver. The two remaining mentions are the comment naming
# that chain.
...
def compute_app_bootstrap_footprint(bot_id: str) -> dict:
    ...
    for manifest in manifests:
        # AL-1.4b: identity via the ONE resolver. The inline chain here was
        # ``id or pkg_id`` — the REVERSE of the canonical order — so a
        # gallery-installed app was labelled by its slug in this chip and by
        # its ``pkg_id`` in every reader the resolver already backs.
        app_id = resolve_app_id(manifest) or "<unknown>"
