...
Any emitter of the "ready" alert must answer "will the operator actually find
this in the Inbox?" the SAME way the page does, so the "📋 N proposals ready →
Recommendations → Inbox" notification never points at an inbox the proposal
categorically cannot appear in. Having one predicate here prevents the literal
from drifting across emitters.

(``review.py`` was a second emitter — it re-emitted the alert after a security
pass — until it retired 2026-08-14 (#3641). Its successor,
``arbiter/security_screen.py``, is a pure predicate and emits nothing, so
``analyze.py`` is the only emitter today.)
"""
