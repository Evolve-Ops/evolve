...
def _read_run_state(shared_dir: Path, bot_id: str) -> dict | None:
...
def build_signal_for_failing_backup(
    ...
    if classified:
        ...
        fix_steps_str = (
            ...
            f"/Users/{bot_id}/.openclaw/logs/backup.log on the mini"
        )
...
def _ssh_key_present(bot_user: str, bot_id: str) -> bool | None:
    """Return True/False if /Users/{bot_user}/.ssh/evolve-backup-{bot_id}
    ...
    NOT on .ssh/. The sudoers grant we rely on is:
        ...
        evolve ALL=(root) NOPASSWD: /bin/ls /Users/*/.ssh

    granted in /etc/sudoers.d/evolve. We run ``sudo -n /bin/ls`` on the
    directory and look for the filename in stdout. A missing parent
    .ssh/ counts as missing (no key can be present without the dir).
    ...
    try:
        r = subprocess.run(
            ["sudo", "-n", "/bin/ls", f"/Users/{bot_user}/.ssh"],
...
def build_signal_for_missing_ssh_key(
    ...
    (if missing) and copies it to /Users/<bot_user>/.ssh/.
    """
    key_path = f"/Users/{bot_user}/.ssh/evolve-backup-{bot_id}"
    ...
    fix_steps = (
        f"1. SSH to the mini and run: `sudo evolve-admin deploy {bot_id}`\n"
        f"2. Verify the key landed: "
        f"`sudo /bin/ls /Users/{bot_user}/.ssh/evolve-backup-{bot_id}`\n"
        ...
        f"`/Users/evolve/.ssh/evolve-backup-{bot_id}.pub` is registered "
