...
2. **Backup repo visibility.** Phase 1 of the backup architecture
   rework (spec internal/spec-backup-and-data-classification-2026-05-28.md)
...
def build_signal_for_unverified_repo(
    ...
    *,
    ...
    ``reason``: ``"missing_pat"`` (no token configured), ``"unreadable_pat"``
    (a token IS stored but this process cannot decrypt the vault), or
    ``"lookup_failed"`` (token present but API call returned unknown).
    Same Signal type; body differs.

    ``missing_pat`` and ``unreadable_pat`` are kept apart deliberately. They
    used to be one branch (``if not pat``), and the resulting "create a PAT"
    copy is what made the 2026-08-18 outage unreadable: the operator had
    created one. Creating a second would not have helped.
    """
    if reason == "unreadable_pat":
        title = f"{bot_id} backup repo visibility cannot be verified (PAT unreadable)"
        body = "\n".join([
            f"`{bot_id}` has a backup repo configured (`{repo_url}`) and a "
            "pod GitHub PAT **is** stored — but the process that needs it "
            "cannot decrypt the keystore vault.",
            "",
            "**Why**: `{shared}/keystore/vault` and its `.machine-key` are "
            "readable only by `evolve` (PR #3696, deliberately). Whatever ran "
            "this check was not `evolve` and could not read them.",
            "",
            "**This is not a setup step.** Do NOT re-run the Backup → Cloud "
            "wizard — a PAT is already stored and storing it again grants no "
            "vault access. Do NOT widen the vault's permissions either; that "
            "re-opens what #3696 closed.",
        ])
        fix_steps = (
            "1. Confirm which user ran the check — the bot-side backup daemon "
            "is supposed to ask the admin daemon for a verdict "
            "(`/api/backup-bot/visibility`), not read the PAT itself\n"
            "2. Confirm the admin daemon is up: `sudo launchctl print "
            "system/ai.evolve.evolve.admin-ui`\n"
            "3. Confirm `{shared}/admin-daemon.sock` is reachable from the "
            "calling user; `sudo evolve-admin ensure-pod-perms` re-asserts "
            "its mode and group ACE"
        )
    elif reason == "missing_pat":
...
def _build_visibility_signal(
    ...
    *,
    ...
    pat_state_loader=_load_github_pat_state,
) -> dict | None:
    """Decide which visibility Signal (if any) to emit for this bot."""
    # Tri-state, so "a PAT is stored but I can't read it" stops rendering as
    # "you never configured a PAT" — the two need different remediations.
    # ``pat_loader`` is retained for callers/tests that inject a plain value;
    # when it is overridden we honour it and skip the state read.
    if pat_loader is not _load_github_pat:
        pat = pat_loader(config)
        pat_state = _PAT_PRESENT if pat else _PAT_ABSENT
    else:
        pat_state, pat = pat_state_loader(config)
    if pat_state == _PAT_UNREADABLE:
        return build_signal_for_unverified_repo(
            bot_id, repo_url, reason="unreadable_pat",
        )
    if not pat:
        return build_signal_for_unverified_repo(
