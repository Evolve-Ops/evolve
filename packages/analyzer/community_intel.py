...
def summarize_with_llm(sources: list[dict], scan_date: date, config: dict) -> dict | None:
    """
    Call tier3 LLM to summarize gathered sources.
    Returns parsed dict or None if LLM unavailable.
    """
    try:
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"[community_intel] LLM call failed: {result.stderr[:200]}", file=sys.stderr)
            return None
        ...
        print(f"[community_intel] LLM summarization failed: {e}", file=sys.stderr)
        return None
...
def main() -> None:
    ...
    # Summarize
    print("[community_intel] Summarizing with LLM (tier3)...")
    summary = summarize_with_llm(sources, scan_date, config)
    if summary is None:
        print("[community_intel] LLM unavailable — generating fallback report", file=sys.stderr)
