...
# ``ManifestMention.app_id`` came from an ``instance_id or id`` chain and now
# calls the resolver. The two remaining mentions are the comment naming it.
    ...
    *,
    ...
    for entry in entries:
        ...
        out.append(ManifestMention(
            # AL-1.4b: identity via the ONE resolver. Was
            # ``instance_id or id`` — an INVERTED chain that also skipped
            # ``pkg_id``, so a manifest surfaced here under a different id
            # than the audits this toolkit exists to investigate use.
            app_id=resolve_app_id(data),
            app_name=str(
