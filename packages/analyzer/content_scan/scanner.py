...
Spec: internal/spec-prompt-injection-scanner-2026-05-10.md §5.2.
...
def _read_text(path: Path, *, timeout: float = 5.0) -> tuple[str | None, str | None]:
    ...
    *traverse* to the file and find it absent; on an ACL-degraded pod (the
    ...
    try:
        ...
        # Present and readable, just not valid text — a file truncated
        # mid-multibyte character. DAMAGE, not absence and not an access
        # failure, so it must reach the caller as an err rather than escaping
        # (a UnicodeDecodeError is a ValueError, so the OSError arm below never
        # caught it: it propagated out of _scan_target, aborted the whole run,
        # and was swallowed in audit.py — which is why the very signal meant to
        # report a damaged doc could never fire for the damage that matters
        # most). No sudo fallback: the bytes are already readable, so `cat`
        # would fetch the same bytes and fail the same decode. Twin of the
        # seeding-side fix in bot_doc_seeding.read_existing_doc (#3795); this
        # is the DETECTING half of the same blind spot.
...
def _scan_target(
    *,
    ...
    if text is None:
        ...
        # parent) — worth paging. err="undecodable" is a THIRD case: the file
        # is present and readable but its bytes aren't text, i.e. real damage
        # that will not self-clear (see below). Every other err (sudo_rc=N,
        # timeout, os_error/sudo_error) is an access failure: almost always a
        # transient permission/ACL flap, NOT content loss. Conflating the two at alert
        ...
        elif err == "undecodable":
            # SAME type, severity and signature_scope as the access-failure case
            # below — only the operator-facing words differ. Keeping the type
            # keeps the signature stable (and needs no producer/protection
            # registry change), but the standard body would be actively WRONG
            # here: this is damage, it will not clear on its own, and telling
            # the operator to go check file permissions sends them after a
            # problem that does not exist. A signal that fires with the wrong
            # advice is barely better than one that never fires.
            findings.append({
                "type": "content_scan_file_unreadable",
                "severity": "warn",
                "signature_scope": scope,
                "title": f"{bot_id}: {file_relpath} looks damaged",
                "body": (
                    f"{file_relpath} on {bot_id} is there but its contents "
                    "aren't readable as text — the file looks cut off partway "
                    "through, which usually means a write or a restore didn't "
                    "finish. This will not clear on its own. Restore the file "
                    "from a backup."
                ),
                "details": details,
            })
