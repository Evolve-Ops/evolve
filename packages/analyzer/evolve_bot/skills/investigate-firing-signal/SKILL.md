...
When you encounter a firing signal in `pod_state.signals.firing` and the
operator wants triage (or you're proactively offering one), follow this
recipe:

## Steps

1. **Read the signal.** `pod_state.signals.firing` returns the signal's
...
3. **Check for a matching proposal.** `pod_state.proposals.pending`
   ...
   the proposal_id; offer `action.proposal.apply`.
...
4. **Decide the next step:**
   ...
   - **No proposal, direct tool exists** (e.g. `action.bot.restart`,
     `action.signal.snooze`) → describe what you'd run and offer to
     run it.
...
5. **Verify after action.** Every action tool you call returns a
   ...
   `pod_state.bots(bot_id='team-bot-a')` reports status=online.
...
- **Cite every claim.** "Looking at `pod_state.bots(bot_id='team-bot-a')`,
...
>
> 1. `pod_state.signals.firing(bot_id='team-bot-a')` → one signal,
>    `perm_cron_uncapped_agent_turn` on the morning-brief job.
> 2. Signal flavor is `security` → `pod_state.audit(bot_id='team-bot-a')`
>    confirms no other findings; the cron-caps issue is the only one.
> 3. `pod_state.proposals.pending(bot_id='team-bot-a')` → one matching
...
>    to apply it?"
> 5. On confirm: `action.proposal.apply(proposal_id='p-abc12345')` →
>    verify via `pod_state.signals.firing(bot_id='team-bot-a')` showing the
