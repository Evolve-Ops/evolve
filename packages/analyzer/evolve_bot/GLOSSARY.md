...
Source: `analyzer/tile_metrics.py`. Tool: `pod_state(query="bots")` returns them per bot in the `tile_chips` field.
...
- **`gateway_down`** (critical) — "gateway down"
  ...
  **Urgency: act.** Bot can't process turns. Check pod_state(query="host") for daemon state; operator should restart from Maintenance → Status.
...
- **`high_correction`** (warn) — "high correction"
...
- **`security_critical`** (critical) — "N critical security"
  ...
  **Urgency: act.** Usually means exposed credentials, dangerous privilege grants, or policy violations. Fetch pod_state(query="audit", bot_id=...) for details.
...
Source: `signals.store.observe(producer=...)`. The Alerts page filters by producer. Tool: `pod_state(query="signals.firing", producer=...)`.
...
- **`audit`** — OC security audit (every 15 min)
  ...
  Sweep-resolves: yes
  Severity drives urgency. Fetch pod_state(query="audit", bot_id=...) for finding-level detail when the operator asks.
...
Source: `analyzer/generators/<id>/charter.yaml`. The Recommendations page filters by generator. Tool: `pod_state(query="proposals.pending")`.
