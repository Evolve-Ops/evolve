...
Runs every 15 minutes as the `evolve` user. Performs four categories of checks:
  ...
  3. Machine audit   — firewall, FileVault, admin-user gateway, SSH config, macOS updates,
                       listening ports, user accounts, OC binary mtime, and the four
                       incursion detectors (added SSH authorized keys, PAM drift, a
                       scheduled job Evolve did not install, a login from a new source
                       — see packages/analyzer/incursion/)
...
from evolve_config import (
    ...
    bot_home as _bot_home, get_bot_user, user_home as _user_home,
)
import drift_authorization
from evolve_util import assert_safe_sudo_dest
from heal import _is_gateway_proc_line
from platform_profile import find_openclaw_cli, get_profile
...
class Finding:
    ...
    what_it_means: str = ""
    fix_steps: str = ""
    # Event-vs-posture classification (R-3; design:
    # internal/design-security-alert-fatigue-2026-08-31.md §2.3).
    #   "event"   — active compromise / credential exposure / "an attacker
    #               may act right now"; must page immediately.
    #   "posture" — standing configuration violation that needs a one-time
    #               fix (bot holds admin, SSH password path open, sudoers
    #               baseline drift, …); pages once on discovery, then lives
    #               on the Alerts board / digest, never re-pages daily.
    # REQUIRED for level="critical" — __post_init__ rejects an unclassified
    # critical so a new emit site cannot ship without an explicit choice.
    # When genuinely ambiguous, classify EVENT (fail toward paging).
    finding_kind: str = ""

    def __post_init__(self) -> None:
        if self.level == "critical":
            if self.finding_kind not in ("event", "posture"):
                raise ValueError(
                    "critical Finding requires an explicit finding_kind of "
                    f"'event' or 'posture' (got {self.finding_kind!r}): "
                    f"{self.message!r}"
                )
        elif self.finding_kind not in ("", "event", "posture"):
            raise ValueError(
                f"invalid finding_kind {self.finding_kind!r} for "
                f"level={self.level!r} Finding: {self.message!r}"
            )


# ── Logging ───────────────────────────────────────────────────────────────────

def audit_log_path(shared_dir: Path) -> Path:
    """The operator-facing audit log for the pod rooted at ``shared_dir``.

    Derived, never a module constant: a hard-coded ``/Users/Shared/evolve``
    made every test that drove ``dispatch_findings`` append its fixture
    findings — "CRITICAL: sshd PasswordAuthentication enabled" and friends —
    to the REAL log an operator greps during an incident, whatever
    ``tmp_path`` the test was handed. Same shape as ``_send_warn_log``'s
    ``{shared_dir}/logs/audit-warns.jsonl``.
    """
    return shared_dir / "logs" / "audit.log"


def _log(msg: str, shared_dir: Path) -> None:
    """Print ``msg`` and append it to ``{shared_dir}/logs/audit.log``.

    ``shared_dir`` is required and has no default, so a new call site cannot
    silently write to the production log: it either has the pod root in hand
    or it does not type-check. Fail-soft on the write itself — the log is
    observability, never a reason to abort a security run.
    """
    print(msg, flush=True)
    log_file = audit_log_path(shared_dir)
    try:
...
# ── L2 authorized-change gate ─────────────────────────────────────────────────
#
# internal/spec-drift-alert-taxonomy-2026-06-26.md L2. Every baseline check
# below asks the SAME question before it calls a difference a security
# finding: is this change explained by a known authorized event? Explained
# drift becomes an informational row; unexplained drift becomes an
# event-classified CRITICAL that pages every time.
#
# The gate itself lives in drift_authorization (deterministic, read-only, no
# model judgement). Every call goes through the wrapper below so a gate that
# raises cannot take the whole audit down — and note the failure direction:
# a gate that cannot answer returns None, which means "unexplained", which
# means the finding pages. A suppression gate must fail toward doing the work.


def _explain_drift(
    change: "drift_authorization.DriftChange", shared_dir: Path,
) -> "drift_authorization.Explanation | None":
    """Ask the L2 gate about ``change``; ``None`` means nothing explains it."""
    try:
        return drift_authorization.explain(change, shared_dir)
    except Exception as exc:  # noqa: BLE001 — an unusable gate must not suppress
        logger.warning(
            "audit: authorized-change gate failed for %s (%s) — treating the "
            "change as unexplained: %s", change.kind, change.target, exc,
        )
        return None
...
def sha256_git_workspace(bot_id: str, fname: str) -> str | None:
    ...
    try:
        r = subprocess.run(
            ["sudo", get_profile().git, "-c", "safe.directory=*",
             "-C", str(workspace), "show", f"HEAD:{fname}"],
            capture_output=True, timeout=5,
        )
        if r.returncode != 0:
            return None
...
def _send_telegram_direct(
    token: str, chat_id: str, msg: str, shared_dir: Path,
) -> bool:
    """Send a Telegram message directly via Bot API (no OC gateway required).

    Returns ``True`` iff the POST completed without raising (urllib raises
    ``HTTPError`` on any non-2xx, so no-raise means the Bot API accepted
    the message). The caller uses this to decide whether the paging
    episode may be marked delivered.
    """
    ...
    try:
        ...
        _log(f"[audit] CRITICAL alert delivery failed via direct Telegram: {e}", shared_dir)
        return False


def _send_security_alert(msg: str, shared_dir: Path, config: dict) -> bool:
    """Send a CRITICAL audit alert. Returns ``True`` iff it went out.
    ...
    The return value is the delivery outcome the page-on-transition gate
    records on the Signal: ``True`` only when the message actually reached
    the operator (or a digest queue that will deliver it), ``False`` for
    every other ending. A ``False`` here must not mark the firing episode
    paged — see ``dispatch_findings``.

    The keystore fallback fires only on ``_DISPATCH_BROKEN``. A dispatcher
    that is *withholding* the alert (cooldown, 24h identical-content floor,
    source disabled, no recipient) is working as configured, and routing
    around it via the dedicated keystore credentials would be exactly the
    subscription bypass this function disclaims. Those endings return
    ``False`` without a fallback attempt: nothing reached the operator, so
    the episode stays eligible and the next run tries again.
    """
    outcome = _send_via_dispatcher(msg, shared_dir, config)
    if outcome == _DISPATCH_DELIVERED:
        return True
    if outcome == _DISPATCH_SUPPRESSED:
        return False
    ...
    if sec_token_path.exists() and sec_chat_id_path.exists():
        try:
            ...
            if token and chat_id:
                return _send_telegram_direct(token, chat_id, msg, shared_dir)
            _log("[audit] CRITICAL alert dropped — dispatcher failed and "
                 "security-alert keystore is empty", shared_dir)
        except OSError:
            _log("[audit] CRITICAL alert dropped — dispatcher failed and "
                 "security-alert keystore unreadable", shared_dir)
    else:
        _log("[audit] CRITICAL alert dropped — dispatcher failed and no "
             "security-alert keystore fallback is configured", shared_dir)
    return False


# Three ways a dispatcher call can end, from the paging gate's point of view.
# The distinction that matters is NOT "did the dispatcher run" but two separate
# questions: did the message reach the operator (may we mark the firing episode
# paged?), and is the dispatcher itself broken (should we escalate to the direct
# keystore fallback?). Collapsing those into one bool is what let a suppression
# read as a delivery.
_DISPATCH_DELIVERED = "delivered"   # reached the operator, or is queued to
_DISPATCH_SUPPRESSED = "suppressed"  # dispatcher deliberately withheld it
_DISPATCH_BROKEN = "broken"          # dispatcher couldn't do its job


def _send_via_dispatcher(msg: str, shared_dir: Path, config: dict) -> str:
    """Route an audit batch through the alert dispatcher.

    Returns one of ``_DISPATCH_DELIVERED`` / ``_DISPATCH_SUPPRESSED`` /
    ``_DISPATCH_BROKEN``.

    - **DELIVERED** — ``SENT``, or queued to a digest the flusher owns
      (``DEFERRED``, ``BATCHED_RATE_CAP``). The operator will see it, so the
      firing episode may be marked paged.
    - **SUPPRESSED** — ``SUPPRESSED_DISABLED`` / ``_COOLDOWN`` / ``_IDENTICAL``
      / ``NO_RECIPIENT``. Nothing reached the operator, so the episode must
      NOT be marked paged — but the dispatcher is healthy and is withholding
      on purpose, so escalating to the direct keystore path would be the
      subscription bypass this function exists to avoid. Retry next run; when
      the cooldown or 24h identical-content floor lapses, the send goes out.
    - **BROKEN** — ``FAILED``, import failure, an exception, or a result this
      code does not recognize. The direct-Telegram fallback is the right next
      step. An unrecognized result lands here on purpose: it is not a
      confirmed delivery, and for a CRITICAL a duplicate beats silence.

    Why suppression cannot count as delivery, concretely: the dispatcher
    records ``(dedup_key, body_hash, ts)`` on the FAILURE path too, by
    deliberate design (see its "Record cooldown state for BOTH success and
    failure paths" comment). ``audit`` is a ``STATE_PERSISTS`` source, which
    does not opt out of the 24h identical-content floor. So a page that fails
    at t+0 and is retried at t+15m with byte-identical text comes back
    ``SUPPRESSED_IDENTICAL`` — and if that read as "delivered", the retry this
    module now performs would cancel itself on its second run and silence the
    critical for the rest of the episode.
    ...
    Paging policy lives upstream (Phase 5b, page-on-transition): by the
    time a message reaches this function it contains only newly-firing
    findings, so the dedup_key is a residual guard, not the gate.

    catalog_event is ``security.audit_critical`` (CRITICAL / IMMEDIATE),
    not the ``security.audit_finding`` umbrella. The umbrella is ERROR /
    DAILY_DIGEST by deliberate design (workstream D1) because it carries
    the flap-prone WARN+ERROR traffic; annotating THIS batch with it made
    the dispatcher return DEFERRED for every newly-firing CRITICAL on a
    default-configured pod, so "page on transition" landed in the next
    daily digest instead of paging. Cadence is a property of the catalog
    event, so the two need separate events. signal_notifier still maps
    every audit-produced Signal to the umbrella — that is the standing
    board, and it is what keeps this key rare.
    """
    try:
        ...
        _log(f"[audit] dispatcher import failed; falling back to direct: {exc}", shared_dir)
        return _DISPATCH_BROKEN

    delivered = {DispatchResult.SENT, DispatchResult.DEFERRED,
                 DispatchResult.BATCHED_RATE_CAP}
    suppressed = {DispatchResult.SUPPRESSED_DISABLED,
                  DispatchResult.SUPPRESSED_COOLDOWN,
                  DispatchResult.SUPPRESSED_IDENTICAL,
                  DispatchResult.NO_RECIPIENT}
    ...
    try:
        outcome = _dispatch_send(
            ...
            catalog_event="security.audit_critical",
        )
    except Exception as exc:
        _log(f"[audit] dispatcher.send raised; falling back to direct: {exc}", shared_dir)
        return _DISPATCH_BROKEN

    if outcome.result in delivered:
        return _DISPATCH_DELIVERED
    if outcome.result in suppressed:
        _log(f"[audit] dispatcher withheld the CRITICAL page "
             f"({outcome.result}) — episode stays un-paged, retrying next run", shared_dir)
        return _DISPATCH_SUPPRESSED
    return _DISPATCH_BROKEN
...
def _write_findings_snapshot(
    ...
    Callers must only invoke this for a FULL run (see ``_covers_whole_pod``):
    the write is a wholesale overwrite of the pod-wide open set, and
    ``audit_completed_at`` claims a complete run just happened.
    ...
    payload = {
        ...
        "critical": [
            # finding_kind (R-3): "event" | "posture" — lets snapshot
            # readers (pod report, digest) split page-now events from
            # standing posture lines. Additive key; readers .get() it.
    ...
    try:
        ...
        _log(f"[audit] WARN: failed to write current-findings snapshot: {e}", shared_dir)
...
def audit_identity(
    ...
    # Absolute git path from the profile table so the argv matches the §24 sudoers
    # grant; `-c safe.directory=*` because git (2.35.2+) refuses to operate on a
    # bot-owned repo even as root ("detected dubious ownership") — verified live
    # on evolve-vps-pod 2026-07-29.
    r = subprocess.run(
        ["sudo", get_profile().git, "-c", "safe.directory=*",
    ...
    for fname in IDENTITY_FILES:
        ...
        if live_hash != git_hash:
            # L2 authorized-change gate. This composes with — it does not
            # replace — the R-2 flap dwell on this same family
            # (_is_identity_mismatch_finding). The two answer different
            # questions and run in that order: the dwell asks "is this real
            # or a bot's own edit racing the next backup commit?", and only
            # a mismatch that survives 2+ consecutive runs reaches the
            # operator; the gate asks "is a real one authorized?". Dropping
            # the dwell here would re-open the churn the 2026-08-31 design
            # closed, so the message text stays inside the flap family's
            # match and a sustained UNexplained mismatch dwells, then pages
            # as an event.
            explanation = _explain_drift(drift_authorization.DriftChange(
                kind=drift_authorization.KIND_IDENTITY_FILE,
                bot_id=bot_id,
            ...
            if explanation is not None:
                findings.append(Finding(
                    level="ok", category="identity", bot_id=bot_id,
                    message=(
                        f"{bot_id}: {fname} differs from the backup — "
                ...
                findings.append(Finding(
                    ...
                    # event: nothing on record accounts for the instruction
                    # file diverging from its backup. The flap dwell still
                    # applies (see above), so this pages once it is
                    # sustained rather than on the first blip.
                    finding_kind="event",
                    message=f"🔴 CRITICAL: {bot_id} {fname} hash mismatch vs git backup",
                    detail=f"live={live_hash[:12]} backup={git_hash[:12]}",
                    what_it_means=(
                        "This assistant's instruction file is not the one in "
                        "its backup, and no deploy, approved change, or "
                        "action of yours accounts for the difference."
                    ),
                ))
        else:
            findings.append(Finding(
...
def audit_policy_file_permissions(bot_id: str) -> list[Finding]:
    ...
    for fname in LOCKED_POLICY_FILES:
        ...
        if mode & 0o222:  # any write bit set → policy is mutable by SOMEONE
            findings.append(Finding(
                level="critical", category="identity", bot_id=bot_id,
                # posture: a writable policy file is a standing mode
                # violation with a one-line chmod fix — same shape as the
                # bot-privilege posture class (capability, not activity).
...
def audit_config(bot_id: str, config: dict, shared_dir: Path) -> list[Finding]:
    ...
    if bind and bind not in ("127.0.0.1", "localhost", "loopback", ""):
        findings.append(Finding(
            ...
            # event: a non-loopback bind is a hard invariant that never
            # legitimately changes — an off-loopback gateway is reachable
            # from the network RIGHT NOW (ambiguous ⇒ fail toward paging).
            finding_kind="event",
...
def _check_provider_models_registry(bot_id: str, oc: dict, findings: list[Finding]) -> None:
    ...
    findings.append(Finding(
        ...
        fix_steps=(
            ...
            f"3. Verify with: sudo /bin/cat {_bot_home(bot_id)}/.openclaw/openclaw.json "
...
def _check_tier_to_openclaw_drift(
    ...
    # Direct read first (the evolve read ACL is primary — CLAUDE.md §File
    # Access Pattern), sudo /bin/cat only as the fallback for a transient
    # Linux ACL-mask clamp on a parent dir. This audit used to sudo-cat
    # FIRST, which burned a sudo denial per run on every pod (the §3i grant
    # didn't exist until 2026-07-29) while the file was ACL-readable all
    # along — 127 denials/day on the VPS for a file with #effective:r--.
    raw: str | None = None
    try:
        raw = tiers_path.read_text()
    except FileNotFoundError:
        # Benign: bot never visited AI Optimization — no tier source-of-truth.
        return
    except OSError:
        try:
            r = subprocess.run(
            ...
            if r.returncode != 0:
                ...
                # benign case; other errors are sudoers-config problems that
                # surface under audit_evolve_sudoers.
                return
            raw = r.stdout
        except (subprocess.TimeoutExpired, OSError):
            return
    try:
        tiers_file = json.loads(raw)
    ...
    findings.append(Finding(
        ...
        fix_steps=(
            ...
            f"2. Verify with: sudo /bin/cat {_user_home(bot_user)}/.openclaw/openclaw.json "
            f"| python3 -c \"import json,sys; "
...
def audit_evolve_sudoers(shared_dir: Path, config: dict) -> list[Finding]:
    ...
    if current_hash != stored_hash:
        # L2 authorized-change gate (internal/spec-drift-alert-taxonomy-2026-06-26.md).
        # The sudoers file is rendered from code and installed verbatim by a
        # root `refresh-sudoers`, so "authorized" here is content-bound rather
        # than time-bound: the live bytes either ARE what this Evolve version
        # installs (or what its own installer last wrote), or somebody else
        # wrote them. See drift_authorization._explain_sudoers.
        explanation = _explain_drift(drift_authorization.DriftChange(
            kind=drift_authorization.KIND_SUDOERS_BASELINE,
            bot_id="evolve",
            target=str(sudoers_path),
        ...
        if explanation is not None:
            findings.append(Finding(
                level="ok", category="config", bot_id="evolve",
                message=(
                    "evolve: sudoers changed since baseline — "
                    f"{explanation.evidence}"
                ),
            ...
            findings.append(Finding(
                ...
                # event, not posture: after the L2 gate this branch means the
                # grants file is neither what Evolve installs nor what its
                # installer wrote, and nobody recorded changing it. That is a
                # privilege change by an unknown writer — the edr
                # must-always-page floor. It is not flap-gated (the flap
                # family is warn-level perm findings plus the identity-hash
                # race) and not settle-gated (alert level never is), so it
                # pages on cycle one, fresh pod or not.
                finding_kind="event",
                message=(
                    "🔴 CRITICAL: /etc/sudoers.d/evolve changed and no "
                    "authorized change explains it"
                ),
                detail=f"stored={stored_hash[:12]} current={current_hash[:12]}",
                what_it_means=(
                    "The file that decides what Evolve is allowed to do as "
                    "an administrator is not the one Evolve installs, and "
                    "nothing on record accounts for the change. Either "
                    "somebody edited it by hand, or something else did."
                ),
                fix_steps=(
                    "1. Read the live file and compare it with the grants "
                    "Evolve expects: `sudo cat /etc/sudoers.d/evolve`\n"
                    "2. If the change is yours and correct, reinstall the "
                    "rendered file with `sudo evolve-admin refresh-sudoers` "
                    "so the record matches\n"
                    "3. If it is not yours, treat the host as compromised "
    ...
    for line_no, raw_line in enumerate(text.splitlines(), start=1):
        ...
        for pattern, label, what_it_means in _SUDOERS_DANGEROUS_PATTERNS:
            if re.match(pattern, stripped):
                findings.append(Finding(
                    level="critical", category="config", bot_id="evolve",
                    # posture: a standing over-broad grant (capability, not
                    # activity) — same class as bot-holds-admin; the fix is
                    # a one-time narrowing of the sudoers file.
                    finding_kind="posture",
                    message=(
...
# ── Incursion detectors (authorized_keys / PAM / jobs / logins) ──────────────
#
# Four baseline-diff detectors for the persistence moves this file did not
# watch for before 2026-09-02: an added SSH authorized key, PAM policy drift,
# a scheduled job Evolve did not install, and an interactive login from a
# source never seen before. Each lives in its own module under
# ``incursion/`` — read-only, baseline-on-first-run, ``event``-classified on
# an unexplained change — and each is registered below as its own machine
# check so a failure in one is visibly one detector's failure.
#
# The package deliberately does not import ``audit``: keeping the arrow
# one-way is what lets ``python3 -m incursion.report`` run a full pass on a
# live pod without dragging in alert dispatch or the signal store. The
# translation from ``incursion.Observation`` to ``Finding`` is here, at the
# boundary, and is the whole of the coupling.

_INCURSION_DETECTORS = ("authorized_keys", "pam", "job_inventory", "logins")


def _incursion_check(name: str, shared_dir: Path, config: dict) -> list[Finding]:
    """Run one incursion detector, as a machine-category Finding list.

    A detector that cannot be imported or that raises degrades to a single
    ``skipped`` finding naming itself. ``skipped`` (not ``warn``) because a
    broken detector is audit infrastructure failing, not a pod condition —
    the same call this file already makes for a check whose prerequisite is
    missing, and ``_emit_signals_from_findings`` keeps it off the Alerts page.
    """
    try:
        import importlib

        module = importlib.import_module(f"incursion.{name}")
        observations = module.check(shared_dir, config)
    except Exception as exc:  # noqa: BLE001 — one detector must not end the audit
        logger.warning("audit: incursion detector %s failed: %s", name, exc)
        return [Finding(
            level="skipped", category="machine", bot_id=None,
            message=f"machine: incursion {name} detector did not run",
            detail=f"{type(exc).__name__}: {exc}"[:300],
        )]
    return [
        Finding(
            level=o.level, category="machine", bot_id=None,
            message=o.message, detail=o.detail,
            what_it_means=o.what_it_means, fix_steps=o.fix_steps,
            finding_kind=o.finding_kind,
        )
...
_MACHINE_CHECKS: tuple[tuple[str, Any, frozenset[str]], ...] = (
    ...
    # The inverse of admin_user_gateway — a BOT account holding host
    # admin/sudo reaches the same place from the opposite direction. Sits
    # next to its twin deliberately; _ALL_PLATFORMS because the Linux pod has
    # the same exposure through group sudo/wheel and /etc/sudoers.d.
    ("bot_user_privilege", lambda shared_dir, config: _check_bot_user_privilege(shared_dir, config), _ALL_PLATFORMS),
    ...
    # Incursion detectors. _ALL_PLATFORMS: each one resolves its own sources
    # per platform profile internally (``/etc/pam.d`` and ``last`` are the
    # same on both; the job inventory swaps launchd roots for systemd+cron),
    # so there is no macOS-only binary to FileNotFound into a false critical.
    ("incursion_authorized_keys", lambda shared_dir, config: _incursion_check("authorized_keys", shared_dir, config), _ALL_PLATFORMS),
    ("incursion_pam",             lambda shared_dir, config: _incursion_check("pam", shared_dir, config),             _ALL_PLATFORMS),
    ("incursion_job_inventory",   lambda shared_dir, config: _incursion_check("job_inventory", shared_dir, config),   _ALL_PLATFORMS),
    ("incursion_logins",          lambda shared_dir, config: _incursion_check("logins", shared_dir, config),          _ALL_PLATFORMS),
)
...
def _check_firewall() -> list[Finding]:
    ...
    return [Finding(
        ...
        # posture: standing hardening gap, fix-once (enable the firewall).
        finding_kind="posture",
...
def _check_filevault(config: dict | None = None) -> list[Finding]:
    ...
    if "filevault is off" in out_lower:
        ...
        return [Finding(
            level="critical", category="machine", bot_id=None,
            # posture: standing hardening gap, fix-once (turn FileVault on
            # or record a policy acceptance).
            finding_kind="posture",
...
def _check_macos_updates(
    ...
    if security:
        return [Finding(
            level="critical", category="machine", bot_id=None,
            # posture: an unpatched published CVE is urgent but standing —
            # fix-once (install the update); it does not mean an attacker
            # is acting now.
            finding_kind="posture",
...
def _check_linux_firewall(config: dict | None = None) -> list[Finding]:
    ...
    return [Finding(
        level="critical", category="machine", bot_id=None,
        # posture: standing hardening gap, fix-once (enable ufw or accept).
        finding_kind="posture",
...
def _check_linux_disk_encryption(config: dict | None = None) -> list[Finding]:
    ...
    return [Finding(
        level="critical", category="machine", bot_id=None,
        # posture: standing hardening gap, fix-once (LUKS or acceptance).
        finding_kind="posture",
...
def _check_linux_os_updates(config: dict | None = None) -> list[Finding]:
    ...
    if security > 0:
        ...
        return [Finding(
            level="critical", category="machine", bot_id=None,
            # posture: unpatched published CVEs — urgent fix-once (apply
            # updates, enable unattended-upgrades), not attacker-acting-now.
            finding_kind="posture",
...
def _check_admin_user_gateway(config: dict) -> list[Finding]:
    ...
    return [Finding(
        level="critical", category="machine", bot_id=None,
        # event: a LIVE LLM gateway is executing at operator privilege
        # right now — anyone who can prompt-inject it acts as the admin.
        # "Stop the offending gateway IMMEDIATELY" is a page, not a board.
        finding_kind="event",
...
# ── Bot-user host privilege — the OTHER half of the never-do invariant ───────
#
# ``_check_admin_user_gateway`` (above) guards one direction of the privilege
# boundary: the pod ADMIN account must never run an openclaw gateway, because
# an LLM would then execute commands with sudo. The inverse direction reaches
# exactly the same place and was unguarded until 2026-08-02, when a live sweep
# of the Mac mini pod found two legacy bot accounts sitting in the macOS
# ``admin`` group — which stock ``/etc/sudoers``
# (``%admin ALL = (ALL) ALL``) turns into full sudo, and which
# ``com.apple.access_ssh`` nests, making both accounts SSH-reachable with a
# password. Nothing in the audit noticed; the only adjacent alert was
# ``machine: SSH PasswordAuthentication is enabled``, which names the
# transport but not the privilege underneath it.
#
# Detection only, by design. Removing an account from ``admin`` is an operator
# decision with blast radius (a bot's own launchd/keychain state may depend on
# it), so this check names the bot and the mechanism and stops there. There is
# deliberately NO ``policy_acceptance`` escape hatch: this is a never-do
# invariant, the same as its twin above, and the only clearing action is
# actually dropping the privilege.

_MACOS_PRIVILEGED_GROUPS: tuple[str, ...] = ("admin",)
"""macOS groups that confer host privilege. ``admin`` is the stock
``%admin ALL = (ALL) ALL`` sudoers rule AND the group Setup Assistant nests
into ``com.apple.access_ssh`` / ``com.apple.access_screensharing``."""

_LINUX_PRIVILEGED_GROUPS: tuple[str, ...] = ("sudo", "wheel", "admin")
"""Linux groups that confer host privilege. Debian/Ubuntu ships ``%sudo``,
RHEL-family ships ``%wheel``; ``admin`` is the legacy Ubuntu name still
present on upgraded hosts."""

_MACOS_SSH_ACCESS_GROUP = "com.apple.access_ssh"

_MACOS_DS_USER_RECORDS = "/Users"
"""DirectoryService record prefix for local user accounts — the node ``dscl``
reads records under (``dscl . -read <this>/<account> AuthenticationAuthority``),
NOT a filesystem path. It is spelled the same as macOS's home root by
coincidence of Apple's naming: it does not track
``platform_profile.user_home_root``, it does not move when a pod relocates
homes, and it has no Linux counterpart (every caller here is macOS-gated).
Named so that distinction is explicit at the call site rather than reading
as a hardcoded home-root literal."""

_SUDOERS_DROPIN_DIR = "/etc/sudoers.d"

_BOT_PRIVILEGE_STATE_REL = "security/bot-privilege.state.json"
"""Last FULLY-VERIFIED privileged-bot set, relative to ``{shared_dir}``.

The fail-safe anchor. ``dispatch_findings`` mirrors only critical/warn
findings into the Signal store and then sweep-resolves every audit signature
it did not re-emit — so a run that cannot read the group database would
silently auto-resolve a live CRITICAL, i.e. an unreadable source would read
as "compliant". Instead, an unverified run re-emits the last verified
findings VERBATIM (same message → same signature → the signal survives the
sweep) and adds a separate WARN naming the sources it could not read. The
file is written ONLY by a run in which every source was readable.

That rule has a deliberate consequence on a pod whose sudoers has not been
refreshed since this check shipped: the drop-in scan is blind, so the anchor
never seeds and the check sits in "could not verify". It still emits every
finding the readable sources produce — the degradation is "the fail-safe has
nothing to carry", never a false OK — and the WARN names ``refresh-sudoers``
as the fix. Making the dormant grant loud is the point; the sshd check went
the other way and was dark fleet-wide for months (#3462)."""


def _bot_unix_users(config: dict | None) -> dict[str, str]:
    """Map each bot's UNIX account name → its bot id, from network.json.

    Iterates ``bots`` directly (not ``members``): the accounts this check
    exists for are exactly the legacy ones that may have dropped off the
    member roster while their OS account — and its privilege — lived on.
    ``bots[<id>].user`` overrides the id when a bot rides an existing
    account; ``bot_id ≠ account`` is the rule, not the exception.
    """
    bots = (config or {}).get("bots")
    ...
    for bot_id, info in bots.items():
        ...
        if isinstance(info, dict):
            u = info.get("user")
            user = u if isinstance(u, str) and u else None
        users.setdefault(user or bot_id, bot_id)
    return users


def _effective_group_names(user: str) -> tuple[str, set[str]]:
    """Resolve *user*'s EFFECTIVE groups (primary + secondary + nested).

    ``id -Gn`` is the single probe that answers "what privilege does this
    account actually carry?" on both platforms, and it is the only one that
    resolves NESTED groups — the mechanism by which a macOS account in
    ``admin`` is SSH-reachable via ``com.apple.access_ssh`` without ever
    appearing in that group's own ``GroupMembership`` record (the nested
    GUID ``ABCDEFAB-CDEF-ABCD-EFAB-CDEF00000050`` *is* ``admin``). Runs
    unprivileged on both platforms — no sudoers grant, so it cannot go dark
    the way ``sshd -T`` did before #3462.

    Returns ``(status, groups)``:

    * ``"ok"``         — *groups* is authoritative.
    * ``"absent"``     — no such account (a bot declared in network.json but
                         never provisioned on this host). Not a finding.
    * ``"unreadable"`` — the probe itself failed. The caller MUST NOT read
                         this as "not privileged".
    """
    try:
        r = subprocess.run(
            [get_profile().id, "-Gn", user],
            capture_output=True, text=True, timeout=5,
        )
    except (subprocess.TimeoutExpired, OSError):
        return ("unreadable", set())
    if r.returncode != 0:
        # macOS: "id: bob: no such user"; Linux: "id: 'bob': no such user".
        if "no such user" in (r.stderr or "").lower():
            return ("absent", set())
        return ("unreadable", set())
    return ("ok", set(r.stdout.split()))


def _macos_group_members(group: str) -> tuple[str, set[str]]:
    """DIRECT ``GroupMembership`` of a macOS group, via ``dscl -plist``.

    Corroborates :func:`_effective_group_names` and gives the operator the
    exact record to edit. The ``-plist`` form keeps parsing off ad-hoc line
    splitting: a MISSING KEY renders as an empty dict at rc 0, a missing
    RECORD is rc ≠ 0. Unprivileged.
    """
    dscl = get_profile().dscl
    if not dscl:
        return ("absent", set())
    try:
        r = subprocess.run(
            [dscl, "-plist", ".", "-read", f"/Groups/{group}", "GroupMembership"],
            capture_output=True, timeout=5,
        )
    except (subprocess.TimeoutExpired, OSError):
        return ("unreadable", set())
    if r.returncode != 0:
        return ("absent", set())
    try:
        parsed = plistlib.loads(r.stdout or b"")
    except Exception:  # noqa: BLE001 — malformed plist is an unreadable source
        return ("unreadable", set())
    if not isinstance(parsed, dict):
        return ("unreadable", set())
    members = parsed.get("dsAttrTypeStandard:GroupMembership") or []
    if not isinstance(members, list):
        return ("unreadable", set())
    return ("ok", {m for m in members if isinstance(m, str)})


def _linux_group_members(group: str) -> tuple[str, set[str]]:
    """DIRECT members of a Linux group, via ``getent group``. Unprivileged.

    ``getent`` exits 2 when the key does not exist — an ABSENT group, not an
    unreadable one (``wheel`` simply does not exist on Debian/Ubuntu).
    Secondary members only; the primary-group case is covered by
    :func:`_effective_group_names`, which is the authoritative probe.
    """
    getent = get_profile().getent
    if not getent:
        return ("absent", set())
    try:
        r = subprocess.run(
    ...
    if len(parts) < 4:
        return ("ok", set())
    return ("ok", {m for m in parts[3].split(",") if m})


def _sudoers_dropin_text(name: str) -> str | None:
    """Read one ``/etc/sudoers.d`` drop-in; None when it cannot be read.

    Always via the §25 ``{cat} /etc/sudoers.d/*`` grant, with no direct-read
    fast path: drop-ins are mode 0440 root on both platforms, so a direct
    read is never the path that wins for the evolve user, and when the audit
    DOES run as root (operator CLI) ``sudo -n`` succeeds anyway. One code
    path means what the grant covers and what the check runs cannot diverge.
    """
    try:
        r = subprocess.run(
            ["sudo", "-n", get_profile().cat, f"{_SUDOERS_DROPIN_DIR}/{name}"],
            capture_output=True, text=True, timeout=5,
        )
    except (subprocess.TimeoutExpired, OSError):
        return None
...
def _sudoers_dropin_names() -> list[str] | None:
    """List ``/etc/sudoers.d``; None when the directory cannot be listed.

    §25 grant ``{ls} -1 /etc/sudoers.d``. The directory happens to be 0755 on
    macOS (a direct ``listdir`` would work there) and 0750 root:root on
    Debian/Ubuntu; the check takes the granted path on both so the two
    platforms fail — and succeed — identically.
    """
    try:
        r = subprocess.run(
            ["sudo", "-n", get_profile().ls, "-1", _SUDOERS_DROPIN_DIR],
            capture_output=True, text=True, timeout=5,
        )
    except (subprocess.TimeoutExpired, OSError):
        return None
...
def _sudoers_users_named(text: str, users: set[str]) -> set[str]:
    """Which of *users* a sudoers file names as a user spec.

    Best-effort by design, and narrow on purpose: the FIRST token of a rule
    line (``bot-a ALL=(ALL) NOPASSWD: …``) and the right-hand side of a
    ``User_Alias``. Group specs (``%admin``) are NOT parsed here — effective
    group membership is already the authoritative probe above, and parsing
    them twice would double-report the same mechanism.

    Not resolved: ``#include`` / ``@includedir`` chains, and an alias defined
    in one file but used in another. The finding says so, so an operator
    reading it knows the scan is a floor, not a ceiling.
    """
    hits: set[str] = set()
    for raw in text.splitlines():
        ...
        if not line:
            continue
        low = line.lower()
        if low.startswith(("defaults", "runas_alias", "host_alias", "cmnd_alias")):
            continue
        if low.startswith("user_alias"):
            _, _, rhs = line.partition("=")
            candidates = {t for t in re.split(r"[\s,]+", rhs) if t}
        else:
            candidates = {line.split(None, 1)[0]}
        hits |= candidates & users
    return hits


def _macos_password_auth_users(users: set[str]) -> tuple[set[str], set[str]]:
    """Split *users* into (password-capable, unreadable) on macOS.

    A local password is a ``ShadowHash`` entry in the account's
    ``AuthenticationAuthority`` record. Bots never need interactive login —
    the newer accounts on the fleet have no such record at all — so a bot
    that can password-authenticate is an unnecessary credential to guess,
    and (combined with ``com.apple.access_ssh``) a remote one.
    """
    dscl = get_profile().dscl
    if not dscl:
        return (set(), set())
    ...
    for user in sorted(users):
        try:
            r = subprocess.run(
                [dscl, "-plist", ".", "-read",
                 f"{_MACOS_DS_USER_RECORDS}/{user}", "AuthenticationAuthority"],
                capture_output=True, timeout=5,
            )
        except (subprocess.TimeoutExpired, OSError):
            unreadable.add(user)
            continue
        if r.returncode != 0:
            # Record absent — the account was resolved by `id` moments ago,
            # so treat a read failure here as unreadable, not as "no auth".
            unreadable.add(user)
            continue
        try:
            parsed = plistlib.loads(r.stdout or b"")
            ...
            unreadable.add(user)
            continue
        entries = parsed.get("dsAttrTypeStandard:AuthenticationAuthority") or []
        if any("shadowhash" in str(e).lower() for e in entries):
            capable.add(user)
    return (capable, unreadable)


def _bot_privilege_state_path(shared_dir: Path) -> Path:
    return shared_dir / _BOT_PRIVILEGE_STATE_REL


def _read_bot_privilege_state(shared_dir: Path) -> dict[str, dict]:
    try:
        raw = json.loads(_bot_privilege_state_path(shared_dir).read_text())
    except (OSError, ValueError):
        return {}
    entries = raw.get("privileged") if isinstance(raw, dict) else None
    if not isinstance(entries, dict):
        return {}
    return {u: e for u, e in entries.items() if isinstance(u, str) and isinstance(e, dict)}


def _write_bot_privilege_state(shared_dir: Path, privileged: dict[str, dict]) -> None:
    path = _bot_privilege_state_path(shared_dir)
    tmp = path.with_suffix(".json.tmp")
    try:
        path.parent.mkdir(parents=True, exist_ok=True)
        tmp.write_text(json.dumps({"privileged": privileged}, indent=2, sort_keys=True))
        os.replace(tmp, path)
    except OSError:
        # Losing the anchor degrades the fail-safe to "re-fires next readable
        # run", never to a false OK — the live findings above are unaffected.
        with contextlib.suppress(OSError):
            tmp.unlink()


def _bot_privilege_finding(user: str, bot_id: str, mechanisms: list[str],
                           *, verified: bool) -> Finding:
    """One CRITICAL per privileged bot account.

    The message carries ONLY the bot id — ``_audit_signature`` hashes
    ``(category, bot_id, message)``, so keeping the mechanism list in
    ``detail`` means a mechanism changing (admin group → sudoers drop-in)
    updates one signal instead of resolving one and opening another. It is
    also what lets an unverified run re-emit the anchored finding with a
    byte-identical message and keep the signal alive through the sweep.
    """
    detail = "; ".join(mechanisms)
    if not verified:
        detail = (
            f"{detail}\n(carried over from the last verified run — this run "
            f"could not read every privilege source; see the accompanying "
            f"'could not verify' warning)"
        )
    return Finding(
        level="critical", category="machine", bot_id=bot_id,
        # posture: standing privilege (capability, not activity) — the fix
        # is a one-time operator demotion; see the design doc's baseline.
        finding_kind="posture",
        message=(
            f"🔴 CRITICAL: bot account {user!r} holds host admin/sudo privilege"
        ),
        detail=detail,
        what_it_means=(
            f"The bot {bot_id!r} runs an openclaw gateway as the OS account "
            f"{user!r}, and that account carries host administrator rights. "
            "That is the same failure as an LLM gateway running under the pod "
            "admin user, reached from the opposite direction: anyone who can "
            "prompt-inject this bot can run commands at operator privilege — "
            "over every sibling bot's .openclaw tree, the evolve service, the "
            "service manager, and the deploy checkout. On macOS the admin "
            "group also nests into com.apple.access_ssh, so the account is "
            "reachable over SSH; if it can also password-authenticate, that "
            "is a guessable remote path to root. Bot accounts must hold no "
            "host privilege at all."
        ),
        fix_steps=(
            "This is a detection-only finding — dropping the privilege is an "
            "operator decision, because a legacy bot's launchd/keychain state "
            "may have been set up while it held admin.\n"
            "1. Confirm the mechanism named in the detail above.\n"
            "2. macOS — remove the account from the admin group:\n"
            f"     sudo dseditgroup -o edit -d {user} -t user admin\n"
            "   Linux — remove it from the privileged group:\n"
            f"     sudo gpasswd -d {user} sudo\n"
            "3. If the mechanism is a /etc/sudoers.d drop-in, delete or narrow "
            "that file (validate with `sudo visudo -c` before saving).\n"
            "4. Re-check the bot still starts: `sudo evolve-admin deploy "
            f"{bot_id}` and confirm its gateway comes up.\n"
            "5. Audit what the account did while privileged — its shell "
            "history and the gateway's exec log — and rotate any credential "
            "it could reach."
        ),
    )


def _check_bot_user_privilege(shared_dir: Path, config: dict) -> list[Finding]:
    """Fire CRITICAL when a bot account holds host admin/sudo privilege.

    The inverse of :func:`_check_admin_user_gateway`; see the section comment
    above for why both directions of the invariant matter and why this half
    was unguarded until 2026-08-02.

    Mechanisms checked (all probes unprivileged except the sudoers-drop-in
    scan, which uses the §25 ``ls``/``cat`` grants):

    * effective membership in a privileged group — ``admin`` on macOS,
      ``sudo``/``wheel``/``admin`` on Linux — resolved through ``id -Gn`` so
      NESTED membership counts;
    * a ``/etc/sudoers.d`` drop-in naming the account as a user spec;
    * (macOS, WARN not CRITICAL) an account that can password-authenticate
      at all, reported once for the whole pod.

    Unreadable sources never read as compliant: an unverified run re-emits
    the last verified findings verbatim and adds a WARN naming what it could
    not read. See :data:`_BOT_PRIVILEGE_STATE_REL`.
    """
    bot_users = _bot_unix_users(config)
    if not bot_users:
        return [Finding(
            level="skipped", category="machine", bot_id=None,
            message="machine: bot host-privilege check skipped (no bots in network.json)",
        )]

    profile = get_profile()
    macos = profile.name == "macos"
    priv_groups = _MACOS_PRIVILEGED_GROUPS if macos else _LINUX_PRIVILEGED_GROUPS

    unreadable_sources: list[str] = []
    # user -> ordered mechanism strings
    mechanisms: dict[str, list[str]] = {}
    present_users: set[str] = set()

    # 1. Effective group membership (authoritative; resolves nesting).
    for user in sorted(bot_users):
        status, groups = _effective_group_names(user)
        if status == "absent":
            continue
        if status == "unreadable":
            unreadable_sources.append(f"effective groups of {user!r} (id -Gn)")
            continue
        present_users.add(user)
        hits = sorted(set(groups) & set(priv_groups))
        for g in hits:
            mechanisms.setdefault(user, []).append(
                f"member of the {g!r} group"
                + (" (stock /etc/sudoers grants %admin full sudo)"
                   if macos and g == "admin" else "")
            )
        if macos and hits and _MACOS_SSH_ACCESS_GROUP in groups:
            mechanisms.setdefault(user, []).append(
                f"SSH-reachable via {_MACOS_SSH_ACCESS_GROUP} "
                "(nested from the admin group)"
            )

    # 2. Direct group records — corroboration the operator can act on. A
    #    mismatch against step 1 means nesting, which the detail should say.
    for group in priv_groups:
        status, direct = (
            _macos_group_members(group) if macos else _linux_group_members(group)
        )
        if status == "unreadable":
            unreadable_sources.append(f"{group!r} group record")
            continue
        for user in sorted(direct & present_users):
            if user in mechanisms:
                mechanisms[user].append(f"listed directly in the {group!r} group record")

    # 3. /etc/sudoers.d drop-ins naming a bot account.
    names = _sudoers_dropin_names()
    if names is None:
        unreadable_sources.append(f"{_SUDOERS_DROPIN_DIR} (listing)")
    else:
        for name in names:
            text = _sudoers_dropin_text(name)
            if text is None:
                unreadable_sources.append(f"{_SUDOERS_DROPIN_DIR}/{name}")
                continue
            for user in sorted(_sudoers_users_named(text, present_users)):
                mechanisms.setdefault(user, []).append(
                    f"named as a user spec in {_SUDOERS_DROPIN_DIR}/{name}"
                )

    verified = not unreadable_sources
    live = {
        user: {"bot_id": bot_users[user], "mechanisms": mechs}
        for user, mechs in mechanisms.items()
    }

    findings: list[Finding] = []
    if verified:
        _write_bot_privilege_state(shared_dir, live)
        emit = live
    else:
        # Union with the last verified set so an unreadable source can never
        # sweep-resolve a live CRITICAL. Live entries win — their mechanism
        # list is fresher.
        emit = {**_read_bot_privilege_state(shared_dir), **live}

    for user in sorted(emit):
        entry = emit[user]
        bot_id = entry.get("bot_id") or bot_users.get(user) or user
        mechs = [m for m in (entry.get("mechanisms") or []) if isinstance(m, str)]
        findings.append(_bot_privilege_finding(
            user, str(bot_id), mechs or ["mechanism not recorded"],
            verified=verified or user in live,
        ))

    if not verified:
        findings.append(Finding(
            level="warn", category="machine", bot_id=None,
            message="machine: bot host-privilege check could not verify every source",
            detail="\n".join(unreadable_sources),
            what_it_means=(
                "At least one privilege source was unreadable this run, so the "
                "check cannot say a bot account is clean — it is holding the "
                "last verified result rather than reporting compliance. The "
                "usual cause is a missing sudo grant: the /etc/sudoers.d "
                "probes were added in the same change as this check, and new "
                "grants stay dormant until an operator refreshes them."
            ),
            fix_steps=(
                "1. Install the current grants (this is manual by design):\n"
                "     sudo evolve-admin refresh-sudoers\n"
                "2. Re-run the check:\n"
                "     sudo evolve-admin audit --category machine\n"
                "3. If a source is still unreadable, the detail above names "
                "it — check that the file/record exists and that the evolve "
                "user's grant matches the exact argv."
            ),
        ))
    elif not emit:
        findings.append(Finding(
            level="ok", category="machine", bot_id=None,
            message=(
                f"machine: no bot account holds host privilege "
                f"({len(present_users)} account(s) checked)"
            ),
        ))

    # 4. macOS-only, WARN: bots that can password-authenticate at all.
    if macos and present_users:
        capable, pw_unreadable = _macos_password_auth_users(present_users)
        if capable:
            findings.append(Finding(
                level="warn", category="machine", bot_id=None,
                message=(
                    "machine: bot account(s) can password-authenticate: "
                    + ", ".join(sorted(capable))
                ),
                detail=(
                    "AuthenticationAuthority carries a ShadowHash entry for "
                    "each account listed."
                ),
                what_it_means=(
                    "Bots never need an interactive login — they are driven by "
                    "launchd/systemd, not by a person typing a password. A "
                    "local password is one more credential that can be guessed "
                    "or reused, and it becomes a REMOTE one the moment the "
                    "account is also SSH-reachable. Newer bot accounts on the "
                    "fleet already have no such record, so this is a legacy "
                    "residue rather than something Evolve provisions."
                ),
                fix_steps=(
                    "1. Confirm the account has no human who logs in as it.\n"
                    "2. Remove the local password authority:\n"
                    f"     sudo dscl . -delete {_MACOS_DS_USER_RECORDS}"
                    "/<account> AuthenticationAuthority\n"
                    f"     sudo dscl . -delete {_MACOS_DS_USER_RECORDS}"
                    "/<account> ShadowHashData\n"
                    "3. Re-deploy the bot and confirm its gateway still "
                    "starts: `sudo evolve-admin deploy <bot_id>`."
                ),
            ))
        if pw_unreadable:
            findings.append(Finding(
                level="skipped", category="machine", bot_id=None,
                message="machine: password-auth probe unreadable for some bot account(s)",
                detail=", ".join(sorted(pw_unreadable)),
            ))

    return findings


def _sshd_effective_config(text: str) -> dict[str, str]:
    """Parse ``sshd -T`` output into a lowercased key → value map.

    ``sshd -T`` prints one ``keyword value`` pair per line with the keyword
    already lowercased. Exact-key lookup replaces the substring matching this
    check used to do: with one directive that was safe, but the moment a
    second directive matters, ``"…authentication yes" in text`` starts
    matching whichever option happens to share the suffix. Repeated keys
    (``hostkey``, ``port``) keep the last value; nothing here reads one.
    """
    config: dict[str, str] = {}
    for line in text.splitlines():
        ...
        if not parts:
            continue
        config[parts[0].lower()] = (parts[1].strip().lower() if len(parts) > 1 else "")
    return config


def _check_ssh_config() -> list[Finding]:
    """SSH exposure checks — is a PASSWORD path reachable, and can root log in?

    ``PasswordAuthentication no`` alone does NOT close password login, and
    reading only that directive is how this check reported compliant on a
    host that still had one open (verified on the mini 2026-08-02, after the
    operator applied ``PasswordAuthentication no``):
    ...
    ``UsePAM yes`` routes keyboard-interactive through PAM, which
    authenticates against the account password — so the server was still
    advertising a password-backed method while the audit's
    "PasswordAuthentication is enabled" CRITICAL auto-resolved. Same shape as
    the bot-privilege gap this change also closes: reading ONE directive and
    declaring the invariant satisfied.

    The two paths are reported as SEPARATE findings on purpose. Folding them
    into one message would (a) change the existing finding's text, and
    ``_audit_signature`` hashes the message — every pod's live
    PasswordAuthentication signal would resolve and reopen as a new one; and
    (b) leave an operator who has already set ``PasswordAuthentication no``
    staring at an alert that names a directive they just fixed, which reads
    as stale and gets dismissed. Each finding names the directive responsible
    for it and nothing else.

    Both keys come out of the ``sshd -T`` output already being read, so this
    costs no extra probe and needs no new sudoers grant.
    """
    findings: list[Finding] = []
    try:
        # Absolute path from the profile table: /usr/sbin is not on the sudoers
        # secure_path, so a bare `sshd` argv could never resolve under sudo —
        # this check was dark on every pod until the §24 grant + this path fix
        # (2026-07-29 VPS denial audit: 91 unresolvable denials/day).
        r = subprocess.run(
            ["sudo", get_profile().sshd, "-T"],
            capture_output=True, text=True, timeout=5,
        )
        if r.returncode != 0:
            return [Finding(
        ...
        cfg = _sshd_effective_config(r.stdout)

        password_open = cfg.get("passwordauthentication") == "yes"
        # ChallengeResponseAuthentication is the pre-8.7 alias for the same
        # mechanism; some sshd builds still emit it, and a host that set only
        # the alias would slip through a kbdinteractive-only read.
        kbd_open = (
            cfg.get("kbdinteractiveauthentication") == "yes"
            or cfg.get("challengeresponseauthentication") == "yes"
        )
        pam_on = cfg.get("usepam") == "yes"
        pam_password_open = kbd_open and pam_on

        if password_open:
            # Message deliberately byte-identical to the pre-2026-08-02 text:
            # any pod where this is live keeps its existing Signal.
            findings.append(Finding(
                level="critical", category="machine", bot_id=None,
                # posture: fix-once SSH hardening (design-doc baseline).
                finding_kind="posture",
                message="🔴 CRITICAL: SSH PasswordAuthentication is enabled",
            ))
        if pam_password_open:
            kbd_directive = (
                "KbdInteractiveAuthentication"
                if cfg.get("kbdinteractiveauthentication") == "yes"
                else "ChallengeResponseAuthentication"
            )
            findings.append(Finding(
                level="critical", category="machine", bot_id=None,
                # posture: fix-once SSH hardening (design-doc baseline).
                finding_kind="posture",
                message=(
                    "🔴 CRITICAL: SSH keyboard-interactive/PAM leaves a "
                    "password login path open"
                ),
                detail=(
                    f"{kbd_directive} yes + UsePAM yes"
                    + (
                        " (PasswordAuthentication is already no — this is a "
                        "SECOND, independent password path, not a stale alert)"
                        if not password_open else ""
                    )
                ),
                what_it_means=(
                    "Setting PasswordAuthentication no does not by itself "
                    "close password login. With UsePAM yes, the "
                    "keyboard-interactive method hands authentication to PAM, "
                    "which checks the account password — so the server still "
                    "advertises a password-backed method and remains "
                    "guessable over the network. This matters most on a pod "
                    "where a bot account can password-authenticate: that "
                    "combination is a remote path to whatever privilege the "
                    "account holds."
                ),
                fix_steps=(
                    "1. Confirm what the server actually advertises (sends no "
                    "credential):\n"
                    "     ssh -o PreferredAuthentications=none "
                    "-o PubkeyAuthentication=no <user>@<host>\n"
                    "   'keyboard-interactive' in the denial line means the "
                    "path is open.\n"
                    "2. Close it in a drop-in (do NOT edit sshd_config "
                    "directly — pods carry an Evolve hardening drop-in):\n"
                    "     KbdInteractiveAuthentication no\n"
                    "3. Validate before reloading, or you can lock yourself "
                    "out:\n"
                    "     sudo sshd -t\n"
                    "4. Reload sshd, keeping your current session open until "
                    "you have confirmed a NEW session still connects.\n"
                    "5. Re-run the probe from step 1 — the denial line should "
                    "no longer offer keyboard-interactive."
                ),
            ))
        if not password_open and not pam_password_open:
            findings.append(Finding(
                level="ok", category="machine", bot_id=None,
                message="machine: SSH password login closed (password + keyboard-interactive/PAM)",
            ))
        if "permitrootlogin yes" in cfg_text or "permitrootlogin without-password" in cfg_text:
            findings.append(Finding(
                level="critical", category="machine", bot_id=None,
                # posture: fix-once SSH hardening, same family as the
                # password-path findings above.
                finding_kind="posture",
...
def _retired_bot_accounts(shared_dir: Path) -> set[str]:
    """Account names with a retirement archive under ``archived-bots/``.

    ``retire_bot`` writes ``{shared_dir}/archived-bots/<bot>-<YYYY-MM-DD>[-n]/``
    (see ``evolve_admin.retire._resolve_archive_root``), so the leading segment
    before the date is the retired bot's id — which is also its account name in
    the common case. Unreadable or missing archive dir → empty set, which is
    the conservative direction here: it means a genuinely-retired account falls
    back to the CRITICAL "new account" branch rather than being silently
    excused.

    NOTE the directory is ``archived-bots/``, not ``retired/``. Several
    docstrings named ``retired/`` for a long time and that stale name is
    exactly what made the 2026-08-02 investigation conclude retire-bot had
    never run on the pod when in fact it had.
    """
    try:
        entries = [p.name for p in (shared_dir / "archived-bots").iterdir() if p.is_dir()]
    except OSError:
        return set()
    accounts: set[str] = set()
    for name in entries:
        # Strip the trailing -<YYYY-MM-DD>[-n] the archive root appends.
        stem = re.sub(r"-\d{4}-\d{2}-\d{2}(-\d+)?$", "", name)
        if stem:
            accounts.add(stem)
    return accounts
...
def _check_user_accounts(shared_dir: Path, config: dict | None = None) -> list[Finding]:
    ...
    Accounts with a retirement archive (``_retired_bot_accounts``) are split
    out of the CRITICAL branch into their own accurate warn-level finding:
    they are decommissioned bots whose accounts `retire-bot` preserves by
    design, not newly-created accounts, and calling them new for weeks on end
    trains the operator to ignore this check (2026-08-02, `ledger`).
    """
    findings: list[Finding] = []
    ...
    # Split off accounts that are DECOMMISSIONED BOTS, not new accounts. A
    # retired bot leaves the roster (so it drops out of `known_bot_users`) but
    # keeps its account (`retire-bot` preserves it by design), and it was
    # created after the baseline file was written — so it lands in `new_users`
    # and fires 🔴 CRITICAL forever.
    #
    # That is not a cosmetic wording problem. The 2026-08-02 finding: `ledger`
    # was retired on 2026-06-15 and this check called it a NEW account every
    # cycle for seven weeks. Because the message was obviously wrong about what
    # it was looking at, it read as a false positive and nobody acted — while
    # the account really did still hold a live Telegram bot token. A check that
    # cries wolf about the wrong animal trains the operator to ignore it.
    #
    # An archive record under `archived-bots/` is proof the account is a
    # retired bot. Those get their own accurate, non-critical finding here, and
    # the orphaned_bot_account producer owns the detail (which credentials are
    # still live and where to revoke each).
    retired_users = _retired_bot_accounts(shared_dir) & new_users
    new_users -= retired_users

    if new_users:
        findings.append(Finding(
            level="critical", category="machine", bot_id=None,
            # event: an account outside the baseline is a classic
            # persistence mechanism — possible active compromise
            # (ambiguous ⇒ fail toward paging).
            finding_kind="event",
    ...
    if retired_users:
        findings.append(Finding(
            level="warn", category="machine", bot_id=None,
            message=(
                "machine: retired bot account(s) still present: "
                f"{', '.join(sorted(retired_users))}"
            ),
            detail=(
                "These are decommissioned bots, NOT new accounts — each has a "
                "retirement archive under archived-bots/. retire-bot preserves "
                "the account and home by design, so their channel tokens, "
                "gateway tokens, and SSH keys are still valid until revoked at "
                "the source. The orphaned_bot_account monitor enumerates "
                "exactly what is still live in each and where to revoke it; "
                "delete-bot removes the account once you no longer need it."
            ),
        ))
    if removed_users:
        findings.append(Finding(
...
def _check_listening_ports(shared_dir: Path) -> list[Finding]:
    ...
    try:
        # Platform lsof path (macOS /usr/sbin vs Linux /usr/bin) so the argv
        # matches the §24 sudoers grant on both OSes.
        r = subprocess.run(
            ["sudo", get_profile().lsof, "-iTCP", "-sTCP:LISTEN", "-n", "-P"],
            capture_output=True, text=True, timeout=10,
        )
        if r.returncode != 0:
...
def _check_oc_binary_mtime(shared_dir: Path) -> list[Finding]:
    ...
    The baseline stores the resolved path, mtime and version. When mtime
    changes we re-read the version: if the version also changed, it's a clean
    upgrade (auto-refresh the baseline, emit OK). If mtime changed without a
    version bump, that's a suspicious replacement — emit warn.

    The binary is resolved through the shared
    :func:`platform_profile.find_openclaw_cli`, not the local two-entry
    Homebrew list this used to keep: that list saw only the two macOS symlinks,
    so an install that lives under node_modules (or is only reachable via PATH)
    read as "binary not found" — the watch silently never armed. It was also
    the reason this check could not be widened past ``applies_to={"macos"}``:
    none of its candidates exist on a Linux pod.
    """
    baseline_path = shared_dir / "security" / "oc-binary-mtime.baseline"
    resolved = find_openclaw_cli()
    oc_path = Path(resolved) if resolved else None
    ...
    def _write_baseline(mt: str, ver: str | None) -> bool:
        try:
            ...
            baseline_path.write_text(json.dumps(
                {"mtime": mt, "version": ver, "path": str(oc_path)}
            ))
            return True
        except OSError:
            return False
    ...
    try:
        ...
        if isinstance(baseline, dict):
            ...
            stored_path = baseline.get("path")
    except json.JSONDecodeError:
        pass

    # The resolver picked a different binary than the baseline was taken
    # from (a reinstall that moved the install root, or the pre-resolver
    # baseline on a pod whose openclaw isn't at the old Homebrew path).
    # Two different files' mtimes are not comparable, so rebase rather than
    # report a replacement that didn't happen.
    if stored_path and stored_path != str(oc_path):
        _write_baseline(mtime, current_version)
        return [Finding(level="ok", category="machine", bot_id=None,
                        message="machine: openclaw binary path moved; baseline rebased",
                        detail=f"was={stored_path} now={oc_path} version={current_version or 'unknown'}")]
...
#
# audit_cost() was removed in Phase E1 of internal/spec-alert-subscriptions-
...
def audit_shell_config(bot_ids: list[str], shared_dir: Path) -> list[Finding]:
    ...
    for bot_id in bot_ids:
        ...
        if stored is None:
            ...
            # Deliberately NOT run through the L2 authorized-change gate
            # (drift_authorization). Nothing in Evolve writes a bot's .zshrc —
            # deploy only grants evolve READ on it, for this very hash check —
            # and no admin-surface record can name the file either (oc_keys
            # are openclaw.json top-level keys, not filenames), so an
            # allow-set for this kind could never fire in production and
            # would only have made the finding louder. It stays a posture
            # finding: fix-once, by the operator reading the diff and then
            # blessing the new hash through the Alerts-page "Accept current
            # .zshrc as new baseline" action (_remediation_for_finding →
            # reset_baseline(bot_id, "shell", shared_dir)). This branch never
            # re-records the baseline itself, so the finding stands until
            # that action is taken — the message below is the exact string
            # the remediation mapping keys on.
            findings.append(Finding(
                level="critical", category="identity", bot_id=bot_id,
                finding_kind="posture",
                ...
                what_it_means=(
                    "The startup file for this account's shell is not what "
                    "it was when the baseline was taken. Whatever that file "
                    "says now runs every time the account opens a shell. "
                    "Nothing in Evolve writes this file, so the change came "
                    "from the operator or from something off-path."
                ),
                fix_steps=(
                    f"1. Read {_bot_home(bot_id)}/.zshrc and see what changed\n"
                    f"2. If the change is yours, use the \"Accept current "
                    f".zshrc as new baseline for {bot_id}\" action on this "
                    "alert — that is the only thing that clears it; the "
                    "audit never re-records this baseline on its own\n"
                    "3. If it is not yours, treat the host as compromised "
                    "before you change anything else"
                ),
            ))
...
def audit_script_inventory(bot_id: str, shared_dir: Path) -> list[Finding]:
    ...
    if new_files or missing_files:
        ...
        # L2 authorized-change gate. Deploy installs and removes workspace
        # scripts, and the deploy hook already resets this baseline when it
        # does — but the reset races the audit, so the drift shows up anyway
        # for one or more cycles. A deploy or an applied proposal inside the
        # window accounts for it; nothing else does.
        #
        # This stays WARN when unexplained rather than being escalated to a
        # CRITICAL event: a new script in a bot's workspace is not in the
        # auth/permission/credential class the must-always-page floor covers,
        # and escalating a check that already fires on every redeploy would
        # add pages before it removed any. The gate's job here is to stop the
        # explained ones being reported at all.
        content_hash = hashlib.sha256(
            "\n".join(current_files).encode("utf-8")
        ).hexdigest()
        explanation = _explain_drift(drift_authorization.DriftChange(
            kind=drift_authorization.KIND_SCRIPT_INVENTORY,
            bot_id=bot_id,
            target=workspace,
        ...
        if explanation is not None:
            findings.append(Finding(
                level="ok", category="identity", bot_id=bot_id,
                message=(
                    f"{bot_id}: script inventory changed "
                    f"({', '.join(parts)}) — {explanation.evidence}"
                ),
            ...
            findings.append(Finding(
                level="warn", category="identity", bot_id=bot_id,
                message=(
                    f"{bot_id}: script inventory drift ({', '.join(parts)}) "
                    "with no authorized change to explain it"
                ),
                detail=" | ".join(detail_lines),
            ))
    else:
    ...
    for fpath in workspace.rglob("*"):
        ...
        for pattern, label in _SECRET_PATTERNS:
            if pattern.search(content):
                ...
                findings.append(Finding(
                    level="critical", category="identity", bot_id=bot_id,
                    # event: live credential exposure — the design doc's
                    # canonical page-now class.
                    finding_kind="event",
...
def _audit_cron_quarantine(bot_id: str) -> list[Finding]:
    """Surface cron jobs OpenClaw ≥2026.7 parked in cron/jobs-quarantine.json.

    The gateway's SQLite import-once migration VALIDATES each jobs.json entry
    (a job needs a ``payload`` object) and moves invalid ones here SILENTLY —
    no lastRunStatus update, no gateway error, no alert; the job just never
    runs again. The primary bot's ``security:cve-scan-discover`` cron sat
    quarantined this way from 2026-07-28 with no CVE scans running and nothing
    firing. One finding per quarantined job so the Signal signature is stable
    per (bot, name, reason) and clears via sweep-resolve when the quarantine
    empties. A missing file is the benign common case.
    """
    findings: list[Finding] = []
    q_path = _bot_home(bot_id) / ".openclaw" / "cron" / "jobs-quarantine.json"

    raw: str | None = None
    try:
        raw = q_path.read_text()
    except FileNotFoundError:
        return findings
    except OSError:
        try:
            r = subprocess.run(
                ["sudo", "/bin/cat", str(q_path)],
                capture_output=True, text=True, timeout=5,
            )
            if r.returncode == 0:
                raw = r.stdout
    ...
    if raw is None:
        findings.append(Finding(
            ...
            message=f"{bot_id}: cron/jobs-quarantine.json read denied",
            detail="audit user lacks ACL/sudo read for the bot's cron/jobs-quarantine.json",
        ))
        return findings
    ...
    try:
        ...
        findings.append(Finding(
            level="warn", category="config", bot_id=bot_id,
            message=f"{bot_id}: cannot parse cron/jobs-quarantine.json",
        ))
        return findings

    entries = data.get("jobs", []) if isinstance(data, dict) else (data if isinstance(data, list) else [])
    for q in entries:
        ...
        reason = q.get("reason") or "unknown"
        is_critical_job = (
            isinstance(name, str)
        ...
        findings.append(Finding(
            ...
            # posture: a quarantined security/healthcheck job is a standing
            # coverage gap with a fix-once remediation (design-doc baseline).
            finding_kind="posture" if is_critical_job else "",
            message=(
                f"{msg_prefix}: {bot_id} cron job {name!r} quarantined by "
                f"OpenClaw ({reason}) — the job is not running"
            ),
            what_it_means=(
                f"OpenClaw ≥2026.7 validates every cron/jobs.json entry when it "
                f"imports the file into its SQLite store at gateway start. The "
                f"entry for {name!r} failed validation ({reason!r}) and was moved "
                f"to cron/jobs-quarantine.json. Quarantine is silent: the job's "
                f"lastRunStatus never updates and the gateway raises no error — "
                f"the job simply never fires until someone re-installs a valid "
                f"version of it."
            ),
            fix_steps=(
                f"1. Inspect the quarantined entry:\n"
                f"   sudo /bin/cat ~{get_bot_user(bot_id)}/.openclaw/cron/jobs-quarantine.json "
                f"| jq '.jobs[] | select(.job.name==\"{name}\")'\n"
                f"2. If it is an Evolve app cron in the legacy shape ('task' + "
                f"string schedule), re-run the app install / deploy — the "
                f"installer now writes the payload-based schema — then restart "
                f"the bot's gateway so it imports the corrected jobs.json.\n"
                f"3. Remove the entry from jobs-quarantine.json once the "
                f"corrected job is live (verify with: openclaw cron list, run "
                f"as the bot user), so this finding clears."
            ),
        ))
    return findings
...
def audit_cron_health(bot_id: str, shared_dir: Path) -> list[Finding]:
    ...
    # Quarantine first: it must fire even when jobs.json itself is absent —
    # after the ≥2026.7 import the gateway RENAMES jobs.json to
    # jobs.json.migrated, so the quarantined-job case usually coincides with
    # the "no jobs.json" early-return below.
    findings: list[Finding] = _audit_cron_quarantine(bot_id)
    jobs_path = _bot_home(bot_id) / ".openclaw" / "cron" / "jobs.json"

    # Try ACL read first, fall back to sudo /bin/cat. A MISSING file is the
    # benign no-cron-jobs case, NOT a fallback trigger: FileNotFoundError is
    # an OSError subclass, so the old blanket `except OSError` sudo-cat'ed a
    # nonexistent path on every run — a guaranteed sudo denial (116/day on
    # the VPS, where darwin has no cron/ dir at all) and a misleading
    # "read denied" finding for a bot that simply has no cron jobs.
    raw: str | None = None
    try:
        ...
        findings.append(Finding(
            level="ok", category="config", bot_id=bot_id,
            message=f"{bot_id}: no cron jobs.json (no cron jobs configured)",
        ))
        return findings
    except OSError:
        try:
    ...
    for job in jobs:
        ...
        if isinstance(consecutive_errors, (int, float)) and consecutive_errors > 2:
            ...
            findings.append(Finding(
                level=level, category="config", bot_id=bot_id,
                # posture: a repeatedly-failing security/healthcheck job is
                # the same standing-coverage-gap class as quarantine above.
                finding_kind="posture" if is_critical_job else "",
                message=(
...
_OC_POLICY_OVERRIDES_SUPPRESS: dict[str, str] = {
    ...
    # internal/spec-app-derived-permissions-2026-05-24.md §"Why full as default".
    ...
    "tools.exec.security_full_configured": (
        ...
        "internal/spec-app-derived-permissions-2026-05-24.md"
    ),
    ...
    "gateway.trusted_proxies_missing": (
        ...
        "protect — see "
        "internal/diagnosis-trusted-proxies-loopback-suppress-2026-05-28.md"
    ),
    ...
    "security.trust_model.multi_user_heuristic": (
        ...
        "See internal/diagnosis-oc-noisy-advisories-2026-06-04.md."
    ),
...
def audit_oc_security(bot_id: str, shared_dir: Path) -> list[Finding]:
    ...
    if audit_result is not None:
        ...
        for item in oc_findings:
            ...
            if sev == "critical":
                ...
                findings.append(Finding(
                    level="critical", category="config", bot_id=bot_id,
                    # event: upstream OC criticals are an open set we cannot
                    # enumerate per-check (it includes credential-exposure
                    # classes like world-readable token files), so the whole
                    # passthrough fails toward paging.
                    finding_kind="event",
                    message=msg,
                    detail=oc_detail,
    ...
    #
    # #3566 audit D-2 — the correction below is a ROOT `chmod`, and `chmod`
    # follows a symlink at its argument. `.openclaw` is bot-owned, so the bot
    # can replace openclaw.json with a link and aim a root `chmod 600` at any
    # file on the box, every 15 minutes: a relabel/DoS primitive (same class as
    # #3587/#3590/#3591 for oc_model, #3597/#3602 for secret_config_perms and
    # deploy). Two things close it here:
    #
    #   * the mode is read with `lstat`, not `stat`. `stat` — and the `getfacl`
    #     behind acl_masked_owner_only — FOLLOW, so a link aimed at a victim
    #     that happens to be 0600 (or mask-artifact-private on Linux) would read
    #     back as "permissions OK" and close the check silently, leaving the
    #     plant unreported. With lstat a link never reaches either OK branch.
    #   * the destination goes through `evolve_util.assert_safe_sudo_dest`
    #     immediately before the chmod. A refusal is reported as its own `warn`
    #     Finding naming the planted path — a link here is an attack indicator,
    #     not routine drift, so it must not be a silent skip.
    from runtime.perms import get_perms
    oc_path = _bot_home(bot_id) / ".openclaw" / "openclaw.json"
    try:
        oc_st = oc_path.lstat()
        mode = oc_st.st_mode & 0o7777
        ...
        perms = f"{mode:04o}"
        # A symlinked leaf can never be blessed by either OK branch — both of
        # them resolve the link and would report the VICTIM's privacy as this
        # bot's. Route it to the gate below instead, which names the plant.
        planted = _stat.S_ISLNK(oc_st.st_mode)
        if not planted and perms == "0600":
            findings.append(Finding(
                level="ok", category="config", bot_id=bot_id,
                message=f"{bot_id}: openclaw.json permissions OK ({perms})",
            ))
        elif not planted and get_perms().acl_masked_owner_only(oc_path):
            findings.append(Finding(
                level="ok", category="config", bot_id=bot_id,
            ...
            # No `anchor=` (#3566 audit D-2 residual, #3601): this relpath is
            # FLAT, so its parent IS `.openclaw` — the shallowest component the
            # bot can plant at — and the parent is already lstat'd. `<home>` is
            # bot-owned but the home ENTRY needs write on /Users (root's), so no
            # link can appear at it. The intermediate walk would add nothing.
            # The hard-link leg of the same helper DOES apply here, and is the
            # worse variant on the mini: an unprivileged macOS user can link a
            # file it neither owns nor can read, and a hard link defeats every
            # symlink check because it IS a real regular file.
            #
            # Re-checked HERE rather than once at the top of the block: the
            # acl_masked_owner_only branch above shells out to getfacl, and the
            # docstring's "re-check between cp and the chown/chmod repair" note
            # applies to any window between the check and the privileged call.
            # This is still lstat-then-subprocess (TOCTOU, stated in the
            # helper); it converts a persistent plant that fires on every
            # 15-minute audit into a race that has to be timed against one.
            try:
                assert_safe_sudo_dest(oc_path)
            except PermissionError as e:
                findings.append(Finding(
                    level="warn", category="config", bot_id=bot_id,
                    message=f"{bot_id}: openclaw.json permission repair REFUSED "
                            "(unsafe destination)",
                    detail=f"{oc_path}: {e}",
                    what_it_means=(
                        "The audit wanted to correct this bot's openclaw.json "
                        "back to 0600, but the destination is not a plain file "
                        "in a plain directory — most likely a symlink planted "
                        "where the config should be. The correction runs "
                        "`sudo /bin/chmod 600` as root and chmod follows "
                        "symlinks, so running it would have relabelled "
                        "whatever the link points at. Nothing was changed. "
                        "The bot user owns its own .openclaw directory, so a "
                        "link here is something the bot (or anything running "
                        "as it) placed deliberately — treat it as an attack "
                        "indicator, not as ordinary permission drift."
                    ),
                    fix_steps=(
                        "1. Look at it WITHOUT following the link:\n"
                        f"   ls -l {oc_path}\n"
                        f"   readlink {oc_path}\n"
                        "2. If it is a symlink, the bot's real config is gone "
                        "or hidden — capture the link target for the incident "
                        "record before touching anything.\n"
                        "3. Remove the link and restore the bot's own config, "
                        "then re-run the audit; the 0600 correction resumes "
                        "once the destination is a real file again.\n"
                        "4. Treat the bot's tokens as exposed and rotate them."
                    ),
                ))
            else:
...
# ── Alert dispatch ────────────────────────────────────────────────────────────
#
# Critical delivery is page-on-transition (Phase 5b of
# internal/spec-alerts-signal-store-2026-05-07.md;
# internal/design-security-alert-fatigue-2026-08-31.md R-1+R-5): each
# finding's mirrored Signal is the dedup state, and a push fires only
# when the Signal newly ENTERS the firing state. The former batch-hash
# layer (audit-critical-dedup.json, 7-day resend) is retired — it
# re-broadcast the entire standing set whenever any one member changed.
...
def _audit_signature(finding: Finding) -> str:
    ...
    Phase 3 of internal/spec-alerts-signal-store-2026-05-07.md. Findings
...
# Which Finding categories each ``--category`` value actually exercises.
# The audit-producer Signal type is ``audit_<Finding.category>``, so this
# doubles as the sweep scope for a partial run: a ``--category machine`` run
# re-derives every machine finding and nothing else, so it may sweep-resolve
# ``audit_machine`` Signals and must leave every other type firing.
#
# ``machine`` and ``process`` share a coverage set because main() runs
# audit_machine + audit_process together for either flag, and audit_process
# emits category="machine" findings too.
#
# The sub-monitor categories map to the EMPTY set: mcp / plugins / hooks /
# content_scan / permissions / app_permissions produce no audit-producer
# findings at all (each sub-monitor owns its own producer name and runs its
# own type-scoped sweep), so such a run must sweep nothing under "audit".
_CATEGORY_FINDING_COVERAGE: dict[str, frozenset[str]] = {
    "identity": frozenset({"identity"}),
    "config": frozenset({"config"}),
    "machine": frozenset({"machine"}),
    "process": frozenset({"machine"}),
    "proposal": frozenset({"proposal"}),
    "mcp": frozenset(),
    "plugins": frozenset(),
    "hooks": frozenset(),
    "content_scan": frozenset(),
...
def _sweep_scope(
    *, category: str | None, bot: str | None
) -> tuple[set[str] | None, set[str] | None]:
    """Narrow the audit sweep to what THIS run actually scanned.

    Returns ``(sweep_types, sweep_bot_ids)`` for ``signals.store.sweep_resolve``
    — ``None`` on a dimension means "no restriction" (a full run behaves
    exactly as before).

    A partial run computes ``kept_signatures`` from a subset of the checks,
    so an unscoped sweep auto-resolves every finding the run never looked
    for. That was cosmetic while every standing critical re-paged on every
    run; since page-on-transition (R-1 of
    internal/design-security-alert-fatigue-2026-08-31.md) it is an alert
    bomb — the wrongly-resolved Signals REOPEN on the next full run, and a
    reopen is a firing transition, so every standing critical pages again.

    Both dimensions fail toward NOT sweeping. Leaving a genuinely-cleared
    Signal firing is self-healing (the next full run resolves it); resolving
    a still-true Signal is the re-page bomb. So an unrecognized ``--category``
    (argparse's ``choices`` should make this unreachable) sweeps nothing
    rather than everything.

    Each dimension mirrors the truthiness test ``main()`` uses for that SAME
    flag, and the two flags genuinely differ — the asymmetry below is
    deliberate, not an oversight:

    * ``bot`` — ``main()`` narrows the roster under ``if args.bot:``, so a
      falsy ``--bot ""`` leaves the FULL roster in place and the run has full
      per-bot coverage. Hence the truthiness test here. Testing ``is not
      None`` instead would return ``{""}``, which matches no Signal's
      ``bot_id``, silently disabling auto-clear for an invocation that had
      every right to sweep.
    * ``category`` — ``main()`` gates every check block on
      ``run_all = args.category is None``, so a falsy-but-not-None
      ``--category ""`` runs NO block at all and its coverage really is
      empty. ``is None`` is therefore the correct test, and loosening it to a
      truthiness test would hand an empty-coverage run an UNRESTRICTED sweep —
      exactly the alert bomb this function exists to prevent.
    """
    sweep_types: set[str] | None = None
    if category is not None:
        covered = _CATEGORY_FINDING_COVERAGE.get(category, frozenset())
        sweep_types = {f"audit_{c}" for c in covered}
    # ``--bot X`` visits only X's per-bot checks, so only X's Signals were
    # re-derived. Pod-scope Signals (bot_id None) and other bots' Signals are
    # excluded — a --bot run's pod-wide coverage is partial too (audit_process
    # sees one bot's processes), so it has no standing to clear them.
    # Truthiness, not ``is not None``, so an empty ``--bot ""`` agrees with
    # main()'s own ``if args.bot:`` roster test — see the docstring.
    sweep_bot_ids: set[str] | None = {bot} if bot else None
    return sweep_types, sweep_bot_ids


def _covers_whole_pod(
    sweep_types: set[str] | None, sweep_bot_ids: set[str] | None
) -> bool:
    """Did this run re-derive EVERY audit-producer finding for every bot?

    The single place that reads a ``_sweep_scope`` result as a full-vs-partial
    verdict — shared by the current-findings snapshot gate in
    ``dispatch_findings`` and the two absent-branch sweeps in
    ``_emit_signals_from_findings`` (which spells it ``is_partial_run``, its
    negation). Both must agree on what a partial run is; one predicate is how. ``None`` on a dimension means "no restriction", so ``(None, None)``
    — and only that — is a full run. Deriving it here rather than re-reading
    ``args.category`` / ``args.bot`` keeps ONE definition of this run's
    coverage. Both directions of the biconditional are pinned, so a later
    change to ``_sweep_scope`` cannot silently reclassify runs:
    ``test_sweep_scope_full_run_is_unrestricted`` holds a full run to
    ``(None, None)`` — the dangerous direction, since misreading a full run
    as partial would stop the findings snapshot from ever being written
    again (a permanently-stale pod report) — and
    ``test_every_flagged_run_is_partial`` holds the converse over every
    ``--category`` choice.
    """
    return sweep_types is None and sweep_bot_ids is None


def _sweep_covers_signature(
    signature: str,
    sweep_types: set[str] | None,
    sweep_bot_ids: set[str] | None,
) -> bool:
    """Whether this run's coverage includes the condition behind ``signature``.

    The flap-dwell clear-sweep needs the same narrowing ``_sweep_scope`` gives
    the Signal sweep, but it cannot filter on the store's ``type`` / ``bot_id``
    fields: a dwell ledger entry records the FLAP-FAMILY type
    (``audit_perm_flap`` / ``audit_identity_flap``) and a count, and carries no
    bot at all. What it does carry is the signature — and audit's signatures
    are structured. ``_audit_signature`` builds
    ``audit:<category>:<bot_id|pod>:<digest>`` and hashes the MESSAGE into the
    digest, so finding text can never inject an extra colon. That format is the
    contract this function depends on; a test pins it.

    Applies the same two tests as ``store.sweep_resolve`` — type, then bot,
    with a pod-scope entry (``bot_id`` None) dropping out of coverage once
    ``sweep_bot_ids`` is set, since a ``--bot`` run's pod-wide coverage is
    partial too and has no standing to clear pod-scope dwell state.

    Two inputs are read out of the signature rather than off a field, so they
    can diverge from what ``sweep_resolve`` would decide. ``bot_id`` — unlike
    the message — is interpolated into the key verbatim, so a colon in it
    shifts the split; and a bot literally named ``pod`` is indistinguishable
    from the pod-scope sentinel (a collision ``_audit_signature`` already has
    upstream of here: bot_id None, "" and "pod" all mint one signature). Both
    diverge toward RETAINING the entry, never toward clearing one wrongly, so
    neither can cause a missed page — and neither is reachable with macOS
    usernames, which admit no colon. Pinned by test.

    Fails toward NOT clearing, like every other decision on this path: an
    unparseable or non-audit signature is left alone rather than reset.
    """
    parts = signature.split(":", 3)
    if len(parts) != 4 or parts[0] != "audit":
        return False
    _producer, category, scope, _digest = parts
    if sweep_types is not None and f"audit_{category}" not in sweep_types:
        return False
    if sweep_bot_ids is not None:
        bot_id = None if scope == "pod" else scope
        if bot_id not in sweep_bot_ids:
            return False
    return True
...
def _remediation_for_finding(finding: Finding):
    ...
    if (
        ...
        return Remediation(
            ...
            confirm=(
                f"Reads the current {_bot_home(finding.bot_id)}/.zshrc and "
                ...
                f"  - You did not edit {_bot_home(finding.bot_id)}/.zshrc "
                f"yourself.\n"
                ...
                f"{_bot_home(finding.bot_id)}/.zshrc"
            ),
        )
...
def _audit_severity_tag(finding: Finding) -> tuple[str, int]:
    ...
    internal/spec-severity-framework-2026-05-18.md §2.
    """
...
def _is_bringup_transient_finding(finding: Finding) -> bool:
    ...
    See internal/spec-pod-bringup-settle-2026-06-23.md.
    """
...
def _is_flap_prone_perm_finding(finding: Finding) -> bool:
    ...
    CRITICAL is the must-page floor (internal/spec-drift-alert-taxonomy-2026-06-26.md,
...
# Identity-hash mismatch finding family (the edit-vs-backup race). A bot
# legitimately editing its own AGENTS.md/SOUL.md/HEARTBEAT.md opens a
# live-vs-git-backup hash mismatch window that closes at the next backup
# commit, so the finding flaps in/out — and because critical alert delivery is
# a whole-batch broadcast fingerprinted over the entire sorted finding list,
# each flap re-broadcast ALL standing criticals to the operator (~2 full
# re-broadcasts/day observed live). Matched case-insensitively as substrings of
# the finding ``message``; both shapes come from ``audit_identity``:
#
#   * "🔴 CRITICAL: {bot} {fname} hash mismatch vs git backup"
#     (IDENTITY_FILES, level=critical)
#   * "{bot}: {fname} changed outside proposal pipeline"
#     (EVOLVE_PROCEDURE_FILES on the primary bot, level=warn — the same
#     compare-live-to-last-backup-commit race, same ``live=… backup=…`` detail)
#
# The ``.zshrc``-style "hash changed since baseline" findings are a DIFFERENT
# family (baseline drift, not a backup race) and deliberately do not match.
_IDENTITY_MISMATCH_FLAP_FRAGMENTS: tuple[str, ...] = (
    "hash mismatch vs git backup",
    "changed outside proposal pipeline",
)

# Stable flap_gate ledger ``type`` for audit's identity-hash dwell entries —
# distinct from _AUDIT_PERM_FLAP_TYPE so each family's per-run clear-sweep
# (``note_cleared_absent``) resets only its OWN counters in the shared pending
# dir. As with the perm family, is_transient_prone is overridden
# (``transient=True``) so this value is diagnostic + filter only.
_AUDIT_IDENTITY_FLAP_TYPE = "audit_identity_flap"


def _is_identity_mismatch_finding(finding: Finding) -> bool:
    """Whether a finding is an identity-hash mismatch vs the git backup —
    the edit-vs-backup race family subject to flap hysteresis (must dwell
    N≥2 consecutive runs before paging).

    Unlike ``_is_flap_prone_perm_finding`` this family INCLUDES its
    critical-level shape: R-2 of
    internal/design-security-alert-fatigue-2026-08-31.md deliberately excludes
    the identity-hash mismatch from the must-page floor, because a one-run
    mismatch is overwhelmingly a bot's legitimate self-edit racing the next
    backup commit, not tampering. A mismatch that PERSISTS 2+ consecutive
    audit runs still fires CRITICAL exactly as before — only the single-run
    blip is withheld. Credential-exposure criticals and every other finding
    family are untouched (the fragments only match ``audit_identity``'s two
    backup-comparison message shapes, and the category guard pins it further).
    """
    if finding.category != "identity":
        return False
    message = (finding.message or "").lower()
    return any(frag in message for frag in _IDENTITY_MISMATCH_FLAP_FRAGMENTS)
...
def _emit_signals_from_findings(
    ...
    *,
    ...
    sweep_types: set[str] | None = None,
    sweep_bot_ids: set[str] | None = None,
) -> tuple[set[str], dict[str, tuple[Any, str]], set[str]]:
    """Mirror each finding to a Signal; sweep-resolve the rest.

    Returns ``(kept_signatures, critical_signals, withheld_signatures)``:

    - ``critical_signals`` maps each critical finding's signature to
      ``(signal, observe_outcome)`` — the Signal object just written and
      which find-or-create branch ``observe()`` took (created / reopened /
      changed / unchanged / dismissed_bump). The Telegram delivery path
      gates paging on this (page-on-transition) and audits the dispatch
      on each Signal.
    - ``withheld_signatures`` holds the signatures of findings withheld
      by a flap-gate dwell this run (still real, but deliberately not
      observed yet). ``dispatch_findings`` excludes withheld criticals
      from delivery — a one-run identity-hash blip never pages, and must
      not trip the fail-toward-paging fallback for store failures. Empty
      on the import-failure path, so a missing signals package degrades
      to paging everything rather than silencing criticals.

    ``now`` is injectable so the flap-hysteresis dwell ledger is deterministic
    under test; live callers leave it ``None`` (= wall clock).

    ``sweep_types`` / ``sweep_bot_ids`` scope the run's ABSENT-branch sweeps
    to what it actually scanned (see ``_sweep_scope``). Both default to
    ``None`` = full coverage, which is correct only for a full run. They gate
    two things: the Signal auto-resolve sweep is narrowed to the given
    types/bots, and the flap-dwell clear-sweep is narrowed to the same
    coverage via ``_sweep_covers_signature``.
    """
    try:
    ...
    critical_signals: dict[str, tuple[Any, str]] = {}
    ...
    # Same, for the identity-hash mismatch family (the edit-vs-backup race).
    # Kept as a separate set + ledger type so each family's clear-sweep only
    # resets its own dwell counters.
    present_identity_flap_sigs: set[str] = set()
    # Findings withheld by a flap dwell this run — returned to the caller so
    # the batched critical alert can exclude still-dwelling criticals.
    withheld_signatures: set[str] = set()
    ...
    for finding in criticals + warns:
        ...
        # internal/spec-pod-bringup-settle-2026-06-23.md. Withheld findings are
        ...
        # internal/spec-drift-alert-taxonomy-2026-06-26.md) and fall straight through
        ...
        # The identity-hash mismatch family (R-2 of
        # internal/design-security-alert-fatigue-2026-08-31.md) dwells through the
        # same machinery under its own ledger type. It is the one family whose
        # CRITICAL shape dwells too: a one-run live-vs-backup mismatch is a
        # bot's legitimate self-edit racing the next backup commit, and each
        # such blip re-broadcast the entire standing critical batch. A
        # sustained (2+ consecutive runs) mismatch still fires CRITICAL
        # exactly as before.
        flap_type: str | None = None
        if flap_gate is not None:
            if _is_flap_prone_perm_finding(finding):
                flap_type = _AUDIT_PERM_FLAP_TYPE
                present_perm_flap_sigs.add(signature)
            elif _is_identity_mismatch_finding(finding):
                flap_type = _AUDIT_IDENTITY_FLAP_TYPE
                present_identity_flap_sigs.add(signature)
        if flap_gate is not None and flap_type is not None:
            ...
            if not verdict.page:
                withheld_signatures.add(signature)
                continue
        ...
        try:
            ...
            # Event-vs-posture classification (R-3). Every critical carries
            # one (Finding.__post_init__ enforces it); downstream delivery
            # policy and the Alerts page read it from details. Omitted when
            # unset so non-critical signals don't grow a noise key.
            if finding.finding_kind:
                details_payload["finding_kind"] = finding.finding_kind
            ...
            sig, outcome = signals_store.observe_with_outcome(
                shared_dir,
                signature=signature,
                producer="audit",
            ...
            continue
        if finding.level == "critical":
            critical_signals[signature] = (sig, outcome)

    # Did this run examine only a subset of the checks? Both sweeps below are
    # ABSENT-branch sweeps — they act on what is missing from this run's
    # keep-set — so each is only sound when the run actually looked for
    # everything it is about to clear. ``_covers_whole_pod`` is the single
    # derivation of that verdict, shared with the current-findings snapshot
    # gate in dispatch_findings; spelling the predicate a second time here
    # would let the two drift apart on what "partial" means.
    is_partial_run = not _covers_whole_pod(sweep_types, sweep_bot_ids)

    # Sweep-resolve audit signals whose conditions are no longer present —
    # scoped to the types/bots this run actually re-derived, so a partial
    # (--bot / --category) run cannot clear findings it never looked for.
    # An empty sweep_types means "this run derived no audit-producer
    # findings at all" (a sub-monitor category), so skip the sweep entirely.
    if sweep_types is None or sweep_types:
        try:
            signals_store.sweep_resolve(
                ...
                types=sweep_types,
                bot_ids=sweep_bot_ids,
            )
        except Exception:
            pass

    # Reset the flap-dwell counter for any tracked signature that was observed
    # on a prior run but is absent this run — the consecutive-cycle reset that
    # makes a single-cycle flap never accumulate toward the dwell threshold.
    # Each family sweeps only its OWN ledger type (the pending dir is shared
    # across producers), so audit never touches pod_perms_drift / acl_drift
    # counters and the perm and identity families never reset each other.
    #
    # Narrowed to this run's coverage, for exactly the reason the sweep-resolve
    # above is: this is an absent-branch sweep, and "absent this run" only
    # means "cleared" when the run actually looked. Unscoped, a --bot /
    # --category run resets the dwell for conditions it never examined, so a
    # genuine finding one cycle into its dwell restarts and its page slips a
    # cadence.
    #
    # The narrowing cannot reuse the store's fields — a ledger entry records
    # the flap-family type and a count, and carries no bot — but it needs no
    # new ledger field either: the entry carries the signature, and audit's
    # signatures encode both dimensions (see _sweep_covers_signature). The
    # predicate goes to flap_gate as an opaque callable, so the shared ledger
    # module never learns a caller's key shape.
    #
    # A full run passes no predicate, keeping that path byte-identical.
    if flap_gate is not None:
        covers_this_run = (
            (lambda sig: _sweep_covers_signature(sig, sweep_types, sweep_bot_ids))
            if is_partial_run
            else None
        )
        for flap_family_type, present_sigs in (
            (_AUDIT_PERM_FLAP_TYPE, present_perm_flap_sigs),
            (_AUDIT_IDENTITY_FLAP_TYPE, present_identity_flap_sigs),
        ):
            try:
                flap_gate.note_cleared_absent(
                    shared_dir,
                    present_sigs,
                    type_filter=flap_family_type,
                    signature_filter=covers_this_run,
                )
            except Exception as exc:  # noqa: BLE001 — best-effort dwell reset
                _log(f"[audit] flap-dwell clear-sweep failed: {exc}", shared_dir)

    return kept_signatures, critical_signals, withheld_signatures


def _parse_signal_ts(ts: str | None) -> datetime | None:
    if not ts:
        return None
    try:
...
def _signal_newly_firing(sig: Any) -> bool:
    """True when the Signal entered ``firing`` after its last successful page.

    The page-on-transition predicate: covers every path INTO firing —
    created, resolve → reopen, and a snooze-expiry wake — while a
    standing firing Signal (already paged for this episode) stays
    silent no matter how many runs re-observe it. Unparseable or
    inconsistent timestamps fail toward paging: a broken suppression
    gate must never silence a CRITICAL.
    """
    if sig.state != "firing":
        return False
    entered = None
    for tr in reversed(sig.state_history):
        if tr.to_state == "firing":
            entered = _parse_signal_ts(tr.at)
            break
    last_paged = None
    for d in reversed(sig.deliveries):
        if d.suppressed_reason is None:
            last_paged = _parse_signal_ts(d.at)
            break
    if last_paged is None:
        return True  # firing and never successfully paged
    if entered is None:
        return False  # paged before; no parseable firing entry → standing
    try:
        return entered > last_paged
    except TypeError:  # mixed aware/naive timestamps
        return True


# Marker for a page that did not reach the operator — the send broke, or the
# dispatcher withheld it (cooldown / 24h identical-content floor / source
# disabled / no recipient). Recorded as a suppressed_reason so
# `_signal_newly_firing` excludes it: the firing episode stays un-paged and the
# next audit run tries again.
_PAGE_UNDELIVERED_REASON = "not delivered — page did not reach the operator"
...
def _record_telegram_delivery(
    ...
    *,
    ...
    ``suppressed_reason=None`` is the only shape that counts as a
    successful page: ``_signal_newly_firing`` skips every entry that
    carries a reason, so a delivery recorded with one leaves the episode
    eligible to page again on the next run. That covers both the operator
    suppressions (snoozed/dismissed) and a send that FAILED end-to-end
    (``_PAGE_UNDELIVERED_REASON``).

    Consecutive entries with the same reason collapse into one — the
    audit runs every 15 minutes, and a Signal snoozed for a week must not
    accumulate ~670 identical "signal snoozed" rows (same for a channel
    that stays down).
    """
    try:
        from signals import store as signals_store
    ...
    for _signature, signal_id in critical_ids.items():
        ...
        if (
            suppressed_reason is not None
            and sig.deliveries
            and sig.deliveries[-1].suppressed_reason == suppressed_reason
        ):
            continue
        try:
...
def dispatch_findings(
    ...
    *,
    ...
    """Log all findings; page newly-firing CRITICALs, accumulate WARNs.

    ``sweep_types`` / ``sweep_bot_ids`` come from ``_sweep_scope`` and confine
    the auto-resolve sweep to the coverage of this run. They also gate the
    current-findings snapshot, which only a full run may overwrite. Defaults
    (``None``, ``None``) are the full-run behaviour: sweep every audit Signal
    ...
    for f in findings:
        if f.level == "critical":
            _log(f"[audit] CRITICAL: {f.message}" + (f" — {f.detail}" if f.detail else ""), shared_dir)
            criticals.append(f)
        elif f.level == "warn":
            _log(f"[audit] WARN: {f.message}" + (f" — {f.detail}" if f.detail else ""), shared_dir)
            warns.append(f)
            ...
            _log(f"[audit] SKIPPED: {f.message}" + (f" — {f.detail}" if f.detail else ""), shared_dir)
        else:
            _log(f"[audit] OK: {f.message}", shared_dir)

    if dry_run:
        if criticals:
            _log(f"[audit] [dry-run] {len(criticals)} CRITICAL finding(s); "
                 "page-on-transition delivery skipped (no signals written)", shared_dir)
        return
    ...
    #
    # Only a FULL run may write it. ``criticals``/``warns`` hold just the
    # checks this run ran, and the write is a wholesale overwrite, so a
    # partial run would replace the pod-wide open set with its own subset —
    # under-reporting the Security section until the next full run (and
    # ``--category mcp``, whose findings never reach dispatch_findings at
    # all, would blank it outright). Refreshing ``audit_completed_at`` would
    # compound it: the stamp is the report's liveness beacon (a stale
    # snapshot is how a dead audit daemon becomes visible — the 2026-05-05
    # two-day blind spot), so a partial run must not vouch for a full one.
    # Skipping leaves the last full-run snapshot standing and self-heals on
    # the next 15-minute scheduled run.
    if _covers_whole_pod(sweep_types, sweep_bot_ids):
        _write_findings_snapshot(criticals, warns, shared_dir)
    else:
        _log("[audit] partial run (--bot/--category): leaving "
             "current-findings.json from the last full run untouched", shared_dir)

    # Phase 3 of the alerts/signal-store consolidation
    # (internal/spec-alerts-signal-store-2026-05-07.md): mirror every
    ...
    _kept, critical_signals, withheld = _emit_signals_from_findings(
        criticals, warns, shared_dir,
        sweep_types=sweep_types, sweep_bot_ids=sweep_bot_ids,
    )

    # Page-on-transition (Phase 5b; internal/design-security-alert-fatigue-
    # 2026-08-31.md R-1+R-5): a critical finding pages only when its Signal
    # newly enters the firing state — created, reopened after resolve, or
    # waked from an expired snooze. Standing findings stay silent here and
    # live on the Alerts page; snoozed/dismissed signals suppress the push
    # (R-5) with an audit-trail entry.
    to_page: list[Finding] = []
    page_ids: dict[str, str] = {}
    n_dwelling = 0
    for c in criticals:
        signature = _audit_signature(c)
        if signature in withheld:
            # Still dwelling in the flap gate (R-2: the identity-hash
            # edit-vs-backup blip). Deliberately not observed this run, so
            # there is no Signal to gate on — and it must NOT trip the
            # fail-toward-paging fallback below. Once it dwells N≥2 runs
            # its Signal is created → newly firing → pages normally.
            n_dwelling += 1
            continue
        entry = critical_signals.get(signature)
        if entry is None:
            # Signal store unavailable or observe() failed — the gate can't
            # be evaluated, and a suppression gate must fail toward paging:
            # a wedged store must never silence a CRITICAL.
            if signature not in page_ids:
                to_page.append(c)
                page_ids[signature] = ""
            continue
        sig, _outcome = entry
        if sig.state in ("snoozed", "dismissed"):
            # R-5: the operator parked this signal from the Alerts page.
            # A wake after snooze expiry re-enters firing and pages on a
            # later run; dismissed stays quiet until explicitly re-opened.
            _record_telegram_delivery(
                {signature: sig.id}, shared_dir,
                suppressed_reason=f"signal {sig.state} by operator",
            )
        elif _signal_newly_firing(sig):
            to_page.append(c)
            page_ids[signature] = sig.id
    if n_dwelling:
        _log(f"[audit] Withheld {n_dwelling} dwelling CRITICAL finding(s) "
             "from delivery (flap gate: must persist 2 consecutive runs)", shared_dir)
    if to_page:
        ...
        outcome = _send_security_alert("\n".join(lines), shared_dir, config)
        # A page counts as delivered only on an explicit True. The batch-hash
        # resend layer that used to re-page a failed send every 7 days is gone
        # (#3919), so recording a failed send as a successful page would
        # silence the critical for the rest of its firing episode. Anything
        # other than True — including an unknown outcome — records the
        # failure marker instead and re-pages next run: a delivery gate must
        # fail toward paging.
        delivered = outcome is True
        if not delivered:
            _log(f"[audit] page of {len(to_page)} CRITICAL finding(s) was not "
                 "delivered — episode stays un-paged, retrying next run", shared_dir)
        _record_telegram_delivery(
            {s: i for s, i in page_ids.items() if i}, shared_dir,
            suppressed_reason=None if delivered else _PAGE_UNDELIVERED_REASON,
        )
    elif criticals:
        _log(f"[audit] {len(criticals)} standing CRITICAL finding(s), none "
             "newly firing — no page (see Alerts page)", shared_dir)
    ...
    _log(f"[audit] Run complete: {len(criticals)} critical, {len(warns)} warn, "
         f"{len([f for f in findings if f.level == 'ok'])} ok", shared_dir)
...
def main() -> None:
    ...
    _log(f"[audit] Starting — v{AUDIT_VERSION}, {len(all_bots)} bot(s)", shared_dir)
    ...
    # A partial run's kept_signatures only covers the checks it ran, so the
    # sweep must be narrowed to match or it auto-resolves everything else —
    # which then REOPENS (and pages) on the next full run.
    sweep_types, sweep_bot_ids = _sweep_scope(category=args.category, bot=args.bot)
    dispatch_findings(
        all_findings, shared_dir, config, dry_run=args.dry_run,
        sweep_types=sweep_types, sweep_bot_ids=sweep_bot_ids,
    )
    ...
    if run_all or args.category == "mcp":
        try:
            ...
            _log(
                ...
                f"{result['swept_resolved']} signal(s) auto-resolved", shared_dir
            )
        except Exception as exc:  # noqa: BLE001
            _log(f"[audit] MCP monitor failed: {exc}", shared_dir)
    ...
    if run_all or args.category == "plugins":
        try:
            ...
            _log(f"[audit] Plugin monitor failed: {exc}", shared_dir)
    ...
    if run_all or args.category == "hooks":
        try:
            ...
            _log(f"[audit] Hook monitor failed: {exc}", shared_dir)
    ...
    if run_all or args.category == "content_scan":
        try:
            ...
            _log(f"[audit] Content scan failed: {exc}", shared_dir)
    ...
    if run_all or args.category == "permissions":
        try:
            ...
            _log(f"[audit] Permission monitor failed: {exc}", shared_dir)
    ...
    if run_all or args.category in ("permissions", "app_permissions"):
        try:
            ...
            _log(f"[audit] App-manifest monitor failed: {exc}", shared_dir)
...
def _reset_baselines(shared_dir: Path) -> None:
    ...
    baselines = [
        ...
        # The incursion detectors' baselines. Removing one is the documented
        # "accept current state" path each detector's fix_steps names, so
        # --reset-baselines has to reach them too or the operator is told to
        # do by hand what this flag exists to do.
        baseline_dir / "baselines" / f"incursion-{name}.json"
        for name in _INCURSION_DETECTORS
    ]
    for b in baselines:
        if b.exists():
            b.unlink()
            _log(f"[audit] Removed baseline: {b.name}", shared_dir)

    # Remove per-bot OC audit timestamps
    for ts_file in baseline_dir.glob("last-oc-audit-*"):
        ts_file.unlink()
        _log(f"[audit] Removed baseline: {ts_file.name}", shared_dir)

    _log("[audit] Baselines reset — they will be recreated on next run", shared_dir)
