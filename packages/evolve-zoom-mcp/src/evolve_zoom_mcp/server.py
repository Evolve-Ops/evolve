...
def build_server(context: RuntimeContext) -> Server:
    ...
    decorators: handlers are constructor keyword arguments now, and they take a
    ``ServerRequestContext`` plus the request's typed params and return the
    typed Result rather than a bare list. The bodies below are unchanged — they
    still build the same tools and return the same content — so what the wire
    carries is identical; only the registration and the envelope moved.
    """
    ...
    async def _call_tool(
        ...
        if name in WRITE_TOOL_NAMES:
            content = await _call_write_tool(context, name, arguments)
        else:
            content = await _call_proxied_tool(context, name, arguments)
        # `_err` marks failures in the TEXT, which is what the 1.x contract
        # carried; `is_error` is left False so a caller reading either the text
        # or the flag sees what it saw before. Changing that is a wire change
        # and belongs in its own PR, not inside a dependency migration.
        return CallToolResult(content=list(content))

    return Server(
        _SERVER_NAME, on_list_tools=_list_tools, on_call_tool=_call_tool,
    )
