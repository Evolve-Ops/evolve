...
## PM dispatch lane — claim before you build

If you are about to build against a brief in `internal/dispatch/{queued,inflight}/` —
whether you were handed the id directly or you are a chip the dispatcher launched — run
`python3 tools/meta-dispatch-move start <id>` the moment you cut your branch (see
`tools/meta-dispatch-move`, `internal/meta-dispatch-procedure.md`). This is the claim
(D-TP5, `internal/decision-rig-throughput-2026-09-15.md`): it records your branch on the
brief and, when reachable, on its meta-state ledger row, so a second builder picking up
the same id sees it before spending an hour on a PR nobody needed.

**If it refuses with exit 4, someone is already on it.** The refusal names the holder
(their branch or session) and how long ago they started. Pick a different brief, or if
the claim looks stale, tell the operator so a human can
`tools/meta-dispatch-move abandon <id>` with a reason — never re-run `start` in a loop
hoping it clears, and never build against a claimed id anyway on the theory that yours
will be the good copy. This closes the exact gap that let one brief get built twice by an
app session and a chip seventeen minutes apart (#4266/#4267).
...
**Preflight scans UNCOMMITTED files; CI does not.** The repo-wide guards
(reserved-token scrub, PII scrub, launchctl-seam gate, public-manifest
cut-line check) enumerate with `git ls-files` — tracked at HEAD. A file you
have not `git add`ed yet is invisible to them, so preflight used to report
`scrub-guard` PASS on a brand-new fixture carrying real bot names and CI went
red the moment it was committed (PR #4281). Preflight now sets
`EVOLVE_SCAN_UNTRACKED=1` for every gate, which widens those guards to
tracked + untracked-and-not-ignored. CI deliberately does NOT set it, so no CI
job's scope moved.

The trade-off is the scratch file: a stray file in your working tree can now
red a gate CI would pass. That is the safe direction, and the fix is to
gitignore it or keep it outside the checkout — never to widen a guard's
allowlist. `tools/scan_scope.py` is the single definition of the scope and
carries `REPO_SCANNERS`, the per-gate record of which scanners see
uncommitted files and *why* the tracked-only ones must not; a new repo-wide
scanner that skips that registry fails `test_scan_scope.py`.

**Preflight also names open PRs editing your files.** After the gate table it
prints one line per open pull request whose changed files overlap yours. It is a
**warning, never a gate** — two chips legitimately edit one package — and the
right response is usually to rebase on the named PR, or to check you are not
building something it already built.

This is the guard for a collision the lane's own file hold cannot see. D-PM13's
`touches:` check in `tools/meta-dispatch-eligible` defers a candidate brief that
overlaps work in `internal/dispatch/inflight/`, and it is a good guard — but it
only sees briefs. A chip spawned straight from a session (`spawn_task`) has no
brief, and an operator app session's open PR is not an `inflight/` row, so on
2026-09-22 those two edited the same four files in parallel: the chip landed
first and the PR it was derived from went CONFLICTING and had to be
re-integrated by hand. Neither party passed the lane's gate; both run
`tools/preflight`, which is why the check lives there.

**So: when you spawn a follow-up chip out of work that is still an open PR, say
so in its prompt — name the PR and what it changed.** The chip still branches
from a clean `main` (that rule is unchanged), but it can then read the PR
instead of rediscovering it, and it will not re-document a spec section its
parent already wrote. `PREFLIGHT_SKIP_PR_OVERLAP=1` turns the check off; the
only good reason is that `gh` is unavailable and the line is noise.

**Writing it as a lane brief instead? Use `derived_from_pr:`.** That front-matter
field (a PR number or a list) records the same fact where the tooling can carry
it: `launch --headless` renders it into the chip's prompt — the only thing a
headless chip reads — and `meta-dispatch-move start` prints it to whoever claims
the brief. It is **provenance, not a dependency**: it never holds the brief.
`depends_on: [pr:N]` is the field that holds, and naming one PR in both is
refused. Full row in `internal/dispatch/README.md`'s front-matter schema.
