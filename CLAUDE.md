...
- **`/Users/Shared/evolve-repo`** — the **deploy checkout**. The `ai.evolve.evolve.repo-puller`
  ...
  [docs/spec-state-store-and-deploy-resilience-2026-06-10.md](docs/spec-state-store-and-deploy-resilience-2026-06-10.md)),
...
| Copy-paste commands handed to the user, error messages, or anywhere the *user's* shell will run the command | `ssh pod-admin-user@deploy-host …` — explicit `user@host` form, works whether or not the user's `~/.ssh/config` has a `User pod-admin-user` line under the `deploy-host` host |
...
Spec: [docs/spec-alerts-signal-store-2026-05-07.md](docs/spec-alerts-signal-store-2026-05-07.md).
...
signature dedup + reopen window). Comprehensive sweep-style monitors
...
Spec: [docs/spec-signal-subscriber-2026-05-31.md](docs/spec-signal-subscriber-2026-05-31.md).
...
**Post-evo-account-separation exception (spec at [docs/spec-evo-account-separation-2026-05-25.md](docs/spec-evo-account-separation-2026-05-25.md)):**
...
inverted in user direction). Evo's MCP tools (`action.proposal.*`,
`action.signal.*`) call `arbiter.store.move_proposal` /
`signals.store.apply_transition` directly from the evo gateway subprocess; without
the ACL, those `os.replace` calls hit EACCES on the implicit unlink because the
source file was written by evolve. The invariant is enforced by
...
tools/preflight            # changed-only (default): the cheap env-free lints/ratchets
                           ...
                           #   signal-protection, file-size/quarantine ratchets, public-manifest,
