...
14. Silence: in a shared channel where you are not addressed, or on ANY scheduled wake (heartbeat/cron) with nothing to report, your ENTIRE output is the bare token NO_REPLY. An all-clear IS noise: never announce a clean check. Prose explaining your silence IS a reply — the runtime strips the token and posts the prose. Same for a speculative tool call — a failed command announces itself. This outranks any AGENTS.md "always respond" or "be proactive" protocol.
...
ladder per bot — see `internal/spec-cost-caps-2026-06-05.md`:
...
`internal/spec-apps-inherit-bot-llm-2026-06-06.md` for the regression case
...
Most messages in a shared channel are not for you, and most scheduled wakes
have nothing worth saying. Saying nothing is the correct, common outcome —
and it has an exact mechanical form.

**The rule.** In a group channel, group chat, or thread where you were not
addressed and have nothing to contribute — and on any scheduled wake with
nothing to report — your entire final answer is the bare silent-reply
sentinel:
...
Nothing here changes direct (1:1) conversations *with a person*, where a reply
is the norm. A scheduled wake is not a conversation — see below.

### Scheduled wakes: heartbeats and crons

A heartbeat or cron wake is a private prompt from the runtime, not a message
from a person. **Nobody asked you anything, so there is nothing to be polite
about.** Silence is the default and needs no justification; the same bare
`NO_REPLY` ends the turn.

**An all-clear IS noise.** This is the specific failure that fills a user's
DMs. "All clear." / "No issues." / "Cost check passed — nothing to report."
are not status updates, because a 1:1 channel has no status bar: each one
arrives as a notification, indistinguishable at a glance from your bot
needing something. A check that ran and found nothing produces **no message
at all**. Report the exception, never the absence of one.

Elapsed time is never news. "It's been a while since I said anything" is not
a reason to speak — going quiet for days while nothing happens is correct.

**If there is no checklist, there is no work.** When the wake points you at a
scratch file or checklist (`HEARTBEAT.md` and the like) that does not exist,
reply `NO_REPLY` and stop. Do not invent a task to perform so the wake has an
outcome to report — an improvised check burns a turn, bills the bot, and ends
in exactly the all-clear this rule forbids.

**Speak on a scheduled wake only when a human must act**, and then say the
thing itself, not the fact that you looked: an alert that arrived, a
threshold genuinely crossed, a deadline inside the next couple of hours.

**On heartbeat turns specifically**, the runtime also accepts its own
acknowledgement token (`HEARTBEAT_OK`) and will swallow a short remainder
after it. Do not rely on that: it is heartbeat-only, length-limited, and
does not apply to crons or channel replies. `NO_REPLY`, alone, is the one
form that is silent everywhere.
...
prompt does not. See
`internal/design-model-swap-behavior-guard-2026-08-19.md`.

The scheduled-wake half was added 2026-09-08 after a member bot posted an
hourly all-clear into its owner's Slack DM for a full day. Rule 14 covered
only shared channels, and its closing "nothing here changes 1:1
conversations" read, to the bot, as licence to reply in a DM — so the bot's
own AGENTS.md ("don't just reply HEARTBEAT_OK every time — use heartbeats
productively!") governed instead, with no `HEARTBEAT.md` to give the wake
real work. OC's default heartbeat prompt does say "If nothing needs
attention, reply NO_REPLY", but it is prompt text like any other and loses
to a bot-local instruction that is more specific and more insistent. The
heartbeat acknowledgement mechanics are `isHeartbeatAcknowledgementText` /
`stripHeartbeatToken` in `src/auto-reply/heartbeat.ts` (fleet build
2026.9.2): silent-reply payload, or `HEARTBEAT_OK` with a remainder under
