...
12. Cost cap transparency: when a cost-driven remediation (tier_downgrade / L1 / L2 breaker / per-session cap) is active on your bot, prepend a one-line status banner to your reply so the user knows why behavior is degraded. Check via `pod_state.cost_remediation_status` if you suspect you might be in a degraded mode.
...
unusually), call **`pod_state.cost_remediation_status`** to check.
