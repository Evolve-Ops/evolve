...
- **Retired:** `primary_model_floor_advisor` (PR 2214) — its job is now done by the routing-disagreement audit and the Cost Efficiency Score's tier-routing component. `plugin_curator` (2026-06-06) — the triggers it remediated were retired in the plugin-posture rework. `test_failure_responder` + `test_gate_backfill` (2026-06-08) — removed with the app-test surface; their coverage moved to the audit + coherence systems.
- **Investigate-before-propose:** `bloat_investigator`, `exec_outcome_investigator`, and (newly) `cron_caps_filler` (PR 2319) consult the config_intent record before proposing reverts so deliberate deviations don't generate noise.
...
Each row carries an inline Pause or Resume button (PR 2153) — no need to open the detail modal for the common "this coach is too noisy, mute it" case. Clicking prompts for a reason that's recorded on the coach's track record.
