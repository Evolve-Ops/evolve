...
- **Overview page** in the dashboard — the release drawer shows an update
  banner when bots are waiting on a newer version. Banners are tiered from
  `RELEASES.yaml` in the repo (security / feature / maintenance): security
  notices cannot be snoozed, the others can. On canary pods the same drawer
  shows the pointer, candidate, and soak countdown.
