...
1. **Verify the admin server is running** — from the mini, run
...
**Recommended — `evolve-admin connect`.** Installs a persistent launchd agent
...
`evolve-admin connect --uninstall`. The `mini` value is the SSH host of the
Mac — replace it if your `~/.ssh/config` uses a different alias, or pass
the full `user@host` form.

**Equivalent without installing evolve-admin on the laptop.** A plain ssh
one-liner does the same thing for one session:

```bash
ssh -N -L 5050:localhost:5050 mini &
```
...
```bash
# in ~/.zshrc or ~/.bashrc
alias pod='ssh -N -L 5050:localhost:5050 mini'
```
