...
related_specs:
  - internal/spec-config-intent-system-2026-05-21.md
---
...
posture?"*. Evo can read the audit findings via `pod_state(query="audit")`, audit
specific apps via `app_action(action="audit")`, snooze / dismiss / resolve signals, and
...
The posture reshape shipped 2026-06-04 (PR 2279 / PR 2284) replaces the old baseline-snapshot drift signals — which fired on every plugin version bump and produced noise — with provenance-based trust. The signal now fires on "the plugin was loaded from somewhere we can't vouch for," not on "the plugin's hash changed since last week."

Producer: `plugin_monitor`. Spec: `internal/spec-plugin-posture-rework-2026-06-06.md` (current); `internal/spec-plugin-inventory-2026-05-10.md` covers the inventory layer.
...
3. **Phase 3 LLM inference** — for drift the system observes but doesn't have a paper trail for, an LLM pass infers the likely reason from session history and asks you to confirm. This shows up as a small banner on the queue (PR 2326, Phase 4.1 queued banner) — clicking through walks you through a one-question confirm.
