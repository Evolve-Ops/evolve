...
Without the Continuity Engine, these evaporate when the session ends. The engine
intercepts them, turns them into structured tasks, queues them, and runs them at
the right time.

Tasks that only write files or make simple API calls run without asking. Tasks
that need judgment, send messages, or make changes sit in a queue until you
approve them — approve from this dashboard (Maintenance → Pending tasks), or
ask a bot via `evo summary` to surface the queue.

The engine runs on a 15-minute cycle. Most of the time it's invisible — tasks
complete in the background and you see the results next session. A morning digest
at 8 AM summarizes overnight work.
...
|---|---|
| `evolve-admin setup` | Initial pod setup wizard |
...
**Continuity Engine** — extracts deferred tasks from sessions and runs them
autonomously when safe to do so, or surfaces them for approval. Mostly invisible
unless a task blocks.
