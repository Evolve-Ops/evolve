...
The graduated ladder shipped May–June 2026 replaces the old single-cap model with three independent levers, each more disruptive than the last. Each fires on today's spend in pod local TZ.
...
#### What "Reactivate this bot" means

Bringing a bot back is a statement about the spend, not just about the breaker file: **the spend that tripped it is accepted, and the cap starts counting from that moment.**

Reactivating records today's running total as an *acceptance marker*. Every later cap check measures spend **since** the marker, so the bot does not immediately trip again on the same dollars. The tile and the Circuit Breakers panel say so — "accepted $27.49 at 21:40; counting from then" — and the bot's own cap message repeats it, so a later "$5.20 of $5.00" reads correctly next to a day total of $32.69.

Two things this does **not** do:

- **It does not raise the cap.** A genuinely new spike of a full cap's worth after you reactivate trips the breaker again. If you want a higher ceiling, change the cap (or answer `continue` to the bot's checkpoint, which grants a stated increment until midnight).
- **It does not survive the day.** The marker belongs to the pod-local day it was taken in. At midnight today's spend restarts at $0 and the marker is simply never consulted again.

An automatic expiry is not an acceptance: when a breaker clears on its own after 24h, no marker is written and the next check measures the whole day, exactly as before.

#### One message per trip, and none for background work

A tripped breaker used to announce itself on **every blocked attempt** — heartbeats, scheduled jobs and inbound messages alike — so a busy channel filled with "Your message could not be sent…" every few minutes, addressed to people who could do nothing about it. It now speaks once and then stays quiet:

| Trigger | What the channel sees |
|---|---|
| Heartbeat, cron, scheduled run | **Nothing.** The turn is stopped before any model call and logged with `outcome: breaker_refused`. |
| A person's message | The cap-reached reply, **once per trip per conversation**. Later messages in that conversation get silence until the breaker clears. |
| The owner replying `continue` or `stop` | Always answered — the cap has to stay liftable from chat. |

A new trip is new news, so it speaks again. So does reactivation: bringing the bot back resets the one-message budget along with everything else.

#### Evolve's own work stops too

While the breaker is tripped, Evolve's internal cheap-LLM helpers — the tier classifier, preflight router, session judge and session summariser — refuse to run. Before this, a "paused" bot's plugin kept making its own model calls: one measured hour on 2026-09-07 saw the preflight router's run count go 10,407 → 13,282 while the bot itself was silent. Reactivation lifts the refusal with the bot; there is nothing separate to clear.
...
Cascade is off by default. It's a treatment-group experiment shipped 2026-06-05 (PR 2291) — bots in the treatment group get the chip and the live cascading behavior; bots outside don't. Once the per-bot effect on spend and quality is well-characterized this will graduate from experiment to default.
...
**Important:** Spending caps are enforced by Evolve's cost-cap ladder, not by the API provider. Set caps at your provider too (your provider's console (e.g. console.anthropic.com)) as a backstop in case the cap mechanism itself fails — defense in depth.
