...
A single page, sized for a thumb. Sections down the page — **Bot**, then
**Today**, **Inbox**, **Later**, **Done** and **Dropped** — and cards inside
them. Tap a card to move it to another day-bucket, to hand it to the bot (or
take it back), or to split it into your half and the bot's half. Tap **+** to
add one. It follows your phone's light and dark setting, and you can add it
to your home screen so it opens like an app.

The lanes are about *when* — inbox is untriaged, dropped is "not doing this",
which is deliberately not the same as done. Who a card is on is separate:
hand a card over and it moves to the **Bot** section at the top, whatever day
bucket it was in. Handing it over is an offer — the bot accepts it, or comes
back and says why it can't.

Cards the bot made for you sometimes carry a quiet extra line under the
title: how long a film runs, how many pages a book is, why you saved it.
Those facts are captured once, when the card is made, and each one records
where it came from.

There is no separate login. The link *is* the key, which is why the two
sections below matter.

## Your bot sees the same board

There is one board, not two. Everything on the phone page is the same list
your bot reads and writes when you talk to it — "add *book the dentist* to
today", "what's on my plate this week?", "hand the scan to you". It does that
through a tool the pod gives it, so what you tap and what you say end up in
the same place.

That also means your bot should never keep a task list of its own in a note
or a file. If yours does, it predates the board; tell it to use the board
tool from now on.
...
The code is about 90 characters wide and 45 lines tall, so make the terminal
window big — wide *and* tall — before you run the command. If it's too narrow
the pod says so rather than drawing a code your phone couldn't read. If you're
piping the output somewhere, add `--no-qr` to get the link on its own.
...
If the phone won't read it, make the window wider and taller and run the
command again, in a terminal that shows colour — the code is drawn in colour,
and without it the pod has to fall back to a shape some fonts smear. Either
way the link printed just above the code always works: type or paste that into
the phone's browser instead.
...
**"The network connection was lost", or the page resets as it loads.** The
phone reached the pod and the connection was cut immediately. On a Mac this
is almost always the built-in firewall: it is on, and it doesn't recognise
the Python that Evolve's admin service runs. Traffic from the Mac to itself
is exempt, so the admin pages keep working and nothing else looks wrong —
only the phone fails. One line on the pod fixes it:

```bash
sudo evolve-admin ensure-pod-perms
```

Then restart the admin service, which is the part people miss — the firewall
decides once per running program, so the rule doesn't apply to a service that
was already running:

```bash
sudo launchctl kickstart -k system/ai.evolve.evolve.admin-ui
```

Evolve also writes a plain line in the admin log the first time this happens
("inbound connection … dropped before any bytes") and raises an alert, so you
don't have to recognise it from the phone's error message.
