...
The User Profile Inferrer keeps each bot's per-user profile current by extracting new facts from the bot's own session transcripts. It runs **inside each bot at session_end**, using the bot's own LLM credentials — never as a centralized service. Spec: `docs/spec-user-profile-2026-05-07.md`.
