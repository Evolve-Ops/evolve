...
The two red rows hold the upgrade. The model-ref row blocks because Evolve
owns model routing: a `doctor` rewrite silently changes **which model answers
your users**. The upgrade will not start until you have read the from → to
list and confirmed it. That confirmation is a judgement about your pod, which
is why Evolve asks rather than deciding.

Preflight is read-only and idempotent — running it twice changes nothing and
tells you the same thing, so re-check as often as you like.
