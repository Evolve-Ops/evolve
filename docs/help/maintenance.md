---
title: "Help: Maintenance Page"
slug: maintenance
audience: public
last_reviewed: 2026-09-09
concepts:
  - maintenance
  - openclaw-compatibility
  - gateway-status
  - cron-jobs
  - openclaw-version
  - admin-server
  - logs
ui_surface: admin.maintenance
related_specs: []
---

# Help: Maintenance Page

The Maintenance page is the operational control center for the pod's infrastructure — gateway status, cron jobs, OpenClaw version, logs, and the admin server itself. Use this page when something is broken or you need to check on the health of Evolve's own infrastructure.

**Ask evo to act, not narrate.** This is the page where chat shortcuts pay
off most:

- *"pause all bots"* / *"resume all bots"* — pod-wide pause with the
  destructive-tier confirm gate (you'll be asked to confirm).
- *"restart team-bot-a"* / *"redeploy evolve"* — single-bot operations without
  hunting for the right row.
- *"any infra daemons down?"* — evo summarizes `pod_state(query="host")` for you.
- *"show team-bot-a's recent errors"* — `pod_state(query="errors")` returns the last raw
  log lines for triage.

The destructive operations (pause-all, remove) always require explicit
`confirm: true` regardless of authority tier — defense in depth.

---

## Subtabs

### Status

Shows the live status of every bot's OC gateway. The table shows:
- **Bot** — bot name and user
- **Gateway** — responding / not responding (HTTP health probe)
- **Port** — the port the gateway is listening on
- **Last heartbeat** — when the bot last sent a heartbeat (indicating active use)
- **Uptime** — how long the gateway has been running

**Auto-refresh** checkbox — keeps the table updating every 30 seconds.

**Refresh** button — manual refresh.

If a gateway is down, you can restart it from this tab. Evolve's `heal.py` also runs every ~5 minutes and restarts failed gateways automatically.

*Note: This page probes each gateway via HTTP. A bot appearing "offline" here doesn't necessarily mean the bot is broken — the gateway process could be healthy but not yet responding (still starting up), or the port configuration could be wrong.*

### Cron Jobs

All scheduled jobs running on every bot, aggregated in one view. Data comes from `openclaw cron list --json` on each bot. Shows:
- Job name and description
- Schedule (cron expression)
- Last run time and result
- Next scheduled run
- Status (active, disabled, failing)

**Auto-refresh** (60s) keeps this view current.

Common Evolve cron jobs on each bot:
- `evolve-measure` — daily at 01:00, aggregates session metrics
- `evolve-analyze` — weekly Sunday 02:00, runs pattern detectors and generates proposals
- `evolve-heal` — every 5 minutes, checks gateway health and restarts if needed
- `evolve-audit` — every 15 minutes, security audit
- `evolve-defer-runner` — every 2 minutes, fires bot-scheduled Continuity Engine defers (pod-wide, runs as the evolve user)
- `pod_perms_drift_monitor` — hourly, watches for between-deploy ACL / ownership drift on shared directories. Emits a Signal when a managed dir's permissions change outside a deploy. Added PR 2217.

### Infra Jobs

LaunchDaemon plists installed in `/Library/LaunchDaemons/` — the OS-level services that keep Evolve's own jobs running. The table shows:

- **Label** — the launchd service identifier (e.g., `ai.evolve.evolve.admin-ui`)
- **Schedule** — when the job runs: `on-demand` for manually-triggered jobs, `every Ns` for interval jobs, or a calendar spec for time-based jobs
- **Loaded** — whether the service is currently registered with launchd (✓ loaded / not loaded)
- **Enabled** — whether the service will auto-start on reboot; a loaded-but-disabled job will vanish after the next restart
- **PID** — the process ID if the job is actively running; `—` if stopped
- **Last Exit** — exit code from the most recent run; `0` is success, non-zero (shown in red) indicates a failure

This is distinct from cron jobs (which run inside OC): Infra Jobs are macOS launchd services that run even when OC isn't running.

If an infra job shows as not loaded, it may have been unloaded by a macOS update or a system restart. Use `sudo launchctl bootstrap system <plist-path>` to reload it.

### OC Version

Shows the installed version of OpenClaw on each bot vs. the latest available on the npm registry. If a bot is behind, an upgrade notice appears here.

**Refresh** checks npm for the latest version.

To upgrade OpenClaw: go to System subtab → the **OpenClaw runtime** card.

**These are two different upgrades and they are not interchangeable.**

| Card | What it changes | What it does not |
|---|---|---|
| **OpenClaw runtime** | The OpenClaw binary every bot runs on | Nothing about Evolve |
| **Upgrade Evolve Pod** | Evolve's own code and plugin | Nothing about OpenClaw |

On 2026-09-07 one click on the wrong one moved the runtime under nine bots at
once and took a day to recover. Each card now names what it touches, and the
OpenClaw card carries a red caption saying it changes the runtime under every
bot.

#### The preflight table

Before anything is switched, Evolve fetches the target version to a scratch
location — **it is not installed and nothing is linked** — and runs that
version's own `config validate` and `doctor` against each bot's config, as
that bot, as a dry run. `doctor --fix` is never run here. The result is one
row per bot:

| Column | What it means |
|---|---|
| **invalid keys** | Keys the new version rejects that the old one accepted |
| **retired surfaces** | Settings the new version has retired |
| **model refs rewritten** | **Red, and blocks the upgrade.** Model references `doctor --fix` would change, `from → to` |
| **agent workspace outside home** | **Red, and blocks the upgrade.** An agent pointing outside its own bot's home — accepted by older OpenClaw, fatal from 2026.9 on |
| **retired in `modelPolicy.allow`** | A retired model left in the allow-list that is not your primary or a fallback |
| **channel packages** | Per-bot `@openclaw/*` packages on a different release line, which need reinstalling |

The two red rows hold the upgrade, and so does a third case the table itself
can't show: **any bot the preflight could not fully check also blocks** — a
failed fetch, an unreadable config, a `doctor` reply Evolve doesn't recognize
the shape of. "We didn't get a clear look" is treated the same as "we looked
and found something", never the same as "clean" — a preflight that goes
quiet on a failure is worse than no preflight at all.

The model-ref row blocks because Evolve owns model routing: a `doctor`
rewrite silently changes **which model answers your users**. The **Run
upgrade now** button opens a plan that includes this preflight; the upgrade
will not start until a clean run for that *exact* installed-version → target
pair exists and you have ticked that you've read it. That confirmation is a
judgement about your pod, which is why Evolve asks rather than deciding — and
the server enforces the same rule, so this isn't just a button being greyed
out.

Preflight is read-only and idempotent — running it twice changes nothing and
tells you the same thing. It is cached per exact from → to pair, so opening
the plan again for a pair you've already checked shows the cached result
instead of re-fetching and re-running nine bots' worth of `doctor`; **Re-check**
in that panel runs it again on demand.


#### Versions are installed side by side, and pinned per bot

Evolve keeps each OpenClaw version in its own directory under its own prefix
(`/Users/Shared/evolve-oc/<version>/`), and **never links it into
`/opt/homebrew/bin`**. Each bot's registry row names the version it runs, and
its launchd job is rendered from that name.

This exists because of what one click did on 2026-09-07: a single Homebrew
cask moved under nine launchd jobs, the previous version was gone the same
instant, and there was no way back. With versions installed beside each other:

| You want to | You run |
|---|---|
| Add a version without touching anything running | `sudo evolve-admin oc install 2026.9.4` |
| See what is installed and who runs what | `sudo evolve-admin oc list` |
| Move one bot onto a version | `sudo evolve-admin oc pin <bot> 2026.9.4` |
| **Roll that bot back** | `sudo evolve-admin oc pin <bot> <previous>` |
| Delete a version nothing runs | `sudo evolve-admin oc remove <version>` |

**Rollback is the same command as the upgrade, backwards.** That is deliberate:
a recovery path that only runs on the worst day it will ever have is a path
nobody has tested. Here it is the path you already used to get there.

`oc remove` **refuses while any bot pins that version** — and also whenever it
cannot tell: an unreadable `network.json`, a bot row or pin it cannot parse, or
a gateway job still running that version all count as "in use". That refusal is
what keeps a rollback available — a version a bot is running can never be
deleted out from under it.

One bot's gateway restarts per `oc pin`, never the fleet. `oc pin` refuses a
version Evolve has not validated (the same check the Update card uses) unless
you add `--override`; every pin, and every override, is logged. It reports
"restarted on <version>" only after it has seen the gateway come back healthy
and running that version; otherwise it says the outcome is unknown.

**Migrating an existing pod:** `sudo evolve-admin oc adopt` copies the
OpenClaw you already have into the store as its first version, checks the copy
runs, and prints the per-bot pin commands. It **copies** the existing install — Homebrew keeps working
exactly as it does now — and it deliberately does not restart nine gateways for
you. One bot at a time, with a human watching, is the whole lesson.

In the **Bot Versions** table, a pinned bot's OC Version cell reads
`v<version> · pinned`. A bot with no pin runs whatever the global binary
resolves to and has no per-bot rollback; its tooltip says so.

#### "Not yet validated by Evolve" — what that means

The update notice answers two questions, not one. *Is there a newer OpenClaw?*
comes from npm. *Does Evolve work on it?* comes from Evolve's **compatibility
contract** — a suite of tests for every assumption Evolve makes about
OpenClaw, run against that exact version before it is ever offered to you.

- **Validated** — the contract passed on this version. The notice offers the
  guarded upgrade in the usual way.
- **Not yet validated** — either the contract has not been run against this
  version yet ("contract run pending"), or it ran and something broke
  ("failed on: …", naming the checks). Evolve will not upgrade your pod onto
  it, and the button is not shown.

This exists because of a real failure. One click once moved nine bots to a new
OpenClaw at the same time; four config keys had been quietly retired, a
plugin hook began firing where it never had, and a silent reply stopped being
silent. None of it was in a changelog. Recovering took a day. The contract
turns each of those assumptions into a test that runs first.

**Upgrade anyway.** It is your pod, so the option is there — "Upgrade
anyway…" opens a dialog listing the checks that fail and asks you to type a
sentence that names them. That is deliberate: a confirmation you can click
through without reading is not a decision. The override is recorded, and it
expires on its own — if a later contract run finds a *different* set of broken
checks, you are asked again with the new names.

**What to do while you wait.** Nothing, usually. Validated versions appear
here automatically as Evolve's nightly compatibility run confirms them, and a
version that fails is a bug Evolve fixes on its side — not something to
configure on your pod.

### Gateway Logs

Live log output from each bot's OC gateway process. Select a bot from the tabs.

**Auto-refresh** (10s) streams new log lines.

Logs are read from the bot's OC log file (shown at the bottom of the panel). Look here for:
- Plugin errors (TypeScript errors from the Evolve plugin)
- Gateway startup/restart events
- API errors (rate limits, auth failures)
- Model routing decisions (if debug logging is enabled)

### Admin Server

Status and controls for the Evolve admin Flask server itself (the server running this UI).

**Service Status Card:**
- Shows whether the admin server is running as a launchd service
- **↺ Restart Server** — restarts the Flask process (you'll briefly lose connectivity)
- **⬇ Install as Service** — installs the admin server as a persistent launchd service (if not already)
- **✕ Uninstall Service** — removes the launchd service (server won't restart after reboot)
- **View Logs** — shows the last 100 lines of the admin server log

**Diagnostic Report Card:**
- Captures system info, configuration summary, and recent error logs into a report
- **✉ Send Report** — emails the report (requires email configuration)
- **⬇ Save Report** — downloads the report as a file

### Setup

The tunnel setup wizard for accessing the admin UI from your laptop via SSH tunnel (for when the pod host isn't on the same network). Steps:
1. Install admin server as a persistent service
2. Set up SSH keys between laptop and the pod host
3. Configure tunnel parameters (hostname, port, SSH key path)
4. Download setup scripts
5. Add a browser URL shortcut
6. Verify the connection

This is only needed if you want to access the admin UI remotely. If you're on the same network, just browse to `http://<pod-host-ip>:5050`.

### Claude Access (MCP Bridge)

Configuration for connecting Claude Desktop to your pod via the MCP Bridge.

**MCP Bridge Card** — shows whether the MCP Bridge is running on port 5051.

**Bot Context Access Card** — shows which bots Claude Desktop can read/write context for.

**Claude Desktop Config Card** — generates the JSON snippet to paste into `~/Library/Application Support/Claude/claude_desktop_config.json`. Two modes:
- **Same machine (localhost)** — if Claude Desktop is on the same host as the pod
- **Remote (Tailscale)** — if Claude Desktop is on your laptop connecting over Tailscale VPN

**Recent Activity** — shows recent MCP Bridge calls (writes only toggle to filter).

### System

**Evolve Version & Sync Status** — shows current Evolve version vs. the latest in the repo. If your install is behind the repo, an upgrade option appears.

**Upgrade Evolve Pod** (Evolve only — does not touch the OpenClaw runtime; for
that see **OC Version** above):
- **Upgrade All Bots** — updates the Evolve plugin on every bot and restarts their gateways
- **Upgrade Selected Bot** — single bot upgrade
- **Skip plugin rebuild** — skips TypeScript compilation (faster, use when only Python code changed)
- **Dry run** — shows what would happen without making changes

**Danger Zone:**
- **Uninstall Evolve** — removes Evolve from the pod. This uninstalls the plugin from all bots, removes launchd services, and optionally removes the `evolve` user. Bot users and their OC config are NOT removed.

### Pod Health

A comprehensive health scan of the entire pod. Shows a scored checklist of checks per bot:

- **Evolve plugin installed and active** — is the TypeScript plugin running?
- **Primary model reachable** — can the bot make an API call to its configured model?
- **Auth order** — is the most cost-effective auth method listed first?
- **Gateway bind address** — should always be `127.0.0.1`, not `0.0.0.0`

**OpenClaw Runtime** checks (added after the 2026-09-07 upgrade, when every one
of these was false on every bot and no surface said so):

- **Config validates under the installed OpenClaw** — a config the running
  binary rejects. Usually survivable until it isn't.
- **Channel packages match the runtime** — per-bot `@openclaw/*` packages on a
  different release line than the binary. This is what kills a channel with
  `Package subpath './plugin-sdk/channel-streaming' is not defined`.
- **Runtime validated by Evolve** — whether the installed version passed
  Evolve's compatibility contract. "No run recorded" is reported as *not
  validated*, never as healthy: absence of a run is not a pass.

All three are warnings rather than failures — your pod may be running fine on
an unvalidated runtime, and the honest statement is "nobody checked".
- **Evolve plugin installed** — plugin present and responding at `/evolve/status`
- **API key presence** — expected providers have keys configured

**Scan Now** — runs a fresh health scan (takes 15–30 seconds for a full pod).

**Fix Issues** — for issues with automatic fixes available, applies them in bulk. Not all issues have automatic fixes — some require manual action (shown with instructions).

---

## Common Questions

**A bot's gateway is down. How do I restart it?**
Go to Status subtab, find the bot, and click restart. Alternatively, heal.py runs every 5 minutes and will restart it automatically. If it keeps going down, check Gateway Logs for errors.

**The admin server shows as not running as a service. Should I worry?**
If you're running the admin server manually (e.g., from a terminal), it won't survive a reboot. Install it as a service from the Admin Server subtab so it auto-starts. This is essential for a production pod.

**How do I access the admin UI from my laptop?**
Use the Setup subtab to configure an SSH tunnel. You'll download a script that establishes the tunnel and optionally adds a browser shortcut. Requires Tailscale or a direct SSH connection to the pod host.

**Pod Health shows critical issues — where do I start?**
Start with the highest-severity issues (red ❌). Critical issues usually involve: plugin not installed, gateway binding to 0.0.0.0, or no working API keys. Most have specific fix instructions shown inline. Run Fix Issues for any that have automatic remediation.
