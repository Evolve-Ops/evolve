...
#### Suggestion screening and application
Two modules used to appear here and no longer exist. `review.py` (the standing
security review, "every 2 minutes") was retired in August 2026 and its
auto-reject rules now run inside the arbiter's own routing gate. `apply.py` (the
per-bot applier, "every 5 minutes") was retired shortly after, when it turned
out to have been watching a folder suggestions never reach — it had applied
nothing in its entire recorded history.

Both jobs are now part of the Better Engine refresh below rather than separate
scheduled modules: a suggestion is screened when it enters the queue, applied on
the admin host once it is approved (with the same backup, health check and
auto-rollback), and checked afterwards by Check-in. There is nothing to enable
or disable for either, and nothing runs on a 2- or 5-minute timer for them.
