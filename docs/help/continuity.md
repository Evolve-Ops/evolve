...
You currently can't — there is no manual task-creation form or CLI command. The
task queue is populated by the extractor from your bot sessions. If a task you
expected isn't there, mention it explicitly in a bot conversation and the
extractor will pick it up on its next pass.
