...
Discovery runs three ways: **on arrival** (once per bot, the first time Evolve
deploys to it — so an adopted pod has a populated Discovered tab before you
touch anything), **weekly** (a Sunday sweep, so a habit that forms later still
surfaces), and **on demand** when you click **Sync all bots** on the Discovered
tab. All three run the same 5-phase pipeline:
...
Scheduled jobs are read from **both** places a bot can have them: the user crontab, and OpenClaw's own cron store (which is where schedules you created with `openclaw cron` live). A job that is switched off is not counted, and a bot whose store can't be read simply contributes no schedules — the scan still completes.
...
Option 1 (recommended): Click **Sync all bots** on the Discovered tab — the scanner detects apps from workspace evidence and generates manifests automatically, including assigning IDs and embedding markers in all found files.
...
**Documents** ships its files as a files-pack (as EA Pack does) and, unlike
the forge route, can be installed deterministically without a forge run. It
teaches the bot to keep any document-shaped output (an itinerary, a letter,
a report) as a file in the workspace's `docs/` folder, reply with a short
summary and the path instead of the document, apply revisions as small
edits, and render the PDF or HTML with a script — so a change to one line
does not re-send the whole document through the model. An operator installs
it from the deploy box with
`sudo evolve-admin application install-gallery-pack --pkg p-38da680b --bot <bot> --apply`;
the same command without `--apply` previews what would land and writes nothing.
