...
The Apps page (in the **Improve** bucket) is where you manage installable applications and the forge jobs that build them. It has four tabs: **Installed**, **Gallery**, **Forge Jobs**, and **Reliability**.

**Coherence + Reconciliation surface.** Each installed app's card now carries small **Coherence** and **Drift** chips along its top edge — the operator-facing surface for the coherence + reconciliation framework shipped 2026-06 ([spec](../spec-app-coherence-and-reconciliation-2026-06-05.md)). The full breakdown is in the [manifest modal](#coherence--drift-per-app-in-the-manifest-modal) section below.
...
- *"install morning-brief on team-bot-a"* — direct gallery install via
  `action.app.install`. Forge runs the install asynchronously; evo polls
...
- *"check the status of job j-abc12345"* — `pod_state.forge_job` returns
  the job's current step + completed steps.

---

## Installed tab — app manifests

The Installed tab manages **app manifests** — the structured contracts that define what each bot is supposed to do, which files implement it, how to verify it's working, and whether it's improving over time. Manifests are the source of truth for every app running on a bot.
...
Click either chip to open the manifest modal scrolled to the **Coherence + Drift** section. Empty when there's nothing to report; populated when the audit framework's coherence passes ([PR 2270](https://github.com/evolve-ops/evolve/pull/2270) Pass C1 / [PR 2271](https://github.com/evolve-ops/evolve/pull/2271) Pass C2+C3) or the reconciliation pass find something.

The section is the operator-facing surface for [spec-app-coherence-and-reconciliation-2026-06-05.md](../spec-app-coherence-and-reconciliation-2026-06-05.md). Three concepts cleanly separated:
...
- **Override pre-deploy gate…** *(only when status is `incoherent`)* — bypass the pre-deploy coherence gate ([PR 2263](https://github.com/evolve-ops/evolve/pull/2263) / [PR 2325](https://github.com/evolve-ops/evolve/pull/2325)) for the current finding set. Use sparingly; the gate exists to keep visibly-broken manifests off the bot.
...
**Bot-side coherence repair.** When you approve a drift or accept a finding, the bot itself can be tasked to land the change ([PR 2336](https://github.com/evolve-ops/evolve/pull/2336)) — the admin daemon dispatches a repair turn into the bot's session and you can watch the result in Forge Jobs. The same grammar is reachable via chat: *"evo app-changes"* lists pending coherence findings across all apps; *"evo app-coherence team-bot-a morning-brief"* drills into one app.
