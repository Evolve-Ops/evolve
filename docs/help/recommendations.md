...
- **Config patches** apply automatically — backup, write, gateway restart, health check, auto-rollback if the health check fails. Evolve does this itself on the admin host; there is no separate per-bot job to check.
