...
Run the full wizard — it creates accounts, installs OpenClaw where it's
missing, and deploys everything:
```bash
sudo evolve-admin setup --fresh
```

**This is also the path when you already run OpenClaw bots.** `--fresh` does
not mean "wipe": it adopts what is already on the box (existing installs,
accounts and `openclaw.json` files are left alone) and adds the Evolve service
layer around them. See
[Install on macOS](help/install-macos.md) or
[Install on a Linux VPS](help/install-linux-vsp.md) → "I already run
OpenClaw bots".

`sudo evolve-admin setup` *without* `--fresh` is the config wizard for a pod
that already has that service layer — it rewrites `network.json` and re-deploys
each member. It does not create the `evolve` service account or install the
daemons, so on a machine that has neither it refuses and points you back at
`--fresh`.
