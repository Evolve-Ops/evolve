...
```bash
sudo git clone https://github.com/<your-org>/evolve /Users/Shared/evolve-repo
...
| task-runner | Every 15 min | Continuity Engine task queue |
| morning-digest | Daily 08:00 | Daily summary to primary bot |
...
Deploy rebuilds the plugin and reinstalls all jobs automatically.
...
**Month 2 (optional):**
```bash
evolve-admin modules enable continuity_engine
evolve-admin modules tune continuity_engine budgetFloor 0.20
```
...
`review.py` may be auto-rejecting them. Check:
```bash
