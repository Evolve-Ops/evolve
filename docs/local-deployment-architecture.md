...
*Note: `security-bot` watchdog user retired — security monitoring delivered by Security Protocol v2 running as `evolve` user (git backup + audit.py + HMAC signing). See docs/spec-security-protocol.md.*
...
The `evolve` user handles all of this via a combination of direct ownership of `/Users/Shared/evolve/` and narrow sudo grants defined in `/etc/sudoers.d/evolve` (see `docs/spec-sudoers.md`).
...
```
All grants are defined in `/etc/sudoers.d/evolve`. See `docs/spec-sudoers.md` for the full list and rationale.
...
The admin UI shows a banner when a newer version is available but does not attempt to run the install — the `evolve` process does not own the Homebrew prefix and should not. Autonomous updates of system-level software by a background process are intentionally out of scope. See `docs/spec-sudoers.md` §"What Evolve Should NEVER Be Granted" for the npm rationale.
