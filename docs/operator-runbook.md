...
## Scenario: a suggestion applied but the change didn't stick

> **Note (2026-08-18):** this scenario used to be headed "apply.py ran but the
> change didn't stick". The per-bot `apply.py` daemon and its
> `proposals/apply-results/` records are retired — the directory survives on
> older pods but nothing writes to it any more. Suggestions are applied by
> `arbiter.apply` on the admin host; check the proposal's own JSON under
> `{shared_dir}/proposals/` and the admin server log rather than the paths
> below, which are kept for reading historical records.
...
- The change required a longer startup time than the applier's post-restart health-check wait
...
## Scenario: A bot's scheduled follow-up never fired (Continuity Engine)

Under Continuity v2 the bot schedules its own follow-ups via the `defer`
plugin tool; the pod-wide defer runner fires due rows every 2 minutes. (The
v1 extractor/approval-queue stack and the `evolve-admin tasks` CLI were
removed — there is no admin task queue.)

**Check what the bot has queued:**
```bash
cat /Users/<bot-id>/.openclaw/workspace/evolve/defer-queue.jsonl
```

**Check whether it fired and failed:**
```bash
cat /Users/<bot-id>/.openclaw/workspace/evolve/defer-archive.jsonl
# failed rows carry the error in the "result" field
```

**Check the runner itself:**
The Maintenance page shows a one-line defer-runner health summary; the log is
at `/Users/evolve/.openclaw/logs/evolve-defer-runner.log`.

**If the queue is empty:** the bot never called `defer` — it promised without
scheduling. Check the bot's `tier` is `full` (lower tiers don't get the tool)
and that `bots.<id>.continuity_engine.enabled` isn't false in `network.json`.
...
| ~~apply.py~~ (retired 2026-08-18) | `/Users/Shared/evolve/proposals/apply-results/` — historical records only; nothing writes here now |
| ~~review.py~~ (retired 2026-08-14) | `/Users/Shared/evolve/proposals/rejected/` — historical records only |
