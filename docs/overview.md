...
analyze.py runs 12 pattern detectors → generates specific proposals
  ↓ (every 2 min)
review.py screens proposals (auto-rejects unsafe ones via security_rules.json)
  ↓ (human action)
Operator approves or rejects in the dashboard
  ↓ (every 5 min)
apply.py deploys to the target bot, health-checks, auto-rolls back if broken
  ↓ (7 days later)
...
Persistent task tracking that makes stateless bots feel continuous.

Every session a bot has is stateless — it starts fresh with no memory of prior
conversations beyond what's in its workspace files. The Continuity Engine bridges
that gap:

- Extracts tasks from session transcripts
- Surfaces pending tasks at the start of subsequent sessions
- Tracks pre-approved recurring tasks (weekly review, cost summary, etc.)
- Requires explicit approval before any task runs
- Maintains a rolling view of what's in-flight across the pod

**The problem it solves:** Users repeat context at the start of every session. Bots
make commitments they don't keep because there's no mechanism to carry them forward.
The Continuity Engine makes the bot feel like it remembers, even though it doesn't.
