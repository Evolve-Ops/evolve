...
│  apply.py             — apply approved proposals (5min, per bot)│
...
|-------|-------------------|
| **Per bot, as the bot's own user** (installed by `deploy_bot`) | apply.py every 5 min · cost_event_converter.py every 15 min · app_audit_runner.py Tier-2 every 6 h + Tier-3 hourly · backup.py daily 02:00 · doctor_pass_runner.py daily 03:17 |
...
```
primary-bot (primary)                  member-bot (member)
  ...
  → routing: auto-eligible?            → applies within /Users/<bot-id>/ context
      yes → approved_auto              → no sudo needed
      no  → human approves in UI       → health check + rollback if broken
  → apply.py on target bot applies it  → proposal moves to applied/
...
4. **apply.py safety gates** and the **post-apply verify daemon** (below).
