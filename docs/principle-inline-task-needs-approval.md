...
> **2026-07-31 note:** the Continuity v1 extraction stack this principle was
> distilled from (task_extractor / task_queue / task_runner / inline_executor)
> was removed in PR #835 — Continuity v2 has no LLM-extraction path and no
> CE-owned inline-code execution at all, so no live code currently exercises
> the rule. The principle itself remains binding for any future path that
> turns LLM output into executable inline actions. References below to
> `continuity-engine.md` sections describe the v1 doc — see that file's git
> history; the current doc covers v2.
...
- [docs/architecture.md](architecture.md) §"Security Model" — the broader security pipeline this principle sits within
