...
> **Design spec:** [docs/spec-model-rungs-and-roles-2026-06-09.md](spec-model-rungs-and-roles-2026-06-09.md).
...
`power`, `max`, `judge`) is a stable job description; it resolves to a **rung**
...
A role is a map entry in `network.json::models.roles`: role ID → rung slug (or a
small object for constrained roles). One namespace, used everywhere — tool
schemas, routing config, annotations, admin-UI values.
...
| `judge` | `sonnet-class` + provider constraint | Cross-model evaluation. Selected by provider diversity, not strength. |

"Grunt", "Workhorse", "Power", "Max", and "Judge" survive only as **display
labels** in the admin UI. The old user-choice enum (`fast | standard | power`)
and the old internal tier names collapse into this single role namespace —
there is no more tierN↔name↔choice triple-translation.

### The judge role — the one constrained role

`judge` is the only structured role:

```json
"judge": { "rung": "sonnet-class", "provider": "not-standard" }
```

`provider: "not-standard"` is the Goodhart's-Law constraint promoted from prose
to a **checkable invariant**. Role resolution picks the first model in the rung's
cluster whose provider differs from `standard`'s resolved provider. Config
validation — both the loader and `evolve-admin models set` — **errors** if no
such model exists. See [the judge constraint](#the-judge-constraint-goodharts-law)
for the rationale.
...
| Role | `fast`, `standard`, `power`, `max`, `judge` | Which compute/cost class the model call uses |
...
`models set <role> <rung>` validates the judge provider-diversity invariant and
warns if a re-point changes `costClass` by more than one step. `--yes` skips the
confirmation prompt for scripted/deploy use.
...
tiers (`tier3`→`fast`, `tier2`→`standard`, `tier1`→`power`, `tier0`→`judge`) and
...
## The judge constraint (Goodhart's Law)

The cross-model judge role exists because of Goodhart's Law: *"When a measure
becomes a target, it ceases to be a good measure."* If Evolve used Claude to
evaluate Claude's own proposals, it would systematically bias toward
Claude-flavored solutions. Requiring `judge`'s provider to differ from
`standard`'s provider buys independent evaluation.

This used to be prose. It is now the checkable `provider: "not-standard"`
constraint on the `judge` role: resolution picks the first model in the rung
whose provider differs from `standard`'s resolved provider, and config
validation (loader + `evolve-admin models set`) errors if no such model exists.
Practically:

- `standard` = Anthropic → `judge` must resolve to OpenAI or Google
- `standard` = OpenAI → `judge` must resolve to Anthropic or Google
- `standard` = Google → `judge` must resolve to Anthropic or OpenAI
...
`defaultRole` validates against `{fast, standard, power}` — `max` and `judge` are
not legal per-bot defaults (`max` because it is pull-only; see above).
...
|---|---|---|---|
| `tier0` | Judge | `judge` | `sonnet-class` + provider `not-standard` |
