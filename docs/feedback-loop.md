...
admin server validates the proposal → validation result
    │
    ▼ (on approval, on the admin host)
arbiter.apply applies change → snapshot → health check → rollback if needed
    │
    ▼ (every 5 min)
...
Validation result: `pass` / `fail` / `needs-human` / `error`. A non-`pass` result blocks the apply (investigation and workflow_change proposals bypass the check).

### Stage 5: Application

> **Changed 2026-08-18.** This stage used to be described as `apply.py` running
> on the target bot as that bot's own user. That daemon is retired — it polled
> `proposals/approved/`, a directory no arbiter status maps to, and had applied
> nothing in its recorded history. `arbiter.apply` on the admin host, running as
> the `evolve` service user, is and was the applier that runs.

`arbiter.apply` applies the change:
1. Refuses anything not in `approved_auto` / `approved_human`, and defers if the target bot's circuit breaker is tripped
2. Captures a snapshot and attaches it as the proposal's revert plan — no snapshot, no apply (claim-carrying proposals only; claim-less ones have nothing to revert and skip the step)
3. Dispatches to the Applier registered for the action kind, which writes the patch
4. Restarts the gateway (for config changes) and health-checks it
5. If unhealthy: rollback via the revert plan
6. Transitions the proposal to `applied/`; the verify daemon owns the post-horizon verdict
...
When a proposal applies successfully, a pending outcome record is written to `feedback/pending-outcomes.jsonl` (`outcome.py` promotes it to `feedback/outcomes.jsonl` once the operator responds):
```json
{
  "proposal_id": "prop-...",
  "target_bot": "<bot-id>",
