...
| `spendCapAction` | `"checkpoint"` | What to do when a cap is hit — see the table below. |
| `maxSessionContextTokens` | `100000` | Sessions above this input-token total are flagged in the health report. |

### `spendCapAction` values

Every action stops the bot's **background** work — heartbeats and scheduled
jobs — the moment the cap trips. They differ in what happens to a **person**
talking to the bot.

| Value | What a person sees |
|-------|--------------------|
| `checkpoint` **(default)** | The next reply is a fixed message: the cap, today's spend, what was paused, and two choices — `continue` (adds a stated amount to today's cap, until midnight) or `stop` (paused until tomorrow). Nothing is sent to a model until that's answered, so the held turn costs $0. Only the bot's **owner** (pod admin or primary user) can say continue; anyone else is pointed at the owner. |
| `downgrade-tier` | The bot keeps answering, but on its cheapest model for the rest of the day. Replies get noticeably less careful, an app that declared a minimum model tier runs below it, and every turn still bills. **Not recommended** — this was the default until 2026-09-04 and is kept only for operators who deliberately want it. |
| `alert-only` | The operator is alerted; conversation is untouched. |
| `pause-crons`, `suspend-bot` | Legacy heavier actions, now owned by the L2 cost rung. |

A **soft warning** fires once per bot per day at 80% of the cap, so the
conversation about raising it can happen before the trip.
...
| `cost.levers` | object | no | Per-bot opt-outs for the experience-preserving cost levers, e.g. `{"housekeeping_cheap_rung": false}`. Every lever is **on** unless set to `false`. See "Cost levers" below. |
...
Some cost savings change nothing about the conversation — the same model answers, with the same context — and only remove redundant work. These ship **on for every bot**; a bot opts out with an explicit `false`. A pod-wide `cost.levers` block at the top level of network.json sets the default for every bot, and a bot's own block wins over it:

```json
"cost": { "levers": { "housekeeping_cheap_rung": true } },
"bots": {
  "team-bot-a": { "role": "member", "port": 19001, "cost": { "levers": { "housekeeping_cheap_rung": false } } }
}
```

The Cost page shows each lever's state on the bot's tile. Anything other than `true`/`false` is ignored (a typo is not an opt-out).

| Lever | What it does when on | When off |
|-------|----------------------|----------|
| `housekeeping_cheap_rung` | The **memory flush** (OpenClaw's "store durable memories now" turn before it summarizes a long conversation) runs on the bot's `fast` role model, with only its `read`/append-only `write` tools and none of Evolve's per-turn context. **Compaction** (the summary itself) runs on the `fast` model with thinking off (`compaction.model`, `compaction.thinkingLevel: "off"`, written on deploy). Both are recorded in the turns log as `source: memory_flush` and shown on the weekly receipt on their own line under each bot. | Both run on the conversation's model, as OpenClaw does by default. Deploy removes the `compaction.model`/`thinkingLevel` values it wrote. |

The memory flush still writes the same note to `memory/<date>.md`; only the model that writes it changes. The flush's thinking level follows the session's (OpenClaw offers no per-run knob), clamped to what the cheap model supports.

### Compaction budget

OpenClaw summarizes older history ("compaction") when a conversation grows too large. Since OpenClaw 2026.9.2 the old `compaction.reserveTokensFloor` knob is retired and the reserve is fixed at 20k tokens, so with nothing else set a model with a 1M-token window (Sonnet 5, Opus 5) keeps ~980k tokens of history before compacting — and every message re-reads all of it, and the memory flush before compaction carries all of it too.

Evolve sets a **budget** instead: `agents.defaults.compaction.maxActiveTranscriptBytes`, the size of the active transcript at which OpenClaw compacts regardless of model, with `compaction.memoryFlush.forceFlushTranscriptBytes` at 80% of it so the memory note is always written on an earlier turn than the summary.

| Cost profile | Budget | Roughly (history tokens) |
|--------------|--------|--------------------------|
| Conservative | 256 KB | ~85k |
| Balanced (deploy default) | 400 KB | ~130k |
| Unrestricted (debug) | none of its own | window − 20k |

The trade-off: a smaller budget makes every message cheaper and compacts earlier, so older turns are kept as a summary rather than verbatim; a larger one keeps more exact history at a higher per-message cost. Chat-style bots, whose sessions are long and whose old turns rarely matter word-for-word, gain the most from a small budget. Change it per bot on the Cost page ("Compaction budget"). Deploy only fills the budget in when it is missing — it never overwrites a value you set — but it always keeps the flush threshold below the budget, because a flush that fires after compaction would lose the note.
...
gh paths take effect on the live pod.
...
| Type | Description | Forge required | Applier action |
|------|-------------|----------------|-----------------|
...
with a `forge_notes` field. Run `scripts/migrate-validation-results.py` to
rename the directory and field in place. (The dual-location read described here
lived in the retired `apply.py`; migrate rather than relying on it.)
...
Controls which Evolve components are active and their tuning parameters. All modules default to enabled except `slack_signals`.
...
"modules": {
  ...
  "forge":             { "enabled": true },
  "apply":             { "enabled": true },
  "continuity_engine": { "enabled": true },
...
| `forge` | enabled | — |
| `apply` | enabled | — |
| `continuity_engine` | enabled | — (also togglable per-bot via `bots.<id>.continuity_engine.enabled`; the defer runner skips disabled bots) |
...
**Stage 4 (optional):** re-enable continuity_engine (default-on; disabled in Stage 1 above), expansion, slack_signals
