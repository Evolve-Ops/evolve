...
Reference impl: `StruggleDetector` in the tier-cascade work returns `score: null` when `event.messages` is malformed, and emits a `cascade_payload_unexpected` Signal so the contract drift is observable (`spec-tier-cascade-2026-05-26.md` §"OC payload contract drift").
...
- `spec-tier-cascade-2026-05-26.md` §"OC payload contract drift" — the canonical `score: null` worked example
...
- `internal/incident-cost-alerting-blackout-2026-05-20.md` — the cautionary tale of silent degradation
