...
def test_session_instruction_keeps_the_move_in_one_turn_without_polling():
    """A move once spent 28 minutes calling `subagents list` 171 times while a
    transcription subagent ran; every status check re-sent the whole
    conversation. The instruction the session actually runs on must forbid the
    delegation and the poll, and the usage doc must say the same thing."""
    body = json.loads(MANIFEST.read_text())["scheduled_actions"][0]["install"]["body"]
    assert "NEVER spawn a subagent" in body
    assert "NEVER poll" in body
    assert "`subagents list`" in body and "`agents_wait`" in body
    assert "lags every move since the last `solve --save`" in body

    doc = (MANIFEST.parent / "files" / "docs" / "crossplay-coach.md").read_text()
    assert "### One turn, no helpers" in doc
    assert "**Never poll.**" in doc
