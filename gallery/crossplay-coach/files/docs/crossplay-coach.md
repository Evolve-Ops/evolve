...
Do the whole move yourself, in the turn the screenshot arrived in. Do not
spawn a subagent to transcribe or solve. A subagent reads the same pixels with
the same model, and waiting on it costs more than anything else this app can
do: every status check is another full model call over the whole conversation.

- **Never poll.** Do not call `subagents list` or `agents_wait` repeatedly. If
  a helper is already running, end your turn; its result arrives as its own
  event.
- **Bound the image work.** Make two focused passes over the board, plus one
  set of crops if the image is too small. If `audit` still reports REVIEW after
  one correction, ask the user about the coordinates it names, and stop.
- **A saved game lags the board.** `crossplay-data/games/<game-id>.json` holds
  the position as of the last successful `solve --save`, not the current
  one. Use it to know which words were already played; take every newer word
  from the screenshot.
