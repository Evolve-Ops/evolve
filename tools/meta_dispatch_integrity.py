...
# ── the lane-state branch, named once ────────────────────────────────────────
#
# `tools/meta-dispatch-move land` pushes here and `tools/meta-dispatch-eligible
# --lane-state-pr` qualifies a PR by it, so the two must agree on both strings or the
# writer's PRs stop matching the reader's rule — silently, in the direction that refuses
# every merge, which is the flood these constants were introduced to end (19 unmerged
# one-line PRs, 2026-09-05/06). Same reasoning as "one splitter, not two" above.
#
# `LAND_TRAILER` is the DISPATCHER'S IDENTITY. The git author cannot serve: `commit-tree`
# stamps the ambient identity, which in every checkout that runs the lane is the
# operator's own, so an author check would qualify a human's hand-push to the same branch
# name. A trailer is written by exactly one code path, travels with the commit, and
# survives the force-push.
LANE_STATE_BRANCH = "lane/state"
LAND_TRAILER = "Lane-State: meta-dispatch-move land"

# The four lane state dirs. `reviews/` is listed but is NEVER auto-mergeable: a review
# file is the PM's artifact and its own route.
LANE_STATE_DIRS = ("queued", "inflight", "done", "reviews")
LANE_AUTO_MERGEABLE_DIRS = ("queued", "inflight", "done")
...
# ── the THIRD self-healing shape: a merged chip's leftover marker ────────────
#
# `inflight/<id>.md` is working-tree-local BY CONSTRUCTION (`main` is protected, so an
# unattended dispatcher cannot commit its own bookkeeping), while the chip's own PR moves
# `queued/<id>.md -> done/<id>.md` and merges. Nothing removes the marker. So every
# SUCCESSFUL chip merge leaves one id in `inflight/` AND `done/` — a shape neither of the
# two shapes above covers, so it landed in `conflicts[]` and stopped the whole lane, for
# every id, until a human ran `meta-dispatch-move done <id>` on the machine holding the
# marker (observed live on 2026-09-10 after #4183;
# `internal/finding-inflight-done-after-merge-2026-09-10.md`).
#
# That is the D-PM8 argument again, sharper: the better the lane works the more often it
# fires, because it fires once per success. A guard whose trigger rate is the lane's own
# throughput is the failure mode D-PM8 exists to prevent.
#
# The EVIDENCE here is stronger than either shape above. `queued`+`inflight` rests on a
# digest comparison and `queued`+`done` on byte-identity; this rests on a MERGED PR that
# carried the `done/` entry, named by the entry's own `pr:`. The marker is provably stale
# because the work it tracks is finished and its text is on `main`.

REPAIR_MERGED_CHIP_MARKER = "inflight-marker-superseded-by-merge"


def classify_merged_chip_marker(marker_body: str, done_body: str,
                                done_pr: "int | None",
                                pr_merged: "bool | None") -> "Duplicate | None":
    """Is this `inflight/` + `done/` pair a merged chip's leftover marker, or a conflict?

    `None` = a plain lane-conflict, reported and blocking exactly as it was before. The
    only non-`None` answer is `ACTION_REPAIR`: there is no poke variant, because the
    shapes this refuses are not "explained but unproven" — they are unexplained, and the
    guard is there for them.

    FOUR THINGS MUST HOLD, and each one is the reason a different way of being wrong
    cannot get through:

      * **a `pr:` on the `done/` entry** — it is what says a PR carried this brief to
        `done/` rather than someone hand-filing it there. No `pr:` and the marker is not
        provably the stale half of anything;
      * **that PR is MERGED** — an OPEN PR means the chip is still running and its marker
        is holding a live build slot correctly; a CLOSED-UNMERGED one means the work was
        abandoned, and which of the two copies is canonical is then a judgment call;
      * **the marker still has a body** — the body comparison IS the safety argument, and
        the empty-body shape is the 2026-08-25 loss;
      * **the `done/` body CONTAINS the marker's body** — verbatim, so no text can be
        lost by dropping the marker. Containment rather than equality because a chip or
        the operator legitimately APPENDS an outcome note to a `done/` entry. This is the
        same predicate `tools/meta-dispatch-move done` re-checks in
        `_check_durable_carries_the_brief` before it unlinks anything, and it is stated
        here for the reason `classify_duplicate` is: a reader that promises a repair the
        writer then refuses is the silent disagreement the one-predicate rule exists to
        prevent.

    **UNDETERMINABLE MERGED-NESS IS NOT MERGED.** `pr_merged` is tri-state on purpose:
    `True` only when the run positively observed the merge (the dispatcher injects the
    numbers from its own single `gh pr list --state all` page). `None` — no network, an
    API error, a `gh` page the PR was not on — takes the SAME branch as `False`, so
    uncertainty keeps the stop rather than releasing it. That direction matters more here
    than anywhere else in this module: the thing being deleted is the lane's only local
    record that a chip is in flight, and deleting it on an unproven merge frees a build
    slot the lane has not actually finished with.

    Note what this never decides: WHICH file is dropped. The `inflight/` marker is the
    only one this shape can ever remove — the `done/` entry is on `main`, carried there by
    the merge that is the evidence — and the verb that acts on it (`done`) takes no other
    path, so "never delete the durable copy" is a property of the code's shape.
    """
    m_body = (marker_body or "").strip()
    if not m_body:
        return None                 # an empty body is reported, never "repaired" away
    if done_pr is None:
        return None
    if pr_merged is not True:       # False AND None — uncertainty keeps the stop
        return None
    if m_body not in (done_body or ""):
        return None
    pr = int(done_pr)
    return Duplicate(ACTION_REPAIR, REPAIR_MERGED_CHIP_MARKER, "merged-pr",
                     "chip merged as PR #%d; done/ carries the brief" % pr, pr)
...
def duplicate_log_line(brief_id: str, dup: "Duplicate") -> str:
    ...
    if dup.shape == REPAIR_MERGED_CHIP_MARKER:
        return "dropped the in-flight marker for %s (%s)" % (brief_id, dup.reason)
...
# ── `dispatched_at`: the lane's ONE definition of "when did this card appear?" ──
#
# WHY IT EXISTS (operator, 2026-09-07): "I often lose cards because there are so many
# ticks that I have to click through each of the tiles and this is how they can get
# buried. … if the dispatch could note what time the chip was created, I would know where
# to look for it." The lane already recorded `dispatched: <DATE>`, which answers "which
# day" and cannot answer "which tile in today's pile" — and with the dispatcher running
# round the clock, a day is 24 tiles deep.
#
# THREE READERS, ONE SPELLING, for the same reason `split_front_matter` and
# `classify_duplicate` live here rather than in either tool: `tools/meta-dispatch-move`
# WRITES the stamp at `launch`, `tools/meta-dispatch-eligible` renders the "cards waiting
# for a tap" line from it, and `tools/meta-queue` renders group (E) from the copy in the
# meta-state chip row. Three parsers for one field is three chances to disagree about
# whether a card is 3 hours old or unreadable.
#
# UTC ON DISK, LOCAL ON SCREEN, and neither half is negotiable. The stamp is stored
# `YYYY-MM-DDTHH:MM:SSZ` because the lane's other clock (`heartbeat`'s `run`, `now`) is
# UTC and a lane whose two timestamps are in different zones cannot be read against
# itself. It is DISPLAYED in the host's local zone because the operator's question is
# "where in today's tray do I look", and that is a question about their own wall clock.

DISPATCHED_AT = "dispatched_at"
DISPATCHED = "dispatched"

# ── derived_from_pr: provenance, NOT a dependency ────────────────────────────

DERIVED_FROM_PR = "derived_from_pr"


def parse_derived_from_pr(raw) -> "list[int] | None":
    """`derived_from_pr:` → an ordered, de-duplicated list of PR numbers.

    The field records that a brief was WRITTEN FROM work which was still an open
    pull request at the time — a follow-up someone split out of a PR while
    reviewing it. It is provenance, and it is deliberately NOT the lane's
    existing `depends_on: [pr:N]`, which HOLDS a brief until that PR merges:

    | field | meaning | effect on dispatch |
    |---|---|---|
    | `depends_on: [pr:N]` | cannot be built until N is in main | the brief is HELD |
    | `derived_from_pr: N` | was written from N while N was open | none — it is carried into the chip's prompt |

    Both readings are right for different work, and the 2026-09-22 collision is
    what made the second one necessary: a follow-up chip was spawned out of an
    open PR, branched from a clean `main` (correctly — that rule is unchanged),
    and could not read the spec section its own brief cited, because that
    section existed only in the parent's unmerged branch. It wrote a second
    document under the same name. Holding the chip would have been the wrong
    remedy — the operator's ruling on 2026-09-22 was *warn and let it proceed* —
    so what this field buys is the POINTER, handed to the chip in its prompt.

    Accepts a scalar or a list, in every spelling `parse_pr_ref` takes
    (`4413`, `#4413`, `pr:4413`). Returns `None` when the field is absent, and
    raises `ValueError` on anything that is not a PR reference — a provenance
    field nobody can resolve is worse than no field, because it reads as
    answered.
    """
    if raw is None:
        return None
    ...
    for item in items:
        n = parse_pr_ref(item)
        if n is None:
            raise ValueError("%s: %r is not a PR reference" % (DERIVED_FROM_PR, item))
        if n not in out:
            out.append(n)
    if not out:
        # An empty list reads as "declared and answered: derived from nothing",
        # which is the same silence as omitting the field while looking like a
        # decision. Omit it instead.
        raise ValueError("%s is empty — omit the field instead" % DERIVED_FROM_PR)
    return out


def dispatch_stamp(now: "datetime.datetime | None" = None) -> str:
    """`YYYY-MM-DDTHH:MM:SSZ` — the stamp written into `dispatched_at`.

    Second resolution and a literal `Z`, matching `tools/meta-dispatch-move`'s `now`
    verb and its heartbeat, so the three stamps a tick produces are comparable without
...
def parse_dispatch_stamp(raw) -> "datetime.datetime | None":
    """A `dispatched_at` value → an aware UTC datetime, or None when unusable.

    NEVER raises, and a NAIVE stamp is rejected rather than assumed-UTC. That rejection
    is the same rule `_elapsed_seconds` states in the mover: a local time silently
    wearing a `Z` is how a 7-hour offset gets reported as elapsed time, and an age of
    "-7h" or "31h" on a card the operator prepared 20 minutes ago is worse than no age
    at all. An unreadable stamp costs the AGE, never the line.
    """
    if raw is None:
        return None
...
def local_hhmm(raw) -> str | None:
    """A `dispatched_at` value → `HH:MM` on the HOST's wall clock, or None.

    The zone comes from the host (`astimezone()` with no argument, which reads
    `/etc/localtime` — the same source `evolve_admin.config.detect_system_timezone`
    reads). Deliberately NOT `resolve_pod_timezone`: these tools run in the operator's
    dev checkout, not on a pod, they import nothing from `evolve_admin`, and the
    question the field answers ("where in my tray is this card") is about the operator's
    own clock rather than a pod's configured zone.
    """
    dt = parse_dispatch_stamp(raw)
    return None if dt is None else dt.astimezone().strftime("%H:%M")


def local_tzname(raw=None) -> str:
    """The host zone's abbreviation (`PDT`, `UTC`, …) for the stamp's instant.

    Taken AT the stamp's instant rather than at "now", so a card prepared before a DST
    change is labelled with the abbreviation that was in force when it was prepared.
    """
    dt = parse_dispatch_stamp(raw) or datetime.datetime.now(datetime.timezone.utc)
    return dt.astimezone().strftime("%Z")


def age_hours(raw, now: "datetime.datetime | None" = None) -> int | None:
    """Whole hours from `dispatched_at` to `now`, floored, or None when unusable.

    Floored rather than rounded, and clamped at 0: a card stamped 40 minutes ago reads
    `0h`, which is honest about "this hour's tick prepared it" — the reading the
    operator actually wants from the first tile. A NEGATIVE age (a stamp from the
    future, i.e. a clock the run cannot trust) returns 0 rather than a fiction.
    """
    dt = parse_dispatch_stamp(raw)
    if dt is None:
        return None
    ...
    if ref.tzinfo is None:
        return None
    delta = (ref.astimezone(datetime.timezone.utc) - dt).total_seconds()
    return max(0, int(delta // 3600))


def age_label(raw, now: "datetime.datetime | None" = None) -> str:
    """`age_hours` as `<n>h`, or `?h` when the stamp is missing or unreadable.

    `?h` rather than an omitted clause: a card whose age nobody can compute must still
    occupy a slot on the waiting line, because the card the operator has lost is
    precisely the one whose bookkeeping went wrong.
    """
    h = age_hours(raw, now)
    return "?h" if h is None else "%dh" % h
