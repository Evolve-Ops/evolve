...
# The third way headless can be unavailable, and the only one that is not a file read:
# the launcher is PROVABLY not working. See `launcher_state`.
MISSING_STALLED = "stalled-launcher"
...
ALIVE = "alive"
STALLED = "stalled"
# The third verdict, added 2026-09-14 after `stalled` was wrong four times out of four.
# A no-branch window says how long ago the launch was; it does not say the session is dead.
# `UNKNOWN` is what the lane reports when the window has elapsed but SOMETHING — a recorded
# PR, a ref, a worktree still being written — says work exists. It is reported, it is
# poked, and it never trips the launcher breaker.
...
def launcher_state(liveness_states) -> tuple:
    """(ok, detail) — is the LAUNCHER itself working, judged on its own recent output?

    `check_prereqs` answers two questions, *is `claude` granted in Bash?* and *was the
    dangerous-mode disclaimer accepted?*, and neither of them is *does a launched session
    do work*. On 2026-09-13 that gap cost the lane a day: four consecutive headless
    launches produced no commit, no branch and not one byte written into their own
    worktrees, `check_prereqs` reported `headless: available` throughout with perfect
    accuracy, and `HEADLESS_PER_TICK` kept feeding the dead launcher until four of six
    build slots held sessions that were never going to build anything
    (`internal/finding-headless-launch-is-0-for-4-2026-09-13.md`).

    This is the missing half of `classify_liveness`. That function already detects the
    shape — a session with no branch 45 minutes after launch — and its own docstring says
    the verdict is "reported, never acted on". Acting on it belongs HERE rather than in the
    relaunch path, because the two decisions are opposites: the reconciler's inverse-guard
    asks *may I start this brief AGAIN*, and this asks *may I start ANY brief this way*.

    FAIL CLOSED, and self-clearing. One STALLED entry is enough — a launcher is not a
    thing that half works, and the cost of being wrong is a card the operator taps
    (throughput the lane has actually been getting) rather than a slot burned on silence.
    It clears the moment the stalled entries do, and the negative fact it reads is one the
    checkout already holds, so it costs no probe, no API call and no extra tick.

    A missing fact is still not a negative fact: `classify_liveness` returns ALIVE for
    anything it cannot parse, so an unreadable lane cannot trip this breaker.

    ONLY A CORROBORATED STALL COUNTS, and that is not this function's job — it counts
    `STALLED` and nothing else. The caller must already have downgraded an uncorroborated
    stall to `UNKNOWN`. This split is deliberate and was paid for: written on 2026-09-13
    against four sessions read as dead, this breaker would have closed headless routing
    fleet-wide on three chips that were merely parked and delivered ~21 hours later. The
    window is a clock reading; the verdict needs a second source
    (`meta-dispatch-eligible.headless_liveness_report`, which checks the recorded PR, the
    chip's ref, and the worktree's own mtimes — all local, all free).
    """
    stalled = [s for s in liveness_states if s == STALLED]
    if not stalled:
        return True, ""
    return False, ("%d in-flight headless session(s) are STALLED — a launcher whose last "
                   "launches produced nothing is not available, whatever the grant and "
                   "disclaimer say; clear or relaunch them and routing restores itself "
                   "(internal/finding-headless-launch-is-0-for-4-2026-09-13.md)"
                   % len(stalled))
...
# ── provenance carried into the prompt ───────────────────────────────────────


def derived_from_preamble(prs) -> str:
    """The lines prepended to a chip's prompt when its brief carries
    `derived_from_pr:` — or `""` when it does not.

    THE PROMPT IS THE ONLY THING A HEADLESS CHIP READS. `launch --headless`
    passes the entry body verbatim and nothing else, so a field that stayed in
    front matter would be invisible to the one reader it exists for. The
    provenance is therefore rendered INTO the prompt, above the body, and it
    says the two things the 2026-09-22 chip needed and did not have: which PR
    its brief was written from, and that the PR may not be in `main` yet.

    It does NOT tell the chip to branch from that PR. Chips branch from a clean
    `main` — that rule is unchanged, and stacking a chip on an unmerged branch
    trades one collision for a stale base. What changes is that the chip can
    READ the parent (`gh pr diff`) instead of rediscovering it, which is the
    difference between extending a spec section and writing a second one under
    the same name.
    """
    if not prs:
        return ""
    refs = ", ".join("#%d" % n for n in prs)
    many = len(prs) > 1
    return (
        "PROVENANCE — this brief was written from work in PR%s %s, which %s open at "
        "the time.\n"
        "Read %s before you build (`gh pr view <n>`, `gh pr diff <n>`): if it has not "
        "merged yet, the files and any doc section it names may not exist on `main`, "
        "and the work you are extending lives in that PR, not in your checkout.\n"
        "Branch from a clean `main` as usual — do NOT stack on it — but do not "
        "re-derive what it already did, and do not write a second document under a "
        "name it already used.\n\n"
        % ("s" if many else "", refs, "were" if many else "was",
...
# ── session-id parsing (shared by `launch` and `probe_background_login`) ─────

# Shapes a session/agent id can arrive in on the CLI's launch banner. Matched loosely and
# non-fatally: an id we FAIL to recognise costs a `session: null` in the entry and nothing
# else, because `launch: headless` (not the session id) is what every downstream reader
# gates on. Inventing an id that the CLI never issued would be worse than recording none —
# it is the shape of the #3849 orphan-id loss, a lane keyed on a value nothing else holds.
# Promoted here from `tools/meta-dispatch-move` (2026-09-16, rig-preflight brief) because
# `probe_background_login` below needs the identical parse to find its own throwaway
# session among `claude agents --json`'s rows, and a second copy is a second answer to
# "what did the CLI call this session".
SESSION_ID_RE = re.compile(
    r"\b(?:(?:agent|task|sess|session|bg)[_-][A-Za-z0-9]{6,}"
    r"|[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})\b")

# CLI v2.1.268 prints a BARE NUMERIC id — `backgrounded · 13440514` — which matches none of
# the prefixed shapes above, so before this every headless launch recorded `session: null`
# even when it started cleanly. Anchored on the banner word rather than added to the loose
# alternation on purpose: a bare run of digits is the one id shape that would also match a
# pid, a duration, a byte count or a timestamp anywhere else in the output, and a WRONG id
# is worse than none (the #3849 orphan-id shape). Tried second, so a prefixed id in the
# same output still wins.
BACKGROUNDED_ID_RE = re.compile(r"backgrounded\s*[·:]?\s*([A-Za-z0-9][A-Za-z0-9_-]{3,})")


def parse_session_id(stdout: str, stderr: str = "") -> str | None:
    for blob in (stdout or "", stderr or ""):
        m = SESSION_ID_RE.search(blob)
        if m:
            return m.group(0)
    for blob in (stdout or "", stderr or ""):
        m = BACKGROUNDED_ID_RE.search(blob)
        if m:
            return m.group(1)
    return None


# ── the login probe (D-TP1) ───────────────────────────────────────────────────

# The tick's fourth precondition, and the one `check_prereqs` deliberately does not test
# (see its own module docstring and `disclaimer_state`'s: a probe is the wrong tool for a
# question that changes once, ever). Login is the opposite shape — a session token that
# EXPIRES on its own clock, invisibly to a file read — so the argument against probing the
# disclaimer does not transfer: there is no file this can read instead. On 2026-09-13/14 a
# `claude -p` echo answered "is the CLI present", printed `ok`, and four background
# sessions sat at "login required" for 21 hours while nothing said so
# (`internal/finding-headless-launch-is-0-for-4-2026-09-13.md`, corrected). This spends one
# real, cheap, throwaway session per TICK (never per brief — the caller runs it once and
# reuses the verdict) to answer the one question a file cannot.

LOGIN_OK = "ok"
LOGIN_EXPIRED = "login_expired"
# Never `LOGIN_OK` on ambiguity. Anything the probe cannot positively read as completed
# with the expected reply, or positively read as a login prompt, is UNKNOWN — the same
# fail-toward-the-card direction `check_prereqs` already takes, extended to a question a
# static read cannot answer at all.
LOGIN_UNKNOWN = "unknown"

PROBE_PROMPT = "reply PROBE_OK"
PROBE_REPLY = "PROBE_OK"
PROBE_POLL_TIMEOUT_S = 60
PROBE_POLL_INTERVAL_S = 2

_LOGIN_TEXT_RE = re.compile(r"login\s*expired|please\s*run\s*/login|run\s*/login", re.IGNORECASE)


def _default_probe_runner(argv, cwd):
    """The one place the probe invokes `claude`. Tests replace this and nothing else, the
    same seam discipline `tools/meta-dispatch-move._run_claude` uses for the real launch."""
    return subprocess.run(argv, cwd=str(cwd), capture_output=True, text=True, timeout=120)


def _agent_record_id(rec) -> str | None:
    if not isinstance(rec, dict):
        return None
    for key in ("id", "session_id", "sessionId", "agent_id", "agentId"):
        v = rec.get(key)
        if v:
            return str(v)
    return None
...
def _classify_agent_record(rec: dict) -> tuple:
    """(state, detail) read off one `claude agents --json` row for the probe's session.

    The exact JSON schema is not pinned anywhere in this codebase (only prose readings of
    `status`/`state` from live probes — `internal/meta-dispatch-procedure.md`'s Transport
    deviation section). So this reads `status` and `state` where present, and falls back to
    scanning the WHOLE record for the reply text or a login-prompt sentence, rather than
    refusing outright on a field name it does not recognise — the fail-safe direction is
    UNKNOWN either way, never OK, so a schema drift costs a slower verdict, not a wrong one.
    """
    ...
    try:
        ...
        blob = str(rec)
    completed = status in ("completed", "done") or state in ("completed", "done")
    if completed:
        if PROBE_REPLY in blob:
            return LOGIN_OK, "probe session completed with the expected reply"
        return (LOGIN_UNKNOWN,
                "probe session completed (status=%r state=%r) but did not carry %r"
                % (rec.get("status"), rec.get("state"), PROBE_REPLY))
    login_signal = (status == "blocked" or state == "blocked"
                    or "awaiting input" in status or "awaiting input" in state
                    or bool(_LOGIN_TEXT_RE.search(blob)))
    if login_signal:
        return (LOGIN_EXPIRED,
                "probe session reports status=%r state=%r — reads as a login prompt"
                % (rec.get("status"), rec.get("state")))
    return (LOGIN_UNKNOWN,
            "probe session status=%r state=%r — neither completed nor a recognised "
            "login-expired shape" % (rec.get("status"), rec.get("state")))


def probe_background_login(*, executable: str = "claude", scratch_dir=None, runner=None,
                            timeout_s: float = PROBE_POLL_TIMEOUT_S,
                            interval_s: float = PROBE_POLL_INTERVAL_S,
                            sleep=None, clock=None) -> tuple:
    """(state, detail) — does the background `claude` CLI actually have a live login?

    Spawns a THROWAWAY `claude --bg --dangerously-skip-permissions "reply PROBE_OK"` in a
    scratch directory that is never the repo (the caller's checkout is never touched —
    guardrail), then polls `claude agents --json` for that exact session id up to
    `timeout_s`. `completed` carrying the reply is the only `LOGIN_OK`; `blocked` /
    `awaiting input` / a login-prompt sentence is `LOGIN_EXPIRED`; everything else —
    including a session whose id never shows up at all, a `claude --bg` that fails to
    start, or a poll that times out mid-run — is `LOGIN_UNKNOWN`, which the caller treats
    exactly as red (see `rig_preflight` guardrail: unknown never authorises a launch).

    `runner`/`sleep`/`clock` are injectable so a test never spends a real 60 seconds or a
    real API call — the same seam shape as `_run_claude` and `corroborate` elsewhere in
    this lane.
    """
    ...
    tmp_holder = None
    if scratch is None:
        tmp_holder = tempfile.mkdtemp(prefix="meta-rig-login-")
        scratch = Path(tmp_holder)
    try:
        argv = build_argv(PROBE_PROMPT, executable=executable)
        try:
            proc = run(argv, scratch)
        except (OSError, subprocess.SubprocessError) as e:
            return LOGIN_UNKNOWN, "could not start the probe session: %s" % e
        if proc.returncode != 0:
            detail = (proc.stderr or proc.stdout or "").strip()[:300]
            return (LOGIN_UNKNOWN,
                    "`%s --bg` exited %d for the probe: %s" % (executable, proc.returncode,
                                                                detail))
        session_id = parse_session_id(proc.stdout, proc.stderr)
        if not session_id:
            return (LOGIN_UNKNOWN,
                    "probe session started but its id could not be parsed from the launch "
                    "banner")
        ...
        while True:
            try:
                agents_proc = run([executable, "agents", "--json"], scratch)
            except (OSError, subprocess.SubprocessError) as e:
                return LOGIN_UNKNOWN, "could not list agents: %s" % e
            if agents_proc.returncode == 0:
                try:
                    rows = json.loads(agents_proc.stdout or "[]")
                except ValueError:
                    rows = []
                if isinstance(rows, list):
                    rec = next((r for r in rows if _agent_record_id(r) == session_id), None)
                    if rec is not None:
                        state, detail = _classify_agent_record(rec)
                        last_seen = (state, detail)
        ...
        if last_seen is not None:
            return last_seen
        return (LOGIN_UNKNOWN,
                "probe session %s never appeared in `%s agents --json` within %ds"
                % (session_id, executable, int(timeout_s)))
    finally:
        if owns_scratch and tmp_holder:
