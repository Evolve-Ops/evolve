...
PD-3 (docs/spec-public-2026-06-18.md §5): Evolve's dev apparatus ships PUBLIC but
...
The load-bearing files (why each cannot be edited in place):
  ...
  * ``docs/runbook-public-cutover.md`` — a verify step keeps a real ``ssh <admin>@<host>``
    command so the private-CI residue test
    (``test_known_residue_paths_still_contain_tokens``) stays green and the runbook
    stays operationally accurate.
...
inside ``tools/publish-public``. This module makes the five files above token-clean in
...
#
# Evolve's public repository is published as curated, squashed release commits
# (see docs/runbook-public-cutover.md), all under a single project maintainer
# identity, so no per-commit identity remapping is required here.
#
...
_TARGETS: dict[str, Callable[[str], str]] = {
    ...
    "docs/runbook-public-cutover.md": _apply_replacements,
...
def sanitize_public_text(rel_posix: str, text: str) -> str:
    ...
    # sweep. Some targets (the cutover runbook) intentionally reference the
    # private cjalden origin in verification steps ("Expect ... cjalden ... NOT
    # evolve-ops"); a blunt rewrite would corrupt those into self-contradiction.
    # cjalden is not a reserved token, so leaving it in a sanitized target is
